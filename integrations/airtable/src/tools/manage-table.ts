...
export let manageTableTool = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'update']).describe('Whether to create a new table or update an existing one'),
    ...
    fields: z.array(z.object({
      ...
      options: z.record(z.string(), z.any()).optional().describe('Field type-specific options (e.g. choices for select fields)'),
    })).optional().describe('Fields to create with the table (required for create action)'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Table fields (returned for create action)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      if (!ctx.input.fields || ctx.input.fields.length === 0) throw new Error('fields are required when creating a table');
      ...
      let result = await client.createTable(ctx.input.tableName, mappedFields, ctx.input.description);
