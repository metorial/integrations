...
let snappedPointSchema = z.object({
  ...
  originalIndex: z.number().optional().describe('Index of the corresponding original input point (only for non-interpolated points)'),
  placeId: z.string().optional().describe('Place ID of the road segment'),
});

export let snapToRoadsTool = SlateTool.create(
  spec,
  {
    name: 'Snap to Roads',
    key: 'snap_to_roads',
  ...
  .input(z.object({
    ...
    interpolate: z.boolean().optional().describe('If true, returns additional interpolated points for smoother path along the road'),
  }))
  .output(z.object({
