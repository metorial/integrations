...
export let validateAddressTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    addressLines: z.array(z.string()).describe('Address lines (e.g. ["1600 Amphitheatre Parkway"])'),
    regionCode: z.string().optional().describe('Two-letter country/region code (e.g. "US", "GB")'),
    locality: z.string().optional().describe('City or town name'),
    enableUspsCass: z.boolean().optional().describe('Enable USPS CASS processing (US/PR addresses only)'),
  }))
  .output(z.object({
    ...
    hasInferredComponents: z.boolean().optional().describe('Whether any components were inferred'),
    hasReplacedComponents: z.boolean().optional().describe('Whether any components were corrected/replaced'),
    inputGranularity: z.string().optional().describe('Granularity of the input address'),
    validationGranularity: z.string().optional().describe('Granularity the address was validated to'),
