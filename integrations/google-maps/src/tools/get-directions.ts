...
let routeLegSchema = z.object({
  ...
  })).optional().describe('Turn-by-turn navigation steps'),
});
...
export let getDirectionsTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    avoid: z.array(z.enum(['tolls', 'highways', 'ferries', 'indoor'])).optional().describe('Features to avoid'),
    units: z.enum(['metric', 'imperial']).optional().describe('Unit system for distances'),
    departureTime: z.string().optional().describe('Departure time as Unix timestamp in seconds, or "now"'),
    arrivalTime: z.string().optional().describe('Desired arrival time as Unix timestamp (transit only)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let routes = rawRoutes.map((route) => {
      ...
      let totalDistanceText = legs.map((l) => l.distanceText).filter(Boolean).join(' + ');
      let totalDurationText = legs.map((l) => l.durationText).filter(Boolean).join(' + ');
