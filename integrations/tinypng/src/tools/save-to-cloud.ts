...
}).optional().describe('Optional resize parameters to apply before storing');

export let saveToCloud = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    sourceUrl: z.string().describe('Publicly accessible URL of the image to compress and store'),
    storage: z.discriminatedUnion('service', [s3ConfigSchema, gcsConfigSchema]).describe('Cloud storage destination configuration'),
    cacheControl: z.string().optional().describe('Cache-Control header value, e.g. "public, max-age=31536000"'),
    resize: resizeSchema,
    convertTo: z.string().optional().describe('Target image format MIME type for conversion, e.g. "image/webp"'),
    background: z.string().optional().describe('Background color when converting transparent images. Hex value or "white"/"black".'),
    preserve: z.array(z.enum(['copyright', 'creation', 'location'])).optional().describe('Metadata to preserve'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    ctx.info(`Uploading to ${ctx.input.storage.service === 's3' ? 'Amazon S3' : 'Google Cloud Storage'}...`);
    let storeResult = await client.storeToCloud(
    ...
    let serviceName = ctx.input.storage.service === 's3' ? 'Amazon S3' : 'Google Cloud Storage';
