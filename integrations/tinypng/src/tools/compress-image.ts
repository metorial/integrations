...
export let compressImage = SlateTool.create(
  spec,
  {
    name: 'Compress Image',
    key: 'compress_image',
  ...
  .input(z.object({
    ...
    preserve: z.array(z.enum(['copyright', 'creation', 'location'])).optional().describe('Metadata to preserve in the compressed image. "creation" and "location" are JPEG only.'),
  }))
  .output(z.object({
    ...
    compressionRatio: z.number().describe('Compression ratio (0-1, lower is better compression)'),
