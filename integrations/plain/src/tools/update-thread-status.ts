...
export let updateThreadStatus = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    snoozeDurationSeconds: z.number().optional().describe('Snooze duration in seconds (required when status is SNOOZED)'),
  }))
  .output(z.object({
    threadId: z.string().describe('Plain thread ID'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.status === 'DONE') {
      ...
      let res = await client.snoozeThread(ctx.input.threadId, ctx.input.snoozeDurationSeconds * 1000);
      thread = res.thread;
    }
