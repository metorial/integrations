...
export let exportEvents = SlateTool.create(
  spec,
  {
    name: 'Export Events',
    key: 'export_events',
    ...
    constraints: [
      'Export is processed asynchronously — returns a job ID',
    ],
  }
)
  .input(z.object({
    format: z.enum(['csv', 'json']).describe('Output format for the export'),
    conditions: z.array(z.any()).describe('Filter conditions for the export (e.g. event names, flow IDs, segment IDs)'),
    startTime: z.string().describe('Start time for the export range (ISO 8601 or milliseconds)'),
    endTime: z.string().optional().describe('End time for the export range (ISO 8601 or milliseconds)'),
