...
export let manageProduct = SlateTool.create(
  ...
  .input(z.object({
    ...
    productType: z.enum(['PHYSICAL', 'SERVICE', 'GIFT_CARD']).optional().describe('Product type (required for create)'),
    ...
    variants: z.array(z.object({
      ...
      attributes: z.record(z.string(), z.string()).optional().describe('Variant attributes (e.g., color, size)')
    })).optional().describe('Product variants (required for create)')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      if (!ctx.input.storePageId || !ctx.input.productType || !ctx.input.variants?.length) {
        throw new Error('storePageId, productType, and at least one variant are required to create a product');
      }
