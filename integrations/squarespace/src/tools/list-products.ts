...
export let listProducts = SlateTool.create(
  ...
  .input(z.object({
    ...
    modifiedAfter: z.string().optional().describe('ISO 8601 UTC datetime — only return products modified after this date'),
    modifiedBefore: z.string().optional().describe('ISO 8601 UTC datetime — only return products modified before this date'),
    type: z.string().optional().describe('Comma-separated product types: PHYSICAL, SERVICE, GIFT_CARD, DIGITAL')
  }))
  .output(z.object({
