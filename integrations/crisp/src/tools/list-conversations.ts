...
export let listConversations = SlateTool.create(
  ...
  .input(z.object({
    ...
    searchType: z.enum(['text', 'segment', 'filter']).optional().describe('Type of search to perform'),
    ...
    filterNotResolved: z.boolean().optional().describe('Filter to only unresolved conversations'),
    filterAssigned: z.boolean().optional().describe('Filter to only assigned conversations'),
    filterUnassigned: z.boolean().optional().describe('Filter to only unassigned conversations'),
    filterInboxId: z.string().optional().describe('Filter by specific inbox ID'),
    filterDateStart: z.string().optional().describe('Filter conversations from this date (ISO 8601)'),
    filterDateEnd: z.string().optional().describe('Filter conversations until this date (ISO 8601)'),
  }))
  .output(z.object({
