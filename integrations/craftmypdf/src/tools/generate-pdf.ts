...
You can also load data from an external URL instead of passing it inline.`,
  ...
  .input(
    z.object({
      ...
      loadDataFrom: z.string().optional().describe('External URL to load template data from. Overrides templateData if both provided.'),
      version: z.number().optional().describe('Specific template version to use. Defaults to the latest version.'),
      expiration: z.number().optional().describe('Expiration time in minutes for the generated PDF URL (1-10080). Default is 5.'),
      outputFile: z.string().optional().describe('Output filename. Default is "output.pdf".'),
      imageResampleRes: z.number().optional().describe('Image resampling resolution in DPI (72, 150, 300, 600, 1200).'),
      passwordProtected: z.boolean().optional().describe('Enable password protection for the generated PDF.'),
      password: z.string().optional().describe('Password for the protected PDF. Required if passwordProtected is true.'),
      pdfVersion: z.string().optional().describe('PDF standard version: "1.4", "1.7", or "Auto".'),
    })
  )
  .output(
    z.object({
