...
Use "pdfUrls" for URL-based merging, or "templates" for template-based merging.`,
  ...
  .input(
    z.object({
      pdfUrls: z.array(z.string()).optional().describe('List of PDF URLs to merge into one document. Use this for URL-based merging.'),
      templates: z
        .array(
          z.object({
            templateId: z.string().describe('Template ID to use for this section.'),
            templateData: z.record(z.string(), z.unknown()).optional().describe('JSON data for this specific template.'),
          })
        )
        .optional()
        .describe('List of templates with their data for multi-template merging.'),
      sharedData: z.record(z.string(), z.unknown()).optional().describe('Fallback JSON data for templates that do not specify their own data.'),
      paging: z.enum(['continuous', 'reset']).optional().describe('Page numbering: "continuous" for sequential numbering, "reset" to restart numbering per document.'),
      expiration: z.number().optional().describe('Expiration time in minutes for the merged PDF URL (1-10080).'),
