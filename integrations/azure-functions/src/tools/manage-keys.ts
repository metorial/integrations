...
export let manageKeys = SlateTool.create(
  ...
  .input(z.object({
    ...
    scope: z.enum(['function', 'host']).describe('Whether to manage function-level or host-level keys'),
    action: z.enum(['list', 'create', 'delete']).describe('Action to perform'),
    functionName: z.string().optional().describe('Function name (required when scope is "function")'),
    keyName: z.string().optional().describe('Name of the key (required for create and delete actions)'),
    keyValue: z.string().optional().describe('Value for the key (optional for create — Azure generates one if omitted)'),
  }))
  .output(z.object({
    keys: z.record(z.string(), z.string()).optional().describe('Map of key names to values (for list action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    ctx.info(`${action} ${scope} key(s) for ${appName}${functionName ? '/' + functionName : ''}`);
    ...
    if (action === 'create') {
      let result: any;
      if (scope === 'function') {
        result = await client.createOrUpdateFunctionKey(appName, functionName!, keyName!, keyValue);
      } else {
