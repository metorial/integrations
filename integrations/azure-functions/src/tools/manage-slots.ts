...
export let manageSlots = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'get', 'create', 'delete', 'swap']).describe('Action to perform'),
    slotName: z.string().optional().describe('Name of the slot (required for get, create, delete, and swap)'),
    targetSlotName: z.string().default('production').describe('Target slot name for swap (defaults to "production")'),
    location: z.string().optional().describe('Azure region for creating a new slot (required for create)'),
  }))
  .output(z.object({
