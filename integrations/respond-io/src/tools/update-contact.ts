...
export let updateContact = SlateTool.create(
  ...
  .input(z.object({
    identifierType: z.enum(['id', 'phone', 'email']).describe('Type of identifier to find the contact'),
    ...
    customFields: z.record(z.string(), z.any()).optional().describe('Custom field values to update as key-value pairs'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.updateContact(ctx.input.identifierType, ctx.input.identifierValue, fields);
