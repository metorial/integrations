...
export let auth = SlateAuth.create()
  .output(z.object({
    token: z.string().describe('Bearer token for API requests (API key for IDP, OAuth access token for Connect)'),
    authMethod: z.enum(['api_key', 'oauth_client_credentials']).describe('Which auth method was used'),
  }))
  .addTokenAuth({
    type: 'auth.token',
  ...
  .addCustomAuth({
    ...
    inputSchema: z.object({
      ...
      scopes: z.string().describe('Comma-separated scopes, e.g. "connect:fulfillment,connect:post_checkout"'),
    }),
