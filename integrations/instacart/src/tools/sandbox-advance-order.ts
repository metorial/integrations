...
export let sandboxAdvanceOrder = SlateTool.create(
  spec,
  {
...
Only available in the **development** environment.`,
  ...
  .input(z.object({
    ...
    batchId: z.string().optional().describe('Existing batch ID (if previously created). Omit to create a new shopper and batch.'),
  }))
  .output(z.object({
    shopperId: z.string().optional().describe('Test shopper ID (only returned when creating a new batch)'),
