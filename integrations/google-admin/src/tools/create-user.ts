...
export let createUser = SlateTool.create(
  ...
  .input(z.object({
    primaryEmail: z.string().describe('Primary email address for the new user (e.g. john@example.com)'),
    givenName: z.string().describe('First name of the user'),
    familyName: z.string().describe('Last name of the user'),
    password: z.string().optional().describe('Password for the new user. Must meet complexity requirements. If omitted, a random password is generated.'),
    orgUnitPath: z.string().optional().describe('Organizational unit path (e.g. /Engineering). Defaults to root (/).'),
    suspended: z.boolean().optional().describe('Create the user in a suspended state'),
    changePasswordAtNextLogin: z.boolean().optional().describe('Require password change on first login'),
    recoveryEmail: z.string().optional().describe('Recovery email address for the user'),
    recoveryPhone: z.string().optional().describe('Recovery phone number (must start with + and include country code)'),
    phones: z.array(z.object({
      value: z.string().describe('Phone number'),
      type: z.string().describe('Phone type (e.g. work, home, mobile)')
    })).optional().describe('Phone numbers for the user'),
    organizations: z.array(z.object({
      name: z.string().optional().describe('Organization name'),
      title: z.string().optional().describe('Job title'),
      department: z.string().optional().describe('Department name'),
      primary: z.boolean().optional().describe('Whether this is the primary organization')
    })).optional().describe('Organization details for the user')
  }))
  .output(z.object({
