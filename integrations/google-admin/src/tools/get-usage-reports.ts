...
export let getUsageReports = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    date: z.string().describe('Date for the report in YYYY-MM-DD format (must be at least 2 days ago)'),
    userKey: z.string().optional().describe('User email or "all" for all users (only for user reports)'),
    parameters: z.string().optional().describe('Comma-separated list of report parameters to include'),
    filters: z.string().optional().describe('Filter for user reports (e.g. "accounts:is_disabled==true")'),
