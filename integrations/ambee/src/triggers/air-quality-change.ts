...
export let airQualityChange = SlateTrigger.create(
  spec,
  {
    name: 'Air Quality Change',
    key: 'air_quality_change',
    description: 'Monitors air quality at a specified location and triggers when the AQI or category changes from the last known state.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    aqi: z.number().describe('Current AQI value'),
    previousAqi: z.number().optional().describe('Previous AQI value'),
    category: z.string().optional().describe('Current AQI category (Good, Moderate, Unhealthy for Sensitive Groups, Unhealthy, Very Unhealthy, Hazardous)'),
    previousCategory: z.string().optional().describe('Previous AQI category'),
    lat: z.number().optional().describe('Latitude'),
    lng: z.number().optional().describe('Longitude'),
    city: z.string().optional().describe('City name'),
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let hasChanged = previousAqi !== undefined && (currentAqi !== previousAqi || currentCategory !== previousCategory);
    ...
    handleEvent: async (ctx) => {
      let changeType = ctx.input.previousCategory !== ctx.input.category
        ? 'category_changed'
        : 'aqi_changed';
