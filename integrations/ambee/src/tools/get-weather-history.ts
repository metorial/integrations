...
let weatherHistoryRecordSchema = z.object({
...
export let getWeatherHistory = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.getWeatherHistory(ctx.input.lat, ctx.input.lng, ctx.input.from, ctx.input.to, units);
