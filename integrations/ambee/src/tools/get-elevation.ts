...
let elevationRecordSchema = z.object({
  geometry: z.object({
    type: z.string().optional(),
    coordinates: z.array(z.number()).optional(),
  }).passthrough().optional().describe('GeoJSON geometry'),
...
export let getElevation = SlateTool.create(
  spec,
  {
    name: 'Get Elevation',
    key: 'get_elevation',
  ...
  .handleInvocation(async (ctx) => {
    ...
    let summary = elevationRecords.length > 0
