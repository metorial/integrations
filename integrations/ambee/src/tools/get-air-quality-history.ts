...
let historicalAqRecordSchema = z.object({
...
export let getAirQualityHistory = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    countryCode: z.string().optional().describe('ISO country code (required with postalCode)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.postalCode && ctx.input.countryCode) {
      result = await client.getAirQualityHistoryByPostalCode(
        ctx.input.postalCode, ctx.input.countryCode, ctx.input.from, ctx.input.to
      );
    } else if (ctx.input.lat !== undefined && ctx.input.lng !== undefined) {
      result = await client.getAirQualityHistoryByLatLng(
        ctx.input.lat, ctx.input.lng, ctx.input.from, ctx.input.to
      );
    } else {
