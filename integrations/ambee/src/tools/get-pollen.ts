...
let pollenRecordSchema = z.object({
  Count: z.object({
  ...
  Species: z.any().optional().describe('Species-level risk data (when speciesRisk is enabled)'),
...
export let getPollen = SlateTool.create(
  spec,
  {
    name: 'Get Pollen',
    key: 'get_pollen',
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.place) {
      ...
      result = await client.getPollenByLatLng(ctx.input.lat, ctx.input.lng, ctx.input.speciesRisk);
    } else {
    ...
    let summary = pollenData.length > 0 && pollenData[0]?.Count
