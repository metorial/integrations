...
let pollenForecastSchema = z.object({
  Count: z.object({
...
export let getPollenForecast = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Forecast covers up to 48 hours ahead.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.place) {
      ...
      result = await client.getPollenForecastByLatLng(ctx.input.lat, ctx.input.lng, ctx.input.speciesRisk);
    } else {
