...
Supports filtering by fields like source.author.id, state, read, priority, assignee, and more.`,
  ...
  .input(z.object({
    mode: z.enum(['list', 'search']).default('list').describe('Whether to list all conversations or search'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.mode === 'search' && ctx.input.query) {
      result = await client.searchConversations(ctx.input.query, ctx.input.paginationCursor, ctx.input.perPage);
    } else {
      result = await client.listConversations({ perPage: ctx.input.perPage, startingAfter: ctx.input.startingAfter });
    }
