...
let filterSchema: z.ZodType<any> = z.lazy(() => z.object({
  field: z.string().optional().describe('Field to filter on (e.g., "email", "name", "role", "created_at", custom_attributes.*)'),
  operator: z.enum(['=', '!=', '>', '<', '~', '!~', 'IN', 'NIN', 'starts_with', 'ends_with', 'contains', 'NOT_contains']).optional().describe('Comparison operator'),
  value: z.any().optional().describe('Value to compare against'),
  AND: z.array(z.lazy(() => filterSchema)).optional().describe('Array of filters combined with AND'),
  OR: z.array(z.lazy(() => filterSchema)).optional().describe('Array of filters combined with OR')
}));
...
Returns paginated results with cursor-based pagination.`,
  ...
  .input(z.object({
    ...
    paginationCursor: z.string().optional().describe('Cursor for fetching the next page of results')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.searchContacts(ctx.input.query, ctx.input.paginationCursor, ctx.input.perPage);
