...
let smsRecipientSchema = z.object({
  ...
  from: z.string().optional().describe('Custom sender ID or phone number (alphanumeric, max 11 characters)'),
  schedule: z.number().optional().describe('Unix timestamp to schedule the message for future delivery'),
  customString: z.string().optional().describe('Your own reference string for tracking'),
  country: z.string().optional().describe('ISO 3166-1 alpha-2 country code to auto-format the number'),
});
