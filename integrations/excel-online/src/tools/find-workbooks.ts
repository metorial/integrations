...
export let findWorkbooks = SlateTool.create(
  spec,
  {
    name: 'Find Workbooks',
    key: 'find_workbooks',
  ...
  .input(z.object({
    ...
    folderId: z.string().optional().describe('Folder ID to list workbooks from (for list action, omit for root)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of workbooks found'),
    workbook: z.object({
      workbookItemId: z.string(),
      name: z.string(),
      webUrl: z.string().optional(),
    ...
    }).optional().describe('Workbook file metadata'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'get': {
        if (!ctx.input.workbookItemId) throw new Error('workbookItemId is required for get action');
