...
export let manageWorksheets = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    worksheetIdOrName: z.string().optional().describe('Worksheet ID or name (required for get, update, delete)'),
    name: z.string().optional().describe('New name for the worksheet (for create or update)'),
    position: z.number().optional().describe('Zero-based position index for reordering (for update)'),
    visibility: z.enum(['Visible', 'Hidden', 'VeryHidden']).optional().describe('Visibility state (for update)'),
    sessionId: z.string().optional().describe('Optional workbook session ID for batch operations'),
  }))
  .output(z.object({
    worksheets: z.array(z.object({
      ...
      position: z.number(),
      visibility: z.string(),
    })).optional().describe('List of worksheets (for list action)'),
    worksheet: z.object({
      worksheetId: z.string(),
      name: z.string(),
      position: z.number(),
      visibility: z.string(),
    }).optional().describe('Worksheet details (for get, create, update actions)'),
    deleted: z.boolean().optional().describe('Whether the worksheet was successfully deleted'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'get': {
        if (!ctx.input.worksheetIdOrName) throw new Error('worksheetIdOrName is required for get action');
        let ws = await client.getWorksheet(ctx.input.workbookItemId, ctx.input.worksheetIdOrName);
        return {
      ...
      case 'update': {
        if (!ctx.input.worksheetIdOrName) throw new Error('worksheetIdOrName is required for update action');
        let props: any = {};
        ...
        let ws = await client.updateWorksheet(ctx.input.workbookItemId, ctx.input.worksheetIdOrName, props);
        return {
          output: { worksheet: mapWorksheet(ws) },
      ...
      case 'delete': {
        if (!ctx.input.worksheetIdOrName) throw new Error('worksheetIdOrName is required for delete action');
