...
Use **override** to replace existing values rather than merging. Tags added here are additive; use the dedicated tag management tool to remove tags.`,
  ...
  .input(z.object({
    ...
    override: z.boolean().optional().describe('Set to true to replace existing values instead of merging'),
    customFields: z.record(z.string(), z.string()).optional().describe('Custom fields as key-value pairs using internal names (e.g., field_123_city)'),
    ...
    from: z.string().optional().describe('Filter contacts created from this date (ISO 8601)'),
  ...
  .output(z.object({
    ...
    contacts: z.array(z.record(z.string(), z.unknown())).optional().describe('List of contact records'),
  }))
