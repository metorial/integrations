...
export let manageBroadcast = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'send_test', 'activity']).describe('Action to perform'),
    broadcastId: z.string().optional().describe('Broadcast ID (for get, send_test, activity)'),
    ...
    deliveryType: z.enum(['now', 'later', 'draft']).optional().describe('When to deliver the broadcast'),
    deliverAt: z.string().optional().describe('Scheduled delivery time (ISO 8601, used with deliveryType "later")'),
  ...
  .output(z.object({
    ...
    broadcasts: z.array(z.record(z.string(), z.unknown())).optional().describe('List of broadcast records'),
    activityReport: z.record(z.string(), z.unknown()).optional().describe('Broadcast activity report with opens, clicks, bounces, etc.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'send_test') {
      ...
      return {
        output: { success: result.success as boolean, statusMessage: result.message as string },
