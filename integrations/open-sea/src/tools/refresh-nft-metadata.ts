...
export let refreshNftMetadata = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'The refresh is asynchronous — metadata may not be updated immediately.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    chain: z.string().optional().describe('Blockchain the NFT is on (e.g. ethereum, polygon). Defaults to config chain or ethereum.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    await client.refreshNftMetadata(chain, ctx.input.contractAddress, ctx.input.tokenIdentifier);
