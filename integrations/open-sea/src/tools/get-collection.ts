...
export let getCollection = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeStats: z.boolean().optional().describe('Whether to also fetch collection statistics (floor price, volume, etc.)'),
  ...
  .output(z.object({
    ...
    contracts: z.array(contractSchema).describe('Smart contracts associated with this collection'),
    ...
    collectionOffersEnabled: z.boolean().nullable().describe('Whether collection offers are enabled'),
    stats: z.object({
    ...
    }).nullable().describe('Collection statistics (only included if includeStats is true)'),
    traits: z.record(z.string(), z.record(z.string(), z.number())).nullable().describe('Trait definitions with counts (only included if includeTraits is true)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let result = {
      ...
      safelistStatus: collectionData.safelist_request_status ?? collectionData.safelist_status ?? null,
