...
export let projectEventsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Project Events',
    key: 'project_events',
    description: '[Polling fallback] Polls for new events in a MongoDB Atlas project. Detects cluster changes, user modifications, backup events, network changes, and other administrative operations.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      if (!projectId) throw new Error('projectId is required in config for project event polling.');
      ...
      let newLastPolledAt = newEvents.length > 0
        ? newEvents[0].created
    ...
    handleEvent: async (ctx) => {
      ...
      let isAlert = (input.eventTypeName || '').includes('ALERT') ||
