...
export let getClusterMetricsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    processId: z.string().optional().describe('Process ID (hostname:port format). Required for measurements.'),
    granularity: z.string().optional().describe('Measurement granularity (e.g., PT1M, PT5M, PT1H, P1D)'),
    ...
    partitionName: z.string().optional().describe('Disk partition name (for disk measurements, e.g., "data")'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'get_disk_measurements') {
      if (!ctx.input.partitionName) throw new Error('partitionName is required for disk measurements.');
      let result = await client.getDiskMeasurements(projectId, ctx.input.processId, ctx.input.partitionName, {
        granularity: ctx.input.granularity,
        period: ctx.input.period,
        start: ctx.input.start,
        end: ctx.input.end,
