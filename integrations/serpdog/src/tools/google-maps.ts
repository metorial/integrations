...
export let googleMapsSearch = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    coordinates: z.string().optional().describe('GPS coordinates in format "@latitude,longitude,zoom" (e.g., "@40.7455096,-74.0083012,15.1z")'),
    language: z.string().optional().describe('Language of results (e.g., "en_us"). Defaults to "en_us".'),
    page: z.number().optional().describe('Page offset (0, 10, 20, etc.)'),
    googleDomain: z.string().optional().describe('Google domain (e.g., "google.co.in"). Defaults to "google.com".'),
    dataId: z.string().optional().describe('Google Maps data ID for fetching reviews, photos, or posts for a specific place'),
    retrieve: z.enum(['search', 'reviews', 'photos', 'posts']).optional().describe('What to retrieve. Defaults to "search". Use "reviews", "photos", or "posts" with a dataId.'),
    reviewSortBy: z.enum(['qualityScore', 'newestFirst', 'ratingHigh', 'ratingLow']).optional().describe('Sort order for reviews'),
