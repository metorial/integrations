...
export let bingSearch = SlateTool.create(
  ...
  .input(z.object({
    ...
    latitude: z.string().optional().describe('Latitude coordinate for location-based results'),
    longitude: z.string().optional().describe('Longitude coordinate for location-based results'),
    market: z.string().optional().describe('Market origin (e.g., "en-US")'),
    offset: z.number().optional().describe('Offset for organic results pagination. Defaults to 1.'),
    count: z.number().optional().describe('Number of results per page'),
    safeSearch: z.enum(['off', 'moderate', 'strict']).optional().describe('Safe search filter. Defaults to "off".'),
