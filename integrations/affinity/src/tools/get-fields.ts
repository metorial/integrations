...
let fieldSchema = z.object({
  ...
  listId: z.number().nullable().describe('ID of the list this field belongs to (null for global fields)'),
  enrichmentSource: z.string().nullable().describe('Source of enrichment data, if applicable'),
  valueType: z.number().describe('Type of value this field holds (0=person, 1=org, 2=dropdown, 3=number, 4=date, 5=location, 6=text, 7=ranked-dropdown)'),
  ...
  })).optional().describe('Available options for dropdown fields'),
});
...
- **0** = Person, **1** = Organization, **2** = Dropdown, **3** = Number, **4** = Date, **5** = Location, **6** = Text, **7** = Ranked dropdown`,
  ...
  .input(z.object({
    ...
    entityType: z.number().optional().describe('Filter by entity type (0=person, 1=organization, 8=opportunity)'),
