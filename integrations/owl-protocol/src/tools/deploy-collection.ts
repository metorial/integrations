...
export let deployCollection = SlateTool.create(
  spec,
  {
...
Returns the deployed contract address and transaction details.`,
  ...
  .input(z.object({
    chainId: z.number().describe('Blockchain network chain ID to deploy on (e.g. 150150 for Owl testnet)'),
    name: z.string().describe('Name of the collection'),
    symbol: z.string().describe('Ticker symbol of the collection (3-4 characters)'),
    baseUri: z.string().optional().describe('Base URI for token metadata. Defaults to the Owl Protocol hosted endpoint.'),
    royaltyReceiver: z.string().optional().describe('ERC2981 royalty receiver address. Defaults to your smart account address.'),
    feeNumerator: z.string().optional().describe('ERC2981 royalty amount in basis points (e.g. "500" for 5%). Defaults to 500.'),
    contractImage: z.string().optional().describe('Base64-encoded image for the contract metadata'),
    contractImageSuffix: z.string().optional().describe('File extension of the contract image (e.g. "png", "jpg")'),
  }))
  .output(z.object({
    ...
    deployParams: z.record(z.string(), z.unknown()).optional().describe('Additional deployment parameters'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (!chainId) {
      throw new Error('chainId is required. Provide it in the input or set a default in the configuration.');
    }

    ctx.info({ message: 'Deploying collection', name: ctx.input.name, symbol: ctx.input.symbol, chainId });
