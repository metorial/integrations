...
When creating, **resourceId**, **projectId**, **start**, and **end** are required. Supports percentage-based and minute-based allocation, repeating bookings, and approval workflows.`,
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'delete', 'delete_bulk']).describe('Operation to perform'),
    bookingId: z.string().optional().describe('Booking ID, required for update and single delete'),
    bookingIds: z.array(z.string()).optional().describe('Array of booking IDs for bulk delete'),
    resourceId: z.string().optional().describe('Resource ID to schedule, required for create'),
    projectId: z.string().optional().describe('Project ID to schedule on, required for create'),
    ...
    state: z.enum(['STATE_DAY_MINUTE', 'STATE_PERCENTAGE', 'STATE_TOTAL_MINUTE']).optional().describe('Booking state type'),
    stateValue: z.number().optional().describe('Value for the state (percentage or minutes)'),
    allDay: z.boolean().optional().describe('Whether the booking is all day'),
    type: z.enum(['SCHEDULED', 'WAITING_FOR_APPROVAL']).optional().describe('Booking type. Use WAITING_FOR_APPROVAL for booking requests.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { action, bookingId, bookingIds, resourceId, projectId, allowOverschedule, ...fields } = ctx.input;
    ...
    if (action === 'delete_bulk') {
      if (!bookingIds || bookingIds.length === 0) throw new Error('bookingIds is required for bulk delete');
