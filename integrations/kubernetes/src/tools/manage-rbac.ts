...
let ruleSchema = z.object({
  ...
  verbs: z.array(z.string()).describe('Allowed verbs (e.g. "get", "list", "create", "update", "delete", "watch")'),
...
let subjectSchema = z.object({
  ...
  subjectNamespace: z.string().optional().describe('Namespace (required for ServiceAccount subjects)'),
});

export let manageRbac = SlateTool.create(
  spec,
  {
    name: 'Manage RBAC',
    key: 'manage_rbac',
...
Use this to define access control policies for users, groups, and service accounts.`,
  ...
  .input(z.object({
    ...
    resourceKind: z.enum(['roles', 'clusterroles', 'rolebindings', 'clusterrolebindings']).describe('RBAC resource type'),
    resourceName: z.string().describe('Name of the RBAC resource'),
    namespace: z.string().optional().describe('Namespace (required for Roles and RoleBindings)'),
    rules: z.array(ruleSchema).optional().describe('Permission rules (for Roles/ClusterRoles)'),
    roleRef: z.object({
    ...
    }).optional().describe('Role reference (for Bindings)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.manifest) {
      ...
      let body: any = {
        ...
        subjects: ctx.input.subjects?.map((s) => ({
          kind: s.subjectKind,
          name: s.subjectName,
