...
export let uploadBulkFile = SlateTool.create(
  spec,
  {
...
export let checkBulkStatus = SlateTool.create(
  spec,
  {
...
Use the job ID returned by the **Upload Bulk Verification File** tool.`,
  ...
  .input(z.object({
    jobId: z.string().describe('The job/file ID returned from uploading a bulk verification file'),
  }))
  .output(z.object({
    ...
    status: z.string().describe('Current processing status (e.g., "finished", "processing")'),
    downloadLinkAll: z.string().describe('Download link for all results (available when finished)'),
    downloadLinkOk: z.string().describe('Download link for verified/passed results only (available when finished)'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
    ...
    let progressText = result.status === 'finished'
      ? 'Processing complete.'
