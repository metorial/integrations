...
export let listSalesInvoices = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    state: z.enum(['all', 'draft', 'open', 'scheduled', 'pending_payment', 'late', 'reminded', 'paid', 'uncollectible']).optional().describe('Filter by invoice state'),
    period: z.string().optional().describe('Filter by period: "this_month", "prev_month", "this_quarter", "this_year", "prev_year", or custom range "YYYYMMDD..YYYYMMDD"'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let mapped = invoices.map((inv: any) => ({
      ...
      contactName: inv.contact?.company_name || `${inv.contact?.firstname || ''} ${inv.contact?.lastname || ''}`.trim() || null,
