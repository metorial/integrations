...
export let createSalesInvoice = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    pricesAreInclTax: z.boolean().optional().describe('Whether prices include tax (default: true)'),
    ...
    lineItems: z.array(z.object({
      ...
      productId: z.string().optional().describe('Product ID (auto-fills description, price, tax rate)'),
      period: z.string().optional().describe('Period for the line item (e.g., "20240101..20240131")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.pricesAreInclTax !== undefined) invoiceData.prices_are_incl_tax = ctx.input.pricesAreInclTax;
