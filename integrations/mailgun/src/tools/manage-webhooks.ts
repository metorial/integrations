...
export let listWebhooks = SlateTool.create(
  ...
  .output(z.object({
    webhooks: z.record(z.string(), z.object({
...
export let createWebhook = SlateTool.create(
  ...
  .input(z.object({
    ...
    eventType: z.enum(['accepted', 'delivered', 'opened', 'clicked', 'unsubscribed', 'complained', 'permanent_fail', 'temporary_fail'])
