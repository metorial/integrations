...
let eventOutputSchema = z.object({
  ...
  eventType: z.string().describe('Event type (accepted, delivered, failed, opened, clicked, etc.)'),
...
Filter by event type, recipient, sender, subject, date range, and more. Useful for tracking email delivery status and debugging issues.`,
  ...
  .input(z.object({
    ...
    eventType: z.enum(['accepted', 'rejected', 'delivered', 'failed', 'opened', 'clicked', 'unsubscribed', 'complained', 'stored']).optional().describe('Filter by event type'),
    ...
    severity: z.enum(['permanent', 'temporary']).optional().describe('Filter failed events by severity'),
