...
export let getProofRequest = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    presentation: z.any().optional().describe('The submitted verifiable presentation (if verified)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let status = result?.verified === true ? 'verified' : (result?.expired ? 'expired' : 'pending');
