...
let contactSchema = z.object({
  ...
  status: z.enum(['Active', 'Unsubscribed', 'Bounced']).optional().describe('Email subscription status'),
  smsStatus: z.enum(['Active', 'Unsubscribed', 'Bounced']).optional().describe('SMS subscription status'),
...
export let getContact = SlateTool.create(
  ...
  .output(z.object({
    ...
    mailingLists: z.array(z.any()).optional().describe('Mailing lists this contact belongs to'),
...
export let listContacts = SlateTool.create(
  ...
  .input(z.object({
    status: z.enum(['Active', 'Unsubscribed', 'Bounced']).optional().describe('Filter by contact status'),
    searchTerm: z.string().optional().describe('Search by email or SMS number'),
    fromDate: z.string().optional().describe('Filter by status change from date (YYYY-MM-DD)'),
