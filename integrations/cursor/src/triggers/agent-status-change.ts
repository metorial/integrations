...
export let agentStatusChange = SlateTrigger.create(
  spec,
  {
    name: 'Agent Status Change',
    key: 'agent_status_change',
    description: 'Triggered when a Cursor cloud agent reaches FINISHED or ERROR status. Configure the webhook URL when launching agents using the Launch Agent tool.',
  }
)
  .input(z.object({
