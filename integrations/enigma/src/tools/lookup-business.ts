...
export let lookupBusiness = SlateTool.create(
  spec,
  {
...
Specify which attributes to retrieve, or leave empty for default attributes. Supports time-series data (merchant transaction signals) with configurable lookback periods.`,
  ...
  .input(z.object({
    enigmaId: z.string().describe('The Enigma ID of the business to look up (obtained from the Match Business tool)'),
    attributes: z.array(z.string()).optional().describe('Specific attributes to retrieve (e.g., "names", "addresses", "websites", "industries", "associated_people", "phone_numbers", "corporate_structure", "operating_status", "headcount", "business_bankruptcy", "state_registrations", "verification_score"). Leave empty for default attributes.'),
    lookbackMonths: z.number().int().min(1).optional().describe('Number of months of historical data to retrieve for time-series attributes (e.g., merchant transaction signals). Omit for most recent month only.'),
  }))
  .output(z.object({
    ...
    industries: z.any().optional().describe('Industry classifications (NAICS, SIC, MCC codes)'),
    ...
    businessLocationEnigmaIds: z.any().optional().describe('Associated business location IDs'),
    dataSources: z.any().optional().describe('Data sources used for this profile'),
    rawAttributes: z.record(z.string(), z.any()).optional().describe('All raw attributes returned by the API'),
  }))
