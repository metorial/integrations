...
export let manageStateTool = SlateTool.create(
  spec,
  {
    name: 'Manage State',
    key: 'manage_state',
  ...
  .input(z.object({
    ...
    stateType: z.enum(['conversation', 'user', 'bot', 'workflow', 'integration', 'task']).describe('Scope of the state'),
    resourceId: z.string().describe('ID of the resource (conversationId, userId, or botId depending on stateType)'),
    stateName: z.string().describe('Name of the state as declared in the bot definition'),
    payload: z.record(z.string(), z.unknown()).optional().describe('State data (required for set and patch)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'get') {
      let result = await client.getState(ctx.input.stateType, ctx.input.resourceId, ctx.input.stateName);
      let s = result.state;
      return {
        output: {
          stateId: s.id,
    ...
    if (ctx.input.action === 'set') {
      ...
      let result = await client.setState(ctx.input.stateType, ctx.input.resourceId, ctx.input.stateName, ctx.input.payload);
      let s = result.state;
      return {
        output: {
          stateId: s.id,
    ...
    if (ctx.input.action === 'patch') {
      ...
      let result = await client.patchState(ctx.input.stateType, ctx.input.resourceId, ctx.input.stateName, ctx.input.payload);
      let s = result.state;
      return {
        output: {
          stateId: s.id,
