...
export let getPipeline = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeTestReport: z.boolean().optional().describe('Whether to include the test report summary')
  }))
  .output(z.object({
    pipelineId: z.number(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includeTestReport) {
      try {
        testReport = await client.getPipelineTestReportSummary(projectId, ctx.input.pipelineId);
      } catch {
