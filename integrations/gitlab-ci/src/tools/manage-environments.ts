...
export let manageEnvironments = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'get', 'create', 'update', 'stop', 'delete']).describe('Action to perform'),
    environmentId: z.number().optional().describe('Environment ID (required for get, update, stop, delete)'),
    name: z.string().optional().describe('Environment name (required for create, optional filter for list)'),
    search: z.string().optional().describe('Search filter for list'),
    states: z.enum(['available', 'stopped', 'stopping']).optional().describe('Filter by state for list'),
    externalUrl: z.string().optional().describe('External URL for the environment'),
    tier: z.enum(['production', 'staging', 'testing', 'development', 'other']).optional().describe('Deployment tier')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'list') {
      let result = await client.listEnvironments(projectId, { name, search, states }) as any[];
