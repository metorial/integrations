...
export let manageTriggers = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'create', 'update', 'delete', 'fire']).describe('Action to perform'),
    ...
    ref: z.string().optional().describe('Branch or tag to trigger pipeline on (required for fire)'),
    variables: z.record(z.string(), z.string()).optional().describe('Key-value pairs of variables to pass to the triggered pipeline')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update') {
      ...
      let t = await client.updatePipelineTrigger(projectId, ctx.input.triggerId, ctx.input.description);
      return {
        output: { trigger: mapTrigger(t) },
    ...
    let p = await client.triggerPipeline(projectId, ctx.input.triggerToken, ctx.input.ref, ctx.input.variables) as any;
    return {
      output: {
        pipeline: {
          pipelineId: p.id,
