...
export let lintCiConfig = SlateTool.create(
  spec,
  {
    name: 'Lint CI Config',
    key: 'lint_ci_config',
  ...
  .input(z.object({
    ...
    dryRun: z.boolean().optional().describe('Simulate pipeline creation to catch runtime errors'),
    includeMergedYaml: z.boolean().optional().describe('Include the merged YAML output after includes are resolved'),
