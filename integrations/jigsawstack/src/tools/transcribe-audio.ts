...
export let transcribeAudio = SlateTool.create(
  ...
  .input(z.object({
    ...
    fileStoreKey: z.string().optional().describe('File store key of a previously uploaded audio/video file'),
    language: z.string().optional().describe('Language code or "auto" for automatic detection'),
    translate: z.boolean().optional().describe('Translate the transcript to English (default: false)'),
    bySpeaker: z.boolean().optional().describe('Separate text by speaker (diarization) (default: false)'),
    webhookUrl: z.string().optional().describe('Webhook URL for async processing - results will be POSTed there when ready'),
    batchSize: z.number().optional().describe('Max chunks per response (default: 30, max: 40)'),
    chunkDuration: z.number().optional().describe('Duration per audio chunk in seconds (default: 3, max: 15)'),
  }))
  .output(z.object({
    success: z.boolean(),
    ...
    })).optional().describe('Timestamped transcript segments'),
    speakers: z.array(z.object({
      speaker: z.string().optional(),
      text: z.string().optional(),
      timestamp: z.array(z.number()).optional(),
    })).optional().describe('Speaker-separated transcripts (when bySpeaker is enabled)'),
    ...
    status: z.string().optional().describe('Processing status when webhook is used (e.g., "processing")'),
