...
export let extractFromImage = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Multi-page PDFs support up to 10 pages per request.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    fileStoreKey: z.string().optional().describe('File store key of a previously uploaded image or PDF'),
    prompt: z.union([
      z.string(),
      z.array(z.string()),
    ]).optional().describe('What to extract. Use a string for a general description, or an array of field names (e.g., ["total_price", "tax", "date"]).'),
    fineGrained: z.boolean().optional().describe('Enable enhanced OCR for improved text positioning'),
    pageRange: z.array(z.number()).optional().describe('Two-element array [startPage, endPage] for multi-page PDFs'),
  }))
  .output(z.object({
    success: z.boolean(),
    context: z.unknown().describe('Extracted data - string for single prompt, or object with keys matching prompt items'),
    ...
    })).optional().describe('Text sections with positioning data'),
