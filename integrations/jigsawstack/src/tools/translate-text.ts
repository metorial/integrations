...
export let translateText = SlateTool.create(
  ...
  .input(z.object({
    text: z.union([z.string(), z.array(z.string())]).describe('Text to translate. Can be a single string or an array of strings for batch translation.'),
    targetLanguage: z.string().describe('Target language code (e.g., "es", "fr", "de", "ja")'),
    currentLanguage: z.string().optional().describe('Source language code. Auto-detected if not provided.'),
  }))
  .output(z.object({
    success: z.boolean(),
    translatedText: z.union([z.string(), z.array(z.string())]).describe('Translated text matching the input format (single string or array)'),
  }))
