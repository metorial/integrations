...
export let predictTimeSeries = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    dataset: z.array(z.object({
      ...
      value: z.union([z.number(), z.string()]).describe('Numeric value for the data point'),
    })).describe('Historical data points (5-1000 entries)'),
    steps: z.number().optional().describe('Number of future data points to forecast (default: 5, max: 500)'),
  }))
  .output(z.object({
    success: z.boolean(),
