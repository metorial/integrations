...
export let taskCompleted = SlateTrigger.create(
  spec,
  {
    name: 'Task Completed',
    key: 'task_completed',
    description: 'Receives webhook notifications when long-running JigsawStack tasks complete (e.g., speech-to-text transcription). Configure the webhook URL in your JigsawStack API request using the webhook_url parameter.',
    instructions: [
  ...
  .input(z.object({
    ...
    speakers: z.array(z.object({
      ...
      text: z.string().optional(),
      timestamp: z.array(z.number()).optional(),
    })).optional().describe('Speaker-separated transcripts'),
        ...
        inputs: [
          {
            ...
            speakers: data.speakers as Array<{ speaker?: string; text?: string; timestamp?: number[] }> | undefined,
