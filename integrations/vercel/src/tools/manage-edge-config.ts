...
export let manageEdgeConfigTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'delete', 'get_items', 'update_items']).describe('Action to perform'),
    edgeConfigId: z.string().optional().describe('Edge Config ID (required for get, delete, get_items, update_items)'),
    slug: z.string().optional().describe('Edge Config slug/name (required for create)'),
    items: z.array(z.object({
      operation: z.enum(['create', 'update', 'upsert', 'delete']).describe('Operation type'),
      key: z.string().describe('Item key'),
      value: z.any().optional().describe('Item value (not needed for delete)'),
    })).optional().describe('Items to modify (for update_items)'),
  }))
  .output(z.object({
    edgeConfigs: z.array(z.object({
    ...
    })).optional().describe('List of Edge Config stores'),
    edgeConfig: z.object({
      edgeConfigId: z.string().describe('Edge Config ID'),
      slug: z.string().optional().describe('Edge Config slug'),
      createdAt: z.number().optional().describe('Creation timestamp'),
      itemCount: z.number().optional().describe('Number of items'),
    }).optional().describe('Edge Config details'),
    items: z.array(z.object({
      key: z.string().describe('Item key'),
      value: z.any().describe('Item value'),
      createdAt: z.number().optional().describe('Creation timestamp'),
      updatedAt: z.number().optional().describe('Last update timestamp'),
    })).optional().describe('Edge Config items'),
