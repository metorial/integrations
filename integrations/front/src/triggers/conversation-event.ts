...
export let conversationEvent = SlateTrigger.create(
  spec,
  {
    ...
    description: '[Polling fallback] Triggers when conversation events occur in Front, including assignments, status changes, messages, tags, comments, and more. Polls the Events API for new activity.',
  }
)
  .input(z.object({
    ...
    conversationSubject: z.string().optional().describe('Subject of the affected conversation'),
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let newTimestamp = events.length > 0
        ? Math.max(...events.map((e) => e.emitted_at))
        : lastEventTimestamp;
