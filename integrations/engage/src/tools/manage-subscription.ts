...
export let manageSubscription = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['subscribe', 'update_status', 'remove', 'add_to_lists', 'remove_from_lists'])
      .describe('Operation to perform'),
    listId: z.string().optional().describe('List ID (required for subscribe, update_status, remove)'),
    uid: z.string().optional().describe('User UID (required for update_status, remove, add_to_lists, remove_from_lists)'),
    subscribed: z.boolean().optional().describe('Subscription status (for update_status: true=subscribe, false=unsubscribe)'),
    listIds: z.array(z.string()).optional().describe('List IDs for bulk add/remove operations'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update_status') {
      ...
      if (subscribed === undefined) throw new Error('subscribed is required for update_status action');
    ...
    if (action === 'add_to_lists') {
      ...
      if (!listIds || listIds.length === 0) throw new Error('listIds is required for add_to_lists action');
    ...
    if (!listIds || listIds.length === 0) throw new Error('listIds is required for remove_from_lists action');
