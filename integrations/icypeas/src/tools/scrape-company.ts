...
export let scrapeCompany = SlateTool.create(
  spec,
  {
    name: 'Scrape Company',
    key: 'scrape_company',
    ...
    constraints: [
      'Maximum 50 company URLs per request when using bulk mode.'
    ],
    tags: {
      readOnly: true,
      destructive: false
  ...
  .input(z.object({
    companyUrls: z.array(z.string()).describe('One or more LinkedIn company profile URLs to scrape')
  }))
  .output(z.object({
