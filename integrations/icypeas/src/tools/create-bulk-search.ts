...
export let createBulkSearch = SlateTool.create(
  spec,
  {
...
For **domain-search**: each row should be \`[domainOrCompany]\`.`,
    constraints: [
      'Maximum 5,000 items per bulk search.'
    ],
    tags: {
      readOnly: false,
      destructive: false
  ...
  .input(z.object({
    ...
    task: z.enum(['email-search', 'email-verification', 'domain-search']).describe('Type of bulk search to perform'),
    rows: z.array(z.array(z.string())).describe('Array of rows to process. Format depends on task type.')
  }))
  .output(z.object({
