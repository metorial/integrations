...
export let searchLeads = SlateTool.create(
  ...
  .input(z.object({
    countOnly: z.boolean().optional().describe('If true, only count matching results without retrieving data (free)'),
    ...
    currentCompanyWebsite: includeExcludeSchema.describe('Filter by current company website/domain'),
    pastCompanyWebsite: includeExcludeSchema.describe('Filter by past company website/domain'),
    location: includeExcludeSchema.describe('Filter by location (city, country name, or alpha-2 code)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let filterFields = [
      'currentJobTitle', 'pastJobTitle', 'currentCompanyName', 'pastCompanyName',
      'currentCompanyWebsite', 'pastCompanyWebsite', 'location', 'school',
      'languages', 'skills', 'keyword', 'firstname', 'lastname'
    ] as const;
