...
Optionally configure start/end dates, minimum purchase amount, usage limits, and internal notes.`,
  ...
  .input(z.object({
    ...
    couponType: z.enum(['single', 'multi', 'collection']).describe('"single" for one product, "multi" for all products, "collection" for a product collection'),
    percentOff: z.number().min(0).max(100).optional().describe('Percentage discount (0-100). Mutually exclusive with amountOff.'),
    amountOff: z.number().optional().describe('Fixed discount amount in cents. Mutually exclusive with percentOff.'),
    productKey: z.string().optional().describe('Unique product key, required when couponType is "single"'),
    collectionId: z.string().optional().describe('Unique collection ID, required when couponType is "collection"'),
    ...
    minimumPurchaseAmount: z.number().optional().describe('Minimum purchase amount in cents required to use this coupon'),
    usageLimit: z.number().optional().describe('Maximum number of times this coupon can be redeemed'),
    notes: z.string().optional().describe('Internal notes about the coupon (not visible to customers)')
  }))
  .output(z.object({
    ...
    productKey: z.string().nullable().describe('Product key if coupon is scoped to a single product'),
    collectionId: z.string().nullable().describe('Collection ID if coupon is scoped to a collection'),
