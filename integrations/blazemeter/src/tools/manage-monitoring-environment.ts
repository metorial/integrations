...
export let manageMonitoringEnvironment = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    initialVariables: z.record(z.string(), z.string()).optional().describe('Key-value pairs of variables used in test steps'),
    regions: z.array(z.string()).optional().describe('Monitoring regions (e.g., "us1", "eu1", "ap1")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'update') {
      ...
      let env = await client.updateEnvironment(ctx.input.bucketKey, ctx.input.testId, ctx.input.environmentId, {
        name: ctx.input.name,
        initialVariables: ctx.input.initialVariables,
        regions: ctx.input.regions,
        webhooks: ctx.input.webhooks,
