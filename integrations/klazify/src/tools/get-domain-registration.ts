...
export let getDomainRegistration = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    domainAgeDate: z.string().nullable().describe('Domain original registration date (YYYY-MM-DD)'),
    domainAgeDaysAgo: z.string().nullable().describe('Number of days since domain was registered'),
    domainExpirationDate: z.string().nullable().describe('Domain expiration date (YYYY-MM-DD)'),
    domainExpirationDaysLeft: z.string().nullable().describe('Number of days until domain expires'),
  }))
