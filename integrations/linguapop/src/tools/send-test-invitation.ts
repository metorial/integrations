...
export let sendTestInvitation = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    languageCode: z.string().describe('Language code for the test (e.g., eng, ita, spa, ger, fra)'),
    externalId: z.string().optional().describe('Optional external identifier for mapping to your CRM or internal system'),
    sendEmail: z.boolean().optional().describe('Whether Linguapop should send the invitation email to the candidate. If false, use the returned test URL to deliver the test yourself'),
    kioskMode: z.boolean().optional().describe('Whether to generate a kiosk code so the candidate can access the test through a configured kiosk'),
    includeReading: z.boolean().optional().describe('Whether to include a reading section in addition to the core grammar test'),
    includeListening: z.boolean().optional().describe('Whether to include a listening section in addition to the core grammar test'),
    returnUrl: z.string().optional().describe('URL to redirect the candidate to after completing the test'),
    callbackUrl: z.string().optional().describe('URL to receive test results via webhook when the test is completed'),
  }))
  .output(z.object({
    ...
    kioskCode: z.string().optional().describe('Kiosk code for accessing the test through a configured kiosk (only present if kiosk mode was enabled)'),
  }))
