...
export let listServicesTool = SlateTool.create(
  spec,
  {
    name: 'List Services',
    key: 'list_services',
...
export let createServiceTool = SlateTool.create(
  ...
  .input(z.object({
    name: z.string().describe('Name of the service'),
    info: z.string().optional().describe('Description or additional information about the service'),
    color: z.string().optional().describe('Color code for the service (e.g., hex color)'),
    acronym: z.string().optional().describe('Short acronym for the service'),
    active: z.boolean().optional().describe('Whether the service is active (defaults to true)'),
    externalId: z.string().optional().describe('External ID for syncing with other systems')
  }))
