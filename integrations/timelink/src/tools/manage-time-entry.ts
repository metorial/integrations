...
export let searchTimeEntriesTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    after: z.string().optional().describe('Return entries after this date/time (ISO 8601 format)'),
    before: z.string().optional().describe('Return entries before this date/time (ISO 8601 format)'),
...
export let getTimeEntryTool = SlateTool.create(
  spec,
  {
    name: 'Get Time Entry',
    key: 'get_time_entry',
...
export let createTimeEntryTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    userId: z.number().optional().describe('ID of the user (defaults to authenticated user)'),
    paid: z.boolean().optional().describe('Whether the time entry has been paid'),
    billable: z.boolean().optional().describe('Whether the time entry is billable'),
    externalId: z.string().optional().describe('External ID for syncing with other systems')
  }))
...
export let updateTimeEntryTool = SlateTool.create(
  spec,
  {
...
export let deleteTimeEntryTool = SlateTool.create(
  spec,
  {
