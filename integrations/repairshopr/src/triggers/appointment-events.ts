...
export let appointmentEvents = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers when an appointment is created or updated. Configure the webhook URL in RepairShopr under Admin > Notification Center.',
  }
)
  .input(z.object({
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      let appointment = ctx.input.webhookPayload?.appointment || ctx.input.webhookPayload || {};
