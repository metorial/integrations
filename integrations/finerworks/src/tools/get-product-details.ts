...
export let getProductDetails = SlateTool.create(
  ...
  .input(z.object({
    items: z.array(z.object({
      ...
      productQty: z.number().int().min(1).max(999).default(1).describe('Quantity for pricing calculation'),
    })).min(1).max(25).describe('SKUs to look up'),
  }))
  .output(z.object({
    products: z.array(z.object({
      ...
      }).optional().describe('Physical dimensions'),
