...
export let scheduleMessageTool = SlateTool.create(
  spec,
  {
...
The scheduled time must be in **UTC ISO-8601 format** (e.g. \`2025-03-01T14:00:00Z\`).`,
  ...
  .input(z.object({
    ...
    scheduledSendTime: z.string().describe('UTC ISO-8601 datetime to publish (e.g. 2025-03-01T14:00:00Z)'),
    mediaUrls: z.array(z.object({
      url: z.string().describe('Public URL of the media file')
    })).optional().describe('Media URLs to attach'),
    mediaIds: z.array(z.string()).optional().describe('Media IDs from previous uploads to attach'),
    tags: z.array(z.string()).optional().describe('Tags to apply to the message'),
    location: z.object({
      latitude: z.number().describe('Latitude coordinate'),
      longitude: z.number().describe('Longitude coordinate')
    }).optional().describe('Location to tag in the post'),
    emailNotification: z.boolean().optional().describe('Send email notification when published'),
    webhookUrls: z.array(z.string()).optional().describe('Webhook URLs for status change notifications')
  }))
  .output(z.object({
