...
export let getPostEngagement = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {
        ...
        likeCount: summary.likes?.aggregatedTotalLikes ?? summary.totalShareStatistics?.likeCount,
        commentCount: summary.comments?.aggregatedTotalComments ?? summary.totalShareStatistics?.commentCount,
