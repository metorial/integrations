...
export let manageJobs = SlateTool.create(
  ...
  .input(z.object({
    ...
    instanceIds: z.array(z.number()).optional().describe('Filter by scheduler or time clock IDs'),
    jobIds: z.array(z.string()).optional().describe('Filter by job IDs'),
    jobNames: z.array(z.string()).optional().describe('Filter by job names (case-sensitive)'),
