...
export let manageTasks = SlateTool.create(
  ...
  .input(z.object({
    ...
    taskId: z.string().optional().describe('Task ID (for update_task, delete_task, sub-task operations)'),
    subTaskId: z.string().optional().describe('Sub-task ID (for update_sub_task, delete_sub_task)'),
    taskBody: z.object({
      ...
      status: z.enum(['draft', 'published', 'completed']).optional().describe('Task status'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update_sub_task') {
      ...
      let result = await client.updateSubTask(taskBoardId, taskId, subTaskId, ctx.input.subTaskBody);
      return {
        output: { result },
