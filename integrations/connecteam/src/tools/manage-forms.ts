...
export let manageForms = SlateTool.create(
  spec,
  {
    name: 'Manage Forms',
    key: 'manage_forms',
  ...
  .input(z.object({
    ...
    formId: z.number().optional().describe('Form ID (required for get_form, list_submissions, get_submission)'),
    ...
    submittingStartTimestamp: z.number().optional().describe('Filter submissions after this Unix timestamp'),
    submittingEndTime: z.number().optional().describe('Filter submissions before this Unix timestamp'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'get_submission') {
      ...
      let result = await client.getFormSubmission(ctx.input.formId, ctx.input.formSubmissionId);
      return {
        output: { result },
