...
export let sendChatMessage = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    target: z.enum(['conversation', 'private_message']).describe('Send to an existing conversation or as a private message to a user'),
    conversationId: z.string().optional().describe('Conversation ID (required for "conversation" target)'),
    userId: z.string().optional().describe('User ID to send private message to (required for "private_message" target)'),
    ...
    })).optional().describe('File attachments (upload via Attachments API first)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.target === 'conversation') {
      if (!ctx.input.conversationId) throw new Error('conversationId is required for conversation target.');
