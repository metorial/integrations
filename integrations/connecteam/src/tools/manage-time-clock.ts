...
export let manageTimeClock = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    timeClockId: z.string().optional().describe('Time clock ID (required for all actions except list_time_clocks)'),
    ...
    startDate: z.string().optional().describe('Start date filter (YYYY-MM-DD) for time activities and timesheet'),
    endDate: z.string().optional().describe('End date filter (YYYY-MM-DD) for time activities and timesheet'),
