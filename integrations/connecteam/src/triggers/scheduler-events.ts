...
export let schedulerEvents = SlateTrigger.create(
  spec,
  {
    name: 'Scheduler Events',
    key: 'scheduler_events',
    description: 'Triggers when shifts are created, updated, or deleted, and when availability statuses change. For group shifts assigned to multiple users, each user triggers a separate event.',
  }
)
  .input(z.object({
    ...
    availabilityData: z.any().optional().describe('Availability data (for availability events)'),
  }))
  .output(z.object({
    ...
    availabilityType: z.string().optional().describe('Availability type: available or unavailable'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let isAvailability = eventType === 'availability_status_created' || eventType === 'availability_status_deleted';
