...
export let timeActivityEvents = SlateTrigger.create(
  spec,
  {
    name: 'Time Activity Events',
    key: 'time_activity_events',
    description: 'Triggers when time clock activities occur: clock in/out, admin additions/edits/deletions, and request approvals. Requires a specific time clock ID for webhook registration.',
  }
)
  .input(z.object({
    ...
    timeActivity: z.any().optional().describe('Time activity details (for non-delete events)'),
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      let { eventType, requestId, userId, timeClockId, activityType, timeActivity, timeActivityId } = ctx.input;
