...
export let trackSale = SlateTool.create(
  spec,
  {
    name: 'Track Sale',
    key: 'track_sale',
  ...
  .input(z.object({
    ...
    paymentProcessor: z.enum(['stripe', 'shopify', 'polar', 'paddle', 'revenuecat', 'custom']).optional().describe('Payment processor used'),
    invoiceId: z.string().optional().describe('Invoice ID for idempotency'),
    metadata: z.record(z.string(), z.unknown()).optional().describe('Additional sale metadata'),
    clickId: z.string().optional().describe('Click ID for direct attribution via dub_id cookie'),
