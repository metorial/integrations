...
export let manageTags = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'delete', 'list']).describe('The operation to perform'),
    ...
    color: z.enum(['red', 'yellow', 'green', 'blue', 'purple', 'brown', 'gray', 'pink']).optional().describe('Tag color'),
  ...
  .output(z.object({
    ...
    tag: z.object({
      ...
      name: z.string(),
      color: z.string()
    }).optional().describe('Created/updated tag'),
