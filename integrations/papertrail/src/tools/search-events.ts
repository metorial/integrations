...
export let searchEvents = SlateTool.create(
  ...
  .input(z.object({
    query: z.string().optional().describe('Search query string using Papertrail search syntax'),
    ...
    minTime: z.number().optional().describe('Earliest timestamp (Unix epoch seconds) to include'),
    maxTime: z.number().optional().describe('Latest timestamp (Unix epoch seconds) to include'),
    minId: z.string().optional().describe('Return events after this event ID (for pagination)'),
    maxId: z.string().optional().describe('Return events before this event ID (for pagination)'),
    limit: z.number().optional().describe('Maximum number of events to return (default 1000, max 10000)'),
    tail: z.boolean().optional().describe('Enable live tail mode to prioritize recency over completeness'),
  }))
  .output(z.object({
    events: z.array(logEventSchema).describe('Array of matching log events'),
    minId: z.string().optional().describe('Smallest event ID in the result set, for backward pagination'),
    maxId: z.string().optional().describe('Largest event ID in the result set, for forward pagination'),
    reachedTimeLimit: z.boolean().optional().describe('Whether the search time limit was reached before all results were returned'),
    minTimeAt: z.string().optional().describe('Oldest timestamp searched during the request'),
