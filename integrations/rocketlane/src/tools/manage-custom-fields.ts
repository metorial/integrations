...
export let listCustomFields = SlateTool.create(
  ...
  .input(z.object({
    entityType: z.enum(['project', 'task', 'company']).describe('Type of entity to list custom fields for'),
  }))
  .output(z.object({
    fields: z.array(z.object({
      fieldId: z.number().describe('Field ID'),
      fieldLabel: z.string().optional().describe('Field label'),
      fieldType: z.string().optional().describe('Field type (e.g. text, dropdown, number)'),
      options: z.array(z.any()).optional().describe('Available options (for dropdown fields)'),
...
export let createCustomField = SlateTool.create(
  ...
  .input(z.object({
    ...
    fieldType: z.string().describe('Field type (e.g. text, number, dropdown, date, checkbox)'),
    entityType: z.string().describe('Entity type this field belongs to (project, task, or company)'),
    options: z.array(z.object({
      label: z.string().describe('Option label'),
      value: z.string().describe('Option value'),
    })).optional().describe('Options for dropdown-type fields'),
  }))
  .output(z.object({
