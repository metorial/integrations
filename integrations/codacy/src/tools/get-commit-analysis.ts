...
export let getCommitAnalysis = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    commitSha: z.string().optional().describe('Specific commit SHA to get analysis for. If omitted, lists recent commits.'),
    branchName: z.string().optional().describe('Branch name to filter commits (used when listing commits).'),
    cursor: z.string().optional().describe('Pagination cursor for listing commits.'),
    limit: z.number().min(1).max(100).optional().describe('Maximum number of commits to return when listing (1-100).'),
  }))
  .output(z.object({
    commit: z.object({
    ...
    }).optional().describe('Single commit analysis (when commitSha provided).'),
    ...
    })).optional().describe('List of commits (when listing).'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.commitSha) {
      let response = await client.getCommitWithAnalysis(ctx.input.repositoryName, ctx.input.commitSha);
