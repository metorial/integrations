...
export let manageRepositoryToken = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum of 100 tokens per repository.',
    ],
    tags: {
      destructive: true,
      readOnly: false,
  ...
  .output(z.object({
    tokens: z.array(z.any()).optional().describe('List of repository tokens (for list action).'),
    createdToken: z.string().optional().describe('Newly created repository API token value (for create action). Store this securely as it cannot be retrieved later.'),
    deleted: z.boolean().optional().describe('Whether the token was deleted (for delete action).'),
  }))
