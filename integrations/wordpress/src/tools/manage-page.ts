...
export let listPagesTool = SlateTool.create(
  spec,
  {
    name: 'List Pages',
    key: 'list_pages',
  ...
  .input(z.object({
    status: z.string().optional().describe('Filter by page status (draft, publish, pending, private)'),
    ...
    orderBy: z.string().optional().describe('Order results by field (date, title, modified)'),
...
export let createPageTool = SlateTool.create(
  spec,
  {
    name: 'Create Page',
    key: 'create_page',
  ...
  .input(z.object({
    ...
    status: z.enum(['draft', 'publish', 'pending', 'private']).optional().describe('Page status. Defaults to "draft"'),
    parentId: z.string().optional().describe('ID of the parent page for hierarchical organization'),
...
export let updatePageTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['draft', 'publish', 'pending', 'private']).optional().describe('New page status'),
