...
export let composeMusic = SlateTool.create(
  spec,
  {
    name: 'Compose Music',
    key: 'compose_music',
  ...
  .input(z.object({
    prompt: z.string().describe('Text prompt describing the desired music (genre, mood, style, optional lyrics)'),
    musicLengthMs: z.number().min(3000).max(600000).optional().describe('Desired length of the music in milliseconds (3000-600000)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let promptPreview = ctx.input.prompt.length > 80 ? ctx.input.prompt.slice(0, 80) + '...' : ctx.input.prompt;
    let durationInfo = ctx.input.musicLengthMs ? ` (${(ctx.input.musicLengthMs / 1000).toFixed(1)}s)` : '';
