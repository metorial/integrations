...
export let sendBatchEvents = SlateTool.create(
  spec,
  {
...
Ideal for bulk imports, historical data backfills, or high-throughput event ingestion.`,
    constraints: [
      'Maximum batch size is 4MB total and 32KB per individual event.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    batch: z.array(z.object({
      type: z.enum(['identify', 'track', 'page', 'screen', 'group', 'alias']).describe('Event type'),
      ...
      properties: z.record(z.string(), z.any()).optional().describe('Event/page/screen properties'),
      ...
      integrations: z.record(z.string(), z.any()).optional().describe('Destination-specific flags'),
