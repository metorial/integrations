...
export let searchReports = SlateTool.create(
  spec,
  {
    name: 'Run GAQL Query',
    key: 'search_reports',
...
Use this tool to build custom reports, fetch performance metrics, or look up specific resources. The query follows the GAQL syntax: \`SELECT fields FROM resource WHERE conditions ORDER BY field LIMIT n\`.`,
  ...
  .input(z.object({
    ...
    pageSize: z.number().optional().describe('Maximum number of results per page (default: 10000)'),
    pageToken: z.string().optional().describe('Page token for pagination'),
  }))
  .output(z.object({
    results: z.array(z.any()).describe('Array of result rows, each containing the requested fields'),
    totalResultsCount: z.string().optional().describe('Total number of results matching the query'),
