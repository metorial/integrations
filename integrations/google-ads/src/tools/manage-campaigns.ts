...
let campaignTypeEnum = z.enum([
  'SEARCH', 'DISPLAY', 'SHOPPING', 'VIDEO', 'PERFORMANCE_MAX',
  'MULTI_CHANNEL', 'LOCAL', 'SMART', 'HOTEL', 'LOCAL_SERVICES',
...
When creating a campaign, a campaign budget is automatically created if \`dailyBudgetMicros\` is provided. For updating, only the specified fields are modified.`,
  ...
  .input(z.object({
    ...
    advertisingChannelType: campaignTypeEnum.optional().describe('Campaign type (required for create)'),
    dailyBudgetMicros: z.string().optional().describe('Daily budget in micros (e.g., "5000000" for $5.00). A budget resource is created automatically for new campaigns.'),
    existingBudgetResourceName: z.string().optional().describe('Resource name of an existing campaign budget to use instead of creating one'),
    ...
    biddingStrategyType: z.string().optional().describe('Bidding strategy type, e.g., MANUAL_CPC, MAXIMIZE_CONVERSIONS, TARGET_CPA, TARGET_ROAS'),
    targetCpaMicros: z.string().optional().describe('Target CPA in micros (for TARGET_CPA bidding)'),
    targetRoas: z.number().optional().describe('Target ROAS value (for TARGET_ROAS bidding, e.g., 3.5 for 350% ROAS)'),
    ...
    }).optional().describe('Network targeting settings'),
  }))
  .output(z.object({
    campaignResourceName: z.string().optional().describe('Resource name of the created/updated campaign'),
    budgetResourceName: z.string().optional().describe('Resource name of the created budget (if applicable)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (operation === 'remove') {
      if (!ctx.input.campaignId) throw new Error('campaignId is required for remove operation');
    ...
    if (ctx.input.name !== undefined) { updateData.name = ctx.input.name; updateMaskFields.push('name'); }
    if (ctx.input.status !== undefined) { updateData.status = ctx.input.status; updateMaskFields.push('status'); }
    if (ctx.input.startDate !== undefined) { updateData.startDate = ctx.input.startDate; updateMaskFields.push('startDate'); }
    if (ctx.input.endDate !== undefined) { updateData.endDate = ctx.input.endDate; updateMaskFields.push('endDate'); }
    if (ctx.input.networkSettings !== undefined) { updateData.networkSettings = ctx.input.networkSettings; updateMaskFields.push('networkSettings'); }
    if (ctx.input.existingBudgetResourceName !== undefined) { updateData.campaignBudget = ctx.input.existingBudgetResourceName; updateMaskFields.push('campaignBudget'); }
