...
export let manageBiddingStrategies = SlateTool.create(
  spec,
  {
...
For campaign-level bidding, use the Manage Campaigns tool instead. This tool is specifically for shared/portfolio bidding strategies.`,
  ...
  .input(z.object({
    ...
    biddingStrategyId: z.string().optional().describe('Bidding strategy ID (required for update/remove)'),
    ...
    }).optional().describe('Target CPA strategy configuration'),
    targetRoas: z.object({
      targetRoas: z.number().optional().describe('Target ROAS value (e.g., 3.5 for 350%)'),
    }).optional().describe('Target ROAS strategy configuration'),
    maximizeConversions: z.object({
      targetCpaMicros: z.string().optional().describe('Optional target CPA limit'),
    }).optional().describe('Maximize Conversions strategy configuration'),
    maximizeConversionValue: z.object({
      targetRoas: z.number().optional().describe('Optional target ROAS limit'),
    }).optional().describe('Maximize Conversion Value strategy configuration'),
    targetSpend: z.object({
      cpcBidCeilingMicros: z.string().optional().describe('Maximum CPC bid limit in micros'),
    }).optional().describe('Target Spend strategy configuration'),
  }))
  .output(z.object({
    biddingStrategyResourceName: z.string().optional().describe('Resource name of the bidding strategy'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (operation === 'create') {
      ...
      if (ctx.input.maximizeConversions) strategyData.maximizeConversions = ctx.input.maximizeConversions;
      if (ctx.input.maximizeConversionValue) strategyData.maximizeConversionValue = ctx.input.maximizeConversionValue;
      ...
      return {
        output: { biddingStrategyResourceName: result.results?.[0]?.resourceName, mutateResults: result },
    ...
    if (ctx.input.targetCpa) { updateData.targetCpa = ctx.input.targetCpa; maskFields.push('targetCpa'); }
    if (ctx.input.targetRoas) { updateData.targetRoas = ctx.input.targetRoas; maskFields.push('targetRoas'); }
    if (ctx.input.maximizeConversions) { updateData.maximizeConversions = ctx.input.maximizeConversions; maskFields.push('maximizeConversions'); }
    if (ctx.input.maximizeConversionValue) { updateData.maximizeConversionValue = ctx.input.maximizeConversionValue; maskFields.push('maximizeConversionValue'); }
    if (ctx.input.targetSpend) { updateData.targetSpend = ctx.input.targetSpend; maskFields.push('targetSpend'); }
