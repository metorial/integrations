...
Each conversion requires a Google Click ID (gclid) to link the offline event back to the original ad click.`,
  ...
  .input(z.object({
    ...
    conversions: z.array(z.object({
      ...
      conversionAction: z.string().describe('Conversion action resource name (e.g., customers/{id}/conversionActions/{id})'),
      conversionDateTime: z.string().describe('When the conversion occurred, e.g., "2024-01-15 14:30:00-05:00"'),
      conversionValue: z.number().optional().describe('Monetary value of the conversion'),
      currencyCode: z.string().optional().describe('Currency code for the conversion value (e.g., USD)'),
      orderId: z.string().optional().describe('Unique order/transaction ID for deduplication'),
    })).describe('List of conversions to upload'),
    partialFailure: z.boolean().optional().describe('If true, valid conversions are uploaded even if some fail (default: true)'),
  }))
  .output(z.object({
