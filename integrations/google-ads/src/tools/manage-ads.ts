...
export let manageAds = SlateTool.create(
  spec,
  {
    name: 'Manage Ads',
    key: 'manage_ads',
...
For responsive search ads, provide headlines and descriptions. Google will automatically test combinations. Pin headlines/descriptions to specific positions if needed.`,
  ...
  .input(z.object({
    ...
    adGroupAdResourceName: z.string().optional().describe('Full resource name of the ad group ad (required for update/remove, e.g., customers/{id}/adGroupAds/{adGroupId}~{adId})'),
    status: z.enum(['ENABLED', 'PAUSED', 'REMOVED']).optional().describe('Ad status'),
    adType: z.enum(['RESPONSIVE_SEARCH_AD', 'RESPONSIVE_DISPLAY_AD', 'EXPANDED_TEXT_AD']).optional().describe('Type of ad to create'),
    responsiveSearchAd: z.object({
      ...
      descriptions: z.array(z.object({
        ...
        pinnedField: z.string().optional().describe('Pin position, e.g., DESCRIPTION_1'),
    ...
    }).optional().describe('Responsive search ad content'),
  ...
  .output(z.object({
    adGroupAdResourceName: z.string().optional().describe('Resource name of the ad group ad'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (operation === 'remove') {
      if (!ctx.input.adGroupAdResourceName) throw new Error('adGroupAdResourceName is required for remove');
      let result = await client.mutateAdGroupAds(cid, [{ remove: ctx.input.adGroupAdResourceName }]);
      return {
        output: { adGroupAdResourceName: ctx.input.adGroupAdResourceName, mutateResults: result },
    ...
    if (operation === 'create') {
      ...
      if (ctx.input.trackingUrlTemplate) adData.ad.trackingUrlTemplate = ctx.input.trackingUrlTemplate;
      ...
      return {
        output: { adGroupAdResourceName: result.results?.[0]?.resourceName, mutateResults: result },
        message: `Ad created in ad group **${ctx.input.adGroupId}**.`,
      };
    }

    // Update
    if (!ctx.input.adGroupAdResourceName) throw new Error('adGroupAdResourceName is required for update');
