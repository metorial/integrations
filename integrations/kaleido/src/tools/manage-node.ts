...
Nodes run the blockchain protocol and can be configured with different sizes and roles (signer/non-signer).`,
  ...
  .input(z.object({
    action: z.enum(['create', 'get', 'list', 'update', 'delete', 'start', 'stop', 'suspend']).describe('Action to perform on the node'),
    consortiumId: z.string().describe('Consortium ID'),
    environmentId: z.string().describe('Environment ID'),
    nodeId: z.string().optional().describe('Node ID (required for get, update, delete, start, stop, suspend)'),
    name: z.string().optional().describe('Node name (required for create)'),
    membershipId: z.string().optional().describe('Membership ID that owns the node (required for create)'),
    role: z.enum(['signer', 'non-signer']).optional().describe('Node role in the consensus algorithm'),
    size: z.enum(['small', 'medium', 'large']).optional().describe('Node size affecting resource allocation'),
  }))
  .output(z.object({
    ...
    actionPerformed: z.string().optional().describe('Action that was performed (start, stop, suspend)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'get') {
      let result = await client.getNode(ctx.input.consortiumId, ctx.input.environmentId, ctx.input.nodeId);
      return {
        output: {
          nodeId: result._id,
          name: result.name,
    ...
    if (ctx.input.action === 'update') {
      ...
      let result = await client.updateNode(ctx.input.consortiumId, ctx.input.environmentId, ctx.input.nodeId, updateParams);
      return {
        output: {
          nodeId: result._id,
          name: result.name,
    ...
    if (ctx.input.action === 'delete') {
      await client.deleteNode(ctx.input.consortiumId, ctx.input.environmentId, ctx.input.nodeId);
      return {
        output: {
    ...
    // start, stop, suspend
    await client.performNodeAction(ctx.input.consortiumId, ctx.input.environmentId, ctx.input.nodeId, ctx.input.action);
    return {
      output: {
        nodeId: ctx.input.nodeId,
