...
export let userEvent = SlateTrigger.create(
  spec,
  {
    name: 'User Event',
    key: 'user_event',
    description: 'Triggers on user events including availability changes and email unsubscriptions.',
  }
)
  .input(z.object({
    eventType: z.string().describe('Type of user event: user_availability_updated or user_unsubscribed'),
    eventId: z.string().describe('Unique event identifier for deduplication'),
    userId: z.number().optional().describe('Drift user ID'),
    availability: z.string().optional().describe('New availability status (for availability events)'),
