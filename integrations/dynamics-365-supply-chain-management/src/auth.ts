...
let oauthScopes = [
  {
    ...
    description:
      'Access the selected Dynamics 365 Supply Chain Management environment as the signed-in user.',
...
let authServiceError = (message: string, reason = 'dynamics_supply_chain_auth_error') =>
...
let probeProfile = async (output: FinOpsAuthOutput) => {
  ...
  await requestAxiosData<unknown>(
    'Dynamics 365 Supply Chain Management profile probe',
    () =>
...
export let auth = SlateAuth.create()
  ...
  .addOauth<FinOpsOAuthInput>({
    ...
    handleCallback: async (ctx: FinOpsOAuthCallbackContext) => {
      ...
      let data = await requestToken(
        tenant,
        'Dynamics 365 Supply Chain Management OAuth token exchange',
        new URLSearchParams({
          client_id: ctx.clientId,
    ...
    handleTokenRefresh: async (ctx: FinOpsOAuthRefreshContext) => {
      if (!ctx.output.refreshToken) {
        throw authServiceError(
          'No Microsoft refresh token is available. Reconnect Dynamics 365 Supply Chain Management with offline_access enabled.',
      ...
      let data = await requestToken(
        tenant,
        'Dynamics 365 Supply Chain Management OAuth token refresh',
        new URLSearchParams({
          client_id: ctx.clientId,
          client_secret: ctx.clientSecret,
  ...
  .addCustomAuth({
    ...
    getOutput: async (ctx: FinOpsClientCredentialsContext) => {
      ...
      let data = await requestToken(
        tenant,
        'Dynamics 365 Supply Chain Management client credentials token exchange',
        new URLSearchParams({
