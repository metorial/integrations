...
let eventScoreSchema = z.object({
  ...
  scores: z.array(scoreEntrySchema).nullable().describe('Current scores for each team (null if game has not started)'),
  lastUpdate: z.string().nullable().describe('ISO timestamp of last score update'),
});

export let getScoresTool = SlateTool.create(
  spec,
  {
    name: 'Get Scores',
    key: 'get_scores',
...
Event IDs match those from the odds endpoints for easy correlation.`,
  ...
  .input(z.object({
    ...
    daysFrom: z.number().min(1).max(3).optional().describe('Include completed games from the last 1-3 days'),
