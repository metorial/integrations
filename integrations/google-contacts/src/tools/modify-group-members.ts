...
export let modifyGroupMembers = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    groupResourceName: z.string().describe('Resource name of the group (e.g., "contactGroups/abc123" or "contactGroups/starred")'),
    addContactResourceNames: z.array(z.string()).optional().describe('Contact resource names to add (e.g., ["people/c12345"])'),
    removeContactResourceNames: z.array(z.string()).optional().describe('Contact resource names to remove'),
  }))
  .output(z.object({
    notFoundResourceNames: z.array(z.string()).optional().describe('Resource names that were not found'),
    canNotRemoveLastContactGroupResourceNames: z.array(z.string()).optional().describe('Contacts that cannot be removed from their last group'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {
        ...
        canNotRemoveLastContactGroupResourceNames: result?.canNotRemoveLastContactGroupResourceNames,
      },
