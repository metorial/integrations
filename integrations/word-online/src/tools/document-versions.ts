...
export let documentVersions = SlateTool.create(
  spec,
  {
...
By default, lists all available versions. Optionally, provide a version ID to restore the document to that specific version.`,
  ...
  .input(z.object({
    ...
    restoreVersionId: z.string().optional().describe('If provided, restores the document to this version. Otherwise lists all versions.'),
  }))
  .output(z.object({
    versions: z.array(z.object({
      ...
      modifiedBy: z.string().optional().describe('Display name of the user who created this version'),
      size: z.number().optional().describe('Size of this version in bytes'),
    })).optional().describe('List of versions (returned when listing, not when restoring)'),
