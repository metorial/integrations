...
For large files, use the **Create Upload Session** tool instead.`,
    ...
    constraints: [
      'Direct upload is limited to files up to 4MB in size.',
    ],
  }
)
  .input(z.object({
    fileName: z.string().describe('Name of the file to create (e.g. "Report.docx")'),
    parentFolderId: z.string().optional().describe('ID of the parent folder. Provide either parentFolderId or folderPath.'),
    folderPath: z.string().optional().describe('Path to the target folder (e.g. "/Documents"). Provide either parentFolderId or folderPath.'),
    content: z.string().describe('The file content to upload (base64 encoded for binary files, or plain text)'),
    contentType: z.string().optional().describe('MIME type of the content. Defaults to Word document MIME type.'),
  }))
  .output(z.object({
