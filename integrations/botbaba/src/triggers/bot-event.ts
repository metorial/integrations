...
export let botEvent = SlateTrigger.create(
  spec,
  {
    name: 'Bot Event',
    key: 'bot_event',
  ...
  .input(z.object({
    eventType: z.string().describe('Type of the event (e.g., payment_success, payment_failed, chat_complete, chat_incomplete, cart_abandon)'),
  ...
  .output(z.object({
    ...
    rawPayload: z.record(z.string(), z.unknown()).describe('Complete raw event payload from Botbaba'),
  }))
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let normalizedType = String(eventType).toLowerCase().replace(/[\s-]+/g, '_');

      let eventId = (data.event_id ?? data.eventId ?? data.id ?? `${normalizedType}_${Date.now()}`) as string;
      let botId = (data.bot_id ?? data.botId) as string | undefined;
      let chatId = (data.chat_id ?? data.chatId ?? data.session_id ?? data.sessionId) as string | undefined;
    ...
    handleEvent: async (ctx) => {
      ...
      let mobileNumber = (payload.mobile ?? payload.mobileNumber ?? payload.phone ?? payload.mobile_number) as string | undefined;
      let userName = (payload.name ?? payload.userName ?? payload.user_name ?? payload.customer_name) as string | undefined;
      let email = (payload.email ?? payload.user_email ?? payload.customer_email) as string | undefined;
      let cartTotal = (payload.cart_total ?? payload.cartTotal ?? payload.amount ?? payload.total) as number | undefined;
