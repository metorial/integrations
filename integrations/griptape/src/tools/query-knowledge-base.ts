...
export let queryKnowledgeBase = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['query', 'search']).describe('Whether to query (LLM-processed) or search (raw results)'),
    knowledgeBaseId: z.string().describe('ID of the knowledge base to query'),
    query: z.string().describe('Natural language query string'),
    queryArgs: z.record(z.string(), z.any()).optional().describe('Additional query arguments (for query action)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'query') {
      results = await client.queryKnowledgeBase(ctx.input.knowledgeBaseId, ctx.input.query, ctx.input.queryArgs);
    } else {
