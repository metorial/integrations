...
export let searchEvents = SlateTool.create(
  spec,
  {
    name: 'Search Events',
    key: 'search_events',
...
**Note**: SeatGeek does not support purchasing tickets via API. Use the returned URL to direct users to SeatGeek for ticket purchases.`,
  ...
  .input(z.object({
    query: z.string().optional().describe('Full-text search query (e.g., "yankees march", "taylor swift")'),
    ...
    venueState: z.string().optional().describe('Filter by venue state (two-letter abbreviation)'),
    venueCountry: z.string().optional().describe('Filter by venue country (two-letter code)'),
    taxonomyName: z.string().optional().describe('Filter by taxonomy name (e.g., "sports", "concert", "theater")'),
    taxonomyId: z.number().optional().describe('Filter by taxonomy ID'),
    dateFrom: z.string().optional().describe('Filter events on or after this date (ISO 8601: YYYY-MM-DD or YYYY-MM-DDTHH:MM:SS)'),
    dateTo: z.string().optional().describe('Filter events on or before this date (ISO 8601: YYYY-MM-DD or YYYY-MM-DDTHH:MM:SS)'),
    ...
    range: z.string().optional().describe('Search radius for geolocation (e.g., "30mi", "50km"). Default: 30mi'),
    ...
    sort: z.enum(['datetime_local.asc', 'datetime_local.desc', 'score.asc', 'score.desc', 'announce_date.asc', 'announce_date.desc', 'id.asc', 'id.desc']).optional().describe('Sort order for results. Default: datetime_utc.asc'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.performerIds?.length) params['performers.id'] = ctx.input.performerIds.join(',');
    if (ctx.input.performerSlugs?.length) params['performers.slug'] = ctx.input.performerSlugs.join(',');
    ...
    if (ctx.input.lowestPriceMin) params['lowest_price.gte'] = String(ctx.input.lowestPriceMin);
    if (ctx.input.lowestPriceMax) params['lowest_price.lte'] = String(ctx.input.lowestPriceMax);
    if (ctx.input.averagePriceMin) params['average_price.gte'] = String(ctx.input.averagePriceMin);
    if (ctx.input.averagePriceMax) params['average_price.lte'] = String(ctx.input.averagePriceMax);
    ...
    if (events.length > 0) {
      ...
      for (let e of events.slice(0, 5)) {
        summaryParts.push(`- **${e.title}** at ${e.venue.name}, ${e.venue.city} — ${e.datetimeLocal}${e.stats.lowestPrice ? ` (from $${e.stats.lowestPrice})` : ''}`);
      }
    }
