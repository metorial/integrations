...
export let getPerformerDetails = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    images: z.record(z.string(), z.string()).describe('Performer images in multiple sizes (huge, large, medium, small, banner, etc.)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let genreStr = output.genres.length > 0 ? ` | Genres: ${output.genres.map(g => g.name).join(', ')}` : '';
    let linkStr = output.links.length > 0 ? ` | Links: ${output.links.map(l => l.provider).join(', ')}` : '';
    let divStr = output.divisions.length > 0 ? ` | ${output.divisions.map(d => `${d.displayType}: ${d.displayName}`).join(', ')}` : '';
