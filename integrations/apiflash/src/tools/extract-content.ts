...
Useful for scraping content from JavaScript-rendered pages that require a real browser.`,
  ...
  .input(z.object({
    url: z.string().describe('Target webpage URL to extract content from (must include http:// or https://)'),
    extractHtml: z.boolean().optional().describe('Extract the page HTML. Defaults to true'),
    extractText: z.boolean().optional().describe('Extract the page text content. Defaults to true'),
    waitFor: z.string().optional().describe('CSS selector to wait for before extracting content'),
