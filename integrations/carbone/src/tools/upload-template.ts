...
export let uploadTemplate = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    versioning: z.boolean().optional().describe('Enable template versioning. When enabled, a unique template ID is created along with a version ID.'),
    templateId: z.string().optional().describe('Template ID to associate this upload with (for versioning). Only used when versioning is enabled.'),
    ...
    category: z.string().optional().describe('Category or folder to group the template under.'),
    deployedAt: z.number().optional().describe('UTC Unix timestamp indicating when the template version should be considered deployed.'),
    expireAt: z.number().optional().describe('UTC Unix timestamp after which the template will be automatically deleted.'),
  }))
  .output(z.object({
