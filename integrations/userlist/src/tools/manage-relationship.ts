...
export let manageRelationship = SlateTool.create(
  spec,
  {
...
Use \`action: "delete"\` to remove the relationship, or omit/set to \`"upsert"\` to create or update it.`,
  ...
  .input(z.object({
    ...
    action: z.enum(['upsert', 'delete']).default('upsert').describe('Action to perform: "upsert" to create/update, "delete" to remove.'),
    properties: z.record(z.string(), z.unknown()).optional().describe('Custom properties for the relationship (e.g. role). Only used for upsert.'),
  }))
  .output(z.object({
