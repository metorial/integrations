...
export let bulkRunPipelineTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    runs: z.array(z.object({
      inputs: z.record(z.string(), z.unknown()).describe('Key-value pairs of input data for this run'),
      conversationId: z.string().optional().describe('Conversation ID for maintaining context'),
    })).describe('Array of run configurations, each with its own inputs'),
  }))
  .output(z.object({
    runOutputs: z.array(z.object({
      runId: z.string().describe('ID of the pipeline run'),
      outputs: z.record(z.string(), z.unknown()).describe('Key-value pairs of output data'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let runOutputs = (result.run_outputs ?? []).map((r: { run_id?: string; outputs?: Record<string, unknown> }) => ({
      runId: r.run_id ?? '',
      outputs: r.outputs ?? {},
    }));
