...
export let listPipelinesTool = SlateTool.create(
  ...
  .input(z.object({
    includeShared: z.boolean().optional().default(false).describe('Include pipelines shared with you'),
    verbose: z.boolean().optional().default(false).describe('Include full pipeline objects in the response'),
  }))
  .output(z.object({
    pipelineIds: z.array(z.string()).describe('List of pipeline IDs'),
    pipelines: z.array(z.record(z.string(), z.unknown())).optional().describe('Full pipeline objects (when verbose is true)'),
  }))
