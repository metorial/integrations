...
- **remove_background**: Remove the background from an image, leaving the subject on a transparent background.`,
  ...
  .input(z.object({
    operation: z.enum(['erase', 'inpaint', 'outpaint', 'search_and_replace', 'search_and_recolor', 'remove_background']).describe('The editing operation to perform.'),
    image: z.string().describe('Base64-encoded input image to edit.'),
    prompt: z.string().optional().describe('Text prompt describing the desired edit result. Required for inpaint, search_and_replace, and search_and_recolor.'),
    negativePrompt: z.string().optional().describe('Text describing what to avoid in the edited result.'),
    mask: z.string().optional().describe('Base64-encoded mask image. White/transparent areas indicate regions to edit. Used by erase and inpaint.'),
    searchPrompt: z.string().optional().describe('Text describing the object to find and replace. Required for search_and_replace.'),
    selectPrompt: z.string().optional().describe('Text describing the object to find and recolor. Required for search_and_recolor.'),
    left: z.number().min(0).optional().describe('Pixels to extend to the left. Used by outpaint.'),
    right: z.number().min(0).optional().describe('Pixels to extend to the right. Used by outpaint.'),
    up: z.number().min(0).optional().describe('Pixels to extend upward. Used by outpaint.'),
    down: z.number().min(0).optional().describe('Pixels to extend downward. Used by outpaint.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (input.operation) {
      ...
      case 'search_and_replace':
        if (!input.prompt) throw new Error('Prompt is required for search_and_replace operation.');
        if (!input.searchPrompt) throw new Error('searchPrompt is required for search_and_replace operation.');
      ...
      case 'search_and_recolor':
        if (!input.prompt) throw new Error('Prompt is required for search_and_recolor operation.');
        if (!input.selectPrompt) throw new Error('selectPrompt is required for search_and_recolor operation.');
