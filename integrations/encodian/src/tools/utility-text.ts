...
export let utilityText = SlateTool.create(
  spec,
  {
    name: 'Text Utilities',
    key: 'utility_text',
  ...
  .input(z.object({
    ...
    caseFormat: z.string().optional().describe('Case format (e.g., Upper, Lower, Title, Sentence)'),
    searchValue: z.string().optional().describe('Value to search for (for contains/replace)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {
        result: result.result || result.Result || result.data || result.Data || result.isValid || result,
