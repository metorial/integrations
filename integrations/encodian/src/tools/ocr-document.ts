...
export let ocrDocument = SlateTool.create(
  spec,
  {
    name: 'OCR Document',
    key: 'ocr_document',
...
Supports standard and AI-enhanced OCR modes with configurable language, quality, and clean-up options. Can process PDF documents and image files (JPG, PNG, TIFF, BMP).`,
  ...
  .input(z.object({
    sourceType: z.enum(['pdf', 'image']).describe('Whether the source is a PDF or an image file'),
    ocrMode: z.enum(['standard', 'ai']).optional().describe('OCR mode: standard or AI-enhanced (default: standard)'),
    fileContent: z.string().describe('Base64-encoded file content'),
    fileName: z.string().optional().describe('Filename with extension (required for image OCR)'),
    imageType: z.enum(['JPG', 'PNG', 'TIFF', 'BMP']).optional().describe('Image type (required for image OCR)'),
  ...
  .output(z.object({
    ...
    fileContent: z.string().optional().describe('Base64-encoded searchable PDF (for PDF OCR)'),
