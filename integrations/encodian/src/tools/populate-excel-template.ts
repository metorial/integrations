...
export let populateExcelTemplate = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    operation: z.enum(['populate', 'add_rows']).describe('Whether to populate a template or add rows to an existing workbook'),
    fileContent: z.string().describe('Base64-encoded Excel file content'),
    jsonData: z.string().describe('JSON string containing the data to populate or the rows to add'),
    calculateWorkbook: z.boolean().optional().describe('Recalculate workbook formulas after population'),
    ...
    insertionColumn: z.number().optional().describe('Column number to insert at (for add_rows)'),
    tableName: z.string().optional().describe('Target table name within the worksheet (for add_rows)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.operation === 'populate') {
      ...
      if (ctx.input.insertionColumn !== undefined) body.insertionColumn = ctx.input.insertionColumn;
