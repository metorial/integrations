...
export let barcodeRead = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    sourceType: z.enum(['document', 'image']).describe('Whether the source is a document (PDF, etc.) or an image file'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {
        scannedCodes: result.barcodes || result.Barcodes || result.qrCodes || result.QrCodes || result,
