...
let optionContractSchema = z.object({
  ticker: z.string().optional().describe('Options contract ticker (e.g., O:AAPL230616C00150000)'),
...
let optionSnapshotSchema = z.object({
  ...
  }).optional().describe('Current day OHLCV data'),
  lastQuote: z.object({
  ...
  }).optional().describe('Last quote data'),
  greeks: z.object({
  ...
  underlyingAssetPrice: z.number().optional().describe('Current price of the underlying asset'),
...
export let getOptionsChain = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'Set includeSnapshots to true to get live pricing data and Greeks.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    expirationDateFrom: z.string().optional().describe('Expiration date on or after (YYYY-MM-DD)'),
    expirationDateTo: z.string().optional().describe('Expiration date on or before (YYYY-MM-DD)'),
    ...
    includeSnapshots: z.boolean().optional().default(false).describe('Include live snapshot data with Greeks and quotes'),
    order: z.enum(['asc', 'desc']).optional().default('asc').describe('Sort order'),
    limit: z.number().optional().default(25).describe('Maximum number of contracts to return'),
  }))
  .output(z.object({
    contracts: z.array(optionContractSchema).optional().describe('Option contract reference data'),
    snapshots: z.array(optionSnapshotSchema).optional().describe('Option contract snapshots with live data'),
