...
let endpointSchema = z.object({
  ...
  currentState: z.string().optional().describe('Current state of the endpoint (init, active, idle, suspended)'),
  autoscalingLimitMinCu: z.number().optional().describe('Minimum compute units for autoscaling'),
  autoscalingLimitMaxCu: z.number().optional().describe('Maximum compute units for autoscaling'),
  suspendTimeoutSeconds: z.number().optional().describe('Seconds of inactivity before the endpoint is suspended'),
...
export let createEndpoint = SlateTool.create(
  ...
  .input(z.object({
    ...
    type: z.enum(['read_write', 'read_only']).describe('Endpoint type: read_write for primary or read_only for replicas'),
    autoscalingLimitMinCu: z.number().optional().describe('Minimum compute units (e.g., 0.25, 0.5, 1, 2, 4)'),
    autoscalingLimitMaxCu: z.number().optional().describe('Maximum compute units (e.g., 0.25, 0.5, 1, 2, 4, 8)'),
    suspendTimeoutSeconds: z.number().optional().describe('Seconds of inactivity before the endpoint suspends (0 to disable auto-suspend)'),
  }))
...
export let updateEndpoint = SlateTool.create(
  ...
  .input(z.object({
    ...
    suspendTimeoutSeconds: z.number().optional().describe('New suspend timeout in seconds (0 to disable)'),
  }))
  .output(endpointSchema)
  .handleInvocation(async (ctx) => {
    let client = new NeonClient({ token: ctx.auth.token });
...
export let controlEndpoint = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['start', 'suspend', 'restart']).describe('Action to perform on the endpoint'),
  }))
  .output(endpointSchema)
  .handleInvocation(async (ctx) => {
    let client = new NeonClient({ token: ctx.auth.token });
