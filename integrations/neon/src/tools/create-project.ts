...
export let createProject = SlateTool.create(
  ...
  .input(z.object({
    name: z.string().optional().describe('Name for the new project. Auto-generated if not specified.'),
    regionId: z.string().optional().describe('Region for the project (e.g., "aws-us-east-2", "aws-eu-central-1"). Defaults to the closest region.'),
    pgVersion: z.number().optional().describe('PostgreSQL version (e.g., 14, 15, 16, 17). Defaults to the latest supported version.'),
    orgId: z.string().optional().describe('Organization ID to create the project in. Creates in personal account if not specified.'),
  }))
  .output(z.object({
    ...
    defaultBranchId: z.string().optional().describe('ID of the default branch created with the project'),
    connectionUri: z.string().optional().describe('Connection URI for the default branch database'),
  }))
