...
export let newUnsubscribe = SlateTrigger.create(
  spec,
  {
    ...
    key: 'new_unsubscribe',
    description: 'Triggers when a contact is unsubscribed or removed from the account. Polls for recently unsubscribed contacts.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    email: z.string().describe('Contact email address'),
    unsubscriptionDate: z.string().describe('Date of unsubscription'),
    unsubscriptionType: z.string().describe('Type/reason of unsubscription (e.g. hardBounce, softBounce, manual, abuseLink, feedbackLoop)'),
  }))
  .polling({
    options: {
