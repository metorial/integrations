...
Useful for verifying customer phone numbers, enriching contact data with carrier and location information, and ensuring numbers are properly formatted.`,
  ...
  .input(z.object({
    phone: z.string().describe('The phone number to validate. Can include country prefix with leading "+" or be a local number.'),
    defaultCountry: z.string().optional().describe('Two-letter ISO country code (e.g., "US", "DE"). Used when the phone number does not include a country prefix. If omitted, the country is inferred from the request IP address.'),
  }))
  .output(z.object({
    ...
    phoneType: z.string().nullable().describe('Type of phone number: mobile, fixed_line, toll_free, premium_rate, shared_cost, voip, personal_number, pager, uan, voicemail, or unknown'),
    ...
    countryPrefix: z.string().nullable().describe('International dialing prefix for the country'),
    internationalNumber: z.string().nullable().describe('Phone number formatted in international format with leading "+"'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (result.phoneValid) {
      let parts = [`**${result.internationalNumber || result.phone}** is a valid ${result.phoneType || 'unknown type'} number`];
