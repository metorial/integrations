...
export let lookupIp = SlateTool.create(
  ...
  .input(z.object({
    ip: z.string().optional().describe('IPv4 or IPv6 address to look up. Omit to look up the caller\'s own IP address.')
  }))
  .output(z.object({
    ...
    org: z.string().optional().describe('Organization/ISP string (e.g. "AS15169 Google LLC")'),
    postal: z.string().optional().describe('Postal/ZIP code'),
    timezone: z.string().optional().describe('Timezone identifier (e.g. America/Los_Angeles)'),
    geo: z.object({
    ...
    }).optional().describe('Detailed geolocation object (Core tier and above)'),
    asn: z.object({
    ...
    }).optional().describe('ASN information (Core tier and above)'),
    company: z.object({
      name: z.string().optional(),
      domain: z.string().optional(),
      type: z.string().optional()
    }).optional().describe('Company information (Plus/Enterprise tier)'),
    carrier: z.object({
      name: z.string().optional(),
      mcc: z.string().optional(),
      mnc: z.string().optional()
    }).optional().describe('Mobile carrier information (Plus/Enterprise tier)'),
    privacy: z.object({
    ...
    }).optional().describe('Privacy/anonymity detection (Core tier and above)'),
    abuse: z.object({
    ...
    }).optional().describe('Abuse contact information'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (result.city || result.geo?.city) locationParts.push(result.city || result.geo?.city || '');
    if (result.region || result.geo?.region) locationParts.push(result.region || result.geo?.region || '');
    if (result.country || result.geo?.country_code) locationParts.push(result.country || result.geo?.country_code || '');
