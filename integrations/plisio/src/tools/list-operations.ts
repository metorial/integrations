...
export let listOperations = SlateTool.create(
  ...
  .input(z.object({
    ...
    type: z.enum(['cash_in', 'cash_out', 'mass_cash_out', 'invoice']).optional().describe('Filter by operation type'),
    status: z.enum(['new', 'pending', 'pending internal', 'expired', 'completed', 'mismatch', 'error', 'cancelled']).optional().describe('Filter by operation status'),
    currency: z.string().optional().describe('Filter by cryptocurrency ID (e.g. BTC, ETH)'),
    search: z.string().optional().describe('Search by transaction ID, order number, or email'),
  }))
  .output(z.object({
