...
export let getCityAirQuality = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    pollution: z.object({
      timestamp: z.string().describe('Measurement timestamp (ISO 8601)'),
      aqiUs: z.number().describe('AQI value based on US EPA standard'),
      mainPollutantUs: z.string().describe('Main pollutant for US AQI (p2=PM2.5, p1=PM10, o3=Ozone, n2=NO2, s2=SO2, co=CO)'),
      aqiChina: z.number().describe('AQI value based on China MEP standard'),
