...
export let manageEnvironment = SlateTool.create(
  ...
  .input(z.object({
    ...
    environmentId: z.number().optional().describe('Deployment environment ID (required for get, delete)'),
    environmentName: z.string().optional().describe('Environment name (required for create)'),
    provider: z.string().optional().describe('Deployment provider type (required for create)'),
    providerSettings: z.record(z.string(), z.unknown()).optional().describe('Provider-specific settings'),
    ...
    environmentSettings: z.record(z.string(), z.unknown()).optional().describe('Full environment settings for update'),
  }))
  .output(z.object({
    environments: z.array(z.record(z.string(), z.unknown())).optional().describe('List of environments'),
    environment: z.record(z.string(), z.unknown()).optional().describe('Environment details'),
