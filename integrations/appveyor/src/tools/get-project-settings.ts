...
export let getProjectSettings = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    format: z.enum(['json', 'yaml', 'envVars']).default('json').describe('Output format for settings'),
  }))
  .output(z.object({
    settings: z.record(z.string(), z.unknown()).optional().describe('Project settings as JSON'),
    yamlConfig: z.string().optional().describe('Project settings as YAML'),
    environmentVariables: z.array(z.record(z.string(), z.unknown())).optional().describe('Project environment variables'),
  }))
