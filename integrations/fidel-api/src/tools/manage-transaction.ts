...
let transactionSchema = z.object({
  ...
  locationId: z.string().optional().describe('ID of the location where the transaction occurred'),
  ...
  updated: z.string().optional().describe('ISO 8601 date when the transaction was last updated'),
  datetime: z.string().optional().describe('ISO 8601 date of the transaction'),
  offer: z.record(z.string(), z.any()).optional().nullable().describe('Offer qualification details if applicable'),
});
...
export let listTransactions = SlateTool.create(
  ...
  .input(z.object({
    ...
    cardId: z.string().optional().describe('Optional card ID to filter transactions for a specific card'),
...
export let createTestTransaction = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'This endpoint only works with test mode API keys (sk_test_*).',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    ...
    locationId: z.string().describe('ID of the location where the simulated transaction occurs'),
