...
export let issueEventsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Issue Events',
    key: 'issue_events',
    description: 'Triggers when issues (including service requests) are created, updated, or deleted. Includes changelog for updates.',
  }
)
  .input(z.object({
    ...
    })).optional().describe('Changed fields (for updates)'),
    userAccountId: z.string().optional().describe('Account ID of the user who triggered the event'),
    userName: z.string().optional().describe('Display name of the user who triggered the event'),
  }))
  .output(z.object({
    issueId: z.string().describe('Issue ID'),
    issueKey: z.string().describe('Issue key'),
