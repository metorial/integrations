...
export let listRequestTypesTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    includeFields: z.boolean().optional().describe('Whether to include field definitions for each request type'),
  ...
  .output(z.object({
    ...
    requestTypes: z.array(z.object({
      ...
      groupIds: z.array(z.string()).optional().describe('Group IDs the request type belongs to'),
      fields: z.array(z.object({
        fieldId: z.string().describe('Field ID'),
        name: z.string().describe('Field name'),
        required: z.boolean().describe('Whether the field is required'),
        description: z.string().optional().describe('Field description'),
      })).optional().describe('Field definitions for this request type'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.getRequestTypes(ctx.input.serviceDeskId, ctx.input.start, ctx.input.limit);

    let requestTypes = await Promise.all((result.values || []).map(async (rt: any) => {
      let requestType: any = {
      ...
      if (ctx.input.includeFields) {
        try {
          let fieldsResult = await client.getRequestTypeFields(ctx.input.serviceDeskId, String(rt.id));
