...
let embedSchema = z.object({
  ...
  color: z.number().optional().describe('Color code of the embed (integer, e.g. 0x00FF00 for green)'),
  ...
  fields: z.array(embedFieldSchema).optional().describe('Array of embed field objects (max 25)'),
}).describe('Discord embed object');

let messageReferenceSchema = z.object({
...
export let sendMessage = SlateTool.create(
  ...
  .input(z.object({
    ...
    content: z.string().optional().describe('Text content of the message (up to 2000 characters)'),
    embeds: z.array(embedSchema).optional().describe('Array of embed objects to include in the message (max 10)'),
    messageReference: messageReferenceSchema.optional().describe('Reference to a message to reply to'),
