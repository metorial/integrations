...
export let scanEvents = SlateTrigger.create(
  spec,
  {
    name: 'Scan Events',
    key: 'scan_events',
    description: 'Triggers when a pass is scanned with the Passcreator Smart Scan companion app or via the API.',
  }
)
  .input(z.object({
    appScanId: z.string().describe('Unique identifier of the scan'),
    scanStatus: z.number().describe('Scan status code (0=voided, 1=already voided, 2=attendance, 3=not found, 4=check-in, 5=already checked in, 6=check-out, 7=not checked in)'),
    ...
    appConfigurationId: z.string().optional().describe('App configuration used for the scan'),
    ...
    additionalPropertiesScan: z.record(z.string(), z.any()).optional().describe('Additional scan properties'),
    additionalPropertiesPass: z.record(z.string(), z.any()).optional().describe('Additional pass properties'),
  }))
  .output(z.object({
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      return {
        ...
        id: ctx.input.appScanId || `scan-${ctx.input.passId}-${ctx.input.createdOn || Date.now()}`,
        output: {
