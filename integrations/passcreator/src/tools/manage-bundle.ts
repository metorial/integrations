...
export let manageBundle = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'A bundle can contain a maximum of 10 passes.',
    ],
    tags: {
      destructive: false,
    },
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'get', 'void', 'unvoid', 'delete']).describe('Action to perform on the bundle'),
    bundleId: z.string().optional().describe('Bundle identifier (required for update, get, void, unvoid, delete)'),
    passIds: z.array(z.string()).optional().describe('Array of pass identifiers (required for create and update, max 10)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'get': {
        if (!ctx.input.bundleId) throw new Error('bundleId is required for retrieving a bundle');
      ...
      case 'void':
      case 'unvoid': {
        if (!ctx.input.bundleId) throw new Error('bundleId is required for voiding/unvoiding a bundle');
