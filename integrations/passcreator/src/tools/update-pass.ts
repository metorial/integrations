...
export let updatePass = SlateTool.create(
  ...
  .input(z.object({
    ...
    voided: z.boolean().optional().describe('Set to true to void (invalidate) the pass, false to unvoid'),
    expirationDate: z.string().optional().describe('New expiration date in "Y-m-d H:i" format'),
    relevantDate: z.string().optional().describe('New relevant date in "Y-m-d H:i" format'),
    storedValue: z.number().optional().describe('Updated stored value (e.g., loyalty balance)'),
    ...
    })).optional().describe('Updated location triggers'),
    customFields: z.record(z.string(), z.any()).optional().describe('Updated custom field values keyed by field key'),
    targetTemplateId: z.string().optional().describe('Move the pass to a different template (must be in the same project)'),
  }))
  .output(z.object({
    passId: z.string().describe('Identifier of the updated pass'),
    moved: z.boolean().optional().describe('Whether the pass was moved to a different template'),
  }))
