...
export let bulkUpdatePasses = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    updates: z.record(z.string(), z.any()).describe('Field values to apply to all matching passes'),
    identifiers: z.array(z.string()).optional().describe('Specific pass identifiers to update'),
    ...
    query: z.object({
      ...
      }))).optional().describe('Filter condition groups'),
    }).optional().describe('Query language filter'),
    fullReplace: z.boolean().optional().default(false).describe('Use full replace (POST) instead of partial update (PATCH)'),
  }))
  .output(z.object({
    processUrl: z.string().optional().describe('URL to check the status of the async bulk operation'),
