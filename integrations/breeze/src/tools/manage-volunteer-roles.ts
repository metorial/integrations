...
export let manageVolunteerRoles = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    quantity: z.number().optional().describe('Number of volunteers needed for this role (for create)'),
  ...
  .output(z.object({
    ...
    success: z.boolean().optional().describe('Whether the operation succeeded (for remove action)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'list': {
        let roles = await client.listVolunteerRoles(ctx.input.instanceId, ctx.input.showQuantity);
      ...
      case 'create': {
        ...
        let role = await client.addVolunteerRole(ctx.input.instanceId, ctx.input.name, ctx.input.quantity);
        return {
          output: { role },
