...
export let listForms = SlateTool.create(
  ...
  .input(z.object({
    isArchived: z.boolean().optional().describe('Filter by archived status. True for archived forms, false for active forms, omit for all.'),
    formId: z.string().optional().describe('If provided, also retrieves the field definitions for this specific form'),
  }))
  .output(z.object({
    forms: z.array(z.any()).describe('Array of form objects'),
    fields: z.array(z.any()).optional().describe('Form field definitions (when formId is provided)'),
  }))
