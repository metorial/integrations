...
export let searchPlaces = SlateTool.create(
  ...
  .input(z.object({
    ...
    categories: z.string().optional().describe('Comma-separated Foursquare category IDs to filter by'),
    ...
    openAt: z.string().optional().describe('ISO 8601 datetime to check if place is open at a specific time'),
    ...
    ne: z.string().optional().describe('Northeast corner of bounding box (latitude,longitude)'),
    sw: z.string().optional().describe('Southwest corner of bounding box (latitude,longitude)'),
  }))
  .output(z.object({
