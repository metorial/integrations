...
export let getRecruiting = SlateTool.create(
  spec,
  {
    name: 'Get Recruiting',
    key: 'get_recruiting',
  ...
  .input(z.object({
    ...
    classification: z.enum(['HighSchool', 'JUCO', 'PrepSchool']).optional().describe('Recruit classification type'),
    includeTeamRankings: z.boolean().optional().default(true).describe('Include team recruiting class rankings'),
    includePlayerRankings: z.boolean().optional().default(true).describe('Include individual recruit player rankings'),
    includePositionGroups: z.boolean().optional().default(false).describe('Include aggregated positional group recruiting data'),
  }))
  .output(z.object({
    recruits: z.array(z.any()).optional().describe('Individual recruit ratings and rankings'),
    teamRankings: z.array(z.any()).optional().describe('Team recruiting class rankings'),
    positionGroups: z.array(z.any()).optional().describe('Aggregated positional group recruiting data'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let parts: string[] = [];
    if (results.recruits) parts.push(`${Array.isArray(results.recruits) ? results.recruits.length : 0} recruit(s)`);
