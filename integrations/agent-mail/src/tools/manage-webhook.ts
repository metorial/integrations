...
Available event types: \`message.received\`, \`message.sent\`, \`message.delivered\`, \`message.bounced\`, \`message.complained\`, \`message.rejected\`, \`domain.verified\`.`,
  ...
  .input(z.object({
    ...
    inboxIds: z.array(z.string()).optional().describe('Scope to specific inbox IDs (max 10)'),
  ...
  .output(z.object({
    ...
    secret: z.string().optional().describe('Webhook secret for signature verification (only on create)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      if (!ctx.input.eventTypes?.length) throw new Error('eventTypes is required for create action');
