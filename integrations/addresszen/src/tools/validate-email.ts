...
export let validateEmail = SlateTool.create(
  ...
  .output(z.object({
    ...
    disposable: z.boolean().optional().describe('Whether the email uses a disposable email provider'),
    reachable: z.string().optional().describe('Email reachability status'),
    roleAccount: z.boolean().optional().describe('Whether this is a role-based email address (e.g., admin@, info@)'),
    freeProvider: z.boolean().optional().describe('Whether the email uses a free email provider (e.g., Gmail, Yahoo)'),
