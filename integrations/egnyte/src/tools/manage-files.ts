...
export let createFolderTool = SlateTool.create(
  ...
  .input(z.object({
    folderPath: z.string().describe('Full path for the new folder (e.g. "/Shared/Projects/NewProject")'),
  }))
  .output(z.object({
...
export let copyItemTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    permissions: z.enum(['inherit_from_parent', 'keep_original']).optional().describe('How to handle permissions for the copy'),
  }))
  .output(z.object({
    sourcePath: z.string(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    await client.copyFileOrFolder(ctx.input.sourcePath, ctx.input.destinationPath, ctx.input.permissions);
...
export let moveItemTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    sourcePath: z.string().describe('Path of the source file or folder'),
    destinationPath: z.string().describe('Destination path including the new name'),
    permissions: z.enum(['inherit_from_parent', 'keep_original']).optional().describe('How to handle permissions after move'),
  }))
  .output(z.object({
    sourcePath: z.string(),
    destinationPath: z.string(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    await client.moveFileOrFolder(ctx.input.sourcePath, ctx.input.destinationPath, ctx.input.permissions);
...
export let deleteItemTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    entryId: z.string().optional().describe('Specific file version entry ID to delete (deletes only that version)'),
  }))
  .output(z.object({
    path: z.string(),
    deleted: z.boolean(),
...
export let lockFileTool = SlateTool.create(
  spec,
  {
