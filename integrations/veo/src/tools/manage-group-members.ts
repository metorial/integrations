...
export let manageGroupMembers = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'add']).describe('Action to perform: "list" to get members, "add" to add a member'),
    userId: z.string().optional().describe('ID of the user to add (required when action is "add")'),
    admin: z.boolean().optional().default(false).describe('Whether to grant group admin privileges (only for "add" action)'),
  }))
  .output(z.object({
    members: z.array(z.record(z.string(), z.any())).optional().describe('List of group members (when action is "list")'),
    addedUserId: z.string().optional().describe('ID of the user added (when action is "add")'),
  }))
