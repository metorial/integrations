...
export let updateLink = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Links belonging to published issues cannot be updated.',
    ],
    tags: {
      destructive: false,
    },
  ...
  .input(z.object({
    ...
    issueId: z.string().optional().describe('Draft issue ID to move the link to, or empty string to move back to collected items'),
  }))
  .output(z.object({
