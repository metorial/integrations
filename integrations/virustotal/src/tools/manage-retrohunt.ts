...
export let manageRetrohunt = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['create', 'get', 'list', 'get_matches']).describe('Operation to perform'),
    ...
    notificationEmail: z.string().optional().describe('Email for completion notification (for create)'),
  ...
  .output(z.object({
    ...
    }).optional().describe('Retrohunt job details'),
    jobs: z.array(z.object({
      jobId: z.string().describe('Retrohunt job ID'),
      status: z.string().optional().describe('Job status'),
      numMatchingFiles: z.number().optional().describe('Number of matching files'),
      progress: z.number().optional().describe('Job progress percentage')
    })).optional().describe('List of Retrohunt jobs'),
    matchingFiles: z.array(z.object({
    ...
    })).optional().describe('Matching files for a job'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'create': {
        ...
        let result = await client.createRetrohuntJob(ctx.input.rules, ctx.input.corpus, ctx.input.notificationEmail);
      ...
      case 'get_matches': {
        ...
        let result = await client.getRetrohuntJobMatchingFiles(ctx.input.jobId, ctx.input.limit, ctx.input.cursor);
