...
export let getCreatorInfo = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({}))
  .output(z.object({
    creatorAvatarUrl: z.string().optional().describe('Creator avatar URL (expires in ~2 hours).'),
    ...
    privacyLevelOptions: z.array(z.string()).optional().describe('Available privacy levels for posting (e.g. PUBLIC_TO_EVERYONE, SELF_ONLY).'),
    commentDisabled: z.boolean().optional().describe('Whether comments are disabled for this creator.'),
    duetDisabled: z.boolean().optional().describe('Whether duets are disabled for this creator.'),
    stitchDisabled: z.boolean().optional().describe('Whether stitches are disabled for this creator.'),
    maxVideoPostDurationSec: z.number().optional().describe('Maximum allowed video duration in seconds.'),
  }))
