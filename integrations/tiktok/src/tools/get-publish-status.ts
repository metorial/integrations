...
export let getPublishStatus = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    publishId: z.string().describe('The publish ID returned from a video or photo post initialization.'),
  }))
  .output(z.object({
    ...
    postIds: z.array(z.string()).optional().describe('Public post IDs once the content is published and moderated.'),
  }))
