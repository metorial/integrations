...
export let createTimeEntry = SlateTool.create(
  ...
  .input(z.object({
    ...
    stop: z.string().optional().describe('Stop time in ISO 8601 format. Required for completed entries.'),
    ...
    billable: z.boolean().optional().describe('Whether the time entry is billable (requires paid plan)'),
  }))
  .output(z.object({
