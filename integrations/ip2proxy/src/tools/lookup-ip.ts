...
The amount of detail returned depends on the package tier configured in the integration settings (PX1 through PX11).`,
  ...
  .input(z.object({
    ipAddress: z.string().optional().describe('The IPv4 or IPv6 address to look up. If omitted, the server\'s own IP is queried.'),
    package: z.enum([
      'PX1', 'PX2', 'PX3', 'PX4', 'PX5',
      'PX6', 'PX7', 'PX8', 'PX9', 'PX10', 'PX11'
    ]).optional().describe('Override the default package tier for this query. Controls which fields are returned and credits consumed.')
  }))
  .output(z.object({
    response: z.string().describe('Response status from the API (e.g., "OK" for success).'),
    isProxy: z.string().describe('Whether the IP is a proxy: "YES", "NO", or "-" if not applicable.'),
    proxyType: z.string().optional().describe('Type of proxy detected: VPN, TOR, DCH, PUB, WEB, SES, RES, CPN, EPN, or "-" if not a proxy.'),
    ...
    usageType: z.string().optional().describe('Usage type classification (e.g., ISP, COM, ORG, GOV, MIL, EDU, LIB, CDN, DCH, SES, RSV).'),
    asn: z.string().optional().describe('Autonomous System Number.'),
    asName: z.string().optional().describe('Autonomous System name.'),
    lastSeen: z.string().optional().describe('Number of days since the proxy was last seen.'),
    threat: z.string().optional().describe('Threat classification (e.g., SPAM, SCANNER, BOTNET, or "-").'),
    vpnProvider: z.string().optional().describe('Name of the VPN provider, if identified (PX11 only).')
  }))
  .handleInvocation(async (ctx) => {
    ...
    let proxyDetail = result.proxyType && result.proxyType !== '-'
      ? ` (type: **${result.proxyType}**)`
      : '';
    let locationParts = [result.cityName, result.regionName, result.countryName].filter(p => p && p !== '-');
