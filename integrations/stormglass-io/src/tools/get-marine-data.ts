...
export let getMarineDataTool = SlateTool.create(
  spec,
  {
...
Ideal for ocean and coastal applications such as surfing, fishing, shipping, and offshore operations.`,
  ...
  .input(z.object({
    ...
    parameters: z.array(z.string()).describe('Marine parameters to retrieve (e.g., "waveHeight", "wavePeriod", "waveDirection", "swellHeight", "swellDirection", "swellPeriod", "secondarySwellHeight", "secondarySwellDirection", "secondarySwellPeriod", "windWaveHeight", "windWaveDirection", "windWavePeriod", "waterTemperature", "currentSpeed", "currentDirection", "seaLevel", "iceCover", "seaFloorDepth")'),
    ...
    sources: z.array(z.string()).optional().describe('Data sources to query (e.g., "sg", "noaa", "metno", "dwd"). Defaults to all sources.'),
  }))
  .output(z.object({
    hours: z.array(z.object({
      time: z.string().describe('UTC timestamp for this data point'),
    }).catchall(sourceValueSchema)).describe('Hourly marine data points'),
