...
Use this to enrich company records with real-time intelligence and buying signals from 25+ signal categories.`,
  ...
  .input(z.object({
    ...
    signalTypes: z.array(z.string()).optional().describe('Filter by specific signal types (e.g., "news", "earnings-transcript", "8k", "10k")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let signals = Array.isArray(result?.signals) ? result.signals : Array.isArray(result) ? result : [];
