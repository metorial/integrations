...
export let updateSheet = SlateTool.create(
  spec,
  {
    name: 'Update Sheet',
    key: 'update_sheet',
  ...
  .input(z.object({
    ...
    action: z.enum(['rename', 'copy', 'move']).optional().describe('Action to perform: rename (default), copy, or move'),
    destinationType: z.enum(['home', 'folder', 'workspace']).optional().describe('Destination type for copy/move'),
    destinationId: z.string().optional().describe('Destination folder or workspace ID for copy/move'),
