...
export let getBrandMentions = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    platform: z.enum(['youtube', 'tiktok']).describe('Social media platform (only YouTube and TikTok support brand mentions)'),
    channel: z.string().describe('Channel name or ID'),
  }))
  .output(z.object({
    mentions: z.any().optional().describe('Brand mention performance data across time periods'),
