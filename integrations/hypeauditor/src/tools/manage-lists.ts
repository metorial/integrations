...
export let manageLists = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['get_all', 'get_reports', 'add', 'remove']).describe('Operation to perform'),
    listId: z.number().optional().describe('List ID (required for get_reports, add, and remove)'),
    channelId: z.string().optional().describe('Channel/user ID of the influencer to add'),
    socialNetwork: z.enum(['instagram', 'youtube', 'tiktok', 'twitter', 'twitch', 'snapchat']).optional().describe('Social network of the influencer (required for add)'),
    reportId: z.number().optional().describe('Report ID to remove (required for remove action)'),
  ...
  .output(z.object({
    ...
    reports: z.array(z.any()).optional().describe('Array of influencer reports in the list (for get_reports)'),
