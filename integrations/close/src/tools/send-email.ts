...
export let sendEmail = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['draft', 'outbox', 'sent']).optional().describe('Email status: "draft" to save, "outbox" to queue for sending (default), "sent" to log as already sent'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let toAddresses = (result.to || []).map((r: any) => typeof r === 'string' ? r : r.email || r);
    let ccAddresses = (result.cc || []).map((r: any) => typeof r === 'string' ? r : r.email || r);
    let bccAddresses = (result.bcc || []).map((r: any) => typeof r === 'string' ? r : r.email || r);
    let senderAddress = result.sender
      ? (typeof result.sender === 'string' ? result.sender : result.sender.email || result.sender)
      : undefined;
