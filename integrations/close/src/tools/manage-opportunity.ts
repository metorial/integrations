...
When updating: provide the opportunityId along with any fields to change.`,
  ...
  .input(z.object({
    opportunityId: z.string().optional().describe('Opportunity ID to update. Omit to create a new opportunity.'),
    leadId: z.string().optional().describe('Lead ID to associate the opportunity with (required when creating)'),
    statusId: z.string().optional().describe('Status ID for the opportunity'),
    confidence: z.number().min(0).max(100).optional().describe('Confidence percentage (0-100)'),
    value: z.number().optional().describe('Monetary value in cents'),
    valuePeriod: z.enum(['one_time', 'monthly', 'annual']).optional().describe('Value period for recurring deals'),
    ...
    dateWon: z.string().optional().describe('Date the opportunity was won (ISO 8601 date string)'),
