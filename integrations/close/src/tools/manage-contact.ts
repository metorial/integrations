...
When creating: provide leadId and at least a name. When updating: provide contactId along with any fields to change.`,
  ...
  .input(z.object({
    ...
    leadId: z.string().optional().describe('Lead ID to associate the contact with (required when creating)'),
    ...
    })).optional().describe('Phone numbers for the contact'),
    urls: z.array(z.object({
      url: z.string().describe('URL'),
      type: z.string().describe('URL type (e.g., "url")'),
    })).optional().describe('URLs for the contact'),
    customFields: z.record(z.string(), z.any()).optional().describe('Custom field values keyed by custom field ID (e.g., "custom.cf_xxx")'),
  }))
  .output(z.object({
