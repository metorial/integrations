...
export let processImage = SlateTool.create(
  spec,
  {
    name: 'Process Image',
    key: 'process_image',
  ...
  .input(z.object({
    ...
    lossy: z.boolean().optional().describe('Whether to use lossy compression (smaller file, slight quality loss). Defaults to lossless.'),
    quality: z.number().optional().describe('Quality level from 1 (most compressed) to 100 (highest quality). Only applies with lossy compression.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let output = {
      ...
      compressedSize: result.compressed_size != null ? Number(result.compressed_size) : undefined,
      savedPercentage: result.saved_percentage != null ? Number(result.saved_percentage) : undefined,
    };
