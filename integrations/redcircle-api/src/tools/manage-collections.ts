...
export let manageCollections = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'update', 'start', 'delete']).describe('Action to perform on collections.'),
    collectionId: z.string().optional().describe('Collection ID. Required for get, update, start, and delete actions.'),

    // List filters
    searchTerm: z.string().optional().describe('Filter collections by name (list action).'),
    status: z.enum(['all', 'idle', 'queued', 'running']).optional().describe('Filter by collection status (list action).'),
    sortBy: z.enum(['created_at', 'last_run', 'name', 'priority', 'status']).optional().describe('Sort collections (list action).'),
    sortDirection: z.enum(['ascending', 'descending']).optional().describe('Sort direction (list action).'),
    ...
    scheduleType: z.enum(['daily', 'weekly', 'monthly', 'manual', 'minutes']).optional().describe('Schedule type (create/update).'),
    scheduleHours: z.array(z.number()).optional().describe('Hours of day to run [0-23] (create/update).'),
    scheduleDaysOfWeek: z.array(z.number()).optional().describe('Days of week [0=Sunday, 6=Saturday] (create/update, weekly schedule).'),
    scheduleDaysOfMonth: z.array(z.number()).optional().describe('Days of month [1-31] (create/update, monthly schedule).'),
    scheduleMinutes: z.string().optional().describe('Minutes interval, e.g. "every_5_minutes", "every_hour" (create/update, minutes schedule).'),
    enabled: z.boolean().optional().describe('Whether the collection is enabled for scheduled runs (create/update).'),
    priority: z.enum(['highest', 'high', 'normal', 'low', 'lowest']).optional().describe('Collection priority (create/update).'),
    requestsType: z.enum(['mixed', 'product', 'reviews', 'search', 'category', 'store_stock']).optional().describe('Type of requests in this collection (create/update).'),
    notificationEmail: z.string().optional().describe('Email for completion notifications (create/update).'),
    notificationWebhook: z.string().optional().describe('Webhook URL for completion notifications (create/update).'),
    notificationAsJson: z.boolean().optional().describe('Include JSON download links in webhook body (create/update).'),
    notificationAsCsv: z.boolean().optional().describe('Include CSV download links in webhook body (create/update).'),
    destinationIds: z.array(z.string()).optional().describe('Cloud storage destination IDs for auto-upload (create/update).'),
  }))
  .output(z.object({
    collection: z.any().optional().describe('Collection details (for get, create, update, start actions).'),
    collections: z.array(z.any()).optional().describe('Array of collections (for list action).'),
    collectionsCount: z.number().optional().describe('Total number of collections found (list action).'),
    message: z.string().optional().describe('Confirmation message for start/delete actions.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      if (ctx.input.scheduleDaysOfWeek) body.schedule_days_of_week = ctx.input.scheduleDaysOfWeek;
      if (ctx.input.scheduleDaysOfMonth) body.schedule_days_of_month = ctx.input.scheduleDaysOfMonth;
      ...
      if (ctx.input.notificationWebhook) body.notification_webhook = ctx.input.notificationWebhook;
      if (ctx.input.notificationAsJson !== undefined) body.notification_as_json = ctx.input.notificationAsJson;
      if (ctx.input.notificationAsCsv !== undefined) body.notification_as_csv = ctx.input.notificationAsCsv;
