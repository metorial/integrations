...
export let manageZipcodes = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list', 'add', 'delete']).describe('Action to perform: list, add, or delete zipcodes.'),
    zipcodes: z.array(z.string()).optional().describe('Array of US zipcodes to add or delete. Required for add/delete actions.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'list') {
      ...
      let totalZipcodes = Object.values(data.zipcodes ?? {}).reduce((sum: number, arr: any) => sum + (Array.isArray(arr) ? arr.length : 0), 0);
    ...
    if (ctx.input.action === 'add') {
      let zipcodeEntries = (ctx.input.zipcodes ?? []).map(z => ({ zipcode: z, domain: 'target.com' }));
