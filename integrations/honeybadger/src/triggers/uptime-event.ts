...
export let uptimeEvent = SlateTrigger.create(
  spec,
  {
    name: 'Uptime Event',
    key: 'uptime_event',
    description: 'Triggers when an uptime event occurs — site goes down, comes back up, or an SSL certificate is about to expire.',
  },
)
  .input(z.object({
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      let { eventType, siteId, siteName, siteUrl, projectId, projectName, outageDownAt, outageUpAt, outageStatus, outageReason } = ctx.input;
