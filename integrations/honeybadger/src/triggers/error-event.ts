...
export let errorEvent = SlateTrigger.create(
  spec,
  {
    name: 'Error Event',
    key: 'error_event',
    description: 'Triggers when an error event occurs in Honeybadger — including new errors, resolved, unresolved, assigned, commented, or rate exceeded events.',
  },
)
  .input(z.object({
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      let { eventType, faultId, projectId, projectName, faultUrl, faultKlass, faultMessage, faultComponent, faultAction, faultEnvironment, faultAssignee, noticeMessage, noticeUrl, noticeCreatedAt, commentBody, commentAuthor } = ctx.input;
