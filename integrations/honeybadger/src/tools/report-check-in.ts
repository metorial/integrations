...
export let reportCheckIn = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    if (!ctx.auth.projectToken) {
      throw new Error('A project API key is required to report check-ins. Configure it in your authentication settings.');
    }
