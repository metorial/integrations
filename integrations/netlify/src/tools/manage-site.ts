...
export let createSite = SlateTool.create(
  ...
  .input(z.object({
    ...
    accountSlug: z.string().optional().describe('Account/team slug to create the site under'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.functionsDirectory) buildSettings.functions_dir = ctx.input.functionsDirectory;
...
export let updateSite = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.publishDirectory !== undefined) buildSettings.dir = ctx.input.publishDirectory;
    if (ctx.input.functionsDirectory !== undefined) buildSettings.functions_dir = ctx.input.functionsDirectory;
