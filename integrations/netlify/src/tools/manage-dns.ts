...
export let manageDnsRecords = SlateTool.create(
  ...
  .input(z.object({
    ...
    recordType: z.string().optional().describe('DNS record type, e.g., A, AAAA, CNAME, MX, TXT (required for create)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('DNS records (returned for list action)'),
    createdRecord: z.object({
      recordId: z.string().describe('Created record ID'),
      hostname: z.string().describe('Record hostname'),
      recordType: z.string().describe('Record type'),
      value: z.string().describe('Record value'),
    }).optional().describe('Created DNS record (returned for create action)'),
    deleted: z.boolean().optional().describe('Whether the record was deleted (returned for delete action)'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
    ...
    switch (ctx.input.action) {
      ...
      case 'create': {
        if (!ctx.input.recordType || !ctx.input.hostname || !ctx.input.value) {
          throw new Error('recordType, hostname, and value are required for creating a DNS record');
        }
...
export let manageDnsZone = SlateTool.create(
  ...
  .output(z.object({
    zone: z.object({
      zoneId: z.string().describe('DNS zone identifier'),
      name: z.string().describe('Zone domain name'),
    }).optional().describe('Created zone (returned for create action)'),
