...
export let domainAvailability = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    domain: z.string().optional().describe('Single domain name to check availability for (e.g. "example.com")'),
    domainNames: z.array(z.string()).optional().describe('Array of domain names for bulk availability check (max 100)'),
    suggest: z.boolean().optional().describe('Whether to return alternative domain suggestions (single domain only)'),
    suggestionCount: z.number().optional().describe('Number of suggestions to return (default 5, max 100)'),
  }))
  .output(z.object({
