...
export let domainWhoisLookup = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Bulk lookups support up to 100 domains per request.',
    ],
    tags: {
      destructive: false,
      readOnly: true,
  ...
  .input(z.object({
    domainName: z.string().optional().describe('Single domain name to look up (e.g. "example.com")'),
    domainNames: z.array(z.string()).optional().describe('Array of domain names for bulk lookup (max 100)'),
  }))
  .output(z.object({
