...
export let getDataQuality = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    maxNoValuePagesPercent: thresholdSchema.describe('Maximum no-value pages percentage check'),
    minColumnRecords: z.record(z.string(), thresholdSchema).optional().describe('Per-column fill rate checks'),
  }))
