...
export let detectWeb = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    maxResults: z.number().optional().describe('Maximum number of web entities to return (default: 10)'),
    includeGeoResults: z.boolean().optional().describe('Include results derived from geo information in the image'),
  }))
  .output(z.object({
    ...
    matchingPages: z.array(z.object({
    ...
    fullMatchingImageUrls: z.array(z.string()).describe('URLs of full-matching images found on the web'),
    partialMatchingImageUrls: z.array(z.string()).describe('URLs of partially matching images found on the web'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: bestGuessLabels.length > 0
