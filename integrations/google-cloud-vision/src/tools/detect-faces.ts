...
export let detectFaces = SlateTool.create(
  spec,
  {
    name: 'Detect Faces',
    key: 'detect_faces',
  ...
  .input(z.object({
    ...
    maxResults: z.number().optional().describe('Maximum number of faces to return (default: 10)'),
  }))
  .output(z.object({
    faces: z.array(z.object({
      ...
      landmarkingConfidence: z.number().describe('Confidence of facial landmark detection'),
      ...
      underExposedLikelihood: likelihoodSchema.describe('Likelihood that the face is under-exposed'),
      blurredLikelihood: likelihoodSchema.describe('Likelihood that the face is blurred'),
      headwearLikelihood: likelihoodSchema.describe('Likelihood that the face has headwear'),
    })),
  }))
