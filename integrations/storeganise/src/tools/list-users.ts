...
export let listUsersTool = SlateTool.create(
  ...
  .input(z.object({
    search: z.string().optional().describe('Search query to filter users by name, email, or other attributes'),
    ids: z.string().optional().describe('Comma-separated list of user IDs to fetch (up to 50)'),
    updatedAfter: z.string().optional().describe('Only return users updated after this UTC timestamp'),
