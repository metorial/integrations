...
let routeSchema = z.object({
  ...
  weightName: z.string().optional().describe('Weight profile used (e.g., routability, duration)'),
  legs: z.array(z.object({
    ...
    })).optional().describe('Turn-by-turn steps'),
  })).optional().describe('Route legs between waypoints'),
});

export let directionsTool = SlateTool.create(
  spec,
  {
    name: 'Directions',
    key: 'directions',
  ...
  .input(z.object({
    coordinates: z.string().describe('Semicolon-separated "longitude,latitude" pairs (e.g., "-73.989,40.733;-74.006,40.714")'),
    ...
    language: z.string().optional().describe('Language for instructions (IETF tag, e.g., "en", "fr")'),
    overview: z.enum(['full', 'simplified', 'false']).optional().describe('Detail level of route geometry'),
    annotations: z.string().optional().describe('Comma-separated annotations: duration, distance, speed, congestion, congestion_numeric'),
    exclude: z.string().optional().describe('Comma-separated road types to exclude: toll, motorway, ferry, unpaved, cash_only_tolls'),
  }))
  .output(z.object({
    routes: z.array(routeSchema).describe('Calculated routes'),
    waypoints: z.array(z.object({
      name: z.string().optional().describe('Waypoint street name'),
      location: z.array(z.number()).optional().describe('[longitude, latitude] snapped to road'),
    })).optional().describe('Snapped waypoint locations'),
