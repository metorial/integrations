...
export let manageDatasetFeaturesTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    featureId: z.string().optional().describe('Feature ID (required for get, upsert, delete)'),
    geometry: z.any().optional().describe('GeoJSON geometry object (for upsert)'),
    properties: z.record(z.string(), z.any()).optional().describe('Feature properties (for upsert)'),
