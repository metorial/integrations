...
export let manageUploadsTool = SlateTool.create(
  spec,
  {
    name: 'Manage Uploads',
    key: 'manage_uploads',
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'credentials', 'delete']).describe('Operation to perform'),
    uploadId: z.string().optional().describe('Upload ID (required for get, delete)'),
    tilesetId: z.string().optional().describe('Target tileset ID in "username.tilesetname" format (for create)'),
    url: z.string().optional().describe('Staged data URL (for create; typically an S3 URL from credentials)'),
  ...
  .output(z.object({
    ...
    }).optional().describe('S3 staging credentials (for credentials)'),
