...
export let isochroneTool = SlateTool.create(
  spec,
  {
    name: 'Isochrone',
    key: 'isochrone',
  ...
  .input(z.object({
    ...
    contoursMinutes: z.string().optional().describe('Comma-separated travel time contours in minutes (e.g., "5,10,15")'),
    contoursMeters: z.string().optional().describe('Comma-separated travel distance contours in meters (e.g., "1000,5000,10000")'),
    polygons: z.boolean().optional().describe('Return filled polygons instead of line contours'),
    contoursColors: z.string().optional().describe('Comma-separated hex colors for contours (e.g., "ff0000,00ff00,0000ff")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.getIsochrone(ctx.input.profile, ctx.input.longitude, ctx.input.latitude, {
