...
export let manageInvoices = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'delete', 'send', 'markAsSent']).describe('Action to perform'),
    invoiceId: z.number().optional().describe('Invoice ID (required for update/delete/send/markAsSent)'),
    ...
    currencyCode: z.string().optional().describe('Three-letter currency code (e.g. USD, CAD)'),
    ...
    emailRecipients: z.array(z.string()).optional().describe('Email addresses for sending (required for "send" action)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let buildPayload = () => {
      ...
      if (ctx.input.dueOffsetDays !== undefined) payload.due_offset_days = ctx.input.dueOffsetDays;
      if (ctx.input.currencyCode !== undefined) payload.currency_code = ctx.input.currencyCode;
      if (ctx.input.invoiceNumber !== undefined) payload.invoice_number = ctx.input.invoiceNumber;
      if (ctx.input.poNumber !== undefined) payload.po_number = ctx.input.poNumber;
      if (ctx.input.discountValue !== undefined) payload.discount_value = ctx.input.discountValue;
    ...
    if (ctx.input.action === 'send') {
      ...
      if (!ctx.input.emailRecipients?.length) throw new Error('emailRecipients are required for send action');
      let result = await fbClient.sendInvoiceByEmail(ctx.input.invoiceId, ctx.input.emailRecipients);
      return {
        output: mapResult(result),
