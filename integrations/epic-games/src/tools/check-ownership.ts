...
export let checkOwnership = SlateTool.create(
  spec,
  {
...
Requires OAuth authentication with the account to verify.`,
  ...
  .input(z.object({
    ...
    catalogItemIds: z.array(z.string()).optional().describe('Catalog item IDs to check, in format "sandboxId:catalogItemId"'),
    sandboxId: z.string().optional().describe('Sandbox ID to list all owned items in. Uses the configured sandboxId if not provided.'),
  }))
  .output(z.object({
    ownership: z.array(ownershipItemSchema).describe('Ownership status for each queried item'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let data = await client.checkOwnership(ctx.input.accountId, ctx.input.catalogItemIds, sandboxId);
