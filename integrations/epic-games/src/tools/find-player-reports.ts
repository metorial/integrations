...
export let findPlayerReports = SlateTool.create(
  spec,
  {
...
At least one of **reportingPlayerId** or **reportedPlayerId** must be provided. Supports pagination and sorting.`,
  ...
  .input(z.object({
    reportingPlayerId: z.string().optional().describe('Filter by the player who submitted the report'),
    reportedPlayerId: z.string().optional().describe('Filter by the player who was reported'),
    ...
    order: z.enum(['time:desc', 'time:asc', 'reasonId:asc', 'reasonId:desc']).default('time:desc').describe('Sort order for results'),
