...
export let sendPlayerReport = SlateTool.create(
  spec,
  {
...
Reports feed into moderation workflows and can be queried via the **Find Player Reports** tool.`,
  ...
  .input(z.object({
    reportingPlayerId: z.string().describe('Product User ID of the player making the report'),
    reportedPlayerId: z.string().describe('Product User ID of the player being reported'),
    reasonId: z.number().int().min(1).max(7).describe('Report reason: 1=Cheating, 2=Exploiting, 3=Offensive Profile, 4=Verbal Abuse, 5=Scamming, 6=Spamming, 7=Other'),
    message: z.string().max(1024).optional().describe('Additional message/details from the reporter'),
    context: z.string().max(4096).optional().describe('JSON string with additional context about the incident'),
    time: z.string().optional().describe('When the incident occurred (ISO 8601). Defaults to current time.'),
  }))
  .output(z.object({
