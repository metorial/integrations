...
export let sanctionsSync = SlateTrigger.create(
  spec,
  {
    name: 'Sanctions Changed',
    key: 'sanctions_sync',
    description: 'Triggers when sanctions are created, updated, or removed for your deployment. Uses the EOS sanctions sync API to incrementally detect changes.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let inputs = elements.map((el: any) => ({
        logId: el.logId,
        eventType: (eventTypeMap[el.eventType] ?? 'created') as 'created' | 'updated' | 'deleted',
