...
export let videoSearch = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    duration: z.enum(['short', 'medium', 'long']).optional().describe('Video duration filter'),
    quality: z.enum(['high']).optional().describe('Video quality filter'),
    sourcePlatform: z.enum(['youtube', 'tiktok', 'facebook', 'dailymotion', 'twitch', 'vimeo']).optional()
  ...
  .handleInvocation(async (ctx) => {
    ...
    let relatedSearches = (data.related_searches || []).map((r: any) => r.query).filter(Boolean);
