...
export let imageSearch = SlateTool.create(
  ...
  .input(z.object({
    ...
    size: z.enum(['large', 'medium', 'icon', 'qsvga', 'vga', 'svga', 'xga', '2mp', '4mp', '6mp', '8mp', '10mp', '12mp', '15mp', '20mp', '40mp', '70mp']).optional()
      .describe('Image size filter'),
    color: z.enum(['red', 'orange', 'yellow', 'green', 'teal', 'blue', 'purple', 'pink', 'white', 'gray', 'black', 'brown', 'black_and_white', 'transparent']).optional()
      .describe('Color filter'),
    imageType: z.enum(['clipart', 'line_drawing', 'gif', 'photo', 'face']).optional()
    ...
    usageRights: z.enum(['creative_commons_licenses', 'commercial_or_other_licenses']).optional()
  ...
  .handleInvocation(async (ctx) => {
    ...
    let suggestions = (data.suggestions || []).map((s: any) => s.query || s.title || s).filter(Boolean);
