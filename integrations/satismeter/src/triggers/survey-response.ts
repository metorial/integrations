...
export let surveyResponseTrigger = SlateTrigger.create(
  spec,
  {
    ...
    key: 'survey_response',
    description: 'Triggers when a user interacts with a survey: answers the first question, completes all questions, or dismisses the survey.',
  }
)
  .input(z.object({
    event: z.enum(['answered', 'completed', 'dismissed']).describe('Type of survey interaction'),
    ...
    category: z.string().optional().describe('Response category (promoter, passive, detractor)'),
    surveyCompleted: z.boolean().optional().describe('Whether the survey was fully completed'),
    ...
    userTraits: z.record(z.string(), z.unknown()).optional().describe('Respondent custom traits'),
    location: z.object({
      country: z.string().optional(),
      city: z.string().optional(),
    }).optional().describe('Respondent location'),
  }))
  .output(z.object({
    responseId: z.string().describe('Unique identifier of the response'),
    event: z.string().describe('Type of survey interaction (answered, completed, dismissed)'),
    rating: z.number().optional().describe('Numeric rating (0-10 scale)'),
    feedback: z.string().optional().describe('Textual feedback from the user'),
    answers: z.array(z.any()).optional().describe('Array of answer objects'),
    category: z.string().optional().describe('Response category (promoter, passive, detractor)'),
