...
export let listExtensions = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    extensionType: z.enum(['User', 'Department', 'Announcement', 'SharedLinesGroup', 'PagingOnly', 'IvrMenu', 'ParkLocation']).optional().describe('Filter by extension type'),
    status: z.enum(['Enabled', 'Disabled', 'NotActivated']).optional().describe('Filter by extension status'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let extensions = (result.records || []).map((ext: any) => ({
      ...
      name: ext.name || ext.contact?.firstName ? `${ext.contact?.firstName || ''} ${ext.contact?.lastName || ''}`.trim() : '',
      type: ext.type || '',
      status: ext.status || '',
