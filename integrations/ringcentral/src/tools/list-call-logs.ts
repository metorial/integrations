...
let callLogRecordSchema = z.object({
  ...
  }).optional().describe('Caller information'),
  to: z.object({
    phoneNumber: z.string().optional(),
    name: z.string().optional(),
    extensionId: z.string().optional(),
  }).optional().describe('Callee information'),
  recording: z.object({
    id: z.string().optional(),
    contentUri: z.string().optional(),
    type: z.string().optional(),
  }).optional().describe('Recording information if available'),
});

export let listCallLogs = SlateTool.create(
  spec,
  {
    name: 'List Call Logs',
    key: 'list_call_logs',
  ...
  .input(z.object({
    scope: z.enum(['extension', 'account']).describe('Whether to list call logs for a specific extension or the entire account'),
    extensionId: z.string().optional().describe('Extension ID to retrieve call logs for (only used when scope is "extension", defaults to current user)'),
    dateFrom: z.string().optional().describe('Start date/time in ISO 8601 format (e.g. "2024-01-01T00:00:00Z")'),
    ...
    view: z.enum(['Simple', 'Detailed']).optional().describe('Level of detail in the response (Detailed includes recording info)'),
