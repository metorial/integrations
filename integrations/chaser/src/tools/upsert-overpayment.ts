...
export let upsertOverpayment = SlateTool.create(spec, {
  ...
  .input(z.object({
    overpaymentInternalId: z.string().optional().describe('Internal Chaser overpayment ID or "ext_{overpaymentId}" for updates. Omit to create.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.overpaymentInternalId) {
      result = await client.updateOverpayment(ctx.input.overpaymentInternalId, ctx.input.overpayment);
    } else {
