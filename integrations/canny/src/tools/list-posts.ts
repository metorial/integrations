...
export let listPostsTool = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum 100 posts per request (default 10).',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    status: z.string().optional().describe('Filter by status (open, under review, planned, in progress, complete, closed)'),
    search: z.string().optional().describe('Search keyword in post titles and details'),
    sort: z.enum(['newest', 'oldest', 'relevance', 'score', 'statusChanged', 'trending']).optional().describe('Sort order'),
