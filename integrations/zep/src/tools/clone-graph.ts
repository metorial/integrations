...
export let cloneGraph = SlateTool.create(
  spec,
  {
    name: 'Clone Graph',
    key: 'clone_graph',
  ...
  .input(z.object({
    sourceUserId: z.string().optional().describe('User ID of the source graph to clone (provide this or sourceGraphId)'),
    sourceGraphId: z.string().optional().describe('Graph ID of the source standalone graph to clone (provide this or sourceUserId)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let source = ctx.input.sourceUserId ? `user **${ctx.input.sourceUserId}**` : `graph **${ctx.input.sourceGraphId}**`;
    return {
      output: { cloned: true },
