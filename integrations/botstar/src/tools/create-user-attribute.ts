...
export let createUserAttribute = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    fieldType: z.enum(['string', 'number', 'date', 'boolean']).describe('Data type of the attribute'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.createUserAttribute(ctx.input.botId, ctx.input.fieldName, ctx.input.fieldType);
