...
export let sendMessage = SlateTool.create(
  ...
  .input(z.object({
    ...
    message: z.record(z.string(), z.any()).describe('Message object to send (e.g. { "text": "Hello" })'),
    messageTag: z.enum(['CONFIRMED_EVENT_UPDATE', 'POST_PURCHASE_UPDATE', 'ACCOUNT_UPDATE']).optional().describe('Message tag for subscription messaging compliance'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.sendMessage(ctx.input.recipientId, ctx.input.message, ctx.input.messageTag);
