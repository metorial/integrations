...
let completionDataSchema = z.object({
  ...
  status: z.string().optional().describe('Order status: unscheduled, scheduled, on_route, servicing, success, failed, rejected, cancelled'),
  startTime: z.record(z.string(), z.unknown()).optional().describe('Service start time (unixTimestamp, utcTime, localTime)'),
  endTime: z.record(z.string(), z.unknown()).optional().describe('Service end time (unixTimestamp, utcTime, localTime)'),
  ...
  startPosition: z.record(z.string(), z.unknown()).optional().describe('GPS position at service start'),
  endPosition: z.record(z.string(), z.unknown()).optional().describe('GPS position at service end'),
  signatures: z.array(z.record(z.string(), z.unknown())).optional().describe('Digital signature data'),
  photos: z.array(z.record(z.string(), z.unknown())).optional().describe('Photo proof of delivery'),
  barcodes: z.array(z.record(z.string(), z.unknown())).optional().describe('Barcode scan results'),
});

export let getCompletionDetails = SlateTool.create(
  spec,
  {
