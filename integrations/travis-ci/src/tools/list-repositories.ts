...
export let listRepositories = SlateTool.create(
  ...
  .input(z.object({
    ownerLogin: z.string().optional().describe('Filter repositories by owner login name. If omitted, returns repositories for the authenticated user.'),
    active: z.boolean().optional().describe('Filter by whether the repository is active on Travis CI.'),
    ...
    sortBy: z.string().optional().describe('Sort field. Options: id, github_id, owner_name, name, active, default_branch.last_build. Append :desc for reverse order.'),
  }))
  .output(z.object({
