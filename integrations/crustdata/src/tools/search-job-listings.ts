...
let jobConditionSchema = z.object({
  column: z.string().describe('Column to filter on (e.g., "job_title", "job_category", "job_location", "workplace_type", "department", "openings_count").'),
...
Supports 35+ search filters for title, category, location, workplace type, and department.`,
  ...
  .input(z.object({
    tickers: z.array(z.string()).describe('Company tickers or domain names to search job listings for.'),
    filterOp: z.enum(['and', 'or']).optional().describe('Logical operator to combine filter conditions.'),
    conditions: z.array(jobConditionSchema).optional().describe('Filter conditions for job listings.'),
  ...
  .output(z.object({
    jobListings: z.array(z.record(z.string(), z.unknown())).describe('Array of job listing records.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let filters = normalizedConditions.length > 0 ? {
    ...
    let jobListings = Array.isArray(result) ? result : (result.data ?? result.job_listings ?? []);
