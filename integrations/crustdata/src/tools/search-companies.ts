...
let searchFilterSchema = z.object({
  filterType: z.string().describe('Filter type identifier (e.g., "COMPANY_HEADCOUNT", "REGION", "INDUSTRY", "ANNUAL_REVENUE", "COMPANY_HEADCOUNT_GROWTH", "DEPARTMENT_HEADCOUNT", "NUM_OF_FOLLOWERS", "KEYWORD", "JOB_OPPORTUNITIES", "FORTUNE", "ACCOUNT_ACTIVITIES", "IN_THE_NEWS").'),
  type: z.string().optional().describe('Filter operator: "in", "not in", or "between". Omit for boolean filters like IN_THE_NEWS.'),
  value: z.unknown().optional().describe('Filter value. Arrays for "in"/"not in" (e.g., ["10,001+"]). Object with min/max for "between" (e.g., {"min": 1, "max": 500}). Omit for boolean filters.'),
  subFilter: z.string().optional().describe('Sub-filter for specific filter types (e.g., currency code "USD" for ANNUAL_REVENUE, department name for DEPARTMENT_HEADCOUNT).'),
});
...
Useful for building prospect lists, market scans, and ICP-based company discovery.`,
  ...
  .output(z.object({
    companies: z.array(z.record(z.string(), z.unknown())).describe('Array of matching company profiles.'),
