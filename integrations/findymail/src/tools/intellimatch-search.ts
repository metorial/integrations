...
export let intellimatchSearch = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    query: z.string().describe('Natural language description of target companies/contacts, e.g. "SaaS companies in US with 50-200 employees".'),
    limit: z.number().optional().describe('Maximum number of results to return.'),
    findContact: z.boolean().optional().default(false).describe('Also find contacts at matching companies.'),
    findEmail: z.boolean().optional().default(false).describe('Also find verified emails for contacts.'),
  }))
  .output(z.object({
    searchId: z.string().optional().describe('ID of the search, can be used to check status or retrieve results later.'),
    ...
    })).optional().describe('Search results if available immediately.'),
    status: z.string().optional().describe('Status of the search (e.g. "completed", "processing").'),
  }))
