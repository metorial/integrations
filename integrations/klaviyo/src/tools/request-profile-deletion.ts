...
export let requestProfileDeletion = SlateTool.create(
  spec,
  {
...
The profile and all associated data will be permanently deleted.`,
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!ctx.input.profileId && !ctx.input.email && !ctx.input.phoneNumber) {
      throw new Error('At least one identifier (profileId, email, or phoneNumber) is required');
    }
    ...
    let identifier = ctx.input.profileId ?? ctx.input.email ?? ctx.input.phoneNumber ?? 'unknown';
