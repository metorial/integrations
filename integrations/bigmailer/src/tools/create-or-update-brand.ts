...
export let createOrUpdateBrand = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    brandId: z.string().optional().describe('ID of the brand to update. Omit to create a new brand.'),
    ...
    bounceDangerPercent: z.number().min(1).max(15).optional().describe('Bounce percentage threshold to auto-pause campaigns (1-15)'),
    maxSoftBounces: z.number().min(0).max(20).optional().describe('Max soft bounces before marking undeliverable (0-20)'),
    websiteUrl: z.string().optional().describe('URL of website associated with the brand'),
    unsubscribeText: z.string().optional().describe('Message displayed on the unsubscribe page'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.bounceDangerPercent !== undefined) payload.bounce_danger_percent = ctx.input.bounceDangerPercent;
    if (ctx.input.maxSoftBounces !== undefined) payload.max_soft_bounces = ctx.input.maxSoftBounces;
    if (ctx.input.websiteUrl !== undefined) payload.url = ctx.input.websiteUrl;
    if (ctx.input.unsubscribeText !== undefined) payload.unsubscribe_text = ctx.input.unsubscribeText;
