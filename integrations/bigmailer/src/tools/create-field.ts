...
export let createField = SlateTool.create(
  ...
  .input(z.object({
    ...
    mergeTagName: z.string().describe('Name used to reference the field in templates and API (max 50 chars)'),
    fieldType: z.enum(['date', 'integer', 'text']).describe('Data type of the field'),
    sampleValue: z.string().optional().describe('Sample value used when sending test campaigns (max 50 chars)')
  }))
  .output(z.object({
