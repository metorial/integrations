...
export let createTraining = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    destination: z.string().describe('Destination model in owner/name format where the trained version will be created'),
    input: z.record(z.string(), z.any()).describe('Training input parameters (varies by model)'),
