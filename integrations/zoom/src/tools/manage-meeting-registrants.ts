...
export let manageMeetingRegistrants = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'add']).describe('Action to perform: list registrants or add a new one'),
    registrant: z.object({
    ...
    }).optional().describe('Registrant details (required when action is "add")'),
    status: z.enum(['approved', 'pending', 'denied']).optional().describe('Filter by registration status (for listing)'),
  ...
  .output(z.object({
    ...
    registrantUrl: z.string().optional().describe('Unique join URL for the registrant (when adding)'),
    totalRecords: z.number().optional().describe('Total number of registrants (when listing)'),
    ...
    })).optional().describe('List of registrants (when listing)'),
  }))
