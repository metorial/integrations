...
export let sendChatMessage = SlateTool.create(
  ...
  .input(z.object({
    userId: z.string().default('me').describe('Sender user ID or email. Use "me" for the authenticated user'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let recipient = ctx.input.toChannel ? `channel ${ctx.input.toChannel}` : `contact ${ctx.input.toContact}`;
