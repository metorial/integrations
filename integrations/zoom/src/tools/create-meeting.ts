...
let recurrenceSchema = z.object({
  ...
  weeklyDays: z.string().optional().describe('Comma-separated days: 1=Sun, 2=Mon, ... 7=Sat'),
  monthlyDay: z.number().optional().describe('Day of the month for monthly recurrence'),
  monthlyWeek: z.number().optional().describe('Week of the month: -1=last, 1=first, 2=second, 3=third, 4=fourth'),
  monthlyWeekDay: z.number().optional().describe('Day of the week for monthly recurrence'),
  endTimes: z.number().optional().describe('Number of times the meeting recurs before ending'),
  endDateTime: z.string().optional().describe('End date-time in UTC (yyyy-MM-ddTHH:mm:ssZ)'),
}).optional().describe('Recurrence settings for recurring meetings');
...
let settingsSchema = z.object({
  ...
  approvalType: z.number().optional().describe('0=auto, 1=manual, 2=no registration required'),
  registrationType: z.number().optional().describe('1=register once, 2=register each time, 3=register once and choose occurrences'),
  audio: z.enum(['both', 'telephony', 'voip']).optional().describe('Audio options'),
  autoRecording: z.enum(['local', 'cloud', 'none']).optional().describe('Automatic recording'),
  ...
  alternativeHosts: z.string().optional().describe('Comma-separated email addresses of alternative hosts'),
  breakoutRoom: z.object({
    ...
    })).optional().describe('Pre-assigned breakout rooms'),
  }).optional().describe('Breakout room settings'),
...
export let createMeeting = SlateTool.create(
  ...
  .input(z.object({
    ...
    type: z.number().default(2).describe('1=instant, 2=scheduled, 3=recurring no fixed time, 8=recurring with fixed time'),
    startTime: z.string().optional().describe('Meeting start time in UTC (yyyy-MM-ddTHH:mm:ssZ). Required for scheduled meetings'),
