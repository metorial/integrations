...
export let userEvents = SlateTrigger.create(
  spec,
  {
    name: 'User Events',
    key: 'user_events',
    description: 'Triggers on Zoom user lifecycle events: created, activated, deactivated, updated, deleted, and disassociated.',
  }
)
  .input(z.object({
    eventType: z.string().describe('The specific event type (e.g., user.created, user.updated)'),
