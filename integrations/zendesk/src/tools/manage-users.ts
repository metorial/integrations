...
export let createUser = SlateTool.create(
  ...
  .input(z.object({
    ...
    role: z.enum(['end-user', 'agent', 'admin']).optional().default('end-user').describe('Role of the user'),
...
export let updateUser = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.organizationId !== undefined) userData.organization_id = ctx.input.organizationId;
