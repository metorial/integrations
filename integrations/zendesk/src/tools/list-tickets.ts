...
export let listTickets = SlateTool.create(
  ...
  .input(z.object({
    ...
    perPage: z.number().optional().default(25).describe('Number of tickets per page (max 100)'),
    sortBy: z.enum(['assignee', 'assignee.name', 'created_at', 'group', 'id', 'locale', 'requester', 'requester.name', 'status', 'subject', 'updated_at']).optional().describe('Field to sort by'),
