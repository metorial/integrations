...
export let createCreative = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    campaignId: z.string().describe('Numeric ID of the campaign'),
    intendedStatus: z.enum(['ACTIVE', 'PAUSED', 'ARCHIVED']).default('ACTIVE').describe('Intended status of the creative'),
    content: z.any().describe('Creative content configuration (format depends on campaign type)'),
    isTest: z.boolean().optional().describe('Whether this is a test creative'),
  }))
  .output(z.object({
...
export let updateCreative = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    intendedStatus: z.enum(['ACTIVE', 'PAUSED', 'ARCHIVED']).optional().describe('New intended status'),
