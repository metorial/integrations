...
export let listCampaigns = SlateTool.create(
  ...
  .input(z.object({
    ...
    statuses: z.array(z.string()).optional().describe('Filter by statuses (e.g., ["ACTIVE", "PAUSED"])'),
...
export let createCampaign = SlateTool.create(
  ...
  .input(z.object({
    ...
    name: z.string().describe('Campaign name'),
    objectiveType: z.string().describe('Campaign objective (e.g., BRAND_AWARENESS, WEBSITE_VISITS, LEAD_GENERATION)'),
    type: z.string().describe('Campaign type (e.g., SPONSORED_UPDATES, TEXT_AD, SPONSORED_INMAILS)'),
    costType: z.string().describe('Cost type (e.g., CPM, CPC, CPV)'),
    status: z.enum(['ACTIVE', 'PAUSED', 'ARCHIVED', 'DRAFT']).default('DRAFT').describe('Initial campaign status'),
    dailyBudget: budgetSchema.optional().describe('Daily budget'),
    ...
    }).optional().describe('Locale for the campaign'),
    ...
    optimizationTargetType: z.string().optional().describe('Optimization target (e.g., MAX_CLICK, MAX_IMPRESSION, MAX_CONVERSION)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.audienceExpansionEnabled !== undefined) data['audienceExpansionEnabled'] = ctx.input.audienceExpansionEnabled;
    if (ctx.input.offsiteDeliveryEnabled !== undefined) data['offsiteDeliveryEnabled'] = ctx.input.offsiteDeliveryEnabled;
    if (ctx.input.optimizationTargetType) data['optimizationTargetType'] = ctx.input.optimizationTargetType;
...
export let updateCampaign = SlateTool.create(
  ...
  .input(z.object({
    ...
    audienceExpansionEnabled: z.boolean().optional().describe('Enable/disable audience expansion'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.audienceExpansionEnabled !== undefined) patch['audienceExpansionEnabled'] = ctx.input.audienceExpansionEnabled;
    if (ctx.input.optimizationTargetType) patch['optimizationTargetType'] = ctx.input.optimizationTargetType;
