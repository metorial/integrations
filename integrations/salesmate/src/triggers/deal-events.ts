...
export let dealEvents = SlateTrigger.create(
  spec,
  {
    name: 'Deal Events',
    key: 'deal_events',
    description: 'Triggers when deals are created or updated in Salesmate. Captures stage changes, status changes, and other deal modifications.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let fields = ['title', 'pipeline', 'stage', 'status', 'dealValue', 'owner', 'createdAt', 'modifiedAt'];
