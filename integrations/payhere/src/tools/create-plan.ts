...
export let createPlan = SlateTool.create(
  spec,
  {
    name: 'Create Plan',
    key: 'create_plan',
  ...
  .input(z.object({
    paymentType: z.enum(['recurring', 'one_off']).describe('Whether this is a recurring subscription or one-off payment'),
    name: z.string().describe('Display name of the plan'),
    description: z.string().optional().describe('Description of the product/service shown to customers'),
    price: z.number().optional().describe('Plan price (omit for donation-style plans)'),
    currency: z.string().optional().describe('3-letter currency code (e.g. "usd", "gbp", "eur")'),
    userSelectsAmount: z.boolean().optional().describe('Set to true for donation-style plans where customer selects amount'),
    ...
    successUrl: z.string().optional().describe('URL to redirect customer to after successful payment'),
    webhookUrl: z.string().optional().describe('URL for Payhere to send payment webhooks to'),
    ...
    billingInterval: z.enum(['week', 'month', 'year']).optional().describe('Billing frequency (recurring plans only)'),
    setupFee: z.number().optional().describe('One-time setup fee charged on first payment (recurring plans only)'),
    minBillingCycles: z.number().optional().describe('Minimum billing cycles before customer can cancel (recurring plans only)'),
    billingDay: z.number().optional().describe('Day of month to charge (recurring plans only)'),
    cancelAfter: z.number().optional().describe('Automatically cancel subscription after N payments (recurring plans only)')
  }))
  .output(z.object({
