...
let emailResultSchema = z.object({
  ...
  status: z.string().describe('Validation status: valid, invalid, catch-all, unknown, spamtrap, abuse, or do_not_mail'),
  ...
  didYouMean: z.string().nullable().describe('Suggested correction if a possible typo was detected'),
  ...
  activeInDays: z.string().nullable().optional().describe('Days since last activity, if activity data was requested'),
  ...
  firstName: z.string().nullable().describe('First name associated with the email, if available'),
  lastName: z.string().nullable().describe('Last name associated with the email, if available'),
  ...
  region: z.string().nullable().describe('Region/state associated with the email, if available'),
...
export let validateEmail = SlateTool.create(
  spec,
  {
    name: 'Validate Email',
    key: 'validate_email',
...
For a single email, pass it directly. For multiple emails (up to 200), pass an array to use batch validation.`,
  ...
  .input(z.object({
    emails: z.union([
      ...
      z.array(z.object({
        ...
        ipAddress: z.string().optional().describe('IP address the email signed up from'),
      })).describe('Array of emails (up to 200) for batch validation'),
    ]).describe('Email address(es) to validate'),
    ipAddress: z.string().optional().describe('IP address the email signed up from (single validation only)'),
    timeout: z.number().optional().describe('Seconds to wait before returning unknown/greylisted (3-60 for single, 10-120 for batch)'),
    activityData: z.boolean().optional().describe('Set to true to append activity data to results'),
    verifyPlus: z.boolean().optional().describe('Set to true to use enhanced Verify+ validation'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Any errors encountered during batch validation'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (isBatch) {
      ...
      let mapped = (result.email_batch || []).map((r: Record<string, unknown>) => mapValidationResult(r));
