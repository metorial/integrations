...
export let manageEmployee = SlateTool.create(
  ...
  .input(z.object({
    operation: z.enum(['create', 'update']).describe('Whether to create a new record or update an existing one'),
    entitySet: z.string().describe('The OData entity set to operate on (e.g., "User", "EmpJob", "PerPersonal", "EmpCompensation")'),
    keys: z.record(z.string(), z.union([z.string(), z.number()])).optional().describe('Compound key fields to identify the record for updates (e.g., { "userId": "user1", "seqNumber": 1, "startDate": "2024-01-01" })'),
    fields: z.record(z.string(), z.unknown()).describe('Field values to set on the record'),
  }))
  .output(z.object({
    record: z.record(z.string(), z.unknown()).optional().describe('The created or updated record (returned for create operations)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.operation === 'create') {
      ...
      if (Object.keys(keys).length === 1) {
        ...
        await client.updateEntityByCompoundKey(ctx.input.entitySet, keys as Record<string, string | number>, ctx.input.fields);
      }
