...
export let getJobApplication = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    filter: z.string().optional().describe('OData $filter expression for searching applications (e.g., "jobReqId eq 12345")'),
    select: z.string().optional().describe('Comma-separated fields to return'),
    expand: z.string().optional().describe('Navigation properties to expand (e.g., "candidate,jobRequisition")'),
    top: z.number().optional().describe('Maximum records to return when searching').default(50),
    skip: z.number().optional().describe('Number of records to skip'),
  }))
  .output(z.object({
    application: z.record(z.string(), z.unknown()).optional().describe('Single application record (when applicationId is provided)'),
    applications: z.array(z.record(z.string(), z.unknown())).optional().describe('List of applications (when searching)'),
