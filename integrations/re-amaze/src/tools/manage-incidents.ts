...
let incidentUpdateSchema = z.object({
  status: z.string().describe('Update status (investigating, identified, monitoring, resolved)'),
...
let incidentSystemSchema = z.object({
  ...
  status: z.string().describe('System status (operational, degraded_performance, partial_outage, major_outage, under_maintenance)'),
});
...
let incidentSchema = z.object({
  ...
  })).optional().describe('Incident status updates'),
...
export let listIncidents = SlateTool.create(
  ...
  .input(z.object({
    activeOnly: z.boolean().optional().default(false).describe('When true, returns only active/unresolved incidents'),
  }))
  .output(z.object({
...
export let createIncident = SlateTool.create(
  ...
  .input(z.object({
    title: z.string().describe('Incident title'),
    updates: z.array(incidentUpdateSchema).describe('Initial status updates for the incident'),
    systems: z.array(z.object({
      systemId: z.number().describe('System ID to associate'),
      status: z.enum(['operational', 'degraded_performance', 'partial_outage', 'major_outage', 'under_maintenance']).describe('System status'),
    })).optional().describe('Systems affected by this incident'),
  }))
...
export let updateIncident = SlateTool.create(
  ...
  .input(z.object({
    ...
    updates: z.array(incidentUpdateSchema).optional().describe('New status updates to add to the incident'),
    systems: z.array(z.object({
      incidentSystemId: z.number().optional().describe('Existing incident-system association ID (for updating existing associations)'),
      systemId: z.number().describe('System ID'),
      status: z.enum(['operational', 'degraded_performance', 'partial_outage', 'major_outage', 'under_maintenance']).describe('Updated system status'),
    })).optional().describe('Updated system associations'),
  }))
  .output(incidentSchema)
  .handleInvocation(async (ctx) => {
    let client = new Client({
