...
export let managePermissions = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['get', 'set', 'get_workflow_permissions', 'set_workflow_permissions']).describe('Action to perform'),
    enabled: z.boolean().optional().describe('Whether Actions is enabled, for "set" action'),
    allowedActions: z.enum(['all', 'local_only', 'selected']).optional().describe('Which actions are allowed, for "set" action'),
    defaultWorkflowPermissions: z.enum(['read', 'write']).optional().describe('Default GITHUB_TOKEN permissions, for "set_workflow_permissions"'),
    canApprovePullRequestReviews: z.boolean().optional().describe('Whether GITHUB_TOKEN can approve PRs, for "set_workflow_permissions"'),
  }))
  .output(z.object({
    ...
    defaultWorkflowPermissions: z.string().optional().describe('Default GITHUB_TOKEN permissions (read or write)'),
    canApprovePullRequestReviews: z.boolean().optional().describe('Whether GITHUB_TOKEN can approve PRs'),
