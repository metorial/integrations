...
let secretSchema = z.object({
  ...
  visibility: z.string().optional().describe('Visibility for org secrets (all, private, selected)'),
});
...
export let manageSecrets = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Secret values cannot be read through the API, only metadata is returned.',
    ],
    tags: {
      destructive: false,
    },
  ...
  .input(z.object({
    owner: z.string().optional().describe('Repository owner, required for repo and environment scopes'),
    repo: z.string().optional().describe('Repository name, required for repo and environment scopes'),
    ...
    environmentName: z.string().optional().describe('Environment name, required for environment scope'),
    action: z.enum(['list', 'get', 'create_or_update', 'delete', 'get_public_key']).describe('Action to perform'),
    secretName: z.string().optional().describe('Secret name, required for get/create_or_update/delete'),
    encryptedValue: z.string().optional().describe('Base64-encoded encrypted secret value, for create_or_update'),
    keyId: z.string().optional().describe('Public key ID, for create_or_update'),
    visibility: z.enum(['all', 'private', 'selected']).optional().describe('Visibility for org secrets'),
    selectedRepositoryIds: z.array(z.number()).optional().describe('Repository IDs for "selected" visibility org secrets'),
  ...
  .output(z.object({
    ...
    publicKey: z.string().optional().describe('Base64-encoded public key for encrypting secrets'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { scope, action, owner, repo, org, environmentName, secretName, perPage, page } = ctx.input;
    ...
    if (action === 'get_public_key') {
      ...
      if (scope === 'org') {
        ...
        if (!owner || !repo || !environmentName) throw new Error('owner, repo, and environmentName are required for environment scope.');
    ...
    if (action === 'list') {
      ...
      if (scope === 'org') {
        ...
        if (!owner || !repo || !environmentName) throw new Error('owner, repo, and environmentName are required.');
        data = await client.listEnvironmentSecrets(owner, repo, environmentName, { perPage, page });
      } else {
        if (!owner || !repo) throw new Error('owner and repo are required for repo scope.');
    ...
    if (action === 'create_or_update') {
      if (!secretName || !ctx.input.encryptedValue || !ctx.input.keyId) {
        throw new Error('secretName, encryptedValue, and keyId are required for create_or_update.');
      }
      if (scope === 'org') {
        if (!org) throw new Error('org is required.');
        ...
        if (!owner || !repo || !environmentName) throw new Error('owner, repo, and environmentName are required.');
        await client.createOrUpdateEnvironmentSecret(owner, repo, environmentName, secretName, ctx.input.encryptedValue, ctx.input.keyId);
      } else {
        if (!owner || !repo) throw new Error('owner and repo are required.');
        await client.createOrUpdateRepoSecret(owner, repo, secretName, ctx.input.encryptedValue, ctx.input.keyId);
      }
      return {
        output: { created: true },
