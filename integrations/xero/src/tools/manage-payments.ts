...
export let createPayment = SlateTool.create(
  ...
  .input(z.object({
    invoiceId: z.string().optional().describe('Invoice ID to pay (use either this or creditNoteId)'),
    creditNoteId: z.string().optional().describe('Credit note ID to pay (use either this or invoiceId)'),
    ...
    reference: z.string().optional().describe('Payment reference'),
    currencyRate: z.number().optional().describe('Currency exchange rate for multi-currency payments'),
  }))
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.creditNoteId) paymentData.CreditNote = { CreditNoteID: ctx.input.creditNoteId };
...
export let listPayments = SlateTool.create(
  ...
  .input(z.object({
    ...
    modifiedAfter: z.string().optional().describe('Only return payments modified after this date (ISO 8601)'),
    where: z.string().optional().describe('Xero API where filter expression, e.g. `Status=="AUTHORISED"`'),
