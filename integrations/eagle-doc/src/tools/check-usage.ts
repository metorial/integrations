...
export let checkUsage = SlateTool.create(
  ...
  .input(z.object({
    usageType: z.enum(['current', 'monthly', 'logs', 'quota']).describe('Type of usage data to retrieve'),
  }))
  .output(z.object({
    ...
    }).optional().describe('Current month usage (when usageType is "current")'),
    monthlyUsage: z.array(z.object({
      quotaUsed: z.number().optional(),
      quotaDate: z.string().optional(),
      additionalInfo: z.record(z.string(), z.any()).optional(),
    }).passthrough()).optional().describe('Monthly usage history (when usageType is "monthly")'),
    requestLogs: z.array(z.object({
      pages: z.number().optional(),
      time: z.string().optional(),
      timeRequested: z.string().optional(),
    }).passthrough()).optional().describe('Recent API call logs (when usageType is "logs")'),
    managementQuota: z.object({
      quota: z.number().optional(),
      quotaUsed: z.number().optional(),
      currentMonth: z.string().optional(),
    }).optional().describe('Management quota info (when usageType is "quota")'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.usageType) {
      case 'current': {
        ...
        if (data.overUsage > 0) message += ` Over-usage: ${data.overUsage} pages ($${data.overUsageCost}).`;
        break;
      }
      case 'monthly': {
