...
export let subtaskEventsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Subtask Events',
    key: 'subtask_events',
    description: 'Triggers when a subtask is created, updated, or deleted on a card. Configure a board webhook in Kanbanize to send events to the provided webhook URL.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    isFinished: z.number().optional().nullable().describe('Whether the subtask is finished (0 or 1)'),
  }))
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      if (ctx.input.event !== 'Subtask deleted') {
        try {
          ...
          subtaskDetails = await client.getSubtask(ctx.input.cardId, ctx.input.subtaskId) ?? {};
        } catch {
