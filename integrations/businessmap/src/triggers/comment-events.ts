...
export let commentEventsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Comment Events',
    key: 'comment_events',
    description: 'Triggers when a comment is created, updated, or deleted on a card. Configure a board webhook in Kanbanize to send events to the provided webhook URL.',
  }
)
  .input(z.object({
    ...
    handleEvent: async (ctx) => {
      ...
      if (ctx.input.event !== 'Comment deleted') {
        try {
          ...
          commentDetails = await client.getComment(ctx.input.cardId, ctx.input.commentId) ?? {};
        } catch {
