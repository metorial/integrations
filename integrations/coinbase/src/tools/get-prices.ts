...
export let getPrices = SlateTool.create(
  ...
  .input(z.object({
    ...
    priceType: z.enum(['spot', 'buy', 'sell']).default('spot').describe('Type of price to retrieve'),
    date: z.string().optional().describe('Historical date for spot price (YYYY-MM-DD format)'),
  }))
  .output(z.object({
