...
let taskSchema = z.object({
  ...
  deadline: z.object({
    date: z.string(),
  }).nullable().describe('Hard deadline'),
  duration: z.object({
    amount: z.number(),
    unit: z.enum(['minute', 'day']),
  }).nullable().describe('Estimated duration'),
...
export let getTasks = SlateTool.create(
  ...
  .input(z.object({
    ...
    filter: z.string().optional().describe('Todoist filter query, e.g. "today", "priority 1 & overdue"'),
