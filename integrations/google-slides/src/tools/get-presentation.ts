...
let pageElementSchema = z.object({
  ...
  sheetsChart: z.any().optional().describe('Sheets chart properties if this is a linked chart'),
  elementGroup: z.any().optional().describe('Group properties if this is a group of elements')
}).passthrough();

let slideSchema = z.object({
  ...
  slideProperties: z.any().optional().describe('Slide-specific properties including layout and notes references'),
  pageType: z.string().optional().describe('Type of the page (SLIDE, MASTER, LAYOUT, NOTES)')
}).passthrough();

export let getPresentation = SlateTool.create(
  spec,
  {
