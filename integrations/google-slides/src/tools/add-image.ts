...
export let addImage = SlateTool.create(
  spec,
  {
    name: 'Add Image',
    key: 'add_image',
  ...
  .input(z.object({
    ...
    mode: z.enum(['insert', 'replace_shapes']).describe('Insert an image directly or replace shapes matching text with an image'),

    imageUrl: z.string().describe('Publicly accessible URL of the image to insert'),

    slideObjectId: z.string().optional().describe('Object ID of the slide to insert the image onto (for insert mode)'),
    widthPt: z.number().optional().describe('Width of the image in points (for insert mode)'),
    heightPt: z.number().optional().describe('Height of the image in points (for insert mode)'),
    translateXPt: z.number().optional().describe('X position offset in points from the top-left (for insert mode)'),
    translateYPt: z.number().optional().describe('Y position offset in points from the top-left (for insert mode)'),

    findText: z.string().optional().describe('Text pattern to match shapes for replacement (for replace_shapes mode)'),
    replaceMethod: z.enum(['CENTER_INSIDE', 'CENTER_CROP']).optional().describe('How to fit the image in the replaced shape (for replace_shapes mode, defaults to CENTER_INSIDE)')
  }))
  .output(z.object({
    presentationId: z.string().describe('ID of the presentation'),
    mode: z.string().describe('Mode used'),
    createdImageId: z.string().optional().describe('Object ID of the created image (for insert mode)'),
    shapesReplaced: z.number().optional().describe('Number of shapes replaced (for replace_shapes mode)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (mode === 'insert') {
      ...
      result = await client.createImage(presentationId, {
        ...
        size: ctx.input.widthPt && ctx.input.heightPt
