...
export let listCollections = SlateTool.create(
  ...
  .input(z.object({
    source: z.enum(['featured', 'personal']).describe('Whether to list Pexels-featured collections or your own personal collections'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = ctx.input.source === 'featured'
      ? await client.getFeaturedCollections({ page: ctx.input.page, perPage: ctx.input.perPage })
