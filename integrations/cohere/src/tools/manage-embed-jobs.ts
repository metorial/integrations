...
let embedJobOutputSchema = z.object({
  ...
  status: z.string().describe('Current status: processing, complete, cancelling, cancelled, or failed'),
...
export let createEmbedJobTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    inputType: z.enum(['search_document', 'search_query', 'classification', 'clustering']).describe('How the embeddings will be used'),
    name: z.string().optional().describe('Name for the embed job'),
    embeddingTypes: z.array(z.enum(['float', 'int8', 'uint8', 'binary', 'ubinary'])).optional().describe('Output embedding formats'),
    truncate: z.enum(['START', 'END']).optional().describe('Truncation strategy for oversized inputs (default: END)')
  }))
  .output(z.object({
...
export let listEmbedJobsTool = SlateTool.create(
  spec,
  {
...
export let getEmbedJobTool = SlateTool.create(
  spec,
  {
    name: 'Get Embed Job',
    key: 'get_embed_job',
...
export let cancelEmbedJobTool = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    cancelled: z.boolean().describe('Whether the cancellation request was submitted successfully')
  }))
  .handleInvocation(async (ctx) => {
    let client = new CohereClient({ token: ctx.auth.token });
