...
export let manageLink = SlateTool.create(
  spec,
  {
    name: 'Manage FanLink',
    key: 'manage_link',
...
Links can include tracking pixels, email capture forms, social sharing metadata, and affiliate codes.`,
  ...
  .input(z.object({
    action: z.enum(['create', 'get', 'update', 'delete', 'expand', 'validate_path']).describe('Operation to perform'),
    linkId: z.number().optional().describe('Link ID (required for get, update, delete)'),
    title: z.string().optional().describe('Link title'),
    targetType: z.enum(['music', 'podcast', 'livestream', 'event', 'tour', 'biglink', 'fundraiser', 'smartlink', 'custom']).optional().describe('Link type'),
    targetUrl: z.string().optional().describe('Destination URL'),
    ...
    services: z.array(z.record(z.string(), z.any())).optional().describe('Array of service links for the landing page'),
    socialSettings: z.array(z.record(z.string(), z.any())).optional().describe('Social profile links'),
    ...
    pixels: z.array(z.record(z.string(), z.any())).optional().describe('Tracking pixels (Facebook, Google, Twitter, Snapchat, TikTok)'),
    affiliateCodes: z.record(z.string(), z.any()).optional().describe('iTunes and Amazon affiliate codes'),
    showSocials: z.boolean().optional().describe('Show social profile links on landing page'),
    skipLandingPage: z.boolean().optional().describe('Redirect directly instead of showing landing page'),
    sortServicesByClickthroughs: z.boolean().optional().describe('Auto-sort services by click count'),
    header: z.string().optional().describe('Landing page header text'),
    ...
    url: z.string().optional().describe('Source URL for expand action (streaming service link)'),
  ...
  .output(z.object({
    ...
    pathValid: z.boolean().optional().describe('Whether the path is available (validate_path action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'validate_path') {
      ...
      return {
        ...
        message: result.valid ? `Path "${ctx.input.path}" is **available**.` : `Path "${ctx.input.path}" is **not available**: ${result.reason}.`,
      };
    }
    ...
    if (ctx.input.metadataDescription) data.metadata_description = ctx.input.metadataDescription;
    ...
    if (ctx.input.skipLandingPage !== undefined) data.skip_landing_page = ctx.input.skipLandingPage;
    if (ctx.input.sortServicesByClickthroughs !== undefined) data.sort_services_by_clickthroughs = ctx.input.sortServicesByClickthroughs;
