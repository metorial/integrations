...
export let managePlaybook = SlateTool.create(
  spec,
  {
...
Each playbook campaign can contain subcampaigns of type ad, attachment, link, or message.`,
  ...
  .input(z.object({
    ...
    playbookCampaignId: z.number().optional().describe('Playbook campaign ID (required for get, update, delete)'),
    title: z.string().optional().describe('Campaign title'),
    templateId: z.number().optional().describe('Playbook template ID (required for create)'),
    status: z.enum(['active', 'paused', 'inactive', 'draft']).optional().describe('Campaign status'),
    isArchived: z.boolean().optional().describe('Archive the campaign'),
    subcampaigns: z.array(z.object({
      type: z.enum(['ad', 'attachment', 'link', 'message']).describe('Subcampaign type'),
      adCampaign: z.record(z.string(), z.any()).optional().describe('Ad campaign configuration'),
      attachment: z.record(z.string(), z.any()).optional().describe('Attachment configuration'),
      link: z.record(z.string(), z.any()).optional().describe('Link configuration'),
      messageCampaign: z.record(z.string(), z.any()).optional().describe('Message campaign configuration'),
    })).optional().describe('Subcampaigns within the playbook'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'get') {
      if (!playbookCampaignId) throw new Error('playbookCampaignId is required for get action');
    ...
    if (action === 'delete') {
      if (!playbookCampaignId) throw new Error('playbookCampaignId is required for delete action');
    ...
    // update
    if (!playbookCampaignId) throw new Error('playbookCampaignId is required for update action');
