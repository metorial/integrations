...
export let campaignStatusChange = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers when an ad campaign changes status (e.g., becomes active, paused, error, or inactive). Polls all campaigns and detects status changes between runs.',
  }
)
  .input(z.object({
