...
export let manageDataSource = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'delete', 'add_text', 'link_to_assistant']).describe('Operation to perform'),
    datasourceId: z.string().optional().describe('Data source ID (required for delete, add_text, link_to_assistant)'),
    name: z.string().optional().describe('Name for the data source (used in create/add_text)'),
    text: z.string().optional().describe('Text content to add (for add_text action)'),
    assistantId: z.string().optional().describe('Assistant ID to link (for link_to_assistant action)'),
  }))
  .output(z.object({
