...
export let makeCall = SlateTool.create(
  ...
  .input(z.object({
    widgetId: z.string().describe('Widget ID to use for the call (must have telephony configured)'),
    phoneNumber: z.string().describe('Phone number to call in E.164 format'),
    promptVariables: z.record(z.string(), z.string()).optional().describe('Dynamic variables for prompt personalization, e.g. { "customerName": "John" }'),
  }))
  .output(z.object({
