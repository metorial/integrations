...
let callSummarySchema = z.object({
  ...
  simpleTranscript: z.string().nullable().optional().describe('Simple text transcript (only if requested)'),
});
...
export let listCalls = SlateTool.create(
  ...
  .input(z.object({
    ...
    order: z.string().optional().describe('Sort order: "created_at desc", "created_at asc", "performed_at desc", "performed_at asc", "updated_at desc", "updated_at asc"'),
    dateFilter: z.enum(['created_at', 'performed_at', 'updated_at']).optional().describe('Which date field to filter on (default: created_at)'),
    ...
    sourceIds: z.array(z.string()).optional().describe('Filter by call IDs within the integration'),
    ownerUuids: z.array(z.string()).optional().describe('Filter by owner user UUIDs'),
    participatingUserUuids: z.array(z.string()).optional().describe('Filter by participating user UUIDs'),
    customerPhoneNumbers: z.array(z.string()).optional().describe('Filter by customer phone numbers'),
    customerEmailAddresses: z.array(z.string()).optional().describe('Filter by customer email addresses'),
    withSimpleTranscript: z.boolean().optional().describe('Include simple text transcript in results (default: false)'),
  }))
  .output(z.object({
