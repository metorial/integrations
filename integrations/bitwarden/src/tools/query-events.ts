...
let eventSchema = z.object({
  eventType: z.number().describe('Numeric event type code (e.g. 1000=UserLoggedIn, 1100=CipherCreated, 1500=OrgUserInvited)'),
...
export let queryEvents = SlateTool.create(
  ...
  .input(z.object({
    start: z.string().optional().describe('Start date in ISO 8601 format (e.g. 2024-01-01T00:00:00Z)'),
    end: z.string().optional().describe('End date in ISO 8601 format'),
    actingUserId: z.string().optional().describe('Filter events by the user who performed the action'),
