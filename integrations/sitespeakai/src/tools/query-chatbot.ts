...
export let queryChatbot = SlateTool.create(
  spec,
  {
    name: 'Query Chatbot',
    key: 'query_chatbot',
  ...
  .input(z.object({
    chatbotId: z.string().describe('The ID of the chatbot to query. Found on the chatbot settings page.'),
    prompt: z.string().describe('The question or query text to send to the chatbot.'),
    conversationId: z.string().optional().describe('Optional conversation ID to group related questions and answers into a single conversation thread.'),
    format: z.enum(['html', 'markdown']).optional().describe('Response format. Defaults to markdown.'),
  }))
  .output(z.object({
