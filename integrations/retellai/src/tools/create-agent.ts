...
export let createAgent = SlateTool.create(
  ...
  .input(z.object({
    ...
    responseEngine: z.any().describe('Response engine configuration object. Can be retell-llm, custom-llm, or conversation-flow type.'),
    language: z.string().optional().describe('Language/dialect for speech recognition (e.g. en-US, fr-FR, zh-CN)'),
    ...
    enableBackchannel: z.boolean().optional().describe('Enable conversational interjections like "uh-huh"'),
    ambientSound: z.string().optional().describe('Background ambient sound (coffee-shop, convention-hall, summer-outdoor, mountain-outdoor, static-noise, call-center)'),
    additionalSettings: z.record(z.string(), z.any()).optional().describe('Additional agent settings to pass through (voice_temperature, pii_config, guardrail_config, etc.)'),
  }))
  .output(z.object({
