...
export let listCalls = SlateTool.create(
  ...
  .input(z.object({
    limit: z.number().min(1).max(1000).optional().describe('Maximum number of calls to return (1-1000, default 50)'),
    sortOrder: z.enum(['ascending', 'descending']).optional().describe('Sort direction by start timestamp (default descending)'),
    ...
    filterCallStatus: z.array(z.enum(['not_connected', 'ongoing', 'ended', 'error'])).optional().describe('Filter by call status'),
    filterCallType: z.array(z.enum(['web_call', 'phone_call'])).optional().describe('Filter by call type'),
    filterDirection: z.array(z.enum(['inbound', 'outbound'])).optional().describe('Filter by call direction'),
    filterSentiment: z.array(z.enum(['Negative', 'Positive', 'Neutral', 'Unknown'])).optional().describe('Filter by user sentiment'),
  }))
  .output(z.object({
