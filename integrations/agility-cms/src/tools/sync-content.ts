...
export let syncContent = SlateTool.create(
  ...
  .input(z.object({
    syncType: z.enum(['items', 'pages']).default('items').describe('Type of content to sync: "items" for content items or "pages" for pages'),
    syncToken: z.string().default('0').describe('Sync token from previous sync call. Use "0" for initial full sync.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = ctx.input.syncType === 'pages'
