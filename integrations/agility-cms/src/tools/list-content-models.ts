...
export let listContentModels = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    modelType: z.enum(['content', 'page_modules']).default('content').describe('Type of models to list: "content" for content models, "page_modules" for page module definitions'),
    includeDefaults: z.boolean().default(false).describe('Whether to include default system models'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let models = ctx.input.modelType === 'page_modules'
