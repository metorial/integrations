...
export let listContent = SlateTool.create(
  ...
  .input(z.object({
    referenceName: z.string().describe('Reference name of the content list/model (will be lowercased automatically)'),
    ...
    sort: z.string().optional().describe('Field to sort by (e.g., "properties.modified", "properties.created", "fields.title")'),
    direction: z.enum(['ASC', 'DESC']).optional().describe('Sort direction'),
    filter: z.string().optional().describe('Filter expression (e.g., fields.title[eq]"Hello")'),
    fields: z.string().optional().describe('Comma-separated list of fields to return'),
    contentLinkDepth: z.number().optional().describe('Levels of linked content to resolve (1-5)'),
    locale: z.string().optional().describe('Locale code override'),
    apiType: z.enum(['fetch', 'preview']).default('fetch').describe('Use "fetch" for published or "preview" for staging content'),
  }))
  .output(z.object({
