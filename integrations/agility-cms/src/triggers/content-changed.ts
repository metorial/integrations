...
export let contentChanged = SlateTrigger.create(
  spec,
  {
    name: 'Content Changed',
    key: 'content_changed',
    description: 'Triggers when content items or pages are published, saved, or go through workflow changes (approval requested, approved, declined) in Agility CMS.',
  }
)
  .input(z.object({
    ...
    contentVersionId: z.number().optional().describe('Content version ID (for content events)'),
    referenceName: z.string().optional().describe('Content model reference name (for content events)'),
  ...
  .output(z.object({
    resourceType: z.enum(['content', 'page']).describe('Whether the change affected a content item or a page'),
    ...
    referenceName: z.string().optional().describe('Content model reference name (content events only)'),
