...
export let getReverseEmailResult = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    enrichmentId: z.string().describe('The enrichment ID returned from the reverse email lookup request'),
  }))
  .output(z.object({
    enrichmentId: z.string().describe('Lookup batch ID'),
    name: z.string().optional().describe('Batch name'),
    status: z.enum(['CREATED', 'IN_PROGRESS', 'CANCELED', 'CREDITS_INSUFFICIENT', 'FINISHED', 'RATE_LIMIT', 'UNKNOWN']).describe('Current lookup status'),
    ...
    let records = (result.data ?? []).map((entry: any) => {
      ...
      return {
        ...
        profile: profile ? {
          ...
          currentEmployment: profile.employment?.current ? mapEmployment(profile.employment.current) : undefined,
