...
let memberOutputSchema = z.object({
  ...
  customFields: z.record(z.string(), z.string()).describe('Custom field values for the subscriber'),
});

export let getSubscribers = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    memberId: z.string().optional().describe('ID or email of a specific subscriber to retrieve'),
    state: z.enum(['active', 'unsubscribed', 'cleaned']).optional().describe('Filter subscribers by state'),
  }))
  .output(z.object({
...
export let addSubscriber = SlateTool.create(
  spec,
  {
    name: 'Add Subscriber',
    key: 'add_subscriber',
  ...
  .input(z.object({
    ...
    customFields: z.record(z.string(), z.string()).optional().describe('Custom field values keyed by field tag'),
    upsert: z.boolean().optional().describe('If true, updates existing subscriber instead of failing on duplicate'),
    suppressReactivation: z.boolean().optional().describe('If true, does not reactivate previously unsubscribed contacts'),
    suppressEmailNotification: z.boolean().optional().describe('If true, suppresses notification emails for this API signup'),
    ignoreDoubleOptin: z.boolean().optional().describe('If true, skips sending the double opt-in confirmation email'),
  }))
...
export let updateSubscriber = SlateTool.create(
  ...
  .input(z.object({
    ...
    state: z.enum(['active', 'unsubscribed']).optional().describe('New state for the subscriber'),
    customFields: z.record(z.string(), z.string()).optional().describe('Custom field values to update, keyed by field tag'),
  }))
  .output(memberOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
...
export let bulkSyncSubscribers = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    actions: z.array(z.enum(['add', 'update', 'unsubscribe_excluded'])).describe('Actions to perform: add new, update existing, and/or unsubscribe members not in the list'),
    members: z.array(z.object({
      memberId: z.string().optional().describe('Existing member ID (required for updates without email)'),
      email: z.string().optional().describe('Email address (required for adding new members)'),
      customFields: z.record(z.string(), z.string()).optional().describe('Custom field values keyed by field tag'),
