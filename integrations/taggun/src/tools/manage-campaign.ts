...
let campaignSettingsSchema = z.object({
  ...
  merchantNames: z.array(z.string()).optional().describe('List of participating merchant names'),
  ...
  })).optional().describe('Eligible product definitions for the campaign'),
  fraudDetection: z.boolean().optional().describe('Enable fraud detection for this campaign'),
  ...
  smartValidate: z.string().optional().describe('Natural language SmartValidate prompt for custom validation logic'),
  skip: z.boolean().optional().describe('Skip certain validation checks'),
  returnFromTheList: z.boolean().optional().describe('Return normalized merchant names from the known list'),
}).optional();
...
Use **list** to see all campaigns, **get** to retrieve a specific campaign, **create** to set up a new campaign with validation rules, **update** to modify settings, or **delete** to remove a campaign.`,
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'update', 'delete']).describe('The campaign management action to perform'),
    campaignId: z.string().optional().describe('Campaign ID (required for get, create, update, delete)'),
    settings: campaignSettingsSchema.describe('Campaign settings (required for create/update)'),
  }))
  .output(z.object({
    campaignIds: z.array(z.string()).nullable().optional().describe('List of campaign IDs (list action)'),
    ...
    deleted: z.boolean().nullable().optional().describe('Whether the campaign was deleted (delete action)'),
...
let mapSettingsToApi = (settings: any) => {
  ...
  if (settings.fraudDetection !== undefined) apiSettings['fraudDetection'] = settings.fraudDetection;
  ...
  if (settings.returnFromTheList !== undefined) apiSettings['returnFromTheList'] = settings.returnFromTheList;
