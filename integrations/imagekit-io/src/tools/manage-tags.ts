...
export let manageTags = SlateTool.create(
  ...
  .input(z.object({
    operation: z.enum(['add', 'remove', 'remove_ai']).describe('"add" to add tags, "remove" to remove tags, "remove_ai" to remove AI-generated tags'),
    fileIds: z.array(z.string()).min(1).max(50).describe('File IDs to apply the tag operation to (max 50)'),
    tags: z.array(z.string()).min(1).describe('Tags to add or remove'),
  }))
  .output(z.object({
    successfulFileIds: z.array(z.string()).describe('File IDs that were successfully updated'),
  }))
