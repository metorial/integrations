...
export let manageFolders = SlateTool.create(
  ...
  .input(z.object({
    operation: z.enum(['create', 'delete', 'copy', 'move']).describe('Folder operation to perform'),
    folderName: z.string().optional().describe('Name of the folder to create (for create operation)'),
    parentFolderPath: z.string().optional().describe('Parent folder path for creating a folder, e.g. "/" or "/images/"'),
    folderPath: z.string().optional().describe('Path of the folder (for delete, copy, move operations)'),
    destinationPath: z.string().optional().describe('Destination path (for copy and move operations)'),
    includeVersions: z.boolean().optional().describe('Include file versions when copying a folder'),
  }))
  .output(z.object({
    success: z.boolean().describe('Whether the operation was initiated successfully'),
    jobId: z.string().optional().describe('Bulk job ID for async copy/move operations — use to check status'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.operation === 'delete') {
      if (!ctx.input.folderPath) throw new Error('folderPath is required for delete operation');
    ...
    if (ctx.input.operation === 'copy') {
      ...
      let result = await client.copyFolder(ctx.input.folderPath, ctx.input.destinationPath, ctx.input.includeVersions);
