...
export let getFile = SlateTool.create(
  ...
  .output(z.object({
    ...
    customCoordinates: z.string().optional().nullable().describe('Custom focus area coordinates'),
    customMetadata: z.record(z.string(), z.any()).optional().nullable().describe('Custom metadata'),
    isPublished: z.boolean().optional().describe('Whether the file is published'),
    extensionStatus: z.record(z.string(), z.any()).optional().nullable().describe('Status of applied extensions'),
    versionInfo: z.object({
      versionId: z.string(),
      versionName: z.string(),
    }).optional().nullable().describe('Current version info'),
