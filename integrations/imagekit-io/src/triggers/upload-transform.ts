...
export let uploadTransform = SlateTrigger.create(
  spec,
  {
    name: 'Upload Transformation',
    key: 'upload_transform',
    description: 'Triggered when a pre-upload or post-upload transformation succeeds or fails. Covers both pre-transform and post-transform events for uploaded files.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    phase: z.enum(['pre', 'post']).describe('Whether this was a pre-upload or post-upload transformation'),
    status: z.enum(['success', 'error']).describe('Transformation result status'),
    transformationValue: z.string().optional().describe('The transformation string that was applied'),
    fileId: z.string().optional().describe('File ID of the uploaded asset'),
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      let phase: 'pre' | 'post' = ctx.input.eventType.includes('pre-transform') ? 'pre' : 'post';
      let status: 'success' | 'error' = ctx.input.eventType.includes('.success') ? 'success' : 'error';
