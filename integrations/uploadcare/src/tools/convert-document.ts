...
export let convertDocument = SlateTool.create(
  ...
  .input(z.object({
    ...
    targetFormat: z.enum(['doc', 'docx', 'xls', 'xlsx', 'odt', 'ods', 'rtf', 'txt', 'pdf', 'jpg', 'png']).describe('Target format for conversion'),
    page: z.number().optional().describe('Page number to convert (1-based, for jpg/png output only)'),
    dpi: z.enum(['72', '96', '150', '200', '250', '300', '600']).optional().describe('DPI for image output (for jpg/png only)'),
    quality: z.number().min(10).max(100).optional().describe('Quality for image output (10-100, for jpg/png only)'),
    store: z.boolean().optional().describe('Whether to permanently store the converted file'),
    saveInGroup: z.boolean().optional().describe('Save multi-page conversion as a file group instead of zip (jpg/png only)'),
  }))
  .output(z.object({
    conversionToken: z.number().describe('Token for checking conversion status'),
    resultFileId: z.string().describe('UUID of the result file (may not be ready immediately)'),
    problems: z.record(z.string(), z.string()).describe('Any problems encountered during conversion'),
  }))
...
export let getDocumentConversionStatus = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    conversionToken: z.number().describe('Conversion token returned by the Convert Document tool'),
  }))
  .output(z.object({
    status: z.enum(['pending', 'processing', 'finished', 'failed', 'cancelled']).describe('Current conversion status'),
    error: z.string().nullable().describe('Error message if the conversion failed'),
    resultFileId: z.string().nullable().describe('UUID of the converted file (available when finished)'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client(ctx.auth);
