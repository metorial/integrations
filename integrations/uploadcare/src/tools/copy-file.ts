...
export let copyFile = SlateTool.create(
  spec,
  {
    name: 'Copy File',
    key: 'copy_file',
  ...
  .input(z.object({
    source: z.string().describe('File UUID or CDN URL (with optional transformations) to copy'),
    copyType: z.enum(['local', 'remote']).describe('Whether to create a local copy or remote copy to custom S3 storage'),
    store: z.boolean().optional().describe('Whether to permanently store the local copy'),
    targetStorage: z.string().optional().describe('Target custom S3 storage name (required for remote copies)'),
    makePublic: z.boolean().optional().describe('Whether copied files should be publicly accessible (remote copy only)'),
    pattern: z.string().optional().describe('File naming pattern for the target storage (remote copy only)'),
  }))
  .output(z.object({
    resultType: z.enum(['file', 'url']).describe('Type of result: "file" for local copy, "url" for remote copy'),
