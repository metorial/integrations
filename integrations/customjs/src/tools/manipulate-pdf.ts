...
export let manipulatePdf = SlateTool.create(
  spec,
  {
    name: 'Manipulate PDF',
    key: 'manipulate_pdf',
  ...
  .input(z.object({
    operation: z.enum(['merge', 'extract_pages', 'compress', 'to_text', 'to_png', 'read_form_fields', 'fill_form']).describe('The PDF operation to perform.'),
    pdfUrl: z.string().optional().describe('URL of the PDF file to process. Required for all operations except merge.'),
    pdfUrls: z.array(z.string()).optional().describe('Array of PDF URLs to merge. Required for the merge operation.'),
    pageRange: z.string().optional().describe('Page range to extract, e.g., "1-3" or "5". Required for extract_pages.'),
    formFields: z.record(z.string(), z.string()).optional().describe('Map of form field names to values. Required for fill_form.'),
  }))
  .output(z.object({
    resultBase64: z.string().optional().describe('Base64-encoded binary result (PDF or PNG). Returned for merge, extract_pages, compress, to_png, and fill_form.'),
    text: z.string().optional().describe('Extracted text content. Returned for to_text.'),
    formFieldNames: z.any().optional().describe('Form field names and types. Returned for read_form_fields.'),
  }))
