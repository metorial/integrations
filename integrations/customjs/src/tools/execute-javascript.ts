...
export let executeJavascript = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    code: z.string().describe('JavaScript code to execute. Must contain a `return` statement.'),
    input: z.string().optional().describe('Input data passed to the code as the `input` variable (string). Use JSON.stringify for objects.'),
    returnBinary: z.boolean().optional().default(false).describe('Set to true if the code returns binary data (e.g., files, images).'),
  }))
  .output(z.object({
