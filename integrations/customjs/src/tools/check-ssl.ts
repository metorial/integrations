...
export let checkSsl = SlateTool.create(
  ...
  .input(z.object({
    domain: z.string().describe('Domain name to check, e.g., "example.com". Do not include the protocol (https://).'),
  }))
  .output(z.object({
    certificateDetails: z.any().describe('SSL certificate details including expiration dates, issuer info, and validity status.'),
  }))
