...
export let scaleService = SlateTool.create(
  spec,
  {
    name: 'Scale Service',
    key: 'scale_service',
  ...
  .input(z.object({
    ...
    cpuTarget: z.number().optional().describe('Target CPU utilization percentage (autoscale mode)'),
    memoryTarget: z.number().optional().describe('Target memory utilization percentage (autoscale mode)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (mode === 'manual') {
      if (ctx.input.numInstances === undefined) throw new Error('numInstances is required for manual scaling');
    ...
    if (mode === 'autoscale') {
      ...
      if (ctx.input.cpuTarget !== undefined || ctx.input.memoryTarget !== undefined) {
        config.criteria = {};
        if (ctx.input.cpuTarget !== undefined) config.criteria.cpu = { enabled: true, percentage: ctx.input.cpuTarget };
        if (ctx.input.memoryTarget !== undefined) config.criteria.memory = { enabled: true, percentage: ctx.input.memoryTarget };
      }
