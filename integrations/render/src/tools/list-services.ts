...
let serviceSchema = z.object({
  ...
  type: z.string().describe('Service type (web_service, private_service, background_worker, static_site, cron_job)'),
...
export let listServices = SlateTool.create(
  ...
  .input(z.object({
    ...
    serviceType: z.enum(['web_service', 'private_service', 'background_worker', 'static_site', 'cron_job']).optional().describe('Filter by service type'),
