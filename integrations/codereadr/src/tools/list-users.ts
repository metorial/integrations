...
export let listUsers = SlateTool.create(
  ...
  .input(z.object({
    userId: z.string().optional().describe('Specific user ID to retrieve. Leave empty to retrieve all users.'),
  }))
  .output(z.object({
    users: z.array(z.object({
      ...
      })).optional().describe('Services the user is authorized for'),
    }).passthrough()).describe('List of app users'),
  }))
