...
export let generateBarcode = SlateTool.create(
  ...
  .input(z.object({
    ...
    barcodeType: z.enum(['qr', 'pdf417']).optional().describe('Barcode format. Defaults to "qr".'),
    size: z.string().optional().describe('Barcode size from 1-10 (50px increments)'),
    fileType: z.enum(['jpg', 'png', 'gif']).optional().describe('Image format. Defaults to "gif".'),
    ...
    textAlignment: z.enum(['L', 'C', 'R']).optional().describe('Text alignment: L=left, C=center, R=right'),
    errorCorrection: z.enum(['L', 'M', 'Q', 'H']).optional().describe('Error correction level. L=lowest, H=highest. Defaults to "L".'),
  }))
  .output(z.object({
