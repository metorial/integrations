...
let domainFilterSchema = z.array(
  z.union([
    z.string().describe('Logical operator: "&", "|", or "!"'),
    z.tuple([
      z.string().describe('Field name'),
      z.string().describe('Operator (=, !=, >, <, >=, <=, like, ilike, in, not in, child_of, etc.)'),
      z.unknown().describe('Value to compare against'),
    ]),
  ])
).describe('Odoo domain filter: array of condition tuples [field, operator, value] and optional logical operators ("&", "|", "!")');

let normalizeDomainFilter = (domain: Array<string | [string?, string?, unknown?, ...unknown[]]>): OdooDomainFilter => {
  return domain
    .map((item) => {
...
Common models: **res.partner** (contacts), **crm.lead** (leads/opportunities), **sale.order** (sales orders), **account.move** (invoices/bills), **product.product** (products), **stock.picking** (transfers), **project.task** (tasks), **hr.employee** (employees).`,
  ...
  .input(z.object({
    model: z.string().describe('The Odoo model to search (e.g., "res.partner", "sale.order", "crm.lead")'),
    domain: domainFilterSchema.default([]).describe('Domain filter conditions. Empty array [] returns all records.'),
    fields: z.array(z.string()).optional().describe('Specific field names to return. Omit to return all fields.'),
    limit: z.number().optional().default(50).describe('Maximum number of records to return (default: 50)'),
    offset: z.number().optional().default(0).describe('Number of records to skip for pagination (default: 0)'),
    order: z.string().optional().describe('Sort order (e.g., "name asc", "create_date desc")'),
  }))
  .output(z.object({
    records: z.array(z.record(z.string(), z.unknown())).describe('Array of matching records'),
    count: z.number().describe('Total count of records matching the domain (ignoring limit/offset)'),
  }))
