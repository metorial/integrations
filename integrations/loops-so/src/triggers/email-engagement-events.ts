...
export let emailEngagementEvents = SlateTrigger.create(
  spec,
  {
    name: 'Email Engagement Events',
    key: 'email_engagement_events',
    description: 'Triggers on email engagement activities: delivery, bounces, opens, clicks, unsubscribes, resubscribes, and spam reports.'
  }
)
  ...
  .output(z.object({
    ...
    sourceType: z.enum(['campaign', 'loop', 'transactional']).describe('Type of email that generated this engagement'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let emailData = data.email as { id?: string; emailMessageId?: string; subject?: string } | undefined;
      let contactIdentity = data.contactIdentity as { id?: string; email?: string; userId?: string | null } | undefined;
    ...
    handleEvent: async (ctx) => {
      let { eventName, eventTime, webhookId, contactIdentity, email, sourceType: rawSourceType, campaignId, loopId, transactionalId } = ctx.input;
      ...
      let engagementType = (engagementTypeMap[eventName] || 'delivered') as
        'delivered' | 'soft_bounced' | 'hard_bounced' | 'opened' | 'clicked' | 'unsubscribed' | 'resubscribed' | 'spam_reported';
