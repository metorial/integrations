...
export let manageDatasource = SlateTool.create(
  ...
  .input(z.object({
    ...
    datasourceId: z.string().optional().describe('Data source ID (required for update and delete)'),
    ...
    connector: z.string().optional().describe('Connector type (required for create, e.g., "simple_rest")'),
    ...
    properties: z.record(z.string(), z.any()).optional().describe('Connector-specific properties'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      if (!ctx.input.connector) throw new Error('Connector is required when creating a data source');
    ...
    if (ctx.input.action === 'update') {
      ...
      if (ctx.input.name !== undefined || ctx.input.description !== undefined || ctx.input.refreshInterval !== undefined) {
