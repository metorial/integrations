...
export let getStability = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    releaseStage: z.string().optional().describe('Filter by release stage (e.g., production)'),
  }))
  .output(z.object({
    stabilityTrend: z.any().describe('Stability trend data including release stability scores'),
  }))
