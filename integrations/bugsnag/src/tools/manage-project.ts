...
export let manageProject = SlateTool.create(
  ...
  .input(z.object({
    ...
    type: z.string().optional().describe('Project platform type, e.g. js, ruby, python, android, ios (for create)'),
    releaseStages: z.array(z.string()).optional().describe('Release stages to configure (for update)'),
    globalGrouping: z.array(z.string()).optional().describe('Global grouping rules (for update)'),
    locationGrouping: z.array(z.string()).optional().describe('Location grouping rules (for update)'),
    discardedAppVersions: z.array(z.string()).optional().describe('App versions to discard (for update)'),
    discardedErrors: z.array(z.string()).optional().describe('Error classes to discard (for update)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update') {
      ...
      if (ctx.input.locationGrouping) updateData.location_grouping = ctx.input.locationGrouping;
      if (ctx.input.discardedAppVersions) updateData.discarded_app_versions = ctx.input.discardedAppVersions;
