...
export let shortenUrl = SlateTool.create(
  ...
  .input(z.object({
    ...
    alias: z.string().optional().describe('Custom short link alias (back-half). Must not already be taken.'),
    useBrandedDomain: z.boolean().optional().describe('Use your branded/custom domain instead of cutt.ly. Requires a paid subscription with an approved domain.'),
    publicStats: z.boolean().optional().describe('Make click statistics publicly accessible. Available from Single plan.'),
    noTitle: z.boolean().optional().describe('Disable page title fetching for faster response. Available on Team Enterprise plan only.'),
  }))
  .output(z.object({
