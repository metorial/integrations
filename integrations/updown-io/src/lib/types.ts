...
export let checkSchema = z.object({
  ...
  downSince: z.string().optional().nullable().describe('ISO 8601 timestamp when the site went down'),
  upSince: z.string().optional().nullable().describe('ISO 8601 timestamp when the site came back up'),
  ...
  nextCheckAt: z.string().optional().nullable().describe('ISO 8601 timestamp of next scheduled check'),
  createdAt: z.string().optional().nullable().describe('ISO 8601 timestamp of creation'),
  muteUntil: z.string().optional().nullable().describe('ISO 8601 timestamp until which alerts are muted'),
  faviconUrl: z.string().optional().nullable().describe('URL of the monitored site favicon'),
  customHeaders: z.record(z.string(), z.string()).optional().nullable().describe('Custom HTTP headers sent with checks'),
...
export let downtimeSchema = z.object({
  ...
  endedAt: z.string().optional().nullable().describe('ISO 8601 timestamp when downtime ended (null if ongoing)'),
  duration: z.number().optional().nullable().describe('Duration of downtime in seconds'),
  partial: z.boolean().optional().nullable().describe('Whether downtime was partial (e.g. IPv6-only)'),
});
...
export let metricsRequestsSchema = z.object({
  ...
  byResponseTime: z.record(z.string(), z.number()).optional().nullable().describe('Request count distribution by response time bucket'),
});
...
export let recipientSchema = z.object({
  ...
  type: z.string().describe('Recipient type (email, sms, webhook, slack_compatible, msteams, etc.)'),
  ...
  selected: z.boolean().optional().describe('Whether recipient is selected by default for new checks'),
});
...
export let statusPageSchema = z.object({
  ...
  description: z.string().optional().nullable().describe('Description displayed on the status page'),
  visibility: z.string().optional().nullable().describe('Visibility setting: public, protected, or private'),
  checks: z.array(z.string()).optional().nullable().describe('Ordered list of check tokens displayed on the page'),
});
