...
export let updateCheck = SlateTool.create(
  ...
  .input(z.object({
    ...
    type: z.enum(['https', 'http', 'icmp', 'tcp', 'tcps', 'pulse']).optional().describe('Protocol type'),
    ...
    muteUntil: z.string().optional().nullable().describe('Mute alerts until this timestamp, "recovery", or "forever"'),
  }))
  .output(checkSchema)
