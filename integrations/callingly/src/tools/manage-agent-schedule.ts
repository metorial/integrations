...
let dayScheduleSchema = z.object({
  ...
  times: z.array(timeWindowSchema).optional().describe('Time windows when the agent is available'),
});

export let manageAgentSchedule = SlateTool.create(
  spec,
  {
...
- To **update** the schedule, provide the agentId and the schedule array.`,
  ...
  .input(z.object({
    ...
    schedule: z.array(dayScheduleSchema).optional().describe('Weekly schedule to set. Omit to retrieve current schedule.'),
  }))
  .output(z.object({
    agentId: z.string().describe('ID of the agent'),
    schedule: z.array(z.object({
      ...
      })).optional().describe('Time windows'),
