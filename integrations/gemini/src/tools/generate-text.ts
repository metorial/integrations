...
let partSchema = z.union([
  z.object({
    text: z.string().describe('Text content'),
  }),
  z.object({
    inlineData: z.object({
      mimeType: z.string().describe('MIME type of the data (e.g. "image/png", "audio/mp3")'),
  ...
  z.object({
    fileData: z.object({
      ...
      fileUri: z.string().describe('URI of the file (from File API or Google Cloud Storage)'),
...
export let generateText = SlateTool.create(
  ...
  .input(z.object({
    model: z.string().describe('Model ID to use (e.g. "gemini-2.0-flash", "gemini-1.5-pro", "gemini-2.5-pro-preview-06-05")'),
    messages: z.array(messageSchema).describe('Conversation messages with role and content parts'),
    systemInstruction: z.string().optional().describe('System instruction text to guide model behavior'),
    temperature: z.number().min(0).max(2).optional().describe('Controls randomness. Lower values are more deterministic.'),
    ...
    stopSequences: z.array(z.string()).optional().describe('Sequences that will stop generation'),
    responseMimeType: z.enum(['text/plain', 'application/json']).optional().describe('MIME type for the response format'),
    responseSchema: z.any().optional().describe('JSON schema to enforce on the output when responseMimeType is "application/json"'),
    candidateCount: z.number().min(1).max(8).optional().describe('Number of response candidates to generate'),
    safetySettings: z.array(safetySettingSchema).optional().describe('Safety filter settings'),
    cachedContentName: z.string().optional().describe('Resource name of cached content to reuse (e.g. "cachedContents/abc123")'),
    enableCodeExecution: z.boolean().optional().describe('Enable the built-in code execution tool'),
    enableGoogleSearch: z.boolean().optional().describe('Enable Google Search grounding'),
    thinkingBudget: z.number().optional().describe('Token budget for model thinking/reasoning (for models that support dynamic thinking)'),
  }))
  .output(z.object({
    text: z.string().nullable().describe('Generated text content'),
    finishReason: z.string().nullable().describe('Why generation stopped (e.g. "STOP", "MAX_TOKENS", "SAFETY")'),
    safetyRatings: z.array(z.object({
      category: z.string(),
      probability: z.string(),
    })).optional().describe('Safety ratings for the response'),
    citationSources: z.array(z.object({
      startIndex: z.number().optional(),
      endIndex: z.number().optional(),
      uri: z.string().optional(),
    })).optional().describe('Citation sources if grounding was used'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.temperature !== undefined) generationConfig.temperature = ctx.input.temperature;
    if (ctx.input.maxOutputTokens !== undefined) generationConfig.maxOutputTokens = ctx.input.maxOutputTokens;
    ...
    if (ctx.input.responseMimeType) generationConfig.responseMimeType = ctx.input.responseMimeType;
