...
export let generateImage = SlateTool.create(
  ...
  .input(z.object({
    model: z.string().describe('Model ID for image generation (e.g. "gemini-2.0-flash-exp", "imagen-3.0-generate-002")'),
    prompt: z.string().describe('Text prompt describing the image to generate or edits to make'),
    referenceImageBase64: z.string().optional().describe('Base64-encoded reference image for editing tasks'),
    referenceImageMimeType: z.string().optional().describe('MIME type of the reference image (e.g. "image/png", "image/jpeg")'),
    responseMimeType: z.enum(['image/png', 'image/jpeg']).optional().describe('Desired output image format'),
    numberOfImages: z.number().min(1).max(4).optional().describe('Number of images to generate'),
    aspectRatio: z.string().optional().describe('Aspect ratio (e.g. "1:1", "16:9", "4:3", "3:4", "9:16")'),
    negativePrompt: z.string().optional().describe('Description of what to avoid in the generated image (Imagen models only)'),
  }))
  .output(z.object({
