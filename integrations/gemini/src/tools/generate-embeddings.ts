...
export let generateEmbeddings = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'Use "gemini-embedding-exp-03-07" or "text-embedding-004" as the model.',
  ...
  .input(z.object({
    model: z.string().describe('Embedding model ID (e.g. "text-embedding-004", "gemini-embedding-exp-03-07")'),
    texts: z.array(z.string()).min(1).describe('Array of text strings to generate embeddings for'),
    taskType: z.enum([
      'RETRIEVAL_QUERY',
      'RETRIEVAL_DOCUMENT',
      'SEMANTIC_SIMILARITY',
      ...
      'QUESTION_ANSWERING',
      'FACT_VERIFICATION',
    ]).optional().describe('Task type to optimize the embeddings for'),
    title: z.string().optional().describe('Title for the content (only used with RETRIEVAL_DOCUMENT task type)'),
    outputDimensionality: z.number().optional().describe('Desired embedding dimension. Smaller dimensions reduce storage cost.'),
  }))
  .output(z.object({
