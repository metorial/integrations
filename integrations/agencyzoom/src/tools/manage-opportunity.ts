...
export let manageOpportunity = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'delete']).describe('Action to perform on the opportunity'),
    opportunityId: z.string().optional().describe('ID of the opportunity (required for "update" and "delete" actions)'),
    carrier: z.string().optional().describe('Carrier name or ID for the opportunity'),
    productLine: z.string().optional().describe('Product line name or ID (e.g. "Auto", "Home")'),
    ...
    }).optional().describe('Property address details for the opportunity'),
  }))
  .output(z.object({
    opportunity: z.record(z.string(), z.any()).optional().describe('Opportunity data (for "create" and "update" actions)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'create': {
        ...
        if (ctx.input.propertyAddress !== undefined) data.propertyAddress = ctx.input.propertyAddress;
