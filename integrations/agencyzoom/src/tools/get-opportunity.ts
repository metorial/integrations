...
export let getOpportunity = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeDrivers: z.boolean().optional().describe('Whether to include associated drivers in the response'),
    includeVehicles: z.boolean().optional().describe('Whether to include associated vehicles in the response'),
  }))
  .output(z.object({
    opportunity: z.record(z.string(), z.any()).describe('Opportunity detail data'),
    drivers: z.array(z.record(z.string(), z.any())).optional().describe('Array of drivers associated with the opportunity'),
    vehicles: z.array(z.record(z.string(), z.any())).optional().describe('Array of vehicles associated with the opportunity'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includeDrivers) {
      ...
      drivers = Array.isArray(driversResult) ? driversResult : (driversResult.data ?? driversResult.drivers ?? []);
    }

    if (ctx.input.includeVehicles) {
      let vehiclesResult = await client.getOpportunityVehicles(ctx.input.opportunityId);
      vehicles = Array.isArray(vehiclesResult) ? vehiclesResult : (vehiclesResult.data ?? vehiclesResult.vehicles ?? []);
    }
