...
export let addCustomerNote = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    note: z.record(z.string(), z.any()).describe('The newly created note record with all fields'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let preview = ctx.input.note.length > 80 ? ctx.input.note.substring(0, 80) + '...' : ctx.input.note;
