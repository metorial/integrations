...
export let searchLeads = SlateTool.create(
  ...
  .input(z.object({
    search: z.string().optional().describe('Free-text search query to match against lead name, email, phone, etc.'),
    status: z.enum(['new', 'contacted', 'quoted', 'won', 'lost', 'x-dated']).optional().describe('Filter leads by their current status'),
    producerId: z.string().optional().describe('Filter leads assigned to a specific producer by their ID'),
    leadSourceId: z.string().optional().describe('Filter leads by their lead source ID'),
    pipelineId: z.string().optional().describe('Filter leads belonging to a specific pipeline by ID'),
    stageId: z.string().optional().describe('Filter leads at a specific pipeline stage by ID'),
    fromDate: z.string().optional().describe('Start date for filtering leads by creation date (ISO 8601 format, e.g. "2024-01-01")'),
    toDate: z.string().optional().describe('End date for filtering leads by creation date (ISO 8601 format, e.g. "2024-12-31")'),
    offset: z.number().optional().describe('Number of records to skip for pagination (default 0)'),
    limit: z.number().optional().describe('Maximum number of leads to return (default varies by API)'),
  }))
  .output(z.object({
    leads: z.array(z.object({
      ...
      createdAt: z.string().optional().describe('ISO 8601 timestamp when the lead was created'),
