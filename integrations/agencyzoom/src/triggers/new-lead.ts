...
export let newLeadTrigger = SlateTrigger.create(
  spec,
  {
    name: 'New Lead',
    key: 'new_lead',
    description: 'Triggers when a new lead is created in AgencyZoom. Polls for recently created leads.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let leads = Array.isArray(result) ? result : (result?.data || result?.leads || result?.items || []);
