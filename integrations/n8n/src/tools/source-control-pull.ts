...
export let sourceControlPull = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    autoPublish: z.enum(['none', 'all', 'published']).optional().describe('Auto-publish setting: none (do not publish), all (publish all pulled workflows), or published (only publish previously published workflows)'),
    variables: z.record(z.string(), z.string()).optional().describe('Variable overrides as key-value pairs')
  }))
  .output(z.object({
    pullResult: z.any().describe('Import result including affected workflows, credentials, tags, and variables')
  }))
