...
export let createSubscription = SlateTool.create(
  ...
  .input(z.object({
    ...
    startTime: z.string().optional().describe('Subscription start time in ISO 8601 format. Defaults to now.'),
    customId: z.string().optional().describe('Custom ID to track the subscription in your system'),
    returnUrl: z.string().optional().describe('URL to redirect subscriber after approval'),
    cancelUrl: z.string().optional().describe('URL to redirect subscriber after cancellation'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.subscriberEmail || ctx.input.subscriberFirstName || ctx.input.subscriberLastName) {
      subscriber = {};
      ...
      if (ctx.input.subscriberFirstName || ctx.input.subscriberLastName) {
        subscriber.name = {};
        if (ctx.input.subscriberFirstName) subscriber.name.given_name = ctx.input.subscriberFirstName;
        if (ctx.input.subscriberLastName) subscriber.name.surname = ctx.input.subscriberLastName;
      }
    }
