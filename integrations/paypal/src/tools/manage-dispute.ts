...
export let manageDispute = SlateTool.create(
  spec,
  {
    name: 'Manage Dispute',
    key: 'manage_dispute',
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'acceptClaim', 'provideEvidence', 'escalate']).describe('Action to perform'),
    disputeId: z.string().optional().describe('Dispute ID (required for get/accept/evidence/escalate)'),
    startTime: z.string().optional().describe('Filter disputes from this ISO 8601 time (for list)'),
    disputeState: z.string().optional().describe('Filter by state: REQUIRED_ACTION, UNDER_REVIEW, RESOLVED, OTHER (for list)'),
    transactionId: z.string().optional().describe('Filter by disputed transaction ID (for list)'),
    pageSize: z.number().optional().describe('Number of results per page'),
    note: z.string().optional().describe('Note for accept/escalate actions'),
    acceptClaimReason: z.string().optional().describe('Reason for accepting: DID_NOT_SHIP, TOO_TIME_CONSUMING, LOST_IN_MAIL, NOT_RELEVANT (for acceptClaim)'),
    evidenceType: z.string().optional().describe('Evidence type for provideEvidence (e.g. PROOF_OF_FULFILLMENT)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'acceptClaim': {
        if (!ctx.input.disputeId) throw new Error('disputeId is required for acceptClaim action');
      ...
      case 'provideEvidence': {
        if (!ctx.input.disputeId) throw new Error('disputeId is required for provideEvidence action');
      ...
      case 'escalate': {
        ...
        await client.escalateDispute(ctx.input.disputeId, ctx.input.note || 'Escalated by integration');
        return {
          output: {
            disputeId: ctx.input.disputeId,
            status: 'UNDER_REVIEW',
