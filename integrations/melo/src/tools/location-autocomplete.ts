...
export let locationAutocomplete = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    excludedCityIds: z.array(z.string()).optional().describe('City IRIs to exclude from results'),
    excludedDepartmentIds: z.array(z.string()).optional().describe('Department IRIs to exclude from results'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.excludedDepartmentIds) params['excludedDepartmentsIds[]'] = ctx.input.excludedDepartmentIds;
