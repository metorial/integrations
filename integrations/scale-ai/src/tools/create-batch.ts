...
export let createBatch = SlateTool.create(
  ...
  .input(z.object({
    ...
    callbackUrl: z.string().optional().describe('URL or email for batch completion notification'),
    calibrationBatch: z.boolean().optional().describe('Whether this is a calibration batch (Rapid projects only)'),
    selfLabelBatch: z.boolean().optional().describe('Whether this is a self-label batch (Rapid projects only)'),
  }))
  .output(z.object({
