...
export let createProject = SlateTool.create(
  ...
  .input(z.object({
    ...
    taskType: z.string().describe('Task type for all tasks in this project (e.g., imageannotation, textcollection, documenttranscription, videoannotation, lidarannotation)'),
    ...
    defaultParams: z.record(z.string(), z.any()).optional().describe('Default parameters inherited by tasks created under this project'),
    pipeline: z.enum(['standard_task', 'consensus_task']).optional().describe('Pipeline type (Studio projects only)'),
    consensusAttempts: z.number().optional().describe('Number of consensus attempts (Studio consensus projects only)'),
  }))
  .output(z.object({
