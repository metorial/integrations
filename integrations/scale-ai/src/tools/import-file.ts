...
export let importFile = SlateTool.create(
  ...
  .input(z.object({
    ...
    projectName: z.string().optional().describe('Project to associate the file with (required for Scale Rapid)'),
  }))
  .output(z.object({
    attachmentUrl: z.string().optional().describe('Scale-hosted attachment URL (scaledata://) for use in task creation'),
  }).passthrough())
