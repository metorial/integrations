...
export let taskStatusChanged = SlateTrigger.create(
  spec,
  {
    name: 'Task Status Changed',
    key: 'task_status_changed',
    description: '[Polling fallback] Polls for tasks that have been updated since the last check. Detects task completions, cancellations, and errors across a project or batch.',
    instructions: [
  ...
  .output(z.object({
    taskId: z.string().describe('ID of the updated task'),
    status: z.string().describe('Current task status (pending, completed, canceled, error)'),
    taskType: z.string().optional().describe('Type of the task'),
