...
let recipientSchema = z.object({
  ...
  country: z.string().optional().describe('ISO 3166-1 Alpha-2 country code (e.g. US, GB, CA)'),
...
export let sendPostcard = SlateTool.create(
  ...
  .input(z.object({
    ...
    front: z.string().optional().describe('URL or base64-encoded image for the postcard front (JPG/PDF)'),
    back: z.string().optional().describe('URL or base64-encoded image for the postcard back (JPG/PDF)'),
    message: z.string().optional().describe('Text message for the back of the postcard'),
    templateId: z.number().optional().describe('Pre-configured template ID to use instead of front/back images'),
    ...
    test: z.boolean().optional().describe('Generate a sample PDF without sending or charging'),
    postUnverified: z.boolean().optional().describe('Send even if address cannot be verified (defaults to true)'),
  }))
  .output(z.object({
