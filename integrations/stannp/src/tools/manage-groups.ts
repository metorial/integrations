...
export let deleteGroup = SlateTool.create(
  ...
  .input(z.object({
    ...
    deleteRecipients: z.boolean().optional().describe('Also permanently delete all recipients in the group'),
  }))
  .output(z.object({
...
export let addRecipientsToGroup = SlateTool.create(
  spec,
  {
...
export let removeRecipientsFromGroup = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    recipientIds: z.array(z.number()).describe('List of recipient IDs to remove from the group'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.removeRecipientsFromGroup(ctx.input.groupId, ctx.input.recipientIds);
