...
export let listQueries = SlateTool.create(
  spec,
  {
    name: 'List Queries',
    key: 'list_queries',
  ...
  .input(z.object({
    ...
    status: z.enum(['waiting', 'in progress', 'ready']).optional().describe('Filter by query analysis status'),
    source: z.enum(['neuron', 'neuron-api']).optional().describe('Filter by creation source'),
    createdAfter: z.string().optional().describe('Filter queries created after this ISO 8601 date'),
