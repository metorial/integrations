...
export let getLimits = SlateTool.create(
  spec,
  {
...
Useful for checking how many tests you can still run before hitting the hourly limit.`,
  ...
  .output(
    z.object({
      ...
      creditsRemaining: z.number().nullable().optional().describe('Remaining credits (authenticated users only)'),
    })
  )
  .handleInvocation(async (ctx) => {
    ...
    let create = (rateLimit?.measurements as Record<string, unknown> | undefined)?.create as Record<string, unknown> | undefined;

    let creditsRemaining: number | null | undefined = undefined;
    if (data.credits && typeof data.credits === 'object' && 'remaining' in (data.credits as Record<string, unknown>)) {
