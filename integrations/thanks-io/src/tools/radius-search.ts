...
export let radiusSearch = SlateTool.create(
  spec,
  {
    name: 'Radius Search',
    key: 'radius_search',
...
Results are added to an existing mailing list or a newly created one. Costs $0.05 per record.`,
    constraints: [
      'Each record costs $0.05.',
      'Maximum of 10,000 records per search.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    ...
    recordCount: z.number().min(1).max(10000).describe('Number of records to return (1-10000)'),
    mailingListId: z.number().optional().describe('Existing mailing list ID to add results to (creates new list if omitted)'),
    recordTypes: z.enum([
      'all', 'likelytomove', 'likelytorefi', 'absenteeowner', 'highnetworth',
      'majorityhomeequity', 'homefreeclear', 'underwater', 'kidsinhousehold',
      'newhomeowner', 'firsttimehomebuyer', 'renters', 'retiring', 'retired',
      'pool', 'onlybusinesses', 'newbusiness'
    ]).optional().default('all').describe('Type of records to search for'),
    ...
    usePropertyOwner: z.boolean().optional().describe('Use property owner name instead of resident'),
    includeSearchAddress: z.boolean().optional().describe('Include the center address in results'),
  }))
  .output(z.object({
    mailingListId: z.number().optional().describe('Mailing list ID where results were added'),
