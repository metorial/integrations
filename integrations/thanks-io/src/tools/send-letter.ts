...
Set **preview** to true to generate a visual preview without placing an order.`,
  ...
  .input(z.object({
    ...
    frontImageUrl: z.string().optional().describe('URL of the background image for the letter'),
    imageTemplateId: z.number().optional().describe('ID of a saved image template'),
    message: z.string().optional().describe('Handwritten message (supports %FIRST_NAME%, etc.)'),
    ...
    pdfOnlyUrl: z.string().optional().describe('URL to a complete PDF (skips cover letter generation)'),
