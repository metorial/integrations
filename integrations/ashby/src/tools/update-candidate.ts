...
export let updateCandidateTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    emailType: z.enum(['Personal', 'Work', 'Other']).optional().describe('Type of email address'),
    phone: z.string().optional().describe('Phone number to set as primary'),
    phoneType: z.enum(['Personal', 'Work', 'Other', 'Mobile']).optional().describe('Type of phone number'),
    socialLinks: z.array(z.object({
      type: z.string().describe('Social link type (e.g., LinkedIn, GitHub, Twitter)'),
      url: z.string().describe('Social link URL'),
    })).optional().describe('Social links to set on the candidate profile'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { candidateId, name, email, emailType, phone, phoneType, socialLinks, tagId, note, projectId } = ctx.input;

    // Update profile fields if any are provided
    if (name !== undefined || email !== undefined || phone !== undefined || socialLinks !== undefined) {
    ...
    let candidateName = candidate.name || [candidate.firstName, candidate.lastName].filter(Boolean).join(' ') || 'Unknown';
