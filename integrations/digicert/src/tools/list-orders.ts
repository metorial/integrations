...
let orderSchema = z.object({
  ...
  status: z.string().describe('Order status (e.g., "issued", "pending", "rejected", "revoked", "expired")'),
...
export let listOrders = SlateTool.create(
  ...
  .input(z.object({
    status: z.enum(['pending', 'rejected', 'processing', 'issued', 'revoked', 'canceled', 'needs_csr', 'needs_approval', 'expired', 'reissue_pending', 'reissue_revoked']).optional().describe('Filter orders by status'),
    productNameId: z.string().optional().describe('Filter by product name ID (e.g., "ssl_plus", "ssl_ev_plus")'),
    commonName: z.string().optional().describe('Filter by common name (domain)'),
    offset: z.number().optional().describe('Pagination offset (default 0)'),
    limit: z.number().optional().describe('Number of results to return (default 25, max 1000)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.commonName) params.filters = { ...params.filters, common_name: ctx.input.commonName };
