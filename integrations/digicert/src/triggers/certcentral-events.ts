...
export let certcentralEvents = SlateTrigger.create(
  spec,
  {
    name: 'CertCentral Events',
    key: 'certcentral_events',
    description: 'Receive webhook events from DigiCert CertCentral including certificate issuance, revocation, order rejection, and domain/organization validation changes.',
  }
)
  .input(z.object({
    eventType: z.string().describe('Event type (e.g., "certificate_issued", "certificate_revoked", "order_rejected", "organization_validated", "domain_validated")'),
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      // Organization events
      if (eventType === 'organization_validated' || eventType === 'organization_expired' || eventType === 'organization_revalidation_notice') {
        ...
        output.status = eventType === 'organization_validated' ? 'validated' : eventType === 'organization_expired' ? 'expired' : 'revalidation_needed';
        output.dateCreated = payload.date;
      }

      // Domain events
      if (eventType === 'domain_validated' || eventType === 'domain_expired' || eventType === 'domain_revalidation_notice') {
        output.domainId = payload.domain_id || payload.domain?.id;
        output.domainName = payload.domain_name || payload.domain?.name;
        output.validationType = payload.validation_type;
        output.validUntil = payload.validated_until || payload.expires_at;
        output.status = eventType === 'domain_validated' ? 'validated' : eventType === 'domain_expired' ? 'expired' : 'revalidation_needed';
        output.dateCreated = payload.date;
      }
