...
export let createPod = SlateTool.create(
  spec,
  {
    name: 'Create Pod',
    key: 'create_pod',
  ...
  .input(z.object({
    ...
    imageName: z.string().describe('Container image to use, e.g. "runpod/pytorch:2.1.0-py3.10-cuda11.8.0-devel-ubuntu22.04"'),
    cloudType: z.enum(['SECURE', 'COMMUNITY']).optional().describe('Cloud type: SECURE (default) or COMMUNITY'),
    computeType: z.enum(['GPU', 'CPU']).optional().describe('Compute type: GPU (default) or CPU'),
    gpuTypeIds: z.array(z.string()).optional().describe('Acceptable GPU model IDs, e.g. ["NVIDIA A100-SXM4-80GB"]'),
    ...
    containerDiskInGb: z.number().optional().describe('Container disk size in GB (default: 50)'),
    ...
    ports: z.array(z.string()).optional().describe('Exposed ports, e.g. ["8888/http", "22/tcp"]'),
    env: z.record(z.string(), z.string()).optional().describe('Environment variables as key-value pairs'),
    interruptible: z.boolean().optional().describe('Use spot/interruptible pricing (default: false)'),
