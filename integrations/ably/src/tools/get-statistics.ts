...
export let getStatistics = SlateTool.create(
  ...
  .input(z.object({
    ...
    limit: z.number().optional().describe('Maximum number of stat records to return (default: 100, max: 1000)'),
    ...
    unit: z.enum(['minute', 'hour', 'day', 'month']).optional().describe('Aggregation unit for statistics (default: "minute")'),
  }))
  .output(z.object({
    statistics: z.array(z.any()).describe('Array of statistics records with message counts, connections, API requests, etc.'),
  }))
