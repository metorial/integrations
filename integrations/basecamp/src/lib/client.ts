...
export class Client {
  ...
  async listTodoLists(projectId: string, todoSetId: string, params?: { status?: string; page?: number }): Promise<any[]> {
    let query: Record<string, string> = {};
    if (params?.status) query.status = params.status;
    if (params?.page) query.page = String(params.page);
    let response = await this.axios.get(`/buckets/${projectId}/todosets/${todoSetId}/todolists.json`, { params: query });
    return response.data;
  }
  ...
  async createTodoList(projectId: string, todoSetId: string, data: { name: string; description?: string }): Promise<any> {
    let response = await this.axios.post(`/buckets/${projectId}/todosets/${todoSetId}/todolists.json`, data);
    return response.data;
  }

  async updateTodoList(projectId: string, todoListId: string, data: { name: string; description?: string }): Promise<any> {
    let response = await this.axios.put(`/buckets/${projectId}/todolists/${todoListId}.json`, data);
    return response.data;
  }

  // ---- To-dos ----

  async listTodos(projectId: string, todoListId: string, params?: { status?: string; completed?: boolean; page?: number }): Promise<any[]> {
    let query: Record<string, string> = {};
    if (params?.status) query.status = params.status;
    if (params?.completed !== undefined) query.completed = String(params.completed);
    if (params?.page) query.page = String(params.page);
    let response = await this.axios.get(`/buckets/${projectId}/todolists/${todoListId}/todos.json`, { params: query });
    return response.data;
  }
  ...
  async createTodo(projectId: string, todoListId: string, data: {
    content: string;
    description?: string;
    assigneeIds?: number[];
    ...
    if (data.completionSubscriberIds) body.completion_subscriber_ids = data.completionSubscriberIds;
    ...
    let response = await this.axios.post(`/buckets/${projectId}/todolists/${todoListId}/todos.json`, body);
    return response.data;
  }

  async updateTodo(projectId: string, todoId: string, data: {
    content?: string;
    description?: string;
    assigneeIds?: number[];
    completionSubscriberIds?: number[];
    ...
    if (data.completionSubscriberIds !== undefined) body.completion_subscriber_ids = data.completionSubscriberIds;
    if (data.notify !== undefined) body.notify = data.notify;
  ...
  async getMessageBoard(projectId: string, messageBoardId: string): Promise<any> {
    let response = await this.axios.get(`/buckets/${projectId}/message_boards/${messageBoardId}.json`);
    return response.data;
  }

  // ---- Messages ----

  async listMessages(projectId: string, messageBoardId: string, params?: { page?: number }): Promise<any[]> {
    let query: Record<string, string> = {};
    if (params?.page) query.page = String(params.page);
    let response = await this.axios.get(`/buckets/${projectId}/message_boards/${messageBoardId}/messages.json`, { params: query });
    return response.data;
  }
  ...
  async createMessage(projectId: string, messageBoardId: string, data: {
    subject: string;
    content?: string;
    categoryId?: number;
  }): Promise<any> {
    ...
    let response = await this.axios.post(`/buckets/${projectId}/message_boards/${messageBoardId}/messages.json`, body);
    return response.data;
  }

  async updateMessage(projectId: string, messageId: string, data: {
    subject?: string;
    content?: string;
    categoryId?: number;
  }): Promise<any> {
    ...
    if (data.content !== undefined) body.content = data.content;
    if (data.categoryId !== undefined) body.category_id = data.categoryId;
    let response = await this.axios.put(`/buckets/${projectId}/messages/${messageId}.json`, body);
    return response.data;
  }

  // ---- Comments ----

  async listComments(projectId: string, recordingId: string, params?: { page?: number }): Promise<any[]> {
    let query: Record<string, string> = {};
    if (params?.page) query.page = String(params.page);
    let response = await this.axios.get(`/buckets/${projectId}/recordings/${recordingId}/comments.json`, { params: query });
    return response.data;
  }
  ...
  async createComment(projectId: string, recordingId: string, data: { content: string }): Promise<any> {
    let response = await this.axios.post(`/buckets/${projectId}/recordings/${recordingId}/comments.json`, {
      content: data.content,
    });
    return response.data;
  }

  async updateComment(projectId: string, commentId: string, data: { content: string }): Promise<any> {
  ...
  async listCampfireLines(projectId: string, chatId: string, params?: { page?: number }): Promise<any[]> {
    let query: Record<string, string> = {};
    if (params?.page) query.page = String(params.page);
    let response = await this.axios.get(`/buckets/${projectId}/chats/${chatId}/lines.json`, { params: query });
    return response.data;
  }

  async createCampfireLine(projectId: string, chatId: string, data: { content: string }): Promise<any> {
  ...
  async listScheduleEntries(projectId: string, scheduleId: string, params?: { status?: string; page?: number }): Promise<any[]> {
    let query: Record<string, string> = {};
    if (params?.status) query.status = params.status;
    if (params?.page) query.page = String(params.page);
    let response = await this.axios.get(`/buckets/${projectId}/schedules/${scheduleId}/entries.json`, { params: query });
    return response.data;
  }

  async getScheduleEntry(projectId: string, entryId: string): Promise<any> {
    let response = await this.axios.get(`/buckets/${projectId}/schedule_entries/${entryId}.json`);
    return response.data;
  }

  async createScheduleEntry(projectId: string, scheduleId: string, data: {
    summary: string;
    startsAt: string;
    endsAt: string;
    description?: string;
    ...
    if (data.notify !== undefined) body.notify = data.notify;
    let response = await this.axios.post(`/buckets/${projectId}/schedules/${scheduleId}/entries.json`, body);
    return response.data;
  }

  async updateScheduleEntry(projectId: string, entryId: string, data: {
    summary?: string;
    startsAt?: string;
    endsAt?: string;
    description?: string;
    ...
    if (data.allDay !== undefined) body.all_day = data.allDay;
    if (data.notify !== undefined) body.notify = data.notify;
    let response = await this.axios.put(`/buckets/${projectId}/schedule_entries/${entryId}.json`, body);
    return response.data;
  }
  ...
  async trashRecording(projectId: string, recordingId: string): Promise<void> {
    await this.axios.put(`/buckets/${projectId}/recordings/${recordingId}/status/trashed.json`);
  }

  async archiveRecording(projectId: string, recordingId: string): Promise<void> {
    await this.axios.put(`/buckets/${projectId}/recordings/${recordingId}/status/archived.json`);
  }
  ...
  async createWebhook(projectId: string, data: { payloadUrl: string; types?: string[] }): Promise<any> {
    let body: Record<string, any> = {
  ...
  async updateWebhook(projectId: string, webhookId: string, data: { payloadUrl?: string; types?: string[]; active?: boolean }): Promise<any> {
    let body: Record<string, any> = {};
    ...
    let response = await this.axios.put(`/buckets/${projectId}/webhooks/${webhookId}.json`, body);
    return response.data;
  }
