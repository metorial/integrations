...
To **update**, provide the \`projectId\`, \`commentId\`, and new \`content\`.`,
  ...
  .input(z.object({
    ...
    recordingId: z.string().optional().describe('ID of the recording to comment on (required for create)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      if (!ctx.input.recordingId) throw new Error('recordingId is required for creating a comment');
