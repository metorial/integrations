...
export let messageReceived = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggered when a new incoming message is received from a contact on any channel (WhatsApp, Instagram, Messenger, Telegram, Live Chat).',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    channel: z.string().optional().describe('Messaging channel (whatsapp, instagram, messenger, telegram, livechat)'),
    ...
    messageType: z.string().optional().describe('Message type (text, image, document, etc.)'),
