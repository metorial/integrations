...
export let manageClassParticipants = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    status: z.enum(['BOOKED', 'WAITLISTED']).optional().describe('Participant status (for add, defaults to BOOKED)'),
    reason: z.string().optional().describe('Cancellation reason (for cancel)'),
  }))
  .output(z.object({
    success: z.boolean().describe('Whether the operation succeeded'),
    participants: z.array(z.record(z.string(), z.any())).optional().describe('List of participants (for list action)'),
    participant: z.record(z.string(), z.any()).optional().describe('Added participant record'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'list') {
      ...
      let participants = Array.isArray(result) ? result : (result.participants || result.data || []);
      return {
