...
export let getEodPrices = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    exchange: z.string().optional().describe('Filter by stock exchange MIC code (e.g. "XNAS" for NASDAQ)'),
    sort: z.enum(['ASC', 'DESC']).optional().describe('Sort order by date. Defaults to DESC'),
    latest: z.boolean().optional().describe('If true, returns only the most recent end-of-day prices'),
