...
export let manageFollowing = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['follow', 'unfollow', 'check', 'getFollowedArtists', 'followPlaylist', 'unfollowPlaylist']).describe('Following operation to perform'),
    type: z.enum(['artist', 'user']).optional().describe('Entity type for follow/unfollow/check operations'),
    ids: z.array(z.string()).optional().describe('Spotify IDs of artists or users'),
    playlistId: z.string().optional().describe('Spotify playlist ID (for followPlaylist/unfollowPlaylist)'),
    isPublic: z.boolean().optional().describe('Whether to publicly show that the user follows the playlist'),
    limit: z.number().min(1).max(50).optional().describe('Max results for getFollowedArtists (default 20)'),
    after: z.string().optional().describe('Cursor for pagination of followed artists'),
  }))
  .output(z.object({
    followedArtists: z.array(z.object({
      artistId: z.string(),
      name: z.string(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'follow') {
      ...
      if (!ctx.input.ids || ctx.input.ids.length === 0) throw new Error('ids is required for "follow" action');
    ...
    if (action === 'unfollow') {
      ...
      if (!ctx.input.ids || ctx.input.ids.length === 0) throw new Error('ids is required for "unfollow" action');
    ...
    if (action === 'check') {
      ...
      if (!ctx.input.ids || ctx.input.ids.length === 0) throw new Error('ids is required for "check" action');
    ...
    if (action === 'followPlaylist') {
      if (!ctx.input.playlistId) throw new Error('playlistId is required for "followPlaylist" action');
    ...
    if (action === 'unfollowPlaylist') {
      if (!ctx.input.playlistId) throw new Error('playlistId is required for "unfollowPlaylist" action');
