...
export let controlPlayback = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum([
      'getState', 'getCurrentlyPlaying', 'getDevices', 'getQueue',
      'play', 'pause', 'next', 'previous',
      'seek', 'setVolume', 'setShuffle', 'setRepeat',
      'transferPlayback', 'addToQueue',
    ...
    offsetPosition: z.number().optional().describe('Zero-based position in context to start playing'),
    offsetUri: z.string().optional().describe('Spotify URI within context to start playing from'),
    ...
    play: z.boolean().optional().describe('Whether to start playing immediately after transfer'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'getState' || action === 'getCurrentlyPlaying') {
      ...
      let trackName = state.item ? `**${state.item.name}** by ${state.item.artists.map(a => a.name).join(', ')}` : 'nothing';
      return {
    ...
    if (action === 'getQueue') {
      ...
      let queue = {
        currentlyPlaying: result.currently_playing ? {
          ...
          artists: result.currently_playing.artists.map(a => ({ artistId: a.id, name: a.name })),
    ...
    if (action === 'play') {
      ...
      if (ctx.input.offsetPosition !== undefined) offset = { position: ctx.input.offsetPosition };
    ...
    if (action === 'seek') {
      if (ctx.input.positionMs === undefined) throw new Error('positionMs is required for "seek" action');
      await client.seekToPosition(ctx.input.positionMs, ctx.input.deviceId);
      return { output: { success: true }, message: `Seeked to ${Math.round(ctx.input.positionMs / 1000)}s.` };
    }

    if (action === 'setVolume') {
      if (ctx.input.volumePercent === undefined) throw new Error('volumePercent is required for "setVolume" action');
      await client.setVolume(ctx.input.volumePercent, ctx.input.deviceId);
      return { output: { success: true }, message: `Volume set to ${ctx.input.volumePercent}%.` };
    }

    if (action === 'setShuffle') {
      if (ctx.input.shuffle === undefined) throw new Error('shuffle is required for "setShuffle" action');
      await client.toggleShuffle(ctx.input.shuffle, ctx.input.deviceId);
      return { output: { success: true }, message: `Shuffle ${ctx.input.shuffle ? 'enabled' : 'disabled'}.` };
    }

    if (action === 'setRepeat') {
      if (!ctx.input.repeatMode) throw new Error('repeatMode is required for "setRepeat" action');
      await client.setRepeatMode(ctx.input.repeatMode, ctx.input.deviceId);
      return { output: { success: true }, message: `Repeat mode set to ${ctx.input.repeatMode}.` };
    }

    if (action === 'transferPlayback') {
      if (!ctx.input.deviceId) throw new Error('deviceId is required for "transferPlayback" action');
