...
export let getArtist = SlateTool.create(
  spec,
  {
    name: 'Get Artist',
    key: 'get_artist',
  ...
  .input(z.object({
    ...
    include: z.array(z.enum(['topTracks', 'albums', 'relatedArtists'])).optional().describe('Additional data to include beyond basic artist info'),
    albumTypes: z.string().optional().describe('Comma-separated album types to filter: album, single, appears_on, compilation'),
    market: z.string().optional().describe('ISO 3166-1 alpha-2 country code'),
    albumLimit: z.number().min(1).max(50).optional().describe('Max albums to return (default 20)'),
  }))
  .output(z.object({
    artistId: z.string(),
    name: z.string(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let topTracksResult = includes.includes('topTracks')
      ? (await client.getArtistTopTracks(ctx.input.artistId, ctx.input.market)).tracks.map(t => ({
          trackId: t.id,
          name: t.name,
