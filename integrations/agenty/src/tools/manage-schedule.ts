...
export let manageSchedule = SlateTool.create(
  ...
  .input(z.object({
    ...
    cronExpression: z.string().optional().describe('CRON expression for scheduling. Required when action is "create".'),
    enabled: z.boolean().optional().describe('Whether the schedule is enabled. Defaults to true. Used when action is "create".'),
  }))
  .output(z.object({
    statusCode: z.number().optional().describe('HTTP status code.'),
    message: z.string().optional().describe('Status message.'),
    schedule: z.any().optional().nullable().describe('Schedule details when using "get" action.'),
  }))
