...
export let manageAgentInput = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['get', 'update']).describe('"get" to retrieve current input, "update" to modify it.'),
    inputType: z.enum(['manual', 'list', 'agent']).optional().describe('Type of input source. Required when action is "update".'),
    urls: z.array(z.string()).optional().describe('Array of URLs to scrape. Used when inputType is "manual".'),
    sourceId: z.string().optional().describe('List ID or source agent ID. Used when inputType is "list" or "agent".'),
    field: z.string().optional().describe('Column/field name to use as URL source from the list or agent output.'),
