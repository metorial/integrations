...
export let listJobs = SlateTool.create(
  ...
  .input(z.object({
    agentId: z.string().optional().describe('Filter jobs by a specific agent ID. If omitted, returns jobs from all agents.'),
    offset: z.number().optional().describe('Number of jobs to skip for pagination. Defaults to 0.'),
    limit: z.number().optional().describe('Maximum number of jobs to return.'),
  }))
  .output(z.object({
    total: z.number().optional().nullable().describe('Total number of jobs matching the query.'),
    ...
    jobs: z.array(z.object({
      ...
      status: z.string().describe('Job status (e.g. "completed", "running", "failed", "stopped").'),
