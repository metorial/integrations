...
let inputSchema = z
  .object({
    ...
    email: z.string().email().optional().describe('Contact email to update instead of using an ID'),
    name: z.string().optional().describe('New display name for the record'),
    fields: z.record(z.string(), z.any()).optional().describe('System and custom fields to update'),
  ...
  .refine((value) => value.name !== undefined || value.fields !== undefined || value.profileImageUrl !== undefined, {
...
export let updateObject = SlateTool.create(
  spec,
  {
    name: 'Update Object',
    key: 'update_object',
    description: 'Update one or more fields on an existing item record. Supports both system fields and custom fields, leaving unspecified fields unchanged.',
  }
)
  .input(inputSchema)
  .output(z.object({
