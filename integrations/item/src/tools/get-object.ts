...
let inputSchema = z
  .object({
    ...
    includeAllFields: z.boolean().optional().default(false).describe('Include all system fields in the response'),
    includeSummary: z.boolean().optional().default(false).describe('Include the AI summary for contacts or companies when available'),
  })
...
export let getObject = SlateTool.create(
  spec,
  {
    name: 'Get Object',
    key: 'get_object',
    description: 'Fetch a single item record by ID, or by email for contacts. Useful for retrieving the full flattened record with all available fields.',
    tags: {
      readOnly: true,
    },
  }
