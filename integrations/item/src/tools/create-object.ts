...
export let createObject = SlateTool.create(
  spec,
  {
    name: 'Create Object',
    key: 'create_object',
    description: 'Create a new contact, company, or custom object record in item. Contacts and companies may deduplicate automatically based on item matching rules.',
  }
)
  .input(z.object({
    ...
    fields: z.record(z.string(), z.any()).optional().describe('Additional system and custom fields to set on the record'),
    profileImageUrl: z.string().url().optional().describe('Avatar URL for contacts or logo URL for companies'),
  }))
  .output(z.object({
    objectRecord: z.record(z.string(), z.any()).describe('The created or deduplicated record'),
  }))
