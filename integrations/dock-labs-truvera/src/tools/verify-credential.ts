...
Returns a verification status with a boolean result.`,
  ...
  .input(z.object({
    document: z.record(z.string(), z.unknown()).describe('The Verifiable Credential or Verifiable Presentation JSON-LD document to verify'),
  }))
  .output(z.object({
    verified: z.boolean().describe('Whether the document passed verification'),
    results: z.array(z.record(z.string(), z.unknown())).optional().describe('Detailed verification results'),
  }))
