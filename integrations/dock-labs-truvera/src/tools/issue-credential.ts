...
Optionally reference a schema for structure enforcement and a design template for visual presentation.`,
  ...
  .input(z.object({
    ...
    credentialType: z.array(z.string()).optional().describe('Credential type(s), e.g. ["UniversityDegreeCredential"]'),
    subjectClaims: z.record(z.string(), z.unknown()).describe('Claims/attributes about the subject to include in the credential'),
    issuanceDate: z.string().optional().describe('ISO 8601 issuance date. Defaults to current time'),
    expirationDate: z.string().optional().describe('ISO 8601 expiration date for the credential'),
    ...
    anchor: z.boolean().optional().describe('Anchor the credential hash on the Dock blockchain'),
    persist: z.boolean().optional().describe('Store the credential encrypted on Dock servers'),
    password: z.string().optional().describe('Password for encrypting the persisted credential'),
    distribute: z.boolean().optional().describe('Send the credential to the recipient'),
    recipientEmail: z.string().optional().describe('Email address to send the credential to'),
    schemaUri: z.string().optional().describe('URI of a credential schema to enforce structure'),
    templateId: z.string().optional().describe('ID of a certificate design template to apply'),
    context: z.array(z.string()).optional().describe('Additional JSON-LD context URIs'),
    status: z.record(z.string(), z.unknown()).optional().describe('Credential status for revocation. Include registry and type information'),
  }))
  .output(z.object({
    credentialId: z.string().optional().describe('Unique identifier for the issued credential'),
    credential: z.record(z.string(), z.unknown()).describe('The full issued Verifiable Credential document'),
  }))
