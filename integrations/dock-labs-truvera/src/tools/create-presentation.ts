...
export let createPresentation = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    credentials: z.array(z.record(z.string(), z.unknown())).describe('Array of Verifiable Credential JSON-LD documents to include'),
  ...
  .output(z.object({
    presentation: z.record(z.string(), z.unknown()).describe('The signed Verifiable Presentation document'),
  }))
