...
export let manageDataBreach = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['create', 'get', 'list', 'update', 'delete', 'evaluate']).describe('Action to perform'),
    ...
    discoveryDate: z.string().optional().describe('Date when the breach was discovered (ISO 8601)'),
    notificationDate: z.string().optional().describe('Date authorities/subjects were notified (ISO 8601)'),
    ...
    isDraft: z.boolean().optional().describe('Save evaluation as draft (for evaluate action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (action) {
      ...
      case 'update': {
        ...
        if (ctx.input.breachDate !== undefined) updatePayload.breachDate = ctx.input.breachDate;
        if (ctx.input.discoveryDate !== undefined) updatePayload.discoveryDate = ctx.input.discoveryDate;
        if (ctx.input.notificationDate !== undefined) updatePayload.notificationDate = ctx.input.notificationDate;
        if (ctx.input.affectedDataSubjects !== undefined) updatePayload.affectedDataSubjects = ctx.input.affectedDataSubjects;
        if (ctx.input.affectedDataTypes !== undefined) updatePayload.affectedDataTypes = ctx.input.affectedDataTypes;
        if (ctx.input.breachType !== undefined) updatePayload.breachType = ctx.input.breachType;
