...
export let paymentNotification = SlateTrigger.create(
  spec,
  {
    name: 'Payment Notification',
    key: 'payment_notification',
    description: 'Receive webhook notifications when payments are made, payouts are sent, or transfers occur. Covers all payment lifecycle events including completed, partial, pending, and payout statuses.'
  }
)
  .input(z.object({
    paymentId: z.string().describe('Unique payment identifier'),
    amount: z.string().describe('Payment amount'),
    paymentMethod: z.string().describe('Payment method used (e.g., "solana", "bitcoin", "paypal")'),
    currency: z.string().describe('Fiat currency (e.g., "usd")'),
    status: z.string().describe('Payment status (e.g., "yes", "pending", "payout", "paid_partial")'),
    ...
    }).optional().describe('Shipping address if collected'),
