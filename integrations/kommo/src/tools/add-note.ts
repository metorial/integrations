...
export let addNoteTool = SlateTool.create(
  spec,
  {
    name: 'Add Note',
    key: 'add_note',
  ...
  .input(z.object({
    entityType: z.enum(['leads', 'contacts', 'companies']).describe('Entity type to add the note to'),
    entityId: z.number().describe('ID of the entity to add the note to'),
    noteType: z.enum([
      'common', 'call_in', 'call_out', 'sms_in', 'sms_out',
      'service_message', 'extended_service_message', 'geolocation',
    ]).describe('Type of note'),
    noteParams: z.record(z.string(), z.any()).describe('Note parameters specific to the note type (e.g., { text: "..." } for common notes, { phone: "...", duration: 60, source: "..." } for call notes)'),
  }))
  .output(z.object({
