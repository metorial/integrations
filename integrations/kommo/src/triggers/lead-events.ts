...
export let leadEventsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Lead Events',
    key: 'lead_events',
    description: 'Triggers when a lead is added, updated, deleted, restored, changes status, changes responsible user, or receives a note.',
  }
)
  .input(z.object({
      ...
      for (let [webhookKey, eventType] of Object.entries(eventTypes)) {
        ...
        for (let item of items) {
          inputs.push({
            ...
            oldPipelineId: item.old_pipeline_id != null ? Number(item.old_pipeline_id) : undefined,
