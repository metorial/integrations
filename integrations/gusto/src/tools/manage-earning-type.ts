...
export let manageEarningType = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    defaultEarningTypes: z.array(z.object({
      earningTypeId: z.string().describe('UUID of the earning type'),
      name: z.string().optional().describe('Name'),
    })).optional().describe('Default earning types (for list action)'),
    customEarningTypes: z.array(z.object({
      earningTypeId: z.string().describe('UUID of the earning type'),
      name: z.string().optional().describe('Name'),
    })).optional().describe('Custom earning types'),
    earningType: z.object({
      earningTypeId: z.string().describe('UUID of the earning type'),
      name: z.string().optional().describe('Name'),
    }).optional().describe('Created or updated earning type'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'update': {
        ...
        let result = await client.updateEarningType(ctx.input.companyId, ctx.input.earningTypeId, {
          name: ctx.input.name,
        });
        return {
          output: {
