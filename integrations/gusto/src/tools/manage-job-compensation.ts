...
export let manageJobCompensation = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list_jobs', 'create_job', 'update_job', 'list_compensations', 'create_compensation', 'update_compensation'])
      .describe('The action to perform'),
    employeeId: z.string().optional().describe('Employee UUID (required for list_jobs/create_job)'),
    jobId: z.string().optional().describe('Job UUID (required for update_job/list_compensations/create_compensation)'),
    compensationId: z.string().optional().describe('Compensation UUID (required for update_compensation)'),
    ...
    paymentUnit: z.string().optional().describe('Payment unit (Hour, Week, Month, Year, Paycheck)'),
    flsaStatus: z.string().optional().describe('FLSA status (Exempt, Salaried Nonexempt, Nonexempt, Owner)'),
    effectiveDate: z.string().optional().describe('Effective date for the compensation change (YYYY-MM-DD)'),
  }))
  .output(z.object({
    jobs: z.array(z.object({
      ...
      currentCompensationRate: z.string().optional().describe('Current compensation rate'),
      currentPaymentUnit: z.string().optional().describe('Current payment unit'),
    })).optional().describe('List of jobs'),
    job: z.object({
      jobId: z.string().describe('UUID of the job'),
      title: z.string().optional().describe('Job title'),
      version: z.string().optional().describe('Resource version'),
    }).optional().describe('Created or updated job'),
    ...
    })).optional().describe('List of compensations'),
    compensation: z.object({
      ...
      effectiveDate: z.string().optional().describe('Effective date'),
      version: z.string().optional().describe('Resource version'),
    }).optional().describe('Created or updated compensation'),
  }))
