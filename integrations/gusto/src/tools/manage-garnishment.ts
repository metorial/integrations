...
export let manageGarnishment = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    recurringChildSupport: z.boolean().optional().describe('Whether this is recurring child support'),
  ...
  .output(z.object({
    garnishments: z.array(z.object({
      ...
      courtOrdered: z.boolean().optional().describe('Whether court-ordered'),
    })).optional().describe('List of garnishments (for list action)'),
    garnishment: z.object({
      garnishmentId: z.string().describe('UUID of the garnishment'),
      description: z.string().optional().describe('Description'),
      active: z.boolean().optional().describe('Whether active'),
      amount: z.number().optional().describe('Amount per pay period'),
    ...
    }).optional().describe('Single garnishment (for create/update)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'update': {
        ...
        if (ctx.input.annualMaximum !== undefined) data.annual_maximum = ctx.input.annualMaximum;
        if (ctx.input.payPeriodMaximum !== undefined) data.pay_period_maximum = ctx.input.payPeriodMaximum;
