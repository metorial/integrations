...
export let manageRankTracker = SlateTool.create(
  spec,
  {
...
Management endpoints are free (no API unit cost).`,
  ...
  .input(z.object({
    ...
    projectId: z.string().optional().describe('Project ID (required for delete-project, list-keywords, add-keywords, remove-keywords)'),
    projectName: z.string().optional().describe('Name for the new project (required for create-project)'),
    target: z.string().optional().describe('Target domain for the new project (required for create-project)'),
    keywords: z.array(z.string()).optional().describe('Keywords to add/remove or initial keywords for new project'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'list-projects':
        ...
        break;
      case 'create-project':
        if (!ctx.input.projectName) throw new Error('projectName is required for create-project');
      ...
      case 'add-keywords':
        ...
        if (!ctx.input.keywords || ctx.input.keywords.length === 0) throw new Error('keywords are required for add-keywords');
        result = await client.addRankTrackerKeywords({ projectId: ctx.input.projectId, keywords: ctx.input.keywords });
        break;
      case 'remove-keywords':
        if (!ctx.input.projectId) throw new Error('projectId is required for remove-keywords');
        if (!ctx.input.keywords || ctx.input.keywords.length === 0) throw new Error('keywords are required for remove-keywords');
        result = await client.removeRankTrackerKeywords({ projectId: ctx.input.projectId, keywords: ctx.input.keywords });
        break;
    }
