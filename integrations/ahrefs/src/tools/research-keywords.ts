...
export let researchKeywords = SlateTool.create(
  spec,
  {
...
Use to evaluate keyword potential, assess ranking difficulty, and compare multiple keywords at once.`,
  ...
  .input(z.object({
    ...
    keywords: z.array(z.string()).optional().describe('List of keywords to research (for bulk lookup)'),
    country: z.string().optional().describe('Two-letter country code for localized results (e.g., "us", "uk"). Defaults to "us".'),
    select: z.string().optional().describe('Comma-separated list of fields to return'),
  }))
  .output(z.object({
    overview: z.any().describe('Keyword metrics including volume, difficulty, CPC, and SERP features'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let keywordDisplay = ctx.input.keyword || (ctx.input.keywords ? ctx.input.keywords.join(', ') : 'keywords');
    return {
      output: {
        overview: result,
      },
