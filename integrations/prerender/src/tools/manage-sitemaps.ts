...
export let manageSitemaps = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'update', 'delete', 'crawl']).describe('The operation to perform.'),
    sitemapUrl: z.string().optional().describe('The XML sitemap URL. Required for `create`.'),
    sitemapId: z.string().optional().describe('The sitemap ID. Required for `update`, `delete`, and `crawl`.'),
    revisitInterval: z.enum(['weekly', 'daily', 'hourly']).optional().describe('How often Prerender should recheck the sitemap. Used in `create` and `update`.'),
    adaptiveType: z.enum(['desktop', 'mobile']).optional().describe('Rendering type for sitemap URLs. Used in `create` and `update`.'),
    page: z.number().optional().describe('Page number for pagination when listing sitemaps.'),
    pageSize: z.number().optional().describe('Number of results per page when listing sitemaps.'),
  }))
  .output(z.object({
