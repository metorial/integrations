...
export let listInvoices = SlateTool.create(
  ...
  .input(z.object({
    status: z.enum(['sent', 'draft', 'overdue', 'paid', 'void', 'unpaid', 'partially_paid', 'viewed']).optional().describe('Filter by invoice status'),
    ...
    sortColumn: z.enum(['customer_name', 'invoice_number', 'date', 'due_date', 'total', 'balance', 'created_time']).optional().describe('Column to sort results by'),
    page: z.number().optional().default(1).describe('Page number for pagination (starts at 1)'),
    perPage: z.number().optional().default(200).describe('Number of invoices per page (max 200)'),
  }))
  .output(z.object({
