...
Supports pagination via **page** and **perPage** parameters.`,
  ...
  .input(z.object({
    status: z.enum(['open', 'closed', 'void']).optional().describe('Filter credit notes by status'),
    customerName: z.string().optional().describe('Filter credit notes by customer name'),
    creditNoteNumber: z.string().optional().describe('Filter credit notes by credit note number'),
    ...
    sortColumn: z.string().optional().describe('Column to sort results by (e.g. date, total, balance, customer_name)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.creditNoteNumber !== undefined) params.creditnote_number = ctx.input.creditNoteNumber;
