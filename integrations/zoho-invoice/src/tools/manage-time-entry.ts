...
export let manageTimeEntry = SlateTool.create(
  ...
  .input(z.object({
    timeEntryId: z.string().optional().describe('ID of the time entry to update. If omitted, a new time entry is created.'),
    projectId: z.string().optional().describe('ID of the project (required when creating)'),
    taskId: z.string().optional().describe('ID of the task within the project (required when creating)'),
    userId: z.string().optional().describe('ID of the user logging time (required when creating)'),
    logDate: z.string().optional().describe('Date for the time entry in yyyy-mm-dd format (required when creating)'),
    ...
    notes: z.string().max(500).optional().describe('Notes for the time entry (max 500 characters)'),
  }))
  .output(z.object({
