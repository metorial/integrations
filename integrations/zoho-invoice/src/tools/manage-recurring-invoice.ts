...
export let manageRecurringInvoice = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    recurringInvoiceId: z.string().optional().describe('ID of the recurring invoice to update. If omitted, a new recurring invoice is created.'),
    customerId: z.string().optional().describe('Customer ID for the recurring invoice (required when creating)'),
    recurrenceName: z.string().optional().describe('Name for the recurring invoice profile (required when creating)'),
    recurrenceFrequency: z.enum(['weekly', 'biweekly', 'monthly', 'bimonthly', 'quarterly', 'half_yearly', 'yearly', 'every_2_years']).optional().describe('How often the invoice recurs (required when creating)'),
    startDate: z.string().optional().describe('Start date for recurrence in yyyy-mm-dd format'),
    ...
    lineItems: z.array(z.object({
      ...
      discount: z.union([z.string(), z.number()]).optional().describe('Discount amount or percentage for this line item'),
    })).optional().describe('Array of line items for the recurring invoice'),
    paymentTerms: z.number().optional().describe('Payment terms in days (e.g. 15, 30, 60)'),
    discount: z.union([z.string(), z.number()]).optional().describe('Discount to apply to the invoice total'),
    discountType: z.enum(['entity_level', 'item_level']).optional().describe('Whether the discount applies at the entity or item level'),
  }))
  .output(z.object({
    ...
    nextInvoiceDate: z.string().optional().describe('Date when the next invoice will be generated'),
    total: z.number().optional().describe('Total amount of the recurring invoice'),
    createdTime: z.string().optional().describe('Timestamp when the recurring invoice was created'),
    lastModifiedTime: z.string().optional().describe('Timestamp when the recurring invoice was last modified'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.recurrenceFrequency) payload.recurrence_frequency = ctx.input.recurrenceFrequency;
