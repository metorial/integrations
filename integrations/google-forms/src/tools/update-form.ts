...
Accepts an array of update requests that are applied atomically. Each request can be one of: **updateFormInfo**, **updateSettings**, **createItem**, **updateItem**, **moveItem**, or **deleteItem**.`,
  ...
  .input(z.object({
    ...
    requests: z.array(z.object({
      updateFormInfo: z.object({
        ...
        updateMask: z.string().describe('Comma-separated field paths to update, e.g. "title", "description", or "title,description"'),
      }).optional().describe('Update the form title or description'),
      updateSettings: z.object({
        settings: z.object({
          quizSettings: z.object({
            isQuiz: z.boolean().optional().describe('Whether the form should be a quiz'),
          }).optional().describe('Quiz-specific settings'),
        }).describe('Settings to update'),
        updateMask: z.string().describe('Comma-separated field paths to update, e.g. "quizSettings.isQuiz"'),
      }).optional().describe('Update form settings such as quiz mode'),
      createItem: z.object({
        item: z.any().describe('The item to create. Must include the appropriate kind (questionItem, pageBreakItem, textItem, imageItem, videoItem, questionGroupItem)'),
        location: z.object({
          index: z.number().describe('Zero-based index where the item should be inserted'),
        }).describe('Where to insert the item'),
      }).optional().describe('Add a new item (question, section, text, image, video) to the form'),
      updateItem: z.object({
      ...
      }).optional().describe('Update an existing item on the form'),
      moveItem: z.object({
      ...
      }).optional().describe('Move an item to a different position'),
      deleteItem: z.object({
        location: z.object({
          index: z.number().describe('Zero-based index of the item to delete'),
        }).describe('Location of the item to delete'),
      }).optional().describe('Delete an item from the form'),
    })).describe('Array of update operations to apply'),
    includeFormInResponse: z.boolean().optional().describe('If true, the full updated form is returned in the response'),
  }))
  .output(z.object({
    formId: z.string().describe('ID of the updated form'),
    replies: z.array(z.any()).optional().describe('Replies for each request (e.g., created item IDs)'),
    updatedForm: z.any().optional().describe('The full updated form object, if includeFormInResponse was true'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let operationSummary = ctx.input.requests.map((req) => {
