...
let responseSummarySchema = z.object({
  ...
  lastSubmittedTime: z.string().optional().describe('Timestamp of most recent submission (ISO 8601)'),
...
Supports pagination and timestamp filtering to retrieve only recent submissions.`,
  ...
  .input(z.object({
    ...
    submittedAfter: z.string().optional().describe('Only return responses submitted after this timestamp (ISO 8601 format, e.g. "2024-01-01T00:00:00Z")'),
    pageSize: z.number().optional().describe('Maximum number of responses to return per page (max 5000)'),
  ...
  .output(z.object({
    ...
    nextPageToken: z.string().optional().describe('Token to use for retrieving the next page'),
  }))
