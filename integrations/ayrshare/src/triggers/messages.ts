...
export let messagesTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Message Activity',
    key: 'message_activity',
    description: 'Triggered on direct messaging activity for Facebook and Instagram. Covers new messages, message read receipts, and reactions on messages. Requires the Messaging Add-On.',
  },
)
  .input(z.object({
    action: z.string().describe('Webhook action type'),
    subAction: z.string().optional().describe('Sub-action: messageCreated, messageRead, reactionCreated, reactionDeleted'),
    ...
    senderDetails: z.record(z.string(), z.unknown()).optional().describe('Sender information'),
  ...
  .output(z.object({
    subAction: z.string().optional().describe('Sub-action type: messageCreated, messageRead, reactionCreated, reactionDeleted'),
