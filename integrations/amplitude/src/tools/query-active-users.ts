...
export let queryActiveUsersTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    metric: z.enum(['active', 'new', 'paying', 'power']).optional().describe('Metric to retrieve. Defaults to active users.'),
    interval: z.number().optional().describe('Time interval: -300000 (real-time), 1 (daily), 7 (weekly), 30 (monthly). Default is 1.'),
    segment: z.string().optional().describe('Segment definition as JSON to filter users.'),
    groupBy: z.string().optional().describe('User property to group results by (e.g., "country", "platform").')
  }))
  .output(z.object({
