...
export let updateBot = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'Widget settings are cached for up to 5 minutes on CDN after updating.',
    ],
  }
)
  .input(z.object({
    ...
    customPrompt: z.string().optional().describe('Custom system prompt instructions for the bot'),
    ...
    allowedDomains: z.array(z.string()).optional().describe('Hostnames allowed to embed the widget (empty array = all domains)'),
    color: z.string().optional().describe('Hex color for widget (e.g. "#1292EE")'),
    icon: z.enum(['default', 'comments', 'robot', 'life-ring', 'question', 'book']).optional().describe('Widget icon'),
    ...
    supportLink: z.string().optional().describe('URL that appears after bot response for support'),
    showCopyButton: z.boolean().optional().describe('Display copy-to-clipboard button on answers'),
    ...
    }).optional().describe('Custom UI label overrides'),
  }))
  .output(z.object({
