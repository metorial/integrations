...
export let listEnvelopes = SlateTool.create(
  spec,
  {
    name: 'List Envelopes',
    key: 'list_envelopes',
...
Use **fromDate** to specify the start of the search window (required unless envelopeIds are provided).`,
  ...
  .input(z.object({
    fromDate: z.string().optional().describe('Start date for the search (ISO 8601 format, e.g., "2024-01-01T00:00:00Z"). Required unless envelopeIds is provided.'),
    toDate: z.string().optional().describe('End date for the search (ISO 8601 format)'),
    status: z.string().optional().describe('Comma-separated envelope statuses to filter (e.g., "completed", "sent,delivered")'),
    fromToStatus: z.string().optional().describe('Status type for date filtering: changed, created, sent, delivered, signed, completed, declined, voided. Default: changed'),
    searchText: z.string().optional().describe('Text to search for in envelope data'),
    envelopeIds: z.string().optional().describe('Comma-separated list of specific envelope IDs to retrieve'),
    count: z.number().optional().default(25).describe('Number of results to return (max 1000)'),
    startPosition: z.number().optional().describe('Starting position for pagination'),
    orderBy: z.enum(['created', 'last_modified', 'status']).optional().describe('Field to sort results by'),
    order: z.enum(['asc', 'desc']).optional().default('desc').describe('Sort direction'),
  }))
  .output(z.object({
    totalSetSize: z.string().optional().describe('Total number of envelopes matching the query'),
