...
export let listTemplates = SlateTool.create(
  ...
  .input(z.object({
    ...
    sharedByMe: z.boolean().optional().describe('Filter to templates shared by the current user'),
  }))
  .output(z.object({
    totalSetSize: z.string().optional().describe('Total number of templates matching the query'),
