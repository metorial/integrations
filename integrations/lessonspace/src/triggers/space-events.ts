...
export let spaceEvents = SlateTrigger.create(
  spec,
  {
    name: 'Space Events',
    key: 'space_events',
    description: 'Listens for real-time events from Lessonspace spaces including session lifecycle, user activity, chat messages, co-browsing, transcription completion, and summary completion.',
  }
)
  .input(z.object({
    eventType: z.string().describe('The type of webhook event (e.g., session.start, user.join).'),
  ...
  .output(z.object({
    webhookEvent: z.string().describe('The webhook event type header (e.g., session.start, user.join, chat.message).'),
    webhookId: z.string().describe('Unique identifier for this webhook delivery.'),
    timestamp: z.string().describe('ISO 8601 timestamp of when the event occurred.'),
    sessionId: z.string().optional().describe('Session ID associated with the event, if available.'),
    roomId: z.string().optional().describe('Space/room ID associated with the event, if available.'),
    userId: z.string().optional().describe('User ID associated with the event, if applicable.'),
    userName: z.string().optional().describe('User name associated with the event, if applicable.'),
    isGuest: z.boolean().optional().describe('Whether the user is a guest, if applicable.'),
    messageContent: z.string().optional().describe('Chat message content, if this is a chat.message event.'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let timestamp = ctx.request.headers.get('x-webhook-timestamp') || new Date().toISOString();
