...
export let manageEmailIdentity = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['create', 'get', 'delete', 'list', 'configureDkim', 'configureMailFrom', 'configureFeedback']).describe('Operation to perform'),
    emailIdentity: z.string().optional().describe('Email address or domain (required for all except "list")'),
    configurationSetName: z.string().optional().describe('Configuration set to associate with the identity'),
    dkimSigningEnabled: z.boolean().optional().describe('Enable or disable DKIM signing (for "configureDkim")'),
    mailFromDomain: z.string().optional().describe('Custom MAIL FROM domain (for "configureMailFrom")'),
    behaviorOnMxFailure: z.enum(['USE_DEFAULT_VALUE', 'REJECT_MESSAGE']).optional().describe('Behavior when MX lookup fails'),
    emailForwardingEnabled: z.boolean().optional().describe('Enable feedback forwarding (for "configureFeedback")'),
  ...
  .output(z.object({
    ...
    }).optional().describe('DKIM configuration details'),
    ...
    }).optional().describe('MAIL FROM configuration'),
    ...
    })).optional().describe('List of identities'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'configureDkim') {
      await client.putEmailIdentityDkimAttributes(ctx.input.emailIdentity!, ctx.input.dkimSigningEnabled ?? true);
      return {
        output: { identityName: ctx.input.emailIdentity },
    ...
    if (action === 'configureFeedback') {
      await client.putEmailIdentityFeedbackAttributes(ctx.input.emailIdentity!, ctx.input.emailForwardingEnabled ?? true);
      return {
        output: { identityName: ctx.input.emailIdentity },
