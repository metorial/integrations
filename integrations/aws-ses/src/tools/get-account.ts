...
export let getAccount = SlateTool.create(
  ...
  .input(z.object({}))
  .output(z.object({
    dedicatedIpAutoWarmupEnabled: z.boolean().describe('Whether automatic IP warmup is enabled'),
    enforcementStatus: z.string().describe('Account enforcement status (HEALTHY, PROBATION, SHUTDOWN)'),
    productionAccessEnabled: z.boolean().describe('Whether the account has left the SES sandbox'),
    ...
    }).optional().describe('Account suppression settings'),
    vdmAttributes: z.object({
      vdmEnabled: z.string(),
    ...
    }).optional().describe('Virtual Deliverability Manager settings'),
  }))
