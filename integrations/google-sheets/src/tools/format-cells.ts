...
let borderStyleSchema = z.object({
  style: z.enum(['DOTTED', 'DASHED', 'SOLID', 'SOLID_MEDIUM', 'SOLID_THICK', 'DOUBLE', 'NONE']).optional().describe('Border line style'),
  color: colorSchema.describe('Border color'),
}).optional();

export let formatCells = SlateTool.create(
  spec,
  {
    name: 'Format Cells',
    key: 'format_cells',
  ...
  .input(z.object({
    ...
    wrapStrategy: z.enum(['OVERFLOW_CELL', 'LEGACY_WRAP', 'CLIP', 'WRAP']).optional().describe('How text wrapping is handled'),
    numberFormatType: z.enum(['TEXT', 'NUMBER', 'PERCENT', 'CURRENCY', 'DATE', 'TIME', 'DATE_TIME', 'SCIENTIFIC']).optional().describe('Number format type'),
    numberFormatPattern: z.string().optional().describe('Custom number format pattern (e.g., "#,##0.00", "yyyy-mm-dd")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (input.bold !== undefined) { textFormat.bold = input.bold; textFields.push('bold'); }
    if (input.italic !== undefined) { textFormat.italic = input.italic; textFields.push('italic'); }
    if (input.strikethrough !== undefined) { textFormat.strikethrough = input.strikethrough; textFields.push('strikethrough'); }
    if (input.underline !== undefined) { textFormat.underline = input.underline; textFields.push('underline'); }
    if (input.fontSize !== undefined) { textFormat.fontSize = input.fontSize; textFields.push('fontSize'); }
    if (input.fontFamily !== undefined) { textFormat.fontFamily = input.fontFamily; textFields.push('fontFamily'); }
    ...
    if (input.topBorder) { borders.top = buildBorder(input.topBorder); hasBorders = true; }
    if (input.bottomBorder) { borders.bottom = buildBorder(input.bottomBorder); hasBorders = true; }
    if (input.leftBorder) { borders.left = buildBorder(input.leftBorder); hasBorders = true; }
    if (input.rightBorder) { borders.right = buildBorder(input.rightBorder); hasBorders = true; }
