...
export let batchUpdate = SlateTool.create(
  spec,
  {
    name: 'Batch Update',
    key: 'batch_update',
...
Use this for complex updates that need to be applied together, or when you need to perform operations not covered by other tools (e.g., conditional formatting rules, merge cells, add charts, pivot tables, filter views).`,
  ...
  .input(z.object({
    ...
    requests: z.array(z.record(z.string(), z.any())).describe('Array of batch update request objects. Each object should have a single key identifying the operation type.'),
  }))
  .output(z.object({
