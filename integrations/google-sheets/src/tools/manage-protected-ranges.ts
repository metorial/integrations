...
export let manageProtectedRanges = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    protectedRangeId: z.number().optional().describe('ID of the protected range to delete (required for delete)'),
    sheetId: z.number().optional().describe('Sheet ID to protect (required for add)'),
    startRowIndex: z.number().optional().describe('Start row index of the range to protect (0-based, inclusive). Omit to protect entire sheet.'),
    ...
    warningOnly: z.boolean().optional().describe('If true, show a warning instead of preventing edits. Defaults to false.'),
    editors: z.array(z.string()).optional().describe('Email addresses of users who can edit the protected range'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (input.action === 'add') {
      if (input.sheetId === undefined) throw new Error('sheetId is required for adding protection');
    ...
    if (input.action === 'delete') {
      if (input.protectedRangeId === undefined) throw new Error('protectedRangeId is required for deleting protection');
