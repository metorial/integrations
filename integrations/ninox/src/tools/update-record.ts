...
export let updateRecord = SlateTool.create(
  ...
  .input(z.object({
    ...
    fields: z.record(z.string(), z.any()).describe('Fields to update, keyed by field name. Only specified fields are changed.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let updated = await client.updateRecord(ctx.input.teamId, ctx.input.databaseId, ctx.input.tableId, ctx.input.recordId, ctx.input.fields);
