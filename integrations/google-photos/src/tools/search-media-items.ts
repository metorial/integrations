...
let dateSchema = z.object({
  ...
  month: z.number().min(0).max(12).optional().describe('Month (1-12). Use 0 to match any month.'),
  day: z.number().min(0).max(31).optional().describe('Day of month (1-31). Use 0 to match any day.'),
});

export let searchMediaItems = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    albumId: z.string().optional().describe('Filter by album ID (cannot be used with other filters)'),
    pageSize: z.number().min(1).max(100).optional().describe('Maximum results to return (1-100, default 25)'),
    ...
    })).max(5).optional().describe('Filter by date ranges (max 5)'),
    contentCategories: z.array(z.enum([
      ...
      'ARTS', 'CRAFTS', 'FASHION', 'HOUSES', 'GARDENS', 'FLOWERS',
      'HOLIDAYS',
    ])).optional().describe('Filter by content categories'),
    mediaTypes: z.array(z.enum(['ALL_MEDIA', 'VIDEO', 'PHOTO'])).optional().describe('Filter by media type'),
    onlyFavorites: z.boolean().optional().describe('If true, only return favorited items'),
    orderBy: z.enum(['MediaMetadata.creation_time', 'MediaMetadata.creation_time desc']).optional().describe('Sort order (only works with date filters)'),
  }))
  .output(z.object({
