...
export let updateAlbum = SlateTool.create(
  spec,
  {
    name: 'Update Album',
    key: 'update_album',
  ...
  .input(z.object({
    ...
    title: z.string().max(500).optional().describe('New title for the album (max 500 characters)'),
    coverPhotoMediaItemId: z.string().optional().describe('Media item ID to set as the album cover photo'),
  }))
  .output(z.object({
