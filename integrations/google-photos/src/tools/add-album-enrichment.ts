...
export let addAlbumEnrichment = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    location: locationSchema.optional().describe('Location details for a location enrichment'),
    mapOrigin: locationSchema.optional().describe('Origin location for a map enrichment'),
    mapDestination: locationSchema.optional().describe('Destination location for a map enrichment'),
    position: z.enum(['FIRST_IN_ALBUM', 'LAST_IN_ALBUM', 'AFTER_MEDIA_ITEM', 'AFTER_ENRICHMENT_ITEM']).optional().describe('Position in the album to place the enrichment (default: LAST_IN_ALBUM)'),
    relativeMediaItemId: z.string().optional().describe('Media item ID to place enrichment after (when position is AFTER_MEDIA_ITEM)'),
    relativeEnrichmentItemId: z.string().optional().describe('Enrichment item ID to place enrichment after (when position is AFTER_ENRICHMENT_ITEM)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.text) {
      ...
      if (ctx.input.location.latitude !== undefined && ctx.input.location.longitude !== undefined) {
        loc.latlng = { latitude: ctx.input.location.latitude, longitude: ctx.input.location.longitude };
      }
      enrichment.locationEnrichment = { location: loc };
      enrichmentType = 'location';
    } else if (ctx.input.mapOrigin && ctx.input.mapDestination) {
      let buildLocation = (l: { locationName: string; latitude?: number; longitude?: number }) => {
      ...
      throw new Error('Provide exactly one enrichment type: text, location, or map (origin + destination).');
    }
    ...
    let result = await client.addEnrichmentToAlbum(ctx.input.albumId, enrichment, albumPosition);
