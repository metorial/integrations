...
let PIPELINE_EVENTS = ['pipeline.created', 'pipeline.deleted', 'pipeline.status_changed'] as const;
...
export let pipelineEvents = SlateTrigger.create(
  spec,
  {
    ...
    key: 'pipeline_events',
    description: 'Triggers when a candidate is added to, removed from, or changes status in a job pipeline.',
  }
)
  .input(z.object({
