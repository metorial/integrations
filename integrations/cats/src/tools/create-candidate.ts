...
export let createCandidate = SlateTool.create(
  ...
  .input(z.object({
    ...
    countryCode: z.string().optional().describe('ISO 3166 Alpha-2 country code (e.g. US, GB)'),
    ...
    checkDuplicate: z.boolean().optional().describe('Check for duplicate candidates before creating'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let candidateId = result?.id?.toString() ?? result?._links?.self?.href?.split('/').pop() ?? '';
