...
export let manageTags = SlateTool.create(
  ...
  .input(z.object({
    resourceType: z.enum(['candidates', 'contacts', 'companies', 'jobs']).describe('Type of record'),
    resourceId: z.string().describe('Record ID'),
    action: z.enum(['get', 'add', 'replace']).describe('"get" to retrieve, "add" to append, "replace" to replace all tags'),
    tagNames: z.array(z.string()).optional().describe('Tag names to add or replace (required for add/replace)'),
  }))
  .output(z.object({
    tags: z.array(z.object({
      tagId: z.string().optional().describe('Tag ID'),
      tagName: z.string().optional().describe('Tag name'),
    })).optional().describe('Current tags on the record'),
