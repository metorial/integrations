...
export let addressSchema = z.object({
  ...
  line: z.string().describe('Address lines (full address excluding name, postcode, and country)'),
  postcode: z.string().optional().describe('Postcode / ZIP code'),
  country: z.string().optional().describe('2-character ISO 3166-1 alpha-2 country code (defaults to GB)'),
});

export let recipientSchema = z.object({
  address: addressSchema.describe('Recipient address'),
  variables: z.record(z.string(), z.any()).optional().describe('Template variable values for this recipient'),
});
...
export let envelopeEnum = z.enum([
  'c5', 'c4', 'c4_plus', 'a4_box',
  'postcard_a6', 'postcard_a5', 'postcard_a5_enveloped',
]);

export let letterStatusEnum = z.enum([
  'draft', 'waiting_to_print', 'printing', 'enclosing',
  'shipping', 'sent', 'cancelled', 'returned', 'failed_wrong_address',
]);
...
export let letterSchema = z.object({
  ...
  }).nullable().optional().describe('Return information if letter was returned'),
...
export let splittingSchema = z.object({
  method: z.enum(['none', 'split_on_phrase', 'split_on_pages', 'mailing_list']).optional().describe('Splitting method'),
  phrase: z.string().optional().describe('Phrase to split on (for split_on_phrase method)'),
  pages: z.number().optional().describe('Number of pages per split (for split_on_pages method)'),
});

export let printingSchema = z.object({
  doubleSided: z.enum(['no', 'yes', 'mixed']).optional().describe('Double-sided printing option'),
...
export let postageSchema = z.object({
  ...
  idealEnvelope: z.enum(['c5', 'c4', 'c4_plus', 'a4_box', 'postcard_a6', 'postcard_a5', 'postcard_a5_enveloped']).optional().describe('Preferred envelope/postcard size'),
...
export let nudgeSchema = z.object({
  x: z.number().optional().describe('Horizontal offset in mm (positive = right, negative = left)'),
...
export let extraDocumentSchema = z.object({
  ...
  applyBackground: z.boolean().optional().describe('Whether to apply background to this document'),
...
export let printJobSchema = z.object({
  ...
  confirmed: z.boolean().optional().describe('Whether the print job is confirmed for printing'),
  ...
  letters: z.array(letterSchema).optional().describe('Individual mail items in this print job'),
  ...
  addressWindow: z.enum(['left', 'right']).optional().nullable().describe('Address window position'),
...
export let backgroundSchema = z.object({
  ...
  }).optional().describe('File information'),
...
export let mailingListSchema = z.object({
  ...
  variables: z.array(z.object({ name: z.string() })).optional().describe('Template variables available'),
  longestAddressLine: z.string().optional().describe('Longest address line for layout validation'),
  ...
  }).optional().describe('Address validation status'),
  created: z.number().optional().describe('UNIX timestamp when created'),
});
...
export let mailingListRecipientSchema = z.object({
  ...
  addressValidationStatus: z.enum(['passed', 'failed', 'pending', 'not_requested']).optional().describe('Address validation status'),
  created: z.number().optional().describe('UNIX timestamp when created'),
});
