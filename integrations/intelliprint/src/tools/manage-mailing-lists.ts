...
export let createMailingList = SlateTool.create(
  ...
  .input(z.object({
    ...
    requestAddressValidation: z.boolean().optional().describe('Request address validation (paid add-on)'),
  }))
...
export let getMailingList = SlateTool.create(
  spec,
  {
...
export let listMailingLists = SlateTool.create(
  ...
  .input(z.object({
    ...
    sortField: z.enum(['created', 'name', 'recipients']).optional().describe('Field to sort by'),
...
export let updateMailingList = SlateTool.create(
  ...
  .input(z.object({
    ...
    deleteOldRecipients: z.boolean().optional().describe('Delete existing recipients before adding new ones'),
    requestAddressValidation: z.boolean().optional().describe('Request address validation (paid add-on)'),
  }))
  .output(mailingListSchema)
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.recipients) params.recipients = ctx.input.recipients;
    if (ctx.input.deleteOldRecipients !== undefined) params.delete_old_recipients = ctx.input.deleteOldRecipients;
    if (ctx.input.requestAddressValidation !== undefined) {
      params.address_validation = { requested: ctx.input.requestAddressValidation };
    }
