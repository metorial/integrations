...
export let upsertVectorData = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    stopNodeId: z.string().optional().describe('Target a specific vector store node within the chatflow'),
    overrideConfig: z.record(z.string(), z.any()).optional().describe('Configuration overrides for the upsert operation'),
  }))
  .output(z.object({
