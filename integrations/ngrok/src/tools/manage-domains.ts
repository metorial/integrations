...
let certMgmtStatusSchema = z.object({
  renewsAt: z.string().optional().nullable(),
  provisioningJob: z.object({
...
let domainOutputSchema = z.object({
  ...
  certificateManagementPolicy: z.object({
    authority: z.string().optional().nullable(),
    privateKeyType: z.string().optional().nullable()
  }).optional().nullable().describe('Automatic certificate management policy'),
  certificateManagementStatus: certMgmtStatusSchema.describe('Certificate management status'),
  acmeChallengeCnameTarget: z.string().optional().nullable().describe('ACME challenge CNAME target')
});
...
export let listDomains = SlateTool.create(
  ...
  .input(z.object({
    beforeId: z.string().optional().describe('Pagination cursor - return entries before this ID'),
...
export let createDomain = SlateTool.create(
  ...
  .input(z.object({
    domain: z.string().describe('Hostname to reserve (e.g., app.example.com or *.example.com)'),
    description: z.string().optional().describe('Human-readable description (max 255 bytes)'),
    ...
    certificateManagementPolicy: z.object({
      ...
      privateKeyType: z.string().optional().describe('Private key type (e.g., "ecdsa", "rsa")')
    }).optional().describe('Automatic certificate management policy (mutually exclusive with certificateId)')
  }))
  .output(domainOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new NgrokClient(ctx.auth.token);
...
export let updateDomain = SlateTool.create(
  ...
  .input(z.object({
    ...
    }).optional().nullable().describe('New certificate management policy (null to remove)')
  }))
  .output(domainOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new NgrokClient(ctx.auth.token);
