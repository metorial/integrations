...
let credentialOutputSchema = z.object({
  ...
  authtokenValue: z.string().optional().nullable().describe('Authtoken value (only at creation)'),
...
export let createCredential = SlateTool.create(
  spec,
  {
    ...
    instructions: ['Save the returned authtokenValue immediately as it cannot be retrieved later.', 'ACL rules use the format "bind:domain.example.com" or "bind:*.example.com".'],
  ...
  .input(z.object({
    ...
    acl: z.array(z.string()).optional().describe('ACL bind rules (e.g., ["bind:example.ngrok.io"])'),
