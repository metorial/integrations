...
export let upsertGeofenceTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    tag: z.string().describe('Geofence tag (group name). Used with externalId to uniquely identify the geofence.'),
    externalId: z.string().describe('External ID mapping to your internal database. Used with tag to uniquely identify the geofence.'),
    ...
    radius: z.number().optional().describe('Radius in meters (circle, 10-10000) or minutes (isochrone)'),
    metadata: z.record(z.string(), z.string()).optional().describe('Custom key-value metadata'),
    enabled: z.boolean().optional().describe('Whether the geofence generates events'),
    userId: z.string().optional().describe('Restrict events to a specific user'),
    disableAfter: z.string().optional().describe('ISO 8601 datetime to auto-disable the geofence'),
  }))
...
export let listGeofencesTool = SlateTool.create(
  spec,
  {
    name: 'List Geofences',
    key: 'list_geofences',
  ...
  .input(z.object({
    ...
    limit: z.number().optional().describe('Max number of geofences to return (1-1000, default 100)'),
    createdBefore: z.string().optional().describe('ISO 8601 datetime cursor for pagination — return geofences created before this time'),
    createdAfter: z.string().optional().describe('ISO 8601 datetime cursor — return geofences created after this time'),
  }))
  .output(z.object({
...
export let getGeofenceTool = SlateTool.create(
  spec,
  {
    name: 'Get Geofence',
    key: 'get_geofence',
  ...
  .input(z.object({
    geofenceId: z.string().optional().describe('Radar internal geofence ID. Use this OR tag+externalId.'),
    tag: z.string().optional().describe('Geofence tag. Must be used together with externalId.'),
    externalId: z.string().optional().describe('Geofence external ID. Must be used together with tag.'),
    includeUsers: z.boolean().optional().describe('If true, also return users currently inside this geofence'),
  }))
  .output(z.object({
    geofence: geofenceSchema,
    users: z.array(z.any()).optional().describe('Users currently inside the geofence (if requested)'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new RadarClient({ token: ctx.auth.token });
    ...
    if (ctx.input.geofenceId) {
      ...
      geofenceResult = await client.getGeofenceByTagAndExternalId(ctx.input.tag, ctx.input.externalId);
...
export let deleteGeofenceTool = SlateTool.create(
  spec,
  {
