...
export let feedbackWebhook = SlateTrigger.create(
  spec,
  {
    name: 'Feedback Event',
    key: 'feedback_event',
    description: 'Triggers when feedback is created, updated, or deleted in Gleap. Configure the webhook URL in the Gleap dashboard under Integrations.'
  }
)
  .input(z.object({
    eventType: z.string().describe('Type of event (e.g. feedback_created, feedback_updated, feedback_deleted)'),
  ...
  .output(z.object({
    ticketId: z.string().optional().describe('ID of the affected ticket'),
    ticketType: z.string().optional().describe('Type of the ticket (e.g. BUG, FEATURE_REQUEST)'),
    ticketStatus: z.string().optional().describe('Current status of the ticket'),
    ticketTitle: z.string().optional().describe('Title of the ticket'),
    ticketPriority: z.string().optional().describe('Priority of the ticket'),
    sessionId: z.string().optional().describe('Session ID of the user who submitted the feedback'),
