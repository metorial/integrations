...
export let manageChatter = SlateTool.create(
  spec,
  {
    name: 'Manage Chatter',
    key: 'manage_chatter',
  ...
  .input(z.object({
    action: z.enum(['getFeed', 'postFeedItem']).describe('Action to perform: get a feed or post to a feed'),
    feedType: z.string().optional().default('news').describe('Feed type for getFeed (e.g., news, record, to)'),
    subjectId: z.string().optional().describe('The record or group ID to get feed for or post to'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.getChatterFeed(ctx.input.feedType ?? 'news', ctx.input.subjectId);
    return {
      output: { chatterResult: result },
