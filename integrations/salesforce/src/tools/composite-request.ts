...
export let compositeRequest = SlateTool.create(
  spec,
  {
...
- **collection**: Perform batch CRUD operations on up to 200 records of the same type.`,
  ...
  .input(z.object({
    mode: z.enum(['composite', 'tree', 'collection']).describe('The composite operation mode to use'),
    allOrNone: z.boolean().optional().default(false).describe('If true, all subrequests must succeed or all will be rolled back'),

    subrequests: z.array(z.object({
      method: z.enum(['GET', 'POST', 'PATCH', 'DELETE']).describe('HTTP method for this subrequest'),
      url: z.string().describe('The API URL for this subrequest (e.g., /services/data/v62.0/sobjects/Account)'),
    ...
    })).optional().describe('Subrequests for composite mode'),

    treeObjectType: z.string().optional().describe('Object type for tree mode (e.g., Account)'),
    treeRecords: z.array(z.record(z.string(), z.any())).optional().describe('Records with nested children for tree mode'),

    collectionMethod: z.enum(['POST', 'PATCH', 'DELETE']).optional().describe('Operation for collection mode'),
    collectionRecords: z.array(z.record(z.string(), z.any())).optional().describe('Records for collection mode (must include "attributes" with "type" for POST/PATCH)')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.mode === 'collection') {
      if (!ctx.input.collectionMethod || !ctx.input.collectionRecords) {
        throw new Error('collectionMethod and collectionRecords are required for collection mode');
      }
