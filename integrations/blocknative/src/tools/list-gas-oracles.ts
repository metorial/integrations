...
let oracleSchema = z.object({
  ...
  addressByVersion: z.record(z.string(), z.string()).describe('Oracle contract addresses keyed by version (e.g., {"2": "0x..."})'),
...
export let listGasOracles = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    ...
    let labels = oracles.slice(0, 10).map(o => o.label).join(', ');
