...
export let createProject = SlateTool.create(
  ...
  .input(z.object({
    ...
    region: z.string().optional().describe('AWS region for the default database (e.g., "us-east-1", "eu-west-1")'),
    workspaceId: z.string().optional().describe('Workspace ID to create the project in. If omitted, uses the configured default or the first available workspace.'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Databases created with the project'),
  }))
