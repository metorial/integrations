...
export let createLibraryTemplate = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    fileInfos: z.array(z.object({
      transientDocumentId: z.string().describe('ID of a previously uploaded transient document')
    })).describe('Documents to use in the template'),
    templateTypes: z.array(z.enum(['DOCUMENT', 'FORM_FIELD_LAYER'])).describe('Types of template to create'),
    sharingMode: z.enum(['USER', 'GROUP', 'ACCOUNT']).optional().describe('Sharing scope for the template. Defaults to user-level sharing.'),
    state: z.enum(['ACTIVE', 'AUTHORING']).optional().describe('Initial state of the template. Defaults to "ACTIVE".')
  }))
  .output(z.object({
