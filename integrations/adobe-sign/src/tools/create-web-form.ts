...
export let createWebForm = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    participantSetsInfo: z.array(z.object({
      memberInfos: z.array(z.object({
        email: z.string().describe('Email address of the participant')
      })).describe('Participant members'),
      role: z.enum(['SIGNER', 'APPROVER', 'ACCEPTOR', 'FORM_FILLER', 'CERTIFIED_RECIPIENT']).describe('Role of the participants')
    })).optional().describe('Pre-defined participant sets for the web form'),
    state: z.enum(['ACTIVE', 'DRAFT', 'AUTHORING']).optional().describe('Initial state of the web form. Defaults to "ACTIVE".')
  }))
  .output(z.object({
