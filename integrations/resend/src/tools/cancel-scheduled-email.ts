...
export let cancelScheduledEmail = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['cancel', 'reschedule']).describe('Whether to cancel or reschedule the email.'),
    scheduledAt: z.string().optional().describe('New scheduled time in ISO 8601 format. Required when action is "reschedule".'),
  }))
  .output(z.object({
