...
export let createBroadcast = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    html: z.string().optional().describe('HTML content. Supports contact property variables.'),
    ...
    topicId: z.string().optional().describe('Scope to a specific topic for subscription management.'),
    send: z.boolean().optional().describe('Set to true to send immediately. Default is false (draft).'),
    scheduledAt: z.string().optional().describe('Schedule time (ISO 8601 or natural language). Requires send: true.'),
  }))
  .output(z.object({
...
export let getBroadcast = SlateTool.create(
  spec,
  {
    name: 'Get Broadcast',
    key: 'get_broadcast',
...
export let sendBroadcast = SlateTool.create(
  spec,
  {
    name: 'Send Broadcast',
    key: 'send_broadcast',
  ...
  .input(z.object({
    ...
    scheduledAt: z.string().optional().describe('Schedule time (ISO 8601 or natural language). Omit to send immediately.'),
  }))
  .output(z.object({
    broadcastId: z.string().describe('ID of the broadcast.'),
  }))
...
export let deleteBroadcast = SlateTool.create(
  spec,
  {
