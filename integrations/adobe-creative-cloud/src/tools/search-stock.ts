...
export let searchStock = SlateTool.create(
  spec,
  {
    name: 'Search Stock',
    key: 'search_stock',
  ...
  .input(z.object({
    ...
    contentType: z.enum(['photo', 'illustration', 'vector', 'video', 'template', '3d', 'audio']).optional().describe('Filter by content type'),
    orientation: z.enum(['horizontal', 'vertical', 'square', 'panoramic']).optional().describe('Filter by orientation'),
