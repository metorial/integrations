...
export let manageLightroomCatalog = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    operation: z.enum(['getCatalog', 'listAssets', 'listAlbums', 'getAlbumAssets']).describe('Operation to perform'),
    catalogId: z.string().optional().describe('Catalog ID (required for listAssets, listAlbums, getAlbumAssets). Get from getCatalog first.'),
    albumId: z.string().optional().describe('Album ID (required for getAlbumAssets)'),
    limit: z.number().optional().describe('Maximum results to return'),
    capturedAfter: z.string().optional().describe('Filter assets captured after this date (ISO 8601)'),
    capturedBefore: z.string().optional().describe('Filter assets captured before this date (ISO 8601)'),
  }))
  .output(z.object({
    ...
    })).optional().describe('List of assets'),
    ...
    })).optional().describe('List of albums'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.operation) {
      ...
      case 'getAlbumAssets': {
        ...
        let result = await client.listAlbumAssets(ctx.input.catalogId, ctx.input.albumId, { limit: ctx.input.limit });
        let resources = result.resources || [];
        let assets = resources.map((a: any) => ({
