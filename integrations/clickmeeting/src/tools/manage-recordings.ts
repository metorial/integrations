...
export let manageRecordings = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    recordingId: z.string().optional().describe('ID of the specific recording to delete (required for "delete" action)')
  }))
  .output(z.object({
    recordings: z.array(z.record(z.string(), z.unknown())).optional().describe('List of recordings (for "list" action)'),
