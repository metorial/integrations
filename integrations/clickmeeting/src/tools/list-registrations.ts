...
export let listRegistrations = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    status: z.enum(['active', 'pending']).default('active').describe('Registration status filter'),
    sessionId: z.string().optional().describe('Optional session ID to filter registrations for a specific session')
  }))
  .output(z.object({
    registrations: z.array(z.record(z.string(), z.unknown())).describe('List of registered attendees')
  }))
