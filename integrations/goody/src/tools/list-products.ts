...
let brandSchema = z.object({
  ...
  freeShippingMinimum: z.number().nullable().describe('Minimum order for free shipping in cents'),
});
...
let productSchema = z.object({
  ...
  recipientDescription: z.string().nullable().describe('Description shown to the gift recipient'),
  ...
  variantsLabel: z.string().nullable().describe('Label for variant selection (e.g. "Color", "Size")'),
...
export let listProducts = SlateTool.create(
  ...
  .input(z.object({
    ...
    countryCode: z.string().optional().describe('Filter products by shipping country code (e.g. "US")'),
  }))
  .output(z.object({
