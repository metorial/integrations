...
export let research = SlateTool.create(
  spec,
  {
    name: 'Research',
    key: 'research',
  ...
  .input(z.object({
    ...
    model: z.enum(['mini', 'pro', 'auto']).optional().describe('Research model to use. Defaults to "auto"'),
    outputSchema: z.record(z.string(), z.unknown()).optional().describe('JSON Schema defining the desired structure for the research output'),
    citationFormat: z.enum(['numbered', 'mla', 'apa', 'chicago']).optional().describe('Citation format style. Defaults to "numbered"'),
  }))
  .output(z.object({
    ...
    content: z.unknown().optional().describe('Research report content (string or structured object if outputSchema was provided)'),
    sources: z.array(z.object({
      title: z.string().describe('Source title'),
      url: z.string().describe('Source URL'),
      favicon: z.string().optional().describe('Source favicon URL'),
    })).optional().describe('Sources cited in the research report'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    for (let i = 0; i < maxAttempts && (result.status === 'pending' || result.status === 'in_progress'); i++) {
