...
export let webSearch = SlateTool.create(
  ...
  .input(z.object({
    ...
    searchDepth: z.enum(['ultra-fast', 'fast', 'basic', 'advanced']).optional().describe('Controls latency vs relevance tradeoff. Defaults to "basic"'),
    topic: z.enum(['general', 'news', 'finance']).optional().describe('Category to tailor result sources. Defaults to "general"'),
    maxResults: z.number().min(0).max(20).optional().describe('Maximum number of results to return (0-20). Defaults to 5'),
    chunksPerSource: z.number().min(1).max(3).optional().describe('Max content chunks per source (1-3). Only for advanced/fast depth'),
    timeRange: z.enum(['day', 'week', 'month', 'year']).optional().describe('Filter results by time range'),
    ...
    includeAnswer: z.union([z.boolean(), z.enum(['basic', 'advanced'])]).optional().describe('Include an LLM-generated answer. Use "advanced" for higher quality'),
    includeRawContent: z.union([z.boolean(), z.enum(['markdown', 'text'])]).optional().describe('Include full cleaned page content in markdown or text format'),
    includeImages: z.boolean().optional().describe('Include image results'),
    includeImageDescriptions: z.boolean().optional().describe('Add descriptions to image results'),
    includeDomains: z.array(z.string()).optional().describe('Only include results from these domains'),
    excludeDomains: z.array(z.string()).optional().describe('Exclude results from these domains'),
    country: z.string().optional().describe('Two-letter country code to boost results from (only works with "general" topic)'),
    autoParameters: z.boolean().optional().describe('Let Tavily auto-configure parameters based on query intent'),
    exactMatch: z.boolean().optional().describe('Only return results containing exact quoted phrases from the query'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Image search results'),
    ...
    autoParameters: z.record(z.string(), z.unknown()).optional().describe('Auto-configured parameters used'),
