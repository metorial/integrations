...
export let extractContent = SlateTool.create(
  ...
  .input(z.object({
    ...
    query: z.string().optional().describe('Query to rerank extracted content chunks by relevance'),
    chunksPerSource: z.number().min(1).max(5).optional().describe('Max relevant chunks per source (1-5). Requires a query'),
    extractDepth: z.enum(['basic', 'advanced']).optional().describe('Extraction depth. "advanced" retrieves tables and embedded content'),
    includeImages: z.boolean().optional().describe('Include images found on the pages'),
    format: z.enum(['markdown', 'text']).optional().describe('Output format for extracted content. Defaults to "markdown"'),
    timeout: z.number().min(1).max(60).optional().describe('Maximum seconds before timeout (1-60)'),
  }))
  .output(z.object({
