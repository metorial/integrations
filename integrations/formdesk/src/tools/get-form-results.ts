...
export let getFormResults = SlateTool.create(
  ...
  .input(z.object({
    ...
    createdAfter: z.string().optional().describe('Only return results created after this date (ISO 8601)'),
    createdBefore: z.string().optional().describe('Only return results created before this date (ISO 8601)'),
    changedAfter: z.string().optional().describe('Only return results changed after this date (ISO 8601)'),
    changedBefore: z.string().optional().describe('Only return results changed before this date (ISO 8601)'),
    completedAfter: z.string().optional().describe('Only return results completed after this date (ISO 8601)'),
    completedBefore: z.string().optional().describe('Only return results completed before this date (ISO 8601)'),
    status: z.string().optional().describe('Filter by result status'),
    syncStatus: z.enum(['all', 'unprocessed', 'processed']).optional().describe('Filter by synchronization status'),
  ...
  .output(z.object({
    resultIds: z.array(z.string()).describe('List of result entry IDs matching the query criteria'),
    totalCount: z.number().optional().describe('Total number of matching results if provided by the API'),
  }))
