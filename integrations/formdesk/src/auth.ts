...
export let auth = SlateAuth.create()
  ...
  .addCustomAuth({
    ...
    inputSchema: z.object({
      ...
      domain: z.string().describe('The unique account name (folder of forms) found in your Formdesk account settings.'),
    }),
    ...
    getProfile: async (ctx: { output: { token: string; host: string; domain: string; }; input: { token: string; domain: string; }; }) => {
      return {
        profile: {
