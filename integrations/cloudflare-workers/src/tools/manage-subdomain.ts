...
export let getSubdomain = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    scriptName: z.string().optional().describe('Optionally check whether a specific Worker is enabled on workers.dev'),
  }))
  .output(z.object({
    subdomain: z.string().optional().describe('Account workers.dev subdomain name'),
    scriptEnabled: z.boolean().optional().describe('Whether the specified Worker is enabled on workers.dev'),
    previewsEnabled: z.boolean().optional().describe('Whether previews are enabled for the specified Worker on workers.dev'),
  }))
...
export let setScriptSubdomain = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    previewsEnabled: z.boolean().optional().describe('Whether to enable preview URLs on workers.dev'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    let client = createClient(ctx);
    let result = await client.setScriptSubdomain(ctx.input.scriptName, ctx.input.enabled, ctx.input.previewsEnabled);
