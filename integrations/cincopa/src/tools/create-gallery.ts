...
export let createGallery = SlateTool.create(
  spec,
  {
    name: 'Create Gallery',
    key: 'create_gallery',
  ...
  .input(z.object({
    ...
    copyArgsFrom: z.string().optional().describe('Gallery ID to copy all settings from (overrides template)'),
    masterGalleryId: z.string().optional().describe('Master gallery ID to synchronize settings with'),
  ...
  .output(z.object({
    ...
    uploadUrl: z.string().optional().describe('Temporary URL for uploading media to this gallery (expires after 24h)'),
