...
export let assetEvents = SlateTrigger.create(
  spec,
  {
    name: 'Asset Events',
    key: 'asset_events',
    description: 'Triggered when assets are uploaded, updated, or deleted in your Cincopa account.'
  }
)
  .input(z.object({
    eventType: z.string().describe('Type of asset event (e.g., asset.uploaded, asset.updated, asset.deleted)'),
  ...
  .output(z.object({
    ...
    assetType: z.string().optional().describe('Type of the affected asset (video, image, audio, other)'),
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      let eventId = body?.id || body?.event_id || `${eventType}-${body?.rid || ''}-${Date.now()}`;
