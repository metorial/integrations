...
export let evaluateSignalTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    clientTransactionId: z.string().describe('Your unique transaction identifier (max 36 chars)'),
    amount: z.number().describe('Transaction amount in dollars'),
    userPresent: z.boolean().optional().describe('Whether the user is currently present in the session'),
  }))
  .output(z.object({
    customerInitiatedReturnRisk: z.number().describe('Risk score for customer-initiated returns (higher = riskier)'),
    bankInitiatedReturnRisk: z.number().describe('Risk score for bank-initiated returns (higher = riskier)'),
    availableBalance: z.number().nullable().optional().describe('Available account balance if known'),
    currentBalance: z.number().nullable().optional().describe('Current account balance if known'),
    rulesetResult: z.string().nullable().optional().describe('Ruleset decision: ACCEPT, REROUTE, or REVIEW'),
