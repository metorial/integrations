...
export let getTransferTool = SlateTool.create(
  spec,
  {
    name: 'Get Transfer',
    key: 'get_transfer',
  ...
  .output(z.object({
    ...
    status: z.string().describe('Transfer status: pending, posted, settled, funds_available, cancelled, failed, returned'),
    ...
    failureReason: z.any().nullable().optional().describe('Failure reason if failed or returned'),
  }))
