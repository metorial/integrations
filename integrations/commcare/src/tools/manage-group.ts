...
To create, provide a name. To update, provide the groupId along with the fields to change.`,
  ...
  .input(z.object({
    groupId: z.string().optional().describe('ID of an existing group to update. Omit to create a new group.'),
    name: z.string().optional().describe('Name for the group (required for creation)'),
    caseSharing: z.boolean().optional().describe('Whether the group shares cases between members'),
    reporting: z.boolean().optional().describe('Whether the group is used for reporting'),
    userIds: z.array(z.string()).optional().describe('List of mobile worker user IDs to include in the group'),
    metadata: z.record(z.string(), z.any()).optional().describe('Custom group metadata as key-value pairs'),
  }))
  .output(z.object({
    groupId: z.string(),
    groupName: z.string(),
