...
let caseEntrySchema = z.object({
  caseId: z.string().optional().describe('Case ID for updating an existing case. Omit to create a new case.'),
  ...
  }).nullable()).optional().describe('Relationships to other cases. Set to null to remove an index.'),
});

export let bulkCases = SlateTool.create(
  spec,
  {
...
Supports up to 100 cases per request. Useful for batch imports, migrations, or synchronized updates.`,
  ...
  .input(z.object({
    cases: z.array(caseEntrySchema).min(1).max(100).describe('Array of case objects to create or update'),
  }))
  .output(z.object({
    success: z.boolean(),
