...
export let createServer = SlateTool.create(
  ...
  .input(z.object({
    ...
    protocolType: z.enum([
      'ssh', 'ftp', 'ftps', 's3', 's3_compatible', 'rackspace',
      'digitalocean', 'shopify', 'netlify', 'heroku', 'elastic_beanstalk', 'shell',
    ...
    environment: z.string().optional().describe('Environment label (e.g., Production, Staging, Development)'),
    emailNotifyOn: z.enum(['never', 'failure', 'always']).optional().describe('When to send email notifications'),
    rootPath: z.string().optional().describe('Subdirectory in the repository to deploy'),
    serverGroupIdentifier: z.string().optional().describe('Identifier of the server group to add this server to'),
    ...
    useSshKeys: z.boolean().optional().describe('Use SSH keys instead of password (SSH only)'),
    ...
    customEndpoint: z.string().optional().describe('Custom S3 endpoint (for S3-compatible storage)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.emailNotifyOn !== undefined) serverData.email_notify_on = ctx.input.emailNotifyOn;
    if (ctx.input.rootPath !== undefined) serverData.root_path = ctx.input.rootPath;
    if (ctx.input.serverGroupIdentifier !== undefined) serverData.server_group_identifier = ctx.input.serverGroupIdentifier;
    ...
    if (ctx.input.secretAccessKey !== undefined) serverData.secret_access_key = ctx.input.secretAccessKey;
    if (ctx.input.customEndpoint !== undefined) serverData.custom_endpoint = ctx.input.customEndpoint;
