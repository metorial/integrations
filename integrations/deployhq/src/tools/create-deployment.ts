...
export let createDeployment = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    startRevision: z.string().optional().describe('Starting revision (commit SHA). Leave empty to deploy from beginning.'),
    endRevision: z.string().optional().describe('Ending revision (commit SHA). Defaults to latest.'),
    branch: z.string().optional().describe('Branch to deploy from'),
    mode: z.enum(['queue', 'preview']).optional().describe('Deployment mode: "queue" for immediate, "preview" to see changes first'),
    copyConfigFiles: z.boolean().optional().describe('Whether to copy config files during deployment'),
    runBuildCommands: z.boolean().optional().describe('Whether to run build pipeline commands'),
    useBuildCache: z.boolean().optional().describe('Whether to use the build cache'),
    deployEntireRepository: z.boolean().optional().describe('Deploy all files from the first to latest commit'),
    configFilesDeployment: z.boolean().optional().describe('Set to true for config-file-only deployments'),
    schedule: z.enum(['future', 'daily', 'weekly', 'monthly']).optional().describe('Schedule type for scheduled deployments'),
    ...
    scheduleWeekday: z.number().optional().describe('Day of week for weekly schedule (0=Sunday, 6=Saturday)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.startRevision !== undefined) deploymentData.start_revision = ctx.input.startRevision;
    if (ctx.input.endRevision !== undefined) deploymentData.end_revision = ctx.input.endRevision;
    ...
    if (ctx.input.copyConfigFiles !== undefined) deploymentData.copy_config_files = ctx.input.copyConfigFiles;
    if (ctx.input.runBuildCommands !== undefined) deploymentData.run_build_commands = ctx.input.runBuildCommands;
    if (ctx.input.useBuildCache !== undefined) deploymentData.use_build_cache = ctx.input.useBuildCache;
    if (ctx.input.deployEntireRepository !== undefined) deploymentData.deploy_entire_repository = ctx.input.deployEntireRepository;
    if (ctx.input.configFilesDeployment !== undefined) deploymentData.config_files_deployment = ctx.input.configFilesDeployment;
    if (ctx.input.schedule !== undefined) {
      ...
      if (ctx.input.scheduleMinute !== undefined) deploymentData.minute = ctx.input.scheduleMinute;
      if (ctx.input.scheduleWeekday !== undefined) deploymentData.weekday = ctx.input.scheduleWeekday;
    }
