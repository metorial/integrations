...
Provide the table path in the format \`PUBLISHER/TABLE_CODE\` (e.g., \`ZACKS/FC\`, \`SHARADAR/SF1\`, \`QDL/EOD\`).`,
  ...
  .input(z.object({
    tablePath: z.string().describe('Table path in the format PUBLISHER/TABLE_CODE (e.g., ZACKS/FC, SHARADAR/SF1).'),
    filters: z.record(z.string(), z.string()).optional().describe('Row-level filters as key-value pairs. Keys are column names, optionally with .gt, .lt, .gte, .lte suffixes (e.g., {"ticker": "AAPL", "date.gte": "2023-01-01"}).'),
    columns: z.array(z.string()).optional().describe('List of specific column names to return. If omitted, all columns are returned.'),
    perPage: z.number().optional().describe('Number of rows per page (max 10,000).'),
    cursorId: z.string().optional().describe('Cursor ID for pagination, obtained from a previous response\'s nextCursorId.'),
  }))
  .output(z.object({
    ...
    nextCursorId: z.string().nullable().describe('Cursor ID for fetching the next page. Null if no more data.'),
  }))
