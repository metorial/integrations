...
export class MondayClient {
  ...
  async getBoards(options?: {
    ...
    if (options?.workspaceIds?.length) args.push(`workspace_ids: [${options.workspaceIds.join(', ')}]`);

    let argsStr = args.length > 0 ? `(${args.join(', ')})` : '';
    let data = await this.query(`{ boards${argsStr} { id name description state board_kind workspace_id columns { id title type } groups { id title color } owners { id name } } }`);
    return data.boards;
  }
  ...
  async createBoard(params: {
    ...
    if (params.description) args.push(`description: "${this.escapeGraphQL(params.description)}"`);
    ...
    let data = await this.query(`mutation { create_board(${args.join(', ')}) { id name description state board_kind } }`);
    return data.create_board;
  }

  async updateBoard(boardId: string, attribute: string, newValue: string): Promise<any> {
    let data = await this.query(`mutation { update_board(board_id: ${boardId}, board_attribute: ${attribute}, new_value: "${this.escapeGraphQL(newValue)}") }`);
    return data.update_board;
  }

  async archiveBoard(boardId: string): Promise<any> {
    let data = await this.query(`mutation { archive_board(board_id: ${boardId}) { id name state } }`);
  ...
  async getItems(ids: string[]): Promise<any[]> {
    let data = await this.query(`{ items(ids: [${ids.join(', ')}]) { id name state created_at updated_at group { id title } board { id name } column_values { id type text value } subitems { id name } } }`);
    return data.items;
  }
  ...
  async getBoardItems(boardId: string, options?: {
    ...
    if (options?.groupId) {
      let data = await this.query(`{ boards(ids: [${boardId}]) { groups(ids: ["${this.escapeGraphQL(options.groupId)}"]) { items_page${itemsPageArgsStr} { cursor items { id name state created_at updated_at group { id title } column_values { id type text value } subitems { id name } } } } } }`);
    ...
    let data = await this.query(`{ boards(ids: [${boardId}]) { items_page${itemsPageArgsStr} { cursor items { id name state created_at updated_at group { id title } column_values { id type text value } subitems { id name } } } } }`);
  ...
  async updateColumnValues(boardId: string, itemId: string, columnValues: Record<string, any>, createLabelsIfMissing?: boolean): Promise<any> {
    let variables: Record<string, any> = {
  ...
  async moveItemToGroup(itemId: string, groupId: string): Promise<any> {
    let data = await this.query(`mutation { move_item_to_group(item_id: ${itemId}, group_id: "${this.escapeGraphQL(groupId)}") { id name group { id title } } }`);
    return data.move_item_to_group;
  }

  async archiveItem(itemId: string): Promise<any> {
    let data = await this.query(`mutation { archive_item(item_id: ${itemId}) { id name state } }`);
  ...
  async getGroups(boardId: string): Promise<any[]> {
    let data = await this.query(`{ boards(ids: [${boardId}]) { groups { id title color archived deleted position } } }`);
    return data.boards?.[0]?.groups || [];
  }

  async createGroup(boardId: string, groupName: string, options?: { color?: string }): Promise<any> {
    ...
    let data = await this.query(`mutation { create_group(${args.join(', ')}) { id title color } }`);
    return data.create_group;
  }

  async updateGroup(boardId: string, groupId: string, attribute: string, newValue: string): Promise<any> {
    let data = await this.query(`mutation { update_group(board_id: ${boardId}, group_id: "${this.escapeGraphQL(groupId)}", group_attribute: ${attribute}, new_value: "${this.escapeGraphQL(newValue)}") { id title color } }`);
    return data.update_group;
  }

  async deleteGroup(boardId: string, groupId: string): Promise<any> {
    let data = await this.query(`mutation { delete_group(board_id: ${boardId}, group_id: "${this.escapeGraphQL(groupId)}") { id } }`);
    return data.delete_group;
  }

  async archiveGroup(boardId: string, groupId: string): Promise<any> {
    let data = await this.query(`mutation { archive_group(board_id: ${boardId}, group_id: "${this.escapeGraphQL(groupId)}") { id } }`);
  ...
  async getColumns(boardId: string): Promise<any[]> {
    let data = await this.query(`{ boards(ids: [${boardId}]) { columns { id title type description settings_str } } }`);
    return data.boards?.[0]?.columns || [];
  }

  async createColumn(boardId: string, title: string, columnType: string, options?: { description?: string; defaults?: string }): Promise<any> {
    let args = [`board_id: ${boardId}`, `title: "${this.escapeGraphQL(title)}"`, `column_type: ${columnType}`];
    if (options?.description) args.push(`description: "${this.escapeGraphQL(options.description)}"`);
    if (options?.defaults) args.push(`defaults: ${JSON.stringify(options.defaults)}`);

    let data = await this.query(`mutation { create_column(${args.join(', ')}) { id title type description } }`);
    return data.create_column;
  }

  async deleteColumn(boardId: string, columnId: string): Promise<any> {
    let data = await this.query(`mutation { delete_column(board_id: ${boardId}, column_id: "${this.escapeGraphQL(columnId)}") { id } }`);
    return data.delete_column;
  }

  // ==================== Updates ====================

  async getUpdates(options?: { ids?: string[]; limit?: number; page?: number }): Promise<any[]> {
    let args: string[] = [];
    if (options?.ids?.length) args.push(`ids: [${options.ids.join(', ')}]`);
    if (options?.limit) args.push(`limit: ${options.limit}`);
    if (options?.page) args.push(`page: ${options.page}`);
    ...
    let argsStr = args.length > 0 ? `(${args.join(', ')})` : '';
    let data = await this.query(`{ updates${argsStr} { id body text_body created_at updated_at creator_id creator { id name } item_id replies { id body text_body created_at creator_id } } }`);
    return data.updates;
  }

  async getItemUpdates(itemId: string, limit?: number): Promise<any[]> {
    let limitStr = limit ? `(limit: ${limit})` : '';
    let data = await this.query(`{ items(ids: [${itemId}]) { updates${limitStr} { id body text_body created_at updated_at creator_id creator { id name } replies { id body text_body created_at creator_id } } } }`);
  ...
  async getUsers(options?: { ids?: string[]; limit?: number; page?: number; emails?: string[]; name?: string }): Promise<any[]> {
    let args: string[] = [];
    if (options?.ids?.length) args.push(`ids: [${options.ids.join(', ')}]`);
    if (options?.limit) args.push(`limit: ${options.limit}`);
    if (options?.page) args.push(`page: ${options.page}`);
    if (options?.emails?.length) args.push(`emails: [${options.emails.map(e => `"${this.escapeGraphQL(e)}"`).join(', ')}]`);
    if (options?.name) args.push(`name: "${this.escapeGraphQL(options.name)}"`);

    let argsStr = args.length > 0 ? `(${args.join(', ')})` : '';
    let data = await this.query(`{ users${argsStr} { id name email url title phone mobile_phone location enabled is_admin is_guest created_at photo_original teams { id name } } }`);
    return data.users;
  }

  async getMe(): Promise<any> {
    let data = await this.query(`{ me { id name email url title phone mobile_phone location enabled is_admin created_at photo_original account { id name } teams { id name } } }`);
    return data.me;
  }
  ...
  async getTeams(ids?: string[]): Promise<any[]> {
    ...
    let data = await this.query(`{ teams${argsStr} { id name picture_url owners { id name } users { id name } } }`);
    return data.teams;
  }

  // ==================== Workspaces ====================

  async getWorkspaces(options?: { ids?: string[]; limit?: number; page?: number; kind?: string }): Promise<any[]> {
    let args: string[] = [];
    if (options?.ids?.length) args.push(`ids: [${options.ids.join(', ')}]`);
    if (options?.limit) args.push(`limit: ${options.limit}`);
    if (options?.page) args.push(`page: ${options.page}`);
    ...
    let argsStr = args.length > 0 ? `(${args.join(', ')})` : '';
    let data = await this.query(`{ workspaces${argsStr} { id name kind description created_at state is_default_workspace } }`);
  ...
  async createWorkspace(name: string, kind: string, description?: string): Promise<any> {
    ...
    let data = await this.query(`mutation { create_workspace(${args.join(', ')}) { id name kind description } }`);
    return data.create_workspace;
  }

  async updateWorkspace(workspaceId: string, attributes: { name?: string; description?: string }): Promise<any> {
    let attrParts: string[] = [];
    if (attributes.name) attrParts.push(`name: "${this.escapeGraphQL(attributes.name)}"`);
    if (attributes.description) attrParts.push(`description: "${this.escapeGraphQL(attributes.description)}"`);

    if (attrParts.length === 0) return null;

    let data = await this.query(`mutation { update_workspace(id: ${workspaceId}, attributes: {${attrParts.join(', ')}}) { id name description } }`);
    return data.update_workspace;
  }

  async deleteWorkspace(workspaceId: string): Promise<any> {
    let data = await this.query(`mutation { delete_workspace(workspace_id: ${workspaceId}) { id } }`);
  ...
  async getWebhooks(boardId: string): Promise<any[]> {
    let data = await this.query(`{ webhooks(board_id: ${boardId}) { id event board_id config } }`);
    return data.webhooks;
  }

  async createWebhook(boardId: string, url: string, event: string, config?: Record<string, any>): Promise<any> {
    let args = [`board_id: ${boardId}`, `url: "${this.escapeGraphQL(url)}"`, `event: ${event}`];
    if (config) args.push(`config: "${this.escapeGraphQL(JSON.stringify(config))}"`);

    let data = await this.query(`mutation { create_webhook(${args.join(', ')}) { id board_id } }`);
  ...
  async createNotification(userId: string, targetId: string, text: string, targetType: string): Promise<any> {
    let variables = {
  ...
  async getActivityLogs(boardIds: string[], options?: { limit?: number; page?: number; from?: string; to?: string }): Promise<any[]> {
    ...
    let data = await this.query(`{ boards(ids: [${boardIds.join(', ')}]) { activity_logs(${args.slice(0).join(', ')}) { id event data entity account_id created_at user_id } } }`);
