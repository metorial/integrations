...
let imageSearchResultSchema = z.object({
...
export let imageSearch = SlateTool.create(
  spec,
  {
    name: 'Image Search',
    key: 'image_search',
  ...
  .input(z.object({
    ...
    numResults: z.number().optional().describe('Number of results to return (max 100 per page)'),
    start: z.number().optional().describe('Result offset for pagination'),
    filterSize: z.enum(['m', 'l', 'i']).optional().describe('Filter by image size: m=medium, l=large, i=icon'),
    filterColor: z.string().optional().describe('Filter by color: "color", "gray", "trans", or a specific color like "red", "blue"'),
    filterType: z.enum(['clipart', 'lineart', 'gif']).optional().describe('Filter by image type'),
    filterTime: z.enum(['d', 'w', 'm', 'y']).optional().describe('Filter by time: d=day, w=week, m=month, y=year'),
  }))
