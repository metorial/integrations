...
let reverseImageResultSchema = z.object({
...
let reverseImageSearchResultSchema = z.object({
  query: z.record(z.string(), z.any()).optional().describe('Echo of query parameters'),
  organicResults: z.array(reverseImageResultSchema).optional().describe('Pages where the image appears'),
}).passthrough();

export let reverseImageSearch = SlateTool.create(
  spec,
  {
