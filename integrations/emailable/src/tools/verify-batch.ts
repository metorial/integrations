...
export let verifyBatch = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    emails: z.array(z.string()).min(1).max(50000).describe('List of email addresses to verify'),
    callbackUrl: z.string().optional().describe('URL to receive results via HTTP POST when the batch completes. Must return HTTP 200; retries hourly for up to 3 days on failure.'),
    responseFields: z.array(z.string()).optional().describe('Limit which fields are returned in results (e.g. ["email", "state", "score"])'),
    retries: z.boolean().optional().describe('Whether to retry failed verifications. Defaults to true. Disabling speeds up verification.')
  }))
  .output(z.object({
    batchId: z.string().describe('Unique identifier for the batch, used to check status and retrieve results'),
