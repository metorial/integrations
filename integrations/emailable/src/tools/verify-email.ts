...
export let verifyEmail = SlateTool.create(
  ...
  .input(z.object({
    ...
    smtp: z.boolean().optional().describe('Set to false to disable SMTP verification for faster results (reduces accuracy)'),
    acceptAll: z.boolean().optional().describe('Enable accept-all (catch-all) detection. Increases response time but improves accuracy for catch-all domains.'),
    timeout: z.number().min(2).max(10).optional().describe('Custom timeout in seconds (2-10). Default is determined by the server.')
  }))
  .output(z.object({
    ...
    acceptAll: z.boolean().describe('Whether the mail server accepts all addresses regardless of validity'),
    disposable: z.boolean().describe('Whether the email is hosted on a disposable/temporary email service'),
    free: z.boolean().describe('Whether the email is hosted by a free email provider'),
    role: z.boolean().describe('Whether the email is a role-based address (e.g. support@, info@)'),
    ...
    didYouMean: z.string().nullable().describe('Suggested correction for misspelled email addresses'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let stateEmoji = result.state === 'deliverable' ? '✅' :
      result.state === 'undeliverable' ? '❌' :
      result.state === 'risky' ? '⚠️' : '❓';
