...
export let getLogoUrl = SlateTool.create(
  spec,
  {
    name: 'Get Logo URL',
    key: 'get_logo_url',
...
Supports customizable dimensions, theme variants, multiple logo types, output format selection, and smart fallback options.`,
  ...
  .input(z.object({
    identifier: z.string().describe('Brand identifier: domain (e.g. "nike.com"), ticker, ISIN, or crypto symbol'),
    logoType: z.enum(['icon', 'logo', 'symbol']).optional().describe('Logo type. "icon" is the square icon, "logo" is the full horizontal logo, "symbol" is the brand symbol. Defaults to "icon".'),
    theme: z.enum(['light', 'dark']).optional().describe('Theme variant for the logo'),
    height: z.number().optional().describe('Desired height in pixels (aspect ratio is preserved)'),
    width: z.number().optional().describe('Desired width in pixels (aspect ratio is preserved)'),
    fallback: z.enum(['brandfetch', 'transparent', 'lettermark', '404']).optional().describe('Fallback behavior when logo is not found. "brandfetch" shows Brandfetch default, "transparent" returns transparent image, "lettermark" generates a letter-based logo, "404" returns HTTP 404.'),
    format: z.enum(['png', 'jpg', 'svg', 'webp']).optional().describe('Output image format. Defaults to WebP.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    if (!ctx.auth.clientId) {
      throw new Error('Client ID is required for the Logo API. Please configure your Brandfetch Client ID in the authentication settings.');
    }
