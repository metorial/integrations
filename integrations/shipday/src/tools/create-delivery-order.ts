...
let addressBreakdownSchema = z.object({
...
export let createDeliveryOrder = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    restaurantPhoneNumber: z.string().optional().describe('Restaurant phone number with country code'),
    ...
    expectedDeliveryTime: z.string().optional().describe('Expected delivery time (hh:mm:ss)'),
    ...
    deliveryInstruction: z.string().optional().describe('Special instructions for the driver'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.restaurantPhoneNumber) body.restaurantPhoneNumber = ctx.input.restaurantPhoneNumber;
    if (ctx.input.expectedDeliveryDate) body.expectedDeliveryDate = ctx.input.expectedDeliveryDate;
    if (ctx.input.expectedPickupTime) body.expectedPickupTime = ctx.input.expectedPickupTime;
    if (ctx.input.expectedDeliveryTime) body.expectedDeliveryTime = ctx.input.expectedDeliveryTime;
    if (ctx.input.pickupLatitude !== undefined) body.pickupLatitude = ctx.input.pickupLatitude;
    if (ctx.input.pickupLongitude !== undefined) body.pickupLongitude = ctx.input.pickupLongitude;
    if (ctx.input.deliveryLatitude !== undefined) body.deliveryLatitude = ctx.input.deliveryLatitude;
    if (ctx.input.deliveryLongitude !== undefined) body.deliveryLongitude = ctx.input.deliveryLongitude;
    ...
    if (ctx.input.deliveryInstruction) body.deliveryInstruction = ctx.input.deliveryInstruction;
