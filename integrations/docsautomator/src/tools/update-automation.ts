...
export let updateAutomation = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    saveGoogleDoc: z.boolean().optional().describe('Whether to save a Google Doc version on generation.'),
    dateFormat: z.string().optional().describe('Date format setting.'),
    locale: z.string().optional().describe('Locale setting.'),
    newDocumentNameField: z.string().optional().describe('Field used to name generated documents.'),
    overwriteAttachment: z.boolean().optional().describe('Whether to overwrite existing attachments.'),
    googleDocDestinationFolderUrl: z.string().optional().describe('Destination folder URL for Google Docs.'),
    formatNumbersWithLocale: z.boolean().optional().describe('Whether to format numbers with locale settings.'),
  }))
  .output(z.object({
