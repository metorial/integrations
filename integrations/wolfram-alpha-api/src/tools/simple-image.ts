...
export let simpleImage = SlateTool.create(
  spec,
  {
...
Useful for embedding visual results or sharing computed answers as images. Supports customization of layout, colors, font size, and image width.`,
    ...
    constraints: [
      'Does not support disambiguation or interactive drilldown.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    layout: z.enum(['divider', 'labelbar']).optional().describe('Layout style for the image'),
    background: z.string().optional().describe('Background color as hex (e.g., "F5F5F5")'),
    foreground: z.string().optional().describe('Foreground/text color as hex (e.g., "000000")'),
