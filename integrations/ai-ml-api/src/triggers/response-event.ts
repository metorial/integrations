...
export let responseEvent = SlateTrigger.create(
  spec,
  {
    name: 'Response Event',
    key: 'response_event',
    description: 'Triggered when an asynchronous background response completes or fails. Receives webhook notifications for response.completed and response.failed events.',
  }
)
  .input(z.object({
    eventType: z.string().describe('Type of the event, e.g. "response.completed" or "response.failed"'),
