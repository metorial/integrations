...
let sourceSchema = z.object({
  ...
  updatedAt: z.string().nullable().describe('ISO 8601 timestamp when the source was last updated'),
  pausedAt: z.string().nullable().describe('ISO 8601 timestamp if the source is paused, null otherwise'),
  systemPausedAt: z.string().nullable().describe('ISO 8601 timestamp if system-paused, null otherwise'),
  stale: z.boolean().nullable().describe('Whether the source data is stale'),
  reportCard: z.any().optional().describe('Configuration status and report card for the source'),
});
