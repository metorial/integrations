...
export let pushData = SlateTool.create(
  spec,
  {
    name: 'Push Data',
    key: 'push_data',
  ...
  .input(z.object({
    ...
    schema: z.record(z.string(), z.any()).describe('JSON Schema describing the record structure (e.g., {"properties": {"id": {"type": "integer"}, "name": {"type": "string"}}})'),
    records: z.array(z.record(z.string(), z.any())).describe('Array of data records to push'),
    keyNames: z.array(z.string()).optional().describe('Primary key field names for upsert loading. Omit for append-only.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    if (!ctx.config.clientId) {
      throw new Error('Stitch client ID is required for Import API operations. Set it in the configuration.');
    }
...
export let validateData = SlateTool.create(
  spec,
  {
    name: 'Validate Data',
    key: 'validate_data',
  ...
  .input(z.object({
    ...
    records: z.array(z.record(z.string(), z.any())).describe('Array of data records to validate'),
