...
- **style**: Transfer the visual style from a reference image to generate new content`,
  ...
  .input(z.object({
    ...
    controlStrength: z.number().min(0).max(1).optional().describe('How strongly the input image influences the output (0-1)'),
    negativePrompt: z.string().optional().describe('What to exclude from the generated image'),
