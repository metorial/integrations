...
let aspectRatioEnum = z.enum(['1:1', '16:9', '21:9', '2:3', '3:2', '4:5', '5:4', '9:16', '9:21']).optional().describe('Aspect ratio of the generated image');

let outputFormatEnum = z.enum(['png', 'jpeg', 'webp']).optional().default('png').describe('Output image format');
...
]).optional().describe('Visual style preset to guide the generation');
...
- **SD 3.5 Medium**: Balanced quality and speed (3.5 credits)`,
  ...
  .input(z.object({
    ...
    model: z.enum(['ultra', 'core', 'sd3.5-large', 'sd3.5-large-turbo', 'sd3.5-medium']).default('core').describe('Model to use for generation'),
    negativePrompt: z.string().max(10000).optional().describe('What to exclude from the generated image. Not supported with turbo models.'),
    aspectRatio: aspectRatioEnum,
    seed: z.number().int().min(0).max(4294967294).optional().describe('Seed for reproducible generation. 0 or omit for random.'),
  ...
  .output(z.object({
    ...
    finishReason: z.string().describe('Generation finish reason (SUCCESS or CONTENT_FILTERED)'),
  }))
