...
export let generate3D = SlateTool.create(
  ...
  .input(z.object({
    ...
    textureResolution: z.enum(['512', '1024', '2048']).optional().default('1024').describe('Resolution of the output texture in pixels'),
    foregroundRatio: z.number().min(0.1).max(1.0).optional().describe('How much of the image is foreground (0.1-1.0, default 0.85)'),
    remesh: z.enum(['none', 'triangle', 'quad']).optional().describe('Remeshing algorithm to apply to the output mesh'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.generateStableFast3D({
      image: input.image,
      textureResolution: input.textureResolution ? parseInt(input.textureResolution) : undefined,
