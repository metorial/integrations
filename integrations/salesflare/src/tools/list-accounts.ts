...
export let listAccounts = SlateTool.create(
  ...
  .input(z.object({
    ...
    creationAfter: z.string().optional().describe('Filter accounts created after this date (ISO 8601)'),
    creationBefore: z.string().optional().describe('Filter accounts created before this date (ISO 8601)'),
    ...
    orderBy: z.array(z.string()).optional().describe('Sort order, e.g. ["name asc", "creation_date desc"]'),
  }))
  .output(z.object({
