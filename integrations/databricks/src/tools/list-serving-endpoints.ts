...
export let listServingEndpoints = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    endpoints: z.array(z.object({
      ...
      servedEntities: z.array(z.object({
        entityName: z.string().optional().describe('Name of the served model or entity'),
        entityVersion: z.string().optional().describe('Version of the served entity'),
        state: z.string().optional().describe('Entity deployment state'),
      })).optional().describe('Deployed entities/models'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let mapped = endpoints.map((e: any) => ({
      ...
      lastUpdatedTimestamp: e.last_updated_timestamp ? String(e.last_updated_timestamp) : undefined,
      endpointType: e.endpoint_type,
      servedEntities: (e.config?.served_entities ?? e.config?.served_models ?? []).map((s: any) => ({
