...
Use **import** to upload notebook content (base64-encoded). Use **export** to download a notebook. Use **delete** to remove a notebook or folder.`,
  ...
  .input(z.object({
    ...
    language: z.enum(['PYTHON', 'SCALA', 'SQL', 'R']).optional().describe('Notebook language (for import)'),
    format: z.enum(['SOURCE', 'HTML', 'JUPYTER', 'DBC']).optional().describe('Import/export format (default: SOURCE)'),
