...
export let manageServiceLinks = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    serviceLinkId: z.number().optional().describe('Service link ID (required for "update" and "delete" actions)'),
    name: z.string().optional().describe('Service link display name (required for "create")'),
    template: z.string().optional().describe('URL template with variables (required for "create")'),
  }))
  .output(z.object({
    serviceLink: z.object({
    ...
    }).optional().describe('Single service link'),
    serviceLinks: z.array(z.object({
      serviceLinkId: z.number().describe('Service link ID'),
      name: z.string().describe('Service link name'),
      template: z.string().describe('URL template'),
    })).optional().describe('List of service links'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'update') {
      if (!ctx.input.serviceLinkId) throw new Error('serviceLinkId is required for "update" action');
    ...
    if (ctx.input.action === 'delete') {
      if (!ctx.input.serviceLinkId) throw new Error('serviceLinkId is required for "delete" action');
