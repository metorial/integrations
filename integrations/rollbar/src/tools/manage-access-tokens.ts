...
Requires an **account-level** access token or a project token with write scope.`,
  ...
  .input(z.object({
    ...
    tokenValue: z.string().optional().describe('Access token string value (required for "update" and "delete" actions)'),
    name: z.string().optional().describe('Token name (required for "create" action)'),
    scopes: z.array(z.enum(['post_server_item', 'post_client_item', 'read', 'write'])).optional().describe('Token scopes (required for "create" action)'),
    rateLimitWindowSize: z.number().optional().describe('Rate limit window size in seconds (for "create" and "update")'),
    rateLimitWindowCount: z.number().optional().describe('Maximum number of calls in the rate limit window (for "create" and "update")'),
  }))
  .output(z.object({
    ...
    }).optional().describe('Single access token (for create/update)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'update') {
      ...
      let result = await client.updateProjectAccessToken(ctx.input.projectId, ctx.input.tokenValue, {
        rate_limit_window_size: ctx.input.rateLimitWindowSize,
        rate_limit_window_count: ctx.input.rateLimitWindowCount,
      });
      let token = mapToken(result?.result);
