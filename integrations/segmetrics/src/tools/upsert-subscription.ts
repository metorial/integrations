...
export let upsertSubscription = SlateTool.create(
  spec,
  {
...
All monetary amounts are specified in **cents**.`,
  ...
  .input(z.object({
    ...
    contactId: z.string().optional().describe('The contact ID. Contact must already exist if using this field.'),
    ...
    productName: z.string().optional().describe('Name of the product. If the product does not exist, it will be created.'),
    ...
    lastBillDate: z.string().optional().describe('Date the subscription was last billed (YYYY-MM-DD HH:MM:SS).'),
    ...
    trialStart: z.string().optional().describe('Trial period start date (YYYY-MM-DD HH:MM:SS).'),
    trialEnd: z.string().optional().describe('Trial period end date (YYYY-MM-DD HH:MM:SS).'),
    cancellationDate: z.string().optional().describe('Date the subscription was cancelled (YYYY-MM-DD HH:MM:SS).'),
    endDate: z.string().optional().describe('Date the subscription ends (YYYY-MM-DD HH:MM:SS).'),
  }))
  .output(z.object({
