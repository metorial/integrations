...
export let recordAdPerformance = SlateTool.create(
  spec,
  {
...
Spend amounts are in **cents**.`,
  ...
  .input(z.object({
    ...
    adName: z.string().optional().describe('Name of the ad. If provided with adId, the ad will be created/updated.'),
    ...
    }).optional().describe('Ad campaign to associate. Will be created if it does not exist.'),
    adSet: z.object({
    ...
    }).optional().describe('Ad set to associate. Will be created if it does not exist.'),
  }))
  .output(z.object({
