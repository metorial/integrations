...
export let createTransaction = SlateTool.create(
  ...
  .input(z.object({
    ...
    fields: z.record(z.string(), z.any()).optional().describe('Fields for the transaction (name, address, etc.)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.transactionTemplateId) attrs['transaction-template-id'] = ctx.input.transactionTemplateId;
...
export let tagTransaction = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    transactionId: z.string().describe('Transaction ID'),
    attributes: z.record(z.string(), z.any()).optional().describe('Updated transaction attributes'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new PersonaClient({ token: ctx.auth.token });
    let result: any;
