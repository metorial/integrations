...
export let createReport = SlateTool.create(
  spec,
  {
    name: 'Create Report',
    key: 'create_report',
...
Provide the report template ID and relevant attributes for the report type.`,
  ...
  .input(z.object({
    ...
    term: z.string().optional().describe('Search term (for adverse media / watchlist reports)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.addressSubdivision) attrs['address-subdivision'] = ctx.input.addressSubdivision;
    if (ctx.input.addressPostalCode) attrs['address-postal-code'] = ctx.input.addressPostalCode;
    if (ctx.input.addressCountryCode) attrs['address-country-code'] = ctx.input.addressCountryCode;
...
export let getReport = SlateTool.create(
  ...
  .output(z.object({
    ...
    attributes: z.record(z.string(), z.any()).optional().describe('Full report attributes including findings'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new PersonaClient({ token: ctx.auth.token });
...
export let listReports = SlateTool.create(
  ...
  .input(z.object({
    ...
    filterStatus: z.string().optional().describe('Filter by status (pending, ready, errored)'),
