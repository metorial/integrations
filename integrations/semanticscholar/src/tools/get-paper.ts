...
let paperDetailSchema = z.object({
  ...
  externalIds: z.record(z.string(), z.any()).nullable().optional().describe('External IDs (DOI, ArXiv, PMID, etc.)'),
  journal: z.object({
    name: z.string().nullable().optional(),
    volume: z.string().nullable().optional(),
    pages: z.string().nullable().optional(),
  }).nullable().optional().describe('Journal information'),
  publicationVenue: z.object({
    id: z.string().nullable().optional(),
    name: z.string().nullable().optional(),
    type: z.string().nullable().optional(),
    url: z.string().nullable().optional(),
  }).passthrough().nullable().optional().describe('Publication venue details'),
  citationStyles: z.object({
    bibtex: z.string().nullable().optional(),
  }).nullable().optional().describe('Citation styles'),
  embedding: z.object({
    model: z.string().nullable().optional(),
    vector: z.array(z.number()).nullable().optional(),
  }).nullable().optional().describe('SPECTER2 embedding vector'),
}).passthrough();

export let getPaper = SlateTool.create(
  spec,
  {
...
Use the **fields** parameter to control which metadata is returned.`,
    ...
    constraints: [
      'Batch lookup supports a maximum of 500 paper IDs.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    paperId: z.string().optional().describe('Single paper identifier (S2 ID, DOI:xxx, ARXIV:xxx, CorpusId:xxx, PMID:xxx, PMCID:xxx, MAG:xxx, ACL:xxx, or URL)'),
    paperIds: z.array(z.string()).optional().describe('Array of paper identifiers for batch lookup (max 500)'),
    fields: z.string().optional().describe('Comma-separated list of fields to return (e.g. "title,abstract,authors,year,citationCount,tldr,openAccessPdf,embedding,citationStyles"). Defaults to "paperId,title".'),
  }))
  .output(z.object({
    papers: z.array(paperDetailSchema.nullable()).describe('Paper details (null entries for papers not found in batch mode)'),
  }))
