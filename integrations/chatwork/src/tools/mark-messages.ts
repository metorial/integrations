...
export let markMessages = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['read', 'unread']).describe('"read" marks messages up to this ID as read; "unread" marks messages from this ID onward as unread')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = ctx.input.action === 'read'
