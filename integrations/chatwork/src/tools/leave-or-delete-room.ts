...
export let leaveOrDeleteRoom = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Deleting a room is irreversible and removes all content for all members.'
    ],
    tags: {
      destructive: true
    }
  ...
  .input(z.object({
    ...
    action: z.enum(['leave', 'delete']).describe('"leave" to leave the room, "delete" to permanently delete it')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: { success: true },
      message: ctx.input.action === 'delete'
