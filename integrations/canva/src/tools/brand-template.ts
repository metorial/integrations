...
export let listBrandTemplates = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    dataset: z.enum(['any', 'non_empty']).optional().describe('Filter by dataset: "non_empty" for templates that support autofill'),
    limit: z.number().min(1).max(100).optional().describe('Number of results per page (1-100, default 25)'),
...
export let getBrandTemplate = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    includeDataset: z.boolean().optional().describe('Whether to also fetch the autofill dataset for this template'),
  }))
  .output(z.object({
    brandTemplateId: z.string().describe('The brand template ID'),
    ...
    dataset: z.record(z.string(), z.object({
      type: z.string().describe('Field type: "image", "text", or "chart"'),
    })).optional().describe('Autofill dataset fields (when includeDataset is true)'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
...
export let autofillBrandTemplate = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    fields: z.record(z.string(), z.unknown()).describe('Key-value pairs of data fields to fill. Keys must match the template dataset field names.'),
    title: z.string().min(1).max(255).optional().describe('Title for the generated design (defaults to the template title)'),
  }))
  .output(z.object({
    ...
    design: z.object({
      ...
      createdAt: z.number(),
      updatedAt: z.number(),
    }).optional().describe('The generated design (present when status is "success")'),
