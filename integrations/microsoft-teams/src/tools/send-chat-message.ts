...
export let sendChatMessage = SlateTool.create(
  ...
  .input(z.object({
    ...
    memberUserIds: z.array(z.string()).optional().describe('User IDs to create a new chat with (if chatId not provided). Include the authenticated user if needed.'),
    content: z.string().describe('Message content'),
    contentType: z.enum(['text', 'html']).default('text').describe('Content type of the message'),
  }))
  .output(z.object({
