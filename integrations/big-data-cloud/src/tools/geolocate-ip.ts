...
let confidenceAreaPointSchema = z.object({
...
let locationSchema = z.object({
  ...
  isoPrincipalSubdivisionCode: z.string().optional().describe('Principal subdivision ISO code'),
...
export let geolocateIpTool = SlateTool.create(
  spec,
  {
    name: 'Geolocate IP',
    key: 'geolocate_ip',
...
If no IP is provided, the caller's own IP is geolocated.`,
  ...
  .input(z.object({
    ip: z.string().optional().describe('IPv4 or IPv6 address to geolocate. If omitted, the caller\'s IP is used.'),
  }))
  .output(z.object({
    ip: z.string().describe('The IP address that was geolocated'),
    isReachableGlobally: z.boolean().optional().describe('Whether the IP is present on the global routing table'),
    ...
    confidence: z.string().optional().describe('Geolocation confidence: low, moderate, or high'),
    confidenceArea: z.array(confidenceAreaPointSchema).optional().describe('Polygonal confidence area boundary points'),
    network: z.object({
      ...
      }).passthrough()).optional().describe('Network carriers / ISPs'),
    }).passthrough().optional().describe('Network/ASN information'),
    ...
    }).passthrough().optional().describe('Security hazard assessment for the IP'),
    securityThreat: z.string().optional().describe('Overall security threat classification'),
