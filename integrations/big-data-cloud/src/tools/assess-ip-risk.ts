...
export let assessIpRiskTool = SlateTool.create(
  spec,
  {
    name: 'Assess IP Risk',
    key: 'assess_ip_risk',
...
If no IP is provided, the caller's own IP is assessed.`,
  ...
  .input(z.object({
    ip: z.string().optional().describe('IPv4 or IPv6 address to assess. If omitted, the caller\'s IP is used.'),
  }))
  .output(z.object({
    ip: z.string().describe('The IP address assessed'),
    securityThreat: z.string().optional().describe('Overall threat level: low, moderate, high, or unknown'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (hazard.isSpamhausDrop || hazard.isSpamhausEdrop || hazard.isSpamhausAsnDrop) flags.push('Spamhaus listed');
    if (hazard.isBlacklistedUceprotect || hazard.isBlacklistedBlocklistDe) flags.push('Blacklisted');
