...
export let captureScreenshot = SlateTool.create(
  ...
  .input(z.object({
    ...
    language: z.string().optional().describe('ISO 639-1 language code for the browser (e.g. "en", "fr", "de")'),
    metadata: z.string().optional().describe('Custom metadata to attach'),
    webhookUrl: z.string().optional().describe('URL to receive a POST when the screenshot is ready'),
  }))
  .output(z.object({
    screenshotUid: z.string().describe('UID of the screenshot'),
    status: z.string().describe('Rendering status'),
    screenshotImageUrl: z.string().nullable().describe('URL of the captured screenshot image'),
