...
export let generateAnimatedGif = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum of 30 frames per animated GIF.',
      'GIF generation is asynchronous.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    ...
    inputMediaUrl: z.string().optional().describe('URL of a video to auto-generate frame thumbnails from'),
    fps: z.number().optional().describe('Frames per second (default varies)'),
    frameDurations: z.array(z.number()).optional().describe('Duration in seconds for each frame'),
