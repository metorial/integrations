...
export let scrapeIdealista = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    tld: z.string().optional().describe('Top-level domain: ".com" (Spain), ".it" (Italy), ".pt" (Portugal)'),
    lang: z.string().optional().describe('Language code for results (e.g. "en", "es", "it", "pt", "fr", "de")'),
  }))
  .output(z.object({
    results: z.record(z.string(), z.unknown()).describe('Structured property data from Idealista'),
  }))
