...
export let createRepositoryTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    organization: z.string().optional().describe('Organization namespace; if omitted, creates under the authenticated user'),
    private: z.boolean().optional().default(false).describe('Whether the repository should be private'),
    sdk: z.enum(['gradio', 'streamlit', 'docker', 'static']).optional().describe('SDK for Spaces (required for space type)'),
...
export let deleteRepositoryTool = SlateTool.create(
  spec,
  {
...
export let getRepositoryInfoTool = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    })).optional().describe('List of files in the repository'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new HubClient({ token: ctx.auth.token });
...
export let updateRepositoryVisibilityTool = SlateTool.create(
  spec,
  {
