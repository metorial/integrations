...
export let manageCategory = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['get_tree', 'get', 'create', 'update', 'delete', 'assign_product', 'remove_product']).describe('Category operation'),
    categoryId: z.number().optional().describe('Category ID (for get, update, delete, assign_product, remove_product)'),
    name: z.string().optional().describe('Category name'),
    parentId: z.number().optional().describe('Parent category ID'),
    isActive: z.boolean().optional().describe('Whether the category is enabled'),
    position: z.number().optional().describe('Sort position'),
    includeInMenu: z.boolean().optional().describe('Whether to include in navigation menu'),
    productSku: z.string().optional().describe('Product SKU (for assign_product, remove_product)'),
    productPosition: z.number().optional().describe('Product position within the category (for assign_product)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'assign_product') {
      ...
      await client.assignProductToCategory(ctx.input.categoryId, ctx.input.productSku, ctx.input.productPosition);
      return {
        output: { success: true },
    ...
    if (ctx.input.includeInMenu !== undefined) categoryData.include_in_menu = ctx.input.includeInMenu;
