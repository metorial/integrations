...
export let manageInventory = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['get_stock', 'get_sources', 'update_sources', 'check_salable']).describe('Inventory action'),
    sku: z.string().optional().describe('Product SKU (for get_stock, get_sources, check_salable)'),
    stockId: z.number().optional().describe('Stock ID to check salability against (for check_salable)'),
    sourceItems: z.array(sourceItemSchema).optional().describe('Source items to update (for update_sources)'),
  }))
  .output(z.object({
    stockItem: z.object({
    ...
    }).optional().describe('Legacy stock item details'),
    ...
    })).optional().describe('Multi-source inventory items'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'update_sources') {
      ...
      await client.saveSourceItems(ctx.input.sourceItems.map(s => ({
        sku: s.sku,
    ...
    if (ctx.input.stockId === undefined) throw new Error('stockId is required for check_salable action');
