...
let filterSchema = z.object({
  field: z.string().describe('Field name to filter by (e.g. name, sku, status, type_id, price, created_at)'),
  value: z.string().describe('Value to compare against'),
  conditionType: z.string().optional().describe('Comparison operator: eq, neq, gt, gteq, lt, lteq, like, in, nin, notnull, null (default: eq)'),
});
...
export let searchProducts = SlateTool.create(
  ...
  .input(z.object({
    filters: z.array(filterSchema).optional().describe('Search filters (combined with AND logic)'),
    sortField: z.string().optional().describe('Field to sort by (e.g. name, price, created_at)'),
