...
}).optional().describe('Transfer generated PDF to S3-compatible storage');
...
export let generatePdf = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Each PDF page costs 2 credits for canvas sizes up to 4000px.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    pages: z.array(pageSchema).min(1).describe('Array of pages to include in the PDF, each with a template UUID and optional layer data'),
    filename: z.string().optional().describe('Output filename'),
    imageQuality: z.enum(['high', 'medium', 'low']).optional().describe('Image quality for embedded images'),
    dpi: z.number().optional().describe('Output DPI (96, 150, or 300)'),
    colorMode: z.enum(['rgb', 'cmyk']).optional().describe('Output color mode'),
    colorProfile: z.string().optional().describe('Color profile (e.g., sRGB, CMYK profiles)'),
  ...
  .output(z.object({
    ...
    pdfUrl: z.string().nullable().describe('URL of the generated PDF (null if still queued)'),
