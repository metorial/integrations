...
let layerSchema = z.object({
  ...
  type: z.string().describe('Layer type (text, picture, shape, browserframe, barcode, rating, subtitle)'),
});
...
export let manageTemplates = SlateTool.create(
  ...
  .input(z.object({
    ...
    templateUuid: z.string().optional().describe('Template UUID (required for get, update, delete)'),
    title: z.string().optional().describe('Template title (required for create; optional for update)'),
    ...
    tags: z.array(z.string()).optional().describe('Tags for the template (max 10; for create or update)'),
    customData: z.string().optional().describe('Custom reference data (max 255 chars; for create or update)'),
    fromTemplate: z.string().optional().describe('UUID of template to duplicate from (for create)'),
    addToCollections: z.array(z.string()).optional().describe('Collection IDs to add the template to (for create)'),
    collectionId: z.string().optional().describe('Filter templates by collection ID (for list)'),
    titleFilter: z.string().optional().describe('Filter templates by title (for list)'),
    orderBy: z.string().optional().describe('Sort order: "created_at-asc", "created_at-desc", "updated_at-asc", "updated_at-desc", "title-asc", "title-desc" (for list)'),
    page: z.number().optional().describe('Page number for pagination (for list)'),
  }))
  .output(z.object({
    template: templateOutputSchema.optional().describe('Single template (for get, create, update)'),
    templates: z.array(templateOutputSchema).optional().describe('List of templates (for list)'),
    hasNextPage: z.boolean().optional().describe('Whether more pages are available (for list)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let mapTemplate = (t: { uuid: string; title: string; thumbnail: string | null; tags: string[]; custom_data: string | null; layers: Array<{ name: string; type: string }> }) => ({
      templateUuid: t.uuid,
      title: t.title,
    ...
    switch (ctx.input.action) {
      ...
      case 'get': {
        if (!ctx.input.templateUuid) throw new Error('templateUuid is required for "get" action');
      ...
      case 'update': {
        if (!ctx.input.templateUuid) throw new Error('templateUuid is required for "update" action');
      ...
      case 'delete': {
        if (!ctx.input.templateUuid) throw new Error('templateUuid is required for "delete" action');
