...
let entitySchema = z.object({
  ...
  lookups: z.array(z.string()).optional().describe('Lookup strategies (e.g., "keywords", "free-text")'),
  builtin: z.boolean().optional().describe('Whether this is a built-in entity'),
  keywords: z.array(keywordSchema).optional().describe('Keyword values for keyword-type entities'),
});

export let listEntities = SlateTool.create(
  spec,
  {
    name: 'List Entities',
    key: 'list_entities',
...
export let getEntity = SlateTool.create(
  spec,
  {
    name: 'Get Entity',
    key: 'get_entity',
  ...
  .input(z.object({
    entityName: z.string().describe('Name or ID of the entity (e.g., "wit$datetime" or "my_entity")'),
  }))
  .output(entitySchema)
  .handleInvocation(async (ctx) => {
    let client = new Client({
...
export let createEntity = SlateTool.create(
  spec,
  {
    name: 'Create Entity',
    key: 'create_entity',
  ...
  .input(z.object({
    ...
    lookups: z.array(z.enum(['keywords', 'free-text'])).optional().describe('Lookup strategies for the entity'),
    keywords: z.array(keywordSchema).optional().describe('Initial keyword values (for keyword-type entities)'),
  }))
  .output(entitySchema)
  .handleInvocation(async (ctx) => {
    let client = new Client({
...
export let updateEntity = SlateTool.create(
  spec,
  {
    name: 'Update Entity',
    key: 'update_entity',
...
export let deleteEntity = SlateTool.create(
  spec,
  {
    name: 'Delete Entity',
    key: 'delete_entity',
...
export let manageEntityKeywords = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['add_keyword', 'delete_keyword', 'add_synonym', 'delete_synonym']).describe('Action to perform'),
    keyword: z.string().describe('The canonical keyword value'),
    synonyms: z.array(z.string()).optional().describe('Synonym expressions (required for add_keyword)'),
    synonym: z.string().optional().describe('Single synonym (required for add_synonym and delete_synonym)'),
  }))
  .output(z.object({
