...
export let checkAvailability = SlateTool.create(
  ...
  .input(z.object({
    ...
    month: z.string().optional().describe('Month to check for available dates (e.g. "2024-03"). Returns available dates.'),
    date: z.string().optional().describe('Specific date to check for available time slots (e.g. "2024-03-15"). Returns available times.'),
    calendarId: z.number().optional().describe('Filter by specific calendar ID'),
    timezone: z.string().optional().describe('Timezone for time conversion (IANA format, e.g. "America/New_York")'),
  }))
  .output(z.object({
    availableDates: z.array(z.string()).optional().describe('List of available dates (when querying by month)'),
    availableTimes: z.array(z.string()).optional().describe('List of available time slots (when querying by date)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {},
      message: 'Please provide either a **month** (for available dates) or a **date** (for available time slots).',
    };
  }).build();
