...
export let expenseEvents = SlateTrigger.create(
  spec,
  {
    name: 'Expense Events',
    key: 'expense_events',
    description: 'Triggered when expense events occur, including creation, submission, approval, decline, reimbursement, updates, and deletion.',
  }
)
  .input(z.object({
  ...
  .webhook({
    autoRegisterWebhook: async (ctx) => {
      ...
      let result = await client.createWebhookCallback(ctx.input.webhookBaseUrl, EXPENSE_EVENTS);
    ...
    handleRequest: async (ctx) => {
      ...
      let eventId: string = data?.event_id ?? data?.id ?? `${eventType}-${expenseId ?? employmentId}-${Date.now()}`;
