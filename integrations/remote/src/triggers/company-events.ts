...
export let companyEvents = SlateTrigger.create(
  spec,
  {
    ...
    key: 'company_events',
    description: 'Triggered when company-level events occur, including company activation/archival, manager changes, billing documents, timesheet submissions, and contract amendment lifecycle events.',
  }
)
  .input(z.object({
  ...
  .webhook({
    autoRegisterWebhook: async (ctx) => {
      ...
      let result = await client.createWebhookCallback(ctx.input.webhookBaseUrl, COMPANY_EVENTS);
    ...
    handleRequest: async (ctx) => {
      ...
      let resourceId: string | undefined = data?.resource_id ?? data?.company_id ?? data?.employment_id ?? undefined;
      let eventType: string = data?.event_type ?? '';
      let eventId: string = data?.event_id ?? data?.id ?? `${eventType}-${resourceId ?? ''}-${Date.now()}`;
