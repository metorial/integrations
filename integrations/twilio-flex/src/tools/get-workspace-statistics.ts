...
export let getWorkspaceStatisticsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    scope: z.enum(['workspace', 'task_queue', 'worker']).default('workspace').describe('Statistics scope'),
    taskQueueSid: z.string().optional().describe('Task Queue SID (required when scope is task_queue)'),
    workerSid: z.string().optional().describe('Worker SID (required when scope is worker)'),
    minutesFilter: z.number().optional().describe('Filter cumulative statistics to the last N minutes'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.scope === 'task_queue') {
      if (!ctx.input.taskQueueSid) throw new Error('taskQueueSid is required for task_queue scope');
      result = await client.getTaskQueueStatistics(ctx.input.workspaceSid, ctx.input.taskQueueSid, params);
    } else if (ctx.input.scope === 'worker') {
      if (!ctx.input.workerSid) throw new Error('workerSid is required for worker scope');
      result = await client.getWorkerStatistics(ctx.input.workspaceSid, ctx.input.workerSid, params);
    } else {
