...
let workflowSchema = z.object({
  ...
  fallbackAssignmentCallbackUrl: z.string().optional().describe('Fallback assignment callback URL'),
  taskReservationTimeout: z.number().optional().describe('Task reservation timeout in seconds'),
  dateCreated: z.string().optional().describe('Date created'),
  dateUpdated: z.string().optional().describe('Date last updated')
});

export let manageWorkflowsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    workflowSid: z.string().optional().describe('Workflow SID (required for get/update/delete)'),
    friendlyName: z.string().optional().describe('Friendly name'),
    configuration: z.string().optional().describe('Workflow configuration as JSON string'),
    assignmentCallbackUrl: z.string().optional().describe('Assignment callback URL'),
    fallbackAssignmentCallbackUrl: z.string().optional().describe('Fallback assignment callback URL'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'update') {
      ...
      let w = await client.updateWorkflow(ctx.input.workspaceSid, ctx.input.workflowSid, params);
      return {
        output: {
          workflows: [{
            workflowSid: w.sid,
