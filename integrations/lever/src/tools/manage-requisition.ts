...
export let manageRequisitionTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    requisitionId: z.string().optional().describe('Requisition ID (required for update/delete)'),
    ...
    }).optional().describe('Compensation band'),
    ...
    status: z.enum(['open', 'onHold', 'closed', 'draft']).optional().describe('Requisition status'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'delete') {
      if (!ctx.input.requisitionId) throw new Error('requisitionId is required for delete action');
