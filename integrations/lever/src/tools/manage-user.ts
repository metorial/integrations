...
export let manageUserTool = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'deactivate', 'reactivate']).describe('Action to perform'),
    userId: z.string().optional().describe('User ID (required for update/deactivate/reactivate)'),
    name: z.string().optional().describe('User full name'),
    email: z.string().optional().describe('User email address'),
    accessRole: z.enum(['super admin', 'admin', 'team member', 'limited team member', 'interviewer']).optional().describe('User access role'),
    externalDirectoryId: z.string().optional().describe('External directory ID for HRIS integration'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'update') {
      ...
      if (ctx.input.externalDirectoryId) updateData.externalDirectoryId = ctx.input.externalDirectoryId;
    ...
    if (ctx.input.externalDirectoryId) createData.externalDirectoryId = ctx.input.externalDirectoryId;
