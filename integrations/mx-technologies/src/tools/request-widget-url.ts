...
export let requestWidgetUrl = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    widgetType: z.string().describe('Type of widget (e.g., "connect_widget", "pulse_widget", "budget_widget", "spending_widget")'),
    colorScheme: z.string().optional().describe('Color scheme (e.g., "light", "dark")'),
    currentInstitutionCode: z.string().optional().describe('Pre-select a specific institution in the widget'),
    currentMemberGuid: z.string().optional().describe('Pre-load a specific member for reconnection'),
    disableInstitutionSearch: z.boolean().optional().describe('Disable institution search in the widget'),
    mode: z.string().optional().describe('Widget mode (e.g., "verification", "aggregation")'),
    updateCredentials: z.boolean().optional().describe('Open widget in credential update mode'),
    waitForFullAggregation: z.boolean().optional().describe('Wait for full aggregation before closing'),
  }))
  .output(z.object({
