...
let suggestionSchema = z.object({
  resourceType: z.string().describe('Type of suggestion: events, attractions, venues, products'),
...
export let suggestSearchTool = SlateTool.create(
  spec,
  {
    name: 'Suggest Search',
    key: 'suggest_search',
  ...
  .input(z.object({
    ...
    resource: z.string().optional().describe('Limit to resource type: events, attractions, venues, products (comma-separated for multiple)'),
