...
export let subscriptionEvent = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggered when a subscription charge succeeds, fails, or when a subscription is cancelled. Covers all recurring billing lifecycle events.',
  }
)
  .input(z.object({
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let isSubscriptionEvent = eventType === 'subscription.cancelled'
    ...
    handleEvent: async (ctx) => {
      let type = ctx.input.eventType === 'subscription.cancelled'
