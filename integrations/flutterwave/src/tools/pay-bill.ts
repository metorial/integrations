...
export let payBill = SlateTool.create(
  ...
  .input(z.object({
    ...
    customer: z.string().describe('Customer identifier (phone number, meter number, smartcard number, etc.)'),
    amount: z.number().describe('Bill amount to pay'),
    type: z.string().describe('Bill type (AIRTIME, DATA, DSTV, GOTV, PHCN, etc.)'),
    recurrence: z.enum(['ONCE', 'HOURLY', 'DAILY', 'WEEKLY', 'MONTHLY']).optional().describe('Payment recurrence schedule'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.createBillPayment(
      ctx.input.billerCode,
      ctx.input.itemCode,
      {
