...
export let createLoginLink = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    targetUrl: z.string().describe('Target URL path within Postalytics (e.g. "dashboard", "template-create", "contact-import", "campaign-create")'),
    resourceId: z.string().optional().describe('Optional resource ID to target a specific template, campaign, etc.'),
  }))
  .output(z.object({
