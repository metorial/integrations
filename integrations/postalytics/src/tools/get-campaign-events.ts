...
export let getCampaignEvents = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    contactDataId: z.string().optional().describe('Specific contact data ID to filter events'),
  ...
  .output(z.object({
    events: z.record(z.string(), z.unknown()).describe('Campaign event data including delivery and response tracking details'),
  }))
