...
export let manageSuppressionLists = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    }).optional().describe('Contact to add to suppression list (required for add_contact)'),
  }))
  .output(z.object({
    suppressionLists: z.array(z.record(z.string(), z.unknown())).optional().describe('Array of suppression lists'),
