...
export let manageAccount = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['me', 'get', 'create', 'update', 'delete', 'integrations']).describe('The action to perform'),
    accountId: z.string().optional().describe('Account ID (required for get, delete)'),
    username: z.string().optional().describe('Username for new account (required for create)'),
    password: z.string().optional().describe('Password for new account (required for create)'),
  ...
  .output(z.object({
    ...
    integrations: z.record(z.string(), z.unknown()).optional().describe('Active integrations'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update') {
      ...
      if (ctx.input.emailAddress !== undefined) updateParams.email_address = ctx.input.emailAddress;
