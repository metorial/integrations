...
export let mailEvent = SlateTrigger.create(
  spec,
  {
    name: 'Mail Event',
    key: 'mail_event',
    description: 'Triggers when a mail lifecycle or online response event occurs for a campaign. Covers all event types: created, addressed, printed, mailed, in transit, in local area, processed for delivery, re-routed, return to sender, pURL opened, and pURL completed.',
  }
)
  .input(z.object({
    ...
    contactDataId: z.string().optional().describe('Contact data ID for matching back to internal records'),
    dropId: z.string().optional().describe('Campaign mail drop identifier'),
    recipient: z.record(z.string(), z.unknown()).optional().describe('Recipient address and info'),
    ...
    fields: z.record(z.string(), z.unknown()).optional().describe('Variable data fields from the contact record'),
    wasAddressUpdated: z.boolean().optional().describe('Whether the address was corrected by CASS/NCOA'),
    raw: z.record(z.string(), z.unknown()).describe('Full raw webhook payload'),
  }))
  .output(z.object({
    eventType: z.string().describe('Descriptive event type (e.g. created, mailed, purl_opened)'),
    eventDate: z.string().describe('Timestamp of the event'),
    expectedDeliveryDate: z.string().optional().describe('Estimated delivery date if available'),
    contactDataId: z.string().optional().describe('Contact data ID for matching to your records'),
    dropId: z.string().optional().describe('Campaign mail drop identifier'),
    ...
    wasAddressUpdated: z.boolean().optional().describe('Whether address was corrected by CASS/NCOA validation'),
    variableFields: z.record(z.string(), z.unknown()).optional().describe('Variable data fields from the contact record'),
    raw: z.record(z.string(), z.unknown()).describe('Full raw webhook payload'),
  }))
  .webhook({
        ...
        inputs: [
          {
            ...
            expectedDeliveryDate: (data.expectedDeliveryDate || data.expected_delivery_date) as string | undefined,
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: {
          ...
          recipientFirstName: (recipient.firstName || recipient.first_name) as string | undefined,
          ...
          recipientStreet: (recipient.address || recipient.address_street) as string | undefined,
