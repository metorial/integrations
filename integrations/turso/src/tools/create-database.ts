...
export let createDatabase = SlateTool.create(
  ...
  .input(z.object({
    ...
    seed: z.object({
      ...
      name: z.string().optional().describe('Name of the source database (for type "database")'),
      url: z.string().optional().describe('URL of the dump file (for type "dump")'),
      timestamp: z.string().optional().describe('Point-in-time recovery timestamp (ISO 8601) for database seed'),
    }).optional().describe('Seed configuration to initialize the database with data'),
    sizeLimit: z.string().optional().describe('Maximum database size (e.g., "256mb", "1gb")'),
    isSchema: z.boolean().optional().describe('Whether to create a schema database for multi-DB schema management'),
    schema: z.string().optional().describe('Name of a parent schema database to inherit schema from'),
  }))
  .output(z.object({
