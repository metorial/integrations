...
export let auditLogActivity = SlateTrigger.create(
  spec,
  {
    name: 'Audit Log Activity',
    key: 'audit_log_activity',
    description: 'Polls for new audit log entries in the organization. Triggers whenever new activity is detected, such as database creation, member changes, or token operations.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let lastSeenAt = (ctx.state as Record<string, unknown>)?.lastSeenAt as string | undefined;

      let result = await client.listAuditLogs({ page: 1, pageSize: 50 });
      let logs = result.audit_logs;

      let newLogs = lastSeenAt
        ? logs.filter((log) => log.created_at > lastSeenAt)
        : logs;

      let updatedLastSeenAt = logs.length > 0
        ? logs.reduce((max, log) => log.created_at > max ? log.created_at : max, logs[0]!.created_at)
        : lastSeenAt;
