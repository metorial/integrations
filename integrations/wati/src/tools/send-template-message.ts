...
unlike session messages which require an active conversation window. Supports dynamic parameter substitution and optional scheduling for future delivery.`,
  ...
  .input(z.object({
    ...
    target: z.string().describe('Recipient: phone number with country code (e.g. 14155552671), contact ID, or Channel:PhoneNumber.'),
    ...
    })).optional().describe('Dynamic parameter values for template placeholders.'),
    channel: z.string().optional().describe('Specific channel name or phone number to send from. Defaults to primary channel.'),
    scheduledAt: z.string().optional().describe('ISO 8601 timestamp to schedule the message for future delivery. If omitted, sends immediately.'),
  }))
  .output(z.object({
