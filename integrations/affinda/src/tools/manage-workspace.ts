...
export let listWorkspaces = SlateTool.create(
  ...
  .input(z.object({
    organizationIdentifier: z.string().describe('Organization identifier to list workspaces for.'),
  ...
  .output(z.object({
    workspaces: z.array(z.object({
      ...
      organizationIdentifier: z.string().optional().describe('Organization this workspace belongs to.'),
      documentCount: z.number().optional().describe('Number of documents in the workspace.')
  ...
  .handleInvocation(async (ctx) => {
    ...
    let workspaces = (Array.isArray(result) ? result : result.results ?? []).map((ws: any) => ({
