...
Set **wait** to \`true\` to receive parsed results immediately, or \`false\` to get the document identifier and poll later.`,
  ...
  .input(z.object({
    url: z.string().optional().describe('Publicly accessible URL of the document to process.'),
    workspaceIdentifier: z.string().optional().describe('Workspace identifier to upload the document to. Falls back to the configured default workspace.'),
    collectionIdentifier: z.string().optional().describe('Collection identifier to upload to. If omitted, the document is classified and routed automatically within the workspace.'),
    documentTypeIdentifier: z.string().optional().describe('Document type identifier. If specified, workspace must also be specified.'),
    wait: z.boolean().default(true).describe('If true, waits for parsing to complete and returns extracted data. If false, returns immediately with document metadata.'),
    identifier: z.string().optional().describe('Custom document identifier to assign.'),
    customIdentifier: z.string().optional().describe('Additional custom identifier for external reference.'),
    fileName: z.string().optional().describe('Override file name for the uploaded document.'),
    language: z.string().optional().describe('Language code hint for the document (e.g., "en", "fr").'),
    rejectDuplicates: z.boolean().optional().describe('If true, rejects duplicate documents without consuming credits.'),
    lowPriority: z.boolean().optional().describe('Mark as low priority for processing.'),
    compact: z.boolean().optional().describe('If true and wait is true, returns a compact version of the parsed result.'),
    deleteAfterParse: z.boolean().optional().describe('If true, deletes the document data after parsing. Only works with wait: true.'),
    enableValidationTool: z.boolean().optional().describe('If true, makes the document viewable in the Affinda validation interface.')
  }))
  .output(z.object({
    ...
    extractedData: z.any().optional().describe('Structured JSON data extracted from the document (only present when wait is true).')
  }))
