...
export let redactResume = SlateTool.create(
  spec,
  {
    name: 'Redact Resume',
    key: 'redact_resume',
...
Returns a base64-encoded PDF with the selected fields redacted. The original document is not modified.`,
  ...
  .input(z.object({
    documentIdentifier: z.string().describe('Identifier of the parsed resume document to redact.'),
    redactHeadshot: z.boolean().optional().describe('Redact the candidate headshot photo. Defaults to true.'),
    redactPersonalDetails: z.boolean().optional().describe('Redact personal details (name, address, phone, email). Defaults to true.'),
    redactWorkDetails: z.boolean().optional().describe('Redact work details such as company names. Defaults to true.'),
    redactEducationDetails: z.boolean().optional().describe('Redact education details such as university names. Defaults to true.'),
    redactReferees: z.boolean().optional().describe('Redact referee information. Defaults to true.'),
    redactLocations: z.boolean().optional().describe('Redact location names. Defaults to true.'),
    redactDates: z.boolean().optional().describe('Redact dates. Defaults to true.'),
    redactGender: z.boolean().optional().describe('Redact gender information. Defaults to true.'),
    redactPdfMetadata: z.boolean().optional().describe('Redact PDF metadata. Defaults to true.')
  }))
  .output(z.object({
    redactedPdfBase64: z.string().describe('Base64-encoded content of the redacted PDF file.')
  }))
