...
export let runCampaign = SlateTool.create(
  spec,
  {
    name: 'Run Campaign',
    key: 'run_campaign',
  ...
  .input(z.object({
    ...
    recipients: z.array(z.object({
      to: z.array(z.object({
        ...
        mobiles: z.string().optional().describe('Recipient mobile with country code'),
        variables: z.record(z.string(), z.object({
          type: z.string().describe('Variable type'),
          value: z.string().describe('Variable value'),
        })).optional().describe('Per-recipient template variables'),
      ...
      variables: z.record(z.string(), z.object({
        type: z.string().describe('Variable type'),
        value: z.string().describe('Variable value'),
      })).optional().describe('Group-level template variables'),
    ...
    })).optional().describe('Campaign attachments'),
    replyTo: z.array(z.object({
      name: z.string().optional(),
      email: z.string().optional(),
    })).optional().describe('Reply-to addresses'),
    ...
    timezoneBy: z.enum(['company', 'contact', 'manual']).optional().describe('Timezone source'),
  }))
  .output(z.object({
