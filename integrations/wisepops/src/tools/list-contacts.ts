...
Supports filtering by collection date and by specific campaign. Each contact includes the collection timestamp, campaign ID, IP address, country code, and all collected form fields.`,
  ...
  .input(z.object({
    collectedAfter: z.string().optional().describe('ISO 8601 UTC date to filter contacts collected after this date, e.g. "2024-01-01T00:00:00Z".'),
    wisepopId: z.number().optional().describe('Campaign ID to retrieve contacts from a specific popup campaign only.'),
    pageSize: z.number().min(1).max(1000).optional().describe('Number of results per request (1-1000). Defaults to 100.'),
  }))
  .output(z.object({
    contacts: z.array(z.object({
      collectedAt: z.string().describe('ISO 8601 timestamp when the contact was collected.'),
      ...
      fields: z.record(z.string(), z.string()).describe('Form fields collected from the visitor (e.g. email, name, custom fields).'),
