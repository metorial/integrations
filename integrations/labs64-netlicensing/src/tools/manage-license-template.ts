...
export let manageLicenseTemplate = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    licenseTemplateNumber: z.string().optional().describe('Template identifier. Required for update/delete. Optional for create.'),
    productModuleNumber: z.string().optional().describe('Parent product module number. Required for create.'),
    name: z.string().optional().describe('Template name'),
    active: z.boolean().optional().describe('Whether the template is active'),
    licenseType: z.string().optional().describe('License type (e.g., FEATURE, TIMEVOLUME, FLOATING, QUANTITY)'),
    price: z.number().optional().describe('License price'),
    currency: z.string().optional().describe('Currency code (e.g., EUR, USD)'),
    automatic: z.boolean().optional().describe('If true, license is automatically created for new licensees'),
    hidden: z.boolean().optional().describe('If true, template is hidden in the shop'),
    hideLicenses: z.boolean().optional().describe('If true, licenses from this template are hidden'),
    timeVolume: z.number().optional().describe('Time volume value for subscription/time-based models'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'delete') {
      if (!licenseTemplateNumber) throw new Error('licenseTemplateNumber is required for delete');
    ...
    if (action === 'update') {
      if (!licenseTemplateNumber) throw new Error('licenseTemplateNumber is required for update');
