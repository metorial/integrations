...
export let transferLicenses = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    sourceLicenseeNumber: z.string().describe('Licensee number to transfer licenses from (must be marked for transfer)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    await client.transferLicenses(ctx.input.targetLicenseeNumber, ctx.input.sourceLicenseeNumber);
    return {
      output: {
        transferred: true,
