...
export let manageSettings = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum([
      ...
      'list_forwarding',
      'list_send_as', 'update_send_as',
    ]).describe('Settings operation to perform.'),

    // Vacation responder fields
    enableAutoReply: z.boolean().optional().describe('Enable or disable vacation auto-reply.'),
    ...
    restrictToDomain: z.boolean().optional().describe('Only auto-reply within your organization domain.'),
    ...
    filterCriteria: z.object({
      ...
      negatedQuery: z.string().optional().describe('Negated query (messages NOT matching).'),
    ...
    }).optional().describe('Filter matching criteria (for create_filter).'),
    ...
    }).optional().describe('Actions to perform on matching messages (for create_filter).'),

    // Send-as fields
    sendAsEmail: z.string().optional().describe('Send-as email address (for update_send_as).'),
  ...
  .output(z.object({
    ...
    }).optional().describe('Vacation responder settings.'),
    ...
    })).optional().describe('Mail filters.'),

    createdFilter: z.object({
      filterId: z.string(),
    }).optional().describe('Newly created filter.'),

    forwardingAddresses: z.array(z.object({
      forwardingEmail: z.string(),
      verificationStatus: z.string(),
    })).optional().describe('Forwarding addresses.'),

    sendAsAliases: z.array(z.object({
    ...
    })).optional().describe('Send-as aliases.'),

    updatedSendAs: z.object({
      sendAsEmail: z.string(),
      displayName: z.string().optional(),
    }).optional().describe('Updated send-as alias.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update_send_as') {
      if (!ctx.input.sendAsEmail) throw new Error('sendAsEmail is required for update_send_as');
