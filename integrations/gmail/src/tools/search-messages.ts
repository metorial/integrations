...
Use the same search operators available in the Gmail search bar: \`from:\`, \`to:\`, \`subject:\`, \`has:attachment\`, \`after:\`, \`before:\`, \`is:unread\`, \`label:\`, etc.`,
  ...
  .input(z.object({
    query: z.string().optional().describe('Gmail search query using standard operators (from:, to:, subject:, has:attachment, etc.).'),
    labelIds: z.array(z.string()).optional().describe('Filter by label IDs (e.g., INBOX, SENT, STARRED, UNREAD).'),
    maxResults: z.number().optional().default(20).describe('Maximum number of messages to return (1-500).'),
    pageToken: z.string().optional().describe('Page token for fetching the next page of results.'),
