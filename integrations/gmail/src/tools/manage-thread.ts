...
export let manageThread = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['get', 'list', 'modify_labels', 'trash', 'untrash', 'delete']).describe('Thread operation to perform.'),
    threadId: z.string().optional().describe('Thread ID (required for get, modify_labels, trash, untrash, delete).'),
    query: z.string().optional().describe('Gmail search query (for list action).'),
    labelIds: z.array(z.string()).optional().describe('Filter by label IDs (for list) or labels to add (for modify_labels).'),
    addLabelIds: z.array(z.string()).optional().describe('Label IDs to add (for modify_labels).'),
    removeLabelIds: z.array(z.string()).optional().describe('Label IDs to remove (for modify_labels).'),
    maxResults: z.number().optional().default(20).describe('Maximum threads to return (for list).'),
    pageToken: z.string().optional().describe('Page token for list pagination.'),
    includeSpamTrash: z.boolean().optional().default(false).describe('Include threads in SPAM/TRASH (for list).'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Messages in the thread.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'modify_labels') {
      if (!ctx.input.threadId) throw new Error('threadId is required for modify_labels action');
      let thread = await client.modifyThread(ctx.input.threadId, ctx.input.addLabelIds, ctx.input.removeLabelIds);
