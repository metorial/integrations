...
export let manageCampaigns = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'metrics']).describe('Operation: list all campaigns, create a new blast campaign, or get campaign metrics'),
    name: z.string().optional().describe('Campaign name (required for create)'),
    listIds: z.array(z.number()).optional().describe('List IDs to target (required for create)'),
    templateId: z.number().optional().describe('Template ID for the campaign (required for create)'),
    suppressionListIds: z.array(z.number()).optional().describe('List IDs to suppress from the campaign'),
  ...
  .output(z.object({
    ...
    metrics: z.record(z.string(), z.any()).optional().describe('Campaign performance metrics'),
