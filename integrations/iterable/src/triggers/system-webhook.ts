...
export let systemWebhook = SlateTrigger.create(
  spec,
  {
    name: 'System Webhook',
    key: 'system_webhook',
    description: 'Receives system webhook events from Iterable including email, push, SMS, in-app, web push, embedded messaging, WhatsApp, subscription, and journey events. Configure the webhook URL in Iterable under Integrations > System Webhooks.',
    instructions: [
  ...
  .input(z.object({
    eventName: z.string().describe('Name of the event (e.g. emailSend, emailOpen, pushBounce, smsSend)'),
    ...
    eventFields: z.record(z.string(), z.any()).optional().describe('Event-specific data fields including campaign info, template info, timestamps'),
  ...
  .output(z.object({
    ...
    channel: z.string().optional().describe('Messaging channel (email, push, sms, inApp, webPush, embedded, whatsApp)'),
    eventFields: z.record(z.string(), z.any()).optional().describe('Additional event-specific data fields')
  }))
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      else if (eventName.startsWith('whatsApp') || eventName.startsWith('whatsapp')) channel = 'whatsApp';
      ...
      let type = eventName.replace(/([A-Z])/g, '.$1').toLowerCase().replace(/^\./, '');
      ...
      let eventId = raw.messageId || raw.transactionId || `${eventName}-${ctx.input.email || ctx.input.userId}-${dataFields.createdAt || dataFields.sentAt || Date.now()}`;
      ...
      return {
        ...
        output: {
          ...
          createdAt: dataFields.createdAt ? String(dataFields.createdAt) : (raw.createdAt ? String(raw.createdAt) : undefined),
          channel,
          eventFields: dataFields
        }
      };
