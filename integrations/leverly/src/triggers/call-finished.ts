...
export let callFinished = SlateTrigger.create(
  spec,
  {
    name: 'Call Finished',
    key: 'call_finished',
    description: 'Triggers when a call sequence with a lead finishes. Configure the webhook URL in your Leverly dashboard under HTTP Postback settings to receive call result data.',
  }
)
  .input(z.object({
    ...
    rawPayload: z.record(z.string(), z.any()).describe('Full raw payload from the Leverly HTTP postback'),
  }))
  .output(z.object({
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let callStatus = data.callStatus || data.call_status || data.CallStatus || data.status || '';
      let callDuration = data.callDuration || data.call_duration || data.CallDuration || data.duration || '';
      let callResult = data.callResult || data.call_result || data.CallResult || data.result || '';
      ...
      let recordingUrl = data.recordingUrl || data.recording_url || data.RecordingUrl || data.recording || '';
