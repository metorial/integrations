...
export let createCall = SlateTool.create(
  ...
  .input(z.object({
    phone: z.string().describe('Lead phone number (required). Include country code for best results, e.g. +11234567890.'),
    ...
    parameter1: z.string().optional().describe('Custom parameter 1 for additional lead data'),
    parameter2: z.string().optional().describe('Custom parameter 2 for additional lead data'),
    ...
    leadSource: z.string().optional().describe('Source of the lead (e.g., "Facebook Ads", "Website Form")'),
    ...
    vendorLeadId: z.string().optional().describe('External lead ID from the originating vendor or CRM'),
    callDelay: z.number().optional().describe('Delay in seconds before the call is initiated'),
    groupId: z.string().optional().describe('Leverly group ID to route the lead to a specific sales group'),
  }))
  .output(z.object({
    submitted: z.boolean().describe('Whether the lead was successfully submitted to Leverly'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let leadName = [ctx.input.firstName, ctx.input.lastName].filter(Boolean).join(' ') || 'Unknown';
