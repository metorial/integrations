...
let restorationSchema = z.object({
  upscale: z.enum(['smart_enhance', 'smart_resize', 'faces', 'digital_art', 'photo']).optional().describe('AI upscaling mode'),
  decompress: z.enum(['moderate', 'strong', 'auto']).optional().describe('Remove JPEG compression artifacts'),
  polish: z.boolean().optional().describe('Apply AI polish to enhance overall image quality'),
}).optional().describe('AI restoration operations');

let adjustmentsSchema = z.object({
  hdr: z.number().min(0).max(100).optional().describe('HDR intensity (0-100)'),
  exposure: z.number().min(-100).max(100).optional().describe('Exposure adjustment (-100 to 100)'),
...
}).optional().describe('Color and lighting adjustments');

let backgroundSchema = z.object({
  remove: z.union([z.boolean(), z.object({
    category: z.enum(['general', 'cars', 'products']).optional(),
    clipping: z.boolean().optional(),
  })]).optional().describe('Remove background. Pass true for auto-detection or an object with category.'),
  blur: z.union([z.boolean(), z.object({
    type: z.enum(['regular', 'lens']).optional(),
    level: z.enum(['low', 'medium', 'high']).optional(),
    category: z.enum(['general', 'cars', 'products']).optional(),
  })]).optional().describe('Blur background. Pass true for defaults or an object with options.'),
  color: z.string().optional().describe('Set background to a solid hex color (e.g. "#FFFFFF") or "transparent"'),
}).optional().describe('Background manipulation');

let resizingSchema = z.object({
  width: z.union([z.number(), z.string()]).optional().describe('Target width in pixels, percentage (e.g. "150%"), or "auto"'),
  height: z.union([z.number(), z.string()]).optional().describe('Target height in pixels, percentage (e.g. "150%"), or "auto"'),
  fit: z.enum(['bounds', 'cover', 'canvas', 'outpaint', 'crop']).optional().describe('Resize fit mode'),
}).optional().describe('Resize dimensions and fit mode');

let privacySchema = z.object({
  blurCarPlate: z.boolean().optional().describe('Blur license plates for privacy compliance'),
}).optional().describe('Privacy features');

let generativeSchema = z.object({
  ...
  }).optional().describe('Apply generative style transfer from a reference image'),
}).optional().describe('Generative AI operations');

let outputSchema = z.object({
  format: z.enum(['jpeg', 'png', 'webp', 'avif', 'tiff']).optional().describe('Output image format'),
  quality: z.number().min(1).max(100).optional().describe('Output quality (1-100, for lossy formats)'),
  destination: z.string().optional().describe('Storage URI destination (e.g. "storage://my-bucket/output/image.jpg")'),
...
Provide the source image as a public URL or storage URI and specify the desired operations. Results are returned as temporary URLs (24h lifespan) or saved to a connected storage destination.`,
    ...
    constraints: [
      'Supported input formats: BMP, GIF, JPEG, PNG, TIFF, WEBP, AVIF, HEIC.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    ...
    padding: z.string().optional().describe('Padding as percentage (e.g. "10%") or dual values (e.g. "5% 10%")'),
    privacy: privacySchema,
    generative: generativeSchema,
    output: outputSchema,
    async: z.boolean().optional().describe('If true, process asynchronously and return a task ID for polling'),
  }))
  .output(z.object({
    ...
    }).optional().describe('Metadata about the input image'),
    ...
    }).optional().describe('Metadata and URL of the processed image'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.output) {
      ...
      if (ctx.input.output.destination) outputConfig.destination = ctx.input.output.destination;
