...
export let refundInvoice = SlateTool.create(
  spec,
  {
    name: 'Refund Invoice',
    key: 'refund_invoice',
  ...
  .input(z.object({
    ...
    refundVariant: z.enum(['RateThen', 'CurrentRate', 'Fiat', 'OverpaidAmount', 'Custom']).optional().describe('Refund calculation method. RateThen = use original rate, CurrentRate = current exchange rate, Fiat = refund in fiat, Custom = specify a custom amount.'),
    paymentMethod: z.string().optional().describe('Payment method for the refund (e.g., BTC, BTC-LightningNetwork)'),
    name: z.string().optional().describe('Refund name/label'),
    description: z.string().optional().describe('Refund description'),
    subtractPercentage: z.number().optional().describe('Percentage to subtract from the refund (0-100)'),
    customAmount: z.number().optional().describe('Custom refund amount (when refundVariant is Custom)'),
    customCurrency: z.string().optional().describe('Currency for custom amount (when refundVariant is Custom)'),
  }))
  .output(z.object({
    refundLink: z.string().optional().describe('URL where the customer can claim the refund'),
