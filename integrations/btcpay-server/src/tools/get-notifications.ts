...
export let getNotifications = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['list', 'mark_read', 'delete']).optional().default('list').describe('Action to perform'),
    notificationId: z.string().optional().describe('Notification ID (for mark_read, delete)'),
