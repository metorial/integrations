...
export let emailEvents = SlateTrigger.create(
  spec,
  {
    name: 'Email Events',
    key: 'email_events',
    description: 'Receives email delivery and engagement events from SendGrid Event Webhook. Covers delivery events (processed, delivered, deferred, bounce, dropped), engagement events (open, click, unsubscribe, spam report, group unsubscribe/resubscribe), and account events.',
    instructions: [
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let inputs = body.map((event: any) => ({
        ...
        category: Array.isArray(event.category) ? event.category : (event.category ? [event.category] : undefined),
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: {
          ...
          timestamp: ctx.input.timestamp ? new Date(ctx.input.timestamp * 1000).toISOString() : new Date().toISOString(),
