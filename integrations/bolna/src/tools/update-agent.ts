...
export let updateAgent = SlateTool.create(
  ...
  .input(z.object({
    ...
    webhookUrl: z.string().nullable().optional().describe('New webhook URL, or null to remove'),
    ...
    synthesizer: z.object({
      ...
      }).optional().describe('Provider-specific config'),
      stream: z.boolean().optional(),
      bufferSize: z.number().optional(),
      audioFormat: z.string().optional()
    }).optional().describe('Updated synthesizer configuration'),
    telephonyProvider: z.enum(['twilio', 'plivo', 'exotel', 'vobiz', 'sip-trunk', 'default']).optional().describe('Telephony provider to use')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (input.welcomeMessage !== undefined) agentConfig.agent_welcome_message = input.welcomeMessage;
