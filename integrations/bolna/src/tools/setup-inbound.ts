...
export let setupInbound = SlateTool.create(
  spec,
  {
    name: 'Setup Inbound',
    key: 'setup_inbound',
  ...
  .input(z.object({
    action: z.enum(['link', 'unlink']).describe('"link" to assign an agent to a phone number, "unlink" to remove the assignment'),
    phoneNumberId: z.string().describe('ID of the phone number'),
    agentId: z.string().optional().describe('ID of the agent to link (required for "link" action)')
  }))
  .output(z.object({
