...
export let makeCall = SlateTool.create(
  ...
  .input(z.object({
    ...
    fromPhoneNumber: z.string().optional().describe('Caller phone number in E.164 format. If omitted, uses Bolna default.'),
    scheduledAt: z.string().optional().describe('ISO 8601 datetime with timezone to schedule the call'),
    userData: z.record(z.string(), z.any()).optional().describe('Dynamic context variables for personalizing the conversation (e.g., { "customerName": "John", "plan": "Premium" })'),
    retryConfig: z.object({
      ...
      retryOnStatuses: z.array(z.enum(['no-answer', 'busy', 'failed', 'error'])).optional().describe('Statuses that trigger a retry'),
      retryOnVoicemail: z.boolean().optional().describe('Retry when voicemail is detected'),
      retryIntervalsMinutes: z.array(z.number()).optional().describe('Delay between retries in minutes')
    }).optional().describe('Retry configuration for unanswered calls'),
    bypassCallGuardrails: z.boolean().optional().describe('Bypass time-based calling restrictions')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (input.retryConfig) {
      retryConfig = {
        ...
        ...(input.retryConfig.retryOnStatuses && { retry_on_statuses: input.retryConfig.retryOnStatuses }),
        ...(input.retryConfig.retryOnVoicemail !== undefined && { retry_on_voicemail: input.retryConfig.retryOnVoicemail }),
