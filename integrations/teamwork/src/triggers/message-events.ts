...
export let messageEvents = SlateTrigger.create(
  spec,
  {
    name: 'Message Events',
    key: 'message_events',
    description: 'Triggers when a message (discussion post) is created, updated, or deleted in Teamwork.',
  }
)
  .input(z.object({
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let messageId = message.id ? String(message.id) : (event.objectId ? String(event.objectId) : '');
      ...
      return {
        inputs: [{
          ...
          messageId,
          projectId: message.projectId ? String(message.projectId) : (message['project-id'] ? String(message['project-id']) : undefined),
          eventPayload: data,
        }],
      };
    },
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: {
          ...
          authorName: user.firstName ? `${user.firstName} ${user.lastName || ''}`.trim() : undefined,
          createdAt: message['created-on'] || message.createdOn || message.createdAt || undefined,
        },
      };
    },
  })
