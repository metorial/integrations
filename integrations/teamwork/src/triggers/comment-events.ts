...
export let commentEvents = SlateTrigger.create(
  spec,
  {
    name: 'Comment Events',
    key: 'comment_events',
    description: 'Triggers when a comment is created, updated, or deleted on any resource (tasks, milestones, notebooks, files) in Teamwork.',
  }
)
  .input(z.object({
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let commentId = comment.id ? String(comment.id) : (event.objectId ? String(event.objectId) : '');
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: {
          ...
          resourceId: comment.objectId ? String(comment.objectId) : (comment['commentable-id'] ? String(comment['commentable-id']) : undefined),
          projectId: comment.projectId ? String(comment.projectId) : (comment['project-id'] ? String(comment['project-id']) : undefined),
          authorId: user.id ? String(user.id) : undefined,
          authorName: user.firstName ? `${user.firstName} ${user.lastName || ''}`.trim() : (user['first-name'] ? `${user['first-name']} ${user['last-name'] || ''}`.trim() : undefined),
          createdAt: comment['created-on'] || comment.createdOn || comment.createdAt || undefined,
        },
      };
    },
  })
