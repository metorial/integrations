...
export let manageTimeEntry = SlateTool.create(
  ...
  .input(z.object({
    ...
    timeEntryId: z.string().optional().describe('Time entry ID (required for update/delete)'),
    projectId: z.string().optional().describe('Project ID (for create)'),
    taskId: z.string().optional().describe('Task ID (for create)'),
    personId: z.string().optional().describe('Person ID logging the time (required for create)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update') {
      if (!ctx.input.timeEntryId) throw new Error('timeEntryId is required to update a time entry');
    ...
    if (action === 'delete') {
      if (!ctx.input.timeEntryId) throw new Error('timeEntryId is required to delete a time entry');
