...
export let createBypassCodes = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'Bypass codes are single-use and should be shared securely with the user.',
    ],
  }
)
  .input(z.object({
    userId: z.string().describe('The Duo user ID to generate bypass codes for'),
    count: z.number().optional().describe('Number of bypass codes to generate (default: 1, max: 10)'),
    validSecs: z.number().optional().describe('Validity period in seconds (0 for no expiration)'),
  }))
  .output(z.object({
