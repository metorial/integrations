...
export let manageApplicationsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    clientId: z.string().optional().describe('Application client ID (required for get, update, delete)'),
    name: z.string().optional().describe('Application name (required for create)'),
    appType: z.enum(['native', 'spa', 'regular_web', 'non_interactive']).optional().describe('Application type'),
    ...
    allowedLogoutUrls: z.array(z.string()).optional().describe('Allowed logout redirect URLs'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'list') {
      ...
      return {
        output: { applications: apps.map((a: any) => ({ clientId: a.clientId, name: a.name, appType: a.appType, description: a.description })) },
