...
export let manageOrganizationsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    organizationId: z.string().optional().describe('Organization ID (required for get, update, delete)'),
    name: z.string().optional().describe('Organization name (required for create; must be lowercase and use hyphens)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      let branding = (ctx.input.logoUrl || ctx.input.primaryColor || ctx.input.pageBackgroundColor) ? {
        logoUrl: ctx.input.logoUrl,
        colors: {
          ...(ctx.input.primaryColor ? { primary: ctx.input.primaryColor } : {}),
          ...(ctx.input.pageBackgroundColor ? { page_background: ctx.input.pageBackgroundColor } : {}),
        },
      } : undefined;
    ...
    if (ctx.input.action === 'update') {
      if (!ctx.input.organizationId) throw new Error('organizationId is required for update action');
      let branding = (ctx.input.logoUrl || ctx.input.primaryColor || ctx.input.pageBackgroundColor) ? {
        logoUrl: ctx.input.logoUrl,
        colors: {
          ...(ctx.input.primaryColor ? { primary: ctx.input.primaryColor } : {}),
