...
export let getChapter = SlateTool.create(
  spec,
  {
    name: 'Get Chapter',
    key: 'get_chapter',
  ...
  .input(z.object({
    ...
    bookId: z.string().optional().describe('Book ID to list all chapters for that book (e.g., "GEN")'),
    chapterId: z.string().optional().describe('Specific chapter ID to retrieve content (e.g., "GEN.1")'),
    contentType: z.enum(['html', 'json', 'text']).optional().describe('Content format. Defaults to HTML.'),
    ...
    includeChapterNumbers: z.boolean().optional().describe('Include chapter numbers in the content'),
    includeVerseNumbers: z.boolean().optional().describe('Include verse numbers in the content'),
    includeVerseSpans: z.boolean().optional().describe('Include verse spans in the content'),
  }))
  .output(z.object({
    chapters: z.array(chapterSummarySchema).optional().describe('List of chapters (when listing)'),
    ...
    fumsId: z.string().optional().describe('FUMS tracking token for reporting scripture engagement'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    throw new Error('Either bookId (to list chapters) or chapterId (to retrieve content) must be provided.');
  })
  .build();
