...
export let postEvent = SlateTool.create(
  spec,
  {
    name: 'Post Event',
    key: 'post_event',
  ...
  .input(z.object({
    ...
    tags: z.array(z.string()).optional().describe('Tags to associate, e.g. ["env:production"]'),
    alertType: z.enum(['error', 'warning', 'info', 'success', 'user_update', 'recommendation', 'snapshot']).optional().describe('Alert type for the event'),
    aggregationKey: z.string().optional().describe('Key for aggregating related events'),
    sourceTypeName: z.string().optional().describe('Source type name, e.g. "nagios", "hudson", "jenkins"')
  }))
  .output(z.object({
