...
export let manageSlo = SlateTool.create(
  ...
  .input(z.object({
    ...
    type: z.enum(['monitor', 'metric']).optional().describe('SLO type (required for creation)'),
    description: z.string().optional().describe('SLO description'),
    tags: z.array(z.string()).optional().describe('Tags for the SLO'),
    thresholds: z.array(thresholdSchema).optional().describe('SLO target thresholds (required for creation)'),
    monitorIds: z.array(z.number()).optional().describe('Monitor IDs for monitor-based SLOs'),
    numeratorQuery: z.string().optional().describe('Numerator query for metric-based SLOs'),
    denominatorQuery: z.string().optional().describe('Denominator query for metric-based SLOs'),
  ...
  .output(z.object({
    ...
    })).optional().describe('SLO thresholds'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let query = numeratorQuery && denominatorQuery
