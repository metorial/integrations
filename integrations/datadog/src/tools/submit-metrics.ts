...
export let submitMetrics = SlateTool.create(
  spec,
  {
    name: 'Submit Metrics',
    key: 'submit_metrics',
  ...
  .input(z.object({
    series: z.array(z.object({
      ...
      type: z.number().optional().describe('Metric type: 0=unspecified, 1=count, 2=rate, 3=gauge'),
      points: z.array(z.tuple([z.number(), z.number()])).describe('Array of [timestamp, value] tuples'),
      host: z.string().optional().describe('Hostname to associate with the metric'),
      tags: z.array(z.string()).optional().describe('Tags to associate, e.g. ["env:production", "service:web"]')
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.submitMetrics(ctx.input.series as Array<{
      metric: string;
      type?: number;
