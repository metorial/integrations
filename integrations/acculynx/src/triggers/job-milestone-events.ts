...
export let jobMilestoneEventsTrigger = SlateTrigger.create(
  spec,
  {
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      let inputs = events.map((event: any) => ({
        topicName: event.topicName ?? event.topic ?? event.type ?? 'job.milestone.current_changed',
    ...
    handleEvent: async (ctx) => {
      ...
      let type = topicName === 'job.milestone.status.current_changed'
