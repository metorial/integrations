...
export let manageAppointmentsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    appointmentId: z.string().optional().describe('Specific appointment ID to get details (requires calendarId)'),
    jobId: z.string().optional().describe('Job ID to get or set the initial appointment'),
    startDate: z.string().optional().describe('Start date for appointment listing in YYYY-MM-DD format (requires calendarId)'),
    endDate: z.string().optional().describe('End date for appointment listing in YYYY-MM-DD format (requires calendarId)'),
    startDateTime: z.string().optional().describe('Start date/time for initial appointment (ISO 8601 format, for creating/updating)'),
    endDateTime: z.string().optional().describe('End date/time for initial appointment (ISO 8601 format, for creating/updating)'),
    appointmentCalendarId: z.string().optional().describe('Calendar to assign the initial appointment to'),
    appointmentDescription: z.string().optional().describe('Description for the initial appointment'),
    pageSize: z.number().optional().describe('Number of items per page'),
    pageStartIndex: z.number().optional().describe('Index of the first element to return'),
  }))
  .output(z.object({
    calendars: z.array(z.record(z.string(), z.any())).optional().describe('Available calendars'),
    appointments: z.array(z.record(z.string(), z.any())).optional().describe('Appointment list'),
    appointment: z.record(z.string(), z.any()).optional().describe('Appointment details'),
    initialAppointment: z.record(z.string(), z.any()).optional().describe('Initial appointment for a job'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.calendarId && ctx.input.appointmentId) {
      let appointment = await client.getAppointment(ctx.input.calendarId, ctx.input.appointmentId);
      return {
        output: { appointment },
    ...
    if (ctx.input.calendarId) {
      ...
      let appointments = Array.isArray(result) ? result : (result?.items ?? result?.data ?? []);
      return {
        output: { appointments },
    ...
    let calendars = Array.isArray(result) ? result : (result?.items ?? result?.data ?? [result]);
    return {
      output: { calendars },
