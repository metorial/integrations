...
export let listUserKpis = SlateTool.create(
  spec,
  {
...
export let assignKpiToUser = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    sortOrder: z.number().describe('Display order on KPI entry screen'),
    userTarget: z.number().optional().nullable().describe('User-specific target (overrides KPI default)')
  }))
  .output(z.object({
...
export let removeKpiFromUser = SlateTool.create(
  spec,
  {
