...
export let manageSourceFileTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['upload', 'update', 'delete', 'create_directory', 'create_branch']).describe('Action to perform'),
    fileName: z.string().optional().describe('File name (required for upload)'),
    fileContent: z.string().optional().describe('File content as a string (required for upload/update)'),
