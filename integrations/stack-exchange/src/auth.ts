...
export let auth = SlateAuth.create()
  ...
  .addOauth({
    ...
    scopes: [
      ...
      {
        title: 'Write Access',
        description: 'Ability to perform write operations such as posting comments and editing.',
        scope: 'write_access',
      },
      {
        title: 'Private Info',
    ...
    inputSchema: z.object({
      apiKey: z.string().describe('Your Stack Exchange API key (application key) from stackapps.com'),
    }),
    ...
    handleCallback: async (ctx) => {
      let response = await oauthAxios.post('/oauth/access_token/json', new URLSearchParams({
        client_id: ctx.clientId,
    ...
    getProfile: async (ctx: { output: { token: string; key: string }; input: { apiKey: string }; scopes: string[] }) => {
