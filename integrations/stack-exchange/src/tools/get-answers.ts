...
let answerSchema = z.object({
  ...
  lastActivityDate: z.string().optional().describe('When the answer was last edited (ISO 8601)'),
...
export let getAnswers = SlateTool.create(
  spec,
  {
    name: 'Get Answers',
    key: 'get_answers',
  ...
  .input(z.object({
    ...
    answerIds: z.array(z.string()).optional().describe('Retrieve specific answers by their IDs'),
    sort: z.enum(['activity', 'creation', 'votes']).optional().describe('How to sort answers'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let answers = result.items.map((a: any) => ({
      ...
      lastActivityDate: a.last_activity_date ? new Date(a.last_activity_date * 1000).toISOString() : undefined,
