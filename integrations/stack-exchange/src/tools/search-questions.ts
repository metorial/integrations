...
export let searchQuestions = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    tagged: z.string().optional().describe('Semicolon-delimited tags to filter by (e.g., "python;django")'),
    notTagged: z.string().optional().describe('Semicolon-delimited tags to exclude'),
    hasAcceptedAnswer: z.boolean().optional().describe('Filter to questions with/without an accepted answer'),
    ...
    fromDate: z.string().optional().describe('Only return questions created after this date (ISO 8601)'),
    toDate: z.string().optional().describe('Only return questions created before this date (ISO 8601)'),
    sort: z.enum(['activity', 'creation', 'votes', 'relevance']).optional().describe('How to sort results'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let useAdvanced = !!(ctx.input.query || ctx.input.hasAcceptedAnswer !== undefined || ctx.input.isClosed !== undefined || ctx.input.minAnswers || ctx.input.minViews || ctx.input.fromDate || ctx.input.toDate);
