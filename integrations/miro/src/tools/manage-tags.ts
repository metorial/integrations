...
export let createTag = SlateTool.create(
  ...
  .input(z.object({
    ...
    fillColor: z.string().optional().describe('Color of the tag (e.g., "red", "blue", "yellow")')
  }))
  .output(z.object({
...
export let getTags = SlateTool.create(
  ...
  .input(z.object({
    boardId: z.string().describe('ID of the board'),
    itemId: z.string().optional().describe('If provided, retrieves tags attached to this specific item'),
...
export let attachTag = SlateTool.create(
  spec,
  {
    name: 'Attach Tag',
    key: 'attach_tag',
    ...
    instructions: [
      'Tags created via the REST API require a board refresh to appear.'
    ],
    tags: {
      destructive: false,
      readOnly: false
...
export let detachTag = SlateTool.create(
  spec,
  {
    name: 'Detach Tag',
    key: 'detach_tag',
