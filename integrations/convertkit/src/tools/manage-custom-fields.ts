...
export let manageCustomFields = SlateTool.create(
  ...
  .output(z.object({
    ...
    customField: z.object({
      ...
      fieldKey: z.string().describe('Field key for use in Liquid tags'),
      fieldLabel: z.string().describe('Human-readable field label'),
    }).optional().describe('Single custom field (for create, update)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (input.action === 'list') {
      let result = await client.listCustomFields({ perPage: input.perPage, after: input.cursor });
    ...
    if (input.action === 'create') {
      ...
      return {
        output: {
          customField: { fieldId: f.id, fieldName: f.name, fieldKey: f.key, fieldLabel: f.label },
        },
