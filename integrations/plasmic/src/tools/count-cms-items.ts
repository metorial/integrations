...
export let countCmsItems = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    where: z.record(z.string(), z.unknown()).optional().describe('Filter conditions as a JSON object'),
    locale: z.string().optional().describe('Locale code for counting localized content'),
    draft: z.boolean().optional().describe('Whether to count draft (unpublished) versions. Requires CMS secret token'),
  }))
  .output(z.object({
