...
export let getProjectModel = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    mode: z.enum(['preview', 'published', 'versioned']).default('published').describe('Data retrieval mode'),
    version: z.string().optional().describe('Version number when using versioned mode'),
  }))
  .output(z.object({
    model: z.record(z.string(), z.unknown()).describe('Full JSON representation of the project model'),
  }))
  .handleInvocation(async (ctx) => {
    if (!ctx.auth.projectId || !ctx.auth.projectToken) {
      throw new Error('Project ID and project token are required to retrieve the project model');
    }
