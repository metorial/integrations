...
export let runProcScript = SlateTool.create(
  spec,
  {
    name: 'Run ProcScript',
    key: 'run_procscript',
...
Can run synchronously (waits for result) or in the background (fire-and-forget).`,
  ...
  .input(z.object({
    scriptName: z.string().describe('The filename of the ProcScript to execute (as shown in ProcFu editor)'),
    payload: z.record(z.string(), z.any()).optional().describe('Optional JSON object to pass to the script as the payload variable'),
    runInBackground: z.boolean().optional().default(false).describe('If true, executes the script in the background without waiting for results'),
  }))
  .output(z.object({
