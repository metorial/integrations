...
export let deleteMessage = SlateTool.create(
  ...
  .input(z.object({
    ...
    receiptHandle: z.string().optional().describe('Receipt handle of the single message to delete'),
    ...
    })).optional().describe('Array of messages to delete in batch (max 10). Use this instead of receiptHandle for batch deletions.'),
  }))
  .output(z.object({
    deleted: z.boolean().describe('Whether the deletion was successful'),
    successful: z.array(z.string()).optional().describe('Entry IDs that were successfully deleted (batch mode)'),
    ...
    })).optional().describe('Entries that failed to delete (batch mode)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (!ctx.input.receiptHandle) {
      throw new Error('Either "receiptHandle" for single deletion or "entries" for batch deletion must be provided.');
    }
