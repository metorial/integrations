...
export let sendMessage = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Message body size limit is 256 KB.',
      'Delay range is 0-900 seconds.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    ...
    messageAttributes: z.record(z.string(), z.object({
      dataType: z.string().describe('Attribute data type (e.g., "String", "Number", "Binary")'),
      stringValue: z.string().optional().describe('String value of the attribute'),
    })).optional().describe('Custom message attributes as key-value pairs'),
    messageGroupId: z.string().optional().describe('Message group ID for FIFO queues (required for FIFO)'),
  ...
  .output(z.object({
    ...
    md5OfMessageBody: z.string().describe('MD5 digest of the message body for integrity verification'),
