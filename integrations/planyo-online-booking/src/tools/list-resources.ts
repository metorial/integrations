...
export let listResources = SlateTool.create(
  ...
  .input(z.object({
    page: z.number().optional().describe('Page number (zero-based, max 100 results per page)'),
    listPublishedOnly: z.boolean().optional().describe('Only return published resources'),
    listReservableOnly: z.boolean().optional().describe('Only return directly reservable resources'),
    resourceTypes: z.enum(['resource', 'bundle', 'package']).optional().describe('Filter by resource type'),
    sort: z.string().optional().describe('Sort by: resname, site, location, or a custom property name'),
    customProperties: z.record(z.string(), z.string()).optional().describe('Filter by custom resource properties'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {
        resources,
        totalCount: result?.resource_count != null ? Number(result.resource_count) : resources.length,
