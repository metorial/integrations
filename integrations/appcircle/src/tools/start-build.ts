...
export let startBuild = SlateTool.create(
  ...
  .input(z.object({
    ...
    branchId: z.string().optional().describe('ID of the branch to build. Provide either branchId or branchName.'),
    branchName: z.string().optional().describe('Name of the branch to build. Provide either branchId or branchName.'),
    configurationId: z.string().optional().describe('ID of the build configuration to use'),
    environmentVariables: z.record(z.string(), z.string()).optional().describe('Key-value pairs of environment variables to pass to the build'),
  }))
  .output(z.object({
