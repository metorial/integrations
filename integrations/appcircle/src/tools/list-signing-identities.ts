...
export let listSigningIdentities = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    identityType: z.enum(['certificates', 'provisioning_profiles', 'keystores', 'all']).default('all')
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.identityType === 'all' || ctx.input.identityType === 'provisioning_profiles') {
    ...
    let total = (result.certificates?.length ?? 0) +
