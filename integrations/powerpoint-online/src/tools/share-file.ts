...
export let shareFile = SlateTool.create(
  ...
  .input(z.object({
    itemId: z.string().optional().describe('ID of the file to share. Provide either itemId or itemPath.'),
    ...
    shareType: z.enum(['link', 'invite']).describe('How to share: "link" creates a sharing link, "invite" sends invitations to specific users.'),
    linkType: z.enum(['view', 'edit', 'embed']).optional().describe('Type of sharing link. Required for "link" shareType.'),
    ...
    expirationDateTime: z.string().optional().describe('ISO 8601 expiration time for the sharing link.'),
    recipientEmails: z.array(z.string()).optional().describe('Email addresses of users to invite. Required for "invite" shareType.'),
    roles: z.array(z.enum(['read', 'write', 'owner'])).optional().describe('Roles to grant invited users. Default: ["read"]. Required for "invite" shareType.'),
    invitationMessage: z.string().optional().describe('Custom message included in the invitation email.'),
    sendNotification: z.boolean().optional().describe('Whether to send an email notification to invited users. Default: true.'),
  }))
  .output(z.object({
