...
let polygonSchema = z.object({
  bucket: z.number().describe('Bucket index (0-based) - smaller buckets represent closer areas'),
  geometry: z.unknown().describe('GeoJSON Polygon geometry with coordinates'),
});

export let calculateIsochrone = SlateTool.create(
  spec,
  {
  ...
  .input(
    z.object({
      ...
      timeLimit: z
        .number()
        .optional()
        .describe('Travel time limit in seconds (default 600). Used for isochrone calculation.'),
      distanceLimit: z
        .number()
        .optional()
        .describe('Travel distance limit in meters. Used for isodistance calculation (overrides timeLimit).'),
      buckets: z
        .number()
        .optional()
        .describe('Number of nested isochrone rings (1-20, default 1). Time/distance is divided evenly.'),
      reverseFlow: z
        .boolean()
        .optional()
        .describe('If true, compute areas from which the center point is reachable (default false)'),
    }),
  )
  .output(
    z.object({
