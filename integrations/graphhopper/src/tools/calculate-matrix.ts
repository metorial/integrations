...
let normalizePoints = (points?: Array<[number?, number?, ...unknown[]]>): Array<[number, number]> | undefined => {
...
Returns distances in meters, times in seconds, and routing weights. Useful for comparing travel times between many locations or as input for route optimization.`,
  ...
  .input(
    z.object({
      ...
      failFast: z
        .boolean()
        .optional()
        .describe('If false, returns null for unreachable pairs instead of failing (default: false)'),
    }),
  )
  .output(
    z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let rowCount = result.distances?.length || result.times?.length || result.weights?.length || 0;
    let colCount = rowCount > 0
      ? (result.distances?.[0]?.length || result.times?.[0]?.length || result.weights?.[0]?.length || 0)
      : 0;
