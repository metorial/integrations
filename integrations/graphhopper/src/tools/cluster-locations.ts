...
export let clusterLocations = SlateTool.create(
  spec,
  {
...
Supports capacity constraints (min/max quantity per cluster) and predefined cluster centers.`,
  ...
  .input(
    z.object({
      customers: z.array(customerSchema).min(1).describe('Customer/location points to cluster'),
      numClusters: z
        .number()
        .optional()
      ...
      minQuantity: z
        .number()
        .optional()
        .describe('Minimum total quantity per cluster'),
      maxQuantity: z
        .number()
        .optional()
        .describe('Maximum total quantity per cluster'),
      profile: z
        .enum([
  ...
  .output(
    z.object({
      clusters: z.array(clusterResultSchema).describe('Resulting clusters with assigned customers'),
