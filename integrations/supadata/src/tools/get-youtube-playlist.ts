...
export let getYouTubePlaylist = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    includeVideos: z.boolean().optional().describe('If true, also fetch the list of video IDs from the playlist'),
  ...
  .output(z.object({
    ...
    videoIds: z.array(z.string()).optional().describe('List of video IDs in the playlist (when includeVideos is true)'),
  }))
