...
export let auditDataAccess = SlateTool.create(
  spec,
  {
...
This helps with compliance and security monitoring by tracking API and UI data access patterns.`,
  ...
  .input(z.object({
    ...
    dimensions: z.array(z.enum([
      'accessorEmail', 'accessMechanism', 'epochTimeMicros', 'propertyId', 'propertyName',
      'quotaCategory', 'reportType', 'isRestrictedDataModel'
    ])).optional().describe('Dimensions for the access report. Common: accessorEmail, accessMechanism.'),
    metrics: z.array(z.enum([
      'accessCount'
    ])).optional().describe('Metrics for the access report. Default: accessCount.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let requestDimensions = (ctx.input.dimensions || ['accessorEmail', 'accessMechanism']).map(d => ({ dimensionName: d }));
