...
Also supports listing and creating Measurement Protocol secrets for a specific data stream.`,
  ...
  .input(
    z.object({
      action: z
        .enum(['list', 'get', 'create', 'update', 'delete', 'list_secrets', 'create_secret'])
        .describe(
          'Action to perform. "list_secrets" and "create_secret" manage Measurement Protocol secrets for a stream.'
        ),
      dataStreamId: z
        .string()
        .optional()
        .describe(
          'ID of the data stream (required for get, update, delete, list_secrets, and create_secret).'
        ),
      streamType: z
  ...
  .handleInvocation(async ctx => {
    ...
    if (action === 'update') {
      if (!ctx.input.dataStreamId)
        throw new Error('dataStreamId is required for update action.');
    ...
    if (action === 'delete') {
      if (!ctx.input.dataStreamId)
        throw new Error('dataStreamId is required for delete action.');
    ...
    if (action === 'list_secrets') {
      if (!ctx.input.dataStreamId)
        throw new Error('dataStreamId is required for list_secrets action.');
