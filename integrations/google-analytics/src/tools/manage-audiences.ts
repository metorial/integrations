...
export let manageAudiences = SlateTool.create(
  spec,
  {
...
Use **list** to see all audiences, **create** to define new audience segments, **update** to modify audience display name or description, and **archive** to remove them.`,
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'update', 'archive']).describe('Action to perform on audiences.'),
    audienceId: z.string().optional().describe('ID of the audience (required for update and archive actions).'),
    displayName: z.string().optional().describe('Display name for the audience (required for create, optional for update).'),
    description: z.string().optional().describe('Description of the audience.'),
    membershipDurationDays: z.number().optional().describe('Duration in days that a user remains in the audience (for create). Max 540. Set to -1 for maximum duration.'),
    filterClauses: z.array(z.object({
      clauseType: z.enum(['INCLUDE', 'EXCLUDE']).optional().describe('Whether to include or exclude users matching this clause.'),
      simpleFilter: z.object({
        scope: z.enum(['AUDIENCE_FILTER_SCOPE_UNSPECIFIED', 'AUDIENCE_FILTER_SCOPE_WITHIN_SAME_EVENT', 'AUDIENCE_FILTER_SCOPE_WITHIN_SAME_SESSION', 'AUDIENCE_FILTER_SCOPE_ACROSS_ALL_SESSIONS']).optional(),
        filterExpression: z.any().describe('The filter expression for this clause.'),
      }).optional(),
    })).optional().describe('Filter clauses defining the audience criteria (for create).'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      if (!ctx.input.displayName || !ctx.input.filterClauses) {
        throw new Error('displayName and filterClauses are required for creating an audience.');
      }
      let body: any = {
    ...
    if (ctx.input.action === 'update') {
      ...
      let result = await client.updateAudience(ctx.input.audienceId, updateFields.join(','), body);
      return {
        output: { audience: result },
