...
export let manageCustomMetrics = SlateTool.create(
  spec,
  {
...
Use **list** to see all custom metrics, **create** to add new ones, **update** to modify display names, descriptions, or measurement units, and **archive** to remove them.`,
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'update', 'archive']).describe('Action to perform on custom metrics.'),
    customMetricId: z.string().optional().describe('ID of the custom metric (required for update and archive actions).'),
    parameterName: z.string().optional().describe('Event parameter name for the custom metric (required for create). Cannot be changed after creation.'),
    displayName: z.string().optional().describe('Display name for the custom metric (required for create, optional for update).'),
    description: z.string().optional().describe('Description of the custom metric.'),
    scope: z.enum(['EVENT']).optional().describe('Scope of the custom metric (required for create). Currently only EVENT scope is supported.'),
    measurementUnit: z.enum(['STANDARD', 'CURRENCY', 'FEET', 'METERS', 'KILOMETERS', 'MILES', 'MILLISECONDS', 'SECONDS', 'MINUTES', 'HOURS']).optional().describe('Measurement unit for the custom metric (required for create).'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of custom metrics (for list action).'),
    customMetric: z.object({
      ...
      measurementUnit: z.string().optional(),
      restrictedMetricType: z.array(z.string()).optional(),
    }).optional().describe('Created or updated custom metric (for create/update actions).'),
    nextPageToken: z.string().optional(),
    archived: z.boolean().optional().describe('Whether the custom metric was archived (for archive action).'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      if (!ctx.input.parameterName || !ctx.input.displayName || !ctx.input.scope || !ctx.input.measurementUnit) {
        throw new Error('parameterName, displayName, scope, and measurementUnit are required for creating a custom metric.');
      }
    ...
    if (ctx.input.action === 'update') {
      ...
      let result = await client.updateCustomMetric(ctx.input.customMetricId, updateFields.join(','), body);
      return {
        output: { customMetric: result },
