...
export let listIdentities = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    let total = (result.meta as Record<string, unknown>)?.total as number ?? identities.length;
...
export let createIdentity = SlateTool.create(
  spec,
  {
...
export let updateIdentity = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    addNote: z.boolean().optional().describe('Whether to include a personal note.'),
    personalNote: z.string().optional().describe('Updated personal note text (max 250 characters).'),
  }))
  .output(z.object({
...
export let deleteIdentity = SlateTool.create(
  spec,
  {
