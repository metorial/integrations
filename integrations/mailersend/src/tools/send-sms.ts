...
Supports personalization variables using {{variable}} syntax for dynamic content per recipient.`,
  ...
  .input(z.object({
    from: z.string().describe('Sender phone number in E.164 format (must belong to your account).'),
    to: z.array(z.string()).min(1).describe('Recipient phone numbers in E.164 format (1-50).'),
    ...
    })).optional().describe('Dynamic content personalization per recipient.'),
  }))
  .output(z.object({
