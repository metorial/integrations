...
export let smsEvents = SlateTrigger.create(
  spec,
  {
    name: 'SMS Events',
    key: 'sms_events',
    description: 'Receive real-time notifications for SMS lifecycle events including sent, delivered, and failed status updates.',
  }
)
  .input(z.object({
  ...
  .webhook({
    autoRegisterWebhook: async (ctx) => {
      ...
      return {
        registrationDetails: { smsWebhooks: [] as Array<{ smsWebhookId: string; smsNumberId: string }> },
      };
    },

    autoUnregisterWebhook: async (ctx) => {
      let client = new Client({ token: ctx.auth.token });
      let details = ctx.input.registrationDetails as { smsWebhooks: Array<{ smsWebhookId: string; smsNumberId: string }> };
