...
export let listFormats = SlateTool.create(
  spec,
  {
...
Useful for checking if a specific conversion is supported before attempting it.`,
  ...
  .input(z.object({
    inputFormat: z.string().optional().describe('Filter by input format (e.g., "pdf", "docx")'),
    outputFormat: z.string().optional().describe('Filter by output format (e.g., "pdf", "png")'),
