...
Useful for creating preview images, document thumbnails, or video frames.`,
  ...
  .input(z.object({
    ...
    outputFormat: z.enum(['png', 'jpg', 'webp']).default('png').describe('Thumbnail output format'),
    ...
    timestamp: z.string().optional().describe('Timestamp for video thumbnails (e.g., "00:00:05")'),
    tag: z.string().optional().describe('Tag to label the job'),
    waitForCompletion: z.boolean().optional().default(true).describe('Wait for thumbnail generation to complete'),
  }))
  .output(z.object({
