...
export let manageSitemap = SlateTool.create(
  spec,
  {
    name: 'Manage Sitemap',
    key: 'manage_sitemap',
  ...
  .input(z.object({
    ...
    sitemapUrl: z.string().optional().describe('The sitemap URL — required for "get", "submit", and "delete" actions'),
    sitemapIndex: z.string().optional().describe('For "list" action only: filter sitemaps that belong to this sitemap index URL')
  }))
  .output(z.object({
    ...
    sitemaps: z.array(z.object({
      ...
      lastSubmitted: z.string().optional().describe('Date/time of submission (RFC 3339)'),
      lastDownloaded: z.string().optional().describe('Date/time last downloaded by Google (RFC 3339)'),
      isPending: z.boolean().optional().describe('Whether the sitemap is still pending processing'),
      isSitemapsIndex: z.boolean().optional().describe('Whether this is a sitemap index file'),
      type: z.string().optional().describe('Sitemap type (e.g., sitemap, rssFeed, atomFeed)'),
      ...
      contents: z.array(z.object({
        ...
        submitted: z.number().optional().describe('Number of submitted URLs of this type')
      })).optional().describe('Breakdown by content type')
    })).optional().describe('List of sitemaps (for "list" and "get" actions)')
  }))
