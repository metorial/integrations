...
export let manageSite = SlateTool.create(
  spec,
  {
    name: 'Manage Site',
    key: 'manage_site',
  ...
  .input(z.object({
    action: z.enum(['add', 'remove', 'get']).describe('Action to perform on the site property'),
    siteUrl: z.string().describe('The site URL (e.g., "http://www.example.com/" or "sc-domain:example.com")')
  }))
  .output(z.object({
    siteUrl: z.string().describe('The site URL'),
    permissionLevel: z.string().optional().describe('User permission level (returned for "get" and "add" actions)'),
