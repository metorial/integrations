...
export let manageApplication = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['Approved', 'Denied']).optional().describe('New status for the application'),
    filterStatus: z.string().optional().describe('Filter by status when listing (Pending, Approved, Denied)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.applicationId && ctx.input.status) {
      let app = await client.updateApplication(ctx.input.teamSlug, ctx.input.applicationId, ctx.input.status);
      return {
        output: {
          applications: [{
