...
export let queryRecords = SlateTool.create(
  ...
  .input(z.object({
    tableName: z.string().describe('Name of the ServiceNow table to query (e.g. "incident", "change_request", "sys_user")'),
    query: z.string().optional().describe('Encoded query string to filter records (e.g. "priority=1^state!=7")'),
    fields: z.array(z.string()).optional().describe('List of field names to return. If omitted, all fields are returned.'),
    limit: z.number().optional().default(20).describe('Maximum number of records to return (default 20)'),
    ...
    displayValue: z.enum(['true', 'false', 'all']).optional().describe('Return display values, actual values, or both'),
  }))
  .output(z.object({
    records: z.array(z.record(z.string(), z.any())).describe('Array of matching records'),
    totalCount: z.number().optional().describe('Total number of matching records (if available)'),
