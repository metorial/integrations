...
export let triggerJobRunTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    cause: z.string().optional().describe('Reason for triggering the run (shown in run history)'),
    ...
    schemaOverride: z.string().optional().describe('Override the target schema for this run'),
    dbtVersionOverride: z.string().optional().describe('Override the dbt version for this run'),
    threadsOverride: z.number().optional().describe('Override the number of threads'),
    targetNameOverride: z.string().optional().describe('Override the target.name context variable'),
    generateDocsOverride: z.boolean().optional().describe('Override whether docs are generated'),
    timeoutSecondsOverride: z.number().optional().describe('Override the run timeout in seconds'),
    stepsOverride: z.array(z.string()).optional().describe('Override the list of dbt commands to execute')
  }))
  .output(z.object({
    ...
    status: z.number().describe('Run status code (1=Queued, 2=Starting, 3=Running, 10=Success, 20=Error, 30=Cancelled)'),
