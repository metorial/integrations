...
Available filter fields include: \`stages\`, \`status\` (qualified/disqualified), \`tags\`, \`sources\`, \`departments\`, \`offers\`, among others.`,
  ...
  .input(z.object({
    ...
    filters: z.string().optional().describe('JSON array of filter objects for advanced search, e.g. [{"field": "status", "in": ["qualified"]}]'),
    limit: z.number().optional().describe('Maximum number of candidates to return (default 60, max 10000)'),
    page: z.number().optional().describe('Page number for paginated results (used with advanced search)'),
    sortBy: z.string().optional().describe('Sort field: created_at_desc, created_at_asc, candidate_name_asc, candidate_name_desc, relevance, etc.'),
  ...
  .output(z.object({
    ...
    total: z.number().optional().describe('Total number of matching candidates (advanced search only)'),
  }))
