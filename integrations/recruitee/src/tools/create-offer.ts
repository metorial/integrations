...
export let createOffer = SlateTool.create(
  ...
  .input(z.object({
    ...
    kind: z.enum(['job', 'talent_pool']).optional().describe('Type: "job" (default) or "talent_pool"'),
    ...
    status: z.enum(['draft', 'published', 'internal', 'closed', 'archived']).optional().describe('Initial status. Default is "draft"'),
  }))
  .output(z.object({
