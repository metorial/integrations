...
let locationInfoSchema = z.object({
...
let fundingInfoSchema = z.object({
...
let companyResultSchema = z.object({
  ...
  revenueRange: z.object({
    ...
    description: z.string().optional(),
  }).optional().describe('Revenue range'),
  ...
  companyHierarchy: z.object({
    isUltimate: z.boolean().optional(),
    parent: z.object({ companyId: z.string().optional(), name: z.string().optional() }).optional(),
    ultimateParent: z.object({ companyId: z.string().optional(), name: z.string().optional() }).optional(),
  }).optional().describe('Company hierarchy (parent/ultimate parent)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let results = (result.results ?? []).map((r: any) => ({
      ...
      companyHierarchy: r.companyHierarchy ? {
        ...
        parent: r.companyHierarchy.parent ? { companyId: r.companyHierarchy.parent.id, name: r.companyHierarchy.parent.name } : undefined,
        ultimateParent: r.companyHierarchy.ultimateParent ? { companyId: r.companyHierarchy.ultimateParent.id, name: r.companyHierarchy.ultimateParent.name } : undefined,
      } : undefined,
