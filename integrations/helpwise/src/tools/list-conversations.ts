...
export let listConversations = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.string().optional().describe('Filter by conversation status (e.g., open, closed)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let conversations = Array.isArray(result) ? result : (result.conversations ?? result.threads ?? result.data ?? []);
