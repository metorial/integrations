...
export let listMessages = SlateTool.create(
  ...
  .input(z.object({
    ...
    conversationId: z.string().describe('ID of the conversation/thread to retrieve messages from'),
    limit: z.number().optional().describe('Maximum number of messages to return'),
    page: z.number().optional().describe('Page number for pagination'),
  }))
  .output(z.object({
    messages: z.array(z.record(z.string(), z.any())).describe('List of messages in the conversation'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let messages = Array.isArray(result) ? result : (result.messages ?? result.emails ?? result.data ?? []);
