...
export let conversationEvents = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers when conversation lifecycle events occur, including creation, closure, reopening, deletion, assignment, note additions, and tag applications.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    conversationId: z.string().describe('ID of the affected conversation'),
    mailboxId: z.string().optional().describe('ID of the mailbox the conversation belongs to'),
    ...
    tagName: z.string().optional().describe('Name of the tag applied (for tag_applied events)'),
  ...
  .webhook({
    ...
    autoUnregisterWebhook: async (ctx) => {
      let client = new Client({ token: ctx.auth.token });
      let details = ctx.input.registrationDetails as { webhooks: Array<{ webhookId: string; eventType: string }> };
    ...
    handleRequest: async (ctx) => {
      ...
      let conversationId = (
        conversation.id ?? conversation.conversation_id ?? conversation.thread_id ?? data.conversation_id ?? data.thread_id ?? ''
      ).toString();
    ...
    handleEvent: async (ctx) => {
      ...
      if (!normalizedType.startsWith('conversation.') && !normalizedType.startsWith('conversation_')) {
      ...
      return {
        ...
        output: {
          conversationId,
          mailboxId: (conversation.mailbox_id ?? conversation.mailboxId ?? rawPayload.mailbox_id ?? '').toString() || undefined,
          subject: conversation.subject ?? rawPayload.subject,
          status: conversation.status ?? rawPayload.status,
          assignedTo: (rawPayload.assigned_to ?? rawPayload.assignee ?? conversation.assigned_to ?? '').toString() || undefined,
          tagName: rawPayload.tag_name ?? rawPayload.tag?.name,
          noteBody: rawPayload.note?.body ?? rawPayload.note_body,
          customerEmail: conversation.customer_email ?? conversation.from_email ?? rawPayload.customer_email,
