...
export let messageEvents = SlateTrigger.create(
  spec,
  {
    name: 'Message Events',
    key: 'message_events',
    description: 'Triggers when a new message is sent or received in a conversation, including agent replies to customers and customer replies to emails.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    messageId: z.string().describe('ID of the message'),
    conversationId: z.string().optional().describe('ID of the conversation the message belongs to'),
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      let messageId = (
        message.id ?? message.message_id ?? data.message_id ?? ''
      ).toString();
    ...
    handleEvent: async (ctx) => {
      ...
      let normalizedType = eventType.includes('agent') ? 'message.agent_reply' : 'message.customer_reply';
      ...
      return {
        ...
        output: {
          messageId,
          conversationId: (message.thread_id ?? message.conversation_id ?? rawPayload.thread_id ?? rawPayload.conversation_id ?? '').toString() || undefined,
          mailboxId: (message.mailbox_id ?? rawPayload.mailbox_id ?? '').toString() || undefined,
