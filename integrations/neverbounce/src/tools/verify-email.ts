...
export let verifyEmailTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    addressInfo: z.boolean().optional().describe('Include parsed address components (host, domain, TLD, etc.)'),
    creditsInfo: z.boolean().optional().describe('Include remaining credit balance information'),
    timeout: z.number().optional().describe('Timeout in seconds for the verification attempt'),
  }))
  .output(z.object({
    result: z.string().describe('Verification result: valid, invalid, disposable, catchall, or unknown'),
    flags: z.array(z.string()).describe('Verification flags (e.g., has_dns, has_dns_mx, smtp_connectable)'),
    suggestedCorrection: z.string().describe('Suggested email correction if a typo was detected, empty string if none'),
