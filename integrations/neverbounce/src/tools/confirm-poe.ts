...
export let confirmPoeTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    verificationResult: z.string().describe('The verification result returned by the widget (e.g., valid, invalid)'),
    transactionId: z.string().describe('The transaction ID from the widget (nb-transaction-id)'),
    confirmationToken: z.string().describe('The confirmation token from the widget (nb-confirmation-token)'),
  }))
  .output(z.object({
    tokenConfirmed: z.boolean().describe('Whether the verification token is confirmed as authentic'),
  }))
