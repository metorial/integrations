...
export let listSubmissions = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum 50 submissions per page.',
    ],
    tags: {
      destructive: false,
      readOnly: true,
  ...
  .input(z.object({
    ...
    limit: z.number().min(1).max(50).optional().describe('Number of results per page (1-50, default: 25)'),
    page: z.number().min(1).optional().describe('Page number for pagination (default: 1)'),
    sort: z.string().optional().describe('Sort field and order, e.g. "createdAt:-1" for newest first'),
    filter: z.string().optional().describe('Comma-separated filters, e.g. "date:7,spam:false,attachments:true"'),
  }))
  .output(z.object({
    ...
    pagination: z.object({
      ...
      previousPage: z.number().nullable().describe('Previous page number, or null if on first page'),
      nextPage: z.number().nullable().describe('Next page number, or null if on last page'),
      totalPages: z.number().describe('Total number of pages'),
      totalSubmissions: z.number().describe('Total number of submissions matching the query'),
    }).describe('Pagination details'),
    submissions: z.array(z.record(z.string(), z.any())).describe('Array of submission objects, each containing the submitted fields and metadata'),
  }))
