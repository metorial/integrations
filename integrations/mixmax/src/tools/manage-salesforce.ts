...
export let searchSalesforce = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    email: z.string().optional().describe('Email address to look up a specific contact or lead'),
  }))
  .output(z.object({
...
export let manageSalesforceRecord = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    objectType: z.enum(['account', 'contact', 'lead', 'opportunity', 'task']).describe('Salesforce object type'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'update') {
      ...
      await client.updateSalesforceRecord(ctx.input.objectType, ctx.input.recordId, ctx.input.fields);
      return {
        output: {
