...
export let verifySpeaker = SlateTool.create(
  spec,
  {
    name: 'Verify Speaker',
    key: 'verify_speaker',
...
Uses text-independent verification — the speaker can say anything.`,
  ...
  .input(z.object({
    profileId: z.string().describe('ID of the enrolled verification profile to match against'),
    audioBase64: z.string().describe('Base64-encoded audio data (WAV PCM 16kHz mono) of the speaker to verify'),
  }))
  .output(z.object({
