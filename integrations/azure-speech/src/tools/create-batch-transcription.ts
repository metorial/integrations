...
export let createBatchTranscription = SlateTool.create(
  spec,
  {
...
The job runs asynchronously — use the **Get Batch Transcription** tool to check status and retrieve results.`,
  ...
  .input(z.object({
    ...
    contentUrls: z.array(z.string()).optional().describe('List of audio file URLs to transcribe. Mutually exclusive with contentContainerUrl.'),
    contentContainerUrl: z.string().optional().describe('Azure Blob Storage container URL containing audio files. Mutually exclusive with contentUrls.'),
    timeToLiveHours: z.number().optional().describe('Hours to retain results after completion (6-744, default 48)'),
    wordLevelTimestampsEnabled: z.boolean().optional().describe('Include word-level timestamps in results'),
    diarizationEnabled: z.boolean().optional().describe('Enable speaker diarization for 2 speakers'),
    diarizationMinSpeakers: z.number().optional().describe('Minimum number of speakers for diarization (for 3+ speakers)'),
    diarizationMaxSpeakers: z.number().optional().describe('Maximum number of speakers for diarization (max 36)'),
    punctuationMode: z.enum(['None', 'Dictated', 'Automatic', 'DictatedAndAutomatic']).optional().describe('Punctuation handling mode'),
    profanityFilterMode: z.enum(['None', 'Masked', 'Removed', 'Tags']).optional().describe('Profanity filter mode'),
    modelUri: z.string().optional().describe('Full URI of a custom or Whisper model to use instead of the default base model'),
    languageIdentificationLocales: z.array(z.string()).optional().describe('Candidate locales for language identification (2-10 locales)'),
  }))
  .output(z.object({
    transcriptionId: z.string().describe('Unique identifier of the created transcription job'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let diarization = (input.diarizationMinSpeakers || input.diarizationMaxSpeakers)
      ? {
