...
export let listSupportedConversions = SlateTool.create(
  spec,
  {
...
Use this to dynamically determine what conversions are available before performing them.`,
  ...
  .input(z.object({
    sourceFormat: z.string().optional().describe('List all formats this source can convert to (e.g., "pdf", "docx")'),
    destinationFormat: z.string().optional().describe('List all formats that can convert to this destination (e.g., "pdf", "jpg")'),
  }))
  .output(z.object({
    canConvert: z.boolean().nullable().describe('Whether the specific source→destination conversion is supported (when both formats provided)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.sourceFormat && ctx.input.destinationFormat) {
      let supported = await client.canConvert(ctx.input.sourceFormat, ctx.input.destinationFormat);
      return {
        output: {
          canConvert: supported,
          conversions: supported
            ? [{ sourceFormat: ctx.input.sourceFormat, destinationFormat: ctx.input.destinationFormat }]
            : [],
        },
        message: supported
