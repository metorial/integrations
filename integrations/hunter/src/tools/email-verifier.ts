...
export let emailVerifier = SlateTool.create(
  ...
  .output(z.object({
    ...
    status: z.string().describe('Verification status: valid, invalid, accept_all, webmail, disposable, or unknown'),
    ...
    disposable: z.boolean().nullable().describe('Whether the email is from a disposable provider'),
    webmail: z.boolean().nullable().describe('Whether the email is from a webmail provider'),
    mxRecords: z.boolean().nullable().describe('Whether MX records are found for the domain'),
