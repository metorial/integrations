...
export let generateImage = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Each image generation consumes 1 account credit.'
    ],
    tags: {
      destructive: false,
      readOnly: false
  ...
  .input(z.object({
    ...
    elements: z.record(z.string(), z.any()).optional().describe('Key-value pairs to override template element properties (e.g., text content, colors, image sources)')
  }))
  .output(z.object({
