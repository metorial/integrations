...
export let getProjectAnalytics = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    runs: z.array(z.record(z.string(), z.unknown())).describe('List of run log entries with run_id, date, status, latency, token counts, inputs, outputs, etc.'),
  }))
...
export let getOrganizationAnalytics = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    projects: z.array(z.record(z.string(), z.unknown())).describe('List of project run summaries with total runs, errors, tokens, users per project'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({
      token: ctx.auth.token,
...
export let getStorageAnalytics = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    knowledgeBases: z.array(z.record(z.string(), z.unknown())).optional().describe('Per-knowledge-base storage usage details'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({
      token: ctx.auth.token,
