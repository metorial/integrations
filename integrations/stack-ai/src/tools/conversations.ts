...
export let listConversations = SlateTool.create(
  ...
  .output(z.object({
    conversations: z.array(z.record(z.string(), z.unknown())).describe('List of conversation objects with id, title, created_at, etc.'),
...
export let manageConversation = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['rename', 'archive', 'delete']).describe('The action to perform on the conversation'),
    title: z.string().optional().describe('New title for the conversation (required for rename action)'),
    archive: z.boolean().optional().describe('Set to true to archive, false to unarchive (for archive action, default true)'),
  }))
  .output(z.object({
    conversation: z.record(z.string(), z.unknown()).optional().describe('The updated conversation object (not returned for delete)'),
...
export let listUserConversations = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    conversations: z.array(z.record(z.string(), z.unknown())).describe('List of all user conversations with id, title, user_id, created_at, etc.'),
    hasMore: z.boolean().describe('Whether more conversations are available'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({
