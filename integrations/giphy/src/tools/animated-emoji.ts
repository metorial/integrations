...
export let animatedEmoji = SlateTool.create(
  spec,
  {
    name: 'Animated Emoji',
    key: 'animated_emoji',
  ...
  .input(z.object({
    emojiGifId: z.string().optional().describe('GIF ID of a specific emoji to fetch variations for. Omit to browse the emoji catalog.'),
    limit: z.number().min(1).max(50).optional().describe('Number of emojis to return when browsing (1-50)'),
    offset: z.number().min(0).optional().describe('Results offset for pagination when browsing'),
  }))
  .output(z.object({
    emojis: z.array(gifSchema).describe('Array of animated emoji or emoji variations'),
    pagination: paginationSchema.optional().describe('Pagination information (only for catalog browsing)'),
  }))
