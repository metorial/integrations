...
export let randomGif = SlateTool.create(
  spec,
  {
    name: 'Random GIF',
    key: 'random_gif',
  ...
  .input(z.object({
    tag: z.string().optional().describe('Optional tag to filter random content by a word or phrase'),
    contentType: z.enum(['gifs', 'stickers']).default('gifs').describe('Type of random content to return'),
