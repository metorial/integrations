...
Optionally supports ordered signing, custom titles/messages, and test mode for validation.`,
  ...
  .input(z.object({
    templateId: z.string().describe('ID of the Signaturely template to use for the signature request'),
    signers: z.array(z.object({
      name: z.string().describe('Full name of the signer'),
      email: z.string().describe('Email address of the signer'),
      role: z.string().describe('Role assigned to this signer, must match a role defined in the template'),
      order: z.number().optional().describe('Signing order for this signer (used when isOrdered is true)'),
      fields: z.record(z.string(), z.string()).optional().describe('Key-value pairs mapping template field tags to pre-filled values'),
    ...
    message: z.string().optional().describe('Custom message included in the signing invitation email'),
    isOrdered: z.boolean().optional().describe('Whether signers must sign in a specific order'),
    testMode: z.boolean().optional().describe('When true, creates a test signature request that does not count against your quota'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    ctx.info({ message: 'Creating signature request from template', templateId: ctx.input.templateId });
