...
export let manageEnvironment = SlateTool.create(
  ...
  .input(z.object({
    ...
    yamlContent: z.string().optional().describe('YAML content for the environment (required for update)'),
  }))
  .output(z.object({
    action: z.string(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'create':
        ...
        break;
      case 'read':
        yamlContent = await client.getEnvironment(org, ctx.input.projectName, ctx.input.environmentName);
        break;
      case 'update':
        if (!ctx.input.yamlContent) throw new Error('yamlContent is required when updating an environment');
        await client.updateEnvironment(org, ctx.input.projectName, ctx.input.environmentName, ctx.input.yamlContent);
        break;
      case 'delete':
