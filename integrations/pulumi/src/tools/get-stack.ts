...
export let getStack = SlateTool.create(
  spec,
  {
    name: 'Get Stack',
    key: 'get_stack',
  ...
  .input(z.object({
    ...
    includeOutputs: z.boolean().optional().describe('If true, also fetch stack outputs from the exported state'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includeOutputs) {
      try {
        let exportData = await client.getStackExport(org, ctx.input.projectName, ctx.input.stackName);
