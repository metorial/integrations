...
export let runActor = SlateTool.create(
  spec,
  {
    name: 'Run Actor',
    key: 'run_actor',
...
Use this to execute web scrapers, automation tools, or any Actor from the Apify Store or your own custom Actors.`,
  ...
  .input(z.object({
    ...
    synchronous: z.boolean().optional().default(false).describe('If true, waits for the run to complete and returns dataset items directly (max 5 min)'),
    timeout: z.number().optional().describe('Timeout in seconds for the run'),
    memory: z.number().optional().describe('Memory limit in MB (must be a power of 2, min 128)'),
    build: z.string().optional().describe('Build tag or number to run (e.g. "latest", "beta")'),
  }))
  .output(z.object({
    runId: z.string().optional().describe('ID of the Actor run (async mode)'),
    actorId: z.string().optional().describe('ID of the Actor'),
    status: z.string().optional().describe('Run status (e.g. READY, RUNNING, SUCCEEDED, FAILED)'),
    ...
    defaultDatasetId: z.string().optional().describe('ID of the default dataset for this run'),
    defaultKeyValueStoreId: z.string().optional().describe('ID of the default key-value store for this run'),
    defaultRequestQueueId: z.string().optional().describe('ID of the default request queue for this run'),
    datasetItems: z.array(z.record(z.string(), z.any())).optional().describe('Dataset items (only in synchronous mode)'),
  }))
