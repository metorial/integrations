...
export let manageSchedule = SlateTool.create(
  ...
  .input(z.object({
    ...
    scheduleId: z.string().optional().describe('Schedule ID (required for get/update/delete)'),
    name: z.string().optional().describe('Schedule name (required for create)'),
    cronExpression: z.string().optional().describe('Cron expression (e.g. "0 0 * * *" for daily at midnight)'),
    timezone: z.string().optional().describe('Timezone (e.g. "America/New_York")'),
    isEnabled: z.boolean().optional().describe('Whether the schedule is active'),
    isExclusive: z.boolean().optional().describe('If true, skip scheduled runs if previous is still running'),
    ...
    })).optional().describe('Scheduled actions (Actors and Tasks to run)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('Schedule list (for list action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'update') {
      ...
      if (ctx.input.cronExpression !== undefined) body.cronExpression = ctx.input.cronExpression;
      if (ctx.input.timezone !== undefined) body.timezone = ctx.input.timezone;
      if (ctx.input.isEnabled !== undefined) body.isEnabled = ctx.input.isEnabled;
      if (ctx.input.isExclusive !== undefined) body.isExclusive = ctx.input.isExclusive;
      if (ctx.input.description !== undefined) body.description = ctx.input.description;
