...
export let manageActor = SlateTool.create(
  spec,
  {
    name: 'Manage Actor',
    key: 'manage_actor',
  ...
  .input(z.object({
    ...
    actorId: z.string().optional().describe('Actor ID or full name (required for get/update/delete)'),
    ...
    }).optional().describe('Default run configuration'),
  }))
  .output(z.object({
    ...
    defaultRunOptions: z.record(z.string(), z.any()).optional().describe('Default run options'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      if (ctx.input.defaultRunOptions !== undefined) {
        ...
        if (ctx.input.defaultRunOptions.build !== undefined) body.defaultRunOptions.build = ctx.input.defaultRunOptions.build;
        if (ctx.input.defaultRunOptions.timeout !== undefined) body.defaultRunOptions.timeoutSecs = ctx.input.defaultRunOptions.timeout;
        if (ctx.input.defaultRunOptions.memory !== undefined) body.defaultRunOptions.memoryMbytes = ctx.input.defaultRunOptions.memory;
      }
