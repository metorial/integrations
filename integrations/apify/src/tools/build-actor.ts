...
export let buildActor = SlateTool.create(
  spec,
  {
    name: 'Build Actor',
    key: 'build_actor',
  ...
  .input(z.object({
    ...
    useCache: z.boolean().optional().describe('Whether to use Docker layer cache (for build action)'),
  ...
  .output(z.object({
    ...
    status: z.string().optional().describe('Build status (READY, RUNNING, SUCCEEDED, FAILED, ABORTED, TIMED-OUT)'),
    ...
    builds: z.array(z.object({
      ...
      buildNumber: z.string().optional().describe('Build number'),
    })).optional().describe('Build list (for list action)'),
