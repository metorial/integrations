...
export let getCustomFieldValues = SlateTool.create(
  spec,
  {
...
export let updateCustomFieldValues = SlateTool.create(
  spec,
  {
...
Provide the record ID and a map of custom field names to their new values.`,
  ...
  .input(z.object({
    recordId: z.number().describe('ID of the gage or calibration record'),
    fields: z.record(z.string(), z.any()).describe('Map of custom field names to their new values'),
  }))
  .output(z.object({
    success: z.boolean().describe('Whether the operation was successful'),
