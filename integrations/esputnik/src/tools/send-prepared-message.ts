...
let recipientSchema = z.object({
  ...
  locator: z.string().optional().describe('Recipient identifier (email for email, phone for SMS/Viber)'),
  jsonParam: z.string().optional().describe('JSON string with dynamic parameters for Velocity template substitution'),
  languageCode: z.string().optional().describe('Language code for multilanguage messages'),
  email: z.boolean().optional().describe('Set true for email channel, false for other channels'),
  fromName: z.string().optional().describe('Sender name override'),
  externalRequestId: z.string().optional().describe('Custom ID to track message delivery status')
});

export let sendPreparedMessage = SlateTool.create(
  spec,
  {
...
Dynamic content can be injected via Velocity template parameters per recipient using the \`jsonParam\` field.`,
  ...
  .input(z.object({
    ...
    recipients: z.array(recipientSchema).min(1).max(1000).describe('Recipients to send the message to')
  }))
  .output(z.object({
