...
let channelSchema = z.object({
  type: z.enum(['email', 'sms', 'viber', 'mobilepush', 'webpush', 'appinbox', 'widget', 'inapp']).describe('Channel type'),
...
export let bulkUpsertContacts = SlateTool.create(
  spec,
  {
...
Use this for importing batches of contacts efficiently.`,
    constraints: [
      'Maximum 3,000 contacts per request'
    ],
    tags: {
      destructive: false,
      readOnly: false
  ...
  .input(z.object({
    ...
    dedupeOn: z.enum(['email', 'sms', 'push', 'email_or_sms', 'id', 'externalCustomerId', 'fieldId']).describe('Deduplication strategy'),
    fieldId: z.number().optional().describe('Custom field ID to use for deduplication (when dedupeOn is "fieldId")'),
    groupNames: z.array(z.string()).optional().describe('Segment names to add contacts to'),
    groupNamesExclude: z.array(z.string()).optional().describe('Segment names to remove contacts from'),
    restoreDeleted: z.boolean().optional().describe('Restore previously deleted contacts if matched'),
    eventKeyForNewContacts: z.string().optional().describe('Event key to generate for newly created contacts')
  }))
  .output(z.object({
    asyncSessionId: z.string().optional().describe('Async import session ID for tracking progress'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.restoreDeleted !== undefined) payload.restoreDeleted = ctx.input.restoreDeleted;
    if (ctx.input.eventKeyForNewContacts) payload.eventKeyForNewContacts = ctx.input.eventKeyForNewContacts;
