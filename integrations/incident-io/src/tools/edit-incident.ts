...
export let editIncident = SlateTool.create(
  spec,
  {
    name: 'Edit Incident',
    key: 'edit_incident',
  ...
  .input(z.object({
    ...
    notifyIncidentChannel: z.boolean().default(true).describe('Whether to send a notification to the incident Slack channel'),
    name: z.string().optional().describe('Updated incident name'),
    summary: z.string().optional().describe('Updated incident summary'),
    callUrl: z.string().optional().describe('URL of the active call/meeting for the incident'),
    ...
    })).optional().describe('Updated role assignments'),
    ...
    })).optional().describe('Updated timestamp values'),
  }))
  .output(z.object({
    incidentId: z.string(),
    reference: z.string(),
