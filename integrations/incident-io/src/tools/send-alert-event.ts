...
export let sendAlertEvent = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Rate limited to 120 events per minute per alert source.',
    ],
    tags: {
      destructive: false,
    },
  ...
  .input(z.object({
    alertSourceConfigId: z.string().describe('ID of the HTTP alert source configuration to send the event to'),
    title: z.string().describe('Title of the alert'),
    status: z.enum(['firing', 'resolved']).describe('Whether the alert is firing or resolved'),
    description: z.string().optional().describe('Detailed description of the alert'),
    deduplicationKey: z.string().optional().describe('Unique key to deduplicate alerts; alerts with the same key are treated as the same alert'),
    metadata: z.record(z.string(), z.string()).optional().describe('Key-value pairs of custom metadata to attach to the alert'),
    sourceUrl: z.string().optional().describe('URL linking back to the alert source for context'),
  }))
  .output(z.object({
