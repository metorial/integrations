...
export let crawlerEvent = SlateTrigger.create(
  spec,
  {
    name: 'Crawler Event',
    key: 'crawler_event',
    description: 'Triggered by crawler webhook events including page visits and crawl completion. Receives real-time callbacks as the crawler progresses.',
  }
)
  .input(z.object({
    eventName: z.string().describe('Crawler event name (crawler_url_visited or crawler_finished).'),
    ...
    depth: z.number().optional().describe('Link depth from starting URL (for url_visited events).'),
  ...
  .output(z.object({
    ...
    state: z.any().optional().describe('Crawl state information (for crawler_finished events).'),
  }))
  .webhook({
