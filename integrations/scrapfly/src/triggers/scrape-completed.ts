...
export let scrapeCompleted = SlateTrigger.create(
  spec,
  {
    name: 'Scrape Completed',
    key: 'scrape_completed',
    description: 'Triggered when an asynchronous scrape request completes. Receives the full scrape result payload via webhook.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    extractedData: z.any().optional().describe('Extracted structured data if extraction was configured.'),
