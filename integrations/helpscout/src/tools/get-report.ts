...
export let getReport = SlateTool.create(
  spec,
  {
    ...
    tags: { readOnly: true },
    constraints: [
      'Date format must be ISO 8601 (e.g., "2024-01-01T00:00:00Z").',
    ],
  }
)
  .input(z.object({
    reportType: z.enum(['company', 'conversations', 'happiness']).describe('Type of report to retrieve'),
    ...
    mailboxes: z.string().optional().describe('Comma-separated list of mailbox IDs to filter by'),
    tags: z.string().optional().describe('Comma-separated list of tag IDs to filter by'),
    types: z.string().optional().describe('Comma-separated list of conversation types to filter by'),
    folders: z.string().optional().describe('Comma-separated list of folder IDs to filter by'),
  }))
  .output(z.object({
    report: z.record(z.string(), z.any()).describe('Report data (structure varies by report type)'),
  }))
