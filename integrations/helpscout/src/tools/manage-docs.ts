...
export let manageDocs = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Only available when a Docs API key has been configured.',
    ],
  }
)
  .input(z.object({
    ...
    visibility: z.enum(['public', 'private']).optional().describe('Visibility for collections'),
  ...
  .handleInvocation(async (ctx) => {
    if (!ctx.auth.docsApiKey) {
      throw new Error('Docs API key is not configured. Please set it during authentication setup.');
    }
    ...
    switch (ctx.input.action) {
      ...
      case 'create_collection': {
        if (!ctx.input.siteId || !ctx.input.name) throw new Error('Site ID and name are required');
        ...
        return {
          output: {
            collection: { collectionId: c?.id ?? '', name: ctx.input.name, visibility: ctx.input.visibility },
            success: true,
          },
      ...
      case 'update_collection': {
        ...
        return { output: { success: true }, message: `Updated collection **#${ctx.input.collectionId}**.` };
      }

      case 'delete_collection': {
        if (!ctx.input.collectionId) throw new Error('Collection ID is required');
        await client.deleteCollection(ctx.input.collectionId);
        return { output: { success: true }, message: `Deleted collection **#${ctx.input.collectionId}**.` };
      }

      case 'list_categories': {
        if (!ctx.input.collectionId) throw new Error('Collection ID is required');
        let data = await client.listCategories(ctx.input.collectionId, { page: ctx.input.page });
      ...
      case 'create_category': {
        if (!ctx.input.collectionId || !ctx.input.name) throw new Error('Collection ID and name are required');
        await client.createCategory(ctx.input.collectionId, {
          name: ctx.input.name,
          description: ctx.input.description,
          order: ctx.input.order,
        });
        return { output: { success: true }, message: `Created category **"${ctx.input.name}"**.` };
      }
      ...
      case 'update_category': {
        ...
        return { output: { success: true }, message: `Updated category **#${ctx.input.categoryId}**.` };
      }

      case 'delete_category': {
        if (!ctx.input.categoryId) throw new Error('Category ID is required');
        await client.deleteCategory(ctx.input.categoryId);
        return { output: { success: true }, message: `Deleted category **#${ctx.input.categoryId}**.` };
      }
      ...
      case 'create_article': {
        ...
        return {
          output: {
            article: { articleId: a?.id ?? '', name: ctx.input.name, status: ctx.input.status },
            success: true,
          },
      ...
      case 'update_article': {
        ...
        return { output: { success: true }, message: `Updated article **#${ctx.input.articleId}**.` };
      }

      case 'delete_article': {
        if (!ctx.input.articleId) throw new Error('Article ID is required');
        await client.deleteArticle(ctx.input.articleId);
        return { output: { success: true }, message: `Deleted article **#${ctx.input.articleId}**.` };
      }
