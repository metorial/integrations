...
export let manageRoutingKeys = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    routingKey: z.string().optional().describe('Name of the routing key (required for create)'),
    targets: z.array(z.object({
      policySlug: z.string().describe('Escalation policy slug to route to'),
    })).optional().describe('Policy targets for the routing key (required for create)'),
  }))
  .output(z.object({
    routingKeys: z.array(z.any()).optional().describe('List of routing keys with their team associations'),
