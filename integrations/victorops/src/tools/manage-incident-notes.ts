...
export let manageIncidentNotes = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'update', 'delete']).describe('Action to perform on notes'),
    incidentNumber: z.string().describe('Incident number to manage notes for'),
    noteName: z.string().optional().describe('Name/ID of the note (required for update and delete)'),
    content: z.string().optional().describe('Content of the note (required for create and update)'),
  }))
  .output(z.object({
