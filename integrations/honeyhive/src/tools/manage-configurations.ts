...
export let listConfigurations = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    environment: z.enum(['dev', 'staging', 'prod']).optional().describe('Filter by environment'),
  ...
  .output(z.object({
    configurations: z.array(z.object({
      ...
      parameters: z.record(z.string(), z.any()).optional().describe('Model parameters and prompt template'),
...
export let createConfiguration = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    parameters: z.record(z.string(), z.any()).describe('Model parameters including call_type, model, hyperparameters, and template'),
    environments: z.array(z.string()).optional().describe('Environments to deploy to (e.g., ["dev", "staging", "prod"])'),
    type: z.enum(['LLM', 'pipeline']).optional().default('LLM').describe('Configuration type'),
    userProperties: z.record(z.string(), z.any()).optional().describe('User property targeting rules'),
  }))
  .output(z.object({
...
export let updateConfiguration = SlateTool.create(
  spec,
  {
...
export let deleteConfiguration = SlateTool.create(
  spec,
  {
