...
export let listDatasets = SlateTool.create(
  ...
  .input(z.object({
    ...
    type: z.enum(['evaluation', 'fine-tuning']).optional().describe('Filter by dataset type'),
...
export let createDataset = SlateTool.create(
  ...
  .input(z.object({
    ...
    type: z.enum(['evaluation', 'fine-tuning']).optional().default('evaluation').describe('Type of dataset'),
    pipelineType: z.enum(['event', 'session']).optional().default('event').describe('Pipeline type'),
...
export let addDatapointsToDataset = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    mapping: z.object({
      ...
      groundTruth: z.array(z.string()).describe('Keys from records to use as ground truth'),
      history: z.array(z.string()).default([]).describe('Keys from records to use as chat history'),
