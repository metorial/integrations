...
export let verificationCompleted = SlateTrigger.create(
  spec,
  {
    name: 'Verification Completed',
    key: 'verification_completed',
    description: 'Triggered when an identity verification, AML screening, or other check is completed. Delivers the verification outcome (positive or negative) and associated details in real time via webhook.',
  },
)
  .input(z.object({
    eventType: z.string().describe('Type of event (e.g., verification.completed, aml_screening.completed)'),
  ...
  .output(z.object({
    checkId: z.string().describe('ID of the completed check'),
    checkType: z.string().describe('Type of check (verification, aml_screening, proof_of_address, background_check, credit_check, kyb_report)'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let deriveCheckType = (eventType: string): string => {
        if (eventType.includes('aml') || eventType.includes('screening')) return 'aml_screening';
        if (eventType.includes('proof') || eventType.includes('address')) return 'proof_of_address';
      ...
      let inputs = events.map((event: any) => {
        let eventType = event.event_type || event.type || event.event || 'verification.completed';
        let checkData = event.data || event.payload || event;

        return {
          eventType,
          eventId: event.event_id || event.id || `${checkData.id || checkData.check_id || 'unknown'}_${Date.now()}`,
          checkId: checkData.id || checkData.check_id || checkData.verification_id || checkData.screening_id || checkData.report_id || '',
