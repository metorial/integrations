...
export let runAmlScreening = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    dateOfBirth: z.string().optional().describe('Date of birth in ISO 8601 format (YYYY-MM-DD) for improved matching'),
    screeningTypes: z.array(z.enum(['pep', 'sanctions', 'adverse_media'])).optional().describe('Types of screening to perform. Defaults to all three if not specified'),
    referenceId: z.string().optional().describe('Your own reference ID to correlate this screening with your records'),
  }))
  .output(z.object({
    screeningId: z.string().describe('Unique ID of the AML screening'),
    status: z.string().describe('Current status of the screening (e.g., pending, completed)'),
    screeningTypes: z.array(z.string()).optional().describe('Types of screening performed'),
    matchesFound: z.boolean().optional().describe('Whether any potential matches were found'),
    ...
    adverseMediaMatch: z.boolean().optional().describe('Whether an adverse media match was found'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {
        ...
        screeningTypes: result.screening_types || ctx.input.screeningTypes || ['pep', 'sanctions', 'adverse_media'],
        matchesFound,
