...
export let manageConfigVars = SlateTool.create(
  spec,
  {
...
Requires **read-protected** or **write-protected** OAuth scopes.`,
    ...
    constraints: [
      'Requires read-protected or write-protected OAuth scope.'
    ]
  }
)
  .input(z.object({
    appIdOrName: z.string().describe('App name or unique identifier'),
    updates: z.record(z.string(), z.string().nullable()).optional().describe('Key-value pairs to set. Set a value to null to remove the variable.')
  }))
  .output(z.object({
    configVars: z.record(z.string(), z.string()).describe('All current config vars for the app after any updates')
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (updates && Object.keys(updates).length > 0) {
      ...
      let setKeys = Object.entries(updates).filter(([_, v]) => v !== null).map(([k]) => k);
      let removedKeys = Object.entries(updates).filter(([_, v]) => v === null).map(([k]) => k);
      let parts: string[] = [];
