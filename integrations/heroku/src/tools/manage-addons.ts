...
export let manageAddons = SlateTool.create(
  spec,
  {
    name: 'Manage Add-ons',
    key: 'manage_addons',
...
- \`delete\`: Remove an add-on from an app.`,
  ...
  .input(z.object({
    ...
    appIdOrName: z.string().optional().describe('App name or ID (required for create and delete)'),
    addonIdOrName: z.string().optional().describe('Add-on name or ID (required for get, update, delete)'),
    plan: z.string().optional().describe('Add-on plan identifier (required for create, optional for update)'),
    addonName: z.string().optional().describe('Custom name for the add-on'),
    addonConfig: z.record(z.string(), z.string()).optional().describe('Configuration for the add-on (for create)')
  }))
  .output(z.object({
    ...
    })).optional().describe('List of add-ons'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'get') {
      if (!ctx.input.addonIdOrName) throw new Error('addonIdOrName is required for "get" action.');
    ...
    if (action === 'create') {
      if (!ctx.input.appIdOrName) throw new Error('appIdOrName is required for "create" action.');
    ...
    if (action === 'update') {
      if (!ctx.input.addonIdOrName) throw new Error('addonIdOrName is required for "update" action.');
    ...
    // delete
    if (!ctx.input.appIdOrName) throw new Error('appIdOrName is required for "delete" action.');
    if (!ctx.input.addonIdOrName) throw new Error('addonIdOrName is required for "delete" action.');
