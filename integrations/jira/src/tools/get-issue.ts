...
export let getIssueTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    fields: z.array(z.string()).optional().describe('Specific field keys to return (e.g., ["summary", "status", "assignee"]). Returns all fields if omitted.'),
    expand: z.array(z.string()).optional().describe('Additional data to expand (e.g., ["changelog", "transitions", "renderedFields"]).'),
  }))
  .output(z.object({
    ...
    }).optional().nullable().describe('The assigned user.'),
    reporter: z.object({
      accountId: z.string(),
      displayName: z.string(),
    }).optional().nullable().describe('The reporter.'),
