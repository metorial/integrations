...
export let updateZoneSettingsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list', 'update']).describe('Whether to list all settings or update one'),
    zoneId: z.string().describe('Zone ID'),
    settingId: z.string().optional().describe('Setting identifier to update (e.g. ssl, security_level, development_mode)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let response = await client.updateZoneSetting(ctx.input.zoneId, ctx.input.settingId, ctx.input.value);
    return {
      output: {
        updatedSetting: {
