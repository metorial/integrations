...
export let manageSslCertificatesTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum([
      'list_certificate_packs', 'list_custom_certificates', 'list_origin_ca',
      'get_ssl_mode', 'update_ssl_mode',
      'create_origin_ca', 'revoke_origin_ca'
    ]).describe('Operation to perform'),
    zoneId: z.string().describe('Zone ID'),
    sslMode: z.string().optional().describe('SSL/TLS encryption mode (off, flexible, full, strict)'),
    hostnames: z.array(z.string()).optional().describe('Hostnames for Origin CA certificate'),
    requestedValidity: z.number().optional().describe('Certificate validity in days (default 5475 = 15 years)'),
