...
let eventTypeSchema = z.object({
  eventName: z.string().describe('Event type name (e.g., "note.created", "segment.created", "recording.event.custom", "metric.alert")'),
  subcategory: z.string().optional().describe('Subcategory, required for "recording.event.custom" events')
});

export let manageWebhookEndpoint = SlateTool.create(
  spec,
  {
...
**To delete**: provide endpointId and set deleteEndpoint to true.`,
  ...
  .input(z.object({
    endpointId: z.string().optional().describe('ID of an existing endpoint to update or delete'),
    deleteEndpoint: z.boolean().optional().describe('Set to true to delete the endpoint (requires endpointId)'),
    ...
    secret: z.string().optional().describe('Shared secret for HMAC-SHA256 payload signature verification'),
