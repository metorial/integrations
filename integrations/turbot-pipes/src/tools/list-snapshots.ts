...
let snapshotSchema = z.object({
  ...
  visibility: z.string().optional().describe('Snapshot visibility (workspace or anyone_with_link)'),
...
export let getSnapshot = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    let snapshot = await client.getSnapshot(ownerHandle, ctx.input.workspaceHandle, ctx.input.snapshotId);
