...
let workspaceSchema = z.object({
  ...
  instanceType: z.string().optional().describe('Workspace instance type (e.g. db1.shared, db1.small)'),
...
Each workspace includes a dedicated Steampipe database instance, Powerpipe server, and Flowpipe server.`,
  ...
  .input(z.object({
    scope: z.enum(['user', 'org', 'all']).default('all').describe('Scope to list workspaces from: user-owned, organization, or all accessible'),
    userHandle: z.string().optional().describe('User handle (required when scope is "user")'),
    orgHandle: z.string().optional().describe('Organization handle (required when scope is "org")'),
