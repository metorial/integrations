...
export let getRecommender = SlateTool.create(spec, {
  ...
  .output(recommenderSchema.extend({
    streamName: z.string().optional().describe('Name of the stream used for recommendations'),
    eligibility: z.record(z.string(), z.any()).optional().describe('Eligibility rules configuration'),
    biasMitigation: z.record(z.string(), z.any()).optional().describe('Bias mitigation configuration'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new FaradayClient({ token: ctx.auth.token });
