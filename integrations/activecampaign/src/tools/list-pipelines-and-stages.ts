...
export let listPipelinesAndStages = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    pipelineId: z.string().optional().describe('If provided, only list stages for this pipeline')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let pipelines = await Promise.all(pipelinesRaw.map(async (p: any) => {
