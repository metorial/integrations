...
export let createPost = SlateTool.create(
  ...
  .input(z.object({
    ...
    mediaIds: z.array(z.string()).optional().describe('Array of media IDs to attach (uploaded separately)'),
    pollOptions: z.array(z.string()).optional().describe('Poll options (2-4 choices)'),
    pollDurationMinutes: z.number().optional().describe('Poll duration in minutes (default 1440)')
  }))
  .output(z.object({
