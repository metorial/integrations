...
export let manageBlockMute = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['block', 'unblock', 'mute', 'unmute', 'list_blocked', 'list_muted']).describe('Action to perform'),
    userId: z.string().describe('Authenticated user ID'),
    targetUserId: z.string().optional().describe('Target user ID (required for block/unblock/mute/unmute)'),
