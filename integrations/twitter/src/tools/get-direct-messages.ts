...
export let getDirectMessages = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    conversationId: z.string().optional().describe('Conversation ID to get messages from (omit to get all recent DMs)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result;
    if (conversationId) {
      result = await client.getDmConversationEvents(conversationId, { maxResults, paginationToken });
    } else {
