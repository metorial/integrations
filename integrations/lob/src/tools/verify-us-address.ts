...
export let verifyUSAddress = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    primaryLine: z.string().optional().describe('Primary address line (e.g., "185 Berry St")'),
    ...
    address: z.string().optional().describe('Freeform single-line address (alternative to structured fields)'),
  }))
  .output(z.object({
    verificationId: z.string().optional().nullable().describe('Verification ID'),
    deliverability: z.string().describe('Deliverability status: deliverable, deliverable_unnecessary_unit, deliverable_incorrect_unit, deliverable_missing_unit, undeliverable'),
    ...
    lastLine: z.string().optional().nullable().describe('City, state, and ZIP in standard format'),
    components: z.any().optional().nullable().describe('Parsed address components (city, state, ZIP, county, etc.)'),
    deliverabilityAnalysis: z.any().optional().nullable().describe('Detailed DPV analysis (confirmation, CMRA, vacant, active, footnotes)'),
    lobConfidenceScore: z.number().optional().nullable().describe('Lob confidence score (0-100)'),
...
export let autocompleteUSAddress = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    addressPrefix: z.string().describe('Partial address string to autocomplete (at least 3 characters)'),
