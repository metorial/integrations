...
}).optional().describe('Proxy configuration for IP-matching requirements. When provided, the task type automatically switches to the proxy variant.');

export let solveCaptcha = SlateTool.create(
  spec,
  {
    name: 'Solve Captcha',
    key: 'solve_captcha',
...
Proxy configuration is optional; when provided, the proxy variant of the task type is used automatically.`,
  ...
  .input(z.object({
    ...
    websiteKey: z.string().optional().describe('Site key / public key for the captcha (required for all types except GeeTest)'),

    // reCAPTCHA V2 specific
    isInvisible: z.boolean().optional().describe('Set to true for invisible reCAPTCHA V2 or invisible hCaptcha'),
    recaptchaDataSValue: z.string().optional().describe('Value of the data-s parameter for Google search reCAPTCHA V2'),
    apiDomain: z.string().optional().describe('Domain used to load reCAPTCHA script (e.g. recaptcha.net)'),

    // reCAPTCHA V3 specific
    pageAction: z.string().optional().describe('Action parameter for reCAPTCHA V3 (value of the "action" parameter in grecaptcha.execute)'),
    ...
    // FunCaptcha specific
    funcaptchaApiJSSubdomain: z.string().optional().describe('Custom subdomain used to load the FunCaptcha JS widget'),
    funcaptchaData: z.string().optional().describe('Additional data payload for FunCaptcha, serialized as JSON string'),

    // GeeTest specific
    gt: z.string().optional().describe('GeeTest gt parameter (public key, required for GeeTest)'),
    challenge: z.string().optional().describe('GeeTest challenge token (required for GeeTest v3)'),
    geetestApiServerSubdomain: z.string().optional().describe('GeeTest API server subdomain'),
    geetestVersion: z.number().optional().describe('GeeTest version: 3 or 4 (default: 3)'),
    initParameters: z.record(z.string(), z.unknown()).optional().describe('GeeTest v4 initialization parameters (must include captcha_id)'),

    // Turnstile specific
    turnstileAction: z.string().optional().describe('Turnstile action parameter (cData.action value)'),
    ...
    // hCaptcha specific
    enterprisePayload: z.record(z.string(), z.unknown()).optional().describe('hCaptcha Enterprise payload data'),

    // Common
    userAgent: z.string().optional().describe('Browser User-Agent to use when solving the captcha'),
    cookies: z.string().optional().describe('Cookies to use when solving (format: "name1=value1; name2=value2")'),
    proxy: proxySchema,
