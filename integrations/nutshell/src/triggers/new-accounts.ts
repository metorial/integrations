...
export let newAccounts = SlateTrigger.create(
  spec,
  {
    name: 'New Accounts',
    key: 'new_accounts',
    description: '[Polling fallback] Polls for newly created accounts (companies) in Nutshell CRM. Detects accounts added since the last check.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let highestId = results.length > 0 ? Math.max(...results.map((a: any) => a.id)) : lastSeenId;
