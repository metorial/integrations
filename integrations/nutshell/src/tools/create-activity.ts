...
export let createActivity = SlateTool.create(
  ...
  .input(z.object({
    ...
    activityTypeId: z.number().optional().describe('Activity type ID (use Find Activity Types to discover available types)'),
    leadId: z.number().optional().describe('ID of the lead to associate with this activity'),
    contactIds: z.array(z.number()).optional().describe('IDs of contacts participating in this activity'),
    startTime: z.string().optional().describe('Start time for the activity (ISO 8601 format)'),
    ...
    status: z.number().optional().describe('Activity status (0=scheduled, 1=logged, 2=canceled)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.contactIds) {
      activityData.participants = ctx.input.contactIds.map(id => ({ entityType: 'Contacts', id }));
    }
