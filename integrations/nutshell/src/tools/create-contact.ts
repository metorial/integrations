...
export let createContact = SlateTool.create(
  ...
  .input(z.object({
    ...
    }).optional().describe('Mailing address for the contact'),
    accountId: z.number().optional().describe('ID of the account (company) to associate with this contact'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.accountId) contactData.accounts = [{ entityType: 'Accounts', id: ctx.input.accountId }];
