...
export let createMigrationDocument = SlateTool.create(
  spec,
  {
...
Requires a Migration API token.`,
  ...
  .input(z.object({
    ...
    lang: z.string().optional().describe('Language code (defaults to repository master locale)'),
    tags: z.array(z.string()).optional().describe('Tags to apply to the document'),
    alternateLanguageDocumentId: z.string().optional().describe('ID of existing document to link as an alternate language'),
    data: z.record(z.string(), z.any()).describe('Document field data matching the custom type schema'),
  }))
...
export let updateMigrationDocument = SlateTool.create(
  spec,
  {
...
Requires a Migration API token.`,
  ...
  .input(z.object({
    ...
    alternateLanguageDocumentId: z.string().optional().describe('ID of existing document to link as alternate language'),
