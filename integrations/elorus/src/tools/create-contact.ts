...
export let createContact = SlateTool.create(
  ...
  .input(z.object({
    ...
    isClient: z.boolean().optional().describe('Whether this contact is a client. Defaults to true.'),
    ...
    customId: z.string().optional().describe('Custom external identifier (API-only, not visible in the Elorus UI).'),
  }))
  .output(z.object({
