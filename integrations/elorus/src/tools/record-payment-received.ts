...
export let recordPaymentReceived = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    transactionType: z.enum(['dp', 'icm']).describe('"dp" for client payment (against invoices), "icm" for general income.'),
    ...
    paymentMethod: z.string().optional().describe('Payment method: "1"=domestic bank, "2"=foreign bank, "3"=cash, "4"=cheque, "6"=web banking, "7"=POS.'),
    currencyCode: z.string().optional().describe('Currency code (e.g. "EUR").'),
    incomeType: z.enum(['contract', 'rental']).optional().describe('Income type, only for "icm" transaction type.'),
