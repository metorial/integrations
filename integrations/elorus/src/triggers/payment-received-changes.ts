...
export let paymentReceivedChanges = SlateTrigger.create(
  spec,
  {
    name: 'Payment Received Changes',
    key: 'payment_received_changes',
    description: 'Triggers when payments received (cash receipts) are created or modified in your Elorus organization. Covers client payments and income entries.',
  }
)
  .input(z.object({
    cashReceiptId: z.string().describe('The cash receipt ID.'),
    eventType: z.enum(['created', 'updated']).describe('Whether the payment was newly created or updated.'),
  ...
  .output(z.object({
    ...
    transactionType: z.string().optional().describe('Transaction type (dp=client payment, icm=income).'),
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let inputs = result.results.map((receipt: any) => ({
        ...
        eventType: (knownIds.includes(receipt.id) ? 'updated' : 'created') as 'created' | 'updated',
        cashReceipt: receipt,
      }));
