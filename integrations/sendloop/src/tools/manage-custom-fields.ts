...
export let manageCustomFields = SlateTool.create(
  ...
  .input(z.object({
    ...
    fieldName: z.string().optional().describe('Name of the custom field (required for create)'),
    fieldType: z.string().optional().describe('Type of the custom field (e.g., "text", "number", "date")')
  }))
  .output(z.object({
    customFields: z.array(z.record(z.string(), z.any())).optional().describe('Array of custom field objects (for list action)'),
    customFieldId: z.string().optional().describe('ID of the created or deleted custom field'),
