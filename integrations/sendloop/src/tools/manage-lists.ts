...
export let manageLists = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'delete']).describe('The operation to perform on the subscriber list'),
    listId: z.string().optional().describe('ID of the list (required for update and delete)'),
    name: z.string().optional().describe('Name of the subscriber list (required for create)'),
    optInMode: z.enum(['single', 'double']).optional().describe('Opt-in mode: single (immediate) or double (confirmation email)'),
    notifyOnSubscribe: z.boolean().optional().describe('Whether to send notification when someone subscribes'),
    notifyOnUnsubscribe: z.boolean().optional().describe('Whether to send notification when someone unsubscribes'),
    notifyEmail: z.string().optional().describe('Email address to receive subscription notifications')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { action, listId, name, optInMode, notifyOnSubscribe, notifyOnUnsubscribe, notifyEmail } = ctx.input;
    ...
    if (action === 'create') {
      ...
      let result = await client.createList({
        ...
        notifyOnSubscribe: notifyOnSubscribe !== undefined ? (notifyOnSubscribe ? '1' : '0') : undefined,
        notifyOnUnsubscribe: notifyOnUnsubscribe !== undefined ? (notifyOnUnsubscribe ? '1' : '0') : undefined,
        notifyEmail
      });
