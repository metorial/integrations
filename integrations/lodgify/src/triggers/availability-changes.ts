...
export let availabilityChanges = SlateTrigger.create(
  spec,
  {
    name: 'Availability Changes',
    key: 'availability_changes',
    description: 'Triggers when the availability calendar changes for a property, such as dates being blocked or opened.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    startDate: z.string().optional().describe('Start date of the changed availability period'),
  ...
  .webhook({
    autoRegisterWebhook: async (ctx) => {
      ...
      let result = await client.subscribeWebhook('availability_change', ctx.input.webhookBaseUrl);
