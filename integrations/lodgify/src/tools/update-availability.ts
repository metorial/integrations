...
export let updateAvailability = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    roomUpdates: z.array(z.object({
      ...
      periods: z.array(z.object({
        ...
        isAvailable: z.boolean().describe('Whether the room should be available during this period'),
        minStay: z.number().optional().describe('Minimum number of nights for this period'),
      })).min(1).describe('Date periods to update'),
    })).min(1).describe('Room type availability updates'),
  }))
  .output(z.object({
