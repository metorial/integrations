...
export let listRuns = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Cannot filter by both flow and contact simultaneously.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    before: z.string().optional().describe('Return only runs modified before this date (ISO 8601)'),
    after: z.string().optional().describe('Return only runs modified after this date (ISO 8601)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let runs = result.results.map(r => ({
      ...
      values: Object.fromEntries(
        Object.entries(r.values).map(([key, v]) => [key, {
          value: v.value,
          category: v.category,
          name: v.name,
          time: v.time,
