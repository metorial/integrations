...
export let manageCampaignEvents = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    eventUuid: z.string().optional().describe('UUID of the campaign event (required for update/delete)'),
    campaignUuid: z.string().optional().describe('UUID of the campaign (required for create)'),
    relativeTo: z.string().optional().describe('Key of the contact date field the event is relative to'),
    offset: z.number().optional().describe('Offset from the date field (positive or negative integer)'),
    unit: z.enum(['minutes', 'hours', 'days', 'weeks']).optional().describe('Unit of the offset'),
    deliveryHour: z.number().optional().describe('Hour of the day to deliver (0-24, or -1 to match contact field)'),
    message: z.record(z.string(), z.string()).optional().describe('Message translations by language code (mutually exclusive with flowUuid)'),
    flowUuid: z.string().optional().describe('UUID of the flow to start (mutually exclusive with message)'),
  }))
  .output(z.object({
