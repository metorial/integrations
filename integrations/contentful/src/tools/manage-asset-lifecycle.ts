...
export let manageAssetLifecycle = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['publish', 'unpublish', 'archive', 'unarchive', 'delete']).describe('Lifecycle action to perform.'),
    version: z.number().optional().describe('Current version of the asset. Fetched automatically if omitted.')
  }))
  .output(z.object({
    assetId: z.string().describe('ID of the asset.'),
