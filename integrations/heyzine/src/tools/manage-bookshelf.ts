...
export let manageBookshelf = SlateTool.create(
  spec,
  {
...
Bookshelves are collections of flipbooks displayed on a shared page.`,
  ...
  .input(z.object({
    action: z.enum(['list_bookshelves', 'list_flipbooks', 'add_flipbook', 'remove_flipbook']).describe('Action to perform on bookshelves.'),
    bookshelfId: z.string().optional().describe('Bookshelf identifier. Required for all actions except "list_bookshelves".'),
    flipbookId: z.string().optional().describe('Flipbook identifier. Required for "add_flipbook" and "remove_flipbook" actions.'),
    position: z.number().optional().describe('Position to insert the flipbook at in the bookshelf. Only used with "add_flipbook".'),
  }))
  .output(z.object({
    bookshelves: z.array(z.object({
    ...
    })).optional().describe('List of bookshelves (only for "list_bookshelves" action).'),
    ...
    })).optional().describe('List of flipbooks in the bookshelf (only for "list_flipbooks" action).'),
    success: z.boolean().optional().describe('Whether the add/remove operation was successful.'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'list_flipbooks') {
      if (!ctx.input.bookshelfId) throw new Error('bookshelfId is required for list_flipbooks action.');
    ...
    if (ctx.input.action === 'add_flipbook') {
      if (!ctx.input.bookshelfId) throw new Error('bookshelfId is required for add_flipbook action.');
      if (!ctx.input.flipbookId) throw new Error('flipbookId is required for add_flipbook action.');

      await client.addFlipbookToBookshelf(ctx.input.bookshelfId, ctx.input.flipbookId, ctx.input.position);
    ...
    if (ctx.input.action === 'remove_flipbook') {
      if (!ctx.input.bookshelfId) throw new Error('bookshelfId is required for remove_flipbook action.');
      if (!ctx.input.flipbookId) throw new Error('flipbookId is required for remove_flipbook action.');
