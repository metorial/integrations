...
export let manageAccessList = SlateTool.create(
  spec,
  {
...
Supports multiple access types including username/password pairs, Google accounts, password-only, one-time passwords, and email-based access.`,
  ...
  .input(z.object({
    ...
    action: z.enum(['add', 'remove']).describe('Whether to add or remove users from the access list.'),
    accessType: z.enum(['user_pass', 'google', 'pass_only', 'otp', 'email_link', 'send_code']).describe('Type of access credential: "user_pass" for username+password, "google" for Google Sign-In, "pass_only" for password only, "otp" for one-time password, "email_link" or "send_code" for email-based access.'),
    users: z.array(z.object({
      user: z.string().optional().describe('Username or email for login. Required for user_pass, google, email_link, send_code access types.'),
      password: z.string().optional().describe('Password for login. Required for user_pass, pass_only, otp access types.'),
