...
export let updateSocialMetadata = SlateTool.create(
  spec,
  {
...
This controls how the resource appears when shared on social media platforms via Open Graph tags.`,
  ...
  .input(z.object({
    resourceType: z.enum(['flipbook', 'bookshelf']).describe('Whether to update social metadata for a flipbook or bookshelf.'),
    ...
    thumbnailUrl: z.string().optional().describe('URL of the thumbnail image for social sharing.'),
  }))
  .output(z.object({
