...
export let bulkCreateRecords = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    dataType: z.string().describe('Name of the Bubble data type (table) to create records in.'),
    records: z.array(z.record(z.string(), z.any())).describe('Array of records to create. Each record is an object of field name to value pairs.'),
  }))
  .output(z.object({
    recordIds: z.array(z.string()).describe('Array of unique IDs for the newly created records.'),
