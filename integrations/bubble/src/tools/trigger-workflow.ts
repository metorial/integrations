...
If the workflow is configured to return data, the response will contain the returned values.`,
  ...
  .input(z.object({
    workflowName: z.string().describe('Name of the API workflow to trigger, as defined in the Bubble editor.'),
    parameters: z.record(z.string(), z.any()).optional().describe('Key-value pairs of parameters to pass to the workflow. Parameter names must match the workflow definition.'),
  }))
  .output(z.object({
    response: z.any().describe('Response data returned by the workflow, if the workflow is configured to return data. May be null if the workflow does not return data.'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let response = await client.triggerWorkflow(ctx.input.workflowName, ctx.input.parameters ?? {});
