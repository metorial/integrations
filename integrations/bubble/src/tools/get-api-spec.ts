...
export let getApiSpec = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'The Swagger spec must be enabled in Settings → API in the Bubble editor.',
    ],
    tags: {
      destructive: false,
      readOnly: true,
  ...
  .input(z.object({}))
  .output(z.object({
    specification: z.any().describe('The OpenAPI/Swagger specification object describing all exposed data types and workflows.'),
  }))
