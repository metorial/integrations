...
export let listContentItems = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    continuationToken: z.string().optional().describe('Continuation token for paginating through results. Omit for the first page.'),
  }))
  .output(z.object({
    ...
    continuationToken: z.string().optional().describe('Token for fetching the next page of results'),
  }))
