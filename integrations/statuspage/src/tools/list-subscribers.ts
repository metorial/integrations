...
export let listSubscribers = SlateTool.create(
  ...
  .input(z.object({
    type: z.enum(['email', 'sms', 'webhook', 'slack', 'teams', 'integration_partner']).optional().describe('Filter by subscriber type'),
    state: z.enum(['active', 'unconfirmed', 'quarantined']).optional().describe('Filter by subscriber state'),
