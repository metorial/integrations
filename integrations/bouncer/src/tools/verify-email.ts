...
let emailRecordSchema = z.object({
  ...
  reason: z.string().describe('Reason for the status, e.g. accepted_email, invalid_email, rejected_email, dns_error, timeout'),
  domain: z.object({
    name: z.string().describe('Domain name'),
    acceptAll: z.string().describe('Whether domain accepts all emails (yes/no)'),
    disposable: z.string().describe('Whether domain is a disposable email provider (yes/no)'),
    free: z.string().describe('Whether domain is a free email provider (yes/no)'),
  }).describe('Domain information'),
  account: z.object({
    role: z.string().describe('Whether the address is role-based like info@ or support@ (yes/no)'),
  ...
  retryAfter: z.string().optional().describe('ISO timestamp for greylisted addresses requiring retry'),
  didYouMean: z.string().optional().describe('Suggested correction for potentially misspelled emails'),
});
...
export let verifyEmail = SlateTool.create(
  ...
  .input(z.object({
    ...
    timeout: z.number().optional().describe('Verification timeout in seconds. Defaults to 10, maximum 30.'),
  }))
