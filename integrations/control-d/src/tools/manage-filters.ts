...
export let manageFilters = SlateTool.create(
  spec,
  {
    name: 'Manage Filters',
    key: 'manage_filters',
  ...
  .input(z.object({
    ...
    includeExternal: z.boolean().optional().describe('Include third-party filters when listing (default: false)'),
    filters: z.array(z.object({
      filterId: z.string().describe('Filter identifier (e.g., "ads", "malware", "porn_strict")'),
      enabled: z.boolean().describe('Whether to enable or disable the filter'),
    })).optional().describe('Filters to update (required for update operation)'),
  }))
  .output(z.object({
    filters: z.array(z.object({
