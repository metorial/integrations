...
export let manageCustomRules = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    folderId: z.string().optional().describe('Folder ID to scope rules (default: root folder "0")'),
    hostnames: z.array(z.string()).optional().describe('Domain names for the rule (required for create/update)'),
    hostname: z.string().optional().describe('Single hostname to delete (required for delete)'),
    ...
    via: z.string().optional().describe('Target: IP/hostname for Spoof, proxy code for Redirect (e.g., "JFK")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { operation, profileId, folderId, hostnames, hostname, action, enabled, via, viaV6, group } = ctx.input;

    if (operation === 'create') {
      if (!hostnames || hostnames.length === 0) throw new Error('hostnames is required for create');
      ...
      let rules = await client.listCustomRules(profileId, folderId || (group !== undefined ? String(group) : undefined));
      return {
        output: {
          rules: rules.map((r) => ({
            hostname: r.PK,
    ...
    if (operation === 'update') {
      if (!hostnames || hostnames.length === 0) throw new Error('hostnames is required for update');
