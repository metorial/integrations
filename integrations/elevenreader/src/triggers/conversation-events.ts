...
export let conversationEventsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Conversation Events',
    key: 'conversation_events',
    description: 'Triggered when a conversational AI agent call completes, including post-call transcription, audio, and call initiation failures. Configure the webhook URL in your ElevenLabs workspace settings.',
    instructions: [
  ...
  .input(z.object({
    ...
    audioBase64: z.string().optional().describe('Base64-encoded MP3 of the conversation (audio webhook only)'),
    errorMessage: z.string().optional().describe('Error message for call initiation failures'),
  }))
  .output(z.object({
    conversationId: z.string().optional().describe('ID of the conversation'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let eventId = body.event_id || body.conversation_id || `${Date.now()}-${Math.random().toString(36).slice(2)}`;
