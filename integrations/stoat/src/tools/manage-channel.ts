...
export let createChannel = SlateTool.create(
  spec,
  {
    name: 'Create Channel',
    key: 'create_channel',
  ...
  .input(z.object({
    serverId: z.string().optional().describe('ID of the server to create the channel in (omit for group DM)'),
    name: z.string().describe('Name for the new channel or group'),
    type: z.enum(['Text', 'Voice']).optional().describe('Channel type (only for server channels)'),
    ...
    users: z.array(z.string()).optional().describe('User IDs to add to a group DM (required when creating a group)'),
  }))
  .output(z.object({
...
export let fetchChannel = SlateTool.create(
  spec,
  {
    name: 'Fetch Channel',
    key: 'fetch_channel',
  ...
  .output(z.object({
    ...
    channelType: z.string().describe('Type of the channel (TextChannel, VoiceChannel, Group, DirectMessage, SavedMessages)'),
...
export let editChannel = SlateTool.create(
  spec,
  {
    name: 'Edit Channel',
    key: 'edit_channel',
  ...
  .input(z.object({
    ...
    removeFields: z.array(z.enum(['Icon', 'Description'])).optional().describe('Fields to remove/clear'),
  }))
  .output(z.object({
