...
export let fetchServer = SlateTool.create(
  spec,
  {
    name: 'Fetch Server',
    key: 'fetch_server',
  ...
  .input(z.object({
    ...
    includeChannels: z.boolean().optional().describe('Include full channel objects in the response'),
  }))
  .output(z.object({
...
export let editServer = SlateTool.create(
  spec,
  {
    name: 'Edit Server',
    key: 'edit_server',
  ...
  .input(z.object({
    ...
    discoverable: z.boolean().optional().describe('Whether the server should be discoverable'),
    analytics: z.boolean().optional().describe('Whether to enable analytics'),
    removeFields: z.array(z.enum(['Icon', 'Banner', 'Description'])).optional().describe('Fields to remove/clear from the server'),
  }))
  .output(z.object({
...
export let deleteServer = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    leaveSilently: z.boolean().optional().describe('Leave without sending a notification to other members'),
  }))
  .output(z.object({
