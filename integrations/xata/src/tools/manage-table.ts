...
export let getTableSchema = SlateTool.create(
  ...
  .output(z.object({
    ...
    columns: z.array(z.object({
      ...
      columnType: z.string().describe('Type of the column (e.g., string, int, float, bool, text, email, vector, file, link, datetime, json)'),
      ...
      unique: z.boolean().optional().describe('Whether the column has a unique constraint'),
      link: z.any().optional().describe('Link configuration if column type is "link"'),
      vector: z.any().optional().describe('Vector configuration if column type is "vector"'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.getTableColumns(ctx.input.databaseName, branch, ctx.input.tableName);
...
export let addTableColumn = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    columnType: z.string().describe('Type of the column (e.g., "string", "int", "float", "bool", "text", "email", "datetime", "vector", "link", "file", "json")'),
    notNull: z.boolean().optional().describe('Whether the column is required'),
    defaultValue: z.any().optional().describe('Default value for the column'),
    unique: z.boolean().optional().describe('Whether the column should have a unique constraint'),
    link: z.object({
      table: z.string().describe('Target table for the link'),
    }).optional().describe('Link configuration (required when columnType is "link")'),
    vector: z.object({
      dimension: z.number().describe('Vector dimension (2-10000)'),
    }).optional().describe('Vector configuration (required when columnType is "vector")'),
  }))
  .output(z.object({
