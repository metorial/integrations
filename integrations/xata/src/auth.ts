...
export let auth = SlateAuth.create()
  ...
  .addTokenAuth({
    ...
    inputSchema: z.object({
      apiKey: z.string().describe('Xata API key (starts with "xau_"). Create one in Account Settings.'),
    }),
    getOutput: async (ctx) => {
      return {
        output: {
  ...
  .addOauth({
    ...
    scopes: [
      {
        title: 'Admin Access',
        description: 'Full read and write access to all Xata resources (same permissions as an API key).',
        scope: 'admin:all',
      },
    ],
