...
export let manageRag = SlateTool.create(
  spec,
  {
  ...
  .input(
    z.object({
      action: z.enum(['upload', 'list', 'delete']).describe('Operation to perform on RAG collections'),
      collection: z.string().optional().describe('Collection name (required for "upload" and "delete" actions)'),
      documentUrl: z.string().optional().describe('URL of the document to upload (required for "upload" action). Supports PDF, DOC, DOCX, TXT, CSV, XLS, XLSX.'),
      metatag: z.string().optional().describe('Categorization tag for the uploaded document'),
      deleteAll: z.boolean().optional().describe('Set to true to delete the entire collection (for "delete" action)'),
      documentIds: z.array(z.string()).optional().describe('Specific document IDs to delete from the collection'),
    }),
  )
  .output(
    z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'upload') {
      if (!ctx.input.collection) throw new Error('"collection" is required for the upload action.');
      if (!ctx.input.documentUrl) throw new Error('"documentUrl" is required for the upload action.');
    ...
    if (ctx.input.action === 'delete') {
      if (!ctx.input.collection) throw new Error('"collection" is required for the delete action.');
