...
export let createEmbeddings = SlateTool.create(
  ...
  .input(
    z.object({
      model: z.string().describe('Embedding model identifier (e.g., "text-embedding-3-small", "text-embedding-3-large")'),
      input: z.union([z.string(), z.array(z.string())]).describe('Text to embed. Can be a single string or an array of strings.'),
      encodingFormat: z.enum(['float', 'np']).optional().describe('Output encoding format'),
      dimensions: z.number().optional().describe('Number of output vector dimensions (up to 1536)'),
    }),
  )
  .output(
    z.object({
