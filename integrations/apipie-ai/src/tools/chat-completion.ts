...
let toolFunctionSchema = z.object({
  ...
  function: z.object({
    ...
    parameters: z.record(z.string(), z.unknown()).optional().describe('The parameters the function accepts as a JSON Schema object'),
  }),
});
...
export let chatCompletion = SlateTool.create(
  ...
  .input(
    z.object({
      model: z.string().describe('Model identifier (e.g., "gpt-4o", "claude-3-opus", "gemini-pro")'),
      messages: z.array(messageSchema).describe('Array of conversation messages'),
      provider: z.string().optional().describe('Specific provider to use (e.g., "openrouter", "openai"). Omit for auto-selection.'),
      maxTokens: z.number().optional().describe('Maximum number of tokens to generate'),
      temperature: z.number().min(0).max(2).optional().describe('Sampling temperature (0-2). Lower is more deterministic.'),
      ...
      frequencyPenalty: z.number().optional().describe('Penalty for repeated tokens (-2 to 2)'),
      presencePenalty: z.number().optional().describe('Penalty for previously used tokens (-2 to 2)'),
      n: z.number().optional().describe('Number of completions to generate'),
      routing: z.enum(['price', 'perf', 'perf_avg']).optional().describe('Routing strategy: "price" for cost-optimized, "perf" for performance-optimized'),
      tools: z.array(toolFunctionSchema).optional().describe('Functions the model may call'),
      toolChoice: z.string().optional().describe('Controls function calling behavior: "auto", "none", or a specific function name'),
      responseFormat: z.record(z.string(), z.unknown()).optional().describe('Response format specification (e.g., { "type": "json_object" })'),
      online: z.boolean().optional().describe('Enable live web search grounding for the response'),
      searches: z.number().optional().describe('Number of web search queries to execute (when online is enabled)'),
      ...
      webSearchContextSize: z.enum(['low', 'medium', 'high']).optional().describe('OpenAI-compatible search context size'),
      ragTune: z.string().optional().describe('RAG collection name to augment the response with your uploaded documents'),
      integrity: z.number().optional().describe('Hallucination reduction level (12 or 13). Queries the model multiple times and uses election.'),
      memory: z.number().optional().describe('Set to 1 to enable persistent conversational memory'),
  ...
  .output(
    z.object({
      ...
      finishReason: z.string().nullable().describe('Reason the generation stopped (e.g., "stop", "length", "tool_calls")'),
      ...
      })).optional().describe('Tool/function calls requested by the model'),
