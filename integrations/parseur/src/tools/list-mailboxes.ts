...
export let listMailboxes = SlateTool.create(
  spec,
  {
    name: 'List Mailboxes',
    key: 'list_mailboxes',
  ...
  .input(z.object({
    ...
    search: z.string().optional().describe('Case-insensitive partial match search on mailbox name'),
    ordering: z.string().optional().describe('Sort field. Prefix with - for descending. Options: name, document_count, template_count, PARSEDOK_count'),
  }))
  .output(z.object({
    ...
    mailboxes: z.array(z.object({
      ...
      emailPrefix: z.string().describe('Email prefix for sending documents to this mailbox'),
