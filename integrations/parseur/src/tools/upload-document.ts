...
export let uploadDocument = SlateTool.create(
  ...
  .input(z.object({
    ...
    bodyHtml: z.string().optional().describe('Email body in HTML format (takes priority over plain text)'),
    bodyPlain: z.string().optional().describe('Email body in plain text format'),
  }))
  .output(z.object({
    submitted: z.boolean().describe('Whether the document was successfully submitted for processing'),
  }))
