...
export let getDocumentDeltas = SlateTool.create(
  spec,
  {
...
Requires deploy key authentication.`,
  ...
  .input(z.object({
    cursor: z.string().optional().describe('Cursor from a previous response to resume fetching changes'),
    tableName: z.string().optional().describe('Filter changes to a specific table')
  }))
  .output(z.object({
    changes: z.array(z.any()).describe('Array of document changes (inserts, updates, deletes)'),
