...
Runs synchronously and returns generated image URLs.`,
  ...
  .input(z.object({
    modelId: z.string().describe('Model endpoint ID, e.g. "fal-ai/flux/schnell", "fal-ai/flux/dev", "fal-ai/stable-diffusion-v35-large"'),
    prompt: z.string().describe('Text prompt describing the desired image'),
    negativePrompt: z.string().optional().describe('Text describing what to avoid in the generated image'),
    imageUrl: z.string().optional().describe('Input image URL for image-to-image generation'),
    imageSize: z.union([
    ...
    ]).optional().describe('Output image size as a preset or custom dimensions'),
    numInferenceSteps: z.number().optional().describe('Number of inference steps (higher = more detail, slower)'),
    guidanceScale: z.number().optional().describe('Guidance scale for prompt adherence, typically 1.0-20.0'),
    ...
    enableSafetyChecker: z.boolean().optional().describe('Whether to enable the safety content checker'),
    loras: z.array(loraSchema).optional().describe('LoRA adapters to apply for customized styles'),
    strength: z.number().optional().describe('Strength for image-to-image transformation, 0.0-1.0'),
  }))
  .output(z.object({
    ...
    timings: z.record(z.string(), z.any()).optional().describe('Timing information for the generation process'),
    hasNsfwConcepts: z.array(z.boolean()).optional().describe('NSFW detection results per image'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.enableSafetyChecker !== undefined) input.enable_safety_checker = ctx.input.enableSafetyChecker;
