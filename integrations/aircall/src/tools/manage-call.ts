...
export let manageCall = SlateTool.create(
  ...
  .input(z.object({
    ...
    transferToUserId: z.number().optional().describe('User ID to transfer the call to (for transfer action)'),
    transferToTeamId: z.number().optional().describe('Team ID to transfer the call to (for transfer action)'),
    transferToNumber: z.string().optional().describe('External phone number to transfer the call to in E.164 format (for transfer action)'),
    dispatchingStrategy: z.enum(['simultaneous', 'random', 'longest_idle']).optional().describe('Strategy for team transfers (default: simultaneous)'),
    commentContent: z.string().optional().describe('Comment text to add to the call (for comment action)'),
