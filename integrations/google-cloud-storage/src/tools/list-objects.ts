...
export let listObjects = SlateTool.create(
  ...
  .input(z.object({
    ...
    prefix: z.string().optional().describe('Filter objects whose names begin with this prefix (e.g., "folder/")'),
    delimiter: z.string().optional().describe('Delimiter for folder-like grouping (typically "/")'),
    ...
    includeVersions: z.boolean().optional().describe('Include all object versions (requires versioning enabled)'),
  }))
  .output(z.object({
    ...
    prefixes: z.array(z.string()).optional().describe('Common prefixes when using a delimiter (simulated folders)'),
