...
export let updateObjectMetadata = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    contentDisposition: z.string().optional().describe('Content-Disposition header (e.g., "attachment; filename=file.txt")'),
    cacheControl: z.string().optional().describe('Cache-Control header (e.g., "public, max-age=3600")'),
    ...
    customMetadata: z.record(z.string(), z.string()).optional().describe('Custom key-value metadata. Replaces all existing custom metadata.'),
    temporaryHold: z.boolean().optional().describe('Place or release a temporary hold on the object'),
    eventBasedHold: z.boolean().optional().describe('Place or release an event-based hold on the object'),
  }))
  .output(z.object({
    objectName: z.string(),
    bucketName: z.string(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.contentDisposition) patchBody.contentDisposition = ctx.input.contentDisposition;
    ...
    if (ctx.input.temporaryHold !== undefined) patchBody.temporaryHold = ctx.input.temporaryHold;
    if (ctx.input.eventBasedHold !== undefined) patchBody.eventBasedHold = ctx.input.eventBasedHold;

    let result = await client.updateObjectMetadata(ctx.input.bucketName, ctx.input.objectName, patchBody);
