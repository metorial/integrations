...
export let sendClipboard = SlateTool.create(
  spec,
  {
    name: 'Send Clipboard',
    key: 'send_clipboard',
  ...
  .input(z.object({
    ...
    sourceDeviceIden: z.string().optional().describe('Identifier of the device sending the clipboard content'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let preview = ctx.input.text.length > 50
      ? ctx.input.text.substring(0, 50) + '...'
      : ctx.input.text;
