...
export let getListeningMessages = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    networks: z.array(z.string()).optional().describe('Networks to filter by (e.g., "INSTAGRAM", "FACEBOOK", "REDDIT", "YOUTUBE", "WEB").'),
    sentiment: z.array(z.string()).optional().describe('Sentiment to filter by (e.g., "POSITIVE", "NEGATIVE", "NEUTRAL").'),
    fields: z.array(z.string()).optional().describe('Fields to return (e.g., "created_time", "text", "from.name", "hashtags").'),
    metrics: z.array(z.string()).optional().describe('Metrics to return per message (e.g., "likes", "replies", "shares_count").'),
