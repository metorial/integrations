...
export let createDraftPost = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    customerProfileIds: z.array(z.number()).describe('Array of customer_profile_id values to publish to.'),
    ...
    scheduledTimes: z.array(z.string()).optional().describe('Array of UTC ISO 8601 timestamps for scheduled delivery (e.g., "2024-06-30T18:20:00Z").'),
    tagIds: z.array(z.number()).optional().describe('Array of tag IDs to apply to the post.')
  }))
  .output(z.object({
    posts: z.array(z.any()).describe('Array of fanned-out publishing post entries created by Sprout.')
  }))
