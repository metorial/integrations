...
export let getCases = SlateTool.create(
  spec,
  {
    name: 'Get Cases',
    key: 'get_cases',
    ...
    constraints: [
      'Date filter ranges must not exceed one week.'
    ],
    tags: {
      readOnly: true
    }
  ...
  .input(z.object({
    filters: z.array(z.string()).describe('Array of filter expressions (e.g., "status.eq(OPEN)", "created_time.in(2024-01-01..2024-01-07)").'),
