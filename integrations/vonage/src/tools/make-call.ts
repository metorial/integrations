...
let nccoActionSchema = z.record(z.string(), z.unknown()).describe('NCCO action object (e.g., {"action": "talk", "text": "Hello"})');
...
Requires the **API Key, Secret & Application JWT** auth method.`,
  ...
  .input(z.object({
    toNumber: z.string().describe('Destination phone number in E.164 format (e.g., "14155550100")'),
    fromNumber: z.string().describe('Caller ID phone number in E.164 format'),
    ncco: z.array(nccoActionSchema).optional().describe('Inline NCCO actions array to control the call flow'),
    answerUrl: z.string().optional().describe('URL returning NCCO JSON to control the call (alternative to inline ncco)'),
    eventUrl: z.string().optional().describe('Webhook URL for call status events'),
    machineDetection: z.enum(['continue', 'hangup']).optional().describe('Answering machine detection behavior'),
    lengthTimer: z.number().optional().describe('Maximum call duration in seconds (default: 7200)'),
