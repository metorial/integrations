...
let externalReferenceSchema = z.object({
...
}).optional().describe('External reference for linking to third-party tools');
...
export let manageTimeEntry = SlateTool.create(
  ...
  .input(z.object({
    ...
    spentDate: z.string().optional().describe('Date the time was spent (YYYY-MM-DD, required for create)'),
    userId: z.number().optional().describe('User ID to assign the entry to (defaults to current user)'),
    hours: z.number().optional().describe('Number of hours tracked (for duration-based entries)'),
    startedTime: z.string().optional().describe('Start time (HH:MM format, for timer-based entries)'),
    endedTime: z.string().optional().describe('End time (HH:MM format, for timer-based entries)'),
