...
export let deploySmartContract = SlateTool.create(
  spec,
  {
...
Use **template deployment** for standard token contracts or **bytecode deployment** for custom contracts.`,
  ...
  .input(z.object({
    source: z.enum(['template', 'bytecode']).describe('Whether to deploy from a template or custom bytecode'),
    ...
    bytecode: z.string().optional().describe('Compiled contract bytecode when deploying from bytecode'),
    network: networkEnum,
    signerWallet: z.string().describe('Wallet address to sign and pay for the deployment transaction'),
    ...
    constructorParams: z.array(z.any()).default([]).describe('Constructor parameters in order'),
    speed: z.enum(['low', 'average', 'fast', 'fastest']).default('average').describe('Gas fee strategy for the transaction'),
  }))
  .output(z.object({
