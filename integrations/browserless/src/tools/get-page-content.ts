...
export let getPageContent = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    html: z.string().optional().describe('Raw HTML to render in the browser instead of navigating to a URL'),
    ...
    }).optional().describe('Wait for a CSS selector before capturing content'),
    waitForTimeout: z.number().optional().describe('Wait a fixed number of milliseconds'),
    bestAttempt: z.boolean().optional().describe('Proceed even when async events fail or timeout'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: { htmlContent: typeof htmlContent === 'string' ? htmlContent : String(htmlContent) },
