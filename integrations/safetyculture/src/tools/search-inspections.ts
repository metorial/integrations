...
export let searchInspections = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    modifiedAfter: z.string().optional().describe('ISO 8601 timestamp to filter inspections modified after this date'),
    templateIds: z.array(z.string()).optional().describe('Filter inspections by one or more template IDs'),
    archived: z.enum(['true', 'false', 'both']).optional().describe('Filter by archived status. Defaults to non-archived.'),
    completed: z.enum(['true', 'false', 'both']).optional().describe('Filter by completion status'),
    limit: z.number().optional().describe('Maximum number of inspections to return (default 100)'),
  }))
  .output(z.object({
