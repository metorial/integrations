...
export let leadCollected = SlateTrigger.create(
  spec,
  {
    name: 'Lead Collected',
    key: 'lead_collected',
    description: 'Triggers when a new lead is collected through a Proofly notification widget (e.g., an email collector form).',
  }
)
  .input(z.object({
    ...
    notificationId: z.string().optional().describe('ID of the notification that collected the lead'),
    campaignId: z.string().optional().describe('ID of the campaign the notification belongs to'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let inputs = events.map((event: any) => ({
        leadId: String(event.id ?? event._id ?? event.leadId ?? event.lead_id ?? `${Date.now()}-${Math.random().toString(36).slice(2, 9)}`),
        submittedInput: event.input ?? event.email ?? event.submittedInput ?? event.submitted_input ?? event.value,
        ...
        notificationId: event.notificationId ?? event.notification_id ? String(event.notificationId ?? event.notification_id) : undefined,
        campaignId: event.campaignId ?? event.campaign_id ? String(event.campaignId ?? event.campaign_id) : undefined,
