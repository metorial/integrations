...
export let manageEnvironment = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'Deleting an environment removes all flag definitions in that environment.',
    ],
  }
)
  .input(z.object({
    ...
    production: z.boolean().optional().describe('Mark as production environment (for create only). Defaults to false.'),
  }))
  .output(z.object({
