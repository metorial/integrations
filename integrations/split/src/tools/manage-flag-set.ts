...
export let manageFlagSet = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    description: z.string().optional().describe('Description for the flag set (for create).'),
    limit: z.number().optional().describe('Max results to return (for list). Defaults to 50.'),
    after: z.string().optional().describe('Pagination cursor (for list).'),
  }))
  .output(z.object({
    flagSets: z.array(z.object({
      flagSetId: z.string(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'create': {
        ...
        if (!ctx.input.flagSetName) throw new Error('flagSetName is required to create a flag set.');
        ...
        return {
          output: {
            flagSet: { flagSetId: fs.id, flagSetName: fs.name, description: fs.description, createdAt: fs.createdAt },
          },
      ...
      case 'list': {
        ...
        let result = await client.listFlagSets(wsId, { limit: ctx.input.limit, after: ctx.input.after });
      ...
      case 'delete': {
        if (!ctx.input.flagSetId) throw new Error('flagSetId is required to delete a flag set.');
