...
export let adminAudit = SlateTrigger.create(
  spec,
  {
    name: 'Admin Audit Log',
    key: 'admin_audit',
    description: 'Triggers on admin-level configuration changes such as project, environment, API key, traffic type, user, group, or security setting modifications. Configure the "Outgoing webhook (Admin audit log)" integration in Split UI to point to this trigger\'s URL.',
  }
)
  .input(z.object({
    auditId: z.string().describe('Unique ID of the audit log entry.'),
    auditLogType: z.string().describe('Object type and change type, e.g., "security_settings.updated".'),
    ...
    currentObject: z.any().optional().describe('Current state of the changed object, null if deleted.'),
    changes: z.record(z.string(), z.any()).optional().describe('Map of changed properties with from/to values.'),
  }))
  .output(z.object({
    auditId: z.string().describe('Unique audit log entry ID.'),
    auditLogType: z.string().describe('Change type identifier (e.g., "environment.created").'),
    ...
    changes: z.record(z.string(), z.any()).optional().describe('Changed properties with before/after values.'),
  }))
  .webhook({
