...
Use for automated messages like order confirmations, password resets, or any triggered email communication.`,
  ...
  .input(z.object({
    ...
    templateId: z.number().optional().describe('Brevo template ID to use instead of inline content'),
    templateParams: z.record(z.string(), z.any()).optional().describe('Dynamic variables for template personalization'),
    tags: z.array(z.string()).optional().describe('Tags for categorizing and tracking the email'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.sendTransactionalEmail({
      ...
      cc: ctx.input.cc,
      bcc: ctx.input.bcc,
      replyTo: ctx.input.replyToEmail ? { email: ctx.input.replyToEmail, name: ctx.input.replyToName } : undefined,
