...
export let createDeal = SlateTool.create(
  ...
  .input(z.object({
    ...
    attributes: z.record(z.string(), z.any()).optional().describe('Custom deal attributes (e.g., deal_stage, pipeline, deal_owner, amount)'),
    linkedContactIds: z.array(z.number()).optional().describe('Contact IDs to link to this deal'),
    linkedCompanyIds: z.array(z.string()).optional().describe('Company IDs to link to this deal')
  }))
  .output(z.object({
...
**Note:** Updating linkedContactIds or linkedCompanyIds replaces the entire list -- omitted IDs will be removed.`,
  ...
  .input(z.object({
    ...
    attributes: z.record(z.string(), z.any()).optional().describe('Deal attributes to update'),
    linkedContactIds: z.array(z.number()).optional().describe('Full list of contact IDs to link (replaces existing)'),
    linkedCompanyIds: z.array(z.string()).optional().describe('Full list of company IDs to link (replaces existing)')
  }))
  .output(z.object({
...
export let listDeals = SlateTool.create(
  ...
  .input(z.object({
    ...
    modifiedSince: z.string().optional().describe('Filter deals modified after this UTC date-time'),
    createdSince: z.string().optional().describe('Filter deals created after this UTC date-time')
  }))
  .output(z.object({
