...
export let uploadBulkList = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    fileContent: z.string().describe('Raw file content as a string (CSV or TXT format with EMAIL column header)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.uploadBulkList(ctx.input.fileName, ctx.input.fileContent, ctx.input.fileType);
