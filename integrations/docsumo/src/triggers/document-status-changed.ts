...
export let documentStatusChanged = SlateTrigger.create(
  spec,
  {
    name: 'Document Status Changed',
    key: 'document_status_changed',
    description: 'Triggers when a document\'s processing status changes in Docsumo, such as when processing completes, a document is approved, or its review status is updated.',
    instructions: [
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let documents = body.data?.document || body.data?.documents || body.document || body.documents;
      if (!documents) {
      ...
      let inputs = docList.map((doc: any) => ({
    ...
    handleEvent: async (ctx) => {
      ...
      let doc = payloadData.document?.[0] || payloadData.document || payload.document || payloadData || payload;
