...
export let uploadImageTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    imageUrl: z.string().optional().describe('Publicly accessible URL of the image to upload'),
    ...
    split: z.enum(['train', 'valid', 'test']).optional().describe('Dataset split to assign the image to'),
  }))
  .output(z.object({
    success: z.boolean().describe('Whether the upload succeeded'),
    duplicate: z.boolean().optional().describe('Whether the image was already present in the dataset'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.imageUrl) {
      ...
      result = await client.uploadImageBase64(ctx.input.projectId, ctx.input.base64Data, options);
    } else {
