...
Use "root" as the collectionId to access the root collection.`,
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'get', 'list_items', 'tree']).describe('The action to perform'),
    collectionId: z.union([z.number(), z.literal('root')]).optional().describe('ID of the collection (required for get, update, list_items). Use "root" for root collection.'),
    ...
    parentId: z.number().nullable().optional().describe('Parent collection ID (null for root level)'),
    ...
    itemModels: z.array(z.enum(['card', 'dashboard', 'collection', 'pulse'])).optional().describe('Filter items by model type (for list_items action)'),
    sortColumn: z.enum(['name', 'last_edited_at', 'last_edited_by', 'model']).optional().describe('Sort column for items'),
    sortDirection: z.enum(['asc', 'desc']).optional().describe('Sort direction for items')
  }))
  .output(z.object({
    collectionId: z.union([z.number(), z.string()]).optional().describe('ID of the collection'),
    ...
    })).optional().describe('Items in the collection (for list_items and tree actions)'),
