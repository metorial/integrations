...
export let manageQuestion = SlateTool.create(
  spec,
  {
...
Set **archived** to true to move a question to the trash.`,
  ...
  .input(z.object({
    ...
    cardId: z.number().optional().describe('ID of the question/card (required for get and update)'),
    ...
    display: z.string().optional().describe('Display type (e.g., table, bar, line, pie, scalar, row, area, combo, pivot, scatter, waterfall, funnel, map)'),
    datasetQuery: z.any().optional().describe('The query definition object containing database, type, and query/native fields'),
    collectionId: z.number().nullable().optional().describe('ID of the collection to place the question in, or null for root'),
