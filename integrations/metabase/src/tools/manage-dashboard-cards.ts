...
export let manageDashboardCards = SlateTool.create(
  spec,
  {
...
Parameter mappings allow you to connect dashboard filters to the card's parameters.`,
  ...
  .input(z.object({
    ...
    cardId: z.number().optional().describe('ID of the question/card to add (required for add)'),
    dashcardId: z.number().optional().describe('ID of the dashcard to remove (required for remove)'),
    row: z.number().optional().describe('Row position on the dashboard grid (default: 0)'),
    col: z.number().optional().describe('Column position on the dashboard grid (default: 0)'),
    ...
    parameterMappings: z.array(z.any()).optional().describe('Parameter mappings connecting dashboard filters to card parameters')
  }))
  .output(z.object({
    dashboardId: z.number().describe('ID of the dashboard'),
