...
let autoScheduledSchema = z.object({
  startDate: z.string().optional().describe('ISO 8601 date for when the task should start being scheduled'),
  deadlineType: z.enum(['HARD', 'SOFT', 'NONE']).optional().describe('Type of deadline. HARD = must be done by due date, SOFT = preferred by due date, NONE = no deadline pressure'),
  schedule: z.string().optional().describe('Schedule to use for auto-scheduling. Defaults to "Work Hours". Must be "Work Hours" when assigning to other users.'),
}).optional().describe('Auto-scheduling configuration. Set to null to disable auto-scheduling.');
...
let taskOutputSchema = z.object({
  ...
  duration: z.union([z.string(), z.number()]).optional().describe('Duration in minutes, "NONE", or "REMINDER"'),
  ...
  schedulingIssue: z.boolean().optional().describe('Whether Motion was unable to schedule this task'),
...
export let createTask = SlateTool.create(
  ...
  .input(z.object({
    ...
    dueDate: z.string().optional().describe('ISO 8601 due date. Required for scheduled tasks.'),
    duration: z.union([z.number(), z.string()]).optional().describe('Duration in minutes (integer > 0), "NONE", or "REMINDER"'),
    status: z.string().optional().describe('Task status. Defaults to workspace default status.'),
    projectId: z.string().optional().describe('ID of the project to associate the task with'),
    description: z.string().optional().describe('Task description in GitHub Flavored Markdown'),
    priority: z.enum(['ASAP', 'HIGH', 'MEDIUM', 'LOW']).optional().describe('Priority level for the task'),
    ...
    autoScheduled: z.union([autoScheduledSchema, z.null()]).optional().describe('Auto-scheduling configuration. Set to null to disable.'),
  }))
