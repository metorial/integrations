...
export let runJob = SlateTool.create(
  ...
  .input(z.object({
    ...
    data: z.array(z.record(z.string(), z.string())).describe('Array of data rows to process. Each row must contain the required data fields for the app.'),
    ...
    callBackUrl: z.string().optional().describe('URL to receive job results when processing completes'),
    noMailOnFinish: z.boolean().optional().describe('Set to true to suppress email notifications when the job finishes'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.createJob({
      ...
      data: ctx.input.data,
      config: (ctx.input.language || ctx.input.country) ? {
