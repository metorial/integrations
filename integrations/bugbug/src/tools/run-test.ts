...
export let runTest = SlateTool.create(
  ...
  .input(z.object({
    ...
    profileId: z.string().optional().describe('Profile ID to use for the run (defines browser, variables, etc.)'),
    overrideVariables: z.record(z.string(), z.string()).optional().describe('Key-value pairs to override test variables for this run'),
  }))
  .output(z.object({
