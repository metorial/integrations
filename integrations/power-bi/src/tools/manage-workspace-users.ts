...
let userSchema = z.object({
  ...
  groupUserAccessRight: z.string().describe('Access role: Admin, Member, Contributor, or Viewer'),
...
export let manageWorkspaceUsers = SlateTool.create(
  ...
  .input(z.object({
    ...
    emailAddress: z.string().optional().describe('Email of the user (required for add/update)'),
    role: z.enum(['Admin', 'Member', 'Contributor', 'Viewer']).optional().describe('Role to assign (required for add/update)'),
    userId: z.string().optional().describe('User ID to remove (required for remove)')
  }))
  .output(z.object({
    users: z.array(userSchema).optional().describe('List of workspace users (for list action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'add') {
      if (!emailAddress || !role) throw new Error('emailAddress and role are required for add');
    ...
    if (action === 'update') {
      if (!emailAddress || !role) throw new Error('emailAddress and role are required for update');
