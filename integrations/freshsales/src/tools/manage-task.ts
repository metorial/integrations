...
export let manageTask = SlateTool.create(
  ...
  .input(z.object({
    taskId: z.number().optional().describe('ID of the task to update. Omit to create a new task.'),
    title: z.string().optional().describe('Task title'),
    description: z.string().optional().describe('Task description'),
    dueDate: z.string().optional().describe('Due date in ISO format (e.g. "2026-03-15T10:00:00Z")'),
    ownerId: z.number().optional().describe('Assigned user ID'),
    targetableId: z.number().optional().describe('ID of the associated record (contact, lead, deal, or account)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.targetableType !== undefined) taskData.targetable_type = ctx.input.targetableType;
