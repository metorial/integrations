...
export let manageSalesActivity = SlateTool.create(
  spec,
  {
...
Use the **listSelectors** tool to find valid activity type and outcome IDs.`,
  ...
  .input(z.object({
    activityId: z.number().optional().describe('ID of the activity to update. Omit to create.'),
    title: z.string().optional().describe('Activity title'),
    notes: z.string().optional().describe('Activity notes/description'),
    salesActivityTypeId: z.number().optional().describe('Activity type ID (use listSelectors to find valid IDs)'),
    ...
    targetableType: z.enum(['Contact', 'Lead', 'Deal', 'SalesAccount']).optional().describe('Type of the associated record'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.salesActivityTypeId !== undefined) activityData.sales_activity_type_id = ctx.input.salesActivityTypeId;
    if (ctx.input.salesActivityOutcomeId !== undefined) activityData.sales_activity_outcome_id = ctx.input.salesActivityOutcomeId;
    if (ctx.input.targetableId !== undefined) activityData.targetable_id = ctx.input.targetableId;
    if (ctx.input.targetableType !== undefined) activityData.targetable_type = ctx.input.targetableType;
