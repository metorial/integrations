...
export let listTasks = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Only returns tasks for the API key owner, not other users.',
    ],
    tags: {
      destructive: false,
      readOnly: true,
  ...
  .input(z.object({
    filter: z.enum(['open', 'due today', 'due tomorrow', 'overdue', 'completed']).describe('Task filter'),
    includeAssociations: z.boolean().optional().describe('Include associated users and target records'),
  }))
  .output(z.object({
