...
Supports custom fields and social media profiles. Use **uniqueIdentifier** to upsert by email or other unique fields.`,
  ...
  .input(z.object({
    contactId: z.number().optional().describe('ID of the contact to update. Omit to create.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.mobileNumber !== undefined) contactData.mobile_number = ctx.input.mobileNumber;
    ...
    if (ctx.input.leadSourceId !== undefined) contactData.lead_source_id = ctx.input.leadSourceId;
    if (ctx.input.contactStatusId !== undefined) contactData.contact_status_id = ctx.input.contactStatusId;
    if (ctx.input.lifecycleStageId !== undefined) contactData.lifecycle_stage_id = ctx.input.lifecycleStageId;
