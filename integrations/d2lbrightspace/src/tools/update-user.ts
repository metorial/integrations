...
export let updateUser = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.externalEmail !== undefined) updateData.ExternalEmail = ctx.input.externalEmail;
    ...
    if (ctx.input.isActive !== undefined) updateData.Activation = { IsActive: ctx.input.isActive };
