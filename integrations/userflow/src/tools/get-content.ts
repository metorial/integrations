...
export let getContent = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeVersions: z.boolean().optional().describe('If true, also fetches content versions'),
    includeSessions: z.boolean().optional().describe('If true, also fetches content sessions'),
  ...
  .output(z.object({
    ...
    versions: z.array(z.record(z.string(), z.unknown())).optional().describe('Content versions if requested'),
    sessions: z.array(z.record(z.string(), z.unknown())).optional().describe('Content sessions if requested'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includeVersions) {
      let versionsResult = await client.listContentVersions({ contentId: ctx.input.contentId });
