...
export let subscriberEvents = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers when subscriber events occur, such as creation, updates, unsubscribes, group changes, bounces, spam reports, and automation activity.',
  }
)
  .input(z.object({
    ...
    automation: z.any().optional().describe('Automation data (for automation-related events)'),
  }))
  .output(z.object({
    ...
    automationId: z.string().optional().describe('Related automation ID (for automation events)'),
    automationName: z.string().optional().describe('Related automation name (for automation events)'),
  }))
  .webhook({
