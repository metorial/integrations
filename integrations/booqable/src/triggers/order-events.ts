...
export let orderEvents = SlateTrigger.create(
  spec,
  {
    name: 'Order Events',
    key: 'order_events',
    description: 'Triggers when an order is created, updated, reserved, started, stopped, canceled, or archived.',
  }
)
  .input(z.object({
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      let attrs = ctx.input.payload?.data?.attributes || ctx.input.payload?.order || ctx.input.payload || {};
