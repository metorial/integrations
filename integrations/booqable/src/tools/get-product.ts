...
export let getProduct = SlateTool.create(
  ...
  .output(z.object({
    productGroup: z.record(z.string(), z.any()).describe('The product group record with all attributes'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client(buildClientConfig(ctx));

    let response = await client.getProductGroup(ctx.input.productGroupId, ['products', 'photos']);
