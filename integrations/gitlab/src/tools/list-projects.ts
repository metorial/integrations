...
export let listProjects = SlateTool.create(
  ...
  .input(z.object({
    ...
    owned: z.boolean().optional().describe('Only list projects owned by the authenticated user'),
    membership: z.boolean().optional().describe('Only list projects the authenticated user is a member of'),
    visibility: z.enum(['public', 'internal', 'private']).optional().describe('Filter by project visibility'),
    archived: z.boolean().optional().describe('Filter by archived status'),
    orderBy: z.enum(['id', 'name', 'path', 'created_at', 'updated_at', 'last_activity_at']).optional().describe('Order by field'),
