...
export let manageApprovals = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    approvalTaskId: z.string().optional().describe('ID of the approval task (required for approve/reject)'),
    subjectTaskId: z.string().optional().describe('ID of the task being reviewed (optional for approve/reject)'),
    comment: z.string().optional().describe('Optional comment for the approval or rejection'),
  }))
  .output(z.object({
    approvals: z.array(approvalSchema).optional().describe('List of approvals (for list action)'),
