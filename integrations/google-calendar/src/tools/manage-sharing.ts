...
export let manageSharing = SlateTool.create(
  spec,
  {
    name: 'Manage Sharing',
    key: 'manage_sharing',
  ...
  .input(z.object({
    ...
    scopeType: z.enum(['user', 'group', 'domain', 'default']).optional().describe('Type of scope to grant access to (required for grant)'),
    scopeValue: z.string().optional().describe('Email address, group email, or domain (required for grant unless scopeType is "default")'),
    role: z.enum(['none', 'freeBusyReader', 'reader', 'writer', 'owner']).optional().describe('Access role to grant or update to (required for grant and update)')
  }))
  .output(z.object({
    ...
    })).optional().describe('List of ACL rules (for list action)'),
    rule: z.object({
      ruleId: z.string().optional().describe('ACL rule ID'),
      scopeType: z.string().optional().describe('Scope type'),
      scopeValue: z.string().optional().describe('Scope value'),
      role: z.string().optional().describe('Access role')
    }).optional().describe('Created/updated ACL rule'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update') {
      ...
      let rule = await client.updateAcl(calendarId, ctx.input.ruleId, { role: ctx.input.role });
      return {
        output: {
          rule: {
            ruleId: rule.id,
