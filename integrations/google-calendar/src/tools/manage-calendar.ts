...
export let manageCalendar = SlateTool.create(
  spec,
  {
...
Use the **action** field to select the operation.`,
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'delete', 'subscribe', 'unsubscribe']).describe('Operation to perform'),
    calendarId: z.string().optional().describe('Calendar ID (required for update, delete, subscribe, unsubscribe)'),
    summary: z.string().optional().describe('Calendar title (used for create and update)'),
    description: z.string().optional().describe('Calendar description (used for create and update)'),
    location: z.string().optional().describe('Calendar location (used for create and update)'),
    timeZone: z.string().optional().describe('IANA time zone (used for create and update)'),
    colorId: z.string().optional().describe('Color ID for the calendar in the list (used for subscribe)'),
    hidden: z.boolean().optional().describe('Whether to hide the calendar (used for subscribe)'),
    selected: z.boolean().optional().describe('Whether the calendar is selected in the UI (used for subscribe)'),
    summaryOverride: z.string().optional().describe('Custom display name (used for subscribe)')
  }))
  .output(z.object({
    ...
    deleted: z.boolean().optional().describe('Whether the calendar was deleted/unsubscribed'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update') {
      if (!ctx.input.calendarId) throw new Error('calendarId is required for updating a calendar');
      let updates: any = {};
    ...
    if (action === 'delete') {
      if (!ctx.input.calendarId) throw new Error('calendarId is required for deleting a calendar');
    ...
    if (action === 'subscribe') {
      if (!ctx.input.calendarId) throw new Error('calendarId is required for subscribing to a calendar');
    ...
    if (action === 'unsubscribe') {
      if (!ctx.input.calendarId) throw new Error('calendarId is required for unsubscribing from a calendar');
