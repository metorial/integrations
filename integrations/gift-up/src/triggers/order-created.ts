...
export let orderCreatedTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Order Created',
    key: 'order_created',
    description: 'Fires when a new order is placed in the Gift Up! checkout or created via the API.',
  }
)
  .input(z.object({
    ...
    }).passthrough()).nullable().describe('Gift cards in the order'),
    customFields: z.array(z.object({
      label: z.string(),
      value: z.any(),
    })).nullable().describe('Custom fields'),
    salesTaxes: z.array(z.object({
      label: z.string(),
      amount: z.number(),
      type: z.string(),
    })).nullable().describe('Sales taxes'),
