...
export let listTransactions = SlateTool.create(
  ...
  .input(z.object({
    ...
    updatedOnOrAfter: z.string().optional().describe('Filter by update date (ISO 8601, UTC)'),
    giftCardCode: z.string().optional().describe('Filter by gift card code'),
    eventType: z.enum(['GiftCardCreated', 'CreditAdded', 'Redeemed', 'Expired', 'Unexpired', 'Voided', 'Reactivated']).optional().describe('Filter by event type'),
