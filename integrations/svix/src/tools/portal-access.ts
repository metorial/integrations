...
export let getPortalAccess = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    applicationId: z.string().describe('Application ID or UID to generate portal access for'),
    featureFlags: z.array(z.string()).optional().describe('Feature flags to restrict which event types are visible in the portal'),
  }))
  .output(z.object({
    portalUrl: z.string().describe('Short-lived URL for the App Portal'),
    portalToken: z.string().describe('Short-lived token for API access scoped to this application'),
  }))
