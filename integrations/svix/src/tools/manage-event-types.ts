...
export let listEventTypes = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeArchived: z.boolean().optional().describe('Whether to include archived event types'),
  ...
  .output(z.object({
    eventTypes: z.array(z.object({
      ...
      schemas: z.record(z.string(), z.unknown()).optional().describe('JSON Schema for the event payload'),
...
export let createEventType = SlateTool.create(
  ...
  .input(z.object({
    name: z.string().describe('Event type name (e.g., "invoice.paid"). Must match [a-zA-Z0-9\\-_.]+'),
    description: z.string().describe('Human-readable description of the event type'),
    schemas: z.record(z.string(), z.unknown()).optional().describe('JSONSchema (Draft 7) defining the expected payload shape'),
    archived: z.boolean().optional().describe('Whether to create the event type as archived'),
    featureFlag: z.string().optional().describe('Feature flag to control visibility of this event type'),
  }))
  .output(z.object({
...
export let deleteEventType = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    eventTypeName: z.string().describe('Name of the event type to delete (e.g., "invoice.paid")'),
  }))
  .output(z.object({
