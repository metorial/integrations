...
export let getEndpointStats = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    includeSecret: z.boolean().optional().describe('Whether to include the endpoint signing secret in the response'),
  }))
  .output(z.object({
    ...
    signingSecret: z.string().optional().describe('Endpoint signing secret (only if includeSecret was true)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includeSecret) {
      ...
      let secret = await client.getEndpointSecret(ctx.input.applicationId, ctx.input.endpointId);
