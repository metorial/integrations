...
Each Supabase project exposes APIs at `https://<project_ref>.supabase.co/`. There are 4 types of API keys: anon and service_role keys are based on the project's JWT secret. Supabase is transitioning to new `publishable` and `secret` key types (prefixed `sb_publishable_` and `sb_secret_`).
