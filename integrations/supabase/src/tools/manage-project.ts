...
export let manageProject = SlateTool.create(
  ...
  .input(z.object({
    ...
    projectRef: z.string().optional().describe('Project reference ID (required for pause, restore, delete)'),
    ...
    region: z.string().optional().describe('Database region, e.g., us-east-1 (required for create)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      if (!ctx.input.name || !ctx.input.organizationId || !ctx.input.region || !ctx.input.dbPass) {
        throw new Error('name, organizationId, region, and dbPass are required for creating a project');
      }
