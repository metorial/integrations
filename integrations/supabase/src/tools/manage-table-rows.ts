...
export let manageTableRows = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['insert', 'update', 'upsert', 'delete']).describe('Operation to perform'),
    rows: z.array(z.record(z.string(), z.any())).optional().describe('Rows to insert or upsert'),
    updates: z.record(z.string(), z.any()).optional().describe('Column values to update (for update action)'),
    filters: z.record(z.string(), z.string()).optional().describe('PostgREST filters to match rows (for update and delete)'),
  ...
  .output(z.object({
    rows: z.array(z.record(z.string(), z.any())).describe('Affected rows (returned from the operation)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!apiKey) {
      throw new Error('Could not retrieve service_role API key for the project. service_role key is required for data modifications.');
    }
    ...
    if (action === 'insert') {
      ...
      if (!ctx.input.filters || Object.keys(ctx.input.filters).length === 0) {
        throw new Error('filters are required for update action to prevent accidental full-table updates');
      }
      ...
      // delete
      if (!ctx.input.filters || Object.keys(ctx.input.filters).length === 0) {
        throw new Error('filters are required for delete action to prevent accidental full-table deletes');
      }
