...
export let createSslTest = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    alertAt: z.array(z.number()).optional().describe('Array of 3 days before expiry to send alerts (e.g. [1, 7, 30])'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { websiteUrl, checkRate, contactGroups, alertAt, alertBroken, alertExpiry, alertMixed, alertReminder, followRedirects, userAgent, ...rest } = ctx.input;
