...
export let downloadRecording = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'The signed download URL expires 6 hours after being generated.',
    ],
    tags: {
      destructive: false,
      readOnly: true,
  ...
  .input(z.object({
    meetingId: z.string().describe('The unique identifier of the meeting whose recording to download.'),
  }))
  .output(z.object({
    downloadUrl: z.string().describe('Signed URL to download the recording. Expires after 6 hours.'),
  }))
