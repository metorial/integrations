...
let offerOutputSchema = z.object({
  ...
  durationInMonths: z.number().nullable().describe('Duration in months for repeating discounts'),
...
export let manageOffer = SlateTool.create(
  spec,
  {
    name: 'Manage Offer',
    key: 'manage_offer',
  ...
  .input(z.object({
    ...
    amount: z.number().optional().describe('Discount amount (percent 0-100, fixed in cents, trial in days)'),
    currency: z.string().optional().describe('Currency for fixed discounts (e.g., "usd")'),
    duration: z.enum(['once', 'forever', 'repeating', 'trial']).optional().describe('How long the discount lasts'),
    durationInMonths: z.number().optional().describe('Duration in months for repeating discounts'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'read') {
      ...
      return { output: { offer }, message: `Retrieved offer **"${offer.name}"** (${offer.status}).` };
    }
    ...
    if (ctx.input.displayDescription !== undefined) data.display_description = ctx.input.displayDescription;
    ...
    if (action === 'create') {
      ...
      let offer = mapOffer(result.offers[0]);
      return { output: { offer }, message: `Created offer **"${offer.name}"** with code \`${offer.code}\`.` };
    }
