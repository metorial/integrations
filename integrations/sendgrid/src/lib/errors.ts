...
export let sendgridServiceError = (message: string) =>
...
export let sendgridApiError = (error: unknown, operation = 'request') => {
  ...
  let response = isRecord(error) ? (error.response as ErrorResponse | undefined) : undefined;
  let serviceError = sendgridServiceError(
    `SendGrid API ${operation} failed: ${statusLabelFor(response)}${extractSendGridMessage(error)}`
  );
  serviceError.data.reason = 'sendgrid_api_error';
