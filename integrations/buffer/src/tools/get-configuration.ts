...
export let getConfigurationTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({}))
  .output(z.object({
    services: z.record(z.string(), z.any()).describe('Configuration for each supported social network service, including character limits, schedule limits, icons, and URLs')
  }))
