...
export let scheduleSms = SlateTool.create(
  spec,
  {
    name: 'Schedule SMS',
    key: 'schedule_sms',
...
Use the **action** field to choose between scheduling a new message, editing an existing schedule, or cancelling one. Cancelled scheduled messages have their credits refunded.`,
  ...
  .input(z.object({
    action: z.enum(['schedule', 'edit', 'cancel']).describe('Action to perform: "schedule" a new SMS, "edit" an existing schedule, or "cancel" a scheduled SMS.'),
    senderId: z.string().optional().describe('Sender ID (required when action is "schedule"). Falls back to configured default.'),
    mobileNumbers: z.string().optional().describe('Recipient mobile number(s), comma-separated (required when action is "schedule").'),
    message: z.string().optional().describe('SMS message text (required when action is "schedule").'),
    scheduleTime: z.string().optional().describe('Scheduled delivery time in YYYY-MM-DD HH:MM:SS format (required for "schedule" and "edit").'),
    scheduleId: z.string().optional().describe('Schedule ID of an existing scheduled SMS (required for "edit" and "cancel").'),
    route: z.string().optional().describe('SMS route (e.g., "transactional", "promotional"). Only used when action is "schedule".'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'schedule') {
      ...
      if (!ctx.input.mobileNumbers) throw new Error('Mobile numbers are required for scheduling.');
      if (!ctx.input.message) throw new Error('Message text is required for scheduling.');
      if (!ctx.input.scheduleTime) throw new Error('Schedule time is required for scheduling.');
    ...
    if (ctx.input.action === 'edit') {
      ...
      if (!ctx.input.scheduleTime) throw new Error('New schedule time is required for editing.');
