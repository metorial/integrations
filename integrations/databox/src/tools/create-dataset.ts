...
export let createDataset = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum 100 columns per dataset.',
      'Maximum 200MB total dataset size.',
    ],
  }
)
  .input(z.object({
    ...
    primaryKeys: z.array(z.string()).optional().describe('Column names used to uniquely identify rows. Omit if no deduplication is needed'),
  }))
  .output(z.object({
