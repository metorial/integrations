...
export let addMemberTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    role: z.enum(['COHOST']).optional().describe('Role to assign. COHOST gives the same abilities as the meeting organizer.')
  }))
  .output(z.object({
...
export let removeMemberTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    memberName: z.string().describe('Full resource name of the member to remove (e.g., "spaces/abc123/members/def456")')
  }))
  .output(z.object({
