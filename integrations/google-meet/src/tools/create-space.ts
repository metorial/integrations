...
let autoGenerationType = z.enum(['ON', 'OFF']).optional().describe('Whether to enable auto-generation');

export let createSpaceTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    accessType: z.enum(['OPEN', 'TRUSTED', 'RESTRICTED']).optional()
      .describe('Who can join without knocking. OPEN = anyone with the link, TRUSTED = trusted users, RESTRICTED = only invited users.'),
    entryPointAccess: z.enum(['ALL', 'CREATOR_APP_ONLY']).optional()
      .describe('Which entry points can be used to join. ALL = any entry point, CREATOR_APP_ONLY = only from the app that created the space.'),
    moderation: z.enum(['ON', 'OFF']).optional()
      .describe('Whether moderation mode is enabled, giving the organizer control over co-host management and feature restrictions.'),
    moderationRestrictions: z.object({
      chatRestriction: z.enum(['HOSTS_ONLY', 'NO_RESTRICTION']).optional().describe('Who can send chat messages.'),
      reactionRestriction: z.enum(['HOSTS_ONLY', 'NO_RESTRICTION']).optional().describe('Who can send reactions.'),
      presentRestriction: z.enum(['HOSTS_ONLY', 'NO_RESTRICTION']).optional().describe('Who can present/share screen.'),
      defaultJoinAsViewerType: z.enum(['ON', 'OFF']).optional().describe('Whether participants join as viewers by default.')
    }).optional().describe('Feature restrictions when moderation is ON.'),
    autoRecording: autoGenerationType.describe('Automatically start recording when the conference begins.'),
    autoTranscription: autoGenerationType.describe('Automatically start transcription when the conference begins.'),
    autoSmartNotes: autoGenerationType.describe('Automatically generate smart notes for the conference.')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.autoRecording || ctx.input.autoTranscription || ctx.input.autoSmartNotes) {
      ...
      if (ctx.input.autoTranscription) {
        artifactConfig.transcriptionConfig = { autoTranscriptionGeneration: ctx.input.autoTranscription };
      }
      if (ctx.input.autoSmartNotes) {
        artifactConfig.smartNotesConfig = { autoSmartNotesGeneration: ctx.input.autoSmartNotes };
      }
    }
