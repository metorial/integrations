...
let suppressionEntrySchema = z.object({
  ...
  cause: z.string().describe('Suppression cause (unsubscribed, temporary_unavailable, permanent_unavailable, complained, blocked)'),
...
export let addSuppression = SlateTool.create(
  ...
  .input(z.object({
    ...
    cause: z.enum(['unsubscribed', 'temporary_unavailable', 'permanent_unavailable', 'complained', 'blocked']).describe('Reason for suppression'),
...
export let getSuppression = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    suppressions: z.array(suppressionEntrySchema).describe('Suppression records for the email address'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({
      token: ctx.auth.token,
...
export let listSuppressions = SlateTool.create(
  ...
  .input(z.object({
    cause: z.enum(['unsubscribed', 'temporary_unavailable', 'permanent_unavailable', 'complained', 'blocked']).optional().describe('Filter by suppression cause'),
    source: z.enum(['user', 'system', 'subscriber']).optional().describe('Filter by suppression source'),
    startTime: z.string().optional().describe('Filter by creation date (ISO 8601, return records created after this time)'),
...
export let deleteSuppression = SlateTool.create(
  spec,
  {
