...
export let createEventDump = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.filterDeliveryStatus) filter['delivery_status'] = ctx.input.filterDeliveryStatus;
...
export let getEventDump = SlateTool.create(
  spec,
  {
    name: 'Get Event Dump',
    key: 'get_event_dump',
  ...
  .output(z.object({
    ...
    downloadUrl: z.string().optional().describe('URL to download the dump file (available when ready)'),
...
export let listEventDumps = SlateTool.create(
  spec,
  {
...
export let deleteEventDump = SlateTool.create(
  spec,
  {
