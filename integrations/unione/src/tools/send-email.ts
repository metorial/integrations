...
let recipientSchema = z.object({
  ...
  substitutions: z.record(z.string(), z.string()).optional().describe('Per-recipient substitutions/merge tags'),
  metadata: z.record(z.string(), z.string()).optional().describe('Per-recipient metadata (max 10 key-value pairs)'),
});
...
Can reference a stored template by ID, include attachments, schedule delivery up to 24 hours ahead, and track opens/clicks.`,
  ...
  .input(z.object({
    ...
    templateEngine: z.enum(['simple', 'velocity', 'liquid', 'none']).optional().describe('Template engine for substitutions'),
    subject: z.string().optional().describe('Email subject line (required if no template)'),
    fromEmail: z.string().optional().describe('Sender email address (required if no template)'),
    ...
    globalSubstitutions: z.record(z.string(), z.string()).optional().describe('Substitutions applied to all recipients'),
    globalMetadata: z.record(z.string(), z.string()).optional().describe('Metadata attached to the email (max 10 key-value pairs)'),
    tags: z.array(z.string()).optional().describe('Tags for categorizing the email (max 4)'),
    headers: z.record(z.string(), z.string()).optional().describe('Custom email headers (most require "X-" prefix)'),
    attachments: z.array(attachmentSchema).optional().describe('File attachments'),
    inlineAttachments: z.array(attachmentSchema).optional().describe('Inline image attachments referenced via content IDs'),
    ...
    sendAt: z.string().optional().describe('Schedule delivery time (ISO 8601 format, up to 24 hours ahead)'),
    idempotenceKey: z.string().optional().describe('Key to prevent duplicate sends within a one-minute window'),
    skipUnsubscribe: z.boolean().optional().describe('Skip appending unsubscribe link (requires support approval)'),
  ...
  .output(z.object({
    ...
    acceptedEmails: z.array(z.string()).describe('List of accepted recipient email addresses'),
    failedEmails: z.record(z.string(), z.string()).optional().describe('Map of rejected emails to rejection reasons'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.globalSubstitutions) message['global_substitutions'] = ctx.input.globalSubstitutions;
    ...
    if (ctx.input.skipUnsubscribe !== undefined) message['skip_unsubscribe'] = ctx.input.skipUnsubscribe ? 1 : 0;
    if (ctx.input.bypassGlobal !== undefined) message['bypass_global'] = ctx.input.bypassGlobal ? 1 : 0;
    if (ctx.input.bypassUnavailable !== undefined) message['bypass_unavailable'] = ctx.input.bypassUnavailable ? 1 : 0;
    if (ctx.input.bypassUnsubscribed !== undefined) message['bypass_unsubscribed'] = ctx.input.bypassUnsubscribed ? 1 : 0;
    if (ctx.input.bypassComplained !== undefined) message['bypass_complained'] = ctx.input.bypassComplained ? 1 : 0;
