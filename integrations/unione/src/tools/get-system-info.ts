...
export let getSystemInfo = SlateTool.create(
  ...
  .output(z.object({
    ...
    accountingPeriodStart: z.string().optional().describe('Start of current accounting period'),
    accountingPeriodEnd: z.string().optional().describe('End of current accounting period'),
    emailsIncluded: z.number().optional().describe('Number of emails included in the current plan'),
    emailsSent: z.number().optional().describe('Number of emails sent in the current period'),
