...
export let createCheckin = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    isActive: z.boolean().optional().describe('Whether the check-in should be active immediately'),
    isAnonymous: z.boolean().optional().describe('Whether responses should be anonymous'),
    frequency: z.string().optional().describe('Scheduling frequency (e.g., "every_day", "weekdays", "weekly")'),
    daysOfWeek: z.array(z.number()).optional().describe('Days of the week for the check-in (0=Sunday, 6=Saturday)'),
    triggerTime: z.string().optional().describe('Time to trigger the check-in (HH:MM format)'),
    isTriggerBased: z.boolean().optional().describe('If true, the check-in is manual-only (no automatic scheduling)'),
  }))
  .output(z.object({
