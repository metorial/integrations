...
export let getCheckin = SlateTool.create(
  spec,
  {
    name: 'Get Check-in',
    key: 'get_checkin',
  ...
  .input(z.object({
    ...
    includeSummary: z.boolean().optional().describe('Whether to include check-in summary data'),
    date: z.string().optional().describe('Date for the summary calculation (YYYY-MM-DD). Only used when includeSummary is true'),
  }))
  .output(z.object({
