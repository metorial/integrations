...
export let sendMessage = SlateTool.create(
  ...
  .input(z.object({
    ...
    targetUserUuids: z.array(z.string()).optional().describe('UUIDs of users to receive the message'),
    targetTeamUuids: z.array(z.string()).optional().describe('UUIDs of teams whose members will receive the message'),
    targetChannels: z.array(z.object({
      channelId: z.string().describe('ID of the channel on the connected chat platform'),
    })).optional().describe('Channels to send the message to'),
    ...
    })).optional().describe('Interactive buttons to include in the message'),
  }))
  .output(z.object({
