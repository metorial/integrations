...
export let searchCrm = SlateTool.create(
  spec,
  {
    name: 'Search CRM',
    key: 'search_crm',
  ...
  .input(z.object({
    objectType: z.string().describe('CRM object type to search (e.g., contacts, companies, deals, tickets)'),
    query: z.string().optional().describe('Full-text search query'),
    filterGroups: z.array(filterGroupSchema).optional().describe('Filter groups (OR logic between groups, AND logic within)'),
    ...
    limit: z.number().min(1).max(200).optional().describe('Number of results to return (max 200)'),
