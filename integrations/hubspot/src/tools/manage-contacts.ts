...
let contactPropertySchema = z.record(z.string(), z.any()).describe('Contact properties as key-value pairs (e.g., email, firstname, lastname, phone, company, website, lifecyclestage)');
...
export let createContact = SlateTool.create(
  ...
  .input(z.object({
    ...
    })).optional().describe('Associations to create with the contact'),
  }))
  .output(contactOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new HubSpotClient(ctx.auth.token);
    let result = await client.createObject('contacts', ctx.input.properties, ctx.input.associations);
...
export let getContact = SlateTool.create(
  ...
  .input(z.object({
    ...
    associations: z.array(z.string()).optional().describe('Associated object types to include (e.g., companies, deals)'),
  }))
  .output(contactOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new HubSpotClient(ctx.auth.token);
    let result = await client.getObject('contacts', ctx.input.contactId, ctx.input.properties, ctx.input.associations);
...
export let updateContact = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    let client = new HubSpotClient(ctx.auth.token);
    let result = await client.updateObject('contacts', ctx.input.contactId, ctx.input.properties);
...
export let listContacts = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    let client = new HubSpotClient(ctx.auth.token);
    let result = await client.listObjects('contacts', ctx.input.limit || 10, ctx.input.after, ctx.input.properties);
