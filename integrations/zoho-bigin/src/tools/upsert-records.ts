...
export let upsertRecords = SlateTool.create(
  spec,
  {
    name: 'Upsert Records',
    key: 'upsert_records',
  ...
  .output(z.object({
    results: z.array(z.object({
      ...
      action: z.string().optional().describe('Whether the record was "insert" or "update"'),
      duplicateField: z.string().optional().describe('The field that matched the duplicate'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.upsertRecords(ctx.input.module, ctx.input.records, ctx.input.duplicateCheckFields);
