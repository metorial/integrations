...
export let listNotes = SlateTool.create(
  ...
  .input(z.object({
    ...
    recordId: z.string()
      .describe('ID of the record to retrieve notes for'),
    page: z.number().optional()
      .describe('Page number (default 1)'),
    perPage: z.number().optional()
      .describe('Notes per page (default 200, max 200)'),
  }))
  .output(z.object({
    notes: z.array(z.record(z.string(), z.any()))
      .describe('Array of note objects'),
    moreRecords: z.boolean().optional()
      .describe('Whether more notes are available'),
  }))
...
export let createNote = SlateTool.create(
  ...
  .input(z.object({
    module: z.enum(['Contacts', 'Accounts', 'Pipelines', 'Products', 'Tasks', 'Events', 'Calls'])
      .describe('Module API name of the parent record'),
    recordId: z.string()
      .describe('ID of the record to add the note to'),
  ...
  .output(z.object({
    noteId: z.string().optional()
      .describe('ID of the created note'),
    status: z.string()
