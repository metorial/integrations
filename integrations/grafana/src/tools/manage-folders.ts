...
export let createFolder = SlateTool.create(
  ...
  .input(z.object({
    ...
    folderUid: z.string().optional().describe('Custom UID for the folder. Auto-generated if not provided.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.createFolder(ctx.input.title, ctx.input.folderUid, ctx.input.parentUid);
...
export let updateFolder = SlateTool.create(
  ...
  .input(z.object({
    ...
    version: z.number().optional().describe('Current version of the folder for optimistic concurrency control'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.updateFolder(ctx.input.folderUid, ctx.input.title, ctx.input.version);
...
export let deleteFolder = SlateTool.create(
  ...
  .input(z.object({
    ...
    forceDeleteRules: z.boolean().optional().describe('Set to true to also delete alert rules in the folder'),
  }))
  .output(z.object({
