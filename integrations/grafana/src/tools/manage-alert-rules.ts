...
export let listAlertRules = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    alertRules: z.array(z.object({
      ...
      forDuration: z.string().optional().describe('Duration the condition must be true before firing'),
      ...
      labels: z.record(z.string(), z.string()).optional().describe('Labels attached to this rule'),
      annotations: z.record(z.string(), z.string()).optional().describe('Annotations for the rule'),
    })),
  }))
...
export let getAlertRule = SlateTool.create(
  spec,
  {
    name: 'Get Alert Rule',
    key: 'get_alert_rule',
...
export let createAlertRule = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    ruleGroup: z.string().describe('Rule group name. Rules in the same group are evaluated together.'),
    condition: z.string().describe('Reference (refId) of the query or expression that defines the alert condition'),
    queries: z.array(z.any()).describe('Array of alert query objects defining data source queries and expressions'),
    forDuration: z.string().optional().describe('Duration condition must be true before firing (e.g. "5m", "1h")'),
    noDataState: z.enum(['NoData', 'Alerting', 'OK']).optional().describe('Behavior when no data is returned'),
    execErrState: z.enum(['Error', 'Alerting', 'OK']).optional().describe('Behavior when an evaluation error occurs'),
    labels: z.record(z.string(), z.string()).optional().describe('Labels to attach to the alert (used for routing notifications)'),
    annotations: z.record(z.string(), z.string()).optional().describe('Annotations for the alert (summary, description, runbook_url, etc.)'),
...
export let updateAlertRule = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    noDataState: z.enum(['NoData', 'Alerting', 'OK']).optional().describe('No-data behavior'),
...
export let deleteAlertRule = SlateTool.create(
  spec,
  {
