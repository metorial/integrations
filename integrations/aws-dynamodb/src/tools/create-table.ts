...
let projectionSchema = z.object({
  ...
  nonKeyAttributes: z.array(z.string()).optional().describe('Non-key attributes to project (only for INCLUDE)'),
});
...
let gsiSchema = z.object({
  ...
  }).optional().describe('Required if table uses PROVISIONED billing'),
});
...
Supports configuring billing mode (on-demand or provisioned), table class, DynamoDB Streams, and tags.`,
  ...
  .input(z.object({
    ...
    keySchema: z.array(keySchemaSchema).min(1).max(2).describe('Primary key schema (1 HASH key, optionally 1 RANGE key)'),
    attributeDefinitions: z.array(attributeDefinitionSchema).min(1).describe('Attribute definitions for key schema attributes'),
    billingMode: z.enum(['PROVISIONED', 'PAY_PER_REQUEST']).optional().default('PAY_PER_REQUEST').describe('Billing mode for the table'),
    provisionedThroughput: z.object({
      readCapacityUnits: z.number().describe('Read capacity units'),
      writeCapacityUnits: z.number().describe('Write capacity units'),
    }).optional().describe('Required when billingMode is PROVISIONED'),
    globalSecondaryIndexes: z.array(gsiSchema).optional().describe('Global secondary indexes'),
    localSecondaryIndexes: z.array(lsiSchema).optional().describe('Local secondary indexes (can only be created at table creation time)'),
    tableClass: z.enum(['STANDARD', 'STANDARD_INFREQUENT_ACCESS']).optional().describe('Table class'),
    enableStreams: z.boolean().optional().describe('Enable DynamoDB Streams'),
    streamViewType: z.enum(['KEYS_ONLY', 'NEW_IMAGE', 'OLD_IMAGE', 'NEW_AND_OLD_IMAGES']).optional().describe('What data to include in stream records'),
    tags: z.array(z.object({
      key: z.string(),
      value: z.string(),
    })).optional().describe('Tags to assign to the table'),
  }))
  .output(z.object({
