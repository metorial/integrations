...
export let geocodeLocation = SlateTool.create(
  ...
  .input(z.object({
    locationName: z.string().optional().describe('City name, optionally with state code and country code separated by commas (e.g. "London,GB")'),
    zipCode: z.string().optional().describe('ZIP/postal code with country code (e.g. "10001,US", "E14,GB")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.zipCode) {
      ...
      let data = await client.geocodeReverse(ctx.input.latitude, ctx.input.longitude, ctx.input.limit);
      results = (data || []).map((r: any) => ({
        name: r.name,
        latitude: r.lat,
        longitude: r.lon,
      ...
      throw new Error('Provide either locationName, zipCode, or latitude+longitude for geocoding.');
    }

    let output = { results };

    let summary = results.map(r => `${r.name}${r.state ? ', ' + r.state : ''}, ${r.country} (${r.latitude}, ${r.longitude})`).join('; ');
