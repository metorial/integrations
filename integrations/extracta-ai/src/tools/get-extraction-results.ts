...
export let getExtractionResultsTool = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    status: z.string().optional().describe('Processing status (e.g., "waiting" if still processing)'),
    ...
    })).optional().describe('Processed files with their extraction results'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let files = result.files?.map((f: { fileName: string; fileId?: string; status: string; result?: Record<string, unknown>; url?: string }) => ({
      fileName: f.fileName,
      fileId: f.fileId,
      status: f.status,
