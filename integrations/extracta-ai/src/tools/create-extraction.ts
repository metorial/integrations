...
let extractionFieldSchema = z.object({
  ...
  type: z.enum(['string', 'object', 'array']).optional().describe('Field type: string, object, or array'),
});

let extractionOptionsSchema = z.object({
  hasTable: z.boolean().optional().describe('Enable table analysis and extraction'),
  hasVisuals: z.boolean().optional().describe('Enable detection of charts, graphs, and diagrams'),
  handwrittenTextRecognition: z.boolean().optional().describe('Enable OCR for handwritten text'),
  checkboxRecognition: z.boolean().optional().describe('Detect checkbox states (checked/unchecked)'),
  longDocument: z.boolean().optional().describe('Optimized processing for large or complex documents'),
  splitPdfPages: z.boolean().optional().describe('Treat each PDF page as a separate extraction unit'),
  specificPageProcessing: z.boolean().optional().describe('Process only a specified range of pages'),
  from: z.number().optional().describe('Start page number (when specificPageProcessing is enabled)'),
  to: z.number().optional().describe('End page number (when specificPageProcessing is enabled)'),
}).optional().describe('Processing options for the extraction');

export let createExtractionTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    language: z.string().describe('Document language for accurate extraction (e.g., "English", "Spanish")'),
