...
export let viewClassificationTool = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    documentTypes: z.array(z.object({
      ...
      uniqueWords: z.array(z.string()).optional().describe('Keywords for identification'),
      extractionId: z.string().optional().describe('Linked extraction template ID'),
    })).optional().describe('Configured document types'),
  }))
