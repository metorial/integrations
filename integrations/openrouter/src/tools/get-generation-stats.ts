...
export let getGenerationStats = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'The generation ID is returned as part of the chat completion response.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    generationId: z.string().describe('The generation ID returned from a chat completion request'),
  }))
  .output(z.object({
    ...
    nativeTokensPrompt: z.number().optional().describe('Native prompt token count from the provider'),
    nativeTokensCompletion: z.number().optional().describe('Native completion token count from the provider'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let output = {
      ...
      promptTokens: (data.tokens_prompt as number) || (data.prompt_tokens as number) || undefined,
      completionTokens: (data.tokens_completion as number) || (data.completion_tokens as number) || undefined,
