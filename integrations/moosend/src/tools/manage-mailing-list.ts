...
export let manageMailingList = SlateTool.create(spec, {
  ...
  .input(z.object({
    ...
    mailingListId: z.string().optional().describe('Mailing list ID (required for update, delete, get)'),
    name: z.string().optional().describe('Mailing list name (required for create, optional for update)'),
    confirmationPage: z.string().optional().describe('URL displayed after subscription'),
    redirectAfterUnsubscribePage: z.string().optional().describe('URL to redirect after unsubscribe'),
    withStatistics: z.boolean().optional().default(false).describe('Include subscriber statistics in results (for get/list)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (action) {
      case 'create': {
        ...
        if (ctx.input.redirectAfterUnsubscribePage) body.RedirectAfterUnsubscribePage = ctx.input.redirectAfterUnsubscribePage;
      ...
      case 'update': {
        if (!ctx.input.mailingListId) throw new Error('mailingListId is required for updating a mailing list');
      ...
      case 'delete': {
        if (!ctx.input.mailingListId) throw new Error('mailingListId is required for deleting a mailing list');
      ...
      case 'get': {
        if (!ctx.input.mailingListId) throw new Error('mailingListId is required for getting mailing list details');
        let result = await client.getMailingList(ctx.input.mailingListId, ctx.input.withStatistics);
        return {
          output: {
            mailingLists: [mapList(result)],
            action,
      ...
      case 'list': {
        let result = await client.getMailingLists(ctx.input.page, ctx.input.pageSize, ctx.input.withStatistics);
