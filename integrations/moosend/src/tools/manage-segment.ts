...
let criteriaSchema = z.object({
  field: z.string().describe('Field to filter on (e.g. "DateAdded", "CampaignOpens", or a custom field name)'),
  comparer: z.enum(['Is', 'IsNot', 'Contains', 'DoesNotContain', 'StartsWith', 'EndsWith', 'IsGreaterThan', 'IsLessThan', 'IsBefore', 'IsAfter']).optional().describe('Comparison operator'),
...
let segmentOutputSchema = z.object({
  ...
  })).optional().describe('Segment criteria/rules'),
});
...
export let manageSegment = SlateTool.create(spec, {
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'delete', 'get', 'list', 'add_criteria', 'update_criteria', 'get_subscribers']).describe('Action to perform'),
    mailingListId: z.string().describe('ID of the mailing list'),
    segmentId: z.string().optional().describe('Segment ID (required for update, delete, get, add_criteria, update_criteria, get_subscribers)'),
    name: z.string().optional().describe('Segment name (required for create)'),
    matchType: z.enum(['All', 'Any']).optional().describe('How criteria match together: "All" (AND) or "Any" (OR)'),
    ...
    pageSize: z.number().optional().default(100).describe('Items per page for get_subscribers'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Subscribers matching segment criteria (for get_subscribers)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (action) {
      case 'create': {
        ...
        let result = await client.createSegment(mailingListId, ctx.input.name, ctx.input.matchType);
        return {
          output: {
      ...
      case 'update': {
        if (!ctx.input.segmentId) throw new Error('segmentId is required for updating a segment');
        if (!ctx.input.name) throw new Error('name is required for updating a segment');
        let result = await client.updateSegment(mailingListId, ctx.input.segmentId, ctx.input.name, ctx.input.matchType);
        return {
          output: {
            segments: [mapSegment(result)],
            action,
      ...
      case 'delete': {
        if (!ctx.input.segmentId) throw new Error('segmentId is required for deleting a segment');
      ...
      case 'get': {
        if (!ctx.input.segmentId) throw new Error('segmentId is required for getting segment details');
      ...
      case 'list': {
        ...
        let segmentsList = (Array.isArray(result) ? result : (result?.Segments as Record<string, unknown>[]) ?? []) as Record<string, unknown>[];
        return {
          output: {
      ...
      case 'update_criteria': {
        if (!ctx.input.segmentId) throw new Error('segmentId is required for updating criteria');
        if (!ctx.input.criteriaId) throw new Error('criteriaId is required for update_criteria');
        ...
        if (ctx.input.criteria.dateFrom) body.DateFrom = ctx.input.criteria.dateFrom;
        if (ctx.input.criteria.dateTo) body.DateTo = ctx.input.criteria.dateTo;
        await client.updateSegmentCriteria(mailingListId, ctx.input.segmentId, ctx.input.criteriaId, body);
        return {
          output: {
            action,
            success: true,
      ...
      case 'get_subscribers': {
        if (!ctx.input.segmentId) throw new Error('segmentId is required for getting segment subscribers');
        let result = await client.getSegmentSubscribers(mailingListId, ctx.input.segmentId, ctx.input.page, ctx.input.pageSize);
...
let mapSegment = (s: Record<string, unknown>) => ({
  ...
  criteria: Array.isArray(s?.Criteria) ? (s.Criteria as Record<string, unknown>[]).map(c => ({
