...
let personalizationSchema = z.object({
  ...
  substitutions: z.record(z.string(), z.string()).optional().describe('Key-value pairs for template variable substitution'),
});
...
export let sendTransactionalEmail = SlateTool.create(spec, {
  ...
  .input(z.object({
    ...
    templateId: z.string().optional().describe('ID of an existing transactional campaign template to use'),
    templateName: z.string().optional().describe('Template name to match an existing campaign or create a new one'),
    ...
    personalizations: z.array(personalizationSchema).min(1).describe('Array of personalizations, each with recipients and optional variable substitutions'),
    attachments: z.array(attachmentSchema).optional().describe('File attachments (base64-encoded)'),
    bypassUnsubscribeManagement: z.boolean().optional().default(false).describe('Bypass unsubscribe management for this send'),
  }))
  .output(z.object({
    success: z.boolean().describe('Whether the emails were sent successfully'),
    recipientCount: z.number().describe('Total number of recipients across all personalizations'),
  }))
