...
export let updateOpportunity = SlateTool.create(
  ...
  .input(z.object({
    ...
    expectedCloseOn: z.string().optional().describe('Updated expected close date (YYYY-MM-DD)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.expectedCloseOn !== undefined) opportunity.expectedCloseOn = ctx.input.expectedCloseOn;
    ...
    if (ctx.input.valueAmount !== undefined || ctx.input.valueCurrency) {
      opportunity.value = {};
      if (ctx.input.valueAmount !== undefined) opportunity.value.amount = ctx.input.valueAmount;
      if (ctx.input.valueCurrency) opportunity.value.currency = ctx.input.valueCurrency;
    }

    if (ctx.input.tags) {
      opportunity.tags = ctx.input.tags.map(t => t.tagId ? { id: t.tagId } : { name: t.name });
    }
