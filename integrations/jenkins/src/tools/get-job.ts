...
export let getJob = SlateTool.create(
  ...
  .output(z.object({
    ...
    inQueue: z.boolean().optional().describe('Whether the job is currently in the build queue'),
    ...
    lastSuccessfulBuildNumber: z.number().optional().nullable().describe('Last successful build number'),
    lastFailedBuildNumber: z.number().optional().nullable().describe('Last failed build number'),
    healthScore: z.number().optional().nullable().describe('Health score (0-100)'),
    healthDescription: z.string().optional().nullable().describe('Health report description'),
