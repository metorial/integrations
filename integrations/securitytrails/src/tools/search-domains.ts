...
export let searchDomains = SlateTool.create(
  spec,
  {
    name: 'Search Domains',
    key: 'search_domains',
...
Use the **query** parameter for DSL syntax like \`whois_email = 'admin@example.com'\`, or use **filter** for simple key-value matching. Optionally retrieve statistics instead of records by setting **statsOnly**.`,
  ...
  .input(z.object({
    query: z.string().optional().describe('DSL query string (e.g., "whois_email = \'admin@example.com\' AND tld = \'com\'")'),
    filter: z.record(z.string(), z.any()).optional().describe('Simple filter object with field-value pairs (e.g., {"ipv4": "1.2.3.4", "tld": "com"})'),
    includeIps: z.boolean().optional().describe('Resolve A records and include IP addresses in results'),
    page: z.number().optional().describe('Page number for pagination'),
    statsOnly: z.boolean().optional().describe('Return statistics (TLD count, hostname count, domain count) instead of records')
  }))
  .output(z.object({
    ...
    }).passthrough().optional().describe('Pagination metadata'),
