...
export let getIpDetails = SlateTool.create(
  spec,
  {
    name: 'Get IP Details',
    key: 'get_ip_details',
  ...
  .input(z.object({
    ...
    includeNeighbors: z.boolean().optional().default(false).describe('Include neighboring IP addresses'),
    includeUserAgents: z.boolean().optional().default(false).describe('Include user agents observed for this IP in the last 30 days')
  }))
  .output(z.object({
    ...
    userAgents: z.any().optional().describe('User agents seen for this IP in the last 30 days')
  }).passthrough())
