...
export let querySuiteQL = SlateTool.create(
  spec,
  {
    name: 'Query SuiteQL',
    key: 'query_suiteql',
...
Use this for complex data retrieval, reporting, cross-record queries, and filtering that goes beyond the standard REST API capabilities.`,
  ...
  .input(z.object({
    query: z.string().describe('SuiteQL query string (SQL-92 syntax). Example: "SELECT id, companyName FROM customer WHERE dateCreated > \'2024-01-01\'"'),
    limit: z.number().optional().describe('Maximum number of results to return per page (default varies by NetSuite)'),
