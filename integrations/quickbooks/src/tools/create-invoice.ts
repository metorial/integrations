...
export let createInvoice = SlateTool.create(
  ...
  .input(z.object({
    ...
    txnDate: z.string().optional().describe('Transaction date (YYYY-MM-DD), defaults to today'),
    customerMemo: z.string().optional().describe('Memo displayed to the customer on the invoice'),
    privateNote: z.string().optional().describe('Private note not visible to the customer'),
    sendEmail: z.boolean().optional().describe('If true, sends the invoice via email after creation'),
    emailAddress: z.string().optional().describe('Email address to send the invoice to (overrides customer default)'),
  }))
  .output(z.object({
