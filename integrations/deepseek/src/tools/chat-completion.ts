...
let messageSchema = z.discriminatedUnion('role', [
  ...
  z.object({
    ...
    reasoningContent: z.string().optional().describe('Previous reasoning content from the assistant (for multi-turn reasoning)'),
    ...
    })).optional().describe('Tool calls made by the assistant'),
  }),
  z.object({
    role: z.literal('tool'),
...
Enables function calling with tool definitions, structured JSON output via response format, and thinking mode for step-by-step reasoning.`,
  ...
  .input(z.object({
    model: z.enum(['deepseek-chat', 'deepseek-reasoner']).default('deepseek-chat').describe('Model to use for the completion'),
    messages: z.array(messageSchema).min(1).describe('Conversation messages'),
    temperature: z.number().min(0).max(2).optional().describe('Sampling temperature (0-2). Higher values produce more random output.'),
    ...
    frequencyPenalty: z.number().min(-2).max(2).optional().describe('Penalizes repeated tokens (-2 to 2)'),
    presencePenalty: z.number().min(-2).max(2).optional().describe('Encourages new topics (-2 to 2)'),
    stop: z.union([z.string(), z.array(z.string())]).optional().describe('Stop sequences that halt generation'),
    responseFormat: z.enum(['text', 'json_object']).optional().describe('Output format. Use json_object for structured JSON responses.'),
    tools: z.array(toolDefinitionSchema).optional().describe('Available tool/function definitions the model may call'),
    toolChoice: z.union([
      z.enum(['none', 'auto', 'required']),
      z.object({
        functionName: z.string().describe('Force the model to call this specific function'),
      }),
    ]).optional().describe('Controls tool invocation behavior'),
    enableThinking: z.boolean().optional().describe('Enable thinking/reasoning mode on deepseek-chat. Automatically enabled for deepseek-reasoner.'),
    thinkingBudgetTokens: z.number().int().positive().optional().describe('Maximum tokens for the thinking/reasoning process'),
    logprobs: z.boolean().optional().describe('Whether to return log probabilities of output tokens'),
    topLogprobs: z.number().int().min(0).max(20).optional().describe('Number of most likely tokens to return per position (0-20)'),
  }))
  .output(z.object({
    ...
    reasoningContent: z.string().nullable().optional().describe('Step-by-step reasoning content (when using thinking mode)'),
    finishReason: z.string().describe('Reason generation stopped: stop, length, content_filter, tool_calls, or insufficient_system_resource'),
    toolCalls: z.array(z.object({
    ...
    cacheHitTokens: z.number().optional().describe('Number of prompt tokens served from cache'),
    cacheMissTokens: z.number().optional().describe('Number of prompt tokens not found in cache'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (output.content) {
      let preview = output.content.length > 200 ? output.content.substring(0, 200) + '...' : output.content;
      messageParts.push(`**Response:** ${preview}`);
    }
    if (output.toolCalls && output.toolCalls.length > 0) {
      messageParts.push(`**Tool calls:** ${output.toolCalls.map((tc) => `\`${tc.functionName}\``).join(', ')}`);
    }
    if (output.reasoningContent) {
      messageParts.push(`**Reasoning:** included (${output.reasoningTokens ?? 'unknown'} tokens)`);
    }
    messageParts.push(`**Usage:** ${output.totalTokens} total tokens (${output.promptTokens} prompt, ${output.completionTokens} completion)`);
