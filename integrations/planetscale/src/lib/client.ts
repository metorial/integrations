...
export class Client {
  ...
  async listDatabases(pagination?: PaginationParams, query?: string): Promise<PaginatedResponse<any>> {
  ...
  async updateDatabase(database: string, data: {
    ...
    if (data.requireApprovalForDeploy !== undefined) body.require_approval_for_deploy = data.requireApprovalForDeploy;
    if (data.foreignKeysEnabled !== undefined) body.foreign_keys_enabled = data.foreignKeysEnabled;
    if (data.allowDataBranching !== undefined) body.allow_data_branching = data.allowDataBranching;
  ...
  async listBranches(database: string, pagination?: PaginationParams): Promise<PaginatedResponse<any>> {
  ...
  async createBranch(database: string, data: {
    name: string;
    parentBranch?: string;
    backupId?: string;
    region?: string;
    ...
    let response = await api.post(this.dbPath(database, '/branches'), body, { headers: this.headers });
    return response.data;
  }
  ...
  async promoteBranch(database: string, branch: string): Promise<any> {
    let response = await api.post(this.branchPath(database, branch, '/promote'), {}, { headers: this.headers });
    return response.data;
  }

  async demoteBranch(database: string, branch: string): Promise<any> {
    let response = await api.post(this.branchPath(database, branch, '/demote'), {}, { headers: this.headers });
    return response.data;
  }

  async enableSafeMigrations(database: string, branch: string): Promise<any> {
    let response = await api.post(this.branchPath(database, branch, '/enable-safe-migrations'), {}, { headers: this.headers });
    return response.data;
  }

  async disableSafeMigrations(database: string, branch: string): Promise<any> {
    let response = await api.post(this.branchPath(database, branch, '/disable-safe-migrations'), {}, { headers: this.headers });
    return response.data;
  }

  async getBranchSchema(database: string, branch: string): Promise<any> {
    let response = await api.get(this.branchPath(database, branch, '/schema'), { headers: this.headers });
    return response.data;
  }

  async lintBranchSchema(database: string, branch: string): Promise<any> {
    let response = await api.post(this.branchPath(database, branch, '/schema/lint'), {}, { headers: this.headers });
    return response.data;
  }

  // ---- Deploy Requests ----

  async listDeployRequests(database: string, pagination?: PaginationParams, state?: string): Promise<PaginatedResponse<any>> {
    let params: Record<string, any> = { ...this.paginationParams(pagination) };
  ...
  async getDeployRequest(database: string, number: number): Promise<any> {
    let response = await api.get(this.dbPath(database, `/deploy-requests/${number}`), { headers: this.headers });
    return response.data;
  }

  async createDeployRequest(database: string, data: {
    branch: string;
    intoBranch: string;
    notes?: string;
    autoCutover?: boolean;
    ...
    let response = await api.post(this.dbPath(database, '/deploy-requests'), body, { headers: this.headers });
    return response.data;
  }

  async deployDeployRequest(database: string, number: number): Promise<any> {
    let response = await api.post(this.dbPath(database, `/deploy-requests/${number}/deploy`), {}, { headers: this.headers });
    return response.data;
  }

  async cancelDeployRequest(database: string, number: number): Promise<any> {
    let response = await api.post(this.dbPath(database, `/deploy-requests/${number}/cancel`), {}, { headers: this.headers });
    return response.data;
  }

  async closeDeployRequest(database: string, number: number): Promise<any> {
    let response = await api.post(this.dbPath(database, `/deploy-requests/${number}/close`), {}, { headers: this.headers });
    return response.data;
  }

  async skipRevertPeriod(database: string, number: number): Promise<any> {
    let response = await api.post(this.dbPath(database, `/deploy-requests/${number}/skip-revert-period`), {}, { headers: this.headers });
    return response.data;
  }

  async completeRevert(database: string, number: number): Promise<any> {
    let response = await api.post(this.dbPath(database, `/deploy-requests/${number}/complete-revert`), {}, { headers: this.headers });
    return response.data;
  }

  async reviewDeployRequest(database: string, number: number, data: {
    state: string;
    body?: string;
  }): Promise<any> {
    let response = await api.post(this.dbPath(database, `/deploy-requests/${number}/reviews`), {
      state: data.state,
      body: data.body,
    }, { headers: this.headers });
    return response.data;
  ...
  async getDeployment(database: string, number: number): Promise<any> {
    let response = await api.get(this.dbPath(database, `/deploy-requests/${number}/deployment`), { headers: this.headers });
    return response.data;
  }

  async listDeployOperations(database: string, number: number, pagination?: PaginationParams): Promise<PaginatedResponse<any>> {
    let response = await api.get(this.dbPath(database, `/deploy-requests/${number}/operations`), {
      headers: this.headers,
      params: this.paginationParams(pagination),
    });
    return this.mapPaginatedResponse(response.data);
  ...
  async listPasswords(database: string, branch: string, pagination?: PaginationParams): Promise<PaginatedResponse<any>> {
  ...
  async getPassword(database: string, branch: string, passwordId: string): Promise<any> {
    let response = await api.get(this.branchPath(database, branch, `/passwords/${passwordId}`), { headers: this.headers });
    return response.data;
  }

  async createPassword(database: string, branch: string, data: {
    name?: string;
    role: string;
    replica?: boolean;
    ttl?: number;
    ...
    let response = await api.post(this.branchPath(database, branch, '/passwords'), body, { headers: this.headers });
    return response.data;
  }

  async updatePassword(database: string, branch: string, passwordId: string, data: {
    name?: string;
    cidrs?: string[];
  }): Promise<any> {
    let body: Record<string, any> = {};
    ...
    let response = await api.patch(this.branchPath(database, branch, `/passwords/${passwordId}`), body, { headers: this.headers });
    return response.data;
  }

  async deletePassword(database: string, branch: string, passwordId: string): Promise<void> {
    await api.delete(this.branchPath(database, branch, `/passwords/${passwordId}`), { headers: this.headers });
  }

  async renewPassword(database: string, branch: string, passwordId: string): Promise<any> {
    let response = await api.post(this.branchPath(database, branch, `/passwords/${passwordId}/renew`), {}, { headers: this.headers });
    return response.data;
  }

  // ---- Backups ----

  async listBackups(database: string, branch: string, pagination?: PaginationParams): Promise<PaginatedResponse<any>> {
  ...
  async getBackup(database: string, branch: string, backupId: string): Promise<any> {
    let response = await api.get(this.branchPath(database, branch, `/backups/${backupId}`), { headers: this.headers });
    return response.data;
  }

  async createBackup(database: string, branch: string, data?: {
    name?: string;
    ...
    let response = await api.post(this.branchPath(database, branch, '/backups'), body, { headers: this.headers });
    return response.data;
  }

  async deleteBackup(database: string, branch: string, backupId: string): Promise<void> {
    await api.delete(this.branchPath(database, branch, `/backups/${backupId}`), { headers: this.headers });
  }

  // ---- Webhooks ----

  async listWebhooks(database: string, pagination?: PaginationParams): Promise<PaginatedResponse<any>> {
  ...
  async getWebhook(database: string, webhookId: string): Promise<any> {
    let response = await api.get(this.dbPath(database, `/webhooks/${webhookId}`), { headers: this.headers });
    return response.data;
  }

  async createWebhook(database: string, data: {
    url: string;
    enabled?: boolean;
    events?: string[];
  }): Promise<any> {
    let response = await api.post(this.dbPath(database, '/webhooks'), data, { headers: this.headers });
    return response.data;
  }

  async updateWebhook(database: string, webhookId: string, data: {
    url?: string;
    enabled?: boolean;
    events?: string[];
  }): Promise<any> {
    let response = await api.patch(this.dbPath(database, `/webhooks/${webhookId}`), data, { headers: this.headers });
    return response.data;
  }

  async deleteWebhook(database: string, webhookId: string): Promise<void> {
    await api.delete(this.dbPath(database, `/webhooks/${webhookId}`), { headers: this.headers });
  }

  async testWebhook(database: string, webhookId: string): Promise<any> {
    let response = await api.post(this.dbPath(database, `/webhooks/${webhookId}/test`), {}, { headers: this.headers });
    return response.data;
  }
  ...
  async getMember(memberId: string): Promise<any> {
    let response = await api.get(this.orgPath(`/members/${memberId}`), { headers: this.headers });
    return response.data;
  }
  ...
  async createServiceToken(data?: { name?: string; ttl?: number }): Promise<any> {
    ...
    let response = await api.post(this.orgPath('/service-tokens'), body, { headers: this.headers });
    return response.data;
  }

  async getServiceToken(tokenId: string): Promise<any> {
    let response = await api.get(this.orgPath(`/service-tokens/${tokenId}`), { headers: this.headers });
    return response.data;
  }
  ...
  async listAuditLogs(pagination?: PaginationParams, filters?: {
    action?: string;
    actorId?: string;
    since?: string;
    until?: string;
