...
export let createBranch = SlateTool.create(
  ...
  .input(z.object({
    ...
    parentBranch: z.string().optional().describe('Parent branch to create from (defaults to database default branch)'),
    ...
    restorePoint: z.string().optional().describe('Point-in-time restore timestamp (PostgreSQL only, ISO 8601)'),
    seedData: z.string().optional().describe('Seed data strategy, e.g. "last_successful_backup"'),
    clusterSize: z.string().optional().describe('Cluster size (required when restoring from backup)'),
  }))
  .output(z.object({
    branchId: z.string(),
    name: z.string(),
