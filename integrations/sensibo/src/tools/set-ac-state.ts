...
export let setAcStateTool = SlateTool.create(
  spec,
  {
    name: 'Set AC State',
    key: 'set_ac_state',
  ...
  .input(z.object({
    ...
    mode: z.enum(['cool', 'heat', 'fan', 'dry', 'auto']).optional().describe('AC operating mode'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let propertyKeys = Object.keys(stateProps).filter((k) => stateProps[k as keyof typeof stateProps] !== undefined);
    ...
    let changes = propertyKeys.map((k) => `${k}=${stateProps[k as keyof typeof stateProps]}`).join(', ');
