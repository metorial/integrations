...
export let listPayments = SlateTool.create(
  ...
  .input(z.object({
    limit: z.number().optional().describe('Number of payments to return (default: 20, max: 100)'),
    offset: z.number().optional().describe('Number of payments to skip (default: 0)'),
    status: z.enum(['pending', 'completed', 'failed']).optional().describe('Filter by payment status'),
    startDate: z.string().optional().describe('Filter payments from this date (ISO 8601 format, e.g., "2024-01-01")'),
    endDate: z.string().optional().describe('Filter payments up to this date (ISO 8601 format, e.g., "2024-12-31")'),
  }))
  .output(z.object({
