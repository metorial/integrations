...
export let listLipsyncs = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    inferenceId: z.string().optional().describe('If provided, returns details for this specific lip-sync inference'),
    title: z.string().optional().describe('Filter by title'),
    status: z.array(z.enum(['draft', 'processing', 'failed', 'suceeded', 'deleted'])).optional().describe('Filter by status'),
