...
export let createPhotoAvatar = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    baseImageUrl: z.string().describe('Public URL of the face image to create the avatar from'),
  ...
  .output(z.object({
    ...
    status: z.string().describe('Current status (draft, processing, published, failed, deleted)'),
