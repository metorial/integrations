...
let photoInferenceSchema = z.object({
  ...
  status: z.string().describe('Generation status (draft, processing, failed, succeeded, deleted)'),
...
export let getPhotoAvatarInferences = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    photoAvatarInferenceId: z.string().optional().describe('If provided, returns details for this specific inference'),
    photoAvatarId: z.string().optional().describe('Filter by photo avatar ID'),
    title: z.string().optional().describe('Filter by title'),
    status: z.array(z.enum(['draft', 'processing', 'failed', 'succeeded', 'deleted'])).optional().describe('Filter by status'),
    skip: z.number().optional().describe('Number of records to skip'),
    limit: z.number().optional().describe('Maximum number of records to return (default 10)'),
    startDatetime: z.string().optional().describe('Filter by creation date after this ISO 8601 datetime'),
    endDatetime: z.string().optional().describe('Filter by creation date before this ISO 8601 datetime'),
  }))
  .output(z.object({
