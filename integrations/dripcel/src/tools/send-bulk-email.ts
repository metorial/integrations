...
export let sendBulkEmail = SlateTool.create(
  ...
  .input(z.object({
    from: z.string().describe('Sender email address (must be a verified email identity in Dripcel)'),
    ...
    filterNonContacts: z.boolean().optional().describe('Filter out addresses that are not existing contacts. Required when using templates with custom fields.'),
    scheduledAt: z.string().optional().describe('Schedule delivery for a future time (ISO 8601 format). If omitted, emails are sent immediately.')
  }))
  .output(z.object({
