...
export let sendSms = SlateTool.create(
  ...
  .input(z.object({
    content: z.string().describe('SMS message content. May include template variables like {{firstname}} for existing contacts.'),
    cell: z.string().describe('Recipient cell number'),
    country: z.string().describe('Country code (e.g. "ZA", "US", "GB", "AU")'),
    deliveryMethod: z.enum(['reverse', 'standard', 'transactional']).describe('Delivery method for the SMS'),
    skipNonContacts: z.boolean().optional().default(true).describe('Skip sending if the recipient is not an existing contact'),
    campaignId: z.string().optional().describe('Campaign ID to associate this send with (enables reply hooks)'),
    testMode: z.boolean().optional().describe('Enable test mode via sendOptions')
  }))
  .output(z.object({
    customerId: z.string().describe('Unique identifier for this send, used for delivery tracking'),
