...
export let annotationsSchema = z.object({
  timezone: z.object({
  ...
  }).optional().describe('Timezone information for the location'),
  currency: z.object({
    name: z.string().optional(),
    isoCode: z.string().optional(),
    symbol: z.string().optional(),
  }).optional().describe('Local currency information'),
  ...
  }).optional().describe('Coordinates in degrees, minutes, seconds format'),
  ...
  mercator: z.object({
    x: z.number().optional(),
    y: z.number().optional(),
  }).optional().describe('Mercator projection coordinates (EPSG:3857)'),
  sun: z.object({
    rise: z.object({
  ...
  }).optional().describe('Sunrise and sunset times as unix timestamps'),
  roadinfo: z.object({
    driveOn: z.string().optional(),
    road: z.string().optional(),
    roadType: z.string().optional(),
    speedIn: z.string().optional(),
  }).optional().describe('Road and driving information'),
  osm: z.object({
    editUrl: z.string().optional(),
    noteUrl: z.string().optional(),
    url: z.string().optional(),
  }).optional().describe('OpenStreetMap links'),
}).optional();
...
export let geocodeResultSchema = z.object({
  ...
  components: z.record(z.string(), z.union([z.string(), z.number()])).describe('Address components (country, city, road, etc.)'),
  bounds: z.object({
    northeast: z.object({ lat: z.number(), lng: z.number() }),
    southwest: z.object({ lat: z.number(), lng: z.number() }),
  }).optional().describe('Bounding box of the result'),
