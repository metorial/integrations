...
export let getPriceData = SlateTool.create(
  spec,
  {
    name: 'Get Price Data',
    key: 'get_price_data',
    ...
    constraints: [
      'Date range for OHLCV data is limited to 29-day windows per request.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    dataType: z.enum(['current', 'daily_ohlcv', 'hourly_ohlcv']).describe('Type of price data to retrieve'),
    startDate: z.string().optional().describe('Start date for OHLCV data in YYYY-MM-DD format'),
