...
export let getPricePrediction = SlateTool.create(
  spec,
  {
...
- **Scenario Analysis**: Detailed scenario-based analysis with base, bear, and moon case predictions including predicted prices, market caps, FDV, and ROI.`,
  ...
  .input(z.object({
    predictionType: z.enum(['price_prediction', 'scenario_analysis']).describe('Type of prediction to retrieve'),
  ...
  .output(z.object({
    predictions: z.array(z.record(z.string(), z.any())).describe('Array of prediction records'),
  }))
