...
export let manageConversation = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'delete', 'export']).describe('Action to perform on the conversation'),
    projectId: z.number().describe('ID of the agent'),
    sessionId: z.string().optional().describe('Session ID of the conversation (required for update, delete, export)'),
    name: z.string().optional().describe('Name for the conversation (used with create and update)'),
  }))
  .output(z.object({
    ...
    exportedContent: z.record(z.string(), z.unknown()).optional().describe('Exported conversation content'),
  }))
