...
export let mergeTemplate = SlateTool.create(
  spec,
  {
    name: 'Merge Template',
    key: 'merge_template',
  ...
  .input(z.object({
    ...
    replacements: z.array(z.object({
      placeholder: z.string().describe('The placeholder text to find (e.g., "{{name}}" or "[[date]]")'),
      value: z.string().describe('The value to replace the placeholder with'),
      matchCase: z.boolean().optional().default(true).describe('Whether to match case when finding placeholders')
    })).min(1).describe('List of placeholder-value pairs to replace')
  }))
  .output(z.object({
