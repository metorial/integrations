...
export let exportData = SlateTool.create(
  ...
  .input(z.object({
    ...
    destinationUris: z.array(z.string()).describe('Cloud Storage destination URIs (gs://bucket/path)'),
    destinationFormat: z.enum(['CSV', 'NEWLINE_DELIMITED_JSON', 'AVRO']).optional().describe('Export format (default CSV)'),
    compression: z.enum(['NONE', 'GZIP', 'DEFLATE', 'SNAPPY', 'ZSTD']).optional().describe('Compression type (default NONE)'),
    fieldDelimiter: z.string().optional().describe('Field delimiter for CSV exports'),
    printHeader: z.boolean().optional().describe('Whether to include a header row in CSV exports (default true)')
  }))
  .output(z.object({
    jobId: z.string(),
    state: z.string(),
