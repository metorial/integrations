...
export let datasetUpdated = SlateTrigger.create(
  spec,
  {
    ...
    key: 'dataset_changed',
    description: 'Triggers when datasets are created, modified, or deleted in the project. Detects new datasets, removed datasets, and metadata changes (name, description, labels, etc.).'
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let currentDatasets = (result.datasets || []).map((d: any) => d.datasetReference.datasetId as string);
