...
export let listEnvelopes = SlateTool.create(
  ...
  .input(z.object({
    status: z.enum(['created', 'pending', 'completed', 'declined', 'voided', 'expired']).optional().describe('Filter envelopes by status'),
  ...
  .output(z.object({
    ...
    envelopes: z.array(z.object({
      ...
      status: z.string().describe('Envelope status (created, pending, completed, declined, voided, expired)'),
...
export let getEnvelopeDetails = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    includeHistory: z.boolean().optional().default(false).describe('Whether to include the activity history (audit trail)'),
  }))
  .output(z.object({
    envelopeId: z.number().describe('Unique envelope ID'),
    title: z.string().optional().describe('Envelope title'),
    ...
    history: z.array(z.record(z.string(), z.unknown())).optional().describe('Activity history / audit trail'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({
      token: ctx.auth.token,
...
export let cancelEnvelope = SlateTool.create(
  spec,
  {
...
export let sendEnvelopeReminder = SlateTool.create(
  spec,
  {
