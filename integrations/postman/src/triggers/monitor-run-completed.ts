...
export let monitorRunCompletedTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Monitor Run Completed',
    key: 'monitor_run_completed',
    description: 'Triggers when a Postman monitor completes a run. Polls monitors and detects new run completions based on the lastRun timestamp.',
  }
)
  .input(z.object({
    monitorId: z.string(),
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let inputs: Array<{ monitorId: string; monitorName: string; uid?: string; lastRunAt: string; status?: string }> = [];
