...
export let manageCommentTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    entityType: z.enum(['collection', 'folder', 'request', 'response', 'api']).describe('Type of entity to comment on'),
    entityId: z.string().describe('ID of the entity (collection ID, folder ID, request ID, response ID, or API ID)'),
    collectionId: z.string().optional().describe('Parent collection ID (required when entityType is folder, request, or response)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update') {
      ...
      let result = await client.updateComment(entityType, entityId, commentId, body, collectionId);
      return {
        output: { comment: result },
