...
let eventSchema = z.object({
  eventName: z.string().describe('Event name (e.g., Purchase, AddToCart, Lead, ViewContent, CompleteRegistration, InitiateCheckout, Search, AddPaymentInfo, Contact, Subscribe)'),
  eventTime: z.number().describe('Unix timestamp of when the event occurred'),
  eventId: z.string().optional().describe('Unique event ID for deduplication with browser pixel'),
  eventSourceUrl: z.string().optional().describe('URL where the event occurred (required for web events)'),
  actionSource: z.enum([
    'website',
    'app',
    'email',
  ...
  customData: z.record(z.string(), z.any()).optional().describe('Additional event data (e.g., { currency: "USD", value: 49.99, content_ids: ["product1"], content_type: "product" })')
});
...
Events are sent to a dataset/pixel ID and used for ad optimization, targeting, and measurement.`,
  ...
  .input(z.object({
    ...
    events: z.array(eventSchema).min(1).max(1000).describe('Array of conversion events to send'),
    testEventCode: z.string().optional().describe('Test event code from Events Manager for validation (events appear in Test Events tab)')
  }))
  .output(z.object({
