...
let ticketEventSchema = z.object({
  ...
  type: z.string().describe('Event type (e.g. "message", "status_change", "assignment_change")'),
  createdAt: z.string().describe('ISO 8601 timestamp when the event occurred'),
  author: z.object({
    id: z.string().optional().describe('Author ID'),
    type: z.string().optional().describe('Author type (e.g. "agent", "requester", "system")'),
    name: z.string().optional().describe('Author display name'),
    email: z.string().optional().describe('Author email address'),
  }).optional().describe('Author of the event'),
  message: z.object({
    text: z.string().optional().describe('Plain text message content'),
    html: z.string().optional().describe('HTML formatted message content'),
  }).optional().describe('Message content if this is a message event'),
  visibility: z.enum(['public', 'private']).optional().describe('Whether the event is public (visible to requester) or private (internal note)'),
  ...
  })).optional().describe('File attachments on this event'),
});
...
let ticketSchema = z.object({
  ...
  events: z.array(ticketEventSchema).optional().describe('Chronological list of events (messages, status changes, etc.) on this ticket'),
  rating: z.object({
