...
export let listPosts = SlateTool.create(
  spec,
  {
    name: 'List Posts',
    key: 'list_posts',
  ...
  .input(z.object({
    ...
    after: z.string().optional().describe('Cursor for pagination — use the endCursor from a previous response'),
    tagSlugs: z.array(z.string()).optional().describe('Filter posts by tag slugs, e.g. ["javascript", "react"]'),
  }))
  .output(z.object({
    posts: z.array(z.object({
      ...
      publishedAt: z.string().nullable().optional().describe('ISO timestamp when published'),
      updatedAt: z.string().nullable().optional().describe('ISO timestamp when last updated'),
      readTimeInMinutes: z.number().nullable().optional().describe('Estimated read time'),
      ...
      authorUsername: z.string().nullable().optional().describe('Username of the author'),
      authorName: z.string().nullable().optional().describe('Display name of the author'),
    ...
    endCursor: z.string().nullable().optional().describe('Cursor to use for fetching the next page'),
    totalDocuments: z.number().nullable().optional().describe('Total number of posts in the publication'),
  }))
