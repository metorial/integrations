...
export let scheduleBotForEventTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    botConfig: z.record(z.string(), z.unknown()).optional().describe('Optional bot configuration (bot_name, recording_config, etc.)'),
  }))
  .output(z.object({
    eventId: z.string().describe('Calendar event ID'),
    scheduled: z.boolean().describe('Whether the bot was successfully scheduled'),
    response: z.record(z.string(), z.unknown()).describe('API response with bot scheduling details'),
  }))
