...
let calendarEventSchema = z.object({
  ...
  meetingPlatform: z.string().nullable().describe('Detected meeting platform (zoom, google_meet, teams, etc.)'),
...
export let listCalendarEventsTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    startTimeAfter: z.string().optional().describe('Filter events starting after this ISO 8601 timestamp'),
    startTimeBefore: z.string().optional().describe('Filter events starting before this ISO 8601 timestamp'),
    updatedAtGte: z.string().optional().describe('Filter events updated at or after this ISO 8601 timestamp'),
    includeDeleted: z.boolean().optional().describe('Whether to include deleted events (default: false)'),
