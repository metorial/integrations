...
export let getTranscriptTool = SlateTool.create(
  ...
  .input(z.object({
    botId: z.string().describe('The unique identifier of the bot whose transcript to retrieve'),
  }))
  .output(z.object({
    botId: z.string().describe('Bot ID the transcript belongs to'),
    entries: z.array(transcriptEntrySchema).describe('Transcript entries with speaker attribution and timing'),
