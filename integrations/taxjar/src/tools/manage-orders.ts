...
let mapOrderLineItems = (items?: Array<{ id?: string; quantity?: number; product_identifier?: string; description?: string; product_tax_code?: string; unit_price?: number; discount?: number; sales_tax?: number }>) => {
  return items?.map((li) => ({
    lineItemId: li.id,
...
export let listOrders = SlateTool.create(
  ...
  .input(z.object({
    ...
    fromTransactionDate: z.string().optional().describe('Start of date range (YYYY-MM-DD or MM/DD/YYYY)'),
    toTransactionDate: z.string().optional().describe('End of date range (YYYY-MM-DD or MM/DD/YYYY)'),
    provider: z.string().optional().describe('Marketplace provider filter (e.g. amazon, ebay)'),
  }))
  .output(z.object({
...
export let createOrder = SlateTool.create(
  ...
  .input(z.object({
    transactionId: z.string().describe('Unique transaction ID for the order (no periods allowed)'),
    transactionDate: z.string().optional().describe('Transaction date (YYYY/MM/DD)'),
    provider: z.string().optional().describe('Marketplace provider (e.g. amazon, ebay, facebook)'),
