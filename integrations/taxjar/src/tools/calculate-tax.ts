...
let lineItemSchema = z.object({
  ...
  productTaxCode: z.string().optional().describe('Product tax code for exemption rules (e.g. 31000 for digital goods)'),
...
export let calculateTax = SlateTool.create(
  ...
  .input(z.object({
    fromCountry: z.string().optional().describe('Origin two-letter ISO country code (e.g. US)'),
    ...
    amount: z.number().optional().describe('Total order amount excluding shipping and tax. If omitted, calculated from line items.'),
    shipping: z.number().describe('Total shipping cost for the order'),
    customerId: z.string().optional().describe('Unique customer identifier for exemption lookup'),
    exemptionType: z.enum(['wholesale', 'government', 'marketplace', 'other', 'non_exempt']).optional().describe('Customer exemption type'),
    nexusAddresses: z.array(nexusAddressSchema).optional().describe('Nexus address locations. Overrides account settings if provided.'),
    lineItems: z.array(lineItemSchema).optional().describe('Individual line items in the order'),
  }))
  .output(z.object({
    ...
    }).optional().describe('Jurisdiction names used for the calculation'),
    breakdown: z.object({
      taxableAmount: z.number().optional(),
      taxCollectable: z.number().optional(),
      combinedTaxRate: z.number().optional(),
    ...
    }).optional().describe('Detailed tax breakdown by jurisdiction'),
  }))
