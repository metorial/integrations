...
export let listUsers = SlateTool.create(
  ...
  .input(z.object({
    externalUserId: z.string().optional().describe('Filter users by your application\'s external user ID'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let criteria = ctx.input.externalUserId ? { externalUserId: ctx.input.externalUserId } : undefined;
...
export let createUser = SlateTool.create(
  ...
  .input(z.object({
    ...
    externalUserId: z.string().describe('Your application\'s unique identifier for this user'),
    generateAccessToken: z.boolean().optional().default(false).describe('Whether to also generate a user access token (valid for 2 days)'),
  }))
  .output(z.object({
    userId: z.string().describe('Tray.io internal user ID'),
    accessToken: z.string().optional().describe('User access token if requested (valid for 2 days)'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new TrayGraphqlClient({
      token: ctx.auth.token,
...
export let generateUserToken = SlateTool.create(
  spec,
  {
