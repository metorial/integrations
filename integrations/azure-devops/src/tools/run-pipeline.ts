...
export let runPipelineTool = SlateTool.create(
  spec,
  {
    name: 'Run Pipeline',
    key: 'run_pipeline',
  ...
  .input(z.object({
    ...
    branch: z.string().optional().describe('Branch to run the pipeline on (e.g. "main" or "refs/heads/feature-branch")'),
    variables: z.record(z.string(), z.string()).optional().describe('Pipeline variables as key-value pairs'),
    templateParameters: z.record(z.string(), z.string()).optional().describe('Template parameters as key-value pairs'),
  }))
  .output(z.object({
    runId: z.number(),
    pipelineId: z.number(),
