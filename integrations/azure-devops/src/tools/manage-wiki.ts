...
export let manageWikiTool = SlateTool.create(
  spec,
  {
    name: 'Manage Wiki',
    key: 'manage_wiki',
  ...
  .input(z.object({
    ...
    action: z.enum(['list_wikis', 'get_page', 'create_or_update_page']).describe('Action to perform'),
    wikiIdentifier: z.string().optional().describe('Wiki name or ID (required for get_page and create_or_update_page)'),
    pagePath: z.string().optional().describe('Wiki page path (e.g. "/Home" or "/Architecture/Overview"). Required for get_page and create_or_update_page.'),
    pageContent: z.string().optional().describe('Markdown content for the page (required for create_or_update_page)'),
    eTag: z.string().optional().describe('ETag / version for optimistic concurrency on updates. Required when updating an existing page.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'get_page') {
      ...
      let page = await client.getWikiPage(project, ctx.input.wikiIdentifier, ctx.input.pagePath, {
        includeContent: true,
    ...
    if (ctx.input.action === 'create_or_update_page') {
      if (!ctx.input.wikiIdentifier || !ctx.input.pagePath || ctx.input.pageContent === undefined) {
        throw new Error('wikiIdentifier, pagePath, and pageContent are required for "create_or_update_page"');
      }
