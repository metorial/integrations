...
export let listBuildsTool = SlateTool.create(
  spec,
  {
    name: 'List Builds',
    key: 'list_builds',
  ...
  .input(z.object({
    ...
    definitionIds: z.array(z.number()).optional().describe('Filter by pipeline/build definition IDs'),
    statusFilter: z.enum(['all', 'cancelling', 'completed', 'inProgress', 'none', 'notStarted', 'postponed']).optional().describe('Filter by build status'),
    resultFilter: z.enum(['canceled', 'failed', 'none', 'partiallySucceeded', 'succeeded']).optional().describe('Filter by build result'),
