...
export let listTechnologySites = SlateTool.create(
  spec,
  {
...
Supports pagination with offset and date filtering to find sites detected since a specific date.`,
  ...
  .input(z.object({
    technology: z.string().describe('Technology name to search for (e.g., "Shopify", "WordPress")'),
    includeMetaData: z.boolean().optional().describe('Include metadata like company names, emails, addresses, and social profiles'),
    offset: z.number().optional().describe('Pagination offset for retrieving additional results'),
    since: z.string().optional().describe('Only return sites detected since this date (Unix timestamp in ms)'),
  }))
  .output(z.object({
    ...
    nextOffset: z.number().optional().describe('Offset value for retrieving the next page of results'),
