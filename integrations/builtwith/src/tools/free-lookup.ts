...
let groupSchema = z.object({
  ...
  }).passthrough()).optional().describe('Categories within this group'),
}).passthrough();

export let freeLookup = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    domain: z.string().describe('Domain to get a technology summary for (e.g., "example.com")'),
  }))
  .output(z.object({
