...
export let resolveIp = SlateTool.create(
  spec,
  {
...
Returns the matched company profile along with a confidence score, match type (business/ISP/hosting), IP geolocation, WHOIS data, and ASN information.`,
  ...
  .output(z.object({
    ...
    matchType: z.string().nullable().optional().describe('Result classification: "business", "isp", or "hosting"'),
    fuzzy: z.boolean().nullable().optional().describe('Whether the match is approximate'),
    confidence: z.number().nullable().optional().describe('Match confidence score from 0 to 1'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let confidencePercent = result.confidence != null ? `${Math.round(result.confidence * 100)}%` : 'N/A';
