...
export let customerEvents = SlateTrigger.create(
  spec,
  {
    ...
    key: 'customer_events',
    description: 'Triggers when a customer is created, updated, or deleted in the CloudCart store.',
  }
)
  .input(z.object({
    ...
    customerAttributes: z.record(z.string(), z.any()).describe('Customer attributes from the webhook payload'),
  }))
  .output(z.object({
    customerId: z.string(),
