...
export let manageCourierShipments = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    manual: z.boolean().optional().describe('If true, create a manual package entry instead of using the courier API'),
    accountId: z.number().optional().describe('Courier API account ID (optional, uses first account if omitted)'),
    fields: z.array(z.object({
      fieldId: z.string().describe('Field ID from getCourierFields'),
      fieldValue: z.string().describe('Field value'),
    })).optional().describe('Courier-specific form fields'),
    ...
    })).optional().describe('List of packages with dimensions'),
    packageNumber: z.string().optional().describe('Tracking number (required for manual shipments)'),
    pickupDate: z.number().optional().describe('Pickup date as unix timestamp (manual shipments)'),
  }))
  .output(z.object({
