...
export let subscribeContact = SlateTool.create(
  spec,
  {
...
Set **doubleOptin** to \`3\` to add contacts without sending a confirmation email.`,
    ...
    constraints: [
      'Rate limit: 300 requests per 60 seconds per API key or IP address.',
    ],
  }
)
  .input(z.object({
    listIds: z.array(z.number()).describe('IDs of lists to subscribe the contact to'),
    fields: z.record(z.string(), z.string()).describe('Contact fields as key-value pairs (e.g., {"email": "user@example.com", "Name": "John"})'),
    ...
    overwrite: z.enum(['0', '1', '2']).optional().describe('0=do not overwrite, 1=overwrite fields, 2=append to lists'),
    requestIp: z.string().optional().describe('IP address of the subscriber'),
    requestTime: z.string().optional().describe('Time of the subscription request (YYYY-MM-DD HH:MM:SS)'),
  }))
  .output(z.object({
