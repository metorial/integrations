...
export let unsubscribeContact = SlateTool.create(
  ...
  .input(z.object({
    method: z.enum(['unsubscribe', 'exclude']).describe('"unsubscribe" marks as opted-out, "exclude" removes from lists entirely'),
    contact: z.string().describe('Email address or phone number of the contact'),
    contactType: z.enum(['email', 'phone']).optional().describe('Type of contact identifier'),
    listIds: z.array(z.number()).optional().describe('Specific list IDs to unsubscribe from. Omit to unsubscribe from all lists.'),
  }))
  .output(z.object({
