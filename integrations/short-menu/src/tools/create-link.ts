...
let tagSchema = z.object({
  name: z.string().describe('Name of the tag. If a tag with this name already exists, it will be reused.'),
  id: z.string().optional().describe('ID of an existing tag. Only provide if the tag already exists in your account.'),
});
...
export let createLink = SlateTool.create(
  ...
  .input(z.object({
    destinationUrl: z.string().describe('The destination URL the short link will redirect to. Must include a scheme (e.g. https://).'),
    domain: z.string().optional().describe('Domain for the short link (e.g. shm.to or a custom domain). Falls back to the configured default domain if not provided.'),
    slug: z.string().optional().describe('Custom slug for the short link (e.g. "my-link" creates shm.to/my-link). Must be unique within the domain. If omitted, a random slug is generated.'),
    tags: z.array(tagSchema).optional().describe('Tags to assign to the short link for organization.'),
  }))
  .output(z.object({
