...
export let getVisitor = SlateTool.create(
  spec,
  {
    name: 'Get Visitor',
    key: 'get_visitor',
  ...
  .output(z.object({
    ...
    metadata: z.any().optional().describe('Visitor metadata including auto and custom fields'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {
        ...
        firstVisit: visitor.metadata?.auto?.firstvisit ? String(visitor.metadata.auto.firstvisit) : undefined,
        lastVisit: visitor.metadata?.auto?.lastvisit ? String(visitor.metadata.auto.lastvisit) : undefined,
