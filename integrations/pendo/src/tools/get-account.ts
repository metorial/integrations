...
export let getAccount = SlateTool.create(
  ...
  .output(z.object({
    ...
    metadata: z.any().optional().describe('Account metadata including auto and custom fields'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {
        ...
        firstVisit: account.metadata?.auto?.firstvisit ? String(account.metadata.auto.firstvisit) : undefined,
        lastVisit: account.metadata?.auto?.lastvisit ? String(account.metadata.auto.lastvisit) : undefined,
