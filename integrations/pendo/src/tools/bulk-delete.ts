...
export let bulkDelete = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Deletion requests are processed in batches and may take up to 21 days.',
    ],
    tags: {
      destructive: true,
    },
  ...
  .input(z.object({
    entityType: z.enum(['visitor', 'account']).describe('Whether to delete visitors or accounts'),
    entityIds: z.array(z.string()).min(1).describe('List of visitor or account IDs to delete'),
  }))
  .output(z.object({
