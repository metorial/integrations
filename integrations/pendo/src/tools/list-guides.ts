...
export let listGuides = SlateTool.create(
  spec,
  {
    name: 'List Guides',
    key: 'list_guides',
  ...
  .input(z.object({
    appId: z.string().optional().describe('Application ID to filter guides for a specific app'),
  }))
  .output(z.object({
    guides: z.array(z.object({
      ...
      isMultiStep: z.boolean().optional().describe('Whether the guide has multiple steps'),
