...
let travelBoundarySchema = z.object({
  mode: z.string().optional().describe('Travel mode, e.g. "driving", "walking", "cycling", "transit"'),
  value: z.number().optional().describe('Travel time or distance value'),
  unit: z.string().optional().describe('Unit for the value, e.g. "minutes", "kilometers", "miles"'),
});
...
export let createLocation = SlateTool.create(
  ...
  .input(z.object({
    ...
    externalId: z.string().optional().describe('External reference ID for linking with other systems'),
    customAttributes: z.record(z.string(), z.any()).optional().describe('Custom key-value attributes for the location'),
    createMissingCustomAttributes: z.boolean().optional().describe('Automatically create custom attribute definitions that do not yet exist'),
    travelBoundary: travelBoundarySchema.optional().describe('Travel boundary (isochrone) to attach to the location'),
  }))
  .output(z.object({
