...
export let createEventType = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    locations: z.array(z.any()).optional().describe('Array of location objects defining where the event takes place'),
    scheduleId: z.number().optional().describe('ID of the availability schedule to use'),
    slotInterval: z.number().optional().describe('Interval between available slots in minutes'),
    ...
    minimumBookingNotice: z.number().optional().describe('Minimum notice in minutes before a booking can be made'),
    beforeEventBuffer: z.number().optional().describe('Buffer time in minutes before the event'),
    afterEventBuffer: z.number().optional().describe('Buffer time in minutes after the event'),
    requiresBookerAuthentication: z.boolean().optional().describe('Require authentication to book'),
    hideCalendarNotes: z.boolean().optional().describe('Hide calendar notes from attendees'),
    metadata: z.record(z.string(), z.any()).optional().describe('Custom metadata for the event type'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.minimumBookingNotice) body.minimumBookingNotice = ctx.input.minimumBookingNotice;
    ...
    if (ctx.input.requiresBookerAuthentication !== undefined) body.requiresBookerAuthentication = ctx.input.requiresBookerAuthentication;
    if (ctx.input.hideCalendarNotes !== undefined) body.hideCalendarNotes = ctx.input.hideCalendarNotes;
