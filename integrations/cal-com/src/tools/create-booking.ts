...
export let createBooking = SlateTool.create(
  ...
  .input(z.object({
    ...
    eventTypeSlug: z.string().optional().describe('Slug of the event type (requires username)'),
    ...
    attendeeTimeZone: z.string().describe('Time zone of the attendee (e.g., America/New_York)'),
    attendeePhoneNumber: z.string().optional().describe('Phone number of the attendee (required if SMS reminders are enabled)'),
    guests: z.array(z.string()).optional().describe('List of guest email addresses to include'),
    ...
    bookingFieldsResponses: z.record(z.string(), z.any()).optional().describe('Responses to custom booking form fields'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.bookingFieldsResponses) body.bookingFieldsResponses = ctx.input.bookingFieldsResponses;
