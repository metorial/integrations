...
let kibanaPrivilegeSchema = z.object({
  ...
  feature: z.record(z.string(), z.array(z.string())).optional().describe('Feature-specific privileges (e.g., {"discover": ["all"], "dashboard": ["read"]})'),
  spaces: z.array(z.string()).describe('Space IDs this privilege applies to (use ["*"] for all spaces)')
});
...
let roleOutputSchema = z.object({
  ...
  elasticsearch: z.object({
    ...
    })).optional().describe('Index-level privileges'),
    runAs: z.array(z.string()).optional().describe('Users this role can impersonate')
  }).optional().describe('Elasticsearch privileges'),
...
export let manageRole = SlateTool.create(
  ...
  .input(z.object({
    ...
    roleName: z.string().describe('Name of the role'),
    elasticsearch: z.object({
      cluster: z.array(z.string()).optional().describe('Cluster privileges (e.g., ["monitor", "manage_index_templates"])'),
      indices: z.array(z.object({
        names: z.array(z.string()).describe('Index names/patterns (e.g., ["logs-*"])'),
        privileges: z.array(z.string()).describe('Index privileges (e.g., ["read", "write"])')
      })).optional().describe('Index-level privileges'),
      runAs: z.array(z.string()).optional().describe('Users this role can run as')
    }).optional().describe('Elasticsearch privileges'),
    kibana: z.array(kibanaPrivilegeSchema).optional().describe('Kibana feature privileges per space')
  }))
