...
export let manageSavedObject = SlateTool.create(
  spec,
  {
...
Provide the action to perform along with the object type and ID.`,
  ...
  .input(z.object({
    action: z.enum(['get', 'create', 'update', 'delete']).describe('Action to perform on the saved object'),
    objectType: z.string().describe('Type of saved object (e.g., "dashboard", "visualization", "index-pattern")'),
    objectId: z.string().optional().describe('ID of the saved object (required for get, update, delete)'),
    attributes: z.record(z.string(), z.any()).optional().describe('Object attributes (for create and update)'),
    ...
    overwrite: z.boolean().optional().describe('Overwrite existing object if ID matches (for create only)'),
    force: z.boolean().optional().describe('Force deletion even if the object has references (for delete only)')
  }))
  .output(z.object({
