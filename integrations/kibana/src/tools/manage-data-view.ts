...
let dataViewOutputSchema = z.object({
  ...
  runtimeFieldMap: z.record(z.string(), z.any()).optional().describe('Runtime field definitions'),
  fieldFormats: z.record(z.string(), z.any()).optional().describe('Field format configurations'),
  sourceFilters: z.array(z.object({ value: z.string() })).optional().describe('Source filters')
});

export let listDataViews = SlateTool.create(
  spec,
  {
...
export let manageDataView = SlateTool.create(
  spec,
  {
...
Supports configuring runtime fields, time fields, field formats, and source filters.`,
  ...
  .input(z.object({
    ...
    dataViewId: z.string().optional().describe('ID of the data view (required for get, update, delete)'),
    title: z.string().optional().describe('Index pattern string (e.g., "logs-*"). Required for create.'),
    name: z.string().optional().describe('Display name for the data view'),
    timeFieldName: z.string().optional().describe('Field used for time-based filtering'),
    runtimeFieldMap: z.record(z.string(), z.any()).optional().describe('Runtime field definitions'),
    fieldFormats: z.record(z.string(), z.any()).optional().describe('Field format configurations'),
    sourceFilters: z.array(z.object({ value: z.string() })).optional().describe('Source filters to exclude fields'),
    allowNoIndex: z.boolean().optional().describe('Allow data view creation even if matching indices do not exist')
  }))
  .output(dataViewOutputSchema.extend({
    deleted: z.boolean().optional().describe('Whether the data view was deleted')
  }))
  .handleInvocation(async (ctx) => {
    let client = createClient(ctx);
    let { action, dataViewId, title, name, timeFieldName, runtimeFieldMap, fieldFormats, sourceFilters, allowNoIndex } = ctx.input;
