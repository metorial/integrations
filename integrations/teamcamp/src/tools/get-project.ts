...
export let getProject = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeGroups: z.boolean().optional().default(false).describe('Whether to also fetch the project task groups'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Task groups within the project (only included when includeGroups is true)'),
  }))
