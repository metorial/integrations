...
export let parseAddress = SlateTool.create(
  spec,
  {
    name: 'Parse Address',
    key: 'parse_address',
  ...
  .input(z.object({
    address: z.string().describe('The full address string to parse (e.g., "922 N 5th St Suite 100A Phoenix AZ 85004")'),
  }))
  .output(z.object({
    parsedAddress: z.record(z.string(), z.any()).describe('The parsed address components (street, city, state, zip, etc.)'),
  }))
