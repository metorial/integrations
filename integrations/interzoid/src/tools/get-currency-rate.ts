...
export let getCurrencyRate = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    currencyCode: z.string().describe('Three-letter ISO currency code (e.g., "EUR", "GBP", "JPY")'),
  }))
  .output(z.object({
    exchangeData: z.record(z.string(), z.any()).describe('Currency exchange rate data including symbol, currency name, country, and rate in USD'),
  }))
