...
export let customDataEnrichment = SlateTool.create(
  spec,
  {
...
This is a **premium API** that consumes multiple credits per call.`,
  ...
  .input(z.object({
    topic: z.string().describe('Description of the data domain (e.g., "detailed information about companies")'),
    lookup: z.string().describe('The value to look up (e.g., "IBM")'),
    outputFields: z.array(z.string()).describe('List of desired output field names (e.g., ["headquarters", "ceo", "revenue"])'),
    model: z.enum(['default', 'model-a', 'model-x', 'model-a-premium']).optional().describe('AI model to use'),
  }))
  .output(z.object({
    enrichedData: z.record(z.string(), z.any()).describe('The AI-generated structured data with requested fields'),
  }))
