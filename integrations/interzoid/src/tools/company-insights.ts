...
These are **premium APIs** that consume multiple credits per call.`,
  ...
  .input(z.object({
    insightType: z.enum(['parent_company', 'executive_profile', 'verification']).describe('Type of company insight to retrieve'),
    lookup: z.string().describe('For parent_company: subsidiary company name. For executive_profile: company name and title (e.g., "Amazon AWS CEO"). For verification: company name.'),
  }))
  .output(z.object({
