...
export let stockAnalysis = SlateTool.create(
  spec,
  {
    name: 'Stock Analysis',
    key: 'stock_analysis',
...
This is a **premium API** that consumes multiple credits per call.`,
  ...
  .input(z.object({
    lookup: z.string().describe('Stock ticker symbol or company name (e.g., "AAPL", "Microsoft")'),
  }))
  .output(z.object({
    stockData: z.record(z.string(), z.any()).describe('Stock analysis data including price, market data, and company information'),
  }))
