...
export let screenConfigSchema = z.object({
  width: z.number().optional().describe('Screen width in pixels'),
  height: z.number().optional().describe('Screen height in pixels'),
}).optional().describe('Screen resolution configuration');

export let sessionOptionsSchema = z.object({
  useStealth: z.boolean().optional().describe('Enable stealth mode for anti-bot detection avoidance'),
  useProxy: z.boolean().optional().describe('Enable built-in rotating residential proxy (requires paid plan)'),
  proxyCountry: z.string().optional().describe('Two-letter country code for proxy location (e.g., "US", "GB")'),
  ...
  proxyServer: z.string().optional().describe('Custom proxy server URL (bring your own proxy)'),
  solveCaptchas: z.boolean().optional().describe('Enable automatic CAPTCHA solving (requires paid plan)'),
  adblock: z.boolean().optional().describe('Block advertisements'),
  trackers: z.boolean().optional().describe('Block trackers'),
  annoyances: z.boolean().optional().describe('Block annoyances (cookie banners, popups, etc.)'),
  acceptCookies: z.boolean().optional().describe('Automatically accept cookie consent dialogs'),
  operatingSystems: z.array(z.string()).optional().describe('Allowed OS for browser fingerprint (e.g., ["windows", "macos"])'),
  device: z.string().optional().describe('Device type for fingerprinting (e.g., "desktop", "mobile")'),
  ...
  enableWebRecording: z.boolean().optional().describe('Enable rrweb DOM recording (enabled by default)'),
  enableVideoWebRecording: z.boolean().optional().describe('Enable MP4 video recording'),
  extensionIds: z.array(z.string()).optional().describe('Chrome extension IDs to attach to the session'),
  saveDownloads: z.boolean().optional().describe('Enable saving downloaded files for retrieval'),
}).optional().describe('Browser session configuration options');

export let scrapeOptionsSchema = z.object({
  formats: z.array(z.enum(['markdown', 'html', 'links', 'screenshot'])).optional().describe('Output formats to return'),
  onlyMainContent: z.boolean().optional().describe('Only extract the main content of the page'),
...
}).optional().describe('Scrape output configuration');

export let scrapeJobDataSchema = z.object({
  metadata: z.record(z.string(), z.any()).optional().describe('Page metadata (title, description, etc.)'),
...
export let sessionDetailSchema = z.object({
  ...
  wsEndpoint: z.string().optional().describe('WebSocket endpoint for Playwright/Puppeteer connection'),
  ...
  webdriverEndpoint: z.string().optional().describe('WebDriver endpoint for Selenium connection'),
