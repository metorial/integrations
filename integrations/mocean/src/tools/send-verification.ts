...
export let sendVerification = SlateTool.create(
  ...
  .input(z.object({
    to: z.string().describe('Recipient phone number with country code (e.g., "60123456789"), or Telegram Chat ID for Telegram channel'),
    brand: z.string().describe('Brand/company name displayed in the verification message'),
    channel: z.enum(['sms', 'telegram']).optional().describe('Delivery channel (default: "sms")'),
    from: z.string().optional().describe('Sender ID displayed to recipient. Required for Telegram (bot username)'),
    codeLength: z.union([z.literal(4), z.literal(6)]).optional().describe('PIN code length: 4 or 6 digits (default: 4)'),
    pinValidity: z.number().optional().describe('Code expiration in seconds (60-3600, default: 300)'),
    nextEventWait: z.number().optional().describe('Seconds before failover to next verification channel (60-900)'),
    enableNumberLookup: z.boolean().optional().describe('Check number validity before sending'),
  }))
  .output(z.object({
    requestId: z.string().optional().describe('Verification request ID for checking or resending'),
    status: z.number().describe('Status code (0 = success)'),
    to: z.string().optional().describe('Recipient phone number'),
    numberReachable: z.string().optional().describe('Number reachability status if Number Lookup was enabled'),
