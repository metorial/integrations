...
export let getObjectTool = SlateTool.create(
  spec,
  {
    name: 'Get Object',
    key: 'get_object',
...
Supports fetching specific versions of versioned objects.`,
  ...
  .input(z.object({
    ...
    versionId: z.string().optional().describe('Specific version ID to retrieve (requires versioning enabled)'),
    metadataOnly: z.boolean().optional().describe('If true, only fetch metadata without downloading content (HEAD request)'),
    range: z.string().optional().describe('Byte range to retrieve (e.g., "bytes=0-999" for first 1000 bytes)')
  }))
  .output(z.object({
    content: z.string().optional().describe('Object content as text (omitted when metadataOnly is true)'),
    ...
    serverSideEncryption: z.string().optional().describe('Server-side encryption algorithm used'),
    metadata: z.record(z.string(), z.string()).describe('Custom user metadata attached to the object')
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.metadataOnly) {
      let meta = await client.headObject(ctx.input.bucketName, ctx.input.objectKey, ctx.input.versionId);
      return {
        output: {
