...
export let listObjectsTool = SlateTool.create(
  spec,
  {
    name: 'List Objects',
    key: 'list_objects',
...
Use the **delimiter** parameter (typically \`/\`) to list only objects at the current "folder" level, with common prefixes representing sub-folders.`,
  ...
  .input(z.object({
    ...
    prefix: z.string().optional().describe('Filter objects by key prefix (e.g., "images/" to list objects in the images folder)'),
    delimiter: z.string().optional().describe('Delimiter for grouping keys (typically "/" for folder-like listing)'),
    maxKeys: z.number().optional().describe('Maximum number of objects to return (default 1000)'),
    continuationToken: z.string().optional().describe('Token from a previous response to get the next page of results'),
    startAfter: z.string().optional().describe('Start listing after this key (for pagination)')
  }))
  .output(z.object({
    ...
    commonPrefixes: z.array(z.string()).describe('Common prefixes when using delimiter (sub-folders)'),
    isTruncated: z.boolean().describe('Whether there are more results to fetch'),
    nextContinuationToken: z.string().optional().describe('Token to use for fetching the next page'),
