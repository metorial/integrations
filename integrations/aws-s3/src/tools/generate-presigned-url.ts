...
export let generatePresignedUrlTool = SlateTool.create(
  spec,
  {
...
Use **GET** for downloads and **PUT** for uploads. URLs are valid for a configurable duration (default 1 hour, max 7 days).`,
  ...
  .input(z.object({
    ...
    method: z.enum(['GET', 'PUT']).optional().describe('HTTP method for the presigned URL (default GET for download, PUT for upload)'),
    expiresInSeconds: z.number().optional().describe('URL validity in seconds (default 3600, max 604800 for 7 days)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let presignedUrl = await client.generatePresignedUrl(ctx.input.bucketName, ctx.input.objectKey, {
      method,
      expiresInSeconds,
