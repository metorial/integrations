...
Use this for tooltips and launchers; for tours and microsurveys use the dedicated tools.`,
  ...
  .input(z.object({
    ...
    experienceId: z.string().optional().describe('Experience ID to retrieve a specific item'),
    limit: z.number().min(1).max(500).optional().describe('Number of items to return (1-500, default 50)'),
  ...
  .output(z.object({
    experience: experienceSchema.optional().describe('Single experience (when fetching by ID)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = type === 'tooltip'
      ? await client.listTooltips({ limit: ctx.input.limit, before: ctx.input.before, after: ctx.input.after })
      : await client.listLaunchers({ limit: ctx.input.limit, before: ctx.input.before, after: ctx.input.after });
