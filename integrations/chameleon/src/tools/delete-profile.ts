...
Use **forget** mode to permanently remove all user data with no recovery (backup retained for 3 months).`,
    instructions: [
      'Provide exactly one of profileId, uid, or email to identify the user.',
    ],
    tags: {
      destructive: true,
    },
  ...
  .input(z.object({
    ...
    mode: z.enum(['clear', 'forget']).describe('"clear" resets the profile data; "forget" permanently deletes the user'),
  }))
  .output(z.object({
    profileId: z.string().optional().describe('ID of the affected profile'),
    deletionId: z.string().optional().describe('ID of the deletion record (forget mode only)'),
  }))
