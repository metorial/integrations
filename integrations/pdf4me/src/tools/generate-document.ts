...
Use for automating contracts, invoices, reports, and other data-driven documents.`,
  ...
  .input(z.object({
    ...
    documentDataText: z.string().optional().describe('JSON data string for document generation'),
    documentDataFile: z.string().optional().describe('Base64-encoded data file content (Excel or JSON file)'),
    fieldMetadata: z.string().optional().describe('Metadata for template fields in JSON format'),
    outputType: z.enum(['pdf', 'docx', 'html']).default('pdf').describe('Output document format'),
    keepPdfEditable: z.boolean().optional().describe('Keep PDF form fields editable in the output'),
  }))
  .output(z.object({
