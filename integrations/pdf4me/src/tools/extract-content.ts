...
Use this to pull structured data out of PDFs for further processing.`,
  ...
  .input(z.object({
    ...
    extractType: z.enum(['text', 'images', 'textAndImages', 'tables', 'regex']).describe('What to extract from the PDF'),
    regexExpression: z.string().optional().describe('Regular expression pattern (required when extractType is "regex")'),
    pages: z.string().optional().describe('Pages to extract from (e.g. "1,2,3" or "all"). Required for regex extraction.'),
  }))
  .output(z.object({
    texts: z.array(z.string()).optional().describe('Extracted text content (one entry per page)'),
    images: z.array(z.object({
      fileName: z.string().describe('Image file name'),
      fileContent: z.string().describe('Base64-encoded image content'),
    })).optional().describe('Extracted images'),
    ...
    })).optional().describe('Extracted tables'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let extractText = ctx.input.extractType === 'text' || ctx.input.extractType === 'textAndImages';
    let extractImage = ctx.input.extractType === 'images' || ctx.input.extractType === 'textAndImages';
