...
export let listContacts = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    let contacts = result.contacts.map((c) => mapContact(c as unknown as Record<string, unknown>));
...
export let createContact = SlateTool.create(
  ...
  .input(z.object({
    ...
    country: z.string().describe('2-character ISO country code'),
    postcode: z.string().optional().describe('Postal/ZIP code'),
    customFields: z.record(z.string(), z.unknown()).optional().describe('Custom field values keyed by field slug'),
  }))
  .output(contactOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new CardlyClient({ token: ctx.auth.token });
...
export let editContact = SlateTool.create(
  spec,
  {
    name: 'Edit Contact',
    key: 'edit_contact',
  ...
  .input(z.object({
    ...
    customFields: z.record(z.string(), z.unknown()).optional().describe('Updated custom field values'),
  }))
  .output(contactOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new CardlyClient({ token: ctx.auth.token });
...
export let syncContact = SlateTool.create(
  spec,
  {
    name: 'Sync Contact',
    key: 'sync_contact',
  ...
  .input(z.object({
    ...
    firstName: z.string().describe('Contact first name'),
    lastName: z.string().describe('Contact last name'),
    email: z.string().optional().describe('Email address (used for matching existing contacts)'),
    externalId: z.string().optional().describe('External/CRM ID (used for matching existing contacts)'),
    company: z.string().optional().describe('Company name'),
    address: z.string().describe('Street address'),
    address2: z.string().optional().describe('Address line 2'),
    city: z.string().describe('City'),
...
export let deleteContact = SlateTool.create(
  ...
  .input(z.object({
    ...
    email: z.string().optional().describe('Email of the contact to delete (alternative to contactId)'),
    externalId: z.string().optional().describe('External ID of the contact to delete (alternative to contactId)'),
  }))
  .output(z.object({
