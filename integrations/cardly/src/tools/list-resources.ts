...
export let listResources = SlateTool.create(
  ...
  .input(z.object({
    includeMedia: z.boolean().optional().describe('Include available media/product types with dimensions and credit costs'),
    includeFonts: z.boolean().optional().describe('Include available handwriting fonts'),
    includeWritingStyles: z.boolean().optional().describe('Include available writing styles/messiness levels'),
    includeDoodles: z.boolean().optional().describe('Include available decorative doodles'),
    organisationOnly: z.boolean().optional().describe('If true, only return resources exclusive to your organisation (applies to fonts and doodles)'),
    limit: z.number().optional().describe('Maximum number of results per resource type (default 25)'),
  }))
  .output(z.object({
    media: z.array(z.object({
      ...
      dimensions: z.record(z.string(), z.unknown()).optional().describe('Physical dimensions'),
      creditCost: z.number().optional().describe('Credit cost per card'),
    })).optional().describe('Available media/product types'),
    fonts: z.array(z.object({
      fontId: z.string().describe('Font ID'),
      name: z.string().describe('Font name'),
      slug: z.string().describe('URL-friendly slug'),
      category: z.string().optional().describe('Font category (cursive, hand, serif, etc.)'),
    })).optional().describe('Available handwriting fonts'),
    writingStyles: z.array(z.object({
      styleId: z.string().describe('Writing style ID'),
      name: z.string().describe('Style name'),
      slug: z.string().describe('URL-friendly slug'),
    })).optional().describe('Available writing styles'),
    doodles: z.array(z.object({
      doodleId: z.string().describe('Doodle ID'),
      name: z.string().describe('Doodle name'),
      slug: z.string().describe('URL-friendly slug'),
      url: z.string().optional().describe('Doodle image URL'),
    })).optional().describe('Available decorative doodles'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new CardlyClient({ token: ctx.auth.token });
    let summaryParts: string[] = [];

    let media: Array<{ mediaId: string; name: string; slug: string; dimensions?: Record<string, unknown>; creditCost?: number }> | undefined;
    let fonts: Array<{ fontId: string; name: string; slug: string; category?: string }> | undefined;
    let writingStyles: Array<{ styleId: string; name: string; slug: string }> | undefined;
    let doodles: Array<{ doodleId: string; name: string; slug: string; url?: string }> | undefined;
    ...
    if (ctx.input.includeFonts) {
      let result = await client.listFonts({ limit: ctx.input.limit, organisationOnly: ctx.input.organisationOnly });
    ...
    if (ctx.input.includeDoodles) {
      let result = await client.listDoodles({ limit: ctx.input.limit, organisationOnly: ctx.input.organisationOnly });
