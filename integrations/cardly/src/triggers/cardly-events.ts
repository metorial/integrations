...
export let cardlyEvents = SlateTrigger.create(
  spec,
  {
    name: 'Cardly Events',
    key: 'cardly_events',
    description: 'Receive near-realtime event notifications for order lifecycle, gift card redemptions, QR code scans, and delivery status changes via webhooks.',
  },
)
  .input(z.object({
    eventType: z.string().describe('Type of the event (e.g. contact.order.created, giftCard.redeemed)'),
    eventId: z.string().describe('Unique event identifier for deduplication'),
    eventPayload: z.record(z.string(), z.unknown()).describe('Full event payload from Cardly'),
  }))
  .output(z.object({
    ...
    address: z.record(z.string(), z.unknown()).optional().describe('Contact or consignment address'),
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: {
          ...
          address: (contact?.address || consignment?.address) as Record<string, unknown> | undefined,
