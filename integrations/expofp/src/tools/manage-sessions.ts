...
let sessionSchema = z.object({
  sessionId: z.number().optional().describe('Session ID (omit for new sessions, provide for updates)'),
...
export let upsertSessions = SlateTool.create(
  spec,
  {
...
export let deleteSessions = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'This action is irreversible. Deleted sessions cannot be recovered.'
    ],
    tags: {
      readOnly: false,
      destructive: true,
