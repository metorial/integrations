...
export let aiExtract = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    aiQuery: z.string().optional().describe('A plain-English question to extract specific information from the page'),
    aiExtractRules: z.record(z.string(), z.string()).optional().describe('Key-value pairs where keys are field names and values describe what data to extract in plain English'),
    aiSelector: z.string().optional().describe('CSS selector to focus AI extraction on a specific part of the page'),
    renderJs: z.boolean().optional().describe('Enable JavaScript rendering for dynamic pages'),
    ...
    waitFor: z.string().optional().describe('CSS selector to wait for before extracting data')
  }))
  .output(z.object({
    extractedData: z.any().describe('The AI-extracted data. For aiQuery, this is the answer string. For aiExtractRules, this is a JSON object with the extracted fields.')
  }))
