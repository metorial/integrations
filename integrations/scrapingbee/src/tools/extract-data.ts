...
export let extractData = SlateTool.create(
  ...
  .input(z.object({
    ...
    extractionRules: z.record(z.string(), z.any()).describe('Extraction rules mapping field names to CSS selectors or extraction config objects'),
  ...
  .output(z.object({
    extractedData: z.record(z.string(), z.any()).describe('The extracted data as a JSON object with field names matching the extraction rules')
  }))
