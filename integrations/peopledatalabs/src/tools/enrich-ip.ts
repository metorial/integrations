...
export let enrichIp = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    returnIfUnmatched: z.boolean().optional().describe('Return partial data even if no full match is found'),
  }))
  .output(z.object({
    ...
    }).nullable().optional().describe('Company associated with the IP address'),
    location: z.object({
      name: z.string().nullable().optional(),
    ...
    }).nullable().optional().describe('Geographic location associated with the IP address'),
    confidence: z.string().nullable().optional().describe('Confidence level of the match (very high, high, moderate, low)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.returnIfUnmatched !== undefined) params.return_if_unmatched = ctx.input.returnIfUnmatched;
