...
export let listEvents = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['FUTURE', 'DRAFT', 'DELETED', 'CLOSED', 'ARCHIVED', 'CANCELED', 'CLONEABLE']).optional().describe('Filter by event status'),
    since: z.string().optional().describe('Only return events created/modified since this date (RFC 3339 format, e.g. 2024-01-01T00:00:00Z)'),
    until: z.string().optional().describe('Only return events created/modified until this date (RFC 3339 format)'),
