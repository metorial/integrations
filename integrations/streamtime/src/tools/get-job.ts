...
export let getJob = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeItems: z.boolean().optional().describe('Also fetch the job items (line items in the job plan)'),
  ...
  .output(z.object({
    ...
    phases: z.array(z.record(z.string(), z.any())).optional().describe('Job phases if requested'),
    items: z.array(z.record(z.string(), z.any())).optional().describe('Job items if requested'),
    milestones: z.array(z.record(z.string(), z.any())).optional().describe('Job milestones if requested'),
  }))
