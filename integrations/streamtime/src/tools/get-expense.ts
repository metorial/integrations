...
export let getExpense = SlateTool.create(
  spec,
  {
    name: 'Get Expense',
    key: 'get_expense',
  ...
  .input(z.object({
    ...
    includePurchaseOrder: z.boolean().optional().describe('Also fetch the associated purchase order'),
    includePurchaseOrderLineItems: z.boolean().optional().describe('Also fetch purchase order line items'),
  }))
  .output(z.object({
    expenseId: z.number().describe('ID of the expense'),
    raw: z.record(z.string(), z.any()).describe('Full expense object'),
    purchaseOrder: z.record(z.string(), z.any()).optional().describe('Associated purchase order if requested'),
    purchaseOrderLineItems: z.array(z.record(z.string(), z.any())).optional().describe('Purchase order line items if requested'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includePurchaseOrderLineItems) {
      try {
        output.purchaseOrderLineItems = await client.listExpensePurchaseOrderLineItems(ctx.input.expenseId);
      } catch {
