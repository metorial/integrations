...
export let manageLabels = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'search', 'delete']).describe('The operation to perform'),
    ...
    createMasterLabel: z.boolean().optional().describe('Whether to also create a master label (for create)'),
    searchEntityIds: z.array(z.number()).optional().describe('Array of entity IDs to search labels for (for search)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (input.action) {
      ...
      case 'create': {
        ...
        if (input.createMasterLabel !== undefined) body.createMasterLabel = input.createMasterLabel;
