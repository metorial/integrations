...
export let manageKeywords = SlateTool.create(
  ...
  .input(z.object({
    ...
    keywords: z.array(z.object({
      ...
      region: z.string().optional().describe('Google search region (e.g., "google.com", "google.co.uk")'),
      ...
      ignoreLocalPack: z.boolean().optional().describe('Whether to ignore local pack results'),
      urlTrackingMethod: z.string().optional().describe('URL tracking method (e.g., "exact", "domain")'),
      ignoreSubDomains: z.boolean().optional().describe('Whether to ignore subdomains in matching'),
    })).optional().describe('Keywords to add (required for "add" action)'),
    keywordId: z.string().optional().describe('Keyword ID to delete (required for "delete" action)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'add') {
      ...
      let addedCount = Array.isArray(result) ? result.length : (result?.keywords?.length ?? ctx.input.keywords.length);
