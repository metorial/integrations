...
export let manageProject = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'archive', 'restore', 'delete']).describe('Action to perform on the project'),
    projectName: z.string().describe('Name of the project (used as identifier)'),
    currencyCode: z.string().optional().describe('3-letter currency code (e.g., "USD"). Only used when creating a project.'),
  }))
  .output(z.object({
