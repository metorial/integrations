...
export let getAiMetrics = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    searchTerms: z.array(z.string()).optional().describe('Filter by specific search queries'),
    engines: z.array(z.string()).optional().describe('Filter by AI engines (e.g., "chatgpt", "perplexity", "gemini")'),
    timeframe: z.enum(['24h', '7d', '30d', '3m', '1y']).optional().describe('Time period for metrics'),
    periodOffset: z.number().optional().describe('Period offset for comparison (-1 to -365)'),
    aggregation: z.enum(['hourly', 'daily', 'weekly', 'monthly']).optional().describe('Data aggregation level'),
  }))
  .output(z.object({
    metrics: z.any().describe('Comprehensive AI visibility metrics including visibility scores, sentiment, mentions, citations, competitor data, and historical trends'),
  }))
