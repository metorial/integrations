...
export let chatBoostUpdatedTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Chat Boost Updated',
    key: 'chat_boost_updated',
    description: 'Triggers when a chat boost is added, changed, or removed. The bot must be an administrator in the chat.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    sourceType: z.string().optional().describe('Source of the boost (e.g. premium, gift_code, giveaway)'),
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      if (data.chat_boost) {
        inputs.push({ updateId: data.update_id, eventType: 'chat_boost', boostData: data.chat_boost });
      } else if (data.removed_chat_boost) {
        inputs.push({ updateId: data.update_id, eventType: 'removed_chat_boost', boostData: data.removed_chat_boost });
      }
