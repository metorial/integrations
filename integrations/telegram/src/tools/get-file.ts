...
export let getFileTool = SlateTool.create(
  ...
  .input(z.object({
    fileId: z.string().describe('File ID from a received message (e.g. from photo, document, audio, video)'),
  }))
  .output(z.object({
    fileId: z.string().describe('File identifier'),
    fileUniqueId: z.string().describe('Unique file identifier that stays the same over time'),
