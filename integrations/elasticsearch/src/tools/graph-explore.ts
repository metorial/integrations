...
export let graphExploreTool = SlateTool.create(
  spec,
  {
    name: 'Graph Explore',
    key: 'graph_explore',
  ...
  .input(z.object({
    ...
    query: z.record(z.string(), z.any()).optional().describe('Query to filter documents for exploration'),
    vertices: z.array(z.object({
      ...
      minDocCount: z.number().optional().describe('Minimum number of documents a term must appear in'),
      include: z.array(z.string()).optional().describe('Only include these specific terms'),
    ...
    connections: z.object({
      vertices: z.array(z.object({
        ...
        minDocCount: z.number().optional().describe('Minimum document count for connected terms'),
      })).optional().describe('Connected vertex definitions'),
    }).optional().describe('Connection definitions for second-hop exploration'),
    controls: z.object({
      useSignificance: z.boolean().optional().describe('Use significance scoring (default: true)'),
      sampleSize: z.number().optional().describe('Number of documents to sample'),
    }).optional().describe('Graph exploration controls'),
  }))
  .output(z.object({
    vertices: z.array(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.controls) {
      body.controls = {};
      if (ctx.input.controls.useSignificance !== undefined) body.controls.use_significance = ctx.input.controls.useSignificance;
      if (ctx.input.controls.sampleSize) body.controls.sample_size = ctx.input.controls.sampleSize;
    }
