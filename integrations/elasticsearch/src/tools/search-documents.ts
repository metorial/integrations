...
export let searchDocumentsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    query: z.record(z.string(), z.any()).optional().describe('Elasticsearch Query DSL query object (e.g. { "match": { "title": "hello" } })'),
    aggregations: z.record(z.string(), z.any()).optional().describe('Aggregation definitions for analytics'),
    sort: z.array(z.any()).optional().describe('Sort criteria (e.g. [{ "timestamp": "desc" }])'),
    ...
    ]).optional().describe('Control which source fields are returned'),
    highlight: z.record(z.string(), z.any()).optional().describe('Highlight configuration for search results'),
  }))
  .output(z.object({
    ...
    hits: z.array(z.object({
      ...
      source: z.record(z.string(), z.any()).optional().describe('Document source fields'),
