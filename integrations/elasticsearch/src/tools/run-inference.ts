...
export let runInferenceTool = SlateTool.create(spec, {
  ...
  .input(
    z.object({
      action: z
        .enum(['perform', 'list', 'get', 'create', 'delete'])
    ...
    switch (ctx.input.action) {
      case 'perform': {
        ...
        if (!ctx.input.inferenceId)
          throw new Error('inferenceId is required for perform action');
      ...
      case 'create': {
        ...
        if (!ctx.input.inferenceId)
          throw new Error('inferenceId is required for create action');
        if (!ctx.input.endpointConfig)
          throw new Error('endpointConfig is required for create action');
      ...
      case 'delete': {
        ...
        if (!ctx.input.inferenceId)
          throw new Error('inferenceId is required for delete action');
