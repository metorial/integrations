...
export let manageWatchTool = SlateTool.create(
  spec,
  {
    name: 'Manage Watch',
    key: 'manage_watch',
  ...
  .input(z.object({
    action: z.enum(['create', 'get', 'delete', 'execute', 'activate', 'deactivate']).describe('Watch action to perform'),
    watchId: z.string().describe('The watch identifier'),
    trigger: z.record(z.string(), z.any()).optional().describe('Schedule trigger (e.g., { "schedule": { "interval": "10m" } })'),
    watchInput: z.record(z.string(), z.any()).optional().describe('Input query/data source for the watch'),
    condition: z.record(z.string(), z.any()).optional().describe('Condition that determines if actions should fire'),
    actions: z.record(z.string(), z.any()).optional().describe('Actions to execute when the condition is met (email, webhook, index, logging)'),
    metadata: z.record(z.string(), z.any()).optional().describe('Optional metadata to attach to the watch'),
  }))
  .output(z.object({
    watchId: z.string().describe('The watch identifier'),
    acknowledged: z.boolean().optional().describe('Whether the request was acknowledged'),
    watchStatus: z.record(z.string(), z.any()).optional().describe('Watch status information'),
    executionResult: z.record(z.string(), z.any()).optional().describe('Results from watch execution'),
    watchDefinition: z.record(z.string(), z.any()).optional().describe('Full watch definition'),
  }))
