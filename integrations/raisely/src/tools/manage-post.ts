...
export let managePost = SlateTool.create(
  ...
  .input(z.object({
    ...
    profileUuid: z.string().optional().describe('Profile UUID (required for list and create)'),
    ...
    limit: z.number().optional().describe('Maximum number of posts to return (for list action)'),
    offset: z.number().optional().describe('Number of posts to skip (for list action)'),
  }))
  .output(z.object({
    posts: z.array(z.record(z.string(), z.any())).optional().describe('List of posts (for list action)'),
    post: z.record(z.string(), z.any()).optional().describe('Post object (for create/update actions)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      let result = await client.createPost(ctx.input.campaignUuid, ctx.input.profileUuid, data);
