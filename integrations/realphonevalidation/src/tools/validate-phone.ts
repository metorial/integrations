...
export let validatePhone = SlateTool.create(
  ...
  .input(z.object({
    ...
    mode: z.enum(['full', 'standard']).default('full').describe('Validation mode: "full" returns caller name and subscriber type; "standard" returns connection status and phone type only'),
  }))
  .output(z.object({
    ...
    callerName: z.string().optional().describe('Subscriber name associated with the phone number (full mode only)'),
    callerType: z.string().optional().describe('Subscriber type: Consumer or Business (full mode only)'),
    errorText: z.string().optional().describe('Error message if validation encountered an issue'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.mode === 'full') {
      ...
      let errorText = normalizeErrorText(result.error_text);
      let phoneType = result.iscell === 'Y' ? 'Mobile' : result.iscell === 'V' ? 'VoIP' : result.iscell === 'N' ? 'Landline' : result.iscell;
