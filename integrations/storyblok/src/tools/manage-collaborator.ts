...
export let manageCollaborator = SlateTool.create(
  ...
  .input(z.object({
    ...
    email: z.string().optional().describe('Email address of the user to add (required for add)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of collaborators (for list action)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    // action === 'remove'
    if (!ctx.input.collaboratorId) throw new Error('collaboratorId is required to remove a collaborator');
