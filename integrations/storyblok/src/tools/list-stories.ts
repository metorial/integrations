...
export let listStories = SlateTool.create(
  ...
  .input(z.object({
    ...
    perPage: z.number().optional().describe('Number of stories per page (default: 25, max: 100)'),
    searchTerm: z.string().optional().describe('Search term to filter stories by name'),
    sortBy: z.string().optional().describe('Field to sort by, e.g. "created_at:desc", "name:asc", "published_at:desc"'),
    withTag: z.string().optional().describe('Filter by tag name'),
    startsWith: z.string().optional().describe('Filter by full_slug prefix, e.g. "blog/" for all blog stories'),
    containComponent: z.string().optional().describe('Filter by component name used in content'),
    ...
    byUuids: z.string().optional().describe('Comma-separated UUIDs to fetch specific stories'),
  }))
  .output(z.object({
