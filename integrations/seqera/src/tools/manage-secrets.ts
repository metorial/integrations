...
export let manageSecrets = SlateTool.create(
  ...
  .input(z.object({
    ...
    value: z.string().optional().describe('Secret value (required for create, optional for update)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      if (!ctx.input.name || !ctx.input.value) throw new Error('name and value are required to create a secret');
