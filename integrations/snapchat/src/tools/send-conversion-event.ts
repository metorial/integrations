...
export let sendConversionEvent = SlateTool.create(spec, {
      ...
      events: z
        .array(
          z.object({
            ...
            eventTime: z
              .string()
              .describe('Event timestamp in ISO 8601 format or Unix epoch milliseconds'),
  ...
  .handleInvocation(async ctx => {
    ...
    let formattedEvents = ctx.input.events.map(event => {
      ...
      if (event.userData.hashedMobileAdId)
        userData.hashed_maids = event.userData.hashedMobileAdId;
      ...
      if (event.customData) {
        ...
        if (event.customData.transactionId)
          customData.transaction_id = event.customData.transactionId;
        if (event.customData.itemCategory)
          customData.item_category = event.customData.itemCategory;
        if (event.customData.itemIds) customData.item_ids = event.customData.itemIds;
        if (event.customData.numberOfItems !== undefined)
          customData.number_items = event.customData.numberOfItems;
