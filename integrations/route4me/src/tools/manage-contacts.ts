...
export let createContact = SlateTool.create(
  ...
  .input(z.object({
    ...
    customFields: z.record(z.string(), z.string()).optional().describe('Custom key-value metadata'),
  }))
  .output(z.object({
...
export let getContacts = SlateTool.create(
  ...
  .input(z.object({
    addressId: z.string().optional().describe('Specific address ID(s) to retrieve (comma-separated)'),
