...
let contentBlockSchema: z.ZodType<Record<string, unknown>> = z.record(z.string(), z.unknown()).describe('Content block (text, image, tool_use, tool_result, etc.)');
...
let toolChoiceSchema = z.record(z.string(), z.unknown()).optional().describe('Tool choice configuration: { type: "auto" }, { type: "any" }, { type: "tool", name: "..." }, or { type: "none" }');

let thinkingSchema = z.record(z.string(), z.unknown()).optional().describe('Extended thinking config: { type: "enabled", budget_tokens: 1024 }, { type: "disabled" }, or { type: "adaptive" }');
...
Provide a conversation history as messages and configure model parameters to control the response.`,
  ...
  .input(z.object({
    model: z.string().describe('Claude model ID, e.g. "claude-sonnet-4-5-20250929", "claude-haiku-4-5"'),
    ...
    system: z.string().optional().describe('System prompt to set context and instructions for Claude'),
    temperature: z.number().min(0).max(1).optional().describe('Sampling temperature (0-1). Lower values are more deterministic'),
    ...
    tools: z.array(toolDefinitionSchema).optional().describe('Tool definitions for function calling'),
  ...
  .output(z.object({
    ...
    content: z.array(z.record(z.string(), z.unknown())).describe('Response content blocks (text, tool_use, thinking, etc.)'),
    model: z.string().describe('Model that generated the response'),
    stopReason: z.string().describe('Reason the model stopped: "end_turn", "max_tokens", "stop_sequence", or "tool_use"'),
    stopSequence: z.string().optional().describe('The stop sequence that caused the model to stop, if applicable'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let messages = ctx.input.messages as Array<{ role: 'user' | 'assistant'; content: string | Array<Record<string, unknown>> }>;
    ...
    let summary = textContent.length > 200 ? textContent.substring(0, 200) + '...' : textContent;
