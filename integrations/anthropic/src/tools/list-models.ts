...
Optionally fetch details for a specific model by providing its ID.`,
  ...
  .input(z.object({
    modelId: z.string().optional().describe('Specific model ID to retrieve details for. If omitted, lists all available models.'),
  ...
  .output(z.object({
    models: z.array(z.record(z.string(), z.unknown())).optional().describe('List of available models (when listing)'),
    model: z.record(z.string(), z.unknown()).optional().describe('Single model details (when fetching by ID)'),
    hasMore: z.boolean().optional().describe('Whether more models are available for pagination'),
  }))
