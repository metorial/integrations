...
export let manageOrganizationMembers = SlateTool.create(
  spec,
  {
...
Use **action** to specify the operation. Requires an Admin API key (sk-ant-admin...).`,
  ...
  .input(z.object({
    action: z.enum([
      'list_members', 'update_member', 'remove_member',
      'invite', 'list_invites', 'delete_invite',
  ...
  .output(z.object({
    members: z.array(z.record(z.string(), z.unknown())).optional().describe('List of organization members'),
    invites: z.array(z.record(z.string(), z.unknown())).optional().describe('List of invitations'),
    member: z.record(z.string(), z.unknown()).optional().describe('Updated or invited member details'),
