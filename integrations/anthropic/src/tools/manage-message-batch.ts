...
let batchRequestSchema = z.object({
  ...
  params: z.record(z.string(), z.unknown()).describe('Standard Messages API parameters (model, max_tokens, messages, etc.)'),
});

let requestCountsSchema = z.object({
  processing: z.number(),
  succeeded: z.number(),
  errored: z.number(),
  canceled: z.number(),
...
let batchOutputSchema = z.object({
  ...
  processingStatus: z.string().optional().describe('Current status: queued, in_progress, succeeded, failed, expired, canceled'),
...
export let manageMessageBatch = SlateTool.create(
  spec,
  {
...
Use **action** to specify the operation: "create", "get", "list", or "cancel".`,
  ...
  .input(z.object({
    ...
    requests: z.array(batchRequestSchema).optional().describe('Batch requests (required for "create")'),
  ...
  .output(z.object({
    batch: batchOutputSchema.optional().describe('Single batch result (for create, get, cancel)'),
