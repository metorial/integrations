...
export let manageWorkspaces = SlateTool.create(
  spec,
  {
...
Use **action** to specify the operation. Requires an Admin API key (sk-ant-admin...).`,
  ...
  .input(z.object({
    action: z.enum([
      ...
      'add_member', 'list_members', 'update_member', 'remove_member',
    ]).describe('Operation to perform'),
    workspaceId: z.string().optional().describe('Workspace ID (required for most operations except create and list)'),
    ...
    workspaceRole: z.enum(['workspace_user', 'workspace_developer', 'workspace_admin', 'workspace_billing']).optional().describe('Workspace role (for add_member, update_member)'),
    includeArchived: z.boolean().optional().describe('Include archived workspaces in list results'),
  ...
  .output(z.object({
    ...
    workspaces: z.array(z.record(z.string(), z.unknown())).optional().describe('List of workspaces'),
    member: z.record(z.string(), z.unknown()).optional().describe('Workspace member details'),
    members: z.array(z.record(z.string(), z.unknown())).optional().describe('List of workspace members'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'update': {
        ...
        let workspace = await client.updateWorkspace(ctx.input.workspaceId, { name: ctx.input.workspaceName });
        return {
          output: { workspace },
      ...
      case 'add_member': {
        if (!ctx.input.workspaceId || !ctx.input.userId || !ctx.input.workspaceRole) {
          throw new Error('workspaceId, userId, and workspaceRole are required for "add_member"');
        }
        let member = await client.addWorkspaceMember(ctx.input.workspaceId, ctx.input.userId, ctx.input.workspaceRole);
        return {
          output: { member },
      ...
      case 'update_member': {
        if (!ctx.input.workspaceId || !ctx.input.userId || !ctx.input.workspaceRole) {
          throw new Error('workspaceId, userId, and workspaceRole are required for "update_member"');
        }
        let member = await client.updateWorkspaceMember(ctx.input.workspaceId, ctx.input.userId, ctx.input.workspaceRole);
        return {
          output: { member },
