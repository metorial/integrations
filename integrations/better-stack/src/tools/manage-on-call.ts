...
let onCallSchema = z.object({
  ...
  onCallNow: z.array(z.record(z.string(), z.any())).nullable().describe('Currently on-call members'),
...
export let manageOnCall = SlateTool.create(
  spec,
  {
    name: 'Manage On-Call',
    key: 'manage_on_call',
  ...
  .input(z.object({
    ...
    resourceId: z.string().optional().describe('Calendar or policy ID (required for get, update, delete)'),
    name: z.string().optional().describe('Name for the calendar or policy'),
    repeatCount: z.number().optional().describe('Number of escalation repeats (for policy create/update)'),
    repeatDelay: z.number().optional().describe('Delay between repeats in seconds (for policy create/update)'),
    steps: z.array(z.record(z.string(), z.any())).optional().describe('Escalation steps configuration (for policy create/update)'),
  ...
  .output(z.object({
    ...
    policies: z.array(escalationPolicySchema).optional().describe('List of escalation policies'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (resource === 'calendar') {
      if (action === 'list') {
        let result = await client.listOnCallCalendars({ page: ctx.input.page, perPage: ctx.input.perPage });
    ...
    // Escalation Policies
    if (action === 'list') {
      let result = await client.listEscalationPolicies({ page: ctx.input.page, perPage: ctx.input.perPage });
