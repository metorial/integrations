...
export let getRecentPosts = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    limit: z.number().min(1).max(100).optional().describe('Number of threads to return (1-100)'),
    startingAfter: z.string().optional().describe('Cursor for pagination — pass the last thread ID from the previous page'),
  }))
  .output(z.object({
