...
export let alertAction = SlateTool.create(
  spec,
  {
    name: 'Alert Action',
    key: 'alert_action',
  ...
  .input(z.object({
    ...
    action: z.enum(['close', 'acknowledge', 'unacknowledge', 'snooze', 'assign', 'escalate', 'add_note', 'add_tags', 'remove_tags']).describe('Action to perform on the alert'),
    note: z.string().optional().describe('Note to include with the action, or the note text for "add_note"'),
    user: z.string().optional().describe('Display name of the request owner'),
    source: z.string().optional().describe('Source of the action'),
    snoozeEndTime: z.string().optional().describe('End time for snooze in ISO 8601 format (required for "snooze" action)'),
    ownerUsername: z.string().optional().describe('Username to assign as owner (for "assign" action)'),
    ownerId: z.string().optional().describe('User ID to assign as owner (for "assign" action)'),
    escalationId: z.string().optional().describe('Escalation ID (for "escalate" action)'),
    escalationName: z.string().optional().describe('Escalation name (for "escalate" action)'),
    tags: z.array(z.string()).optional().describe('Tags to add or remove (for "add_tags" / "remove_tags" actions)'),
  }))
  .output(z.object({
    ...
    switch (ctx.input.action) {
      ...
      case 'escalate':
        if (!ctx.input.escalationId && !ctx.input.escalationName) {
          throw new Error('escalationId or escalationName is required for the escalate action.');
        }
