...
export let createAlert = SlateTool.create(
  ...
  .input(z.object({
    ...
    alias: z.string().optional().describe('Client-defined unique identifier for the alert, used for deduplication'),
    description: z.string().optional().describe('Detailed description of the alert (max 15000 characters)'),
    responders: z.array(responderSchema).optional().describe('Teams, users, escalations, or schedules to notify'),
    visibleTo: z.array(responderSchema).optional().describe('Teams and users the alert will be visible to without sending notifications'),
    actions: z.array(z.string()).optional().describe('Custom actions available for the alert'),
    tags: z.array(z.string()).optional().describe('Tags to attach to the alert'),
    details: z.record(z.string(), z.string()).optional().describe('Custom key-value properties for the alert'),
    entity: z.string().optional().describe('Entity the alert is related to (e.g., server name)'),
    source: z.string().optional().describe('Source of the alert'),
    priority: z.enum(['P1', 'P2', 'P3', 'P4', 'P5']).optional().describe('Alert priority level'),
