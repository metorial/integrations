...
export let updateAlert = SlateTool.create(
  spec,
  {
    name: 'Update Alert',
    key: 'update_alert',
  ...
  .input(z.object({
    ...
    priority: z.enum(['P1', 'P2', 'P3', 'P4', 'P5']).optional().describe('New priority level'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.message !== undefined) {
      let res = await client.updateAlertMessage(ctx.input.alertIdentifier, idType, { message: ctx.input.message });
      requestIds.push(res.requestId);
      updatedFields.push('message');
    }

    if (ctx.input.description !== undefined) {
      let res = await client.updateAlertDescription(ctx.input.alertIdentifier, idType, { description: ctx.input.description });
      requestIds.push(res.requestId);
      updatedFields.push('description');
    }

    if (ctx.input.priority !== undefined) {
      let res = await client.updateAlertPriority(ctx.input.alertIdentifier, idType, { priority: ctx.input.priority });
      requestIds.push(res.requestId);
      updatedFields.push('priority');
    }

    if (updatedFields.length === 0) {
      throw new Error('No fields provided to update. Provide at least one of: message, description, priority.');
    }
