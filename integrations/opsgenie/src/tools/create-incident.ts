...
export let createIncident = SlateTool.create(
  ...
  .input(z.object({
    ...
    })).optional().describe('Teams or users to notify about the incident'),
    ...
    priority: z.enum(['P1', 'P2', 'P3', 'P4', 'P5']).optional().describe('Incident priority (default P3)'),
    note: z.string().optional().describe('Note to add to the incident'),
    impactedServiceIds: z.array(z.string()).optional().describe('IDs of services impacted by this incident'),
    statusPageTitle: z.string().optional().describe('Title for the status page entry'),
    statusPageDetail: z.string().optional().describe('Detail text for the status page entry'),
    notifyStakeholders: z.boolean().optional().describe('Whether to send stakeholder notifications'),
  }))
  .output(z.object({
