...
export let webCrawl = SlateTool.create(
  spec,
  {
    name: 'Web Crawl',
    key: 'web_crawl',
  ...
  .input(z.object({
    ...
    crawlApiKey: z.string().describe('API key for the crawling service (Spider Cloud or Firecrawl)'),
    maxPages: z.number().optional().describe('Maximum number of pages to crawl (defaults to 50)'),
    service: z.enum(['spider', 'firecrawl']).optional().describe('Crawling service to use (defaults to spider)'),
  }))
  .output(z.object({
    results: z.array(crawlResultSchema).describe('Crawled page results with URLs and content'),
