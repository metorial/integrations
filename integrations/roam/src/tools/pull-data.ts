...
export let pullData = SlateTool.create(
  spec,
  {
...
Use this to get detailed information about a known page or block, including its content, children, references, and metadata.`,
  ...
  .input(z.object({
    selector: z.string().describe('Pull pattern/selector, e.g. "[*]" for all attributes, or "[:block/string :block/uid :block/children]"'),
    entityId: z.string().describe('Entity identifier, e.g. [:node/title "Page Name"] or [:block/uid "abc123"]'),
  }))
  .output(z.object({
