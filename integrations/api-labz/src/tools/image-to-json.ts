...
export let imageToJson = SlateTool.create(
  spec,
  {
    name: 'Image to JSON',
    key: 'image_to_json',
...
Useful for extracting data from screenshots, photos of documents, product images, charts, and other visual content.`,
  ...
  .input(z.object({
    schema: z.string().describe('JSON schema or template describing the structure of data to extract from the image'),
  ...
  .output(z.object({
    extractedJson: z.any().describe('Structured JSON data extracted from the image matching the provided schema'),
  }))
