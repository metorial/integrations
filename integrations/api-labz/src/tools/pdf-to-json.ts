...
export let pdfToJson = SlateTool.create(
  spec,
  {
    name: 'PDF to JSON',
    key: 'pdf_to_json',
...
Useful for extracting invoices, receipts, forms, contracts, and other structured documents into machine-readable data.`,
  ...
  .input(z.object({
    schema: z.string().describe('JSON schema or template describing the structure of data to extract, e.g. \'{"invoice_number": "string", "total": "number", "items": [{"name": "string", "price": "number"}]}\''),
  ...
  .output(z.object({
    extractedJson: z.any().describe('Structured JSON data extracted from the PDF matching the provided schema'),
  }))
