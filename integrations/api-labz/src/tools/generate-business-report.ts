...
export let generateBusinessReport = SlateTool.create(
  spec,
  {
...
Useful for creating summaries, trend analyses, KPI reports, and other data-driven business documents.`,
  ...
  .input(z.object({
    instructions: z.string().describe('Instructions for the report, e.g. "Summarize quarterly sales by region and highlight top performers"'),
    rawData: z.string().describe('The raw data to analyze, as a text string (CSV, JSON, or plain text)'),
  }))
  .output(z.object({
