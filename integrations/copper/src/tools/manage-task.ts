...
let taskOutputSchema = z.object({
  ...
  }).nullable().optional().describe('Related resource (entity linked to the task)'),
...
export let createTask = SlateTool.create(
  ...
  .input(z.object({
    ...
    relatedResourceId: z.number().optional().describe('ID of the entity to link this task to'),
    relatedResourceType: z.string().optional().describe('Type of related entity: "person", "company", "opportunity", "lead", or "project"'),
...
export let searchTasks = SlateTool.create(
  ...
  .input(z.object({
    ...
    statuses: z.array(z.string()).optional().describe('Filter by statuses: "Open" or "Completed"'),
