...
export let updateVideo = SlateTool.create(
  ...
  .input(z.object({
    ...
    publicStatsViewable: z.boolean().optional().describe('Whether public stats are viewable'),
    madeForKids: z.boolean().optional().describe('Whether the video is made for kids'),
    selfDeclaredMadeForKids: z.boolean().optional().describe('Self-declared made for kids status'),
    publishAt: z.string().optional().describe('Scheduled publish time (ISO 8601, only for private videos)')
  }))
  .output(z.object({
    videoId: z.string(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.title !== undefined || ctx.input.description !== undefined ||
        ctx.input.tags !== undefined || ctx.input.categoryId !== undefined ||
      ...
      if (ctx.input.defaultLanguage !== undefined) snippet.defaultLanguage = ctx.input.defaultLanguage;
    }

    if (ctx.input.privacyStatus !== undefined || ctx.input.license !== undefined ||
        ctx.input.embeddable !== undefined || ctx.input.publicStatsViewable !== undefined ||
        ctx.input.madeForKids !== undefined || ctx.input.selfDeclaredMadeForKids !== undefined ||
        ctx.input.publishAt !== undefined) {
      parts.push('status');
      status = {};
      if (ctx.input.privacyStatus !== undefined) status.privacyStatus = ctx.input.privacyStatus;
      ...
      if (ctx.input.publicStatsViewable !== undefined) status.publicStatsViewable = ctx.input.publicStatsViewable;
      if (ctx.input.madeForKids !== undefined) status.madeForKids = ctx.input.madeForKids;
      if (ctx.input.selfDeclaredMadeForKids !== undefined) status.selfDeclaredMadeForKids = ctx.input.selfDeclaredMadeForKids;
