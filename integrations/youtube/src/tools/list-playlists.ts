...
export let listPlaylists = SlateTool.create(
  spec,
  {
    name: 'List Playlists',
    key: 'list_playlists',
  ...
  .input(z.object({
    playlistId: z.string().optional().describe('Specific playlist ID(s) to retrieve (comma-separated)'),
    channelId: z.string().optional().describe('Channel ID to list playlists for'),
    mine: z.boolean().optional().describe('Set to true to list the authenticated user\'s playlists'),
