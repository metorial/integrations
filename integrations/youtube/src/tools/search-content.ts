...
export let searchContent = SlateTool.create(
  ...
  .input(z.object({
    ...
    type: z.array(z.enum(['video', 'channel', 'playlist'])).optional().describe('Restrict results to specific resource types'),
    ...
    publishedAfter: z.string().optional().describe('Filter results published after this ISO 8601 date'),
    publishedBefore: z.string().optional().describe('Filter results published before this ISO 8601 date'),
    regionCode: z.string().optional().describe('ISO 3166-1 alpha-2 country code to restrict results'),
    relevanceLanguage: z.string().optional().describe('ISO 639-1 language code for result relevance'),
    videoDuration: z.enum(['any', 'short', 'medium', 'long']).optional().describe('Filter by video duration'),
    videoDefinition: z.enum(['any', 'high', 'standard']).optional().describe('Filter by video definition (HD/SD)'),
    videoType: z.enum(['any', 'episode', 'movie']).optional().describe('Filter by video type'),
    eventType: z.enum(['completed', 'live', 'upcoming']).optional().describe('Filter by broadcast event type'),
    topicId: z.string().optional().describe('Freebase topic ID to filter results')
  }))
  .output(z.object({
    results: z.array(z.object({
      resourceKind: z.string().describe('Kind of resource: youtube#video, youtube#channel, or youtube#playlist'),
