...
let symbolResultSchema = z.object({
  ...
  instrumentType: z.string().optional().describe('Type of instrument (e.g., Common Stock, ETF, Digital Currency)'),
  country: z.string().optional().describe('Country of the exchange'),
  currency: z.string().optional().describe('Currency of the instrument'),
});

export let searchSymbols = SlateTool.create(
  spec,
  {
    name: 'Search Symbols',
    key: 'search_symbols',
...
Use this to find the correct symbol before making other data requests.`,
  ...
  .input(z.object({
    query: z.string().describe('Search query - can be a symbol, company name, or keyword (e.g., "Apple", "AAPL", "bitcoin")'),
    outputsize: z.number().optional().describe('Number of results to return (default: 30, max: 120)'),
  }))
  .output(z.object({
