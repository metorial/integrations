...
export let getMutualFundDetails = SlateTool.create(
  spec,
  {
...
Use this to analyze mutual fund composition, expense ratios, and underlying holdings.`,
  ...
  .output(z.object({
    ...
    profile: z.record(z.string(), z.any()).optional().describe('Fund profile including name, expense ratio, and other metadata'),
    holdings: z.array(z.record(z.string(), z.any())).optional().describe('Array of fund holdings with weights'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let [profileResult, holdingsResult] = await Promise.all([
      ...
      includeHoldings ? client.getMutualFundHoldings(baseParams).catch(() => null) : Promise.resolve(null),
    ]);
