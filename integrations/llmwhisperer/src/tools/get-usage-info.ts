...
export let getUsageInfo = SlateTool.create(
  spec,
  {
    name: 'Get Usage Info',
    key: 'get_usage_info',
  ...
  .output(z.object({
    ...
    monthlyQuota: z.number().describe('Monthly page quota. -1 if not applicable (free plans).'),
    currentPageCount: z.any().describe('Pages processed this month, broken down by mode (native_text, low_cost, high_quality, form).'),
    overagePageCount: z.number().describe('Number of pages exceeding the monthly quota. -1 if not applicable.'),
