...
let channelSchema = z.object({
  integration: z.enum(['email', 'webhook', 'pagerduty', 'slack']).describe('Alert channel type'),
  ...
  headers: z.record(z.string(), z.string()).optional().describe('Custom headers for webhook'),
  bodyTemplate: z.record(z.string(), z.unknown()).optional().describe('Custom body template for webhook'),
  triggerlimit: z.number().optional().describe('Number of lines that must match to trigger alert'),
  triggerinterval: z.string().optional().describe('Time interval for the trigger (e.g., "30", "1m", "15m")'),
  operator: z.string().optional().describe('Alert condition operator (presence, absence)'),
  immediate: z.string().optional().describe('Whether to send alert immediately ("true" or "false")'),
  terminal: z.string().optional().describe('Whether to include terminal output ("true" or "false")'),
...
export let createView = SlateTool.create(
  spec,
  {
    name: 'Create View',
    key: 'create_view',
  ...
  .input(z.object({
    ...
    levels: z.array(z.string()).optional().describe('Log levels to filter (e.g., error, warn, info)'),
    ...
    channels: z.array(channelSchema).optional().describe('Alert channels to attach to the view'),
...
export let updateView = SlateTool.create(
  spec,
  {
    name: 'Update View',
    key: 'update_view',
...
export let deleteView = SlateTool.create(
  spec,
  {
    name: 'Delete View',
    key: 'delete_view',
