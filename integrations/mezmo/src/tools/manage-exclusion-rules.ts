...
let exclusionRuleOutput = z.object({
  ...
  indexOnly: z.boolean().optional().describe('If true, matched lines are indexed but not stored'),
});
...
export let createExclusionRule = SlateTool.create(
  ...
  .input(z.object({
    ...
    indexOnly: z.boolean().optional().describe('If true, lines are indexed (searchable) but not stored long-term'),
  }))
