...
export let getSession = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeVisits: z.boolean().optional().default(false).describe('Whether to include visit history for this session'),
    includeNotes: z.boolean().optional().default(false).describe('Whether to include notes attached to this session'),
  }))
  .output(z.object({
    session: z.record(z.string(), z.unknown()).describe('Session data including metadata, sales status, revenue, and visitor info'),
    visits: z.array(z.record(z.string(), z.unknown())).optional().describe('Visit history for this session (if requested)'),
    notes: z.array(z.record(z.string(), z.unknown())).optional().describe('Notes attached to this session (if requested)'),
  }))
