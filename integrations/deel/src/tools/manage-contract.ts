...
export let manageContract = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['amend', 'sign', 'terminate']).describe('Action to perform on the contract'),
    amendmentDetails: z.record(z.string(), z.any()).optional().describe('For "amend": fields to update on the contract (e.g. compensation_details, scope_of_work)'),
    terminationDate: z.string().optional().describe('For "terminate": date of termination (YYYY-MM-DD)'),
    terminationReason: z.string().optional().describe('For "terminate": reason for termination'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'amend':
        result = await client.amendContract(ctx.input.contractId, ctx.input.amendmentDetails ?? {});
        return {
      ...
      case 'terminate': {
        ...
        if (ctx.input.terminationDate) terminationData.termination_date = ctx.input.terminationDate;
