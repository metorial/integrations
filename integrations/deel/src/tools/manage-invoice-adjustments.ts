...
export let manageInvoiceAdjustments = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    contractId: z.string().optional().describe('Contract ID (for "list" to filter by contract, and for "create")'),
    types: z.array(z.string()).optional().describe('For "list": filter by adjustment types (e.g. "bonus", "expense")'),
    ...
    adjustmentType: z.string().optional().describe('For "create": type of adjustment (e.g. "bonus", "commission", "deduction", "expense", "overtime")'),
    description: z.string().optional().describe('For "create": description of the adjustment'),
    recurring: z.boolean().optional().describe('For "create": whether this is a recurring adjustment'),
    adjustmentId: z.string().optional().describe('For "review": ID of the adjustment to review'),
  ...
  .output(z.object({
    adjustments: z.array(z.record(z.string(), z.any())).optional().describe('List of invoice adjustments (for "list")'),
    adjustment: z.record(z.string(), z.any()).optional().describe('Created or reviewed adjustment'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'create': {
        if (!ctx.input.contractId) throw new Error('contractId is required for creating an invoice adjustment');
      ...
      case 'review': {
        if (!ctx.input.adjustmentId) throw new Error('adjustmentId is required for reviewing an adjustment');
        if (!ctx.input.reviewStatus) throw new Error('reviewStatus is required for reviewing an adjustment');
