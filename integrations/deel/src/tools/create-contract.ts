...
export let createContract = SlateTool.create(
  ...
  .input(z.object({
    type: z.string().describe('Contract type: "ongoing_time_based", "pay_as_you_go_time_based", "payg_milestones", or "payg_task"'),
    ...
    frequency: z.string().optional().describe('Payment frequency: "monthly", "weekly", "semi_monthly"'),
    legalEntityId: z.string().optional().describe('Legal entity ID to associate the contract with'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.amount !== undefined || ctx.input.currencyCode || ctx.input.scale || ctx.input.frequency) {
      data.compensation_details = {};
      if (ctx.input.amount !== undefined) data.compensation_details.amount = ctx.input.amount;
      if (ctx.input.currencyCode) data.compensation_details.currency_code = ctx.input.currencyCode;
