...
export let createPaymentPage = SlateTool.create(
  spec,
  {
...
Amount is in the **smallest currency unit**. Leave amount empty to let the customer enter an amount.`,
  ...
  .input(z.object({
    ...
    amount: z.number().optional().describe('Fixed amount in smallest currency unit. Leave empty for customer-entered amount'),
    slug: z.string().optional().describe('URL slug for the page. Auto-generated from name if not provided'),
    redirectUrl: z.string().optional().describe('URL to redirect customers to after payment'),
...
export let listPaymentPages = SlateTool.create(
  spec,
  {
...
export let updatePaymentPage = SlateTool.create(
  spec,
  {
