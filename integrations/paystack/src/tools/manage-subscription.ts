...
export let createSubscription = SlateTool.create(
  ...
  .input(z.object({
    ...
    authorization: z.string().optional().describe('Authorization code from a previous successful transaction. Required if the customer has multiple authorizations'),
    startDate: z.string().optional().describe('When to start the subscription (ISO 8601). Defaults to now'),
  }))
  .output(z.object({
...
export let getSubscription = SlateTool.create(
  ...
  .output(z.object({
    subscriptionCode: z.string().describe('Subscription code'),
    status: z.string().describe('Subscription status (active, non-renewing, attention, completed, cancelled)'),
    amount: z.number().describe('Subscription amount'),
...
export let disableSubscription = SlateTool.create(
  spec,
  {
...
export let enableSubscription = SlateTool.create(
  spec,
  {
