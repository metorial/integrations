...
- **update**: Update order notes or tags`,
  ...
  .input(z.object({
    ...
    action: z.enum(['close', 'open', 'cancel', 'update']).describe('Action to perform on the order'),
    cancelReason: z.enum(['customer', 'fraud', 'inventory', 'declined', 'other']).optional().describe('Reason for cancellation (only for cancel action)'),
    sendEmail: z.boolean().optional().describe('Whether to notify the customer by email (only for cancel action)'),
    restock: z.boolean().optional().describe('Whether to restock inventory (only for cancel action)'),
