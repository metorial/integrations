...
export let createFulfillment = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    lineItemsByFulfillmentOrder: z.array(z.object({
      ...
      fulfillmentOrderLineItems: z.array(z.object({
        fulfillmentOrderLineItemId: z.string().describe('Line item ID within the fulfillment order'),
    ...
    notifyCustomer: z.boolean().optional().describe('Whether to send shipment notification email')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.trackingNumber || ctx.input.trackingUrl || ctx.input.trackingCompany) {
      ...
      if (ctx.input.trackingNumber) fulfillmentData.tracking_info.number = ctx.input.trackingNumber;
      if (ctx.input.trackingUrl) fulfillmentData.tracking_info.url = ctx.input.trackingUrl;
      if (ctx.input.trackingCompany) fulfillmentData.tracking_info.company = ctx.input.trackingCompany;
    }
