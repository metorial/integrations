...
export let listThemesTool = SlateTool.create(
  spec,
  {
    name: 'List Themes',
    key: 'list_themes',
...
Use theme IDs when generating content to control colors, fonts, and overall visual style.`,
  ...
  .output(z.object({
    themes: z.array(z.object({
      themeId: z.string().describe('Unique theme identifier to use in content generation'),
      name: z.string().describe('Display name of the theme'),
      type: z.string().describe('Theme type: "standard" or "custom"'),
      colorKeywords: z.array(z.string()).optional().describe('Color descriptors for the theme'),
      toneKeywords: z.array(z.string()).optional().describe('Tone/mood descriptors for the theme'),
