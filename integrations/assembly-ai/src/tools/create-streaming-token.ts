...
Use this to securely authenticate client-side streaming without exposing your main API key. Each token is single-use and valid for one streaming session.`,
  ...
  .input(z.object({
    expiresInSeconds: z.number().min(1).max(600).describe('Token validity duration in seconds (1-600).'),
    maxSessionDurationSeconds: z.number().min(60).max(10800).optional().describe('Maximum streaming session duration in seconds (60-10800). Defaults to 10800.'),
  }))
  .output(z.object({
