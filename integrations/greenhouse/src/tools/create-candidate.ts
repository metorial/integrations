...
export let createCandidateTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    websiteAddresses: z.array(z.object({
      value: z.string().describe('Website URL'),
      type: z.enum(['personal', 'company', 'portfolio', 'blog', 'other']).describe('Website type'),
    })).optional().describe('Website addresses'),
    socialMediaAddresses: z.array(z.object({
    ...
    jobIds: z.array(z.string()).optional().describe('Job IDs to create applications for this candidate'),
  }))
  .output(z.object({
