...
export let listOffersTool = SlateTool.create(
  ...
  .input(z.object({
    applicationId: z.string().optional().describe('If provided, list offers only for this application'),
    page: z.number().optional().describe('Page number for pagination (starts at 1)'),
    perPage: z.number().optional().describe('Number of results per page (max 500, default 50)'),
    status: z.enum(['created', 'sent', 'accepted', 'rejected', 'deprecated']).optional().describe('Filter by offer status'),
    createdAfter: z.string().optional().describe('Only return offers created after this ISO 8601 timestamp'),
    createdBefore: z.string().optional().describe('Only return offers created before this ISO 8601 timestamp'),
    updatedAfter: z.string().optional().describe('Only return offers updated after this ISO 8601 timestamp'),
    updatedBefore: z.string().optional().describe('Only return offers updated before this ISO 8601 timestamp'),
  }))
  .output(z.object({
