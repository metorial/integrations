...
export let sendHecEvent = SlateTool.create(
  spec,
  {
    name: 'Send HEC Event',
    key: 'send_hec_event',
  ...
  .input(z.object({
    ...
    })).min(1).describe('Array of events to send')
  }))
  .output(z.object({
...
export let sendHecRawEvent = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    channel: z.string().optional().describe('Request channel GUID (required for raw endpoint)')
  }))
  .output(z.object({
    text: z.string().describe('Response status text'),
    code: z.number().describe('Response status code (0 = success)')
