...
export let manageOrderShipment = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    shippingProvider: z.string().optional().describe('Shipping provider (e.g., "fedex", "ups", "usps")'),
    trackingCarrier: z.string().optional().describe('Tracking carrier name'),
    items: z.array(shipmentItemSchema).optional().describe('Items to include in the shipment'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.shippingProvider) shipmentData.shipping_provider = ctx.input.shippingProvider;
    ...
    if (ctx.input.action === 'update' && ctx.input.shipmentId) {
      let shipment = await client.updateOrderShipment(ctx.input.orderId, ctx.input.shipmentId, shipmentData);
      return {
        output: { shipment },
