...
export let getOrder = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeProducts: z.boolean().optional().describe('Whether to also fetch the order line items/products'),
    includeShipments: z.boolean().optional().describe('Whether to also fetch order shipments'),
    includeTransactions: z.boolean().optional().describe('Whether to also fetch order transactions'),
  }))
  .output(z.object({
