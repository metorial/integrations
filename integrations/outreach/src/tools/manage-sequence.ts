...
Sequences are multi-step campaigns that automate prospect engagement through emails, calls, and tasks.`,
  ...
  .input(z.object({
    ...
    sequenceType: z.enum(['interval', 'date']).optional().describe('Type: "interval" (days between steps) or "date" (specific dates)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let resource = await client.updateSequence(ctx.input.sequenceId, attributes, relationships);
    let flat = flattenResource(resource);
    return {
      output: {
        sequenceId: flat.id,
