...
let subscriptionContractSchema = z.object({
...
export let listSubscriptionContracts = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    searchTerm: z.string().optional().describe('Search by product name, variant name, order number, or customer name'),
  }))
  .output(z.object({
