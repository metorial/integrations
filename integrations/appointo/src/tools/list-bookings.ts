...
export let listBookings = SlateTool.create(
  ...
  .input(z.object({
    limit: z.number().min(1).max(100).optional().describe('Maximum number of bookings to return (max 100, default 100)'),
    ...
    searchTerm: z.string().optional().describe('Search by product name, customer name, email, phone, or order name'),
  }))
  .output(z.object({
