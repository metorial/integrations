...
let configSchema = z.object({
  timezone: z.string().optional().describe('Timezone for the appointment (e.g., "America/New_York")'),
  ...
  startBufferTime: z.number().optional().describe('Buffer time in minutes before the appointment'),
  endBufferTime: z.number().optional().describe('Buffer time in minutes after the appointment'),
...
}).passthrough().optional().describe('Appointment configuration settings');

let slotSchema = z.object({
...
let dayAvailabilitySchema = z.object({
...
let dateOverrideSchema = z.object({
...
export let updateAppointmentConfig = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    config: configSchema,
    availabilities: z.array(dayAvailabilitySchema).optional().describe('Weekly availability schedule'),
    dateOverrides: z.array(dateOverrideSchema).optional().describe('Date-specific availability overrides'),
  }))
  .output(z.object({
