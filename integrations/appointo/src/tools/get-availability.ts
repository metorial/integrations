...
let availabilitySlotSchema = z.object({
...
export let getAvailability = SlateTool.create(
  ...
  .input(z.object({
    ...
    teamMemberIds: z.array(z.number()).optional().describe('Filter by specific team member IDs'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let slots = Array.isArray(result) ? result : (result?.slots ?? result?.availability ?? result?.data ?? []);
