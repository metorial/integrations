...
export let createContact = SlateTool.create(
  ...
  .input(z.object({
    ...
    contactNameOnInvoices: z.boolean().optional().describe('Whether to show contact name on invoices'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.salesTaxRegistrationNumber) contactData.sales_tax_registration_number = ctx.input.salesTaxRegistrationNumber;
    if (ctx.input.locale) contactData.locale = ctx.input.locale;
    if (ctx.input.contactNameOnInvoices !== undefined) contactData.contact_name_on_invoices = ctx.input.contactNameOnInvoices;
    ...
    let name = contact.organisation_name || [contact.first_name, contact.last_name].filter(Boolean).join(' ');
