...
export let listTasks = SlateTool.create(
  ...
  .input(z.object({
    view: z.enum(['all', 'active', 'completed', 'hidden']).optional().describe('Filter tasks by status'),
    sort: z.enum(['name', '-name', 'project', '-project', 'billing_rate', '-billing_rate', 'created_at', '-created_at', 'updated_at', '-updated_at']).optional().describe('Sort order'),
