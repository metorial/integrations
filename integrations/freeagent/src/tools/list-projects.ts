...
export let listProjects = SlateTool.create(
  ...
  .input(z.object({
    view: z.enum(['active', 'completed', 'cancelled', 'hidden']).optional().describe('Filter projects by status'),
    sort: z.enum(['name', '-name', 'contact_name', '-contact_name', 'created_at', '-created_at', 'updated_at', '-updated_at']).optional().describe('Sort order'),
