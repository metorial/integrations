...
export let hostReputationTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    listRating: z.number().optional().describe('Only check lists matching this quality tier or better (1-3, where 1 is highest quality)'),
    zones: z.string().optional().describe('Only check specific DNSBL zones/hosts (comma-separated)')
  }))
  .output(z.object({
