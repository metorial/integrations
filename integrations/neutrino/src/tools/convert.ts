...
export let convertTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    fromType: z.string().describe('Source currency or unit code (e.g., "USD", "km", "celsius")'),
    toType: z.string().describe('Target currency or unit code (e.g., "EUR", "miles", "fahrenheit")'),
    historicalDate: z.string().optional().describe('Date for historical conversion rates (YYYY-MM-DD, YYYY-MM, or YYYY)')
  }))
  .output(z.object({
