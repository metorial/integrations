...
Use POST mode to submit form data or interact with target page APIs. Optionally execute custom JavaScript on the page before retrieving the HTML.`,
  ...
  .input(z.object({
    url: z.string().describe('The full URL of the webpage to scrape (must include protocol, e.g. https://).'),
    method: z.enum(['GET', 'POST']).optional().describe('HTTP method to use. Defaults to GET. Use POST for form submissions.'),
    requestBody: z.string().optional().describe('Request body for POST requests (e.g. form data or JSON string).'),
