...
export let manageBookmarks = SlateTool.create(
  ...
  .input(z.object({
    ...
    scope: z.enum(['personal', 'group']).default('personal').describe('Whether this is a personal or group bookmark'),
    ...
    collectionId: z.string().optional().describe('Collection ID to organize the bookmark into'),
  ...
  .output(z.object({
    bookmarks: z.array(bookmarkSchema).optional().describe('List of bookmarks (for list action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'list') {
      ...
      let bookmarks = scope === 'group' && groupId
