...
- \`hasAttachment:true\` — Emails with attachments`,
  ...
  .input(z.object({
    ...
    searchKey: z.string().describe('Search query using Zoho Mail search syntax (e.g. "from:user@example.com", "subject:meeting", "newMails")'),
    ...
    receivedBefore: z.number().optional().describe('Unix timestamp in milliseconds — only return emails received before this time'),
    includeTo: z.boolean().optional().default(true).describe('Include recipient details in results'),
  }))
  .output(z.object({
