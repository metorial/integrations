...
export let bestPodcasts = SlateTool.create(
  spec,
  {
    name: 'Best Podcasts',
    key: 'best_podcasts',
...
Filter by genre, region, publisher region, language, and sort order. Use the "Get Genres & Regions" tool to discover available genre IDs and region codes.`,
  ...
  .input(z.object({
    genreId: z.string().optional().describe('Genre ID to filter by. Omit for overall best podcasts.'),
    page: z.number().optional().describe('Page number for pagination.'),
    region: z.string().optional().describe('Country code for regional rankings (e.g., "us", "gb"). Defaults to "us".'),
    ...
    sort: z.enum(['recent_added_first', 'oldest_added_first', 'recent_published_first', 'oldest_published_first', 'listen_score']).optional().describe('Sort order for results.'),
