...
export let getEpisode = SlateTool.create(
  spec,
  {
    name: 'Get Episode',
    key: 'get_episode',
...
Optionally include transcript if available (PRO/ENTERPRISE plans).`,
  ...
  .input(z.object({
    ...
    showTranscript: z.boolean().optional().describe('Set to true to include transcript if available. Requires PRO/ENTERPRISE plan.'),
  }))
  .output(z.object({
    ...
    transcript: z.string().optional().describe('Transcript text if available and requested.'),
    podcast: z.object({
