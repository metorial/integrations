...
export let whatsappGroupEventTrigger = SlateTrigger.create(
  spec,
  {
    name: 'WhatsApp Group Event',
    key: 'whatsapp_group_event',
    description: 'Triggers on WhatsApp group activity: messages received, reactions, and membership changes (joins, leaves, removals).',
  }
)
  .input(z.object({
    ...
    participantNumber: z.string().optional().describe('Phone number of the participant involved'),
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      return {
        inputs: [{
          ...
          participantNumber: msg.from_number || msg.from || msg.phone_number || data.participant_number,
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        type: eventMap[ctx.input.eventType] || `whatsapp_group.${ctx.input.eventType.split('.').pop()}`,
