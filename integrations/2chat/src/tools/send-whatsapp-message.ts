...
export let sendWhatsAppMessage = SlateTool.create(
  ...
  .input(z.object({
    fromNumber: z.string().describe('Your connected WhatsApp number (with country code, e.g., +1234567890)'),
    toNumber: z.string().describe('Recipient WhatsApp number (with country code, e.g., +1234567890)'),
    text: z.string().optional().describe('Text content of the message or caption for media'),
    mediaUrl: z.string().optional().describe('Publicly accessible URL of media to send (image, audio, video, PDF, etc.)'),
  }))
  .output(z.object({
