...
export let publishEvent = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'An event must have at least one ticket class before it can be published.',
    ],
    tags: {
      destructive: false,
    },
  ...
  .input(z.object({
    ...
    action: z.enum(['publish', 'unpublish']).describe('Whether to publish or unpublish the event.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: ctx.input.action === 'publish'
