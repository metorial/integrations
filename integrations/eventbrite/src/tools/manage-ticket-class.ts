...
export let manageTicketClass = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    cost: z.string().optional().describe('Price string (e.g., "USD,1000" for $10.00). Omit for free tickets.'),
    ...
    hidden: z.boolean().optional().describe('Whether the ticket class is hidden from public view.'),
  }))
  .output(z.object({
    ticketClasses: z.array(ticketClassSchema).optional().describe('List of ticket classes (for list action).'),
    ticketClass: ticketClassSchema.optional().describe('The created or updated ticket class.'),
