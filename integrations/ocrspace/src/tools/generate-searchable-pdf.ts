...
let languageEnum = z.enum([
  'ara', 'bul', 'chs', 'cht', 'hrv', 'cze', 'dan', 'dut', 'eng', 'fin',
  'fre', 'ger', 'gre', 'hun', 'kor', 'ita', 'jpn', 'pol', 'por', 'rus',
  'slv', 'spa', 'swe', 'tha', 'tur', 'ukr', 'vnm', 'auto',
]);

export let generateSearchablePdf = SlateTool.create(
  spec,
  {
...
Returns a download URL for the generated PDF (valid for **1 hour**) along with the extracted text.`,
  ...
  .input(z.object({
    ...
    base64Image: z.string().optional().describe('Base64-encoded image/PDF with content-type prefix (e.g. "data:image/png;base64,...")'),
    language: languageEnum.optional().default('eng').describe('Language code for OCR recognition'),
    ocrEngine: z.enum(['1', '2']).optional().default('1').describe('OCR engine to use: "1" (fastest) or "2" (complex backgrounds). Engine 3 is not supported for PDF generation.'),
    hideTextLayer: z.boolean().optional().default(false).describe('Hide the text layer in the output PDF (text is present but not visible)'),
    detectOrientation: z.boolean().optional().default(false).describe('Auto-rotate the image before processing'),
    scale: z.boolean().optional().default(false).describe('Upscale low-resolution images to improve OCR results'),
  }))
  .output(z.object({
    searchablePdfUrl: z.string().describe('Download URL for the generated searchable PDF (valid for 1 hour)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!result.searchablePdfUrl) {
      throw new Error('Searchable PDF generation failed — no download URL was returned. Ensure you are not using Engine 3.');
    }
