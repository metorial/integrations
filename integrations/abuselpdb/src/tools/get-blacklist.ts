...
export let getBlacklist = SlateTool.create(
  spec,
  {
    name: 'Get Blacklist',
    key: 'get_blacklist',
...
Use this to populate firewall blocklists or analyze global threat trends.`,
  ...
  .input(z.object({
    confidenceMinimum: z.number().min(25).max(100).optional().describe('Minimum abuse confidence score threshold (25–100, default 100)'),
    limit: z.number().optional().describe('Maximum number of IPs to return (depends on plan tier)'),
    onlyCountries: z.string().optional().describe('Comma-separated ISO 3166 alpha-2 country codes to include'),
    exceptCountries: z.string().optional().describe('Comma-separated ISO 3166 alpha-2 country codes to exclude'),
    ipVersion: z.union([z.literal(4), z.literal(6)]).optional().describe('Filter by IP version: 4 (IPv4) or 6 (IPv6)'),
  }))
  .output(z.object({
