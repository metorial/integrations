...
export let listAgents = SlateTool.create(
  ...
  .input(z.object({
    includeUsers: z.boolean().optional().default(true).describe('Include agents/users in the response'),
    includeTeams: z.boolean().optional().default(true).describe('Include teams in the response'),
    withInvited: z.boolean().optional().describe('Include invited (unconfirmed) users'),
    roles: z.string().optional().describe('Filter users by role (comma-separated: admin, agent, collaborator, customer)')
  }))
  .output(z.object({
