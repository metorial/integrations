...
export let updateUser = SlateTool.create(
  ...
  .input(z.object({
    ...
    metadata: z.record(z.string(), z.any()).optional().describe('Key-value metadata to set on the user (replaces existing metadata)'),
  }))
  .output(z.object({
...
export let deleteUser = SlateTool.create(
  ...
  .input(z.object({
    ...
    courseAction: z.enum(['reallocate', 'delete', 'ignore']).describe('How to handle courses owned by this user'),
    reallocateTo: z.string().optional().describe('User identifier to reallocate courses to (required when courseAction is "reallocate")'),
    clientIdentifier: z.string().optional().describe('Filter user deletion by client identifier'),
  }))
  .output(z.object({
