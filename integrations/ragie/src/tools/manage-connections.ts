...
Use this to manage the lifecycle of your data source integrations.`,
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'get_stats', 'update', 'enable', 'disable', 'sync', 'delete']).describe('Action to perform'),
    connectionId: z.string().optional().describe('Connection ID (required for all actions except "list")'),
    metadata: z.record(z.string(), z.any()).optional().describe('Connection metadata to update (for "update" action)'),
    partitionStrategy: z.string().optional().describe('Partition strategy to set (for "update" action)'),
    keepFiles: z.boolean().optional().describe('Whether to retain documents when deleting a connection'),
  }))
  .output(z.object({
