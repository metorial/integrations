...
Use this to add content to your Ragie knowledge base. Documents go through processing states and become retrievable once indexed.`,
  ...
  .input(z.object({
    source: z.enum(['url', 'raw']).describe('Source type: "url" for publicly accessible URL, "raw" for text/JSON data'),
    url: z.string().optional().describe('Publicly accessible URL to ingest (required when source is "url")'),
    rawData: z.union([z.string(), z.record(z.string(), z.any())]).optional().describe('Text or JSON data to ingest (required when source is "raw")'),
    name: z.string().optional().describe('Custom name for the document. Defaults to filename or timestamp.'),
    metadata: z.record(z.string(), z.any()).optional().describe('Key-value metadata to attach. Supports string, number, boolean, and string array values.'),
    mode: z.enum(['fast', 'hi_res']).optional().describe('Import mode. "fast" extracts text only (default), "hi_res" extracts images and tables.'),
    externalId: z.string().optional().describe('External reference identifier for the document'),
    partition: z.string().optional().describe('Partition to scope the document to. Overrides default partition from config.'),
  }))
  .output(z.object({
