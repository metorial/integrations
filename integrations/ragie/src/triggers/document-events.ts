...
export let documentEvents = SlateTrigger.create(
  spec,
  {
    ...
    key: 'document_events',
    description: 'Triggers when document status changes (indexed, keyword_indexed, ready, failed) or when a document is deleted. Also fires when entities are extracted from a document.',
  }
)
  .input(z.object({
    ...
    connectionId: z.string().nullable().optional().describe('Connection ID if from a connection'),
    ...
    instructionId: z.string().optional().describe('Instruction ID for entity_extracted events'),
  ...
  .output(z.object({
    ...
    error: z.string().nullable().optional().describe('Error details if document processing failed'),
