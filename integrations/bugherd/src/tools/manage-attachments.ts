...
export let addAttachment = SlateTool.create(
  spec,
  {
    name: 'Add Attachment',
    key: 'add_attachment',
...
export let deleteAttachment = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    let client = new BugherdClient(ctx.auth.token);
    await client.deleteAttachment(ctx.input.projectId, ctx.input.taskId, ctx.input.attachmentId);
