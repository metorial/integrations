...
export let groupChatReceived = SlateTrigger.create(
  spec,
  {
    name: 'Group Chat Received',
    key: 'group_chat_received',
    description: 'Triggers when a new chat message is posted in a specific Habitica group (party or guild).',
  },
)
  .input(z.object({
  ...
  .webhook({
    autoRegisterWebhook: async (ctx) => {
      ...
      if (!groupId) {
        throw new Error('User is not in a party. A group ID is required for group chat webhooks.');
      }
