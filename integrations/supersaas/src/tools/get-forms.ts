...
export let listFormTemplatesTool = SlateTool.create(
  spec,
  {
...
export let getFormEntriesTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    formEntryId: z.string().optional().describe('Retrieve a single form entry by its ID. If provided, other filters are ignored.'),
    formTemplateId: z.string().optional().describe('Form template ID to retrieve entries for. Required when not using formEntryId.'),
    from: z.string().optional().describe('Only return forms updated after this time (ISO format YYYY-MM-DD HH:MM:SS)'),
    userId: z.string().optional().describe('Filter by user ID, username, or foreign key'),
    limit: z.number().optional().describe('Maximum number of results (default 100)'),
    offset: z.number().optional().describe('Pagination offset'),
  }))
  .output(z.object({
    formEntries: z.array(formEntrySchema).optional().describe('List of form entries (when querying multiple)'),
...
let mapFormEntry = (data: any): any => {
  ...
  return {
    ...
    appointmentId: data.reservation_process_id != null ? String(data.reservation_process_id) : undefined,
