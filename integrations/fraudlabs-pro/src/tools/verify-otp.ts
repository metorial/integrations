...
export let verifyOtp = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'Use the smsTransactionId returned from the Send SMS Verification tool.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    smsTransactionId: z.string().describe('Transaction ID from the Send SMS Verification response'),
