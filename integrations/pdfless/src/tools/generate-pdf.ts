...
Templates support Handlebars-like expressions for variable interpolation, conditionals, loops, and barcode generation.`,
  ...
  .input(z.object({
    templateId: z.string().describe('Unique identifier of the template to use for PDF generation'),
    templatePayload: z.record(z.string(), z.unknown()).describe('JSON object containing dynamic data to populate template variables'),
    referenceId: z.string().optional().describe('Caller-defined reference identifier for tracking purposes'),
    encryption: z.object({
      userPassword: z.string().optional().describe('Password required to open the PDF document'),
      ownerPassword: z.string().optional().describe('Owner password to restrict permissions on the PDF'),
      allowPrinting: z.boolean().optional().describe('Allow printing the document (defaults to true)'),
      allowModifying: z.boolean().optional().describe('Allow modifying the document (defaults to true)'),
      allowContentCopying: z.boolean().optional().describe('Allow copying content from the document (defaults to true)'),
      allowModifyAnnotations: z.boolean().optional().describe('Allow modifying annotations (defaults to true)'),
      allowFormFilling: z.boolean().optional().describe('Allow filling in forms (defaults to true)'),
      allowScreenReaders: z.boolean().optional().describe('Allow screen reader access (defaults to true)'),
      allowDocumentAssembly: z.boolean().optional().describe('Allow document assembly (defaults to true)'),
    }).optional().describe('Encryption and permission settings for the generated PDF'),
  }))
  .output(z.object({
    ...
    downloadUrl: z.string().describe('Temporary URL to download the generated PDF (expires in 10 minutes)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    ctx.info({ message: 'PDF generated successfully', templateId: result.templateId, downloadUrl: result.downloadUrl });
