...
import { buildReplyHeaders, defaultReplySubject, defaultReplyTo, pickReplyTarget } from '../lib/reply-context';
...
export let sendReply = SlateTool.create(
  ...
  .input(z.object({
    ...
    replyToMessageId: z.string().optional().describe('Parent message ID; defaults to the latest message in the thread.'),
    body: z.string().describe('Reply body (plain text or HTML).'),
    isHtml: z.boolean().optional().default(false),
    to: z.array(z.string()).optional().describe('Recipients; defaults from Reply-To/From of the parent message.'),
