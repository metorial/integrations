...
export let manageSharedLabels = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    visibility: z.enum(['organization', 'delegates']).optional().describe('Who can see the label'),
    ...
    limit: z.number().min(1).max(200).optional().describe('Max labels to return (list only)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.shareWithOrganization !== undefined) fields.share_with_organization = ctx.input.shareWithOrganization;
    ...
    if (ctx.input.action === 'create') {
      if (!ctx.input.organizationId) throw new Error('organizationId is required for creating labels');
