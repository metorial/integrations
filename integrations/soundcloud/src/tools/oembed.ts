...
export let getOEmbed = SlateTool.create(
  ...
  .input(z.object({
    ...
    maxWidth: z.number().optional().describe('Maximum width of the embedded player in pixels'),
    maxHeight: z.number().optional().describe('Maximum height of the embedded player in pixels (default: 166 for tracks, 450 for sets)'),
    autoPlay: z.boolean().optional().describe('Enable autoplay on page load (default false)'),
    showComments: z.boolean().optional().describe('Show timed comments in the player (default true)'),
    color: z.string().optional().describe('Primary widget color as hex triplet without # (e.g., "ff0066")')
  }))
  .output(z.object({
