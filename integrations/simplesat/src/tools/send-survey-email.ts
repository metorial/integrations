...
export let sendSurveyEmail = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    customerCustomAttributes: z.record(z.string(), z.string()).optional().describe('Custom customer attributes'),
    teamMemberId: z.string().optional().describe('ID of the team member associated with this interaction'),
    ...
    ticketCustomAttributes: z.record(z.string(), z.string()).optional().describe('Custom ticket attributes'),
  }))
  .output(z.object({
    result: z.record(z.string(), z.unknown()).describe('API response from sending the survey email'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });

    let teamMember = (ctx.input.teamMemberId || ctx.input.teamMemberEmail || ctx.input.teamMemberName)
      ? {
    ...
    let ticket = (ctx.input.ticketId || ctx.input.ticketSubject || ctx.input.ticketCustomAttributes)
      ? {
