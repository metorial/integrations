...
export let getTranscript = SlateTool.create(
  ...
  .input(z.object({
    ...
    format: z.enum(['text', 'json']).optional().describe('Output format: "text" for plain text, "json" for structured JSON with timestamps (default: "text")'),
    translationLanguage: z.string().optional().describe('Language code to retrieve translated transcript (must have been enabled during job submission)'),
    includeSummary: z.boolean().optional().describe('Also fetch the transcript summary (must have been enabled during job submission)'),
  }))
  .output(z.object({
    transcript: z.string().optional().describe('Plain text transcript (when format is "text")'),
    ...
    })).optional().describe('Structured transcript with speaker labels and timestamps (when format is "json")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.translationLanguage) {
      translatedTranscript = await client.getTranslatedTranscriptText(ctx.input.jobId, ctx.input.translationLanguage);
    }
    ...
    let snippetText = transcript || (monologues
      ? monologues.map((m) => m.elements.map((e) => e.value).join('')).join('\n')
      : '');
    let snippet = snippetText.length > 200 ? snippetText.substring(0, 200) + '...' : snippetText;
