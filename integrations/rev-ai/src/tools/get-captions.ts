...
export let getCaptions = SlateTool.create(
  spec,
  {
    name: 'Get Captions',
    key: 'get_captions',
  ...
  .input(z.object({
    ...
    format: z.enum(['srt', 'vtt']).describe('Caption format: "srt" for SubRip or "vtt" for WebVTT'),
    speakerChannel: z.number().optional().describe('Speaker channel to get captions for (for multi-channel audio)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let captions = await client.getCaptions(ctx.input.jobId, ctx.input.format, ctx.input.speakerChannel);
