...
export let submitTranscriptionJob = SlateTool.create(
  spec,
  {
...
Returns the created job with its ID and status. Poll the job status or use webhooks to know when it completes.`,
  ...
  .input(z.object({
    ...
    language: z.string().optional().describe('ISO 639-1 language code (default: "en"). Use "cmn" for Mandarin.'),
    transcriber: z.enum(['machine', 'human', 'low_cost']).optional().describe('Transcription engine: "machine" (default), "human", or "low_cost" (Reverb Turbo)'),
    metadata: z.string().optional().describe('Optional metadata to associate with the job (max 512 chars)'),
    skipDiarization: z.boolean().optional().describe('Skip speaker diarization (default: false)'),
    skipPunctuation: z.boolean().optional().describe('Skip punctuation (default: false)'),
    filterProfanity: z.boolean().optional().describe('Filter profanity from results (default: false)'),
    removeDisfluencies: z.boolean().optional().describe('Remove filler words like "umm", "ah" (default: false)'),
    removeAtmospherics: z.boolean().optional().describe('Remove atmospherics like <laugh>, <affirmative> from output'),
    verbatim: z.boolean().optional().describe('Include every syllable including disfluencies and false starts'),
    skipPostprocessing: z.boolean().optional().describe('Skip inverse text normalization, casing, and punctuation steps'),
    speakerChannelsCount: z.number().optional().describe('Number of speaker channels (1-8) for multi-channel audio'),
    customVocabularyId: z.string().optional().describe('ID of a pre-compiled custom vocabulary to use'),
    customVocabularies: z.array(z.object({
      phrases: z.array(z.string()).describe('List of custom words or phrases'),
    })).optional().describe('Inline custom vocabulary phrases for improving accuracy'),
    translationTargetLanguages: z.array(z.string()).optional().describe('Target language codes to translate the transcript into'),
    enableSummarization: z.boolean().optional().describe('Enable transcript summarization'),
    deleteAfterSeconds: z.number().optional().describe('Auto-delete job after this many seconds (0-2592000)'),
    diarizationType: z.enum(['standard', 'premium']).optional().describe('Speaker diarization type'),
    expectedSpeakerCount: z.number().optional().describe('Expected number of speakers for better diarization'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let diarizationConfig = ctx.input.diarizationType || ctx.input.expectedSpeakerCount
      ? {
