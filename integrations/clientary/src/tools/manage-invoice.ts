...
export let updateInvoice = SlateTool.create(
  ...
  .input(z.object({
    ...
    tax3: z.number().optional().describe('Third tax rate percentage'),
    items: z.array(z.object({
      invoiceItemId: z.number().optional().describe('ID of existing item to update. Omit to create new item.'),
      title: z.string().optional().describe('Line item title'),
      quantity: z.number().optional().describe('Quantity'),
      price: z.number().optional().describe('Unit price'),
      destroy: z.boolean().optional().describe('Set true to remove this item (requires invoiceItemId)')
...
export let getInvoices = SlateTool.create(
  ...
  .input(z.object({
    invoiceId: z.number().optional().describe('ID of a specific invoice to retrieve. If omitted, lists invoices.'),
    ...
    updatedSince: z.string().optional().describe('Filter invoices updated since this date (YYYY-MM-DD)')
  }))
  .output(z.object({
...
let mapInvoice = (inv: any) => ({
  ...
  items: inv.invoice_items ? inv.invoice_items.map((item: any) => ({
