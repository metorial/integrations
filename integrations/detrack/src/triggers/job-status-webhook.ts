...
export let jobStatusWebhookTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Job Status Update',
    key: 'job_status_update',
    description: 'Triggers when a job status changes in Detrack. Covers all status updates including info received, scheduled, heading to, arrived, completed, partially completed, failed, and driver assignment changes. Configure the webhook URL in Detrack under Settings > Delivery or Collection > Webhook URL.',
    instructions: [
  ...
  .input(z.object({
    ...
    milestones: z.array(z.record(z.string(), z.unknown())).optional().describe('Job milestones'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let jobData = (body.data && typeof body.data === 'object' ? body.data : body) as Record<string, unknown>;
      ...
      let milestones = Array.isArray(jobData.milestones) ? jobData.milestones as Record<string, unknown>[] : undefined;
      ...
      return {
        inputs: [{
          ...
          trackingNumber: jobData.tracking_number ? String(jobData.tracking_number) : undefined,
          deliverTo: jobData.deliver_to_collect_from ? String(jobData.deliver_to_collect_from) : undefined,
