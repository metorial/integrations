...
export let updateIncident = SlateTool.create(
  ...
  .input(z.object({
    ...
    assigneeIds: z.array(z.string()).optional().describe('User IDs to reassign the incident to'),
    ...
    snoozeDurationSeconds: z.number().optional().describe('Duration in seconds to snooze the incident'),
    mergeSourceIncidentIds: z.array(z.string()).optional().describe('Incident IDs to merge into this incident'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.snoozeDurationSeconds) {
      incident = await client.snoozeIncident(ctx.input.incidentId, ctx.input.snoozeDurationSeconds, ctx.input.fromEmail);
    ...
    if (ctx.input.mergeSourceIncidentIds && ctx.input.mergeSourceIncidentIds.length > 0) {
      incident = await client.mergeIncidents(ctx.input.incidentId, ctx.input.mergeSourceIncidentIds, ctx.input.fromEmail);
      actions.push(`merged ${ctx.input.mergeSourceIncidentIds.length} incident(s)`);
    }

    // Handle core update
    let hasUpdate = ctx.input.status || ctx.input.title || ctx.input.urgency ||
      ctx.input.escalationPolicyId || ctx.input.assigneeIds || ctx.input.priorityId ||
      ctx.input.conferenceNumber || ctx.input.conferenceUrl || ctx.input.resolution;

    if (hasUpdate) {
      incident = await client.updateIncident(ctx.input.incidentId, {
    ...
    if (ctx.input.noteContent) {
      let note = await client.addIncidentNote(ctx.input.incidentId, ctx.input.noteContent, ctx.input.fromEmail);
      noteId = note.id;
