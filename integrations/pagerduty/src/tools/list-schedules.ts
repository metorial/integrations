...
export let listSchedules = SlateTool.create(
  ...
  .output(z.object({
    schedules: z.array(z.object({
      ...
      escalationPolicyNames: z.array(z.string()).optional().describe('Associated escalation policy names'),
    })),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let schedules = result.schedules.map(s => ({
      ...
      escalationPolicyNames: s.escalation_policies?.map(ep => ep.summary).filter(Boolean) as string[] | undefined,
    }));
