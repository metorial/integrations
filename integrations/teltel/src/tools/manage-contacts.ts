...
export let manageContactsTool = SlateTool.create(
  spec,
  {
...
When dynamically adding contacts via the API, enable the "Disable auto stop" option in the campaign settings.`,
  ...
  .input(z.object({
    action: z.enum(['add', 'add_bulk', 'list', 'delete']).describe('Action to perform on campaign contacts'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'add_bulk': {
        ...
        result = await client.addCampaignContactsBulk(ctx.input.campaignId, ctx.input.contacts);
      ...
      case 'list': {
        ...
        let contacts = Array.isArray(result) ? result : (result?.data ?? result?.contacts ?? []);
        return {
          output: {
            success: true,
            contacts,
