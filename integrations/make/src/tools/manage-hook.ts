...
export let manageHook = SlateTool.create(
  spec,
  {
    name: 'Manage Webhook',
    key: 'manage_hook',
  ...
  .input(z.object({
    action: z.enum(['get', 'create', 'rename', 'enable', 'disable', 'ping', 'delete']).describe('Action to perform'),
    hookId: z.number().optional().describe('Hook ID (required for get, rename, enable, disable, ping, delete)'),
    ...
    typeName: z.string().optional().describe('Hook type, e.g. "gateway-webhook" or "gateway-mailhook" (required for create)'),
  }))
  .output(z.object({
