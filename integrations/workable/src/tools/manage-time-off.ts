...
export let manageTimeOffTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['listCategories', 'listRequests', 'createRequest', 'getBalances']).describe('The action to perform'),
    employeeId: z.string().optional().describe('Employee ID (required for createRequest and getBalances)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('Time-off categories'),
    ...
    })).optional().describe('Time-off requests'),
    createdRequest: z.object({
      requestId: z.string().optional(),
      status: z.string().optional()
    }).optional().describe('Created request details'),
    balances: z.array(z.object({
      categoryName: z.string().optional(),
      balance: z.number().optional(),
      used: z.number().optional(),
      total: z.number().optional()
    })).optional().describe('Time-off balances'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'listCategories': {
        ...
        let categories = (result.categories || result.time_off_categories || []).map((c: any) => ({
          categoryId: c.id,
          name: c.name,
          kind: c.kind
        }));
