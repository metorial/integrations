...
export let enrichContact = SlateTool.create(
  spec,
  {
    name: 'Enrich Contact',
    key: 'enrich_contact',
...
The enrichment is asynchronous — use the **Get Enrichment Result** tool to retrieve results once processing completes, or configure a webhook callback URL to receive results automatically.`,
  ...
  .input(z.object({
    profileUrl: z.string().optional().describe('LinkedIn profile URL (e.g., https://www.linkedin.com/in/johndoe). Most accurate identification method.'),
    fullName: z.string().optional().describe('Full name of the contact. Must be combined with companyName or companyDomain.'),
    companyName: z.string().optional().describe('Company name where the contact works. Used with fullName.'),
    companyDomain: z.string().optional().describe('Company domain (e.g., acme.com). Used with fullName.'),
    email: z.string().optional().describe('Email address of the contact. Can be used as the sole identifier.'),
    enrichmentLevel: z.enum(['none', 'partial', 'phone', 'full']).default('partial').describe('Level of enrichment: "none" = LinkedIn profile only, "partial" = profile + emails, "phone" = profile + phone numbers, "full" = profile + emails + phone numbers.'),
    acceptWork: z.boolean().optional().describe('Whether to include work email addresses in results. Defaults to true.'),
    acceptPersonal: z.boolean().optional().describe('Whether to include personal email addresses in results. Defaults to true.'),
    callbackUrl: z.string().optional().describe('Webhook URL to receive results when enrichment completes, overriding the default webhook URL.'),
  }))
  .output(z.object({
    revealId: z.number().describe('Unique ID of the enrichment request. Use this to retrieve results later.'),
    status: z.string().describe('Current status of the enrichment request (e.g., "queued", "resolving").'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    ctx.info({ message: 'Individual reveal started', revealId: result.data.id, status: result.data.status });
