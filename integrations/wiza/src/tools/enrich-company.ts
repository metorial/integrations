...
Provide at least one identifier: company name, domain, LinkedIn ID, or LinkedIn slug.`,
  ...
  .input(z.object({
    ...
    companyDomain: z.string().optional().describe('Company website domain (e.g., "wiza.co").'),
    companyLinkedinId: z.string().optional().describe('LinkedIn company ID (numeric).'),
    companyLinkedinSlug: z.string().optional().describe('LinkedIn company URL slug (e.g., "wiza" from linkedin.com/company/wiza).'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let parts = [`Company enrichment completed for **${d.company_name || 'unknown company'}**`];
