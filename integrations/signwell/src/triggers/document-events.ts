...
export let documentEvents = SlateTrigger.create(
  spec,
  {
    ...
    key: 'document_events',
    description: 'Triggers when a document lifecycle event occurs, such as created, sent, viewed, signed, completed, expired, canceled, declined, bounced, or error.',
  }
)
  .input(z.object({
    ...
    metadata: z.record(z.string(), z.string()).optional().describe('Custom metadata attached to the document'),
  ...
  .output(z.object({
    ...
    })).optional().describe('Document recipients and their statuses'),
