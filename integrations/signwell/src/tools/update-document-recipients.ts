...
export let updateDocumentRecipients = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    })).optional().describe('Updated recipients list'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new SignWellClient({ token: ctx.auth.token });

    let result = await client.updateDocumentRecipients(ctx.input.documentId, ctx.input.recipients);
