...
let urlMetricsResultSchema = z.object({
  ...
  externalPagesToSubdomain: z.number().optional().describe('External links to this subdomain'),
  rootDomainsToSubdomain: z.number().optional().describe('Root domains linking to this subdomain'),
  pagesToRootDomain: z.number().optional().describe('Total links to this root domain'),
  externalPagesToRootDomain: z.number().optional().describe('External links to this root domain'),
  rootDomainsToRootDomain: z.number().optional().describe('Root domains linking to this root domain'),
}).passthrough();
...
let mapResult = (r: any) => {
  ...
  if (r.external_pages_to_page !== undefined) mapped.externalPagesToPage = r.external_pages_to_page;
  ...
  if (r.external_pages_to_subdomain !== undefined) mapped.externalPagesToSubdomain = r.external_pages_to_subdomain;
  if (r.root_domains_to_subdomain !== undefined) mapped.rootDomainsToSubdomain = r.root_domains_to_subdomain;
  if (r.pages_to_root_domain !== undefined) mapped.pagesToRootDomain = r.pages_to_root_domain;
  if (r.external_pages_to_root_domain !== undefined) mapped.externalPagesToRootDomain = r.external_pages_to_root_domain;
  if (r.root_domains_to_root_domain !== undefined) mapped.rootDomainsToRootDomain = r.root_domains_to_root_domain;
  return mapped;
};

export let getUrlMetricsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    targets: z.array(z.string()).min(1).max(50).describe('URLs or domains to analyze (up to 50)'),
    includeDistributions: z.boolean().optional().describe('Include bucketed link distribution metrics'),
  }))
  .output(z.object({
