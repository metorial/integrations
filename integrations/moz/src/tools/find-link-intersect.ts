...
let intersectResultSchema = z.object({
  ...
  matchingTargetCount: z.number().optional().describe('Number of matched targets this source links to'),
}).passthrough();

export let findLinkIntersectTool = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum of 6 total targets across isLinkingTo and notLinkingTo combined.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    isLinkingTo: z.array(intersectTargetSchema).min(1).describe('Sites that sources must link to (competitors)'),
    notLinkingTo: z.array(intersectTargetSchema).optional().describe('Sites that sources must NOT link to (your site)'),
    minimumMatchingTargets: z.number().optional().describe('Minimum number of targets a source must link to'),
    sourceScope: z.enum(['page', 'subdomain', 'root_domain']).optional().describe('Scope of source results'),
  ...
  .output(z.object({
    results: z.array(intersectResultSchema).describe('Sources that link to the specified targets'),
