...
let identifierSchema = z.object({
  type: z.enum(['email', 'phone']).describe('Identifier type'),
  id: z.string().describe('The email address or phone number'),
  channels: z.object({
    email: z.object({
      status: z.enum(['subscribed', 'nonSubscribed', 'unsubscribed']).describe('Email subscription status'),
      statusDate: z.string().optional().describe('ISO 8601 timestamp of status change'),
    }).optional(),
    sms: z.object({
      status: z.enum(['subscribed', 'nonSubscribed', 'unsubscribed']).describe('SMS subscription status'),
      statusDate: z.string().optional().describe('ISO 8601 timestamp of status change'),
    }).optional(),
  }).optional().describe('Channel subscription statuses'),
  consent: z.object({
  ...
  }).optional().describe('Consent details for this identifier'),
...
export let createContact = SlateTool.create(
  ...
  .input(z.object({
    identifiers: z.array(identifierSchema).optional().describe('Email and/or phone identifiers with subscription statuses'),
    email: z.string().optional().describe('Contact email (shorthand — creates an email identifier automatically)'),
    firstName: z.string().optional().describe('First name'),
    lastName: z.string().optional().describe('Last name'),
    phone: z.array(z.string()).optional().describe('Phone numbers'),
    ...
    customProperties: z.record(z.string(), z.any()).optional().describe('Custom key-value properties'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.sendWelcomeEmail !== undefined) body.sendWelcomeEmail = ctx.input.sendWelcomeEmail;
