...
export let recordEvent = SlateTool.create(
  spec,
  {
    name: 'Record Event',
    key: 'record_event',
  ...
  .input(z.object({
    email: z.string().optional().describe('Subscriber email address. Either email or subscriberId must be provided.'),
    subscriberId: z.string().optional().describe('Subscriber ID. Either email or subscriberId must be provided.'),
    action: z.string().describe('The event action name, e.g. "Signed up for a trial".'),
    properties: z.record(z.string(), z.any()).optional().describe('Custom key-value properties to attach to the event.'),
    occurredAt: z.string().optional().describe('ISO-8601 timestamp when the event occurred. Defaults to now.'),
