...
export let manageTags = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['apply', 'remove', 'list']).describe('"apply" to add a tag, "remove" to delete a tag from a subscriber, "list" to get all account tags.'),
    email: z.string().optional().describe('Subscriber email address. Required for apply and remove actions.'),
    tag: z.string().optional().describe('The tag name. Required for apply and remove actions.'),
  }))
  .output(z.object({
    tags: z.array(z.string()).optional().describe('List of all tags (when action is "list").'),
    applied: z.boolean().optional().describe('Whether the tag was applied (when action is "apply").'),
    removed: z.boolean().optional().describe('Whether the tag was removed (when action is "remove").'),
  }))
