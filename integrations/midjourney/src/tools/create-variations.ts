...
export let createVariations = SlateTool.create(
  spec,
  {
  ...
  .input(
    z.object({
      parentTaskId: z.string().describe('Task ID of the original generation task to create variations from'),
      index: z.enum(['1', '2', '3', '4', 'strong', 'subtle']).describe('Image index (1-4) for classic variations, or "strong"/"subtle" for vary modes'),
      waitForResult: z.boolean().optional().default(false).describe('If true, polls until variations complete and returns image URLs'),
    })
  )
  .output(
    z.object({
      ...
      imageUrls: z.array(z.string()).optional().describe('URLs of the individual variation images'),
    })
  )
