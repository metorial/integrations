...
export let updateTemplate = SlateTool.create(
  ...
  .input(z.object({
    ...
    passDescription: z.record(z.string(), z.any()).optional().describe('Updated pass configuration (logoText, foregroundColor, backgroundColor, barcode, etc.)'),
    actions: z.record(z.string(), z.any()).optional().describe('Template actions configuration (verification/redemption behavior)'),
    branding: z.record(z.string(), z.any()).optional().describe('Template branding settings'),
    restrictions: z.record(z.string(), z.any()).optional().describe('Distribution restriction settings'),
    payment: z.record(z.string(), z.any()).optional().describe('Payment settings (PayPal, Stripe, or PAYMILL)'),
  }))
  .output(z.object({
