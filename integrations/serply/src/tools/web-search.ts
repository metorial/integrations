...
export let webSearch = SlateTool.create(
  ...
  .input(z.object({
    query: z.string().describe('Search query string. Supports Google search operators like site:, intitle:, etc.'),
    num: z.number().optional().describe('Number of results to return'),
    start: z.number().optional().describe('Start offset for pagination'),
    language: z.string().optional().describe('Search language filter (e.g., lang_en, lang_es, lang_fr)'),
    interfaceLanguage: z.string().optional().describe('Interface language code (e.g., en, es, fr)'),
