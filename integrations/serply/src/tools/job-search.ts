...
export let jobSearch = SlateTool.create(
  spec,
  {
    name: 'Job Search',
    key: 'job_search',
  ...
  .input(z.object({
    query: z.string().describe('Job search query, e.g., "nurse practitioner", "data analyst work from home"'),
    proxyLocation: z.enum(['US', 'CA']).optional().describe('Location for job results (US or CA only)'),
