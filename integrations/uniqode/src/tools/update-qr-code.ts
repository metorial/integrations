...
export let updateQrCode = SlateTool.create(
  spec,
  {
    name: 'Update QR Code',
    key: 'update_qr_code',
  ...
  .input(z.object({
    ...
    campaign: z.object({
      contentType: z.number().optional().describe('Campaign type: 0=None, 1=Custom URL, 2=Landing Page, 3=Form'),
      customUrl: z.string().optional().describe('Target URL (for contentType 1)'),
      markdownCardId: z.number().optional().describe('Landing page ID (for contentType 2)'),
    ...
    attributes: z.record(z.string(), z.unknown()).optional().describe('Updated visual design attributes (color, logo, pattern, etc.)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.locationEnabled !== undefined) data.location_enabled = ctx.input.locationEnabled;
    ...
    if (ctx.input.campaign) {
      ...
      if (ctx.input.campaign.contentType !== undefined) campaignData.content_type = ctx.input.campaign.contentType;
