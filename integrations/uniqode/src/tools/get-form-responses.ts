...
export let getFormResponses = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list_forms', 'get_responses']).describe('Action: "list_forms" to browse forms, "get_responses" to get submissions'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of forms (for list_forms)'),
    totalFormCount: z.number().optional().describe('Total form count'),
    responses: z.array(z.record(z.string(), z.unknown())).optional().describe('Form response submissions (for get_responses)'),
