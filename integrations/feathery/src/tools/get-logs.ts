...
export let getLogs = SlateTool.create(
  ...
  .input(z.object({
    logType: z.enum(['api_connector', 'form_email', 'email_issues']).describe('Type of logs to retrieve'),
    formId: z.string().optional().describe('Form ID (required for api_connector and form_email log types)'),
    ...
    eventType: z.enum(['Bounce', 'Complaint']).optional().describe('Event type filter (only for email_issues)'),
  }))
  .output(z.object({
