...
export let createSubmission = SlateTool.create(
  ...
  .input(z.object({
    ...
    fields: z.record(z.string(), z.any()).describe('Key-value mapping of field IDs to their values'),
    complete: z.boolean().optional().describe('Mark the submission as complete, triggering form completion events'),
    documents: z.array(z.object({
      documentId: z.string().describe('Document template ID to generate'),
      outputLocation: z.string().optional().describe('Where to store the generated document'),
    })).optional().describe('Documents to generate with the submission'),
  }))
  .output(z.object({
