...
export let getProduct = SlateTool.create(
  ...
  .input(z.object({
    ...
    ]).optional().describe('eBay domain to retrieve the product from. Defaults to configured domain. Ignored if url is provided.'),
    epid: z.string().optional().describe('eBay Product ID (EPID) to look up.'),
    gtin: z.string().optional().describe('GTIN, ISBN, UPC, or EAN code to look up. Automatically converted to an EPID.'),
    url: z.string().optional().describe('Direct URL to an eBay product page. Takes precedence over epid and ebayDomain.'),
    skipGtinCache: z.boolean().optional().describe('Force a fresh GTIN-to-EPID lookup instead of using the 2-month cache. Costs 2 credits.'),
    includePartsCompatibility: z.boolean().optional().describe('Include parts compatibility data. Only for ebay.com and ebay.co.uk. Costs 2 credits.'),
  }))
  .output(z.object({
    product: z.any().describe('Full product data including title, price, condition, images, attributes, seller info, shipping, returns, and more.'),
    isMaster: z.boolean().optional().describe('Whether this is a master product page (summary of all listings) or an individual listing.'),
  }))
