...
export let getSellerProfile = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    isStore: z.boolean().optional().describe('Set to true for eBay storefront sellers (uses /str/ URL format instead of /usr/).'),
  }))
  .output(z.object({
    sellerProfile: z.any().describe('Seller profile data including name, positive rating, followers, location, description, and top-rated status.'),
  }))
