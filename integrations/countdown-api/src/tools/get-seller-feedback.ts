...
export let getSellerFeedback = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    feedbackType: z.enum(['all', 'received_as_buyer', 'received_as_seller', 'left_for_others']).optional().describe('Category of feedback to retrieve.'),
    searchTerm: z.string().optional().describe('Search term to filter feedback. Requires feedbackType to be received_as_seller.'),
    timePeriod: z.enum(['all', 'one_month', 'six_months', 'twelve_months']).optional().describe('Time period for feedback.'),
    overallRating: z.enum(['positive', 'neutral', 'negative']).optional().describe('Filter by overall rating.'),
    ...
    num: z.enum(['25', '50', '100', '200']).optional().describe('Number of feedback entries per page.'),
  }))
  .output(z.object({
    feedbackEntries: z.array(z.any()).describe('Array of feedback entries.'),
    feedbackSummary: z.any().optional().describe('Feedback summary with overall rating statistics.'),
