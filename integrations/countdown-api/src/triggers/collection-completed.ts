...
let downloadLinksSchema = z.object({
  pages: z.array(z.string()).optional().describe('Array of download URLs for individual pages.'),
...
export let collectionCompleted = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers when a bulk request collection completes and a new result set is available for download. The collection must have its notification_webhook configured to point at this trigger.',
  }
)
  .input(z.object({
    ...
    downloadLinks: z.any().optional().describe('Download links for results in JSON, JSON Lines, and/or CSV.'),
  }))
  .output(z.object({
    collectionId: z.string().describe('The collection ID.'),
    collectionName: z.string().describe('The collection name.'),
    ...
    requestsFailed: z.number().describe('Number of requests that failed.'),
    jsonDownloadUrl: z.string().optional().describe('Download URL for JSON results (all pages).'),
    jsonlinesDownloadUrl: z.string().optional().describe('Download URL for JSON Lines results (all pages).'),
    csvDownloadUrl: z.string().optional().describe('Download URL for CSV results (all pages).'),
  }))
  .webhook({
