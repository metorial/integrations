...
export let claimReward = SlateTool.create(
  spec,
  {
    name: 'Claim Reward',
    key: 'claim_reward',
  ...
  .input(z.object({
    action: z.enum(['claim', 'reverse']).default('claim').describe('Action: "claim" to claim a reward, "reverse" to undo a claim'),
    contactUuid: z.string().optional().describe('UUID of the contact claiming the reward (required for claim)'),
    rewardUuid: z.string().optional().describe('UUID of the reward to claim (required for claim)'),
    shopUuid: z.string().optional().describe('UUID of the shop (uses config default if not provided, required for claim)'),
    rewardReceptionUuid: z.string().optional().describe('UUID of the reward reception to reverse (required for reverse)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'reverse') {
      if (!ctx.input.rewardReceptionUuid) throw new Error('rewardReceptionUuid is required for reverse');
