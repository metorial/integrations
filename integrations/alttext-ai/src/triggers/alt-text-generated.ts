...
export let altTextGenerated = SlateTrigger.create(
  spec,
  {
    name: 'Alt Text Generated',
    key: 'alt_text_generated',
    description: 'Triggers when new alt text is generated for an image in your AltText.ai library.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let state = ctx.state as { lastSeenTimestamp?: string; seenAssetIds?: string[] } | undefined;
      ...
      let updatedSeenIds = [
        ...newImages.map((img) => img.asset_id),
        ...seenAssetIds,
      ].slice(0, 200);
