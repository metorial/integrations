...
export let manageDidcommConnection = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['create_invitation', 'receive_invitation', 'update']).describe('Action to perform'),
    reusable: z.boolean().optional().describe('Whether the invitation can be used multiple times (create_invitation)'),
    did: z.string().optional().describe('DID to use for the invitation (create_invitation)'),
    goal: z.string().optional().describe('Goal description for the invitation (create_invitation)'),
    invitationUri: z.string().optional().describe('Invitation URI to accept (receive_invitation)'),
    displayName: z.string().optional().describe('Display name for the connection (receive_invitation, update)'),
