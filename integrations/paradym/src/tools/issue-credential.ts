...
- **direct**: Directly signs and returns an SD-JWT VC credential without an exchange protocol.`,
  ...
  .input(z.object({
    ...
    credentials: z.array(z.object({
      credentialTemplateId: z.string().describe('ID of the credential template to issue'),
      attributes: z.record(z.string(), z.any()).optional().describe('Attribute values to populate the credential with'),
    })).describe('Credentials to issue (multiple supported for openid4vc; single for didcomm and direct)'),
    expirationInMinutes: z.number().optional().describe('Offer expiration time in minutes (default 15, openid4vc only)'),
    didcommConnectionId: z.string().optional().describe('Existing DIDComm connection ID to send offer to (didcomm only)'),
    createInvitation: z.boolean().optional().describe('Create a new DIDComm invitation for the offer (didcomm only, default true if no connectionId)'),
  }))
  .output(z.object({
    ...
    sdJwtVc: z.any().optional().describe('The signed SD-JWT VC credential (direct issuance only)'),
