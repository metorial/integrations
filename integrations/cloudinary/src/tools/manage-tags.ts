...
export let manageTags = SlateTool.create(
  ...
  .input(z.object({
    ...
    command: z.enum(['add', 'remove', 'replace', 'set_exclusive', 'remove_all']).describe(
    ...
    resourceType: z.enum(['image', 'video', 'raw']).default('image').describe('Resource type of the assets.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let commandLabel = ctx.input.command === 'add' ? 'Added' : ctx.input.command === 'remove' ? 'Removed' : ctx.input.command === 'replace' ? 'Replaced with' : ctx.input.command === 'set_exclusive' ? 'Set exclusive' : 'Removed all';
