...
export let updateAsset = SlateTool.create(
  ...
  .input(z.object({
    ...
    resourceType: z.enum(['image', 'video', 'raw']).default('image').describe('Resource type of the asset.'),
    type: z.enum(['upload', 'fetch', 'private', 'authenticated']).default('upload').describe('Delivery type of the asset.'),
    newPublicId: z.string().optional().describe('New public ID to rename the asset to.'),
    overwriteOnRename: z.boolean().optional().describe('Whether to overwrite an existing asset when renaming.'),
    tags: z.array(z.string()).optional().describe('Replace existing tags with these tags.'),
    context: z.record(z.string(), z.string()).optional().describe('Contextual metadata key-value pairs to set.'),
    metadata: z.record(z.string(), z.string()).optional().describe('Structured metadata key-value pairs to set.'),
    ...
    moderationStatus: z.enum(['approved', 'rejected', 'pending']).optional().describe('Update moderation status.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let hasUpdates = ctx.input.tags || ctx.input.context || ctx.input.metadata ||
      ctx.input.displayName || ctx.input.assetFolder || ctx.input.moderationStatus;
    ...
    if (hasUpdates) {
      ...
      if (ctx.input.moderationStatus) actions.push(`set moderation to ${ctx.input.moderationStatus}`);
    }

    // Fetch updated resource
    let resource = await client.getResource(targetPublicId, ctx.input.resourceType, ctx.input.type);
