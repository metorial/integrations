...
export let searchAssets = SlateTool.create(
  ...
  .input(z.object({
    expression: z.string().optional().describe('Search expression in Lucene-like query language. If omitted, returns the most recently created assets.'),
    sortBy: z.array(z.object({
      field: z.string().describe('Field to sort by (e.g., "created_at", "public_id", "bytes").'),
      direction: z.enum(['asc', 'desc']).describe('Sort direction.'),
    })).optional().describe('Fields to sort results by.'),
    maxResults: z.number().optional().describe('Maximum number of results to return (up to 500, default 50).'),
    nextCursor: z.string().optional().describe('Cursor for paginating through results.'),
    withField: z.array(z.string()).optional().describe('Additional fields to include in results (e.g., "tags", "context", "metadata").'),
    aggregate: z.array(z.string()).optional().describe('Fields to get aggregate counts for (e.g., "format", "resource_type").'),
  }))
  .output(z.object({
