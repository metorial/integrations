...
export let manageFolders = SlateTool.create(
  ...
  .input(z.object({
    ...
    path: z.string().optional().describe('Folder path. For "list", specifies the parent folder (omit for root). For "create" and "delete", specifies the folder path.'),
    maxResults: z.number().optional().describe('Maximum number of folders to return (for "list" action).'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of folders (for "list" action).'),
    nextCursor: z.string().optional().describe('Cursor for the next page.'),
    created: z.boolean().optional().describe('Whether the folder was created (for "create" action).'),
    deleted: z.array(z.string()).optional().describe('Deleted folder paths (for "delete" action).'),
  }))
