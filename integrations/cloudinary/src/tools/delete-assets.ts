...
export let deleteAssets = SlateTool.create(
  spec,
  {
    name: 'Delete Assets',
    key: 'delete_assets',
  ...
  .input(z.object({
    publicIds: z.array(z.string()).optional().describe('List of public IDs to delete (up to 100).'),
    prefix: z.string().optional().describe('Delete all assets whose public ID starts with this prefix.'),
    tag: z.string().optional().describe('Delete all assets with this tag.'),
    resourceType: z.enum(['image', 'video', 'raw']).default('image').describe('Resource type of assets to delete.'),
    type: z.enum(['upload', 'fetch', 'private', 'authenticated']).default('upload').describe('Delivery type of assets to delete. Not used when deleting by tag.'),
  }))
  .output(z.object({
    deleted: z.record(z.string(), z.string()).describe('Map of public IDs to their deletion status.'),
    partial: z.boolean().describe('Whether only a partial set was deleted (more remain to be deleted).'),
  }))
