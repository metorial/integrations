...
export let getJob = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeArtifacts: z.boolean().optional().describe('Whether to include artifacts in the response'),
    includeTests: z.boolean().optional().describe('Whether to include test metadata in the response')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includeArtifacts) {
      let artifactsResponse = await client.getJobArtifacts(ctx.input.projectSlug, ctx.input.jobNumber);
    ...
    let tests: { items: { name?: string; classname?: string; result?: string; runTime?: number; message?: string; source?: string }[]; totalCount?: number } | undefined;
    if (ctx.input.includeTests) {
      let testsResponse = await client.getJobTestMetadata(ctx.input.projectSlug, ctx.input.jobNumber);
      tests = {
