...
- To **delete** a folder: provide the folderId and set delete to true. This removes the folder, all subfolders, and all monitors within.`,
  ...
  .input(z.object({
    folderId: z.string().optional().describe('Folder ID (required for update/delete, omit for create)'),
    name: z.string().optional().describe('Folder name (required for create, optional for update)'),
    parentId: z.string().optional().describe('Parent folder ID (used when creating or updating)'),
    delete: z.boolean().optional().describe('Set to true to delete the folder and all its contents'),
  }))
  .output(z.object({
