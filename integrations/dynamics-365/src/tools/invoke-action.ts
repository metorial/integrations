...
export let invokeFunction = SlateTool.create(
  ...
  .input(z.object({
    functionName: z.string().describe('Name of the function to invoke (e.g., "WhoAmI", "RetrieveCurrentOrganization")'),
    parameters: z.record(z.string(), z.any()).optional().describe('Function parameters as key-value pairs'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.invokeUnboundFunction(ctx.input.functionName, ctx.input.parameters);
...
export let invokeAction = SlateTool.create(
  spec,
  {
    name: 'Invoke Action',
    key: 'invoke_action',
  ...
  .input(z.object({
    actionName: z.string().describe('Name of the action to invoke (e.g., "QualifyLead", "CloseIncident", or a custom action name)'),
    requestBody: z.record(z.string(), z.any()).optional().describe('Action request parameters'),
    entitySetName: z.string().optional().describe('Entity set name for bound actions (e.g., "leads")'),
