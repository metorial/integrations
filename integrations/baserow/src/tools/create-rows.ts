...
export let createRows = SlateTool.create(
  spec,
  {
    name: 'Create Rows',
    key: 'create_rows',
  ...
  .input(z.object({
    ...
    rows: z.array(z.record(z.string(), z.any())).min(1).describe('Array of row objects with field name/ID to value mappings'),
    userFieldNames: z.boolean().optional().default(true).describe('Use human-readable field names instead of field IDs')
  }))
  .output(z.object({
    createdRows: z.array(z.record(z.string(), z.any())).describe('Array of created row objects with their assigned IDs and field values')
  }))
