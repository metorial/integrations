...
export let listRows = SlateTool.create(
  spec,
  {
    name: 'List Rows',
    key: 'list_rows',
  ...
  .input(z.object({
    ...
    orderBy: z.string().optional().describe('Comma-separated field names/IDs to sort by. Prefix with - for descending, + or nothing for ascending'),
    filterType: z.enum(['AND', 'OR']).optional().describe('How multiple filters are combined'),
    filters: z.record(z.string(), z.string()).optional().describe('Filter parameters as key-value pairs, e.g. { "filter__field_123__equal": "value" }'),
    viewId: z.number().optional().describe('Scope results to only rows matching this view\'s filters'),
    userFieldNames: z.boolean().optional().default(true).describe('Use human-readable field names instead of field IDs'),
    include: z.string().optional().describe('Comma-separated list of field names to include in response'),
    exclude: z.string().optional().describe('Comma-separated list of field names to exclude from response')
  }))
  .output(z.object({
    ...
    rows: z.array(z.record(z.string(), z.any())).describe('Array of row objects with field values')
  }))
