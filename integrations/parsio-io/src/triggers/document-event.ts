...
export let documentEvent = SlateTrigger.create(
  spec,
  {
    name: 'Document Event',
    key: 'document_event',
    description: 'Triggers when a document is parsed, fails to parse, or is received in a Parsio.io mailbox. Supports all document-level webhook events.',
  }
)
  .input(z.object({
    event: z.string().describe('The event type (e.g. doc.parsed, doc.fail, doc.received, doc.parsed.flat, table.parsed)'),
    documentId: z.string().optional().describe('ID of the document that triggered the event'),
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: {
          ...
          documentName: (rawPayload.name as string) || (rawPayload.subject as string) || undefined,
          senderEmail: (rawPayload.from as string) || undefined,
          receivedAt: (rawPayload.created_at as string) || (rawPayload.createdAt as string) || undefined,
        },
      };
    },
  })
