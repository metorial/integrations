...
export let updatePlayerSettings = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    settings: z.record(z.string(), z.any()).describe('Player settings as key-value pairs to apply to the video.'),
  }))
  .output(z.object({
    result: z.record(z.string(), z.any()).describe('Response from Spotlightr after updating player settings.'),
  }))
