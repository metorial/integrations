...
Returns the email address along with validity status, SMTP check results, and catch-all detection. Can also return just the email domain if a full email is not needed.`,
  ...
  .input(z.object({
    ...
    resultType: z.enum(['full_email', 'domain_only']).optional().default('full_email').describe('Whether to return the full email address or just the domain'),
    country: z.string().optional().describe('Person\'s location to improve accuracy for regional domains (e.g. "Japan", "France", "General")'),
    linkedinCompanySlug: z.string().optional().describe('LinkedIn company URL slug to improve match rate (e.g. "google" from linkedin.com/company/google)'),
  }))
  .output(z.object({
    ...
    mostProbableEmail: z.string().optional().describe('Most probable email if exact match not found (no credit charge)'),
