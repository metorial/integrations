...
Optionally retrieve premium LinkedIn data, financial/Crunchbase data, French SIREN directory info, and website traffic analytics.`,
  ...
  .input(z.object({
    companyIdentifier: z.string().describe('Company name, website URL, or SIREN number to look up'),
    companyPremium: z.boolean().optional().default(true).describe('Include detailed LinkedIn company data (employees, specialties, locations)'),
  ...
  .output(z.object({
    company: z.any().optional().describe('Company data including name, industry, size, locations, specialties, funding'),
