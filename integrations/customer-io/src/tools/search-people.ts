...
export let searchPeople = SlateTool.create(
  ...
  .input(z.object({
    filter: z.record(z.string(), z.unknown()).describe('The filter object using Customer.io filter syntax'),
  }))
  .output(z.object({
    people: z.array(z.record(z.string(), z.unknown())).describe('Array of matching person records'),
