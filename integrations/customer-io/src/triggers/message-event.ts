...
export let messageEvent = SlateTrigger.create(
  spec,
  {
    name: 'Message Event',
    key: 'message_event',
    description: 'Triggered when a message-related event occurs in Customer.io — such as emails sent, opened, clicked, bounced; SMS delivered; push notifications opened; and more. Configure a Reporting Webhook in Customer.io to point to this trigger\'s URL.',
  }
)
  .input(z.object({
    eventId: z.string().describe('Unique event identifier'),
    objectType: z.string().describe('The message channel type (email, sms, push, in_app, slack, webhook)'),
    metric: z.string().describe('The event metric (e.g. sent, opened, clicked, bounced, delivered, failed)'),
    ...
    failureMessage: z.string().optional().describe('Failure reason (for failed/bounced events)'),
  ...
  .output(z.object({
    ...
    objectType: z.string().describe('The message channel type (email, sms, push, in_app, slack, webhook)'),
    metric: z.string().describe('The event action (sent, opened, clicked, bounced, delivered, failed, etc.)'),
    customerId: z.string().optional().describe('Customer ID associated with the event'),
    emailAddress: z.string().optional().describe('Email address of the person'),
    ...
    broadcastId: z.number().optional().describe('The broadcast ID if triggered by a broadcast'),
    newsletterId: z.number().optional().describe('The newsletter ID if triggered by a newsletter'),
