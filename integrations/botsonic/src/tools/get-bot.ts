...
export let getBot = SlateTool.create(
  spec,
  {
    name: 'Get Bot',
    key: 'get_bot',
  ...
  .input(z.object({
    ...
    includeApiKey: z.boolean().optional().describe('If true, also retrieves the bot-specific API key'),
  }))
  .output(z.object({
    ...
    botInfoConfig: z.record(z.string(), z.any()).describe('Bot display and info configuration'),
    botPromptConfig: z.record(z.string(), z.any()).describe('Bot prompt and behavior configuration'),
