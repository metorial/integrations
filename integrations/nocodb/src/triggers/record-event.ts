...
export let recordEvent = SlateTrigger.create(
  spec,
  {
    name: 'Record Event',
    key: 'record_event',
    description: 'Triggers when a record is inserted, updated, or deleted in a NocoDB table via webhook. Configure a webhook on your NocoDB table using the Manage Webhook tool, pointing it at the provided webhook URL.',
    instructions: [
  ...
  .input(z.object({
    ...
    recordData: z.record(z.string(), z.any()).optional().describe('The record data after the event'),
    previousData: z.record(z.string(), z.any()).optional().describe('The record data before the event (for updates)'),
  }))
  .output(z.object({
    tableId: z.string().describe('The table ID'),
    rowId: z.string().describe('The affected row ID'),
    recordData: z.record(z.string(), z.any()).optional().describe('Record data after the event'),
    previousData: z.record(z.string(), z.any()).optional().describe('Record data before the event'),
  }))
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let previousRows = body?.previous_data
        ? (Array.isArray(body.previous_data) ? body.previous_data : [body.previous_data])
        : [];
