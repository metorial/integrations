...
export let listSubmissions = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'In-progress submissions are only available on the Business plan.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    limit: z.number().min(1).max(150).optional().describe('Max submissions per request (1-150, default 50)'),
    ...
    beforeDate: z.string().optional().describe('Filter submissions before this ISO 8601 date'),
    status: z.enum(['finished', 'in_progress']).optional().describe('Submission status filter (default: finished)'),
    sort: z.enum(['asc', 'desc']).optional().describe('Sort order by submission time (default: asc)'),
    search: z.string().optional().describe('Text search filter across submissions'),
    includeEditLink: z.boolean().optional().describe('Include an edit link for each submission'),
