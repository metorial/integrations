...
let insightValueSchema = z.object({
  ...
  })).optional().describe('Metric values over time'),
...
export let getInsightsTool = SlateTool.create(
  ...
  .input(z.object({
    mediaId: z.string().optional().describe('Media ID to get insights for. If omitted, returns account-level insights.'),
    metrics: z.array(z.string()).optional().describe('Specific metrics to retrieve. If omitted, returns default metrics.'),
    period: z.string().optional().describe('Time period for account insights: "day", "week", "days_28", or "lifetime"'),
    since: z.string().optional().describe('Start date for account insights (ISO 8601 date string)'),
    until: z.string().optional().describe('End date for account insights (ISO 8601 date string)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.mediaId) {
      let defaultMetrics = ['impressions', 'reach', 'engagement', 'saved', 'video_views', 'shares'];
    ...
    let defaultMetrics = ['impressions', 'reach', 'profile_views', 'accounts_engaged', 'total_interactions'];
    let metrics = ctx.input.metrics || defaultMetrics;
