...
let mediaItemSchema = z.object({
  ...
  timestamp: z.string().optional().describe('ISO 8601 timestamp of when the media was published'),
  ...
  })).optional().describe('Child media items for carousel albums'),
});
...
export let getMediaTool = SlateTool.create(
  ...
  .input(z.object({
    mediaId: z.string().optional().describe('Specific media ID to retrieve. If provided, returns detailed info for that single post.'),
    userId: z.string().optional().describe('User ID to list media from. Defaults to the authenticated user.'),
    limit: z.number().optional().describe('Maximum number of media items to return (default: 25, max: 100)'),
