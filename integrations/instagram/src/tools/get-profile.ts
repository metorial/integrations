...
export let getProfileTool = SlateTool.create(
  ...
  .input(z.object({
    userId: z.string().optional().describe('Instagram user ID. Defaults to the authenticated user if not provided.'),
    targetUsername: z.string().optional().describe('Look up another Business/Creator account by username via Business Discovery. Cannot be used with userId.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.targetUsername) {
      let discovery = await client.businessDiscovery(effectiveUserId, ctx.input.targetUsername);
      return {
        output: {
          userId: discovery.id || '',
