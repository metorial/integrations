...
export let getFormResponses = SlateTool.create(
  ...
  .input(z.object({
    ...
    responseId: z.string().optional().describe('Specific response ID to retrieve a single response'),
    page: z.number().optional().describe('Page number for pagination (default: 1)'),
    limit: z.number().optional().describe('Number of responses per page (default: 10)'),
  }))
  .output(z.object({
    responses: z.array(z.record(z.string(), z.any())).describe('List of form responses with answers and metadata'),
    pagination: z.object({
