...
export let listCustomProperties = SlateTool.create(
  spec,
  {
  ...
  .output(
    z.object({
      customProperties: z.array(z.string()).describe('List of custom property names configured for the site'),
    })
  )
...
export let createCustomProperty = SlateTool.create(
  spec,
  {
  ...
  .output(
    z.object({
      created: z.boolean().describe('Whether the custom property was created or already exists'),
    })
  )
  .handleInvocation(async (ctx) => {
    let client = new SitesClient({
...
export let deleteCustomProperty = SlateTool.create(
  spec,
  {
