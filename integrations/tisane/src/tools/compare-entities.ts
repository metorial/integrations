...
export let compareEntities = SlateTool.create(
  spec,
  {
...
Useful for name matching, deduplication, and identity resolution across different languages and naming conventions.`,
    ...
    constraints: [
      'Only person entities are currently supported.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    entityType: z.enum(['person']).default('person').describe('Entity type to compare. Currently only "person" is supported.'),
  }))
  .output(z.object({
    comparisonResult: z.string().describe('Result: "same", "different", or "no_single_entity"'),
    differences: z.array(z.string()).optional().describe('Specific differences when result is "different": given_name, surname, title, social_role, suffix, variation'),
  }))
