...
export let detectLanguage = SlateTool.create(
  ...
  .input(z.object({
    ...
    languageHints: z.string().optional().describe('Pipe-delimited language code hints (e.g., "en|fr|de") to narrow detection'),
    delimiter: z.string().optional().describe('Regex delimiter for segmenting multi-line text (e.g., "\\\\n" for line-by-line detection)'),
  }))
  .output(z.object({
    languages: z.any().describe('Detected languages with offset breakdown showing which parts of text are in which language'),
  }))
