...
Handles obfuscated and slang content, preserving emotional register — profanities and slurs are translated faithfully.`,
  ...
  .input(z.object({
    fromLanguage: z.string().describe('IETF language code for the source language (e.g., "ja"). Use "*" for auto-detection.'),
    toLanguage: z.string().describe('IETF language code for the target language (e.g., "en")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let action = ctx.input.fromLanguage === ctx.input.toLanguage ? 'Paraphrased' : 'Translated';
