...
export let getSlotDetails = SlateTool.create(
  spec,
  {
...
Optionally include sync committee duties, attestation duties, withdrawals, and deposits for the slot.`,
  ...
  .input(z.object({
    ...
    includeSyncCommitteeDuties: z.boolean().optional().describe('Include sync committee duties and rewards for this slot'),
    includeAttestationDuties: z.boolean().optional().describe('Include attestation duties and rewards for this slot'),
    includeWithdrawals: z.boolean().optional().describe('Include withdrawals processed in this slot'),
    includeDeposits: z.boolean().optional().describe('Include deposits processed in this slot'),
  }))
  .output(z.object({
    slotOverview: z.any().describe('Slot overview with proposer, status, execution payload, etc.'),
