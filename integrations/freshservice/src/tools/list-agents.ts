...
export let listAgents = SlateTool.create(
  ...
  .input(z.object({
    query: z.string().optional().describe('Filter query (e.g. "active:true", "email:\'agent@example.com\'")'),
...
export let getAgent = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: `Retrieved agent **#${agent.id}**: "${agent.first_name} ${agent.last_name || ''}"`.trim(),
    };
  }).build();
