...
export let createRecord = SlateTool.create(
  ...
  .input(z.object({
    ...
    fields: z.record(z.string(), z.any()).describe('Field values as key-value pairs matching the model schema'),
    publish: z.boolean().optional().describe('If true, publish the record immediately after creation'),
  }))
  .output(z.object({
