...
export let getGoogleQA = SlateTool.create(
  spec,
  {
    name: 'Get Google Q&A',
    key: 'get_google_qa',
  ...
  .input(z.object({
    ...
    status: z.string().optional().describe('Comma-separated statuses: open, closed, reported, removed'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (data.errorCode !== undefined && data.errorCode !== 0) {
      throw new Error(`Failed to get Google Q&A: ${data.errorMessage} (code: ${data.errorCode})`);
    }

    return {
      output: {
        questions: data.questions ?? {},
        totalCount: typeof data.total === 'string' ? parseInt(data.total, 10) : (data.total ?? 0),
        page: data.page ?? 1,
        pages: data.pages ?? 1,
      },
