...
export let reviewPullRequest = SlateTool.create(
  spec,
  {
...
Optionally include inline comments on specific files and lines. Can also request reviewers.`,
  ...
  .input(z.object({
    ...
    action: z.enum(['APPROVE', 'REQUEST_CHANGES', 'COMMENT', 'request_reviewers']).describe('Review action or request reviewers'),
    ...
    reviewers: z.array(z.string()).optional().describe('Usernames to request review from (only for request_reviewers action)'),
    teamReviewers: z.array(z.string()).optional().describe('Team slugs to request review from (only for request_reviewers action)'),
  }))
  .output(z.object({
    ...
    requestedReviewers: z.array(z.string()).optional().describe('Requested reviewer usernames'),
  }))
