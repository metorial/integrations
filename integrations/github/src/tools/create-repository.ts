...
Supports initializing with a README, gitignore template, and license.`,
  ...
  .input(z.object({
    ...
    gitignoreTemplate: z.string().optional().describe('Gitignore template name (e.g., "Node", "Python")'),
    licenseTemplate: z.string().optional().describe('License template (e.g., "mit", "apache-2.0")'),
    org: z.string().optional().describe('Organization to create the repository in. If omitted, creates under the authenticated user.'),
  }))
  .output(z.object({
