...
When updating: provide the issue number along with fields to change (title, body, state, labels, assignees).`,
  ...
  .input(z.object({
    ...
    issueNumber: z.number().optional().describe('Issue number to update. Omit to create a new issue.'),
    ...
    milestone: z.number().nullable().optional().describe('Milestone number to set, or null to remove'),
  }))
  .output(z.object({
