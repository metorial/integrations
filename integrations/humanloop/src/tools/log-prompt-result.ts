...
export let logPromptResult = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    inputs: z.record(z.string(), z.any()).optional().describe('Input variables used for the call'),
    ...
    }).optional().describe('Full output message object'),
    messages: z.array(z.object({
      role: z.string().describe('Role of the message'),
      content: z.string().describe('Content of the message'),
    })).optional().describe('Conversation messages'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.promptTokens !== undefined || ctx.input.outputTokens !== undefined) {
      body.usage = {};
      if (ctx.input.promptTokens !== undefined) body.usage.prompt_tokens = ctx.input.promptTokens;
      if (ctx.input.outputTokens !== undefined) body.usage.completion_tokens = ctx.input.outputTokens;
    }
