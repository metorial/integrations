...
export let manageTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    path: z.string().optional().describe('Path for the tool (e.g. "folder/my-tool"). Used for create.'),
    toolType: z.enum(['json_schema', 'python', 'snippet', 'get_api_call', 'pinecone_search', 'google', 'mock']).optional().describe('Type of tool'),
    functionName: z.string().optional().describe('Name of the function'),
    functionDescription: z.string().optional().describe('Description of what the function does'),
    parameters: z.record(z.string(), z.any()).optional().describe('JSON Schema for the function parameters'),
    sourceCode: z.string().optional().describe('Source code for Python tools'),
    setupValues: z.record(z.string(), z.any()).optional().describe('Setup values for the tool (e.g. API keys for search tools)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      if (ctx.input.functionName || ctx.input.functionDescription || ctx.input.parameters) {
        ...
        if (ctx.input.functionDescription) body.function.description = ctx.input.functionDescription;
