...
export let provideValidationFeedback = SlateTool.create(
  spec,
  {
...
Helps Google improve future validation results by indicating whether the validated address, the original user-provided address, or a manually edited version was ultimately used.`,
  ...
  .input(z.object({
    responseId: z.string().describe('The response ID from the first validation response in the sequence of validation attempts for this address'),
    conclusion: z.enum([
      'VALIDATED_VERSION_USED',
      'USER_VERSION_USED',
      'UNVALIDATED_VERSION_USED',
      'UNUSED',
    ]).describe('The outcome of the validation: VALIDATED_VERSION_USED (used the API-corrected address), USER_VERSION_USED (used the original user-provided address), UNVALIDATED_VERSION_USED (used a user-edited address without re-validating), UNUSED (transaction was abandoned)'),
  }))
  .output(z.object({
