...
let addressComponentSchema = z.object({
  ...
  componentType: z.string().describe('The type of the address component (e.g. street_number, route, locality, administrative_area_level_1, country, postal_code)'),
  confirmationLevel: z.string().describe('Confirmation level: CONFIRMED, UNCONFIRMED_BUT_PLAUSIBLE, or UNCONFIRMED_AND_SUSPICIOUS'),
  inferred: z.boolean().optional().describe('Whether this component was not part of the input but was inferred'),
  spellCorrected: z.boolean().optional().describe('Whether the component value was spell-corrected'),
  replaced: z.boolean().optional().describe('Whether the component value was replaced entirely'),
  unexpected: z.boolean().optional().describe('Whether this component was not expected for the address'),
});

let verdictSchema = z.object({
  inputGranularity: z.string().optional().describe('Granularity of the input address (e.g. SUB_PREMISE, PREMISE, ROUTE, OTHER)'),
  validationGranularity: z.string().optional().describe('Granularity level the API was able to validate to'),
  geocodeGranularity: z.string().optional().describe('Granularity of the geocode generated for this address'),
  addressComplete: z.boolean().optional().describe('Whether the address is considered complete'),
  hasUnconfirmedComponents: z.boolean().optional().describe('Whether any component could not be confirmed'),
  hasInferredComponents: z.boolean().optional().describe('Whether any component was inferred that was not in the input'),
  hasReplacedComponents: z.boolean().optional().describe('Whether any component was replaced'),
  hasSpellCorrectedComponents: z.boolean().optional().describe('Whether any component was spell-corrected'),
  possibleNextAction: z.string().optional().describe('Suggested next action: FIX, CONFIRM_ADD_SUBPREMISES, CONFIRM, or ACCEPT'),
});
...
let geocodeSchema = z.object({
  ...
  }).optional().describe('Plus Code for the location'),
  bounds: z.object({
    low: z.object({ latitude: z.number(), longitude: z.number() }),
    high: z.object({ latitude: z.number(), longitude: z.number() }),
  }).optional().describe('Bounding box of the geocoded location'),
...
let uspsDataSchema = z.object({
  ...
  }).optional().describe('USPS standardized address'),
  ...
  dpvCmra: z.string().optional().describe('Whether the address is a CMRA (Commercial Mail Receiving Agency)'),
...
let validatedAddressSchema = z.object({
  formattedAddress: z.string().optional().describe('The validated and formatted address string'),
  postalAddress: postalAddressSchema.optional().describe('The structured postal address'),
  addressComponents: z.array(addressComponentSchema).optional().describe('Individual address components with validation details'),
  missingComponentTypes: z.array(z.string()).optional().describe('Component types that were expected but missing from the address'),
  unconfirmedComponentTypes: z.array(z.string()).optional().describe('Component types that could not be confirmed'),
  unresolvedTokens: z.array(z.string()).optional().describe('Tokens in the input that could not be resolved'),
});
...
When re-validating a previously validated address, provide the \`previousResponseId\` to improve accuracy.`,
  ...
  .input(z.object({
    addressLines: z.array(z.string()).describe('Address lines (e.g. street address, apt number). Use this for unstructured or partially structured addresses.'),
    regionCode: z.string().optional().describe('CLDR region code of the address country (e.g. "US", "FR", "JP"). Highly recommended for accuracy.'),
    locality: z.string().optional().describe('City or town name'),
    administrativeArea: z.string().optional().describe('State, province, or administrative area'),
    postalCode: z.string().optional().describe('Postal or ZIP code'),
    languageCode: z.string().optional().describe('BCP-47 language code for the input address (e.g. "en", "fr")'),
    organization: z.string().optional().describe('Organization or business name at the address'),
    recipients: z.array(z.string()).optional().describe('Recipients at the address'),
    enableUspsCass: z.boolean().optional().describe('Enable USPS CASS processing for US/Puerto Rico addresses to get ZIP+4, delivery point codes, and carrier route info'),
    returnEnglishLatinAddress: z.boolean().optional().describe('Return the address in English Latin script (preview feature)'),
    previousResponseId: z.string().optional().describe('Response ID from a prior validation of this address, used when re-validating a corrected address'),
    sessionToken: z.string().optional().describe('Session token for billing grouping of validation requests'),
  }))
  .output(z.object({
    responseId: z.string().describe('Unique response ID. Use this for provideValidationFeedback or as previousResponseId in subsequent validations.'),
    verdict: verdictSchema.describe('Overall assessment of the address quality and completeness'),
    validatedAddress: validatedAddressSchema.describe('The validated and standardized address with component-level details'),
    geocode: geocodeSchema.optional().describe('Geocode coordinates and place information for the address'),
    addressMetadata: metadataSchema.optional().describe('Metadata about the address type (residential, business, PO Box)'),
    uspsData: uspsDataSchema.describe('USPS-specific data (only populated when enableUspsCass is true for US/PR addresses)'),
    englishLatinAddress: validatedAddressSchema.optional().describe('Address formatted in English Latin script (only when returnEnglishLatinAddress is true)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (result.geocode?.location) {
      messageParts.push(`**Coordinates:** ${result.geocode.location.latitude}, ${result.geocode.location.longitude}`);
    }
