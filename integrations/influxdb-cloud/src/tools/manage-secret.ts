...
export let upsertSecrets = SlateTool.create(
  spec,
  {
    name: 'Upsert Secrets',
    key: 'upsert_secrets',
...
Secrets can be referenced in Flux scripts using \`secrets.get()\`.`,
  ...
  .input(z.object({
    secrets: z.record(z.string(), z.string()).describe('Key-value pairs of secret names and their values'),
  }))
  .output(z.object({
...
export let deleteSecrets = SlateTool.create(
  spec,
  {
    name: 'Delete Secrets',
    key: 'delete_secrets',
