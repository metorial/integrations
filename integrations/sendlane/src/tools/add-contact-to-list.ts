...
export let addContactToList = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    emailConsent: z.boolean().optional().describe('Whether the contact has opted in to email marketing'),
    smsConsent: z.boolean().optional().describe('Whether the contact has opted in to SMS marketing'),
    customFields: z.record(z.string(), z.string()).optional().describe('Custom field values as key-value pairs (field name → value)'),
  }))
  .output(z.object({
    success: z.boolean(),
  }))
