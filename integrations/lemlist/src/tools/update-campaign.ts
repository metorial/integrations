...
export let updateCampaign = SlateTool.create(
  ...
  .input(z.object({
    ...
    stopOnEmailReplied: z.boolean().optional().describe('Stop outreach when a lead replies to an email'),
    stopOnMeetingBooked: z.boolean().optional().describe('Stop outreach when a meeting is booked'),
    stopOnLinkClicked: z.boolean().optional().describe('Stop outreach when a link is clicked'),
    autoLeadInterest: z.boolean().optional().describe('Automatically mark leads as interested'),
