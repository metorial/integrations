...
export let sendLeadEmail = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    templateId: z.number().optional().describe('Email template ID (for template-based emails)'),
    ...
    userId: z.number().optional().describe('User ID of the sender (must have a connected inbox)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.templateId) {
      ...
      throw new Error('Provide either templateId for a template email, or both subject and body for a custom email.');
    }
