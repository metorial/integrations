...
export let manageServices = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'start', 'stop', 'restart', 'reload']).describe('Action to perform'),
    service: z.string().optional().describe('Service name for start/stop/restart/reload (e.g. nginx, apache2, mysql, mariadb, php8.2-fpm, redis, postfix, ssh, ufw)'),
  }))
  .output(z.object({
    services: z.array(z.record(z.string(), z.unknown())).optional().describe('List of services with status'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!ctx.input.service) throw new Error('service name is required for start/stop/restart/reload actions');

    let result = await client.updateService(orgId, serverId, ctx.input.service, action);
    return {
      output: {
        responseMessage: (result as Record<string, unknown>).message as string || `Service ${action} completed`,
        services: undefined,
      },
