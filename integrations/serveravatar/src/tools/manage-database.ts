...
export let manageDatabase = SlateTool.create(
  ...
  .input(z.object({
    ...
    serverId: z.string().optional().describe('Server ID (required for create and destroy, optional filter for list)'),
    ...
    username: z.string().optional().describe('Database username, min 5 alphanumeric chars (for create action)'),
    password: z.string().optional().describe('Database password, min 8 chars (for create action)'),
    connectionPreference: z.enum(['localhost', 'everywhere', 'specific_ip_addresses']).optional().describe('Connection preference (for create action)'),
    allowedIps: z.array(z.string()).optional().describe('Allowed IPv4 addresses (for create with specific_ip_addresses)'),
  }))
  .output(z.object({
    databases: z.array(z.record(z.string(), z.unknown())).optional().describe('List of databases'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      ...
      if (!ctx.input.databaseName) throw new Error('databaseName is required for create action');
      ...
      return {
        output: {
          responseMessage: (result as Record<string, unknown>).message as string || 'Database created',
          databases: undefined,
    ...
    if (action === 'destroy') {
      ...
      let result = await client.destroyDatabase(orgId, ctx.input.serverId, ctx.input.databaseId);
      return {
        output: {
          responseMessage: (result as Record<string, unknown>).message as string || 'Database deleted',
          databases: undefined,
          pagination: undefined,
        },
