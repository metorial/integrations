...
let itemSchema: z.ZodType<any> = z.object({
  ...
  children: z.array(z.lazy(() => itemSchema)).optional().describe('Child items for composite fields like Matrix or Multi Scale'),
});

export let getFormItems = SlateTool.create(
  spec,
  {
    name: 'Get Form Items',
    key: 'get_form_items',
  ...
  .input(z.object({
    formDir: z.string().describe('Form directory identifier for the form to retrieve items from'),
    resultsLabels: z.string().optional().describe('Apply custom Results Labels to the item data'),
  }))
  .output(z.object({
