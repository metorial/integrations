...
let issueSchema = z.object({
  ...
  reference: z.record(z.string(), z.unknown()).optional().describe('Reference details for the issue'),
...
export let manageIssues = SlateTool.create(
  spec,
  {
    name: 'Manage Issues',
    key: 'manage_issues',
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'update_status', 'dismiss']).describe('Action to perform'),
    issueId: z.string().optional().describe('Issue ID (required for get, update_status, dismiss)'),
    status: z.enum(['OPENED', 'ACKNOWLEDGED', 'RESOLVED', 'IGNORED']).optional().describe('New status (for update_status) or filter (for list)'),
    type: z.enum(['delivery', 'transformation', 'backpressure']).optional().describe('Filter by issue type (for list)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'update_status': {
        let issue = await client.updateIssue(ctx.input.issueId!, { status: ctx.input.status! });
        return {
          output: { issue: mapIssue(issue) },
