...
export let eventSuccessful = SlateTrigger.create(
  spec,
  {
    name: 'Event Delivered Successfully',
    key: 'event_successful',
    description: 'Triggers when an event is successfully delivered to a destination. Contains the payload of the response from the destination URL. Useful for implementing asynchronous response workflows.',
  }
)
  .input(z.object({
    ...
    destinationResponse: z.unknown().optional().describe('Response payload from the destination'),
  ...
  .output(z.object({
    ...
    responseStatus: z.number().optional().describe('HTTP response status from the destination'),
    destinationResponse: z.unknown().optional().describe('Response payload from the destination URL'),
  }))
  .webhook({
    ...
    autoUnregisterWebhook: async (ctx) => {
      let client = new Client({ token: ctx.auth.token, apiVersion: ctx.config.apiVersion });
      let details = ctx.input.registrationDetails as { sourceId: string; destinationId: string; connectionId: string };

      try { await client.deleteConnection(details.connectionId); } catch (_e) { /* ignore */ }
      try { await client.deleteDestination(details.destinationId); } catch (_e) { /* ignore */ }
      try { await client.deleteSource(details.sourceId); } catch (_e) { /* ignore */ }
    },
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: {
          ...
          destinationId: (eventData.destination_id || connection.destination_id) as string | undefined,
          responseStatus: (eventData.response_status || raw.response_status) as number | undefined,
