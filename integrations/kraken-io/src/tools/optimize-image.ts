...
let resizeSchema = z.object({
  strategy: z.enum(['exact', 'portrait', 'landscape', 'auto', 'crop', 'square', 'fit', 'fill']).describe('Resize strategy. "exact" ignores aspect ratio. "portrait" sets exact height. "landscape" sets exact width. "auto" picks portrait or landscape. "crop" extracts exact dimensions. "square" crops to square. "fit" crops and resizes to fit. "fill" maintains aspect ratio and fills unused space.'),
  ...
  cropMode: z.string().optional().describe('Crop direction: n, s, e, w, c, nw, ne, sw, se (or t, b, l, r, tl, tr, bl, br). Defaults to center.'),
  background: z.string().optional().describe('Background color for fill strategy (HEX, RGB, or RGBA)'),
}).optional().describe('Resize the image before optimization');

let convertSchema = z.object({
  format: z.enum(['jpeg', 'png', 'gif', 'webp', 'avif']).describe('Target image format'),
  background: z.string().optional().describe('Background color when converting transparent formats to opaque (HEX, RGB, or RGBA). Defaults to white.'),
  keepExtension: z.boolean().optional().describe('Keep the original file extension after conversion'),
}).optional().describe('Convert image to a different format');

let s3StoreSchema = z.object({
  ...
  acl: z.enum(['public_read', 'private']).optional().describe('Object ACL permissions. Defaults to public_read.'),
}).optional().describe('Store optimized image directly to Amazon S3');

export let optimizeImageTool = SlateTool.create(
  spec,
  {
    name: 'Optimize Image',
    key: 'optimize_image',
...
The optimized image URL is available for **one hour** only.`,
  ...
  .input(z.object({
    ...
    lossy: z.boolean().optional().describe('Enable lossy optimization for maximum compression. Default is lossless.'),
    quality: z.number().min(1).max(100).optional().describe('Custom quality level (1-100) for lossy compression. Overrides default lossy settings.'),
    samplingScheme: z.enum(['4:2:0', '4:2:2', '4:4:4']).optional().describe('Chroma subsampling scheme for JPEG compression. "4:2:0" is default and smallest. "4:4:4" preserves maximum quality.'),
    resize: resizeSchema,
    convert: convertSchema,
    preserveMeta: z.array(z.enum(['date', 'copyright', 'geotag', 'orientation', 'profile'])).optional().describe('Metadata types to preserve. By default all metadata is stripped.'),
    autoOrient: z.boolean().optional().describe('Automatically correct image orientation based on EXIF data'),
    s3Store: s3StoreSchema,
    callbackUrl: z.string().optional().describe('URL to receive optimization results via POST when processing completes. If provided, the API returns immediately.'),
  }))
  .output(z.object({
    ...
    requestId: z.string().optional().describe('Request ID returned when using callback URL mode'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let convertOptions = ctx.input.convert ? {
      ...
      ...(ctx.input.convert.keepExtension !== undefined && { keep_extension: ctx.input.convert.keepExtension }),
    } : undefined;
    ...
    let savedPercent = optimizeResult.original_size && optimizeResult.saved_bytes
