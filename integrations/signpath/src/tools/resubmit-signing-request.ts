...
export let resubmitSigningRequest = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    originalSigningRequestId: z.string().describe('ID of the original signing request to resubmit'),
    signingPolicySlug: z.string().describe('Slug of the new signing policy to use'),
    description: z.string().optional().describe('Optional description for the resubmitted request')
  }))
  .output(z.object({
