...
let invoiceLineItemSchema = z.object({
  ...
  }).optional().describe('Associated product'),
...
let invoiceOutputSchema = z.object({
  ...
  status: z.string().describe('Invoice status (DRAFT, SAVED, SENT, VIEWED, PAID, OVERDUE, etc.)'),
  ...
  }).optional().describe('Customer associated with the invoice'),
  currency: z.object({
    code: z.string().optional(),
    symbol: z.string().optional(),
    name: z.string().optional()
  ...
  disableCreditCardPayments: z.boolean().optional().describe('Whether credit card payments are disabled'),
...
let mapInvoice = (inv: any) => ({
  ...
  items: inv.items?.map((item: any) => ({
    ...
    amount: item.amount,
    product: item.product ? { productId: item.product.id, name: item.product.name } : undefined,
...
let lineItemInputSchema = z.object({
  ...
  })).optional().describe('Taxes to apply to this line item')
});
...
export let createInvoice = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['DRAFT', 'SAVED']).optional().describe('Invoice status (default: DRAFT)'),
    ...
    disableCreditCardPayments: z.boolean().optional().describe('Disable credit card payments for this invoice'),
    disableBankPayments: z.boolean().optional().describe('Disable bank payments for this invoice')
  }))
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!result.didSucceed) {
      throw new Error(`Failed to create invoice: ${result.inputErrors.map(e => e.message).join(', ')}`);
    }
...
export let updateInvoice = SlateTool.create(
  ...
  .input(z.object({
    ...
    items: z.array(lineItemInputSchema).optional().describe('Updated line items (replaces all existing items)'),
    disableCreditCardPayments: z.boolean().optional().describe('Disable credit card payments'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!result.didSucceed) {
      throw new Error(`Failed to update invoice: ${result.inputErrors.map(e => e.message).join(', ')}`);
    }
...
export let deleteInvoice = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!result.didSucceed) {
      throw new Error(`Failed to delete invoice: ${result.inputErrors.map(e => e.message).join(', ')}`);
    }
...
export let sendInvoice = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!result.didSucceed) {
      throw new Error(`Failed to send invoice: ${result.inputErrors.map(e => e.message).join(', ')}`);
    }
...
export let approveInvoice = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!result.didSucceed) {
      throw new Error(`Failed to approve invoice: ${result.inputErrors.map(e => e.message).join(', ')}`);
    }
...
export let cloneInvoice = SlateTool.create(
  spec,
  {
    name: 'Clone Invoice',
    key: 'clone_invoice',
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!result.didSucceed) {
      throw new Error(`Failed to clone invoice: ${result.inputErrors.map(e => e.message).join(', ')}`);
    }
...
export let markInvoiceSent = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    sentAt: z.string().optional().describe('Timestamp when the invoice was sent (ISO 8601 format)')
  }))
  .output(invoiceOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new WaveClient(ctx.auth.token);
    ...
    if (!result.didSucceed) {
      throw new Error(`Failed to mark invoice as sent: ${result.inputErrors.map(e => e.message).join(', ')}`);
    }
