...
Use **DEPOSIT** when the business receives money and **WITHDRAWAL** when the business spends money. Line items categorize the transaction using **INCREASE** or **DECREASE** balance directions. The total of line item amounts must equal the anchor amount.`,
  ...
  .input(z.object({
    ...
    externalId: z.string().describe('A unique external reference ID for this transaction (used for idempotency)'),
    ...
    anchor: z.object({
      ...
      direction: z.enum(['DEPOSIT', 'WITHDRAWAL']).describe('DEPOSIT = receiving money, WITHDRAWAL = spending money')
    }).describe('The bank/credit card account and direction of the transaction'),
    lineItems: z.array(z.object({
      accountId: z.string().describe('ID of the categorization account (e.g., income or expense account)'),
      amount: z.number().describe('Line item amount (positive value)'),
      balance: z.enum(['INCREASE', 'DECREASE', 'DEBIT', 'CREDIT']).describe('Balance direction: INCREASE or DECREASE are recommended'),
      taxes: z.array(z.object({
        salesTaxId: z.string().describe('ID of the sales tax to apply')
      })).optional().describe('Sales taxes to apply to this line item')
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!result.didSucceed) {
      throw new Error(`Failed to create transaction: ${result.inputErrors.map(e => e.message).join(', ')}`);
    }
