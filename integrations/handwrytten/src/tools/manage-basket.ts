...
export let manageBasket = SlateTool.create(
  spec,
  {
    name: 'Manage Basket',
    key: 'manage_basket',
  ...
  .input(z.object({
    ...
    returnAddressId: z.string().optional().describe('Return address ID from address book (for add)'),
    addressId: z.string().optional().describe('Recipient address ID from address book (for add)'),
    recipients: z.array(recipientAddressSchema).optional().describe('Inline recipient addresses (for add, if not using addressId)'),
    giftCardDenominationId: z.string().optional().describe('Gift card denomination ID (for add)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'view') {
      let [basket, count] = await Promise.all([
        client.getBasket(),
        client.getBasketCount(),
      ]);
