...
export let generatePdf = SlateTool.create(
  ...
  .input(z.object({
    ...
    templateData: z.record(z.string(), z.unknown()).describe('Key-value pairs matching the template variables. Supports strings and arrays of objects for table rows.'),
    outputFormat: z.enum(['base64', 'url']).default('base64').describe('Output format: "base64" returns the PDF as a base64-encoded string, "url" returns a temporary download URL valid for 15 minutes'),
  }))
  .output(z.object({
    base64: z.string().optional().describe('Base64-encoded PDF content (when outputFormat is "base64")'),
    downloadUrl: z.string().optional().describe('Temporary download URL for the generated PDF, valid for 15 minutes (when outputFormat is "url")'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: ctx.input.outputFormat === 'url'
