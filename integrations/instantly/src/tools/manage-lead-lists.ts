...
export let manageLeadLists = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    listId: z.string().optional().describe('ID of the lead list (for "update" and "delete" actions).'),
    name: z.string().optional().describe('Name for the lead list (for "create" and "update" actions).'),
    search: z.string().optional().describe('Search lead lists by name (for "list" action).'),
    limit: z.number().min(1).max(100).optional().describe('Number of lists to return (for "list" action).'),
  ...
  .output(z.object({
    lists: z.array(z.object({
      ...
      hasEnrichmentTask: z.boolean().optional().describe('Whether enrichment task exists'),
      timestampCreated: z.string().optional().describe('Creation timestamp'),
    })).optional().describe('Lead lists (for "list" action)'),
