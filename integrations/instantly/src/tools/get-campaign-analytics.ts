...
export let getCampaignAnalytics = SlateTool.create(
  ...
  .input(z.object({
    campaignId: z.string().optional().describe('Single campaign ID. Omit for all campaigns.'),
    campaignIds: z.array(z.string()).optional().describe('Multiple campaign IDs to include.'),
    startDate: z.string().optional().describe('Start date in YYYY-MM-DD format.'),
    endDate: z.string().optional().describe('End date in YYYY-MM-DD format.'),
    view: z.enum(['summary', 'overview', 'daily', 'steps']).optional().default('summary').describe('Type of analytics view.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (view === 'overview') {
      analytics = await client.getCampaignAnalyticsOverview({ campaignId, campaignIds, startDate, endDate });
    } else if (view === 'daily') {
      analytics = await client.getDailyCampaignAnalytics({ campaignId, startDate, endDate });
    } else if (view === 'steps') {
      analytics = await client.getStepAnalytics({ campaignId, startDate, endDate, includeOpportunitiesCount: true });
    } else {
      analytics = await client.getCampaignAnalytics({ campaignId, campaignIds, startDate, endDate });
    }
