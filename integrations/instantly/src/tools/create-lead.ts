...
export let createLead = SlateTool.create(
  ...
  .input(z.object({
    email: z.string().optional().describe('Lead email address. Required when adding to a campaign.'),
    ...
    skipIfInWorkspace: z.boolean().optional().describe('Skip if the lead already exists in the workspace.'),
    skipIfInCampaign: z.boolean().optional().describe('Skip if the lead already exists in the campaign.'),
    customVariables: z.record(z.string(), z.any()).optional().describe('Custom variables for personalization. Values must be string, number, boolean, or null.'),
  }))
  .output(z.object({
