...
export let listCarriers = SlateTool.create(
  spec,
  {
    name: 'List Carriers',
    key: 'list_carriers',
  ...
  .input(z.object({
    includeServicesForCarrierId: z.string().optional().describe('Carrier ID to include detailed services for'),
    includePackagesForCarrierId: z.string().optional().describe('Carrier ID to include package types for')
  }))
  .output(z.object({
    carriers: z.array(z.object({
      ...
      requiresFundedAmount: z.boolean().describe('Whether the carrier requires funded amount'),
      supportsLabelMessages: z.boolean().describe('Whether the carrier supports label messages')
    })),
    ...
    })).optional().describe('Available services for the requested carrier'),
    packageTypes: z.array(z.object({
      packageCode: z.string().describe('Package code'),
      name: z.string().describe('Package type name'),
      description: z.string().optional().describe('Description')
    })).optional().describe('Package types for the requested carrier')
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includeServicesForCarrierId) {
      let servicesResult = await client.listCarrierServices(ctx.input.includeServicesForCarrierId);
    ...
    if (ctx.input.includePackagesForCarrierId) {
      let packagesResult = await client.listCarrierPackageTypes(ctx.input.includePackagesForCarrierId);
