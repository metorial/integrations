...
export let purchaseLabel = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    }).optional().describe('Inline shipment details for single-call label creation'),
    carrierAccount: z.string().optional().describe('Carrier account ID (required for single-call)'),
    servicelevelToken: z.string().optional().describe('Service level token (e.g. usps_priority, fedex_ground)'),
  ...
  .output(z.object({
    ...
    commercialInvoiceUrl: z.string().optional().describe('URL for commercial invoice (international)'),
