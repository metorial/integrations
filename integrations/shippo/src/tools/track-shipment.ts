...
let trackingEventSchema = z.object({
  status: z.string().optional().describe('Tracking status (PRE_TRANSIT, TRANSIT, DELIVERED, RETURNED, FAILURE, UNKNOWN)'),
  ...
  }).optional().describe('Location of the tracking event'),
});

export let trackShipment = SlateTool.create(
  spec,
  {
    name: 'Track Shipment',
    key: 'track_shipment',
  ...
  .output(z.object({
    ...
    trackingHistory: z.array(trackingEventSchema).optional().describe('Full tracking event history'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.getTrackingStatus(ctx.input.carrier, ctx.input.trackingNumber) as Record<string, any>;
