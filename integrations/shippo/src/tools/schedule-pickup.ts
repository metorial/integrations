...
export let schedulePickup = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Only supported for USPS and DHL Express carriers.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    ...
    location: z.object({
      buildingLocationType: z.enum([
        'Front Door', 'Back Door', 'Side Door', 'Knock on Door', 'Ring Bell',
        'Mail Room', 'Office', 'Reception', 'In/At Mailbox', 'Other',
      ]).optional().describe('Where the package is located'),
      buildingType: z.enum(['apartment', 'building', 'department', 'floor', 'room', 'suite']).optional(),
