...
export let batchEvents = SlateTrigger.create(
  spec,
  {
    name: 'Batch Events',
    key: 'batch_events',
    description: 'Triggered when a batch label operation is created or purchased. Covers both batch_created and batch_purchased events.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    labelUrl: z.string().optional().describe('URL for batch label download (available after purchase)'),
  }))
  .webhook({
