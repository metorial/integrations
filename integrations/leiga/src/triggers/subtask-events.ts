...
export let subtaskEventsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Subtask Events',
    key: 'subtask_events',
    description: 'Triggered when subtasks are added, updated, or deleted on issues in a Leiga project.',
  }
)
  .input(z.object({
    eventType: z.string().describe('Event type (e.g. Subtask.Add, Subtask.Update, Subtask.Delete)'),
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        type: typeMap[eventType] || `subtask.${eventType.toLowerCase().replace('subtask.', '')}`,
