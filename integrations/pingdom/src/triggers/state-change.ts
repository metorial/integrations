...
export let stateChange = SlateTrigger.create(
  spec,
  {
    name: 'Check State Change',
    key: 'state_change',
    description: 'Triggers when an uptime or transaction check changes state (e.g. UP → DOWN, DOWN → UP, SUCCESS → FAILING).',
  }
)
  .input(z.object({
    ...
    }).optional().describe('First probe server details'),
    secondProbe: z.object({
      ip: z.string().optional(),
      ipv6: z.string().optional(),
      location: z.string().optional(),
    }).optional().describe('Second probe server details'),
  }))
  .output(z.object({
    checkId: z.number().describe('Pingdom check ID'),
    checkName: z.string().describe('Check name'),
    ...
    secondProbeLocation: z.string().optional().describe('Location of second confirming probe'),
  }))
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      if (ctx.input.checkParams && typeof ctx.input.checkParams === 'object') {
        hostname = ctx.input.checkParams.hostname || ctx.input.checkParams.basic_auth?.hostname;
      }
