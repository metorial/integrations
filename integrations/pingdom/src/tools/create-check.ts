...
export let createCheck = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    resolution: z.number().optional().describe('Check interval in minutes: 1, 5, 15, 30, or 60. Default: 5'),
    ...
    responseTimeThreshold: z.number().optional().describe('Response time threshold in ms before triggering alert'),
    sendNotificationWhenDown: z.number().optional().describe('Number of consecutive failures before alerting'),
    notifyAgainEvery: z.number().optional().describe('Notify again every N results while down. 0 = no repeat'),
    notifyWhenBackup: z.boolean().optional().describe('Send notification when check recovers'),
    ...
    probeFilters: z.string().optional().describe('Probe filters, e.g. "region:NA" or "region:EU"'),
    url: z.string().optional().describe('Path to check on the hostname (HTTP/HTTPS checks). Default: "/"'),
    ...
    shouldContain: z.string().optional().describe('Response must contain this string (HTTP)'),
    shouldNotContain: z.string().optional().describe('Response must NOT contain this string (HTTP)'),
    postData: z.string().optional().describe('POST data body (HTTP)'),
    requestHeaders: z.record(z.string(), z.string()).optional().describe('Custom HTTP request headers'),
    verifyCertificate: z.boolean().optional().describe('Verify SSL certificate (HTTP). Default: true'),
    sslDownDaysBefore: z.number().optional().describe('Alert N days before SSL certificate expires'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.responseTimeThreshold !== undefined) data.responsetime_threshold = ctx.input.responseTimeThreshold;
    if (ctx.input.sendNotificationWhenDown !== undefined) data.sendnotificationwhendown = ctx.input.sendNotificationWhenDown;
    if (ctx.input.notifyAgainEvery !== undefined) data.notifyagainevery = ctx.input.notifyAgainEvery;
    if (ctx.input.notifyWhenBackup !== undefined) data.notifywhenbackup = ctx.input.notifyWhenBackup;
    if (ctx.input.customMessage) data.custom_message = ctx.input.customMessage;
    if (ctx.input.integrationIds?.length) data.integrationids = ctx.input.integrationIds.join(',');
    ...
    if (ctx.input.verifyCertificate !== undefined) data.verify_certificate = ctx.input.verifyCertificate;
    if (ctx.input.sslDownDaysBefore !== undefined) data.ssl_down_days_before = ctx.input.sslDownDaysBefore;
