...
export let listImages = SlateTool.create(
  ...
  .input(z.object({
    imageId: z.string().optional().describe('If provided, retrieves details for this specific image. Otherwise lists images with pagination.'),
    page: z.number().optional().describe('Page number for pagination (when listing images).'),
    pageSize: z.number().optional().describe('Number of images per page (when listing images).'),
  }))
  .output(z.object({
    images: z.any().optional().describe('Paginated list of images (when listing).'),
    imageDetails: z.any().optional().describe('Details of a specific image (when imageId is provided).'),
  }))
