...
export let registerParticipant = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    eventId: z.string().optional().describe('The event ID to register for. Required if referenceUrl is not provided.'),
    dateId: z.string().optional().describe('A specific event date/session ID to register for. Only used with eventId.'),
    referenceUrl: z.string().optional().describe('A Demio registration reference URL to register via. Alternative to eventId.'),
    name: z.string().describe('Full name of the registrant'),
    email: z.string().describe('Email address of the registrant'),
    customFields: z.record(z.string(), z.string()).optional().describe('Custom fields as key-value pairs. Keys must be Demio custom field unique identifiers.'),
  }))
  .output(z.object({
    hash: z.string().describe('Unique registration hash'),
    joinLink: z.string().describe('Unique join link for the registrant to access the webinar'),
  }))
