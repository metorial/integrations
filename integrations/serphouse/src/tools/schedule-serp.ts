...
let taskSchema = z.object({
  ...
  serpType: z.enum(['web', 'news', 'image', 'shop']).default('web').describe('Type of search results'),
  ...
  postbackUrl: z.string().optional().describe('Webhook URL to receive full results via POST when task completes'),
  pingbackUrl: z.string().optional().describe('Notification URL to receive a GET request when task completes'),
});

export let scheduleSerp = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum of 100 tasks per request.',
    ],
    tags: {
      readOnly: false,
    },
  ...
  .output(z.object({
    ...
    scheduledTasks: z.array(z.any()).describe('Array of scheduled task objects with assigned IDs'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let apiTasks = ctx.input.tasks.map((task) => ({
      ...
      ...(task.filterSimilarResults !== undefined ? { gfilter: task.filterSimilarResults ? 1 : 0 } : {}),
