...
export let sendOtp = SlateTool.create(spec, {
  ...
  .input(z.object({
    messages: z.array(z.object({
      ...
      timeout: z.number().describe('Timeout in seconds before falling back to next channel (0-300, use 0 for last channel)'),
      messageType: z.string().optional().describe('Message type (varies by channel)'),
      channelData: z.record(z.string(), z.any()).describe('Channel-specific payload data'),
    })).describe('Ordered list of channel delivery attempts'),
    webhookUrl: z.string().optional().describe('URL to receive success/failure webhook notifications'),
  ...
  .output(z.object({
    ...
    requestIds: z.array(z.string()).optional().describe('Request IDs for each channel attempt'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: success ? `OTP delivery initiated across ${ctx.input.messages.length} channel(s).` : `Failed to send OTP: ${result.message}`,
    };
  })
  .build();
