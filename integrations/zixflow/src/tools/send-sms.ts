...
export let sendSms = SlateTool.create(spec, {
  ...
  constraints: [
    'DLT template ID and entity ID are required for sending SMS in India.',
  ],
  tags: {
    destructive: false,
    readOnly: false,
  ...
  .input(z.object({
    ...
    number: z.string().describe('Recipient phone number with country prefix (e.g., "919090909090")'),
    message: z.string().describe('SMS message content'),
    isFlash: z.boolean().optional().describe('Send as flash SMS (displays immediately on screen)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: result.status ? `SMS sent to ${ctx.input.number} via ${ctx.input.route} route.` : `Failed to send SMS: ${result.message}`,
    };
  })
  .build();
