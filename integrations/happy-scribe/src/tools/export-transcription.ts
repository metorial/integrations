...
export let exportTranscription = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    format: z.enum(['txt', 'docx', 'pdf', 'srt', 'vtt', 'stl', 'avid', 'html', 'premiere', 'maxqda', 'json', 'fcp']).describe('Export format.'),
    showTimestamps: z.boolean().optional().describe('Include timestamps in the export (txt, docx, pdf only).'),
    timestampsFrequency: z.number().optional().describe('Frequency of inline timestamps in seconds (5-60). Requires showTimestamps to be true.'),
    showSpeakers: z.boolean().optional().describe('Include speaker labels in the export (txt, docx, pdf only).'),
    showComments: z.boolean().optional().describe('Include comments in the export (txt, docx, pdf only).'),
    showHighlights: z.boolean().optional().describe('Include highlights in the export (docx, pdf only).'),
    showHighlightsOnly: z.boolean().optional().describe('Export only highlighted portions (docx, pdf only).'),
  }))
  .output(z.object({
    ...
    downloadUrl: z.string().optional().nullable().describe('URL to download the exported file (available when state is "ready").'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: currentExport.state === 'ready'
