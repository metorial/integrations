...
Supports pagination for browsing large post histories.`,
  ...
  .input(
    z.object({
      platform: z.enum(['tiktok', 'x', 'instagram']).optional().describe('Filter by target platform'),
      accountId: z.string().optional().describe('Filter by connected account ID'),
      status: z.enum(['queued', 'processing', 'published', 'failed', 'scheduled']).optional().describe('Filter by post status'),
