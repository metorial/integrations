...
export let channelEvents = SlateTrigger.create(
  spec,
  {
    name: 'Channel Events',
    key: 'channel_events',
    description: 'Triggers on channel lifecycle and membership events, including channel creation, removal, property changes, freeze/unfreeze, user joins, leaves, invitations, and invitation declines.',
  }
)
  .input(z.object({
    ...
    })).optional().describe('Users involved in the event'),
    inviter: z.object({
      userId: z.string().optional(),
      nickname: z.string().optional(),
    }).optional().describe('User who triggered the invitation'),
  }))
  .output(z.object({
    channelUrl: z.string().optional().describe('Channel URL'),
    channelName: z.string().optional().describe('Channel name'),
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      let isChannelEvent = category.includes('channel:create') ||
