...
export let generateAiReply = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    })).min(1).describe('Conversation history to generate a reply from'),
    useStreamingResponse: z.boolean().optional().describe('Whether to use streaming response mode'),
  }))
  .output(z.object({
    replyMessages: z.array(z.object({
