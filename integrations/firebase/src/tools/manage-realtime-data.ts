...
export let manageRealtimeData = SlateTool.create(
  spec,
  {
...
Requires the **databaseUrl** to be configured in the project settings.`,
    ...
    constraints: [
      'Requires databaseUrl to be set in the project configuration.',
    ],
    tags: {
      destructive: true,
      readOnly: false,
  ...
  .input(z.object({
    operation: z.enum(['get', 'set', 'push', 'update', 'delete']).describe('Operation to perform'),
    path: z.string().describe('Database path, e.g. "users/abc" or "messages"'),
    data: z.any().optional().describe('Data payload. Required for set, push, and update operations.'),
    query: z.object({
      orderBy: z.string().optional().describe('Child key, value, or priority to order by. Use "$key", "$value", or a field name.'),
      startAt: z.union([z.string(), z.number()]).optional().describe('Start value for range query'),
      endAt: z.union([z.string(), z.number()]).optional().describe('End value for range query'),
      equalTo: z.union([z.string(), z.number(), z.boolean()]).optional().describe('Exact match filter'),
      ...
      shallow: z.boolean().optional().describe('Return only keys (no nested data). Useful for large datasets.'),
    }).optional().describe('Query parameters for get operations'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    if (!ctx.config.databaseUrl) {
      throw new Error('databaseUrl must be configured in project settings for Realtime Database operations');
    }
