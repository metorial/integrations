...
let columnDefinition = z.object({
  ...
  datatype: z.string().describe('Snowflake data type (e.g. VARCHAR, NUMBER(10,2), BOOLEAN, TIMESTAMP_NTZ)'),
...
export let manageTable = SlateTool.create(
  ...
  .input(z.object({
    ...
    tableName: z.string().optional().describe('Table name (required for get, create, delete)'),
    like: z.string().optional().describe('SQL LIKE pattern to filter tables when listing'),
    showLimit: z.number().optional().describe('Maximum number of tables to return when listing'),
    createMode: z.enum(['errorIfExists', 'orReplace', 'ifNotExists']).optional().describe('Creation behavior for create action'),
    columns: z.array(columnDefinition).optional().describe('Column definitions for creating a table'),
    comment: z.string().optional().describe('Table comment'),
    clusterBy: z.array(z.string()).optional().describe('Clustering key columns for the table'),
    ...
    ifExists: z.boolean().optional().describe('When true, delete succeeds even if the table does not exist'),
  }))
  .output(z.object({
    tables: z.array(z.record(z.string(), z.any())).optional().describe('List of tables (for list action)'),
    table: z.record(z.string(), z.any()).optional().describe('Table details (for get/create actions)'),
    deleted: z.boolean().optional().describe('Whether the table was deleted (for delete action)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      ...
      let table = await client.createTable(databaseName, schemaName, body, ctx.input.createMode);
      return {
        output: { table },
