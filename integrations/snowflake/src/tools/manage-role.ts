...
export let manageRole = SlateTool.create(
  ...
  .input(z.object({
    ...
    showLimit: z.number().optional().describe('Maximum number of roles to return when listing'),
    ...
    ifExists: z.boolean().optional().describe('When true, delete succeeds even if the role does not exist'),
  }))
  .output(z.object({
    roles: z.array(z.record(z.string(), z.any())).optional().describe('List of roles (for list action)'),
    role: z.record(z.string(), z.any()).optional().describe('Role details (for get/create actions)'),
