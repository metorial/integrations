...
export let getAveragesTool = SlateTool.create(
  spec,
  {
    name: 'Get Averages',
    key: 'get_averages',
  ...
  .input(z.object({
    ...
    attributeNames: z.string().optional().describe('Comma-separated attribute names to filter by'),
    includeHistorical: z.boolean().optional().describe('Include historical weekly averages'),
    dateMin: z.string().optional().describe('Earliest date for historical averages (YYYY-MM-DD)'),
    dateMax: z.string().optional().describe('Latest date for historical averages (YYYY-MM-DD)'),
