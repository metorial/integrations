...
export let createAttributeTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    attributes: z.array(z.object({
      templateName: z.string().optional().describe('Template name to create from (e.g. "facebook_reactions")'),
      label: z.string().optional().describe('Human-readable label for a custom attribute (e.g. "Time Reading")'),
      group: z.string().optional().describe('Group for a custom attribute (e.g. "media", "activity", "health")'),
      valueType: z.number().optional().describe('Value type for a custom attribute: 0=quantity, 1=decimal, 2=string, 3=duration, 4=time of day, 5=percentage, 7=boolean, 8=scale'),
      manual: z.boolean().optional().describe('Whether data will be entered manually by the user'),
    })).min(1).max(35).describe('Attributes to create'),
  }))
  .output(z.object({
