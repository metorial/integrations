...
export let listInvoicesTool = SlateTool.create(
  spec,
  {
    name: 'List Invoices',
    key: 'list_invoices',
  ...
  .input(z.object({
    ...
    status: z.enum(['draft', 'sent', 'overdue', 'paid', 'void', 'unpaid', 'partially_paid', 'viewed']).optional().describe('Filter by invoice status'),
    ...
    searchText: z.string().optional().describe('Search invoices by invoice number, customer name, or reference number'),
    page: z.number().optional().default(1),
    perPage: z.number().optional().default(200),
    sortColumn: z.enum(['customer_name', 'invoice_number', 'date', 'due_date', 'total', 'balance', 'created_time']).optional(),
