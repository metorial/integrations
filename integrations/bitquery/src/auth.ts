...
export let auth = SlateAuth.create()
  .output(z.object({
    token: z.string(),
    apiVersion: z.enum(['v1', 'v2']).describe('Which API version this token authenticates for'),
  }))
  .addTokenAuth({
    type: 'auth.token',
    name: 'API Key (V1)',
  ...
  .addTokenAuth({
    ...
    key: 'bearer_token_v2',
    inputSchema: z.object({
      bearerToken: z.string().describe('Your Bitquery V2 OAuth Bearer token (starts with ory_...)'),
    }),
    getOutput: async (ctx) => {
      return {
        output: {
