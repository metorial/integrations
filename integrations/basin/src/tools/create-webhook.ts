...
export let createWebhook = SlateTool.create(
  ...
  .input(z.object({
    ...
    format: z.string().optional().describe('Payload format: "json", "form_encoded", or a custom template format.'),
    triggerWhenSpam: z.boolean().optional().describe('Whether to fire webhooks for spam submissions. Defaults to false.'),
    enabled: z.boolean().optional().describe('Whether the webhook is active. Defaults to true.'),
  }))
  .output(z.object({
