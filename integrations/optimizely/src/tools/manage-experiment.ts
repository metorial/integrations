...
export let manageExperiment = SlateTool.create(
  spec,
  {
...
Use this to set up new experiments with variations and metrics, modify existing experiments, or retrieve experiment configuration and results.`,
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'get', 'delete', 'list', 'results']).describe('Action to perform'),
    projectId: z.number().optional().describe('Project ID (required for create and list)'),
    experimentId: z.number().optional().describe('Experiment ID (required for get, update, delete, results)'),
    name: z.string().optional().describe('Experiment name (for create/update)'),
    description: z.string().optional().describe('Experiment description (for create/update)'),
    type: z.string().optional().describe('Experiment type, e.g. "a/b" (for create)'),
    holdback: z.number().optional().describe('Holdback percentage 0-10000 (for create/update)'),
    status: z.string().optional().describe('Filter by status (for list)'),
    variations: z.array(variationSchema).optional().describe('Variations for the experiment (for create/update)'),
    metrics: z.array(metricSchema).optional().describe('Metrics to track (for create/update)'),
    audienceConditions: z.string().optional().describe('Audience conditions JSON string (for create/update)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'list': {
        if (!ctx.input.projectId) throw new Error('projectId is required for listing experiments');
      ...
      case 'create': {
        if (!ctx.input.projectId) throw new Error('projectId is required for creating experiments');
      ...
      case 'update': {
        ...
        if (ctx.input.variations !== undefined) updateData.variations = ctx.input.variations.map(v => ({ name: v.name, weight: v.weight }));
        if (ctx.input.metrics !== undefined) updateData.metrics = ctx.input.metrics.map(m => ({ aggregator: m.aggregator, event_id: m.eventId, scope: m.scope }));
        if (ctx.input.audienceConditions !== undefined) updateData.audience_conditions = ctx.input.audienceConditions;
