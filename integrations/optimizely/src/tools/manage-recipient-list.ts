...
export let manageRecipientList = SlateTool.create(
  spec,
  {
...
Recipient lists hold subscriber data that mailings are sent to. Use this to manage lists, add or remove individual recipients.`,
  ...
  .input(z.object({
    action: z.enum(['create', 'get', 'list', 'add_recipient', 'remove_recipient']).describe('Action to perform'),
    listId: z.number().optional().describe('Recipient list ID (required for get, add_recipient, remove_recipient)'),
    ...
    recipientId: z.number().optional().describe('Recipient ID to remove (for remove_recipient)'),
    attributes: z.record(z.string(), z.any()).optional().describe('Additional recipient attributes (for add_recipient)'),
