...
export let participateInPollTool = SlateTool.create(
  spec,
  {
...
Retrieve the poll first to see available options and their order.`,
  ...
  .input(z.object({
    ...
    participantEmail: z.string().optional().describe('Email of the participant (required if poll has askEmail enabled)'),
    preferences: z.array(z.number()).describe('Voting preferences for each option in order (0=no, 1=yes, 2=if need be)')
  }))
  .output(z.object({
