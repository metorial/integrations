...
export let manageDataItems = SlateTool.create(
  spec,
  {
...
Works with any Wix content management database collection by specifying the collection ID.`,
  ...
  .input(z.object({
    ...
    itemData: z.record(z.string(), z.any()).optional().describe('Item data as key-value pairs matching the collection schema (for create/update)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'update': {
        ...
        let result = await client.updateDataItem(ctx.input.collectionId, ctx.input.itemId, ctx.input.itemData);
        return {
          output: { dataItem: result.dataItem },
