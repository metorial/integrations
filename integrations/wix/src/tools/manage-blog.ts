...
export let manageBlog = SlateTool.create(
  spec,
  {
    name: 'Manage Blog',
    key: 'manage_blog',
...
Blog posts include title, content (rich text), categories, tags, SEO settings, and cover media.`,
  ...
  .input(z.object({
    action: z.enum(['list_posts', 'get_post', 'list_drafts', 'create_draft', 'update_draft', 'publish_draft']).describe('Operation to perform'),
    postId: z.string().optional().describe('Post ID (for get_post)'),
    draftPostId: z.string().optional().describe('Draft post ID (for update_draft, publish_draft)'),
    title: z.string().optional().describe('Post title (for create_draft/update_draft)'),
    richContent: z.any().optional().describe('Rich text content object (for create_draft/update_draft)'),
    excerpt: z.string().optional().describe('Post excerpt/summary (for create_draft/update_draft)'),
    featured: z.boolean().optional().describe('Whether the post is featured (for create_draft/update_draft)'),
    categoryIds: z.array(z.string()).optional().describe('Category IDs to assign (for create_draft/update_draft)'),
    tagIds: z.array(z.string()).optional().describe('Tag IDs to assign (for create_draft/update_draft)'),
    memberId: z.string().optional().describe('Member ID of the post author (required for third-party apps)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'update_draft': {
        if (!ctx.input.draftPostId) throw new Error('draftPostId is required for update_draft action');
        let draftPost: Record<string, any> = {};
        if (ctx.input.title) draftPost.title = ctx.input.title;
        if (ctx.input.richContent) draftPost.richContent = ctx.input.richContent;
        if (ctx.input.excerpt) draftPost.excerpt = ctx.input.excerpt;
      ...
      case 'publish_draft': {
        if (!ctx.input.draftPostId) throw new Error('draftPostId is required for publish_draft action');
