...
Returns the current status, and when complete, the full transcription results or error details.`,
  ...
  .input(z.object({
    operationName: z.string().describe('Full operation resource name (e.g. "projects/my-project/locations/us-central1/operations/12345").'),
  }))
  .output(z.object({
    ...
    }).optional().describe('Error details if the operation failed.'),
    response: z.record(z.string(), z.unknown()).optional().describe('Operation result when complete. For batch transcription, contains transcription results keyed by file URI.'),
    metadata: z.record(z.string(), z.unknown()).optional().describe('Operation metadata with progress information.'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let statusMessage = response.done
      ? (response.error ? `Operation **failed**: ${response.error.message}` : 'Operation **completed** successfully.')
