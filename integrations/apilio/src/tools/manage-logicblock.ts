...
export let manageLogicblock = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    active: z.boolean().describe('Set to true to activate or false to deactivate the logicblock'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let logicblock = await client.updateLogicblockActive(ctx.input.logicblockId, ctx.input.active);
