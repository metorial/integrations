...
export let updateNumericVariable = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['set', 'add', 'subtract']).describe('Action to perform: set an absolute value, add to current, or subtract from current'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let actionLabel = ctx.input.action === 'set' ? 'set to' : ctx.input.action === 'add' ? 'increased by' : 'decreased by';
