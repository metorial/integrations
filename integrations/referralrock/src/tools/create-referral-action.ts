...
export let createReferralAction = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    referralQuery: z.string().describe('Referral ID, email, or external ID to associate the action with'),
    programQuery: z.string().optional().describe('Program ID, name, or title'),
    name: z.string().optional().describe('Action name'),
    externalIdentifier: z.string().optional().describe('External ID to prevent duplicate actions'),
  }))
  .output(z.object({
