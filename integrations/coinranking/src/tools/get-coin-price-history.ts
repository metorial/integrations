...
export let getCoinPriceHistory = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'Free plan users are limited to a maximum time period of 1 year (1y).',
    ],
    tags: {
      destructive: false,
      readOnly: true,
  ...
  .input(z.object({
    ...
    referenceCurrencyUuid: z.string().optional().describe('UUID of currency for price calculation'),
    timePeriod: z.enum(['1h', '3h', '12h', '24h', '7d', '30d', '3m', '1y', '3y', '5y']).optional().describe('Time range for the price history. Defaults to 24h.'),
  }))
  .output(z.object({
