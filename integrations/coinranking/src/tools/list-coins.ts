...
export let listCoins = SlateTool.create(
  ...
  .input(z.object({
    ...
    tags: z.array(z.string()).optional().describe('Filter by tags (e.g. defi, stablecoin, nft)'),
    tiers: z.array(z.enum(['1', '2', '3'])).optional().describe('Filter by coin quality tiers (1 = highest quality)'),
    orderBy: z.enum(['marketCap', 'price', '24hVolume', 'change', 'listedAt']).optional().describe('Sort criteria'),
    ...
    limit: z.number().optional().describe('Number of results (max 100 for free plan, default 50)'),
