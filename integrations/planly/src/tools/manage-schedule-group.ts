...
let scheduleEntryInput = z.object({
  ...
  options: z.record(z.string(), z.unknown()).optional().describe('Platform-specific post options')
});
...
export let manageScheduleGroup = SlateTool.create(
  spec,
  {
...
Use **create** to make a new group (as draft or scheduled), **update** to modify an existing group, or **delete** to remove groups.`,
  ...
  .input(z.object({
    ...
    scheduleGroupIds: z.array(z.string()).optional().describe('Group IDs to delete (required for delete action)'),
    scheduleGroupId: z.string().optional().describe('Existing group ID (required for update action)'),
    ...
    schedules: z.array(scheduleEntryInput).optional().describe('Schedules in the group (required for create/update)'),
    validateOnly: z.boolean().optional().describe('Validate without creating (for create/update)')
  }))
  .output(z.object({
    groups: z.array(scheduleGroupResultSchema).optional().describe('Created or updated schedule groups'),
