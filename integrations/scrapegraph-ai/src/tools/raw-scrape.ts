...
export let rawScrape = SlateTool.create(
  spec,
  {
    name: 'Fetch Raw HTML',
    key: 'raw_scrape',
...
Optionally extracts branding information (colors, fonts, typography, UI styles, metadata).`,
  ...
  .input(z.object({
    ...
    branding: z.boolean().optional().describe('Extract brand design and metadata (colors, fonts, typography, UI styles)'),
    stealth: z.boolean().optional().describe('Enable anti-detection mode for protected sites (adds credits)'),
    waitMs: z.number().optional().describe('Milliseconds to wait for page load and JS rendering (default: 3000)'),
