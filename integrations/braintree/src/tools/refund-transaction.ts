...
export let refundTransaction = SlateTool.create(
  spec,
  {
...
Use "refund" for settled transactions and "reverse" to automatically void or refund based on status.`,
  ...
  .input(z.object({
    transactionId: z.string().describe('Braintree GraphQL transaction ID to refund or reverse'),
    mode: z.enum(['refund', 'reverse']).default('refund').describe('Whether to refund (for settled) or reverse (auto-decide void vs refund)'),
    amount: z.string().optional().describe('Amount to refund. Omit for full refund. Only applies to mode "refund".'),
