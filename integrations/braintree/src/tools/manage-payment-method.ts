...
export let vaultPaymentMethod = SlateTool.create(
  spec,
  {
...
By default, credit cards are verified before vaulting.`,
  ...
  .input(z.object({
    ...
    customerId: z.string().optional().describe('Customer ID to associate the payment method with'),
    verifyCard: z.boolean().optional().describe('Whether to verify the card before vaulting (default: true for cards)'),
  }))
  .output(z.object({
    paymentMethodId: z.string().optional().describe('GraphQL payment method ID'),
    legacyId: z.string().optional().describe('Legacy payment method token'),
    ...
    verificationStatus: z.string().optional().nullable().describe('Verification status if a card was verified'),
  }))
...
export let findPaymentMethod = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    ...
    let pm = parsed.creditCard || parsed.paypalAccount || parsed.venmoAccount || parsed.usBankAccount || parsed;
    let type = parsed.creditCard ? 'credit_card' : parsed.paypalAccount ? 'paypal' : parsed.venmoAccount ? 'venmo' : parsed.usBankAccount ? 'us_bank_account' : 'unknown';
...
export let deletePaymentMethod = SlateTool.create(
  spec,
  {
