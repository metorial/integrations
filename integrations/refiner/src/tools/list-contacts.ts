...
let contactSchema = z.object({
  ...
  remoteId: z.string().nullable().describe('External user ID used when identifying the contact'),
  ...
  firstSeenAt: z.string().nullable().describe('ISO 8601 timestamp of when the contact was first seen'),
  lastSeenAt: z.string().nullable().describe('ISO 8601 timestamp of when the contact was last seen'),
  attributes: z.record(z.string(), z.unknown()).describe('Custom traits and attributes of the contact'),
  ...
  }).nullable().describe('Account the contact is associated with'),
});
...
export let listContacts = SlateTool.create(
  ...
  .input(z.object({
    orderBy: z.enum(['first_seen_at', 'last_seen_at', 'last_form_submission_at']).optional().describe('Sort order for results'),
    surveyUuid: z.string().optional().describe('Filter contacts who responded to this survey'),
    ...
    pageCursor: z.string().optional().describe('Cursor for large dataset pagination (recommended for >10,000 entries)'),
