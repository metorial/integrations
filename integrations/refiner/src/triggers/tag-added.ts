...
export let tagAdded = SlateTrigger.create(
  spec,
  {
    name: 'Tag Added',
    key: 'tag_added',
    description: 'Triggers when a tag is added to a survey response. Useful for routing tagged responses to external systems for follow-up actions.',
  },
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    responseData: z.record(z.string(), z.unknown()).nullable().describe('Survey response data'),
