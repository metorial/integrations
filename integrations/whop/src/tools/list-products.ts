...
export let listProducts = SlateTool.create(
  ...
  .input(z.object({
    ...
    visibilities: z.array(z.enum(['visible', 'hidden', 'archived', 'quick_link'])).optional().describe('Filter by visibility status'),
    order: z.enum(['active_memberships_count', 'created_at', 'usd_gmv', 'usd_gmv_30_days']).optional().describe('Sort order field'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!companyId) {
      throw new Error('companyId is required. Provide it in the input or configure it globally.');
    }
