...
let emailResultSchema = z.object({
  ...
  freeAccount: z.string().optional().describe('Whether the email is from a free email provider'),
});
...
export let verifyEmail = SlateTool.create(
  ...
  .output(z.object({
    results: z.array(emailResultSchema).describe('Verification results for each email address'),
  }))
