...
let contentOptionsSchema = z.object({
  text: z.union([
    z.boolean(),
    z.object({
      maxCharacters: z.number().optional().describe('Maximum number of characters to return'),
      includeHtmlTags: z.boolean().optional().describe('Whether to include HTML tags in the text'),
    }),
  ]).optional().describe('Return the full text content of results as markdown'),
  highlights: z.union([
    z.boolean(),
    z.object({
      maxCharacters: z.number().optional().describe('Maximum number of characters per highlight'),
      query: z.string().optional().describe('Custom query for highlight extraction'),
    }),
  ]).optional().describe('Return key excerpts most relevant to the query'),
  summary: z.object({
    query: z.string().optional().describe('Custom query to tailor the summary'),
  }).optional().describe('Return a concise AI-generated summary'),
  livecrawl: z.enum(['always', 'preferred', 'fallback', 'never']).optional().describe('Live crawl mode for fetching fresh content'),
  ...
  }).optional().describe('Additional data to extract'),
}).optional().describe('Controls what content to retrieve for each result');
...
Filter results by domains, dates, and text content. Optionally retrieve full text, highlights, and summaries inline.`,
  ...
  .input(z.object({
    ...
    searchType: z.enum(['neural', 'auto', 'fast', 'deep']).optional().describe('Search method to use. Default: auto'),
    category: z.enum(['company', 'research paper', 'news', 'tweet', 'personal site', 'financial report', 'people']).optional().describe('Focus on a specific content category'),
    ...
    startPublishedDate: z.string().optional().describe('Filter results published after this ISO 8601 date'),
    endPublishedDate: z.string().optional().describe('Filter results published before this ISO 8601 date'),
    startCrawlDate: z.string().optional().describe('Filter results crawled after this ISO 8601 date'),
    endCrawlDate: z.string().optional().describe('Filter results crawled before this ISO 8601 date'),
    includeText: z.array(z.string()).optional().describe('Only include results containing these text strings'),
    excludeText: z.array(z.string()).optional().describe('Exclude results containing these text strings'),
