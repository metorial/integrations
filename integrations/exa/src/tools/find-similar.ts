...
export let findSimilarTool = SlateTool.create(
  spec,
  {
    name: 'Find Similar',
    key: 'find_similar',
...
Supports filtering by domains, dates, and text content. Optionally retrieve full text, highlights, and summaries.`,
  ...
  .input(z.object({
    ...
    numResults: z.number().min(1).max(100).optional().describe('Number of results (1-100, default: 10)'),
    includeDomains: z.array(z.string()).optional().describe('Only include results from these domains'),
    excludeDomains: z.array(z.string()).optional().describe('Exclude results from these domains'),
    startPublishedDate: z.string().optional().describe('Filter by publish date start (ISO 8601)'),
    endPublishedDate: z.string().optional().describe('Filter by publish date end (ISO 8601)'),
    ...
    includeText: z.array(z.string()).optional().describe('Only include results containing these strings'),
    excludeText: z.array(z.string()).optional().describe('Exclude results containing these strings'),
    contents: z.object({
      text: z.union([z.boolean(), z.object({ maxCharacters: z.number().optional() })]).optional(),
      highlights: z.union([z.boolean(), z.object({ maxCharacters: z.number().optional(), query: z.string().optional() })]).optional(),
