...
let pullZoneSchema = z.object({
  ...
  hostnames: z.array(z.object({
    ...
    isSystemHostname: z.boolean().optional().describe('Whether this is a system hostname'),
    hasCertificate: z.boolean().optional().describe('Whether a certificate is configured'),
  })).optional().describe('Configured hostnames'),
  storageZoneId: z.number().optional().describe('Linked storage zone ID'),
  cacheControlMaxAgeOverride: z.number().optional().describe('Cache control max age override in seconds'),
  ...
  zoneSecurityEnabled: z.boolean().optional().describe('Whether token authentication is enabled'),
...
export let managePullZone = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    pullZoneId: z.number().optional().describe('Pull zone ID. Required for get, update, and delete actions.'),
    search: z.string().optional().describe('Search term for filtering pull zones (list action only)'),
    page: z.number().optional().describe('Page number for pagination (list action only)'),
    perPage: z.number().optional().describe('Number of results per page, max 1000 (list action only)'),
    ...
    enableGeoZoneASIA: z.boolean().optional().describe('Enable Asia geo zone (create/update)'),
    enableGeoZoneSA: z.boolean().optional().describe('Enable South America geo zone (create/update)'),
    enableGeoZoneAF: z.boolean().optional().describe('Enable Africa geo zone (create/update)'),
    allowedReferrers: z.array(z.string()).optional().describe('List of allowed referrer domains (create/update)'),
    blockedReferrers: z.array(z.string()).optional().describe('List of blocked referrer domains (create/update)'),
    blockedIps: z.array(z.string()).optional().describe('List of blocked IP addresses (create/update)'),
    zoneSecurityEnabled: z.boolean().optional().describe('Enable token authentication (create/update)'),
    ignoreQueryStrings: z.boolean().optional().describe('Ignore query strings for caching (create/update)'),
    enableAutoSSL: z.boolean().optional().describe('Enable automatic SSL (create/update)'),
    enableWebSockets: z.boolean().optional().describe('Enable WebSocket support (create/update)'),
    cacheControlMaxAgeOverride: z.number().optional().describe('Cache control max age override in seconds (update)'),
    verifyOriginSSL: z.boolean().optional().describe('Verify origin SSL certificate (create/update)'),
  }))
  .output(z.object({
    pullZone: pullZoneSchema.optional().describe('Pull zone details (for get, create, update)'),
    pullZones: z.array(pullZoneSchema).optional().describe('List of pull zones (for list action)'),
    totalItems: z.number().optional().describe('Total number of pull zones (for list action)'),
    currentPage: z.number().optional().describe('Current page number (for list action)'),
    deleted: z.boolean().optional().describe('Whether the pull zone was deleted (for delete action)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'create': {
        ...
        if (ctx.input.enableGeoZoneUS !== undefined) data.EnableGeoZoneUS = ctx.input.enableGeoZoneUS;
        if (ctx.input.enableGeoZoneEU !== undefined) data.EnableGeoZoneEU = ctx.input.enableGeoZoneEU;
        if (ctx.input.enableGeoZoneASIA !== undefined) data.EnableGeoZoneASIA = ctx.input.enableGeoZoneASIA;
        if (ctx.input.enableGeoZoneSA !== undefined) data.EnableGeoZoneSA = ctx.input.enableGeoZoneSA;
        if (ctx.input.enableGeoZoneAF !== undefined) data.EnableGeoZoneAF = ctx.input.enableGeoZoneAF;
        ...
        if (ctx.input.zoneSecurityEnabled !== undefined) data.ZoneSecurityEnabled = ctx.input.zoneSecurityEnabled;
        if (ctx.input.ignoreQueryStrings !== undefined) data.IgnoreQueryStrings = ctx.input.ignoreQueryStrings;
        if (ctx.input.enableAutoSSL !== undefined) data.EnableAutoSSL = ctx.input.enableAutoSSL;
        if (ctx.input.enableWebSockets !== undefined) data.EnableWebSockets = ctx.input.enableWebSockets;
        if (ctx.input.verifyOriginSSL !== undefined) data.VerifyOriginSSL = ctx.input.verifyOriginSSL;
      ...
      case 'update': {
        ...
        if (ctx.input.storageZoneId !== undefined) data.StorageZoneId = ctx.input.storageZoneId;
        if (ctx.input.enableGeoZoneUS !== undefined) data.EnableGeoZoneUS = ctx.input.enableGeoZoneUS;
        if (ctx.input.enableGeoZoneEU !== undefined) data.EnableGeoZoneEU = ctx.input.enableGeoZoneEU;
        if (ctx.input.enableGeoZoneASIA !== undefined) data.EnableGeoZoneASIA = ctx.input.enableGeoZoneASIA;
        if (ctx.input.enableGeoZoneSA !== undefined) data.EnableGeoZoneSA = ctx.input.enableGeoZoneSA;
        ...
        if (ctx.input.enableAutoSSL !== undefined) data.EnableAutoSSL = ctx.input.enableAutoSSL;
        if (ctx.input.enableWebSockets !== undefined) data.EnableWebSockets = ctx.input.enableWebSockets;
        if (ctx.input.cacheControlMaxAgeOverride !== undefined) data.CacheControlMaxAgeOverride = ctx.input.cacheControlMaxAgeOverride;
        if (ctx.input.verifyOriginSSL !== undefined) data.VerifyOriginSSL = ctx.input.verifyOriginSSL;
