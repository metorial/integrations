...
let videoLibrarySchema = z.object({
  ...
  apiKey: z.string().optional().describe('Library API key (only if includeAccessKey is true)'),
  ...
  enableTokenAuthentication: z.boolean().optional().describe('Whether token auth is enabled'),
  ...
  enabledResolutions: z.string().optional().describe('Comma-separated list of enabled resolutions'),
}).passthrough();

export let manageVideoLibrary = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    videoLibraryId: z.number().optional().describe('Video library ID. Required for get, update, and delete.'),
    includeAccessKey: z.boolean().optional().describe('Include the library API key in the response (list/get)'),
    name: z.string().optional().describe('Library name (create/update)'),
    replicationRegions: z.array(z.string()).optional().describe('Replication region codes (create)'),
    webhookUrl: z.string().optional().describe('Webhook URL for video processing notifications (update)'),
    enableTokenAuthentication: z.boolean().optional().describe('Enable token authentication (update)'),
    ...
    keepOriginalFiles: z.boolean().optional().describe('Keep original uploaded files (update)'),
    enabledResolutions: z.string().optional().describe('Comma-separated enabled resolutions, e.g. "240p,360p,480p,720p,1080p" (update)'),
  ...
  .output(z.object({
    ...
    videoLibraries: z.array(videoLibrarySchema).optional().describe('List of video libraries'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'get': {
        let lib = await client.getVideoLibrary(ctx.input.videoLibraryId!, ctx.input.includeAccessKey);
        return {
      ...
      case 'update': {
        ...
        if (ctx.input.enableTokenAuthentication !== undefined) data.EnableTokenAuthentication = ctx.input.enableTokenAuthentication;
        if (ctx.input.enableDrm !== undefined) data.EnableDRM = ctx.input.enableDrm;
        if (ctx.input.enableMp4Fallback !== undefined) data.EnableMP4Fallback = ctx.input.enableMp4Fallback;
        if (ctx.input.keepOriginalFiles !== undefined) data.KeepOriginalFiles = ctx.input.keepOriginalFiles;
        if (ctx.input.enabledResolutions !== undefined) data.EnabledResolutions = ctx.input.enabledResolutions;
        if (ctx.input.playerKeyColor !== undefined) data.PlayerKeyColor = ctx.input.playerKeyColor;
