...
export let searchGrantAwards = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    awardType: z.enum(['prime', 'subgrant']).default('prime').describe('Type of grant award to search'),
    ...
    cfdaProgramNumber: z.string().optional().describe('CFDA/Assistance Listing program number (e.g. "93.778") - prime grants only'),
    searchId: z.string().optional().describe('HigherGov saved search ID (prime grants only)'),
    ordering: z.string().optional().describe('Sort order (e.g. -action_date, -total_obligated_amount)'),
  ...
  .output(z.object({
    awards: z.array(z.record(z.string(), z.unknown())).describe('List of grant award records'),
