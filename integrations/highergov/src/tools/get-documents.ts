...
export let getDocuments = SlateTool.create(
  spec,
  {
    name: 'Get Documents',
    key: 'get_documents',
  ...
  .input(z.object({
    relatedKey: z.string().describe('Document-related key from the document_path field of an opportunity or award'),
  ...
  .output(z.object({
    documents: z.array(z.record(z.string(), z.unknown())).describe('List of document records with download URLs'),
