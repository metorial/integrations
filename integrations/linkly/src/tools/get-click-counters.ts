...
export let getClickCounters = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Rate limited — may return 429 errors on excessive use.'
    ],
    tags: {
      destructive: false,
      readOnly: true
  ...
  .input(z.object({
    counter: z.enum(['country', 'platform', 'browser', 'referer', 'isp', 'link_id', 'top_params']).describe('Dimension to group clicks by'),
