...
export let manageWorkerTool = SlateTool.create(
  spec,
  {
    name: 'Manage Worker',
    key: 'manage_worker',
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'delete']).describe('Action to perform on the worker'),
    workerName: z.string().optional().describe('Worker name to target'),
    groupName: z.string().optional().describe('Group name to target (applies action to all workers in group)'),
    customerId: z.number().optional().describe('Customer ID. If omitted, the main account is used.'),
    workerType: z.enum(['nvidia', 'amd', 'asic']).optional().describe('Worker type (required for create)'),
    system: z.string().optional().describe('System type, e.g. "windows", "msos", "antminer" (required for create)'),
    ...
    set: z.record(z.string(), z.any()).optional().describe('Object of values to set. Keys: name, ip, username, password, notes, client, config, clocktune, profitswitch, electricityprice, consumption'),
    add: z.record(z.string(), z.any()).optional().describe('Object of values to add. Keys: group (comma-separated group names)'),
    remove: z.record(z.string(), z.any()).optional().describe('Object of values to remove. Keys: group (comma-separated group names)')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    if (!ctx.auth.token) {
      throw new Error('Bearer token is required for worker management. Use the "API Credentials" authentication method.');
    }
    ...
    switch (ctx.input.action) {
      case 'create': {
        if (!ctx.input.workerName || !ctx.input.workerType || !ctx.input.system) {
          throw new Error('workerName, workerType, and system are required to create a worker.');
        }
      ...
      case 'update': {
        ctx.progress(`Updating worker${ctx.input.workerName ? ' ' + ctx.input.workerName : ''}...`);
      ...
      case 'delete': {
        ctx.progress(`Deleting worker${ctx.input.workerName ? ' ' + ctx.input.workerName : ''}...`);
