...
export let createBanner = SlateTool.create(
  spec,
  {
    name: 'Create Banner',
    key: 'create_banner',
...
The tool will wait for the banner to finish processing and return the resulting files with download URLs.`,
  ...
  .input(z.object({
    ...
    modes: z.record(z.string(), z.any()).describe('Output format modes. Keys are format names (png, jpeg, webp, html5, video). Values are format-specific options (e.g., { "png": {}, "video": { "fps": 30, "crf": 18 } })'),
    sizeIds: z.union([z.string(), z.array(z.string())]).optional().describe('Size IDs to generate. Use "all" for all available sizes, or provide specific size IDs. Omit to use the default size only.'),
    overrides: z.record(z.string(), z.any()).optional().describe('Template content overrides. Keys are item names from the editor. Values can include text, url (for images), and css properties (e.g., { "Text_item_1": { "text": "Hello", "css": { "color": "red" } } })'),
    waitForCompletion: z.boolean().optional().describe('Whether to poll and wait for the banner to be ready. Defaults to true.')
  }))
  .output(z.object({
