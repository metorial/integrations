...
export let shortenLink = SlateTool.create(
  spec,
  {
    name: 'Shorten Link',
    key: 'shorten_link',
...
Use this to create trackable short links for social media posts with optional promotional overlays.`,
  ...
  .input(z.object({
    ...
    ctaId: z.number().int().describe('CTA identifier to attach to the shortened link. Use the "List Link CTAs" tool to get available CTA IDs.'),
  }))
  .output(z.object({
