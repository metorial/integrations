...
export let workflowEvents = SlateTrigger.create(
  spec,
  {
    name: 'Workflow Events',
    key: 'workflow_events',
    description: 'Receive real-time notifications when events occur in Hystruct workflows, such as job completions or data updates.',
  }
)
  .input(z.object({
    ...
    eventPayload: z.record(z.string(), z.unknown()).describe('The full event payload from Hystruct.'),
  }))
  .output(z.object({
  ...
  .webhook({
    ...
    autoUnregisterWebhook: async (ctx) => {
      ...
      let webhook = webhooksResponse.webhooks.find(
        (w) => w.url === ctx.input.webhookBaseUrl
      );
