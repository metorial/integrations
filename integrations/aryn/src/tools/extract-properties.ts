...
export let extractProperties = SlateTool.create(
  spec,
  {
...
Define the properties to extract including name, type, description, and optional defaults. The LLM will analyze each document and extract the requested values.`,
  ...
  .input(z.object({
    ...
    properties: z.array(z.object({
      ...
      fieldType: z.enum(['String', 'Number', 'Boolean']).describe('Data type of the property'),
      description: z.string().optional().describe('Description to guide the LLM on what to extract'),
      defaultValue: z.string().optional().describe('Default value if extraction fails'),
      examples: z.array(z.string()).optional().describe('Example values to guide extraction'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    ctx.info(`Extracting ${ctx.input.properties.length} properties from DocSet ${ctx.input.docsetId}`);
...
export let deleteProperties = SlateTool.create(
  spec,
  {
...
export let suggestProperties = SlateTool.create(
  spec,
  {
