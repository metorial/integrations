...
export let updateAsset = SlateTool.create(
  ...
  .input(z.object({
    ...
    customFields: z.record(z.string(), z.string()).optional().describe('Updated custom metadata fields (key-value pairs)'),
  }))
  .output(z.object({
    ...
    customFields: z.record(z.string(), z.string()).optional().describe('Updated custom fields'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.updateAsset(ctx.input.sourceId, ctx.input.originPath, attributes);
    let a = result.data;
