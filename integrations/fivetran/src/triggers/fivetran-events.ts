...
export let fivetranEvents = SlateTrigger.create(
  spec,
  {
    name: 'Fivetran Events',
    key: 'fivetran_events',
    description: 'Receives webhook notifications for sync events, transformation run events, and dbt run events from Fivetran.',
  },
)
  .input(z.object({
    event: z.string().describe('Event type (e.g., sync_start, sync_end, transformation_run_succeeded)'),
    ...
    eventStatus: z.string().optional().describe('Status from event data (e.g., SUCCESSFUL, FAILURE_WITH_TASK)'),
  ...
  .output(z.object({
    ...
    status: z.string().optional().describe('Status from event data (e.g., SUCCESSFUL, FAILURE_WITH_TASK)'),
