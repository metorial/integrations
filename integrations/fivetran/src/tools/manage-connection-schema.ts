...
export let getConnectionSchema = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    schemaChangeHandling: z.string().optional().describe('How new schema changes are handled: ALLOW_ALL, ALLOW_COLUMNS, or BLOCK_ALL'),
    schemas: z.record(z.string(), z.any()).optional().describe('Schema configuration map with enabled/disabled tables and columns'),
  }))
...
export let updateConnectionSchema = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    connectionId: z.string().describe('ID of the connection'),
    schemaChangeHandling: z.enum(['ALLOW_ALL', 'ALLOW_COLUMNS', 'BLOCK_ALL']).optional().describe('How to handle new schema elements'),
    schemas: z.record(z.string(), z.any()).optional().describe('Schema configuration object with enabled/disabled schemas, tables, and columns'),
  }))
  .output(z.object({
    schemaChangeHandling: z.string().optional().describe('Updated schema change handling mode'),
    schemas: z.record(z.string(), z.any()).optional().describe('Updated schema configuration'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new FivetranClient(ctx.auth.token);

    let body: Record<string, any> = {};
    if (ctx.input.schemaChangeHandling) body.schema_change_handling = ctx.input.schemaChangeHandling;
...
export let reloadConnectionSchema = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    connectionId: z.string().describe('ID of the connection'),
    excludeMode: z.enum(['EXCLUDE', 'PRESERVE']).optional().describe('EXCLUDE disables all new items by default, PRESERVE keeps current settings'),
  }))
  .output(z.object({
    schemaChangeHandling: z.string().optional().describe('Schema change handling mode'),
    schemas: z.record(z.string(), z.any()).optional().describe('Reloaded schema configuration'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new FivetranClient(ctx.auth.token);
    let schema = await client.reloadConnectionSchema(ctx.input.connectionId, ctx.input.excludeMode);
