...
let connectorTypeSchema = z.object({
  ...
  serviceStatus: z.string().optional().describe('Availability status (general_availability, beta, sunset, etc.)'),
...
export let listConnectorTypes = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({}))
  .output(z.object({
    connectorTypes: z.array(connectorTypeSchema).describe('List of available connector types'),
  }))
...
export let getConnectorType = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    serviceId: z.string().describe('Service identifier (e.g., "github", "salesforce", "google_sheets")'),
  }))
  .output(z.object({
