...
export let getGroup = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeConnections: z.boolean().optional().default(false).describe('Also list all connections in this group'),
    includeUsers: z.boolean().optional().default(false).describe('Also list all users in this group'),
  }))
  .output(z.object({
    group: groupOutputSchema,
    connections: z.array(z.record(z.string(), z.any())).optional().describe('Connections in the group (if requested)'),
    users: z.array(z.record(z.string(), z.any())).optional().describe('Users in the group (if requested)'),
  }))
