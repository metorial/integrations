...
export let getPhotoAlbum = SlateTool.create(
  spec,
  {
...
Covers photo albums, products, digital products, properties, and classifieds.`,
  ...
  .input(z.object({
    ...
    property: z.string().optional().describe('The column/field name to search by. Used when groupId is not provided.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.groupId) {
      ...
      result = await client.getPortfolioGroupByProperty(ctx.input.property, ctx.input.propertyValue);
    } else {
...
export let createPhotoAlbum = SlateTool.create(
  spec,
  {
...
Supports photo albums, products, digital products, properties, and classifieds with multiple images.`,
  ...
  .input(z.object({
    ...
    dataType: z.string().optional().describe('The data type identifier. Defaults to "4" for multi-image.'),
    ...
    propertyStatus: z.string().optional().describe('Property status (for real estate listings).'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.autoImageImport !== undefined) data.auto_image_import = ctx.input.autoImageImport ? '1' : '0';
...
export let updatePhotoAlbum = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Only one album can be updated at a time.',
    ],
  }
)
  .input(z.object({
...
export let deletePhotoAlbum = SlateTool.create(
  spec,
  {
...
export let searchPhotoAlbums = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    additionalFilters: z.record(z.string(), z.string()).optional().describe('Any additional search filters.'),
  }))
  .output(z.object({
    status: z.string().describe('Response status from the API.'),
