...
export let getCategory = SlateTool.create(
  spec,
  {
    name: 'Get Post Type',
    key: 'get_post_type',
...
export let createCategory = SlateTool.create(
  ...
  .input(z.object({
    ...
    additionalFields: z.record(z.string(), z.string()).optional().describe('Any additional configuration fields as key-value pairs.'),
  }))
  .output(z.object({
    status: z.string().describe('Response status from the API.'),
...
export let searchCategories = SlateTool.create(
  spec,
  {
