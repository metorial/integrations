...
Use data type "20" for image posts and "9" for video posts.`,
  ...
  .input(z.object({
    ...
    dataType: z.string().optional().describe('The data type identifier (e.g., "20" for images, "9" for videos). Defaults to "20".'),
    postTitle: z.string().optional().describe('Title of the post.'),
    postContent: z.string().optional().describe('Body content of the post.'),
    postStatus: z.string().optional().describe('Status of the post (e.g., "active", "pending").'),
    postImage: z.string().optional().describe('URL of the post image.'),
    autoImageImport: z.boolean().optional().describe('Whether to automatically import the image.'),
    postTags: z.string().optional().describe('Comma-separated tags for the post.'),
    postLiveDate: z.string().optional().describe('Date when the post goes live (YYYY-MM-DD).'),
    postExpireDate: z.string().optional().describe('Date when the post expires (YYYY-MM-DD).'),
