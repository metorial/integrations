...
export let listTimeEntries = SlateTool.create(
  ...
  .input(z.object({
    ...
    createdFrom: z.string().optional().describe('Filter entries from this date (yyyymmdd format)'),
    createdTo: z.string().optional().describe('Filter entries to this date (yyyymmdd format)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let timeEntries = data.map((e: any) => ({
      ...
      billed: e.billed === 1 || e.billed === true ? true : e.billed === 0 || e.billed === false ? false : undefined,
