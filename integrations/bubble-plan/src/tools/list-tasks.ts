...
export let listTasks = SlateTool.create(
  ...
  .input(z.object({
    ...
    dueFrom: z.string().optional().describe('Filter tasks due from this date (yyyymmdd format)'),
    dueTo: z.string().optional().describe('Filter tasks due to this date (yyyymmdd format)'),
    createdFrom: z.string().optional().describe('Filter tasks created from this date (yyyymmdd format)'),
    createdTo: z.string().optional().describe('Filter tasks created to this date (yyyymmdd format)'),
    completedFrom: z.string().optional().describe('Filter tasks completed from this date (yyyymmdd format)'),
    completedTo: z.string().optional().describe('Filter tasks completed to this date (yyyymmdd format)'),
    tag: z.string().optional().describe('Filter tasks by tag'),
    status: z.enum(['open', 'completed', 'active', 'onhold', 'inactive', 'archived']).optional().describe('Filter tasks by status'),
