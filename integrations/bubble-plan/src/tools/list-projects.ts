...
export let listProjects = SlateTool.create(
  ...
  .input(z.object({
    ...
    dueFrom: z.string().optional().describe('Filter projects due from this date (yyyymmdd format)'),
    dueTo: z.string().optional().describe('Filter projects due to this date (yyyymmdd format)'),
    createdFrom: z.string().optional().describe('Filter projects created from this date (yyyymmdd format)'),
    createdTo: z.string().optional().describe('Filter projects created to this date (yyyymmdd format)'),
    tag: z.string().optional().describe('Filter projects by tag'),
    status: z.enum(['open', 'closed', 'templates', 'archived']).optional().describe('Filter projects by status'),
    order: z.enum(['projectname', 'duedate', 'progress', 'billablehours', 'latest']).optional().describe('Sort order for results'),
