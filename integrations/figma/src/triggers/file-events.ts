...
export let fileEvents = SlateTrigger.create(
  spec,
  {
    name: 'File Events',
    key: 'file_events',
    description: 'Triggers when files are updated, deleted, or receive new version entries in a Figma team. Covers FILE_UPDATE, FILE_DELETE, and FILE_VERSION_UPDATE events.',
    instructions: [
  ...
  .input(z.object({
    eventType: z.string().describe('The Figma event type (FILE_UPDATE, FILE_DELETE, FILE_VERSION_UPDATE, PING)'),
    webhookId: z.string().optional().describe('ID of the webhook that triggered this event'),
    passcode: z.string().optional().describe('Passcode from the webhook payload for verification'),
    ...
    description: z.string().optional().describe('Version description for FILE_VERSION_UPDATE events'),
  ...
  .output(z.object({
    ...
    versionLabel: z.string().optional().describe('Version label (for version update events)'),
    versionDescription: z.string().optional().describe('Version description (for version update events)'),
    triggeredBy: z.object({
      userId: z.string().optional(),
      handle: z.string().optional(),
      imageUrl: z.string().optional()
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      let type = eventTypeMap[ctx.input.eventType] || `file.${ctx.input.eventType.toLowerCase()}`;
