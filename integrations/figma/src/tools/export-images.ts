...
export let exportImages = SlateTool.create(
  spec,
  {
    name: 'Export Images',
    key: 'export_images',
  ...
  .input(z.object({
    ...
    format: z.enum(['png', 'jpg', 'svg', 'pdf']).optional().default('png').describe('Image output format'),
    scale: z.number().optional().describe('Image scale factor (0.01 to 4)'),
    svgOutlineText: z.boolean().optional().describe('For SVG: whether text elements should be outlined'),
    svgIncludeId: z.boolean().optional().describe('For SVG: whether to include id attributes for all elements'),
    svgIncludeNodeId: z.boolean().optional().describe('For SVG: whether to include node IDs as data attributes'),
    svgSimplifyStroke: z.boolean().optional().describe('For SVG: whether to simplify inside/outside strokes'),
    contentsOnly: z.boolean().optional().describe('Whether to render only the content and not clip to the node bounds'),
    useAbsoluteBounds: z.boolean().optional().describe('Use full dimensions of the node regardless of cropping')
  }))
  .output(z.object({
    images: z.record(z.string(), z.string().nullable()).describe('Map of node IDs to image download URLs (null if rendering failed)')
  }))
