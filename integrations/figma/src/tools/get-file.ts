...
export let getFile = SlateTool.create(
  ...
  .input(z.object({
    ...
    nodeIds: z.array(z.string()).optional().describe('Specific node IDs to retrieve. If omitted, returns the full file'),
    version: z.string().optional().describe('File version ID to retrieve a specific version'),
    depth: z.number().optional().describe('Depth of the node tree to return. Omit for full depth'),
  ...
  .output(z.object({
    ...
    components: z.record(z.string(), z.any()).optional().describe('Map of component metadata by node ID'),
    styles: z.record(z.string(), z.any()).optional().describe('Map of style metadata by style ID')
  }))
