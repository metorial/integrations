...
export let generateAssetTool = SlateTool.create(
  spec,
  {
...
- **heygen**: Avatar video generation.`,
  ...
  .input(z.object({
    provider: z.enum(['shotstack', 'elevenlabs', 'd-id', 'stability-ai', 'heygen']).describe('AI provider to use'),
    options: z.record(z.string(), z.any()).describe('Provider-specific generation options (e.g. type, text, prompt, voice, imageUrl)'),
  }))
  .output(z.object({
...
export let getGeneratedAssetTool = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    type: z.string().optional().describe('Generation type (e.g. text-to-speech, text-to-image)'),
