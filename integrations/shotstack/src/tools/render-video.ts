...
let destinationSchema = z.object({
  provider: z.enum(['shotstack', 's3', 'mux', 'google-cloud-storage', 'google-drive', 'vimeo']).describe('Destination provider'),
  exclude: z.boolean().optional().describe('Exclude this destination (only for shotstack provider)'),
  options: z.record(z.string(), z.any()).optional().describe('Provider-specific options (e.g. bucket, region, prefix for S3)'),
});

let outputSchema = z.object({
  format: z.enum(['mp4', 'gif', 'jpg', 'png', 'bmp', 'mp3']).optional().describe('Output format'),
  resolution: z.enum(['preview', 'mobile', 'sd', 'hd', '1080']).optional().describe('Output resolution preset'),
  aspectRatio: z.enum(['16:9', '9:16', '1:1', '4:5', '4:3']).optional().describe('Aspect ratio'),
  size: z.object({
  ...
  }).optional().describe('Custom output size (mutually exclusive with resolution/aspectRatio)'),
  ...
  scaleTo: z.enum(['preview', 'mobile', 'sd', 'hd', '1080']).optional().describe('Scale to resolution'),
  ...
  }).optional().describe('Render a specific range of the timeline'),
  poster: z.object({
    capture: z.number().describe('Frame capture time in seconds'),
  }).optional().describe('Generate a poster image'),
  thumbnail: z.object({
    capture: z.number().describe('Frame capture time in seconds'),
    scale: z.number().optional().describe('Scale factor (0-1)'),
  }).optional().describe('Generate a thumbnail image'),
  destinations: z.array(destinationSchema).optional().describe('Output destinations'),
});

let clipAssetSchema = z.record(z.string(), z.any()).describe('Asset object (video, image, title, text, audio, html, etc.) with a "type" field');

let transitionSchema = z.object({
  in: z.string().optional().describe('Transition in effect (e.g. fade, reveal, wipeLeft, slideLeft, zoom)'),
...
let transformSchema = z.object({
  ...
  }).optional().describe('Flip transformation'),
});

let clipSchema = z.object({
  asset: clipAssetSchema,
  start: z.number().describe('Start time in seconds on the timeline'),
  length: z.union([z.number(), z.literal('auto')]).describe('Duration in seconds, or "auto" to match asset length'),
  fit: z.enum(['cover', 'contain', 'crop', 'none']).optional().describe('How the asset fits within the frame'),
  scale: z.number().optional().describe('Scale factor'),
  position: z.enum(['top', 'topRight', 'right', 'bottomRight', 'bottom', 'bottomLeft', 'left', 'topLeft', 'center']).optional().describe('Position within the frame'),
  offset: z.object({ x: z.number(), y: z.number() }).optional().describe('Offset from position (-1 to 1)'),
  transition: transitionSchema.optional().describe('Transition effects'),
  effect: z.string().optional().describe('Motion effect (zoomIn, zoomOut, slideLeft, slideRight, slideUp, slideDown)'),
  filter: z.string().optional().describe('Visual filter (boost, contrast, darken, greyscale, lighten, muted, invert, negative)'),
...
let timelineSchema = z.object({
  tracks: z.array(trackSchema).describe('Array of tracks. Tracks are layered with the first track on top.'),
  ...
  fonts: z.array(z.object({ src: z.string().describe('URL of font file') })).optional().describe('Custom fonts to load'),
  cache: z.boolean().optional().describe('Cache source assets for faster rendering'),
});

export let renderVideoTool = SlateTool.create(
  spec,
  {
    name: 'Render Video',
    key: 'render_video',
...
Use merge fields to substitute dynamic values into the edit. Configure output destinations including Shotstack CDN, AWS S3, Mux, Google Cloud Storage, Google Drive, and Vimeo.`,
  ...
  .input(z.object({
    ...
    merge: z.array(mergeFieldSchema).optional().describe('Merge field replacements for dynamic content'),
    callback: z.string().optional().describe('Webhook URL for render completion notifications'),
  }))
  .output(z.object({
