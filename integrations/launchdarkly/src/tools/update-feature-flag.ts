...
export let updateFeatureFlag = SlateTool.create(
  ...
  .input(z.object({
    ...
    environmentKey: z.string().optional().describe('Environment key (required for targeting/toggle changes). Falls back to config default.'),
    instructions: z.array(z.record(z.string(), z.any())).describe('Array of semantic patch instructions. Each instruction is an object with a "kind" field and kind-specific parameters. Common kinds: "turnFlagOn", "turnFlagOff", "updateName", "updateDescription", "addTags", "removeTags", "updateFallthroughVariationOrRollout", "addTargets", "removeTargets", "addRule", "removeRule", "addPrerequisite", "removePrerequisite".'),
  }))
  .output(z.object({
