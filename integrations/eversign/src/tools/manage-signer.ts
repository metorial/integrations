...
export let manageSigner = SlateTool.create(
  spec,
  {
    name: 'Manage Signer',
    key: 'manage_signer',
  ...
  .input(z.object({
    ...
    action: z.enum(['remind', 'reassign']).describe('Action to perform: send a reminder or reassign to a new signer'),
    newSignerName: z.string().optional().describe('New signer name (required for reassign)'),
    newSignerEmail: z.string().optional().describe('New signer email (required for reassign)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: ctx.input.action === 'remind'
