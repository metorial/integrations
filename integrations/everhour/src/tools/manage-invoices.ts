...
export let createInvoice = SlateTool.create(
  ...
  .input(z.object({
    ...
    dateFrom: z.string().optional().describe('Start date for time/expense inclusion (YYYY-MM-DD)'),
    dateTill: z.string().optional().describe('End date for time/expense inclusion (YYYY-MM-DD)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includeExpenses !== undefined) data.includeExpenses = ctx.input.includeExpenses;
