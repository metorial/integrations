...
export let listProjects = SlateTool.create(
  ...
  .input(z.object({
    ...
    platform: z.string().optional().describe('Filter by platform: ev, as, jr, tr, cl, mo, gh, b3, etc.'),
...
export let updateProject = SlateTool.create(
  ...
  .input(z.object({
    ...
    }).optional().describe('Billing configuration'),
    budget: z.object({
    ...
    }).optional().describe('Budget configuration'),
    rate: z.object({
      type: z.enum(['project_rate', 'user_rate']).describe('Rate type'),
      rate: z.number().optional().describe('Flat rate in cents (for project_rate)'),
      userRateOverrides: z.record(z.string(), z.number()).optional().describe('Per-user rate overrides (user ID -> cents)'),
    }).optional().describe('Rate configuration'),
  }))
  .output(projectSchema)
  .handleInvocation(async (ctx) => {
    let client = new EverhourClient(ctx.auth.token);
