...
let assignmentSchema = z.object({
  ...
  timeOffPeriod: z.string().optional().describe('Time off period: full-day, half-day, or quarter-day'),
  status: z.string().optional().describe('Status (for time-off: pending or approved)'),
});

export let listAssignments = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    type: z.enum(['assignment', 'time-off']).optional().describe('Filter by assignment type'),
...
export let createAssignment = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    timeSeconds: z.number().optional().describe('Scheduled time in seconds (for project type)'),
    timeOffTypeId: z.number().optional().describe('Time off type ID (required for time-off type)'),
    timeOffPeriod: z.enum(['full-day', 'half-day', 'quarter-day']).optional().describe('Time off period'),
    status: z.enum(['pending', 'approved']).optional().describe('Initial status (for time-off)'),
    forceOverride: z.boolean().optional().describe('Force override existing assignments'),
  }))
  .output(z.object({
    assignments: z.array(assignmentSchema).describe('Created assignments (may be multiple when assigning multiple users)'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new EverhourClient(ctx.auth.token);
    let data: any = {
...
export let updateAssignment = SlateTool.create(
  spec,
  {
...
export let deleteAssignment = SlateTool.create(
  spec,
  {
