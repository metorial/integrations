...
export let calculateIsoline = SlateTool.create(
  ...
  .input(z.object({
    ...
    type: z.enum(['time', 'distance']).describe('"time" for isochrone (seconds), "distance" for isodistance (meters)'),
    mode: travelModeEnum.describe('Travel mode'),
    range: z.string().describe('Comma-separated range values (seconds for time, meters for distance)'),
    ...
    routeType: z.enum(['balanced', 'short', 'less_maneuvers']).optional().describe('Route optimization type'),
  ...
  .output(z.object({
    isolines: z.array(z.object({
      range: z.number().optional().describe('Range value for this isoline'),
      geometry: z.any().describe('GeoJSON Polygon/MultiPolygon geometry of the reachable area'),
    })).describe('Isoline polygons'),
    isolineId: z.string().optional().describe('Isoline ID for retrieving results later (if calculation is still processing)'),
