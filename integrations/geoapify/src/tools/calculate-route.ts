...
export let calculateRoute = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Routes cost 1 credit per waypoint pair plus 1 additional credit per 500km.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    waypoints: z.array(waypointSchema).min(2).describe('Ordered list of waypoints (at least 2)'),
    mode: travelModeEnum.describe('Travel mode'),
    type: z.enum(['balanced', 'short', 'less_maneuvers']).optional().describe('Route optimization type (default: balanced)'),
    units: z.enum(['metric', 'imperial']).optional().describe('Measurement system (default: metric)'),
    lang: z.string().optional().describe('ISO 639-1 language code for instructions'),
    avoid: z.string().optional().describe('Road types to avoid (e.g. "tolls|ferries|highways")'),
    details: z.string().optional().describe('Extra response data: "instruction_details", "route_details", "elevation" (comma-separated)'),
    traffic: z.enum(['free_flow', 'approximated']).optional().describe('Traffic model (default: free_flow)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let waypointsStr = ctx.input.waypoints
