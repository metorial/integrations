...
export let autocompleteAddress = SlateTool.create(
  ...
  .input(z.object({
    ...
    type: z.enum(['country', 'state', 'city', 'postcode', 'street', 'amenity']).optional().describe('Filter suggestions by location type'),
    ...
    filter: z.string().optional().describe('Geographic filter constraint (e.g. "countrycode:us")'),
    bias: z.string().optional().describe('Geographic proximity bias (e.g. "proximity:lon,lat")'),
  }))
  .output(z.object({
