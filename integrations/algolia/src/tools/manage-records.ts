...
export let manageRecords = SlateTool.create(
  spec,
  {
    name: 'Manage Records',
    key: 'manage_records',
  ...
  .input(z.object({
    action: z.enum(['get', 'add', 'update', 'partialUpdate', 'delete', 'batch', 'clear', 'deleteBy']).describe('The record operation to perform.'),
    indexName: z.string().describe('Name of the Algolia index to operate on.'),
    objectId: z.string().optional().describe('The objectID of the record (required for get, update, partialUpdate, delete).'),
    record: z.record(z.string(), z.any()).optional().describe('Record data as key-value pairs (for add, update, partialUpdate).'),
    attributesToRetrieve: z.array(z.string()).optional().describe('List of attributes to retrieve (for get). If omitted, all attributes are returned.'),
    createIfNotExists: z.boolean().optional().describe('Whether to create the record if it does not exist (for partialUpdate). Defaults to true.'),
    batchRequests: z.array(z.object({
      action: z.string().describe('Batch action type (e.g., "addObject", "updateObject", "deleteObject", "partialUpdateObject", "partialUpdateObjectNoCreate").'),
      body: z.record(z.string(), z.any()).describe('Record data for this batch operation.'),
    })).optional().describe('Array of batch operations (for batch action).'),
    deleteByFilters: z.record(z.string(), z.any()).optional().describe('Filter parameters for deleteBy (e.g., { "filters": "category:books", "numericFilters": ["price < 10"] }).'),
  }))
  .output(z.any())
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'get') {
      ...
      let result = await client.getRecord(indexName, ctx.input.objectId, ctx.input.attributesToRetrieve);
    ...
    if (action === 'batch') {
      if (!ctx.input.batchRequests || ctx.input.batchRequests.length === 0) {
        throw new Error('batchRequests is required and must not be empty for the "batch" action.');
      }
