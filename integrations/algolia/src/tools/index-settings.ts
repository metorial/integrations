...
export let indexSettings = SlateTool.create(
  spec,
  {
    name: 'Index Settings',
    key: 'index_settings',
  ...
  .input(z.object({
    action: z.enum(['get', 'update']).describe('The action to perform: "get" to retrieve settings, "update" to modify settings'),
    indexName: z.string().describe('Name of the Algolia index'),
    settings: z.record(z.string(), z.any()).optional().describe('Settings object with fields to update (required for update action). Supports any valid Algolia index setting such as searchableAttributes, attributesForFaceting, customRanking, ranking, replicas, etc.'),
    forwardToReplicas: z.boolean().optional().describe('Whether to forward settings changes to replica indices (only for update action)'),
  }))
  .output(z.any())
  .handleInvocation(async (ctx) => {
    ...
    if (!ctx.input.settings || Object.keys(ctx.input.settings).length === 0) {
      throw new Error('Settings object is required and must not be empty when updating index settings.');
    }

    let result = await client.setSettings(indexName, ctx.input.settings, ctx.input.forwardToReplicas);
