...
export let searchAnalytics = SlateTool.create(
  spec,
  {
...
All metrics support date range filtering, result limiting, and analytics tag filtering.`,
  ...
  .input(z.object({
    ...
    startDate: z.string().optional().describe('Start date for the analytics period in ISO 8601 format (e.g., "2024-01-01")'),
    endDate: z.string().optional().describe('End date for the analytics period in ISO 8601 format (e.g., "2024-01-31")'),
    limit: z.number().optional().describe('Maximum number of results to return'),
    tags: z.string().optional().describe('Analytics tag to filter by (only includes searches that were tagged with this value)'),
  }))
  .output(z.any())
  .handleInvocation(async (ctx) => {
    ...
    let metricFns: Record<string, (indexName: string, params?: Record<string, any>) => Promise<any>> = {
