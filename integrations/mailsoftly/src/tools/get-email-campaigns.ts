...
export let getEmailCampaigns = SlateTool.create(
  spec,
  {
...
- Provide an **emailId** to fetch the status and readiness of a specific email draft.`,
  ...
  .input(z.object({
    emailId: z.string().optional().describe('ID of a specific email draft to check status for.'),
  }))
  .output(z.object({
    campaigns: z.array(z.any()).optional().describe('List of email campaigns (when listing all).'),
    emailStatus: z.any().optional().describe('Status of the specific email draft (when emailId provided).'),
  }))
