...
export let getContacts = SlateTool.create(
  spec,
  {
    name: 'Get Contacts',
    key: 'get_contacts',
  ...
  .input(z.object({
    contactId: z.string().optional().describe('ID of a specific contact to retrieve. Omit to list all contacts.'),
    detailed: z.boolean().optional().describe('If true and contactId is provided, returns detailed contact information including custom fields.'),
  }))
  .output(z.object({
    contacts: z.array(z.any()).optional().describe('List of contacts (when no contactId provided).'),
