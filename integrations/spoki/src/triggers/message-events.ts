...
export let messageEvents = SlateTrigger.create(
  spec,
  {
    name: 'Message Events',
    key: 'message_events',
    description: 'Triggers when a WhatsApp message is sent or received in Spoki, and when message statuses change (e.g., delivered, read).',
  }
)
  .input(z.object({
    ...
    messageType: z.string().optional().describe('Type of message (text, image, template, etc.)'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let messageId = message?.id ? String(message.id) : data?.message_id ? String(data.message_id) : undefined;
      let eventId = data?.id ? String(data.id) : `${normalizedType}-${messageId || ''}-${Date.now()}`;

      let direction: string | undefined;
      if (normalizedType === 'message.received') {
        direction = 'inbound';
      } else if (normalizedType === 'message.sent' || normalizedType === 'message.delivered' || normalizedType === 'message.read') {
        direction = 'outbound';
      } else {
      ...
      return {
        inputs: [{
          ...
          eventId,
          messageId,
          contactId: message?.contact_id ? String(message.contact_id) : data?.contact_id ? String(data.contact_id) : undefined,
