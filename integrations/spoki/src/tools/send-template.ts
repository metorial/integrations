...
Can be used outside the 24-hour conversation window.`,
  ...
  .input(z.object({
    ...
    language: z.string().optional().describe('Language code for the template (e.g., "en", "it")'),
    customFields: z.record(z.string(), z.string()).optional().describe('Custom field values to populate template variables'),
  }))
  .output(z.object({
