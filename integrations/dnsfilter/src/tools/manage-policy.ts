...
export let managePolicy = SlateTool.create(
  spec,
  {
    name: 'Manage Policy',
    key: 'manage_policy',
...
- To **delete**: provide policyId and set action to "delete".`,
  ...
  .input(z.object({
    ...
    organizationId: z.string().optional().describe('Organization ID to associate with (for create)'),
    allowListDomains: z.array(z.string()).optional().describe('Domains to add to the allow list (whitelist)'),
    blockListDomains: z.array(z.string()).optional().describe('Domains to add to the block list (blacklist)'),
    blockedCategoryIds: z.array(z.string()).optional().describe('Category IDs to block'),
    blockedApplicationIds: z.array(z.string()).optional().describe('Application IDs to block (AppAware)'),
    attributes: z.record(z.string(), z.any()).optional().describe('Additional policy attributes to set'),
  }))
  .output(z.object({
    policy: z.record(z.string(), z.any()).optional().describe('Policy details (for create/update)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.blockedApplicationIds) params.blocked_applications = ctx.input.blockedApplicationIds;
