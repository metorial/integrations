...
export let getQueryLog = SlateTool.create(
  spec,
  {
    name: 'Get Query Log',
    key: 'get_query_log',
    ...
    constraints: [
      'Query logs are limited to a 72-hour time range per request.',
    ],
  }
)
  .input(z.object({
    ...
    filters: z.record(z.string(), z.any()).optional().describe('Additional filter parameters'),
  }))
  .output(z.object({
