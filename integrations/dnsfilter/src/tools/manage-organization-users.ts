...
export let manageOrganizationUsers = SlateTool.create(
  spec,
  {
...
- **remove**: Remove a user from the organization (does not delete their account).`,
  ...
  .input(z.object({
    ...
    role: z.string().optional().describe('User role, e.g. "admin" or "read_only" (for add/update)'),
    attributes: z.record(z.string(), z.any()).optional().describe('Additional user attributes'),
  }))
  .output(z.object({
