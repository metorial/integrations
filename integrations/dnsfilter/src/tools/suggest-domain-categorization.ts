...
export let suggestDomainCategorization = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    type: z.enum(['category', 'threat']).describe('"category" to suggest a recategorization, "threat" to report a threat'),
    domain: z.string().describe('Fully qualified domain name'),
    suggestedCategory: z.string().optional().describe('Suggested category (for category type)'),
