...
export let createGroup = SlateTool.create(
  ...
  .input(z.object({
    ...
    spokeId: z.string().describe('Unique identifier for the group in your third-party application'),
...
export let updateGroup = SlateTool.create(
  ...
  .input(z.object({
    ...
    userIds: z.array(z.string()).optional().describe('Updated array of user IDs for the group'),
    version: z.number().optional().describe('Current version number for optimistic concurrency'),
  }))
  .output(groupOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new RipplingClient({ token: ctx.auth.token });
