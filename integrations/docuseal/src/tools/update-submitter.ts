...
export let updateSubmitter = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    values: z.record(z.string(), z.any()).optional().describe('Field values to pre-fill or update (field name -> value)'),
    ...
    completed: z.boolean().optional().describe('Set true to auto-complete signing for this submitter'),
    ...
    }).optional().describe('Custom email message for re-sent notifications'),
  }))
  .output(z.object({
