...
let asteroidSummarySchema = z.object({
  ...
  estimatedDiameterMinKm: z.number().optional().describe('Estimated minimum diameter in kilometers'),
  estimatedDiameterMaxKm: z.number().optional().describe('Estimated maximum diameter in kilometers'),
  isPotentiallyHazardous: z.boolean().describe('Whether the asteroid is classified as potentially hazardous'),
...
export let searchAsteroids = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Date range feed lookups are limited to 7 days.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    startDate: z.string().optional().describe('Start date for close approach lookup (YYYY-MM-DD). Required when searching by date range.'),
    endDate: z.string().optional().describe('End date for close approach lookup (YYYY-MM-DD). Defaults to 7 days after startDate.'),
    asteroidId: z.string().optional().describe('SPK-ID of a specific asteroid to look up. Overrides date range search.'),
    browse: z.boolean().optional().describe('Set to true to browse the full asteroid catalog (paginated).'),
