...
export let getTechPortProject = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    updatedSince: z.string().optional().describe('List projects updated since this date (YYYY-MM-DD). Used when projectId is not specified.'),
  }))
  .output(z.object({
    project: z.record(z.string(), z.any()).optional().describe('Full project details when looking up by ID'),
    projectIds: z.array(z.number()).optional().describe('List of project IDs when searching by updated date'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let projectIds = (result.projects || result || []).map((p: any) => typeof p === 'number' ? p : p.projectId);
