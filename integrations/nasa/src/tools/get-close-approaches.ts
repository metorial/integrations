...
export let getCloseApproaches = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    dateMin: z.string().optional().describe('Minimum close approach date (YYYY-MM-DD or "now" for current date)'),
    dateMax: z.string().optional().describe('Maximum close approach date (YYYY-MM-DD or "+60" for 60 days from now)'),
    distMax: z.string().optional().describe('Maximum approach distance filter (e.g., "0.05" in AU, or "10LD" for lunar distances)'),
    body: z.string().optional().describe('Close approach body (default "Earth"). Use "ALL" for all bodies.'),
  }))
  .output(z.object({
    ...
    data: z.array(z.array(z.string())).describe('Close approach data rows (values correspond to fields)'),
  }))
