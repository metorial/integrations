...
let checkoutSessionOutputSchema = z.object({
  ...
  billingAddressCollection: z.boolean().optional().describe('Whether billing address is collected')
});

export let listCheckoutSessions = SlateTool.create(
  spec,
  {
...
export let createCheckoutSession = SlateTool.create(
  ...
  .input(z.object({
    ...
    billingAddressCollection: z.boolean().optional().describe('Whether to collect billing address'),
    items: z.array(z.object({
      ...
      currency: z.string().optional().describe('Currency code')
    })).optional().describe('Line items for the checkout'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.billingAddressCollection !== undefined) data.billing_address_collection = ctx.input.billingAddressCollection;
