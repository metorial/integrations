...
let resourceTypeEnum = z.enum([
  ...
  'batchprediction', 'evaluation', 'cluster', 'anomaly',
  'association', 'topicmodel', 'timeseries', 'pca',
  'fusion', 'optiml', 'script', 'library', 'execution',
  'project', 'externalconnector', 'configuration'
]);
...
Use filters to narrow results by name, tags, creation date, and other resource-specific fields.`,
  ...
  .input(z.object({
    ...
    orderBy: z.string().optional().describe('Field to sort by. Prefix with "-" for descending order (e.g., "-created")'),
    name: z.string().optional().describe('Filter by exact name'),
    tags: z.string().optional().describe('Filter by tag'),
    projectId: z.string().optional().describe('Filter by project ID'),
    filters: z.record(z.string(), z.string()).optional().describe('Additional API filters as key-value pairs (e.g., {"created__gt": "2024-01-01", "size__gt": "1048576"})')
  }))
  .output(z.object({
