...
export let createBatchPrediction = SlateTool.create(
  spec,
  {
...
The batch prediction runs asynchronously; check its status to know when results are ready.`,
  ...
  .input(z.object({
    modelId: z.string().describe('Resource ID of the model (e.g., "model/abc123", "ensemble/abc123", "deepnet/abc123")'),
    ...
    includeConfidence: z.boolean().optional().describe('Include confidence scores in output'),
    outputFields: z.array(z.string()).optional().describe('List of field IDs to include in the output'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includeConfidence !== undefined) body.confidence = ctx.input.includeConfidence;
