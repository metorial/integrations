...
export let listProjects = SlateTool.create(
  ...
  .input(z.object({
    projectId: z.string().optional().describe('If provided, fetches detailed info for this specific project instead of listing all projects.'),
  }))
  .output(z.object({
    projects: z.array(z.object({
      projectId: z.string().describe('The project ID.'),
      displayName: z.string().describe('Human-readable project name.'),
      organizationId: z.string().optional().nullable().describe('ID of the organization this project belongs to.'),
      createdAt: z.string().optional().describe('When the project was created.'),
      studioHost: z.string().optional().nullable().describe('The studio hostname if deployed.'),
      members: z.array(z.object({
        userId: z.string().describe('User ID of the member.'),
        role: z.string().describe('Role of the member in the project.'),
      })).optional().describe('List of project members.'),
    }).passthrough()).optional().describe('List of projects (when no specific projectId is provided).'),
    project: z.any().optional().describe('Detailed project information (when a specific projectId is provided).'),
  }))
