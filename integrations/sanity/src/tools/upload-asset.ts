...
export let uploadAsset = SlateTool.create(
  ...
  .input(z.object({
    assetType: z.enum(['image', 'file']).describe('Type of asset to upload: "image" for images, "file" for other files (PDFs, videos, etc).'),
  ...
  .output(z.object({
    ...
    }).passthrough().describe('The created asset document.'),
  }))
