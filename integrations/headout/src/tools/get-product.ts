...
let variantInputFieldSchema = z.object({
  fieldId: z.string().describe('Input field identifier'),
  name: z.string().describe('Display name'),
  dataType: z.string().describe('Data type (INT, FLOAT, STRING, BOOL, ENUM)'),
  level: z.string().describe('Level at which the field applies (PRIMARY_CUSTOMER, ALL_CUSTOMERS, VARIANT)'),
  validation: z.object({
...
let variantSchema = z.object({
  ...
  inventoryType: z.string().optional().describe('Inventory type (e.g., FIXED_START_FIXED_DURATION)'),
  ...
  }).optional().describe('Min/max participants'),
  cashback: z.object({
    value: z.number().optional(),
    type: z.string().optional()
  }).optional().describe('Cashback details'),
  ticketDeliveryInfoHtml: z.string().optional(),
  inputFields: z.array(variantInputFieldSchema).optional().describe('Required input fields for booking')
}).passthrough();
...
let productSchema = z.object({
  ...
  }).passthrough()).optional().describe('Product images'),
  content: z.array(z.object({
    title: z.string().optional(),
    html: z.string().optional()
  })).optional().describe('Rich text content sections'),
...
Each product can have multiple variants (e.g., "Skip the Line", "General Admission") with their own pricing, inventory type, and required input fields.`,
  ...
  .input(z.object({
    ...
    languageCode: z.string().optional().describe('Override language (EN, ES, FR, IT, DE, PT, NL)'),
    fetchVariants: z.boolean().optional().describe('Whether to include variant details (default: true)')
  }))
