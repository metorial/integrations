...
let pricingPersonSchema = z.object({
...
let inventoryItemSchema = z.object({
  ...
  pricing: z.object({
    persons: z.array(pricingPersonSchema).optional().describe('Per-person pricing (when priceType is PER_PERSON)'),
    groups: z.array(pricingGroupSchema).optional().describe('Per-group pricing (when priceType is PER_GROUP)')
  }).optional().describe('Pricing details for this time slot')
}).passthrough();

export let getInventory = SlateTool.create(
  spec,
  {
...
Use this before creating a booking to find available time slots and accurate prices.`,
    ...
    constraints: [
      'Active inventory is maintained for approximately 90 days per product.'
    ],
    tags: {
      readOnly: true
    }
  ...
  .input(z.object({
    ...
    startDateTime: z.string().optional().describe('Start of date range (YYYY-MM-DDThh:mm:ss). Defaults to current time.'),
    endDateTime: z.string().optional().describe('End of date range (YYYY-MM-DDThh:mm:ss). Defaults to 30 days from start.'),
