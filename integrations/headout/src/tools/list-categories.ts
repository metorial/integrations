...
Categories (e.g., "Tickets", "Tours") and subcategories (e.g., "Landmarks", "Museums") can be used to filter product searches.`,
  ...
  .input(z.object({
    ...
    includeSubcategories: z.boolean().optional().describe('Also fetch subcategories (default: true)'),
  ...
  .output(z.object({
    ...
    subcategories: z.array(subcategorySchema).optional().describe('List of subcategories (if requested)')
  }))
  .handleInvocation(async (ctx) => {
    ...
    let categories = (categoriesResult.items ?? categoriesResult.categories ?? []).map((c: any) => ({
    ...
    if (includeSubcategories) {
      ...
      subcategories = (subcategoriesResult.items ?? subcategoriesResult.subcategories ?? []).map((s: any) => ({
