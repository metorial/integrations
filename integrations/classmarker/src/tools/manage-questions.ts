...
let questionOutputSchema = z.object({
  ...
  questionType: z.string().describe('Type of question (e.g., multiplechoice, truefalse, essay)'),
  ...
  incorrectFeedback: z.string().optional().describe('Feedback shown when answered incorrectly'),
...
export let getQuestions = SlateTool.create(
  spec,
  {
    name: 'Get Questions',
    key: 'get_questions',
  ...
  .input(z.object({
    questionId: z.number().optional().describe('ID of a specific question to retrieve. Omit to list all questions.'),
    page: z.number().optional().describe('Page number for pagination when listing all questions (200 per page)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let questions = (Array.isArray(rawQuestions) ? rawQuestions : Object.values(rawQuestions)).map(mapQuestion);
...
export let createOrUpdateQuestion = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    questionId: z.number().optional().describe('ID of an existing question to update. Omit to create a new question.'),
    questionType: z.enum(['multiplechoice', 'multipleresponse', 'truefalse', 'essay']).describe('Type of question'),
    question: z.string().describe('The question text'),
    ...
    correctFeedback: z.string().optional().describe('Feedback shown when answered correctly'),
    incorrectFeedback: z.string().optional().describe('Feedback shown when answered incorrectly'),
    options: z.record(z.string(), questionOptionSchema).optional().describe('Answer options keyed by letter (e.g., { "A": { "content": "Option 1" }, "B": { "content": "Option 2" } })'),
    correctOptions: z.array(z.string()).optional().describe('Array of correct option keys (e.g., ["A", "C"])'),
    gradeStyle: z.enum(['partial_with_deduction', 'partial_without_deduction', 'off']).optional().describe('Grading style for multiple response questions'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { questionId, questionType, question, categoryId, points, randomAnswers, correctFeedback, incorrectFeedback, options, correctOptions, gradeStyle } = ctx.input;
    ...
    let resultQuestionId = data?.data?.question?.question_id || data?.question_id || questionId;
