...
export let listCategories = SlateTool.create(
  ...
  .input(z.object({}))
  .output(z.object({
    parentCategories: z.array(parentCategorySchema).describe('List of parent categories with subcategories'),
  }))
...
export let manageCategory = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'update']).describe('Whether to create a new category or update an existing one'),
    level: z.enum(['parent', 'subcategory']).describe('Whether this is a parent category or subcategory'),
    name: z.string().describe('Name of the category'),
    parentCategoryId: z.number().optional().describe('Required when creating a subcategory or updating a parent category. For subcategories, the parent to assign it to. For updating parents, the ID of the parent to update.'),
    categoryId: z.number().optional().describe('Required when updating a subcategory. The ID of the subcategory to update.'),
  }))
  .output(z.object({
