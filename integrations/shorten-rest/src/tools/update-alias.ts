...
let destinationSchema = z.object({
  ...
  os: z.string().optional().describe('Operating system for OS-targeting (e.g., "iOS", "Android", "Windows").'),
});
...
let snippetSchema = z.object({
  ...
  parameters: z.record(z.string(), z.string()).optional().describe('Key-value parameters for the pixel provider.'),
});

export let updateAlias = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    domainName: z.string().optional().describe('The domain the alias belongs to. Defaults to "short.fyi" if omitted.'),
    destinations: z.array(destinationSchema).optional().describe('Updated list of destination URLs with optional geo/OS targeting.'),
    metatags: z.array(metatagSchema).optional().describe('Updated meta tags for social sharing.'),
