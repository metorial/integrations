...
export let manageJob = SlateTool.create(
  ...
  .input(z.object({
    ...
    unblockFields: z.record(z.string(), z.string()).optional().describe('Fields to pass when unblocking (for block steps with input fields)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'retry') {
      j = await client.retryJob(ctx.input.pipelineSlug, ctx.input.buildNumber, ctx.input.jobId);
    } else {
      j = await client.unblockJob(ctx.input.pipelineSlug, ctx.input.buildNumber, ctx.input.jobId, ctx.input.unblockFields);
    }
    ...
    return {
      ...
      message: ctx.input.action === 'retry'
