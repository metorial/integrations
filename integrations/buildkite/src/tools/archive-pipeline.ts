...
export let archivePipeline = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['archive', 'unarchive']).describe('Whether to archive or unarchive the pipeline'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let p = ctx.input.action === 'archive'
