...
let buildSchema = z.object({
  ...
  state: z.string().describe('Current state (running, scheduled, passed, failed, blocked, canceled, etc.)'),
...
export let listBuilds = SlateTool.create(
  spec,
  {
    name: 'List Builds',
    key: 'list_builds',
  ...
  .input(z.object({
    pipelineSlug: z.string().optional().describe('Filter builds to a specific pipeline slug. If omitted, lists builds across all pipelines.'),
    state: z.enum(['running', 'scheduled', 'passed', 'failed', 'blocked', 'canceled', 'canceling', 'skipped', 'not_run', 'finished']).optional().describe('Filter by build state'),
    branch: z.string().optional().describe('Filter by branch name'),
    commit: z.string().optional().describe('Filter by commit SHA'),
    createdFrom: z.string().optional().describe('Filter builds created after this date (ISO 8601 format)'),
    createdTo: z.string().optional().describe('Filter builds created before this date (ISO 8601 format)'),
