...
export let buildEvents = SlateTrigger.create(
  spec,
  {
    name: 'Build Events',
    key: 'build_events',
    description: 'Triggers when a build changes state (scheduled, running, failing, finished, skipped). Configure a webhook in your Buildkite organization\'s Notification Services settings and point it to the provided webhook URL.',
  }
)
  .input(z.object({
    eventType: z.string().describe('Type of build event (build.scheduled, build.running, build.failing, build.finished, build.skipped)'),
