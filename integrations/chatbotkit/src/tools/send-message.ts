...
export let sendMessageTool = SlateTool.create(
  ...
  .input(z.object({
    conversationId: z.string().optional().describe('Existing conversation ID (omit for stateless completion)'),
    text: z.string().describe('Message text to send'),
    botId: z.string().optional().describe('Bot ID (used for stateless completion when no conversationId)'),
    backstory: z.string().optional().describe('Override backstory (for stateless completion)'),
    ...
    skillsetId: z.string().optional().describe('Override skillset (for stateless completion)'),
    receiveResponse: z.boolean().optional().default(true).describe('Whether to also receive the bot response'),
    meta: z.record(z.string(), z.any()).optional().describe('Arbitrary metadata for the message'),
  }))
  .output(z.object({
    conversationId: z.string().optional().describe('Conversation ID'),
    sentText: z.string().describe('Text that was sent'),
    responseText: z.string().optional().describe('Bot response text (if receiveResponse was true)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { conversationId, text, botId, backstory, model, datasetId, skillsetId, receiveResponse, meta } = ctx.input;

    if (!conversationId) {
      let result = await client.completeConversation({ text, botId, backstory, model, datasetId, skillsetId, meta });
      return {
        output: {
