...
Use \`channelId\` to specify which channel to query, or leave blank to query the authenticated user's channel.`,
  ...
  .input(z.object({
    channelId: z.string().optional().describe('YouTube channel ID to query. Leave empty to use the authenticated user\'s channel ("channel==MINE").'),
    contentOwnerId: z.string().optional().describe('Content owner ID for content owner reports. Use this instead of channelId for content owner queries.'),
    ...
    metrics: z.string().describe('Comma-separated list of metrics to retrieve, e.g. "views,likes,estimatedMinutesWatched,subscribersGained".'),
    dimensions: z.string().optional().describe('Comma-separated list of dimensions to group by, e.g. "day", "country", "video", "ageGroup,gender".'),
    filters: z.string().optional().describe('Semicolon-separated filters, e.g. "video==VIDEO_ID;country==US".'),
    sort: z.string().optional().describe('Comma-separated sort fields, prefix with "-" for descending, e.g. "-views".'),
    maxResults: z.number().optional().describe('Maximum number of rows to return.'),
    startIndex: z.number().optional().describe('1-based index of first row to retrieve for pagination.'),
    currency: z.string().optional().describe('Currency code for financial metrics (default: USD).'),
    includeHistoricalChannelData: z.boolean().optional().describe('Include data from before the channel was linked to a content owner.'),
  }))
  .output(z.object({
    ...
    rows: z.array(z.array(z.union([z.string(), z.number()]))).describe('Result rows, each containing values matching the column headers.'),
