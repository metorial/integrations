...
export let listAssignmentsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    bucket: z.enum(['past', 'overdue', 'undated', 'ungraded', 'unsubmitted', 'upcoming', 'future']).optional().describe('Filter by assignment bucket'),
    orderBy: z.enum(['position', 'name', 'due_at']).optional().describe('Sort order'),
    includeSubmission: z.boolean().optional().describe('Include current user submission data'),
  }))
  .output(z.object({
    assignments: z.array(z.object({
      assignmentId: z.string().describe('Assignment ID'),
      name: z.string().describe('Assignment name'),
      description: z.string().optional().nullable().describe('Assignment description HTML'),
