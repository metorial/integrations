...
export let manageCourseTool = SlateTool.create(
  spec,
  {
    name: 'Manage Course',
    key: 'manage_course',
  ...
  .input(z.object({
    ...
    accountId: z.string().optional().describe('Account ID (required for creating a new course)'),
    ...
    isPublicToAuthUsers: z.boolean().optional().describe('Whether course is visible to authenticated users'),
    ...
    allowStudentWikiEdits: z.boolean().optional().describe('Allow students to edit wiki pages'),
    allowWikiComments: z.boolean().optional().describe('Allow comments on wiki pages'),
    allowStudentForumAttachments: z.boolean().optional().describe('Allow file attachments in discussions'),
    defaultView: z.enum(['feed', 'wiki', 'modules', 'assignments', 'syllabus']).optional().describe('Course homepage layout'),
    syllabusBody: z.string().optional().describe('Syllabus HTML content'),
    termId: z.string().optional().describe('Enrollment term ID'),
    restrictEnrollmentsToCourseDates: z.boolean().optional().describe('Restrict enrollment to course dates'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.isPublicToAuthUsers !== undefined) courseData.is_public_to_auth_users = ctx.input.isPublicToAuthUsers;
    if (ctx.input.publicSyllabus !== undefined) courseData.public_syllabus = ctx.input.publicSyllabus;
    if (ctx.input.publicDescription) courseData.public_description = ctx.input.publicDescription;
    if (ctx.input.allowStudentWikiEdits !== undefined) courseData.allow_student_wiki_edits = ctx.input.allowStudentWikiEdits;
    if (ctx.input.allowWikiComments !== undefined) courseData.allow_wiki_comments = ctx.input.allowWikiComments;
    if (ctx.input.allowStudentForumAttachments !== undefined) courseData.allow_student_forum_attachments = ctx.input.allowStudentForumAttachments;
    ...
    if (ctx.input.restrictEnrollmentsToCourseDates !== undefined) courseData.restrict_enrollments_to_course_dates = ctx.input.restrictEnrollmentsToCourseDates;

    let result: any;
    let actionDesc: string;

    if (ctx.input.action === 'create') {
      if (!ctx.input.accountId) throw new Error('accountId is required when creating a course');
      ...
      if (!ctx.input.courseId) throw new Error('courseId is required when deleting/concluding a course');
