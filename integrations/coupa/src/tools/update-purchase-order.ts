...
export let updatePurchaseOrder = SlateTool.create(
  ...
  .input(z.object({
    ...
    shipToAddress: z.object({
      addressId: z.number(),
    }).optional().describe('New ship-to address reference'),
    currency: z.object({
      code: z.string(),
    }).optional().describe('Updated currency'),
    ...
    })).optional().describe('Order lines to update — include line ID for existing lines'),
