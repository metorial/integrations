...
export let manageEmailList = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['list_bounces', 'list_unsubscribes', 'remove_bounces', 'remove_spam']).describe('Operation to perform'),
    emails: z.array(z.string()).optional().describe('Email addresses to remove (required for remove_bounces and remove_spam)'),
