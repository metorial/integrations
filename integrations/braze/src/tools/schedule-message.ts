...
export let scheduleMessage = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['create', 'delete']).describe('"create" to schedule a message, "delete" to cancel a scheduled message'),
    scheduleId: z.string().optional().describe('Schedule ID to cancel (required for delete action)'),
    externalUserIds: z.array(z.string()).optional().describe('External user IDs to send to (for create action)'),
    broadcast: z.boolean().optional().describe('Set to true for broadcast send (for create action)'),
    scheduledTime: z.string().optional().describe('ISO 8601 datetime for when to send the message (required for create action)'),
    inLocalTime: z.boolean().optional().describe('If true, deliver the message in each user\'s local timezone'),
    messages: z.object({
    ...
    }).optional().describe('Channel-specific message objects (for create action)')
  }))
  .output(z.object({
    scheduleId: z.string().optional().describe('Schedule ID for the created scheduled message'),
