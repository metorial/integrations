...
export let previewContent = SlateTool.create(
  spec,
  {
...
Useful for building preview environments where editors can see content before it goes live. The query syntax is identical to the published content query, but results include draft entries that haven't been published yet.`,
  ...
  .input(z.object({
    query: z.string().describe('The GraphQL query string to execute against the Contentful Content Preview API.'),
    variables: z.record(z.string(), z.any()).optional().describe('Optional variables for the GraphQL query.')
  }))
  .output(z.object({
    queryResult: z.any().describe('The data returned by the GraphQL query (includes draft content).'),
  ...
  .handleInvocation(async (ctx) => {
    if (!ctx.auth.previewToken) {
      throw new Error('A Content Preview API (CPA) token is required to preview draft content. Configure it in the authentication settings.');
    }
