...
export let introspectSchema = SlateTool.create(
  spec,
  {
...
Use this to understand what queries are available before using the **Query Content** or **Preview Content** tools. The schema is auto-generated from your Contentful content model and updates whenever content types change.`,
  ...
  .input(z.object({
    includeSystemTypes: z.boolean().optional().default(false).describe('Whether to include internal GraphQL system types (types starting with "__"). Defaults to false.')
  }))
  .output(z.object({
    queryTypeName: z.string().optional().describe('Name of the root query type.'),
    contentTypes: z.array(contentTypeSchema).describe('Available content types and their fields.')
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (result.errors && result.errors.length > 0) {
      throw new Error(`Schema introspection failed: ${result.errors.map((e: any) => e.message).join(', ')}`);
    }
