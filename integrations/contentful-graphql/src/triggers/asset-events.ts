...
export let assetEvents = SlateTrigger.create(
  spec,
  {
    name: 'Asset Events',
    key: 'asset_events',
    description: 'Triggers when an asset is created, saved, published, unpublished, archived, unarchived, or deleted in your Contentful space.'
  }
)
  .input(z.object({
    ...
    fields: z.record(z.string(), z.any()).optional().describe('Asset fields (title, description, file) at the time of the event.'),
