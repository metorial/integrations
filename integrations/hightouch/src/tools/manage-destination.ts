...
let destinationSchema = z.object({
  ...
  syncs: z.array(z.number()).optional().describe('IDs of syncs sending data to this destination'),
...
export let listDestinations = SlateTool.create(
  ...
  .input(z.object({
    limit: z.number().optional().describe('Max number of destinations to return (default 100)'),
...
export let createDestination = SlateTool.create(
  ...
  .input(z.object({
    ...
    configuration: z.record(z.string(), z.any()).describe('Destination configuration (varies by type)'),
  }))
  .output(destinationSchema)
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
