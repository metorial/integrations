...
Common conversion types include: \`convert.pdf_to_word\`, \`convert.json_to_excel\`, \`convert.website_to_pdf\`, \`convert.mp4_to_mp3\`, \`convert.png_to_webp\`, \`convert.markdown_to_pdf\`, \`convert.ocr_png_to_text\`, and many more. Use the **List Conversions** tool to see all available types and their options.`,
  ...
  .input(z.object({
    conversionType: z.string().describe('The conversion type identifier, e.g. "convert.pdf_to_word", "convert.json_to_excel", "convert.website_to_pdf"'),
    fileId: z.string().optional().describe('ID of a previously uploaded file to convert. Required for file-based conversions.'),
    conversionOptions: z.record(z.string(), z.unknown()).optional().describe('Conversion-specific options (e.g. { "url": "https://example.com" } for website conversions, { "delimiter": "comma" } for CSV output, { "quality": 90 } for image output)'),
    callbackUrl: z.string().optional().describe('Webhook URL to receive a notification when the task completes'),
    waitForCompletion: z.boolean().optional().default(false).describe('If true, polls the task until it completes and returns the final status. Otherwise returns immediately after task creation.'),
  }))
  .output(z.object({
    ...
    resultFileId: z.string().nullable().describe('File ID of the converted result (available when status is SUCCESS)'),
