...
export let extractEvents = SlateTool.create(
  spec,
  {
    name: 'Extract Events',
    key: 'extract_events',
  ...
  .input(z.object({
    ...
    ]).optional().describe('Timeframe to filter events'),
    filters: z.array(z.object({
      propertyName: z.string().describe('The event property to filter on'),
      operator: z.string().describe('The filter operator (eq, ne, lt, gt, exists, in, contains, etc.)'),
      propertyValue: z.any().describe('The value to compare against'),
    })).optional().describe('Filters to narrow extracted events'),
    propertyNames: z.array(z.string()).optional().describe('Specific properties to include in the output. If omitted, all properties are returned.'),
  ...
  .output(z.object({
    events: z.array(z.record(z.string(), z.any())).describe('Array of extracted event objects'),
