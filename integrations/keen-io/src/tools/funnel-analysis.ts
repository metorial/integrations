...
let funnelStepSchema = z.object({
  ...
  actorProperty: z.string().describe('The property that identifies the actor (e.g. "user.id")'),
  timeframe: z.union([
    z.string(),
    z.object({
      start: z.string(),
  ...
  ]).optional().describe('Timeframe for this step'),
  filters: z.array(z.object({
    propertyName: z.string(),
    operator: z.string(),
    propertyValue: z.any(),
  })).optional().describe('Filters for this step'),
  inverted: z.boolean().optional().describe('If true, matches actors who did NOT perform this step'),
  optional: z.boolean().optional().describe('If true, this step is optional in the funnel sequence'),
  withActors: z.boolean().optional().describe('If true, return the list of actor IDs for this step'),
}).describe('A step in the funnel analysis');

export let funnelAnalysis = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    steps: z.array(funnelStepSchema).min(2).describe('Array of funnel steps (minimum 2). Steps are evaluated in order.'),
  }))
  .output(z.object({
    result: z.array(z.number()).describe('Array of counts at each funnel step'),
    stepsInfo: z.array(z.any()).optional().describe('Detailed information for each step, including actor IDs if requested'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let stepLabels = ctx.input.steps.map((s, i) => `Step ${i + 1} (${s.collectionName}): **${resultArray[i] ?? 'N/A'}**`);
