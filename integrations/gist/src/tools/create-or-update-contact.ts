...
export let createOrUpdateContact = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum 250 custom properties per contact.',
    ],
  }
)
  .input(z.object({
    email: z.string().optional().describe('Contact email address'),
    userId: z.string().optional().describe('External user ID. Setting this makes the contact a "user" type'),
    ...
    customProperties: z.record(z.string(), z.any()).optional().describe('Custom properties to set. Values must be strings, numbers, or booleans'),
    unsubscribedFromEmails: z.boolean().optional().describe('Whether to unsubscribe from emails'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.unsubscribedFromEmails !== undefined) body.unsubscribed_from_emails = ctx.input.unsubscribedFromEmails;
