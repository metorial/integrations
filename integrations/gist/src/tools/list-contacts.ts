...
export let listContacts = SlateTool.create(
  ...
  .input(z.object({
    ...
    orderBy: z.enum(['created_at', 'updated_at', 'last_seen_at', 'signed_up_at']).optional().describe('Sort field'),
    status: z.enum(['active', 'unsubscribed', 'bounced']).optional().describe('Filter by subscription status'),
    ...
    createdSince: z.string().optional().describe('Filter contacts created after this UNIX timestamp'),
  }))
  .output(z.object({
