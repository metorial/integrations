...
export let sendTransactionalEmail = SlateTool.create(
  ...
  .input(z.object({
    campaignId: z.string().optional().describe('Campaign template ID. If provided, sends using this template. Otherwise sends a custom email.'),
    fromEmail: z.string().optional().describe('Sender email address (must be from a verified domain). Required when not using a campaign template.'),
    fromName: z.string().optional().describe('Sender display name. Required when not using a campaign template.'),
    toEmail: z.string().describe('Recipient email address'),
    toName: z.string().optional().describe('Recipient display name'),
    subject: z.string().optional().describe('Email subject line. Required when not using a campaign template.'),
    html: z.string().optional().describe('HTML email body'),
    text: z.string().optional().describe('Plain text email body'),
    variables: z.record(z.string(), z.string()).optional().describe('Personalization variables for Liquid template tags'),
    attachments: z.record(z.string(), z.string()).optional().describe('Attachments as key-value pairs (filename: public HTTPS URL)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!ctx.input.fromEmail || !ctx.input.fromName || !ctx.input.subject) {
      throw new Error('fromEmail, fromName, and subject are required when not using a campaign template');
    }
