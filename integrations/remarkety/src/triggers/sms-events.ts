...
export let smsEventsTrigger = SlateTrigger.create(spec, {
  name: 'SMS Events',
  key: 'sms_events',
  description: 'Triggers when SMS events occur in Remarkety, including sent, clicked, replied, and unsubscribed events.'
})
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      return {
        inputs: [{
          ...
          recipientCountryCode: data.recipient_country_code ? String(data.recipient_country_code) : undefined,
    ...
    handleEvent: async (ctx) => {
      ...
      let type = typeMap[ctx.input.eventType] || `sms.${ctx.input.eventType.replace('sms/', '')}`;
      let id = ctx.input.umk || `${ctx.input.eventType}-${ctx.input.toPhone || ctx.input.email || ''}-${ctx.input.timestamp}`;
