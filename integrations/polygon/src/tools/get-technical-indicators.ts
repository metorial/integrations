...
export let getTechnicalIndicators = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    timespan: z.enum(['second', 'minute', 'hour', 'day', 'week', 'month', 'quarter', 'year']).optional().describe('Aggregate timespan. Defaults to day.'),
    window: z.number().int().optional().describe('Window size for SMA/EMA/RSI (e.g., 14 for RSI, 50 for SMA)'),
    shortWindow: z.number().int().optional().describe('MACD short-period EMA window (default 12)'),
    longWindow: z.number().int().optional().describe('MACD long-period EMA window (default 26)'),
    signalWindow: z.number().int().optional().describe('MACD signal line EMA window (default 9)'),
    seriesType: z.enum(['open', 'close', 'high', 'low']).optional().describe('Price series to use. Defaults to close.'),
    adjusted: z.boolean().optional().describe('Whether aggregates are adjusted for splits. Defaults to true.'),
    timestampFrom: z.string().optional().describe('Start timestamp filter (YYYY-MM-DD or Unix ms)'),
    timestampTo: z.string().optional().describe('End timestamp filter (YYYY-MM-DD or Unix ms)'),
  ...
  .output(z.object({
    ...
    values: z.array(z.object({
      ...
      value: z.number().optional().describe('Indicator value (SMA/EMA/RSI value, or MACD line value)'),
