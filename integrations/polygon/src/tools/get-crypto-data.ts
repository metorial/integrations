...
export let getCryptoData = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    tickers: z.string().optional().describe('Comma-separated list of crypto tickers with X: prefix (e.g., "X:BTCUSD" or "X:BTCUSD,X:ETHUSD"). Leave empty for all crypto tickers.'),
  }))
  .output(z.object({
    snapshots: z.array(z.object({
      ...
      prevDay: z.object({
        ...
        volume: z.number().optional(),
        volumeWeightedAvgPrice: z.number().optional(),
      }).optional().describe('Previous day aggregate'),
      lastTrade: z.object({
