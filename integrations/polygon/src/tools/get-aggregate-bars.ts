...
let aggregateBarSchema = z.object({
  ...
  timestamp: z.number().optional().describe('Unix millisecond timestamp for the start of the aggregate window'),
  transactionCount: z.number().optional().describe('Number of transactions in the aggregate window'),
...
export let getAggregateBars = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum of 50,000 results per request (default 5,000).',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    multiplier: z.number().int().positive().describe('The size of the timespan multiplier (e.g., 1 for 1-minute bars, 5 for 5-minute bars)'),
    timespan: z.enum(['second', 'minute', 'hour', 'day', 'week', 'month', 'quarter', 'year']).describe('The size of the time window'),
    from: z.string().describe('Start of the aggregate time window (YYYY-MM-DD or Unix ms timestamp)'),
    to: z.string().describe('End of the aggregate time window (YYYY-MM-DD or Unix ms timestamp)'),
    adjusted: z.boolean().optional().describe('Whether results are adjusted for splits. Defaults to true.'),
    sort: z.enum(['asc', 'desc']).optional().describe('Sort order by timestamp. Defaults to asc.'),
    limit: z.number().int().optional().describe('Max number of results (max 50000, default 5000)'),
  }))
  .output(z.object({
    ...
    queryCount: z.number().optional().describe('Number of aggregates used to generate the response'),
