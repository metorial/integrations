...
export let manageTaskAssignments = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    taskId: z.number().optional().describe('Task ID to assign the user to (required for create)'),
  }))
  .output(z.object({
    assignments: z.array(z.record(z.string(), z.unknown())).optional().describe('List of task assignments (for list action)'),
    assignment: z.record(z.string(), z.unknown()).optional().describe('Created task assignment'),
