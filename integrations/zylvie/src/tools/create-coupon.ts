...
export let createCoupon = SlateTool.create(
  spec,
  {
    name: 'Create Coupon',
    key: 'create_coupon',
  ...
  .input(z.object({
    code: z.string().describe('Coupon code (must be unique, at least 2 characters, case-insensitive)'),
    type: z.enum(['percentage', 'fixed']).describe('Discount type'),
    amount: z.number().describe('Discount value (0.1-100 for percentage, 0.1-999999.99 for fixed)'),

    isStorewide: z.boolean().optional().describe('Whether the coupon applies to all products (defaults to true)'),
    products: z.array(z.string()).optional().describe('Product IDs this coupon applies to (required if isStorewide is false)'),
    isLive: z.boolean().optional().describe('Whether the coupon is active (defaults to false)'),
    limit: z.number().optional().describe('Maximum number of redemptions'),
    durationInMonths: z.number().optional().describe('For subscriptions: how many months the discount applies'),
    start: z.string().optional().describe('Start date in ISO 8601 format (e.g. "2024-12-31T23:59:59Z")'),
    end: z.string().optional().describe('Expiration date in ISO 8601 format (must be after start)'),
    affiliateId: z.string().optional().describe('Affiliate ID to associate with this coupon'),
    requiresSubscriptionProduct: z.string().optional().describe('Product ID - coupon can only be used by active subscribers of this product'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.durationInMonths !== undefined) data.duration_in_months = ctx.input.durationInMonths;
    ...
    if (ctx.input.requiresSubscriptionProduct !== undefined) data.requires_subscription_product = ctx.input.requiresSubscriptionProduct;
