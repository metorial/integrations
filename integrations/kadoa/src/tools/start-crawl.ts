...
Provide a single URL or multiple URLs from the same domain. Returns a session ID for tracking progress.`,
  ...
  .input(z.object({
    ...
    startUrls: z.array(z.string()).optional().describe('Multiple URLs to crawl (same domain)'),
    maxDepth: z.number().optional().describe('Maximum crawl depth (default varies, max 200)'),
