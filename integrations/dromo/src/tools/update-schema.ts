...
let fieldInputSchema = z.object({
  ...
  type: z.string().optional().describe('Field type: string, number, email, phone, url, currency, percentage, etc.'),
  description: z.string().optional().describe('Help text for the field'),
  required: z.boolean().optional().describe('Whether the field is required'),
  alternateMatches: z.array(z.string()).optional().describe('Alternate column names that match this field'),
  validators: z.array(z.any()).optional().describe('Validation rules for this field'),
});

export let updateSchema = SlateTool.create(
  spec,
  {
    name: 'Update Schema',
    key: 'update_schema',
  ...
  .input(z.object({
    ...
    settings: z.record(z.string(), z.any()).optional().describe('Optional schema-level settings'),
  }))
  .output(z.object({
