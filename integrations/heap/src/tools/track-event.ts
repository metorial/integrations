...
let eventSchema = z.object({
  identity: z.string().optional().describe('User identity (e.g., email). Provide either identity or userId, not both.'),
  userId: z.string().optional().describe('Heap user ID from the SDK. Must be a string representation of a number between 0 and 2^53 - 1. Provide either identity or userId, not both.'),
  event: z.string().describe('Name of the custom event. Limited to 1024 characters.'),
  properties: z.record(z.string(), z.union([z.string(), z.number(), z.boolean()])).optional().describe('Key-value pairs of event properties. Keys must be under 512 characters, values must be under 1024 characters.'),
  timestamp: z.string().optional().describe('ISO 8601 timestamp for the event (e.g., "2024-01-15T10:30:00+00:00"). Defaults to the current time.'),
  idempotencyKey: z.string().optional().describe('Unique key to prevent duplicate event ingestion. Subsequent calls with the same key are ignored.'),
  sessionId: z.string().optional().describe('Session identifier to associate the event with a specific user session.'),
});
...
Use this for events that need to exactly match your backend data, such as completed transactions, or events not available for client-side capture.`,
  ...
  .input(z.object({
    identity: z.string().optional().describe('User identity for a single event. Provide either this or userId, not both.'),
    userId: z.string().optional().describe('Heap user ID for a single event. Provide either this or identity, not both.'),
    event: z.string().optional().describe('Event name for a single event.'),
    properties: z.record(z.string(), z.union([z.string(), z.number(), z.boolean()])).optional().describe('Properties for a single event.'),
    ...
    events: z.array(eventSchema).optional().describe('Array of events for bulk tracking. Max 1000 events per request.'),
  }))
  .output(z.object({
    tracked: z.number().describe('Number of events successfully sent to Heap.'),
    mode: z.enum(['single', 'bulk']).describe('Whether the request was processed as a single event or bulk.'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (!ctx.input.event) {
      throw new Error('Either "event" (for single tracking) or "events" (for bulk tracking) must be provided.');
    }
