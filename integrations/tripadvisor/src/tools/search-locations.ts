...
export let searchLocations = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    searchQuery: z.string().describe('Text to search for (e.g., "Eiffel Tower", "Bora Bora", "The French Laundry")'),
    category: z.enum(['hotels', 'attractions', 'restaurants', 'geos']).optional().describe('Filter results by location type'),
    phone: z.string().optional().describe('Phone number to narrow results (any format, no "+" prefix)'),
    address: z.string().optional().describe('Address to narrow results'),
    latLong: z.string().optional().describe('Latitude/longitude pair to scope search (e.g., "42.3455,-71.10767")'),
    radius: z.number().optional().describe('Search radius distance from the latLong point (must be > 0)'),
