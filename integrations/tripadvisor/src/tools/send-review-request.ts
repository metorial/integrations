...
let guestRequestSchema = z.object({
  ...
  partnerRequestId: z.string().optional().describe('Your own booking/reference ID for tracking'),
  ...
  checkout: z.string().describe('Check-out date in ISO-8601 format (YYYY-MM-DD). Email is sent 2 days after this date.'),
  language: z.string().optional().describe('Language code for the review request email (e.g., "en", "fr")'),
  country: z.string().describe('ISO 3166-1 alpha-2 country code of the guest (e.g., "US", "GB")'),
});
...
export let sendReviewRequest = SlateTool.create(
  spec,
  {
