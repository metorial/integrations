...
export let searchExercises = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    name: z.string().optional().describe('Exercise name or partial name (e.g. "press", "squat")'),
    type: z.enum(['cardio', 'olympic_weightlifting', 'plyometrics', 'powerlifting', 'strength', 'stretching', 'strongman']).optional().describe('Exercise type'),
    muscle: z.string().optional().describe('Target muscle group (e.g. biceps, chest, quadriceps, hamstrings, abdominals)'),
    difficulty: z.enum(['beginner', 'intermediate', 'expert']).optional().describe('Difficulty level'),
  }))
  .output(z.object({
