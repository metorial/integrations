...
let emailVersionSchema = z.object({
  ...
  signature: z.enum(['SENDER', 'NONE']).optional().default('SENDER').describe('Signature mode'),
...
let stepSchema = z.object({
  type: z.enum(['EMAIL', 'LINKEDIN_VIEW_PROFILE', 'LINKEDIN_CONNECT', 'LINKEDIN_MESSAGE']).optional().default('EMAIL').describe('Step type'),
  versions: z.array(emailVersionSchema).optional().describe('Email content versions for A/B testing (up to 5). Required for EMAIL steps.'),
  delayDays: z.number().optional().describe('Number of days to wait before sending this step (after previous step)'),
  deliveryTime: z.record(z.string(), z.array(deliveryTimeIntervalSchema)).optional().describe('Delivery time windows per day of week (e.g., {"monday": [{"from":"08:00","to":"17:00"}]})'),
  linkedinMessage: z.string().optional().describe('Message text for LinkedIn message/connect steps'),
});
...
export let createCampaign = SlateTool.create(
  ...
  .input(z.object({
    ...
    emailAccountIds: z.array(z.number()).optional().describe('IDs of email accounts (mailboxes) to send from'),
    timezone: z.string().optional().describe('Campaign timezone (e.g., "America/New_York")'),
    prospectTimezone: z.boolean().optional().describe('Whether to use the prospect\'s timezone for sending'),
    dailyEnroll: z.number().optional().describe('Maximum number of new prospects to contact per day'),
    gdprUnsubscribe: z.boolean().optional().describe('Include GDPR-compliant unsubscribe link'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.prospectTimezone !== undefined) settings.prospect_timezone = ctx.input.prospectTimezone;
    if (ctx.input.dailyEnroll !== undefined) settings.daily_enroll = ctx.input.dailyEnroll;
    if (ctx.input.gdprUnsubscribe !== undefined) settings.gdpr_unsubscribe = ctx.input.gdprUnsubscribe;
    if (ctx.input.listUnsubscribe !== undefined) settings.list_unsubscribe = ctx.input.listUnsubscribe;
    if (ctx.input.trackOpens !== undefined) settings.open_disabled_list = !ctx.input.trackOpens;
