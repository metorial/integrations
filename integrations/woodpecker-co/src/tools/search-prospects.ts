...
let prospectSchema = z.object({
  ...
  status: z.string().optional().describe('Global prospect status (ACTIVE, BLACKLIST, REPLIED, BOUNCED, INVALID)'),
...
export let searchProspects = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    search: z.string().optional().describe('Comma-separated search criteria using field=value format (e.g., "email=john@example.com,company=Acme")'),
    status: z.enum(['ACTIVE', 'BOUNCED', 'REPLIED', 'BLACKLIST', 'INVALID']).optional().describe('Filter by global prospect status'),
    campaignId: z.number().optional().describe('Filter prospects enrolled in a specific campaign'),
    interested: z.enum(['INTERESTED', 'MAYBE-LATER', 'NOT-INTERESTED', 'NOT-MARKED']).optional().describe('Filter by interest level'),
    contacted: z.boolean().optional().describe('Filter by whether prospect was contacted'),
    includeCampaignDetails: z.boolean().optional().default(false).describe('Include campaign enrollment details in results'),
    ...
    sort: z.string().optional().describe('Sort field with direction prefix: +field (asc) or -field (desc)'),
  }))
  .output(z.object({
