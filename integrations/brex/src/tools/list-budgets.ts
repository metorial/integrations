...
let budgetSchema = z.object({
  ...
  periodType: z.string().optional().describe('Budget period: MONTHLY, QUARTERLY, YEARLY, ONE_TIME'),
  limit: z.object({
    amount: z.number().describe('Limit amount in cents'),
    currency: z.string().nullable().describe('Currency code'),
  }).nullable().optional().describe('Spend limit for the budget'),
  currentPeriodBalance: z.object({
    amount: z.number().describe('Available balance in cents'),
    currency: z.string().nullable().describe('Currency code'),
  }).nullable().optional().describe('Remaining balance for the current period'),
});
  ...
  .handleInvocation(async (ctx) => {
    ...
    let budgets = result.items.map((b: any) => ({
      ...
      currentPeriodBalance: b.current_period_balance
        ? { amount: b.current_period_balance.amount, currency: b.current_period_balance.currency }
        : null,
    }));
