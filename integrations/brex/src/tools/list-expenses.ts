...
let expenseSchema = z.object({
  ...
  purchasedAt: z.string().nullable().optional().describe('ISO 8601 timestamp of when the purchase was made'),
...
export let listExpenses = SlateTool.create(
  ...
  .input(z.object({
    updatedAtStart: z.string().optional().describe('Filter expenses updated after this ISO 8601 timestamp'),
    expand: z.array(z.enum(['merchant', 'budget', 'user', 'department', 'location', 'receipts'])).optional().describe('Related data to include in the response'),
