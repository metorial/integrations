...
export let brexEvents = SlateTrigger.create(
  spec,
  {
    name: 'Brex Events',
    key: 'brex_events',
  ...
  .output(z.object({
    ...
    paymentStatus: z.string().nullable().optional().describe('Payment status (for expense events)'),
    paymentType: z.string().nullable().optional().describe('Payment type (for expense events)'),
    amount: z.object({
      amount: z.number().describe('Amount in cents'),
      currency: z.string().nullable().describe('Currency code'),
    }).nullable().optional().describe('Transaction amount'),
    merchantDescription: z.string().nullable().optional().describe('Merchant description (for expense events)'),
    transferId: z.string().nullable().optional().describe('Transfer ID (for transfer events)'),
    ...
    userStatus: z.string().nullable().optional().describe('Updated user status (for user events)'),
    referralId: z.string().nullable().optional().describe('Referral ID (for referral events)'),
    applicationStatus: z.string().nullable().optional().describe('Application status (for referral events)'),
  }))
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      if (eventType === 'EXPENSE_PAYMENT_UPDATED') {
        ...
        output.amount = payload.amount
          ...
          : null;
        output.merchantDescription = payload.merchant_description ?? payload.merchant?.raw_descriptor ?? null;
