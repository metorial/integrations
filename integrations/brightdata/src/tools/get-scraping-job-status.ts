...
export let getScrapingJobStatus = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    snapshotId: z.string().describe('The snapshot ID returned when the scraping job was triggered.'),
  }))
  .output(z.object({
    ...
    status: z.string().describe('Current status: "starting", "running", "ready", or "failed".'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let statusEmoji = result.status === 'ready' ? '✅' : result.status === 'failed' ? '❌' : '⏳';
