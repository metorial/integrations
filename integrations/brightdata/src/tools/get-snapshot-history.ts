...
export let getSnapshotHistory = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    datasetId: z.string().describe('ID of the dataset/scraper to retrieve snapshot history for.'),
  }))
  .output(z.object({
    snapshots: z.array(z.record(z.string(), z.unknown())).describe('List of snapshots with their metadata (ID, status, timestamps).'),
  }))
