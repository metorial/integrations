...
export let searchSerp = SlateTool.create(
  ...
  .input(z.object({
    ...
    url: z.string().describe('Full search engine URL with query parameters. Example: "https://www.google.com/search?q=pizza&gl=us&hl=en&brd_json=1"'),
    format: z.enum(['raw', 'json']).default('raw').describe('API response format. "raw" returns the search engine page as-is, "json" returns structured data.'),
    country: z.string().optional().describe('Two-letter country code for geo-targeted search results.'),
  }))
  .output(z.object({
    content: z.string().describe('The SERP results in the requested format (raw HTML or JSON string).'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let contentPreview = result.content.length > 300
