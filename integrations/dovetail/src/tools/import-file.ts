...
export let importFile = SlateTool.create(
  spec,
  {
    name: 'Import File',
    key: 'import_file',
  ...
  .input(z.object({
    targetType: z.enum(['note', 'data', 'insight', 'doc']).describe('What to create from the imported file'),
    url: z.string().describe('Public URL for direct file download'),
    mimeType: z.string().optional().describe('MIME type of the file (required if URL lacks file extension)'),
    title: z.string().optional().describe('Title for the imported resource (insight only)'),
    projectId: z.string().optional().describe('Project to associate the import with (data only)'),
  }))
  .output(z.object({
    resourceId: z.string(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.targetType === 'data') {
      let data = await client.importDataFromFile(ctx.input.url, ctx.input.projectId, ctx.input.mimeType);
      return {
        output: {
          resourceId: data.id,
          resourceType: 'data',
    ...
    if (ctx.input.targetType === 'insight') {
      let insight = await client.importInsightFromFile(ctx.input.url, ctx.input.title, ctx.input.mimeType);
      return {
        output: {
