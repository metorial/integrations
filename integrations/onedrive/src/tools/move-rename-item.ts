...
export let moveRenameItemTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    driveId: z.string().optional().describe('ID of the drive containing the item. Defaults to the user\'s personal OneDrive.'),
    itemId: z.string().describe('ID of the item to move or rename'),
    destinationFolderId: z.string().optional().describe('ID of the destination folder to move the item to'),
    destinationDriveId: z.string().optional().describe('ID of the destination drive if moving across drives'),
