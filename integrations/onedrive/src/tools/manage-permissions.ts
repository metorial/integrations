...
let permissionSchema = z.object({
  ...
  linkScope: z.string().optional().describe('Scope of the link (anonymous, organization, users)'),
  expirationDateTime: z.string().optional().describe('When the permission expires'),
});

export let managePermissionsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'remove']).describe('"list" returns all permissions; "remove" deletes a specific permission'),
    permissionId: z.string().optional().describe('ID of the permission to remove (required for "remove" action)'),
  }))
  .output(z.object({
    permissions: z.array(permissionSchema).optional().describe('List of permissions (for "list" action)'),
    removed: z.boolean().optional().describe('Whether the permission was removed (for "remove" action)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    // Remove action
    if (!ctx.input.permissionId) throw new Error('permissionId is required for the "remove" action');
