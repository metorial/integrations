...
export let auth = SlateAuth.create()
  ...
  .addOauth({
    ...
    inputSchema: z.object({
      tenantId: z.string().optional().describe('Azure AD tenant ID. Leave empty for "common" (any Microsoft account).'),
    }),
    ...
    handleTokenRefresh: async (ctx) => {
      if (!ctx.output.refreshToken) {
        throw new Error('No refresh token available. Re-authorize with the offline_access scope.');
      }
