...
}).describe('Address of the credit note recipient. Provide either contactId or inline address fields.');
...
export let createCreditNote = SlateTool.create(
  ...
  .input(z.object({
    ...
    paymentConditions: paymentConditionsSchema.optional().describe('Payment conditions for the credit note'),
    ...
    voucherDate: z.string().optional().describe('Credit note date in ISO 8601 format (e.g. 2024-01-15). Defaults to today.'),
    finalize: z.boolean().optional().describe('If true, the credit note is finalized immediately and cannot be edited further'),
    precedingSalesVoucherId: z.string().optional().describe('ID of a preceding sales voucher (e.g. an invoice) to link to this credit note'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.paymentConditions) creditNoteData.paymentConditions = ctx.input.paymentConditions;
    if (ctx.input.shippingConditions) creditNoteData.shippingConditions = ctx.input.shippingConditions;
