...
let contactSummarySchema = z.object({
  ...
  }).optional().describe('Assigned roles'),
  company: z.object({
    ...
    })).optional().describe('Contact persons at the company'),
...
export let listContacts = SlateTool.create(
  ...
  .input(z.object({
    ...
    name: z.string().optional().describe('Filter contacts by name (company name or person name)'),
  ...
  .output(z.object({
    contacts: z.array(contactSummarySchema).describe('List of contacts matching the filters'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let filterSummary = filtersApplied.length > 0 ? ` (filtered by ${filtersApplied.join(', ')})` : '';
