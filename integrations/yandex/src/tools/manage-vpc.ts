...
export let listNetworks = SlateTool.create(
  spec,
  {
    name: 'List Networks',
    key: 'list_networks',
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await vpc.listNetworks(ctx.auth, folderId, ctx.input.pageSize, ctx.input.pageToken);
...
export let listSubnets = SlateTool.create(
  spec,
  {
    name: 'List Subnets',
    key: 'list_subnets',
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await vpc.listSubnets(ctx.auth, folderId, ctx.input.pageSize, ctx.input.pageToken);
...
export let manageNetwork = SlateTool.create(
  spec,
  {
    name: 'Manage Network',
    key: 'manage_network',
...
export let manageSubnet = SlateTool.create(
  spec,
  {
    name: 'Manage Subnet',
    key: 'manage_subnet',
  ...
  .input(z.object({
    ...
    zoneId: z.string().optional().describe('Availability zone (required for create, e.g. ru-central1-a)'),
    v4CidrBlocks: z.array(z.string()).optional().describe('IPv4 CIDR blocks (required for create, e.g. ["10.1.0.0/24"])'),
    labels: z.record(z.string(), z.string()).optional().describe('Labels'),
  ...
  .handleInvocation(async (ctx) => {
    if (ctx.input.action === 'create') {
      ...
      if (!ctx.input.v4CidrBlocks) throw new Error('v4CidrBlocks is required for subnet creation');
