...
export let listFunctions = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await functions.listFunctions(ctx.auth, folderId, ctx.input.pageSize, ctx.input.pageToken);
...
export let invokeFunction = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    let result = await functions.invokeFunction(ctx.auth, ctx.input.functionId, ctx.input.payload);
