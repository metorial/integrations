...
export let readLogs = SlateTool.create(
  spec,
  {
    name: 'Read Logs',
    key: 'read_logs',
  ...
  .input(z.object({
    ...
    levels: z.array(z.enum(['TRACE', 'DEBUG', 'INFO', 'WARN', 'ERROR', 'FATAL'])).optional().describe('Filter by log levels'),
...
export let writeLogs = SlateTool.create(
  spec,
  {
    name: 'Write Logs',
    key: 'write_logs',
  ...
  .input(z.object({
    ...
    folderId: z.string().optional().describe('Folder ID (uses default log group if logGroupId not specified)'),
    entries: z.array(z.object({
      timestamp: z.string().describe('Entry timestamp in RFC 3339 format'),
      level: z.enum(['TRACE', 'DEBUG', 'INFO', 'WARN', 'ERROR', 'FATAL']).optional().describe('Log level'),
      message: z.string().describe('Log message'),
      jsonPayload: z.record(z.string(), z.unknown()).optional().describe('Structured data payload'),
