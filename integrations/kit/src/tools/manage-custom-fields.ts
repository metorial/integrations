...
export let manageCustomFields = SlateTool.create(
  ...
  .input(z.object({
    ...
    customFieldId: z.number().optional().describe('Custom field ID (required for update and delete)'),
    label: z.string().optional().describe('Custom field label (required for create and update)')
  }))
  .output(z.object({
    ...
    })).optional().describe('List of custom fields (for list action)'),
    customField: z.object({
      customFieldId: z.number().describe('Unique custom field ID'),
      key: z.string().describe('Custom field key'),
      label: z.string().describe('Custom field display label')
    }).optional().describe('Created or updated custom field'),
