...
export let updateFunction = SlateTool.create(
  ...
  .input(z.object({
    ...
    code: z.object({
      s3Bucket: z.string().optional().describe('S3 bucket containing the new code package'),
      ...
      publish: z.boolean().optional().describe('Publish a new version after updating code'),
      architectures: z.array(z.enum(['x86_64', 'arm64'])).optional().describe('Target architecture')
    }).optional().describe('New function code'),
    configuration: z.object({
      ...
      environment: z.record(z.string(), z.string()).optional().describe('New environment variables (replaces existing)'),
      layers: z.array(z.string()).optional().describe('New layer ARNs (replaces existing)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.code) {
      ...
      if (ctx.input.code.s3ObjectVersion) codeParams['S3ObjectVersion'] = ctx.input.code.s3ObjectVersion;
      ...
      if (ctx.input.code.architectures) codeParams['Architectures'] = ctx.input.code.architectures;
    ...
    if (ctx.input.configuration) {
      ...
      if (cfg.ephemeralStorageSize) configParams['EphemeralStorage'] = { Size: cfg.ephemeralStorageSize };
      ...
      lastResult = await client.updateFunctionConfiguration(ctx.input.functionName, configParams);
      configUpdated = true;
    }
