...
export let manageLayer = SlateTool.create(
  spec,
  {
    name: 'Manage Layer',
    key: 'manage_layer',
  ...
  .input(z.object({
    action: z.enum(['publish', 'get', 'delete', 'list_layers', 'list_versions']).describe('Operation to perform'),
    layerName: z.string().optional().describe('Layer name (required for publish/get/delete/list_versions)'),
    versionNumber: z.number().optional().describe('Layer version number (required for get/delete)'),
    content: z.object({
    ...
    }).optional().describe('Layer content (required for publish)'),
    compatibleRuntimes: z.array(z.string()).optional().describe('Compatible runtimes (e.g., ["nodejs22.x", "python3.13"])'),
    compatibleArchitectures: z.array(z.enum(['x86_64', 'arm64'])).optional().describe('Compatible architectures'),
    ...
    compatibleRuntimeFilter: z.string().optional().describe('Filter layers by compatible runtime (for list_layers)')
  }))
  .output(z.object({
    ...
    })).optional().describe('List of layers (for list_layers)'),
    versions: z.array(z.object({
      layerVersionArn: z.string().optional(),
      versionNumber: z.number().optional(),
      description: z.string().optional(),
      compatibleRuntimes: z.array(z.string()).optional()
    })).optional().describe('List of layer versions (for list_versions)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'list_layers') {
      let result = await client.listLayers(undefined, undefined, ctx.input.compatibleRuntimeFilter);
    ...
    if (ctx.input.content.s3ObjectVersion) contentObj['S3ObjectVersion'] = ctx.input.content.s3ObjectVersion;
    ...
    if (ctx.input.compatibleArchitectures) params['CompatibleArchitectures'] = ctx.input.compatibleArchitectures;
