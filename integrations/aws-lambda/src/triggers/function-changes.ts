...
export let functionChanges = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Polls for new, updated, or deleted Lambda functions by comparing snapshots of the function list. Detects when functions are created, modified (code or configuration changes), or removed.'
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      for (let [arn, current] of Object.entries(currentMap)) {
        ...
        } else if (previous.lastModified !== current.lastModified || previous.codeSha256 !== current.codeSha256) {
