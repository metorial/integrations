...
export let createMonitor = SlateTool.create(
  ...
  .input(z.object({
    ...
    keywordValue: z.string().optional().describe('Keyword to search for (required for keyword monitors)'),
    keywordType: z.enum(['exists', 'not_exists']).optional().describe('Whether keyword should exist or not exist on the page'),
    keywordCaseSensitive: z.boolean().optional().describe('Whether keyword matching is case-sensitive (default true)'),
    portSubType: z.enum(['http', 'https', 'ftp', 'smtp', 'pop3', 'imap', 'custom']).optional().describe('Port sub-type (required for port monitors)'),
    portNumber: z.number().optional().describe('Custom port number (required when portSubType is "custom")'),
    interval: z.number().optional().describe('Check interval in seconds'),
    timeout: z.number().optional().describe('Timeout in seconds (1-60, HTTP/keyword/port only)'),
    httpMethod: z.enum(['head', 'get', 'post', 'put', 'patch', 'delete', 'options']).optional().describe('HTTP method to use'),
    postValue: z.string().optional().describe('Request body data for POST/PUT/PATCH/DELETE/OPTIONS'),
    postContentType: z.enum(['text/html', 'application/json']).optional().describe('Content type for POST body'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let typeMap: Record<string, number> = {
      http: 1, keyword: 2, ping: 3, port: 4, heartbeat: 5
    };
