...
export let monitorStatusChanges = SlateTrigger.create(
  spec,
  {
    name: 'Monitor Status Changes',
    key: 'monitor_status_changes',
    description: 'Triggers when a monitor changes status (e.g. goes down, comes back up, gets paused). Polls UptimeRobot for current monitor statuses and emits events when changes are detected.'
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    friendlyName: z.string().describe('Display name of the monitor'),
    url: z.string().describe('URL or IP being monitored'),
    monitorType: z.number().describe('Monitor type: 1=HTTP(s), 2=Keyword, 3=Ping, 4=Port, 5=Heartbeat'),
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let previousStatuses: Record<string, number> = (ctx.state?.monitorStatuses as Record<string, number>) || {};
    ...
    handleEvent: async (ctx) => {
      ...
      let previousLabel = statusLabels[ctx.input.previousStatus] || `Unknown (${ctx.input.previousStatus})`;
      let currentLabel = statusLabels[ctx.input.currentStatus] || `Unknown (${ctx.input.currentStatus})`;

      let eventType = 'monitor.status_changed';
      if (ctx.input.currentStatus === 9 || ctx.input.currentStatus === 8) {
        eventType = 'monitor.down';
      } else if (ctx.input.currentStatus === 2 && (ctx.input.previousStatus === 9 || ctx.input.previousStatus === 8)) {
