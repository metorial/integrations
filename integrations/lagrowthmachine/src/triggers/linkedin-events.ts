...
export let linkedinEvents = SlateTrigger.create(
  spec,
  {
    ...
    key: 'linkedin_events',
    description: 'Triggers when LinkedIn-related events occur in La Growth Machine, including connection requests sent/accepted, messages sent/replied, and profile visits.',
  }
)
  .input(z.object({
...
let mapLinkedInEventType = (type: string | undefined): string => {
  ...
  if (normalized.includes('request') && normalized.includes('accept')) return 'linkedin.contact_request_accepted';
  if (normalized.includes('request') && normalized.includes('sent')) return 'linkedin.contact_request_sent';
  if (normalized.includes('request')) return 'linkedin.contact_request_sent';
  if (normalized.includes('replied') || normalized.includes('reply')) return 'linkedin.message_replied';
  if (normalized.includes('visit')) return 'linkedin.profile_visited';
  if (normalized.includes('message') && normalized.includes('sent')) return 'linkedin.message_sent';
