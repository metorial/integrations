...
export let installationEventsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Installation Events',
    key: 'installation_events',
    description: 'Triggers when a Sentry integration is installed or uninstalled by a user. Useful for tracking which organizations have your integration active.'
  }
)
  .input(z.object({
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      let installation = ctx.input.payload?.data?.installation || ctx.input.payload?.data || {};
      ...
      return {
        ...
        output: {
          installationId: String(installation.uuid || installation.id || ctx.input.installationId),
