...
export let manageReleaseTool = SlateTool.create(
  spec,
  {
    name: 'Manage Release',
    key: 'manage_release',
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'update', 'delete', 'deploy']).describe('Action to perform'),
    version: z.string().optional().describe('Release version identifier (required for get/create/update/delete/deploy)'),
    projects: z.array(z.string()).optional().describe('Project slugs to associate (required for create)'),
    ref: z.string().optional().describe('Git ref (commit SHA or tag)'),
    url: z.string().optional().describe('URL for the release'),
    dateReleased: z.string().optional().describe('ISO 8601 date when the release was deployed'),
    ...
    })).optional().describe('Commits to associate with the release'),
    environment: z.string().optional().describe('Deploy environment (required for deploy action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      if (!ctx.input.version) throw new Error('version is required');
      if (!ctx.input.projects || ctx.input.projects.length === 0) throw new Error('projects array is required');
