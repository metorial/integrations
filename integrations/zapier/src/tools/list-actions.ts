...
Filter by action type to find triggers (READ), searches (SEARCH), or write actions (WRITE).`,
  ...
  .input(z.object({
    ...
    actionType: z.enum(['READ', 'READ_BULK', 'SEARCH', 'SEARCH_AND_WRITE', 'SEARCH_OR_WRITE', 'WRITE']).optional()
      .describe('Filter by action type. READ = triggers, WRITE = actions, SEARCH = searches.'),
  }))
  .output(z.object({
