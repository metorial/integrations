...
export let getInventory = SlateTool.create(
  ...
  .input(z.object({
    catalogObjectIds: z.array(z.string()).describe('Catalog object IDs (item variations) to get inventory counts for'),
    locationIds: z.array(z.string()).optional().describe('Filter by specific location IDs'),
    states: z.array(z.enum(['IN_STOCK', 'SOLD', 'RETURNED_BY_CUSTOMER', 'RESERVED_FOR_SALE', 'ORDERED_FROM_VENDOR', 'RECEIVED_FROM_VENDOR', 'IN_TRANSIT_TO', 'NONE', 'WASTE', 'UNLINKED_RETURN', 'COMPOSED', 'DECOMPOSED'])).optional().describe('Filter by inventory state'),
