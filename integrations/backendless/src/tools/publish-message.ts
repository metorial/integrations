...
export let publishMessage = SlateTool.create(
  ...
  .input(z.object({
    ...
    message: z.unknown().describe('Message content to publish (string, number, object, or array)'),
    publisherId: z.string().optional().describe('Identifier of the message publisher'),
    headers: z.record(z.string(), z.string()).optional().describe('Custom headers for message filtering by subscribers'),
    publishAt: z.number().optional().describe('Timestamp (milliseconds) to schedule future delivery'),
    repeatEvery: z.number().optional().describe('Interval in seconds for recurring delivery'),
    repeatExpiresAt: z.number().optional().describe('Timestamp (milliseconds) when recurring delivery should stop')
  }))
  .output(z.object({
