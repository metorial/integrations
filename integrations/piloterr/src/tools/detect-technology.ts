...
export let detectTechnology = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Response time is typically 10-15 seconds.'
    ],
    tags: {
      readOnly: true
    }
  ...
  .input(z.object({
    ...
    mode: z.enum(['simple', 'expert']).default('expert').describe('Detection mode: "expert" for detailed info with confidence scores, "simple" for a concise summary')
  }))
  .output(z.object({
    urls: z.any().optional().describe('Analyzed URLs'),
    cms: z.string().nullable().optional().describe('Detected CMS (simple mode)'),
    technologies: z.array(technologySchema).optional().describe('Detected technologies with details')
  }))
