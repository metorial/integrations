...
export let manageCampaign = SlateTool.create(
  ...
  .input(z.object({
    campaignId: z.string().optional().describe('Campaign ID for updating an existing campaign. Omit to create a new campaign.'),
    campaignName: z.string().optional().describe('Name of the campaign'),
    senderName: z.string().optional().describe('Sender display name'),
    senderEmail: z.string().optional().describe('Sender email address (must use a verified domain)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.campaignId) {
      ...
      if (!ctx.input.campaignName || !ctx.input.senderName || !ctx.input.senderEmail || !ctx.input.subject || !ctx.input.htmlContent) {
        throw new Error('When creating a new campaign, campaignName, senderName, senderEmail, subject, and htmlContent are all required.');
      }
