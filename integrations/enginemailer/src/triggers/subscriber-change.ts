...
export let subscriberChange = SlateTrigger.create(
  spec,
  {
    name: 'Subscriber Change',
    key: 'subscriber_change',
    description: '[Polling fallback] Polls for subscriber changes including new subscribers, modifications, deletions, unsubscribes, tagging, and untagging events.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let [newSubs, modifiedSubs, deletedSubs, unsubscribes, tagged, untagged] = await Promise.allSettled([
        ...
        if (result.status === 'fulfilled' && result.value) {
          ...
          if (Array.isArray(items)) {
            for (let item of items) {
              let email = item.email ?? item.Email ?? item.emailaddress ?? item.EmailAddress ?? '';
              if (email) {
                inputs.push({
                  changeType,
