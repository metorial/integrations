...
export let checkoutEvents = SlateTrigger.create(
  spec,
  {
    ...
    key: 'checkout_events',
    description: 'Triggered when a Checkout session is completed or expires. Use this to fulfill orders, activate services, or handle abandoned checkouts.',
  }
)
  .input(z.object({
    eventId: z.string().describe('Stripe event ID'),
    eventType: z.string().describe('Event type (checkout.session.completed or checkout.session.expired)'),
