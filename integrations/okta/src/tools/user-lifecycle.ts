...
export let userLifecycleTool = SlateTool.create(
  spec,
  {
    name: 'User Lifecycle',
    key: 'user_lifecycle',
  ...
  .input(z.object({
    ...
    sendEmail: z.boolean().optional().describe('Whether to send a notification email to the user (applies to activate, deactivate, reset_password)'),
  }))
  .output(z.object({
    userId: z.string(),
    action: z.string().describe('The action that was performed'),
    success: z.boolean(),
    activationUrl: z.string().optional().describe('Activation URL returned when activating with sendEmail=false'),
    resetPasswordUrl: z.string().optional().describe('Password reset URL returned when resetting with sendEmail=false'),
  }))
