...
export let createUserTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    login: z.string().optional().describe('Okta login identifier. Defaults to email if not provided'),
    mobilePhone: z.string().optional().describe('Mobile phone number'),
    title: z.string().optional().describe('Job title'),
    department: z.string().optional().describe('Department'),
    organization: z.string().optional().describe('Organization'),
    additionalProfile: z.record(z.string(), z.any()).optional().describe('Additional custom profile attributes'),
    password: z.string().optional().describe('Initial password for the user. If omitted, user will be emailed an activation link'),
    groupIds: z.array(z.string()).optional().describe('IDs of groups to assign the user to'),
    activate: z.boolean().optional().describe('Activate the user immediately (default true). If false, the user is created in STAGED status'),
  }))
  .output(z.object({
