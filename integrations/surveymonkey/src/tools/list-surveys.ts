...
export let listSurveys = SlateTool.create(
  ...
  .input(z.object({
    ...
    sortBy: z.enum(['title', 'date_modified', 'num_responses']).optional().describe('Field to sort by'),
    sortOrder: z.enum(['ASC', 'DESC']).optional().describe('Sort direction'),
    title: z.string().optional().describe('Filter by survey title (partial match)'),
    startModifiedAt: z.string().optional().describe('Filter surveys modified after this date (YYYY-MM-DDTHH:MM:SS)'),
    endModifiedAt: z.string().optional().describe('Filter surveys modified before this date (YYYY-MM-DDTHH:MM:SS)'),
