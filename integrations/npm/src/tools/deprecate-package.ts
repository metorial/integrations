...
export let deprecatePackage = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    message: z.string().describe('Deprecation message shown to users. Use empty string to undeprecate.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    await client.deprecatePackageVersion(ctx.input.packageName, ctx.input.version, ctx.input.message);
