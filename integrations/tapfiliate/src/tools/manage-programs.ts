...
export let manageProgramAffiliate = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    coupon: z.string().optional().describe('Coupon code to assign (for "add" or "update" action)'),
  }))
  .output(z.object({
    affiliateId: z.string().describe('ID of the affiliate'),
    programId: z.string().describe('ID of the program'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'add':
        ...
        break;
      case 'approve':
        result = await client.approveAffiliateForProgram(ctx.input.programId, ctx.input.affiliateId);
        break;
      case 'disapprove':
        result = await client.disapproveAffiliateForProgram(ctx.input.programId, ctx.input.affiliateId);
        break;
      case 'update':
        result = await client.updateAffiliateInProgram(ctx.input.programId, ctx.input.affiliateId, {
          coupon: ctx.input.coupon,
          approved: ctx.input.approved,
        });
        break;
...
export let listProgramAffiliates = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    let client = new TapfiliateClient({ token: ctx.auth.token });
    let results = await client.listProgramAffiliates(ctx.input.programId, { page: ctx.input.page });
