...
export let getCrmData = SlateTool.create(spec, {
  ...
  .input(
    z.object({
      mode: z
        .enum(['objects', 'associations'])
        .describe('Whether to retrieve CRM objects or manual CRM-call associations'),
      integrationId: z
        .string()
        .optional()
        .describe('CRM integration ID (required for objects mode)'),
      objectType: z
        .string()
        .optional()
        .describe(
          'CRM object type: Account, Contact, Deal, Lead, User (required for objects mode)'
        ),
      objectCrmIds: z
        .array(z.string())
        .optional()
        .describe('CRM IDs of the objects to retrieve (required for objects mode)'),
      fromDateTime: z
        .string()
        .optional()
        .describe(
          'Start datetime for associations (required for associations mode, ISO 8601)'
        ),
  ...
  .output(
    z.object({
      crmObjects: z
        ...
        .optional()
        .describe('Map of CRM object IDs to their data (objects mode)'),
      associations: z
        .array(z.any())
        .optional()
        .describe('Manual CRM-call associations (associations mode)'),
  ...
  .handleInvocation(async ctx => {
    ...
    if (ctx.input.mode === 'objects') {
      if (!ctx.input.integrationId || !ctx.input.objectType || !ctx.input.objectCrmIds) {
        throw new Error(
          'integrationId, objectType, and objectCrmIds are required for objects mode.'
        );
      }
