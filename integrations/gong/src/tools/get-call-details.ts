...
let partySchema = z.object({
  ...
  affiliation: z.string().optional().describe('Whether the participant is internal or external'),
...
let callDetailSchema = z.object({
  ...
  })).optional().describe('Topics discussed'),
  trackers: z.array(z.object({
    name: z.string().optional(),
    count: z.number().optional(),
    occurrences: z.array(z.any()).optional(),
  })).optional().describe('Tracker matches found'),
  ...
  personInteractionStats: z.array(z.any()).optional().describe('Per-person interaction statistics'),
...
export let getCallDetails = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    includeContent: z.boolean().optional().describe('Include topics, trackers, brief, outline, highlights, key points, and call outcome'),
    includeInteraction: z.boolean().optional().describe('Include interaction stats, questions, and speakers'),
    ...
    let result = await client.getCallsExtensive({
      ...
      contentSelector: {
        exposedFields: {
          ...
          interaction: Object.keys(interactionFields).length > 0 ? interactionFields as any : undefined,
