...
export let getFileContent = SlateTool.create(
  ...
  .input(z.object({
    ...
    version: z.string().optional().describe('Version identifier (branch name, tag name, or commit SHA)'),
    versionType: z.enum(['branch', 'tag', 'commit']).optional().describe('Type of version identifier'),
    isFolder: z.boolean().optional().describe('Set to true to list folder contents instead of retrieving file content'),
    recursionLevel: z.enum(['none', 'oneLevel', 'full']).optional().describe('Recursion level for folder listing (default: "oneLevel")'),
  }))
  .output(z.object({
    ...
    })).optional().describe('List of items in the folder (for folders only)'),
  }))
