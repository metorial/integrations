...
export let getDocument = SlateTool.create(
  ...
  .input(z.object({
    doctype: z.string().describe('The DocType of the document (e.g., "Customer", "Sales Order", "Item")'),
    documentName: z.string().describe('The unique name/ID of the document'),
    fields: z.array(z.string()).optional().describe('Specific fields to return. If omitted, all fields are returned.')
  }))
  .output(z.object({
    document: z.record(z.string(), z.any()).describe('The retrieved document with all requested fields')
  }))
  .handleInvocation(async (ctx) => {
    ...
    let document = await client.getDocument(ctx.input.doctype, ctx.input.documentName, ctx.input.fields);
