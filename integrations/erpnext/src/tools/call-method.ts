...
export let callMethod = SlateTool.create(
  spec,
  {
    name: 'Call Method',
    key: 'call_method',
  ...
  .input(z.object({
    method: z.string().describe('The dotted path to the server method (e.g., "frappe.client.get_list")'),
    httpMethod: z.enum(['GET', 'POST']).optional().describe('HTTP method to use. Defaults to POST.'),
