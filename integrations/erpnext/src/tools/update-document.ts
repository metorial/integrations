...
export let updateDocument = SlateTool.create(
  ...
  .input(z.object({
    ...
    documentData: z.record(z.string(), z.any()).describe('Fields to update as key-value pairs (partial update supported)')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let document = await client.updateDocument(ctx.input.doctype, ctx.input.documentName, ctx.input.documentData);
