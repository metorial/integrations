...
export let listPurchases = SlateTool.create(
  ...
  .input(z.object({
    status: z.enum(['ACT', 'PND', 'RFD', 'ERR', 'CAN', 'HLD']).optional().describe('Filter by purchase status'),
    ...
    customerEmail: z.string().optional().describe('Filter by customer email (starts-with match, case-insensitive)'),
    customerFirstName: z.string().optional().describe('Filter by customer first name (starts-with match, case-insensitive)'),
    customerLastName: z.string().optional().describe('Filter by customer last name (starts-with match, case-insensitive)'),
    dateMin: z.string().optional().describe('Minimum date filter (PHP strtotime format, e.g. "2024-01-01")'),
    dateMax: z.string().optional().describe('Maximum date filter (PHP strtotime format, e.g. "2024-12-31")'),
    total: z.string().optional().describe('Total amount to compare against'),
    totalOp: z.enum(['eq', 'ne', 'gt', 'lt']).optional().describe('Comparison operator for total: eq, ne, gt, or lt'),
    ship: z.boolean().optional().describe('Filter for purchases with unshipped tangible goods'),
