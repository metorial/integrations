...
export let getListsTool = SlateTool.create(
  spec,
  {
    name: 'Get Lists',
    key: 'get_lists',
  ...
  .input(z.object({
    listSlug: z.string().optional().describe('Specific list slug or ID. If omitted, returns all lists.'),
  }))
  .output(z.object({
...
export let addListEntryTool = SlateTool.create(
  spec,
  {
    name: 'Add List Entry',
    key: 'add_list_entry',
  ...
  .input(z.object({
    ...
    parentObject: z.string().describe('The object slug the record belongs to (e.g. "people", "companies")'),
    entryValues: z.record(z.string(), z.any()).optional().describe('Additional attribute values for the list entry'),
    upsert: z.boolean().optional().default(false).describe('If true, creates or updates the entry based on the parent record'),
  }))
  .output(z.object({
...
export let updateListEntryTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    entryValues: z.record(z.string(), z.any()).describe('Attribute values to update, keyed by attribute slug'),
...
export let deleteListEntryTool = SlateTool.create(
  spec,
  {
...
export let queryListEntriesTool = SlateTool.create(
  spec,
  {
