...
export let recordEventsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Record Events',
    key: 'record_events',
    description: 'Triggers when records are created, updated, deleted, or merged in Attio. Covers all object types (People, Companies, Deals, custom objects).',
  }
)
  .input(z.object({
    ...
    duplicateRecordId: z.string().optional().describe('Duplicate record ID (for merge events)'),
