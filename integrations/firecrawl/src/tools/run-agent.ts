  ...
  .input(
    z.object({
      prompt: z
        .string()
        .describe(
          'Natural language description of what data to gather (max 10,000 characters)'
        ),
      urls: z.array(z.string()).optional().describe('URLs to constrain the agent to'),
      schema: z
        .record(z.string(), z.any())
        .optional()
        .describe('JSON schema for structuring extracted data'),
      maxCredits: z
        .number()
        .optional()
        .describe('Maximum credits the agent can consume (default: 2500)'),
      strictConstrainToURLs: z
        .boolean()
        .optional()
        .describe('If true, agent will only visit the provided URLs'),
      model: z
        .enum(['spark-1-mini', 'spark-1-pro'])
        .optional()
        .describe(
          'Model to use: spark-1-mini (default, 60% cheaper) or spark-1-pro (higher accuracy)'
        )
    })
  )
  .output(
    z.object({
      agentId: z
        .string()
        .describe('Unique ID for the agent job, use to check status and retrieve results')
    })
  )
