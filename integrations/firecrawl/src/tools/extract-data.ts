...
export let extractDataTool = SlateTool.create(
  spec,
  {
    name: 'Extract Data',
    key: 'extract_data',
...
Supports wildcard URLs (e.g., \`example.com/*\`) to extract data across an entire domain. Can optionally follow links outside the specified domain.`,
  ...
  .input(z.object({
    urls: z.array(z.string()).describe('URLs to extract data from. Supports wildcards like example.com/*'),
    prompt: z.string().optional().describe('Natural language description of what data to extract'),
    schema: z.record(z.string(), z.any()).optional().describe('JSON schema defining the desired output structure'),
    enableWebSearch: z.boolean().optional().describe('Allow the extractor to follow links outside the specified domain'),
    showSources: z.boolean().optional().describe('Include source information in the response'),
  }))
  .output(z.object({
    extractId: z.string().describe('Unique ID for the extract job, use to check status and retrieve results'),
  }))
