...
let searchResultSchema = z.object({
  ...
  markdown: z
    .string()
    .optional()
    .describe('Full page content as markdown (if scrapeOptions included)'),
...
Supports advanced query operators like exact phrases, site filters, and file type filters.`,
  ...
  .input(
    z.object({
      query: z
        .string()
        .describe(
          'Search query (max 500 characters). Supports operators: "", -, site:, filetype:, intitle:, inurl:'
        ),
      limit: z
        .number()
        .optional()
      ...
      scrapeContent: z
        ...
        .optional()
        .describe(
          'If true, scrapes full content from each result page and returns it as markdown'
        ),
      categories: z
      ...
      timeFilter: z
        ...
        .optional()
        .describe(
          'Time-based filter, e.g., qdr:d (last day), qdr:w (last week), qdr:m (last month), qdr:y (last year)'
        )
    })
  )
  .output(
    z.object({
      results: z
        ...
        .describe('Search results with optional scraped content'),
  ...
  .handleInvocation(async ctx => {
    ...
    let scrapeOptions = ctx.input.scrapeContent
