...
export let listBlobs = SlateTool.create(
  spec,
  {
    name: 'List Blobs',
    key: 'list_blobs',
  ...
  .input(z.object({
    ...
    prefix: z.string().optional().describe('Filter blobs whose names begin with this prefix (e.g. "images/")'),
    delimiter: z.string().optional().describe('Character to group blob names hierarchically (usually "/")'),
    maxResults: z.number().optional().describe('Maximum number of blobs to return'),
    marker: z.string().optional().describe('Continuation token for pagination from a previous request'),
  ...
  .output(z.object({
    ...
    blobPrefixes: z.array(z.string()).describe('Virtual directory prefixes when using delimiter'),
