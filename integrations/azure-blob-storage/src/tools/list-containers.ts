...
export let listContainers = SlateTool.create(
  ...
  .input(z.object({
    prefix: z.string().optional().describe('Filter containers whose names begin with this prefix'),
    maxResults: z.number().optional().describe('Maximum number of containers to return (default: all)'),
  }))
  .output(z.object({
    containers: z.array(z.object({
      ...
      hasImmutabilityPolicy: z.boolean().describe('Whether the container has an immutability policy'),
      hasLegalHold: z.boolean().describe('Whether the container has a legal hold'),
      publicAccess: z.string().describe('Public access level (private, blob, container)'),
