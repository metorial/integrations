...
export let getPageInsights = SlateTool.create(
  spec,
  {
...
Provide specific metric names or use the defaults. Supports filtering by date range and aggregation period.`,
  ...
  .input(z.object({
    ...
    metrics: z.array(z.string()).optional().describe('List of metric names to retrieve. Defaults to common Page metrics.'),
    period: z.enum(['day', 'week', 'days_28', 'lifetime']).optional().describe('Aggregation period for the metrics'),
    since: z.string().optional().describe('Start date for the date range (ISO date, e.g. 2024-01-01)'),
    until: z.string().optional().describe('End date for the date range (ISO date, e.g. 2024-01-31)'),
  }))
  .output(z.object({
