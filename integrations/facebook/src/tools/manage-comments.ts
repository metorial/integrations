...
Use \`action\` to specify the operation: **list** to retrieve comments, **create** to post a new comment, **update** to edit an existing comment, **delete** to remove a comment, or **hide** to toggle comment visibility.`,
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'update', 'delete', 'hide']).describe('Action to perform'),
    objectId: z.string().describe('Post or object ID to list/create comments on, or comment ID for update/delete/hide'),
    pageId: z.string().optional().describe('Page ID if acting on a Page post (used to obtain Page access token)'),
    message: z.string().optional().describe('Comment text (for create and update actions)'),
    isHidden: z.boolean().optional().describe('Whether to hide (true) or unhide (false) the comment (for hide action)'),
    limit: z.number().optional().describe('Max comments to return (for list action, default: 25)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of comments (for list action)'),
    commentId: z.string().optional().describe('Created or managed comment ID'),
    success: z.boolean().optional().describe('Whether the operation succeeded (for update/delete/hide)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      let result = await client.postComment(ctx.input.objectId, ctx.input.message || '', pageAccessToken);
      return {
    ...
    return {
      output: { success: true, commentId: ctx.input.objectId },
      message: ctx.input.isHidden === false
