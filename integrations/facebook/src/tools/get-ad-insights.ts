...
export let getAdInsights = SlateTool.create(
  spec,
  {
...
Use \`action\` to select the operation.`,
  ...
  .input(z.object({
    action: z.enum(['list_accounts', 'list_campaigns', 'campaign_insights']).describe('Operation to perform'),
    adAccountId: z.string().optional().describe('Ad account ID (required for list_campaigns, e.g. act_123456)'),
    campaignId: z.string().optional().describe('Campaign ID (required for campaign_insights)'),
    datePreset: z.string().optional().describe('Predefined date range (e.g. last_30d, this_month)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of ad accounts'),
    campaigns: z.array(z.object({
      ...
      lifetimeBudget: z.string().optional().describe('Lifetime budget in currency subunits'),
      startTime: z.string().optional().describe('Campaign start time'),
      stopTime: z.string().optional().describe('Campaign stop time'),
    })).optional().describe('List of campaigns'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'list_campaigns') {
      ...
      let result = await client.getCampaigns(ctx.input.adAccountId, { limit: ctx.input.limit });
      return {
        output: {
