...
export let getHistoricalAnalytics = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    from: z.string().optional().describe('Start date/time in ISO 8601 format or relative format (e.g. "2024-01-01", "-30d")'),
    to: z.string().optional().describe('End date/time in ISO 8601 format or relative format (e.g. "2024-01-31", "now")'),
    interval: z.string().optional().describe('Aggregation interval for time series (e.g. "hour", "day", "week", "month"). Only for aggregate dimension.'),
    limit: z.string().optional().describe('Maximum number of results to return'),
  }))
  .output(z.object({
    trends: z.any().describe('Historical analytics data for the requested dimension and time range'),
  }))
