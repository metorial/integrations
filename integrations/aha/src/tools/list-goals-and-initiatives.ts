...
export let listGoalsAndInitiatives = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    resourceType: z.enum(['goals', 'initiatives', 'both']).default('both').describe('Which strategic records to list'),
    productId: z.string().optional().describe('Filter by product ID or reference prefix'),
    updatedSince: z.string().optional().describe('Only return records updated since this date (ISO 8601)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('Goals (when resourceType is goals or both)'),
    initiatives: z.array(z.object({
      ...
      progress: z.number().optional().describe('Progress percentage'),
      url: z.string().optional().describe('Aha! URL'),
    })).optional().describe('Initiatives (when resourceType is initiatives or both)'),
  }))
