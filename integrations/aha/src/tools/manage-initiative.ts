...
export let manageInitiative = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    initiativeId: z.string().optional().describe('Initiative ID or reference number (required for update/delete)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      if (!ctx.input.productId) throw new Error('productId is required to create an initiative');
