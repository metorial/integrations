...
export let getNetworkInfoTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list_peering', 'get_peering', 'list_private_endpoints']).describe('Action to perform'),
    ...
    cloudProvider: z.enum(['AWS', 'AZURE', 'GCP']).optional().describe('Cloud provider filter'),
  }))
  .output(z.object({
    peeringConnections: z.array(peeringConnectionSchema).optional().describe('List of peering connections'),
    peeringConnection: peeringConnectionSchema.optional().describe('Single peering connection'),
    privateEndpoints: z.array(z.record(z.string(), z.any())).optional().describe('List of private endpoint services'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'list_peering') {
      ...
      return {
        output: { peeringConnections: connections, totalCount: result.totalCount ?? connections.length },
