...
export let getMetricsTool = SlateTool.create(
  spec,
  {
    name: 'Get Metrics',
    key: 'get_metrics',
  ...
  .input(z.object({
    action: z.enum(['list_processes', 'get_measurements', 'get_disk_measurements']).describe('Action to perform'),
    projectId: z.string().optional().describe('Project ID. Falls back to configured projectId.'),
    processId: z.string().optional().describe('Process ID in "hostname:port" format (for get_measurements, get_disk_measurements)'),
    granularity: z.string().optional().describe('Measurement granularity (PT1M, PT5M, PT1H, P1D)'),
    period: z.string().optional().describe('Time period (e.g., PT1H, P1D, P7D). Mutually exclusive with start/end.'),
    ...
    partitionName: z.string().optional().describe('Disk partition name (for get_disk_measurements, usually "data")'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'get_disk_measurements') {
      ...
      let result = await client.getProcessDiskMeasurements(projectId, ctx.input.processId, partition, params);
      let measurements = (result.measurements || []).map((m: any) => ({
        name: m.name,
        units: m.units,
        dataPoints: (m.dataPoints || []).map((dp: any) => ({
