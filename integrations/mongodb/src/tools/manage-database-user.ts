...
let roleSchema = z.object({
  databaseName: z.string().describe('Database the role applies to. Use "admin" for built-in roles.'),
  roleName: z.string().describe('Name of the role (e.g., readWrite, dbAdmin, atlasAdmin, read)'),
  collectionName: z.string().optional().describe('Collection the role applies to. Omit for database-level roles.'),
});
...
let dbUserOutputSchema = z.object({
  ...
  scopes: z.array(scopeSchema).optional().describe('Clusters/data lakes the user has access to'),
  deleteAfterDate: z.string().optional().describe('ISO 8601 timestamp when the user will be automatically deleted'),
...
export let manageDatabaseUserTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    password: z.string().optional().describe('Password for SCRAM authentication (required for create with SCRAM)'),
    databaseName: z.string().optional().describe('Authentication database (usually "admin" for SCRAM). Required for create.'),
    roles: z.array(roleSchema).optional().describe('Database roles to assign'),
    scopes: z.array(scopeSchema).optional().describe('Restrict access to specific clusters or data lakes'),
    deleteAfterDate: z.string().optional().describe('ISO 8601 timestamp to auto-delete the user'),
    awsIAMType: z.enum(['NONE', 'USER', 'ROLE']).optional().describe('AWS IAM auth type'),
    x509Type: z.enum(['NONE', 'MANAGED', 'CUSTOMER']).optional().describe('X.509 certificate auth type'),
    ldapAuthType: z.enum(['NONE', 'USER', 'GROUP']).optional().describe('LDAP auth type'),
  }))
  .output(z.object({
    users: z.array(dbUserOutputSchema).optional().describe('List of database users (for list action)'),
    user: dbUserOutputSchema.optional().describe('Single database user (for get, create, update)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      if (!ctx.input.username) throw new Error('username is required');
      if (!ctx.input.roles || ctx.input.roles.length === 0) throw new Error('At least one role is required');
      let authDb = ctx.input.databaseName || 'admin';
    ...
    if (ctx.input.action === 'update') {
      ...
      let user = await client.updateDatabaseUser(projectId, authDb, ctx.input.username, payload);
      return {
        output: {
          user: {
            username: user.username,
