...
export let createEmbeddingsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    model: z.string().default('mistral-embed').describe('Embedding model ID (e.g., "mistral-embed", "codestral-embed")'),
    input: z.union([z.string(), z.array(z.string())]).describe('Text or array of texts to embed'),
    encodingFormat: z.enum(['float', 'base64']).optional().describe('Output encoding format'),
    outputDimension: z.number().optional().describe('Custom embedding dimensionality (if supported by model)')
  }))
  .output(z.object({
