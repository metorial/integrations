...
let jobSchema = z.object({
  ...
  status: z.string().describe('Job status (QUEUED, STARTED, VALIDATING, VALIDATED, RUNNING, FAILED_VALIDATION, FAILED, SUCCESS, CANCELLED, CANCELLATION_REQUESTED)'),
  model: z.string().describe('Base model being fine-tuned'),
  fineTunedModel: z.string().nullable().optional().describe('Resulting fine-tuned model ID (available after success)'),
...
export let createFineTuningJobTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    model: z.string().describe('Base model to fine-tune (e.g., "open-mistral-nemo", "mistral-small-latest")'),
    trainingFiles: z.array(z.object({
    ...
    hyperparameters: z.object({
      ...
      fimRatio: z.number().optional().describe('Fill-in-the-middle ratio (for code models)'),
    ...
    dryRun: z.boolean().optional().describe('Validate configuration without starting training'),
...
export let getFineTuningJobTool = SlateTool.create(
  spec,
  {
...
export let listFineTuningJobsTool = SlateTool.create(
  spec,
  {
...
export let cancelFineTuningJobTool = SlateTool.create(
  spec,
  {
