...
let batchJobSchema = z.object({
  ...
  status: z.string().describe('Job status (QUEUED, RUNNING, SUCCESS, FAILED, TIMEOUT_EXCEEDED, CANCELLATION_REQUESTED, CANCELLED)'),
...
export let createBatchJobTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    endpoint: z.string().describe('Target API endpoint path (e.g., "/v1/chat/completions", "/v1/embeddings")'),
    ...
    timeoutHours: z.number().optional().describe('Job timeout in hours (default 24, max 168)'),
...
export let getBatchJobTool = SlateTool.create(
  spec,
  {
    name: 'Get Batch Job',
    key: 'get_batch_job',
...
export let listBatchJobsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    status: z.enum(['QUEUED', 'RUNNING', 'SUCCESS', 'FAILED', 'TIMEOUT_EXCEEDED', 'CANCELLATION_REQUESTED', 'CANCELLED']).optional().describe('Filter by job status'),
...
export let cancelBatchJobTool = SlateTool.create(
  spec,
  {
