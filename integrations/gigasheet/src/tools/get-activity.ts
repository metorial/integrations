...
export let getActivity = SlateTool.create(
  ...
  .output(z.object({
    activities: z.array(z.record(z.string(), z.unknown())).describe('Activity history entries'),
    totalCount: z.record(z.string(), z.unknown()).optional().describe('Total count of activities'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.searchQuery) {
      activities = await client.searchActivityHistory(ctx.input.sheetHandle, ctx.input.searchQuery);
    } else {
