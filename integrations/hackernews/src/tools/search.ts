...
Supports filtering by content type, author, date range, and minimum points. Results can be sorted by relevance or date.`,
  ...
  .input(z.object({
    query: z.string().optional().describe('Search query text. Leave empty to browse by filters only.'),
    sortBy: z.enum(['relevance', 'date']).optional().default('relevance').describe('Sort order: relevance (default) or date (newest first)'),
    contentType: z.enum(['story', 'comment', 'ask_hn', 'show_hn', 'job']).optional().describe('Filter by content type'),
    ...
    hitsPerPage: z.number().optional().default(20).describe('Number of results per page (default: 20, max: 1000)'),
  ...
  .output(z.object({
    ...
    hits: z.array(z.object({
      ...
      storyText: z.string().optional().describe('Text content of the story (for text posts)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let response = ctx.input.sortBy === 'date'
