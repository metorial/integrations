...
export let manageContact = SlateTool.create(
  ...
  .input(z.object({
    contactUuid: z.string().optional().describe('UUID of an existing contact to update. Omit to create a new contact.'),
    companyUuid: z.string().optional().describe('UUID of the company to link the contact to (required for creation)'),
