...
export let updateTicket = SlateTool.create(
  ...
  .input(z.object({
    ...
    priority: z.string().optional().describe('Updated priority (e.g., Low, Medium, High, Urgent)'),
    status: z.string().optional().describe('Updated status (e.g., Open, On Hold, Escalated, Closed)'),
    ...
    moveToDepartmentId: z.string().optional().describe('Department ID to move the ticket to (triggers a move operation)'),
    mergeWithTicketIds: z.array(z.string()).optional().describe('List of ticket IDs to merge into this ticket'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { ticketId, moveToDepartmentId, mergeWithTicketIds, customFields, ...updateFields } = ctx.input;
