...
export let listTickets = SlateTool.create(
  ...
  .input(z.object({
    ...
    sortBy: z.string().optional().describe('Field to sort by (e.g., modifiedTime, createdTime, dueDate)'),
    sortOrder: z.enum(['asc', 'desc']).optional().describe('Sort order'),
    include: z.string().optional().describe('Comma-separated related data to include (e.g., "contacts,assignee,departments")'),
    from: z.number().optional().describe('Starting index for pagination (0-based)'),
    limit: z.number().optional().describe('Number of tickets to return (max 100, default 10)'),
  }))
  .output(z.object({
