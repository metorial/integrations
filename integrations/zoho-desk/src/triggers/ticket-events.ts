...
export let ticketEvents = SlateTrigger.create(
  spec,
  {
    name: 'Ticket Events',
    key: 'ticket_events',
    description: 'Triggered when a ticket is created, updated, deleted, or when comments, threads, approvals, or attachments are added or changed.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    previousState: z.any().optional().describe('Previous state of the ticket (for update events)'),
    relatedResourceId: z.string().optional().describe('ID of the related resource (comment, thread, attachment, approval)'),
  }))
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: {
          ...
          relatedResourceId: ticket.commentId || ticket.threadId || ticket.attachmentId || ticket.approvalId,
        },
      };
    },
  })
