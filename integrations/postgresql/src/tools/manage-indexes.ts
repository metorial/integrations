...
export let manageIndexes = SlateTool.create(
  spec,
  {
    name: 'Manage Indexes',
    key: 'manage_indexes',
...
Can create unique indexes, partial indexes with WHERE conditions, and multi-column indexes.`,
  ...
  .input(z.object({
    ...
    // Create options
    columns: z.array(z.string()).optional().describe('Column names to include in the index (required for create)'),
    unique: z.boolean().optional().default(false).describe('Whether to create a UNIQUE index'),
    method: z.enum(['btree', 'hash', 'gin', 'gist', 'spgist', 'brin']).optional().default('btree').describe('Index access method'),
    where: z.string().optional().describe('WHERE condition for a partial index'),
    concurrently: z.boolean().optional().default(false).describe('Create index concurrently (non-blocking)'),
