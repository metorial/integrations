...
export let approvalEvents = SlateTrigger.create(
  spec,
  {
    ...
    key: 'approval_events',
    description: 'Triggers on approval workflow changes including status changes, decisions made, and decision resets on both tasks and folders/projects.'
  }
)
  .input(z.object({
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      let type = eventTypeMap[ctx.input.eventType] || `approval.${ctx.input.eventType.toLowerCase()}`;
