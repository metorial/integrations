...
export let manageSubaccount = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    operation: z.enum(['create', 'get', 'update', 'delete', 'list']).describe('Operation to perform'),
    subaccountId: z.string().optional().describe('Subaccount ID (required for get, update, delete)'),
    subaccountName: z.string().optional().describe('Unique name for the subaccount (used in create/update)'),
    userEmail: z.string().optional().describe('Email to invite (generates sign-in link, used in create)'),
    maxCalls: z.number().optional().describe('Maximum concurrent calls (used in create/update)'),
  }))
  .output(z.object({
    subaccount: z.record(z.string(), z.any()).optional().describe('Subaccount details'),
    subaccounts: z.array(z.record(z.string(), z.any())).optional().describe('List of subaccounts'),
