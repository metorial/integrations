...
export let manageContact = SlateTool.create(
  ...
  .input(z.object({
    ...
    phoneNumber: z.string().optional().describe('Contact phone number in E.164 format (used in create/update)'),
    email: z.string().optional().describe('Contact email (used in create/update)'),
    metadata: z.record(z.string(), z.any()).optional().describe('Additional metadata for the contact (used in create/update)'),
  ...
  .output(z.object({
    ...
    contacts: z.array(z.record(z.string(), z.any())).optional().describe('List of contacts (for list operation)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (operation === 'list') {
      let result = await client.listContacts({ limit: ctx.input.limit, offset: ctx.input.offset });
