...
export let manageTimerTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['start', 'pause', 'resume', 'get', 'add_time']).describe('Timer action to perform'),
    durationInSeconds: z.number().optional().describe('Timer duration in seconds (required for "start")'),
    extraTimeInSeconds: z.number().optional().describe('Additional seconds to add (for "add_time")'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (action) {
      case 'start':
        if (!durationInSeconds) throw new Error('durationInSeconds is required for starting a timer');
      ...
      case 'get':
        ...
        break;
      case 'add_time':
        if (!extraTimeInSeconds) throw new Error('extraTimeInSeconds is required for adding time');
...
export let managePrivateModeTool = SlateTool.create(
  spec,
  {
...
export let getChatHistoryTool = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    ...
    let mapped = (messages || []).map((m: any) => ({
      ...
      authorName: m.authorName || m.author?.name || (m.author ? `${m.author.firstName || ''} ${m.author.lastName || ''}`.trim() : undefined),
