...
export let getStrommix = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    period: z.string().optional().describe('Predefined time period, e.g. "last_month", "last_7_days"'),
    from: z.string().optional().describe('Custom range start date (ISO format, e.g. "2024-01-01")'),
    to: z.string().optional().describe('Custom range end date (ISO format, e.g. "2024-01-31")')
  }))
  .output(z.object({
    mix: z.record(z.string(), z.any()).optional().describe('Energy generation mix by source type with production volumes'),
