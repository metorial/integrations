...
let vectorParamsSchema = z.object({
  size: z.number().describe('Dimensionality of the vector'),
  distance: z.enum(['Cosine', 'Euclid', 'Dot', 'Manhattan']).describe('Distance metric for similarity'),
  datatype: z.enum(['float32', 'float16', 'uint8']).optional().describe('Vector storage data type'),
...
export let createCollection = SlateTool.create(
  ...
  .input(z.object({
    ...
    namedVectors: z.record(z.string(), vectorParamsSchema).optional().describe('Named vector spaces, e.g. {"text": {size: 768, distance: "Cosine"}, "image": {size: 512, distance: "Dot"}}'),
    shardNumber: z.number().optional().describe('Number of shards for the collection'),
    replicationFactor: z.number().optional().describe('Replication factor for distributed deployments'),
    onDiskPayload: z.boolean().optional().describe('Whether to store payloads on disk'),
    sparseVectors: z.record(z.string(), z.any()).optional().describe('Sparse vector configuration by name'),
  }))
  .output(z.object({
