...
export let managePayloadIndex = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    fieldSchema: z.any().optional().describe('Field type ("keyword", "integer", "float", "geo", "text", "bool", "datetime", "uuid") or a detailed config object. Required for create.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      result = await client.createPayloadIndex(ctx.input.collectionName, ctx.input.fieldName, ctx.input.fieldSchema, wait);
    } else {
      result = await client.deletePayloadIndex(ctx.input.collectionName, ctx.input.fieldName, wait);
    }
