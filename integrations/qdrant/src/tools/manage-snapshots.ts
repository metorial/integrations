...
export let manageSnapshots = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['create', 'list', 'delete', 'recover']).describe('Snapshot operation to perform'),
    collectionName: z.string().optional().describe('Collection name (omit for full storage snapshots)'),
    snapshotName: z.string().optional().describe('Snapshot name (required for delete)'),
    snapshotLocation: z.string().optional().describe('URL or file path of the snapshot to recover from (required for recover)'),
    recoveryPriority: z.enum(['no_sync', 'snapshot', 'replica']).optional().describe('Conflict resolution priority during recovery'),
  ...
  .output(z.object({
    ...
    }).optional().describe('Created snapshot info (for create action)'),
    snapshots: z.array(z.object({
      snapshotName: z.string().describe('Snapshot name'),
      creationTime: z.string().optional().describe('When the snapshot was created'),
      size: z.number().optional().describe('Snapshot size in bytes'),
    })).optional().describe('List of snapshots (for list action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'delete') {
      if (!ctx.input.snapshotName) throw new Error('snapshotName is required for delete action');
      if (ctx.input.collectionName) {
    ...
    if (ctx.input.action === 'recover') {
      if (!ctx.input.collectionName) throw new Error('collectionName is required for recover action');
      if (!ctx.input.snapshotLocation) throw new Error('snapshotLocation is required for recover action');
      await client.recoverSnapshot(ctx.input.collectionName, {
