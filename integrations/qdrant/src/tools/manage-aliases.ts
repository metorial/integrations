...
export let manageAliases = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    collectionName: z.string().optional().describe('Collection name (required for create, optional for list)'),
    aliasName: z.string().optional().describe('Alias name (required for create, delete, rename)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      await client.updateAliases([
        { create_alias: { collection_name: ctx.input.collectionName, alias_name: ctx.input.aliasName } },
      ]);
      return {
        output: { success: true },
    ...
    if (ctx.input.action === 'delete') {
      ...
      await client.updateAliases([
        { delete_alias: { alias_name: ctx.input.aliasName } },
      ]);
      return {
        output: { success: true },
    ...
    if (ctx.input.action === 'rename') {
      ...
      await client.updateAliases([
        { rename_alias: { old_alias_name: ctx.input.aliasName, new_alias_name: ctx.input.newAliasName } },
      ]);
      return {
        output: { success: true },
