...
export let testTriggerTool = SlateTool.create(
  spec,
  {
    name: 'Test Trigger',
    key: 'test_trigger',
    ...
    instructions: [
      'The user must have the connection enabled for the test to work.',
    ],
  }
)
  .input(z.object({
    connectionId: z.string().describe('The ID of the connection containing the trigger'),
    triggerId: z.string().describe('The trigger identifier (e.g., "google_calendar.any_event_starts")'),
    userId: z.string().describe('The user ID to simulate the trigger event for'),
    userFeatureId: z.string().optional().describe('Optional user feature ID to scope the trigger test'),
  }))
  .output(z.object({
