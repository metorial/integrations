...
export let performQueryTool = SlateTool.create(
  spec,
  {
    name: 'Perform Query',
    key: 'perform_query',
  ...
  .input(z.object({
    ...
    queryId: z.string().describe('The query identifier (e.g., "weather.current_conditions")'),
    userId: z.string().describe('The user ID to run the query for'),
    fields: z.record(z.string(), z.any()).optional().describe('Query field values specific to the query being run'),
