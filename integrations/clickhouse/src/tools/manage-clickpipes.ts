...
export let listClickPipes = SlateTool.create(
  spec,
  {
...
export let getClickPipe = SlateTool.create(
  spec,
  {
    name: 'Get ClickPipe',
    key: 'get_clickpipe',
...
export let createClickPipe = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    source: z.record(z.string(), z.any()).describe('Source configuration (e.g., kafka, s3, kinesis, postgres, mysql)'),
    destination: z.record(z.string(), z.any()).describe('Destination configuration including database, table, and columns'),
  }))
  .output(z.object({
    clickpipeId: z.string(),
    name: z.string().optional(),
...
export let deleteClickPipe = SlateTool.create(
  spec,
  {
...
export let controlClickPipeState = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    serviceId: z.string().describe('ID of the service'),
    clickpipeId: z.string().describe('ID of the ClickPipe'),
    state: z.enum(['running', 'paused', 'stopped']).describe('Desired state for the ClickPipe'),
  }))
  .output(z.object({
    clickpipeId: z.string(),
    state: z.string().optional(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.updateClickPipeState(ctx.input.serviceId, ctx.input.clickpipeId, ctx.input.state);
