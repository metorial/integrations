...
export let getQueryEndpoint = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    endpointConfig: z.record(z.string(), z.any()).describe('Query endpoint configuration details'),
  }))
...
export let upsertQueryEndpoint = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    serviceId: z.string().describe('ID of the service'),
    endpointSettings: z.record(z.string(), z.any()).describe('Query endpoint configuration to create or update'),
  }))
  .output(z.object({
    endpointConfig: z.record(z.string(), z.any()).describe('Created or updated endpoint configuration'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new ClickHouseClient({
      token: ctx.auth.token,
    ...
    let result = await client.upsertQueryEndpoint(ctx.input.serviceId, ctx.input.endpointSettings);
...
export let deleteQueryEndpoint = SlateTool.create(
  spec,
  {
