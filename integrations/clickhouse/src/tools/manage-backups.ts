...
export let getBackupConfiguration = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    configuration: z.record(z.string(), z.any()).describe('Current backup configuration settings'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new ClickHouseClient({
      token: ctx.auth.token,
...
export let updateBackupConfiguration = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    serviceId: z.string().describe('ID of the service'),
    settings: z.record(z.string(), z.any()).describe('Backup configuration settings to update'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.updateBackupConfiguration(ctx.input.serviceId, ctx.input.settings);
