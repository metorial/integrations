...
export let estimatePosition = SlateTool.create(
  spec,
  {
...
Useful for indoor positioning, device tracking without GPS, and approximate location from network data.`,
  ...
  .input(z.object({
    wlan: z.array(wlanMeasurementSchema).optional().describe('Wi-Fi access point measurements'),
    ...
    fallback: z.enum(['area', 'any', 'singleWifi']).optional().describe('Fallback mode when precise positioning is unavailable'),
  }))
  .output(z.object({
