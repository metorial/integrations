...
let taskPlaceSchema = z.object({
  ...
  times: z.array(z.array(z.string())).optional().describe('Time windows as [["start_iso","end_iso"]] pairs'),
});
...
let vehicleTypeSchema = z.object({
  ...
  }).optional().describe('Cost factors'),
  shifts: z.array(z.object({
    start: z.object({
      ...
      location: locationSchema,
    }).optional().describe('Shift end (omit for open-ended routes)'),
  ...
  }).optional().describe('Vehicle limits'),
});
...
export let planTour = SlateTool.create(
  spec,
  {
    name: 'Plan Tour',
    key: 'plan_tour',
...
Returns optimized stop sequences for each vehicle, including arrival/departure times and route distances.`,
    ...
    constraints: [
      'Complex problems with many jobs and vehicles may take longer to solve.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    maxSolverTime: z.number().optional().describe('Maximum solver time in seconds (default 2)'),
    stagnationTime: z.number().optional().describe('Stop if no improvement after this many seconds'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Optimized tours per vehicle'),
    unassigned: z.array(z.object({
      jobId: z.string().optional(),
      reasons: z.array(z.any()).optional(),
    })).optional().describe('Jobs that could not be assigned to any vehicle'),
    statistics: z.object({
      cost: z.number().optional(),
      distance: z.number().optional(),
      duration: z.number().optional(),
      times: z.any().optional(),
    }).optional().describe('Overall solution statistics'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let totalStops = tours?.reduce((sum: number, t: any) => sum + (t.stops?.length || 0), 0) || 0;
