...
export let getTraffic = SlateTool.create(
  spec,
  {
    name: 'Get Traffic',
    key: 'get_traffic',
...
Combine both for a complete traffic picture of an area.`,
  ...
  .input(z.object({
    inArea: z.string().describe('Geographic filter: "bbox:west,south,east,north", "circle:lat,lng;r=radius", or "corridor:POLYLINE;r=radius"'),
    includeFlow: z.boolean().optional().describe('Include traffic flow data (default true)'),
    includeIncidents: z.boolean().optional().describe('Include traffic incidents (default true)'),
    locationReferencing: z.enum(['shape', 'tmc', 'olr', 'none']).optional().describe('Location referencing method (default "shape")'),
    minJamFactor: z.number().optional().describe('Minimum jam factor filter (0-10, flow only)'),
    maxJamFactor: z.number().optional().describe('Maximum jam factor filter (0-10, flow only)'),
  ...
  .output(z.object({
    flow: z.object({
      results: z.array(z.object({
        ...
        currentFlow: z.object({
          ...
          jamFactor: z.number().optional().describe('Jam factor (0=free flow, 10=closed)'),
          confidence: z.number().optional().describe('Data confidence'),
          traversability: z.string().optional().describe('Road traversability status'),
        }).optional(),
      })).optional(),
    }).optional().describe('Traffic flow data'),
    incidents: z.object({
      results: z.array(z.object({
    ...
    }).optional().describe('Traffic incident data'),
  }))
