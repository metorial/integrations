...
Supports truck-specific parameters (weight, height, hazardous goods), toll avoidance, and traffic-aware routing.`,
  ...
  .input(z.object({
    ...
    transportMode: z.enum(['car', 'truck', 'pedestrian', 'bicycle', 'scooter', 'taxi', 'bus']).describe('Transport mode'),
    via: z.array(z.string()).optional().describe('Intermediate waypoints as "lat,lng" strings'),
    returnFields: z.array(z.string()).optional().describe('Response fields to include: "summary", "polyline", "actions", "instructions", "tolls", "elevation", "turnByTurnActions", "routeHandle"'),
    departureTime: z.string().optional().describe('Departure time in ISO 8601 format, or "any" for no traffic consideration'),
    arrivalTime: z.string().optional().describe('Desired arrival time in ISO 8601 format'),
    avoidFeatures: z.array(z.enum(['tollRoad', 'motorway', 'ferry', 'tunnel', 'dirtRoad'])).optional().describe('Road features to avoid'),
    excludeCountries: z.array(z.string()).optional().describe('Countries to exclude (ISO 3166-1 alpha-3 codes like "DEU")'),
    alternatives: z.number().optional().describe('Number of alternative routes to return (0-6)'),
    ...
    truckShippedHazardousGoods: z.array(z.string()).optional().describe('Hazardous goods types (e.g. "explosive", "gas", "flammable")'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let totalDuration = firstRoute?.sections?.reduce((sum: number, s: any) => sum + (s.summary?.duration || 0), 0) || 0;
    let totalLength = firstRoute?.sections?.reduce((sum: number, s: any) => sum + (s.summary?.length || 0), 0) || 0;
