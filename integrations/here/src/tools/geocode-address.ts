...
let geocodeResultSchema = z.object({
  ...
  resultType: z.string().optional().describe('Result type (e.g. houseNumber, street, locality)'),
  ...
  }).optional().describe('Relevance scoring'),
});
...
Use a free-form query string like "240 Washington St, Boston" or structured fields like city, street, postalCode separately.`,
  ...
  .input(z.object({
    query: z.string().optional().describe('Free-form address query (e.g. "240 Washington St, Boston, MA")'),
    qualifiedQuery: z.string().optional().describe('Structured query with semicolon-separated fields like "city=Berlin;street=Friedrichstr;houseNumber=20"'),
    at: z.string().optional().describe('Center point for relevance ranking as "lat,lng" (e.g. "52.5308,13.3847")'),
    inArea: z.string().optional().describe('Geographic filter (e.g. "countryCode:USA", "bbox:west,south,east,north", "circle:lat,lng;r=radius")'),
    limit: z.number().optional().describe('Maximum number of results to return (default 20)'),
    lang: z.string().optional().describe('BCP 47 language code for response (e.g. "en-US", "de-DE")'),
  }))
  .output(z.object({
