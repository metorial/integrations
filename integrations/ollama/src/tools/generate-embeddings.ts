...
export let generateEmbeddings = SlateTool.create(
  ...
  .input(z.object({
    model: z.string().describe('Name of the embedding model to use (e.g., "nomic-embed-text").'),
    input: z.union([
      z.string(),
      z.array(z.string()),
    ]).describe('Text or array of texts to generate embeddings for.'),
    truncate: z.boolean().optional().describe('Automatically truncate inputs exceeding context limits. Defaults to true.'),
  ...
  .output(z.object({
    ...
    embeddings: z.array(z.array(z.number())).describe('Array of embedding vectors (one per input text).'),
