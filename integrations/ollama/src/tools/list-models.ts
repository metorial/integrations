...
export let listModels = SlateTool.create(
  ...
  .input(z.object({
    includeRunning: z.boolean().optional().default(false).describe('Also include currently running/loaded models with runtime details like VRAM usage and context length.'),
  }))
  .output(z.object({
    ...
    runningModels: z.array(z.object({
      name: z.string().describe('Full model name including tag.'),
      model: z.string().describe('Model identifier.'),
      modifiedAt: z.string().describe('ISO 8601 timestamp of last modification.'),
      size: z.number().describe('Model size in bytes.'),
      digest: z.string().describe('SHA256 digest of the model.'),
      details: modelDetailsSchema,
      expiresAt: z.string().optional().describe('When the model will be unloaded from memory.'),
    ...
    })).optional().describe('Currently running/loaded models. Only included when includeRunning is true.'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let runningModels = ctx.input.includeRunning ? await client.listRunningModels() : undefined;
