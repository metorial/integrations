...
export let sendTransactionalEmail = SlateTool.create(
  ...
  .input(z.object({
    mailName: z.string().optional().describe('Name of a pre-built transactional email template'),
    subject: z.string().optional().describe('Email subject line'),
    html: z.string().optional().describe('Custom HTML content for the email body'),
    from: z.string().optional().describe('Sender email address (supports "Name <email>" format)'),
    ...
    bcc: z.string().optional().describe('BCC email address (supports "Name <email>" format)'),
    replacements: z.record(z.string(), z.string()).optional().describe('Placeholder replacements as key-value pairs'),
    store: z.boolean().optional().describe('Whether to store the email in the transactional log (defaults to true)'),
    fake: z.boolean().optional().describe('Simulate sending without actual delivery (for testing)'),
