...
export let listPapersignFolders = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    includeSpaces: z.boolean().optional().describe('Also include Papersign spaces in the response (default false)'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Papersign spaces (only included when includeSpaces is true)'),
  }))
...
export let createPapersignFolder = SlateTool.create(
  spec,
  {
