...
Notes support HTML content and can be pinned to the top of the entity's detail view.`,
  ...
  .input(z.object({
    ...
    content: z.string().optional().describe('Note content (HTML supported, required for create)'),
    ...
    pinnedToOrganizationFlag: z.boolean().optional().describe('Pin note to the organization'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.pinnedToDealFlag !== undefined) body.pinned_to_deal_flag = ctx.input.pinnedToDealFlag ? 1 : 0;
    if (ctx.input.pinnedToPersonFlag !== undefined) body.pinned_to_person_flag = ctx.input.pinnedToPersonFlag ? 1 : 0;
    if (ctx.input.pinnedToOrganizationFlag !== undefined) body.pinned_to_organization_flag = ctx.input.pinnedToOrganizationFlag ? 1 : 0;
    if (ctx.input.pinnedToLeadFlag !== undefined) body.pinned_to_lead_flag = ctx.input.pinnedToLeadFlag ? 1 : 0;
