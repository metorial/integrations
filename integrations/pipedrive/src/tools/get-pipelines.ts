...
export let getPipelines = SlateTool.create(
  spec,
  {
...
Use this to understand the sales pipeline structure and stage configuration.`,
  ...
  .input(z.object({
    pipelineId: z.number().optional().describe('Specific pipeline ID to fetch (with its stages)'),
  ...
  .output(z.object({
    pipelines: z.array(z.object({
      ...
      dealProbability: z.boolean().optional().describe('Whether deal probability is enabled'),
      ...
      stages: z.array(z.object({
        ...
        rottingDays: z.number().optional().nullable().describe('Days before rotting'),
        dealsCount: z.number().optional().describe('Number of deals in stage'),
      })).optional().describe('Stages in this pipeline'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.pipelineId) {
      ...
      return {
        ...
        message: pipeline ? `Found pipeline **"${pipeline.name}"** with **${stages.length}** stage(s).` : 'Pipeline not found.',
      };
    }

    // List all pipelines with stages
    let pipelinesResult = await client.getPipelines();
    let pipelines = await Promise.all((pipelinesResult?.data || []).map(async (pipeline: any) => {
