...
export let manageGroupChannelMembers = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['invite', 'remove', 'list']).describe('"invite" to add members, "remove" to remove members, "list" to list current members'),
    userIds: z.array(z.string()).optional().describe('User IDs to invite or remove (required for invite/remove)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of members (only for list action)'),
