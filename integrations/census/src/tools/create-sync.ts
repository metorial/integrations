...
let mappingSchema = z.object({
  from: z.object({
    type: z.enum(['column', 'constant_value']).describe('Source type: "column" for a database column, "constant_value" for a fixed value.'),
    data: z.union([
      z.string(),
      z.object({
        value: z.string(),
    ...
    ]).describe('Column name (if type is "column") or an object with value and basicType (if type is "constant_value").'),
  }),
  to: z.string().describe('Destination field name.'),
  isPrimaryIdentifier: z.boolean().optional().describe('Whether this mapping is the primary identifier for matching records.'),
});

export let createSync = SlateTool.create(
  spec,
  {
    name: 'Create Sync',
    key: 'create_sync',
  ...
  .input(z.object({
    ...
    operation: z.enum(['upsert', 'update', 'insert', 'mirror', 'append']).describe('Sync behavior: upsert (update or create), update (update only), insert (create only), mirror (full sync with deletes), append (event-style).'),
    sourceConnectionId: z.number().describe('ID of the source connection (data warehouse).'),
    sourceObject: z.object({
      type: z.enum(['model', 'table']).describe('Whether the source is a model or a table.'),
      name: z.string().describe('Name of the model or table.'),
      tableCatalog: z.string().optional().describe('Database/catalog name (for table type).'),
    ...
    destinationConnectionId: z.number().describe('ID of the destination connection (SaaS tool).'),
    destinationObject: z.string().describe('Name of the destination object (e.g., "Contact", "Lead").'),
    mappings: z.array(mappingSchema).describe('Field mappings from source to destination.'),
    scheduleFrequency: z.enum(['never', 'continuous', 'quarter_hourly', 'hourly', 'daily', 'weekly', 'expression']).optional().describe('How often the sync should run.'),
    scheduleDay: z.string().optional().describe('Day of the week for weekly schedules (e.g., "monday").'),
    scheduleHour: z.number().optional().describe('Hour (0-23) for daily/weekly schedules.'),
    scheduleMinute: z.number().optional().describe('Minute (0-59) for daily/weekly schedules.'),
    cronExpression: z.string().optional().describe('Cron expression for expression-based schedules.'),
