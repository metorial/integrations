...
export let manageClips = SlateTool.create(
  spec,
  {
    name: 'Manage Clips',
    key: 'manage_clips',
  ...
  .input(z.object({
    action: z.enum(['create', 'get']).describe('Whether to create a new clip or get existing clips'),
    broadcasterId: z.string().optional().describe('Broadcaster ID (required for create, optional filter for get)'),
    ...
    hasDelay: z.boolean().optional().describe('Add a delay before capturing the clip (for create)'),
    maxResults: z.number().optional().describe('Max clips to return (for get, max 100)'),
    startedAt: z.string().optional().describe('Filter clips created after this date (RFC 3339)'),
    endedAt: z.string().optional().describe('Filter clips created before this date (RFC 3339)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      if (!ctx.input.broadcasterId) throw new Error('broadcasterId is required to create a clip');
    ...
    return {
      ...
      message: clips.length === 0
        ? 'No clips found'
        : `Found **${clips.length}** clips. Top: ${clips.slice(0, 3).map(c => `"${c.title}" (${c.viewCount} views)`).join(', ')}`,
    };
  })
  .build();
