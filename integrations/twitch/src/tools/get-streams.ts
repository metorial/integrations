...
export let getStreams = SlateTool.create(
  spec,
  {
    name: 'Get Streams',
    key: 'get_streams',
  ...
  .input(z.object({
    ...
    maxResults: z.number().optional().describe('Maximum number of results (1-100, default 20)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: mapped.length === 0
        ...
        : `Found **${mapped.length}** live streams. Top: ${mapped.slice(0, 3).map(s => `**${s.userName}** (${s.viewerCount} viewers)`).join(', ')}`,
    };
  })
  .build();
