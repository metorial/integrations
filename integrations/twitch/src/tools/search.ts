...
export let search = SlateTool.create(
  ...
  .input(z.object({
    ...
    type: z.enum(['channels', 'categories']).describe('Whether to search channels or categories/games'),
    liveOnly: z.boolean().optional().describe('Only return live channels (for channel search)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: categories.length === 0
        ...
        : `Found **${categories.length}** categories: ${categories.slice(0, 5).map(c => c.name).join(', ')}`,
    };
  })
  .build();
