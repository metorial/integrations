...
export let sendChatMessage = SlateTool.create(
  ...
  .input(z.object({
    ...
    isAnnouncement: z.boolean().optional().describe('Send as an announcement instead of a regular message'),
    announcementColor: z.enum(['blue', 'green', 'orange', 'purple']).optional().describe('Announcement color'),
  }))
  .output(z.object({
    messageId: z.string().optional().describe('ID of the sent message (not available for announcements)'),
    sent: z.boolean(),
  }))
