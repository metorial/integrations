...
- **Domain lookup** returns company details (name, logo, LinkedIn page, address).`,
  ...
  .input(z.object({
    lookupType: z.enum(['linkedin', 'email', 'domain']).describe('Type of reverse lookup to perform'),
    linkedinUrl: z.string().optional().describe('LinkedIn profile URL (required for linkedin lookup)'),
    emailAddress: z.string().optional().describe('Email address (required for email lookup)'),
  ...
  .output(z.object({
    ...
    constructedTitle: z.string().optional().describe('Constructed title (e.g., "Co-chair at Gates Foundation")'),
    ...
    addresses: z.array(addressSchema).optional().describe('Addresses associated with the lead'),
    totalExperienceInMonths: z.number().optional().describe('Total professional experience in months (person only)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.lookupType) {
      case 'linkedin': {
        if (!ctx.input.linkedinUrl) throw new Error('linkedinUrl is required for LinkedIn lookup');
        result = await client.reverseLookupLinkedIn(ctx.input.linkedinUrl);
        break;
      }
      case 'email': {
        if (!ctx.input.emailAddress) throw new Error('emailAddress is required for email lookup');
    ...
    let rawAddresses = lead.addresses as Array<Record<string, unknown>> | Record<string, unknown> | undefined;
