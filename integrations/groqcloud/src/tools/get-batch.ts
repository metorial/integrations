...
export let getBatch = SlateTool.create(
  ...
  .output(z.object({
    ...
    status: z.string().describe('Current status: validating, failed, in_progress, finalizing, completed, expired, cancelling, or cancelled'),
    ...
    errorFileId: z.string().optional().describe('ID of the error file (available if errors occurred)'),
    ...
    metadata: z.record(z.string(), z.string()).optional().describe('Custom metadata attached to the batch'),
  }))
