...
export let moderateContent = SlateTool.create(
  ...
  .input(z.object({
    ...
    model: z.string().default('openai/gpt-oss-safeguard-20b').describe('Moderation model to use'),
    policy: z.string().optional().describe('Custom safety policy for GPT-OSS-Safeguard. Define what constitutes violations vs. safe content'),
    systemPrompt: z.string().optional().describe('System prompt for the moderation model. Used when not providing a separate policy'),
  }))
  .output(z.object({
