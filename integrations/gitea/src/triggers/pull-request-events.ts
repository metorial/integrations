...
export let pullRequestEvents = SlateTrigger.create(
  spec,
  {
    name: 'Pull Request Events',
    key: 'pull_request_events',
    description: 'Triggers on pull request events including creation, updates, merges, assignment changes, label changes, comments, reviews, and sync events.',
  }
)
  .input(z.object({
    eventType: z.string().describe('Gitea event header value'),
    action: z.string().describe('Sub-action (opened, edited, closed, merged, synchronized, etc.)'),
  ...
  .output(z.object({
    action: z.string().describe('Event action (opened, edited, closed, merged, synchronized, assigned, label_updated, commented, reviewed)'),
    prNumber: z.number().describe('Pull request number'),
    prTitle: z.string().describe('Pull request title'),
    prBody: z.string().describe('Pull request body'),
    prState: z.string().describe('Pull request state'),
    ...
    labels: z.array(z.string()).describe('Current label names'),
    assignees: z.array(z.string()).describe('Current assignee usernames'),
    reviewState: z.string().optional().describe('Review state (APPROVED, CHANGES_REQUESTED, etc.)'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let prEventTypes = [
        ...
        'pull_request_review', 'pull_request_sync',
      ];
      ...
      let labels = ((pr.labels || []) as Array<Record<string, any>>).map((l: Record<string, any>) => String(l.name || ''));
      let assignees = ((pr.assignees || []) as Array<Record<string, any>>).map((a: Record<string, any>) => String(a.login || ''));
