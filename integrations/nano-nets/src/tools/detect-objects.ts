...
export let detectObjects = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'The object detection model must be trained before running predictions.'
    ],
    tags: {
      destructive: false,
      readOnly: true
  ...
  .input(z.object({
    ...
    urls: z.array(z.string()).min(1).describe('URLs of images to analyze for object detection')
  }))
  .output(z.object({
    results: z.array(z.object({
      imageUrl: z.string().describe('URL of the analyzed image'),
      detectedObjects: z.array(detectedObjectSchema).describe('Objects detected in the image')
  ...
  .handleInvocation(async (ctx) => {
    ...
    let totalObjects = results.reduce((sum: number, r: any) => sum + r.detectedObjects.length, 0);
