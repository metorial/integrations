...
export let upsertCustomer = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    customAttributes: z.record(z.string(), z.string()).optional().describe('Custom attribute key-value pairs (new attributes are created automatically)'),
  }))
  .output(z.object({
    customer: z.record(z.string(), z.unknown()).describe('The created or updated customer record'),
  }))
