...
let selectorSchema: z.ZodType<any> = z.object({
  id: z.string().describe('Unique identifier for the selector'),
  type: z.string().describe('Selector type: SelectorText, SelectorLink, SelectorImage, SelectorTable, SelectorHTML, SelectorElement, SelectorElementClick, SelectorPagination, SelectorElementAttribute, SelectorSitemapXmlLink'),
  parentSelectors: z.array(z.string()).describe('Parent selector IDs. Use ["_root"] for top-level selectors'),
  ...
  delay: z.union([z.string(), z.number()]).optional().describe('Delay before extraction in milliseconds'),
}).passthrough();

export let createSitemap = SlateTool.create(
  spec,
  {
    name: 'Create Sitemap',
    key: 'create_sitemap',
  ...
  .input(z.object({
    sitemapName: z.string().describe('Name for the sitemap (used as the sitemap identifier)'),
    startUrls: z.array(z.string()).describe('One or more URLs where the scraper will begin'),
    selectors: z.array(selectorSchema).describe('Array of selector definitions specifying what data to extract'),
  }))
  .output(z.object({
