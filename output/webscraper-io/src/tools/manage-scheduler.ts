...
export let manageScheduler = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['get', 'enable', 'disable']).describe('Action to perform on the scheduler'),
    sitemapId: z.number().describe('ID of the sitemap'),
    cronMinute: z.string().optional().describe('Cron minute expression (required when enabling)'),
    cronHour: z.string().optional().describe('Cron hour expression (required when enabling)'),
    cronDay: z.string().optional().describe('Cron day-of-month expression (required when enabling)'),
    cronMonth: z.string().optional().describe('Cron month expression (required when enabling)'),
    cronWeekday: z.string().optional().describe('Cron day-of-week expression (required when enabling)'),
    cronTimezone: z.string().optional().describe('Timezone string, e.g. "America/New_York"'),
    driver: z.enum(['fast', 'fulljs']).optional().describe('Scraping driver for scheduled jobs'),
    proxy: z.string().optional().describe('Proxy for scheduled jobs'),
    requestInterval: z.number().optional().describe('Delay between requests in milliseconds'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'enable') {
      if (!ctx.input.cronMinute || !ctx.input.cronHour || !ctx.input.cronDay || !ctx.input.cronMonth || !ctx.input.cronWeekday) {
        throw new Error('All cron fields (cronMinute, cronHour, cronDay, cronMonth, cronWeekday) are required when enabling the scheduler.');
      }
