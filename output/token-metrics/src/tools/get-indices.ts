...
export let getIndices = SlateTool.create(
  spec,
  {
    name: 'Get AI Indices',
    key: 'get_indices',
  ...
  .input(z.object({
    queryType: z.enum(['list', 'holdings', 'performance']).describe('What to retrieve: list of indices, holdings of an index, or performance history'),
    indicesType: z.enum(['active', 'passive']).optional().describe('Filter by index type (for list query)'),
    indexId: z.number().optional().describe('Index ID (required for holdings and performance queries)'),
    startDate: z.string().optional().describe('Start date in YYYY-MM-DD format (for performance query)'),
    endDate: z.string().optional().describe('End date in YYYY-MM-DD format (for performance query)'),
