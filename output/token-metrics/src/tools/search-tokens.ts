...
export let searchTokens = SlateTool.create(
  spec,
  {
    name: 'Search Tokens',
    key: 'search_tokens',
  ...
  .input(z.object({
    tokenId: z.string().optional().describe('Comma-separated Token Metrics IDs, e.g. "3375,3306"'),
    symbol: z.string().optional().describe('Comma-separated token symbols, e.g. "BTC,ETH"'),
    tokenName: z.string().optional().describe('Comma-separated token names, e.g. "Bitcoin,Ethereum"'),
    category: z.string().optional().describe('Comma-separated categories, e.g. "layer-1,defi,nft"'),
    exchange: z.string().optional().describe('Comma-separated exchanges, e.g. "binance,gate"'),
    blockchainAddress: z.string().optional().describe('Contract address in format "blockchain-name:0xaddress"'),
  ...
  .output(z.object({
    tokens: z.array(z.record(z.string(), z.any())).describe('Array of token objects with details'),
  }))
