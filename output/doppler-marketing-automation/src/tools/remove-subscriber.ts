...
export let removeSubscriber = SlateTool.create(
  spec,
  {
...
When removing from a list, the subscriber is only disassociated from that list. When unsubscribing from the account, the subscriber will no longer receive any communications.`,
  ...
  .input(z.object({
    action: z.enum(['remove_from_list', 'unsubscribe_from_account']).describe('Whether to remove from a list or unsubscribe from the account entirely'),
    email: z.string().describe('Email address of the subscriber'),
    listId: z.number().optional().describe('ID of the list to remove from. Required for remove_from_list action.'),
    unsubscriptionReason: z.enum([
      'administrative',
    ...
    ]).optional().describe('Reason for unsubscription. Only used with unsubscribe_from_account action.'),
    unsubscriptionComment: z.string().optional().describe('Optional comment for unsubscription (max 200 characters). Only used with unsubscribe_from_account action.'),
  }))
  .output(z.object({
