...
export let exportLeads = SlateTool.create(
  spec,
  {
    name: 'Export Leads',
    key: 'export_leads',
    ...
    constraints: [
      'Limited to 5 export requests per minute.',
    ],
    tags: {
      readOnly: false,
    },
  ...
  .handleInvocation(async (ctx) => {
    ...
    let exportRequest = await client.createExportRequest(
      accountId,
      ctx.input.customFeedId,
      {
