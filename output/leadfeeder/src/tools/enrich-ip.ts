...
export let enrichIp = SlateTool.create(
  ...
  .output(z.object({
    ...
    ipAddress: z.string(),
    confidenceScore: z.string().describe('Confidence level: very high, high, medium, or low'),
  ...
  .handleInvocation(async (ctx) => {
    if (!ctx.auth.ipEnrichToken) {
      throw new Error('IP-Enrich API key is not configured. Please add it in authentication settings.');
    }
