...
export let manageMetadata = SlateTool.create(
  ...
  .input(z.object({
    ...
    key: z.string().optional().describe('Metadata key (required for set and delete actions)'),
  ...
  .output(z.object({
    ...
    deleted: z.boolean().optional().describe('Whether the key was deleted (for delete action)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'set') {
      ...
      let updatedValue = await client.updateMetadataKey(ctx.input.fileId, ctx.input.key, ctx.input.value);
      return {
        output: { updatedValue },
