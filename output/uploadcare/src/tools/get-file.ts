...
export let getFile = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeAppdata: z.boolean().optional().describe('Whether to include add-on results (appdata) in the response'),
  }))
  .output(z.object({
    ...
    datetimeRemoved: z.string().nullable().describe('ISO 8601 removal timestamp, null if not removed'),
    originalFileUrl: z.string().nullable().describe('CDN URL of the original file'),
    contentInfo: z.any().nullable().describe('Content-specific info (image dimensions, video metadata, etc.)'),
    metadata: z.record(z.string(), z.string()).describe('User-defined key-value metadata'),
    appdata: z.any().nullable().describe('Add-on results data (only present if includeAppdata is true)'),
  }))
