...
let webhookSchema = z.object({
  ...
  signingSecret: z.string().nullable().describe('Signing secret for verifying webhook payloads'),
});
...
export let createWebhook = SlateTool.create(
  ...
  .input(z.object({
    ...
    event: z.enum(['file.uploaded', 'file.stored', 'file.deleted', 'file.infected', 'file.info_updated']).describe('Event type to subscribe to'),
    isActive: z.boolean().optional().describe('Whether the webhook is active (default: true)'),
    signingSecret: z.string().optional().describe('Secret key for HMAC-SHA256 signature verification of payloads'),
  }))
...
export let updateWebhook = SlateTool.create(
  ...
  .input(z.object({
    ...
    event: z.enum(['file.uploaded', 'file.stored', 'file.deleted', 'file.infected', 'file.info_updated']).optional().describe('New event type'),
