...
export let listEmailTemplates = SlateTool.create(
  ...
  .input(z.object({
    ...
    searchTitle: z.string().optional().describe('Search templates by title (starts with, min 3 chars)'),
    searchSubject: z.string().optional().describe('Search templates by subject (starts with, min 3 chars)')
  }))
  .output(z.object({
...
export let getEmailTemplate = SlateTool.create(
  spec,
  {
