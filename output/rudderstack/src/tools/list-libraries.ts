...
export let listLibraries = SlateTool.create(
  ...
  .input(z.object({
    libraryId: z.string().optional().describe('If provided, fetch version history for this specific library instead of listing all'),
    versionOrder: z.enum(['asc', 'desc']).optional().describe('Order for version listing'),
  }))
  .output(z.object({
    libraries: z.array(z.record(z.string(), z.any())).optional().describe('List of libraries'),
    versions: z.array(z.record(z.string(), z.any())).optional().describe('Version history if a specific library was queried'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.libraryId) {
      let versions = await client.getLibraryVersions(ctx.input.libraryId, ctx.input.versionOrder);
