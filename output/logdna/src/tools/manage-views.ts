...
let channelSchema = z.object({
  ...
  bodyTemplate: z.string().optional().describe('Body template for webhook, supports {{name}} variables'),
  key: z.string().optional().describe('PagerDuty service key'),
  triggerlimit: z.number().optional().describe('Number of log lines that must match before triggering'),
  triggerinterval: z.string().optional().describe('Time interval for trigger evaluation (e.g., "15m", "30m", "1h")'),
  operator: z.string().optional().describe('Trigger operator: "presence" or "absence"'),
  immediate: z.boolean().optional().describe('Whether to send alert immediately when condition is met'),
...
export let getView = SlateTool.create(
  spec,
  {
    name: 'Get View',
    key: 'get_view',
...
export let createView = SlateTool.create(
  ...
  .input(z.object({
    ...
    levels: z.array(z.string()).optional().describe('Log levels to filter by (e.g., error, warn, info)'),
    tags: z.array(z.string()).optional().describe('Tags to filter by'),
    category: z.array(z.string()).optional().describe('Category IDs to assign the view to'),
    channels: z.array(channelSchema).optional().describe('Alert channels to attach to this view'),
