...
let archiveOutputSchema = z.object({
  integration: z.string().optional().describe('Storage provider type (ibm, s3, azblob, gcs, dos, swift)'),
...
export let saveArchiveConfig = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    integration: z.enum(['ibm', 's3', 'azblob', 'gcs', 'dos', 'swift']).describe('Storage provider type'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.resourceInstanceId) archivePayload.resourceinstanceid = ctx.input.resourceInstanceId;
