...
export let generationStatus = SlateTrigger.create(
  spec,
  {
    name: 'Generation Status Changed',
    key: 'generation_status',
    description: 'Fires when an image generation changes status — created, active, progress, completed, or failed.',
  },
)
  .input(z.object({
    eventType: z.string().describe('Webhook event type (e.g., print.created, print.completed, print.failed)'),
  ...
  .output(z.object({
    ...
    status: z.number().optional().describe('Status code: 0=created, 1=pending, 2=processing, 3=done, 4=error'),
    ...
    })).optional().describe('Generated images (available when completed)'),
