...
export let getCredits = SlateTool.create(
  ...
  .output(z.object({
    credits: z.array(z.object({
      type: z.string().optional().describe('Credit type (e.g., global, f_images, f_images_print, f_images_upscaleHD, f_images_upscaleUHD)'),
      credit: z.number().describe('Number of remaining credits'),
      creditTotal: z.number().optional().describe('Total credits renewed per subscription period'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let summary = credits
      .map((c) => `${c.type ?? 'unknown'}: ${c.unlimited ? 'unlimited' : `${c.credit}${c.creditTotal ? `/${c.creditTotal}` : ''}`}`)
      .join(', ');
