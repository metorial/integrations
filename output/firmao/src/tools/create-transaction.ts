...
export let createTransaction = SlateTool.create(
  ...
  .input(z.object({
    type: z.enum(['INVOICE', 'RECEIPT', 'PROFORMA', 'CORRECTION', 'BILL']).describe('Transaction type'),
    ...
    customerName: z.string().optional().describe('Customer name (used with createCustomerIfNeeded)'),
    createCustomerIfNeeded: z.boolean().optional().describe('Auto-create customer if not found'),
    ...
    connectedDocNumber: z.string().optional().describe('Original document number (for corrections)'),
    connectedDocDate: z.string().optional().describe('Original document date (for corrections)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.createCustomerIfNeeded !== undefined) body.createCustomerIfNeeded = ctx.input.createCustomerIfNeeded;
