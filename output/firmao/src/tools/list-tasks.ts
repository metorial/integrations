...
export let listTasks = SlateTool.create(
  ...
  .input(z.object({
    ...
    nameContains: z.string().optional().describe('Filter tasks whose name contains this value'),
    status: z.string().optional().describe('Filter by status (e.g., WAITING, IN_PROGRESS, DONE)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.projectId !== undefined) filters['project(eq)'] = String(ctx.input.projectId);
