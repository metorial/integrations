...
export let createCustomer = SlateTool.create(
  ...
  .input(z.object({
    ...
    correspondencePostCode: z.string().optional().describe('Correspondence address post code'),
    correspondenceCountry: z.string().optional().describe('Correspondence address country'),
    customFields: z.record(z.string(), z.string()).optional().describe('Custom fields as key-value pairs (e.g., {"customFields.custom5": "value"})'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.employeesNumber !== undefined) body.employeesNumber = ctx.input.employeesNumber;
    if (ctx.input.industryKey) body.industry = { key: ctx.input.industryKey };

    if (ctx.input.officeStreet || ctx.input.officeCity || ctx.input.officePostCode || ctx.input.officeCountry) {
    ...
    if (ctx.input.correspondenceStreet || ctx.input.correspondenceCity || ctx.input.correspondencePostCode || ctx.input.correspondenceCountry) {
