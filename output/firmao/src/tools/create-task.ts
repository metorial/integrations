...
export let createTask = SlateTool.create(
  ...
  .input(z.object({
    ...
    priority: z.string().optional().describe('Priority level (e.g., 100=low, 200=normal, 300=high)'),
    ...
    plannedStartDateType: z.enum(['EXACTLY', 'APPROXIMATELY']).optional().describe('Start date precision'),
    plannedEndDateType: z.enum(['EXACTLY', 'APPROXIMATELY']).optional().describe('End date precision'),
    ...
    responsibleUserIds: z.array(z.number()).optional().describe('IDs of users responsible for the task'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.plannedStartDateType) body.plannedStartDateType = ctx.input.plannedStartDateType;
