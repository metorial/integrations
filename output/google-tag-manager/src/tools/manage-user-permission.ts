...
let containerAccessSchema = z.object({
  ...
  permission: z.enum(['noAccess', 'read', 'edit', 'approve', 'publish']).describe('Permission level for the container')
});
...
let userPermissionOutputSchema = z.object({
  ...
  }).optional().describe('Account-level access'),
  ...
  })).optional().describe('Container-level access entries')
});

export let manageUserPermission = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    permissionId: z.string().optional().describe('User permission ID (required for get, update, delete)'),
    emailAddress: z.string().optional().describe('Email address of the user (required for create)'),
    accountPermission: z.enum(['noAccess', 'read', 'admin']).optional().describe('Account-level permission'),
    containerAccess: z.array(containerAccessSchema).optional().describe('Container-level permission entries')
  }))
  .output(z.object({
    permission: userPermissionOutputSchema.optional().describe('User permission details'),
    permissions: z.array(userPermissionOutputSchema).optional().describe('List of user permissions')
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      if (!ctx.input.emailAddress) throw new Error('emailAddress is required for creating a user permission');
    ...
    if (!permissionId) throw new Error('permissionId is required for get, update, and delete actions');
