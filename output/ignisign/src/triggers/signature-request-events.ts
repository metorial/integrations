...
export let signatureRequestEvents = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggered when signature request state changes occur, such as initialization, update, publish, launch, close, completion, cancellation, or expiration.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    action: z.string().describe('Event action (e.g., INIT, UPDATE, PUBLISH, LAUNCHED, CLOSED, COMPLETED)'),
