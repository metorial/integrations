...
export let createSigner = SlateTool.create(
  spec,
  {
    name: 'Create Signer',
    key: 'create_signer',
  ...
  .input(z.object({
    signerProfileId: z.string().describe('Signer profile ID that determines the signature mechanism and required inputs'),
    ...
    nationality: z.string().optional().describe('Nationality of the signer (ISO 3166-1 alpha-2 code)'),
    ...
    birthCountry: z.string().optional().describe('Birth country of the signer (ISO 3166-1 alpha-2 code)'),
  }))
  .output(z.object({
