...
export let createSignatureRequest = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    signatureProfileId: z.string().optional().describe('Signature profile ID to use for this request'),
    ...
    externalId: z.string().optional().describe('External ID for correlating with your system'),
    documentIds: z.array(z.string()).optional().describe('Document IDs to include in this request'),
    signerIds: z.array(z.string()).optional().describe('Signer IDs who should sign'),
    language: z.string().optional().describe('Language code for the signature session (e.g., EN, FR, DE)'),
    expirationDate: z.string().optional().describe('Expiration date in ISO 8601 format'),
    expirationDateIsActivated: z.boolean().optional().describe('Whether expiration is active'),
    publish: z.boolean().optional().describe('If true, publish the request immediately after setup'),
  }))
  .output(z.object({
    ...
    context: z.any().optional().describe('Full context of the signature request after update/publish'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let hasUpdateData = ctx.input.title || ctx.input.description || ctx.input.externalId ||
      ctx.input.documentIds || ctx.input.signerIds || ctx.input.language ||
      ctx.input.expirationDate || ctx.input.expirationDateIsActivated !== undefined;
