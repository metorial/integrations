...
export let listSignerProfiles = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    signerProfileId: z.string().optional().describe('If provided, also fetches the specific input requirements for this profile'),
  }))
  .output(z.object({
    signerProfiles: z.array(z.any()).describe('List of available signer profiles'),
    inputsNeeded: z.any().optional().describe('Required inputs for the specified signer profile'),
  }))
