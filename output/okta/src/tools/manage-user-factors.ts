...
let factorSchema = z.object({
  factorId: z.string(),
  factorType: z.string().describe('Factor type (sms, email, token:software:totp, push, question, etc.)'),
  ...
  profile: z.record(z.string(), z.any()).optional().describe('Factor-specific profile (e.g. phone number for SMS)'),
});

export let manageUserFactorsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list', 'enroll', 'reset_all']).describe('Operation: list enrolled factors, enroll a new factor, or reset all factors'),
    userId: z.string().describe('Okta user ID'),
    factorType: z.string().optional().describe('Factor type for enrollment (e.g. sms, email, token:software:totp)'),
    provider: z.string().optional().describe('Factor provider for enrollment (e.g. OKTA, GOOGLE)'),
    factorProfile: z.record(z.string(), z.any()).optional().describe('Factor profile data for enrollment (e.g. { phoneNumber: "+1-555-1234" } for SMS)'),
  }))
  .output(z.object({
    userId: z.string(),
    action: z.string(),
    ...
    enrolledFactor: factorSchema.optional().describe('Newly enrolled factor (for enroll action)'),
  }))
