...
export let manageAppAssignmentTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['assign_user', 'unassign_user', 'assign_group', 'unassign_group', 'list_users', 'list_groups']).describe('Operation to perform'),
    appId: z.string().describe('Okta application ID'),
    userId: z.string().optional().describe('User ID (required for assign_user/unassign_user)'),
    groupId: z.string().optional().describe('Group ID (required for assign_group/unassign_group)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('App user assignments (only for list_users)'),
    assignedGroups: z.array(z.object({
      groupId: z.string(),
      priority: z.number().optional(),
    })).optional().describe('App group assignments (only for list_groups)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'list_users') {
      ...
      return {
        output: { appId, action, success: true, assignedUsers, nextCursor, hasMore: !!result.nextUrl },
    ...
    if (action === 'list_groups') {
      ...
      return {
        output: { appId, action, success: true, assignedGroups, nextCursor, hasMore: !!result.nextUrl },
