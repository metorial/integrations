...
Use this to track document progress, find pending signatures, or review completed documents.`,
  ...
  .input(z.object({
    status: z.enum(['draft', 'awaiting', 'completed']).optional().describe('Filter documents by signing status'),
    orderingKey: z.enum(['createdAt', 'updatedAt']).optional().describe('Field to sort results by. Defaults to createdAt'),
    orderingDirection: z.enum(['ASC', 'DESC']).optional().describe('Sort direction. Defaults to DESC (newest first)'),
  ...
  .output(z.object({
    documents: z.array(z.object({
      ...
      updatedAt: z.string().optional().describe('ISO timestamp when the document was last updated'),
    })).describe('List of documents matching the filter criteria'),
    totalCount: z.number().optional().describe('Total number of documents matching the filter'),
  }))
