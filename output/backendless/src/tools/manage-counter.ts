...
export let manageCounter = SlateTool.create(
  spec,
  {
    name: 'Manage Counter',
    key: 'manage_counter',
  ...
  .input(z.object({
    ...
    operation: z.enum(['get', 'increment', 'decrement', 'incrementBy', 'reset']).describe('Operation to perform on the counter'),
    incrementValue: z.number().optional().describe('Amount to increment by (only used with "incrementBy" operation)')
  }))
  .output(z.object({
    counterName: z.string().describe('Name of the counter'),
    value: z.number().optional().describe('Current value of the counter after the operation'),
