...
export let deleteObject = SlateTool.create(
  spec,
  {
    name: 'Delete Object',
    key: 'delete_object',
  ...
  .input(z.object({
    ...
    where: z.string().optional().describe('Where clause for bulk deletion, e.g. "status = \'archived\'"')
  }))
  .output(z.object({
    deletionTime: z.number().optional().describe('Timestamp of the deletion (single delete)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {},
      message: 'No objectId or where clause provided. Please specify either `objectId` for a single delete or `where` for a bulk delete.'
    };
  }).build();
