...
export let sendEmail = SlateTool.create(
  ...
  .input(z.object({
    templateName: z.string().describe('Name of the email template as defined in Backendless Console'),
    toAddresses: z.array(z.string()).optional().describe('List of recipient email addresses'),
    ...
    recipientCriteria: z.string().optional().describe('SQL where clause to select recipients from the Users table, e.g. "age > 21"'),
    uniqueEmails: z.boolean().optional().describe('If true (default), excludes duplicate email addresses'),
    templateValues: z.record(z.string(), z.unknown()).optional().describe('Key-value pairs to substitute in template placeholders'),
    attachments: z.array(z.string()).optional().describe('File paths in Backendless storage to attach to the email')
  }))
  .output(z.object({
    sent: z.boolean().describe('Whether the email was sent successfully'),
    response: z.record(z.string(), z.unknown()).optional().describe('Response details from the email API')
  }))
