...
export let findUsers = SlateTool.create(
  spec,
  {
    name: 'Find Users',
    key: 'find_users',
  ...
  .input(z.object({
    ...
    sortBy: z.array(z.string()).optional().describe('Properties to sort by. Prefix with "-" for descending.'),
    ...
    countOnly: z.boolean().optional().describe('If true, returns only the count of matching users')
  }))
  .output(z.object({
    users: z.array(z.record(z.string(), z.unknown())).optional().describe('Array of matching user objects'),
    singleUser: z.record(z.string(), z.unknown()).optional().describe('Single user when queried by ID'),
    count: z.number().describe('Number of users returned or total count if countOnly is true')
  }))
