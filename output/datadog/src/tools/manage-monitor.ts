...
let monitorOptionsSchema = z.object({
  thresholds: z.object({
  ...
  }).optional().describe('Alert threshold values'),
  ...
  includeTags: z.boolean().optional().describe('Include triggering tags in notification title'),
  requireFullWindow: z.boolean().optional().describe('Require a full window of data for evaluation'),
  evaluationDelay: z.number().optional().describe('Delay in seconds before evaluating')
}).optional().describe('Monitor configuration options');
...
Supports metric alerts, log alerts, anomaly detection, forecast, outlier, APM, and composite monitors.`,
  ...
  .input(z.object({
    monitorId: z.number().optional().describe('Monitor ID to update. Omit to create a new monitor.'),
    name: z.string().optional().describe('Monitor name (required for creation)'),
    type: z.string().optional().describe('Monitor type (required for creation), e.g. "metric alert", "query alert"'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (options) {
      ...
      if (options.thresholds) {
        ...
        if (options.thresholds.critical !== undefined) apiOptions.thresholds.critical = options.thresholds.critical;
        if (options.thresholds.warning !== undefined) apiOptions.thresholds.warning = options.thresholds.warning;
        if (options.thresholds.ok !== undefined) apiOptions.thresholds.ok = options.thresholds.ok;
        if (options.thresholds.criticalRecovery !== undefined) apiOptions.thresholds.critical_recovery = options.thresholds.criticalRecovery;
        if (options.thresholds.warningRecovery !== undefined) apiOptions.thresholds.warning_recovery = options.thresholds.warningRecovery;
      }
      if (options.notifyNoData !== undefined) apiOptions.notify_no_data = options.notifyNoData;
      if (options.noDataTimeframe !== undefined) apiOptions.no_data_timeframe = options.noDataTimeframe;
      if (options.renotifyInterval !== undefined) apiOptions.renotify_interval = options.renotifyInterval;
      if (options.escalationMessage !== undefined) apiOptions.escalation_message = options.escalationMessage;
      if (options.includeTags !== undefined) apiOptions.include_tags = options.includeTags;
      if (options.requireFullWindow !== undefined) apiOptions.require_full_window = options.requireFullWindow;
      if (options.evaluationDelay !== undefined) apiOptions.evaluation_delay = options.evaluationDelay;
    }
