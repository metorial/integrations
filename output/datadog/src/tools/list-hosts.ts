...
export let listHosts = SlateTool.create(
  spec,
  {
    name: 'List Hosts',
    key: 'list_hosts',
  ...
  .input(z.object({
    filter: z.string().optional().describe('Filter hosts by name or tag, e.g. "web" or "env:production"'),
    sortField: z.string().optional().describe('Sort field: "cpu", "iowait", "load", "apps", "name", "status"'),
    ...
    from: z.number().optional().describe('Only return hosts that have reported in the last N seconds')
  }))
  .output(z.object({
