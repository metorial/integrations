...
export let manageSynthetics = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    publicId: z.string().describe('The public ID of the Synthetics test, e.g. "abc-def-ghi"'),
    includeResults: z.boolean().optional().describe('Whether to also fetch recent test results')
  }))
  .output(z.object({
    ...
    })).optional().describe('Recent test results')
  }))
