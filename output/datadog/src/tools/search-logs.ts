...
Uses the Datadog log search query language, e.g. \`service:my-service status:error\`.`,
  ...
  .input(z.object({
    query: z.string().optional().describe('Log search query, e.g. "service:web status:error". Defaults to "*"'),
    ...
    limit: z.number().optional().describe('Maximum number of logs to return (default 50, max 1000)'),
    cursor: z.string().optional().describe('Pagination cursor from a previous response'),
    sort: z.enum(['timestamp', '-timestamp']).optional().describe('Sort order: "timestamp" (ascending) or "-timestamp" (descending)'),
    indexes: z.array(z.string()).optional().describe('Log indexes to search. Defaults to all indexes.')
  }))
  .output(z.object({
