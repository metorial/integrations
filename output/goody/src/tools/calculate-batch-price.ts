...
export let calculateBatchPrice = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    sendMethod: z.enum(['email_and_link', 'link_multiple_custom_list', 'direct_send']).describe('Delivery method'),
    recipients: z.array(recipientInputSchema).min(1).describe('Recipients for pricing calculation'),
    cart: z.array(cartItemSchema).min(1).describe('Products to include'),
  }))
  .output(z.object({
    perRecipientEstimate: z.object({
      ...
      amountProcessingFee: z.number().nullable().describe('Processing fee per recipient in USD cents'),
      amountPreTaxTotal: z.number().describe('Subtotal before tax per recipient in USD cents'),
      estimatedTaxLow: z.number().nullable().describe('Low tax estimate per recipient in USD cents'),
      estimatedTaxHigh: z.number().nullable().describe('High tax estimate per recipient in USD cents'),
      estimatedTotalLow: z.number().nullable().describe('Low total estimate per recipient in USD cents'),
      estimatedTotalHigh: z.number().nullable().describe('High total estimate per recipient in USD cents'),
    }).describe('Per-recipient pricing estimate'),
    totalEstimate: z.object({
      recipientsCount: z.number().describe('Number of recipients'),
      estimatedTotalLow: z.number().nullable().describe('Low total estimate for entire batch in USD cents'),
      estimatedTotalHigh: z.number().nullable().describe('High total estimate for entire batch in USD cents'),
