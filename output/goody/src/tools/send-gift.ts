...
let recipientInputSchema = z.object({
  ...
  email: z.string().optional().describe('Recipient email address (required for email_and_link send method)'),
  phone: z.string().optional().describe('Recipient phone number in E.164 format (e.g. +15552347890), US only'),
});
...
let cartItemSchema = z.object({
  ...
  variants: z.array(z.string()).optional().describe('Array of variant names for direct_send orders'),
});
...
export let sendGift = SlateTool.create(
  spec,
  {
    name: 'Send Gift',
    key: 'send_gift',
  ...
  .input(z.object({
    ...
    sendMethod: z.enum(['email_and_link', 'link_multiple_custom_list']).describe('How the gift is delivered to recipients'),
    ...
    message: z.string().optional().describe('Gift message displayed in the digital unwrapping experience'),
    cardId: z.string().optional().describe('ID of a greeting card to attach'),
    paymentMethodId: z.string().optional().describe('Payment method ID (defaults to first on account)'),
    workspaceId: z.string().optional().describe('Workspace ID (defaults to oldest accessible)'),
    scheduledSendOn: z.string().optional().describe('ISO 8601 timestamp to schedule the gift for future delivery'),
    expiresAt: z.string().optional().describe('ISO 8601 timestamp for gift expiration'),
    swap: z.enum(['single', 'multiple', 'disabled']).optional().describe('Gift swap behavior: single (one swap), multiple (multiple swaps), disabled (no swapping)'),
  }))
  .output(z.object({
    orderBatchId: z.string().describe('ID of the created order batch'),
    sendStatus: z.string().describe('Batch processing status: pending, complete, failed, or canceled'),
    ordersCount: z.number().describe('Total number of orders in the batch'),
    isScheduledSend: z.boolean().describe('Whether the batch is scheduled for future delivery'),
