...
export let getPerson = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeDevices: z.boolean().optional().describe('Also retrieve the person\'s tracked devices'),
  }))
  .output(z.object({
    person: z.record(z.string(), z.any()).describe('Full person profile'),
    feed: z.array(z.record(z.string(), z.any())).optional().describe('Person\'s event feed (if requested)'),
    devices: z.array(z.record(z.string(), z.any())).optional().describe('Person\'s tracked devices (if requested)'),
  }))
