...
export let trackPageview = SlateTool.create(
  spec,
  {
    name: 'Track Pageview',
    key: 'track_pageview',
  ...
  .input(z.object({
    ...
    personId: z.string().optional().describe('Person ID to associate the pageview with (use "email:user@example.com" format)'),
    visitorId: z.string().optional().describe('Anonymous visitor ID to associate the pageview with'),
  }))
  .output(z.object({
