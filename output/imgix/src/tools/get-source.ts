...
export let getSource = SlateTool.create(
  ...
  .output(z.object({
    ...
    deployment: z.record(z.string(), z.any()).optional().describe('Full deployment configuration including type, subdomains, and storage-specific settings'),
    secureUrlToken: z.string().optional().describe('Token used for generating signed/secure URLs'),
