...
export let contentTypeEvents = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers when a content type is created, saved, or deleted in your Contentful space. Useful for monitoring changes to the content model that affect the GraphQL schema.'
  }
)
  .input(z.object({
    eventAction: z.string().describe('The content type action that triggered the event (e.g. create, save, delete).'),
    ...
    contentTypeDescription: z.string().optional().describe('Description of the content type.'),
  ...
  .output(z.object({
    ...
    spaceId: z.string().optional().describe('Space ID where the event occurred.'),
    environmentId: z.string().optional().describe('Environment ID where the event occurred.'),
    displayField: z.string().optional().describe('The field used as the display field for this content type.'),
