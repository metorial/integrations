...
export class MapboxClient {
  ...
  async forwardGeocode(searchText: string, options?: {
  ...
  async getDirections(profile: string, coordinates: string, options?: {
  ...
  async getIsochrone(profile: string, longitude: number, latitude: number, options: {
  ...
  async getMatrix(profile: string, coordinates: string, options?: {
    sources?: string;
    destinations?: string;
    annotations?: string;
  ...
  async matchTrace(profile: string, coordinates: string, options?: {
    geometries?: string;
    overview?: string;
    steps?: boolean;
    annotations?: string;
    ...
    let response = await this.http.get(
      `/matching/v5/mapbox/${profile}/${coordinates}`,
      {
        params: this.params({
          geometries: options?.geometries || 'geojson',
          overview: options?.overview || 'full',
  ...
  async getOptimizedTrip(profile: string, coordinates: string, options?: {
    geometries?: string;
    overview?: string;
    steps?: boolean;
    annotations?: string;
  ...
  async tilequery(tilesetId: string, longitude: number, latitude: number, options?: {
    radius?: number;
    limit?: number;
    layers?: string;
    dedupe?: boolean;
