...
export let matrixTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    sources: z.string().optional().describe('Semicolon-separated indices for source coordinates (e.g., "0;1")'),
    destinations: z.string().optional().describe('Semicolon-separated indices for destination coordinates (e.g., "2;3")'),
    annotations: z.string().optional().describe('Comma-separated: "duration", "distance", or "duration,distance"'),
    fallbackSpeed: z.number().optional().describe('Speed (km/h) for direct path fallback when no route exists'),
  }))
  .output(z.object({
    code: z.string().optional().describe('Response status code'),
    durations: z.array(z.array(z.number().nullable())).optional().describe('Matrix of travel durations in seconds [sources × destinations]'),
    distances: z.array(z.array(z.number().nullable())).optional().describe('Matrix of travel distances in meters [sources × destinations]'),
    sources: z.array(z.object({
      name: z.string().optional(),
      location: z.array(z.number()).optional(),
    })).optional().describe('Snapped source locations'),
    destinations: z.array(z.object({
      name: z.string().optional(),
      location: z.array(z.number()).optional(),
    })).optional().describe('Snapped destination locations'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {
        ...
        destinations: result.destinations?.map((d: any) => ({ name: d.name, location: d.location })),
      },
