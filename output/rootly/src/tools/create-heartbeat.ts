...
Configure the expected interval and notification target for when the heartbeat expires.`,
  ...
  .input(z.object({
    ...
    description: z.string().optional().describe('Description of what this heartbeat monitors'),
    interval: z.number().describe('Expected ping interval'),
    intervalUnit: z.enum(['minutes', 'hours', 'days']).describe('Unit for the interval'),
    notificationTargetType: z.enum(['User', 'Group', 'EscalationPolicy', 'Service']).optional().describe('Type of notification target when heartbeat expires'),
