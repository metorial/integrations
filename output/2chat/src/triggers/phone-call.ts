...
export let phoneCallTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Phone Call Event',
    key: 'phone_call_event',
    description: 'Triggers on phone call lifecycle events: status updates, incoming call completions, and outbound call completions.',
  }
)
  .input(z.object({
    ...
    callStatus: z.string().optional().describe('Call status (ringing, answered, ended, missed, failed)'),
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        type: eventMap[ctx.input.eventType] || `phone_call.${ctx.input.eventType.split('.').pop()}`,
