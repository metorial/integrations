...
let whatsappNumberSchema = z.object({
  ...
  status: z.string().optional().describe('Current status of the number (e.g., ready, disconnected)'),
  channelType: z.string().optional().describe('Channel type (e.g., whatsapp_web, whatsapp_business_api)'),
  createdAt: z.string().optional().describe('When the number was connected'),
});

export let listWhatsAppNumbers = SlateTool.create(
  spec,
  {
