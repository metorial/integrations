...
export let listWhatsAppGroups = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    groupUuid: z.string().optional().describe('UUID of a specific group to get participants for'),
  }))
  .output(z.object({
    groups: z.array(groupSchema).optional().describe('List of WhatsApp groups (when listing all groups)'),
    participants: z.array(participantSchema).optional().describe('List of group participants (when getting a specific group)'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new TwoChatClient({ token: ctx.auth.token });

    if (ctx.input.groupUuid) {
      let result = await client.listGroupParticipants(ctx.input.fromNumber, ctx.input.groupUuid);
