...
export let billingWebhook = SlateTrigger.create(
  spec,
  {
    name: 'Billing Webhook',
    key: 'billing_webhook',
    description: 'Triggers on subscription lifecycle events: created, canceled, expired, or unpaid. Configure the webhook URL in SuiteDash under Integrations > Webhooks > Billing.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    eventType: z.string().describe('Type of subscription event (e.g. created, canceled, expired, unpaid)'),
