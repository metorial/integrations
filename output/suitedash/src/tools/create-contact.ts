...
export let createContact = SlateTool.create(
  ...
  .input(z.object({
    ...
    email: z.string().optional().describe('Email address of the contact (required for prospect and client roles)'),
    role: z.enum(['lead', 'prospect', 'client']).describe('Role assigned to the contact'),
    sendWelcomeEmail: z.boolean().optional().describe('Whether to send a welcome email to the contact'),
    customFields: z.record(z.string(), z.unknown()).optional().describe('Additional custom fields as key-value pairs (use Get Metadata to discover available fields)'),
  }))
  .output(z.object({
