...
export let updateGroup = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.certificateDesignId !== undefined) updateParams.certificateDesignId = ctx.input.certificateDesignId;
    if (ctx.input.badgeDesignId !== undefined) updateParams.badgeDesignId = ctx.input.badgeDesignId;
    if (ctx.input.learningEventUrl !== undefined) updateParams.learningEventUrl = ctx.input.learningEventUrl;
