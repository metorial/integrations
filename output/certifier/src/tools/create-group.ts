...
export let createGroup = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'At least one of certificateDesignId or badgeDesignId is required.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    name: z.string().describe('Group name, used as the [group.name] attribute in credentials'),
    certificateDesignId: z.string().optional().describe('ID of the certificate design to use'),
    badgeDesignId: z.string().optional().describe('ID of the badge design to use'),
    learningEventUrl: z.string().optional().describe('URL of the learning event shown in the digital wallet'),
  }))
  .output(z.object({
