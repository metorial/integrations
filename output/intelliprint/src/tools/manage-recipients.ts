...
export let addRecipient = SlateTool.create(
  spec,
  {
    name: 'Add Recipient',
    key: 'add_recipient',
  ...
  .input(z.object({
    ...
    variables: z.record(z.string(), z.any()).optional().describe('Template variable values for personalization'),
  }))
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: mapped,
      message: `Recipient **${mapped.recipientId}** added to mailing list. Address: ${mapped.address?.name ?? ''} ${mapped.address?.line ?? ''}`.trim(),
    };
  })
  .build();
...
export let getRecipient = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: mapped,
      message: `Recipient **${mapped.recipientId}**: ${mapped.address?.name ?? ''} (${mapped.addressValidationStatus ?? 'unknown'})`.trim(),
    };
  })
  .build();
...
export let updateRecipient = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    variables: z.record(z.string(), z.any()).optional().describe('Updated template variable values'),
  }))
  .output(mailingListRecipientSchema)
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
    ...
    let result = await client.updateRecipient(ctx.input.mailingListId, ctx.input.recipientId, params);
    let mapped = mapRecipient(result);
