...
export let updatePrintJob = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Cannot update a print job that has already been confirmed.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    ...
    confirmed: z.boolean().optional().describe('Set to true to confirm the draft and submit for printing'),
    ...
    }).optional().describe('Updated printing options'),
    ...
    addressWindow: z.enum(['left', 'right']).optional().describe('Updated address window position'),
    ...
    removeLettersContaining: z.string().optional().describe('Remove letters containing this phrase'),
  }))
