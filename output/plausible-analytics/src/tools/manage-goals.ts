...
export let createGoal = SlateTool.create(
  ...
  .input(
    z.object({
      ...
      goalType: z.enum(['event', 'page']).describe('Type of goal: "event" for custom events, "page" for pageview goals'),
      eventName: z.string().optional().describe('Event name for custom event goals (required when goalType is "event")'),
      pagePath: z.string().optional().describe('Page path for pageview goals, supports wildcards (required when goalType is "page")'),
      displayName: z.string().optional().describe('Custom display name for the goal on the dashboard'),
      customProps: z
        .record(z.string(), z.array(z.string()))
        .optional()
        .describe('Custom property constraints. Keys are property names, values are arrays of allowed values.'),
    })
  )
  .output(
    z.object({
...
export let listGoals = SlateTool.create(
  spec,
  {
    name: 'List Goals',
    key: 'list_goals',
