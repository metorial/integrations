...
export let newEmployeeTrigger = SlateTrigger.create(
  spec,
  {
    name: 'New Employee',
    key: 'new_employee',
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let newEmployees = lastKnownId
        ? employees.filter((e) => Number(e.id) > lastKnownId)
        : [];

      let maxId = employees.reduce((max, e) => Math.max(max, Number(e.id) || 0), lastKnownId ?? 0);
