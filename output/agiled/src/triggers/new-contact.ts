...
export let newContactTrigger = SlateTrigger.create(
  spec,
  {
    name: 'New Contact',
    key: 'new_contact',
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let newContacts = lastKnownId
        ? contacts.filter((c) => Number(c.id) > lastKnownId)
        : [];

      let maxId = contacts.reduce((max, c) => Math.max(max, Number(c.id) || 0), lastKnownId ?? 0);
