...
export let listMaintenanceWindows = SlateTool.create(
  ...
  .input(z.object({
    state: z.enum(['pending', 'active', 'paused']).optional().describe('Filter by maintenance window state'),
    page: z.number().optional().describe('Page number for pagination'),
    limit: z.number().optional().describe('Number of results per page'),
  }))
  .output(z.object({
    windows: z.array(z.record(z.string(), z.any())).describe('List of maintenance window objects'),
  }))
