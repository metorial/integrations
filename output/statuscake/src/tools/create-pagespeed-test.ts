...
export let createPagespeedTest = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    checkRate: z.number().describe('Check frequency in seconds (e.g. 300, 600, 1800, 3600, 86400)'),
    locationIso: z.string().describe('Monitoring region ISO code (AU, CA, DE, FR, IN, JP, NL, SG, UK, US, USW)'),
    contactGroups: z.array(z.string()).optional().describe('List of contact group IDs for alerts'),
    alertBigger: z.number().optional().describe('Alert if page size exceeds this value in bytes'),
    alertSlower: z.number().optional().describe('Alert if load time exceeds this value in milliseconds'),
    alertSmaller: z.number().optional().describe('Alert if page size is smaller than this value in bytes'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { websiteUrl, checkRate, locationIso, contactGroups, alertBigger, alertSlower, alertSmaller, ...rest } = ctx.input;
