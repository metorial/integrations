...
export let listMonitoringLocations = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    locationType: z.enum(['uptime', 'pagespeed']).describe('Type of monitoring locations to retrieve'),
    regionCode: z.string().optional().describe('Filter by region code (for uptime locations)'),
  }))
  .output(z.object({
    locations: z.array(z.record(z.string(), z.any())).describe('List of monitoring location objects with IP addresses and region info'),
  }))
