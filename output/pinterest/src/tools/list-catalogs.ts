...
export let listCatalogs = SlateTool.create(
  ...
  .input(z.object({
    resource: z.enum(['catalogs', 'feeds', 'items', 'product_groups']).describe('Type of catalog resource to list'),
    feedId: z.string().optional().describe('Feed ID (required for listing items)'),
    catalogId: z.string().optional().describe('Filter feeds or product groups by catalog ID'),
