...
export let listAdAccounts = SlateTool.create(
  ...
  .input(z.object({
    adAccountId: z.string().optional().describe('If provided, retrieves this specific ad account instead of listing all'),
    includeSharedAccounts: z.boolean().optional().describe('Include shared ad accounts in the results'),
    bookmark: z.string().optional().describe('Pagination bookmark from a previous response'),
    pageSize: z.number().optional().describe('Number of accounts to return (max 250, default 25)'),
  }))
  .output(z.object({
    adAccounts: z.array(z.object({
      ...
      permissions: z.array(z.string()).optional().describe('User permissions for this account'),
      createdTime: z.number().optional().describe('Account creation timestamp'),
    })).optional().describe('List of ad accounts'),
    singleAccount: z.any().optional().describe('Single ad account details when queried by ID'),
