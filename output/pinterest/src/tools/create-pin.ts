...
let mediaSourceSchema = z.object({
  sourceType: z.enum(['image_url', 'image_base64', 'video_id', 'multiple_image_urls']).describe('Type of media source. Use "image_url" for an image URL, "video_id" for a previously uploaded video, or "multiple_image_urls" for carousel pins.'),
  url: z.string().optional().describe('URL of the image (required for image_url source type)'),
  contentType: z.enum(['image/jpeg', 'image/png', 'image/gif', 'image/webp']).optional().describe('MIME type of the image'),
  mediaId: z.string().optional().describe('Media ID for video pins (required for video_id source type)'),
  ...
  })).optional().describe('Items for carousel/multi-image pins (required for multiple_image_urls source type)'),
}).describe('Media source configuration for the pin');

export let createPin = SlateTool.create(
  spec,
  {
    name: 'Create Pin',
    key: 'create_pin',
  ...
  .input(z.object({
    ...
    boardSectionId: z.string().optional().describe('ID of the board section to save the pin to'),
    title: z.string().optional().describe('Title of the pin (max 100 characters)'),
    description: z.string().optional().describe('Description of the pin (max 800 characters)'),
    link: z.string().optional().describe('Destination link URL when the pin is clicked'),
    altText: z.string().optional().describe('Alt text for the pin image (max 500 characters)'),
  ...
  .output(z.object({
    ...
    creativeType: z.string().optional().describe('Type of pin creative (e.g., IMAGE, VIDEO, CAROUSEL)'),
  }))
