...
export let listUsersTeams = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    includeUsers: z.boolean().optional().describe('Include users in results (default: true)'),
    includeTeams: z.boolean().optional().describe('Include teams in results (default: true)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (includeUsers) {
      let userResults = await client.findUsers({ limit: ctx.input.limit, page: ctx.input.page });
    ...
    if (includeTeams) {
      let teamResults = await client.findTeams({ limit: ctx.input.limit, page: ctx.input.page });
