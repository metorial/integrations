...
export let getDeployment = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    let deployment = await client.getDeployment(org, ctx.input.projectName, ctx.input.stackName, ctx.input.deploymentId);

    let logs: any[] | undefined;
    if (ctx.input.includeLogs) {
      try {
        let logResult = await client.getDeploymentLogs(org, ctx.input.projectName, ctx.input.stackName, ctx.input.deploymentId);
