...
export let triggerDeployment = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    operation: z.enum(['update', 'preview', 'refresh', 'destroy']).describe('Deployment operation to execute'),
    inheritSettings: z.boolean().optional().default(true).describe('Inherit deployment settings configured on the stack'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.createDeployment(org, ctx.input.projectName, ctx.input.stackName, {
