...
export let manageAccessTokens = SlateTool.create(
  ...
  .input(z.object({
    ...
    description: z.string().optional().describe('Description for new token (required for create)'),
    expiresAt: z.number().optional().describe('Unix timestamp for token expiry (0 or omit for no expiry, max 2 years)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'create': {
        if (!ctx.input.description) throw new Error('description is required when creating a token');
        let result = await client.createPersonalAccessToken(ctx.input.description, ctx.input.expiresAt);
        return {
          output: {
            createdToken: {
              tokenId: result.id,
