...
let moneyInputSchema = z.object({
  amount: z.number().describe('Amount in smallest currency denomination (e.g., cents for USD)'),
...
export let createPayment = SlateTool.create(
  ...
  .input(z.object({
    ...
    autocomplete: z.boolean().optional().describe('If false, the payment is only authorized and must be completed later. Defaults to true'),
    delayDuration: z.string().optional().describe('Duration to delay capture, e.g., "PT36H" for 36 hours'),
    idempotencyKey: z.string().optional().describe('Unique key to prevent duplicate payments. Auto-generated if omitted'),
  }))
  .output(z.object({
