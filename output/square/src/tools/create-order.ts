...
export let createOrder = SlateTool.create(
  ...
  .input(z.object({
    ...
    lineItems: z.array(z.record(z.string(), z.any())).optional().describe('Line items for the order. Each item should have name, quantity, and base_price_money or catalog_object_id'),
    taxes: z.array(z.record(z.string(), z.any())).optional().describe('Taxes to apply to the order'),
    discounts: z.array(z.record(z.string(), z.any())).optional().describe('Discounts to apply to the order'),
    fulfillments: z.array(z.record(z.string(), z.any())).optional().describe('Fulfillment details (pickup, shipment, or delivery)'),
    ...
    idempotencyKey: z.string().optional().describe('Unique key to prevent duplicate orders. Auto-generated if omitted'),
  }))
  .output(z.object({
