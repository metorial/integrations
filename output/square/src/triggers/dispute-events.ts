...
export let disputeEvents = SlateTrigger.create(
  spec,
  {
    name: 'Dispute Events',
    key: 'dispute_events',
    description: 'Triggered when disputes (chargebacks) are created, evidence is added/removed, or dispute state changes.',
  }
)
  .input(z.object({
    eventType: z.string(),
  ...
  .output(z.object({
    ...
    amountMoney: z.object({ amount: z.number().optional(), currency: z.string().optional() }).optional(),
    createdAt: z.string().optional(),
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: {
          ...
          state: d.state,
          reason: d.reason,
          disputedPaymentId: d.disputed_payment?.payment_id || d.disputed_payments?.[0]?.payment_id,
