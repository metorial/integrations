...
export let addFilesToCollection = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    })).min(1).describe('Files to add to the collection'),
  }))
  .output(z.object({
    addedFiles: z.array(z.object({
      ...
      error: z.string().optional().describe('Error message if the file failed to be added'),
