...
let taskSchema = z.object({
  ...
  notebookTask: z.object({
    notebookPath: z.string().describe('Workspace path to the notebook'),
    baseParameters: z.record(z.string(), z.string()).optional().describe('Base parameters for the notebook'),
  }).optional().describe('Notebook task configuration'),
  sparkPythonTask: z.object({
  ...
  }).optional().describe('Python script task configuration'),
...
export let manageJob = SlateTool.create(
  ...
  .input(z.object({
    ...
    schedule: z.object({
      ...
      pauseStatus: z.enum(['UNPAUSED', 'PAUSED']).optional().describe('Whether the schedule is paused'),
    }).optional().describe('Schedule configuration'),
