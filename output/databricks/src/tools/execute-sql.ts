...
export let executeSql = SlateTool.create(
  ...
  .input(z.object({
    ...
    waitTimeout: z.string().optional().describe('Max wait time (e.g., "50s"). Use "0s" for async.'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Column schema of the result'),
    rows: z.array(z.array(z.string().nullable())).optional().describe('Result data rows (array of arrays)'),
