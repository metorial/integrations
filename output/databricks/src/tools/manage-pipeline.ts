...
export let managePipeline = SlateTool.create(
  ...
  .input(z.object({
    ...
    pipelineId: z.string().optional().describe('Pipeline ID (required for start/stop/delete)'),
    ...
    libraries: z.array(z.object({
      ...
      }).optional().describe('Notebook library'),
    })).optional().describe('Pipeline libraries (required for create)'),
    ...
    configuration: z.record(z.string(), z.string()).optional().describe('Pipeline configuration key-value pairs'),
    fullRefresh: z.boolean().optional().describe('Full refresh when starting (recompute all tables)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      if (!ctx.input.name || !ctx.input.libraries) throw new Error('name and libraries are required for create');
