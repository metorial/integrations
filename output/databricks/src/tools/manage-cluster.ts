...
export let manageCluster = SlateTool.create(
  spec,
  {
    name: 'Manage Cluster',
    key: 'manage_cluster',
...
To **start/restart/stop/delete**, provide \`clusterId\` and the corresponding \`action\`.`,
  ...
  .input(z.object({
    clusterId: z.string().optional().describe('Cluster ID. Required for all actions except create.'),
    action: z.enum(['create', 'edit', 'start', 'restart', 'terminate', 'permanent_delete']).describe('Action to perform on the cluster'),
    clusterName: z.string().optional().describe('Name of the cluster (required for create)'),
    sparkVersion: z.string().optional().describe('Spark runtime version (required for create)'),
    nodeTypeId: z.string().optional().describe('Node type ID for workers (required for create)'),
    ...
    }).optional().describe('Autoscaling configuration'),
    autoterminationMinutes: z.number().optional().describe('Minutes of inactivity before auto-termination'),
    sparkConf: z.record(z.string(), z.string()).optional().describe('Spark configuration key-value pairs'),
    customTags: z.record(z.string(), z.string()).optional().describe('Custom tags for the cluster'),
  }))
  .output(z.object({
