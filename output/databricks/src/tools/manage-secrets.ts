...
export let manageSecrets = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['list_scopes', 'create_scope', 'delete_scope', 'list_secrets', 'put_secret', 'delete_secret']).describe('Action to perform'),
    scope: z.string().optional().describe('Secret scope name (required for most actions)'),
    secretKey: z.string().optional().describe('Secret key name (required for put_secret, delete_secret)'),
    secretValue: z.string().optional().describe('Secret value (required for put_secret)'),
    initialManagePrincipal: z.string().optional().describe('Initial manage principal for scope creation (e.g., "users")'),
  }))
  .output(z.object({
    scopes: z.array(z.object({
      scopeName: z.string().describe('Scope name'),
      backendType: z.string().optional().describe('Backend type (DATABRICKS or AZURE_KEYVAULT)'),
    })).optional().describe('List of secret scopes'),
    secrets: z.array(z.object({
      secretKey: z.string().describe('Secret key name'),
      lastUpdatedTimestamp: z.string().optional().describe('Last update time in epoch ms'),
    })).optional().describe('List of secrets in a scope'),
    ...
    switch (ctx.input.action) {
      ...
      case 'list_secrets': {
        ...
        let mapped = secrets.map((s: any) => ({
          secretKey: s.key,
          lastUpdatedTimestamp: s.last_updated_timestamp ? String(s.last_updated_timestamp) : undefined,
        }));
        return {
