...
export let searchRuns = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    orderBy: z.array(z.string()).optional().describe('Order by clauses (e.g., "metrics.rmse ASC")'),
  ...
  .output(z.object({
    runs: z.array(z.object({
      ...
      status: z.string().optional().describe('Run status (RUNNING, SCHEDULED, FINISHED, FAILED, KILLED)'),
      startTime: z.string().optional().describe('Start time in epoch ms'),
      endTime: z.string().optional().describe('End time in epoch ms'),
      metrics: z.record(z.string(), z.number()).optional().describe('Latest metric values'),
