...
export let managePaymentRequests = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'update', 'archive']).describe('Action to perform'),
    storeId: z.string().describe('Store ID'),
    paymentRequestId: z.string().optional().describe('Payment request ID (for get, update, archive)'),
    title: z.string().optional().describe('Title (required for create, optional for update)'),
    amount: z.number().optional().describe('Amount (required for create, optional for update)'),
    ...
    allowCustomPaymentAmounts: z.boolean().optional().describe('Allow customers to pay custom amounts'),
  }))
  .output(z.object({
    paymentRequest: paymentRequestSchema.optional().describe('Payment request details'),
    paymentRequests: z.array(paymentRequestSchema).optional().describe('List of payment requests'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update') {
      ...
      if (ctx.input.allowCustomPaymentAmounts !== undefined) updateData.allowCustomPaymentAmounts = ctx.input.allowCustomPaymentAmounts;
      let pr = await client.updatePaymentRequest(storeId, ctx.input.paymentRequestId!, updateData);
      return {
        output: { paymentRequest: mapRequest(pr) },
