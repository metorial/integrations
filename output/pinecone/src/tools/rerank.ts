...
export let rerankTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    documents: z.array(z.record(z.string(), z.any())).min(1).describe('Documents to rerank, each with a text field'),
    topN: z.number().int().optional().describe('Number of top results to return (defaults to all)'),
    returnDocuments: z.boolean().optional().describe('Include document content in results (default true)'),
    rankFields: z.array(z.string()).optional().describe('Document fields to consider for ranking (default: ["text"])'),
  }))
  .output(z.object({
    model: z.string().describe('Reranking model used'),
    rankedResults: z.array(z.object({
      originalIndex: z.number().describe('Original index of the document in the input array'),
      relevanceScore: z.number().describe('Relevance score from 0 to 1'),
      document: z.record(z.string(), z.any()).optional().describe('Document content if returnDocuments was true'),
