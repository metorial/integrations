...
export let manageSubscriber = SlateTool.create(
  ...
  .input(z.object({
    email: z.string().describe('The subscriber email address. Required when creating a new subscriber.'),
    newEmail: z.string().optional().describe('A new email address to replace the existing one.'),
    ...
    smsNumber: z.string().optional().describe('SMS-capable phone number, e.g. +16125551212.'),
    smsConsent: z.boolean().optional().describe('Whether the subscriber has given SMS consent.'),
    ...
    removeTags: z.array(z.string()).optional().describe('Tags to remove from the subscriber.'),
    euConsent: z.enum(['granted', 'denied', 'unknown']).optional().describe('GDPR consent status.'),
    euConsentMessage: z.string().optional().describe('The consent message shown to the subscriber.'),
    status: z.enum(['active', 'unsubscribed']).optional().describe('Subscriber status.'),
    initialStatus: z.enum(['active', 'unsubscribed']).optional().describe('Initial status if status should not change.'),
    prospect: z.boolean().optional().describe('Whether the subscriber is a prospect for lead scoring.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.baseLeadScore !== undefined) subscriber['base_lead_score'] = ctx.input.baseLeadScore;
