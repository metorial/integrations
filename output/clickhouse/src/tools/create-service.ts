...
export let createService = SlateTool.create(
  ...
  .input(z.object({
    ...
    ipAccessList: z.array(z.object({
      source: z.string().describe('IP address or CIDR range'),
      description: z.string().optional().describe('Description of this IP entry'),
    })).optional().describe('IP allow list entries'),
    releaseChannel: z.enum(['slow', 'default', 'fast']).optional().describe('Release channel for ClickHouse updates'),
    byocId: z.string().optional().describe('BYOC infrastructure ID to deploy on'),
    tags: z.array(z.object({
      key: z.string(),
      value: z.string(),
    })).optional().describe('Tags to apply to the service'),
  }))
  .output(z.object({
