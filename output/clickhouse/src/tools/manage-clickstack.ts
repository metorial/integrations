...
export let createDashboard = SlateTool.create(
  ...
  .input(z.object({
    ...
    tiles: z.array(z.record(z.string(), z.any())).optional().describe('Dashboard tiles/chart configurations'),
...
export let createAlert = SlateTool.create(
  ...
  .input(z.object({
    ...
    thresholdType: z.enum(['above', 'below']).describe('Trigger when metric is above or below threshold'),
    interval: z.string().describe('Check interval (e.g., 1m, 5m, 15m, 30m, 1h, 6h, 12h, 1d)'),
    source: z.string().optional().describe('Alert source type (saved_search or tile)'),
    channel: z.record(z.string(), z.any()).optional().describe('Notification channel configuration (webhook URL and type)'),
...
export let deleteAlert = SlateTool.create(
  spec,
  {
