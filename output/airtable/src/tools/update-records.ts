...
export let updateRecordsTool = SlateTool.create(
  spec,
  {
    name: 'Update Records',
    key: 'update_records',
  ...
  .input(z.object({
    ...
    records: z.array(z.object({
      ...
      fields: z.record(z.string(), z.any()).describe('Field values to update, keyed by field name'),
    })).min(1).max(10).describe('Array of records to update (max 10)'),
    typecast: z.boolean().optional().describe('Automatically convert string values to appropriate field types'),
    replaceAllFields: z.boolean().optional().describe('If true, performs a full record replacement (unspecified fields will be cleared). Defaults to false (partial update).'),
  }))
  .output(z.object({
    records: z.array(z.object({
      recordId: z.string().describe('Updated record ID'),
      createdTime: z.string().describe('Record creation timestamp'),
      fields: z.record(z.string(), z.any()).describe('Current field values of the updated record'),
    })),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.updateRecords(
