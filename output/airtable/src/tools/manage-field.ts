...
export let manageFieldTool = SlateTool.create(
  spec,
  {
    name: 'Manage Field',
    key: 'manage_field',
  ...
  .input(z.object({
    action: z.enum(['create', 'update']).describe('Whether to create a new field or update an existing one'),
    ...
    fieldType: z.string().optional().describe('Field type (required for create; e.g. singleLineText, number, singleSelect)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      let result = await client.updateField(ctx.input.tableIdOrName, ctx.input.fieldId, updates);
