...
export let manageCommentTool = SlateTool.create(
  spec,
  {
    name: 'Manage Comment',
    key: 'manage_comment',
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'update', 'delete']).describe('The comment action to perform'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of comments (returned for list action)'),
    comment: z.object({
      commentId: z.string().describe('Comment ID'),
      text: z.string().describe('Comment text'),
      authorId: z.string().describe('Author user ID'),
      ...
      authorName: z.string().optional().describe('Author display name'),
      createdTime: z.string().describe('Comment creation timestamp'),
    }).optional().describe('The created or updated comment'),
    deleted: z.boolean().optional().describe('Whether the comment was deleted (returned for delete action)'),
    offset: z.string().optional().describe('Pagination offset for the next page (returned for list action)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      let result = await client.createComment(ctx.input.tableIdOrName, ctx.input.recordId, ctx.input.text);
    ...
    if (ctx.input.action === 'update') {
      ...
      let result = await client.updateComment(ctx.input.tableIdOrName, ctx.input.recordId, ctx.input.commentId, ctx.input.text);
    ...
    await client.deleteComment(ctx.input.tableIdOrName, ctx.input.recordId, ctx.input.commentId);
