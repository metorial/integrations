...
export let listWorkspaceResources = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    resourceType: z.enum(['channels', 'users']).describe('Type of workspace resource to list'),
  }))
  .output(z.object({
    channels: z.array(z.object({
      channelId: z.string().describe('Channel ID'),
      channelName: z.string().optional().describe('Channel name'),
      channelType: z.string().optional().describe('Channel type (whatsapp, messenger, telegram, etc.)'),
    })).optional().describe('Connected channels (when resourceType is "channels")'),
    ...
    })).optional().describe('Workspace users (when resourceType is "users")'),
  }))
