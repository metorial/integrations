...
export let sendTemplateMessage = SlateTool.create(
  ...
  .input(z.object({
    ...
    templateLanguage: z.string().optional().describe('Language code for the template (e.g. en, en_US)'),
    templateParameters: z.array(z.any()).optional().describe('Template parameter values in order'),
    templateComponents: z.array(z.any()).optional().describe('Template components (header, body, buttons) with their parameters'),
  }))
  .output(z.object({
