...
let filterConditionSchema = z.object({
  field: z.string().describe('Contact field name to filter on (e.g. firstName, email, phone, tag)'),
...
export let listContacts = SlateTool.create(
  ...
  .input(z.object({
    ...
    }).optional().describe('Filter conditions for narrowing results'),
    limit: z.number().min(1).max(100).optional().describe('Maximum number of contacts to return (1-100, default: 10)'),
    cursorId: z.string().optional().describe('Cursor ID for pagination from a previous response'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.listContacts(ctx.input.filters, ctx.input.limit, ctx.input.cursorId);
