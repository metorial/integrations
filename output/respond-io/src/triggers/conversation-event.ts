...
export let conversationEvent = SlateTrigger.create(
  ...
  .input(z.object({
    eventType: z.string().describe('Type of conversation event (conversation.opened or conversation.closed)'),
    contactId: z.string().describe('ID of the contact'),
    conversationStatus: z.string().optional().describe('Conversation status (open or closed)'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let normalizedEvent = event.includes('closed') ? 'conversation.closed' : 'conversation.opened';
