...
export let commentEvent = SlateTrigger.create(
  spec,
  {
    name: 'Comment Event',
    key: 'comment_event',
    description: 'Triggers when a new comment (internal note) is added to a contact conversation.',
  }
)
  .input(z.object({
    ...
    handleRequest: async (ctx) => {
      ...
      return {
        inputs: [{
          ...
          authorName: author?.name || (author?.firstName ? `${author.firstName} ${author.lastName || ''}`.trim() : undefined),
