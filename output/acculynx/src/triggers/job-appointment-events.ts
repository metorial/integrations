...
export let jobAppointmentEventsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Job Appointment Events',
    key: 'job_appointment_events',
    description: 'Triggered when an initial appointment is created or updated for a job in AccuLynx.',
  }
)
  .input(z.object({
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      let inputs = events.map((event: any) => ({
        topicName: event.topicName ?? event.topic ?? event.type ?? 'job.appointments.initial_created',
    ...
    handleEvent: async (ctx) => {
      ...
      let type = topicName === 'job.appointments.initial_created'
