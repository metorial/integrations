...
export let getCompanySettingsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    includeInsuranceCompanies: z.boolean().optional().describe('Include insurance companies list'),
  ...
  .output(z.object({
    ...
    insuranceCompanies: z.array(z.record(z.string(), z.any())).optional().describe('Insurance companies'),
    jobCategories: z.array(z.record(z.string(), z.any())).optional().describe('Job categories'),
    ...
    documentFolders: z.array(z.record(z.string(), z.any())).optional().describe('Document folders'),
    photoVideoTags: z.array(z.record(z.string(), z.any())).optional().describe('Photo/video tags'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let noSpecificSelection = !ctx.input.includeGeneral && !ctx.input.includeInsuranceCompanies
      && !ctx.input.includeJobCategories && !ctx.input.includeTradeTypes && !ctx.input.includeWorkTypes
      && !ctx.input.includeLeadSources && !ctx.input.includeDocumentFolders
      && !ctx.input.includePhotoVideoTags && !ctx.input.includeAccountTypes;
    ...
    let toArray = (result: any) => Array.isArray(result) ? result : (result?.items ?? result?.data ?? []);
