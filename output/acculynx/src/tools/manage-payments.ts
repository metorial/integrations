...
export let managePaymentsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    paymentType: z.enum(['received', 'paid', 'additional_expense']).optional().describe('Type of payment to record. Omit to view existing payments.'),
    amount: z.number().optional().describe('Payment amount (required when creating a payment)'),
    date: z.string().optional().describe('Payment date in YYYY-MM-DD format'),
    paymentMethod: z.string().optional().describe('Payment method (for received/paid payments)'),
    referenceNumber: z.string().optional().describe('Reference or check number (for received/paid payments)'),
    accountTypeId: z.string().optional().describe('Account type ID'),
    description: z.string().optional().describe('Description or notes'),
  }))
  .output(z.object({
    payments: z.array(z.record(z.string(), z.any())).optional().describe('List of existing payments'),
    overview: z.record(z.string(), z.any()).optional().describe('Payment overview/summary'),
    createdPayment: z.record(z.string(), z.any()).optional().describe('The newly created payment record'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let payments = Array.isArray(paymentsResult) ? paymentsResult : (paymentsResult?.items ?? paymentsResult?.data ?? []);
