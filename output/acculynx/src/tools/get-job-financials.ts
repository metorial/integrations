...
export let getJobFinancialsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    jobId: z.string().optional().describe('Job ID to retrieve financial summary and accounting status for'),
    financialId: z.string().optional().describe('Financial ID to retrieve detailed financial data'),
    worksheetId: z.string().optional().describe('Worksheet ID to retrieve a specific worksheet (requires financialId)'),
    includeAmendments: z.boolean().optional().describe('Include financial amendments (requires financialId)'),
    includeAccountingStatus: z.boolean().optional().describe('Include accounting integration sync status (requires jobId)'),
  }))
  .output(z.object({
    ...
    amendments: z.array(z.record(z.string(), z.any())).optional().describe('Financial amendments'),
    accountingStatus: z.record(z.string(), z.any()).optional().describe('Accounting integration sync status'),
  }))
