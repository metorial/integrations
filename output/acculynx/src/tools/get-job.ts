...
export let getJobTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeMilestone: z.boolean().optional().describe('Include current milestone information'),
    includeRepresentatives: z.boolean().optional().describe('Include assigned representatives (company rep, sales owner, A/R owner)'),
    includeInsurance: z.boolean().optional().describe('Include insurance and adjuster details'),
  ...
  .output(z.object({
    ...
    milestone: z.record(z.string(), z.any()).optional().describe('Current milestone information'),
    representatives: z.record(z.string(), z.any()).optional().describe('Assigned representatives'),
    insurance: z.record(z.string(), z.any()).optional().describe('Insurance details'),
    adjuster: z.record(z.string(), z.any()).optional().describe('Insurance adjuster details'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includeMilestone) {
      try { milestone = await client.getJobCurrentMilestone(jobId); } catch (e) { /* optional */ }
    }
    if (ctx.input.includeRepresentatives) {
      try { representatives = await client.getJobRepresentatives(jobId); } catch (e) { /* optional */ }
    }
