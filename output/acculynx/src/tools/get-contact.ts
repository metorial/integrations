...
export let getContactTool = SlateTool.create(
  ...
  .output(z.object({
    ...
    emailAddresses: z.array(z.record(z.string(), z.any())).optional().describe('Contact email addresses'),
    phoneNumbers: z.array(z.record(z.string(), z.any())).optional().describe('Contact phone numbers'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    try {
      ...
      emailAddresses = Array.isArray(emailResult) ? emailResult : (emailResult?.items ?? emailResult?.data ?? []);
    } catch (e) {
      emailAddresses = [];
    }
    ...
    try {
      ...
      phoneNumbers = Array.isArray(phoneResult) ? phoneResult : (phoneResult?.items ?? phoneResult?.data ?? []);
    } catch (e) {
      phoneNumbers = [];
    }
