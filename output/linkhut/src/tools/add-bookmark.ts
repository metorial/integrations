...
export let addBookmark = SlateTool.create(
  spec,
  {
    name: 'Add Bookmark',
    key: 'add_bookmark',
  ...
  .input(z.object({
    ...
    timestamp: z.string().optional().describe('ISO 8601 datetime for the bookmark (e.g. 2024-01-15T10:30:00Z)'),
    replace: z.boolean().optional().describe('Whether to replace an existing bookmark at the same URL (default: true)'),
    private: z.boolean().optional().describe('Whether the bookmark should be private (default: false)'),
    readLater: z.boolean().optional().describe('Whether to mark the bookmark as unread/read-later (default: false)'),
  }))
  .output(z.object({
