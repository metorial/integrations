...
export let showModel = SlateTool.create(
  spec,
  {
    name: 'Show Model',
    key: 'show_model',
  ...
  .input(z.object({
    modelName: z.string().describe('Name of the model to show (e.g., "llama3.2", "gemma3:latest").'),
    verbose: z.boolean().optional().describe('Include verbose model info with additional metadata fields.'),
  }))
  .output(z.object({
    ...
    modelInfo: z.record(z.string(), z.unknown()).optional().describe('Additional verbose model metadata.'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let sizeInfo = result.details.parameterSize ? `, ${result.details.parameterSize} parameters` : '';
