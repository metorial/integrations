...
export let createArtifactUrl = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Public download URLs are accessible to anyone with the link.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    artifactUrl: z.string().describe('The authenticated artifact download URL from the build response'),
    expiresAt: z.number().optional().describe('Expiration time as a UNIX timestamp in seconds'),
  }))
  .output(z.object({
