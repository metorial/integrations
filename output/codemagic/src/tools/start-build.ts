...
export let startBuild = SlateTool.create(
  ...
  .input(z.object({
    ...
    labels: z.array(z.string()).optional().describe('Labels to attach to the build for identification'),
    environmentVariables: z.record(z.string(), z.string()).optional().describe('Custom environment variables as key-value pairs'),
    variableGroups: z.array(z.string()).optional().describe('Names of variable groups to include'),
    softwareVersions: z.object({
    ...
    }).optional().describe('Software version overrides'),
    instanceType: z.string().optional().describe('Machine type, e.g. "mac_mini_m2", "mac_pro_m2", "linux_x2"'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.environmentVariables || ctx.input.variableGroups || ctx.input.softwareVersions) {
      environment = {};
      if (ctx.input.environmentVariables) environment.variables = ctx.input.environmentVariables;
      ...
      if (ctx.input.softwareVersions) {
        ...
        if (ctx.input.softwareVersions.flutter) sv.flutter = ctx.input.softwareVersions.flutter;
        if (ctx.input.softwareVersions.cocoapods) sv.cocoapods = ctx.input.softwareVersions.cocoapods;
