...
export let listSmartContracts = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    contractAddress: z.string().optional().describe('Specific contract address to get details for (requires network)'),
    includeFunctions: z.boolean().default(false).describe('Whether to include available functions for a specific contract'),
  ...
  .output(z.object({
    ...
    availableFunctions: z.array(z.any()).optional().describe('Available functions if requested for a specific contract'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.contractAddress && ctx.input.network) {
      let contract = await client.getSmartContract(ctx.input.network, ctx.input.contractAddress);
      let functions: any[] | undefined;

      if (ctx.input.includeFunctions) {
        functions = await client.getAvailableFunctions(ctx.input.network, ctx.input.contractAddress);
      }
