...
export let manageWatchers = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['create', 'list', 'get', 'update', 'delete']).describe('Watcher management action'),
    watcherId: z.string().optional().describe('Watcher ID (required for get, update, delete)'),
    ...
    confirmationsBlocks: z.number().default(12).describe('Block confirmations before triggering (for create)'),
    customEventAbi: z.any().optional().describe('Custom event ABI for EVENT_CUSTOM type (for create)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      if (!ctx.input.address || !ctx.input.network || !ctx.input.watcherType || !ctx.input.webhookUrl) {
        throw new Error('address, network, watcherType, and webhookUrl are required for creating a watcher');
      }
