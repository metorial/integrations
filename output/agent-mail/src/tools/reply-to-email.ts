...
let attachmentSchema = z.object({
  ...
  url: z.string().optional().describe('URL to fetch the attachment from (alternative to content)'),
});
...
export let replyToEmail = SlateTool.create(
  ...
  .input(z.object({
    ...
    replyAll: z.boolean().optional().describe('Reply to all recipients of the original message'),
