...
export let createInbox = SlateTool.create(
  spec,
  {
    name: 'Create Inbox',
    key: 'create_inbox',
  ...
  .input(z.object({
    username: z.string().optional().describe('Username for the inbox email address. Auto-generated if not provided'),
    domain: z.string().optional().describe('Domain for the inbox. Defaults to agentmail.to. Custom domains require a paid plan'),
    displayName: z.string().optional().describe('Display name in format "Display Name <username@domain.com>"'),
    clientId: z.string().optional().describe('Client-side identifier for idempotent creation'),
  }))
  .output(z.object({
