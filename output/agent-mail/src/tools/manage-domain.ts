...
Custom domains require a paid plan. After creating, configure the DNS records and then verify.`,
  ...
  .input(z.object({
    ...
    domain: z.string().optional().describe('Domain name like "example.com" (required for create)'),
    feedbackEnabled: z.boolean().optional().describe('Enable bounce/complaint notifications (for create)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      let domain = await client.createDomain(ctx.input.domain, ctx.input.feedbackEnabled ?? true);
