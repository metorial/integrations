...
export let manageTaskAssignees = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    taskId: z.string().optional().describe('ID of the specific task (omit to manage workflow run assignees)'),
    action: z.enum(['list', 'assign', 'unassign']).describe('Action to perform'),
    email: z.string().optional().describe('Email of the user to assign or unassign (required for assign/unassign)'),
  }))
  .output(z.object({
    assignees: z.array(assigneeSchema).optional().describe('List of current assignees (for list action)'),
