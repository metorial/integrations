...
export let uploadPresentation = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    fileName: z.string().describe('Name of the file including extension, e.g. "Quarterly Report.pptx"'),
    mode: z.enum(['simple', 'session']).default('simple').describe('Upload mode: "simple" for small files with content provided, "session" to create a resumable upload session for large files.'),
    fileContent: z.string().optional().describe('Base64-encoded file content. Required for "simple" mode.'),
    fileSize: z.number().optional().describe('Total file size in bytes. Required for "session" mode.'),
    contentType: z.string().optional().describe('MIME type of the file. Defaults to PowerPoint PPTX format.'),
    parentFolderId: z.string().optional().describe('ID of the destination folder. Omit to upload to the drive root.'),
    parentFolderPath: z.string().optional().describe('Path of the destination folder, e.g. "/Documents/Presentations". Omit to upload to the drive root.'),
    driveId: z.string().optional().describe('ID of the target drive. Omit to use the current user\'s OneDrive.'),
    siteId: z.string().optional().describe('SharePoint site ID. If provided, uploads to the site\'s default document library.'),
    conflictBehavior: z.enum(['rename', 'replace', 'fail']).optional().describe('Behavior when a file with the same name exists. Default: "rename".'),
  }))
  .output(z.object({
    uploadedFile: driveItemOutputSchema.optional().describe('Metadata of the uploaded file (for simple upload)'),
    uploadUrl: z.string().optional().describe('Resumable upload URL (for session mode)'),
    uploadExpiration: z.string().optional().describe('Expiration time of the upload session (for session mode)'),
  }))
