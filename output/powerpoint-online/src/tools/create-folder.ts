...
export let createFolder = SlateTool.create(
  ...
  .input(z.object({
    ...
    parentFolderId: z.string().optional().describe('ID of the parent folder. Omit to create in the drive root.'),
    parentFolderPath: z.string().optional().describe('Path of the parent folder, e.g. "/Documents". Omit to create in the drive root.'),
    driveId: z.string().optional().describe('ID of the target drive.'),
    siteId: z.string().optional().describe('SharePoint site ID.'),
    conflictBehavior: z.enum(['rename', 'replace', 'fail']).optional().describe('What to do if a folder with the same name exists. Default: "rename".'),
  }))
