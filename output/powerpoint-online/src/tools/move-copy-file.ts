...
export let moveCopyFile = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    itemId: z.string().optional().describe('ID of the file or folder to move/copy. Provide either itemId or itemPath.'),
    ...
    destinationDriveId: z.string().optional().describe('ID of the destination drive (for cross-drive operations).'),
    newName: z.string().optional().describe('New name for the file at the destination.'),
  }))
  .output(z.object({
    movedFile: driveItemOutputSchema.optional().describe('Metadata of the moved file (move operations only)'),
    monitorUrl: z.string().optional().describe('URL to monitor copy progress (copy operations only)'),
  }))
