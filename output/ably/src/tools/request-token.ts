...
export let requestToken = SlateTool.create(
  spec,
  {
    name: 'Request Token',
    key: 'request_token',
...
Use this to generate tokens for client-side applications instead of exposing your API key.`,
  ...
  .input(z.object({
    keyName: z.string().describe('The key name portion of the API key (appId.keyId format, e.g. "I2E_JQ.OqUdfg")'),
    ttl: z.number().optional().describe('Time to live in milliseconds. Default is 60 minutes (3600000 ms). Max is 24 hours.'),
    capability: z.record(z.string(), z.array(z.string())).optional().describe('Capabilities map: channel name/pattern -> array of operations (publish, subscribe, presence, history, push-subscribe, push-admin, channel-metadata, statistics)'),
    clientId: z.string().optional().describe('Client ID to bind to the token. When set, the token can only be used by this client.'),
  }))
  .output(z.object({
