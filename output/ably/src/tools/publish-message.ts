...
Use batch mode by providing multiple channel-message pairs for efficient multi-channel publishing.`,
  ...
  .input(z.object({
    channelId: z.string().optional().describe('Channel name to publish to. Required for single message publishing.'),
    name: z.string().optional().describe('Event name for the message'),
    messageData: z.any().optional().describe('Message payload (string, JSON object, or binary data)'),
    clientId: z.string().optional().describe('Client ID to associate with the message'),
    messageId: z.string().optional().describe('Unique message ID for idempotent publishing. If not provided, Ably generates one.'),
    extras: z.record(z.string(), z.any()).optional().describe('Extra message metadata, e.g. push notification configuration'),
    batch: z.array(z.object({
      channels: z.union([z.string(), z.array(z.string())]).describe('One or more channel names to publish to'),
      messages: z.union([
        z.object({
    ...
    })).optional().describe('Batch publish specs. If provided, channelId and other single-message fields are ignored.'),
  }))
  .output(z.object({
    channel: z.string().optional().describe('Channel the message was published to (single mode)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!ctx.input.channelId) {
      throw new Error('channelId is required for single message publishing. Use batch array for batch publishing.');
    }
