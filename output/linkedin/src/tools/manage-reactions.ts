...
let reactionTypeEnum = z.enum(['LIKE', 'PRAISE', 'EMPATHY', 'INTEREST', 'APPRECIATION', 'ENTERTAINMENT', 'MAYBE'])
  .describe('Type of reaction');

export let createReaction = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    ...
    await client.createReaction(ctx.input.postUrn, ctx.input.reactionType);

    return {
      output: { reacted: true },
      message: `Added **${ctx.input.reactionType}** reaction to post \`${ctx.input.postUrn}\`.`,
    };
  })
  .build();

export let getReactions = SlateTool.create(
  spec,
  {
    name: 'Get Reactions',
    key: 'get_reactions',
  ...
  .output(z.object({
    reactions: z.array(z.object({
      ...
      createdAt: z.number().optional().describe('Timestamp when the reaction was created (epoch ms)'),
    })),
...
export let deleteReaction = SlateTool.create(
  spec,
  {
