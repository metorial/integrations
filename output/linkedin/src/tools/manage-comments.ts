...
export let createComment = SlateTool.create(
  ...
  .input(z.object({
    ...
    actorUrn: z.string().describe('URN of the member creating the comment, e.g. "urn:li:person:abc123"'),
    text: z.string().describe('Comment text content'),
    parentCommentUrn: z.string().optional().describe('URN of the parent comment if this is a reply'),
  }))
  .output(z.object({
    ...
    createdAt: z.number().optional().describe('Timestamp when the comment was created (epoch ms)'),
  }))
