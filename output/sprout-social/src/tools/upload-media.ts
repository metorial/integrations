...
export let uploadMedia = SlateTool.create(
  ...
  .input(z.object({
    mediaUrl: z.string().describe('Publicly accessible HTTP/HTTPS URL of the media file to upload.')
  }))
  .output(z.object({
    mediaId: z.string().describe('Sprout Social media ID for use in publishing posts.'),
    expirationTime: z.string().describe('ISO 8601 timestamp when the media will expire if unused.')
  }))
