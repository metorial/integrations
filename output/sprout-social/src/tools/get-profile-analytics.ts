...
export let getProfileAnalytics = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    profileIds: z.array(z.number()).describe('Array of customer_profile_id values to retrieve analytics for.'),
    startDate: z.string().describe('Start date for the reporting period in YYYY-MM-DD format.'),
    endDate: z.string().describe('End date for the reporting period in YYYY-MM-DD format.'),
    metrics: z.array(z.string()).describe('Metrics to retrieve (e.g., "impressions", "reactions", "followers_gained", "engagements", "video_views").'),
    page: z.number().optional().describe('Page number for pagination (1-indexed).')
  }))
  .output(z.object({
    analyticsData: z.array(z.object({
      dimensions: z.any().describe('Dimension values (profile ID and reporting period).'),
