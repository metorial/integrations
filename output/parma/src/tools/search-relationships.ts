...
export let searchRelationships = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    query: z.string().optional().describe('Search query to filter relationships by name, email, company, etc.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let totalCount = Array.isArray(result) ? items.length : (result.total ?? result.total_count ?? items.length);
