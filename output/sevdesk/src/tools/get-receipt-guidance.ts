...
export let getReceiptGuidance = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    forExpense: z.boolean().optional().describe('Get guidance for expenses/incoming vouchers'),
    forRevenue: z.boolean().optional().describe('Get guidance for revenue/outgoing invoices'),
  }))
  .output(z.object({
    guidance: z.any().describe('Receipt guidance data with valid combinations of tax rules, tax rates, and booking accounts'),
  }))
