...
let voucherPositionSchema = z.object({
...
export let createVoucher = SlateTool.create(
  ...
  .input(z.object({
    ...
    creditDebit: z.enum(['C', 'D']).optional().describe('C=Credit (incoming), D=Debit (outgoing)'),
    voucherType: z.enum(['VOU', 'RV']).optional().describe('VOU=Voucher (default), RV=Recurring voucher'),
