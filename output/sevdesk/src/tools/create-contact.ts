...
let communicationWaySchema = z.object({
  type: z.enum(['EMAIL', 'PHONE', 'WEB', 'MOBILE', 'FAX']).describe('Type of communication way'),
  value: z.string().describe('The value (e.g. email address, phone number, URL)'),
  key: z.enum(['1', '2', '3', '4', '5']).optional().describe('Key: 1=Private, 2=Work, 3=Fax, 4=Mobile, 5=Newsletter/Invoice'),
...
let addressSchema = z.object({
  ...
  country: z.string().optional().describe('Country ID (sevDesk country ID, e.g. 1 for Germany)'),
...
export let createContact = SlateTool.create(
  ...
  .input(z.object({
    contactType: z.enum(['person', 'company']).describe('Whether this is a person or company contact'),
    familyName: z.string().optional().describe('Family/last name (required for person contacts)'),
    firstName: z.string().optional().describe('First/given name (for person contacts)'),
    companyName: z.string().optional().describe('Company name (required for company contacts)'),
    categoryId: z.string().describe('Category ID: 3=Customer, 2=Supplier, etc.'),
    customerNumber: z.string().optional().describe('Custom customer number. If omitted, auto-generated.'),
    ...
    gender: z.enum(['m', 'w', '']).optional().describe('Gender: m=male, w=female, empty=not specified'),
    academicTitle: z.string().optional().describe('Academic title (e.g. Dr., Prof.)'),
    parentContactId: z.string().optional().describe('ID of parent contact (for linking person to company)'),
    addresses: z.array(addressSchema).optional().describe('Addresses to add to the contact'),
    communicationWays: z.array(communicationWaySchema).optional().describe('Communication ways to add (email, phone, etc.)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let displayName = contact.name || [ctx.input.firstName, ctx.input.familyName].filter(Boolean).join(' ') || ctx.input.companyName || '';
    ...
    if (ctx.input.addresses?.length) {
      for (let addr of ctx.input.addresses) {
        await client.createContactAddress({
          ...
          zip: addr.zip,
          city: addr.city,
          country: addr.country ? { id: addr.country, objectName: 'StaticCountry' } : undefined,
