...
export let initiateCall = SlateTool.create(
  spec,
  {
    name: 'Initiate Call',
    key: 'initiate_call',
...
Calls can optionally target a specific department.`,
  ...
  .input(z.object({
    ...
    mode: z.enum(['simple', 'call_or_schedule']).default('call_or_schedule').describe('Call mode: "simple" for immediate call, "call_or_schedule" for automatic fallback to scheduling'),
