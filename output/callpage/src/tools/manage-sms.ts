...
export let listSmsTemplates = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    templates: z.array(z.any()).describe('List of SMS template objects with enabled status and text content')
  }))
...
export let upsertSmsTemplate = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'SMS text must not exceed 240 characters.'
    ],
    tags: {
      destructive: false
    }
  ...
  .input(z.object({
    ...
    text: z.string().describe('SMS text content (max 240 characters, supports :company_name, :name, :tel, :url variables)'),
    action: z.enum(['create', 'update']).default('create').describe('Whether to create a new template or update an existing one')
  }))
  .output(z.object({
...
export let resetSmsTemplate = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    widgetId: z.number().describe('The widget ID'),
    messageId: smsMessageIdEnum.optional().describe('Specific template type to reset (omit to reset all)')
  }))
  .output(z.object({
