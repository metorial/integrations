...
export let compareTexts = SlateTool.create(
  spec,
  {
    name: 'Compare Texts',
    key: 'compare_texts',
...
Useful for checking if two texts share common content, detecting paraphrasing, or verifying text originality between documents.`,
  ...
  .input(z.object({
    ...
    secondText: z.string().describe('The second text to compare against the first (up to 120,000 characters)'),
  }))
  .output(z.object({
    similarityScore: z.number().describe('Overall similarity score between the two texts (0-100)'),
    firstTextAnalysis: z.object({
      totalWordCount: z.number().describe('Total words in the first text'),
      matchingWordCount: z.number().describe('Words in the first text matching the second'),
      similarityPercentage: z.number().describe('Percentage of the first text that matches'),
      matchSegments: z.array(z.object({
    ...
    secondTextAnalysis: z.object({
      totalWordCount: z.number().describe('Total words in the second text'),
      matchingWordCount: z.number().describe('Words in the second text matching the first'),
      similarityPercentage: z.number().describe('Percentage of the second text that matches'),
      matchSegments: z.array(z.object({
        type: z.string().describe('Type of match segment'),
        wordCount: z.number().describe('Number of words in this segment'),
        startIndex: z.number().describe('Starting character index of this segment'),
