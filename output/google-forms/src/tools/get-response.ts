...
let answerSchema = z.object({
  questionId: z.string().optional().describe('ID of the question this answer corresponds to'),
...
Useful for inspecting a specific submission in detail.`,
  ...
  .output(z.object({
    ...
    createTime: z.string().optional().describe('Timestamp when the response was first submitted (ISO 8601)'),
    lastSubmittedTime: z.string().optional().describe('Timestamp of the most recent submission (ISO 8601)'),
    respondentEmail: z.string().optional().describe('Email address of the respondent, if collected'),
