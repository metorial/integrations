...
export let manageDataStoreRecord = SlateTool.create(
  spec,
  {
...
Requires the **Data Store API Key** (found under Data Sources > choose a Data Store > "Use Our API").`,
  ...
  .input(z.object({
    ...
    action: z.enum(['add', 'delete_by_id', 'delete_by_match']).describe('The action to perform on the Data Store.'),
    fields: z.record(z.string(), z.any()).optional().describe('Key-value pairs of field names and their values for adding a record.'),
    recordId: z.string().optional().describe('The record ID to delete (required for delete_by_id action).'),
    matchCriteria: z.record(z.string(), z.any()).optional().describe('Field name/value pairs to match for deletion (required for delete_by_match action).'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'add') {
      ...
      result = await client.deleteDataStoreRecordById(ctx.input.dataStoreApiKey, ctx.input.recordId);
      return {
        output: { success: true, result },
      ...
      result = await client.deleteDataStoreRecord(ctx.input.dataStoreApiKey, ctx.input.matchCriteria);
      return {
        output: { success: true, result },
