...
export let getWidgetStats = SlateTool.create(
  spec,
  {
...
Can list all widgets, get aggregated stats, list users per widget, or get per-user stats and images.`,
  ...
  .input(z.object({
    widgetKey: z.string().optional().describe('The widget key. If not provided, lists all widgets or returns aggregated stats.'),
    scope: z.enum(['widgets', 'stats', 'users', 'images']).default('widgets').describe('What to retrieve: "widgets" lists all widgets, "stats" gets aggregated stats, "users" lists users for a widget, "images" lists images.'),
    userId: z.string().optional().describe('A specific user identifier to get per-user stats or images (requires widgetKey).'),
    startDate: z.string().optional().describe('Start date for filtering in ISO 8601 format (e.g. 2024-01-01T00:00Z).'),
    endDate: z.string().optional().describe('End date for filtering in ISO 8601 format (e.g. 2024-12-31T23:59Z).'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.scope === 'stats' || ctx.input.scope === 'widgets') {
      if (ctx.input.userId) {
        let stats = await client.getWidgetUserStats(ctx.input.widgetKey, ctx.input.userId, dateParams);
        return {
          output: { stats },
    ...
    if (ctx.input.scope === 'images') {
      if (ctx.input.userId) {
        let images = await client.getWidgetUserImages(ctx.input.widgetKey, ctx.input.userId, dateParams);
        return {
          output: { images },
