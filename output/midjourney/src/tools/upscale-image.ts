...
export let upscaleImage = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'When using imageUrl directly, the image must not be larger than 2048x2048.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(
    z.object({
      parentTaskId: z.string().optional().describe('Task ID of a previous generation to upscale from'),
      imageUrl: z.string().optional().describe('Direct URL of an image to upscale (alternative to parentTaskId)'),
      scale: z.enum(['2x', '4x']).describe('Upscale factor: "2x" or "4x"'),
      index: z.enum(['1', '2', '3', '4']).optional().describe('Image index (1-4) when upscaling from a grid generation task'),
      waitForResult: z.boolean().optional().default(false).describe('If true, polls until upscale completes and returns the image URL'),
    })
  )
  .output(
    z.object({
