...
export let createDiscountTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    amount: z.number().describe('Discount amount (percentage or cents depending on amountType)'),
    amountType: z.enum(['percent', 'fixed']).describe('Whether the amount is a percentage or fixed value in cents'),
    variantIds: z.array(z.string()).optional().describe('Restrict discount to specific variant IDs'),
    maxRedemptions: z.number().optional().describe('Maximum number of times this discount can be redeemed'),
    startsAt: z.string().optional().describe('ISO 8601 date when the discount becomes active'),
    expiresAt: z.string().optional().describe('ISO 8601 date when the discount expires'),
    duration: z.enum(['once', 'repeating', 'forever']).optional().describe('How long the discount applies to subscriptions'),
    durationInMonths: z.number().optional().describe('Number of months for "repeating" duration'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let response = await client.createDiscount(storeId, {
      name: ctx.input.name,
      code: ctx.input.code,
      amount: ctx.input.amount,
      amountType: ctx.input.amountType,
      isLimitedToProducts: ctx.input.variantIds && ctx.input.variantIds.length > 0 ? true : undefined,
