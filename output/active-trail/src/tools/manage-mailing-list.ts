...
export let listMailingLists = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.listMailingLists({ page: ctx.input.page, limit: ctx.input.limit });
...
export let getMailingListMembers = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    status: z.enum(['Active', 'Unsubscribed', 'Bounced']).optional().describe('Filter by member status'),
...
export let addContactToMailingList = SlateTool.create(
  spec,
  {
