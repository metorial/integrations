...
export let listWhatsAppTemplates = SlateTool.create(
  spec,
  {
...
export let sendWhatsAppCampaign = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    campaignData: z.record(z.string(), z.any()).describe('Full WhatsApp campaign payload as required by the ActiveTrail API'),
  }))
  .output(z.object({
...
export let sendWhatsAppOperationalMessage = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    messageData: z.record(z.string(), z.any()).describe('Full WhatsApp operational message payload as required by the ActiveTrail API'),
  }))
  .output(z.object({
