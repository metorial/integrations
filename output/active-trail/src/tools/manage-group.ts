...
export let getGroup = SlateTool.create(
  ...
  .input(z.object({
    ...
    memberStatus: z.enum(['Active', 'Unsubscribed', 'Bounced']).optional().describe('Filter members by status'),
...
export let addContactToGroup = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    status: z.enum(['Active', 'Unsubscribed']).optional().describe('Subscription status for this group'),
  }))
  .output(z.object({
...
export let removeContactFromGroup = SlateTool.create(
  spec,
  {
