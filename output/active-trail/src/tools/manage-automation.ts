...
export let listAutomations = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.listAutomations({ page: ctx.input.page, limit: ctx.input.limit });
...
export let toggleAutomation = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    let client = new ActiveTrailClient(ctx.auth.token);
    await client.setAutomationActivation(ctx.input.automationId, { is_active: ctx.input.isActive });
    return {
      output: {
        automationId: ctx.input.automationId,
...
export let deleteAutomation = SlateTool.create(
  spec,
  {
...
export let getAutomationReport = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    includeContactsStarted: z.boolean().optional().describe('Include contacts who started the automation'),
    includeContactsEnded: z.boolean().optional().describe('Include contacts who completed the automation'),
  ...
  .output(z.object({
    ...
    contactsStarted: z.array(z.any()).optional().describe('Contacts who started the automation'),
    contactsEnded: z.array(z.any()).optional().describe('Contacts who completed the automation'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new ActiveTrailClient(ctx.auth.token);
    ...
    if (ctx.input.includeContactsStarted) {
      output.contactsStarted = await client.getAutomationContactsStarted(automationId, { page, limit });
    }
    if (ctx.input.includeContactsEnded) {
      output.contactsEnded = await client.getAutomationContactsEnded(automationId, { page, limit });
    }
