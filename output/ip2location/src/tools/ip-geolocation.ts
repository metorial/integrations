...
let proxySchema = z.object({
  ...
  isConsumerPrivacyNetwork: z.boolean().describe('Whether the IP is a consumer privacy network'),
  isEnterprisePrivateNetwork: z.boolean().describe('Whether the IP is an enterprise private network'),
...
}).optional().describe('Proxy and threat detection data (available on Plus and Security plans)');
...
}).optional().describe('Detailed timezone information');
...
Available data varies by plan tier - higher plans include proxy detection, continent/country details with translations, and mobile carrier data.`,
  ...
  .output(z.object({
    ...
    usageType: z.string().optional().describe('IP usage type (e.g., Commercial, ISP, Education)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let locationParts = [output.cityName, output.regionName, output.countryName].filter(Boolean);
    let locationStr = locationParts.join(', ');
    let proxyStr = output.isProxy ? ` | **Proxy detected** (type: ${output.proxy?.proxyType || 'unknown'}, fraud score: ${output.proxy?.fraudScore ?? 'N/A'})` : ' | No proxy detected';
