...
export let createAnalyticsExport = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    timezone: z.string().optional().describe('Timezone for the export (e.g., America/New_York)'),
    type: z.string().describe('Export type (e.g., messages, conversations, tags)'),
    filters: z.record(z.string(), z.any()).optional().describe('Additional filters for the export'),
  }))
  .output(z.object({
    exportId: z.string(),
    status: z.string(),
...
export let getAnalyticsExport = SlateTool.create(
  spec,
  {
...
export let createAnalyticsReport = SlateTool.create(
  ...
  .input(z.object({
    ...
    filters: z.record(z.string(), z.any()).optional().describe('Filters to narrow the report scope'),
  }))
  .output(z.object({
    reportId: z.string(),
    status: z.string(),
