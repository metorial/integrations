...
export let sendMessage = SlateTool.create(
  ...
  .input(z.object({
    channelId: z.string().optional().describe('Channel ID to send a new message from (for starting new conversations)'),
    conversationId: z.string().optional().describe('Conversation ID to reply to (for replies)'),
    ...
    bodyFormat: z.enum(['html', 'markdown']).optional().describe('Format of the body content'),
