...
export let quickAddTask = SlateTool.create(
  spec,
  {
    name: 'Quick Add Task',
    key: 'quick_add_task',
  ...
  .input(z.object({
    text: z.string().describe('Natural language task text, e.g. "Buy milk tomorrow #Shopping @errands p1"'),
  }))
  .output(z.object({
    ...
    }).nullable().describe('Parsed due date'),
