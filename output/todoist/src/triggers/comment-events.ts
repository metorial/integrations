...
export let commentEvents = SlateTrigger.create(
  spec,
  {
    name: 'Comment Events',
    key: 'comment_events',
    description: 'Triggers when comments are created, updated, or deleted on tasks or projects in Todoist.',
  }
)
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: {
          ...
          taskId: data.item_id ? String(data.item_id) : (data.task_id ? String(data.task_id) : null),
