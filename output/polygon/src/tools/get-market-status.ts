...
export let getMarketStatus = SlateTool.create(
  ...
  .input(z.object({
    includeHolidays: z.boolean().optional().describe('Whether to also return upcoming market holidays. Defaults to false.'),
  }))
  .output(z.object({
    afterHours: z.boolean().optional().describe('Whether the market is in after-hours trading'),
    earlyHours: z.boolean().optional().describe('Whether the market is in early-hours trading'),
    market: z.string().optional().describe('Overall market status (open, closed, extended-hours)'),
    ...
    }).optional().describe('Status of major stock exchanges'),
    currencies: z.object({
    ...
    })).optional().describe('Upcoming market holidays'),
  }))
