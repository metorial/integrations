...
export let getMarketNews = SlateTool.create(
  ...
  .input(z.object({
    ...
    publishedAfter: z.string().optional().describe('Only return articles published after this date (YYYY-MM-DD or ISO datetime)'),
    publishedBefore: z.string().optional().describe('Only return articles published before this date (YYYY-MM-DD or ISO datetime)'),
    order: z.enum(['asc', 'desc']).optional().describe('Sort order by publication date'),
    limit: z.number().int().optional().describe('Max number of articles to return (default 10, max 1000)'),
  ...
  .output(z.object({
    articles: z.array(z.object({
      ...
      })).optional().describe('Per-ticker sentiment analysis'),
