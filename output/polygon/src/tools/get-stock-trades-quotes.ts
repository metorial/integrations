...
export let getStockTradesAndQuotes = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    dataType: z.enum(['trades', 'quotes', 'last_trade', 'last_quote']).describe('Type of data to retrieve'),
    timestampFrom: z.string().optional().describe('Filter results from this timestamp (YYYY-MM-DD or nanosecond timestamp)'),
    timestampTo: z.string().optional().describe('Filter results up to this timestamp (YYYY-MM-DD or nanosecond timestamp)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('Trade results'),
    quotes: z.array(z.object({
      ...
      sipTimestamp: z.number().optional(),
      conditions: z.array(z.number()).optional(),
    })).optional().describe('Quote results'),
    lastTrade: z.object({
      price: z.number().optional(),
      size: z.number().optional(),
      timestamp: z.number().optional(),
    ...
    lastQuote: z.object({
      ...
      askSize: z.number().optional(),
      timestamp: z.number().optional(),
    }).optional().describe('Last quote (NBBO)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.dataType === 'last_trade') {
      ...
      return {
        ...
        message: lt ? `Last trade for **${ctx.input.ticker}**: $${lt.p} (size: ${lt.s}).` : `No last trade found for **${ctx.input.ticker}**.`,
      };
    }
    ...
    return {
      ...
      message: lq ? `Last quote for **${ctx.input.ticker}**: Bid $${lq.P} / Ask $${lq.p}.` : `No last quote found for **${ctx.input.ticker}**.`,
    };
  }).build();
