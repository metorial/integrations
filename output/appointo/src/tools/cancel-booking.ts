...
export let cancelBooking = SlateTool.create(
  spec,
  {
    name: 'Cancel Booking',
    key: 'cancel_booking',
  ...
  .input(z.object({
    ...
    customerIds: z.array(z.number()).optional().describe('Specific customer IDs to cancel (omit to cancel the entire booking)'),
  }))
  .output(z.object({
