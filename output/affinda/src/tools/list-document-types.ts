...
export let listDocumentTypes = SlateTool.create(
  ...
  .input(z.object({
    organizationIdentifier: z.string().optional().describe('Filter document types by organization.'),
    workspaceIdentifier: z.string().optional().describe('Filter document types by workspace.')
  }))
  .output(z.object({
    documentTypes: z.array(z.object({
      documentTypeIdentifier: z.string().describe('Unique identifier of the document type.'),
      name: z.string().optional().describe('Display name of the document type.'),
      baseType: z.string().optional().describe('Base type category (e.g., "resume", "invoice").')
  ...
  .handleInvocation(async (ctx) => {
    ...
    let documentTypes = (Array.isArray(result) ? result : result.results ?? []).map((dt: any) => ({
