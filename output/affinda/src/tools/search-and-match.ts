...
export let searchAndMatch = SlateTool.create(
  spec,
  {
    name: 'Search & Match',
...
Returns a ranked shortlist with matching scores for each category.`,
  ...
  .input(z.object({
    searchType: z.enum(['resumes', 'job_descriptions']).describe('Whether to search through resumes or job descriptions.'),
    indices: z.array(z.string()).optional().describe('Index names to search within.'),
    jobDescriptionIdentifier: z.string().optional().describe('Job description identifier to auto-populate search criteria and find matching resumes.'),
    resumeIdentifier: z.string().optional().describe('Resume identifier to find matching job descriptions.'),
    jobTitles: z.array(z.string()).optional().describe('Job titles to search for.'),
    jobTitlesWeight: z.number().optional().describe('Weight for job titles matching (0 to 1).'),
    jobTitlesRequired: z.boolean().optional().describe('Whether job title match is required.'),
    ...
    locationsWeight: z.number().optional().describe('Weight for location matching (0 to 1).'),
    locationsRequired: z.boolean().optional().describe('Whether location match is required.'),
    languages: z.array(languageSchema).optional().describe('Languages to search for.'),
    languagesWeight: z.number().optional().describe('Weight for language matching (0 to 1).'),
    ...
    yearsExperienceWeight: z.number().optional().describe('Weight for years of experience matching (0 to 1).'),
    yearsExperienceRequired: z.boolean().optional().describe('Whether years of experience match is required.'),
    ...
    institutions: z.array(z.string()).optional().describe('Institution names to search for.'),
    institutionsRequired: z.boolean().optional().describe('Whether institution match is required.'),
    educationWeight: z.number().optional().describe('Weight for education matching (0 to 1).'),
  ...
  .output(z.object({
    ...
    results: z.array(z.object({
      documentIdentifier: z.string().optional().describe('Document identifier of the matched result.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.jobDescriptionIdentifier) searchParams.jobDescription = ctx.input.jobDescriptionIdentifier;
    ...
    if (ctx.input.jobTitlesWeight !== undefined) searchParams.jobTitlesWeight = ctx.input.jobTitlesWeight;
    if (ctx.input.jobTitlesRequired !== undefined) searchParams.jobTitlesRequired = ctx.input.jobTitlesRequired;
    if (ctx.input.skills) searchParams.skills = ctx.input.skills;
    if (ctx.input.skillsWeight !== undefined) searchParams.skillsWeight = ctx.input.skillsWeight;
    if (ctx.input.locations) searchParams.locations = ctx.input.locations;
    if (ctx.input.locationsWeight !== undefined) searchParams.locationsWeight = ctx.input.locationsWeight;
    if (ctx.input.locationsRequired !== undefined) searchParams.locationsRequired = ctx.input.locationsRequired;
    if (ctx.input.languages) searchParams.languages = ctx.input.languages;
    if (ctx.input.languagesWeight !== undefined) searchParams.languagesWeight = ctx.input.languagesWeight;
    if (ctx.input.yearsExperienceMin !== undefined) searchParams.yearsExperienceMin = ctx.input.yearsExperienceMin;
    if (ctx.input.yearsExperienceMax !== undefined) searchParams.yearsExperienceMax = ctx.input.yearsExperienceMax;
    if (ctx.input.yearsExperienceWeight !== undefined) searchParams.yearsExperienceWeight = ctx.input.yearsExperienceWeight;
    if (ctx.input.yearsExperienceRequired !== undefined) searchParams.yearsExperienceRequired = ctx.input.yearsExperienceRequired;
    if (ctx.input.degrees) searchParams.degrees = ctx.input.degrees;
    if (ctx.input.degreesRequired !== undefined) searchParams.degreesRequired = ctx.input.degreesRequired;
    if (ctx.input.institutions) searchParams.institutions = ctx.input.institutions;
    if (ctx.input.institutionsRequired !== undefined) searchParams.institutionsRequired = ctx.input.institutionsRequired;
    if (ctx.input.educationWeight !== undefined) searchParams.educationWeight = ctx.input.educationWeight;
    ...
    let result = ctx.input.searchType === 'resumes'
