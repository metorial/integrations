...
export let listDocuments = SlateTool.create(
  ...
  .input(z.object({
    workspaceIdentifier: z.string().optional().describe('Filter by workspace identifier. Falls back to the configured default workspace.'),
    collectionIdentifier: z.string().optional().describe('Filter by collection identifier.'),
    state: z.string().optional().describe('Filter by document state (e.g., "uploaded", "parsed", "validated", "rejected").'),
    ...
    ordering: z.string().optional().describe('Sort field (e.g., "created_dt", "-created_dt", "file_name").'),
    includeData: z.boolean().optional().describe('If true, includes a summary of parsed data for each document.'),
    compact: z.boolean().optional().describe('If true, returns compact document representations.')
  }))
  .output(z.object({
    ...
    documents: z.array(z.object({
      ...
      extractedData: z.any().optional().describe('Summary of parsed data (when includeData is true).')
