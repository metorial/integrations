...
export let manageDatasets = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    datasetName: z.string().optional().describe('Name of the dataset. Required for "create" and "delete" actions.'),
    aclMode: z.enum(['public', 'private', 'custom']).optional().describe('Access control mode for the dataset. Only used when creating a dataset.'),
  }))
  .output(z.object({
    datasets: z.array(z.object({
      name: z.string().describe('Dataset name.'),
      aclMode: z.string().optional().describe('Access control mode.'),
    }).passthrough()).optional().describe('List of datasets (for "list" action).'),
    created: z.object({
      datasetName: z.string().describe('Name of the created dataset.'),
      aclMode: z.string().optional().describe('Access control mode of the created dataset.'),
    }).optional().describe('Created dataset details (for "create" action).'),
    deleted: z.boolean().optional().describe('Whether the dataset was deleted (for "delete" action).'),
  }))
