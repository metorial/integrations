...
export let getDocument = SlateTool.create(
  ...
  .input(z.object({
    ...
    documentIds: z.array(z.string()).optional().describe('Array of document IDs to retrieve multiple documents at once.'),
    revision: z.string().optional().describe('Specific revision ID to retrieve a historical version of the document.'),
    time: z.string().optional().describe('ISO 8601 timestamp to retrieve the document as it was at that point in time.'),
  }))
  .output(z.object({
