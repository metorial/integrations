...
export let queryDocuments = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    query: z.string().describe('GROQ query string. Example: *[_type == "post"]{ title, body, _id }'),
    params: z.record(z.string(), z.any()).optional().describe('Query parameters for safe value substitution. Keys should match $-prefixed variables in the query.'),
    perspective: z.enum(['published', 'previewDrafts', 'raw']).optional().describe('Query perspective. "published" returns only published documents, "previewDrafts" overlays drafts on published, "raw" returns all documents as-is.'),
    useCdn: z.boolean().optional().describe('Use the CDN-cached endpoint for faster reads. Only works for published content.'),
  }))
  .output(z.object({
    result: z.any().describe('Query result data. Shape depends on the GROQ query and projection used.'),
