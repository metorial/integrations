...
export let createDatabase = SlateTool.create(
  ...
  .input(z.object({
    ...
    dryRun: z.boolean().default(false).describe('Estimate costs without creating the database'),
    protocol: z.string().optional().describe('Database protocol (e.g., redis, memcached)'),
    datasetSizeInGb: z.number().optional().describe('Maximum dataset size in GB (required for Pro)'),
    memoryLimitInGb: z.number().optional().describe('Memory limit in GB'),
    replication: z.boolean().optional().describe('Enable replication'),
    dataPersistence: z.string().optional().describe('Data persistence type (e.g., none, aof-every-1-second, snapshot-every-1-hour)'),
    dataEvictionPolicy: z.string().optional().describe('Data eviction policy (e.g., volatile-lru, noeviction)'),
    ...
    }).optional().describe('Throughput settings'),
    modules: z.array(z.object({
      name: z.string().describe('Module name (e.g., RedisJSON, RediSearch, RedisBloom, RedisTimeSeries)'),
