...
let aclRuleSchema = z.object({
  ...
  acl: z.string().optional().describe('Redis ACL string defining permitted commands and key patterns'),
  isDefault: z.boolean().optional().describe('Whether this is a predefined default rule'),
});

export let listAclRules = SlateTool.create(
  spec,
  {
    name: 'List ACL Rules',
    key: 'list_acl_rules',
...
export let createAclRule = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    name: z.string().describe('Rule name'),
    acl: z.string().describe('Redis ACL string (e.g., "+@read ~key:*" for read-only access to keys matching key:*)'),
  }))
  .output(z.object({
...
export let updateAclRule = SlateTool.create(
  spec,
  {
...
export let deleteAclRule = SlateTool.create(
  spec,
  {
