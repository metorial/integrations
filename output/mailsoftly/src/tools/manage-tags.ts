...
- Provide **contactId** with a **tags** array to assign multiple tags at once.`,
  ...
  .input(z.object({
    contactId: z.string().optional().describe('ID of the contact to assign tags to. Omit to list all tags.'),
    tagId: z.string().optional().describe('ID of a single tag to assign (alternative to tagName).'),
    tagName: z.string().optional().describe('Name of a single tag to assign or create (alternative to tagId).'),
    tagsToAssign: z.array(z.object({
      tagName: z.string().describe('Name of the tag.'),
      tagColor: z.string().optional().describe('Color of the tag (e.g., hex code).'),
    })).optional().describe('Array of tags to assign to the contact in bulk.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.tagsToAssign && ctx.input.tagsToAssign.length > 0) {
      let result = await client.assignTagsToContact(ctx.input.contactId, ctx.input.tagsToAssign);
      return {
        output: { result },
