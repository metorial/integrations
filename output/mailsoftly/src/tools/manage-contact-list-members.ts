...
export let manageContactListMembers = SlateTool.create(
  spec,
  {
...
- Provide **contacts** array to add one or more contacts in bulk (contacts will be created if they don't exist).`,
  ...
  .input(z.object({
    ...
    contactId: z.string().optional().describe('ID of a single existing contact to add to the list.'),
    contacts: z.array(z.object({
      email: z.string().describe('Email address of the contact.'),
      firstName: z.string().optional().describe('First name of the contact.'),
      lastName: z.string().optional().describe('Last name of the contact.'),
    })).optional().describe('Array of contacts to add in bulk. Contacts that do not exist will be created.'),
  }))
  .output(z.object({
    contacts: z.array(z.any()).optional().describe('List of contacts in the contact list (when retrieving).'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.contactId) {
      let result = await client.addContactToContactList(ctx.input.contactListId, ctx.input.contactId);
      return {
        output: { result },
    ...
    if (ctx.input.contacts && ctx.input.contacts.length > 0) {
      let result = await client.addContactsToContactList(ctx.input.contactListId, ctx.input.contacts);
      return {
        output: { result },
