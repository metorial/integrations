...
export let createEmailCampaign = SlateTool.create(
  ...
  .input(z.object({
    ...
    contactListId: z.string().optional().describe('ID of the contact list to send the email to.'),
    ...
    })).optional().describe('List of individual recipients. A contact list will be auto-created.'),
    ...
    attachments: z.array(z.string()).max(5).optional().describe('Public URLs to attach to the email (max 5).'),
