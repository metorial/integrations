...
export let chatEvents = SlateTrigger.create(
  spec,
  {
    name: 'Chat Events',
    key: 'chat_events',
    description: 'Triggers when a chat has unread messages or when a note is added to a conversation in Spoki.',
  }
)
  .input(z.object({
    ...
    hasUnreadMessages: z.boolean().optional().describe('Whether the chat currently has unread messages'),
  ...
  .output(z.object({
    ...
    hasUnreadMessages: z.boolean().optional().describe('Whether the chat has unread messages'),
    unreadCount: z.number().optional().describe('Number of unread messages'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let chatId = chat?.id ? String(chat.id) : data?.chat_id ? String(data.chat_id) : undefined;
      let eventId = data?.id ? String(data.id) : `${normalizedType}-${chatId || ''}-${Date.now()}`;
      ...
      return {
        inputs: [{
          ...
          eventId,
          chatId,
          contactId: chat?.contact_id ? String(chat.contact_id) : data?.contact_id ? String(data.contact_id) : undefined,
          phone: chat?.phone || data?.phone,
          hasUnreadMessages: data?.has_unread_messages ?? chat?.has_unread_messages ?? (data?.unread_count ? data.unread_count > 0 : undefined),
