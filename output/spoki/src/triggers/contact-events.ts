...
export let contactEvents = SlateTrigger.create(
  spec,
  {
    name: 'Contact Events',
    key: 'contact_events',
    description: 'Triggers when a contact is created, updated, or deleted in Spoki. Also fires for tag changes, list membership changes, and custom field updates.',
  }
)
  .input(z.object({
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let contactId = contact?.id ? String(contact.id) : data?.contact_id ? String(data.contact_id) : undefined;
      let eventId = data?.id ? String(data.id) : `${normalizedType}-${contactId || ''}-${Date.now()}`;
      ...
      return {
        inputs: [{
          ...
          listId: data?.list_id ? String(data.list_id) : data?.list?.id ? String(data.list.id) : undefined,
          listName: data?.list_name || data?.list?.name,
          fieldName: data?.field_name || data?.field?.name || data?.custom_field_name,
          fieldValue: data?.field_value !== undefined ? String(data.field_value) : data?.field?.value !== undefined ? String(data.field.value) : undefined,
          payload: data,
        }],
      };
    },
