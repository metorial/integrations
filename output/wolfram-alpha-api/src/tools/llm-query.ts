...
export let llmQuery = SlateTool.create(
  spec,
  {
...
Returns computed answers, image URLs, and links back to the Wolfram Alpha website. Best for AI-assisted workflows that need structured textual results.`,
  ...
  .output(z.object({
    llmResponse: z.string().describe('LLM-optimized text response with computed answers and links'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let truncated = responseText.length > 500 ? responseText.substring(0, 500) + '...' : responseText;
