...
export let fullResultsQuery = SlateTool.create(
  spec,
  {
...
Use this when you need detailed, multi-faceted computational answers.`,
    ...
    constraints: [
      'Complex queries may time out. Increase timeout if needed.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    query: z.string().describe('Natural language query to compute (e.g., "integrate x^2 dx", "population of France")'),
    format: z.string().optional().describe('Comma-separated content types: plaintext, image, mathml, sound, minput, moutput'),
    includePodId: z.string().optional().describe('Only include pods with this ID (e.g., "Result", "Input")'),
    ...
    podIndex: z.string().optional().describe('Comma-separated pod indices to include (e.g., "1,2,3")'),
    scanner: z.string().optional().describe('Only include pods from this scanner type (e.g., "Numeric", "Data")'),
    assumptions: z.union([z.string(), z.array(z.string())]).optional().describe('Assumption values for disambiguation, from a previous query response'),
    podState: z.string().optional().describe('Request additional pod states (e.g., "Step-by-step solution", "More digits")'),
    units: z.enum(['metric', 'imperial']).optional().describe('Unit system for results'),
    location: z.string().optional().describe('Location context for the query (e.g., "New York, NY")'),
    ip: z.string().optional().describe('IP address for location-sensitive queries'),
    latLong: z.string().optional().describe('Latitude,longitude for location-sensitive queries (e.g., "40.7,-74.0")'),
    maxWidth: z.number().optional().describe('Maximum image width in pixels'),
    reinterpret: z.boolean().optional().describe('Whether to reinterpret the query if no results found'),
    significantDigits: z.number().optional().describe('Number of significant digits for numeric results'),
  }))
  .output(z.object({
    ...
    pods: z.array(podSchema).optional().describe('Result pods containing the computed answer'),
    assumptions: z.array(assumptionSchema).optional().describe('Disambiguation assumptions available for refining the query'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let assumptions = queryResult?.assumptions ? (Array.isArray(queryResult.assumptions) ? queryResult.assumptions : [queryResult.assumptions]) : [];

    let podSummaries = pods.map((pod: any) => {
      let title = pod.title ?? 'Untitled';
      let texts = (pod.subpods ?? [])
    ...
    return {
      output: {
        ...
        errorMessage: queryResult?.error ? (typeof queryResult.error === 'object' ? queryResult.error.msg : String(queryResult.error)) : undefined,
      },
      message,
    };
  })
