...
export let shortAnswer = SlateTool.create(
  spec,
  {
    name: 'Short Answer',
    key: 'short_answer',
...
Ideal for quick lookups where you need a direct answer without additional context or structured data (e.g., "distance from Earth to Moon", "boiling point of water").`,
    constraints: [
      'Queries that cannot produce a short answer will return an error.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    query: z.string().describe('Natural language query to answer (e.g., "How tall is the Eiffel Tower?")'),
