...
let offerCodeSchema = z.object({
  ...
  amountOff: z.number().optional().describe('Discount amount (cents or percentage based on type)'),
  offerType: z.string().optional().describe('Type of discount: "cents" for fixed amount, "percent" for percentage'),
...
export let manageOfferCodes = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    offerCodeId: z.string().optional().describe('Offer code ID (required for get, update, delete)'),
    name: z.string().optional().describe('Offer code name (required for create)'),
    amountOff: z.number().optional().describe('Discount amount in cents or percentage (required for create)'),
    offerType: z.enum(['cents', 'percent']).optional().describe('Discount type (default: "cents")'),
    maxPurchaseCount: z.number().optional().describe('Maximum uses (0 = unlimited)'),
    universal: z.boolean().optional().describe('Whether the code applies to all products'),
  }))
  .output(z.object({
    offerCode: offerCodeSchema.optional().describe('Single offer code (for get, create, update)'),
    offerCodes: z.array(offerCodeSchema).optional().describe('List of offer codes (for list)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      ...
      if (ctx.input.amountOff === undefined) throw new Error('amountOff is required for create action');
