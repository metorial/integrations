...
let appOutputSchema = z.object({
  ...
  messageablePlayers: z.number().optional().describe('Subscriptions that can receive messages'),
...
export let getApp = SlateTool.create(spec, {
  ...
  .input(z.object({
    targetAppId: z.string().describe('App ID to retrieve (defaults to the configured App ID if omitted)').optional(),
  }))
...
export let createApp = SlateTool.create(spec, {
  ...
  .input(z.object({
    ...
    fcmV1ServiceAccountJson: z.string().optional().describe('Base64-encoded Firebase service account JSON for Android'),
  }))
  .output(appOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new Client({
    ...
    if (ctx.input.fcmV1ServiceAccountJson) body.fcm_v1_service_account_json = ctx.input.fcmV1ServiceAccountJson;
...
export let updateApp = SlateTool.create(spec, {
  ...
  .input(z.object({
    ...
    apnsBundleId: z.string().optional().describe('iOS APNs Bundle ID'),
    apnsP8: z.string().optional().describe('iOS APNs p8 key contents'),
    fcmV1ServiceAccountJson: z.string().optional().describe('Base64-encoded Firebase service account JSON'),
  }))
  .output(appOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new Client({
