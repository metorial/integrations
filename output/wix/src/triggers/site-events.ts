...
export let siteEvents = SlateTrigger.create(
  spec,
  {
    name: 'Site & App Events',
    key: 'site_events',
    description: 'Triggers on site-level events (site published, transferred) and app lifecycle events (installed, removed, plan changes).',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    eventAction: z.string().optional().describe('The action that occurred (e.g., published, installed, removed)'),
