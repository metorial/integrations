...
let emailResultSchema = z.object({
  ...
  result: z.string().describe('Deliverability classification: deliverable, undeliverable, risky, or unknown'),
...
export let getBulkValidationResults = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    batchId: z.string().describe('The batch job ID returned from the bulk validate emails tool'),
  }))
  .output(z.object({
    ...
    finishedAt: z.string().nullable().describe('Timestamp when the batch finished, or null if still processing'),
    results: z.array(emailResultSchema).describe('Validation results for each email in the batch (empty if not yet finished)'),
  }))
