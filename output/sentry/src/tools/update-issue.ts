...
export let updateIssueTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['resolved', 'unresolved', 'ignored']).optional().describe('New status for the issue'),
    substatus: z.enum(['archived_until_escalating', 'archived_until_condition_met', 'archived_forever']).optional().describe('Substatus when ignoring an issue'),
    assignedTo: z.string().optional().describe('Assign to user email, "me", "#team-slug", or "" to unassign'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.assignedTo !== undefined) actions.push(`assigned → ${ctx.input.assignedTo || 'unassigned'}`);
    if (ctx.input.isBookmarked !== undefined) actions.push(ctx.input.isBookmarked ? 'bookmarked' : 'unbookmarked');
