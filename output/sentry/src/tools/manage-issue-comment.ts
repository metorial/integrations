...
export let manageIssueCommentTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    text: z.string().optional().describe('Comment text in markdown (required for create/update)')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      let comment = await client.createIssueComment(ctx.input.issueId, { text: ctx.input.text });
      return {
        output: { comment },
    ...
    if (ctx.input.action === 'update') {
      ...
      if (!ctx.input.text) throw new Error('text is required');
      let comment = await client.updateIssueComment(ctx.input.issueId, ctx.input.commentId, { text: ctx.input.text });
      return {
        output: { comment },
