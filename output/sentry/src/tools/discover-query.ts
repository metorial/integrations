...
export let discoverQueryTool = SlateTool.create(
  spec,
  {
    name: 'Discover Query',
    key: 'discover_query',
  ...
  .input(z.object({
    fields: z.array(z.string()).describe('Fields to return (e.g. ["title", "count()", "last_seen()"])'),
    ...
    statsPeriod: z.string().optional().default('24h').describe('Time range (e.g. "1h", "24h", "7d", "14d")'),
    start: z.string().optional().describe('ISO 8601 start date (alternative to statsPeriod)'),
    end: z.string().optional().describe('ISO 8601 end date (alternative to statsPeriod)'),
    perPage: z.number().optional().default(20).describe('Number of results per page (max 100)')
  }))
  .output(z.object({
