...
export let alertEventsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Alert Events',
    key: 'alert_events',
    description: 'Triggers when an issue alert rule fires or a metric alert rule changes state. For issue alerts, fires when conditions are met. For metric alerts, fires on state transitions (e.g., normal → warning → critical → resolved). Configure the webhook in Settings > Developer Settings.'
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    status: z.string().optional().describe('Alert status (e.g. critical, warning, resolved)'),
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        id: ctx.input.alertId,
        output: {
          alertRuleId: ctx.input.payload?.data?.triggered_rule ? String(ctx.input.payload.data.triggered_rule) : undefined,
          alertRuleName: ctx.input.payload?.data?.triggered_rule_label || ctx.input.payload?.data?.triggered_rule,
