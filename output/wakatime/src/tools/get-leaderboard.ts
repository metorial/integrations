...
export let getLeaderboard = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    leaderboardId: z.string().optional().describe('Private leaderboard ID. Leave empty for the public leaderboard.'),
    language: z.string().optional().describe('Filter by programming language'),
    country: z.string().optional().describe('Filter by country code (public leaderboard only)'),
    hireable: z.boolean().optional().describe('Filter by hireable status (public leaderboard only)'),
    page: z.number().optional().describe('Page number for pagination')
  }))
  .output(z.object({
    currentUser: z.object({
      rank: z.number().optional().describe('Current user rank'),
      runningTotal: z.object({
        ...
        })).optional().describe('Language breakdown')
      }).passthrough().optional()
    }).passthrough().optional().describe('Current user position on the leaderboard'),
    leaders: z.array(z.object({
      ...
      languages: z.array(z.object({
        name: z.string(),
        totalSeconds: z.number()
      })).optional().describe('Top languages')
    }).passthrough()).describe('Ranked list of users'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let currentUser = result.current_user ? {
      ...
      runningTotal: result.current_user.running_total ? {
        ...
        humanReadableTotal: result.current_user.running_total.human_readable_total ?? '0 secs',
