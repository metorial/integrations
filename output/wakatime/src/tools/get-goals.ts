...
export let getGoals = SlateTool.create(
  ...
  .output(z.object({
    goals: z.array(z.object({
      ...
      isCurrentUserOwner: z.boolean().optional().describe('Whether current user owns this goal'),
      statusPercent: z.number().optional().describe('Current progress percentage'),
      currentStatus: z.string().optional().describe('Current goal status (success, fail, pending)'),
      languages: z.array(z.string()).optional().describe('Languages the goal is scoped to'),
      editors: z.array(z.string()).optional().describe('Editors the goal is scoped to'),
      projects: z.array(z.string()).optional().describe('Projects the goal is scoped to'),
      createdAt: z.string().optional().describe('When the goal was created')
    }).passthrough()).describe('List of goals'),
