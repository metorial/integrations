...
export let getOpenBoxOffers = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    skus: z.array(z.string()).optional().describe('Array of product SKUs (up to 100) to check for open box offers'),
    categoryId: z.string().optional().describe('Category ID to browse open box offers, e.g. "abcat0502000"'),
  }))
  .output(z.object({
    count: z.number().describe('Number of products with open box offers'),
    openBoxProducts: z.array(z.object({
