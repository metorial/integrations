...
Use the **query** field for attribute filters using Best Buy query syntax (e.g. \`manufacturer=apple&salePrice<500\`). Use **keyword** for free-text search.`,
  ...
  .input(z.object({
    query: z.string().optional().describe('Attribute filter query using Best Buy syntax, e.g. "manufacturer=apple&salePrice<1000" or "categoryPath.id=abcat0502000"'),
    keyword: z.string().optional().describe('Free-text keyword search term'),
    sort: z.string().optional().describe('Sort order as "attribute.asc" or "attribute.dsc", e.g. "salePrice.asc" or "customerReviewAverage.dsc"'),
    show: z.string().optional().describe('Comma-separated list of fields to return, e.g. "sku,name,salePrice,manufacturer,customerReviewAverage"'),
  ...
  .output(z.object({
    ...
    products: z.array(z.record(z.string(), z.unknown())).describe('Array of product objects with requested fields'),
  }))
