...
export let searchStores = SlateTool.create(
  spec,
  {
    name: 'Search Stores',
    key: 'search_stores',
  ...
  .input(z.object({
    postalCode: z.string().optional().describe('US postal code to search near, e.g. "90210"'),
    ...
    query: z.string().optional().describe('Additional attribute filter, e.g. "storeType=Big Box Store" or "region=CA"'),
    show: z.string().optional().describe('Comma-separated list of fields to return, e.g. "storeId,name,address,city,region,postalCode,phone,hours,distance"'),
  ...
  .output(z.object({
    ...
    stores: z.array(z.record(z.string(), z.unknown())).describe('Array of store objects with location and service details'),
  }))
