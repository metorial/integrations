...
export let checkAvailability = SlateTool.create(
  ...
  .input(z.object({
    ...
    postalCode: z.string().optional().describe('US postal code to find nearby stores with this product'),
    storeId: z.string().optional().describe('Specific Best Buy store ID to check availability at'),
  }))
  .output(z.object({
    total: z.number().describe('Total number of stores with availability information'),
    stores: z.array(z.record(z.string(), z.unknown())).describe('Array of store availability objects including stock status and pickup estimates'),
  }))
