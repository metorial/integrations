...
export let messageStatus = SlateTrigger.create(
  spec,
  {
    ...
    key: 'message_status',
    description: 'Triggered when a message delivery status changes (e.g. queued, sent, delivered, failed, undelivered, read). Configure the status callback URL on individual messages or on your Messaging Service.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    messageSid: z.string().describe('SID of the message'),
    status: z.string().describe('New delivery status (queued, sending, sent, delivered, undelivered, failed, read)'),
    from: z.string().describe('Sender phone number'),
    to: z.string().describe('Recipient phone number'),
    errorCode: z.string().nullable().describe('Error code if message failed or was undelivered'),
