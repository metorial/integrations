...
let messageSchema = z.object({
  ...
  direction: z.string().describe('Message direction (inbound, outbound-api, outbound-call, outbound-reply)'),
...
export let listMessages = SlateTool.create(
  ...
  .input(z.object({
    messageSid: z.string().optional().describe('Fetch a specific message by its SID (starts with SM). If provided, other filters are ignored.'),
    ...
    dateSentAfter: z.string().optional().describe('Filter messages sent after this date (YYYY-MM-DD format).'),
    dateSentBefore: z.string().optional().describe('Filter messages sent before this date (YYYY-MM-DD format).'),
    pageSize: z.number().optional().describe('Number of messages to return per page (max 1000, default 50).'),
  }))
  .output(z.object({
