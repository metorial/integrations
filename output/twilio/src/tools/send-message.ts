...
export let sendMessage = SlateTool.create(
  ...
  .input(z.object({
    to: z.string().describe('Destination phone number in E.164 format (e.g. +15551234567). For WhatsApp, prefix with "whatsapp:".'),
    from: z.string().optional().describe('Twilio phone number to send from in E.164 format. Required if messagingServiceSid is not provided.'),
    body: z.string().optional().describe('Text body of the message (up to 1600 characters). Required unless mediaUrls are provided.'),
    messagingServiceSid: z.string().optional().describe('SID of the Messaging Service to use (starts with MG). Required for scheduled messages.'),
    mediaUrls: z.array(z.string()).optional().describe('URLs of media to include (for MMS). Up to 10 URLs.'),
    statusCallbackUrl: z.string().optional().describe('URL to receive delivery status webhooks for this message.'),
    sendAt: z.string().optional().describe('ISO 8601 datetime to schedule the message for future delivery. Requires messagingServiceSid.'),
  }))
  .output(z.object({
    messageSid: z.string().describe('Unique SID of the created message'),
    status: z.string().describe('Current status of the message (e.g. queued, sending, sent, delivered, scheduled)'),
