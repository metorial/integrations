...
export let sendVerification = SlateTool.create(
  ...
  .input(z.object({
    ...
    to: z.string().describe('Recipient phone number (E.164) or email address to send the verification code to.'),
    channel: z.enum(['sms', 'call', 'email', 'whatsapp']).describe('Delivery channel for the verification code.'),
    locale: z.string().optional().describe('Locale for the verification message (e.g. "en", "es", "fr").'),
    templateSid: z.string().optional().describe('SID of a custom verification template (starts with HJ).'),
  }))
  .output(z.object({
