...
export let searchPhoneNumbers = SlateTool.create(
  ...
  .input(z.object({
    ...
    numberType: z.enum(['Local', 'TollFree', 'Mobile']).default('Local').describe('Type of phone number to search for.'),
    areaCode: z.string().optional().describe('Filter by area code (e.g. "415").'),
    contains: z.string().optional().describe('Pattern to match in the number (e.g. "555" or "AWESOME"). Use * for wildcards.'),
    inRegion: z.string().optional().describe('Filter by state/region (e.g. "CA" for California).'),
    ...
    pageSize: z.number().optional().describe('Number of results to return (max 1000, default 50).'),
  }))
  .output(z.object({
    availableNumbers: z.array(availableNumberSchema).describe('List of available phone numbers'),
  }))
