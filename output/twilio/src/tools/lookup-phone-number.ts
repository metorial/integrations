...
export let lookupPhoneNumber = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    phoneNumber: z.string().describe('Phone number to look up in E.164 format (e.g. +15551234567) or national format.'),
    fields: z.array(z.enum([
      'validation', 'line_type_intelligence', 'caller_name', 'sim_swap',
      'call_forwarding', 'line_status', 'identity_match', 'reassigned_number',
      'sms_pumping_risk', 'phone_number_quality_score',
    ])).optional().describe('Additional data fields to request (paid features).'),
    countryCode: z.string().optional().describe('ISO 3166-1 alpha-2 country code, used when the phone number is in national format.'),
    ...
    addressLine1: z.string().optional().describe('Street address for identity_match lookups.'),
  }))
  .output(z.object({
    ...
    callingCountryCode: z.string().nullable().describe('Calling country code (e.g. "1" for US)'),
    valid: z.boolean().describe('Whether the phone number is valid'),
    validationErrors: z.array(z.string()).nullable().describe('Validation error codes if invalid'),
    lineTypeIntelligence: z.any().nullable().describe('Line type, carrier name, and network codes (if requested)'),
    ...
    reassignedNumber: z.any().nullable().describe('Reassigned number detection (if requested)'),
    smsPumpingRisk: z.any().nullable().describe('SMS pumping risk score (if requested)'),
    phoneNumberQualityScore: z.any().nullable().describe('Phone number quality score (if requested)'),
  }))
