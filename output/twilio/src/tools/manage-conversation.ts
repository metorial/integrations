...
export let manageConversation = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'list', 'get', 'update', 'delete']).describe('Action to perform on conversations.'),
    conversationSid: z.string().optional().describe('SID of the conversation (required for get, update, delete).'),
    ...
    state: z.enum(['active', 'inactive', 'closed']).optional().describe('Conversation state (for create, update, or list filter).'),
    attributes: z.string().optional().describe('JSON string of custom attributes (for create or update).'),
    messagingServiceSid: z.string().optional().describe('Messaging Service SID to associate with the conversation (for create).'),
    pageSize: z.number().optional().describe('Number of results for list (default 50).'),
  }))
  .output(z.object({
    conversations: z.array(conversationSchema).optional().describe('Conversation(s) affected by the action'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'get') {
      if (!ctx.input.conversationSid) throw new Error('conversationSid is required for get action');
    ...
    if (ctx.input.action === 'update') {
      if (!ctx.input.conversationSid) throw new Error('conversationSid is required for update action');
    ...
    if (ctx.input.action === 'delete') {
      if (!ctx.input.conversationSid) throw new Error('conversationSid is required for delete action');
