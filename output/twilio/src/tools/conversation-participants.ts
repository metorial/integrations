...
let participantSchema = z.object({
  ...
  messagingBindingAddress: z.string().nullable().describe('Phone number of the SMS/WhatsApp participant'),
  messagingBindingProxyAddress: z.string().nullable().describe('Twilio number used as proxy for SMS/WhatsApp'),
...
export let conversationParticipants = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    identity: z.string().optional().describe('Chat identity for a chat/SDK participant (for "add" action).'),
    messagingBindingAddress: z.string().optional().describe('Phone number of the SMS/WhatsApp participant (for "add" action).'),
    messagingBindingProxyAddress: z.string().optional().describe('Twilio phone number to use as proxy (for "add" action with SMS/WhatsApp).'),
    participantSid: z.string().optional().describe('SID of the participant to remove (for "remove" action).'),
    attributes: z.string().optional().describe('JSON string of custom attributes (for "add" action).'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'remove') {
      if (!ctx.input.participantSid) throw new Error('participantSid is required for remove action');
      await client.removeConversationParticipant(ctx.input.conversationSid, ctx.input.participantSid);
      return {
        output: { removed: true },
