...
export let sendConversationMessage = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    body: z.string().optional().describe('Message body text (required for "send" action, up to 1600 characters).'),
    author: z.string().optional().describe('Author of the message. Defaults to "system".'),
    attributes: z.string().optional().describe('JSON string of custom attributes (for "send" action).'),
    pageSize: z.number().optional().describe('Number of messages to return for list action (default 50).'),
  ...
  .output(z.object({
    messages: z.array(z.object({
      ...
      participantSid: z.string().nullable().describe('SID of the participant who sent the message'),
