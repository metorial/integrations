...
export let memIt = SlateTool.create(
  spec,
  {
    name: 'Mem It',
    key: 'mem_it',
  ...
  .input(z.object({
    content: z.string().describe('Raw content to process — web pages, emails, transcripts, articles, or plain text.'),
    instructions: z.string().optional().describe('Guidance on how the content should be processed (e.g. "Extract key findings and save as a research note").'),
    context: z.string().optional().describe('Background information to help Mem understand how the input relates to existing knowledge (e.g. "This is related to my Project Alpha research").'),
    timestamp: z.string().optional().describe('When the information was originally encountered, in ISO 8601 format.'),
  }))
  .output(z.object({
    requestId: z.string().describe('The request ID for tracking the asynchronous processing.'),
  }))
