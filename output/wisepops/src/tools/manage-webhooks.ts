...
export let listWebhooks = SlateTool.create(
  ...
  .output(z.object({
    webhooks: z.array(z.object({
      ...
      wisepopId: z.number().optional().describe('Campaign ID if the webhook is scoped to a specific campaign.'),
...
Choose an event type (email, phone, or survey) and provide a target URL. Optionally scope the webhook to a specific campaign.`,
  ...
  .input(z.object({
    event: z.enum(['email', 'phone', 'survey']).describe('Event type that triggers the webhook.'),
    targetUrl: z.string().describe('URL that will receive the webhook POST payloads.'),
    wisepopId: z.number().optional().describe('Campaign ID to scope the webhook to a specific campaign. Omit to receive data from all campaigns.'),
  }))
  .output(z.object({
