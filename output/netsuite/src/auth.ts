...
export let auth = SlateAuth.create()
  ...
  .addOauth({
    ...
    scopes: [
      {
        ...
        description: 'Access to NetSuite REST Web Services API for record CRUD and SuiteQL queries',
    ...
    inputSchema: z.object({
      accountId: z.string().describe('NetSuite Account ID (e.g., "1234567" or "1234567_SB1" for sandbox)'),
    }),
    ...
    handleCallback: async (ctx) => {
      ...
      let response = await httpClient.post(tokenUrl, new URLSearchParams({
  ...
  .addCustomAuth({
    ...
    inputSchema: z.object({
      ...
      consumerSecret: z.string().describe('Consumer Secret from the Integration Record in NetSuite'),
      tokenId: z.string().describe('Token ID generated from the Access Token in NetSuite'),
      tokenSecret: z.string().describe('Token Secret generated from the Access Token in NetSuite'),
    }),
