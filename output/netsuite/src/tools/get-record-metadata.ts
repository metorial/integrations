...
export let getRecordMetadata = SlateTool.create(
  spec,
  {
...
Use this to discover available fields before creating or updating records, or to understand the structure of a record type.`,
  ...
  .input(z.object({
    recordType: z.string().describe('NetSuite record type in camelCase (e.g., "customer", "salesOrder", "invoice", "inventoryItem")'),
  }))
  .output(z.object({
    metadata: z.record(z.string(), z.any()).describe('Record type metadata including fields, sublists, and supported operations'),
  }))
