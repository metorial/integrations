...
let contactInputSchema = z.object({
  ...
  fullName: z.string().optional().describe('Full name of the contact (alternative to firstName + lastName)'),
  ...
  customFields: z.record(z.string(), z.string()).optional().describe('Custom key-value pairs preserved in the response'),
});
...
Use **Get Enrichment Results** to retrieve the enriched data once processing completes.`,
  ...
  .input(z.object({
    contacts: z.array(contactInputSchema).min(1).max(250).describe('Array of contacts to enrich (1 to 250)'),
    sirenEnrichment: z.boolean().optional().describe('Set to true to receive French company registration data (SIREN, SIRET, NAF, VAT)'),
  }))
  .output(z.object({
