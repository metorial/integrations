...
export let manageTags = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'create', 'update', 'delete', 'list_members', 'add_member', 'remove_member']).describe('Action to perform'),
    tagId: z.string().optional().describe('Tag ID (required for update/delete/member operations)'),
    displayName: z.string().optional().describe('Tag display name (for create/update)'),
    memberUserIds: z.array(z.string()).optional().describe('User IDs to add as initial tag members (for create)'),
    userId: z.string().optional().describe('User ID to add to tag (for add_member)'),
    tagMemberId: z.string().optional().describe('Tag member ID to remove (for remove_member)'),
  }))
  .output(z.object({
    ...
    })).optional().describe('List of tag members'),
