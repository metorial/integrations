...
export let manageDataStoreRecords = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'get', 'create', 'update', 'delete']).describe('Action to perform on records'),
    recordKey: z.string().optional().describe('Record key (required for get, update, delete)'),
    recordData: z.record(z.string(), z.any()).optional().describe('Record data as key-value pairs (for create/update)'),
  ...
  .output(z.object({
    records: z.array(z.record(z.string(), z.any())).optional().describe('List of records (for list action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update') {
      ...
      let result = await client.updateDataStoreRecord(dataStoreId, ctx.input.recordKey, ctx.input.recordData);
      return {
        output: {
          record: result,
          recordKey: ctx.input.recordKey,
