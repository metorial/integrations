...
let flexFlowSchema = z.object({
  ...
  channelType: z.string().optional().describe('Channel type (web, sms, facebook, whatsapp, custom)'),
  ...
  integrationFlowSid: z.string().optional().describe('Studio Flow SID (for studio integration)'),
...
export let manageFlexFlowsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    flexFlowSid: z.string().optional().describe('Flex Flow SID (required for get/update/delete)'),
    friendlyName: z.string().optional().describe('Friendly name'),
    chatServiceSid: z.string().optional().describe('Chat Service SID'),
    channelType: z.enum(['web', 'sms', 'facebook', 'whatsapp', 'custom']).optional().describe('Channel type'),
    contactIdentity: z.string().optional().describe('Contact identity'),
    enabled: z.boolean().optional().describe('Whether the Flex Flow is enabled'),
    integrationType: z.enum(['studio', 'external', 'task']).optional().describe('Integration type'),
    integrationFlowSid: z.string().optional().describe('Studio Flow SID (for studio integration)'),
    integrationUrl: z.string().optional().describe('Webhook URL (for external integration)'),
    integrationWorkspaceSid: z.string().optional().describe('TaskRouter Workspace SID (for task integration)'),
    integrationWorkflowSid: z.string().optional().describe('TaskRouter Workflow SID (for task integration)'),
    integrationChannel: z.string().optional().describe('TaskRouter channel unique name (for task integration)'),
