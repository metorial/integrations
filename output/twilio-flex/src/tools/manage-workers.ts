...
export let manageWorkersTool = SlateTool.create(
  spec,
  {
    name: 'Manage Workers',
    key: 'manage_workers',
  ...
  .input(z.object({
    ...
    attributes: z.string().optional().describe('Worker attributes as JSON string'),
    targetWorkersExpression: z.string().optional().describe('Filter expression when listing workers (e.g., "skills HAS \'support\'")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      if (!ctx.input.friendlyName) throw new Error('friendlyName is required to create a worker');
      let params: Record<string, string | undefined> = {
