...
export let createInteractionTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    channelType: z.enum(['sms', 'whatsapp', 'web', 'voice', 'email', 'custom']).describe('Communication channel type'),
    channelProperties: z.record(z.string(), z.string()).describe('Channel-specific properties as a JSON object (e.g., {"type": "sms", "to": "+1234567890", "from": "+0987654321"})'),
    routingWorkflowSid: z.string().describe('TaskRouter Workflow SID for routing this interaction'),
    routingWorkspaceSid: z.string().describe('TaskRouter Workspace SID'),
    routingAttributes: z.record(z.string(), z.string()).optional().describe('Additional routing attributes as key-value pairs'),
    interactionContextJson: z.string().optional().describe('JSON string with additional interaction context')
  }))
  .output(z.object({
