...
export class Client {
  ...
  async listConversations(params: {
    ...
    if (params.filterUnread !== undefined) queryParams['filter_unread'] = String(params.filterUnread ? 1 : 0);
    if (params.filterResolved !== undefined) queryParams['filter_resolved'] = String(params.filterResolved ? 1 : 0);
    if (params.filterNotResolved !== undefined) queryParams['filter_not_resolved'] = String(params.filterNotResolved ? 1 : 0);
    if (params.filterAssigned !== undefined) queryParams['filter_assigned'] = String(params.filterAssigned ? 1 : 0);
    if (params.filterUnassigned !== undefined) queryParams['filter_unassigned'] = String(params.filterUnassigned ? 1 : 0);
  ...
  async createConversation() {
    let response = await axios.post(this.url('/conversation'), {}, {
      headers: this.headers(),
    });
    return response.data.data;
  }
  ...
  async updateConversationMeta(sessionId: string, meta: {
    nickname?: string;
    email?: string;
    phone?: string;
    address?: string;
  ...
  async changeConversationState(sessionId: string, state: 'pending' | 'unresolved' | 'resolved') {
    await axios.patch(this.url(`/conversation/${sessionId}/state`), { state }, {
      headers: this.headers(),
    });
  }
  ...
  async assignConversationRouting(sessionId: string, assigned: { user_id: string } | null) {
    await axios.patch(this.url(`/conversation/${sessionId}/routing`), { assigned }, {
      headers: this.headers(),
    });
  }
  ...
  async updateConversationBlock(sessionId: string, blocked: boolean) {
    await axios.patch(this.url(`/conversation/${sessionId}/block`), { blocked }, {
      headers: this.headers(),
    });
  }
  ...
  async sendMessageInConversation(sessionId: string, message: {
    type: string;
    from: string;
    origin: string;
    content: any;
  ...
  async getMessage(sessionId: string, fingerprint: string) {
    let response = await axios.get(this.url(`/conversation/${sessionId}/message/${fingerprint}`), {
      headers: this.headers(),
    });
    return response.data.data;
  }
  ...
  async updateMessage(sessionId: string, fingerprint: string, content: any) {
    await axios.patch(this.url(`/conversation/${sessionId}/message/${fingerprint}`), { content }, {
      headers: this.headers(),
    });
  }
  ...
  async markMessagesRead(sessionId: string, from: string, origin: string, fingerprints: string[]) {
    await axios.patch(this.url(`/conversation/${sessionId}/read`), {
      from,
      origin,
      fingerprints,
    }, {
  ...
  async markConversationUnread(sessionId: string) {
    await axios.patch(this.url(`/conversation/${sessionId}/unread`), {}, {
      headers: this.headers(),
    });
  }

  async markMessagesDelivered(sessionId: string, from: string, origin: string, fingerprints: string[]) {
    await axios.patch(this.url(`/conversation/${sessionId}/delivered`), {
      from,
      origin,
      fingerprints,
    }, {
  ...
  async listPeopleProfiles(params: {
    pageNumber?: number;
    searchQuery?: string;
    searchType?: string;
    searchOperator?: string;
  ...
  async createPeopleProfile(profile: {
    email?: string;
  ...
  async updatePeopleProfile(peopleId: string, profile: {
    email?: string;
  ...
  async savePeopleProfile(peopleId: string, profile: {
    email?: string;
    person?: { nickname?: string; avatar?: string };
    phone?: string;
    address?: string;
  ...
  async updatePeopleData(peopleId: string, data: Record<string, any>) {
    let response = await axios.patch(this.url(`/people/data/${peopleId}`), { data }, {
      headers: this.headers(),
    });
    return response.data.data;
  }
  ...
  async addPeopleEvent(peopleId: string, event: { text: string; data?: Record<string, any>; color?: string }) {
  ...
  async updatePeopleSubscriptionStatus(peopleId: string, subscription: { email?: boolean }) {
    let response = await axios.patch(this.url(`/people/subscription/${peopleId}`), subscription, {
      headers: this.headers(),
    });
    return response.data.data;
  }
  ...
  async createHelpdeskArticle(localeId: string, article: {
    title: string;
    description?: string;
    content?: string;
    featured?: boolean;
    ...
    let response = await axios.post(this.url(`/helpdesk/locale/${localeId}/article`), article, {
      headers: this.headers(),
    });
    return response.data.data;
  }
  ...
  async getHelpdeskArticle(localeId: string, articleId: string) {
    let response = await axios.get(this.url(`/helpdesk/locale/${localeId}/article/${articleId}`), {
      headers: this.headers(),
    });
    return response.data.data;
  }

  async updateHelpdeskArticle(localeId: string, articleId: string, article: {
    title?: string;
    description?: string;
    content?: string;
    featured?: boolean;
    ...
    let response = await axios.patch(this.url(`/helpdesk/locale/${localeId}/article/${articleId}`), article, {
      headers: this.headers(),
    });
    return response.data.data;
  }
  ...
  async updateCampaign(campaignId: string, campaign: {
    name?: string;
    message?: Record<string, any>;
  }) {
  ...
  async dispatchCampaign(campaignId: string) {
    let response = await axios.post(this.url(`/campaign/${campaignId}/dispatch`), {}, {
      headers: this.headers(),
    });
    return response.data.data;
  }
  ...
  async changeOperatorMembership(userId: string, role: string) {
    let response = await axios.patch(this.url(`/operator/${userId}`), { role }, {
      headers: this.headers(),
    });
    return response.data.data;
  }
  ...
  async batchResolveConversations(sessionIds: string[]) {
    await axios.patch(this.url('/batch/resolve'), { sessions: sessionIds }, {
      headers: this.headers(),
    });
  }

  async batchReadConversations(sessionIds: string[]) {
    await axios.patch(this.url('/batch/read'), { sessions: sessionIds }, {
      headers: this.headers(),
    });
  }

  async batchRemoveConversations(sessionIds: string[]) {
    await axios.patch(this.url('/batch/remove'), { sessions: sessionIds }, {
      headers: this.headers(),
    });
  }
  ...
  async saveConversationParticipants(sessionId: string, participants: { type: string; target: string }[]) {
    await axios.put(this.url(`/conversation/${sessionId}/participants`), { participants }, {
      headers: this.headers(),
    });
  }
  ...
  async requestTranscript(sessionId: string, to: string, email?: string) {
    await axios.post(this.url(`/conversation/${sessionId}/transcript`), { to, email }, {
      headers: this.headers(),
    });
  }
}
