...
export let manageWebsiteSettings = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    settings: z.record(z.string(), z.any()).optional().describe('Settings key-value pairs to update. Omit to read current settings.'),
  }))
  .output(z.object({
