...
export let manageManualTasks = SlateTool.create(
  spec,
  {
...
- Use **done** or **ignored** action with a taskId to update a specific task`,
  ...
  .input(z.object({
    action: z.enum(['list', 'done', 'ignored']).describe('Action to perform: list tasks, or mark a task as done/ignored'),
    taskId: z.number().optional().describe('Task ID to update (required for done/ignored actions)'),
    campaignId: z.number().optional().describe('Filter tasks by campaign ID (for list action)'),
  }))
  .output(z.object({
    ...
    })).optional().describe('List of tasks (for list action)'),
    updated: z.boolean().optional().describe('Whether the task was successfully updated (for done/ignored actions)'),
  }))
