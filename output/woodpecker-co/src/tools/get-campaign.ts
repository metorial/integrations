...
export let getCampaign = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeStatistics: z.boolean().optional().default(false).describe('Whether to also fetch campaign statistics'),
  }))
  .output(z.object({
    campaignId: z.number().describe('Campaign ID'),
    name: z.string().describe('Campaign name'),
    status: z.string().describe('Campaign status'),
    settings: z.any().optional().describe('Campaign settings (timezone, daily enroll, tracking, etc.)'),
    steps: z.any().optional().describe('Campaign steps with email content and delivery times'),
