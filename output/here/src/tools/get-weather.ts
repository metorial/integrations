...
- **nwsAlerts**: US National Weather Service alerts`,
  ...
  .input(z.object({
    products: z.array(z.enum([
      'observation', 'forecast7days', 'forecast7daysSimple',
      'forecastHourly', 'forecastAstronomy', 'alerts', 'nwsAlerts',
    ])).describe('Weather products to retrieve'),
    location: z.string().optional().describe('Coordinates as "lat,lng" (e.g. "52.5251,13.3694")'),
    query: z.string().optional().describe('Location name query (e.g. "Berlin, Germany")'),
    zipCode: z.string().optional().describe('US ZIP code'),
    units: z.enum(['metric', 'imperial']).optional().describe('Measurement units (default metric)'),
    lang: z.string().optional().describe('Language code (e.g. "en-US")'),
    oneObservation: z.boolean().optional().describe('Return only closest weather station observation'),
    hourlyDate: z.string().optional().describe('Specific date for hourly forecast (YYYY-MM-DD)'),
  }))
  .output(z.object({
    places: z.array(z.object({
      ...
      })).optional().describe('Current weather observations'),
      dailyForecasts: z.object({
        forecasts: z.array(z.any()).optional(),
      }).optional().describe('Daily forecast data'),
      hourlyForecasts: z.object({
        forecasts: z.array(z.any()).optional(),
      }).optional().describe('Hourly forecast data'),
      astronomyForecasts: z.object({
        forecasts: z.array(z.any()).optional(),
      }).optional().describe('Astronomy/sun/moon data'),
      alerts: z.array(z.any()).optional().describe('Weather alerts'),
      nwsAlerts: z.object({
        warnings: z.array(z.any()).optional(),
        watches: z.array(z.any()).optional(),
      }).optional().describe('NWS alerts (US only)'),
    })).optional().describe('Weather data per location'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let locationLabel = ctx.input.query || ctx.input.location || ctx.input.zipCode || 'specified location';
