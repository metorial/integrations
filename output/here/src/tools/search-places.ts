...
let placeResultSchema = z.object({
  ...
  })).optional().describe('Place categories'),
  address: z.object({
  ...
  }).optional().describe('Address of the place'),
  position: z.object({
    lat: z.number(),
    lng: z.number(),
  ...
  })).optional().describe('Opening hours'),
});

export let searchPlaces = SlateTool.create(
  spec,
  {
    name: 'Search Places',
    key: 'search_places',
...
Returns place names, addresses, coordinates, categories, contacts, and opening hours.`,
  ...
  .input(z.object({
    query: z.string().optional().describe('Free-text search query (e.g. "coffee shop", "pizza restaurant"). Triggers discover mode.'),
    at: z.string().describe('Search center as "lat,lng" (e.g. "52.5308,13.3847")'),
    categories: z.string().optional().describe('HERE place category IDs, comma-separated (e.g. "100-1000-0009"). Triggers browse mode when no query given.'),
    foodTypes: z.string().optional().describe('Cuisine type IDs for browse mode, comma-separated (e.g. "202,208")'),
    chains: z.string().optional().describe('HERE chain IDs for browse mode, comma-separated'),
    name: z.string().optional().describe('Filter by place name in browse mode'),
    inArea: z.string().optional().describe('Geographic filter (e.g. "countryCode:USA", "circle:lat,lng;r=5000", "bbox:w,s,e,n")'),
