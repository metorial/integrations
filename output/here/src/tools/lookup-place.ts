...
export let lookupPlace = SlateTool.create(
  spec,
  {
    name: 'Lookup Place',
    key: 'lookup_place',
...
HERE place IDs are returned by other search tools (geocode, search places, autosuggest).`,
  ...
  .input(z.object({
    placeId: z.string().describe('HERE place/address ID (e.g. "here:pds:place:276u0vhj-b0bace6448ae4b0fbc1d5e323998a7d2")'),
  ...
  .output(z.object({
    ...
    }).optional().describe('Full address'),
    position: z.object({
      lat: z.number(),
      lng: z.number(),
