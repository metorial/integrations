...
let suggestionSchema = z.object({
  ...
  resultType: z.string().optional().describe('Result type (e.g. place, street, locality, chainQuery, categoryQuery)'),
  address: z.object({
  ...
  }).optional().describe('Address of the suggestion'),
  position: z.object({
    lat: z.number(),
    lng: z.number(),
  }).optional().describe('Coordinates'),
...
export let autosuggestPlaces = SlateTool.create(
  spec,
  {
...
Supports both the autosuggest endpoint (places + query suggestions) and the autocomplete endpoint (address completion only).`,
  ...
  .input(z.object({
    ...
    mode: z.enum(['autosuggest', 'autocomplete']).optional().describe('Suggestion mode: "autosuggest" for places+queries (default), "autocomplete" for addresses only'),
    termsLimit: z.number().optional().describe('Max predictive text terms (0-10, autosuggest mode only)'),
    types: z.string().optional().describe('Limit types in autocomplete mode (e.g. "city,postalCode,area")'),
  }))
  .output(z.object({
