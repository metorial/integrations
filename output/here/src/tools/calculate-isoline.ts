...
let isolineSchema = z.object({
  ...
  })).optional().describe('Isoline polygons'),
});

export let calculateIsoline = SlateTool.create(
  spec,
  {
...
Useful for delivery zone planning, accessibility analysis, and coverage mapping.`,
  ...
  .input(z.object({
    ...
    destination: z.string().optional().describe('End point for reverse isoline as "lat,lng"'),
    transportMode: z.enum(['car', 'truck', 'pedestrian', 'bicycle']).describe('Transport mode'),
    rangeType: z.enum(['time', 'distance', 'consumption']).describe('Range type: "time" (seconds), "distance" (meters), or "consumption"'),
    rangeValues: z.array(z.number()).describe('Range values (e.g. [300, 600, 900] for time in seconds)'),
    departureTime: z.string().optional().describe('Departure time in ISO 8601 or "any"'),
    routingMode: z.enum(['fast', 'short']).optional().describe('Routing optimization: "fast" (default) or "short"'),
    optimizeFor: z.enum(['quality', 'performance', 'balanced']).optional().describe('Optimize for quality or performance'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let rangeDesc = ctx.input.rangeValues
      .map(v => ctx.input.rangeType === 'time' ? `${Math.round(v / 60)} min` : `${(v / 1000).toFixed(1)} km`)
      .join(', ');
