...
Supports **precision mode** for extracting data hidden in HTML tags and **proxy configuration** for geo-restricted pages.`,
    ...
    constraints: [
      'At least one of prompt or attributes must be provided.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    proxyCountry: z.string().optional().describe('Geographic location for proxy routing, e.g. "UnitedStates"'),
    cookies: z.array(cookieSchema).optional().describe('Cookies to send with the request for authenticated or session-based pages'),
  }))
  .output(z.object({
