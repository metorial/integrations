...
export let createScraperTool = SlateTool.create(
  spec,
  {
    name: 'Create Scraper',
    key: 'create_scraper',
...
Ideal for scenarios where the same type of data needs to be extracted from many pages with a similar structure.`,
  ...
  .input(z.object({
    ...
    sampleContent: z.string().optional().describe('Raw HTML or text content to generate the scraper from, as an alternative to sampleUrl'),
    prompt: z.string().optional().describe('Additional instructions for scraper generation'),
    attributes: z.array(attributeSchema).describe('List of named data fields the scraper should extract'),
    proxyCountry: z.string().optional().describe('Geographic location for proxy routing during generation'),
    cookies: z.array(cookieSchema).optional().describe('Cookies for accessing the sample page'),
  }))
  .output(z.object({
