...
export let issueNotification = SlateTrigger.create(
  spec,
  {
    name: 'Issue Opened',
    key: 'issue_opened',
    description: 'Triggers when a new issue is automatically opened in your Hookdeck project (delivery failure, transformation failure, or backpressure).',
  }
)
  .input(z.object({
    ...
    reference: z.record(z.string(), z.unknown()).optional().describe('Issue reference details'),
  ...
  .output(z.object({
    ...
    issueType: z.string().describe('Issue type (delivery, transformation, backpressure)'),
    issueStatus: z.string().describe('Issue status'),
    reference: z.record(z.string(), z.unknown()).optional().describe('Issue reference details (connection, source, destination info)'),
