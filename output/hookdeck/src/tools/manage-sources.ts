...
export let manageSources = SlateTool.create(
  ...
  .input(z.object({
    ...
    sourceId: z.string().optional().describe('Source ID (required for get, update, delete, enable, disable)'),
    name: z.string().optional().describe('Source name (required for create, optional for update/list)'),
    description: z.string().optional().describe('Source description'),
    type: z.string().optional().describe('Source type (e.g. WEBHOOK, STRIPE)'),
    verification: z.record(z.string(), z.unknown()).optional().describe('Verification config (e.g. { type: "STRIPE", configs: { webhook_secret_key: "..." } })'),
    limit: z.number().optional().describe('Max results to return (for list)'),
    cursor: z.string().optional().describe('Pagination cursor (for list)'),
  }))
  .output(z.object({
    source: sourceSchema.optional().describe('Single source (for get/create/update/enable/disable)'),
