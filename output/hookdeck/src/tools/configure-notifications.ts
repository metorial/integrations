...
let notificationConfigSchema = z.object({
  ...
  sourceId: z.string().nullable().optional().describe('Source ID receiving webhook notifications'),
  topics: z.array(z.string()).describe('Subscribed notification topics'),
});

export let configureNotifications = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    enabled: z.boolean().optional().describe('Enable or disable webhook notifications (for update)'),
    sourceId: z.string().optional().describe('Source ID to receive webhook notifications (for update)'),
    topics: z.array(z.enum(['issue.opened', 'event.successful'])).optional().describe('Notification topics to subscribe to (for update)'),
  }))
