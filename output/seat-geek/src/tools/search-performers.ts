...
export let searchPerformers = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    query: z.string().optional().describe('Full-text search query (e.g., "taylor swift", "yankees")'),
    performerIds: z.array(z.number()).optional().describe('Filter by specific performer IDs'),
    slug: z.string().optional().describe('Filter by performer slug'),
    taxonomyName: z.string().optional().describe('Filter by taxonomy name (e.g., "sports", "concert")'),
    taxonomyId: z.number().optional().describe('Filter by taxonomy ID'),
    genreSlug: z.string().optional().describe('Filter by genre slug (e.g., "rock", "hip-hop")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.primaryGenreSlug) params['genres[primary].slug'] = ctx.input.primaryGenreSlug;
    ...
    if (performers.length > 0) {
      ...
      for (let p of performers.slice(0, 5)) {
        let extra = p.numUpcomingEvents > 0 ? ` — ${p.numUpcomingEvents} upcoming events` : ' — no upcoming events';
