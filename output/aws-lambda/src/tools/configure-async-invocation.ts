...
export let configureAsyncInvocation = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    maximumEventAgeInSeconds: z.number().optional().describe('Max event age before discard (60-21600)'),
    onSuccessDestinationArn: z.string().optional().describe('Destination ARN for successful invocations'),
    onFailureDestinationArn: z.string().optional().describe('Destination ARN for failed invocations')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.maximumEventAgeInSeconds !== undefined) params['MaximumEventAgeInSeconds'] = ctx.input.maximumEventAgeInSeconds;

    let destConfig: Record<string, any> = {};
    if (ctx.input.onSuccessDestinationArn) destConfig['OnSuccess'] = { Destination: ctx.input.onSuccessDestinationArn };
    if (ctx.input.onFailureDestinationArn) destConfig['OnFailure'] = { Destination: ctx.input.onFailureDestinationArn };
