...
export let invokeFunction = SlateTool.create(
  ...
  .input(z.object({
    ...
    logType: z.enum(['None', 'Tail']).optional().describe('Set to "Tail" to include execution logs')
  }))
  .output(z.object({
    statusCode: z.number().optional().describe('HTTP status code (200=sync, 202=async, 204=dry run)'),
    response: z.any().optional().describe('Function response payload (synchronous only)'),
    functionError: z.string().optional().describe('Error type if the function returned an error'),
    executedVersion: z.string().optional().describe('Version that was executed'),
    logResult: z.string().optional().describe('Last 4KB of execution log (when logType is Tail)')
  }))
  .handleInvocation(async (ctx) => {
    let client = createClient(ctx.config, ctx.auth);

    let result = await client.invokeFunction(
