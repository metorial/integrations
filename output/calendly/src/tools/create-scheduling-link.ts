...
export let createSchedulingLink = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    eventTypeUri: z.string().describe('URI of the event type to create a scheduling link for'),
    maxEventCount: z.number().optional().describe('Maximum number of events that can be scheduled using this link (default: 1)')
  }))
  .output(z.object({
