...
export let checkAvailability = SlateTool.create(
  ...
  .input(z.object({
    eventTypeUri: z.string().optional().describe('Event type URI to check available slots for'),
    userUri: z.string().optional().describe('User URI to check busy times or availability schedules for'),
  ...
  .output(z.object({
    ...
    })).optional().describe('Available booking slots for the event type'),
    busyTimes: z.array(z.object({
    ...
    })).optional().describe('User busy time periods'),
    availabilitySchedules: z.array(z.object({
      scheduleUri: z.string(),
      name: z.string(),
      isDefault: z.boolean(),
      timezone: z.string(),
      rules: z.array(z.any())
    })).optional().describe('User availability schedules with rules')
  }))
  .handleInvocation(async (ctx) => {
    ...
    return {
      output,
      message: messages.join(' ') || 'No availability data requested. Provide eventTypeUri or userUri with appropriate parameters.'
    };
  })
  .build();
