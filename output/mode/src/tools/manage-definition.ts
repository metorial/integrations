...
export let listDefinitions = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    tokens: z.string().optional().describe('Comma-separated list of definition tokens to filter by'),
  }))
  .output(z.object({
...
export let manageDefinition = SlateTool.create(
  spec,
  {
...
Use **delete** to remove a definition.`,
  ...
  .input(z.object({
    ...
    definitionToken: z.string().optional().describe('Token of the definition (required for update/delete)'),
