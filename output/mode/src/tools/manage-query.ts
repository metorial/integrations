...
export let manageQuery = SlateTool.create(
  spec,
  {
    name: 'Manage Query',
    key: 'manage_query',
...
Use **delete** to remove a query from a report.`,
  ...
  .input(z.object({
    ...
    queryToken: z.string().optional().describe('Token of the query (required for update/delete)'),
    ...
    dataSourceId: z.number().optional().describe('ID of the data source to run the query against (create/update)'),
  }))
  .output(z.object({
    queries: z.array(queryOutputSchema).optional().describe('List of queries (for list action)'),
    query: queryOutputSchema.optional().describe('The created/updated query (for create/update actions)'),
  }))
