...
export let listCollections = SlateTool.create(
  ...
  .input(z.object({
    filter: z.enum(['all', 'custom']).optional().describe('Filter type: "all" returns all collections including unjoined ones, "custom" returns only custom collections'),
...
Use **delete** to remove an empty collection (all reports must be removed first).`,
  ...
  .input(z.object({
    ...
    collectionToken: z.string().optional().describe('Token of the collection (required for update/delete)'),
    name: z.string().optional().describe('Name for the collection (create/update)'),
    description: z.string().optional().describe('Description for the collection (create/update)'),
    spaceType: z.string().optional().describe('Type of collection (create only, defaults to "custom")'),
  }))
