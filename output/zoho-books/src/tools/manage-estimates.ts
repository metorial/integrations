...
export let listEstimatesTool = SlateTool.create(
  spec,
  {
    name: 'List Estimates',
    key: 'list_estimates',
...
export let createEstimateTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    convertToInvoice: z.boolean().optional().default(false).describe('Convert the estimate to an invoice immediately')
  }))
  .output(z.object({
    estimateId: z.string(),
    estimateNumber: z.string().optional(),
...
export let updateEstimateStatusTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['mark_sent', 'mark_accepted', 'mark_declined', 'convert_to_invoice']).describe('Action to perform on the estimate')
  }))
  .output(z.object({
    estimateId: z.string(),
    status: z.string().optional(),
