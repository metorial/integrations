...
export let generatePassword = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    length: z.number().min(1).max(64).default(32).describe('Length of the generated password (1-64, default 32)'),
    characterSets: z.array(z.enum(['LETTERS', 'DIGITS', 'SYMBOLS'])).optional().describe('Character sets to include. Each set listed guarantees at least one character of that type. Defaults to LETTERS and DIGITS.'),
    excludeCharacters: z.string().optional().describe('Characters to exclude from the generated password (e.g., "lI0O" to avoid ambiguous characters)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
      ...
      fields: [
        {
          ...
          recipe: {
            ...
            ...(ctx.input.excludeCharacters ? { excludeCharacters: ctx.input.excludeCharacters } : {}),
          } as any,
        },
      ],
    });
    ...
    try {
      ...
      ctx.warn('Failed to clean up temporary password item. You may want to delete it manually.');
    }
