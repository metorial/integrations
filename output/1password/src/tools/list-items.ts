...
export let listItems = SlateTool.create(
  ...
  .input(z.object({
    ...
    filter: z.string().optional().describe('Optional filter to narrow results. Supports filtering by title (e.g., "title eq \\"My Login\\"") or tag (e.g., "tag eq \\"production\\"").'),
  }))
  .output(z.object({
    items: z.array(z.object({
      itemId: z.string().describe('Unique identifier of the item'),
      title: z.string().describe('Title of the item'),
      category: z.string().describe('Category of the item (e.g., LOGIN, PASSWORD, API_CREDENTIAL)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: { items: mapped },
      message: `Found **${mapped.length}** item(s) in vault.${mapped.length > 0 ? '\n' + mapped.slice(0, 10).map((i) => `- **${i.title}** (${i.category})`).join('\n') + (mapped.length > 10 ? `\n- ...and ${mapped.length - 10} more` : '') : ''}`,
    };
  }).build();
