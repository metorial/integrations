...
export let createItem = SlateTool.create(
  ...
  .input(z.object({
    ...
    category: z.string().describe('Item category: LOGIN, PASSWORD, API_CREDENTIAL, SECURE_NOTE, CREDIT_CARD, IDENTITY, SSH_KEY, DATABASE, etc.'),
    ...
    })).optional().describe('URLs to associate with the item'),
    sections: z.array(z.object({
      sectionId: z.string().describe('Unique ID for the section'),
      label: z.string().optional().describe('Display label for the section'),
    })).optional().describe('Sections to organize fields into'),
    fields: z.array(z.object({
      label: z.string().describe('Label/name for the field'),
      value: z.string().optional().describe('Value of the field'),
      type: z.enum(['STRING', 'CONCEALED', 'URL', 'EMAIL', 'DATE', 'MONTH_YEAR', 'MENU', 'OTP', 'PHONE', 'ADDRESS', 'REFERENCE']).default('STRING').describe('Field type'),
      purpose: z.enum(['USERNAME', 'PASSWORD', 'NOTES', '']).optional().describe('Field purpose for login items'),
      sectionId: z.string().optional().describe('ID of the section this field belongs to'),
      generate: z.boolean().optional().describe('Whether to generate a value for this field (e.g., for passwords)'),
    })).optional().describe('Fields to add to the item'),
  }))
  .output(z.object({
