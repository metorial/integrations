...
export let getComments = SlateTool.create(
  ...
  .input(z.object({
    ...
    limit: z.number().optional().default(10).describe('Maximum number of comments to return'),
  ...
  .output(z.object({
    comments: z.array(z.object({
      ...
      }).optional().describe('Comment vote counts')
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.getComments(apiType, ctx.input.resourceId, ctx.input.limit, ctx.input.cursor);
