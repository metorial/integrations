...
let analysisStatsSchema = z.object({
...
export let getFileReport = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    lastAnalysisStats: analysisStatsSchema.describe('Summary of last analysis results by category'),
    tags: z.array(z.string()).optional().describe('Tags assigned to the file'),
    firstSubmissionDate: z.string().optional().describe('Date of first submission (Unix timestamp)'),
