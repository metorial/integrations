...
export let getAnalysisStatus = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    analysisId: z.string().describe('The analysis ID returned from a file or URL scan submission')
  }))
  .output(z.object({
    ...
    }).optional().describe('Analysis result statistics'),
    ...
    })).optional().describe('Detailed per-engine results (only when completed)')
  }))
