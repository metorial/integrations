...
let envVarValueSchema = z.object({
  ...
  context: z.enum(['all', 'dev', 'branch-deploy', 'deploy-preview', 'production']).describe('Deploy context for this value'),
  contextParameter: z.string().optional().describe('Branch name when context is "branch-deploy"'),
});
...
export let listEnvVars = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    siteId: z.string().optional().describe('Filter by site ID to show site-specific variables'),
  }))
  .output(z.object({
...
export let setEnvVars = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    accountId: z.string().describe('Account/team ID'),
    siteId: z.string().optional().describe('Site ID to scope the variables to a specific site'),
...
export let deleteEnvVar = SlateTool.create(
  spec,
  {
