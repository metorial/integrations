...
export let listAdmins = SlateTool.create(
  ...
  .input(z.object({
    limit: z.number().optional().describe('Maximum number of admins to return (default 100)'),
...
export let createAdmin = SlateTool.create(
  spec,
  {
    name: 'Create Admin',
    key: 'create_admin',
  ...
  .input(z.object({
    ...
    role: z.enum(['Owner', 'Administrator', 'User Manager', 'Security Analyst', 'Application Manager', 'Read-only', 'Billing']).optional().describe('Administrator role (default: Owner)'),
  }))
  .output(z.object({
    adminId: z.string(),
    name: z.string(),
...
export let updateAdmin = SlateTool.create(
  spec,
  {
    name: 'Update Admin',
    key: 'update_admin',
  ...
  .input(z.object({
    ...
    role: z.enum(['Owner', 'Administrator', 'User Manager', 'Security Analyst', 'Application Manager', 'Read-only', 'Billing']).optional().describe('New role'),
  }))
  .output(z.object({
    adminId: z.string(),
    name: z.string(),
...
export let deleteAdmin = SlateTool.create(
  spec,
  {
    name: 'Delete Admin',
    key: 'delete_admin',
