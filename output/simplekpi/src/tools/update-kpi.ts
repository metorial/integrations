...
export let updateKpi = SlateTool.create(
  spec,
  {
    name: 'Update KPI',
    key: 'update_kpi',
  ...
  .input(z.object({
    ...
    description: z.string().optional().nullable().describe('New description (max 150 characters)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.valueDirection !== undefined) data.value_direction = ctx.input.valueDirection;
    if (ctx.input.aggregateFunction !== undefined) data.aggregate_function = ctx.input.aggregateFunction;
