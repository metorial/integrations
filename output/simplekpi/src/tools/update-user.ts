...
export let updateUser = SlateTool.create(
  ...
  .input(z.object({
    ...
    userType: z.enum(['Admin', 'Director', 'Manager', 'User']).optional().describe('New user role'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.canManageUsers !== undefined) data.can_manage_users = ctx.input.canManageUsers;
    if (ctx.input.canAdminSettings !== undefined) data.can_admin_settings = ctx.input.canAdminSettings;
