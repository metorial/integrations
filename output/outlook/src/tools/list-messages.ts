...
export let listMessages = SlateTool.create(
  ...
  .input(z.object({
    folderId: z.string().optional().describe('Mail folder ID or well-known name (e.g., "inbox", "drafts", "sentitems")'),
    search: z.string().optional().describe('Full-text search query'),
    filter: z.string().optional().describe('OData filter expression'),
    orderby: z.string().optional().describe('OData orderby expression (e.g., "receivedDateTime desc")'),
    top: z.number().optional().describe('Maximum number of messages to return (default 10, max 1000)'),
