...
export let manageMessage = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['reply', 'replyAll', 'forward', 'move', 'update', 'delete']).describe('The action to perform'),
    comment: z.string().optional().describe('Comment for reply, replyAll, or forward actions'),
    forwardTo: z.array(recipientSchema).optional().describe('Recipients for forward action'),
    destinationFolderId: z.string().optional().describe('Destination folder ID or well-known name for move action'),
    isRead: z.boolean().optional().describe('Mark message as read/unread (for update action)'),
    importance: z.enum(['low', 'normal', 'high']).optional().describe('Set importance level (for update action)'),
    categories: z.array(z.string()).optional().describe('Set categories (for update action)'),
    flagStatus: z.enum(['notFlagged', 'complete', 'flagged']).optional().describe('Set flag status (for update action)'),
  }))
  .output(z.object({
    success: z.boolean(),
    messageId: z.string().optional().describe('ID of the message after the action (may change on move)'),
  }))
