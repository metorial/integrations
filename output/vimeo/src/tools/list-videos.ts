...
export let listVideosTool = SlateTool.create(
  spec,
  {
    name: 'List My Videos',
    key: 'list_videos',
  ...
  .input(paginationInputSchema.extend({
    query: z.string().optional().describe('Search query to filter videos by title or description'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: `Found **${result.total ?? videos.length}** videos` + (ctx.input.query ? ` matching "${ctx.input.query}"` : '')
    };
  }).build();
