...
import { channelSchema, videoSchema, paginationInputSchema, paginationOutputSchema, mapChannel, mapVideo } from '../lib/schemas';
...
export let getChannelTool = SlateTool.create(
  spec,
  {
    name: 'Get Channel',
    key: 'get_channel',
...
export let createChannelTool = SlateTool.create(
  spec,
  {
    name: 'Create Channel',
    key: 'create_channel',
  ...
  .input(z.object({
    ...
    privacy: z.enum(['anybody', 'moderators', 'user']).optional().describe('Who can access the channel'),
...
export let deleteChannelTool = SlateTool.create(
  spec,
  {
    name: 'Delete Channel',
    key: 'delete_channel',
...
export let listChannelVideosTool = SlateTool.create(
  spec,
  {
  ...
  .input(paginationInputSchema.extend({
    ...
    sort: z.enum(['added', 'alphabetical', 'comments', 'date', 'default', 'duration', 'likes', 'manual', 'modified_time', 'plays']).optional().describe('Sort order for the results')
  }))
  .output(paginationOutputSchema.extend({
...
export let manageChannelVideoTool = SlateTool.create(
  spec,
  {
