...
export let manageDataset = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'delete', 'upload_version']).describe('Action to perform'),
    datasetId: z.string().optional().describe('Dataset ID (required for update, delete, upload_version)'),
    ...
    csvContent: z.string().optional().describe('CSV content to upload as a new dataset version'),
    tsvContent: z.string().optional().describe('TSV content to upload as a new dataset version'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'upload_version') {
      ...
      if (!content) throw new Error('csvContent or tsvContent is required to upload a version');
