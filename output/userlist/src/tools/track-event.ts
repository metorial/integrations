...
Event names are normalized to \`snake_case\` internally.`,
  ...
  .input(z.object({
    name: z.string().describe('Name of the event (e.g. "project_created", "subscription_upgraded"). Normalized to snake_case.'),
    userIdentifier: z.string().optional().describe('Identifier of the user who performed the event.'),
    companyIdentifier: z.string().optional().describe('Identifier of the company context for the event.'),
    occurredAt: z.string().optional().describe('Timestamp when the event occurred, in RFC3339/ISO8601 format. Defaults to now.'),
    properties: z.record(z.string(), z.unknown()).optional().describe('Custom properties describing the event (e.g. plan name, amount).'),
  }))
  .output(z.object({
