...
export let messageEvents = SlateTrigger.create(
  spec,
  {
    name: 'Message Events',
    key: 'message_events',
    description: 'Triggers when a message is enqueued, opened, or clicked in Userlist. Covers email and in-app message delivery lifecycle events.',
  }
)
  .input(z.object({
    ...
    subject: z.string().optional().nullable().describe('Message subject, omitted for transactional messages.'),
    clickedUrl: z.string().optional().nullable().describe('Clicked URL for message_clicked events, omitted for transactional messages.'),
    occurredAt: z.string().describe('When the event occurred.'),
  }))
  .output(z.object({
    messageId: z.string().describe('Unique message instance identifier.'),
    subject: z.string().optional().nullable().describe('Message subject, omitted for transactional messages.'),
    clickedUrl: z.string().optional().nullable().describe('Clicked URL, only present for message_clicked events.'),
    userId: z.string().describe('Internal Userlist user ID of the recipient.'),
    userIdentifier: z.string().optional().nullable().describe('Application-provided user identifier.'),
