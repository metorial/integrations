...
export let relationshipEvents = SlateTrigger.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    relationship: z.record(z.string(), z.unknown()).describe('Raw relationship object from the webhook payload.'),
  ...
  .output(z.object({
    ...
    companyIdentifier: z.string().optional().nullable().describe('Application-provided company identifier.'),
    companyName: z.string().optional().nullable().describe('Company name.'),
    properties: z.record(z.string(), z.unknown()).optional().describe('Custom relationship properties (e.g. role).'),
