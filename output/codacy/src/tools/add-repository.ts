...
export let addRepository = SlateTool.create(
  spec,
  {
    name: 'Add Repository',
    key: 'add_repository',
  ...
  .input(z.object({
    repositoryFullPath: z.string().describe('Full path of the repository on the Git provider (e.g. "my-org/my-repo").'),
    provider: z.enum(['gh', 'ghe', 'gl', 'gle', 'bb', 'bbe']).optional().describe('Git provider identifier. Defaults to the configured provider.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let repoName = ctx.input.repositoryFullPath.split('/').pop() ?? ctx.input.repositoryFullPath;
