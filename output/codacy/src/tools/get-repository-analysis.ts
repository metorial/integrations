...
export let getRepositoryAnalysis = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    includeProgress: z.boolean().optional().default(false).describe('Whether to also fetch analysis progress status.'),
  }))
  .output(z.object({
    ...
    }).optional().describe('Current analysis progress, if requested.'),
  }))
