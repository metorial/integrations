...
export let listOrganizations = SlateTool.create(
  ...
  .input(z.object({
    ...
    limit: z.number().min(1).max(100).optional().describe('Maximum number of organizations to return (1-100).'),
  }))
  .output(z.object({
    organizations: z.array(z.object({
      ...
      joinStatus: z.string().optional().describe('User join status for this organization.'),
    })).describe('List of organizations.'),
    cursor: z.string().optional().describe('Pagination cursor for the next page, if available.'),
