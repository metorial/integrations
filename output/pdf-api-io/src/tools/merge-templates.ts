...
export let mergeTemplates = SlateTool.create(
  ...
  .input(z.object({
    templates: z.array(z.object({
      ...
      templateData: z.record(z.string(), z.unknown()).describe('Key-value pairs matching this template\'s variables'),
    })).min(1).describe('Array of templates to merge, each with its own dynamic data'),
    outputFormat: z.enum(['base64', 'url']).default('base64').describe('Output format: "base64" for base64-encoded PDF, "url" for a temporary download URL valid for 15 minutes'),
  }))
  .output(z.object({
    base64: z.string().optional().describe('Base64-encoded merged PDF content (when outputFormat is "base64")'),
    downloadUrl: z.string().optional().describe('Temporary download URL for the merged PDF, valid for 15 minutes (when outputFormat is "url")'),
  }))
