...
export let manageTableColumns = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    columnValues: z.array(z.array(z.any())).optional().describe('Column values as 2D array [[val1], [val2], ...] (for add)'),
    columnIndex: z.number().optional().describe('Zero-based position to insert the column at (for add)'),
    columnIdOrName: z.string().optional().describe('Column ID or name to delete (for delete)'),
  ...
  .output(z.object({
    columns: z.array(z.object({
      ...
      name: z.string(),
      index: z.number(),
    })).optional().describe('List of table columns (for list)'),
    column: z.object({
      columnId: z.string(),
      name: z.string(),
      index: z.number(),
    }).optional().describe('Added column details (for add)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'list': {
        let columns = await client.getTableColumns(ctx.input.workbookItemId, ctx.input.tableIdOrName);
        return {
      ...
      case 'delete': {
        ...
        if (!idOrName) throw new Error('columnIdOrName or columnName is required for delete action');
        await client.deleteTableColumn(ctx.input.workbookItemId, ctx.input.tableIdOrName, idOrName);
        return {
          output: { deleted: true },
