...
export let manageNamedItems = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    worksheetIdOrName: z.string().optional().describe('Worksheet ID or name to scope the named item to (for list or create)'),
    reference: z.string().optional().describe('Range reference formula (e.g., "Sheet1!$A$1:$D$10") for create'),
  ...
  .output(z.object({
    namedItems: z.array(z.object({
      name: z.string(),
      type: z.string(),
      value: z.any(),
      comment: z.string().optional(),
      visible: z.boolean(),
    })).optional().describe('List of named items'),
    namedItem: z.object({
      name: z.string(),
      type: z.string(),
      value: z.any(),
      comment: z.string().optional(),
      visible: z.boolean(),
    }).optional().describe('Named item details'),
    range: z.object({
      address: z.string(),
      values: z.array(z.array(z.any())),
      rowCount: z.number(),
      columnCount: z.number(),
    }).optional().describe('Range data for named item'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'list': {
        let items = await client.listNamedItems(ctx.input.workbookItemId, ctx.input.worksheetIdOrName);
        return {
