...
export let writeRange = SlateTool.create(
  spec,
  {
    name: 'Write Range',
    key: 'write_range',
  ...
  .input(z.object({
    ...
    action: z.enum(['write', 'clear', 'sort']).default('write').describe('Operation: write values, clear range, or sort range'),
    ...
    numberFormat: z.array(z.array(z.any())).optional().describe('2D array of number format strings to apply'),
    clearApplyTo: z.enum(['All', 'Formats', 'Contents']).optional().describe('What to clear (for clear action, default: All)'),
