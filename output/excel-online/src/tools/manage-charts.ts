...
export let manageCharts = SlateTool.create(
  spec,
  {
    name: 'Manage Charts',
    key: 'manage_charts',
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'get', 'create', 'update', 'delete', 'getImage', 'setData']).describe('Operation to perform'),
    chartName: z.string().optional().describe('Chart name (required for get, update, delete, getImage, setData)'),
    chartType: z.string().optional().describe('Chart type for creation (e.g., "ColumnClustered", "Line", "Pie")'),
    sourceDataRange: z.string().optional().describe('Source data range address (e.g., "Sheet1!A1:D5") for create or setData'),
    seriesBy: z.enum(['Auto', 'Columns', 'Rows']).optional().describe('How to derive data series (default: Auto)'),
    ...
    fittingMode: z.enum(['Fit', 'FitAndCenter', 'Fill']).optional().describe('Image fitting mode (for getImage)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'list': {
        let charts = await client.listCharts(ctx.input.workbookItemId, ctx.input.worksheetIdOrName);
        return {
      ...
      case 'get': {
        ...
        let chart = await client.getChart(ctx.input.workbookItemId, ctx.input.worksheetIdOrName, ctx.input.chartName);
        return {
      ...
      case 'create': {
        ...
        if (!ctx.input.sourceDataRange) throw new Error('sourceDataRange is required for create action');
      ...
      case 'update': {
        ...
        let chart = await client.updateChart(ctx.input.workbookItemId, ctx.input.worksheetIdOrName, ctx.input.chartName, props);
        return {
          output: { chart: mapChart(chart) },
      ...
      case 'delete': {
        ...
        await client.deleteChart(ctx.input.workbookItemId, ctx.input.worksheetIdOrName, ctx.input.chartName);
        return {
          output: { deleted: true },
      ...
      case 'setData': {
        ...
        if (!ctx.input.sourceDataRange) throw new Error('sourceDataRange is required for setData action');
