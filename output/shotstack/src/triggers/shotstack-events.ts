...
export let shotstackEventsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Shotstack Events',
    key: 'shotstack_events',
    description: 'Receives webhook callbacks for render completion, asset hosting, and ingestion events. Configure the webhook URL in your Shotstack render, ingest, or transfer request callback field.',
  },
)
  .input(z.object({
  ...
  .output(z.object({
    resourceId: z.string().describe('ID of the affected resource (render ID, asset ID, or source ID)'),
    ...
    owner: z.string().optional().describe('Account owner ID'),
    completed: z.string().optional().describe('Completion timestamp'),
    renderId: z.string().optional().describe('Associated render ID (for serve/hosting events)'),
  }))
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let errorStr = data.error ? (typeof data.error === 'string' ? data.error : JSON.stringify(data.error)) : undefined;
