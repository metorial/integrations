...
let renditionSchema = z.object({
  ...
  resolution: z.enum(['preview', 'mobile', 'sd', 'hd', '1080']).optional().describe('Resolution preset'),
  size: z.object({
    width: z.number().optional().describe('Width in pixels'),
    height: z.number().optional().describe('Height in pixels'),
  }).optional().describe('Custom output size'),
  ...
  speed: z.object({
    ...
    preservePitch: z.boolean().optional().describe('Preserve audio pitch when changing speed'),
  }).optional().describe('Speed adjustment'),
...
export let ingestSourceTool = SlateTool.create(
  spec,
  {
    name: 'Ingest Source',
    key: 'ingest_source',
  ...
  .input(z.object({
    ...
    renditions: z.array(renditionSchema).optional().describe('Transformations to apply during ingestion'),
    transcription: z.object({
      format: z.enum(['srt', 'vtt', 'json']).describe('Transcription output format'),
    }).optional().describe('Request a transcription of the media'),
    destinations: z.array(destinationSchema).optional().describe('Output destinations'),
    callback: z.string().optional().describe('Webhook URL for ingestion completion notifications'),
  }))
  .output(z.object({
...
export let getSourceTool = SlateTool.create(
  spec,
  {
    name: 'Get Source',
    key: 'get_source',
  ...
  .output(z.object({
    ...
    fps: z.number().optional().describe('Frames per second'),
    outputs: z.record(z.string(), z.any()).optional().describe('Rendition and transcription output details'),
...
export let requestUploadUrlTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    renditions: z.array(renditionSchema).optional().describe('Transformations to apply after upload'),
  }))
  .output(z.object({
