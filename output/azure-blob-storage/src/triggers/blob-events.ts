...
export let blobEvents = SlateTrigger.create(
  spec,
  {
    name: 'Blob Events',
    key: 'blob_events',
    description: 'Receives Azure Event Grid notifications for blob lifecycle events including creation, deletion, tier changes, and renames. Requires an Azure Event Grid subscription pointing to the webhook URL.',
  }
)
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      if (events.length > 0 && events[0]?.eventType === 'Microsoft.EventGrid.SubscriptionValidationEvent') {
    ...
    handleEvent: async (ctx) => {
      ...
      let eventType = typeMap[input.eventType] ?? input.eventType.toLowerCase().replace('microsoft.storage.', '');
