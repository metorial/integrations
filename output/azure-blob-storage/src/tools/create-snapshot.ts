...
export let createSnapshot = SlateTool.create(
  ...
  .input(z.object({
    ...
    metadata: z.record(z.string(), z.string()).optional().describe('Metadata to set on the snapshot'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.createSnapshot(ctx.input.containerName, ctx.input.blobName, ctx.input.metadata);
