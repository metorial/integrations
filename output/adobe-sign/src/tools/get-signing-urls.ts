...
export let getSigningUrls = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    signingUrlSetInfos: z.array(z.object({
      signingUrls: z.array(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let totalUrls = (result.signingUrlSetInfos || []).reduce(
      (sum: number, set: any) => sum + (set.signingUrls?.length || 0), 0
    );
