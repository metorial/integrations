...
let participantSchema = z.object({
  ...
  securityOption: z.object({
    authenticationMethod: z.enum(['NONE', 'PASSWORD', 'PHONE', 'KBA', 'EMAIL_OTP']).optional().describe('Authentication method for this participant')
  }).optional().describe('Security/authentication options for the participant')
});

let participantSetSchema = z.object({
  memberInfos: z.array(participantSchema).describe('List of participants in this set'),
  role: z.enum(['SIGNER', 'APPROVER', 'ACCEPTOR', 'FORM_FILLER', 'CERTIFIED_RECIPIENT', 'DELEGATE_TO_SIGNER', 'DELEGATE_TO_APPROVER']).describe('Role of the participants in this set'),
  order: z.number().optional().describe('Signing order for sequential workflows (1-based). Omit for parallel signing.')
});
...
let fileInfoSchema = z.object({
  ...
  }).optional().describe('URL-based document reference')
});
...
export let createAgreement = SlateTool.create(
  ...
  .input(z.object({
    ...
    participantSetsInfo: z.array(participantSetSchema).describe('List of participant sets (signers, approvers, etc.)'),
    ...
    state: z.enum(['IN_PROCESS', 'DRAFT', 'AUTHORING']).optional().describe('Initial state. "IN_PROCESS" sends immediately, "DRAFT" saves without sending. Defaults to "IN_PROCESS".'),
    ...
    })).optional().describe('Email addresses to CC on agreement notifications'),
    externalId: z.string().optional().describe('External identifier for tracking the agreement in your system'),
    reminderFrequency: z.enum(['DAILY_UNTIL_SIGNED', 'WEEKDAILY_UNTIL_SIGNED', 'EVERY_OTHER_DAY_UNTIL_SIGNED', 'EVERY_THIRD_DAY_UNTIL_SIGNED', 'EVERY_FIFTH_DAY_UNTIL_SIGNED', 'WEEKLY_UNTIL_SIGNED']).optional().describe('Automatic reminder frequency'),
    expirationTime: z.string().optional().describe('Agreement expiration date-time in ISO 8601 format')
  }))
  .output(z.object({
