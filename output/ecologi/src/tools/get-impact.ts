...
export let getImpactTool = SlateTool.create(
  spec,
  {
    name: 'Get Impact',
    key: 'get_impact',
  ...
  .input(z.object({
    ...
    metric: z.enum(['all', 'trees', 'carbon_offset']).default('all').describe('Which metric to retrieve: "all" for combined impact, "trees" for tree count only, or "carbon_offset" for carbon offset only.'),
  }))
  .output(z.object({
    trees: z.number().optional().describe('Total number of trees planted (confirmed/paid).'),
    treesPending: z.number().optional().describe('Number of trees pending payment processing.'),
    carbonOffsetTonnes: z.number().optional().describe('Total tonnes of CO₂e avoided (confirmed/paid).'),
    carbonOffsetTonnesPending: z.number().optional().describe('Tonnes of CO₂e avoided pending payment processing.'),
  }))
