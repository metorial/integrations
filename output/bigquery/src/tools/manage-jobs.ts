...
export let listJobs = SlateTool.create(
  ...
  .input(z.object({
    stateFilter: z.enum(['done', 'pending', 'running']).optional().describe('Filter by job state'),
    ...
    parentJobId: z.string().optional().describe('Filter by parent job ID (for scripting jobs)'),
    minCreationTime: z.string().optional().describe('Minimum creation time filter (epoch milliseconds)'),
    maxCreationTime: z.string().optional().describe('Maximum creation time filter (epoch milliseconds)')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let jobs = (result.jobs || []).map((j: any) => ({
      ...
      jobType: j.configuration ? Object.keys(j.configuration).find(k => k !== 'labels' && k !== 'dryRun' && k !== 'jobType') : undefined,
...
export let getJob = SlateTool.create(
  ...
  .input(z.object({
    ...
    location: z.string().optional().describe('Job location (overrides the configured default)')
  }))
  .output(z.object({
    jobId: z.string(),
    projectId: z.string(),
