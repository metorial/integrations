...
let geographyLevelSchema = z.object({
  ...
  referenceDate: z.string().optional().describe('Reference date for the geography definitions'),
  requires: z.array(z.string()).optional().describe('Parent geography levels required in the "in" clause'),
  wildcards: z.array(z.string()).optional().describe('Geography levels that support wildcard (*) queries'),
  optionalWithWCFor: z.string().optional().describe('Optional wildcard support description')
});

export let getGeography = SlateTool.create(
  spec,
  {
...
Use this to understand how to construct the \`forGeography\` and \`inGeography\` parameters for the Query Census Data tool.`,
  ...
  .output(z.object({
    geographies: z.array(geographyLevelSchema).describe('List of supported geography levels with hierarchy requirements'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let geographies = geoData.map((geo: any) => ({
      ...
      requires: geo.requires ? geo.requires.map((r: any) => typeof r === 'string' ? r : r.toString()) : undefined,
      wildcards: geo.wildcard ? (Array.isArray(geo.wildcard) ? geo.wildcard : [geo.wildcard]) : undefined,
