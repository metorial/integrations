...
let matchedAddressSchema = z.object({
  ...
  }).optional().describe('Geographic coordinates of the matched address'),
  tigerLine: z.object({
    tigerLineId: z.string().optional().describe('TIGER/Line ID'),
    side: z.string().optional().describe('Side of the street (L or R)')
  }).optional().describe('TIGER/Line segment information'),
  ...
  geographies: z.record(z.string(), z.array(z.record(z.string(), z.any()))).optional().describe('Census geographies the address falls within (states, counties, tracts, etc.)')
});
...
Supports structured address input (street, city, state, zip) or a single-line address string. Can also perform reverse geocoding from coordinates to address/geographies.`,
  ...
  .input(z.object({
    ...
    address: z.string().optional().describe('Single-line full address (alternative to structured input, e.g., "4600 Silver Hill Rd, Washington, DC 20233")'),
    ...
    includeGeographies: z.boolean().optional().describe('Set to true to include Census geographies (FIPS codes) in the response'),
    benchmark: z.string().optional().describe('Address benchmark to use (default: "Public_AR_Current")'),
    vintage: z.string().optional().describe('Geography vintage to use when includeGeographies is true (default: "Current_Current")')
  }))
  .output(z.object({
    matches: z.array(matchedAddressSchema).describe('List of matched addresses with coordinates and optionally geographies'),
    inputAddress: z.string().describe('The input address as interpreted by the geocoder')
  }))
  .handleInvocation(async (ctx) => {
    let geocoder = new GeocoderClient();
    let returnType = ctx.input.includeGeographies ? 'geographies' as const : 'locations' as const;
    let result: any;
    ...
    if (ctx.input.longitude !== undefined && ctx.input.latitude !== undefined) {
      ...
      throw new Error('Please provide either a street address, a single-line address, or longitude/latitude coordinates.');
    }

    let addressMatches = result.addressMatches || [];
    let inputAddress = result.input?.address?.address || result.input?.location?.x
