...
export let checkIp = SlateTool.create(
  spec,
  {
...
Use this to assess whether an IP address has been involved in malicious activity.`,
  ...
  .input(z.object({
    ...
    verbose: z.boolean().optional().describe('Include individual abuse reports in the response'),
  }))
  .output(z.object({
    ...
    isWhitelisted: z.boolean().nullable().describe('Whether the IP is on the AbuseIPDB whitelist'),
    ...
    usageType: z.string().nullable().describe('Usage type of the IP (e.g., Data Center, ISP, Commercial)'),
    ...
    reports: z.array(reportSchema).optional().describe('Individual abuse reports (only when verbose is true)'),
  }))
