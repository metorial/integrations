...
export let bulkReport = SlateTool.create(
  spec,
  {
...
Returns the number of successfully saved reports and details about any invalid entries.`,
  ...
  .input(z.object({
    csvContent: z.string().describe('CSV content with headers: IP,Categories,Comment,Timestamp. Each row represents one abuse report.'),
  }))
  .output(z.object({
    savedReports: z.number().describe('Number of reports successfully saved'),
    invalidReports: z.array(invalidReportSchema).describe('List of invalid entries that were rejected'),
  }))
