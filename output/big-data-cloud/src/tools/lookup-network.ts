...
export let lookupNetworkTool = SlateTool.create(
  spec,
  {
...
If no IP is provided, the caller's own IP network is returned.`,
  ...
  .input(z.object({
    ip: z.string().optional().describe('IPv4 or IPv6 address to look up. If omitted, the caller\'s IP is used.'),
  }))
  .output(z.object({
    ...
    isBogon: z.boolean().optional().describe('Whether the IP is a bogon (reserved/unallocated)'),
    ...
    }).passthrough()).optional().describe('Autonomous Systems serving this network'),
  }).passthrough())
  .handleInvocation(async (ctx) => {
    ...
    let carrierNames = carriers
      .slice(0, 3)
      .map((c: { organisation?: string; name?: string }) => c.organisation || c.name || 'unknown')
      .join(', ');
