...
let conditionSchema = z.object({
  column: z.string().describe('Column name to filter on (e.g., "total_investment_usd", "headcount", "largest_headcount_country", "company_website_domain").'),
  type: z.string().describe('Comparison operator: "=" (exact match), ">=" or "=>" (greater than or equal), "<=" (less than or equal), "(.)" (contains).'),
...
Best for data-driven screening with specific numeric thresholds (e.g., headcount > 500, total funding > $10M).`,
  ...
  .output(z.object({
    companies: z.array(z.record(z.string(), z.unknown())).describe('Array of screened company records.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let companies = Array.isArray(result) ? result : (result.data ?? result.companies ?? [result]);
