...
Ideal for sales prospecting and identifying key contacts at target companies.`,
  ...
  .input(z.object({
    decisionMakerTitles: z.array(z.string()).describe('Titles to search for (e.g., ["CEO", "CTO", "VP of Engineering"]).'),
    filterOp: z.enum(['and', 'or']).optional().describe('Logical operator for filter conditions.'),
    conditions: z.array(conditionSchema).optional().describe('Additional filter conditions on company data.'),
  ...
  .output(z.object({
    decisionMakers: z.array(z.record(z.string(), z.unknown())).describe('Array of decision maker records.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let decisionMakers = Array.isArray(result) ? result : (result.data ?? result.decision_makers ?? []);
