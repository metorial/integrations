...
Supports real-time crawling for companies not yet in the database.`,
  ...
  .input(z.object({
    companyDomain: z.string().optional().describe('Company website domain (e.g., "hubspot.com"). Supports comma-separated values for multiple companies.'),
    companyName: z.string().optional().describe('Company name to look up.'),
    companyId: z.string().optional().describe('Crustdata company ID.'),
    fields: z.array(z.string()).optional().describe('Specific fields to retrieve (e.g., ["company_name", "headcount.headcount", "job_openings", "news_articles"]).'),
    enrichRealtime: z.boolean().optional().describe('Enable real-time enrichment for companies not in the database.'),
  }))
  .output(z.object({
    companies: z.array(z.record(z.string(), z.unknown())).describe('Array of enriched company data objects.'),
  }))
