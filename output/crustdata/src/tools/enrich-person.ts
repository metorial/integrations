...
Works with LinkedIn URLs, emails, or name + company combinations.`,
  ...
  .input(z.object({
    linkedinProfileUrl: z.string().optional().describe('LinkedIn profile URL(s). Supports comma-separated values for batch enrichment (e.g., "https://linkedin.com/in/person1,https://linkedin.com/in/person2").'),
    email: z.string().optional().describe('Email address to look up.'),
    name: z.string().optional().describe('Full name of the person (use with companyName for best results).'),
    companyName: z.string().optional().describe('Current company name (used with name for matching).'),
    fields: z.array(z.string()).optional().describe('Specific fields to retrieve.'),
    enrichRealtime: z.boolean().optional().describe('Enable real-time enrichment for unfound profiles.'),
  }))
  .output(z.object({
    profiles: z.array(z.record(z.string(), z.unknown())).describe('Array of enriched person profiles.'),
  }))
