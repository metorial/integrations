...
Supports three modes: person posts, company posts, or keyword-based post search.`,
  ...
  .input(z.object({
    personLinkedinUrl: z.string().optional().describe('LinkedIn profile URL of the person whose posts to retrieve.'),
    companyLinkedinUrl: z.string().optional().describe('LinkedIn company URL whose posts to retrieve.'),
    keyword: z.string().optional().describe('Keyword to search across LinkedIn posts.'),
    sortBy: z.enum(['date', 'relevance']).optional().describe('Sort order for keyword search results.'),
    datePosted: z.string().optional().describe('Time filter for keyword search (e.g., "past-24h", "past-week", "past-month").'),
    page: z.number().optional().describe('Page number for pagination.'),
  }))
  .output(z.object({
    posts: z.array(z.record(z.string(), z.unknown())).describe('Array of social post objects with content and engagement metrics.'),
  }))
