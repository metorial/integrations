...
let peopleFilterSchema = z.object({
  filterType: z.string().describe('Filter type identifier. Options include: "CURRENT_COMPANY", "CURRENT_TITLE", "PAST_TITLE", "PAST_COMPANY", "SENIORITY_LEVEL", "FUNCTION", "REGION", "INDUSTRY", "COMPANY_HEADQUARTERS", "COMPANY_HEADCOUNT", "YEARS_AT_CURRENT_COMPANY", "YEARS_IN_CURRENT_POSITION", "YEARS_OF_EXPERIENCE", "PROFILE_LANGUAGE", "KEYWORD", "POSTED_ON_SOCIAL_MEDIA", "RECENTLY_CHANGED_JOBS", "IN_THE_NEWS".'),
  type: z.string().optional().describe('Filter operator: "in", "not in", or "between". Omit for boolean filters.'),
  value: z.unknown().optional().describe('Filter value. Arrays for "in"/"not in". Object with min/max for "between". Omit for boolean filters.'),
});
...
Supports combining multiple filters for precise targeting.`,
  ...
  constraints: [
    'Returns up to 25 profiles per page.',
  ],
  tags: {
    destructive: false,
    readOnly: true,
  ...
  .output(z.object({
    profiles: z.array(z.record(z.string(), z.unknown())).describe('Array of matching person profiles.'),
