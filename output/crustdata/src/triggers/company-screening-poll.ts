...
export let companyScreeningPoll = SlateTrigger.create(spec, {
  ...
  description: 'Polls for new companies that match specified screening criteria. Detects when new companies appear in search results based on filters like headcount, funding, industry, and growth metrics.',
})
  .input(z.object({
    ...
    pollEvents: async (ctx) => {
      ...
      let result = await client.screenCompanies({
        filters: {
          op: 'and',
          conditions: [
            { column: 'headcount', type: '>=', value: 1, allowNull: false },
          ],
        },
        offset: 0,
        count: 100,
      ...
      let allIds = companies.map((c: Record<string, unknown>) => String(c.company_id ?? c.id ?? '')).filter(Boolean);
      ...
      let inputs = newCompanies.map((c: Record<string, unknown>) => ({
        ...
        companyDomain: c.company_website_domain as string | undefined ?? c.domain as string | undefined,
        companyData: c,
      }));
