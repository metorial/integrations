...
export let runActionTool = SlateTool.create(
  spec,
  {
    name: 'Run Action',
    key: 'run_action',
  ...
  .input(z.object({
    ...
    actionId: z.string().describe('The action identifier (e.g., "google_calendar.create_event")'),
    userId: z.string().describe('The user ID to run the action for'),
    fields: z.record(z.string(), z.any()).optional().describe('Action field values specific to the action being run'),
    userFeatureId: z.string().optional().describe('Optional user feature ID to scope the action'),
  }))
  .output(z.object({
