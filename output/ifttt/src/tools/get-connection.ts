...
export let getConnectionTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    userId: z.string().optional().describe('The user ID to retrieve user-specific connection configuration'),
  }))
  .output(z.object({
    connectionId: z.string().describe('The connection ID'),
    status: z.string().optional().describe('The connection status (e.g., enabled, disabled)'),
    connection: z.any().describe('Full connection details including triggers, actions, queries, and user configuration'),
  }))
