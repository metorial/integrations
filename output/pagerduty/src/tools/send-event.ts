...
export let sendEvent = SlateTool.create(
  ...
  .input(z.object({
    ...
    routingKey: z.string().describe('32-character integration routing key from a PagerDuty service'),
    eventAction: z.enum(['trigger', 'acknowledge', 'resolve']).optional().describe('Alert event action (required for alert events)'),
    summary: z.string().optional().describe('Event summary (required for trigger and change events)'),
    severity: z.enum(['critical', 'error', 'warning', 'info']).optional().describe('Alert severity (for trigger events, defaults to "critical")'),
    ...
    dedupKey: z.string().optional().describe('Deduplication key (required for acknowledge/resolve)'),
    customDetails: z.record(z.string(), z.any()).optional().describe('Additional key-value pairs to include'),
    timestamp: z.string().optional().describe('Event timestamp for change events (ISO 8601)'),
    links: z.array(z.object({
      href: z.string().describe('Link URL'),
      text: z.string().optional().describe('Link display text'),
    })).optional().describe('Related links (for change events)'),
  }))
  .output(z.object({
