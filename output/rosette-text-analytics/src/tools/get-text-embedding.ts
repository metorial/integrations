...
export let getTextEmbeddingTool = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'Use perToken to get individual embeddings for each token in the text.',
    ],
    constraints: [
  ...
  .input(z.object({
    ...
    perToken: z.boolean().optional().describe('Return a vector for each individual token instead of the whole document'),
  }))
  .output(z.object({
    documentEmbedding: z.array(z.number()).optional().describe('Vector representation of the entire input text'),
    tokens: z.array(z.string()).optional().describe('Token strings when perToken is enabled'),
    tokenEmbeddings: z.array(z.array(z.number())).optional().describe('Per-token embedding vectors when perToken is enabled'),
  }))
