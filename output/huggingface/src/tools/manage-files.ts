...
export let listRepoFilesTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    revision: z.string().optional().describe('Git revision (branch, tag, or commit SHA; defaults to "main")')
  }))
  .output(z.object({
...
export let getFileContentTool = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'Only use for text files; binary files will not be readable.'
    ],
    tags: {
      readOnly: true
    }
