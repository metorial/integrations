...
export let updateIncident = SlateTool.create(
  ...
  .input(z.object({
    ...
    message: z.string().optional().describe('Update message body to add to the incident timeline'),
    impactOverride: z.enum(['none', 'minor', 'major', 'critical']).optional().describe('Override the calculated impact level'),
    componentIds: z.record(z.string(), z.string()).optional().describe('Map of component IDs to their updated status'),
    notifySubscribers: z.boolean().optional().describe('Whether to send notifications for this update'),
  ...
  .output(z.object({
    ...
    resolvedAt: z.string().optional().nullable().describe('Resolution timestamp, if resolved'),
