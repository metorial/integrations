...
- To **reactivate**: provide \`subscriberId\` and set \`resubscribe\` to true.`,
  ...
  .input(z.object({
    subscriberId: z.string().optional().describe('ID of an existing subscriber (for unsubscribe or resubscribe)'),
    type: z.enum(['email', 'sms', 'webhook', 'slack', 'teams', 'integration_partner']).optional().describe('Type of subscriber to create'),
    emailAddress: z.string().optional().describe('Email address for email subscribers'),
    phoneNumber: z.string().optional().describe('Phone number with country code for SMS subscribers'),
    phoneCountry: z.string().optional().describe('Two-letter country code for the phone number'),
    webhookEndpoint: z.string().optional().describe('Endpoint URL for webhook subscribers'),
    componentIds: z.array(z.string()).optional().describe('List of component IDs to subscribe to. Omit for page-wide subscription.'),
    skipConfirmationNotification: z.boolean().optional().describe('Skip sending the confirmation notification to the subscriber'),
    unsubscribe: z.boolean().optional().describe('Set to true to unsubscribe the subscriber'),
    resubscribe: z.boolean().optional().describe('Set to true to reactivate a previously unsubscribed subscriber'),
  }))
  .output(z.object({
    ...
    mode: z.string().optional().describe('Subscription mode (e.g. all or specific components)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.skipConfirmationNotification) data.skip_confirmation_notification = ctx.input.skipConfirmationNotification;
