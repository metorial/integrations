...
export let manageRoles = SlateTool.create(
  ...
  .input(z.object({
    ...
    canLogin: z.boolean().optional().default(false).describe('Whether the role can log in (create only)'),
    isSuperuser: z.boolean().optional().default(false).describe('Whether the role is a superuser (create only)'),
    canCreateDb: z.boolean().optional().default(false).describe('Whether the role can create databases (create only)'),
    ifExists: z.boolean().optional().default(false).describe('Add IF EXISTS clause (drop only)'),

    // For grant/revoke
    privileges: z.array(z.enum(['SELECT', 'INSERT', 'UPDATE', 'DELETE', 'TRUNCATE', 'REFERENCES', 'TRIGGER', 'ALL', 'USAGE', 'CREATE', 'CONNECT', 'EXECUTE'])).optional().describe('Privileges to grant or revoke'),
    on: z.object({
      objectType: z.enum(['TABLE', 'SCHEMA', 'DATABASE', 'SEQUENCE', 'FUNCTION', 'ALL TABLES IN SCHEMA', 'ALL SEQUENCES IN SCHEMA']).describe('Type of object to grant/revoke privileges on'),
      objectName: z.string().describe('Name of the object'),
      schemaName: z.string().optional().describe('Schema containing the object (for tables, sequences)'),
    }).optional().describe('Object to grant/revoke privileges on'),
  ...
  .output(z.object({
    ...
    roles: z.array(z.object({
      ...
      connectionLimit: z.number().describe('Maximum concurrent connections (-1 = unlimited)'),
    })).optional().describe('List of roles (for list action)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'list') {
      ...
      if (ctx.input.rolePassword) options.push(`PASSWORD '${ctx.input.rolePassword.replace(/'/g, "''")}'`);
      ...
      if (!ctx.input.privileges?.length) throw new Error('privileges are required for grant/revoke actions');
      if (!ctx.input.on) throw new Error('on (object specification) is required for grant/revoke actions');
      ...
      } else if (objectType === 'ALL TABLES IN SCHEMA' || objectType === 'ALL SEQUENCES IN SCHEMA') {
