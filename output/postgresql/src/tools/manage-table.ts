...
let columnDefinitionSchema = z.object({
  ...
  dataType: z.string().describe('PostgreSQL data type (e.g., text, integer, bigint, boolean, jsonb, timestamptz, uuid)'),
  nullable: z.boolean().optional().default(true).describe('Whether the column allows NULL values'),
  defaultValue: z.string().optional().describe('SQL expression for the default value (e.g., "now()", "0", "gen_random_uuid()")'),
  primaryKey: z.boolean().optional().default(false).describe('Whether this column is the primary key'),
...
For altering tables, supports adding columns, dropping columns, renaming columns, altering column types, and renaming the table.`,
  ...
  .input(z.object({
    ...
    // Create options
    columns: z.array(columnDefinitionSchema).optional().describe('Column definitions (required for create action)'),
    ifNotExists: z.boolean().optional().default(false).describe('Add IF NOT EXISTS clause for create action'),

    // Alter options
    addColumns: z.array(columnDefinitionSchema).optional().describe('Columns to add to the table'),
    dropColumns: z.array(z.string()).optional().describe('Column names to drop from the table'),
    renameColumn: z.object({
      from: z.string().describe('Current column name'),
      to: z.string().describe('New column name'),
    }).optional().describe('Rename a column'),
    ...
    })).optional().describe('Columns to alter'),

    // Drop options
    cascade: z.boolean().optional().default(false).describe('Use CASCADE when dropping (also drops dependent objects)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      for (let col of ctx.input.columns) {
        ...
        if (col.references) {
          let refTable = qualifiedTableName(col.references.tableName, col.references.schemaName);
      ...
      statements.push(`CREATE TABLE ${ifNotExists}${fullTableName} (\n  ${columnDefs.join(',\n  ')}\n)`);
      ...
      if (ctx.input.dropColumns) {
        for (let colName of ctx.input.dropColumns) {
          statements.push(`ALTER TABLE ${fullTableName} DROP COLUMN ${escapeIdentifier(colName)}`);
        }
      }
      ...
      if (ctx.input.alterColumns) {
        for (let alter of ctx.input.alterColumns) {
          ...
          if (alter.setDataType) {
            statements.push(`ALTER TABLE ${fullTableName} ALTER COLUMN ${colName} TYPE ${alter.setDataType}`);
          }
          if (alter.setDefault) {
            statements.push(`ALTER TABLE ${fullTableName} ALTER COLUMN ${colName} SET DEFAULT ${alter.setDefault}`);
          }
          if (alter.dropDefault) {
            statements.push(`ALTER TABLE ${fullTableName} ALTER COLUMN ${colName} DROP DEFAULT`);
          }
          if (alter.setNotNull) {
            statements.push(`ALTER TABLE ${fullTableName} ALTER COLUMN ${colName} SET NOT NULL`);
          }
          if (alter.dropNotNull) {
            statements.push(`ALTER TABLE ${fullTableName} ALTER COLUMN ${colName} DROP NOT NULL`);
          }
        }
      }

      if (ctx.input.renameTable) {
        statements.push(`ALTER TABLE ${fullTableName} RENAME TO ${escapeIdentifier(ctx.input.renameTable)}`);
      }

      if (statements.length === 0) {
        throw new Error('No alter operations specified. Provide addColumns, dropColumns, renameColumn, alterColumns, or renameTable.');
      }
    ...
    let actionLabel = ctx.input.action === 'create' ? 'Created'
      : ctx.input.action === 'alter' ? 'Altered'
      : 'Dropped';
