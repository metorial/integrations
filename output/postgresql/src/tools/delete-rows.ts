...
Supports returning the deleted rows and requires explicit confirmation for full-table deletes.`,
  ...
  .input(z.object({
    ...
    where: z.string().optional().describe('SQL WHERE condition (without the WHERE keyword). Example: "id = 1 AND status = \'inactive\'"'),
    confirmDeleteAll: z.boolean().optional().default(false).describe('Must be true to delete all rows when no WHERE condition is specified'),
    returning: z.boolean().optional().default(true).describe('Whether to return the deleted rows using RETURNING *'),
  }))
  .output(z.object({
    deletedCount: z.number().describe('Number of rows deleted'),
    returnedRows: z.array(z.record(z.string(), z.any())).describe('Deleted rows (if returning was enabled)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (!ctx.input.where && !ctx.input.confirmDeleteAll) {
      throw new Error('No WHERE condition specified. Set confirmDeleteAll to true to delete all rows from the table.');
    }
    ...
    ctx.info(`Deleting rows from ${fullTableName}${ctx.input.where ? ` where ${ctx.input.where}` : ' (all rows)'}`);
