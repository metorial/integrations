...
export let proposalStatusTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Proposal Status Change',
    key: 'proposal_status',
    description: 'Triggers when proposals reach a specific status: new, sent, opened, signed, or paid. Polls the Better Proposals API for proposals matching the selected status.',
  }
)
  .input(z.object({
    proposalId: z.string().describe('Unique identifier of the proposal'),
    proposalStatus: z.enum(['new', 'sent', 'opened', 'signed', 'paid']).describe('The status that triggered the event'),
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      for (let status of statuses) {
        ...
        let proposals = Array.isArray(result.data) ? result.data : (result.data ? [result.data] : []);
