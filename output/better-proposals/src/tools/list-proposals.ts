...
export let listProposals = SlateTool.create(
  ...
  .input(z.object({
    status: z.enum(['all', 'new', 'opened', 'sent', 'signed', 'paid']).optional().default('all').describe('Filter proposals by status. Defaults to all proposals.'),
    page: z.number().optional().describe('Page number for pagination. Increment until an empty list is returned.'),
  }))
  .output(z.object({
