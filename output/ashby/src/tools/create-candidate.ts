...
export let createCandidateTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    emailType: z.enum(['Personal', 'Work', 'Other']).optional().default('Personal').describe('Type of the email address'),
    phone: z.string().optional().describe('Phone number of the candidate'),
    phoneType: z.enum(['Personal', 'Work', 'Other', 'Mobile']).optional().default('Personal').describe('Type of the phone number'),
    ...
    })).optional().describe('Array of social profile links for the candidate')
  }))
  .output(z.object({
