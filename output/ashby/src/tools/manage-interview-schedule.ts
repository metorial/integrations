...
let scheduleOutputSchema = z.object({
  ...
  }).optional().describe('Single schedule result (for create, update, cancel)'),
  schedules: z.array(z.object({
    interviewScheduleId: z.string().describe('Interview schedule ID'),
    applicationId: z.string().optional().describe('Associated application ID'),
    createdAt: z.string().optional().describe('Creation timestamp'),
    updatedAt: z.string().optional().describe('Last updated timestamp'),
  })).optional().describe('List of schedules (for list action)'),
...
export let manageInterviewScheduleTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'cancel', 'list']).describe('The interview schedule action to perform'),
    interviewScheduleId: z.string().optional().describe('Interview schedule ID (required for update and cancel)'),
    applicationId: z.string().optional().describe('Application ID (required for create)'),
    interviewEvents: z.array(z.object({
    ...
    })).optional().describe('Interview events to schedule (for create action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { action, interviewScheduleId, applicationId, interviewEvents, cursor, perPage } = ctx.input;

    if (action === 'create') {
      if (!applicationId) throw new Error('applicationId is required to create an interview schedule');
      if (!interviewEvents || interviewEvents.length === 0) throw new Error('interviewEvents are required to create an interview schedule');
    ...
    if (action === 'update') {
      if (!interviewScheduleId) throw new Error('interviewScheduleId is required to update an interview schedule');
    ...
    if (action === 'cancel') {
      if (!interviewScheduleId) throw new Error('interviewScheduleId is required to cancel an interview schedule');
