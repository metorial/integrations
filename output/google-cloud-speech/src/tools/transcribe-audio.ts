...
Configure language, model, punctuation, word-level details, speaker diarization, and speech adaptation hints.`,
  ...
  .input(z.object({
    audioContent: z.string().optional().describe('Base64-encoded audio data. Provide this OR audioUri, not both.'),
    audioUri: z.string().optional().describe('Google Cloud Storage URI of the audio file (e.g. gs://bucket/file.wav). Provide this OR audioContent, not both.'),
    recognizerId: z.string().optional().describe('ID of a pre-configured recognizer. If omitted, uses the default recognizer with inline config.'),
    model: z.string().optional().describe('Recognition model to use (e.g. "latest_long", "latest_short", "telephony", "chirp", "chirp_2"). Defaults to the recognizer\'s model if a recognizer is specified.'),
    languageCodes: z.array(z.string()).optional().describe('BCP-47 language codes (e.g. ["en-US", "es-ES"]). First code is primary language.'),
    enableAutomaticPunctuation: z.boolean().optional().describe('Enable automatic punctuation in transcription results.'),
    enableWordTimeOffsets: z.boolean().optional().describe('Include start and end time offsets for each word.'),
    enableWordConfidence: z.boolean().optional().describe('Include confidence scores for each word.'),
    enableSpokenPunctuation: z.boolean().optional().describe('Recognize spoken punctuation (e.g. "period" becomes ".").'),
    minSpeakerCount: z.number().optional().describe('Minimum number of speakers for diarization.'),
    maxSpeakerCount: z.number().optional().describe('Maximum number of speakers for diarization.'),
    speechContextPhrases: z.array(z.string()).optional().describe('Phrases to bias recognition towards (speech adaptation hints).'),
    phraseBoost: z.number().optional().describe('Boost value for speech context phrases (0-20). Higher values increase bias.'),
    encoding: z.string().optional().describe('Audio encoding format (e.g. "LINEAR16", "FLAC", "MULAW", "OGG_OPUS", "MP3"). Omit for auto-detection.'),
    sampleRateHertz: z.number().optional().describe('Sample rate in Hertz. Required if encoding is specified.'),
    audioChannelCount: z.number().optional().describe('Number of audio channels. Defaults to 1.'),
  }))
  .output(z.object({
    ...
    results: z.array(z.object({
      ...
      words: z.array(z.object({
        ...
        startOffset: z.string().optional().describe('Start time offset of the word.'),
        ...
        speakerLabel: z.string().optional().describe('Speaker label from diarization.'),
      })).optional().describe('Word-level details, if requested.'),
      channelTag: z.number().optional().describe('Channel number for multi-channel audio.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let fullTranscript = results.map((r) => r.transcript || '').join(' ').trim();
