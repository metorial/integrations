...
export let createRecognizer = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    recognizerId: z.string().describe('Unique ID for the recognizer (lowercase letters, numbers, hyphens).'),
    displayName: z.string().optional().describe('Human-readable display name.'),
    model: z.string().describe('Recognition model (e.g. "latest_long", "latest_short", "telephony", "chirp", "chirp_2").'),
    languageCodes: z.array(z.string()).describe('BCP-47 language codes (e.g. ["en-US"]).'),
    enableAutomaticPunctuation: z.boolean().optional().describe('Enable automatic punctuation by default.'),
    enableWordTimeOffsets: z.boolean().optional().describe('Enable word time offsets by default.'),
    enableWordConfidence: z.boolean().optional().describe('Enable word confidence scores by default.'),
  }))
  .output(z.object({
...
export let listRecognizers = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    pageSize: z.number().optional().describe('Maximum number of recognizers to return (default 100).'),
    pageToken: z.string().optional().describe('Token for fetching the next page of results.'),
  }))
  .output(z.object({
    recognizers: z.array(z.object({
...
export let updateRecognizer = SlateTool.create(
  spec,
  {
...
export let deleteRecognizer = SlateTool.create(
  spec,
  {
...
export let getRecognizer = SlateTool.create(
  spec,
  {
    name: 'Get Recognizer',
    key: 'get_recognizer',
  ...
  .output(z.object({
    ...
    createTime: z.string().optional().describe('Creation timestamp.'),
    updateTime: z.string().optional().describe('Last update timestamp.'),
    defaultRecognitionConfig: z.record(z.string(), z.unknown()).optional().describe('Default recognition configuration.'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new SpeechToTextClient({
      token: ctx.auth.token,
