...
export let listVoices = SlateTool.create(
  ...
  .input(z.object({
    languageCode: z.string().optional().describe('BCP-47 language code to filter voices (e.g. "en-US", "de-DE"). If omitted, returns all available voices.'),
  }))
  .output(z.object({
    voices: z.array(z.object({
      voiceName: z.string().optional().describe('Voice identifier (e.g. "en-US-Wavenet-D").'),
      languageCodes: z.array(z.string()).optional().describe('Supported BCP-47 language codes.'),
      ssmlGender: z.string().optional().describe('Voice gender (MALE, FEMALE, NEUTRAL).'),
      naturalSampleRateHertz: z.number().optional().describe('Native sample rate of the voice in Hertz.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let languageFilter = ctx.input.languageCode ? ` for language **${ctx.input.languageCode}**` : '';
