...
export let batchTranscribeAudio = SlateTool.create(
  spec,
  {
...
Suitable for audio files longer than 1 minute (up to 8 hours). Results can be written to a GCS output location or returned inline when the operation completes.`,
  ...
  .input(z.object({
    fileUris: z.array(z.string()).describe('Google Cloud Storage URIs of audio files to transcribe (e.g. ["gs://bucket/audio1.wav", "gs://bucket/audio2.mp3"]).'),
    recognizerId: z.string().optional().describe('ID of a pre-configured recognizer. If omitted, uses the default recognizer.'),
    model: z.string().optional().describe('Recognition model to use (e.g. "latest_long", "chirp", "chirp_2").'),
    languageCodes: z.array(z.string()).optional().describe('BCP-47 language codes (e.g. ["en-US"]).'),
    outputUri: z.string().optional().describe('GCS URI for writing transcription results. If omitted, results are returned inline.'),
    enableAutomaticPunctuation: z.boolean().optional().describe('Enable automatic punctuation.'),
    enableWordTimeOffsets: z.boolean().optional().describe('Include word time offsets in results.'),
  ...
  .output(z.object({
    operationName: z.string().describe('Name of the long-running operation. Use this with Get Operation to check status.'),
