...
export let alertIssues = SlateTrigger.create(
  spec,
  {
    name: 'Alert Issues',
    key: 'alert_issues',
    description: 'Triggers when alert issues are created, updated, acknowledged, or closed in New Relic. Polls for issue state changes.',
  }
)
  .input(z.object({
    ...
    entityGuids: z.array(z.string()).optional().describe('Entity GUIDs associated with this issue'),
    entityNames: z.array(z.string()).optional().describe('Entity names associated with this issue'),
    conditionName: z.string().optional().describe('Alert condition that triggered this issue'),
    policyName: z.string().optional().describe('Alert policy name'),
    previousState: z.string().optional().describe('Previous state of the issue (for change detection)'),
  }))
  .output(z.object({
    issueId: z.string().describe('Alert issue ID'),
    title: z.string().optional().describe('Issue title'),
