...
export let createChangeTrackingMarker = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    entityGuid: z.string().describe('Entity GUID of the application or service being deployed'),
    ...
    deploymentType: z.enum(['BASIC', 'BLUE_GREEN', 'CANARY', 'OTHER', 'ROLLING', 'SHADOW']).optional().describe('Type of deployment'),
    deepLink: z.string().optional().describe('URL link to the deployment system (e.g. CI/CD pipeline)'),
