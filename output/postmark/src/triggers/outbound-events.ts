...
export let outboundEvents = SlateTrigger.create(
  spec,
  {
    name: 'Outbound Email Events',
    key: 'outbound_events',
    description: 'Triggers on outbound email events including bounces, deliveries, opens, clicks, spam complaints, and subscription changes. Postmark will POST event data to this webhook when any configured event occurs.',
  }
)
  .input(z.object({
    recordType: z.string().describe('Event type: Bounce, Delivery, Open, Click, SpamComplaint, or SubscriptionChange.'),
    ...
    metadata: z.record(z.string(), z.string()).optional().describe('Custom metadata from the original email.'),
    ...
    deliveryDetails: z.string().optional().describe('Delivery details from receiving server.'),
    ...
    platform: z.string().optional().describe('Platform type (Desktop, Mobile, WebMail, etc.).'),
    originalLink: z.string().optional().describe('Original tracked link URL (for click events).'),
    clickLocation: z.string().optional().describe('Where the link was clicked (HTML or Text).'),
    suppressSending: z.boolean().optional().describe('Whether sending is suppressed (for subscription changes).'),
    suppressionReason: z.string().optional().describe('Suppression reason (for subscription changes).'),
    origin: z.string().optional().describe('Change origin (for subscription changes).'),
    changedAt: z.string().optional().describe('Change timestamp (for subscription changes).'),
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      let eventTimestamp = ctx.input.bouncedAt || ctx.input.deliveredAt || ctx.input.receivedAt || ctx.input.changedAt;
