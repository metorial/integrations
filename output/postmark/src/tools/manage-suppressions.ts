...
export let manageSuppressions = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'delete']).describe('Action to perform on suppressions.'),
    streamId: z.string().describe('Message stream ID (e.g., "outbound", "broadcasts").'),
    emailAddresses: z.array(z.string()).optional().describe('Email addresses to suppress or unsuppress (for create/delete actions).'),
    suppressionReason: z.string().optional().describe('Filter by reason: "HardBounce", "SpamComplaint", "ManualSuppression" (for list).'),
    origin: z.string().optional().describe('Filter by origin: "Recipient", "Customer", "Admin" (for list).'),
    ...
    emailAddress: z.string().optional().describe('Filter by specific email address (for list).'),
  }))
  .output(z.object({
    suppressions: z.array(z.object({
      ...
      statusMessage: z.string().optional().describe('Status message (for create/delete).'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      let result = await client.createSuppressions(ctx.input.streamId, ctx.input.emailAddresses);
