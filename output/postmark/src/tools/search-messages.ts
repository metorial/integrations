...
export let searchMessages = SlateTool.create(
  ...
  .input(z.object({
    direction: z.enum(['outbound', 'inbound']).default('outbound').describe('Search outbound (sent) or inbound (received) messages.'),
    count: z.number().min(1).max(500).default(50).describe('Number of messages to return (max 500).'),
    ...
    status: z.string().optional().describe('Filter by status (e.g., "sent", "queued" for outbound; "processed", "blocked" for inbound).'),
  ...
  .output(z.object({
    ...
    messages: z.array(z.object({
      ...
      receivedAt: z.string().describe('Timestamp when the message was received by Postmark.'),
