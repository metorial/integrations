...
export let sendEmail = SlateTool.create(
  ...
  .input(z.object({
    from: z.string().describe('Sender email address. Must be a confirmed sender signature. Supports "Name <email>" format.'),
    to: z.string().describe('Recipient email address(es). Comma-separated for multiple recipients.'),
    cc: z.string().optional().describe('CC recipient email address(es). Comma-separated for multiple.'),
    bcc: z.string().optional().describe('BCC recipient email address(es). Comma-separated for multiple.'),
    ...
    messageStream: z.string().optional().describe('Message stream to send through (e.g., "outbound", "broadcasts").'),
    ...
    metadata: z.record(z.string(), z.string()).optional().describe('Custom metadata key-value pairs to include with the email.'),
  }))
  .output(z.object({
