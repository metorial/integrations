...
export let getBounces = SlateTool.create(
  spec,
  {
    name: 'Get Bounces',
    key: 'get_bounces',
  ...
  .input(z.object({
    action: z.enum(['list', 'stats', 'reactivate']).default('list').describe('Action to perform.'),
    count: z.number().min(1).max(500).default(50).describe('Number of bounces to return (for "list" action).'),
    offset: z.number().min(0).default(0).describe('Offset for pagination (for "list" action).'),
    bounceType: z.string().optional().describe('Filter by bounce type (e.g., "HardBounce", "SoftBounce", "SpamComplaint").'),
    inactive: z.boolean().optional().describe('Filter to only inactive recipients.'),
    emailFilter: z.string().optional().describe('Filter by recipient email address (partial match).'),
  ...
  .output(z.object({
    ...
    })).optional().describe('Bounce type breakdown (for stats).'),
    reactivated: z.boolean().optional().describe('Whether the bounce was successfully reactivated.'),
