...
Returns the synthesized audio as a base64-encoded string.`,
  ...
  .input(z.object({
    text: z.string().optional().describe('Plain text to synthesize. If provided, it will be wrapped in SSML using the specified voice. Mutually exclusive with ssml.'),
    ssml: z.string().optional().describe('Custom SSML markup for fine-grained speech synthesis control. Mutually exclusive with text.'),
    voiceName: z.string().optional().describe('Short name of the voice to use (e.g., "en-US-JennyNeural"). Required when using text input. Ignored when ssml is provided.'),
    language: z.string().optional().default('en-US').describe('Language code (e.g., "en-US"). Used when wrapping plain text in SSML.'),
    outputFormat: z.string().optional().default('riff-24khz-16bit-mono-pcm').describe('Audio output format (e.g., "audio-24khz-48kbitrate-mono-mp3", "riff-24khz-16bit-mono-pcm", "ogg-24khz-16bit-mono-opus").'),
  }))
  .output(z.object({
