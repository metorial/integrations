...
export let recognizeSpeech = SlateTool.create(
  spec,
  {
...
Optionally includes **pronunciation assessment** to evaluate the accuracy, fluency, completeness, and prosody of spoken audio against a reference text.`,
  ...
  .input(z.object({
    audioBase64: z.string().describe('Base64-encoded audio data (WAV PCM 16kHz mono or OGG Opus)'),
    language: z.string().describe('Language of the spoken audio (e.g., "en-US", "de-DE")'),
    audioFormat: z.enum(['wav', 'ogg']).optional().default('wav').describe('Audio format: "wav" or "ogg"'),
    resultFormat: z.enum(['simple', 'detailed']).optional().default('simple').describe('"simple" for basic results, "detailed" for N-best list with confidence scores'),
    profanityHandling: z.enum(['masked', 'removed', 'raw']).optional().describe('How to handle profanity in results'),
    pronunciationAssessment: z.object({
      referenceText: z.string().describe('The expected text to compare pronunciation against'),
      gradingSystem: z.enum(['FivePoint', 'HundredMark']).optional().describe('Scoring scale (default HundredMark)'),
      granularity: z.enum(['Phoneme', 'Word', 'FullText']).optional().describe('Level of scoring detail (default Phoneme)'),
      dimension: z.enum(['Basic', 'Comprehensive']).optional().describe('"Basic" for accuracy only, "Comprehensive" for fluency, completeness, prosody'),
      enableMiscue: z.boolean().optional().describe('Detect omitted, inserted, or repeated words'),
      enableProsodyAssessment: z.boolean().optional().describe('Assess stress, intonation, speed, and rhythm'),
    }).optional().describe('Enable pronunciation assessment with these parameters'),
  }))
  .output(z.object({
    recognitionStatus: z.string().describe('Recognition result status (e.g., "Success", "NoMatch", "Error")'),
    displayText: z.string().optional().describe('Recognized text with punctuation and capitalization'),
    ...
    })).optional().describe('N-best recognition alternatives (only with detailed format or pronunciation assessment)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let contentType = ctx.input.audioFormat === 'ogg'
    ...
    return {
      ...
      message: result.RecognitionStatus === 'Success'
