...
export let batchTranscriptionCompleted = SlateTrigger.create(
  spec,
  {
    name: 'Batch Transcription Completed',
    key: 'batch_transcription_completed',
    description: 'Triggers when a batch transcription job completes (succeeds or fails). Polls for status changes on all transcription jobs.',
  }
)
  .input(z.object({
    transcriptionId: z.string().describe('Unique identifier of the transcription'),
    status: z.string().describe('Terminal status of the transcription ("Succeeded" or "Failed")'),
  ...
  .output(z.object({
    ...
    createdAt: z.string().describe('ISO 8601 creation timestamp'),
    lastUpdatedAt: z.string().describe('ISO 8601 timestamp of last status update'),
    filesUri: z.string().optional().describe('URI to fetch transcription result files (when Succeeded)'),
  }))
  .polling({
    options: {
    ...
    pollEvents: async (ctx) => {
      ...
      let seenIds: Record<string, string> = (ctx.state?.seenCompletedIds as Record<string, string>) || {};
