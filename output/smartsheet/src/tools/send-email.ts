...
export let sendSheetEmail = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    format: z.enum(['PDF', 'EXCEL', 'CSV']).optional().describe('Format for the sheet attachment'),
    rowIds: z.array(z.string()).optional().describe('Send only these specific rows instead of the full sheet'),
    columnIds: z.array(z.string()).optional().describe('Include only these columns when sending rows')
  }))
  .output(z.object({
