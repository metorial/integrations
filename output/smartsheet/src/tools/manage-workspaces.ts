...
export let getWorkspace = SlateTool.create(
  ...
  .output(z.object({
    ...
    })).optional().describe('Sheets in this workspace'),
    folders: z.array(z.object({
      folderId: z.number().describe('Folder ID'),
      name: z.string().describe('Folder name')
    })).optional().describe('Folders in this workspace'),
    reports: z.array(z.object({
      reportId: z.number().describe('Report ID'),
      name: z.string().describe('Report name')
    })).optional().describe('Reports in this workspace'),
    dashboards: z.array(z.object({
      dashboardId: z.number().describe('Dashboard ID'),
      name: z.string().describe('Dashboard name')
    })).optional().describe('Dashboards in this workspace')
  }))
  .handleInvocation(async (ctx) => {
    let client = new SmartsheetClient({ token: ctx.auth.token });
