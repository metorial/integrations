...
export let downloadDocument = SlateTool.create(
  ...
  .input(z.object({
    ...
    type: z.enum(['raw', 'final']).default('final').describe('Download original (raw) document or the final signed version'),
    includeAuditTrail: z.boolean().optional().describe('Include the audit trail with the final document PDF'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let url = await client.getDownloadUrl(ctx.input.documentHash, ctx.input.type, ctx.input.includeAuditTrail);
