...
let signerSchema = z.object({
  ...
  order: z.number().optional().describe('Signing order (starts at 1). Required when useSignerOrder is enabled.'),
  ...
  deliverEmail: z.boolean().optional().describe('Set to false to prevent email delivery to this signer (e.g. for embedded signing)'),
  language: z.string().optional().describe('Two-letter language code for signer communication (e.g. en, de, fr, es)'),
});
...
let recipientSchema = z.object({
  ...
  language: z.string().optional().describe('Two-letter language code for recipient communication'),
});
...
let fieldSchema = z.object({
  type: z.enum(['signature', 'initials', 'date_signed', 'note', 'text', 'checkbox', 'radio', 'dropdown', 'attachment']).describe('Field type'),
  ...
  validationType: z.enum(['email_address', 'letters_only', 'numbers_only']).optional().describe('Validation rule'),
  textSize: z.number().optional().describe('Font size'),
  textColor: z.string().optional().describe('Hex color code for text'),
  textFont: z.string().optional().describe('Font family (arial, calibri, courier_new, helvetica, georgia, times_new_roman)'),
...
export let sendDocument = SlateTool.create(
  ...
  .input(z.object({
    ...
    recipients: z.array(recipientSchema).optional().describe('CC recipients who receive a copy'),
    fields: z.array(z.array(fieldSchema)).optional().describe('2D array of fields grouped per field set'),
    ...
    embeddedSigningEnabled: z.boolean().optional().describe('Enable embedded signing to receive signing URLs'),
    flexibleSigning: z.boolean().optional().describe('Allow signers to place their own signature fields'),
    redirect: z.string().optional().describe('URL to redirect signers to after signing'),
    redirectDecline: z.string().optional().describe('URL to redirect signers to after declining'),
    ...
    meta: z.record(z.string(), z.string()).optional().describe('Custom metadata key-value pairs'),
  }))
  .output(z.object({
    ...
    signers: z.array(z.object({
      ...
      embeddedSigningUrl: z.string().optional().describe('Embedded signing URL if enabled'),
