...
export let sendMessage = SlateTool.create(
  ...
  .input(z.object({
    ...
    eventId: z.string().optional().describe('Unique event ID for idempotency (unique per app for ~24h)'),
    payload: z.record(z.string(), z.unknown()).describe('JSON payload delivered as the webhook body'),
    channels: z.array(z.string()).optional().describe('Channels to tag the message with for endpoint filtering'),
    payloadRetentionPeriod: z.number().optional().describe('Number of days to retain the payload (default 90)'),
    autoCreateApp: z.object({
    ...
    }).optional().describe('If provided, automatically creates the application if it does not exist'),
  }))
  .output(z.object({
