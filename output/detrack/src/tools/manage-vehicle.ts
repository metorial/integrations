...
export let manageVehicleTool = SlateTool.create(
  spec,
  {
    name: 'Manage Vehicle',
    key: 'manage_vehicle',
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'delete']).describe('Operation to perform on the vehicle'),
    name: z.string().describe('Driver/vehicle name — used as the primary identifier'),
    detrackId: z.string().optional().describe('Detrack ID unique to the driver (for update only)'),
  }))
  .output(z.object({
    ...
    raw: z.record(z.string(), z.unknown()).optional().describe('Full vehicle response from Detrack'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let actionLabel = ctx.input.action === 'create' ? 'Created' : ctx.input.action === 'update' ? 'Updated' : 'Deleted';
