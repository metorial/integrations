...
export let takeScreenshot = SlateTool.create(
  ...
  .input(z.object({
    ...
    waitTime: z.number().optional().describe('Milliseconds to wait before taking the screenshot (0-5000)'),
    webhookUrl: z.string().optional().describe('URL to receive a POST notification when the screenshot is ready'),
  }))
  .output(z.object({
