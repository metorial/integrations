...
export let renderformEvents = SlateTrigger.create(
  spec,
  {
    name: 'RenderForm Events',
    key: 'renderform_events',
    description: 'Triggers when a render completes or a template is created, updated, or deleted.',
  }
)
  .input(z.object({
    action: z.enum(['RENDER_COMPLETE', 'TEMPLATE_CREATE', 'TEMPLATE_UPDATE', 'TEMPLATE_DELETE']).describe('The webhook action type'),
    ...
    templateName: z.string().optional().describe('Template name for template lifecycle events'),
  ...
  .output(z.object({
    action: z.string().describe('The event action (RENDER_COMPLETE, TEMPLATE_CREATE, TEMPLATE_UPDATE, TEMPLATE_DELETE)'),
