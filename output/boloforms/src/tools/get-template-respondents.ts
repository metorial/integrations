...
export let getTemplateRespondents = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    respondentDocumentId: z.string().optional().describe('Specific respondent document ID to retrieve a single respondent'),
    page: z.number().optional().describe('Page number for pagination (default: 1)'),
    limit: z.number().optional().describe('Number of results per page (default: 10)'),
  }))
  .output(z.object({
    respondents: z.array(z.record(z.string(), z.any())).describe('List of respondents with signer details and statuses'),
    pagination: z.object({
