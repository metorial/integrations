...
export let timeEntryEvents = SlateTrigger.create(
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let timeEntryId = timeEntry.id ? String(timeEntry.id) : (event.objectId ? String(event.objectId) : '');
      ...
      return {
        inputs: [{
          ...
          timeEntryId,
          projectId: timeEntry.projectId ? String(timeEntry.projectId) : (timeEntry['project-id'] ? String(timeEntry['project-id']) : undefined),
          eventPayload: data,
        }],
      };
    },
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        type: `time_entry.${ctx.input.eventType.replace(/^TIME_ENTRY\./, '').replace(/^TIMEENTRY\./, '').toLowerCase()}`,
        ...
        output: {
          ...
          taskId: te['todo-item-id'] ? String(te['todo-item-id']) : (te.taskId ? String(te.taskId) : undefined),
          personId: te['person-id'] ? String(te['person-id']) : (te.personId ? String(te.personId) : undefined),
          personName: te['person-first-name'] ? `${te['person-first-name']} ${te['person-last-name'] || ''}`.trim() : undefined,
          ...
          isBillable: te.isbillable === '1' || te.isbillable === true || te.isBillable === true || undefined,
