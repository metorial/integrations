...
export let manageTaskList = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    })).optional().describe('List of task lists (for list action)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      ...
      return {
        output: { taskListId: listId ? String(listId) : undefined, taskListName: ctx.input.name },
    ...
    if (action === 'list') {
      ...
      let lists = (result.tasklists || result['todo-lists'] || []).map((tl: any) => ({
        ...
        taskCount: tl['uncompleted-count'] != null ? Number(tl['uncompleted-count']) : undefined,
      }));
      return {
