...
export let listFolders = SlateTool.create(
  ...
  .input(z.object({
    ...
    folderIds: z.array(z.string()).optional().describe('Specific folder/project IDs to retrieve'),
    spaceId: z.string().optional().describe('Space ID to list folders from'),
    projectsOnly: z.boolean().optional().describe('If true, only return projects (not plain folders)'),
    fields: z.array(z.string()).optional().describe('Additional fields to include: description, briefDescription, customFields, etc.')
  }))
  .output(z.object({
