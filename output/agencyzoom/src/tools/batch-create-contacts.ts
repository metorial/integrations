...
export let batchCreateContacts = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    contacts: z.array(z.object({
      customerId: z.string().describe('ID of the customer to associate the contact with'),
      ...
      birthday: z.string().optional().describe('Birthday of the contact (ISO 8601 date format, e.g. "1990-01-15")'),
      address: z.object({
        street: z.string().optional().describe('Street address'),
        city: z.string().optional().describe('City'),
        state: z.string().optional().describe('State or province'),
        zip: z.string().optional().describe('ZIP or postal code'),
      }).optional().describe('Mailing address of the contact'),
    })).min(1).max(5).describe('Array of contacts to create (1 to 5 contacts)'),
  }))
  .output(z.object({
    contacts: z.array(z.record(z.string(), z.any())).describe('Array of created contact records'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let contacts = Array.isArray(result) ? result : (result?.data ?? result?.contacts ?? [result]);
