...
export let searchTasks = SlateTool.create(
  ...
  .input(z.object({
    search: z.string().optional().describe('Search query to filter tasks by title or content'),
    status: z.enum(['open', 'completed']).optional().describe('Filter by task status'),
    type: z.enum(['to-do', 'email', 'call', 'meeting']).optional().describe('Filter by task type'),
    assigneeId: z.string().optional().describe('Filter tasks by assignee ID'),
    fromDate: z.string().optional().describe('Start date for filtering tasks (ISO date string, e.g. "2024-01-01")'),
    toDate: z.string().optional().describe('End date for filtering tasks (ISO date string, e.g. "2024-12-31")'),
    leadId: z.string().optional().describe('Filter tasks associated with a specific lead'),
    customerId: z.string().optional().describe('Filter tasks associated with a specific customer'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let items = Array.isArray(result) ? result : (result.data ?? result.tasks ?? result.items ?? []);
