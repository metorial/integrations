...
export let searchCustomers = SlateTool.create(
  ...
  .input(z.object({
    search: z.string().optional().describe('Search term to filter customers by name, email, phone, or company'),
    type: z.enum(['personal', 'commercial']).optional().describe('Filter by customer type'),
    fromDate: z.string().optional().describe('Filter customers created on or after this date (ISO 8601 format, e.g. "2024-01-01")'),
    toDate: z.string().optional().describe('Filter customers created on or before this date (ISO 8601 format, e.g. "2024-12-31")'),
