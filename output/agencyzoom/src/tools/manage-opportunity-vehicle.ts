...
export let manageOpportunityVehicle = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['create', 'update', 'delete']).describe('Action to perform on the vehicle'),
    vehicleId: z.string().optional().describe('ID of the vehicle (required for "update" and "delete" actions)'),
    ...
    ownershipType: z.string().optional().describe('Vehicle ownership type (e.g. "owned", "leased", "financed")'),
  }))
  .output(z.object({
    vehicle: z.record(z.string(), z.any()).optional().describe('Vehicle data (for "create" and "update" actions)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'create': {
        ...
        if (ctx.input.ownershipType !== undefined) data.ownershipType = ctx.input.ownershipType;
      ...
      case 'update': {
        ...
        let result = await client.updateOpportunityVehicle(ctx.input.opportunityId, ctx.input.vehicleId, data);
        return {
          output: { vehicle: result },
