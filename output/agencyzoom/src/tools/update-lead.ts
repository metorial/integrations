...
export let updateLead = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['update', 'change_status', 'mark_sold']).describe('Operation to perform: "update" fields, "change_status", or "mark_sold"'),
    ...
    agentId: z.string().optional().describe('Updated agent/producer ID (for "update" action)'),
    pipelineId: z.string().optional().describe('Updated pipeline ID (for "update" action)'),
    stageId: z.string().optional().describe('Updated pipeline stage ID (for "update" action)'),
    leadSourceId: z.string().optional().describe('Updated lead source ID (for "update" action)'),
    tags: z.array(z.string()).optional().describe('Updated tags (for "update" action)'),
    customFields: z.record(z.string(), z.any()).optional().describe('Updated custom fields (for "update" action)'),
    status: z.enum(['new', 'contacted', 'quoted', 'won', 'lost', 'x-dated']).optional().describe('New status for the lead (for "change_status" action)'),
    ...
    premium: z.number().optional().describe('Premium amount in dollars (for "mark_sold" action)'),
    items: z.number().optional().describe('Number of policy items (for "mark_sold" action)'),
    effectiveDate: z.string().optional().describe('Policy effective date in ISO 8601 format (for "mark_sold" action)'),
    expiryDate: z.string().optional().describe('Policy expiry date in ISO 8601 format (for "mark_sold" action)'),
  }))
  .output(z.object({
    lead: z.record(z.string(), z.any()).describe('The updated lead data returned by AgencyZoom'),
  }))
