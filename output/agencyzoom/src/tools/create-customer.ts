...
export let createCustomer = SlateTool.create(
  ...
  .input(z.object({
    ...
    type: z.enum(['personal', 'commercial']).optional().describe('Customer type (defaults to personal if not specified)'),
    companyName: z.string().optional().describe('Company name (typically used for commercial customers)'),
    fein: z.string().optional().describe('Federal Employer Identification Number for commercial customers'),
    businessEntity: z.string().optional().describe('Business entity type (e.g. LLC, Corporation, Partnership)'),
    classification: z.string().optional().describe('Business classification or industry code'),
    employeeCount: z.number().optional().describe('Number of employees for commercial customers'),
    annualRevenue: z.number().optional().describe('Annual revenue in dollars for commercial customers'),
    payroll: z.number().optional().describe('Annual payroll amount in dollars for commercial customers'),
    tags: z.array(z.string()).optional().describe('Array of tag names to assign to the customer'),
    customFields: z.record(z.string(), z.any()).optional().describe('Key-value pairs of custom field names and their values'),
  }))
  .output(z.object({
    customer: z.record(z.string(), z.any()).describe('The newly created customer record with all fields'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let customerName = `${customer.firstName || ctx.input.firstName} ${customer.lastName || ctx.input.lastName}`.trim();
