...
export let createLead = SlateTool.create(
  ...
  .input(z.object({
    type: z.enum(['personal', 'business']).describe('Type of lead to create: "personal" for individual or "business" for commercial'),
    ...
    agentId: z.string().optional().describe('ID of the agent/producer to assign the lead to'),
    ...
    customFields: z.record(z.string(), z.any()).optional().describe('Custom field key-value pairs to set on the lead'),
    ...
    fein: z.string().optional().describe('Federal Employer Identification Number (business leads only)'),
    businessEntity: z.string().optional().describe('Business entity type, e.g. LLC, Corporation (business leads only)'),
    classification: z.string().optional().describe('Business classification or industry (business leads only)'),
    employeeCount: z.number().optional().describe('Number of employees (business leads only)'),
    annualRevenue: z.number().optional().describe('Annual revenue in dollars (business leads only)'),
    payroll: z.number().optional().describe('Annual payroll in dollars (business leads only)'),
  }))
  .output(z.object({
    lead: z.record(z.string(), z.any()).describe('The created lead data returned by AgencyZoom'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let result = ctx.input.type === 'business'
