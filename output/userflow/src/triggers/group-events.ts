...
export let groupEvents = SlateTrigger.create(
  spec,
  {
    name: 'Group Events',
    key: 'group_events',
    description: 'Triggers when a group (company) is created or updated in Userflow. For updates, includes both previous and updated attributes.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    attributes: z.record(z.string(), z.unknown()).describe('Current group attributes'),
    previousAttributes: z.record(z.string(), z.unknown()).optional().describe('Previous attribute values (only for group.updated)'),
    updatedAttributes: z.record(z.string(), z.unknown()).optional().describe('Changed attribute values (only for group.updated)'),
