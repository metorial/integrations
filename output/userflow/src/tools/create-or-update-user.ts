...
let attributeOperationSchema = z.union([
  z.string(),
  z.number(),
  z.boolean(),
  z.null(),
...
]).describe('Attribute value or operation object (set, set_once, add, subtract, append, prepend, remove)');

let groupEmbedSchema = z.object({
  groupId: z.string().describe('ID of the group to associate the user with'),
  attributes: z.record(z.string(), z.unknown()).optional().describe('Attributes to set on the group'),
}).describe('Group to associate with the user');

let membershipEmbedSchema = z.object({
  group: z.object({
    groupId: z.string().describe('ID of the group'),
    attributes: z.record(z.string(), z.unknown()).optional().describe('Attributes to set on the group'),
  }).describe('Group reference for the membership'),
  attributes: z.record(z.string(), z.unknown()).optional().describe('Membership-level attributes (e.g. role)'),
...
export let createOrUpdateUser = SlateTool.create(
  ...
  .input(z.object({
    ...
    attributes: z.record(z.string(), attributeOperationSchema).optional().describe('User attributes to set or update'),
    groups: z.array(groupEmbedSchema).optional().describe('Groups to associate user with (cannot be combined with memberships)'),
    memberships: z.array(membershipEmbedSchema).optional().describe('Memberships with group and membership-level attributes (cannot be combined with groups)'),
    pruneMemberships: z.boolean().optional().describe('If true, removes memberships not included in this request'),
  }))
  .output(z.object({
