...
export let detectLanguage = SlateTool.create(
  ...
  .input(z.object({
    ...
    maxResults: z.number().optional().describe('Maximum number of detected locales to return'),
  }))
  .output(z.object({
    detectedLocales: z.array(detectedLocaleSchema).describe('List of detected locales sorted by confidence'),
  }))
