...
let appSchema = z.object({
  ...
  trainingStatus: z.string().optional().describe('Training status: done, scheduled, or ongoing'),
});
...
export let createApp = SlateTool.create(
  ...
  .input(z.object({
    ...
    timezone: z.string().optional().describe('Default timezone for the app (e.g., "America/Los_Angeles")'),
  }))
  .output(z.object({
