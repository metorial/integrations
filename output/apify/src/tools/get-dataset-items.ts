...
export let getDatasetItems = SlateTool.create(
  spec,
  {
...
Use the dataset ID from an Actor run's output or from listing datasets.`,
  ...
  .input(z.object({
    ...
    runId: z.string().optional().describe('Actor run ID to fetch default dataset items from (alternative to datasetId)'),
    limit: z.number().optional().default(100).describe('Maximum number of items to return (max 250000)'),
    offset: z.number().optional().default(0).describe('Pagination offset'),
    fields: z.array(z.string()).optional().describe('Only include these fields in the output'),
    omit: z.array(z.string()).optional().describe('Exclude these fields from the output'),
    descending: z.boolean().optional().default(false).describe('Return items in reverse order'),
    clean: z.boolean().optional().default(false).describe('Remove empty items and hidden fields'),
  }))
  .output(z.object({
