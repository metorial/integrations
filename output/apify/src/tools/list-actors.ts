...
export let listActors = SlateTool.create(
  spec,
  {
    name: 'List Actors',
    key: 'list_actors',
...
Use this to discover available Actors, find Actor IDs for running, or review your Actor inventory.`,
  ...
  .input(z.object({
    myActorsOnly: z.boolean().optional().default(true).describe('If true, only list your own Actors. If false, also includes Actors from the Store.'),
    limit: z.number().optional().default(25).describe('Maximum number of Actors to return'),
    offset: z.number().optional().default(0).describe('Pagination offset'),
    descending: z.boolean().optional().default(true).describe('Sort in descending order by creation date'),
  }))
  .output(z.object({
