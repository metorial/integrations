...
export let emailCount = SlateTool.create(
  spec,
  {
    name: 'Email Count',
    key: 'email_count',
  ...
  .input(z.object({
    domain: z.string().optional().describe('Domain name to count emails for (e.g., "stripe.com")'),
  ...
  .output(z.object({
    ...
    })).optional().describe('Breakdown by department'),
    seniorities: z.array(z.object({
      seniority: z.string().describe('Seniority level'),
      count: z.number().describe('Number of emails at this seniority level'),
    })).optional().describe('Breakdown by seniority level'),
  }))
