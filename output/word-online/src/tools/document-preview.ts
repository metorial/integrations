...
export let documentPreview = SlateTool.create(
  spec,
  {
...
The returned URL can be embedded in an iframe to display a temporary read-only preview of the document.`,
    ...
    constraints: [
      'Only available for documents in SharePoint and OneDrive for Business.',
    ],
  }
)
  .input(z.object({
  ...
  .output(z.object({
    previewGetUrl: z.string().describe('Embeddable GET URL for rendering a preview (short-lived)'),
    previewPostUrl: z.string().describe('Embeddable POST URL for rendering a preview (short-lived)'),
  }))
