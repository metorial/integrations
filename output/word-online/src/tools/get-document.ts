...
Look up a document by its item ID or by its full path within the drive. Returns file metadata including name, size, URLs, and modification history.`,
  ...
  .input(z.object({
    itemId: z.string().optional().describe('The unique ID of the drive item. Provide either itemId or filePath.'),
    filePath: z.string().optional().describe('The path to the file relative to the drive root (e.g. "/Documents/Report.docx"). Provide either itemId or filePath.'),
  }))
  .output(z.object({
    ...
    downloadUrl: z.string().optional().describe('Pre-authenticated download URL (short-lived)'),
  }))
