...
export let createUploadSession = SlateTool.create(
  spec,
  {
...
Returns an upload URL that accepts byte range uploads. Use this for large Word documents or other files that exceed the 4MB direct upload limit.`,
  ...
  .input(z.object({
    parentFolderId: z.string().describe('ID of the parent folder where the file will be created'),
    fileName: z.string().describe('Name of the file to create (e.g. "LargeReport.docx")'),
  }))
  .output(z.object({
    uploadUrl: z.string().describe('The resumable upload URL. Send PUT requests with byte ranges to this URL.'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let session = await client.createUploadSession(ctx.input.parentFolderId, ctx.input.fileName);
