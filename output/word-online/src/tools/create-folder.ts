...
If a folder with the same name already exists, the new folder will be automatically renamed.`,
  ...
  .input(z.object({
    ...
    parentFolderId: z.string().optional().describe('ID of the parent folder. If omitted, creates at the drive root.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let folder = await client.createFolder(ctx.input.parentFolderId || '', ctx.input.folderName);
