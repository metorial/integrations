...
export let hlrLookupTool = SlateTool.create(
  spec,
  {
    name: 'HLR Lookup',
    key: 'hlr_lookup',
...
Useful for cleaning contact lists and reducing failed call/SMS attempts.`,
    constraints: [
      'HLR check rates are approximately 0.005 EUR per lookup.',
    ],
    tags: {
      destructive: false,
      readOnly: true,
  ...
  .input(z.object({
    phoneNumber: z.string().describe('Phone number to verify (in international format, e.g. "+37061234567")'),
  }))
  .output(z.object({
    phoneNumber: z.string().describe('The phone number that was checked'),
    result: z.any().describe('HLR lookup result containing availability, network, and status information'),
  }))
