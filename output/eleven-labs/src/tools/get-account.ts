...
export let getAccount = SlateTool.create(
  ...
  .output(z.object({
    ...
    characterCount: z.number().optional().describe('Characters used in current billing period'),
    characterLimit: z.number().optional().describe('Maximum characters allowed in current billing period'),
    ...
    canUseInstantVoiceCloning: z.boolean().optional().describe('Whether instant voice cloning is available'),
    canUseProfessionalVoiceCloning: z.boolean().optional().describe('Whether professional voice cloning is available'),
    status: z.string().optional().describe('Subscription status'),
    billingPeriod: z.string().optional().describe('Billing period (e.g. monthly_period, annual_period)'),
    nextCharacterCountReset: z.number().optional().describe('Unix timestamp of next character count reset'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {
        ...
        canUseInstantVoiceCloning: subscription['can_use_instant_voice_cloning'] as boolean | undefined,
        canUseProfessionalVoiceCloning: subscription['can_use_professional_voice_cloning'] as boolean | undefined,
