...
export let listVoices = SlateTool.create(
  ...
  .input(z.object({
    search: z.string().optional().describe('Search term to filter voices by name, description, labels, or category'),
    voiceType: z.enum(['personal', 'community', 'default', 'workspace', 'non-default', 'saved']).optional().describe('Filter voices by type'),
    category: z.enum(['premade', 'cloned', 'generated', 'professional']).optional().describe('Filter voices by category'),
    pageSize: z.number().min(1).max(100).optional().describe('Number of voices to return per page (max 100, default 10)'),
    nextPageToken: z.string().optional().describe('Pagination token from a previous response'),
    sort: z.enum(['created_at_unix', 'name']).optional().describe('Field to sort results by'),
