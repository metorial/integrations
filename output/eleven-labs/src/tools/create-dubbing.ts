...
export let createDubbing = SlateTool.create(
  spec,
  {
    name: 'Create Dubbing',
    key: 'create_dubbing',
  ...
  .input(z.object({
    ...
    sourceLang: z.string().optional().describe('Source language code. Defaults to auto-detection.'),
    name: z.string().optional().describe('Name for the dubbing project'),
    numSpeakers: z.number().optional().describe('Number of speakers (0 for auto-detection, recommended max 9)'),
    watermark: z.boolean().optional().describe('Apply a watermark to the output video'),
    highestResolution: z.boolean().optional().describe('Use the highest resolution available'),
  }))
  .output(z.object({
