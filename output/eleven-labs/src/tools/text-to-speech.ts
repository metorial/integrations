...
export let textToSpeech = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum text length depends on your subscription tier and the model used.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    ...
    modelId: z.string().optional().describe('Model ID to use. Defaults to "eleven_multilingual_v2". Use "eleven_flash_v2_5" for low latency.'),
    languageCode: z.string().optional().describe('ISO 639-1 language code to enforce for the model'),
    outputFormat: z.string().optional().describe('Audio output format, e.g. "mp3_44100_128", "pcm_16000", "ulaw_8000". Defaults to mp3.'),
    seed: z.number().optional().describe('Seed for deterministic generation (0-4294967295)'),
    applyTextNormalization: z.enum(['auto', 'on', 'off']).optional().describe('Text normalization mode. Defaults to "auto".'),
    voiceSettings: z.object({
      stability: z.number().min(0).max(1).optional().describe('Voice stability (0-1). Lower values add more variability.'),
      similarityBoost: z.number().min(0).max(1).optional().describe('Similarity boost (0-1). Higher values are more faithful to the original voice.'),
      ...
      speed: z.number().min(0.25).max(4.0).optional().describe('Speed multiplier (0.25-4.0). 1.0 is normal speed.'),
    }).optional().describe('Voice settings overrides for this request'),
  }))
  .output(z.object({
