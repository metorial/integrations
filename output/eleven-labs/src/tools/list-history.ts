...
export let listHistory = SlateTool.create(
  spec,
  {
    name: 'List History',
    key: 'list_history',
  ...
  .input(z.object({
    pageSize: z.number().min(1).max(1000).optional().describe('Number of history items to return (max 1000, default 100)'),
    startAfterHistoryItemId: z.string().optional().describe('History item ID to start after for pagination'),
  }))
  .output(z.object({
    ...
    lastHistoryItemId: z.string().optional().describe('ID of the last item (use for pagination)'),
  }))
