...
export let voiceAgentCall = SlateTrigger.create(
  spec,
  {
    name: 'Voice Agent Call',
    key: 'voice_agent_call',
    description: 'Triggered when a Voice Agent (ElevenAgents) call completes, fails to initiate, or when post-call audio is available. Covers post-call transcription, post-call audio, and call initiation failure events.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    })).optional().describe('Conversation transcript'),
    analysis: z.any().optional().describe('Post-call analysis results'),
    audioBase64: z.string().optional().describe('Base64-encoded call audio (for post_call_audio events)'),
    errorMessage: z.string().optional().describe('Error message for failed call initiations'),
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      let conversationId = (data['conversation_id'] || data['data'] && (data['data'] as Record<string, unknown>)['conversation_id']) as string | undefined;
      let agentId = (data['agent_id'] || data['data'] && (data['data'] as Record<string, unknown>)['agent_id']) as string | undefined;
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: {
          ...
          errorMessage: innerData['error'] as string | undefined || innerData['error_message'] as string | undefined,
