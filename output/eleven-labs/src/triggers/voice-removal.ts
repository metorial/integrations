...
export let voiceRemoval = SlateTrigger.create(
  spec,
  {
    name: 'Voice Removal',
    key: 'voice_removal',
    description: 'Triggered when a shared voice is scheduled for removal, when a removal notice is withdrawn, or when a voice is actually removed and no longer usable.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    removalStatus: z.string().optional().describe('Removal status: scheduled, withdrawn, or removed'),
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: {
          ...
          voiceName: innerData['voice_name'] as string | undefined || innerData['name'] as string | undefined,
          removalStatus,
