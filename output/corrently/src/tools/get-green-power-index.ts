...
export let getGreenPowerIndex = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    forecast: z.array(z.object({
      ...
      gsi: z.number().describe('GreenPowerIndex value 0-100 (higher = more renewable energy)'),
      co2GreenEnergy: z.number().optional().describe('CO2 in grams per kWh for green energy mix'),
      co2Standard: z.number().optional().describe('CO2 in grams per kWh for standard energy mix'),
      energyPrice: z.number().optional().describe('Local/regional energy price modification'),
      solarIndex: z.number().optional().describe('Sub-index for solar energy availability')
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!zip) {
      throw new Error('A German postal code (zip) is required. Provide it as input or configure a default in settings.');
    }
