...
export let createEnergySchedule = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    optimizationMode: z.enum(['price', 'solar', 'emission', 'comfort']).describe('Optimization goal for the schedule'),
    activeHours: z.number().min(1).max(36).describe('Number of active hours required for device operation')
  }))
  .output(z.object({
    schedule: z.array(z.object({
      ...
      active: z.boolean().optional().describe('Whether the device should be active in this time slot'),
