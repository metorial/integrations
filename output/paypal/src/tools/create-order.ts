...
export let createOrder = SlateTool.create(
  ...
  .input(z.object({
    intent: z.enum(['CAPTURE', 'AUTHORIZE']).describe('Payment intent: CAPTURE for immediate payment, AUTHORIZE for deferred capture'),
    purchaseUnits: z.array(z.object({
      referenceId: z.string().optional().describe('Unique reference ID for this purchase unit'),
      currencyCode: z.string().describe('Three-character ISO-4217 currency code (e.g. USD, EUR)'),
      ...
      items: z.array(z.object({
        ...
        category: z.enum(['DIGITAL_GOODS', 'PHYSICAL_GOODS', 'DONATION']).optional().describe('Item category'),
      })).optional().describe('Line items in this purchase unit'),
      shipping: z.object({
    ...
    })).min(1).describe('One or more purchase units for the order'),
  ...
  .output(z.object({
    ...
    })).optional().describe('HATEOAS links for the order'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let purchaseUnits = ctx.input.purchaseUnits.map(pu => {
      ...
      if (pu.items) {
        ...
        let itemTotal = pu.items.reduce((sum, item) => sum + parseFloat(item.unitAmount) * parseInt(item.quantity), 0);
