...
export let addTracking = SlateTool.create(
  spec,
  {
    name: 'Add Tracking',
    key: 'add_tracking',
  ...
  .input(z.object({
    ...
    status: z.enum(['SHIPPED', 'ON_HOLD', 'DELIVERED', 'CANCELLED']).optional().describe('Shipment status. Defaults to SHIPPED.'),
    notifyPayer: z.boolean().optional().describe('Whether to email the tracking info to the buyer. Defaults to true.'),
  }))
  .output(z.object({
    trackerId: z.string().optional().describe('Tracker ID in format transactionId-trackingNumber'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {
        trackerId: tracker.tracking_number_id || `${ctx.input.captureId}-${ctx.input.trackingNumber}`,
