...
export let searchTransactions = SlateTool.create(
  ...
  .input(z.object({
    ...
    transactionStatus: z.string().optional().describe('Filter by status: D (Denied), P (Pending), S (Successful), V (Reversed)'),
    transactionType: z.string().optional().describe('Filter by type (e.g. T0001 for payments, T0003 for subscriptions)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let transactions = details.map((t: any) => {
      ...
      return {
        ...
        payerName: payer.payer_name ? `${payer.payer_name.given_name || ''} ${payer.payer_name.surname || ''}`.trim() : undefined,
