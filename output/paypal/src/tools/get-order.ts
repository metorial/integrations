...
export let getOrder = SlateTool.create(
  ...
  .output(z.object({
    ...
    status: z.string().describe('Order status (CREATED, SAVED, APPROVED, VOIDED, COMPLETED, PAYER_ACTION_REQUIRED)'),
    intent: z.string().optional().describe('Payment intent (CAPTURE or AUTHORIZE)'),
    currencyCode: z.string().optional().describe('Currency code of the primary purchase unit'),
