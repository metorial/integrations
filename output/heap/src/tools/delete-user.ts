...
Primarily used for data privacy compliance (GDPR, CCPA). The user is looked up and then deleted via Heap's SCIM API.`,
  ...
  .input(z.object({
    identity: z.string().optional().describe('The known identity of the user to delete (e.g., email address).'),
  ...
  .handleInvocation(async (ctx) => {
    if (!ctx.input.identity && !ctx.input.userId) {
      throw new Error('Either "identity" or "userId" must be provided to identify the user to delete.');
    }
