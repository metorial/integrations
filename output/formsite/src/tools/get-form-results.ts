...
export let getFormResults = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    afterDate: z.string().optional().describe('Only return results finished after this date (ISO 8601 or YYYY-MM-DD HH:MM:SS)'),
    beforeDate: z.string().optional().describe('Only return results finished before this date (ISO 8601 or YYYY-MM-DD HH:MM:SS)'),
    afterId: z.string().optional().describe('Only return results with ID greater than this value'),
    beforeId: z.string().optional().describe('Only return results with ID less than this value'),
    limit: z.number().optional().describe('Maximum number of results to return (default: 100, max: 500)'),
    page: z.number().optional().describe('Page number for pagination (default: 1)'),
    sortId: z.string().optional().describe('Field to sort by — a meta field name (e.g., result_id, date_finish) or item ID'),
    sortDirection: z.enum(['asc', 'desc']).optional().describe('Sort direction (default: desc)'),
    resultsView: z.string().optional().describe('Apply a predefined Results View to control which columns are returned'),
    searchEquals: z.record(z.string(), z.string()).optional().describe('Search for results where item equals value. Keys are item IDs, values are search terms.'),
    searchContains: z.record(z.string(), z.string()).optional().describe('Search for results where text item contains value. Keys are item IDs.'),
    searchBegins: z.record(z.string(), z.string()).optional().describe('Search for results where text item begins with value. Keys are item IDs.'),
    searchEnds: z.record(z.string(), z.string()).optional().describe('Search for results where text item ends with value. Keys are item IDs.'),
    searchMethod: z.enum(['and', 'or']).optional().describe('Combine multiple search criteria with AND or OR logic (default: and)'),
  }))
  .output(z.object({
