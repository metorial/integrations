...
export let listSubtasks = SlateTool.create(
  ...
  .input(z.object({
    ...
    dueFrom: z.string().optional().describe('Filter subtasks due from this date (yyyymmdd format)'),
    dueTo: z.string().optional().describe('Filter subtasks due to this date (yyyymmdd format)'),
    completedFrom: z.string().optional().describe('Filter subtasks completed from this date (yyyymmdd format)'),
    completedTo: z.string().optional().describe('Filter subtasks completed to this date (yyyymmdd format)'),
    status: z.enum(['open', 'completed', 'active', 'onhold', 'inactive', 'archived']).optional().describe('Filter subtasks by status'),
