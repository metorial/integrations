...
export let sendLetterTool = SlateTool.create(
  spec,
  {
    name: 'Send Letter',
    key: 'send_letter',
  ...
  .input(z.object({
    ...
    recipients: z.array(addressSchema).min(1).describe('List of recipient mailing addresses'),
    colour: z.boolean().optional().describe('Print in colour (default: false / black & white)'),
    duplex: z.boolean().optional().describe('Print double-sided (default: false / single-sided)'),
    priorityPost: z.boolean().optional().describe('Use priority/express postal service (default: false)')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let totalPrice = sentLetters.reduce((sum: number, letter: any) => sum + (parseFloat(letter.price) || 0), 0);
