...
export let sendVoiceTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    messages: z.array(z.object({
      ...
      voiceGender: z.enum(['female', 'male']).optional().describe('Voice gender for text-to-speech (default: female)'),
      voiceLang: z.string().optional().describe('Language code for text-to-speech (e.g., en-us, en-gb, de-de)'),
      schedule: z.number().optional().describe('Unix timestamp for scheduled delivery'),
      customString: z.string().optional().describe('Custom reference string for tracking'),
    ...
    })).min(1).describe('List of voice messages to send')
  }))
  .output(z.object({
