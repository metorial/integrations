...
export let manageProducts = SlateTool.create(
  ...
  .input(z.object({
    ...
    identifier: z.string().optional().describe('Product identifier/SKU (for get by identifier)'),
    ...
    frequencyType: z.enum(['day', 'week', 'month', 'quarter', 'year']).optional().describe('Recurrence frequency type (for create/update)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'create': {
        ...
        if (ctx.input.ledgerAccountId) productData.ledger_account_id = ctx.input.ledgerAccountId;
      ...
      case 'update': {
        ...
        let productData: Record<string, any> = {};
        if (ctx.input.description !== undefined) productData.description = ctx.input.description;
        ...
        if (ctx.input.ledgerAccountId !== undefined) productData.ledger_account_id = ctx.input.ledgerAccountId;
        if (ctx.input.frequencyType !== undefined) productData.frequency_type = ctx.input.frequencyType;
        if (ctx.input.frequency !== undefined) productData.frequency = ctx.input.frequency;
