...
export let listEstimates = SlateTool.create(
  ...
  .input(z.object({
    state: z.enum(['all', 'draft', 'open', 'late', 'accepted', 'rejected', 'billed', 'archived']).optional().describe('Filter by estimate state'),
    period: z.string().optional().describe('Filter by period: "this_month", "prev_month", "this_year", "prev_year", or custom "YYYYMMDD..YYYYMMDD"'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let mapped = estimates.map((e: any) => ({
      ...
      contactName: e.contact?.company_name || `${e.contact?.firstname || ''} ${e.contact?.lastname || ''}`.trim() || null,
