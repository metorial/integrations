...
export let listCallsTool = SlateTool.create(
  ...
  .input(z.object({
    startedAfter: z.string().optional().describe('ISO 8601 timestamp — only include calls started after this time'),
    startedBefore: z.string().optional().describe('ISO 8601 timestamp — only include calls started before this time'),
    targetType: z.enum(['user', 'callcenter', 'department', 'office']).optional().describe('Target type to scope calls'),
