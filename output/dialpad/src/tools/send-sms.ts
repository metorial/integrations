...
export let sendSmsTool = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Rate limited to 100-800 per minute depending on tier.',
    ],
  }
)
  .input(z.object({
    toNumbers: z.array(z.string()).describe('Phone numbers to send the SMS to (E.164 format recommended)'),
    text: z.string().describe('Message text to send'),
    senderId: z.number().optional().describe('User ID to send the SMS from'),
    senderGroupType: z.string().optional().describe('Sender group type (e.g., "department", "callcenter")'),
    senderGroupId: z.number().optional().describe('Sender group ID'),
    inferCountryCode: z.boolean().optional().describe('Whether to infer country code from phone numbers'),
  }))
  .output(z.object({
