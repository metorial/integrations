...
let functionToolSchema = z.object({
  ...
  function: z.object({
    ...
    parameters: z.record(z.string(), z.unknown()).optional().describe('JSON Schema for function parameters'),
  }),
});
...
export let chatCompletion = SlateTool.create(
  ...
  .input(z.object({
    ...
    messages: z.array(messageSchema).min(1).describe('Conversation history as an array of messages'),
    temperature: z.number().min(0).max(2).optional().describe('Controls randomness (0 = deterministic, 2 = creative). Default: 1'),
    maxTokens: z.number().optional().describe('Maximum number of tokens to generate in the response'),
    ...
    ]).optional().describe('Controls how the model uses tools'),
    responseFormat: z.object({
      type: z.enum(['text', 'json_schema']).describe('Response format type'),
      jsonSchema: z.record(z.string(), z.unknown()).optional().describe('JSON Schema for structured output (palmyra-x4/x5 only)'),
    }).optional().describe('Format of the response output'),
  ...
  .output(z.object({
    ...
    choices: z.array(z.object({
      ...
      finishReason: z.string().describe('Reason generation stopped (stop, length, content_filter, tool_calls)'),
      message: z.object({
        ...
        toolCalls: z.array(z.object({
          ...
          functionArguments: z.string().describe('JSON-encoded arguments for the function'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let hasToolCalls = choices.some((c) => c.message.toolCalls && c.message.toolCalls.length > 0);
