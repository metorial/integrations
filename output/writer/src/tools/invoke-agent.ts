...
export let invokeAgent = SlateTool.create(
  spec,
  {
...
export let listAgents = SlateTool.create(
  ...
  .output(z.object({
    agents: z.array(z.record(z.string(), z.unknown())).describe('List of agents with their metadata'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new WriterClient(ctx.auth.token);
...
export let getAgentDetails = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    agent: z.record(z.string(), z.unknown()).describe('Full agent details including inputs and configuration'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new WriterClient(ctx.auth.token);
