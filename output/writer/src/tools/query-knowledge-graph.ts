...
export let queryKnowledgeGraph = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    subqueries: z.boolean().optional().describe('Break complex questions into sub-questions for more comprehensive answers'),
    queryConfig: z.object({
      maxSubquestions: z.number().min(1).max(10).optional().describe('Maximum number of sub-questions (1-10, default: 6)'),
      searchWeight: z.number().min(0).max(100).optional().describe('Balance between keyword and semantic search (0-100, default: 50)'),
      groundingLevel: z.number().min(0).max(1).optional().describe('Fidelity to source material vs creative response (0-1, default: 0)'),
      maxSnippets: z.number().min(1).max(60).optional().describe('Maximum context snippets to retrieve (1-60, default: 30)'),
      maxTokens: z.number().min(100).max(8000).optional().describe('Maximum tokens in the response (100-8000, default: 4000)'),
      inlineCitations: z.boolean().optional().describe('Embed source citations within the answer text'),
      keywordThreshold: z.number().min(0).max(1).optional().describe('Keyword matching threshold (0-1, default: 0.7)'),
      semanticThreshold: z.number().min(0).max(1).optional().describe('Semantic matching threshold (0-1, default: 0.7)'),
    }).optional().describe('Advanced query configuration options'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Decomposed sub-questions and their answers'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let preview = result.answer.length > 300
