...
export class WriterClient {
  ...
  async updateGraph(graphId: string, updates: { name?: string; description?: string }): Promise<GraphResponse> {
  ...
  async addFileToGraph(graphId: string, fileId: string): Promise<FileResponse> {
    let response = await this.axios.post(`/graphs/${graphId}/file`, { file_id: fileId }, {
      headers: this.headers,
    });
  ...
  async queryGraph(request: GraphQuestionRequest): Promise<GraphQuestionResponse> {
    ...
    if (request.queryConfig) {
      ...
      if (request.queryConfig.maxSubquestions !== undefined) qc.max_subquestions = request.queryConfig.maxSubquestions;
      if (request.queryConfig.searchWeight !== undefined) qc.search_weight = request.queryConfig.searchWeight;
      if (request.queryConfig.groundingLevel !== undefined) qc.grounding_level = request.queryConfig.groundingLevel;
      if (request.queryConfig.maxSnippets !== undefined) qc.max_snippets = request.queryConfig.maxSnippets;
      if (request.queryConfig.maxTokens !== undefined) qc.max_tokens = request.queryConfig.maxTokens;
      if (request.queryConfig.inlineCitations !== undefined) qc.inline_citations = request.queryConfig.inlineCitations;
      if (request.queryConfig.keywordThreshold !== undefined) qc.keyword_threshold = request.queryConfig.keywordThreshold;
      if (request.queryConfig.semanticThreshold !== undefined) qc.semantic_threshold = request.queryConfig.semanticThreshold;
  ...
  // Files
  async uploadFile(fileName: string, content: string, contentType: string, graphId?: string): Promise<FileResponse> {
    let url = '/files';
  ...
  async listFiles(params?: { orderBy?: string; order?: string; offset?: number; limit?: number }): Promise<FileResponse[]> {
  ...
  async generateFromApplication(
    ...
    let response = await this.axios.post(`/applications/${applicationId}`, {
      inputs,
    }, {
      headers: this.headers,
    });
  ...
  async listApplications(params?: { offset?: number; limit?: number }): Promise<Array<Record<string, unknown>>> {
    let queryParams: Record<string, string> = {};
    if (params?.offset !== undefined) queryParams.offset = String(params.offset);
    if (params?.limit !== undefined) queryParams.limit = String(params.limit);
