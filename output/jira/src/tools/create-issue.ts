...
export let createIssueTool = SlateTool.create(
  ...
  .input(z.object({
    projectKey: z.string().describe('The project key (e.g., "PROJ") where the issue will be created.'),
    issueTypeName: z.string().describe('The issue type name (e.g., "Task", "Bug", "Story", "Epic", "Sub-task").'),
    summary: z.string().describe('The issue summary/title.'),
    description: z.any().optional().describe('The issue description in Atlassian Document Format (ADF) or a plain text string.'),
    assigneeAccountId: z.string().optional().describe('Account ID of the user to assign the issue to.'),
    priorityName: z.string().optional().describe('Priority name (e.g., "High", "Medium", "Low").'),
    labels: z.array(z.string()).optional().describe('Labels to apply to the issue.'),
    components: z.array(z.string()).optional().describe('Component names to associate with the issue.'),
    parentKey: z.string().optional().describe('Parent issue key for sub-tasks or children of epics.'),
    fixVersions: z.array(z.string()).optional().describe('Version names to set as fix versions.'),
    dueDate: z.string().optional().describe('Due date in YYYY-MM-DD format.'),
    customFields: z.record(z.string(), z.any()).optional().describe('Custom field values keyed by field ID (e.g., {"customfield_10001": "value"}).'),
  }))
  .output(z.object({
