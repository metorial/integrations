...
export let searchUsersTool = SlateTool.create(
  ...
  .output(z.object({
    users: z.array(z.object({
      accountId: z.string().describe('The user account ID (used for assigning issues, etc.).'),
      displayName: z.string().describe('The user display name.'),
      emailAddress: z.string().optional().describe('The user email address (may not be visible depending on privacy settings).'),
