...
export let getTransitionsTool = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    transitions: z.array(z.object({
      transitionId: z.string().describe('The transition ID (used for transitioning the issue).'),
      name: z.string().describe('The transition name.'),
      toStatus: z.string().optional().describe('The target status name after transition.'),
      toStatusCategory: z.string().optional().describe('The target status category (e.g., "To Do", "In Progress", "Done").'),
    })),
  }))
