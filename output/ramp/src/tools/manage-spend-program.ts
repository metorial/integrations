...
export let manageSpendProgram = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    spendProgramId: z.string().optional().describe('Spend program ID (required for get, update)'),
    ...
    primaryCardEnabled: z.boolean().optional().describe('Whether primary card spending is enabled'),
    reimbursementsEnabled: z.boolean().optional().describe('Whether reimbursements are enabled'),
    ...
    allowedCategories: z.array(z.number()).optional().describe('Allowed merchant category codes'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'get') {
      if (!ctx.input.spendProgramId) throw new Error('spendProgramId is required for get action');
    ...
    let buildBody = () => {
      ...
      if (ctx.input.primaryCardEnabled !== undefined || ctx.input.reimbursementsEnabled !== undefined) {
        body.permitted_spend_types = {};
        if (ctx.input.primaryCardEnabled !== undefined) body.permitted_spend_types.primary_card_enabled = ctx.input.primaryCardEnabled;
        if (ctx.input.reimbursementsEnabled !== undefined) body.permitted_spend_types.reimbursements_enabled = ctx.input.reimbursementsEnabled;
      }

      if (ctx.input.amount !== undefined || ctx.input.currencyCode || ctx.input.interval || ctx.input.allowedCategories) {
        body.spending_restrictions = {};
        if (ctx.input.amount !== undefined || ctx.input.currencyCode) {
          body.spending_restrictions.limit = {};
          if (ctx.input.amount !== undefined) body.spending_restrictions.limit.amount = ctx.input.amount;
          if (ctx.input.currencyCode) body.spending_restrictions.limit.currency_code = ctx.input.currencyCode;
        }
        if (ctx.input.interval) body.spending_restrictions.interval = ctx.input.interval;
        if (ctx.input.allowedCategories) body.spending_restrictions.allowed_categories = ctx.input.allowedCategories;
      }
    ...
    if (action === 'update') {
      if (!ctx.input.spendProgramId) throw new Error('spendProgramId is required for update action');
      let spendProgram = await client.updateSpendProgram(ctx.input.spendProgramId, buildBody());
      return {
        output: { spendProgram },
