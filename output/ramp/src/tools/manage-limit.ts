...
export let manageLimit = SlateTool.create(
  spec,
  {
    name: 'Manage Limit',
    key: 'manage_limit',
...
- **terminate**: Permanently terminate a limit.`,
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'update', 'terminate']).describe('Action to perform'),
    limitId: z.string().optional().describe('Limit ID (required for get, update, terminate)'),
    cursor: z.string().optional().describe('Pagination cursor (for list)'),
    pageSize: z.number().min(2).max(100).optional().describe('Results per page (for list)'),
    spendProgramId: z.string().optional().describe('Spend program ID (for list filter or create linkage)'),
    ...
    isShareable: z.boolean().optional().describe('Whether the limit is shareable among multiple users'),
