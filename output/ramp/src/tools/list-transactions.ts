...
export let listTransactions = SlateTool.create(
  ...
  .input(z.object({
    ...
    pageSize: z.number().min(2).max(100).optional().describe('Number of results per page (2-100, default 20)'),
    fromDate: z.string().optional().describe('Filter transactions from this date (ISO 8601 format, e.g. 2024-01-01)'),
    toDate: z.string().optional().describe('Filter transactions until this date (ISO 8601 format)'),
