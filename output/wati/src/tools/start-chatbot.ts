...
export let startChatbot = SlateTool.create(
  spec,
  {
    name: 'Start Chatbot',
    key: 'start_chatbot',
    ...
    constraints: [
      'Chatbot functionality is available on Wati Pro plan and above.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    action: z.enum(['list', 'start']).describe('"list" to retrieve available chatbots, "start" to trigger a chatbot for a contact.'),
    chatbotId: z.string().optional().describe('ID of the chatbot to start. Required for "start" action.'),
    target: z.string().optional().describe('Contact identifier: phone number with country code, contact ID, or Channel:PhoneNumber. Required for "start" action.'),
    pageNumber: z.number().int().min(1).default(1).describe('Page number for listing chatbots (1-based).'),
    pageSize: z.number().int().min(1).max(100).default(50).describe('Number of chatbots per page for listing (max 100).'),
  }))
  .output(z.object({
    chatbots: z.array(chatbotSchema).optional().describe('List of available chatbots (for "list" action).'),
    result: z.boolean().optional().describe('Whether the chatbot was started successfully (for "start" action).'),
  }))
