...
Use "assign_operator" to route a conversation to a specific team member, or "update_status" to change the conversation state (e.g., open, resolved).`,
  ...
  .input(z.object({
    action: z.enum(['assign_operator', 'update_status']).describe('The conversation management action to perform.'),
    target: z.string().describe('Conversation identifier: phone number with country code, conversation ID, or Channel:PhoneNumber.'),
    operatorEmail: z.string().optional().describe('Email address of the operator to assign. Required for "assign_operator".'),
    status: z.string().optional().describe('New conversation status (e.g. "open", "resolved"). Required for "update_status".'),
  }))
  .output(z.object({
