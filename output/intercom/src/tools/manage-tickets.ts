...
Use the "list_ticket_types" action to discover available ticket types before creating tickets.`,
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'reply', 'list_ticket_types']).describe('Operation to perform'),
    ...
    contactIds: z.array(z.string()).optional().describe('Contact IDs to attach (required for create)'),
    ticketAttributes: z.record(z.string(), z.any()).optional().describe('Custom ticket attributes as key-value pairs'),
    companyId: z.string().optional().describe('Company ID to associate with ticket (for create)'),
    state: z.string().optional().describe('Ticket state (for update): submitted, in_progress, waiting_on_customer, resolved'),
    ...
    snoozedUntil: z.string().optional().describe('ISO 8601 timestamp to snooze until (for update)'),
    assignmentAdminId: z.string().optional().describe('Admin ID to assign to (for create/update)'),
    ...
    intercomUserId: z.string().optional().describe('Contact ID replying (for reply as contact)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('Available ticket types (for list_ticket_types)')
  }))
