...
export let manageCompanies = SlateTool.create(
  spec,
  {
...
The create and update operations use the same endpoint — if a company with the given companyId already exists, it will be updated.`,
  ...
  .input(z.object({
    action: z.enum(['create_or_update', 'delete', 'attach_contact', 'detach_contact']).describe('The operation to perform'),
    intercomCompanyId: z.string().optional().describe('Intercom-assigned company ID (required for delete, attach, detach)'),
    contactId: z.string().optional().describe('Contact ID to attach/detach (required for attach_contact/detach_contact)'),
    companyId: z.string().optional().describe('Your external company identifier (for create_or_update)'),
    ...
    remoteCreatedAt: z.string().optional().describe('ISO 8601 timestamp of company creation in your system'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'delete') {
      if (!ctx.input.intercomCompanyId) throw new Error('intercomCompanyId is required for delete');
    ...
    if (action === 'attach_contact') {
      if (!ctx.input.contactId || !ctx.input.intercomCompanyId) {
        throw new Error('Both contactId and intercomCompanyId are required for attach_contact');
      }
      let result = await client.attachContactToCompany(ctx.input.contactId, ctx.input.intercomCompanyId);
      return {
        output: mapCompany(result),
    ...
    if (action === 'detach_contact') {
      if (!ctx.input.contactId || !ctx.input.intercomCompanyId) {
        throw new Error('Both contactId and intercomCompanyId are required for detach_contact');
      }
      let result = await client.detachContactFromCompany(ctx.input.contactId, ctx.input.intercomCompanyId);
      return {
        output: mapCompany(result),
