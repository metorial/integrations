...
export let listInvoices = SlateTool.create(
  ...
  .input(z.object({
    createdAtFrom: z.string().optional().describe('Filter invoices created from this date (ISO 8601)'),
    createdAtTo: z.string().optional().describe('Filter invoices created up to this date (ISO 8601)'),
    paidAtFrom: z.string().optional().describe('Filter invoices paid from this date (ISO 8601)'),
    paidAtTo: z.string().optional().describe('Filter invoices paid up to this date (ISO 8601)'),
    invoiceNumberFrom: z.string().optional().describe('Starting invoice number for range filter'),
    invoiceNumberTo: z.string().optional().describe('Ending invoice number for range filter'),
    sortDirection: z.enum(['asc', 'desc']).optional().describe('Sort direction by invoice number'),
