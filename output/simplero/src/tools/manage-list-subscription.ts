...
export let manageListSubscription = SlateTool.create(
  spec,
  {
...
Bulk subscribe is asynchronous and returns a tracking token.`,
  ...
  .input(z.object({
    action: z.enum(['get_lists', 'subscribe', 'bulk_subscribe', 'unsubscribe', 'find_subscription', 'list_subscriptions']).describe('Action to perform'),
    listId: z.string().optional().describe('Mailing list ID (required for subscribe/unsubscribe/find actions)'),
    ...
    })).optional().describe('Array of subscribers for bulk subscribe (max 1,000)'),
    page: z.number().optional().describe('Page number for listing subscriptions'),
    perPage: z.number().optional().describe('Results per page'),
    status: z.enum(['active', 'unsubscribed', 'suspended']).optional().describe('Filter subscriptions by status'),
  }))
  .output(z.object({
    lists: z.array(z.record(z.string(), z.unknown())).optional().describe('Available mailing lists'),
    subscription: z.record(z.string(), z.unknown()).optional().describe('Subscription record'),
    subscriptions: z.array(z.record(z.string(), z.unknown())).optional().describe('List of subscription records'),
