...
export let startAutomation = SlateTool.create(
  ...
  .input(z.object({
    ...
    email: z.string().optional().describe('Contact email to start the automation for (required for start)'),
    page: z.number().optional().describe('Page number for listing automations (0-indexed)'),
  }))
  .output(z.object({
    automations: z.array(z.record(z.string(), z.unknown())).optional().describe('List of available automations'),
