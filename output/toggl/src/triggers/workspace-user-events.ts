...
export let workspaceUserEventsTrigger = SlateTrigger.create(
  spec,
  {
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      let eventId = metadata.event_id ?? data.event_id ?? `wu_${payload.id ?? 'unknown'}_${Date.now()}`;
