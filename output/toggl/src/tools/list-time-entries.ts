...
export let listTimeEntries = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Limited to a 3-month lookback window from the current date.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    startDate: z.string().optional().describe('Start date in ISO 8601 or YYYY-MM-DD format. Defaults to entries from the past week.'),
