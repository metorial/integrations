...
export let manageViews = SlateTool.create(
  spec,
  {
    name: 'Manage Views',
    key: 'manage_views',
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'get', 'create', 'update', 'delete', 'convert_to_linked_sheet']).describe('View action to perform'),
    viewId: z.string().optional().describe('View ID (required for get, update, delete, convert_to_linked_sheet)'),
    name: z.string().optional().describe('View name (for create or update)'),
    filterModel: z.record(z.string(), z.unknown()).optional().describe('Filter model for the view'),
    sortModel: z.array(z.record(z.string(), z.unknown())).optional().describe('Sort model for the view'),
    columnState: z.array(z.record(z.string(), z.unknown())).optional().describe('Column visibility/state for the view'),
    groupColumns: z.array(z.string()).optional().describe('Group columns for the view'),
    aggregations: z.record(z.string(), z.unknown()).optional().describe('Aggregation settings for the view'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'convert_to_linked_sheet':
        if (!ctx.input.viewId) throw new Error('viewId is required for convert_to_linked_sheet');
        result = await client.createLinkedSheetFromView(ctx.input.sheetHandle, ctx.input.viewId);
        break;
    }
