...
export let filterData = SlateTool.create(
  spec,
  {
    name: 'Filter Data',
    key: 'filter_data',
  ...
  .input(z.object({
    ...
    filterModel: z.record(z.string(), z.unknown()).optional().describe('Filter model object defining filter conditions'),
    savedFilterHandle: z.string().optional().describe('Handle of a saved filter to apply instead of a raw filterModel'),
    ...
    startRow: z.number().optional().default(0).describe('Starting row index for pagination (0-based)'),
    endRow: z.number().optional().default(100).describe('Ending row index for pagination (exclusive)'),
    groupColumns: z.array(z.string()).optional().describe('Columns to group by'),
    aggregations: z.record(z.string(), z.unknown()).optional().describe('Aggregation settings per column'),
  }))
  .output(z.object({
    rows: z.array(z.record(z.string(), z.unknown())).optional().describe('Filtered row data'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let rows = Array.isArray(result?.rows) ? result.rows as Record<string, unknown>[] : undefined;
    let totalRows = typeof result?.totalRows === 'number' ? result.totalRows as number : undefined;
