...
export let deduplicateRows = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['count', 'delete']).describe('Whether to count duplicates or delete them'),
    columns: z.array(z.string()).optional().describe('Columns to consider when determining duplicates. If omitted, all columns are used.'),
  }))
  .output(z.object({
    result: z.record(z.string(), z.unknown()).describe('Deduplication result including count of duplicates found or removed'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: { result },
      message: ctx.input.action === 'count'
