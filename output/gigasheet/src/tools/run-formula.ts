...
export let runFormula = SlateTool.create(
  spec,
  {
    name: 'Run Formula',
    key: 'run_formula',
  ...
  .input(z.object({
    ...
    action: z.enum(['run', 'validate', 'preview', 'ai_build']).default('run').describe('Formula action to perform'),
    formula: z.string().optional().describe('The formula expression to execute (for run, validate, preview)'),
    newColumnName: z.string().optional().describe('Name for the new formula column (for run action)'),
    prompt: z.string().optional().describe('Natural language description for AI formula builder (for ai_build action)'),
  }))
  .output(z.object({
