...
export let modifyRows = SlateTool.create(
  spec,
  {
    name: 'Modify Rows',
    key: 'modify_rows',
  ...
  .input(z.object({
    ...
    rows: z.array(z.record(z.string(), z.unknown())).optional().describe('Rows to append (array of objects keyed by column name)'),
    sourceSheetHandle: z.string().optional().describe('Handle of the source sheet for append_from_sheet'),
    ...
    filterModel: z.record(z.string(), z.unknown()).optional().describe('Filter model for filter-based row deletion'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'append_from_sheet':
        ...
        result = await client.appendFromSheet(ctx.input.sheetHandle, ctx.input.sourceSheetHandle);
        break;
      ...
      case 'delete_rows_matching_filter':
        ...
        result = await client.deleteRowsMatchingFilter(ctx.input.sheetHandle, ctx.input.filterModel);
        break;
      ...
      case 'delete_rows_not_matching_filter':
        ...
        result = await client.deleteRowsNotMatchingFilter(ctx.input.sheetHandle, ctx.input.filterModel);
        break;
    }
