...
export let createAdGroup = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    placementType: z.string().optional().describe('Placement type (e.g. PLACEMENT_TYPE_NORMAL, PLACEMENT_TYPE_AUTOMATIC).'),
    placement: z.array(z.string()).optional().describe('Placements (e.g. ["PLACEMENT_TIKTOK"]).'),
    ...
    scheduleType: z.string().optional().describe('Schedule type: SCHEDULE_START_END or SCHEDULE_FROM_NOW.'),
    scheduleStartTime: z.string().optional().describe('Start time in YYYY-MM-DD HH:MM:SS format (UTC+0).'),
    scheduleEndTime: z.string().optional().describe('End time in YYYY-MM-DD HH:MM:SS format (UTC+0). Required for SCHEDULE_START_END.'),
    optimizeGoal: z.string().optional().describe('Optimization goal (e.g. REACH, CLICK, CONVERT).'),
    billingEvent: z.string().optional().describe('Billing event (e.g. CPM, CPC, OCPM).'),
    bidType: z.string().optional().describe('Bid type (e.g. BID_TYPE_NO_BID, BID_TYPE_CUSTOM).'),
    bid: z.number().optional().describe('Bid amount when using custom bidding.'),
    pacing: z.string().optional().describe('Pacing mode (e.g. PACING_MODE_SMOOTH, PACING_MODE_FAST).'),
    location: z.array(z.string()).optional().describe('Location/region codes for targeting.'),
    gender: z.string().optional().describe('Gender targeting (e.g. GENDER_UNLIMITED, GENDER_MALE, GENDER_FEMALE).'),
    age: z.array(z.string()).optional().describe('Age groups (e.g. ["AGE_18_24", "AGE_25_34"]).'),
  }))
  .output(z.object({
