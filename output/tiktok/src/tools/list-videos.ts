...
export let listVideos = SlateTool.create(
  ...
  .input(z.object({
    cursor: z.number().optional().describe('Pagination cursor from a previous response. Omit for the first page.'),
    maxCount: z.number().min(1).max(20).optional().describe('Number of videos to return per page (1-20, default 20).'),
  }))
  .output(z.object({
