...
export let getAdReport = SlateTool.create(
  spec,
  {
    name: 'Get Ad Report',
    key: 'get_ad_report',
...
Common **metrics**: \`spend\`, \`impressions\`, \`clicks\`, \`ctr\`, \`cpc\`, \`cpm\`, \`conversions\`, \`cost_per_conversion\`, \`reach\`, \`frequency\`.`,
  ...
  .input(z.object({
    ...
    reportType: z.enum(['BASIC', 'AUDIENCE', 'PLAYABLE_MATERIAL', 'CATALOG']).optional().describe('Type of report (default BASIC).'),
    dataLevel: z.enum(['AUCTION_ADVERTISER', 'AUCTION_CAMPAIGN', 'AUCTION_ADGROUP', 'AUCTION_AD']).describe('Granularity of the report data.'),
    dimensions: z.array(z.string()).describe('Dimensions to group data by (e.g. ["stat_time_day", "campaign_id"]).'),
    metrics: z.array(z.string()).describe('Metrics to include (e.g. ["spend", "impressions", "clicks", "ctr"]).'),
  ...
  .output(z.object({
    rows: z.array(z.record(z.string(), z.unknown())).describe('Report data rows with requested dimensions and metrics.'),
