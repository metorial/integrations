...
export let businessWebhook = SlateTrigger.create(
  spec,
  {
    name: 'TikTok Business Events',
    key: 'business_webhook',
    description: 'Receives webhook notifications from TikTok API for Business. Covers lead generation events, ad review status changes, and Creator Marketplace order updates. Configure webhook subscriptions in the TikTok Business API Portal.',
  }
)
  .input(z.object({
    eventType: z.string().describe('The type of business event (e.g. lead.submitted, ad.review_status_changed, creator_marketplace.order_updated).'),
    ...
    payload: z.record(z.string(), z.unknown()).optional().describe('Raw event payload from TikTok Business API.'),
  }))
  .output(z.object({
    advertiserId: z.string().optional().describe('Advertiser ID associated with the event.'),
    ...
    reviewStatus: z.string().optional().describe('New ad review status (e.g. approved, rejected).'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let eventId = body.log_id ?? body.event_id ?? body.request_id ?? `${eventType}-${Date.now()}`;
