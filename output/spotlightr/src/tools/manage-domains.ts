...
export let listWhitelistedDomains = SlateTool.create(
  spec,
  {
...
export let addWhitelistedDomain = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    result: z.record(z.string(), z.any()).describe('Response from Spotlightr after adding the domain.'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
