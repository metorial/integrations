...
export let createVideo = SlateTool.create(
  ...
  .input(z.object({
    ...
    url: z.string().optional().describe('External URL to link (YouTube, Vimeo, Google Drive, etc.).'),
    playerSettingsVideoId: z.string().optional().describe('Video ID from which to copy player settings.'),
    videoGroup: z.string().optional().describe('Project group ID to assign the video to.'),
    hlsEncryption: z.boolean().optional().describe('Whether to enable HLS encryption for the video.'),
  }))
  .output(z.object({
    result: z.record(z.string(), z.any()).describe('Response from Spotlightr after creating the video.'),
  }))
