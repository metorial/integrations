...
export let manageComment = SlateTool.create(
  ...
  .input(z.object({
    ...
    recordType: z.enum(['features', 'epics', 'ideas', 'releases', 'goals', 'initiatives', 'requirements', 'to_dos']).optional().describe('Type of parent record (required for create/list)'),
    ...
    body: z.string().optional().describe('Comment body (HTML supported, required for create/update)'),
  ...
  .output(z.object({
    ...
    comments: z.array(commentOutputSchema).optional().describe('List of comments (for list action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      ...
      let comment = await client.createComment(ctx.input.recordType, ctx.input.recordId, ctx.input.body);
