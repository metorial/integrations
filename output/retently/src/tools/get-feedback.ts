...
export let getFeedback = SlateTool.create(
  spec,
  {
    name: 'Get Feedback',
    key: 'get_feedback',
...
Each response includes score, text feedback, sentiment, topics, tags, and additional question answers.`,
  ...
  .input(z.object({
    feedbackId: z.string().optional().describe('Retrieve a specific feedback response by its ID'),
    ...
    startDate: z.string().optional().describe('Filter responses after this date (ISO 8601 or UNIX timestamp)'),
    endDate: z.string().optional().describe('Filter responses before this date (ISO 8601 or UNIX timestamp)'),
    attributes: z.array(z.object({
      name: z.string().describe('Customer property name'),
      operator: z.string().describe('Filter operator'),
      value: z.union([z.string(), z.number(), z.boolean()]).optional().describe('Filter value')
    })).optional().describe('Filter by custom customer properties'),
