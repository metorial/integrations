...
export let annotateFeedback = SlateTool.create(
  spec,
  {
...
You can choose to append new topics/tags to existing ones or replace them entirely.`,
  ...
  .input(z.object({
    ...
    })).optional().describe('Topics to add to the response'),
    tags: z.array(z.string()).optional().describe('Tags to add to the response'),
    operation: z.enum(['append', 'override']).optional().describe('Whether to append to or replace existing topics/tags (default: override)')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.topics && ctx.input.topics.length > 0) {
      await client.setResponseTopics(ctx.input.responseId, ctx.input.topics, ctx.input.operation);
      topicsUpdated = true;
    }
