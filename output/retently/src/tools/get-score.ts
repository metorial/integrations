...
export let getScore = SlateTool.create(
  spec,
  {
    name: 'Get Score',
    key: 'get_score',
...
Includes the score value, rating distribution, and respondent category breakdowns (e.g., promoters/passives/detractors for NPS).`,
  ...
  .input(z.object({
    metric: z.enum(['nps', 'csat', 'ces', 'star']).describe('Survey metric to retrieve the score for')
  }))
  .output(z.object({
    ...
    ratingDistribution: z.record(z.string(), z.number()).optional().describe('Distribution of ratings by score value'),
