...
export let planEventTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Plan Event',
    key: 'plan_event',
    description: 'Triggers on plan lifecycle events: plan created, plan ended (canceled or exhausted), and plan payment failed. Requires configuring a webhook endpoint in MoonClerk dashboard with the relevant plan topic(s).',
  }
)
  .input(z.object({
    topic: z.string().describe('Webhook topic identifying the plan event type'),
    customer: z.record(z.string(), z.unknown()).describe('Raw customer/plan data from webhook'),
  }))
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      let couponRaw = discountRaw ? (discountRaw.coupon as Record<string, unknown>) ?? null : null;
