...
export let searchKnowledgeGraph = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    query: z.string().describe('DQL (Diffbot Query Language) query string, e.g., type:Organization name:"Apple Inc"'),
    ...
    filter: z.string().optional().describe('Additional filter expression to apply to results'),
  }))
  .output(z.object({
