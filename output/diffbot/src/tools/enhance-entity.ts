...
let enhancedEntitySchema = z.object({
...
export let enhanceEntity = SlateTool.create(
  spec,
  {
    name: 'Enhance Entity',
    key: 'enhance_entity',
  ...
  .input(z.object({
    ...
    description: z.string().optional().describe('Description to help disambiguate the entity'),
    includeCombined: z.boolean().optional().default(false).describe('If true, also returns the person\'s current employer data'),
  }))
  .output(z.object({
    ...
    employerEntity: enhancedEntitySchema.optional().describe('Employer entity data (only when includeCombined is true)'),
  }))
