...
export let sendSms = SlateTool.create(
  ...
  .input(z.object({
    fromNumber: z.string().describe('Sender phone number in E.164 format (e.g., +15551234567). Must be a RingCentral number assigned to the account.'),
    recipientNumbers: z.array(z.string()).min(1).describe('List of recipient phone numbers in E.164 format.'),
    text: z.string().describe('Text content of the SMS message.'),
    mode: z.enum(['p2p', 'a2p']).optional().default('p2p').describe('Messaging mode: "p2p" for person-to-person (default), "a2p" for application-to-person bulk messaging.'),
  }))
  .output(z.object({
