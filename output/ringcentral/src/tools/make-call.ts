...
export let makeCall = SlateTool.create(
  ...
  .input(z.object({
    fromNumber: z.string().describe('Phone number to call from (your RingCentral number that will ring first)'),
    toNumber: z.string().describe('Destination phone number to connect to'),
    callerIdNumber: z.string().optional().describe('Phone number to display as caller ID to the recipient'),
    playPrompt: z.boolean().optional().describe('Whether to play a confirmation prompt before connecting the call (defaults to true)'),
  }))
  .output(z.object({
    ringOutId: z.string().describe('Unique identifier for the RingOut call session'),
    status: z.object({
      callerStatus: z.string().describe('Connection status of the caller leg (e.g. InProgress, Success, Error)'),
      calleeStatus: z.string().describe('Connection status of the callee leg (e.g. InProgress, Success, Error)'),
