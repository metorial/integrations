...
export let getPresence = SlateTool.create(
  ...
  .input(z.object({
    extensionId: z.string().optional().describe('Extension ID to look up. Omit to get the authenticated user\'s presence.'),
  }))
  .output(z.object({
    userStatus: z.string().describe('User-defined availability status (e.g. Available, Busy, Offline)'),
    dndStatus: z.string().describe('Do-not-disturb status (e.g. TakeAllCalls, DoNotAcceptAnyCalls, DoNotAcceptDepartmentCalls, TakeDepartmentCallsOnly)'),
    presenceStatus: z.string().describe('Aggregated presence status (e.g. Available, Busy, Offline)'),
    telephonyStatus: z.string().describe('Telephony line status (e.g. NoCall, Ringing, CallConnected, OnHold)'),
