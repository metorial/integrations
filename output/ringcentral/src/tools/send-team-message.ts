...
export let sendTeamMessage = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    chatId: z.string().describe('The ID of the chat (team, group, or direct message) to post the message to'),
    text: z.string().describe('The text content of the message'),
    attachments: z.array(z.object({
      type: z.string().describe('The type of attachment (e.g., "Card", "Event", "Note")'),
      properties: z.record(z.string(), z.any()).optional().describe('Key-value properties specific to the attachment type'),
    })).optional().describe('Optional array of attachment objects to include with the message'),
  }))
  .output(z.object({
