...
let callPartySchema = z.object({
  ...
  }).optional().describe('Originating party info'),
  to: z.object({
    phoneNumber: z.string().optional(),
    name: z.string().optional(),
    extensionId: z.string().optional(),
  }).optional().describe('Destination party info'),
  status: z.object({
    code: z.string().optional(),
    reason: z.string().optional(),
  }).optional().describe('Current party status'),
...
let outputSchema = z.object({
  ...
  action: z.enum(['hold', 'unhold', 'transfer', 'forward']).describe('The action that was performed'),
...
export let manageCall = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['hold', 'unhold', 'transfer', 'forward']).describe('The call control action to perform'),
    telephonySessionId: z.string().describe('Telephony session ID of the active call'),
    partyId: z.string().describe('Party ID within the telephony session'),
    targetNumber: z.string().optional().describe('Target phone number for transfer or forward actions (required when action is "transfer" or "forward")'),
  }))
  .output(outputSchema)
