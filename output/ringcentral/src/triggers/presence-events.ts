...
export let presenceEvents = SlateTrigger.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    dndStatus: z.string().describe('Do Not Disturb status (TakeAllCalls, DoNotAcceptAnyCalls, etc.)'),
    presenceStatus: z.string().describe('Aggregated presence status (Available, Busy, Offline, etc.)'),
    telephonyStatus: z.string().describe('Telephony status (NoCall, Ringing, CallConnected, etc.)'),
  }))
  .output(z.object({
    extensionId: z.string().describe('Extension ID of the user whose presence changed'),
    userStatus: z.string().describe('User-defined status (Available, Busy, Offline, etc.)'),
