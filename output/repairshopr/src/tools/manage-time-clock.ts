...
export let listClockEntries = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    createdBefore: z.string().optional().describe('Filter entries before this date (YYYY-MM-DD)'),
    createdAfter: z.string().optional().describe('Filter entries after this date (YYYY-MM-DD)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let entries = (result.employee_clock_entries || result.clock_entries || []).map(mapClockEntry);
...
export let createClockEntry = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    endTime: z.string().optional().describe('Clock-out time (ISO 8601 format). Leave empty for an open clock-in.'),
    notes: z.string().optional().describe('Entry notes'),
  }))
...
export let updateClockEntry = SlateTool.create(
  spec,
  {
