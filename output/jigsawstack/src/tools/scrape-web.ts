...
export let scrapeWeb = SlateTool.create(
  spec,
  {
    name: 'Scrape Website',
    key: 'scrape_web',
  ...
  .input(z.object({
    ...
    elementPrompts: z.array(z.string()).optional().describe('Natural language descriptions of data to extract (max 5), e.g. ["product name", "price", "rating"]'),
    selectors: z.array(z.string()).optional().describe('CSS selectors to target specific elements'),
    rootElementSelector: z.string().optional().describe('CSS selector to scope the scraping area (defaults to "main")'),
    scroll: z.boolean().optional().describe('Whether to auto-scroll the full page before scraping'),
    pagePosition: z.number().optional().describe('Page number for paginated results'),
    httpHeaders: z.record(z.string(), z.string()).optional().describe('Custom HTTP headers to include in the request'),
  ...
  .output(z.object({
    ...
    })).optional().describe('Scraped data organized by element prompts or selectors'),
    context: z.record(z.string(), z.array(z.string())).optional().describe('Extracted context keyed by element prompt'),
    selectors: z.record(z.string(), z.array(z.string())).optional().describe('Matched selectors'),
