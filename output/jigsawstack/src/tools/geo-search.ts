...
export let geoSearch = SlateTool.create(
  spec,
  {
    name: 'Geo Search',
    key: 'geo_search',
  ...
  .input(z.object({
    ...
    includeSuggestions: z.boolean().optional().describe('Also return autocomplete search suggestions (default: false)'),
  }))
  .output(z.object({
    success: z.boolean(),
    results: z.array(z.unknown()).optional().describe('Geo search results with addresses, coordinates, and location metadata'),
    suggestions: z.array(z.string()).optional().describe('Search suggestions for autocomplete (if includeSuggestions was true)'),
  }))
