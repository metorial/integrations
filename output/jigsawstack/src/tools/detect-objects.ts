...
export let detectObjects = SlateTool.create(
  spec,
  {
    name: 'Detect Objects',
    key: 'detect_objects',
  ...
  .input(z.object({
    ...
    prompts: z.array(z.string()).optional().describe('Specific objects to search for (1-150 characters each)'),
    features: z.array(z.enum(['object_detection', 'gui'])).optional().describe('Detection features to enable (default: ["object_detection", "gui"])'),
    annotatedImage: z.boolean().optional().describe('Whether to return an annotated image with bounding boxes (default: false)'),
  }))
  .output(z.object({
    success: z.boolean(),
    ...
    })).optional().describe('Detected objects with labels and bounding boxes'),
    guiElements: z.array(z.object({
      content: z.string().optional(),
      bounds: z.unknown().optional(),
    })).optional().describe('Detected GUI elements'),
    tags: z.array(z.string()).optional().describe('Auto-generated descriptive tags'),
    annotatedImageUrl: z.string().optional().describe('URL to annotated image (if requested)'),
  }))
