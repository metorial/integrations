...
export let textToSql = SlateTool.create(
  spec,
  {
    name: 'Text to SQL',
    key: 'text_to_sql',
  ...
  .input(z.object({
    prompt: z.string().describe('Natural language description of the query (minimum 10 characters)'),
    sqlSchema: z.string().optional().describe('SQL schema definition providing table and column context'),
    database: z.enum(['postgresql', 'mysql', 'sqlite']).optional().describe('Target database type'),
    fileStoreKey: z.string().optional().describe('File store key containing the schema (use instead of sqlSchema)'),
  }))
  .output(z.object({
    success: z.boolean(),
