...
export let runToolActivity = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    activityPath: z.string().describe('Name/path of the activity to invoke (e.g., "search", "calculate")'),
    activityInput: z.record(z.string(), z.any()).optional().describe('Input data for the activity'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let results = await client.runToolActivity(ctx.input.toolId, ctx.input.activityPath, ctx.input.activityInput);
