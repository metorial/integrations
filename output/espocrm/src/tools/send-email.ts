...
export let sendEmail = SlateTool.create(
  ...
  .input(z.object({
    ...
    parentType: z.string().optional().describe('Parent entity type to link this email to (e.g., Contact, Account, Lead)'),
    parentId: z.string().optional().describe('Parent record ID to link this email to'),
    attachmentIds: z.array(z.string()).optional().describe('IDs of previously uploaded attachments to include'),
  }))
  .output(z.object({
