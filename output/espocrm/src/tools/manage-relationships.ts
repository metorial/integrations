...
export let manageRelationships = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    entityType: z.string().describe('Entity type of the parent record (e.g., Account, Contact, Lead)'),
    recordId: z.string().describe('ID of the parent record'),
    linkName: z.string().describe('Relationship link name (e.g., contacts, opportunities, meetings)'),
    relatedRecordId: z.string().optional().describe('ID of the record to link or unlink (required for link/unlink)'),
    maxSize: z.number().optional().describe('Maximum number of related records to return (for list action, default 20)'),
    offset: z.number().optional().describe('Offset for pagination (for list action)'),
  }))
  .output(z.object({
    success: z.boolean().describe('Whether the operation succeeded'),
    relatedRecords: z.array(z.record(z.string(), z.any())).optional().describe('List of related records (for list action)'),
    totalRelated: z.number().optional().describe('Total count of related records (for list action)'),
  }))
