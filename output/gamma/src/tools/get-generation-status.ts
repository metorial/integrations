...
export let getGenerationStatusTool = SlateTool.create(
  spec,
  {
...
Returns the current status (pending, completed, or failed), the Gamma URL to view/edit the content, export download URL (if requested), and credit usage information.`,
  ...
  .input(z.object({
    generationId: z.string().describe('The generation ID returned from Generate Content or Generate from Template'),
  }))
  .output(z.object({
    ...
    gammaUrl: z.string().optional().describe('URL to view/edit the generated content in Gamma (available when completed)'),
    exportUrl: z.string().optional().describe('Temporary download URL for PDF or PPTX export (available when completed with exportAs)'),
    creditsDeducted: z.number().optional().describe('Number of credits used for this generation'),
