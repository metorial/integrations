...
let headerFooterPositionSchema = z.object({
  type: z.enum(['text', 'image', 'cardNumber']).describe('Content type for this position'),
  value: z.string().optional().describe('Text value (for type "text") or image URL (for type "image" with custom source)'),
  source: z.string().optional().describe('Image source, e.g. "themeLogo" or "custom" (for type "image")'),
...
let headerFooterSchema = z.object({
  ...
  hideFromFirstCard: z.boolean().optional().describe('Hide header/footer from the first card'),
...
let sharingOptionsSchema = z.object({
...
export let generateContentTool = SlateTool.create(
  spec,
  {
...
Optionally configure tone, audience, language, number of cards, image sourcing, themes, card dimensions, headers/footers, sharing permissions, and export format.`,
  ...
  .input(z.object({
    inputText: z.string().describe('Text content or prompt to generate from. Can include image URLs. Use "\\n---\\n" for explicit card breaks.'),
    textMode: z.enum(['generate', 'condense', 'preserve']).describe('How input text is handled: generate (expand/rewrite), condense (summarize), or preserve (keep as-is)'),
    format: z.enum(['presentation', 'document', 'webpage', 'social']).optional().describe('Output format type. Defaults to "presentation".'),
    themeId: z.string().optional().describe('Theme ID to apply. Use List Themes tool to find available themes.'),
    numCards: z.number().optional().describe('Number of cards/slides to generate (1-60 for Pro, 1-75 for Ultra). Defaults to 10.'),
    cardSplit: z.enum(['auto', 'inputTextBreaks']).optional().describe('How content is split into cards. "auto" uses numCards, "inputTextBreaks" uses "\\n---\\n" breaks in input.'),
    additionalInstructions: z.string().optional().describe('Extra instructions for AI (max 2000 chars), e.g. "Make the titles catchy"'),
    ...
    textOptions: z.object({
      amount: z.enum(['brief', 'medium', 'detailed', 'extensive']).optional().describe('Text volume per card. Defaults to "medium".'),
      tone: z.string().optional().describe('Tone/voice for the content (max 500 chars)'),
      audience: z.string().optional().describe('Target audience description (max 500 chars)'),
      language: z.string().optional().describe('Output language code, e.g. "en", "es", "fr". Defaults to "en".'),
    }).optional().describe('Text generation options'),
    imageOptions: z.object({
      source: z.enum(['aiGenerated', 'pictographic', 'pexels', 'giphy', 'webAllImages', 'webFreeToUse', 'webFreeToUseCommercially', 'placeholder', 'noImages']).optional().describe('Image source. Defaults to "aiGenerated".'),
      model: z.string().optional().describe('AI image model (e.g. "imagen-4-pro", "flux-1-pro", "dall-e-3"). Only used when source is "aiGenerated".'),
      style: z.string().optional().describe('Artistic style for images (max 500 chars), e.g. "photorealistic", "minimal line art"'),
    }).optional().describe('Image configuration'),
    cardOptions: z.object({
      dimensions: z.string().optional().describe('Card dimensions. Presentation: "fluid", "16x9", "4x3". Document: "fluid", "pageless", "letter", "a4". Social: "1x1", "4x5", "9x16".'),
      headerFooter: headerFooterSchema.optional(),
    }).optional().describe('Card layout options'),
