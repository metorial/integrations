...
export let addSurveyResponse = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    personProperties: z.record(z.string(), z.string()).optional().describe('Custom properties to associate with the response'),
    createdAt: z.number().optional().describe('Unix timestamp of when the response was collected. Defaults to now.'),
  }))
  .output(z.object({
