...
export let getAutopilotConfig = SlateTool.create(
  spec,
  {
...
export let addToAutopilot = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    personEmail: z.string().optional().describe('Email address (required for email Autopilot)'),
    personPhoneNumber: z.string().optional().describe('Phone number in E.164 format (required for SMS Autopilot)'),
    personId: z.string().optional().describe('Person ID if already known'),
    personName: z.string().optional().describe('Name of the person'),
    properties: z.record(z.string(), z.string()).optional().describe('Custom properties (e.g., locale, question_product_name)'),
  }))
  .output(z.object({
    person: z.any().nullable().describe('Person details'),
    properties: z.record(z.string(), z.string()).describe('Properties associated with the membership'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
...
export let listAutopilotMembers = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    personPhoneNumber: z.string().optional().describe('Filter by phone number in E.164 format'),
  }))
  .output(z.object({
    members: z.array(z.object({
      ...
      person: z.any().nullable().describe('Person details'),
      nextSurveyRequest: z.any().nullable().describe('Next scheduled survey request details'),
...
export let removeFromAutopilot = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    platform: z.enum(['email', 'sms']).describe('Autopilot platform to remove the person from'),
