...
export let shortenUrl = SlateTool.create(
  ...
  .input(z.object({
    url: z.string().describe('The long URL to shorten (must start with http:// or https://)'),
    domain: z.string().optional().describe('Custom domain for the short link (e.g., go.example.com)'),
    slug: z.string().optional().describe('Custom alias for the short link (letters, numbers, underscores, hyphens only; must be unique)'),
