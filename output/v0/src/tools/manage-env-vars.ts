...
export let listEnvVarsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    decrypted: z.boolean().optional().describe('Whether to return decrypted values (defaults to false)'),
  }))
  .output(z.object({
...
export let createEnvVarsTool = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Fails if a variable key already exists unless upsert is set to true.',
    ],
  }
)
  .input(z.object({
    ...
    upsert: z.boolean().optional().describe('If true, update existing variables instead of failing'),
  }))
  .output(z.object({
...
export let updateEnvVarsTool = SlateTool.create(
  spec,
  {
...
export let deleteEnvVarsTool = SlateTool.create(
  spec,
  {
