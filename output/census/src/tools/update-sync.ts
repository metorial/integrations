...
export let updateSync = SlateTool.create(
  spec,
  {
    name: 'Update Sync',
    key: 'update_sync',
  ...
  .input(z.object({
    ...
    operation: z.enum(['upsert', 'update', 'insert', 'mirror', 'append']).optional().describe('New sync behavior.'),
    paused: z.boolean().optional().describe('Whether to pause or unpause the sync.'),
    scheduleFrequency: z.enum(['never', 'continuous', 'quarter_hourly', 'hourly', 'daily', 'weekly', 'expression']).optional().describe('New schedule frequency.'),
    ...
    })).optional().describe('Updated field mappings (replaces all existing mappings).'),
    failedRunNotificationsEnabled: z.boolean().optional().describe('Enable or disable failed run notifications.'),
    failedRecordNotificationsEnabled: z.boolean().optional().describe('Enable or disable failed record notifications.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.scheduleFrequency !== undefined) updates.scheduleFrequency = ctx.input.scheduleFrequency;
    ...
    if (ctx.input.scheduleMinute !== undefined) updates.scheduleMinute = ctx.input.scheduleMinute;
    if (ctx.input.cronExpression !== undefined) updates.cronExpression = ctx.input.cronExpression;
    if (ctx.input.failedRunNotificationsEnabled !== undefined) updates.failedRunNotificationsEnabled = ctx.input.failedRunNotificationsEnabled;
    if (ctx.input.failedRecordNotificationsEnabled !== undefined) updates.failedRecordNotificationsEnabled = ctx.input.failedRecordNotificationsEnabled;
