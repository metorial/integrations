...
export let manageWebhook = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'delete']).describe('Action to perform on the webhook.'),
    webhookId: z.number().optional().describe('ID of the webhook (required for update and delete).'),
    name: z.string().optional().describe('Name of the webhook (required for create).'),
    endpoint: z.string().optional().describe('HTTPS URL to receive webhook events (required for create).'),
    description: z.string().optional().describe('Description of the webhook.'),
    events: z.array(z.enum(['sync.alert.raised', 'sync.alert.resolved'])).optional().describe('Event types to subscribe to. If omitted, receives all events.'),
  }))
  .output(z.object({
