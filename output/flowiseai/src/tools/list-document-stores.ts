...
export let listDocumentStores = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({}))
  .output(z.object({
    documentStores: z.array(z.object({
      storeId: z.string().describe('Unique identifier of the document store'),
      name: z.string().describe('Name of the document store'),
      description: z.string().optional().nullable().describe('Description of the document store'),
      status: z.string().optional().describe('Status: EMPTY, SYNC, SYNCING, STALE, NEW, UPSERTING, UPSERTED'),
