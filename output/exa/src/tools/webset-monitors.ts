...
export let createMonitorTool = SlateTool.create(
  spec,
  {
    name: 'Create Monitor',
    key: 'create_monitor',
...
Schedules use cron expressions.`,
  ...
  .input(z.object({
    ...
    behaviorType: z.enum(['search', 'refresh']).describe('search: find new content, refresh: update existing items'),
    cronSchedule: z.string().describe('Cron expression for the schedule (e.g., "0 9 * * 1" for every Monday at 9am)'),
  }))
...
export let deleteMonitorTool = SlateTool.create(
  spec,
  {
    name: 'Delete Monitor',
    key: 'delete_monitor',
