...
export let createResearchTool = SlateTool.create(
  spec,
  {
...
Optionally provide an output schema for structured JSON results.`,
  ...
  .input(z.object({
    ...
    model: z.enum(['exa-research-fast', 'exa-research', 'exa-research-pro']).optional().describe('Research model to use (default: exa-research)'),
    outputSchema: z.record(z.string(), z.unknown()).optional().describe('JSON Schema for structured output format'),
  }))
  .output(z.object({
    researchId: z.string().describe('Unique research task identifier for polling'),
    status: z.string().describe('Current status: pending, running, completed, failed, or canceled'),
...
export let getResearchTool = SlateTool.create(
  spec,
  {
...
Returns the current status, and when completed, the full research output with content and cost information.`,
  ...
  .output(z.object({
    ...
    status: z.string().describe('Current status: pending, running, completed, failed, or canceled'),
    content: z.string().optional().describe('Research output content (available when completed)'),
    parsedOutput: z.record(z.string(), z.unknown()).optional().describe('Structured output if an output schema was provided'),
