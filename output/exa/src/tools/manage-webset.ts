...
export let createWebsetTool = SlateTool.create(
  spec,
  {
    name: 'Create Webset',
    key: 'create_webset',
...
Optionally include enrichments to extract additional structured data from found items.`,
  ...
  .input(z.object({
    query: z.string().optional().describe('Natural language search description (1-5000 chars)'),
    count: z.number().optional().describe('Target number of results (default: 10)'),
    entity: z.enum(['company', 'person', 'article', 'research_paper', 'custom']).optional().describe('Entity type to search for'),
    criteria: z.array(z.object({
      description: z.string().describe('Criterion description for verifying results'),
    })).optional().describe('Custom evaluation criteria (max 5)'),
    ...
    })).optional().describe('Data extraction tasks for found items'),
...
export let getWebsetTool = SlateTool.create(
  spec,
  {
    name: 'Get Webset',
    key: 'get_webset',
  ...
  .output(websetOutputSchema.extend({
    searches: z.array(z.record(z.string(), z.unknown())).optional().describe('Associated searches'),
    enrichments: z.array(z.record(z.string(), z.unknown())).optional().describe('Associated enrichments'),
    monitors: z.array(z.record(z.string(), z.unknown())).optional().describe('Associated monitors'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new ExaClient(ctx.auth.token);
...
export let listWebsetsTool = SlateTool.create(
  spec,
  {
    name: 'List Websets',
    key: 'list_websets',
...
export let updateWebsetTool = SlateTool.create(
  spec,
  {
    name: 'Update Webset',
    key: 'update_webset',
...
export let deleteWebsetTool = SlateTool.create(
  spec,
  {
    name: 'Delete Webset',
    key: 'delete_webset',
...
export let cancelWebsetTool = SlateTool.create(
  spec,
  {
    name: 'Cancel Webset',
    key: 'cancel_webset',
