...
export let getContentsTool = SlateTool.create(
  spec,
  {
    name: 'Get Contents',
    key: 'get_contents',
...
Useful when you already have specific URLs and need to extract their content.`,
  ...
  .input(z.object({
    ...
    ]).optional().describe('Return full text content as markdown'),
    highlights: z.union([
      z.boolean(),
      z.object({
    ...
    ]).optional().describe('Return key excerpts'),
    summary: z.object({
      query: z.string().optional().describe('Custom query to tailor the summary'),
    }).optional().describe('Return an AI-generated summary'),
    subpages: z.number().optional().describe('Number of subpages to crawl per URL'),
    livecrawlTimeout: z.number().optional().describe('Timeout for live crawling in milliseconds (default: 10000)'),
  }))
  .output(z.object({
