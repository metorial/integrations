...
export let createExportTool = SlateTool.create(
  spec,
  {
    name: 'Export Webset',
    key: 'export_webset',
...
export let getExportTool = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    exportId: z.string().describe('Export job identifier'),
    status: z.string().describe('Export status'),
    downloadUrl: z.string().optional().describe('URL to download the export file (when completed)'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new ExaClient(ctx.auth.token);
