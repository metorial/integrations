...
export let listEstimates = SlateTool.create(
  ...
  .input(z.object({
    status: z.enum(['sent', 'draft', 'invoiced', 'accepted', 'declined', 'expired']).optional().describe('Filter by estimate status'),
    ...
    searchText: z.string().optional().describe('Search estimates by text across multiple fields'),
    sortColumn: z.string().optional().describe('Column to sort results by (e.g. "estimate_number", "date", "total")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.estimateNumber !== undefined) params.estimate_number = ctx.input.estimateNumber;
