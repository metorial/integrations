...
Supports pagination via **page** and **perPage** parameters.`,
  ...
  .input(z.object({
    ...
    paymentMode: z.string().optional().describe('Filter payments by payment mode (e.g. cash, check, bank_transfer)'),
    searchText: z.string().optional().describe('Search text to filter payments'),
    sortColumn: z.string().optional().describe('Column to sort results by (e.g. date, amount, customer_name)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.referenceNumber !== undefined) params.reference_number = ctx.input.referenceNumber;
