...
export let manageCreditNote = SlateTool.create(
  spec,
  {
...
The **customerId** field is required when creating a new credit note. Provide **lineItems** to specify the items on the credit note.`,
  ...
  .input(z.object({
    creditNoteId: z.string().optional().describe('ID of the credit note to update. If omitted, a new credit note is created.'),
    customerId: z.string().optional().describe('ID of the customer (required when creating a new credit note)'),
    ...
    })).optional().describe('Array of line items for the credit note'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.creditNoteNumber !== undefined) data.creditnote_number = ctx.input.creditNoteNumber;
