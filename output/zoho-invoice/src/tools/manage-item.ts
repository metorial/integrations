...
export let manageItem = SlateTool.create(
  ...
  .input(z.object({
    itemId: z.string().optional().describe('ID of the item to update. If omitted, a new item is created.'),
    name: z.string().optional().describe('Name of the item (required when creating)'),
    rate: z.number().optional().describe('Selling price / rate of the item (required when creating)'),
    ...
    productType: z.enum(['goods', 'service']).optional().describe('Whether the item is a physical good or a service'),
  ...
  .output(z.object({
    ...
    lastModifiedTime: z.string().optional().describe('Timestamp when the item was last modified'),
  }))
