...
let lineItemSchema = z.object({
  ...
  discount: z.string().optional().describe('Discount applied to this line item (e.g. "10%" or "5.00")'),
});
...
Supports line items with pricing, taxes, and discounts.`,
  ...
  .input(z.object({
    estimateId: z.string().optional().describe('Estimate ID to update. Omit to create a new estimate.'),
    customerId: z.string().optional().describe('Customer/contact ID for the estimate (required when creating)'),
    ...
    lineItems: z.array(lineItemSchema).optional().describe('Array of line items for the estimate'),
    ...
    discount: z.string().optional().describe('Discount to apply to the estimate total (e.g. "10%" or "5.00")'),
    discountType: z.enum(['entity_level', 'item_level']).optional().describe('How discounts are applied: at entity level or per item'),
  }))
  .output(z.object({
    estimateId: z.string().describe('Unique estimate ID'),
    estimateNumber: z.string().optional().describe('Estimate number'),
    status: z.string().optional().describe('Estimate status (draft, sent, invoiced, accepted, declined, expired)'),
    ...
    createdTime: z.string().optional().describe('ISO timestamp when the estimate was created'),
    lastModifiedTime: z.string().optional().describe('ISO timestamp when the estimate was last modified'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let buildPayload = () => {
      ...
      if (ctx.input.estimateNumber !== undefined) payload.estimate_number = ctx.input.estimateNumber;
