...
export let listProjects = SlateTool.create(
  ...
  .input(z.object({
    ...
    sortColumn: z.string().optional().describe('Column to sort results by (e.g. "project_name", "created_time")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.status) params.filter_by = ctx.input.status === 'active' ? 'Status.Active' : 'Status.Inactive';
