...
export let invoiceActions = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['mark_as_sent', 'mark_as_draft', 'void', 'write_off', 'cancel_write_off', 'send_email']).describe('Action to perform on the invoice'),
    emailRecipients: z.array(z.string()).optional().describe('Email addresses to send the invoice to (for send_email action)'),
