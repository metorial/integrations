...
export let manageExpenses = SlateTool.create(
  ...
  .output(z.object({
    ...
    status: z.number().nullable().optional().describe('Expense status (0=internal, 1=outstanding, 2=invoiced, 4=recouped)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let buildPayload = () => {
      ...
      if (ctx.input.amount !== undefined) payload.amount = { amount: ctx.input.amount, code: ctx.input.currencyCode || 'USD' };
