...
export let manageEstimates = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    estimateId: z.number().optional().describe('Estimate ID (required for update/delete/send)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'send') {
      ...
      let result = await fbClient.sendEstimateByEmail(ctx.input.estimateId, ctx.input.emailRecipients);
      return {
        output: mapResult(result),
