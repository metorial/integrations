...
export let invoiceEvents = SlateTrigger.create(
  spec,
  {
    name: 'Invoice Events',
    key: 'invoice_events',
    description: 'Triggered when invoices are created, updated, deleted, or sent by email in FreshBooks.',
  }
)
  .input(z.object({
    eventName: z.string(),
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: { invoiceId: ctx.input.objectId, accountId: ctx.input.accountId, eventName: ctx.input.eventName, identityId: ctx.input.identityId },
      };
    },
  }).build();
