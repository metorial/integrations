...
export let taxEvents = SlateTrigger.create(
  spec,
  {
    name: 'Tax Events',
    key: 'tax_events',
    description: 'Triggered when tax configurations are created, updated, or deleted in FreshBooks.',
  }
)
  .input(z.object({
    eventName: z.string(),
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: { taxId: ctx.input.objectId, accountId: ctx.input.accountId, eventName: ctx.input.eventName, identityId: ctx.input.identityId },
      };
    },
  }).build();
