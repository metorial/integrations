...
export let estimateEvents = SlateTrigger.create(
  spec,
  {
    ...
    key: 'estimate_events',
    description: 'Triggered when estimates are created, updated, deleted, or sent by email in FreshBooks.',
  }
)
  .input(z.object({
    eventName: z.string(),
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: { estimateId: ctx.input.objectId, accountId: ctx.input.accountId, eventName: ctx.input.eventName, identityId: ctx.input.identityId },
      };
    },
  }).build();
