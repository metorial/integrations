...
export let manageProject = SlateTool.create(
  ...
  .input(z.object({
    ...
    isBillable: z.boolean().optional().describe('Whether the project is billable (required for create)'),
    billBy: z.string().optional().describe('Billing method: "Tasks", "People", or "none" (required for create)'),
    budgetBy: z.string().optional().describe('Budget method: "project", "project_cost", "task", "task_fees", "person", "none" (required for create)'),
    ...
    notifyWhenOverBudget: z.boolean().optional().describe('Send notifications when over budget'),
    overBudgetNotificationPercentage: z.number().optional().describe('Percentage threshold for over-budget notifications'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      if (!ctx.input.clientId || !ctx.input.name || ctx.input.isBillable === undefined || !ctx.input.billBy || !ctx.input.budgetBy) {
        throw new Error('clientId, name, isBillable, billBy, and budgetBy are required for create');
      }
