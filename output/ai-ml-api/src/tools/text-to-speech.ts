...
Returns a URL to the generated audio file.`,
  ...
  .input(z.object({
    model: z.string().describe('TTS model ID, e.g. "openai/tts-1", "openai/tts-1-hd", "elevenlabs/eleven_multilingual_v2"'),
    text: z.string().min(1).max(4096).describe('Text content to convert to speech (1-4096 characters)'),
    voice: z.string().optional().describe('Voice name. For OpenAI: alloy, ash, ballad, coral, echo, fable, nova, onyx, sage, shimmer, verse'),
    audioFormat: z.enum(['mp3', 'opus', 'aac', 'flac', 'wav', 'pcm']).optional().describe('Output audio format. Default: mp3'),
    speed: z.number().min(0.25).max(4.0).optional().describe('Speech speed. 1.0 is default, lower is slower, higher is faster'),
  }))
  .output(z.object({
