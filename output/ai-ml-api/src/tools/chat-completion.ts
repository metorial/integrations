...
let messageSchema = z.object({
  role: z.enum(['system', 'developer', 'user', 'assistant']).describe('The role of the message author'),
...
Use this for text generation, code generation, reasoning, question answering, and conversational AI.`,
  ...
  .input(z.object({
    model: z.string().describe('Model ID to use, e.g. "gpt-4o", "deepseek/deepseek-r1", "claude-3.5-sonnet"'),
    messages: z.array(messageSchema).describe('Conversation messages with roles and content'),
    temperature: z.number().min(0).max(2).optional().describe('Sampling temperature between 0 and 2. Higher values produce more random output'),
    maxTokens: z.number().optional().describe('Maximum number of tokens to generate'),
    topP: z.number().min(0).max(1).optional().describe('Nucleus sampling: consider tokens with top_p probability mass'),
    frequencyPenalty: z.number().min(-2).max(2).optional().describe('Penalize tokens based on their frequency in text so far'),
    presencePenalty: z.number().min(-2).max(2).optional().describe('Penalize tokens based on whether they appear in text so far'),
    stop: z.array(z.string()).optional().describe('Stop sequences that will halt generation'),
    ...
    webSearch: z.boolean().optional().describe('Enable web search to retrieve real-time information'),
  }))
  .output(z.object({
