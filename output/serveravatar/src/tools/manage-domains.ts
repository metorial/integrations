...
export let manageDomains = SlateTool.create(
  ...
  .output(z.object({
    domains: z.array(z.record(z.string(), z.unknown())).optional().describe('List of domains'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'add') {
      ...
      return {
        output: {
          responseMessage: (result as Record<string, unknown>).message as string || 'Domain added',
          domains: undefined,
        },
    ...
    if (action === 'remove') {
      ...
      let result = await client.removeDomain(orgId, serverId, applicationId, ctx.input.domainId);
      return {
        output: {
          responseMessage: (result as Record<string, unknown>).message as string || 'Domain removed',
          domains: undefined,
        },
