...
export let rewardEvents = SlateTrigger.create(
  spec,
  {
    name: 'Reward Events',
    key: 'reward_events',
    description: 'Triggered when a reward is added, updated, deleted, or issued. Covers the full reward lifecycle including payout events.',
  },
)
  .input(z.object({
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        type: eventTypeMap[ctx.input.eventType] || `reward.${ctx.input.eventType.toLowerCase()}`,
        ...
        output: {
          ...
          recipientEmailAddress: (r.recipientEmailAddress || r.RecipientEmailAddress) as string | undefined,
