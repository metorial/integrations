...
export let updateProject = SlateTool.create(
  ...
  .input(z.object({
    ...
    priorityEnabled: z.boolean().optional().describe('Enable or disable task prioritization for this project'),
    defaultPriority: z.enum(['No Priority', 'Urgent', 'High', 'Medium', 'Low']).optional().describe('Default priority for new tasks when prioritization is enabled'),
    estimateEnabled: z.boolean().optional().describe('Enable or disable time estimation for tasks'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.priorityEnabled !== undefined) updateData.priority = ctx.input.priorityEnabled;
    if (ctx.input.defaultPriority !== undefined) updateData.defaultPriority = ctx.input.defaultPriority;
    if (ctx.input.estimateEnabled !== undefined) updateData.estimate = ctx.input.estimateEnabled;
    if (ctx.input.milestoneEnabled !== undefined) updateData.milestone = ctx.input.milestoneEnabled;
