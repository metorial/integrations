...
Supports duplicate detection when enabled. Use **@odata.bind** annotations in the record data to associate new records with existing ones during creation.`,
  ...
  .input(z.object({
    entitySetName: z.string().describe('OData entity set name (e.g., "accounts", "contacts", "leads", "opportunities", "incidents")'),
    recordData: z.record(z.string(), z.any()).describe('Record field values to set on the new record'),
    detectDuplicates: z.boolean().optional().describe('Enable duplicate detection using existing rules'),
  }))
  .output(z.object({
    record: z.record(z.string(), z.any()).describe('The created record with all returned fields'),
  }))
