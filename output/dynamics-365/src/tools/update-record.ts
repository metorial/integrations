...
export let updateRecord = SlateTool.create(
  ...
  .input(z.object({
    ...
    recordData: z.record(z.string(), z.any()).describe('Fields and values to update on the record'),
  }))
  .output(z.object({
    record: z.record(z.string(), z.any()).describe('The updated record with all returned fields'),
  }))
