...
export let listEntities = SlateTool.create(
  ...
  .input(z.object({
    filter: z.string().optional().describe('OData $filter to narrow entities (e.g., "IsCustomEntity eq true")'),
  }))
  .output(z.object({
...
export let getEntityAttributes = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    entityLogicalName: z.string().describe('Logical name of the entity (e.g., "account", "contact", "lead")'),
  }))
  .output(z.object({
    attributes: z.array(z.object({
      ...
      attributeType: z.string().describe('Data type of the attribute (e.g., String, Integer, DateTime, Lookup)'),
      requiredLevel: z.string().describe('Whether the attribute is required (None, Recommended, ApplicationRequired, SystemRequired)'),
      isValidForCreate: z.boolean().describe('Whether the attribute can be set during creation'),
      isValidForUpdate: z.boolean().describe('Whether the attribute can be modified during updates'),
