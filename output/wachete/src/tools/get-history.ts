...
export let getHistory = SlateTool.create(
  spec,
  {
...
Supports date range filtering and pagination for large datasets.`,
  ...
  .output(z.object({
    entries: z.array(z.object({
      ...
      valueChangedTimestamp: z.string().optional().describe('When the value last changed'),
      ...
      diff: z.array(z.object({
        operation: z.number().optional().describe('Diff operation: 0=equal, -1=delete, 1=insert'),
        text: z.string().optional().describe('Text segment for this diff operation'),
      })).optional().describe('Diff between this value and the previous one (Google diff format)'),
    })).describe('Historical content entries'),
    continuationToken: z.string().optional().describe('Token to fetch the next page of results'),
  }))
