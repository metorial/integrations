...
Configure XPath selectors, check frequency, alert conditions, notification channels, and more.`,
  ...
  .input(z.object({
    ...
    regex: z.string().optional().describe('Regex to extract specific content from the matched XPath'),
    jobType: z.enum(['SinglePage', 'Portal']).default('SinglePage').describe('Monitor type: SinglePage for a single URL, Portal for crawling subpages'),
    recurrenceInSeconds: z.number().optional().describe('How often to check the page, in seconds (e.g., 86400 for daily)'),
    alerts: z.array(z.object({
      type: z.enum(['Error', 'NotEq']).describe('Alert condition type: Error for page errors, NotEq for content changes'),
    })).optional().describe('Alert conditions that trigger notifications'),
    ...
    })).optional().describe('Where to send alert notifications'),
    dynamicContent: z.boolean().optional().describe('Enable JavaScript rendering for pages with dynamically loaded content'),
    proxies: z.array(z.object({
      location: z.enum(['us', 'gb']).describe('Proxy location'),
    })).optional().describe('Monitor from specific geographic locations'),
    ...
    headers: z.array(z.object({
      key: z.string(),
      value: z.string(),
    })).optional().describe('Custom HTTP headers for the request'),
    body: z.string().optional().describe('HTTP request body (for POST requests)'),
    crawlingDepth: z.number().min(1).max(3).optional().describe('Crawling depth for Portal type (1-3)'),
    urlFilter: z.object({
      include: z.array(urlFilterValueSchema).optional().describe('URL patterns to include'),
      exclude: z.array(urlFilterValueSchema).optional().describe('URL patterns to exclude'),
    }).optional().describe('URL filters for Portal crawling'),
    crawlPagesFromAllDomains: z.boolean().optional().describe('Whether to crawl pages from all domains'),
