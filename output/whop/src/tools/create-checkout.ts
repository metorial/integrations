...
Use **mode** \`payment\` with either an existing \`planId\` or an inline \`plan\` definition, or use \`setup\` mode to save a payment method.`,
  ...
  .input(z.object({
    ...
    }).optional().describe('Inline plan definition (for payment mode without planId)'),
    companyId: z.string().optional().describe('Company ID (required for setup mode). Uses config companyId if not provided.'),
