...
export let createTag = SlateTool.create(
  ...
  .input(z.object({
    ...
    tagNames: z.array(z.string())
      .describe('Names of the tags to create'),
  }))
  .output(z.object({
    tags: z.array(z.record(z.string(), z.any()))
      .describe('Created tag objects'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new BiginClient({
      token: ctx.auth.token,
...
export let addTagsToRecords = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    recordIds: z.array(z.string())
      .describe('IDs of the records to tag'),
    tagNames: z.array(z.string())
      .describe('Names of the tags to add'),
    overWrite: z.boolean().optional()
      .describe('If true, existing tags on the records will be replaced. Default is false (append).'),
  }))
  .output(z.object({
    status: z.string()
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.addTagsToRecords(ctx.input.module, ctx.input.recordIds, ctx.input.tagNames, ctx.input.overWrite);
...
export let removeTagsFromRecord = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    module: z.enum(['Contacts', 'Accounts', 'Pipelines', 'Products', 'Tasks', 'Events', 'Calls'])
      .describe('Module API name'),
    recordId: z.string()
      .describe('ID of the record to remove tags from'),
    tagNames: z.array(z.string())
      .describe('Names of the tags to remove'),
  }))
  .output(z.object({
    status: z.string()
      .describe('Operation status'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.removeTagsFromRecord(ctx.input.module, ctx.input.recordId, ctx.input.tagNames);
    let firstItem = result.data?.[0];
