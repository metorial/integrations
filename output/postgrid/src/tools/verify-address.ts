...
export let verifyAddress = SlateTool.create(
  ...
  .input(z.object({
    ...
    structuredAddress: structuredAddressSchema.optional().describe('Structured address fields'),
    includeDetails: z.boolean().optional().describe('Include parsed address details (street name, type, county, etc.)'),
    properCase: z.boolean().optional().describe('Return addresses in proper case instead of uppercase'),
    geocode: z.boolean().optional().describe('Include latitude and longitude (counts as 2 lookups)'),
  }))
  .output(z.object({
    ...
    provinceOrState: z.string().optional().nullable().describe('Standardized province or state'),
    postalOrZip: z.string().optional().nullable().describe('Standardized postal or ZIP code'),
    ...
    errors: z.record(z.string(), z.any()).optional().nullable().describe('Corrections or errors found during verification'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new AddressVerificationClient(ctx.auth.token);

    let address: string | { line1: string; line2?: string; city?: string; provinceOrState?: string; postalOrZip?: string; country?: string };
...
export let verifyInternationalAddress = SlateTool.create(
  spec,
  {
