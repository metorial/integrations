...
export let autocompleteAddress = SlateTool.create(
  ...
  .input(z.object({
    partialStreet: z.string().describe('Partial street address to autocomplete (e.g., "182 Se")'),
  }))
  .output(z.object({
    suggestions: z.array(z.object({
      address: z.string().describe('Suggested street address'),
      city: z.string().optional().nullable().describe('City'),
      postalCodePrefix: z.string().optional().nullable().describe('First digits of the postal code'),
...
export let autocompleteInternationalAddress = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    countriesFilter: z.string().optional().describe('Comma-separated country codes to filter results (e.g., "GB,DE,FR")'),
  }))
  .output(z.object({
    suggestions: z.array(z.object({
      suggestionId: z.string().optional().nullable().describe('Suggestion ID for retrieving the full address'),
      text: z.string().optional().nullable().describe('Primary suggestion text'),
      description: z.string().optional().nullable().describe('Additional description (e.g., city, region)'),
