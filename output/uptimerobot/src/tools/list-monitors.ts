...
let monitorSchema = z.object({
  ...
  status: z.number().describe('Current status: 0=Paused, 1=Not checked yet, 2=Up, 8=Seems down, 9=Down'),
  interval: z.number().describe('Check interval in seconds'),
  uptimeRatio: z.string().optional().describe('Uptime ratio percentages for requested periods'),
  ssl: z.object({
    brand: z.string().optional(),
    product: z.string().nullable().optional(),
    expires: z.number().optional()
  }).optional().describe('SSL certificate information')
});
...
export let listMonitors = SlateTool.create(
  ...
  .input(z.object({
    ...
    types: z.array(z.enum(['http', 'keyword', 'ping', 'port', 'heartbeat'])).optional().describe('Filter by monitor types'),
    statuses: z.array(z.enum(['paused', 'not_checked', 'up', 'seems_down', 'down'])).optional().describe('Filter by current statuses'),
    search: z.string().optional().describe('Search in URL and friendly name'),
    includeUptimeRatio: z.boolean().optional().describe('Include uptime ratio for the last 1, 7, and 30 days'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let monitors = result.monitors.map((m: any) => ({
      ...
      ...(m.ssl && { ssl: { brand: m.ssl.brand, product: m.ssl.product, expires: m.ssl.expires } })
    }));
