...
export let createStatusPage = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeAllMonitors: z.boolean().optional().describe('Include all monitors (default false). If false, provide monitorIds.'),
    monitorIds: z.array(z.number()).optional().describe('Monitor IDs to include on the status page'),
    sort: z.enum(['name_asc', 'name_desc', 'status_up_first', 'status_down_first']).optional().describe('Sort order for monitors on the page'),
    customDomain: z.string().optional().describe('Custom domain URL (e.g. "status.example.com")'),
    password: z.string().optional().describe('Password to protect the status page'),
    active: z.boolean().optional().describe('Whether the status page is active (default true)')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let sortMap: Record<string, number> = {
      name_asc: 1, name_desc: 2, status_up_first: 3, status_down_first: 4
    };
