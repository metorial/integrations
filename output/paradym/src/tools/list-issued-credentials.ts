...
export let listIssuedCredentials = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    filterStatus: z.string().optional().describe('Filter by status (e.g. "active", "revoked")'),
    filterFormat: z.string().optional().describe('Filter by format (e.g. "sd-jwt-vc", "anoncreds")'),
    filterCredentialTemplateId: z.string().optional().describe('Filter by credential template ID'),
  }))
  .output(z.object({
    credentials: z.array(z.object({
      ...
      revocable: z.boolean().optional().describe('Whether the credential can be revoked'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.filterCredentialTemplateId) filter['credentialTemplateId'] = ctx.input.filterCredentialTemplateId;
