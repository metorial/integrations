...
export let createCredentialTemplate = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    type: z.string().optional().describe('Credential type identifier (required for sd-jwt-vc and mdoc)'),
    schema: z.record(z.string(), z.any()).optional().describe('Schema definition (required for anoncreds)'),
    attributes: z.record(z.string(), z.any()).describe('Attribute definitions for the credential'),
    issuer: z.record(z.string(), z.any()).describe('Issuer configuration (e.g. { method: "did:web" } or { method: "did:cheqd", network: "testnet" })'),
    revocable: z.boolean().optional().describe('Whether issued credentials can be revoked (sd-jwt-vc and anoncreds only)'),
    background: z.record(z.string(), z.any()).optional().describe('Background branding (e.g. { color: "#1E3A5F", imageUrl: "..." })'),
    text: z.record(z.string(), z.any()).optional().describe('Text branding (e.g. { color: "#FFFFFF" })'),
    validFrom: z.string().optional().describe('ISO 8601 date for credential validity start (sd-jwt-vc only)'),
