...
let aiConfigSchema = z.object({
  ...
  model: z.string().optional().describe('AI model to use (e.g., "gpt-4o", "gpt-4o-2024-05-13")'),
  systemMessage: z.string().optional().describe('System message defining the bot persona and behavior'),
  temperature: z.number().min(0).max(2).optional().describe('Controls randomness of responses (0-2). Higher values make output more random.'),
  ...
  presencePenalty: z.number().min(-2).max(2).optional().describe('Penalizes new tokens based on whether they appear in the text so far (-2 to 2)'),
  frequencyPenalty: z.number().min(-2).max(2).optional().describe('Penalizes new tokens based on their frequency in the text so far (-2 to 2)'),
}).optional().describe('AI configuration for the chatbot. Required for AI-powered bots.');
...
export let createBot = SlateTool.create(
  ...
  .input(z.object({
    ...
    botCallbackUrl: z.string().optional().describe('URL where Sendbird sends event callbacks when a user messages the bot'),
    botType: z.string().optional().describe('Category or type of the bot (e.g., "AI assistant", "marketer")'),
    isPrivacyMode: z.boolean().optional().describe('Whether the bot only receives messages directed at it (true) or all messages (false)'),
    enableMarkAsRead: z.boolean().optional().describe('Whether the bot automatically marks messages as read'),
    showMember: z.boolean().optional().describe('Whether to include member info in callback payloads'),
    channelInvitationPreference: z.number().optional().describe('Channel invitation preference (0 or 1)'),
    ai: aiConfigSchema,
  }))
  .output(z.object({
