...
export let manageConfigs = SlateTool.create(
  spec,
  {
    name: 'Manage Configs',
    key: 'manage_configs',
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'get', 'create', 'update', 'delete', 'clone', 'lock', 'unlock']).describe('Action to perform'),
    config: z.string().optional().describe('Config name (required for get, update, delete, clone, lock, unlock)'),
    environment: z.string().optional().describe('Environment slug (for list filtering or create)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      if (!ctx.input.environment) throw new Error('environment is required for "create" action');
      if (!ctx.input.name) throw new Error('name is required for "create" action');

      let config = await client.createConfig(ctx.input.project, ctx.input.environment, ctx.input.name);
    ...
    if (ctx.input.action === 'update') {
      ...
      let config = await client.updateConfig(ctx.input.project, ctx.input.config, ctx.input.name);
    ...
    if (ctx.input.action === 'clone') {
      ...
      let config = await client.cloneConfig(ctx.input.project, ctx.input.config, ctx.input.name);
