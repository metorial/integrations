...
let secretEntrySchema = z.object({
  ...
  computed: z.string().optional().describe('Computed secret value (after variable substitution)'),
...
Secrets are environment variables stored in a specific config within a project.`,
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'get', 'set', 'delete']).describe('Action to perform on secrets'),
    secretName: z.string().optional().describe('Name of a specific secret (for "get" or "delete" actions)'),
    secretNames: z.array(z.string()).optional().describe('List of specific secret names to retrieve (for "list" action)'),
    secrets: z.record(z.string(), z.string()).optional().describe('Key-value pairs of secrets to set (for "set" action)'),
    includeDynamicSecrets: z.boolean().optional().describe('Whether to include dynamic secrets in the response'),
    dynamicSecretsTtlSec: z.number().optional().describe('TTL in seconds for dynamic secret leases'),
  }))
  .output(z.object({
    secrets: z.record(z.string(), secretEntrySchema).optional().describe('Map of secret names to their values'),
    secret: z.object({
      name: z.string().optional(),
      raw: z.string().optional(),
      computed: z.string().optional(),
      note: z.string().optional(),
    }).optional().describe('Single secret details (for "get" action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'get') {
      ...
      let result = await client.getSecret(ctx.input.project, ctx.input.config, ctx.input.secretName);
    ...
    if (ctx.input.action === 'set') {
      ...
      let updated = await client.setSecrets(ctx.input.project, ctx.input.config, ctx.input.secrets);
