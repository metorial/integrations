...
export let downloadSecrets = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    secretNames: z.array(z.string()).optional().describe('Specific secret names to include (omit for all)'),
    includeDynamicSecrets: z.boolean().optional().describe('Include dynamic secrets (defaults to false)'),
    dynamicSecretsTtlSec: z.number().optional().describe('TTL in seconds for dynamic secret leases (defaults to 1800)'),
  }))
  .output(z.object({
    secrets: z.record(z.string(), z.string()).describe('Key-value pairs of secret names and their computed values'),
