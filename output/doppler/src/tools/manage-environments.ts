...
export let manageEnvironments = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'get', 'create', 'rename', 'delete']).describe('Action to perform'),
    environment: z.string().optional().describe('Environment slug (required for get, rename, delete)'),
    name: z.string().optional().describe('Environment display name (for create and rename)'),
    slug: z.string().optional().describe('Environment slug identifier (for create and rename)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      let env = await client.createEnvironment(ctx.input.project, ctx.input.name, ctx.input.slug);
    ...
    if (ctx.input.action === 'rename') {
      if (!ctx.input.environment) throw new Error('environment is required for "rename" action');

      let env = await client.renameEnvironment(ctx.input.project, ctx.input.environment, ctx.input.name, ctx.input.slug);
    ...
    if (ctx.input.action === 'delete') {
      if (!ctx.input.environment) throw new Error('environment is required for "delete" action');
