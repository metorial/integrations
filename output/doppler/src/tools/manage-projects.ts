...
export let manageProjects = SlateTool.create(
  ...
  .input(z.object({
    ...
    project: z.string().optional().describe('Project slug or name (required for get, update, delete)'),
    name: z.string().optional().describe('Project name (required for create and update)'),
    description: z.string().optional().describe('Project description (for create and update)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'list') {
      let result = await client.listProjects({ page: ctx.input.page, perPage: ctx.input.perPage });
    ...
    if (ctx.input.action === 'update') {
      ...
      let project = await client.updateProject(ctx.input.project, ctx.input.name, ctx.input.description);
