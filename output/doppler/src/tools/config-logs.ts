...
let configLogSchema = z.object({
  ...
  })).optional().describe('Detailed diff of changes'),
});

export let configLogs = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'get') {
      ...
      let log = await client.getConfigLog(ctx.input.project, ctx.input.config, ctx.input.logId);
    ...
    if (ctx.input.action === 'rollback') {
      ...
      let log = await client.rollbackConfigLog(ctx.input.project, ctx.input.config, ctx.input.logId);
