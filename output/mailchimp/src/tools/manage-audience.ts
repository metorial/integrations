...
let campaignDefaultsSchema = z.object({
...
export let manageAudienceTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    listId: z.string().optional().describe('Audience ID (required for update/delete, omit for create)'),
    ...
    permissionReminder: z.string().optional().describe('Permission reminder text for subscribers'),
    emailTypeOption: z.boolean().optional().describe('Whether to allow subscribers to choose email format'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.listId) {
      ...
      if (ctx.input.permissionReminder) updateData.permission_reminder = ctx.input.permissionReminder;
      if (ctx.input.emailTypeOption !== undefined) updateData.email_type_option = ctx.input.emailTypeOption;
