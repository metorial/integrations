...
export let manageAutomationTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['start', 'pause', 'archive', 'add_subscriber', 'remove_subscriber']).describe('Action to perform'),
    emailAddress: z.string().optional().describe('Subscriber email (required for add/remove subscriber)'),
    emailId: z.string().optional().describe('Automation email ID (required for add_subscriber)'),
  }))
  .output(z.object({
    workflowId: z.string(),
    action: z.string(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (action) {
      ...
      case 'add_subscriber':
        await client.addSubscriberToAutomationQueue(workflowId, ctx.input.emailId!, ctx.input.emailAddress!);
        return {
          output: { workflowId, action, success: true },
