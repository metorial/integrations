...
export let getCampaignStats = SlateTool.create(
  ...
  .input(z.object({
    entityType: z.enum(['campaigns', 'adsquads', 'ads', 'adaccounts']).describe('Type of entity to get stats for'),
    entityId: z.string().describe('ID of the entity'),
    granularity: z.enum(['TOTAL', 'DAY', 'HOUR', 'LIFETIME']).describe('Time granularity for stats'),
    fields: z.array(z.string()).describe('Metric fields to retrieve (e.g., impressions, swipes, spend, conversion_purchases, conversion_sign_ups)'),
    startTime: z.string().optional().describe('Start time in ISO 8601 format (required for DAY/HOUR granularity)'),
    endTime: z.string().optional().describe('End time in ISO 8601 format (required for DAY/HOUR granularity)'),
    breakdown: z.enum(['campaign', 'ad_squad', 'ad']).optional().describe('Breakdown dimension (only for ad account level stats)'),
    swipeUpAttributionWindow: z.enum(['1_DAY', '7_DAY', '28_DAY']).optional().describe('Swipe-up attribution window'),
    viewAttributionWindow: z.enum(['none', '1_HOUR', '3_HOUR', '6_HOUR', '1_DAY', '7_DAY', '28_DAY']).optional().describe('View attribution window')
  }))
  .output(z.object({
    stats: z.any().describe('Stats data with time series or totals depending on granularity'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.swipeUpAttributionWindow) params.swipe_up_attribution_window = ctx.input.swipeUpAttributionWindow;
    if (ctx.input.viewAttributionWindow) params.view_attribution_window = ctx.input.viewAttributionWindow;
