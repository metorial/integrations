...
export let optimizeWaypoints = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum 12 waypoints including origin and destination'
    ],
    tags: {
      readOnly: true
    }
  ...
  .input(z.object({
    waypoints: z.array(z.object({
      ...
      serviceTimeInSeconds: z.number().optional().describe('Time spent at this waypoint in seconds')
    })).min(3).describe('Waypoints to optimize (minimum 3)'),
    ...
    traffic: z.enum(['historical', 'live']).optional().describe('Traffic model (default: historical)'),
    departAt: z.string().optional().describe('Departure time in ISO 8601 format')
  }))
  .output(z.object({
    optimizedOrder: z.array(z.number()).describe('Optimal visiting order as waypoint indices'),
    ...
    })).optional().describe('Individual leg summaries in optimized order')
  }))
