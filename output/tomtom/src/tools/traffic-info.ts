...
export let trafficFlow = SlateTool.create(
  spec,
  {
    name: 'Traffic Flow',
    key: 'traffic_flow',
  ...
  .input(z.object({
    ...
    zoom: z.number().min(0).max(22).describe('Zoom level (0-22) - higher values match smaller roads')
  }))
  .output(z.object({
...
export let trafficIncidents = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    incidents: z.array(z.object({
      ...
      magnitudeOfDelay: z.number().optional().describe('Magnitude of delay (0=unknown, 1=minor, 2=moderate, 3=major, 4=undefined)'),
      ...
      delayInSeconds: z.number().optional().describe('Delay caused by incident in seconds'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let incidents = rawIncidents.map((inc: any) => {
      ...
      let description = events.map((e: any) => e.description).filter(Boolean).join('; ');
