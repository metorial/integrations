...
export let manageContactGroup = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum 20 webhook addresses per contact group'
    ]
  }
)
  .input(z.object({
    ...
    groupName: z.string().optional().describe('Name for the contact group (for create/update)'),
    webhookUrls: z.array(z.string()).optional().describe('Webhook URLs for notifications'),
    emailAddresses: z.array(z.string()).optional().describe('Email addresses for notifications')
  }))
  .output(z.object({
    ...
    })).optional().describe('List of contact groups (for list action)'),
