...
export let reachableRange = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    routeType: z.enum(['fastest', 'shortest', 'eco']).optional().describe('Route optimization type'),
    travelMode: z.enum(['car', 'truck', 'taxi', 'bus', 'van', 'motorcycle', 'bicycle', 'pedestrian']).optional().describe('Travel mode'),
    traffic: z.boolean().optional().describe('Consider real-time traffic'),
    vehicleEngineType: z.enum(['combustion', 'electric']).optional().describe('Vehicle engine type'),
    vehicleMaxSpeed: z.number().optional().describe('Maximum vehicle speed in km/h'),
    avoid: z.array(z.enum(['tollRoads', 'motorways', 'ferries', 'unpavedRoads'])).optional().describe('Road types to avoid')
  }))
  .output(z.object({
    center: z.object({
