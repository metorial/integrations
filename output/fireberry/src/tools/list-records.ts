...
Returns a paginated list of records for the given object type (e.g., account, contact, opportunity).`,
    constraints: [
      'Maximum page size is 50 records.',
  ...
  .input(z.object({
    ...
    pageSize: z.number().min(1).max(50).optional().describe('Number of records per page (max 50, default 50)'),
    pageNumber: z.number().min(1).max(10).optional().describe('Page number to retrieve (max 10, default 1)'),
  }))
  .output(z.object({
    ...
    records: z.array(z.record(z.string(), z.any())).describe('Array of record objects with all fields'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client(ctx.auth.token);
    let result = await client.listRecords(ctx.input.objectType, ctx.input.pageSize, ctx.input.pageNumber);
