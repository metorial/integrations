...
For example, get all contacts related to an account, or all order items for an order.`,
  ...
  .input(z.object({
    ...
    relatedObjectType: z.string().describe('The related object type system name (e.g., "contact", "opportunity", "orderitem")'),
  }))
  .output(z.object({
    primaryKey: z.string().describe('The primary key field name of the related object'),
    primaryField: z.string().describe('The primary display field name of the related object'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.getRelatedRecords(ctx.input.objectType, ctx.input.recordId, ctx.input.relatedObjectType);
