...
export let getExecution = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeLogs: z.boolean().optional().describe('Also fetch raw execution logs (LLM prompts, model requests/responses)')
  }))
  .output(z.object({
    ...
    extractedData: z.any().optional().describe('Structured data extracted from the conversation'),
    ...
    }).optional().describe('Cost breakdown by component in cents'),
    ...
    }).optional().describe('Telephony call metadata'),
    ...
    })).optional().describe('Raw execution logs')
  }))
