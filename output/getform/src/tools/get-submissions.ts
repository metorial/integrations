...
export let getSubmissions = SlateTool.create(
  spec,
  {
...
Returns submission data including block content, status, dates, and pagination metadata.`,
  ...
  .input(z.object({
    ...
    size: z.number().int().min(1).max(100).optional().describe('Number of results per page (1-100). Defaults to 30 for v2 API, 10 for v1.'),
    query: z.string().optional().describe('Search keyword to filter submissions. Searches across all submission data.'),
    includeFiles: z.boolean().optional().describe('When true, includes file attachment URLs in the response.'),
    timezone: z.string().optional().describe('IANA timezone for date formatting (e.g., "Europe/London", "America/New_York"). Defaults to UTC.'),
  }))
  .output(z.object({
    formId: z.string().describe('The form endpoint ID.'),
    submissions: z.array(z.object({
      submissionId: z.number().describe('Numeric ID of the submission.'),
      submissionDate: z.string().describe('Timestamp of when the submission was received.'),
      status: z.string().describe('Submission status (e.g., "active").'),
      submissionStatus: z.string().describe('Workflow status (e.g., "open").'),
      blocks: z.record(z.string(), z.unknown()).describe('Submitted data organized by block type (sender, text, tracking, etc.).'),
      files: z.array(z.object({
        name: z.string().describe('File name.'),
        url: z.string().describe('URL to download the file.'),
      })).optional().describe('Attached files, if includeFiles was true.'),
