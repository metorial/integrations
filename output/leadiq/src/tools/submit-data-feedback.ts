...
export let submitDataFeedback = SlateTool.create(
  spec,
  {
...
Use this to improve data quality by reporting bounced emails, wrong numbers, or confirming accurate contact information.`,
  ...
  .input(z.object({
    ...
    status: z.enum(['Correct', 'Invalid']).describe('Whether the contact info is correct or invalid'),
    invalidReason: z.enum([
      'EmailBounceCode511', 'EmailBounceCode513', 'EmailBounceCode550', 'EmailBounceCode551',
      'EmailBounceCode552', 'EmailBounceCode553', 'WrongPerson', 'Other',
    ]).optional().describe('Reason for marking as invalid (required when status is Invalid)'),
    type: z.enum([
      'PersonalMobile', 'PersonalLandline', 'WorkHQ', 'WorkPhone',
      'WorkEmail', 'PersonalPhone', 'WorkBranch', 'PersonalEmail', 'WorkMobile', 'Fax',
