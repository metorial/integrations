...
export let offboardingEvents = SlateTrigger.create(
  spec,
  {
    name: 'Offboarding Events',
    key: 'offboarding_events',
    description: 'Triggered when offboarding events occur, including submission, review, payroll submission, completion, deletion, and contract termination.',
  }
)
  .input(z.object({
  ...
  .webhook({
    autoRegisterWebhook: async (ctx) => {
      ...
      let result = await client.createWebhookCallback(ctx.input.webhookBaseUrl, OFFBOARDING_EVENTS);
    ...
    handleRequest: async (ctx) => {
      ...
      let offboardingId: string | undefined = data?.offboarding_id ?? data?.resource_id ?? undefined;
      let eventType: string = data?.event_type ?? '';
      let eventId: string = data?.event_id ?? data?.id ?? `${eventType}-${offboardingId ?? employmentId}-${Date.now()}`;
