...
export let updateEmployment = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    basicInformation: z.record(z.string(), z.any()).optional().describe('Updated basic information fields'),
    personalInformation: z.record(z.string(), z.any()).optional().describe('Updated personal information fields'),
    employmentDetails: z.record(z.string(), z.any()).optional().describe('Updated employment detail fields'),
    administrativeDetails: z.record(z.string(), z.any()).optional().describe('Updated administrative detail fields'),
    invite: z.boolean().optional().describe('If true, also invite the employee to start self-enrollment after update'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.personalInformation) data.personal_information = ctx.input.personalInformation;
    if (ctx.input.employmentDetails) data.employment_details = ctx.input.employmentDetails;
    if (ctx.input.administrativeDetails) data.administrative_details = ctx.input.administrativeDetails;
