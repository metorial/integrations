...
export let manageIncentives = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['create', 'create_recurring', 'update', 'delete', 'delete_recurring', 'list', 'list_recurring', 'get']).describe('Action to perform'),
    incentiveId: z.string().optional().describe('Incentive ID (for get, update, delete)'),
    recurringIncentiveId: z.string().optional().describe('Recurring incentive ID (for delete_recurring)'),
    employmentId: z.string().optional().describe('Employment ID (for create, create_recurring, list, list_recurring)'),
    amount: z.number().optional().describe('Incentive amount'),
    amountTaxType: z.string().optional().describe('Tax type: "gross" or "net"'),
    type: z.string().optional().describe('Incentive type (e.g., bonus, commission, signing_bonus)'),
    effectiveDate: z.string().optional().describe('Effective date for one-time incentive (YYYY-MM-DD)'),
    startDate: z.string().optional().describe('Start date for recurring incentive (YYYY-MM-DD)'),
  ...
  .output(z.object({
    ...
    incentives: z.array(z.record(z.string(), z.any())).optional().describe('List of incentive records'),
