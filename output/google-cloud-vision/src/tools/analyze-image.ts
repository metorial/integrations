...
export let analyzeImage = SlateTool.create(
  ...
  .input(z.object({
    ...
    })).min(1).describe('List of detection features to apply'),
    languageHints: z.array(z.string()).optional().describe('Language hints for text detection'),
    cropAspectRatios: z.array(z.number()).optional().describe('Aspect ratios for crop hints'),
    includeGeoResults: z.boolean().optional().describe('Include geo-based results for web detection'),
  }))
  .output(z.object({
    labels: z.array(z.object({
      ...
      confidence: z.number(),
      topicality: z.number(),
    })).optional().describe('Detected labels'),
    objects: z.array(z.object({
      objectId: z.string(),
      name: z.string(),
      confidence: z.number(),
      boundingPoly: boundingPolySchema,
    })).optional().describe('Detected objects with locations'),
    ...
    })).optional().describe('Detected faces with emotions'),
    ...
    })).optional().describe('Detected landmarks'),
    logos: z.array(z.object({
      logoId: z.string(),
      name: z.string(),
      confidence: z.number(),
    })).optional().describe('Detected logos'),
    ...
    }).optional().describe('Safe search ratings'),
    ...
    })).optional().describe('Dominant colors'),
    cropHints: z.array(z.object({
      boundingPoly: boundingPolySchema,
      confidence: z.number(),
      importanceFraction: z.number(),
    })).optional().describe('Suggested crop regions'),
    webEntities: z.array(z.object({
      entityId: z.string(),
      name: z.string(),
      relevanceScore: z.number(),
    })).optional().describe('Related web entities'),
    bestGuessLabels: z.array(z.object({
      label: z.string(),
    })).optional().describe('Best guess labels from web detection'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (result.labelAnnotations) {
      output.labels = result.labelAnnotations.map((l) => ({
        labelId: l.mid, name: l.description, confidence: l.score, topicality: l.topicality,
      }));
    ...
    if (result.localizedObjectAnnotations) {
      output.objects = result.localizedObjectAnnotations.map((o) => ({
        objectId: o.mid, name: o.name, confidence: o.score, boundingPoly: o.boundingPoly,
      }));
    ...
    if (result.landmarkAnnotations) {
      output.landmarks = result.landmarkAnnotations.map((lm) => ({
        landmarkId: lm.mid, name: lm.description, confidence: lm.score,
        locations: (lm.locations ?? []).map((loc) => ({
          latitude: loc.latLng.latitude, longitude: loc.latLng.longitude,
        })),
      }));
    ...
    if (result.logoAnnotations) {
      output.logos = result.logoAnnotations.map((l) => ({
        logoId: l.mid, name: l.description, confidence: l.score,
      }));
    ...
    if (result.imagePropertiesAnnotation) {
      output.dominantColors = result.imagePropertiesAnnotation.dominantColors.colors.map((c) => ({
        red: c.color.red, green: c.color.green, blue: c.color.blue,
        score: c.score, pixelFraction: c.pixelFraction,
      }));
      summaryParts.push(`${result.imagePropertiesAnnotation.dominantColors.colors.length} colors`);
    }

    if (result.cropHintsAnnotation) {
      output.cropHints = result.cropHintsAnnotation.cropHints.map((h) => ({
        boundingPoly: h.boundingPoly, confidence: h.confidence, importanceFraction: h.importanceFraction,
      }));
    ...
    if (result.webDetection) {
      output.webEntities = (result.webDetection.webEntities ?? []).map((e) => ({
        entityId: e.entityId, name: e.description, relevanceScore: e.score,
      }));
