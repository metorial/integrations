...
export let detectObjects = SlateTool.create(
  ...
  .input(z.object({
    ...
    maxResults: z.number().optional().describe('Maximum number of objects to return (default: 10)'),
  }))
  .output(z.object({
    objects: z.array(z.object({
      ...
      boundingPoly: boundingPolySchema.describe('Bounding polygon around the detected object'),
    })),
  }))
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: objects.length > 0
        ? `Detected **${objects.length}** objects: ${objects.slice(0, 5).map((o) => o.name).join(', ')}${objects.length > 5 ? '...' : ''}`
