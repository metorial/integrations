...
export let detectLabels = SlateTool.create(
  spec,
  {
    name: 'Detect Labels',
    key: 'detect_labels',
  ...
  .input(z.object({
    ...
    maxResults: z.number().optional().describe('Maximum number of labels to return (default: 10)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: labels.length > 0
        ? `Detected **${labels.length}** labels: ${labels.slice(0, 5).map((l) => l.name).join(', ')}${labels.length > 5 ? '...' : ''}`
