...
export let getOpenMetrics = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    metric: z.enum(['signups', 'customers', 'integrations']).describe('The metric to retrieve.'),
    since: z.string().optional().describe('Filter results from this date (YYYY-MM-DD format). Only supported for some metrics.'),
  }))
  .output(z.object({
