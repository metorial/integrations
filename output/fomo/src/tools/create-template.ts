...
export let createTemplate = SlateTool.create(
  ...
  .input(z.object({
    name: z.string().describe('Friendly name for the template (e.g., "New Order", "New Lead").'),
    message: z.string().describe('Template message with variable placeholders (e.g., "{{ first_name }} from {{ city }} just purchased {{ title_with_link }}").'),
    markdownEnabled: z.boolean().optional().describe('Enable markdown-flavored syntax in the template (default: false).'),
    imageUrl: z.string().optional().describe('Fallback image URL, ideally under 50kb.'),
    useAvatar: z.boolean().optional().describe('Use Gravatar avatar from email_address (default: false).'),
    useIpMapping: z.boolean().optional().describe('Convert IP address to city/state/country automatically (default: false).'),
  }))
  .output(z.object({
