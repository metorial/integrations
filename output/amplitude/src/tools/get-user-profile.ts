...
export let getUserProfileTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    userId: z.string().optional().describe('User ID to look up. Provide either userId or amplitudeId.'),
  ...
  .output(z.object({
    userData: z.object({
      ...
      computedUserProperties: z.record(z.string(), z.any()).optional().describe('Computed user properties from Amplitude.'),
      cohortIds: z.array(z.string()).optional().describe('List of cohort IDs the user belongs to.'),
      recommendations: z.array(z.any()).optional().describe('Personalized recommendations for the user.')
