...
export let setAssetStatus = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    customStatusId: z.number().optional().describe('Custom status ID if using custom statuses'),
    downtimeType: z.enum(['PLANNED', 'UNPLANNED']).optional().describe('Type of downtime (for OFFLINE status)'),
    ...
    description: z.string().optional().describe('Description of the status change or downtime reason'),
  }))
  .output(z.object({
