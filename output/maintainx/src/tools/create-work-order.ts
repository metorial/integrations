...
export let createWorkOrder = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    status: z.enum(['OPEN', 'IN_PROGRESS', 'ON_HOLD', 'DONE']).optional().describe('Initial status of the work order'),
    workOrderType: z.enum(['REACTIVE', 'PREVENTIVE']).optional().describe('Type of work order'),
    assignees: z.array(z.number()).optional().describe('User IDs to assign to this work order'),
