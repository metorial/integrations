...
export let getMetrics = SlateTool.create(
  ...
  .input(z.object({
    metricType: z.enum([
      'cpu', 'cpu-limit', 'cpu-target',
      'memory', 'memory-limit', 'memory-target',
      'http-requests', 'http-latency', 'bandwidth',
      'disk-usage', 'disk-capacity',
      'instance-count', 'connections', 'replication-lag',
  ...
  .output(z.object({
    ...
    dataPoints: z.array(z.object({
      ...
      labels: z.record(z.string(), z.string()).optional().describe('Labels for this data point'),
    })),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let dataPoints: Array<{ timestamp: string; value?: number; labels?: Record<string, string> }> = [];
