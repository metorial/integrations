...
export let manageDeploys = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['trigger', 'cancel', 'rollback']).describe('Deployment action to perform'),
    ...
    imageUrl: z.string().optional().describe('Docker image URL to deploy (trigger only, for image-backed services)'),
    clearCache: z.enum(['clear', 'do_not_clear']).optional().describe('Whether to clear build cache (trigger only)'),
  }))
  .output(z.object({
