...
export let runQuery = SlateTool.create(
  ...
  .input(
    z.object({
      ...
      fields: z.array(z.string()).describe('Fields to select (e.g., ["users.name", "orders.count"])'),
      filters: z.record(z.string(), z.string()).optional().describe('Filter expressions keyed by field name (e.g., {"users.state": "California"})'),
      sorts: z.array(z.string()).optional().describe('Sort expressions (e.g., ["orders.count desc"])'),
      limit: z.string().optional().describe('Maximum number of rows to return (default "500")'),
      pivots: z.array(z.string()).optional().describe('Fields to pivot on'),
      total: z.boolean().optional().describe('Whether to include totals'),
      queryTimezone: z.string().optional().describe('Timezone for the query (e.g., "America/Los_Angeles")'),
      resultFormat: z.enum(['json', 'json_detail', 'csv', 'txt', 'html', 'md', 'xlsx', 'sql']).optional().describe('Output format for results (default "json")'),
    })
  )
  .output(
    z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let rowCount = Array.isArray(results) ? results.length : (results?.data ? results.data.length : undefined);
