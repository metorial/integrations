...
let userOutputSchema = z.object({
  ...
  verifiedLookerEmployee: z.boolean().optional().describe('Whether a verified Looker employee'),
...
export let manageUser = SlateTool.create(
  ...
  .input(
    z.object({
      action: z.enum(['get', 'get_me', 'search', 'create', 'update', 'delete', 'set_roles']).describe('Action to perform'),
      ...
      isDisabled: z.boolean().optional().describe('Whether the user is disabled'),
      locale: z.string().optional().describe('User locale'),
      roleIds: z.array(z.string()).optional().describe('Role IDs to assign (for set_roles action)'),
