...
let alertOutputSchema = z.object({
  ...
  comparisonType: z.string().optional().describe('Comparison type (e.g., "EQUAL_TO", "GREATER_THAN")'),
...
export let manageAlert = SlateTool.create(
  ...
  .input(
    z.object({
      ...
      comparisonType: z.string().optional().describe('Comparison type (e.g., "EQUAL_TO", "GREATER_THAN", "GREATER_THAN_OR_EQUAL_TO", "LESS_THAN", "LESS_THAN_OR_EQUAL_TO", "INCREASES_BY", "DECREASES_BY")'),
      ...
      destinations: z.array(z.object({
        destinationType: z.string().describe('Destination type (e.g., "EMAIL", "ACTION_HUB")'),
        emailAddress: z.string().optional().describe('Email address for EMAIL type'),
      })).optional().describe('Alert notification destinations'),
      isDisabled: z.boolean().optional().describe('Whether the alert is disabled'),
      isPublic: z.boolean().optional().describe('Whether the alert is public'),
    })
  )
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'create': {
        ...
        if (!ctx.input.fieldTitle || !ctx.input.fieldName) throw new Error('fieldTitle and fieldName are required');
      ...
      case 'update': {
        ...
        if (ctx.input.comparisonType !== undefined) updateBody.comparison_type = ctx.input.comparisonType;
        ...
        if (ctx.input.customTitle !== undefined) updateBody.custom_title = ctx.input.customTitle;
