...
export let listSubscriptions = SlateTool.create(
  ...
  .input(z.object({
    ...
    limit: z.number().optional().describe('Maximum number of subscriptions to return (default 20)'),
    ...
    status: z.string().optional().describe('Filter by status: OK, PAUSED, CANCELLED, FAILING, FAILED'),
    includePrivateData: z.boolean().optional().describe('Include private/custom field data'),
  }))
  .output(z.object({
    subscriptions: z.array(z.record(z.string(), z.any())).describe('List of subscription objects'),
    pagination: z.object({
