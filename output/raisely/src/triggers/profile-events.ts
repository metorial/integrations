...
export let profileEvents = SlateTrigger.create(
  spec,
  {
    name: 'Profile Events',
    key: 'profile_events',
    description: 'Triggers on fundraising profile events: created, updated, deleted, totalUpdated, exerciseTotalUpdated, memberAdded, memberRemoved, joinedTeam, leftTeam, badge.awarded, badge.revoked.',
  },
)
  .input(z.object({
    eventType: z.string().describe('Type of profile event'),
    profileUuid: z.string().describe('UUID of the profile'),
    profile: z.record(z.string(), z.any()).describe('Full profile object from the webhook payload'),
  }))
  .output(z.object({
    profileUuid: z.string().describe('UUID of the profile'),
