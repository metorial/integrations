...
export let createLead = SlateTool.create(
  ...
  .input(z.object({
    customerId: z.string().optional().describe('PageXCRM customer/account identifier. Overrides the default from configuration if provided.'),
    ...
    platform: z.string().optional().describe('Source platform the lead originated from (e.g. website name, ad platform)'),
  ...
  .output(z.object({
    ...
    rawResponse: z.record(z.string(), z.unknown()).optional().describe('Raw response data from the PageXCRM API'),
  }))
  .handleInvocation(async (ctx) => {
    let customerId = ctx.input.customerId ?? ctx.config.customerId;

    if (!customerId) {
      throw new Error('A customer ID is required. Provide it in the tool input or set it in the global configuration.');
    }
