...
export let getEconomicIndicator = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    interval: z.enum(['daily', 'weekly', 'monthly', 'quarterly', 'annual']).optional().describe('Data interval. Not all intervals are available for every indicator.'),
    maturity: z.enum(['3month', '2year', '5year', '7year', '10year', '30year']).optional().describe('Bond maturity for TREASURY_YIELD indicator'),
  }))
  .output(z.object({
