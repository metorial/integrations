...
export let listMeetings = SlateTool.create(
  ...
  .input(z.object({
    query: z.string().optional().describe('Search keyword to filter meetings by name or content.'),
    happenedAfter: z.string().optional().describe('ISO 8601 date string. Only return meetings that happened after this date.'),
    happenedBefore: z.string().optional().describe('ISO 8601 date string. Only return meetings that happened before this date.'),
    participated: z.boolean().optional().describe('Filter by whether the authenticated user participated in the meeting.'),
    meetingType: z.enum(['internal', 'external']).optional().describe('Filter by meeting type.'),
