...
export let getMetrics = SlateTool.create(
  ...
  .input(z.object({
    reportType: z.enum(['topActiveItems', 'occurrenceCounts', 'activatedCounts']).describe('Type of metric report to fetch'),
    hours: z.number().optional().describe('Time window in hours (for topActiveItems, default 24)'),
    ...
    bucketSize: z.enum(['60', '3600', '86400']).optional().describe('Bucket size in seconds (for occurrenceCounts/activatedCounts): 60=minute, 3600=hour, 86400=day'),
  }))
  .output(z.object({
    topItems: z.array(z.object({
      ...
      occurrenceCount: z.number().optional().describe('Number of occurrences in the time window'),
    })).optional().describe('Top active items (for topActiveItems report)'),
    counts: z.array(z.object({
      timestamp: z.number().describe('Unix timestamp of the bucket start'),
      count: z.number().describe('Count in this bucket'),
    })).optional().describe('Time-bucketed counts (for occurrenceCounts/activatedCounts)'),
  }))
