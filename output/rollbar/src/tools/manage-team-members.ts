...
Requires an **account-level** access token.`,
  ...
  .input(z.object({
    ...
    userId: z.number().optional().describe('User ID (required for "add" and "remove" actions)'),
    email: z.string().optional().describe('Email address to invite (required for "invite" action)'),
  }))
  .output(z.object({
