...
export let manageNotificationRules = SlateTool.create(
  spec,
  {
...
Each rule defines which events trigger a notification, with optional filters and custom configuration.`,
  ...
  .input(z.object({
    ...
    channel: z.enum(['webhook', 'slack', 'pagerduty', 'email']).describe('Notification channel'),
    ruleId: z.number().optional().describe('Rule ID (required for "update" and "delete" actions)'),
    ruleConfig: z.record(z.string(), z.any()).optional().describe('Rule configuration object (required for "create" and "update"). Structure varies by channel — typically includes trigger, filters, and channel-specific config.'),
  }))
  .output(z.object({
    rule: z.object({
    ...
    }).optional().describe('Single rule (for create/update)'),
    rules: z.array(z.object({
      ruleId: z.number().describe('Rule ID'),
      trigger: z.string().optional().describe('Event trigger type'),
      enabled: z.boolean().optional().describe('Whether the rule is enabled'),
      config: z.any().optional().describe('Full rule configuration'),
    })).optional().describe('List of rules (for list action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      let result = await client.createNotificationRule(ctx.input.channel, ctx.input.ruleConfig);
    ...
    if (ctx.input.action === 'update') {
      ...
      let result = await client.updateNotificationRule(ctx.input.channel, ctx.input.ruleId, ctx.input.ruleConfig);
      let rule = mapRule(result?.result);
      return {
        output: { rule },
