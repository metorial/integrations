...
export let itemEvent = SlateTrigger.create(
  spec,
  {
    name: 'Item Event',
    key: 'item_event',
    description: 'Triggers on Rollbar item events: new items, occurrences, exponential occurrences, high occurrence rate, item resolved, reopened, or reactivated.',
  }
)
  .input(z.object({
    eventType: z.string().describe('Type of event (new_item, occurrence, exp_repeat_item, item_velocity, resolved_item, reopened_item, reactivated_item)'),
    ...
    }).optional().describe('Occurrence data (if available)'),
    velocityInfo: z.object({
      ...
      threshold: z.number().optional().describe('Number of events that triggered the notification'),
    }).optional().describe('High occurrence rate info (for item_velocity events)'),
    expRepeatInfo: z.object({
      occurrenceThreshold: z.number().optional().describe('Occurrence threshold that was crossed (10, 100, 1000, etc.)'),
    }).optional().describe('Exponential repeat info (for exp_repeat_item events)'),
  }))
  .output(z.object({
    ...
    lastOccurrenceTimestamp: z.number().optional().describe('Unix timestamp of last occurrence'),
    projectId: z.number().optional().describe('Project ID'),
    occurrenceId: z.string().optional().describe('Occurrence ID (if triggered by a specific occurrence)'),
    ...
    velocityWindowSizeDescription: z.string().optional().describe('Human-readable velocity window'),
    velocityThreshold: z.number().optional().describe('Velocity threshold that was reached'),
    occurrenceThreshold: z.number().optional().describe('Exponential occurrence threshold crossed'),
  }))
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let occurrenceData = occurrence.id ? occurrence : (lastOccurrence.id ? lastOccurrence : undefined);
        ...
        inputs: [
          {
            ...
            velocityInfo: eventType === 'item_velocity' ? {
              windowSize: eventData.window_size,
              windowSizeDescription: eventData.window_size_description,
              threshold: eventData.threshold,
            } : undefined,
            expRepeatInfo: eventType === 'exp_repeat_item' ? {
              occurrenceThreshold: eventData.occurrence_threshold || eventData.occurrences,
            } : undefined,
          }
        ],
      };
