...
export let getVariables = SlateTool.create(
  spec,
  {
    name: 'Get Variables',
    key: 'get_variables',
  ...
  .input(z.object({
    ...
    publishedOnly: z.boolean().optional().default(false).describe('If true, only return published/library variables')
  }))
  .output(z.object({
    variables: z.record(z.string(), z.any()).describe('Map of variable ID to variable data'),
    variableCollections: z.record(z.string(), z.any()).describe('Map of collection ID to collection data')
  }))
...
export let updateVariables = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    fileKey: z.string().describe('The key of the Figma file'),
    variableCollections: z.array(z.any()).optional().describe('Variable collections to create/update/delete'),
    variables: z.array(z.any()).optional().describe('Variables to create/update/delete'),
    variableModes: z.array(z.any()).optional().describe('Variable modes to create/update/delete')
  }))
  .output(z.object({
