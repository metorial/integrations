...
For **HTML**: provide raw HTML content with field tags like \`<text-field>\`, \`<signature-field>\`, etc.`,
  ...
  .input(z.object({
    ...
    documents: z.array(z.object({
      ...
      file: z.string().optional().describe('Base64-encoded file content or URL (for PDF/DOCX)'),
      html: z.string().optional().describe('HTML content (for HTML source type)'),
      fields: z.array(z.record(z.string(), z.any())).optional().describe('Field definitions with coordinates'),
    })).optional().describe('Documents array (required for PDF/DOCX)'),
    html: z.string().optional().describe('HTML content (for HTML source type, single document)'),
    ...
    sharedLink: z.boolean().optional().describe('Make template publicly accessible via link'),
    flatten: z.boolean().optional().describe('Remove PDF form fields (PDF only)'),
    removeTags: z.boolean().optional().describe('Remove {{text}} tags from PDF (PDF only, default true)'),
  }))
  .output(z.object({
