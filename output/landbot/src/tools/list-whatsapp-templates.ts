...
export let listWhatsAppTemplatesTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({}))
  .output(z.object({
    templates: z.array(z.record(z.string(), z.any())).describe('List of WhatsApp template records with IDs, names, languages, and statuses')
  }))
  .handleInvocation(async (ctx) => {
    let client = new PlatformClient(ctx.auth.token);
    let result = await client.listWhatsAppTemplates();
    let templates = result.results ?? result.templates ?? (Array.isArray(result) ? result : []);
