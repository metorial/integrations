...
export let listPvPowerSites = SlateTool.create(
  spec,
  {
...
This operation does not consume API request quota.`,
  ...
  .input(z.object({
    resourceId: z.string().optional().describe('If provided, retrieve details for this specific PV Power Site instead of listing all sites'),
  ...
  .output(z.object({
    ...
    site: siteSchema.optional().describe('Single PV Power Site details (when resourceId is provided)'),
