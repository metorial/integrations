...
export let videoEvents = SlateTrigger.create(
  spec,
  {
    name: 'Video Events',
    key: 'video_events',
    description: 'Triggers on HeyGen video lifecycle events including avatar video completion/failure, video agent completion/failure, video translation completion/failure, and personalized video updates.',
  }
)
  .input(z.object({
    eventType: z.string().describe('Type of event (e.g. avatar_video.success, video_translate.fail)'),
    ...
    callbackId: z.string().optional().describe('Custom callback ID if provided during creation'),
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      let eventId = eventData.video_id
