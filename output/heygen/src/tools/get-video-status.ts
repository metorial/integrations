...
export let getVideoStatus = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    status: z.string().describe('Video status: "processing", "completed", "failed", "pending"'),
    ...
    callbackId: z.string().nullable().describe('Custom callback ID if provided during creation'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let statusMessage = result.status === 'completed'
