...
export let createTransaction = SlateTool.create(
  ...
  .input(z.object({
    ...
    method: z.enum(['card', 'ach', 'check', 'cash', 'digital_wallet', 'paypal', 'venmo', 'donor_advised_fund', 'stock', 'in_kind', 'property', 'other']).describe('Payment method'),
    ...
    }).optional().describe('Donor address'),
    fundId: z.string().optional().describe('Fund ID to designate the donation'),
    transactedAt: z.string().optional().describe('When the transaction occurred (ISO 8601). Defaults to now.'),
  }))
  .output(z.object({
