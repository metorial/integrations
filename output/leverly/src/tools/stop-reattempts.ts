...
export let stopReattempts = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    phone: z.string().describe('Phone number of the lead to stop reattempts for. Must match the number originally submitted.'),
  }))
  .output(z.object({
    submitted: z.boolean().describe('Whether the stop reattempts request was successfully submitted'),
