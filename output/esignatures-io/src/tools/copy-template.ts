...
export let copyTemplate = SlateTool.create(
  ...
  .input(z.object({
    ...
    placeholderFields: z.array(placeholderFieldSchema).optional().describe('Placeholder values to customize the copy'),
    targetApiKey: z.string().optional().describe('API key of another eSignatures account to copy the template to'),
  }))
  .output(z.object({
