...
export let queryContract = SlateTool.create(
  spec,
  {
    name: 'Get Contract',
    key: 'query_contract',
  ...
  .output(z.object({
    ...
    status: z.string().optional().describe('Current status of the contract (e.g., sent, signed, withdrawn)'),
    title: z.string().optional().describe('Title of the contract'),
    metadata: z.string().optional().describe('Custom metadata'),
    source: z.string().optional().describe('How the contract was created (e.g., api, dashboard)'),
    test: z.string().optional().describe('Whether this is a test contract'),
    contractPdfUrl: z.string().optional().describe('URL to download the signed PDF (available after all signers have signed, expires in 3 days)'),
