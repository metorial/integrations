...
let signerSchema = z.object({
  ...
  mobile: z.string().optional().describe('Mobile phone number with country code (e.g., +12481234567)'),
  companyName: z.string().optional().describe('Company name of the signer'),
  signingOrder: z.string().optional().describe('Order in which this signer should sign (e.g., "1", "2")'),
  autoSign: z.enum(['yes', 'no']).optional().describe('Whether this signer should auto-sign (typically the sender)'),
  signatureRequestDeliveryMethods: z.array(z.enum(['email', 'sms'])).optional().describe('How to deliver the signature request'),
  signedDocumentDeliveryMethod: z.enum(['email', 'no_delivery']).optional().describe('How to deliver the final signed document'),
  multiFactorAuthentications: z.array(z.enum(['sms_verification_code', 'email_verification_code', 'photo_id'])).optional().describe('MFA methods for signer identity verification'),
...
let emailsSchema = z.object({
  signatureRequestSubject: z.string().optional().describe('Custom subject line for signature request email'),
  signatureRequestText: z.string().optional().describe('Custom body text for signature request email. Use __FULL_NAME__ for dynamic name insertion.'),
  finalContractSubject: z.string().optional().describe('Custom subject line for the signed contract email'),
  finalContractText: z.string().optional().describe('Custom body text for the signed contract email. Use __FULL_NAME__ for dynamic name insertion.'),
  ccEmailAddresses: z.array(z.string()).optional().describe('Email addresses to CC on the signed PDF'),
  replyTo: z.string().optional().describe('Custom reply-to email address'),
}).optional();

let customBrandingSchema = z.object({
...
let contractSignerOutputSchema = z.object({
  ...
  companyName: z.string().optional().describe('Company name of the signer'),
  signPageUrl: z.string().optional().describe('URL for the signer to sign the contract (can be embedded in an iframe with ?embedded=yes)'),
});
...
export let createContract = SlateTool.create(
  ...
  .input(z.object({
    ...
    metadata: z.string().optional().describe('Custom metadata string for tracking (e.g., an internal ID)'),
    expiresInHours: z.string().optional().describe('Number of hours until the contract expires'),
    customWebhookUrl: z.string().optional().describe('Custom webhook URL for this specific contract'),
    assignedUserEmail: z.string().optional().describe('Email of the assigned user in the eSignatures account'),
    labels: z.array(z.string()).optional().describe('Labels for organizing the contract'),
    test: z.enum(['yes', 'no']).optional().describe('Set to "yes" to create a test/demo contract'),
    saveAsDraft: z.enum(['yes', 'no']).optional().describe('Set to "yes" to save as draft without sending'),
    placeholderFields: z.array(placeholderFieldSchema).optional().describe('Values for template placeholder fields'),
    signerFields: z.array(signerFieldSchema).optional().describe('Default values for signer input fields'),
  ...
  .output(z.object({
    ...
    signers: z.array(contractSignerOutputSchema).optional().describe('List of signers with their sign page URLs'),
  }))
