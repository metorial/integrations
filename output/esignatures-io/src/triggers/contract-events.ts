...
let signerSchema = z.object({
  ...
  signerFieldValues: z.record(z.string(), z.any()).optional().describe('Values entered by the signer in form fields'),
});
...
export let contractEvents = SlateTrigger.create(
  spec,
  {
    ...
    key: 'contract_events',
    description: 'Triggered when a contract event occurs: sent to signer, reminder sent, all signers signed, withdrawn, or PDF preview generated. Configure the webhook URL in your eSignatures dashboard or per-contract.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    contractSource: z.string().optional().describe('Source of the contract (api, dashboard, etc.)'),
    isTest: z.string().optional().describe('Whether this is a test contract'),
    contractPdfUrl: z.string().optional().describe('URL to download the signed PDF (available on contract-signed, expires in 3 days)'),
    signers: z.array(signerSchema).optional().describe('Signers on the contract'),
    sentToSigner: signerSchema.optional().describe('The specific signer the contract was sent/reminded to (for sent/reminder events)'),
  }))
  .webhook({
