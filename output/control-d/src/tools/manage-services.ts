...
export let manageServices = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    operation: z.enum(['list_profile', 'list_categories', 'list_category', 'update']).describe('Operation to perform'),
    profileId: z.string().optional().describe('Profile ID (required for list_profile and update)'),
    categoryId: z.string().optional().describe('Service category ID (required for list_category)'),
    serviceId: z.string().optional().describe('Service ID (required for update, e.g., "facebook", "tiktok")'),
    action: z.number().optional().describe('Action: 0=Block, 1=Bypass, 3=Redirect (required for update)'),
    enabled: z.boolean().optional().describe('Enable or disable the service rule (required for update)'),
    via: z.string().optional().describe('Proxy location code for Redirect action (e.g., "JFK", "YYZ")'),
  }))
  .output(z.object({
    services: z.array(z.object({
      ...
      action: z.string().optional().describe('Current action applied (Block, Bypass, Redirect)'),
      ...
      return {
        output: {
          services: svc ? [{
            ...
            action: svc.action ? actionDescription(svc.action.do, svc.action.via) : undefined,
