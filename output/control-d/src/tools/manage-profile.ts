...
export let manageProfile = SlateTool.create(
  ...
  .input(z.object({
    ...
    disableTtl: z.number().optional().describe('Unix timestamp to disable profile until. Set to 0 to re-enable.'),
    lockStatus: z.number().optional().describe('1 to lock, 0 to unlock'),
    lockMessage: z.string().optional().describe('Message shown when locked profile is modified'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { operation, profileId, name, cloneProfileId, disableTtl, lockStatus, lockMessage } = ctx.input;
    ...
    let profile = await client.modifyProfile(profileId, { name, disableTtl, lockStatus, lockMessage });
    return {
      output: { profileId: profile.PK, name: profile.name, updated: profile.updated },
