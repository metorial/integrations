...
export let listNotifications = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({}))
  .output(z.object({
    customEmails: z.array(z.object({
      emailId: z.number().describe('Notification ID'),
      email: z.string().describe('Email address'),
      disabled: z.boolean().describe('Whether notifications are disabled for this email'),
...
export let manageCustomEmail = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    emailId: z.number().optional().describe('Email notification ID (required for enable/disable/delete actions)'),
  }))
  .output(z.object({
...
export let managePostLoadHook = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'A maximum of 10 post-load hooks can be configured per account.',
    ],
    tags: {
      destructive: false,
    },
  ...
  .input(z.object({
    ...
    hookId: z.number().optional().describe('Hook ID (required for enable/disable/delete actions)'),
  }))
  .output(z.object({
