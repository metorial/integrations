...
export let postLoadTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Post-Load Hook',
    key: 'post_load_hook',
    description: 'Triggers when data is loaded into the destination warehouse. Fires on a per-integration, per-table basis for each successful or rejected load operation.',
  }
)
  .input(z.object({
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      let integrationName = data.integration_name || data.source_name || data.name || 'unknown';
      ...
      let loadStatus = data.status || (rowsRejected && rowsRejected > 0 ? 'rejected' : 'loaded');
