...
export let getFileContent = SlateTool.create(
  ...
  .input(z.object({
    ...
    ref: z.string().optional().describe('Branch name, tag, or commit SHA to read from; defaults to the default branch'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let f = await client.getFileContent(ctx.input.owner, ctx.input.repo, ctx.input.path, ctx.input.ref);
...
export let createOrUpdateFile = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    fileSha: z.string().optional().describe('Current SHA of the file (required for updates, omit for new files)'),
  ...
  .handleInvocation(async (ctx) => {
    let client = new GiteaClient({ token: ctx.auth.token, baseUrl: ctx.auth.baseUrl });
    let author = ctx.input.authorName && ctx.input.authorEmail
...
export let deleteFile = SlateTool.create(
  ...
  .input(z.object({
    ...
    fileSha: z.string().describe('Current SHA of the file (use "Get File Content" to retrieve it)'),
    message: z.string().describe('Commit message for the deletion'),
    branch: z.string().optional().describe('Branch to delete from; defaults to the default branch'),
  }))
  .output(z.object({
