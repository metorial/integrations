...
export let listCollaborators = SlateTool.create(
  spec,
  {
...
export let manageCollaborator = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    permission: z.enum(['read', 'write', 'admin']).optional().describe('Permission level when adding (default: write)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'add') {
      await client.addCollaborator(ctx.input.owner, ctx.input.repo, ctx.input.username, ctx.input.permission);
    } else {
