...
export let createRepo = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    autoInit: z.boolean().optional().describe('Initialize repository with a README (default: false)'),
    defaultBranch: z.string().optional().describe('Default branch name (default: main)'),
    gitignores: z.string().optional().describe('Gitignore template name (e.g., "Go", "Node", "Python")'),
    license: z.string().optional().describe('License template name (e.g., "MIT", "Apache-2.0")'),
    organization: z.string().optional().describe('Organization name to create the repo under; if omitted, creates under the authenticated user'),
  }))
  .output(z.object({
