...
export let createOrganization = SlateTool.create(
  ...
  .input(z.object({
    ...
    visibility: z.enum(['public', 'limited', 'private']).optional().describe('Visibility level'),
  }))
  .output(orgOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new GiteaClient({ token: ctx.auth.token, baseUrl: ctx.auth.baseUrl });
...
export let updateOrganization = SlateTool.create(
  ...
  .input(z.object({
    ...
    visibility: z.enum(['public', 'limited', 'private']).optional().describe('New visibility level'),
  }))
  .output(orgOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new GiteaClient({ token: ctx.auth.token, baseUrl: ctx.auth.baseUrl });
