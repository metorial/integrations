...
export let updateLead = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['todo', 'standby', 'won', 'lost', 'cancelled']).optional().describe('New status for the lead'),
    ...
    tags: z.array(z.string()).optional().describe('Tags to set on the lead (replaces existing tags)'),
    remindDate: z.string().optional().describe('Reminder date (YYYY-MM-DD)'),
    remindTime: z.string().optional().describe('Reminder time (HH:MM)'),
    estimatedClosingDate: z.string().optional().describe('Estimated closing date (YYYY-MM-DD)'),
