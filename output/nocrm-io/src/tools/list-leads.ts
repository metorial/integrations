...
export let listLeads = SlateTool.create(
  ...
  .input(z.object({
    status: z.enum(['todo', 'standby', 'won', 'lost', 'cancelled']).optional().describe('Filter by lead status'),
    ...
    fieldValue: z.string().optional().describe('Custom field value to filter by (requires fieldKey)'),
    updatedAfter: z.string().optional().describe('Only return leads updated after this ISO 8601 timestamp'),
    startDate: z.string().optional().describe('Start date for date range filter (YYYY-MM-DD)'),
    endDate: z.string().optional().describe('End date for date range filter (YYYY-MM-DD)'),
    dateRangeType: z.string().optional().describe('Date range type: created, updated, closed, remind'),
    ...
    order: z.string().optional().describe('Sort field (e.g. created_at, updated_at, amount)'),
