...
export let manageLeadComments = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    activityId: z.number().optional().describe('Activity type ID to log (e.g. call, email, meeting) — only for create'),
  }))
  .output(z.object({
    ...
    comment: z.object({
      ...
      userId: z.number().optional().describe('Author user ID'),
      createdAt: z.string().optional().describe('Creation timestamp'),
    }).optional().describe('Created or updated comment'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'update') {
      ...
      let result = await client.updateLeadComment(ctx.input.leadId, ctx.input.commentId, ctx.input.comment);
      return {
        output: { comment: mapComment(result) },
