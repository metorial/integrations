...
export let manageTeams = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'update', 'delete', 'add_member', 'remove_member']).describe('Operation to perform'),
    teamId: z.number().optional().describe('Team ID (required for get, update, delete, add_member, remove_member)'),
    name: z.string().optional().describe('Team name (required for create, optional for update)'),
    userId: z.number().optional().describe('User ID to add or remove (for add_member/remove_member)'),
  }))
  .output(z.object({
    ...
    }).optional().describe('Single team details'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'list') {
      ...
      return {
        output: { teams: teams.map((t: any) => ({ teamId: t.id, name: t.name, memberCount: t.users?.length || t.members?.length })) },
