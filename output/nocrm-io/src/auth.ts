...
export let auth = SlateAuth.create()
  ...
  .addTokenAuth({
    ...
    inputSchema: z.object({
      apiKey: z.string().describe('Admin API key generated from Admin Panel > Integrations > API > API Keys'),
    }),
    getOutput: async (ctx) => {
      return {
        output: {
  ...
  .addCustomAuth({
    ...
    getProfile: async (ctx: { output: { token: string }; input: { subdomain: string; email: string; password: string } }) => {
      let ax = createAxios({
        baseURL: `https://${ctx.input.subdomain}.nocrm.io/api/v2`,
        headers: {
