...
export let getImageTool = SlateTool.create(
  spec,
  {
    name: 'Get Image',
    key: 'get_image',
  ...
  .output(z.object({
    ...
    archivedUri: z.string().optional().describe('Download URL for the original/archive image'),
...
export let updateImageTool = SlateTool.create(
  spec,
  {
    name: 'Update Image',
    key: 'update_image',
...
export let deleteImageTool = SlateTool.create(
  spec,
  {
    name: 'Delete Image',
    key: 'delete_image',
...
export let uploadImageTool = SlateTool.create(
  spec,
  {
...
export let moveImagesTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['move', 'collect']).describe('Whether to move or collect (copy) the images'),
    targetAlbumKey: z.string().describe('Album key of the destination album'),
    imageUris: z.array(z.string()).describe('Array of image API URIs to move or collect (e.g., ["/api/v2/image/ABC123"])'),
  }))
  .output(z.object({
