...
export let contactChanges = SlateTrigger.create(
  spec,
  {
    ...
    key: 'contact_changes',
    description: 'Triggers when contacts (customers or vendors) are created or updated in Zoho Inventory. Polls for recently modified contacts.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      for (let contact of contacts) {
        if (lastPolledAt && contact.last_modified_time && contact.last_modified_time <= lastPolledAt) {
          break;
        }
        newContacts.push(contact);
      }

      let updatedLastPolled = contacts.length > 0 && contacts[0].last_modified_time
