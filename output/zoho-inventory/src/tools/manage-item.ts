...
let lineItemSchema = z.object({
  ...
  itemType: z.enum(['inventory', 'sales', 'purchases', 'sales_and_purchases']).optional().describe('Item type'),
...
export let manageItem = SlateTool.create(
  spec,
  {
    name: 'Manage Item',
    key: 'manage_item',
...
Use without an **itemId** to create a new item, or with an **itemId** to update an existing one.`,
  ...
  .input(z.object({
    itemId: z.string().optional().describe('ID of the item to update. Omit to create a new item.'),
    ...
    purchaseDescription: z.string().optional().describe('Item description for purchase transactions'),
    ...
    isReturnable: z.boolean().optional().describe('Whether the item is returnable'),
    status: z.enum(['active', 'inactive']).optional().describe('Set item status to active or inactive'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.purchaseDescription !== undefined) body.purchase_description = ctx.input.purchaseDescription;
    ...
    if (ctx.input.initialStockRate !== undefined) body.initial_stock_rate = ctx.input.initialStockRate;
