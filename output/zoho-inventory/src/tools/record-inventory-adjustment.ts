...
let adjustmentLineSchema = z.object({
  ...
  quantityAdjusted: z.number().describe('Quantity to adjust (positive to increase, negative to decrease)'),
  warehouseId: z.string().optional().describe('Warehouse ID for the adjustment'),
  description: z.string().optional().describe('Line item description'),
});

export let recordInventoryAdjustment = SlateTool.create(
  spec,
  {
...
Supports multiple line items per adjustment and per-warehouse adjustments.`,
  ...
  .input(z.object({
    ...
    adjustmentType: z.enum(['quantity', 'value']).optional().describe('Adjustment type (default: quantity)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.adjustmentType !== undefined) body.adjustment_type = ctx.input.adjustmentType;
