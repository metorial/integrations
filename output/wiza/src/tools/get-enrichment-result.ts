...
Use the revealId from the **Enrich Contact** tool to check progress and retrieve results.`,
  ...
  .input(z.object({
    revealId: z.number().describe('The ID of the enrichment request returned by Enrich Contact.'),
  }))
  .output(z.object({
    revealId: z.number().describe('Unique ID of the enrichment request.'),
    status: z.string().describe('Current status: "queued", "resolving", "finished", or "failed".'),
    ...
    primaryEmailStatus: z.string().optional().describe('Verification status of the primary email.'),
    ...
    isPremium: z.boolean().optional().describe('Whether the contact has a LinkedIn Premium account.'),
    isOpenToWork: z.boolean().optional().describe('Whether the contact is open to work on LinkedIn.'),
    tenureAtCompany: z.string().optional().describe('How long the contact has been at their current company.'),
    tenureAtRole: z.string().optional().describe('How long the contact has been in their current role.'),
    ...
    workHistory: z.array(z.record(z.string(), z.unknown())).optional().describe('Work experience history.'),
    education: z.array(z.record(z.string(), z.unknown())).optional().describe('Education history.'),
    certifications: z.array(z.record(z.string(), z.unknown())).optional().describe('Certifications from LinkedIn.'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    ctx.info({ message: 'Retrieved enrichment result', revealId: d.id, status: d.status, isComplete: d.is_complete });
    ...
    if (d.mobile_phone || d.phone_number) parts.push(`Phone: ${d.mobile_phone || d.phone_number}`);
