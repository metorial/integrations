...
export let leadEvents = SlateTrigger.create(
  spec,
  {
    name: 'Lead Events',
    key: 'lead_events',
    description: 'Triggered when a lead is captured through a Cincopa video lead generation form. Receives viewer information submitted via in-video forms.'
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    galleryId: z.string().optional().describe('Gallery ID (fid) where the lead was captured'),
