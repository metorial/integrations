...
export let accountEvents = SlateTrigger.create(
  spec,
  {
    name: 'Account Events',
    key: 'account_events',
    description: 'Triggered on account-level activities such as settings changes and traffic usage updates in your Cincopa account.'
  }
)
  .input(z.object({
    eventType: z.string().describe('Type of account event (e.g., account.settings, account.traffic_usage)'),
