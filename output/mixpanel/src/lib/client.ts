...
let getAppBaseUrl = (residency: 'us' | 'eu'): string => {
  return residency === 'eu' ? 'https://eu.mixpanel.com/api/app' : 'https://mixpanel.com/api/app';
};
...
export class MixpanelClient {
  ...
  constructor(config: MixpanelClientConfig) {
    ...
    let basicAuth = config.serviceAccountUsername && config.serviceAccountSecret
  ...
  }>): Promise<{ code: number; numRecordsImported: number; status: string; failedRecords?: Array<{ index: number; insertId?: string; field?: string; message: string }> }> {
  ...
  // User Profiles
  // =====================
  async setUserProperties(distinctId: string, properties: Record<string, unknown>): Promise<{ success: boolean }> {
    let payload = [{
  ...
  async setUserPropertiesOnce(distinctId: string, properties: Record<string, unknown>): Promise<{ success: boolean }> {
    let payload = [{
      $token: this.config.projectToken,
      $distinct_id: distinctId,
      $set_once: properties,
  ...
  async incrementUserProperties(distinctId: string, properties: Record<string, number>): Promise<{ success: boolean }> {
    let payload = [{
      $token: this.config.projectToken,
      $distinct_id: distinctId,
      $add: properties,
  ...
  async appendToUserListProperty(distinctId: string, properties: Record<string, unknown>): Promise<{ success: boolean }> {
    let payload = [{
      $token: this.config.projectToken,
      $distinct_id: distinctId,
      $append: properties,
  ...
  async removeFromUserListProperty(distinctId: string, properties: Record<string, unknown>): Promise<{ success: boolean }> {
    let payload = [{
      $token: this.config.projectToken,
      $distinct_id: distinctId,
      $remove: properties,
  ...
  async unionToUserListProperty(distinctId: string, properties: Record<string, unknown[]>): Promise<{ success: boolean }> {
    let payload = [{
      $token: this.config.projectToken,
      $distinct_id: distinctId,
      $union: properties,
  ...
  async deleteUserProperties(distinctId: string, propertyNames: string[]): Promise<{ success: boolean }> {
    let payload = [{
      $token: this.config.projectToken,
      $distinct_id: distinctId,
      $unset: propertyNames,
  ...
  // Group Profiles
  // =====================
  async setGroupProperties(groupKey: string, groupId: string, properties: Record<string, unknown>): Promise<{ success: boolean }> {
    let payload = [{
      $token: this.config.projectToken,
      $group_key: groupKey,
      $group_id: groupId,
  ...
  }): Promise<{ legendSize: number; series: string[]; values: Record<string, Record<string, number>> }> {
  ...
  // Query: Cohorts
  // =====================
  async listCohorts(): Promise<Array<{ cohortId: number; name: string; description: string; createdAt: string; count: number }>> {
  ...
  }): Promise<{ page: number; pageSize: number; sessionId: string; total: number; results: Array<{ distinctId: string; properties: Record<string, unknown> }> }> {
    let bodyParams: Record<string, string> = {};
    if (params.where) bodyParams['where'] = params.where;
    if (params.distinctId) bodyParams['distinct_id'] = params.distinctId;
    if (params.outputProperties) bodyParams['output_properties'] = JSON.stringify(params.outputProperties);
    if (params.sessionId) bodyParams['session_id'] = params.sessionId;
    if (params.page !== undefined) bodyParams['page'] = String(params.page);
    if (params.filterByCohort !== undefined) bodyParams['filter_by_cohort'] = JSON.stringify({ id: params.filterByCohort });

    let response = await this.queryAxios.post('/query/engage', new URLSearchParams(bodyParams).toString(), {
      params: { project_id: this.config.projectId },
  ...
  async listAnnotations(params?: {
    fromDate?: string;
    toDate?: string;
  }): Promise<Array<{ annotationId: number; date: string; description: string; tags: Array<{ tagId: number; name: string }> }>> {
  ...
  async createAnnotation(params: {
    ...
    let response = await this.appAxios.post(`/projects/${this.config.projectId}/annotations`, {
      date: params.date,
    ...
    return {
      ...
      description: response.data?.description ?? response.data?.results?.description ?? params.description,
    };
  }

  async deleteAnnotation(annotationId: number): Promise<void> {
    await this.appAxios.delete(`/projects/${this.config.projectId}/annotations/${annotationId}`);
  }
  ...
  async mergeIdentities(distinctId1: string, distinctId2: string): Promise<{ success: boolean }> {
    let payload = [{
      event: '$merge',
      properties: {
