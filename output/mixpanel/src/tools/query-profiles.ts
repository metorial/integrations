...
export let queryProfiles = SlateTool.create(
  spec,
  {
...
Returns paginated results with profile properties. Use sessionId and page for pagination.`,
  ...
  .input(z.object({
    where: z.string().optional().describe('Filter expression using Mixpanel segmentation syntax (e.g., \'properties["plan"] == "premium"\')'),
    distinctId: z.string().optional().describe('Specific user distinct ID to look up'),
    outputProperties: z.array(z.string()).optional().describe('Specific property names to return (returns all if omitted)'),
    cohortId: z.number().optional().describe('Filter profiles by cohort membership'),
    sessionId: z.string().optional().describe('Session ID from a previous query for pagination'),
