...
export let querySegmentation = SlateTool.create(
  spec,
  {
...
Useful for understanding how often an event occurs, broken down by property values.`,
  ...
  .input(z.object({
    ...
    unit: z.enum(['minute', 'hour', 'day', 'month']).optional().describe('Time bucket granularity (default: day)'),
    where: z.string().optional().describe('Filter expression for events'),
    limit: z.number().optional().describe('Max number of segmentation values to return (default: 60, max: 10000)'),
    type: z.enum(['general', 'unique', 'average']).optional().describe('Analysis type (default: general)'),
  }))
  .output(z.object({
    ...
    values: z.record(z.string(), z.record(z.string(), z.number())).describe('Nested map of segment values to date-count pairs'),
  }))
