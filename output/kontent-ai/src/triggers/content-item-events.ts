...
export let contentItemEvents = SlateTrigger.create(
  spec,
  {
    name: 'Content Item Events',
    key: 'content_item_events',
    description: 'Triggers when content items are created, changed, deleted, published, unpublished, or when their workflow step changes.',
  }
)
  .input(z.object({
    ...
    deliverySlot: z.string().optional().describe('Whether the event is for preview or published data'),
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      let inputs = notifications
        .filter((n) => n.message?.object_type === 'content_item_variant' || n.message?.object_type === 'content_item')
        .map((n) => ({
