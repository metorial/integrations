...
export let mergePdfs = SlateTool.create(
  spec,
  {
    name: 'Merge PDFs',
    key: 'merge_pdfs',
...
Also supports overlaying one PDF on another (e.g. adding a letterhead or watermark layer).`,
  ...
  .input(z.object({
    mode: z.enum(['merge', 'overlay']).default('merge').describe('"merge" combines PDFs sequentially, "overlay" places one PDF on top of another'),
    documents: z.array(z.string()).optional().describe('Array of base64-encoded PDF file contents to merge (for "merge" mode)'),
    outputFileName: z.string().describe('Desired output file name (e.g. "merged.pdf")'),
    baseFileContent: z.string().optional().describe('Base64-encoded base PDF content (for "overlay" mode)'),
    baseFileName: z.string().optional().describe('Base PDF file name (for "overlay" mode)'),
    layerFileContent: z.string().optional().describe('Base64-encoded overlay/layer PDF content (for "overlay" mode)'),
    layerFileName: z.string().optional().describe('Overlay/layer PDF file name (for "overlay" mode)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.mode === 'overlay') {
      if (!ctx.input.baseFileContent || !ctx.input.baseFileName || !ctx.input.layerFileContent || !ctx.input.layerFileName) {
    ...
    return {
      output: result,
      message: ctx.input.mode === 'overlay'
