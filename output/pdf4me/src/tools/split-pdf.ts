...
Returns an array of split document files.`,
  ...
  .input(z.object({
    ...
    splitMethod: z.enum(['splitAfterPage', 'recurringSplit', 'splitSequence', 'splitRanges', 'byText', 'byBarcode']).describe('Method to use for splitting the PDF'),
    splitPageNumber: z.number().optional().describe('Page number for "splitAfterPage" or interval for "recurringSplit"'),
    splitPages: z.array(z.number()).optional().describe('Array of page numbers for "splitSequence"'),
    splitRangeText: z.string().optional().describe('Page ranges for "splitRanges" (e.g. "1-3,4-6")'),
    searchText: z.string().optional().describe('Text to search for when splitting by text'),
    textSplitType: z.enum(['textIsOnFirstPage', 'textIsOnLastPage']).optional().describe('How to handle the page with the matching text'),
    barcodeFilter: z.string().optional().describe('Barcode filter for "byBarcode" split (e.g. "startsWith", "endsWith", "contains", "equals")'),
    barcodeSearchText: z.string().optional().describe('Barcode data to match for "byBarcode" split'),
    barcodeType: z.string().optional().describe('Barcode type for "byBarcode" split (e.g. "qrCode", "code128")'),
    barcodeSplitPage: z.enum(['barcodeIsOnFirstPage', 'barcodeIsOnLastPage']).optional().describe('How to handle the barcode page in "byBarcode" split'),
  ...
  .output(z.object({
    documents: z.array(z.object({
      ...
      barcodeText: z.string().optional().describe('Barcode text found (for barcode split)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.splitMethod === 'byBarcode') {
      if (!ctx.input.barcodeFilter || !ctx.input.barcodeSearchText || !ctx.input.barcodeType || !ctx.input.barcodeSplitPage) {
        throw new Error('barcodeFilter, barcodeSearchText, barcodeType, and barcodeSplitPage are required for barcode-based splitting');
      }
