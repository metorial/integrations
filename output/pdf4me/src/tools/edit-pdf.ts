...
export let findAndReplaceText = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    pages: z.string().default('all').describe('Pages to apply find/replace on (e.g. "1,2,3" or "all")'),
  }))
  .output(z.object({
...
export let flattenPdf = SlateTool.create(
  spec,
  {
    name: 'Flatten PDF',
    key: 'flatten_pdf',
...
export let repairPdf = SlateTool.create(
  spec,
  {
    name: 'Repair PDF',
    key: 'repair_pdf',
...
export let linearizePdf = SlateTool.create(
  spec,
  {
    name: 'Linearize PDF',
    key: 'linearize_pdf',
  ...
  .input(z.object({
    fileContent: z.string().describe('Base64-encoded PDF file content'),
    fileName: z.string().describe('PDF file name with extension'),
    compressType: z.enum(['Default', 'Web', 'Print']).default('Web').describe('Compression type to apply during linearization'),
  }))
  .output(z.object({
