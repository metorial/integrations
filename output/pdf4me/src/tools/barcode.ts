...
export let addBarcodeToPdf = SlateTool.create(
  spec,
  {
...
Configure position, size, opacity, and text display options.`,
  ...
  .input(z.object({
    ...
    barcodeType: z.string().describe('Barcode type (e.g. "qrCode", "code128", "dataMatrix", "ean13", "upcA", "pdf417", "aztec")'),
    pages: z.string().default('all').describe('Pages to add the barcode to (e.g. "1,2" or "all")'),
    alignX: z.enum(['Left', 'Center', 'Right']).default('Right').describe('Horizontal alignment'),
    alignY: z.enum(['Top', 'Middle', 'Bottom']).default('Top').describe('Vertical alignment'),
...
export let createBarcodeImage = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    text: z.string().describe('Data to encode in the barcode'),
    barcodeType: z.string().describe('Barcode type (e.g. "qrCode", "code128", "dataMatrix", "ean13", "upcA")'),
...
export let readBarcodeFromPdf = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    fileContent: z.string().describe('Base64-encoded PDF file content'),
    fileName: z.string().describe('PDF file name with extension'),
    barcodeTypes: z.array(z.string()).describe('Barcode types to scan for (e.g. ["qrCode", "code128"])'),
