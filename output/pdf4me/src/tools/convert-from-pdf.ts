...
export let convertFromPdf = SlateTool.create(
  spec,
  {
...
Choose the target format and quality type to control the conversion output.`,
  ...
  .input(z.object({
    ...
    targetFormat: z.enum(['word', 'excel', 'powerpoint']).describe('Target format for conversion'),
    qualityType: z.enum(['Draft', 'High']).default('High').describe('Conversion quality: "Draft" for faster processing, "High" for better quality'),
    language: z.string().optional().describe('Language of the source file for better conversion accuracy'),
    mergeAllSheets: z.boolean().optional().describe('For Excel conversion: merge all sheets into one'),
  }))
  .output(z.object({
