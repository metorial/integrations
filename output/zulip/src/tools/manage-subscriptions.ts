...
export let manageSubscriptions = SlateTool.create(
  ...
  .input(z.object({
    ...
    userIds: z.array(z.number()).optional().describe('User IDs to subscribe/unsubscribe. If omitted, applies to the authenticated user')
  }))
  .output(z.object({
    subscribed: z.record(z.string(), z.array(z.string())).optional().describe('Map of user emails to channels they were subscribed to'),
    alreadySubscribed: z.record(z.string(), z.array(z.string())).optional().describe('Map of user emails to channels they were already subscribed to'),
    removed: z.array(z.string()).optional().describe('Channels successfully unsubscribed from'),
    notRemoved: z.array(z.string()).optional().describe('Channels that could not be unsubscribed from')
  }))
