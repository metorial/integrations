...
export let createChannel = SlateTool.create(
  ...
  .input(z.object({
    ...
    isPrivate: z.boolean().optional().describe('Whether the channel should be private (invite-only). Defaults to false'),
    historyPublicToSubscribers: z.boolean().optional().describe('Whether history is available to new subscribers. Defaults to true for public channels')
  }))
  .output(z.object({
    created: z.boolean().describe('Whether the channel was newly created'),
    alreadySubscribed: z.array(z.string()).optional().describe('Channel names that already existed and were subscribed to')
  }))
