...
let messageSchema = z.object({
  ...
  displayRecipient: z.any().describe('Channel name (string) for channel messages, or array of recipients for direct messages'),
  ...
  })).optional().describe('Emoji reactions on the message')
});
...
export let getMessages = SlateTool.create(
  ...
  .input(z.object({
    anchor: z.union([z.string(), z.number()]).optional().describe('Message ID to anchor around, or "newest"/"oldest"/"first_unread". Defaults to "newest"'),
    numBefore: z.number().optional().describe('Number of messages before the anchor to retrieve. Defaults to 20'),
    numAfter: z.number().optional().describe('Number of messages after the anchor to retrieve. Defaults to 0'),
    narrow: z.array(z.object({
      operator: z.string().describe('Filter operator (e.g., "stream", "topic", "sender", "dm", "search", "is")'),
      operand: z.union([z.string(), z.number()]).describe('Value for the filter operator')
    })).optional().describe('Array of narrow filters to apply')
  }))
  .output(z.object({
    ...
    foundNewest: z.boolean().optional().describe('Whether the newest message matching the narrow was found'),
    foundOldest: z.boolean().optional().describe('Whether the oldest message matching the narrow was found')
  }))
