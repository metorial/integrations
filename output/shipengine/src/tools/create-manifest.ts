...
export let createManifest = SlateTool.create(
  ...
  .input(z.object({
    ...
    warehouseId: z.string().optional().describe('Warehouse ID (uses default if not specified)'),
    ...
    excludedLabelIds: z.array(z.string()).optional().describe('Label IDs to exclude from the manifest')
  }))
  .output(z.object({
