...
export let planConnectionEvents = SlateTrigger.create(
  spec,
  {
    name: 'Plan Connection Events',
    key: 'plan_connection_events',
    description: 'Triggers when a member\'s plan connection is created, updated, or canceled. Configure the webhook URL in the Memberstack dashboard under Dev Tools.',
  }
)
  .input(z.object({
    eventType: z.enum([
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      return {
        inputs: [
          {
            eventType: event as 'member.planConnection.created' | 'member.planConnection.updated' | 'member.planConnection.canceled',
