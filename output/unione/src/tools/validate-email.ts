...
Best used for real-time form validation, not bulk validation.`,
  ...
  .output(z.object({
    ...
    validity: z.string().describe('Validation result: valid, invalid, suspicious, or unknown'),
    validityScore: z.number().optional().describe('Validity score from 0 (invalid) to 100 (valid)'),
    ...
    cause: z.string().optional().describe('Cause of invalidity (e.g., disposable, spamtrap, mailbox_not_found, possible_typo)'),
    suggestedCorrection: z.string().optional().describe('Suggested correction for potential typos'),
  }))
