...
let templateBodySchema = z.object({
...
}).optional().describe('Email body content (at least one format required for creation)');
...
export let upsertTemplate = SlateTool.create(
  spec,
  {
...
To update an existing template, provide its **templateId**. To create a new template, omit the templateId.`,
    constraints: [
      'Maximum 10,000 templates per project.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    templateId: z.string().optional().describe('Template UUID (provide to update existing, omit to create new)'),
    name: z.string().optional().describe('Template name'),
    ...
    globalSubstitutions: z.record(z.string(), z.string()).optional().describe('Default substitution values'),
    globalMetadata: z.record(z.string(), z.string()).optional().describe('Default metadata key-value pairs'),
    ...
    inlineAttachments: z.array(attachmentSchema).optional().describe('Inline image attachments'),
  }))
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.globalSubstitutions) template['global_substitutions'] = ctx.input.globalSubstitutions;
    if (ctx.input.globalMetadata) template['global_metadata'] = ctx.input.globalMetadata;
    if (ctx.input.trackLinks !== undefined) template['track_links'] = ctx.input.trackLinks ? 1 : 0;
...
export let getTemplate = SlateTool.create(
  ...
  .output(z.object({
    ...
    }).optional().describe('Email body content'),
    globalSubstitutions: z.record(z.string(), z.string()).optional().describe('Default substitutions'),
...
export let listTemplates = SlateTool.create(
  ...
  .input(z.object({
    limit: z.number().optional().describe('Maximum number of templates to return (default: 50)'),
