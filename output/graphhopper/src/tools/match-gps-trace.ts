...
let matchedPathSchema = z.object({
  ...
  instructions: z.array(matchedInstructionSchema).optional().describe('Turn-by-turn instructions along matched route'),
});

export let matchGpsTrace = SlateTool.create(
  spec,
  {
...
Useful for cleaning noisy GPS data, reconstructing traveled routes, or attaching road attributes (street names, elevation, speed limits) to GPS traces.`,
  ...
  .input(
    z.object({
      ...
      locale: z.string().optional().describe('Language for turn instructions (e.g., "en", "de")'),
      ...
      gpsAccuracy: z.number().optional().describe('GPS accuracy in meters (helps improve matching quality)'),
    }),
  )
  .output(
    z.object({
