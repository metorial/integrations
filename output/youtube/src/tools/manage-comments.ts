...
export let manageComments = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['post', 'reply', 'update', 'delete', 'moderate']).describe('Action to perform'),
    ...
    moderationStatus: z.enum(['heldForReview', 'published', 'rejected']).optional().describe('Moderation status (for moderate action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'post') {
      ...
      if (!ctx.input.parentCommentId) throw new Error('parentCommentId is required for replies');
      ...
      if (!ctx.input.commentIds || ctx.input.commentIds.length === 0) throw new Error('commentIds is required for moderation');
      if (!ctx.input.moderationStatus) throw new Error('moderationStatus is required for moderation');
