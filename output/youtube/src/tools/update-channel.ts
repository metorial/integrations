...
export let updateChannel = SlateTool.create(
  spec,
  {
    name: 'Update Channel',
    key: 'update_channel',
  ...
  .input(z.object({
    ...
    unsubscribedTrailer: z.string().optional().describe('Video ID to use as the channel trailer for unsubscribed viewers'),
    country: z.string().optional().describe('Country the channel is associated with (ISO 3166-1 alpha-2)')
  }))
  .output(z.object({
    channelId: z.string(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.description !== undefined) channelSettings.description = ctx.input.description;
    if (ctx.input.keywords !== undefined) channelSettings.keywords = ctx.input.keywords;
    if (ctx.input.unsubscribedTrailer !== undefined) channelSettings.unsubscribedTrailer = ctx.input.unsubscribedTrailer;
