...
export let enrollMfaFactor = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    displayName: z.string().describe('User-facing name for this device (e.g., "Work Phone")'),
    expiresIn: z.number().optional().describe('OTP expiry in seconds (120-900, default 120)'),
    verified: z.boolean().optional().describe('Set true to pre-verify (SMS/Voice/Email only)'),
    customMessage: z.string().optional().describe('Custom SMS message template (use {{otp_code}} and {{otp_expiry}})'),
  }))
  .output(z.object({
    ...
    typeDisplayName: z.string().nullable().optional().describe('Display name of the factor type'),
    expiresAt: z.string().nullable().optional().describe('ISO8601 expiry timestamp for the OTP'),
  }))
