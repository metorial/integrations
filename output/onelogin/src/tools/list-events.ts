...
export let listEvents = SlateTool.create(
  ...
  .input(z.object({
    eventTypeId: z.number().optional().describe('Filter by event type ID (use Get Event Types to find IDs)'),
    ...
    clientId: z.string().optional().describe('Filter by API client ID that generated the event'),
  ...
  .output(z.object({
    events: z.array(z.object({
      ...
      errorDescription: z.string().nullable().optional().describe('Error description if applicable'),
      riskScore: z.number().nullable().optional().describe('Risk score (when Adaptive Auth is enabled)'),
    })).describe('List of events'),
    afterCursor: z.string().nullable().optional().describe('Cursor for fetching the next page'),
  }))
