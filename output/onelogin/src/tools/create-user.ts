...
export let createUser = SlateTool.create(
  ...
  .input(z.object({
    email: z.string().optional().describe('Email address (required if username not provided)'),
    username: z.string().optional().describe('Username, must be unique (required if email not provided)'),
    ...
    passwordConfirmation: z.string().optional().describe('Password confirmation (required if password is set)'),
    status: z.number().optional().describe('User status (0=Unactivated, 1=Active, 2=Suspended, 7=PendingActivation)'),
    ...
    customAttributes: z.record(z.string(), z.string()).optional().describe('Custom attribute key-value pairs'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.passwordConfirmation) body.password_confirmation = ctx.input.passwordConfirmation;
