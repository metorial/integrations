...
export let createCmsItem = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    modelId: z.string().describe('Unique identifier of the CMS model/table to create the item in'),
    fields: z.record(z.string(), z.unknown()).describe('Field values for the new item. Keys should match the model field identifiers'),
    publish: z.boolean().optional().describe('Whether to immediately publish the item after creation'),
  }))
  .output(z.object({
