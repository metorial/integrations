...
export let updateProject = SlateTool.create(
  ...
  .input(z.object({
    components: z.array(z.object({
      ...
      isPage: z.boolean().optional().describe('Whether this is a page (true) or a component (false)'),
      tplTree: z.record(z.string(), z.unknown()).optional().describe('Element tree structure defining the component content'),
    })).optional().describe('Components or pages to create or update'),
    tokens: z.array(z.object({
      name: z.string().describe('Name of the design token'),
      type: z.string().describe('Token type (e.g., color, spacing, font-family, font-size, line-height, opacity, box-shadow)'),
      value: z.string().describe('Token value (e.g., hex color, pixel value, font name)'),
    })).optional().describe('Design tokens to create or update'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!ctx.auth.projectSecretToken) {
      throw new Error('Project secret token is required for write operations (enterprise only)');
    }
