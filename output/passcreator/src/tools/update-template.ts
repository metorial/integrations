...
export let updateTemplate = SlateTool.create(
  ...
  .input(z.object({
    ...
    updates: z.record(z.string(), z.any()).describe('Fields to update on the template (e.g., name, description, colors, fields, barcode)'),
    publish: z.boolean().optional().default(false).describe('Whether to publish changes to all active passes after updating'),
  }))
  .output(z.object({
