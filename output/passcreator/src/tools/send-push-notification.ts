...
export let sendPushNotification = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    notificationText: z.string().describe('Push notification text. Supports {PropertyName} placeholders.'),
    passId: z.string().optional().describe('Single pass identifier to send notification to'),
    passIds: z.array(z.string()).optional().describe('Array of pass identifiers for bulk notifications (max 500)'),
    templateId: z.string().optional().describe('Template identifier for scheduling a template-wide notification'),
    scheduledDate: z.string().optional().describe('Date/time to schedule the notification in "Y-m-d H:i" format (requires templateId)'),
  }))
  .output(z.object({
    sent: z.boolean().describe('Whether the notification was sent/scheduled successfully'),
    notificationId: z.string().optional().describe('Notification ID for scheduled template notifications'),
  }))
