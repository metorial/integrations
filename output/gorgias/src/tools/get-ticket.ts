...
export let getTicket = SlateTool.create(
  ...
  .output(z.object({
    ...
    }).nullable().describe('Associated customer'),
    assigneeUser: z.object({
    ...
    }).nullable().describe('Assigned agent'),
    ...
    return {
      output: {
        ...
        assigneeUser: t.assignee_user ? {
          ...
          name: [t.assignee_user.firstname, t.assignee_user.lastname].filter(Boolean).join(' ') || null,
