...
export let ticketEvents = SlateTrigger.create(
  spec,
  {
    name: 'Ticket Events',
    key: 'ticket_events',
    description: 'Triggers when a ticket is created, updated, or when a new message is added to a ticket. Receives webhook payloads from Gorgias HTTP integrations.',
  }
)
  .input(z.object({
    eventType: z.enum(['ticket-created', 'ticket-updated', 'ticket-message-created']).describe('Type of ticket event'),
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      let eventType: 'ticket-created' | 'ticket-updated' | 'ticket-message-created' = 'ticket-updated';
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: {
          ...
          customerId: enriched.customer?.id || (enriched.customer_id ? Number(enriched.customer_id) : null),
          assigneeUserId: enriched.assignee_user?.id || (enriched.assignee_user_id ? Number(enriched.assignee_user_id) : null),
          assigneeUserName: enriched.assignee_user
            ? [enriched.assignee_user.firstname, enriched.assignee_user.lastname].filter(Boolean).join(' ') || null
            : enriched.assignee_user_firstname
              ? [enriched.assignee_user_firstname, enriched.assignee_user_lastname].filter(Boolean).join(' ') || null
              : null,
          tags: enriched.tags
