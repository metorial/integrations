...
export let handover = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    recipientId: z.string().describe('Page-Scoped User ID (PSID) of the user in the conversation'),
    action: z.enum(['pass', 'take', 'request']).describe('Handover action to perform'),
    targetAppId: z.string().optional().describe('App ID to pass thread control to (required for "pass" action)'),
    metadata: z.string().optional().describe('Custom metadata string to pass along with the handover action')
  }))
  .output(z.object({
