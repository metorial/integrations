...
let templateSchema = z.object({
  ...
  }).optional().describe('Active template version'),
});
...
Templates support Handlebars variable substitution with helpers like \`if\`, \`unless\`, \`each\`, and \`with\`.`,
  ...
  .input(z.object({
    ...
    description: z.string().optional().describe('Template description'),
    content: z.string().optional().describe('Initial template content (HTML or text). Creates the first version if provided.'),
    versionTag: z.string().optional().describe('Version tag for the initial version (default: "initial")'),
