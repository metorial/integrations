...
export let listDomains = SlateTool.create(
  ...
  .input(z.object({
    limit: z.number().optional().describe('Maximum number of domains to return (default 100, max 1000)'),
    skip: z.number().optional().describe('Number of domains to skip for pagination'),
    state: z.enum(['active', 'unverified', 'disabled']).optional().describe('Filter by domain state'),
  }))
  .output(z.object({
...
export let createDomain = SlateTool.create(
  ...
  .input(z.object({
    ...
    spamAction: z.enum(['disabled', 'block', 'tag']).optional().describe('What to do with spam (default: disabled)'),
