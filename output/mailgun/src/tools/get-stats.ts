...
Useful for monitoring email performance and deliverability.`,
  ...
  .input(z.object({
    ...
    events: z.array(z.enum(['accepted', 'delivered', 'failed', 'opened', 'clicked', 'unsubscribed', 'complained', 'stored']))
      .min(1).describe('Event types to get statistics for'),
    ...
    resolution: z.enum(['hour', 'day', 'month']).optional().describe('Time resolution for the stats (default: day)'),
    duration: z.string().optional().describe('Duration for relative time range (e.g. "7d", "1m")'),
  }))
  .output(z.object({
    stats: z.array(z.object({
      time: z.string().describe('Timestamp for this data point'),
      accepted: z.object({ incoming: z.number(), outgoing: z.number(), total: z.number() }).optional(),
      delivered: z.object({ smtp: z.number(), http: z.number(), total: z.number() }).optional(),
      failed: z.object({
