...
Supports scheduling, tracking options, tags for analytics, custom headers, and reply-to addresses.`,
  ...
  .input(z.object({
    ...
    templateVariables: z.record(z.string(), z.unknown()).optional().describe('Variables to pass into the template'),
    replyTo: z.string().optional().describe('Reply-To email address'),
    tags: z.array(z.string()).optional().describe('Tags for analytics (max 10)'),
    deliveryTime: z.string().optional().describe('Scheduled delivery time in RFC 2822 format'),
    testMode: z.boolean().optional().describe('When true, Mailgun accepts but does not deliver the message'),
    tracking: z.boolean().optional().describe('Enable or disable all tracking'),
    trackingClicks: z.enum(['yes', 'no', 'htmlonly']).optional().describe('Enable click tracking'),
    ...
    customHeaders: z.record(z.string(), z.string()).optional().describe('Custom MIME headers (without the h: prefix)'),
    customVariables: z.record(z.string(), z.string()).optional().describe('Custom JSON variables attached to the message (without the v: prefix)'),
    recipientVariables: z.record(z.string(), z.record(z.string(), z.unknown())).optional().describe('Per-recipient personalization variables keyed by email address'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let recipientCount = ctx.input.to.length + (ctx.input.cc?.length || 0) + (ctx.input.bcc?.length || 0);
