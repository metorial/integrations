...
export let getDomainTracking = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    unsubscribeHtmlFooter: z.string().optional().describe('HTML footer for unsubscribe links'),
    unsubscribeTextFooter: z.string().optional().describe('Text footer for unsubscribe links'),
  }))
...
export let updateDomainTracking = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    trackingType: z.enum(['open', 'click', 'unsubscribe']).describe('Type of tracking to update'),
    active: z.boolean().describe('Whether to enable or disable this tracking type'),
    htmlFooter: z.string().optional().describe('HTML footer for unsubscribe (only for unsubscribe tracking)'),
    textFooter: z.string().optional().describe('Text footer for unsubscribe (only for unsubscribe tracking)'),
  }))
  .output(z.object({
    success: z.boolean(),
  }))
