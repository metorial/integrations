...
export let validateEmail = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Email Validation is a separate paid feature in Mailgun.',
    ],
    tags: { readOnly: true },
  }
)
  ...
  .output(z.object({
    ...
    isDisposableAddress: z.boolean().describe('Whether the address is from a known disposable email provider'),
    isRoleAddress: z.boolean().describe('Whether the address is a role-based address (e.g. admin@, support@)'),
