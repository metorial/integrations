...
export let getWebhooks = SlateTool.create(
  spec,
  {
    name: 'Get Webhooks',
    key: 'get_webhooks',
...
export let createWebhook = SlateTool.create(
  ...
  .input(z.object({
    ...
    event: z.enum(['subscribed', 'modified', 'deactivated']).describe('Event type to listen for'),
    url: z.string().describe('URL to receive webhook POST requests'),
    blocked: z.boolean().optional().default(false).describe('Whether to create the webhook in a blocked state'),
  }))
...
export let updateWebhook = SlateTool.create(
  ...
  .input(z.object({
    ...
    event: z.enum(['subscribed', 'modified', 'deactivated']).optional().describe('New event type'),
