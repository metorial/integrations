...
export let manageReferences = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['add', 'remove']).describe('Whether to add or remove the cross-reference'),
    collectionName: z.string().describe('Source collection name'),
    objectId: z.string().describe('Source object UUID'),
    referenceProperty: z.string().describe('Name of the cross-reference property on the source object'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { action, collectionName, objectId, referenceProperty, targetCollectionName, targetObjectId, tenant } = ctx.input;

    let beacon = `weaviate://localhost/${targetCollectionName}/${targetObjectId}`;

    if (action === 'add') {
      await client.addReference(collectionName, objectId, referenceProperty, { beacon, tenant });
    } else {
      await client.deleteReference(collectionName, objectId, referenceProperty, { beacon, tenant });
    }
