...
export let manageBackup = SlateTool.create(
  spec,
  {
    name: 'Manage Backup',
    key: 'manage_backup',
...
- **status**: Check the status of a backup or restore operation.`,
  ...
  .input(z.object({
    action: z.enum(['create', 'restore', 'status', 'restore_status']).describe('Backup operation to perform'),
    backend: z.enum(['s3', 'gcs', 'azure', 'filesystem']).describe('Storage backend for the backup'),
    backupId: z.string().describe('Unique identifier for the backup'),
    includeCollections: z.array(z.string()).optional().describe('Collections to include (defaults to all)'),
    excludeCollections: z.array(z.string()).optional().describe('Collections to exclude from backup'),
  }))
  .output(z.object({
