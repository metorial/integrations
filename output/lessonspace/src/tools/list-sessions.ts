...
let sessionSchema = z.object({
  ...
  endTime: z.string().nullable().describe('ISO 8601 timestamp of when the session ended, or null if in progress.'),
  ...
  recordingAvailable: z.boolean().describe('Whether a recording is available for the session.'),
  spaceName: z.string().nullable().optional().describe('Name of the space this session belongs to.'),
...
export let listSessions = SlateTool.create(
  ...
  .input(z.object({
    search: z.string().optional().describe('Search term. If a UUID, filters by exact match. Otherwise, partial case-insensitive match on name.'),
    page: z.number().optional().describe('Page number for pagination.'),
    includeSingleUser: z.boolean().optional().describe('Include sessions with only one participant.'),
    ...
    startTimeAfter: z.string().optional().describe('Filter sessions starting after this ISO 8601 timestamp.'),
    startTimeBefore: z.string().optional().describe('Filter sessions starting before this ISO 8601 timestamp.'),
    endTimeAfter: z.string().optional().describe('Filter sessions ending after this ISO 8601 timestamp.'),
    endTimeBefore: z.string().optional().describe('Filter sessions ending before this ISO 8601 timestamp.'),
    spaceUuid: z.string().optional().describe('Filter by space UUID.'),
    launchId: z.string().optional().describe('Filter by the space ID used in the Launch endpoint.'),
    inProgressOnly: z.boolean().optional().describe('Only return sessions that are currently in progress.'),
    userExternalId: z.string().optional().describe('Filter by external user ID provided via the Launch endpoint.'),
    userName: z.string().optional().describe('Filter by user name (partial, case-insensitive).'),
  }))
  .output(z.object({
