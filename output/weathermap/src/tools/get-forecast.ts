...
let forecastEntrySchema = z.object({
  ...
  precipitationProbability: z.number().optional().describe('Probability of precipitation (0-1)'),
...
export let getForecast = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    count: z.number().min(1).max(40).optional().describe('Number of forecast timestamps to return (max 40)')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let data = await client.getForecast(ctx.input.latitude, ctx.input.longitude, ctx.input.count);
    ...
    let output = {
      ...
      sunrise: data.city?.sunrise ? new Date(data.city.sunrise * 1000).toISOString() : undefined,
