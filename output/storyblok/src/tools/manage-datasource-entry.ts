...
export let manageDatasourceEntry = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'delete', 'list']).describe('The entry action to perform'),
    datasourceId: z.string().optional().describe('Datasource ID (required for create, list)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of entries (for list action)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      if (!ctx.input.datasourceId) throw new Error('datasourceId is required to create an entry');
      if (!ctx.input.name) throw new Error('name is required to create an entry');
      if (ctx.input.value === undefined) throw new Error('value is required to create an entry');
