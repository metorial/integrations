...
let addressSchema = z.object({
  ...
  confidence: z.string().optional().describe('Result confidence: exact, interpolated, or fallback'),
...
export let geocodeTool = SlateTool.create(
  ...
  .input(z.object({
    mode: z.enum(['forward', 'reverse', 'ip']).describe('Geocoding mode: forward (address to coordinates), reverse (coordinates to address), or ip (IP to location)'),
    ...
    ip: z.string().optional().describe('IP address (optional for ip mode — uses requester IP if omitted)'),
    layers: z.string().optional().describe('Comma-separated result layers: place, address, postalCode, locality, county, state, country, coarse, fine'),
