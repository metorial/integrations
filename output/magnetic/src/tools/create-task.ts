...
export let createTask = SlateTool.create(
  ...
  .input(z.object({
    ...
    opportunityId: z.string().optional().describe('ID of the opportunity/job to link this task to'),
    ownerId: z.string().optional().describe('User ID of the task owner'),
    billable: z.boolean().optional().describe('Whether time tracked on this task is billable'),
    timeEstimateMinutes: z.number().optional().describe('Estimated time for the task in minutes'),
