...
export let addResult = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    fields: z.record(z.string(), z.string()).describe('Field data as key-value pairs where keys match form field identifiers'),
    processMessages: z.boolean().optional().describe('Whether to trigger configured form messages (e.g., confirmation emails) after adding the result'),
  }))
  .output(z.object({
    resultId: z.string().describe('The unique ID of the newly created result entry'),
    createdFields: z.record(z.string(), z.any()).optional().describe('System fields returned for the created entry'),
  }))
