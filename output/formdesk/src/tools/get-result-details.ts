...
export let getResultDetails = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    includeFiles: z.boolean().optional().describe('Whether to include file attachment details in the response'),
  }))
  .output(z.object({
    resultId: z.string().describe('The unique ID of the result entry'),
    fields: z.record(z.string(), z.any()).describe('All submitted field data as key-value pairs'),
    systemFields: z.record(z.string(), z.any()).optional().describe('System metadata fields (created date, status, etc.)'),
  }))
