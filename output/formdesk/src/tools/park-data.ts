...
export let parkData = SlateTool.create(
  spec,
  {
    name: 'Park Form Data',
    key: 'park_data',
  ...
  .input(z.object({
    ...
    fields: z.record(z.string(), z.string()).describe('Field data as key-value pairs to pre-fill the form with'),
    ...
    preventChange: z.boolean().optional().describe('Whether to prevent changes to the pre-filled fields'),
  }))
  .output(z.object({
    ticket: z.string().describe('The ticket GUID that can be used to open the pre-filled form'),
