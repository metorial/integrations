...
export let uploadFile = SlateTool.create(
  ...
  .input(z.object({
    ...
    mode: z.enum(['add', 'overwrite']).optional().describe('Upload mode: "add" creates new file, "overwrite" replaces existing'),
    autorename: z.boolean().optional().describe('If true, rename the file if a conflict exists'),
