...
let paperSummarySchema = z.object({
...
let authorDetailSchema = z.object({
...
export let getAuthor = SlateTool.create(
  spec,
  {
...
For single author lookup, provide **authorId**. For batch lookup of up to 1,000 authors, provide **authorIds**.`,
    constraints: [
      'Batch lookup supports a maximum of 1,000 author IDs.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    authorIds: z.array(z.string()).optional().describe('Array of author IDs for batch lookup (max 1000)'),
    fields: z.string().optional().describe('Comma-separated list of fields to return (e.g. "name,affiliations,paperCount,citationCount,hIndex,homepage,papers,papers.title,papers.year"). Defaults to "authorId,name".'),
  }))
  .output(z.object({
    authors: z.array(authorDetailSchema.nullable()).describe('Author details (null entries for authors not found in batch mode)'),
  }))
