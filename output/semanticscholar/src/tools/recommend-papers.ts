...
export let recommendPapers = SlateTool.create(
  spec,
  {
...
- **Multi-paper**: Provide a list of positive example paper IDs (and optionally negative examples) to get recommendations based on the combined signal.`,
  ...
  .input(z.object({
    positivePaperIds: z.array(z.string()).describe('Paper IDs to use as positive examples for recommendations'),
    negativePaperIds: z.array(z.string()).optional().describe('Paper IDs to use as negative examples (results should not be related to these)'),
    pool: z.enum(['recent', 'all-cs']).optional().describe('Paper pool for single-paper recommendations: "recent" (default) or "all-cs"'),
    fields: z.string().optional().describe('Comma-separated list of paper fields to return (e.g. "title,authors,year,citationCount,abstract,tldr")'),
    limit: z.number().optional().describe('Maximum number of recommendations (default: 100, max: 500)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let isSinglePaper = ctx.input.positivePaperIds.length === 1 && (!ctx.input.negativePaperIds || ctx.input.negativePaperIds.length === 0);
