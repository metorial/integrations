...
export let manageSuppressionList = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list', 'add']).describe('Operation to perform: "list" to view all suppressed addresses, "add" to suppress a new address'),
    emailAddress: z.string().optional().describe('Email address to add to the suppression list (required for "add" action)')
  }))
  .output(z.object({
    suppressedEmails: z.array(z.record(z.string(), z.any())).optional().describe('Array of suppressed email records (for "list" action)'),
