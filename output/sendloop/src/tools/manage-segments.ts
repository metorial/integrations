...
export let manageSegments = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    segmentName: z.string().optional().describe('Name for the new segment (required for create)'),
    rules: z.string().optional().describe('Segment rules definition (JSON string for create)')
  }))
  .output(z.object({
    segments: z.array(z.record(z.string(), z.any())).optional().describe('Array of segment objects (for list/get)'),
