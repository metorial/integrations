...
let alertSchema = z.object({
  ...
  status: z.string().describe('Alert status (OPEN, TRACKING, INFORMATIONAL, CLOSED, CANCELLED)'),
  ...
  acknowledgedUntil: z.string().optional().describe('ISO 8601 timestamp until alert is acknowledged'),
  acknowledgementComment: z.string().optional().describe('Comment from the user who acknowledged the alert'),
});
...
export let manageAlertsTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['OPEN', 'TRACKING', 'INFORMATIONAL', 'CLOSED', 'CANCELLED']).optional().describe('Filter alerts by status (for list)'),
    acknowledgedUntil: z.string().optional().describe('ISO 8601 timestamp until which to acknowledge the alert'),
    acknowledgementComment: z.string().optional().describe('Comment for the acknowledgement'),
