...
let clusterSummarySchema = z.object({
  ...
  stateName: z.string().describe('Current state (IDLE, CREATING, UPDATING, DELETING, REPAIRING, etc.)'),
  ...
  connectionString: z.string().optional().describe('Standard connection string for the cluster'),
...
export let listClustersTool = SlateTool.create(
  spec,
  {
    name: 'List Clusters',
    key: 'list_clusters',
  ...
  .input(z.object({
    projectId: z.string().optional().describe('Project ID. Falls back to configured projectId if omitted.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let clusters = (result.results || []).map((c: any) => {
      ...
      return {
        ...
        instanceSize: regionConfig?.electableSpecs?.instanceSize || c.providerSettings?.instanceSizeName,
