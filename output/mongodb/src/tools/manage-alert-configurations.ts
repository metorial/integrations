...
let notificationSchema = z.object({
  typeName: z.string().describe('Notification type (e.g., WEBHOOK, EMAIL, SLACK, PAGER_DUTY, OPS_GENIE, DATADOG)'),
...
let matcherSchema = z.object({
  fieldName: z.string().describe('Field to match (e.g., CLUSTER_NAME, REPLICA_SET_NAME, HOSTNAME_AND_PORT)'),
...
let metricThresholdSchema = z.object({
  metricName: z.string().describe('Name of the metric (e.g., CONNECTIONS, OPCOUNTER_CMD, DISK_PARTITION_SPACE_FREE)'),
  operator: z.string().describe('Comparison operator (GREATER_THAN, LESS_THAN)'),
  threshold: z.number().describe('Threshold value'),
  units: z.string().optional().describe('Units for the threshold (e.g., RAW, BITS, BYTES, GIGABYTES)'),
...
let alertConfigSchema = z.object({
  ...
  eventTypeName: z.string().describe('Event type (e.g., OUTSIDE_METRIC_THRESHOLD, HOST_DOWN, REPLICATION_OPLOG_WINDOW_RUNNING_OUT)'),
...
export let manageAlertConfigurationsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    alertConfigId: z.string().optional().describe('Alert configuration ID (for get, update, delete)'),
  ...
  .output(z.object({
    alertConfigs: z.array(alertConfigSchema).optional().describe('List of alert configurations'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'update') {
      ...
      let c = await client.updateAlertConfiguration(projectId, ctx.input.alertConfigId, payload);
      return {
        output: { alertConfig: mapConfig(c) },
