...
let eventSchema = z.object({
  ...
  isGlobalAdmin: z.boolean().optional().describe('Whether the event was triggered by a global admin'),
});
...
export let listEventsTool = SlateTool.create(
  ...
  .input(z.object({
    scope: z.enum(['project', 'organization']).describe('Whether to list project or organization events'),
    projectId: z.string().optional().describe('Project ID (for project scope). Falls back to configured projectId.'),
    organizationId: z.string().optional().describe('Organization ID (for organization scope). Falls back to configured organizationId.'),
