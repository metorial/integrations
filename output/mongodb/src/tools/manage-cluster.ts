...
let replicationSpecSchema = z.object({
  ...
  regionConfigs: z.array(z.object({
    ...
    electableSpecs: z.object({
      ...
      ebsVolumeType: z.string().optional().describe('EBS volume type (STANDARD, PROVISIONED)'),
    }).optional(),
...
export let manageClusterTool = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['get', 'create', 'update', 'delete', 'pause', 'resume']).describe('Action to perform on the cluster'),
    ...
    clusterType: z.enum(['REPLICASET', 'SHARDED', 'GEOSHARDED']).optional().describe('Cluster type (required for create)'),
    mongoDBMajorVersion: z.string().optional().describe('MongoDB major version (e.g., "7.0", "8.0")'),
    ...
    terminationProtectionEnabled: z.boolean().optional().describe('Prevent accidental cluster deletion'),
    replicationSpecs: z.array(replicationSpecSchema).optional().describe('Replication and region configuration'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      if (ctx.input.mongoDBMajorVersion) payload.mongoDBMajorVersion = ctx.input.mongoDBMajorVersion;
      if (ctx.input.diskSizeGB) payload.diskSizeGB = ctx.input.diskSizeGB;
      if (ctx.input.backupEnabled !== undefined) payload.backupEnabled = ctx.input.backupEnabled;
      if (ctx.input.terminationProtectionEnabled !== undefined) payload.terminationProtectionEnabled = ctx.input.terminationProtectionEnabled;
    ...
    if (ctx.input.action === 'pause') {
      let cluster = await client.updateCluster(projectId, ctx.input.clusterName, { paused: true });
      return {
        output: {
          clusterName: cluster.name,
          stateName: cluster.stateName,
    ...
    if (ctx.input.action === 'resume') {
      let cluster = await client.updateCluster(projectId, ctx.input.clusterName, { paused: false });
      return {
        output: {
          clusterName: cluster.name,
          stateName: cluster.stateName,
