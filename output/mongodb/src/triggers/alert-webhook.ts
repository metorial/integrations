...
export let alertWebhookTrigger = SlateTrigger.create(
  ...
  .input(z.object({
    ...
    hostnameAndPort: z.string().optional().describe('Hostname and port of the affected host'),
    ...
    updated: z.string().optional().describe('ISO 8601 timestamp when the alert was last updated'),
    resolved: z.string().optional().describe('ISO 8601 timestamp when the alert was resolved'),
  ...
  .webhook({
    autoRegisterWebhook: async (ctx) => {
      ...
      if (!projectId) throw new Error('projectId is required in configuration to auto-register webhooks');
    ...
    autoUnregisterWebhook: async (ctx) => {
      let details = ctx.input.registrationDetails as { projectId: string; integrationType: string };
      if (!details?.projectId) return;

      let client = new AtlasClient(ctx.auth);
      await client.deleteThirdPartyIntegration(details.projectId, details.integrationType || 'WEBHOOK');
    },
