...
export let screenshotCompleted = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggered when an asynchronous screenshot request completes. Receives the screenshot result payload via webhook.',
  }
)
  .input(z.object({
    ...
    screenshotId: z.string().optional().describe('Unique identifier for this screenshot request.'),
    rawPayload: z.any().describe('Full raw webhook payload.'),
  }))
  .output(z.object({
    url: z.string().optional().describe('Target URL that was screenshotted.'),
    screenshotUrl: z.string().optional().describe('URL where the screenshot image is stored.'),
    screenshotId: z.string().optional().describe('Unique identifier for this screenshot request.'),
  }))
  .webhook({
