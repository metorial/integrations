...
export let startCrawl = SlateTool.create(
  spec,
  {
    name: 'Start Crawl',
    key: 'start_crawl',
  ...
  .input(z.object({
    ...
    pageLimit: z.number().optional().describe('Maximum number of pages to crawl. 0 for unlimited.'),
    maxDepth: z.number().optional().describe('Maximum link depth from the starting URL.'),
    maxDuration: z.number().optional().describe('Maximum crawl duration in seconds (15-10800).'),
    maxApiCredit: z.number().optional().describe('Maximum API credits to spend. 0 for no limit.'),
    excludePaths: z.array(z.string()).optional().describe('URL path patterns to exclude from crawling.'),
    includeOnlyPaths: z.array(z.string()).optional().describe('Only crawl URLs matching these path patterns.'),
    ignoreBasePathRestriction: z.boolean().optional().describe('Allow crawling any path on the same domain.'),
    followExternalLinks: z.boolean().optional().describe('Allow following links to external domains.'),
    allowedExternalDomains: z.array(z.string()).optional().describe('Whitelist of external domains to follow (supports wildcards).'),
    followInternalSubdomains: z.boolean().optional().describe('Allow following links to subdomains.'),
    contentFormats: z.array(z.string()).optional().describe('Output formats: html, clean_html, markdown, text, json, extracted_data, page_metadata.'),
    extractionRules: z.any().optional().describe('Extraction rules for structured data extraction from crawled pages.'),
    renderingDelay: z.number().optional().describe('Wait time in ms after page load (0-25000).'),
    maxConcurrency: z.number().optional().describe('Maximum concurrent scrape requests.'),
    delay: z.string().optional().describe('Delay between requests in milliseconds (0-15000).'),
