...
let meetingEventSummarySchema = z.object({
  ...
  meetingProvider: z.string().nullable().optional().describe('Meeting provider (e.g., google_meeting, teams_meeting)'),
  internal: z.boolean().optional().describe('Whether the meeting is internal'),
  direction: z.string().nullable().optional().describe('Meeting direction: inbound or outbound'),
  ...
  origin: z.string().nullable().optional().describe('How the event was created: api, calendar, or manual'),
...
export let listMeetingEvents = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    order: z.string().optional().describe('Sort order: "created_at desc/asc", "start_time desc/asc", "end_time desc/asc"'),
    createdBy: z.enum(['calendar', 'manual', 'api']).optional().describe('Filter by creation method'),
    dateFilter: z.enum(['start_time', 'end_time']).optional().describe('Which date field to filter on (default: start_time)'),
