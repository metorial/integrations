...
export let listBranches = SlateTool.create(
  ...
  .input(z.object({
    ...
    filter: z.string().optional().describe('Filter refs by prefix (e.g., "heads/main" for branches starting with "main", "heads/" for all branches, "tags/" for all tags)'),
    includeStats: z.boolean().optional().describe('Include ahead/behind statistics for each branch'),
    baseVersion: z.string().optional().describe('Base branch for statistics comparison (e.g., "main")'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includeStats) {
      let stats = await client.getBranchStats(ctx.input.repositoryId, undefined, ctx.input.baseVersion);
      for (let stat of stats) {
        statsMap.set(stat.name, { aheadCount: stat.aheadCount, behindCount: stat.behindCount });
      }
    }
