...
export let commentOnPullRequest = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    threadId: z.number().optional().describe('Existing thread ID to reply to. Omit to create a new thread.'),
    parentCommentId: z.number().optional().describe('Parent comment ID for nested replies within a thread'),
    filePath: z.string().optional().describe('File path for inline code comment (e.g., "/src/app.ts")'),
    ...
    threadStatus: z.enum(['active', 'fixed', 'wontFix', 'closed', 'byDesign', 'pending']).optional().describe('Set or update the thread status'),
  }))
  .output(z.object({
      ...
      let threadContext = ctx.input.filePath
        ? {
            ...
            rightFileEnd: ctx.input.lineEnd
              ...
              : (ctx.input.lineStart ? { line: ctx.input.lineStart, offset: 1 } : undefined),
          }
        : undefined;
