...
export let getPullRequest = SlateTool.create(
  ...
  .output(z.object({
    ...
    reviewers: z.array(z.object({
      displayName: z.string(),
      reviewerId: z.string(),
      vote: z.number().describe('Vote: 10=approved, 5=approved with suggestions, 0=no vote, -5=waiting, -10=rejected'),
    ...
    }).optional().describe('Auto-complete settings'),
    labels: z.array(z.object({
      labelId: z.string(),
      name: z.string(),
      active: z.boolean(),
    })).optional().describe('Labels attached to the PR'),
    ...
    })).optional().describe('Comment threads (if includeThreads is true)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includeThreads) {
      let rawThreads = await client.listCommentThreads(ctx.input.repositoryId, ctx.input.pullRequestId);
