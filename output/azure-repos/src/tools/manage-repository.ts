...
export let createRepository = SlateTool.create(
  ...
  .input(z.object({
    ...
    parentRepositoryId: z.string().optional().describe('ID of the parent repository to fork from'),
    parentProjectId: z.string().optional().describe('Project ID of the parent repository (for cross-project forks)'),
    sourceRef: z.string().optional().describe('Source branch to fork from (e.g., "refs/heads/main")'),
  }))
...
export let updateRepository = SlateTool.create(
  ...
  .input(z.object({
    ...
    defaultBranch: z.string().optional().describe('New default branch (e.g., "refs/heads/main")'),
    isDisabled: z.boolean().optional().describe('Set to true to disable or false to enable the repository'),
  }))
  .output(repositoryOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new Client({
...
export let deleteRepository = SlateTool.create(
  spec,
  {
