...
let filterCriteriaSchema = z.object({
  ...
  ]).optional().describe('Condition type for the filter'),
  conditionValues: z.array(z.string()).optional().describe('Values for the condition'),
});

export let createFilterView = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    startRowIndex: z.number().optional().describe('Start row index of the range (0-based, inclusive)'),
    ...
    criteria: z.array(filterCriteriaSchema).optional().describe('Filter criteria to apply per column'),
