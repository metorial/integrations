...
let deploymentSchema = z.object({
  ...
  versions: z.array(deploymentVersionSchema).optional().describe('Versions in this deployment with traffic split'),
});
...
export let createDeployment = SlateTool.create(
  ...
  .input(z.object({
    ...
    versions: z.array(z.object({
      ...
      percentage: z.number().min(0).max(100).describe('Percentage of traffic for this version'),
    })).min(1).describe('Versions to deploy with traffic percentages (must sum to 100)'),
