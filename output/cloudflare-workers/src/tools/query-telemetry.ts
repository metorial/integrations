...
export let queryTelemetry = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    statistics: z.record(z.string(), z.any()).optional().describe('Query statistics (bytes read, elapsed time, rows read)'),
...
export let listTelemetryKeys = SlateTool.create(
  spec,
  {
