...
export let manageMaintenanceMode = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Only one global maintenance mode can be active at a time.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    ...
    routingKeys: z.array(z.string()).optional().describe('Routing keys to apply maintenance mode to (empty for global). Required for start.'),
    purpose: z.string().optional().describe('Description of the maintenance purpose (required for start)'),
    maintenanceModeId: z.string().optional().describe('ID of the maintenance mode instance to end (required for end)'),
  }))
  .output(z.object({
    activeInstances: z.array(z.any()).optional().describe('Currently active maintenance mode instances'),
