...
export let manageIncident = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['acknowledge', 'resolve', 'acknowledge_user', 'resolve_user', 'reroute']).describe('Action to perform on the incident(s)'),
    userName: z.string().describe('VictorOps username performing the action'),
    incidentNumbers: z.array(z.string()).optional().describe('List of incident numbers to act on (for acknowledge, resolve, reroute)'),
    message: z.string().optional().describe('Optional message to attach to the action'),
    rerouteTargets: z.array(z.object({
    ...
    })).optional().describe('Targets to reroute incidents to (required for reroute action)'),
  }))
  .output(z.object({
