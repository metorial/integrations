...
export let manageEscalationPolicy = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'delete', 'list_by_team']).describe('Action to perform'),
    policySlug: z.string().optional().describe('Policy slug (required for get and delete)'),
    teamSlug: z.string().optional().describe('Team slug (required for list_by_team and create)'),
    name: z.string().optional().describe('Policy name (required for create)'),
    steps: z.array(z.object({
      timeout: z.number().describe('Seconds before escalating to next step (0 = immediate)'),
      entries: z.array(z.object({
        type: z.string().describe('Target type (e.g., "rotationGroup", "user", "policy")'),
        slug: z.string().describe('Target slug identifier'),
      })).describe('Targets for this escalation step'),
    })).optional().describe('Escalation steps (required for create)'),
  }))
  .output(z.object({
