...
export let getServiceTool = SlateTool.create(
  spec,
  {
    name: 'Get Service',
    key: 'get_service',
  ...
  .input(z.object({
    ...
    environmentId: z.string().optional().describe('Environment ID to get environment-specific configuration')
  }))
  .output(z.object({
    ...
    }).nullable().describe('Environment-specific configuration (only when environmentId is provided)')
  }))
...
export let createServiceTool = SlateTool.create(
  spec,
  {
    name: 'Create Service',
    key: 'create_service',
  ...
  .input(z.object({
    ...
    repo: z.string().optional().describe('GitHub repository to connect (format: "owner/repo")'),
...
export let updateServiceTool = SlateTool.create(
  spec,
  {
    name: 'Update Service',
    key: 'update_service',
  ...
  .input(z.object({
    ...
    environmentId: z.string().optional().describe('Environment ID for environment-specific settings'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.environmentId) {
      ...
      if (ctx.input.startCommand !== undefined) instanceUpdate.startCommand = ctx.input.startCommand;
      if (ctx.input.buildCommand !== undefined) instanceUpdate.buildCommand = ctx.input.buildCommand;
      if (ctx.input.rootDirectory !== undefined) instanceUpdate.rootDirectory = ctx.input.rootDirectory;
      if (ctx.input.healthcheckPath !== undefined) instanceUpdate.healthcheckPath = ctx.input.healthcheckPath;
      if (ctx.input.numReplicas !== undefined) instanceUpdate.numReplicas = ctx.input.numReplicas;
      if (ctx.input.region !== undefined) instanceUpdate.region = ctx.input.region;
      if (ctx.input.cronSchedule !== undefined) instanceUpdate.cronSchedule = ctx.input.cronSchedule;

      if (Object.keys(instanceUpdate).length > 0) {
        await client.updateServiceInstance(ctx.input.serviceId, ctx.input.environmentId, instanceUpdate);
      }
    }
...
export let deleteServiceTool = SlateTool.create(
  spec,
  {
    name: 'Delete Service',
    key: 'delete_service',
