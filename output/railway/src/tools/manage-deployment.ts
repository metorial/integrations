...
export let listDeploymentsTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    limit: z.number().optional().default(10).describe('Maximum number of deployments to return (default: 10)')
  }))
  .output(z.object({
    deployments: z.array(z.object({
      deploymentId: z.string().describe('Deployment ID'),
      status: z.string().describe('Deployment status (e.g., SUCCESS, FAILED, BUILDING, DEPLOYING)'),
...
export let getDeploymentTool = SlateTool.create(
  spec,
  {
    name: 'Get Deployment',
    key: 'get_deployment',
...
export let triggerDeploymentTool = SlateTool.create(
  spec,
  {
...
export let rollbackDeploymentTool = SlateTool.create(
  spec,
  {
...
export let getDeploymentLogsTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    logType: z.enum(['build', 'deploy']).default('deploy').describe('Type of logs: "build" for build output, "deploy" for runtime logs'),
    limit: z.number().optional().default(100).describe('Maximum number of log lines to return')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
    let logs = ctx.input.logType === 'build'
