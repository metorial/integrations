...
export let createProduct = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['active', 'draft', 'archived']).optional().describe('Product status (defaults to active)'),
    variants: z.array(z.object({
      title: z.string().optional().describe('Variant title'),
      price: z.string().optional().describe('Variant price'),
      compareAtPrice: z.string().optional().describe('Compare-at price for sale display'),
      ...
      requiresShipping: z.boolean().optional().describe('Whether variant requires shipping'),
    ...
    })).optional().describe('Product images by URL'),
    options: z.array(z.object({
      name: z.string().describe('Option name (e.g., "Size", "Color")')
    })).optional().describe('Product option definitions')
  }))
  .output(z.object({
    productId: z.string(),
    title: z.string(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.variants) {
      productData.variants = ctx.input.variants.map(v => {
        ...
        if (v.inventoryQuantity !== undefined) variant.inventory_quantity = v.inventoryQuantity;
