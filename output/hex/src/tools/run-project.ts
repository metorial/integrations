...
export let runProject = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Rate limited to 20 requests per minute and 60 per hour per project.',
    ],
  }
)
  .input(z.object({
    projectId: z.string().describe('UUID of the project to run'),
    inputParams: z.record(z.string(), z.any()).optional().describe('Custom input parameters to pass to the project run'),
    viewId: z.string().optional().describe('Saved view ID to use for the run'),
    dryRun: z.boolean().optional().describe('If true, validate the run configuration without executing'),
    updateCache: z.boolean().optional().describe('If true, update the cache with the results of this run'),
    updatePublishedResults: z.boolean().optional().describe('If true, update the published app with the run output'),
    useCachedSqlResults: z.boolean().optional().describe('If true, use cached SQL results where available'),
    notifications: z.array(z.object({
      type: z.string().describe('Notification type (e.g. "slack_channel", "hex_user", "hex_group")'),
      target: z.any().describe('Notification target details'),
    })).optional().describe('Notifications to send on run completion'),
  }))
  .output(z.object({
    projectId: z.string(),
    runId: z.string(),
