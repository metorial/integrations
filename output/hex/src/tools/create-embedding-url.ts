...
export let createEmbeddingUrl = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    hexUserAttributes: z.record(z.string(), z.string()).optional().describe('Custom user attributes for the embedded session'),
    scope: z.array(z.string()).optional().describe('Export scopes to enable (e.g. "pdf", "csv")'),
    inputParameters: z.record(z.string(), z.any()).optional().describe('Default input parameter values for the embedded app'),
    expiresIn: z.number().optional().describe('URL expiration time in seconds'),
    theme: z.string().optional().describe('Theme for the embedded app (e.g. "light", "dark")'),
    showPadding: z.boolean().optional().describe('Whether to show padding around the embedded app'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let displayOptions: { theme?: string; showPadding?: boolean; showHeader?: boolean } | undefined;
    if (ctx.input.theme !== undefined || ctx.input.showPadding !== undefined || ctx.input.showHeader !== undefined) {
      displayOptions = {
