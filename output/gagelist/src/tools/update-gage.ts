...
export let updateGage = SlateTool.create(
  spec,
  {
    name: 'Update Gage',
    key: 'update_gage',
...
Provide the gage ID and any fields to update. Only the specified fields will be changed.`,
    instructions: [
      'At least one field besides gageId must be provided to update.',
    ],
    tags: {
      destructive: true,
      readOnly: false,
  ...
  .input(z.object({
    ...
    ]).optional().describe('Updated status of the gage'),
    ...
    lastCalibrationDate: z.string().optional().describe('Updated last calibration date in ISO-8601 format'),
