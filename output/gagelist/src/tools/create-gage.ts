...
export let createGage = SlateTool.create(
  spec,
  {
    name: 'Create Gage',
    key: 'create_gage',
...
Use this to register new equipment for calibration tracking. Provide the gage status, type, manufacturer, and other identifying details.`,
  ...
  .input(z.object({
    status: z.enum([
      'Active', 'Employee Owned', 'Inactive', 'Lost',
      'Out of Calibration', 'Out for Repair', 'Reference Only', 'Retired',
    ...
    condition: z.enum(['New', 'Repaired', 'Used']).optional().describe('Condition when acquired'),
    lastCalibrationDate: z.string().optional().describe('Last calibration date in ISO-8601 format, e.g. "2024-03-15T00:00:00Z"'),
    calibrationDueDate: z.string().optional().describe('Next calibration due date in ISO-8601 format'),
    calibrationInstructions: z.string().optional().describe('Calibration instructions for the gage'),
    ...
    masterStandard: z.string().optional().describe('Whether this is a master standard, e.g. "Yes" or "No"'),
