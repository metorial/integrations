...
export let listGroups = SlateTool.create(
  ...
  .input(z.object({
    organizationId: z.string().optional().describe('Filter groups by organization ID. If omitted, returns all groups.'),
    searchText: z.string().optional().describe('Search groups by name (organization filter required)'),
    page: z.number().optional().describe('Page number (default: 1, organization filter required)'),
    pageSize: z.number().optional().describe('Number of results per page (default: 100, organization filter required)'),
  }))
  .output(z.object({
...
export let getGroupMembers = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    groupIds: z.string().optional().describe('Comma-separated group IDs to retrieve members across multiple groups'),
    organizationId: z.string().optional().describe('Organization ID (for org-scoped group membership lookup)'),
    groupId: z.number().optional().describe('Group ID (for org-scoped group membership lookup)'),
    page: z.number().optional().describe('Page number (default: 1, org-scoped only)'),
    pageSize: z.number().optional().describe('Number of results per page (default: 100, org-scoped only)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.organizationId && ctx.input.groupId) {
      let result = await client.listGroupMemberships(ctx.input.organizationId, ctx.input.groupId, {
        page: ctx.input.page,
        pageSize: ctx.input.pageSize,
      });
...
export let manageGroupMembership = SlateTool.create(
  ...
  .output(z.object({
    ...
    membership: z.record(z.string(), z.unknown()).optional().describe('Membership record (for add action)'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
