...
export let listFamilies = SlateTool.create(
  spec,
  {
    name: 'List Families',
    key: 'list_families',
  ...
  .input(z.object({
    ...
    includeRoles: z.boolean().optional().describe('Also return the list of available family roles'),
  ...
  .output(z.object({
    ...
    roles: z.array(z.record(z.string(), z.unknown())).optional().describe('Available family roles (if requested)'),
...
export let getFamily = SlateTool.create(
  spec,
  {
    name: 'Get Family',
    key: 'get_family',
...
export let createFamily = SlateTool.create(
  spec,
  {
    name: 'Create Family',
    key: 'create_family',
  ...
  .input(z.object({
    members: z.array(z.object({
      ...
      role: z.string().optional().describe('Family role: Primary, Spouse, Child, Grandparent, or Other'),
    })).min(2).describe('Family members (minimum 2 required)'),
  }))
  .output(z.object({
...
export let deleteFamily = SlateTool.create(
  spec,
  {
    name: 'Delete Family',
    key: 'delete_family',
...
export let manageFamilyMembers = SlateTool.create(
  spec,
  {
...
- **replace**: Replace all family members (minimum 2 required)`,
  ...
  .input(z.object({
    ...
    action: z.enum(['add', 'remove', 'updateRole', 'replace']).describe('Operation to perform'),
    members: z.array(z.object({
      personId: z.number().describe('Person ID'),
      role: z.string().optional().describe('Family role: Primary, Spouse, Child, Grandparent, or Other'),
    })).optional().describe('Members to add or set (for add/replace actions)'),
    personId: z.number().optional().describe('Person ID to remove or update role (for remove/updateRole actions)'),
    role: z.string().optional().describe('New role (for updateRole action)'),
  }))
  .output(z.object({
    success: z.boolean().describe('Whether the operation was successful'),
    family: z.record(z.string(), z.unknown()).optional().describe('Updated family record (if available)'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
    ...
    switch (ctx.input.action) {
      ...
      case 'updateRole': {
        ...
        await client.updateFamilyMemberRole(ctx.input.familyId, ctx.input.personId, ctx.input.role);
        break;
      }
      case 'replace': {
