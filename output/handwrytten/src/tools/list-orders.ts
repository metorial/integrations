...
let orderSchema = z.object({
  ...
  status: z.string().optional().describe('Order status (processing, written, complete, problem, cancelled)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let orders = rawOrders.map((o: any) => ({
      ...
      recipientName: o.address_to
        ? [o.address_to.first_name, o.address_to.last_name].filter(Boolean).join(' ') || undefined
        : undefined,
      recipientCity: o.address_to?.city ?? undefined,
      recipientState: o.address_to?.state ?? undefined,
      senderName: o.address_from
        ? [o.address_from.first_name, o.address_from.last_name].filter(Boolean).join(' ') || undefined
        : undefined,
