...
export let taskCompleted = SlateTrigger.create(
  spec,
  {
    name: 'Task Completed',
    key: 'task_completed',
    description: 'Triggered when a DataForSEO task completes. Configure the postback_url parameter when creating tasks to receive results via webhook, or use the pingback_url for notifications with task IDs.',
  },
)
  .input(z.object({
    taskId: z.string().describe('ID of the completed task'),
    taskTag: z.string().optional().describe('Tag associated with the task'),
    isPingback: z.boolean().describe('Whether this is a pingback (true) or postback (false)'),
