...
export let manageBackup = SlateTool.create(
  ...
  .input(z.object({
    ...
    retentionUnit: z.enum(['hour', 'day', 'week', 'month', 'year']).optional().describe('Retention unit (used with create)'),
    retentionValue: z.number().optional().describe('Retention value, 1-1000 (used with create)'),
    emergency: z.boolean().optional().describe('Trigger immediate backup, PostgreSQL only (used with create)'),
