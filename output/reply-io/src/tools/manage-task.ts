...
export let manageTask = SlateTool.create(
  ...
  .input(z.object({
    ...
    taskId: z.string().optional().describe('Task ID (required for get/update/delete; ULID format for create)'),
    ownerId: z.string().optional().describe('Owner/assignee user ID'),
    taskType: z.enum(['email_manual', 'email_auto', 'call', 'linkedin', 'custom']).optional().describe('Type of task'),
    ...
    taskData: z.record(z.string(), z.any()).optional().describe('Task-type-specific configuration data'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { action, taskId, ownerId, taskType, status, description, contactId, accountId, sequenceId, dueDate, taskData } = ctx.input;
