...
export let pushContactToCampaign = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    campaignId: z.number().optional().describe('Campaign ID to push the contact into (required for "push")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { action, email, campaignId, firstName, lastName, company, city, state, country, title, phone, linkedInProfile, customFields } = ctx.input;

    if (action === 'push') {
      if (!campaignId) throw new Error('campaignId is required to push a contact to a campaign');
