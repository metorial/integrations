...
export let timeEntryEvents = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers when a time entry is created, updated, or deleted in Hub Planner. Requires the Timesheets extension.',
  }
)
  .input(z.object({
    eventType: z.string().describe('The webhook event type (e.g. timeEntry.create, timeEntry.update, timeEntry.delete)'),
