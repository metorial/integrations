...
export let listBookingCategories = SlateTool.create(
  spec,
  {
...
Optionally retrieve a specific category by ID.`,
  ...
  .input(z.object({
    categoryId: z.string().optional().describe('Specific category ID to retrieve. If omitted, lists all categories.'),
  ...
  .output(z.object({
    ...
    })).optional().describe('Category groups (if requested)'),
  }))
