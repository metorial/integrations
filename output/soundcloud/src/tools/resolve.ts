...
export let resolveUrl = SlateTool.create(
  spec,
  {
    name: 'Resolve URL',
    key: 'resolve_url',
  ...
  .input(z.object({
    url: z.string().describe('SoundCloud URL to resolve (e.g., "https://soundcloud.com/artist/track-name")')
  }))
  .output(z.object({
    ...
    username: z.string().optional().describe('Username (for users, or uploader for tracks/playlists)'),
    permalinkUrl: z.string().describe('Canonical permalink URL'),
    resource: z.record(z.string(), z.any()).describe('Full resource representation from the API')
  }))
