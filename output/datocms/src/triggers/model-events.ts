...
export let modelEvents = SlateTrigger.create(
  spec,
  {
    name: 'Model Events',
    key: 'model_events',
    description: 'Triggers when content models are created, updated, or deleted. Field changes also trigger model update events.',
  }
)
  .input(z.object({
    eventType: z.enum(['create', 'update', 'delete']).describe('Type of model event'),
    entity: z.any().describe('The model entity data'),
    previousEntity: z.any().optional().describe('Previous state of the model (for update events)'),
