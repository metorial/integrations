...
export let upsertContacts = SlateTool.create(
  ...
  .input(z.object({
    contacts: z.array(z.object({
      ...
      customFields: z.record(z.string(), z.any()).optional().describe('Custom field values (field_id → value)'),
    })).min(1).describe('Contacts to add or update'),
  }))
  .output(z.object({
...
export let searchContacts = SlateTool.create(
  ...
  .input(z.object({
    query: z.string().describe('SGQL query string (e.g. "email LIKE \'%@example.com\' AND first_name = \'John\'")'),
  }))
  .output(z.object({
...
export let deleteContacts = SlateTool.create(
  ...
  .input(z.object({
    contactIds: z.array(z.string()).optional().describe('Contact IDs to delete. Omit to delete all contacts.'),
    deleteAll: z.boolean().optional().describe('Set to true to delete ALL contacts in the account'),
  }))
  .output(z.object({
