...
export let emailEvents = SlateTrigger.create(
  spec,
  {
    name: 'Email Events',
    key: 'email_events',
    description: 'Receives delivery and engagement events from SendGrid via the Event Webhook, including delivered, bounced, opened, clicked, spam reports, unsubscribes, and more.',
  }
)
  .input(z.object({
    ...
    asmGroupId: z.number().optional().describe('Suppression group ID (for unsubscribe events)'),
  ...
  .output(z.object({
    ...
    suppressionGroupId: z.number().optional().describe('Suppression group ID (unsubscribe events)'),
    ...
    handleRequest: async (ctx) => {
      ...
      return {
        inputs: events.map((e: any) => ({
          ...
          category: Array.isArray(e.category) ? e.category : e.category ? [e.category] : undefined,
          reason: e.reason,
          status: e.status,
          url: e.url,
