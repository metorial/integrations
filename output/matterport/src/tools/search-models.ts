...
let modelResultSchema = z.object({
  ...
  state: z.string().nullable().describe('Activation state of the model (active, inactive, pending)'),
...
export let searchModels = SlateTool.create(
  ...
  .input(z.object({
    ...
    offset: z.string().optional().describe('Pagination offset from a previous search result'),
    includeType: z.enum(['demo', 'inactive']).optional().describe('Include special model types in results'),
  }))
  .output(z.object({
    totalResults: z.number().describe('Total number of matching models'),
    nextOffset: z.string().nullable().describe('Offset for fetching the next page of results'),
