...
let sweepLocationSchema = z.object({
  ...
  position: z.object({
    ...
    y: z.number(),
    z: z.number(),
  }).nullable().optional().describe('3D position of the sweep'),
  ...
  panoramas: z.array(z.object({
    ...
    skybox: z.object({
      ...
      faceUrls: z.array(z.string()).nullable().optional().describe('URLs for the six cube-face images'),
    }).nullable().optional(),
  })).describe('Panoramic images at this location'),
});

export let getSweeps = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    resolution: z.enum(['512', '1k', '2k', '4k']).optional().default('2k').describe('Resolution for panoramic images'),
  }))
  .output(z.object({
    locations: z.array(sweepLocationSchema).describe('List of sweep locations with panoramic data'),
  }))
