...
export let getAssets = SlateTool.create(
  spec,
  {
    name: 'Get Assets',
    key: 'get_assets',
  ...
  .output(z.object({
    ...
    bundles: z.array(z.object({
      ...
      })).nullable().optional().describe('Downloadable asset files'),
...
export let purchaseAssetBundle = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    deliverySpeed: z.enum(['normal', 'rush']).optional().default('normal').describe('Delivery speed for the asset'),
  }))
  .output(z.object({
    ...
    assetUrls: z.array(z.string()).nullable().optional().describe('Download URLs for assets if available'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({
      token: ctx.auth.token,
    ...
    let result = await client.purchaseBundle(ctx.input.modelId, ctx.input.bundleId, ctx.input.deliverySpeed);
