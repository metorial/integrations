...
export let getProperty = SlateTool.create(
  spec,
  {
    name: 'Get Property',
    key: 'get_property',
  ...
  .input(z.object({
    ...
    includeRooms: z.boolean().optional().default(true).describe('Whether to also fetch room types for the property'),
  ...
  .output(z.object({
    ...
    rooms: z.array(z.record(z.string(), z.any())).optional().describe('Room types for the property'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let propertyName = property.name ?? property.property_name ?? `Property #${ctx.input.propertyId}`;
