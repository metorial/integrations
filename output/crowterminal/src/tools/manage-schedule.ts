...
Use **action: list** to see existing slots, **create** to add new ones, **toggle** to enable/disable, or **delete** to remove a slot.`,
  ...
  .input(
    z.object({
      action: z.enum(['list', 'create', 'toggle', 'delete']).describe('Operation to perform on schedule slots'),
      accountId: z.string().optional().describe('Account ID to filter slots (for list) or target (for create)'),
      slotId: z.string().optional().describe('Slot ID for toggle or delete operations'),
      dayOfWeek: z.string().optional().describe('Day of the week for a new slot (e.g., "monday", "tuesday")'),
      time: z.string().optional().describe('Time in HH:MM (24h) format for a new slot'),
      platform: z.enum(['tiktok', 'x', 'instagram']).optional().describe('Target platform for a new slot'),
  ...
  .output(
    z.object({
      ...
      ).optional().describe('List of schedule slots (returned for list action)'),
      slot: z.object({
        slotId: z.string().describe('Unique identifier of the schedule slot'),
        dayOfWeek: z.string().describe('Day of the week'),
        time: z.string().describe('Scheduled time in HH:MM format'),
        ...
        accountId: z.string().describe('Associated account ID'),
        enabled: z.boolean().describe('Whether the slot is active'),
      }).optional().describe('The created or updated slot (returned for create/toggle actions)'),
      deleted: z.boolean().optional().describe('Whether the slot was deleted (returned for delete action)'),
    })
  )
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      if (!ctx.input.dayOfWeek || !ctx.input.time || !ctx.input.platform || !ctx.input.accountId) {
        throw new Error('dayOfWeek, time, platform, and accountId are required to create a schedule slot.');
      }
