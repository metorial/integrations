...
export let unprotectData = SlateTool.create(
  spec,
  {
    name: 'Unprotect Data',
    key: 'unprotect_data',
  ...
  .input(z.object({
    protectedFields: z.record(z.string(), z.unknown()).describe('The data object containing previously encrypted/anonymized values.'),
    keys: z.array(z.string()).describe('Array of field names within the data object that should be decrypted/deanonymized.'),
  }))
  .output(z.object({
    fields: z.record(z.string(), z.unknown()).describe('The data object with specified fields decrypted back to their original values.'),
  }))
