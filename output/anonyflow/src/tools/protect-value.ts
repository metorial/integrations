...
export let protectValue = SlateTool.create(
  spec,
  {
    name: 'Protect Value',
    key: 'protect_value',
    ...
    instructions: [
      'Provide a single string value to be encrypted.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    sensitiveValue: z.string().describe('The sensitive value to encrypt/anonymize (e.g., a name, email, phone number, SSN).'),
  }))
  .output(z.object({
    encryptedValue: z.unknown().describe('The encrypted/anonymized representation of the input value.'),
  }))
