...
export let getAiCitations = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    searchTerms: z.array(z.string()).optional().describe('Filter citations by specific search queries'),
  }))
  .output(z.object({
    citations: z.any().describe('Citation metrics including domain/URL aggregations, engine breakdowns, brand analysis, and competitor comparisons'),
  }))
