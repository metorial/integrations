...
export let getAiSentiment = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    searchTerms: z.array(z.string()).optional().describe('Filter sentiment by specific search queries'),
  }))
  .output(z.object({
    sentiment: z.any().describe('Sentiment analysis including brand aggregates, keyword clouds, and engine-specific breakdowns'),
  }))
