...
export let trackEvent = SlateTool.create(
  ...
  .input(z.object({
    title: z.string().describe('Event title/name, e.g. "user_registered", "subscription_upgraded", "invoice_paid"'),
    email: z.string().optional().describe('Email address of the user'),
    userId: z.string().optional().describe('Your own user identifier'),
    leadboxerUserId: z.string().optional().describe('Existing LeadBoxer user ID (uid) to update'),
    ...
    proxy: z.boolean().optional().describe('Set to true to prevent automatic geo/user-agent enrichment from the request IP'),
    customProperties: z.record(z.string(), z.string()).optional().describe('Custom name/value pairs to attach to the event. These properties are copied to the user profile.'),
  }))
  .output(z.object({
