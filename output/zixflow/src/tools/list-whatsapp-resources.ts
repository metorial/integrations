...
export let listWhatsAppResources = SlateTool.create(spec, {
  ...
  .input(z.object({
    resourceType: z.enum(['accounts', 'templates', 'template_variables']).describe('Type of resource to retrieve'),
    phoneId: z.string().optional().describe('WhatsApp phone ID (required for templates and template_variables)'),
    templateName: z.string().optional().describe('Template name (required for template_variables)'),
    limit: z.number().optional().describe('Number of templates to return (for templates, default: 10)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let resources = Array.isArray(result.data) ? result.data : result.data ? [result.data] : [];
