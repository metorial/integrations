...
- **Direct message**: Send a free-form text, image, video, document, audio, or location message (requires an active 24-hour session with the recipient).`,
  ...
  .input(z.object({
    to: z.string().describe('Recipient phone number in international format (e.g., "13023895111")'),
    phoneId: z.string().describe('WhatsApp phone ID from your connected WABA account'),
    mode: z.enum(['template', 'direct']).describe('Message mode: "template" for approved templates, "direct" for free-form messages'),
    templateName: z.string().optional().describe('Template name (required for template mode)'),
    language: z.string().optional().describe('Template language code, e.g., "en" or "en_US" (required for template mode)'),
    variables: z.record(z.string(), z.any()).optional().describe('Template variables as key-value pairs'),
    messageType: z.enum(['text', 'image', 'video', 'document', 'audio', 'location']).optional().describe('Direct message type (required for direct mode)'),
    text: z.object({
    ...
    }).optional().describe('Text message content (for direct/text mode)'),
    ...
    }).optional().describe('Location details (for direct/location mode)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.mode === 'template') {
      ...
      if (messageType === 'text' && ctx.input.text) {
        ...
        if (ctx.input.filename && messageType === 'document') content[messageType].filename = ctx.input.filename;
    ...
    return {
      ...
      message: result.status ? `WhatsApp ${ctx.input.mode} message sent to ${ctx.input.to}.` : `Failed to send WhatsApp message: ${result.message}`,
    };
  })
  .build();
