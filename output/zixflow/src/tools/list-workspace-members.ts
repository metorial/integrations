...
export let listWorkspaceMembers = SlateTool.create(spec, {
  ...
  .input(z.object({
    memberId: z.string().optional().describe('Specific member ID to retrieve. If omitted, returns all members.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.memberId) {
      ...
      return {
        ...
        message: member ? `Retrieved member: **${member.fullName}** (${member.email}).` : 'Member not found.',
      };
    }
