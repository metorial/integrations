...
export let sendEmail = SlateTool.create(spec, {
  ...
  .input(z.object({
    ...
    attachmentIds: z.array(z.string()).optional().describe('Array of attachment IDs from the upload API'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: result.status ? `Email sent to ${ctx.input.to.join(', ')}.` : `Failed to send email: ${result.message}`,
    };
  })
  .build();
