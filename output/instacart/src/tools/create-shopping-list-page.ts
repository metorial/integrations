...
export let createShoppingListPage = SlateTool.create(
  spec,
  {
...
Requires **Developer Platform API key** authentication.`,
  ...
  .input(z.object({
    ...
    imageUrl: z.string().optional().describe('URL to the shopping list image (recommended 500x500 pixels)'),
    linkType: z.enum(['shopping_list', 'recipe']).optional().describe('Type of page to generate (defaults to "shopping_list")'),
    expiresIn: z.number().optional().describe('Number of days until the link expires (max 365)'),
    instructions: z.array(z.string()).optional().describe('Additional context or instructions'),
