...
let ingredientSchema = z.object({
  ...
  displayText: z.string().optional().describe('Display name shown to the user instead of the search name'),
  productIds: z.array(z.number()).optional().describe('Specific Instacart product IDs to match'),
  upcs: z.array(z.string()).optional().describe('Universal Product Codes to match'),
  measurements: z.array(lineItemMeasurementSchema).optional().describe('Quantity and unit specifications'),
  filters: z.object({
    brandName: z.string().optional().describe('Filter by brand name'),
    healthAttributes: z.array(z.string()).optional().describe('Filter by health attributes (e.g., "organic", "gluten_free")'),
  }).optional().describe('Filters to narrow product matching'),
});

export let createRecipePage = SlateTool.create(
  spec,
  {
...
Requires **Developer Platform API key** authentication.`,
  ...
  .input(z.object({
    ...
    imageUrl: z.string().optional().describe('URL to the recipe image (recommended 500x500 pixels)'),
    ...
    externalReferenceId: z.string().optional().describe('External tracking ID for your reference'),
    expiresIn: z.number().optional().describe('Number of days until the link expires (max 365, default 30)'),
    ...
    landingPageConfiguration: z.object({
      partnerLinkbackUrl: z.string().optional().describe('URL to link back to your site from the recipe page'),
      enablePantryItems: z.boolean().optional().describe('Whether to enable pantry item suggestions'),
    }).optional().describe('Landing page behavior settings'),
  }))
  .output(z.object({
