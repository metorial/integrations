...
let orderItemSchema = z.object({
  ...
  replacementPolicy: z.enum(['no_replacements', 'shoppers_choice', 'users_choice']).optional()
    .describe('What to do if this item is unavailable'),
  productCodes: z.array(productCodeSchema).optional().describe('Product identification codes (UPC or RRC)'),
});
...
Requires **Connect OAuth** authentication with the \`connect:fulfillment\` scope.`,
    ...
    constraints: [
      'Delivery orders cannot be cancelled once a shopper is assigned.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    fulfillmentType: z.enum(['delivery', 'pickup']).describe('Whether this is a delivery or pickup order'),
    ...
    initialTipCents: z.number().optional().describe('Initial tip amount in cents (delivery only)'),
    leaveUnattended: z.boolean().optional().describe('Leave order at door (delivery only)'),
    specialInstructions: z.string().optional().describe('Special delivery instructions (delivery only)'),
    ...
    }).optional().describe('Delivery address (delivery only)'),
    userDetails: z.object({
      birthday: z.string().optional().describe('Customer birthday (YYYY-MM-DD) for age-restricted items'),
      phoneNumber: z.string().optional().describe('Customer phone number'),
      smsOptIn: z.boolean().optional().describe('Whether customer opts in to SMS notifications'),
    }).optional().describe('Additional user details for this order'),
  }))
  .output(z.object({
    ...
    fulfillmentDetails: z.object({
      storeLocation: z.string().optional().describe('Store location where the order will be shopped'),
      windowStartsAt: z.string().optional().describe('Fulfillment window start (ISO 8601)'),
      windowEndsAt: z.string().optional().describe('Fulfillment window end (ISO 8601)'),
    }).optional().describe('Fulfillment details'),
    warnings: z.array(z.object({
      message: z.string().describe('Warning message'),
    })).optional().describe('Any warnings about the order'),
  }))
