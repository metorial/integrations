...
export let createCustomer = SlateTool.create(
  ...
  .input(z.object({
    customerId: z.string().describe('Your unique customer identifier (e.g., user ID from your system)'),
    ...
    status: z.enum(['trial', 'lead', 'new', 'paying']).optional().describe('Initial customer status'),
...
export let updateCustomerStatus = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['cancel', 'uncancel']).describe('Whether to cancel or uncancel the customer'),
  }))
  .output(customerSchema)
  .handleInvocation(async (ctx) => {
    let client = new TapfiliateClient({ token: ctx.auth.token });
