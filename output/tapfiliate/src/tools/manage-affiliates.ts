...
export let createAffiliate = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    }).optional().describe('Physical address of the affiliate'),
    metaData: z.record(z.string(), z.string()).optional().describe('Custom key-value metadata to attach'),
  }))
...
export let getAffiliate = SlateTool.create(
  spec,
  {
    name: 'Get Affiliate',
    key: 'get_affiliate',
...
export let listAffiliates = SlateTool.create(
  spec,
  {
...
export let deleteAffiliate = SlateTool.create(
  spec,
  {
...
export let updateAffiliateMetaData = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    metaData: z.record(z.string(), z.string()).describe('Key-value metadata to set. Replaces all existing metadata.'),
  }))
  .output(z.object({
...
export let manageAffiliateGroup = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    affiliateId: z.string().describe('ID of the affiliate'),
    action: z.enum(['assign', 'remove']).describe('Whether to assign to or remove from a group'),
    groupId: z.string().optional().describe('Group ID to assign the affiliate to (required when action is "assign")'),
  }))
  .output(z.object({
    affiliateId: z.string().describe('ID of the affiliate'),
...
export let manageAffiliatePayoutMethod = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    payoutMethod: z.object({
      ...
      paypalEmail: z.string().optional().describe('PayPal email address (when type is "paypal")'),
    }).optional().describe('Payout method details (required when action is "set")'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'get') {
      ...
      if (ctx.input.payoutMethod.paypalEmail) body.paypal_email = ctx.input.payoutMethod.paypalEmail;
