...
export let manageEntityLinksTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    links: z.array(z.object({
      ...
      toEntityType: z.enum(['leads', 'contacts', 'companies', 'catalog_elements']).describe('Target entity type'),
    })).optional().describe('Links to create or remove (required for "link" and "unlink" actions)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'link') {
      ...
      let result = await client.linkEntities(ctx.input.entityType, ctx.input.entityId, apiLinks);
      return {
        output: {
