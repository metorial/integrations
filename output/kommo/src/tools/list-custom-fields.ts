...
let customFieldOutputSchema = z.object({
  ...
  enums: z.array(enumOptionSchema).optional().describe('Available options for select/multiselect fields'),
...
export let listCustomFieldsTool = SlateTool.create(
  ...
  .input(z.object({
    entityType: z.enum(['leads', 'contacts', 'companies']).describe('Entity type to list custom fields for'),
