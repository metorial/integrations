...
export let embedSheetsChart = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['embed', 'refresh']).describe('Embed a new chart or refresh an existing one'),

    spreadsheetId: z.string().optional().describe('ID of the Google Sheets spreadsheet (for embed action)'),
    chartId: z.number().optional().describe('Numeric ID of the chart in the spreadsheet (for embed action)'),
    slideObjectId: z.string().optional().describe('Object ID of the slide to place the chart on (for embed action)'),
    widthPt: z.number().optional().describe('Width of the chart in points (for embed action)'),
    heightPt: z.number().optional().describe('Height of the chart in points (for embed action)'),
    linkingMode: z.enum(['LINKED', 'NOT_LINKED_IMAGE']).optional().describe('Whether the chart stays linked to the spreadsheet (for embed action, defaults to LINKED)'),

    chartObjectId: z.string().optional().describe('Object ID of the existing chart element to refresh (for refresh action)')
  }))
  .output(z.object({
    presentationId: z.string().describe('ID of the presentation'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'embed') {
      if (!ctx.input.spreadsheetId || ctx.input.chartId === undefined || !ctx.input.slideObjectId) {
        throw new Error('spreadsheetId, chartId, and slideObjectId are required for embed action');
      }
