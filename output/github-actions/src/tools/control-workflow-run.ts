...
export let controlWorkflowRun = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['cancel', 'rerun', 'rerun_failed', 'rerun_job', 'delete', 'approve', 'review_deployment']).describe('Action to perform'),
    jobId: z.number().optional().describe('Job ID, required when action is "rerun_job"'),
    deploymentEnvironmentIds: z.array(z.number()).optional().describe('Environment IDs to review, for "review_deployment" action'),
    deploymentState: z.enum(['approved', 'rejected']).optional().describe('Deployment review state'),
    ...
    switch (action) {
      ...
      case 'review_deployment':
        if (!ctx.input.deploymentEnvironmentIds || !ctx.input.deploymentState) {
          throw new Error('deploymentEnvironmentIds and deploymentState are required for "review_deployment".');
        }
