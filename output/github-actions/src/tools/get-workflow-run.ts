...
export let getWorkflowRun = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeJobs: z.boolean().optional().describe('Whether to include jobs and their steps (default false)'),
  }))
  .output(z.object({
    runId: z.number().describe('Workflow run ID'),
    ...
    })).optional().describe('Jobs in the workflow run'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includeJobs) {
      let jobsData = await client.listJobsForRun(ctx.input.owner, ctx.input.repo, ctx.input.runId);
