...
let variableSchema = z.object({
  ...
  visibility: z.string().optional().describe('Visibility for org variables (all, private, selected)'),
});
...
export let manageVariables = SlateTool.create(
  ...
  .input(z.object({
    ...
    variableName: z.string().optional().describe('Variable name, required for get/create/update/delete'),
    variableValue: z.string().optional().describe('Variable value, required for create/update'),
    visibility: z.enum(['all', 'private', 'selected']).optional().describe('Visibility for org variables'),
    selectedRepositoryIds: z.array(z.number()).optional().describe('Repository IDs for "selected" visibility org variables'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { scope, action, owner, repo, org, environmentName, variableName, variableValue, perPage, page } = ctx.input;
    ...
    if (action === 'list') {
      ...
      if (scope === 'org') {
        ...
        data = await client.listEnvironmentVariables(owner, repo, environmentName, { perPage, page });
      } else {
    ...
    if (action === 'create') {
      if (!variableName || variableValue === undefined) throw new Error('variableName and variableValue are required.');
      if (scope === 'org') {
        if (!org) throw new Error('org is required.');
        await client.createOrgVariable(org, variableName, variableValue, ctx.input.visibility ?? 'private', ctx.input.selectedRepositoryIds);
      } else if (scope === 'environment') {
        if (!owner || !repo || !environmentName) throw new Error('owner, repo, and environmentName are required.');
        await client.createEnvironmentVariable(owner, repo, environmentName, variableName, variableValue);
      } else {
        if (!owner || !repo) throw new Error('owner and repo are required.');
    ...
    if (action === 'update') {
      ...
      if (scope === 'org') {
        ...
        if (!owner || !repo || !environmentName) throw new Error('owner, repo, and environmentName are required.');
        await client.updateEnvironmentVariable(owner, repo, environmentName, variableName, { value: variableValue });
      } else {
        if (!owner || !repo) throw new Error('owner and repo are required.');
