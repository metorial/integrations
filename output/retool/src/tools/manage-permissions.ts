...
export let managePermissions = SlateTool.create(
  ...
  .input(z.object({
    ...
    subjectType: z.enum(['user', 'group']).describe('Type of the subject receiving/losing access'),
    subjectId: z.string().describe('ID of the user or group'),
    objectType: z.enum(['app', 'folder', 'resource', 'resource_configuration', 'workflow', 'agent']).describe('Type of the object being granted/revoked access to'),
    objectId: z.string().describe('ID of the object'),
    accessLevel: z.enum(['use', 'edit', 'own']).describe('The access level to grant or revoke'),
  }))
  .output(z.object({
    action: z.string(),
