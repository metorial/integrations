...
let workspaceAssignmentSchema = z.object({
  ...
  workspaceId: z.string().optional().describe('ID of the workspace (optional, can use name instead)'),
  status: z.enum(['active', 'archived']).optional().default('active').describe('User status in this workspace'),
  ...
  })).optional().describe('Groups to assign the user to within this workspace'),
});
...
export let createUser = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['active', 'archived']).optional().default('active').describe('Initial status of the user'),
    workspaces: z.array(workspaceAssignmentSchema).optional().describe('Workspaces to assign the user to'),
  }))
  .output(z.object({
