...
export let binLookupTool = SlateTool.create(
  spec,
  {
    name: 'BIN Lookup',
    key: 'bin_lookup',
  ...
  .input(z.object({
    binNumber: z.string().describe('The BIN/IIN number (first 6, 8, or 10 digits of a card number)'),
    customerIp: z.string().optional().describe('Customer IP address for enhanced fraud detection')
  }))
  .output(z.object({
    ...
    cardCategory: z.string().describe('Card category (e.g., CLASSIC, BUSINESS, PLATINUM, PREPAID)'),
    ...
    ipBlocklisted: z.boolean().describe('Whether customer IP is blocklisted (if customerIp provided)')
  }))
