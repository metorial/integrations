...
let locationSchema = z.object({
  ...
  locationType: z.string().describe('Location type: address, street, city, postal-code, railway, natural, island, administrative'),
  buildingType: z.string().describe('Building type classification')
});

export let geocodeAddressTool = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Rate limited to 6 requests per second'
    ],
    tags: {
      readOnly: true
    }
  ...
  .input(z.object({
    address: z.string().optional().describe('Full or partial address, or place name (unstructured)'),
    ...
    fuzzySearch: z.boolean().optional().describe('Enable fuzzy matching for typos and incomplete input'),
