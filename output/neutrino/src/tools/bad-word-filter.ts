...
export let badWordFilterTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    censorCharacter: z.string().optional().describe('Character to replace bad words with (e.g., "*"). If omitted, no censored version is returned.'),
    catalog: z.enum(['strict', 'obscene']).optional().describe('Word catalog: "strict" (comprehensive, suitable for children) or "obscene" (adult-only, excludes mild language)')
  }))
  .output(z.object({
    ...
    censoredContent: z.string().describe('Content with bad words replaced (only if censorCharacter was provided)')
  }))
