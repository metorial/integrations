...
export let forwardGeocode = SlateTool.create(
  spec,
  {
...
Use **countrycode** or **bounds** to narrow results to a specific region. Use **proximity** to bias results toward a location.`,
  ...
  .input(z.object({
    query: z.string().describe('Address, place name, or location to geocode (e.g., "Berlin", "1600 Pennsylvania Ave NW, Washington DC")'),
    countrycode: z.string().optional().describe('Restrict results to specific countries using comma-separated ISO 3166-1 Alpha 2 codes (e.g., "de", "fr,be,lu")'),
    bounds: z.string().optional().describe('Restrict results to a bounding box as "minLon,minLat,maxLon,maxLat"'),
    proximity: z.string().optional().describe('Bias results toward a location as "lat,lng"'),
    language: z.string().optional().describe('Return results in a specific language using IETF BCP 47 tag (e.g., "en", "de", "native")'),
    limit: z.number().optional().describe('Maximum number of results to return (default: 10, max: 100)'),
    minConfidence: z.number().optional().describe('Minimum confidence score for results (0-10)'),
    noAnnotations: z.boolean().optional().describe('Disable annotations to reduce response size'),
    noDedupe: z.boolean().optional().describe('Disable deduplication of results'),
    noRecord: z.boolean().optional().describe('Prevent OpenCage from keeping a record of the query content'),
    abbrv: z.boolean().optional().describe('Request abbreviated/shorter formatted addresses'),
    addressOnly: z.boolean().optional().describe('Return only address portion of results'),
    roadinfo: z.boolean().optional().describe('Include road and driving information in results'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (response.status.code !== 200) {
      throw new Error(`OpenCage API error: ${response.status.message} (code: ${response.status.code})`);
    }
