...
export let datasourceChanges = SlateTrigger.create(
  spec,
  {
    name: 'Data Source Changes',
    key: 'datasource_changes',
    description: 'Detects new or refreshed data sources by polling the data sources list and comparing refresh timestamps.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let inputs: Array<{ changeType: 'created' | 'refreshed'; datasourceId: string; name: string; connector?: string; dateLastRefresh?: string }> = [];
