...
export let listClients = SlateTool.create(
  ...
  .input(z.object({
    status: z.enum(['active', 'trial', 'setup', 'disabled']).optional().describe('Filter by client status'),
    externalId: z.string().optional().describe('Filter by external identifier'),
    includeDetails: z.boolean().optional().describe('Include associations (groups, users, share rights)'),
