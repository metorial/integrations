...
export let manageKlip = SlateTool.create(
  spec,
  {
    name: 'Manage Klip',
    key: 'manage_klip',
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      return {
        output: { klipId, name: ctx.input.name, description: ctx.input.description, success: true },
    ...
    if (ctx.input.action === 'update') {
      ...
      return {
        output: { klipId: ctx.input.klipId, name: ctx.input.name, description: ctx.input.description, success: true },
