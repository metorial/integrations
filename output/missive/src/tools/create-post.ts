...
Also supports conversation actions like closing, labeling, and assigning.`,
  ...
  .input(z.object({
    ...
    attachments: z.array(postAttachmentSchema).optional().describe('Rich attachment objects'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.addToTeamInbox !== undefined) post.add_to_team_inbox = ctx.input.addToTeamInbox;
    if (ctx.input.close !== undefined) post.close = ctx.input.close;
    if (ctx.input.conversationSubject) post.conversation_subject = ctx.input.conversationSubject;
