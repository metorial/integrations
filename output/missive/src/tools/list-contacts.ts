...
export let listContacts = SlateTool.create(
  ...
  .input(z.object({
    ...
    modifiedSince: z.number().optional().describe('Unix timestamp for incremental sync — returns only contacts modified after this time'),
    includeDeleted: z.boolean().optional().describe('Include deleted contacts'),
    limit: z.number().min(1).max(200).optional().describe('Number of contacts to return (max 200)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let contacts = (data.contacts || []).map((c: any) => ({
      ...
      phoneNumbers: c.infos?.filter((i: any) => i.type === 'phone_number').map((i: any) => i.phone_number),
    }));
