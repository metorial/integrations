...
export let createCustomChannelMessage = SlateTool.create(
  spec,
  {
...
Also supports conversation actions (close, label, assign) as part of message creation.`,
  ...
  .input(z.object({
    ...
    deliveredAt: z.number().optional().describe('Unix timestamp for the message delivery time'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.removeSharedLabels) message.remove_shared_labels = ctx.input.removeSharedLabels;
    if (ctx.input.addToInbox !== undefined) message.add_to_inbox = ctx.input.addToInbox;
    if (ctx.input.addToTeamInbox !== undefined) message.add_to_team_inbox = ctx.input.addToTeamInbox;
    if (ctx.input.close !== undefined) message.close = ctx.input.close;
    if (ctx.input.conversationSubject) message.conversation_subject = ctx.input.conversationSubject;
