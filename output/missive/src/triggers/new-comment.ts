...
export let newComment = SlateTrigger.create(
  spec,
  {
    name: 'New Comment',
    key: 'new_comment',
    description: 'Triggered when a new comment is added to a conversation. Can be filtered to task-related comments, specific authors, or mentioned users.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    mentionedUserIds: z.array(z.string()).optional().describe('User IDs mentioned in the comment'),
