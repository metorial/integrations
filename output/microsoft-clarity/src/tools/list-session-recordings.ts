...
let sessionRecordingSchema = z.record(z.string(), z.any()).describe('Session recording data from Clarity');

export let listSessionRecordings = SlateTool.create(
  spec,
  {
...
Returns a sample of session recordings matching the specified criteria, useful for reviewing user behavior on specific pages or segments.`,
  ...
  .input(z.object({
    startDate: z.string().describe('Start date in UTC ISO 8601 format (e.g. "2025-01-15T00:00:00Z")'),
    endDate: z.string().describe('End date in UTC ISO 8601 format (e.g. "2025-01-16T00:00:00Z")'),
    urls: z.array(z.string()).optional().describe('Filter to recordings that visited these URLs'),
    deviceTypes: z.array(z.string()).optional().describe('Filter by device types (e.g. "Desktop", "Mobile", "Tablet")'),
    browsers: z.array(z.string()).optional().describe('Filter by browser names (e.g. "Chrome", "Firefox", "Safari")'),
    operatingSystems: z.array(z.string()).optional().describe('Filter by operating systems (e.g. "Windows", "macOS", "Android")'),
  ...
  .output(z.object({
    recordings: z.array(sessionRecordingSchema).describe('Array of session recording entries'),
