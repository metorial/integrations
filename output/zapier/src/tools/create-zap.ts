...
export let createZap = SlateTool.create(
  spec,
  {
    name: 'Create Zap',
    key: 'create_zap',
...
Use the **List Actions** and **List Authentications** tools first to get valid action and authentication IDs, and **Get Action Input Fields** to discover required inputs for each step.`,
  ...
  .input(z.object({
    ...
    steps: z.array(z.object({
      ...
      authenticationId: z.string().nullable().describe('Authentication ID from /v2/authentications, or null'),
      inputs: z.record(z.string(), z.any()).describe('Input field values for this step'),
      alias: z.string().nullable().optional().describe('Optional alias for referencing this step (snake_case, max 64 chars)'),
    })).min(1).describe('Ordered list of workflow steps'),
  }))
  .output(z.object({
