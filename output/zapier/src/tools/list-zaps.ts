...
let stepSchema = z.object({
  ...
  authentication: z.any().nullable().describe('Authentication identifier or expanded authentication object'),
  inputs: z.record(z.string(), z.any()).describe('Step input field values'),
  title: z.string().nullable().describe('Step title'),
});

export let listZaps = SlateTool.create(
  spec,
  {
    name: 'List Zaps',
    key: 'list_zaps',
...
Use the **expand** parameter to include full action and authentication objects instead of just IDs.`,
  ...
  .input(z.object({
    expand: z.string().optional().describe('Comma-separated list of fields to expand to full objects, e.g. "steps.action"'),
    includeShared: z.boolean().optional().describe('Include shared Zaps (requires zap:account:all scope)'),
    inputs: z.record(z.string(), z.string()).optional().describe('Filter Zaps by specific input settings, e.g. { "board": "BOARD_ID" }'),
  ...
  .output(z.object({
    zaps: z.array(z.object({
      ...
      lastSuccessfulRunDate: z.string().nullable().describe('ISO 8601 timestamp of the last successful execution'),
