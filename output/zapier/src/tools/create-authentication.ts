...
For OAuth-based apps, users must complete the authorization flow in a browser instead.`,
  ...
  .input(z.object({
    ...
    title: z.string().describe('Label for the new authentication (e.g., "My Company API Key")'),
    authenticationFields: z.record(z.string(), z.string()).describe('Credential fields required by the app (e.g., { "api_key": "sk-..." })'),
  }))
  .output(z.object({
    authenticationId: z.string().describe('Unique identifier for the created authentication'),
