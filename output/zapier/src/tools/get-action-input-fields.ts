...
export let getActionInputFields = SlateTool.create(
  spec,
  {
...
Use this to discover what inputs are needed before creating a Zap step or testing an action. Some fields may depend on the values of other fields.`,
  ...
  .input(z.object({
    actionId: z.string().describe('Action ID from the /v2/actions endpoint, e.g. "uag:87b1c14e-..."'),
    authenticationId: z.string().nullable().describe('Authentication ID to use for resolving dynamic fields'),
    currentInputs: z.record(z.string(), z.any()).optional().describe('Current input field values for resolving dependent fields'),
  }))
  .output(z.object({
