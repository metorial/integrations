...
export let createInvoice = SlateTool.create(
  ...
  .input(z.object({
    currency: z.string().optional().describe('Cryptocurrency ID (e.g. BTC, ETH, LTC). Auto-selected if omitted.'),
    orderName: z.string().describe('Merchant order name'),
    orderNumber: z.string().describe('Unique merchant order number'),
    amount: z.number().optional().describe('Amount in cryptocurrency. Omit if using fiat conversion with sourceCurrency and sourceAmount.'),
    sourceCurrency: z.string().optional().describe('Fiat currency code for automatic conversion (e.g. USD, EUR)'),
    sourceAmount: z.number().optional().describe('Amount in fiat currency to convert to crypto'),
    allowedCryptocurrencies: z.string().optional().describe('Comma-separated list of allowed cryptocurrency IDs (e.g. BTC,ETH,LTC)'),
    description: z.string().optional().describe('Invoice description shown to the customer'),
    callbackUrl: z.string().optional().describe('URL to receive invoice status updates via POST. Append ?json=true for JSON format.'),
    successCallbackUrl: z.string().optional().describe('URL for successful payment callback'),
    failCallbackUrl: z.string().optional().describe('URL for failed payment callback'),
    successInvoiceUrl: z.string().optional().describe('Button link on invoice page after successful payment'),
    failInvoiceUrl: z.string().optional().describe('Button link on invoice page after failed payment'),
  ...
  .output(z.object({
    ...
    invoiceTotalSum: z.string().optional().describe('Total invoice amount in cryptocurrency'),
