...
export let withdraw = SlateTool.create(
  spec,
  {
    name: 'Withdraw',
    key: 'withdraw',
  ...
  .input(z.object({
    ...
    amounts: z.string().describe('Amount to send, or comma-separated amounts matching each address for mass withdrawal'),
    feePlan: z.enum(['normal', 'priority']).optional().describe('Fee plan to use for the transaction'),
  }))
  .output(z.object({
    ...
    sendMany: z.record(z.string(), z.string()).optional().describe('Address-to-amount mapping (mass withdrawal)'),
