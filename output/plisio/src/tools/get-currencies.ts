...
export let getCurrencies = SlateTool.create(
  spec,
  {
    name: 'Get Currencies',
    key: 'get_currencies',
  ...
  .input(z.object({
    fiatCurrency: z.string().optional().describe('Fiat currency code for rate conversion (e.g. USD, EUR, GBP). Defaults to USD.'),
  }))
  .output(z.object({
    currencies: z.array(z.object({
      ...
      invoiceCommissionPercentage: z.string().optional().describe('Invoice commission percentage'),
      hidden: z.boolean().optional().describe('Whether the currency is hidden'),
      maintenance: z.boolean().optional().describe('Whether the currency is under maintenance'),
