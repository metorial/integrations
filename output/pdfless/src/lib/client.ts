...
export class PdflessClient {
  ...
  async generatePdf(params: GeneratePdfParams): Promise<GeneratePdfResult> {
    ...
    if (params.encryption) {
      ...
      if (enc.allowModifying !== undefined) body.encryption_allow_modifying = enc.allowModifying;
      if (enc.allowContentCopying !== undefined) body.encryption_allow_content_copying = enc.allowContentCopying;
      if (enc.allowModifyAnnotations !== undefined) body.encryption_allow_modify_annotations = enc.allowModifyAnnotations;
      if (enc.allowFormFilling !== undefined) body.encryption_allow_form_filling = enc.allowFormFilling;
      if (enc.allowScreenReaders !== undefined) body.encryption_allow_screenreaders = enc.allowScreenReaders;
      if (enc.allowDocumentAssembly !== undefined) body.encryption_allow_document_assembly = enc.allowDocumentAssembly;
    }
