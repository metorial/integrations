...
export let scoreEmail = SlateTool.create(
  spec,
  {
    name: 'Score Email',
    key: 'score_email',
...
A score of 10 is ideal, while 0 indicates a high-risk address (spam trap, inactive, etc.). Useful for evaluating catch-all emails and segmenting lists by engagement likelihood.`,
  ...
  .output(z.object({
    ...
    zerobounceScore: z.number().nullable().describe('AI quality score from 0 to 10, where 10 is ideal'),
    account: z.string().nullable().optional().describe('The local part of the email address'),
    domain: z.string().nullable().optional().describe('The domain of the email address'),
    processedAt: z.string().optional().describe('UTC timestamp when the scoring was processed'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let output = {
      ...
      zerobounceScore: result.zerobounce_score !== undefined ? Number(result.zerobounce_score) : null,
    ...
    let scoreDisplay = output.zerobounceScore !== null ? `**${output.zerobounceScore}/10**` : 'unavailable';
