...
export let manageLibrary = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['getSavedTracks', 'getSavedAlbums', 'saveTracks', 'saveAlbums', 'removeTracks', 'removeAlbums', 'checkTracks', 'checkAlbums']).describe('Library operation to perform'),
    trackIds: z.array(z.string()).optional().describe('Spotify track IDs (for save, remove, or check operations)'),
    albumIds: z.array(z.string()).optional().describe('Spotify album IDs (for save, remove, or check operations)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'saveTracks') {
      if (!ctx.input.trackIds || ctx.input.trackIds.length === 0) throw new Error('trackIds is required for "saveTracks" action');
    ...
    if (action === 'saveAlbums') {
      if (!ctx.input.albumIds || ctx.input.albumIds.length === 0) throw new Error('albumIds is required for "saveAlbums" action');
    ...
    if (action === 'removeTracks') {
      if (!ctx.input.trackIds || ctx.input.trackIds.length === 0) throw new Error('trackIds is required for "removeTracks" action');
    ...
    if (action === 'removeAlbums') {
      if (!ctx.input.albumIds || ctx.input.albumIds.length === 0) throw new Error('albumIds is required for "removeAlbums" action');
    ...
    if (action === 'checkTracks') {
      if (!ctx.input.trackIds || ctx.input.trackIds.length === 0) throw new Error('trackIds is required for "checkTracks" action');
    ...
    if (action === 'checkAlbums') {
      if (!ctx.input.albumIds || ctx.input.albumIds.length === 0) throw new Error('albumIds is required for "checkAlbums" action');
