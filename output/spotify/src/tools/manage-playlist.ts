...
export let managePlaylist = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'update', 'addItems', 'removeItems', 'reorder']).describe('Playlist operation to perform'),
    playlistId: z.string().optional().describe('Spotify playlist ID (required for get, update, addItems, removeItems, reorder)'),
    name: z.string().optional().describe('Playlist name (required for create, optional for update)'),
    ...
    uris: z.array(z.string()).optional().describe('Spotify URIs to add or remove (e.g., spotify:track:xxx)'),
    ...
    limit: z.number().min(1).max(50).optional().describe('Max results per page (default 20)'),
  ...
  .handleInvocation(async (ctx) => {
        ...
        output: {
          playlist: {
            ...
            tracks: playlist.tracks.items.map(item => ({
              ...
              artists: item.track?.artists.map(a => ({ artistId: a.id, name: a.name })) ?? null,
    ...
    if (action === 'addItems') {
      if (!ctx.input.playlistId) throw new Error('playlistId is required for "addItems" action');
      if (!ctx.input.uris || ctx.input.uris.length === 0) throw new Error('uris is required for "addItems" action');

      let result = await client.addItemsToPlaylist(ctx.input.playlistId, ctx.input.uris, ctx.input.position);
    ...
    if (action === 'removeItems') {
      if (!ctx.input.playlistId) throw new Error('playlistId is required for "removeItems" action');
      if (!ctx.input.uris || ctx.input.uris.length === 0) throw new Error('uris is required for "removeItems" action');
    ...
    if (action === 'reorder') {
      if (!ctx.input.playlistId) throw new Error('playlistId is required for "reorder" action');
      if (ctx.input.rangeStart === undefined) throw new Error('rangeStart is required for "reorder" action');
      if (ctx.input.insertBefore === undefined) throw new Error('insertBefore is required for "reorder" action');
