...
export let getTopItems = SlateTool.create(
  spec,
  {
    name: 'Get Top Items',
    key: 'get_top_items',
  ...
  .input(z.object({
    ...
    timeRange: z.enum(['short_term', 'medium_term', 'long_term']).optional().describe('Time range: short_term (4 weeks), medium_term (6 months), long_term (several years). Defaults to medium_term.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let topArtists = ctx.input.type === 'artists'
    ...
    let topTracks = ctx.input.type === 'tracks'
      ? result.items.map((item: any) => ({
          trackId: item.id,
          name: item.name,
    ...
    let timeLabel = ctx.input.timeRange === 'short_term' ? '4 weeks' : ctx.input.timeRange === 'long_term' ? 'all time' : '6 months';
