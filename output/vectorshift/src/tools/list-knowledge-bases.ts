...
export let listKnowledgeBasesTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    includeShared: z.boolean().optional().default(false).describe('Include knowledge bases shared with you'),
    verbose: z.boolean().optional().default(false).describe('Include full knowledge base objects in the response'),
  }))
  .output(z.object({
    knowledgeBaseIds: z.array(z.string()).describe('List of knowledge base IDs'),
    knowledgeBases: z.array(z.record(z.string(), z.unknown())).optional().describe('Full knowledge base objects (when verbose is true)'),
  }))
