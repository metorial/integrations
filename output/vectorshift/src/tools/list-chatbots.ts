...
export let listChatbotsTool = SlateTool.create(
  spec,
  {
    name: 'List Chatbots',
    key: 'list_chatbots',
  ...
  .input(z.object({
    includeShared: z.boolean().optional().default(false).describe('Include chatbots shared with you'),
    verbose: z.boolean().optional().default(false).describe('Include full chatbot objects in the response'),
  }))
  .output(z.object({
    chatbotIds: z.array(z.string()).describe('List of chatbot IDs'),
    chatbots: z.array(z.record(z.string(), z.unknown())).optional().describe('Full chatbot objects (when verbose is true)'),
  }))
