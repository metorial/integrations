...
export let runPipelineTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    inputs: z.record(z.string(), z.unknown()).describe('Key-value pairs of input data for the pipeline'),
    conversationId: z.string().optional().describe('Conversation ID for maintaining context across multiple runs'),
  }))
  .output(z.object({
    runId: z.string().describe('ID of the pipeline run'),
    outputs: z.record(z.string(), z.unknown()).describe('Key-value pairs of output data from the pipeline'),
  }))
