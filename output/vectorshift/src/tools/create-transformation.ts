...
export let createTransformationTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    inputSchema: z.record(z.string(), z.unknown()).describe('Input schema with property definitions'),
    outputSchema: z.record(z.string(), z.unknown()).describe('Output schema with property definitions'),
  }))
  .output(z.object({
