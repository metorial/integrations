...
export let chatbotChangesTrigger = SlateTrigger.create(
  spec,
  {
  ...
  .input(z.object({
    changeType: z.enum(['added', 'removed']).describe('Whether the chatbot was added or removed'),
    chatbotId: z.string().describe('ID of the affected chatbot'),
  }))
  .output(z.object({
    chatbotId: z.string().describe('ID of the affected chatbot'),
    name: z.string().optional().describe('Name of the chatbot (available for added chatbots)'),
