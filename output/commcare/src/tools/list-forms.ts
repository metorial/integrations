...
Filter by form type (xmlns), date received, application, and more. Returns paginated results including form responses and metadata.`,
  ...
  .input(z.object({
    xmlns: z.string().optional().describe('Filter by form XML namespace (form type identifier)'),
    appId: z.string().optional().describe('Filter by application ID'),
    receivedOnStart: z.string().optional().describe('Filter forms received on or after this date (ISO 8601)'),
    receivedOnEnd: z.string().optional().describe('Filter forms received on or before this date (ISO 8601)'),
