...
export let manageCardLinksTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['add_parent', 'remove_parent', 'link', 'unlink', 'list']).describe(
    ...
    targetCardId: z.number().optional().describe('ID of the target card (required for all actions except list)'),
    linkType: z.number().optional().describe('Link type for "link" action (e.g. 0=related, 1=predecessor, 2=successor)'),
  }))
  .output(z.object({
    ...
    })).optional().describe('List of linked cards (for list action)'),
    parentCards: z.array(z.object({
      parentCardId: z.number().describe('Parent card ID'),
    })).optional().describe('List of parent cards'),
    childCards: z.array(z.object({
      childCardId: z.number().describe('Child card ID'),
    })).optional().describe('List of child cards'),
