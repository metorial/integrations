...
export let manageCustomFieldsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'update']).describe('"list" to view all custom fields or "update" to set a field value'),
    fieldId: z.number().optional().describe('Custom field ID (required for update)'),
    value: z.any().optional().describe('New value for the custom field (required for update)'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Custom fields list (for list action)'),
    updatedField: z.object({
      fieldId: z.number().describe('Custom field ID'),
      value: z.any().optional().nullable().describe('Updated value'),
    }).optional().describe('Updated field (for update action)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.updateCustomField(ctx.input.cardId, ctx.input.fieldId, ctx.input.value);
    return {
      output: {
        cardId: ctx.input.cardId,
        updatedField: {
