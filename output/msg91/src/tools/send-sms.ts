...
export let sendSms = SlateTool.create(
  ...
  .input(z.object({
    ...
    recipients: z.array(z.object({
      mobiles: z.string().describe('Mobile number with country code (e.g., 919XXXXXXXXX)'),
    }).catchall(z.string()).describe('Recipient with mobile number and optional template variables')).describe('Array of recipients with their mobile numbers and template variables'),
    sender: z.string().optional().describe('Sender ID (if not defined in the flow)'),
    shortUrl: z.enum(['0', '1']).optional().describe('Enable URL shortening: "1" to enable, "0" to disable'),
    shortUrlExpiry: z.number().optional().describe('URL shortening expiry in seconds'),
    unicode: z.number().optional().describe('Set to 1 for non-English/Unicode content, 0 for English'),
    scheduleAt: z.string().optional().describe('Schedule time in "YYYY-MM-DD HH:mm:ss" format or Unix timestamp'),
  }))
  .output(z.object({
