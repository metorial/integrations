...
export let sendVoiceMessage = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    callType: z.enum(['voice_sms', 'click_to_call']).describe('Type of voice call: "voice_sms" for text-to-speech, "click_to_call" for two-way call'),
    ...
    clientNumber: z.string().optional().describe('Recipient number with country code (required for voice_sms)'),
    destination: z.string().optional().describe('First party number (required for click_to_call)'),
    destinationB: z.array(z.string()).optional().describe('Second party numbers (required for click_to_call)'),
    ...
    variables: z.record(z.string(), z.object({
      ...
      asDigits: z.boolean().optional().describe('Read number digit by digit (for type "number")'),
      currencyCode: z.string().optional().describe('Currency code like "USD", "INR" (for type "currency")'),
    })).optional().describe('Template variables for voice_sms calls'),
  }))
  .output(z.object({
