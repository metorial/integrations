...
export let whatsappDeliveryReport = SlateTrigger.create(
  spec,
  {
    name: 'WhatsApp Delivery Report',
    key: 'whatsapp_delivery_report',
    description: 'Receive delivery status updates and inbound messages for WhatsApp. Covers outbound request sent, delivery reports (Sent, Failed, Delivered, Read), inbound request submitted, and inbound delivery confirmation. Configure the webhook URL in the MSG91 dashboard.',
  }
)
  .input(z.object({
    eventType: z.string().describe('WhatsApp event type (outbound_sent, outbound_report, inbound_request, inbound_report)'),
    ...
    handleRequest: async (ctx) => {
      ...
      return {
        inputs: events.map((event: any) => {
          let direction = event.direction || (event.type?.includes('inbound') ? 'inbound' : 'outbound');
          return {
            ...
            recipientNumber: event.mobile || event.number || event.from || event.to || undefined,
