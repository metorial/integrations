...
let marketSchema = z.object({
  ...
  lastUpdate: z.string().optional().describe('ISO timestamp of the last odds update for this market'),
...
export let getOddsTool = SlateTool.create(
  spec,
  {
    name: 'Get Odds',
    key: 'get_odds',
...
Use \`"upcoming"\` as the sport to get odds across all in-season sports.`,
    ...
    constraints: [
      'Costs 1 API credit per region per market.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    sport: z.string().describe('Sport key (e.g. "americanfootball_nfl") or "upcoming" for all sports'),
    regions: z.string().describe('Comma-separated bookmaker regions: us, us2, uk, au, eu'),
    markets: z.string().optional().describe('Comma-separated markets: h2h, spreads, totals, outrights (default: h2h)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { sport, regions, markets, oddsFormat, eventIds, bookmakers, commenceTimeFrom, commenceTimeTo } = ctx.input;
