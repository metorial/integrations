...
export let manageIncomingWebhook = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    createIncidentRule: z.record(z.string(), z.any()).optional().describe('Rules for when to create incidents from payload'),
    acknowledgeIncidentRule: z.record(z.string(), z.any()).optional().describe('Rules for when to acknowledge incidents from payload'),
    resolveIncidentRule: z.record(z.string(), z.any()).optional().describe('Rules for when to resolve incidents from payload'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'list') {
      let result = await client.listIncomingWebhooks({ page: ctx.input.page, perPage: ctx.input.perPage });
    ...
    if (ctx.input.recoveryPeriod !== undefined) body.recovery_period = ctx.input.recoveryPeriod;
    ...
    if (ctx.input.acknowledgeIncidentRule) body.started_rule = ctx.input.acknowledgeIncidentRule;
