...
export let manageIncident = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'acknowledge', 'resolve', 'get', 'delete']).describe('Action to perform'),
    incidentId: z.string().optional().describe('Incident ID (required for acknowledge, resolve, get, delete)'),
    ...
    acknowledgedBy: z.string().optional().describe('Email of person acknowledging the incident'),
    resolvedBy: z.string().optional().describe('Email of person resolving the incident'),
    includeTimeline: z.boolean().optional().describe('Include timeline events when getting incident details'),
  }))
  .output(z.object({
    ...
    timeline: z.array(z.record(z.string(), z.any())).optional().describe('Timeline events (if requested)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'delete') {
      ...
      return {
        output: { incidentId, name: null, status: null, cause: null, startedAt: null, resolvedAt: null, acknowledgedAt: null, deleted: true },
