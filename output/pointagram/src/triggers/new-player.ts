...
export let newPlayer = SlateTrigger.create(
  spec,
  {
    name: 'New Player',
    key: 'new_player',
    description: 'Triggers when a new player is detected in Pointagram by comparing the current player list against the previously known set.',
  }
)
  .input(z.object({
    ...
    pollEvents: async (ctx) => {
      ...
      return {
        inputs: newPlayers.map((p) => ({
          ...
          playerEmail: p.email ? String(p.email) : (p.player_email ? String(p.player_email) : undefined),
          playerExternalId: p.external_id ? String(p.external_id) : (p.player_external_id ? String(p.player_external_id) : undefined),
          raw: p,
        })),
        updatedState: {
