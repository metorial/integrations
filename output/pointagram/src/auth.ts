...
export let auth = SlateAuth.create()
  ...
  .addCustomAuth({
    ...
    inputSchema: z.object({
      apiKey: z.string().describe('Your Pointagram API key (found in Settings > Integrations > Custom Integration)'),
    ...
    getProfile: async (ctx: { output: { token: string; apiUser: string }; input: { apiKey: string; apiUser: string } }) => {
      return {
        profile: {
