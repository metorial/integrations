...
export let manageTeamMembership = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['add', 'remove']).describe('Whether to add or remove the player from the team'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let identifier = ctx.input.playerName || ctx.input.playerEmail || ctx.input.playerExternalId || ctx.input.playerId || 'unknown';
