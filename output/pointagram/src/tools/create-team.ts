...
export let createTeam = SlateTool.create(
  ...
  .input(z.object({
    ...
    icon: z.string().describe('Team icon name. Use built-in icons like "Bears", "Bulls", "Sharks", "Eagles", "Lions", "Tigers", "Wolves", "Panthers", "Hawks", "Falcons" or a custom-uploaded icon name'),
    filterIgnore: z.boolean().optional().describe('If true, this team is exempt from filter settings'),
  }))
  .output(z.object({
