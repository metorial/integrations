...
export let createPlayer = SlateTool.create(
  spec,
  {
    name: 'Create Player',
    key: 'create_player',
  ...
  .input(z.object({
    ...
    playerEmail: z.string().optional().describe('Email address of the player, used for sending login invitations'),
    playerExternalId: z.string().optional().describe('Your own unique identifier for the player'),
    offline: z.boolean().optional().describe('If true, creates an offline player (managed by admin). If false, the player receives a login invitation.'),
  }))
  .output(z.object({
