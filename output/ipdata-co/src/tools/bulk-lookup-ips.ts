...
export let bulkLookupIps = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ipAddresses: z.array(z.string()).min(1).max(100).describe('List of IPv4 or IPv6 addresses to look up (1-100)'),
    fields: z.array(z.string()).optional().describe('Specific fields to return (e.g. ["ip", "city", "country_name"]). Omit to return all fields.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let countrySummary = countries.length > 0 ? countries.slice(0, 5).join(', ') : 'various locations';
