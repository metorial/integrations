...
export let organizationEvents = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Polls for new organization events including user actions, vault operations, collection changes, group changes, policy updates, and Secrets Manager operations.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    eventTypeCode: z.number().describe('Numeric event type code'),
    eventTypeName: z.string().describe('Human-readable event type (e.g. user.logged_in, item.created)'),
    eventDate: z.string().describe('ISO 8601 timestamp of the event'),
    itemId: z.string().nullable().describe('Related vault item ID'),
    collectionId: z.string().nullable().describe('Related collection ID'),
    groupId: z.string().nullable().describe('Related group ID'),
