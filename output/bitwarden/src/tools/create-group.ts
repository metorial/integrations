...
export let createGroup = SlateTool.create(
  ...
  .input(z.object({
    ...
    accessAll: z.boolean().default(false).describe('Whether the group should have access to all collections'),
    externalId: z.string().optional().describe('External ID for directory sync (max 300 characters)'),
    collections: z.array(z.object({
      collectionId: z.string().describe('Collection ID'),
      readOnly: z.boolean().default(false).describe('Whether access is read-only'),
    })).optional().describe('Collections to assign to the group'),
