...
let policySchema = z.object({
  ...
  policyType: z.number().describe('Policy type: 0=TwoFactorAuthentication, 1=MasterPassword, 2=PasswordGenerator, 3=SingleOrg, 4=RequireSso, 5=OrgDataOwnership, 6=DisableSend, 7=SendOptions, 8=ResetPassword, 9=MaxVaultTimeout, 10=DisablePersonalVaultExport'),
  enabled: z.boolean().describe('Whether the policy is currently enabled'),
  configuration: z.record(z.string(), z.any()).nullable().describe('Policy-specific configuration data'),
});

export let listPolicies = SlateTool.create(
  spec,
  {
    name: 'List Policies',
    key: 'list_policies',
