...
export let manageNotes = SlateTool.create(
  ...
  .input(z.object({
    ...
    scope: z.enum(['personal', 'group']).default('personal').describe('Whether this is a personal or group note'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update') {
      ...
      return {
        output: { note: mapNote(result || { noteId: ctx.input.noteId, ...noteData }), success: true },
