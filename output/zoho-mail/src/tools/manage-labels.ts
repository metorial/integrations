...
export let manageLabels = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update') {
      ...
      let result = await client.updateLabel(accountId, ctx.input.labelId, ctx.input.labelName, ctx.input.color);
      return {
        output: { label: mapLabel(result || { labelId: ctx.input.labelId, labelName: ctx.input.labelName }), success: true },
