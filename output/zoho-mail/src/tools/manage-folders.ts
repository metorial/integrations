...
export let manageFolders = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'create', 'rename', 'delete', 'empty', 'markAsRead']).describe('Operation to perform'),
    folderId: z.string().optional().describe('Folder ID (required for rename, delete, empty, markAsRead)'),
    folderName: z.string().optional().describe('Folder name (required for create, rename)'),
    parentFolderId: z.string().optional().describe('Parent folder ID for creating nested folders'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      ...
      let result = await client.createFolder(accountId, ctx.input.folderName, ctx.input.parentFolderId);
      return {
    ...
    if (action === 'rename') {
      ...
      let result = await client.renameFolder(accountId, ctx.input.folderId, ctx.input.folderName);
      return {
        output: { folder: mapFolder(result || { folderId: ctx.input.folderId, folderName: ctx.input.folderName }), success: true },
