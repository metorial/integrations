...
export let getEmail = SlateTool.create(
  spec,
  {
    name: 'Get Email',
    key: 'get_email',
  ...
  .input(z.object({
    ...
    includeBlockContent: z.boolean().optional().default(true).describe('Include block quote content from email threads'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let [metadata, contentData] = await Promise.all([
      ...
      client.getMessageContent(ctx.input.accountId, ctx.input.folderId, ctx.input.messageId, ctx.input.includeBlockContent),
    ]);
