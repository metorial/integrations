...
export let getClassificationResultsTool = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    files: z.array(z.object({
      ...
      confidence: z.number().optional().describe('Classification confidence score (0-1)'),
      documentType: z.string().optional().describe('Predicted document type'),
      fileUrl: z.string().optional().describe('URL to access the file'),
      linkedExtractionId: z.string().optional().describe('Linked extraction ID if auto-extraction was triggered'),
      linkedBatchId: z.string().optional().describe('Linked batch ID from auto-extraction'),
      linkedFileId: z.string().optional().describe('Linked file ID from auto-extraction'),
    })).optional().describe('Classified files with results'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let processedCount = files?.filter((f: { status: string }) => f.status === 'processed').length || 0;
