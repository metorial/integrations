...
export let createCheck = SlateTool.create(
  ...
  .input(z.object({
    ...
    type: z.enum(['https', 'http', 'icmp', 'tcp', 'tcps', 'pulse']).optional().describe('Protocol type. Defaults to https.'),
    period: z.number().optional().describe('Check interval in seconds (15-3600 for regular, up to 2592000 for pulse)'),
    apdexT: z.number().optional().describe('APDEX performance threshold in seconds (0.125 to 8.0)'),
    enabled: z.boolean().optional().describe('Whether the check is active. Defaults to true.'),
    published: z.boolean().optional().describe('Whether the check status is publicly visible'),
    ...
    httpVerb: z.enum(['GET/HEAD', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS']).optional().describe('HTTP method to use'),
    httpBody: z.string().optional().describe('HTTP body to send with the request'),
    disabledLocations: z.array(z.string()).optional().describe('Monitoring location codes to disable'),
    recipients: z.array(z.string()).optional().describe('Alert recipient IDs to assign'),
    customHeaders: z.record(z.string(), z.string()).optional().describe('Custom HTTP headers to send with each check request'),
    muteUntil: z.string().optional().describe('Mute alerts until this ISO 8601 timestamp, "recovery", or "forever"'),
  }))
  .output(checkSchema)
