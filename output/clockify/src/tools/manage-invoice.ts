...
export let getInvoices = SlateTool.create(
  ...
  .input(z.object({
    status: z.enum(['UNSENT', 'SENT', 'PAID', 'PARTIALLY_PAID', 'VOID', 'OVERDUE']).optional().describe('Filter by invoice status'),
...
export let updateInvoice = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['UNSENT', 'SENT', 'PAID', 'PARTIALLY_PAID', 'VOID', 'OVERDUE']).optional().describe('Updated invoice status'),
