...
export let graphEvent = SlateTrigger.create(
  spec,
  {
    name: 'Graph Event',
    key: 'graph_event',
    description: 'Triggered when graph-related events occur, such as episode processing completion or batch ingestion completion.',
  }
)
  .input(z.object({
    ...
    episodeUuids: z.array(z.string()).optional().describe('Episode UUIDs from batch ingestion'),
  ...
  .output(z.object({
    ...
    graphType: z.string().optional().nullable().describe('Type of graph (user or standalone)'),
    ...
    episodeUuids: z.array(z.string()).optional().nullable().describe('Episode UUIDs from batch ingestion'),
  }))
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      let dedupParts = [eventType, ctx.input.graphId || ctx.input.userId || '', Date.now().toString()];
