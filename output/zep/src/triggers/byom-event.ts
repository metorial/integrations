...
export let byomEvent = SlateTrigger.create(
  spec,
  {
    name: 'BYOM Event',
    key: 'byom_event',
    description: 'Triggered when Bring Your Own Model (BYOM) events occur, such as rate limiting or request failures from your own LLM provider credentials.',
  }
)
  .input(z.object({
    ...
    firstOccurrence: z.string().optional().describe('Timestamp of first occurrence in window'),
  ...
  .output(z.object({
    eventName: z.string().describe('BYOM event name (byom.rate_limited or byom.request_failed)'),
    provider: z.string().optional().nullable().describe('LLM provider that triggered the event'),
    model: z.string().optional().nullable().describe('Model that triggered the event'),
    count: z.number().optional().nullable().describe('Number of occurrences in the 60-second aggregation window'),
    firstOccurrence: z.string().optional().nullable().describe('Timestamp of first occurrence'),
    lastOccurrence: z.string().optional().nullable().describe('Timestamp of last occurrence'),
    windowSeconds: z.number().optional().nullable().describe('Aggregation window duration in seconds'),
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      let eventType = ctx.input.eventName.includes('.') ? ctx.input.eventName : `byom.${ctx.input.eventName}`;

      let dedupId = [eventType, ctx.input.provider || '', ctx.input.model || '', ctx.input.lastOccurrence || Date.now().toString()].join('-');
