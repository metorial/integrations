...
export let setUserSummaryInstructions = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    instructions: z.array(z.object({
      ...
      text: z.string().describe('The instruction text describing how summaries should be generated'),
    })).min(1).describe('List of summary generation instructions'),
    userIds: z.array(z.string()).optional().describe('Specific user IDs to apply instructions to (empty applies project-wide)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let target = ctx.input.userIds?.length ? `${ctx.input.userIds.length} user(s)` : 'project-wide default';
    return {
      output: { success: true },
