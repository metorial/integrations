...
export let addMessages = SlateTool.create(
  spec,
  {
    name: 'Add Messages',
    key: 'add_messages',
  ...
  .input(z.object({
    ...
    })).min(1).describe('Array of messages to add'),
    ignoreRoles: z.array(z.string()).optional().describe('Roles to exclude from graph ingestion'),
    returnContext: z.boolean().optional().describe('Whether to return assembled context in the response'),
    batch: z.boolean().optional().describe('Use batch mode for concurrent processing of messages'),
  }))
  .output(z.object({
    context: z.string().optional().nullable().describe('Assembled context if returnContext was true'),
