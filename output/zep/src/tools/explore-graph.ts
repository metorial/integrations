...
export let exploreGraph = SlateTool.create(
  spec,
  {
    name: 'Explore Graph',
    key: 'explore_graph',
  ...
  .input(z.object({
    action: z.enum(['list_nodes', 'node_edges', 'node_episodes', 'get_edge', 'user_node']).describe('What to explore: list_nodes (all nodes for user), node_edges (edges of a node), node_episodes (episodes mentioning a node), get_edge (single edge), user_node (the user entity node)'),
    userId: z.string().optional().describe('User ID (required for list_nodes and user_node)'),
    nodeUuid: z.string().optional().describe('Node UUID (required for node_edges and node_episodes)'),
  ...
  .output(z.object({
    nodes: z.array(z.object({
      ...
      attributes: z.record(z.string(), z.unknown()).optional().nullable().describe('Node attributes'),
      createdAt: z.string().optional().nullable().describe('Creation timestamp'),
    })).optional().describe('List of graph nodes'),
    ...
    }).optional().nullable().describe('The user entity node'),
  }))
