...
export let extractTextWithCoordinates = SlateTool.create(
  spec,
  {
...
Useful for overlaying recognized text on the original image, building text annotation tools, or extracting text from specific regions.`,
  ...
  .input(z.object({
    ...
    ocrEngine: z.enum(['1', '2']).optional().default('1').describe('OCR engine to use: "1" (fastest) or "2" (complex backgrounds)'),
    ...
    isTable: z.boolean().optional().default(false).describe('Optimize for table-structured documents with line-by-line output'),
  }))
  .output(z.object({
    ...
    pages: z.array(z.object({
      ...
      lines: z.array(lineSchema).describe('Lines of text with word-level bounding box coordinates'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let totalWords = pages.reduce((sum, p) => sum + p.lines.reduce((lsum, l) => lsum + l.words.length, 0), 0);
