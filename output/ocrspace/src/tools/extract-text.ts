...
Enable table mode for structured documents like receipts and invoices.`,
  ...
  .input(z.object({
    ...
    language: languageEnum.optional().default('eng').describe('Language code for OCR recognition. Use "auto" for automatic detection (Engine 3 only).'),
    ocrEngine: z.enum(['1', '2', '3']).optional().default('1').describe('OCR engine to use: "1" (fastest), "2" (complex backgrounds), "3" (best quality)'),
    detectOrientation: z.boolean().optional().default(false).describe('Auto-rotate the image and report detected orientation'),
    scale: z.boolean().optional().default(false).describe('Upscale low-resolution images to improve OCR results'),
    isTable: z.boolean().optional().default(false).describe('Optimize for table-structured documents (receipts, invoices) with line-by-line output'),
    filetype: z.enum(['PDF', 'GIF', 'PNG', 'JPG', 'TIF', 'BMP']).optional().describe('Override auto-detected file type'),
  }))
  .output(z.object({
    ...
    ocrExitCode: z.number().describe('Overall OCR exit code (1 = fully parsed, 2 = partially parsed)'),
