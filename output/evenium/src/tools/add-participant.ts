...
export let addParticipant = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    firstName: z.string().optional().describe('First name (required if not providing contactId)'),
    lastName: z.string().optional().describe('Last name (required if not providing contactId)'),
    email: z.string().optional().describe('Email address (required if not providing contactId)'),
    gender: z.enum(['MALE', 'FEMALE']).optional().describe('Gender'),
    company: z.string().optional().describe('Company name'),
    status: z.enum(['CONFIRMED', 'UNANSWERED', 'CANCELED', 'DECLINED', 'RESERVED', 'OVERBOOKED', 'EXTRA', 'VALID', 'PENDING']).optional().describe('Initial registration status')
  }))
  .output(z.object({
