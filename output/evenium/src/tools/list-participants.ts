...
export let listParticipants = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['CONFIRMED', 'UNANSWERED', 'CANCELED', 'DECLINED', 'RESERVED', 'OVERBOOKED', 'EXTRA', 'VALID', 'PENDING']).optional().describe('Filter by registration status'),
    ...
    since: z.string().optional().describe('Only return participants modified since this date (RFC 3339 format)'),
    until: z.string().optional().describe('Only return participants modified until this date (RFC 3339 format)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let participants = result.guests.map((g) => ({
      ...
      categoryLabel: g.categoryLabel ?? (typeof g.category === 'object' ? g.category?.label : g.category)
    }));
