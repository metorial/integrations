...
let templateSchema = z.object({
  ...
  isServerless: z.boolean().nullable().describe('Whether template is for Serverless endpoints'),
...
export let listTemplates = SlateTool.create(
  ...
  .input(z.object({
    includePublicTemplates: z.boolean().optional().describe('Include community-made public templates'),
    includeRunpodTemplates: z.boolean().optional().describe('Include official RunPod templates'),
    includeEndpointBoundTemplates: z.boolean().optional().describe('Include templates bound to Serverless endpoints'),
  }))
  .output(z.object({
