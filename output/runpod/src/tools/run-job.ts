...
export let runJob = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    jobInput: z.record(z.string(), z.any()).describe('Input payload for the handler (model-specific parameters)'),
    synchronous: z.boolean().optional().describe('Run synchronously and wait for result (default: false)'),
    webhookUrl: z.string().optional().describe('URL to receive a POST callback when the job completes'),
    executionTimeout: z.number().optional().describe('Max execution time in ms once a worker picks up the job'),
  ...
  .output(z.object({
    ...
    status: z.string().describe('Job status: IN_QUEUE, IN_PROGRESS, COMPLETED, FAILED, CANCELLED, TIMED_OUT'),
    jobOutput: z.any().nullable().describe('Job output (only present for synchronous completed jobs)'),
    executionTime: z.number().nullable().describe('Execution time in ms (only for completed jobs)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let message = ctx.input.synchronous && result.status === 'COMPLETED'
