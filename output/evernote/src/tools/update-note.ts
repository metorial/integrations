...
export let updateNoteTool = SlateTool.create(
  spec,
  {
    name: 'Update Note',
    key: 'update_note',
  ...
  .input(z.object({
    ...
    content: z.string().optional().describe('New body content. Plain text, HTML fragment, or full ENML.'),
    notebookGuid: z.string().optional().describe('GUID of the notebook to move the note to'),
    tagNames: z.array(z.string()).optional().describe('Tag names to set on the note (replaces existing tags)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {
        ...
        title: note.title || '',
        updatedAt: note.updated ? new Date(note.updated).toISOString() : new Date().toISOString(),
      },
