...
export let noShowEvents = SlateTrigger.create(
  spec,
  {
    name: 'No-Show Detection Events',
    key: 'no_show_events',
    description: 'Triggers when a host or guest fails to join a Cal Video meeting within the configured time window after the booking start time.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    noShowType: z.enum(['host', 'guest']).describe('Whether the no-show is for a host or guest'),
    organizerEmail: z.string().optional().describe('Email of the organizer'),
    attendees: z.array(z.object({
      email: z.string().optional(),
      name: z.string().optional(),
    })).optional().describe('Attendees of the booking'),
  }))
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      let noShowType: 'host' | 'guest' = ctx.input.triggerEvent === 'AFTER_HOSTS_CAL_VIDEO_NO_SHOW' ? 'host' : 'guest';
