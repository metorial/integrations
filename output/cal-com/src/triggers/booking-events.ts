...
export let bookingEvents = SlateTrigger.create(
  spec,
  {
    name: 'Booking Events',
    key: 'booking_events',
    description: 'Triggers when a booking is created, rescheduled, cancelled, requested, rejected, has its no-show status updated, or when a payment is initiated or completed.',
  }
)
  .input(z.object({
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        type: typeMap[ctx.input.triggerEvent] || `booking.${ctx.input.triggerEvent.toLowerCase()}`,
