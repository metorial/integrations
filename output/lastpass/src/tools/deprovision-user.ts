...
export let deprovisionUser = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['deactivate', 'remove', 'delete']).default('deactivate').describe('Deprovision action: deactivate (block login), remove (leave enterprise), or delete (full removal)'),
  }))
  .output(z.object({
