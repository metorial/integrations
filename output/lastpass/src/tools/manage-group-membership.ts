...
export let manageGroupMembership = SlateTool.create(
  ...
  .input(z.object({
    changes: z.array(z.object({
      ...
      addToGroups: z.array(z.string()).optional().describe('Group names to add the user to'),
      removeFromGroups: z.array(z.string()).optional().describe('Group names to remove the user from'),
    })).min(1).describe('List of group membership changes'),
  }))
  .output(z.object({
