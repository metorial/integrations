...
export let createSpace = SlateTool.create(
  ...
  .input(z.object({
    ...
    isPublic: z.boolean().optional().describe('Make the space publicly discoverable in the organization'),
    description: z.string().optional().describe('Description for the space (required if public)'),
    isAnnouncementOnly: z.boolean().optional().describe('Only allow moderators to post messages')
  }))
...
export let updateSpace = SlateTool.create(
  ...
  .input(z.object({
    ...
    isPublic: z.boolean().optional().describe('Make the space publicly discoverable or private'),
    description: z.string().optional().describe('Updated description for the space'),
    isAnnouncementOnly: z.boolean().optional().describe('Enable or disable announcement-only mode'),
    isReadOnly: z.boolean().optional().describe('Make the space read-only or writable'),
    teamId: z.string().optional().describe('Assign the space to a team (only for unowned spaces)')
  }))
  .output(spaceOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new WebexClient({ token: ctx.auth.token });
