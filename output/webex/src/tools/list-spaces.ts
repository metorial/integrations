...
export let listSpaces = SlateTool.create(
  ...
  .input(z.object({
    ...
    sortBy: z.enum(['id', 'lastactivity', 'created']).optional().describe('Sort order for results'),
    max: z.number().optional().describe('Maximum number of spaces to return (default 100, max 1000)')
  }))
  .output(z.object({
