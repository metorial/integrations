...
export let listTemplates = SlateTool.create(
  ...
  .input(z.object({
    folder: z.string().optional().describe('Folder path to list templates from (e.g., "/invoices")'),
    includeSubFolders: z.boolean().optional().describe('Whether to include templates from subfolders (defaults to true)')
  }))
  .output(z.object({
