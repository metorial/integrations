...
export let searchOrders = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Date range can span up to 35 days',
      'Returns up to 500 orders per request',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    orderStatus: z.array(z.enum(['unscheduled', 'scheduled', 'success', 'failed', 'rejected', 'cancelled'])).optional().describe('Filter by order status(es)'),
    orders: z.array(z.object({
      orderNo: z.string().optional(),
      orderId: z.string().optional(),
    })).optional().describe('Specific orders to look up'),
    includeOrderData: z.boolean().optional().describe('Include full order data in results'),
    includeScheduleInformation: z.boolean().optional().describe('Include scheduling info (driver, stop number, times)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includeOrderData !== undefined) body.includeOrderData = ctx.input.includeOrderData;
    if (ctx.input.includeScheduleInformation !== undefined) body.includeScheduleInformation = ctx.input.includeScheduleInformation;
