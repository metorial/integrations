...
export let createScrapeTask = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    callbackUrl: z.string().optional().describe('Webhook URL to receive results when the task completes'),
    passthrough: z.string().optional().describe('Arbitrary string returned in the callback for verification'),
  }))
  .output(z.object({
...
export let getScrapeTaskResults = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    })).optional().describe('Scraping results if task is complete'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new ScrapingClient(ctx.auth.token);
