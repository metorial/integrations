...
export let getSubscriptions = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    subscriptions: z.array(z.object({
      ...
      trafficPerPeriod: z.number().optional().describe('Traffic allocated per billing period in GB'),
      usersLimit: z.number().optional().describe('Maximum number of sub-users'),
      ipAddressLimit: z.number().optional().describe('Maximum number of whitelisted IPs'),
      validFrom: z.string().optional().describe('Subscription start date'),
      validUntil: z.string().optional().describe('Subscription end date'),
      serviceType: z.string().optional().describe('Service type (e.g. residential_proxies, datacenter_proxies)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let summary = subscriptions.map((s) =>
