...
export let accountEvents = SlateTrigger.create(
  ...
  .output(z.object({
    eventType: z.string().describe('Categorized event type (e.g. payment.failed, usage.threshold_reached)'),
    ...
    usagePercent: z.number().optional().describe('Usage percentage if this is a usage threshold event'),
    subscriptionType: z.string().optional().describe('Subscription or service type if applicable'),
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      let eventId = String(data['id'] || data['event_id'] || `${normalizedType}-${ctx.input.receivedAt}`);
      ...
      return {
        ...
        output: {
          ...
          message: data['message'] != null ? String(data['message']) : (data['description'] != null ? String(data['description']) : undefined),
          username: data['username'] != null ? String(data['username']) : (data['user'] != null ? String(data['user']) : undefined),
          usagePercent: typeof data['usage_percent'] === 'number' ? data['usage_percent'] : (typeof data['usage_percentage'] === 'number' ? data['usage_percentage'] : undefined),
          subscriptionType: data['service_type'] != null ? String(data['service_type']) : (data['subscription_type'] != null ? String(data['subscription_type']) : undefined),
          rawPayload: data,
        },
      };
    },
