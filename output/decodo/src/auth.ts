...
export let auth = SlateAuth.create()
  .output(z.object({
    token: z.string().describe('Base64-encoded Basic Auth token (username:password) for Web Scraping API, or API key for Public API'),
    authMethod: z.enum(['basic', 'api_key']).describe('Which authentication method is being used'),
  }))
  .addCustomAuth({
    type: 'auth.custom',
    ...
    getProfile: async (ctx: { output: { token: string; authMethod: string }; input: { username: string; password: string } }) => {
  ...
  .addTokenAuth({
    ...
    key: 'api_key',
    inputSchema: z.object({
      token: z.string().describe('API key from the Decodo dashboard (Account Settings > API Keys)'),
    }),
    ...
    getProfile: async (ctx: { output: { token: string; authMethod: string }; input: { token: string } }) => {
