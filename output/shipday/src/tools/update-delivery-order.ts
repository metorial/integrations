...
export let updateDeliveryOrder = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    expectedDeliveryDate: z.string().optional().describe('Updated delivery date (yyyy-mm-dd)'),
    ...
    paymentMethod: z.enum(['cash', 'credit_card']).optional().describe('Updated payment method'),

    // Status update
    status: z.enum([
      'STARTED', 'PICKED_UP', 'READY_TO_DELIVER', 'ALREADY_DELIVERED', 'INCOMPLETE', 'FAILED_DELIVERY',
    ]).optional().describe('New order status'),
    ...
    unassignCarrier: z.boolean().optional().describe('Unassign the currently assigned carrier'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let editKeys = [
      ...
      'restaurantPhoneNumber', 'expectedDeliveryDate', 'expectedPickupTime',
      'expectedDeliveryTime', 'tip', 'tax', 'discountAmount', 'deliveryFee',
      'totalCost', 'deliveryInstruction', 'paymentMethod',
    ] as const;
    ...
    if (ctx.input.readyToPickup !== undefined) {
      ...
      actions.push(ctx.input.readyToPickup ? 'Marked as ready for pickup' : 'Unmarked ready for pickup');
    }
