...
let webhookPayloadSchema = z.object({
  ...
  thirdPartyDeliveryOrder: z.record(z.string(), z.unknown()).optional().describe('Third-party delivery data'),
}).passthrough();
...
export let orderStatusUpdate = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Receives real-time notifications when delivery order statuses change in Shipday, including order assignment, pickup, delivery, failure, and other lifecycle events.',
  }
)
  ...
  .output(z.object({
    ...
    event: z.string().optional().describe('Event type (e.g. ORDER_ASSIGNED, ORDER_COMPLETED)'),
    orderStatus: z.string().optional().describe('Current order status (e.g. PICKED_UP, ALREADY_DELIVERED)'),
    timestamp: z.number().optional().describe('Event timestamp'),
    ...
    proofOfDeliveryUrls: z.array(z.string()).optional().describe('Proof of delivery photo URLs'),
  }))
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: {
          ...
          customerName: (deliveryDetails.name ?? deliveryDetails.customer_name) as string | undefined,
