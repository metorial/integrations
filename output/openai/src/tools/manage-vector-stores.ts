...
export let createVectorStore = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    fileIds: z.array(z.string()).optional().describe('File IDs to add to the store on creation'),
    expiresAfterDays: z.number().optional().describe('Number of days after last activity before the store expires'),
    metadata: z.record(z.string(), z.string()).optional().describe('Key-value metadata to attach to the vector store'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.createVectorStore({
      ...
      expiresAfter: ctx.input.expiresAfterDays ? { anchor: 'last_active_at', days: ctx.input.expiresAfterDays } : undefined,
...
export let listVectorStores = SlateTool.create(
  spec,
  {
...
export let searchVectorStore = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    scoreThreshold: z.number().optional().describe('Minimum relevance score threshold (0-1)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.scoreThreshold !== undefined || ctx.input.ranker !== undefined) {
      ...
      if (ctx.input.scoreThreshold !== undefined) rankingOptions.score_threshold = ctx.input.scoreThreshold;
    }
...
export let deleteVectorStore = SlateTool.create(
  spec,
  {
