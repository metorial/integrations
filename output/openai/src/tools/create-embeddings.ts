...
export let createEmbeddings = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    model: z.string().optional().default('text-embedding-3-small').describe('Embedding model to use (e.g. "text-embedding-3-small", "text-embedding-3-large")'),
    input: z.union([z.string(), z.array(z.string())]).describe('Text or array of texts to embed'),
    dimensions: z.number().optional().describe('Number of output dimensions. Allows trading off quality for size.'),
    encodingFormat: z.enum(['float', 'base64']).optional().describe('Format of the returned embeddings'),
    user: z.string().optional().describe('Unique identifier for the end-user'),
  }))
  .output(z.object({
    embeddings: z.array(z.object({
      index: z.number().describe('Index of the input text this embedding corresponds to'),
