...
export let generateImage = SlateTool.create(
  ...
  .input(z.object({
    ...
    model: z.string().optional().describe('Model to use (e.g. "dall-e-3", "gpt-image-1"). Defaults to the latest available model.'),
    n: z.number().min(1).max(10).optional().describe('Number of images to generate (1-10)'),
    quality: z.enum(['standard', 'hd', 'low', 'medium', 'high', 'auto']).optional().describe('Quality of the generated image'),
    size: z.enum(['256x256', '512x512', '1024x1024', '1792x1024', '1024x1792']).optional().describe('Size of the generated image'),
    style: z.enum(['vivid', 'natural']).optional().describe('Style of the generated image (DALL·E 3 only)'),
    outputFormat: z.enum(['url', 'b64_json']).optional().describe('Format of the response. "url" returns temporary URLs, "b64_json" returns base64-encoded images.'),
  ...
  .output(z.object({
    images: z.array(z.object({
      ...
      revisedPrompt: z.string().optional().describe('Revised prompt used for generation (DALL·E 3)'),
