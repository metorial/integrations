...
let filterPropertySchema = z.object({
  property: z.string().describe('Property name to filter on (e.g. "name", "creationTime", "priority", "lifecycleStatus").'),
  operation: z.enum(['EQ', 'CONTAINS', 'NOT_EMPTY', 'GT', 'LT', 'GE', 'LE']).describe('Filter operation.'),
...
Use operations like EQ (equals), CONTAINS, GT/LT (greater/less than) for flexible querying.`,
  ...
  .input(z.object({
    filters: z.array(filterPropertySchema).optional().describe('Filter conditions to apply to the search.'),
    itemTypes: z.array(z.string()).optional().describe('Filter by specific item type identifiers to narrow results.'),
    sortAscending: z.boolean().optional().default(false).describe('Sort results in ascending order by creation time.'),
