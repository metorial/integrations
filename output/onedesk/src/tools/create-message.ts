...
Supports both internal team comments and customer-facing messages.`,
  ...
  .input(z.object({
    conversationExternalId: z.string().describe('External ID of the conversation to post the message to.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let messageId = typeof result === 'string' ? result : (result?.id || result?.externalId || String(result));
