...
export let addLeadToCampaign = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    deduplicate: z.boolean().optional().describe('Check if lead email exists in other campaigns before adding'),
    findEmail: z.boolean().optional().describe('Automatically find the lead email address (uses credits)'),
    verifyEmail: z.boolean().optional().describe('Verify the lead email address (uses credits)'),
    findPhone: z.boolean().optional().describe('Automatically find the lead phone number (uses credits)'),
    linkedinEnrichment: z.boolean().optional().describe('Enrich lead data from LinkedIn (uses credits)'),
    customVariables: z.record(z.string(), z.string()).optional().describe('Additional custom variables as key-value pairs')
  }))
  .output(z.object({
    leadId: z.string(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { campaignId, deduplicate, findEmail, verifyEmail, findPhone, linkedinEnrichment, customVariables, ...leadData } = ctx.input;
