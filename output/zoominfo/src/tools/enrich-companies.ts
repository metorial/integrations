...
export let enrichCompanies = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    matchBy: z.enum(['companyId', 'domain', 'name']).describe('How to match companies: by ZoomInfo company ID, website domain, or company name'),
    companyIds: z.array(z.number()).optional().describe('ZoomInfo company IDs (when matchBy is "companyId")'),
    domains: z.array(z.string()).optional().describe('Company website domains (when matchBy is "domain")'),
    companyNames: z.array(z.string()).optional().describe('Company names (when matchBy is "name")'),
    outputFields: z.array(z.string()).optional().describe('Specific fields to return (e.g., ["companyName", "revenue", "employeeCount", "industry", "website", "city", "state", "country"])'),
  }))
  .output(z.object({
    companies: z.array(z.record(z.string(), z.any())).describe('Enriched company records with full profile data'),
