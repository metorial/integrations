...
export let listSandboxes = SlateTool.create(
  spec,
  {
    name: 'List Sandboxes',
    key: 'list_sandboxes',
...
Supports pagination for large result sets.`,
  ...
  .input(z.object({
    state: z.array(z.enum(['running', 'paused'])).optional().describe('Filter sandboxes by state. Omit to list all.'),
    metadata: z.record(z.string(), z.string()).optional().describe('Filter sandboxes by metadata key-value pairs.'),
    limit: z.number().optional().describe('Maximum number of sandboxes to return (max 100).'),
  ...
  .output(z.object({
    ...
    nextToken: z.string().optional().describe('Token to fetch the next page of results, if more are available.'),
  }))
