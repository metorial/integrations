...
export let webSearch = SlateTool.create(
  ...
  .input(
    z.object({
      ...
      query: z.string().optional().describe('Search query (required for "search" and "search_and_scrape" actions)'),
      url: z.string().optional().describe('URL to scrape (required for "scrape" action)'),
      searchProvider: z.string().optional().describe('Search provider to use (e.g., "google")'),
      ...
      scrapeTopN: z.number().optional().describe('Number of top search results to scrape (for "search_and_scrape" action, default 3)'),
    }),
  )
  .output(
    z.object({
      ...
      })).optional().describe('Web search results'),
      ...
      })).optional().describe('Scraped page content'),
    }),
  )
  .handleInvocation(async (ctx) => {
    let client = createClient(ctx);
    let searchResults: Array<{ url: string; title: string; description?: string }> | undefined;
    let scrapedContent: Array<{ url: string; title?: string; textContent?: string; excerpt?: string }> | undefined;
    ...
    if (searchResults) {
      messageParts.push(`Found **${searchResults.length}** search result${searchResults.length !== 1 ? 's' : ''} for "${ctx.input.query}".`);
    }
    if (scrapedContent) {
      messageParts.push(`Scraped **${scrapedContent.length}** page${scrapedContent.length !== 1 ? 's' : ''}.`);
    }
