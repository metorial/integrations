...
export let sendMessage = SlateTool.create(
  ...
  .input(z.object({
    ...
    blocks: z.array(z.any()).optional().describe('Array of Block Kit block objects for rich message layouts'),
    threadTs: z.string().optional().describe('Timestamp of a parent message to reply in a thread'),
    replyBroadcast: z.boolean().optional().describe('When replying in a thread, also post to the channel'),
    ...
    ephemeral: z.boolean().optional().describe('If true, send an ephemeral message visible only to targetUserId'),
    targetUserId: z.string().optional().describe('User ID for ephemeral messages (required when ephemeral is true)'),
  }))
  .output(z.object({
