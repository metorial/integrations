...
Use this for visual previews, monitoring, or archiving webpage appearances.`,
  ...
  .input(z.object({
    ...
    format: z.enum(['jpeg', 'png', 'webp']).optional().describe('Image format. Defaults to jpeg.'),
    quality: z.number().optional().describe('Image quality from 10 to 80 (increments of 10). Defaults to 80.'),
    fullPage: z.boolean().optional().describe('Capture the entire scrollable page instead of just the viewport.'),
    dimensions: z.enum(['xs', 'sm', 'md', 'lg']).optional().describe('Viewport size preset: xs (mobile 375×812), sm (tablet 1024×768), md (laptop 1366×768), lg (desktop 1920×1080). Defaults to md.'),
    selector: z.string().optional().describe('CSS selector to capture a specific element instead of the full page.'),
    excludeSelectors: z.string().optional().describe('Comma-separated CSS selectors to hide from the screenshot.'),
    blockCookieBanner: z.boolean().optional().describe('Automatically hide cookie consent banners. Defaults to true.'),
    ...
    captureDelay: z.number().optional().describe('Wait time in milliseconds before capturing (0-10000).'),
    navigationTimeout: z.number().optional().describe('Page load timeout in milliseconds (1000-60000). Defaults to 30000.'),
  }))
  .output(z.object({
    screenshotUrl: z.string().describe('URL to the screenshot image (expires after 24 hours)'),
    dimensions: z.object({
      width: z.number().optional(),
      height: z.number().optional(),
    }).optional().describe('Dimensions of the captured screenshot'),
