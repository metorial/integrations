...
Use this to verify NFT ownership, analyze holder distribution, or check recent sales.`,
  ...
  .input(z.object({
    ...
    tokenId: z.string().optional().describe('Specific token ID. If provided, returns owners of that token. If omitted, returns all owners of the contract.'),
    withTokenBalances: z.boolean().optional().describe('Include token balance counts per owner (only for contract-level queries)'),
    checkSpam: z.boolean().optional().default(false).describe('Also check if the contract is classified as spam'),
  ...
  .output(z.object({
    owners: z.array(z.object({
      ...
      })).optional().describe('Token balances per owner'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.tokenId) {
      let result = await client.getOwnersForNFT(ctx.input.contractAddress, ctx.input.tokenId, ctx.input.pageKey);
    ...
    let result = await client.getOwnersForContract(ctx.input.contractAddress, ctx.input.withTokenBalances, ctx.input.pageKey);
