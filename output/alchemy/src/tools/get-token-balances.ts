...
Use this to check what tokens a wallet holds and their balances.`,
  ...
  .input(z.object({
    walletAddress: z.string().describe('The wallet address to get token balances for (e.g., 0x...)'),
    tokenAddresses: z.array(z.string()).optional().describe('Specific ERC-20 contract addresses to check. If omitted, returns all ERC-20 token balances.'),
    pageKey: z.string().optional().describe('Pagination key for fetching the next page of results'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: `Found **${tokenBalances.length}** token balance(s) for \`${ctx.input.walletAddress}\`${tokenBalances.length > 0 ? `: ${tokenBalances.slice(0, 5).map(t => `${t.symbol || t.contractAddress.slice(0, 10)}`).join(', ')}${tokenBalances.length > 5 ? '...' : ''}` : '.'}`,
    };
  })
  .build();
