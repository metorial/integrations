...
Use this to look up transaction history, track asset movements, or audit token flows for a wallet or between addresses.`,
  ...
  .input(z.object({
    ...
    category: z.array(z.enum(['external', 'internal', 'erc20', 'erc721', 'erc1155', 'specialnft'])).default(['external', 'internal', 'erc20', 'erc721', 'erc1155']).describe('Transfer types to include'),
    fromBlock: z.string().optional().describe('Start block in hex (e.g., "0x0") or "latest"'),
    toBlock: z.string().optional().describe('End block in hex or "latest"'),
    contractAddresses: z.array(z.string()).optional().describe('Filter to specific contract addresses'),
    maxCount: z.string().optional().describe('Max number of results to return in hex (e.g., "0x3e8" for 1000)'),
    order: z.enum(['asc', 'desc']).optional().describe('Sort order by block number'),
    withMetadata: z.boolean().optional().default(true).describe('Include block metadata (timestamp) with results'),
    excludeZeroValue: z.boolean().optional().default(true).describe('Exclude zero-value transfers'),
  ...
  .output(z.object({
    transfers: z.array(z.object({
      ...
      }).optional().describe('Raw contract details'),
      metadata: z.object({
        blockTimestamp: z.string().optional().describe('Block timestamp'),
      }).optional().describe('Block metadata'),
