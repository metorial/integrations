...
let lookupCommands = [
  'a', 'mx', 'dns', 'spf', 'dkim', 'dmarc', 'txt', 'ptr',
  'bimi', 'mta-sts', 'aa',
] as const;

export let runLookup = SlateTool.create(
  spec,
  {
    name: 'Run Lookup',
    key: 'run_lookup',
...
For DKIM lookups, provide the selector (e.g. "google") to query a specific DKIM record.`,
    ...
    constraints: [
      'Rate limits apply based on your MXToolbox subscription tier.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    command: z.enum(lookupCommands).describe('The type of lookup to perform (e.g., "mx", "spf", "dkim", "dmarc")'),
    hostname: z.string().describe('The domain name or IP address to look up'),
    selector: z.string().optional().describe('DKIM selector, only used when command is "dkim" (e.g., "google", "default")'),
  }))
  .output(z.object({
    ...
    hasSubscriptions: z.boolean().describe('Whether the queried domain has active monitor subscriptions'),
    ...
    relatedLookups: z.array(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let statusSummary = failedCount > 0
