...
export let postCallWebhook = SlateTrigger.create(
  spec,
  {
    name: 'Post-Call Webhook',
    key: 'post_call_webhook',
    description: 'Receives real-time notifications when a call is completed. Configure the webhook URL in your Synthflow agent or per-call API request using the external_webhook_url parameter.',
  }
)
  .input(z.object({
    callId: z.string().optional().describe('Unique call identifier'),
    callStatus: z.string().optional().describe('Call status (completed, busy, failed, no-answer, etc.)'),
    ...
    collectedVariables: z.record(z.string(), z.any()).optional().describe('Variables collected during the conversation'),
    actionResults: z.array(z.record(z.string(), z.any())).optional().describe('Results from executed custom actions'),
  ...
  .output(z.object({
    ...
    leadPhone: z.string().optional().describe('Lead phone number'),
    leadEmail: z.string().optional().describe('Lead email'),
    collectedVariables: z.record(z.string(), z.any()).optional().describe('Variables collected from conversation flows'),
    actionResults: z.array(z.record(z.string(), z.any())).optional().describe('Results from executed custom actions'),
  }))
  .webhook({
