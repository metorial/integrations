...
Supports phone numbers from 242 countries and territories.`,
  ...
  .input(z.object({
    phoneNumber: z.string().describe('Phone number to validate, in any format (e.g., "+12128675309", "212-867-5309")'),
    address: z.string().optional().describe('ISO country code, country name, or city name to help parse numbers without an international dialing code. Required if the number lacks a "+" prefix.'),
  }))
  .output(z.object({
    isValid: z.boolean().describe('Whether the number is valid per regional numbering rules'),
    isPossible: z.boolean().describe('Whether the number has a theoretically valid structure'),
    numberType: z.string().describe('Type of phone number: MOBILE, FIXED_LINE, FIXED_LINE_OR_MOBILE, VOIP, TOLL_FREE, PREMIUM_RATE, etc.'),
    ...
    isGeographical: z.boolean().describe('Whether the number is tied to a geographic location'),
