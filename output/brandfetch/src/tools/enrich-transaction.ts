...
export let enrichTransaction = SlateTool.create(
  spec,
  {
...
Returns the merchant's brand data including name, domain, logos, industry, and other brand assets.`,
  ...
  .input(z.object({
    transactionLabel: z.string().describe('Raw transaction descriptor text from a bank or credit card statement, e.g. "STARBUCKS 1523 OMAHA NE"'),
    countryCode: z.string().describe('ISO 3166-1 alpha-2 country code for the transaction locale, e.g. "US", "GB", "DE"'),
  }))
  .output(z.object({
    ...
    }).nullable().describe('Company firmographic data'),
  }))
