...
export let manageStorage = SlateTool.create(
  ...
  .input(z.object({
    ...
    bucket: z.string().optional().describe('Storage bucket name. Defaults to <projectId>.appspot.com.'),
    objectPath: z.string().optional().describe('Full object path (required for get, delete, copy)'),
    prefix: z.string().optional().describe('Prefix filter for listing objects (acts like folder path)'),
    delimiter: z.string().optional().describe('Delimiter for listing, typically "/" to list folder-like structure'),
    maxResults: z.number().optional().describe('Maximum objects to return in list. Defaults to 100.'),
    pageToken: z.string().optional().describe('Page token for list continuation'),
    destinationBucket: z.string().optional().describe('Destination bucket for copy. Defaults to source bucket.'),
    destinationObjectPath: z.string().optional().describe('Destination object path for copy'),
    customMetadata: z.record(z.string(), z.string()).optional().describe('Custom metadata to set on the object (get operation also updates if provided)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (operation === 'copy') {
      ...
      if (!ctx.input.destinationObjectPath) throw new Error('destinationObjectPath is required for copy operation');

      let destBucket = ctx.input.destinationBucket || bucket;
      let copiedObject = await client.copyObject(objectPath, destBucket, ctx.input.destinationObjectPath);
