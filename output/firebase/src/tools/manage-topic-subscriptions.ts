...
export let manageTopicSubscriptions = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    operation: z.enum(['subscribe', 'unsubscribe']).describe('Whether to subscribe or unsubscribe devices'),
    topic: z.string().describe('Topic name (without /topics/ prefix)'),
    deviceTokens: z.array(z.string()).min(1).describe('FCM registration tokens of devices to subscribe/unsubscribe'),
  }))
  .output(z.object({
