...
export let sendFcmMessage = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    deviceToken: z.string().optional().describe('FCM registration token of the target device'),
    topic: z.string().optional().describe('Topic name to send to (without /topics/ prefix)'),
    condition: z.string().optional().describe('Topic condition expression, e.g. "\'news\' in topics || \'weather\' in topics"'),
    title: z.string().optional().describe('Notification title'),
    body: z.string().optional().describe('Notification body text'),
    imageUrl: z.string().optional().describe('URL of an image to include in the notification'),
    dataPayload: z.record(z.string(), z.string()).optional().describe('Custom key-value data payload. Values must be strings.'),
    android: z.object({
    ...
    }).optional().describe('Android-specific configuration'),
    apns: z.object({
      headers: z.record(z.string(), z.string()).optional().describe('APNs headers, e.g. apns-priority, apns-expiration'),
      badge: z.number().optional().describe('App badge number'),
      sound: z.string().optional().describe('Sound to play'),
    }).optional().describe('Apple Push Notification service configuration'),
    webpush: z.object({
      headers: z.record(z.string(), z.string()).optional().describe('Webpush protocol headers'),
      link: z.string().optional().describe('URL to open when notification is clicked'),
    }).optional().describe('Web push configuration'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { deviceToken, topic, condition, title, body, imageUrl, dataPayload, android, apns, webpush } = ctx.input;
    ...
    if (android) {
      ...
      if (android.channelId || android.icon || android.color || android.sound || android.clickAction) {
