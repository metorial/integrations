...
export let queryFirestore = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    })).optional().describe('Query filters. If omitted, lists all documents.'),
    orderBy: z.array(z.object({
      field: z.string().describe('Field path to order by'),
      direction: z.enum(['ASCENDING', 'DESCENDING']).optional().describe('Sort direction. Defaults to ASCENDING.'),
    })).optional().describe('Ordering for results'),
    limit: z.number().optional().describe('Maximum number of documents to return'),
    pageSize: z.number().optional().describe('Page size for listing (without filters). Defaults to 20.'),
  ...
  .output(z.object({
    ...
    nextPageToken: z.string().optional().describe('Token for fetching the next page (list mode only)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.listDocuments(collectionPath, {
      ...
      pageToken,
      orderBy: orderBy?.[0] ? `${orderBy[0].field} ${(orderBy[0].direction || 'ASCENDING').toLowerCase()}` : undefined,
    });
