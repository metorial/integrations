...
export let updateRemoteConfig = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    operation: z.enum(['publish', 'rollback']).describe('Whether to publish a new template or roll back to a previous version'),
    etag: z.string().optional().describe('Current ETag from the template (required for publish)'),
    parameters: z.record(z.string(), z.object({
      defaultValue: z.union([
        z.object({ value: z.string() }),
        z.object({ useInAppDefault: z.boolean() }),
      ]).optional().describe('Default value for the parameter'),
      conditionalValues: z.record(z.string(), z.object({ value: z.string() })).optional().describe('Values conditioned on conditions'),
      description: z.string().optional().describe('Parameter description'),
      valueType: z.enum(['STRING', 'NUMBER', 'BOOLEAN', 'JSON']).optional().describe('Expected value type'),
    })).optional().describe('Parameters to publish (required for publish)'),
    ...
    })).optional().describe('Conditions to publish'),
    parameterGroups: z.record(z.string(), z.object({
      description: z.string().optional(),
      parameters: z.record(z.string(), z.any()),
    })).optional().describe('Parameter groups to publish'),
    rollbackVersionNumber: z.string().optional().describe('Version number to roll back to (required for rollback)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (operation === 'publish') {
      ...
      if (!ctx.input.parameters) throw new Error('parameters are required for publish operation');
    ...
    if (operation === 'rollback') {
      if (!ctx.input.rollbackVersionNumber) throw new Error('rollbackVersionNumber is required for rollback operation');
