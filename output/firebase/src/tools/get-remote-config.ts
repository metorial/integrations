...
export let getRemoteConfig = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    includeVersionHistory: z.boolean().optional().describe('Also fetch recent version history. Defaults to false.'),
    historyPageSize: z.number().optional().describe('Number of versions to return. Defaults to 10.'),
    historyPageToken: z.string().optional().describe('Page token for fetching more version history'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Recent version history'),
    historyNextPageToken: z.string().optional().describe('Token for fetching more version history'),
  }))
