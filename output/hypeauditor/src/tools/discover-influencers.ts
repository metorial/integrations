...
export let discoverInfluencers = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    socialNetwork: z.enum(['instagram', 'youtube', 'tiktok', 'twitter', 'twitch']).describe('Social network to search on'),
    page: z.number().optional().describe('Page number (1-500). Default: 1'),
    sandbox: z.boolean().optional().describe('Use sandbox mode for testing without consuming credits. Filters are ignored.'),

    // Text search
    searchKeywords: z.array(z.string()).optional().describe('Keywords to search across bio and content'),
    searchContent: z.array(z.string()).optional().describe('Keywords to search in content only'),
    searchDescription: z.array(z.string()).optional().describe('Keywords to search in bio/description only'),
    nicheSearch: z.string().optional().describe('Natural-language niche descriptor for AI-powered search (e.g., "home decor", "fitness coach")'),

    // Account filters
    accountGeo: z.object({
      country: z.array(z.string()).optional().describe('ISO country codes (e.g., ["us", "gb"])'),
      city: z.array(z.number()).optional().describe('City IDs'),
    }).optional().describe('Account location filter'),
    accountGender: z.enum(['male', 'female']).optional().describe('Account holder gender (Instagram, TikTok)'),
    accountAge: z.object({
      from: z.number().optional(),
      to: z.number().optional(),
    }).optional().describe('Account holder age range (Instagram only)'),
    accountLanguages: z.array(z.string()).optional().describe('ISO 639-1 language codes'),
    accountType: z.enum(['human']).optional().describe('Account type filter (Instagram only)'),
    hasContacts: z.boolean().optional().describe('Filter for accounts with contact info'),
    isVerified: z.number().optional().describe('Filter for verified accounts (1 = verified)'),

    // Audience filters
    audienceGeo: z.object({
    ...
    }).optional().describe('Audience geography filter'),
    audienceGender: z.object({
      gender: z.enum(['male', 'female']),
      percentage: z.number().describe('Minimum percentage'),
    }).optional().describe('Audience gender distribution filter'),
    audienceAge: z.object({
      groups: z.array(z.string()).describe('Age groups: 13_17, 18_24, 25_34, 35_44, 45_54, 55_64, 65'),
      percentage: z.number().describe('Minimum percentage'),
    }).optional().describe('Audience age distribution filter'),

    // Metric filters
    subscribersCount: z.object({
      from: z.number().optional(),
      to: z.number().optional(),
    }).optional().describe('Follower/subscriber count range'),
    engagementRate: z.object({
      from: z.number().optional(),
      to: z.number().optional(),
    }).optional().describe('Engagement rate range'),
    qualityScore: z.object({
      from: z.number().optional(),
      to: z.number().optional(),
    }).optional().describe('Account/Channel Quality Score range (0-100)'),
    viewsAvg: z.object({
      from: z.number().optional(),
      to: z.number().optional(),
    }).optional().describe('Average views range (YouTube, TikTok)'),
    priceRange: z.object({
      from: z.number().optional(),
      to: z.number().optional(),
    }).optional().describe('Post price range estimate'),
    ...
    growth: z.object({
      ...
      from: z.number().optional(),
      to: z.number().optional(),
    }).optional().describe('Follower growth rate percentage filter by period'),
    ...
    }).optional().describe('Content category filter (Instagram, TikTok)'),
  ...
  .output(z.object({
    results: z.array(z.any()).describe('Array of influencer results with basic info, metrics, and features'),
