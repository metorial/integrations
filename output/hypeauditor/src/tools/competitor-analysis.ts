...
export let competitorAnalysis = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    // For create
    username: z.string().optional().describe('Instagram username to analyze (required for create)'),
    dateFrom: z.string().optional().describe('Start date in YYYY-MM-DD format (required for create)'),
    dateTo: z.string().optional().describe('End date in YYYY-MM-DD format (required for create)'),
    audienceGeo: z.array(z.string()).optional().describe('ISO country codes for audience geography filter (e.g., ["us", "gb"])'),
    hashtags: z.array(z.string()).optional().describe('Hashtags to track (e.g., ["#nike", "#justdoit"])'),
  ...
  .output(z.object({
    report: z.any().optional().describe('Report data including EMV, reach, contributors, mentions, and content analysis'),
    posts: z.array(z.any()).optional().describe('Array of posts from the competitor analysis'),
    reportId: z.string().optional().describe('Report ID for tracking async report generation'),
