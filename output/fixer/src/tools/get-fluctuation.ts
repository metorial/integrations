...
export let getFluctuation = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    symbols: z.array(z.string()).optional().describe('List of target currency codes to retrieve fluctuation data for.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let mappedRates: Record<string, { startRate: number; endRate: number; change: number; changePct: number }> = {};
