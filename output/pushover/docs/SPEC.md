...
Note: Pushover's inbound webhooks (described in Features) are for *receiving* external data into Pushover, not for subscribing to Pushover events. There is no general-purpose event subscription or webhook system for listening to arbitrary Pushover events.