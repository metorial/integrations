...
export let manageGroup = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'get', 'rename']).describe('The group operation to perform'),
    groupKey: z.string().optional().describe('Group key (required for get and rename actions)'),
    name: z.string().optional().describe('Group name (required for create and rename actions)'),
  }))
  .output(z.object({
    groups: z.array(z.object({
      groupKey: z.string().describe('Group key identifier'),
      name: z.string().describe('Group name'),
    })).optional().describe('List of groups (returned by list action)'),
    groupKey: z.string().optional().describe('Group key (returned by create and get actions)'),
    groupName: z.string().optional().describe('Group name (returned by get action)'),
    members: z.array(groupMemberSchema).optional().describe('Group members (returned by get action)'),
  }))
