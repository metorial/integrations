...
export let manageGroupMembers = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['add', 'remove', 'disable', 'enable']).describe('The member operation to perform'),
    ...
    memo: z.string().optional().describe('Free-text memo about the member (max 200 characters, only for add action)'),
  }))
  .output(z.object({
