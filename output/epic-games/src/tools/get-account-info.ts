...
let accountInfoSchema = z.object({
  ...
  })).optional().describe('Linked external accounts'),
});
...
Supports batch lookups of up to 50 accounts in a single request. Requires the **basic_profile** scope via OAuth authentication.`,
  ...
  .input(z.object({
    accountIds: z.array(z.string()).min(1).max(50).describe('Epic Games account IDs to look up'),
  }))
  .output(z.object({
    accounts: z.array(accountInfoSchema).describe('Account information for the requested IDs'),
  }))
