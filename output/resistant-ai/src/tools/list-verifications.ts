...
export let listVerifications = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['pending', 'completed', 'expired', 'cancelled']).optional().describe('Filter by verification status'),
    fromDate: z.string().optional().describe('Filter verifications created on or after this date (ISO 8601 format)'),
    toDate: z.string().optional().describe('Filter verifications created on or before this date (ISO 8601 format)'),
  }))
  .output(z.object({
    ...
    totalCount: z.number().optional().describe('Total number of verifications matching the filter'),
