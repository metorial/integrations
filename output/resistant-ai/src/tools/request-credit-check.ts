...
export let requestCreditCheck = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    dateOfBirth: z.string().optional().describe('Date of birth in ISO 8601 format (YYYY-MM-DD)'),
    checkType: z.enum(['report', 'score', 'risk_alert']).optional().describe('Type of credit check: "report" for full credit report, "score" for credit score only, "risk_alert" for credit risk alerts. Defaults to "report"'),
