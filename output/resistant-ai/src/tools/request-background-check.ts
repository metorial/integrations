...
export let requestBackgroundCheck = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Only available for United States criminal records.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .output(z.object({
    ...
    email: z.string().describe('Email address of the individual'),
    reportUrl: z.string().optional().describe('URL to the background check report when completed'),
