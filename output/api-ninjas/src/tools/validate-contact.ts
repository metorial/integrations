...
export let validateContact = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'Provide either an email or a phone number (or both) to validate.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    phoneNumber: z.string().optional().describe('Phone number to validate (include country code, e.g. +14155552671)'),
  }))
  .output(z.object({
    emailValidation: z.object({
      ...
      isDisposable: z.boolean().optional().describe('Whether it is a disposable/temporary email'),
      hasMxRecords: z.boolean().optional().describe('Whether the domain has valid MX records'),
    }).optional().describe('Email validation results'),
    phoneValidation: z.object({
    ...
    }).optional().describe('Phone validation results'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.email) {
      promises.push(
        client.validateEmail(ctx.input.email).then(r => { emailResult = r; })
      );
    }
    if (ctx.input.phoneNumber) {
      promises.push(
        client.validatePhone(ctx.input.phoneNumber).then(r => { phoneResult = r; })
      );
    }
    ...
    if (emailResult) {
      ...
      messages.push(`Email **${ctx.input.email}**: ${emailResult.is_valid ? '✓ valid' : '✗ invalid'}${emailResult.is_disposable ? ' (disposable)' : ''}`);
    }
    ...
    if (phoneResult) {
      ...
      messages.push(`Phone **${ctx.input.phoneNumber}**: ${phoneResult.is_valid ? '✓ valid' : '✗ invalid'}${phoneResult.country ? ` (${phoneResult.country})` : ''}`);
    }
