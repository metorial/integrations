...
let syncSchema = z.object({
  ...
  configuration: z.record(z.string(), z.any()).describe('Sync configuration including field mappings'),
...
export let createSync = SlateTool.create(
  ...
  .input(z.object({
    ...
    configuration: z.record(z.string(), z.any()).describe('Sync configuration including field mappings (varies by destination type)'),
    disabled: z.boolean().optional().default(false).describe('Whether the sync should be created in a disabled state'),
    schedule: z.object({
    ...
    }).optional().describe('Optional schedule configuration'),
  }))
  .output(syncSchema)
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
...
export let updateSync = SlateTool.create(
  ...
  .input(z.object({
    ...
    configuration: z.record(z.string(), z.any()).optional().describe('Updated sync configuration'),
    disabled: z.boolean().optional().describe('Enable or disable the sync'),
    schedule: z.object({
      type: z.string().describe('Schedule type'),
      schedule: z.record(z.string(), z.any()).describe('Schedule configuration'),
    }).optional().describe('Updated schedule configuration'),
  }))
  .output(syncSchema)
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
