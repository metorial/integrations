...
export let triggerSync = SlateTool.create(
  ...
  .input(z.object({
    ...
    syncSlug: z.string().optional().describe('Slug of the sync to trigger (alternative to syncId)'),
    fullResync: z.boolean().optional().describe('Whether to resync all rows, ignoring previously synced rows'),
    resetCdc: z.boolean().optional().describe('Whether to sync all rows without executing changes on the destination'),
  }))
  .output(z.object({
...
export let triggerSyncSequence = SlateTool.create(
  spec,
  {
