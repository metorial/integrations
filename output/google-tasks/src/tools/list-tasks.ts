...
export let listTasks = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum of 20,000 non-hidden tasks per list.'
    ],
    tags: {
      readOnly: true
    }
  ...
  .input(z.object({
    ...
    completedMin: z.string().optional().describe('Lower bound for completion date (RFC 3339 format)'),
    completedMax: z.string().optional().describe('Upper bound for completion date (RFC 3339 format)'),
    updatedMin: z.string().optional().describe('Only return tasks modified after this time (RFC 3339 format)')
  }))
  .output(z.object({
