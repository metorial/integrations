...
export let createTask = SlateTool.create(
  ...
  .input(z.object({
    ...
    notes: z.string().optional().describe('Description/notes for the task (max 8192 characters)'),
    due: z.string().optional().describe('Due date in RFC 3339 format (time portion is discarded)'),
    status: z.enum(['needsAction', 'completed']).optional().describe('Task status (default: "needsAction")'),
    parentTaskId: z.string().optional().describe('Parent task ID to create this as a subtask'),
    previousTaskId: z.string().optional().describe('Sibling task ID to place this task after')
  }))
  .output(z.object({
