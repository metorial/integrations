...
let issueCategorySchema = z.object({
  ...
  })).optional().describe('Items within this category'),
});
...
export let getIssue = SlateTool.create(
  ...
  .input(z.object({
    ...
    issueNumber: z.string().describe('Issue number for published issues, or database ID for drafts'),
  ...
  .output(z.object({
    ...
    publishedAt: z.string().nullable().describe('ISO 8601 publication date, null for drafts'),
    updatedAt: z.string().describe('ISO 8601 last updated date'),
    categories: z.array(issueCategorySchema).optional().describe('Categories with their items'),
