...
export let countTokens = SlateTool.create(
  ...
  .input(z.object({
    ...
    tools: z.array(z.record(z.string(), z.unknown())).optional().describe('Tool definitions to include in the count'),
  }))
  .output(z.object({
    inputTokens: z.number().describe('Total number of tokens across messages, system prompt, and tools'),
  }))
