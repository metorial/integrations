...
export let createFeatureFlag = SlateTool.create(
  ...
  .input(z.object({
    ...
    temporary: z.boolean().optional().describe('Whether this is a temporary flag (default false)'),
    ...
    })).optional().describe('Custom variations. Omit for default boolean true/false.'),
    defaults: z.object({
      onVariation: z.number().describe('Index of the variation to serve when targeting is on'),
      offVariation: z.number().describe('Index of the variation to serve when the flag is off'),
    }).optional().describe('Default on/off variation indices'),
    clientSideAvailability: z.object({
      usingMobileKey: z.boolean().optional().describe('Make flag available to mobile SDKs'),
      usingEnvironmentId: z.boolean().optional().describe('Make flag available to client-side SDKs'),
    }).optional().describe('Client-side SDK availability settings'),
  }))
  .output(z.object({
