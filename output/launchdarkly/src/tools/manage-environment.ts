...
export let manageEnvironment = SlateTool.create(
  ...
  .input(z.object({
    ...
    color: z.string().optional().describe('Environment color hex code, e.g. "417505" (required for create)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update') {
      ...
      if (ctx.input.color !== undefined) patches.push({ op: 'replace', path: '/color', value: ctx.input.color });
      if (ctx.input.requireComments !== undefined) patches.push({ op: 'replace', path: '/requireComments', value: ctx.input.requireComments });
      if (ctx.input.confirmChanges !== undefined) patches.push({ op: 'replace', path: '/confirmChanges', value: ctx.input.confirmChanges });
      if (ctx.input.secureMode !== undefined) patches.push({ op: 'replace', path: '/secureMode', value: ctx.input.secureMode });
      if (ctx.input.defaultTrackEvents !== undefined) patches.push({ op: 'replace', path: '/defaultTrackEvents', value: ctx.input.defaultTrackEvents });
      if (ctx.input.tags !== undefined) patches.push({ op: 'replace', path: '/tags', value: ctx.input.tags });
