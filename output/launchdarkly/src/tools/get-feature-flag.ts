...
export let getFeatureFlag = SlateTool.create(
  ...
  .input(z.object({
    ...
    environmentKey: z.string().optional().describe('Environment key to scope the response to a specific environment'),
  }))
  .output(z.object({
    ...
    on: z.boolean().optional().describe('Whether the flag is on in the specified environment'),
    offVariationIndex: z.number().optional().describe('Off variation index in the specified environment'),
    prerequisites: z.array(z.object({
      flagKey: z.string(),
      variationIndex: z.number(),
    })).optional().describe('Prerequisite flags'),
    targets: z.array(z.object({
      contextKind: z.string().optional(),
      values: z.array(z.string()),
      variationIndex: z.number(),
    })).optional().describe('Individual targeting'),
    rules: z.array(z.object({
      ruleId: z.string().optional(),
      description: z.string().optional(),
      variationIndex: z.number().optional(),
      rollout: z.any().optional(),
    })).optional().describe('Targeting rules'),
    fallthrough: z.object({
      variationIndex: z.number().optional(),
      rollout: z.any().optional(),
    }).optional().describe('Default serve when targeting is on'),
  }))
