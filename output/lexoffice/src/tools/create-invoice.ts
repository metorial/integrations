...
}).describe('Address of the invoice recipient. Provide either contactId or inline address fields.');
...
export let createInvoice = SlateTool.create(
  ...
  .input(z.object({
    ...
    paymentConditions: paymentConditionsSchema.optional().describe('Payment conditions for the invoice'),
    ...
    voucherDate: z.string().optional().describe('Invoice date in ISO 8601 format (e.g. 2024-01-15). Defaults to today.'),
    finalize: z.boolean().optional().describe('If true, the invoice is finalized immediately and cannot be edited further'),
    precedingSalesVoucherId: z.string().optional().describe('ID of a preceding sales voucher (e.g. a quotation) to link to this invoice'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.paymentConditions) invoiceData.paymentConditions = ctx.input.paymentConditions;
    if (ctx.input.shippingConditions) invoiceData.shippingConditions = ctx.input.shippingConditions;
