...
}).describe('Address of the order confirmation recipient. Provide either contactId or inline address fields.');
...
export let createOrderConfirmation = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    lineItems: z.array(lineItemSchema).min(1).describe('Line items for the order confirmation'),
    ...
    paymentConditions: paymentConditionsSchema.optional().describe('Payment conditions for the order confirmation'),
    ...
    voucherDate: z.string().optional().describe('Order confirmation date in ISO 8601 format (e.g. 2024-01-15). Defaults to today.'),
    finalize: z.boolean().optional().describe('If true, the order confirmation is finalized immediately and cannot be edited further'),
    precedingSalesVoucherId: z.string().optional().describe('ID of a preceding sales voucher (e.g. a quotation) to link to this order confirmation'),
  }))
  .output(z.object({
    ...
    updatedDate: z.string().describe('Timestamp when the order confirmation was last updated'),
    version: z.number().describe('Version number of the order confirmation for optimistic locking'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.shippingConditions) orderData.shippingConditions = ctx.input.shippingConditions;
