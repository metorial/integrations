...
let eventSchema = z.object({
  ...
  title: z.record(z.string(), z.string()).describe('Event title in available languages (keyed by language code)'),
  ...
  description: z.record(z.string(), z.string()).describe('Event description in available languages (keyed by language code)'),
});
...
export let searchEvents = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum of 300 results per page.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    maxResults: z.number().optional().describe('Maximum number of results per page (default: 16, max: 300)'),
    dateRangeStart: z.string().optional().describe('Start date for filtering events (format: YYYY-MM-DD, default: today)'),
    dateRangeEnd: z.string().optional().describe('End date for filtering events (format: YYYY-MM-DD, default: furthest event date)'),
    coins: z.string().optional().describe('Comma-separated list of coin names to filter by (e.g., "bitcoin,ethereum")'),
    categories: z.string().optional().describe('Comma-separated list of category IDs to filter by (e.g., "1,2,3")'),
    sortBy: z.enum(['created_desc', 'hot_events']).optional().describe('Sort order for results'),
    showOnly: z.enum(['hot_events']).optional().describe('Filter to show only specific types of events'),
  }))
  .output(z.object({
    events: z.array(eventSchema).describe('List of cryptocurrency events matching the search criteria'),
