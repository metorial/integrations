...
export let manageCmsContent = SlateTool.create(
  spec,
  {
...
Content items are managed as structured data with typed properties. Use this to manage the full content lifecycle including publishing.`,
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'get', 'delete', 'list', 'publish', 'list_types', 'get_type']).describe('Action to perform'),
    contentKey: z.string().optional().describe('Content key/ID (required for get, update, delete, publish)'),
    contentTypeKey: z.string().optional().describe('Content type key (for create, get_type, or filtering list)'),
    parentKey: z.string().optional().describe('Parent content key (for create or filtering list)'),
    ...
    properties: z.record(z.string(), z.any()).optional().describe('Content properties (for create/update)'),
