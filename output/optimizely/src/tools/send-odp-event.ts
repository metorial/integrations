...
export let sendOdpEvent = SlateTool.create(
  spec,
  {
    name: 'Send ODP Event',
    key: 'send_odp_event',
...
Also supports querying historical events for a customer.`,
  ...
  .input(z.object({
    ...
    eventType: z.string().optional().describe('Event type (for send, e.g., "purchase", "pageview")'),
    eventAction: z.string().optional().describe('Event action (for send, e.g., "completed", "viewed")'),
    identifiers: z.record(z.string(), z.string()).optional().describe('Customer identifiers (for send/send_batch)'),
    eventData: z.record(z.string(), z.any()).optional().describe('Additional event data (for send)'),
    ...
    })).optional().describe('Batch of events to send (for send_batch)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'send_batch': {
        if (!ctx.input.events || ctx.input.events.length === 0) throw new Error('events array is required');
        await client.sendEvents(ctx.input.events.map(e => ({
          type: e.type,
          action: e.action,
