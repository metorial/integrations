...
export let manageCmpTask = SlateTool.create(
  spec,
  {
...
Tasks represent content work items that move through workflow stages. Use this to manage the full lifecycle of content tasks including commenting.`,
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'get', 'delete', 'list', 'add_comment', 'list_comments']).describe('Action to perform'),
    taskId: z.string().optional().describe('Task ID (required for get, update, delete, add_comment, list_comments)'),
    ...
    campaignId: z.string().optional().describe('Campaign to associate the task with (for create)'),
    assigneeId: z.string().optional().describe('User ID to assign (for create/update)'),
    dueDate: z.string().optional().describe('Due date in ISO 8601 format (for create/update)'),
    workflowId: z.string().optional().describe('Workflow ID (for create)'),
    customFields: z.record(z.string(), z.any()).optional().describe('Custom field values (for create/update)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'add_comment': {
        ...
        let comment = await client.createTaskComment(ctx.input.taskId, { body: ctx.input.commentBody });
        return {
          output: { comment },
