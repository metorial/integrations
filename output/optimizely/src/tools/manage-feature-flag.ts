...
export let manageFeatureFlag = SlateTool.create(
  spec,
  {
...
Use this to manage feature flag lifecycle including toggling flags on/off per environment and configuring rulesets.`,
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'get', 'delete', 'list', 'enable', 'disable', 'get_ruleset', 'update_ruleset']).describe('Action to perform'),
    projectId: z.number().describe('Project ID'),
    flagKey: z.string().optional().describe('Flag key (required for get, update, delete, enable, disable, get_ruleset, update_ruleset)'),
    environmentKey: z.string().optional().describe('Environment key (required for enable, disable, get_ruleset, update_ruleset)'),
    ...
    rulesetData: z.record(z.string(), z.any()).optional().describe('Ruleset configuration data (for update_ruleset)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'enable': {
        ...
        let ruleset = await client.enableFlag(ctx.input.projectId, ctx.input.flagKey, ctx.input.environmentKey);
        return {
          output: { ruleset },
      ...
      case 'disable': {
        if (!ctx.input.flagKey) throw new Error('flagKey is required');
        if (!ctx.input.environmentKey) throw new Error('environmentKey is required');
        let ruleset = await client.disableFlag(ctx.input.projectId, ctx.input.flagKey, ctx.input.environmentKey);
        return {
          output: { ruleset },
      ...
      case 'get_ruleset': {
        if (!ctx.input.flagKey) throw new Error('flagKey is required');
        if (!ctx.input.environmentKey) throw new Error('environmentKey is required');
        let ruleset = await client.listRulesets(ctx.input.projectId, ctx.input.flagKey, ctx.input.environmentKey);
        return {
          output: { ruleset },
      ...
      case 'update_ruleset': {
        ...
        let ruleset = await client.updateRuleset(ctx.input.projectId, ctx.input.flagKey, ctx.input.environmentKey, ctx.input.rulesetData);
        return {
          output: { ruleset },
