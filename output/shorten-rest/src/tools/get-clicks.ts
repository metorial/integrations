...
export let getClicks = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    cursor: z.string().optional().describe('Pagination cursor from a previous response to fetch the next page of clicks.'),
    limit: z.number().optional().describe('Maximum number of click records to return per page.'),
  }))
  .output(z.object({
    clicks: z.array(z.object({
      ...
      destination: z.string().describe('The destination URL the visitor was redirected to.'),
