...
let destinationSchema = z.object({
  ...
  country: z.string().optional().describe('Two-letter country code (e.g., "US", "GB") for geo-targeting. If omitted, this destination applies to all countries.'),
  os: z.string().optional().describe('Operating system for OS-targeting (e.g., "iOS", "Android", "Windows", "macOS", "Linux"). OS destinations take priority over country destinations.'),
});
...
let snippetSchema = z.object({
  snippetId: z.string().describe('Snippet identifier for the tracking pixel provider (e.g., Facebook, Google Analytics, Pinterest, Taboola).'),
  parameters: z.record(z.string(), z.string()).optional().describe('Key-value parameters specific to the pixel provider.'),
});

export let createAlias = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    aliasName: z.string().optional().describe('Custom alias path for the short URL (e.g., "my-link"). If omitted, a random 7-character string is generated. Supports the @rnd macro for partial randomization.'),
    domainName: z.string().optional().describe('Domain to create the alias on (e.g., "your.domain.com"). Defaults to "short.fyi" if omitted.'),
    destinations: z.array(destinationSchema).min(1).describe('One or more destination URLs. Include a default destination (no country/OS) and optional geo/OS-targeted destinations.'),
    metatags: z.array(metatagSchema).optional().describe('Custom meta tags for controlling how the short URL appears when shared on social media.'),
    snippets: z.array(snippetSchema).optional().describe('Tracking pixel snippets to fire when the short URL is clicked.'),
  }))
  .output(z.object({
