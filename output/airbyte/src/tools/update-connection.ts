...
export let updateConnectionTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    status: z.enum(['active', 'inactive', 'deprecated']).optional().describe('Connection status.'),
    schedule: z.object({
      scheduleType: z.enum(['manual', 'cron']).describe('Schedule type for the connection.'),
      cronExpression: z.string().optional().describe('Cron expression for automated syncs (UTC).'),
    }).optional().describe('Updated sync schedule.'),
    ...
    streams: z.array(streamConfigSchema).optional().describe('Full replacement stream configurations.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.namespaceDefinition !== undefined) body.namespaceDefinition = ctx.input.namespaceDefinition;
    if (ctx.input.namespaceFormat !== undefined) body.namespaceFormat = ctx.input.namespaceFormat;
    if (ctx.input.prefix !== undefined) body.prefix = ctx.input.prefix;
    if (ctx.input.nonBreakingSchemaUpdatesBehavior !== undefined) body.nonBreakingSchemaUpdatesBehavior = ctx.input.nonBreakingSchemaUpdatesBehavior;
