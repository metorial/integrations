...
export let createDestinationTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    destinationType: z.string().describe('The connector type (e.g. "bigquery", "snowflake", "postgres", "s3").'),
    configuration: z.record(z.string(), z.any()).describe('Destination-specific configuration including credentials and connection settings.'),
  }))
  .output(z.object({
