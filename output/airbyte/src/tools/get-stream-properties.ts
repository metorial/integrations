...
export let getStreamPropertiesTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    destinationId: z.string().optional().describe('UUID of the destination to check compatibility with.'),
    ignoreCache: z.boolean().optional().describe('If true, pull the latest schema from the source instead of using the cache.'),
  }))
  .output(z.object({
