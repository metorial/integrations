...
let transactionSchema = z.object({
  ...
  amount: z.number().describe('Transaction amount. Positive = money leaving account, negative = money entering.'),
  ...
  paymentChannel: z.string().optional().describe('Payment channel: in store, online, or other'),
  ...
  }).optional().describe('Transaction location'),
});

export let syncTransactionsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    cursor: z.string().optional().describe('Cursor from a previous sync response. Omit for initial sync.'),
    count: z.number().optional().describe('Max number of transactions to return per page (default 100, max 500)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.syncTransactions(ctx.input.accessToken, ctx.input.cursor, ctx.input.count);
