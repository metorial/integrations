...
let enrichedTransactionSchema = z.object({
  ...
  paymentChannel: z.string().nullable().optional().describe('Payment channel: in store, online, other'),
  ...
  categoryConfidence: z.string().nullable().optional().describe('Confidence: VERY_HIGH, HIGH, MEDIUM, LOW'),
  ...
  })).optional().describe('Identified counterparties'),
  location: z.object({
  ...
  }).optional().describe('Enriched location data'),
});

export let enrichTransactionsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    accountType: z.enum(['depository', 'credit']).describe('Type of account the transactions are from'),
