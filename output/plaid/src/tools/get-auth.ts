...
export let getAuthTool = SlateTool.create(
  spec,
  {
    name: 'Get Auth',
    key: 'get_auth',
  ...
  .input(z.object({
    ...
    accountIds: z.array(z.string()).optional().describe('Specific account IDs to retrieve auth data for'),
  }))
  .output(z.object({
    ...
    international: z.array(internationalNumberSchema).describe('International IBAN/BIC numbers'),
  }))
