...
export let itemWebhookTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Item Webhook',
    key: 'item_webhook',
    description: 'Receives webhook notifications about Plaid Item status changes, including errors, credential updates needed, pending expirations, and login repairs.',
  }
)
  .input(z.object({
    webhookType: z.string().describe('Webhook type (e.g. ITEM)'),
    webhookCode: z.string().describe('Webhook code (e.g. ERROR, PENDING_EXPIRATION, LOGIN_REPAIRED)'),
    itemId: z.string().describe('Plaid Item ID'),
    error: z.any().nullable().optional().describe('Error details if applicable'),
    consentExpirationTime: z.string().nullable().optional().describe('Consent expiration time if applicable'),
    environment: z.string().optional().describe('Plaid environment'),
  }))
  .output(z.object({
    itemId: z.string().describe('Affected Item ID'),
    webhookCode: z.string().describe('Event code: ERROR, PENDING_EXPIRATION, LOGIN_REPAIRED, USER_PERMISSION_REVOKED, USER_ACCOUNT_REVOKED, NEW_ACCOUNTS_AVAILABLE'),
    errorCode: z.string().nullable().optional().describe('Plaid error code if in error state'),
    ...
    consentExpirationTime: z.string().nullable().optional().describe('When consent expires (ISO 8601)'),
