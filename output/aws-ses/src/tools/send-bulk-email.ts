...
export let sendBulkEmail = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    defaultTemplateData: z.string().optional().describe('Default JSON string of template replacement data'),
    ...
    defaultTags: z.array(z.object({
      name: z.string(),
      value: z.string(),
    })).optional().describe('Default message tags applied to all entries'),
    entries: z.array(z.object({
      toAddresses: z.array(z.string()).describe('To recipient email addresses for this entry'),
      ...
      replacementTemplateData: z.string().optional().describe('Per-recipient JSON template data overriding defaults'),
      replacementTags: z.array(z.object({
        name: z.string(),
        value: z.string(),
      })).optional().describe('Per-recipient message tags'),
    })).describe('List of bulk email entries, one per recipient group'),
  }))
  .output(z.object({
    bulkEmailEntryResults: z.array(z.object({
