...
export let manageEmailTemplate = SlateTool.create(
  spec,
  {
...
Use the **testRender** action to preview how a template renders with sample data.`,
  ...
  .input(z.object({
    action: z.enum(['create', 'get', 'update', 'delete', 'list', 'testRender']).describe('Operation to perform'),
    templateName: z.string().optional().describe('Template name (required for all actions except "list")'),
    ...
    templateData: z.string().optional().describe('JSON string of sample data for test rendering'),
  ...
  .output(z.object({
    ...
    renderedTemplate: z.string().optional().describe('Rendered template output from testRender'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'testRender') {
      let result = await client.testRenderEmailTemplate(ctx.input.templateName!, ctx.input.templateData || '{}');
      return {
        output: {
          templateName: ctx.input.templateName,
