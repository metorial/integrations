...
export let manageEventDestination = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    configurationSetName: z.string().describe('Configuration set to manage event destinations for'),
    eventDestinationName: z.string().optional().describe('Event destination name (required for create/delete)'),
    matchingEventTypes: z.array(z.string()).optional().describe('Event types to publish (required for create)'),
    ...
    cloudWatchDimensions: z.array(z.object({
      dimensionName: z.string(),
      dimensionValueSource: z.enum(['MESSAGE_TAG', 'EMAIL_HEADER', 'LINK_TAG']),
      defaultDimensionValue: z.string(),
    })).optional().describe('CloudWatch dimension configurations'),
  }))
  .output(z.object({
    eventDestinations: z.array(z.object({
      name: z.string(),
      enabled: z.boolean(),
    ...
    })).optional().describe('List of event destinations'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      await client.createConfigurationSetEventDestination({
        ...
        snsDestination: ctx.input.snsTopicArn ? { topicArn: ctx.input.snsTopicArn } : undefined,
        eventBridgeDestination: ctx.input.eventBusArn ? { eventBusArn: ctx.input.eventBusArn } : undefined,
    ...
    if (action === 'list') {
      let result = await client.getConfigurationSetEventDestinations(ctx.input.configurationSetName);
      return {
