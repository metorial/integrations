...
export let eventTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Event Changes',
    key: 'event_changes',
    description: 'Triggers when an event is created or edited in WaiverFile. Covers both new events and edits to existing events.',
  }
)
  .input(z.object({
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      let eventType: 'newevent' | 'editevent' = lastSegment === 'editevent' ? 'editevent' : 'newevent';
    ...
    handleEvent: async (ctx) => {
      ...
      let waiverEventId = payload?.WaiverEventID ?? payload?.waiverEventID ?? String(Date.now());
      let lastModified = payload?.LastModified ?? payload?.lastModified ?? undefined;

      let dedupId = eventType === 'editevent'
