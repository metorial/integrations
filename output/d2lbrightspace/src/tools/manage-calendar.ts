...
export let listCalendarEvents = SlateTool.create(
  spec,
  {
...
export let createCalendarEvent = SlateTool.create(
  spec,
  {
...
export let updateCalendarEvent = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.startDateTime !== undefined) updateData.StartDateTime = ctx.input.startDateTime;
    if (ctx.input.endDateTime !== undefined) updateData.EndDateTime = ctx.input.endDateTime;

    let result = await client.updateCalendarEvent(ctx.input.orgUnitId, ctx.input.calendarEventId, updateData);
