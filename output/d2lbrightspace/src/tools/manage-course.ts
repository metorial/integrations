  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {
        ...
        path: course.Path,
        courseTemplateId: course.CourseTemplate?.Identifier ? String(course.CourseTemplate.Identifier) : undefined,
        semesterId: course.Semester?.Identifier ? String(course.Semester.Identifier) : undefined,
        departmentId: course.Department?.Identifier ? String(course.Department.Identifier) : undefined,
...
export let createCourse = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    let course = await client.createCourse({
      ...
      Description: ctx.input.description ? { Content: ctx.input.description, Type: 'Html' } : null,
...
export let updateCourse = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.canSelfRegister !== undefined) updateData.CanSelfRegister = ctx.input.canSelfRegister;
