...
export let listGroupsAndSections = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    groupCategoryId: z.string().optional().describe('If provided, lists groups within this category'),
    listSections: z.boolean().optional().describe('If true, lists sections instead of groups'),
  }))
  .output(z.object({
    groupCategories: z.array(z.object({
    ...
    })).optional().describe('Group categories (when no groupCategoryId provided)'),
    ...
    })).optional().describe('Groups within a category'),
...
export let manageGroupEnrollment = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'enroll') {
      await client.enrollUserInGroup(ctx.input.orgUnitId, ctx.input.groupCategoryId, ctx.input.groupId, ctx.input.userId);
      return {
        output: { success: true },
    ...
    await client.removeUserFromGroup(ctx.input.orgUnitId, ctx.input.groupCategoryId, ctx.input.groupId, String(ctx.input.userId));
    return {
      output: { success: true },
