...
export let createAnnouncement = SlateTool.create(
  ...
  .input(z.object({
    ...
    isPublished: z.boolean().optional().default(true).describe('Whether to publish immediately'),
    isPinned: z.boolean().optional().describe('Whether to pin the announcement'),
    startDate: z.string().optional().describe('When the announcement becomes visible (ISO 8601)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.showAuthorInfo !== undefined) newsData.IsAuthorInfoShown = ctx.input.showAuthorInfo;
...
export let updateAnnouncement = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.body !== undefined) updateData.Body = { Content: ctx.input.body, Type: 'Html' };
    ...
    let result = await client.updateNewsItem(ctx.input.orgUnitId, ctx.input.newsItemId, updateData);
