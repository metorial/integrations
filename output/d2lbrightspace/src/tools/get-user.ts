...
export let getUser = SlateTool.create(
  ...
  .input(z.object({
    userId: z.string().optional().describe('Brightspace user ID to retrieve. If omitted, returns the current authenticated user.'),
    userName: z.string().optional().describe('Look up a user by their username'),
    externalEmail: z.string().optional().describe('Look up a user by their external email address'),
    orgDefinedId: z.string().optional().describe('Look up a user by their organization-defined ID'),
  }))
  .output(z.object({
