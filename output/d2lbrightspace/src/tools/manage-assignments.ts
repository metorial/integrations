...
let assignmentSchema = z.object({
  ...
  submissionType: z.number().optional().describe('Submission type (0=File, 1=Text, 2=OnPaper, 3=Observed, 4=FileOrText)'),
...
export let getAssignment = SlateTool.create(
  spec,
  {
    name: 'Get Assignment',
    key: 'get_assignment',
...
export let createAssignment = SlateTool.create(
  ...
  .input(z.object({
    ...
    submissionType: z.number().optional().default(0).describe('Submission type: 0=File, 1=Text, 2=OnPaper, 3=Observed, 4=FileOrText'),
...
export let getAssignmentSubmissions = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    submissions: z.array(z.object({
      ...
      status: z.number().optional().describe('Status (0=Unsubmitted, 1=Submitted, 2=Draft, 3=Published)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result: any;
    if (ctx.input.userId) {
      result = await client.getDropboxUserSubmissions(ctx.input.orgUnitId, ctx.input.folderId, ctx.input.userId);
    } else {
