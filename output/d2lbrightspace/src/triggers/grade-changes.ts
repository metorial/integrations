...
export let gradeChanges = SlateTrigger.create(
  spec,
  {
    name: 'Grade Changes',
    key: 'grade_changes',
    description: 'Triggers when grade values are updated for students in a course. Monitors grade items in a specified org unit and detects changes to student scores.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let enrollmentItems = enrollments?.Items || (Array.isArray(enrollments) ? enrollments : []);

      // Limit to first 5 org units
      let orgUnitIds = enrollmentItems.slice(0, 5).map((e: any) => String(e.OrgUnit?.Id)).filter(Boolean);
      ...
      for (let orgUnitId of orgUnitIds) {
        try {
          ...
          let gradeItems = Array.isArray(gradeObjects) ? gradeObjects : (gradeObjects?.Objects || []);
          ...
          for (let gradeItem of gradeItems.slice(0, 5)) {
            ...
            try {
              ...
              for (let gv of gradeValues) {
                ...
                if (state?.gradeSnapshots && state.gradeSnapshots[key] && state.gradeSnapshots[key] !== snapshotValue) {
                  inputs.push({
