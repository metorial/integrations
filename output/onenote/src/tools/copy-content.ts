...
export let copyContent = SlateTool.create(
  spec,
  {
    name: 'Copy Content',
    key: 'copy_content',
  ...
  .input(z.object({
    resourceType: z.enum(['notebook', 'section', 'page']).describe('Type of resource to copy'),
    resourceId: z.string().describe('The ID of the notebook, section, or page to copy'),
    destinationSectionId: z.string().optional().describe('Destination section ID (for copying pages)'),
    destinationNotebookId: z.string().optional().describe('Destination notebook ID (for copying sections)'),
    destinationSectionGroupId: z.string().optional().describe('Destination section group ID (for copying sections)'),
    renameAs: z.string().optional().describe('New name for the copied resource (notebooks and sections only)'),
