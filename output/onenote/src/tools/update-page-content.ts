...
export let updatePageContent = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    operations: z.array(z.object({
      target: z.string().describe('Target element identifier, e.g. "#element-id", "body", or "title"'),
      action: z.enum(['replace', 'append', 'delete', 'insert', 'prepend']).describe('The type of patch operation'),
      content: z.string().optional().describe('HTML content for replace, append, insert, or prepend actions'),
      position: z.enum(['after', 'before']).optional().describe('Position for insert action relative to the target'),
