...
export let createSectionGroup = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    parentSectionGroupId: z.string().optional().describe('The ID of the parent section group for nesting'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.parentSectionGroupId) {
      sectionGroup = await client.createNestedSectionGroup(ctx.input.parentSectionGroupId, ctx.input.displayName);
    } else {
      sectionGroup = await client.createSectionGroup(ctx.input.notebookId!, ctx.input.displayName);
    }
