...
export let createProduct = SlateTool.create(
  ...
  .input(z.object({
    ...
    batchId: z.string().optional().describe('Batch ID to associate the product with for bulk operations.'),
    description: z.string().optional().describe('Text description of the product.'),
    tags: z.array(z.string()).optional().describe('Labels for categorizing and searching products.'),
    dimensions: z.string().optional().describe('Physical dimensions of the product.'),
    externalUrl: z.string().optional().describe('Link to the product page on your eCommerce store.'),
  }))
  .output(z.object({
