...
export let createReservation = SlateTool.create(
  spec,
  {
...
Supports custom form fields, voucher codes, pricing overrides, and shopping cart grouping.`,
  ...
  .input(z.object({
    ...
    startTime: z.string().describe('Rental start time (e.g. "2024-06-15 10:00" or Unix timestamp)'),
    endTime: z.string().describe('Rental end time (e.g. "2024-06-15 14:00" or Unix timestamp)'),
    quantity: z.number().describe('Number of units to reserve'),
    email: z.string().optional().describe('Customer email address'),
    firstName: z.string().optional().describe('Customer first name'),
    lastName: z.string().optional().describe('Customer last name'),
    userId: z.string().optional().describe('Existing Planyo user ID (alternative to email/firstName)'),
    ...
    adminMode: z.boolean().optional().describe('Bypass form field requirements and constraints'),
    sendNotifications: z.boolean().optional().describe('Send email notifications to the customer (default: true)'),
    ...
    customFields: z.record(z.string(), z.string()).optional().describe('Custom reservation form fields as key-value pairs'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {
        ...
        originalPrice: result.original_price != null ? Number(result.original_price) : undefined,
