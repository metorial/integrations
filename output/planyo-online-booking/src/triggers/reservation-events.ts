...
export let reservationEvents = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers on reservation lifecycle events including creation, confirmation, cancellation, modification, check-in/out, no-show, and price updates.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    isCustomerTriggered: z.boolean().optional().describe('Whether the event was triggered by the customer'),
    previousStatus: z.string().optional().describe('Previous status (for cancellation events)'),
    confirmationMethod: z.string().optional().describe('Confirmation method (for confirmation events)'),
  }))
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: {
          ...
          isCustomerTriggered: raw.is_event_fired_by_customer === '1' || raw.is_event_fired_by_customer === 1,
