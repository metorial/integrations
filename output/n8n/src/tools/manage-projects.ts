...
export let manageProjects = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'update', 'delete', 'list_members', 'add_members', 'remove_member']).describe('The project operation to perform'),
    projectId: z.string().optional().describe('Project ID (required for update, delete, and member operations)'),
    ...
    })).optional().describe('Members to add (for add_members action)'),
  ...
  .output(z.object({
    ...
    project: z.object({
      ...
      createdAt: z.string().optional().describe('Creation timestamp'),
      updatedAt: z.string().optional().describe('Last update timestamp')
    }).optional().describe('Single project result'),
    members: z.array(z.object({
      userId: z.string().describe('User ID'),
      email: z.string().optional().describe('User email'),
      role: z.string().optional().describe('User role in the project')
    })).optional().describe('Project members'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'list': {
        let result = await client.listProjects({ limit: ctx.input.limit, cursor: ctx.input.cursor });
      ...
      case 'update': {
        if (!ctx.input.projectId) throw new Error('projectId is required for updating a project');
      ...
      case 'delete': {
        if (!ctx.input.projectId) throw new Error('projectId is required for deleting a project');
      ...
      case 'list_members': {
        ...
        let result = await client.listProjectMembers(ctx.input.projectId, { limit: ctx.input.limit, cursor: ctx.input.cursor });
      ...
      case 'add_members': {
        ...
        if (!ctx.input.members || ctx.input.members.length === 0) throw new Error('members array is required');
      ...
      case 'remove_member': {
        if (!ctx.input.projectId) throw new Error('projectId is required for removing a member');
