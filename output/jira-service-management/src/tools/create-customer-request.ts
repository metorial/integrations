...
export let createCustomerRequestTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    requestFieldValues: z.record(z.string(), z.any()).describe('Field values for the request (e.g., { "summary": "...", "description": "..." })'),
    raiseOnBehalfOf: z.string().optional().describe('Account ID of the customer to raise the request on behalf of'),
    requestParticipants: z.array(z.string()).optional().describe('Account IDs to add as request participants'),
  }))
  .output(z.object({
