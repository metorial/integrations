...
export let getSlaInformationTool = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    slaMetrics: z.array(z.object({
      ...
      completedCycles: z.array(z.object({
      ...
      })).optional().describe('Completed SLA cycles'),
      ongoingCycle: z.object({
        startTime: z.string().optional().describe('Cycle start time'),
        breached: z.boolean().optional().describe('Whether the SLA has been breached'),
        paused: z.boolean().optional().describe('Whether the SLA cycle is currently paused'),
        withinCalendarHours: z.boolean().optional().describe('Whether currently within calendar hours'),
        goalDuration: z.any().optional().describe('SLA goal duration'),
        elapsedTime: z.any().optional().describe('Elapsed time'),
        remainingTime: z.any().optional().describe('Remaining time'),
      }).optional().describe('Currently ongoing SLA cycle'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let breachedCount = slaMetrics.filter((s: any) =>
      s.ongoingCycle?.breached || s.completedCycles?.some((c: any) => c.breached)
    ).length;
