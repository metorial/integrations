...
export let auth = SlateAuth.create()
  ...
  .addOauth({
    ...
    scopes: [
      {
        title: 'Read Requests',
        description: 'Read customer requests, approvals, attachments, comments, SLAs, and request types',
  ...
  .addCustomAuth({
    ...
    inputSchema: z.object({
      ...
      siteDomain: z.string().describe('Your Atlassian site domain (e.g., "yoursite" from yoursite.atlassian.net)'),
    }),

    getOutput: async (ctx: { input: { email: string; token: string; siteDomain: string } }) => {
    ...
    getProfile: async (ctx: { output: { token: string; refreshToken?: string; expiresAt?: string }; input: { email: string; token: string; siteDomain: string } }) => {
      let ax = createAxios({
