...
export let batchCreateOrUpdateTool = SlateTool.create(spec, {
  ...
  .input(
    z.object({
      ...
      conflictField: z
        .string()
        .describe('Fully qualified field name used for duplicate detection (e.g., "Project/ExternalId")'),
      conflictAction: z
        .enum(['update-latest', 'skip-create'])
        .default('update-latest')
        .describe('What to do when a duplicate is found: "update-latest" to update, "skip-create" to skip'),
    })
  )
  .output(
    z.object({
