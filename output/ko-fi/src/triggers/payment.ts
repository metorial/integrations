...
let shopItemSchema = z.object({
  ...
  variationName: z.string().nullable().optional().describe('Name of the product variation selected, if any'),
...
export let paymentTrigger = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers when a payment is made on your Ko-fi page, including donations, subscriptions, shop orders, and commissions.',
  }
)
  .input(z.object({
    ...
    isFirstSubscriptionPayment: z.boolean().describe('Whether this is the first payment of a subscription'),
    kofiTransactionId: z.string().describe('Unique Ko-fi transaction ID'),
    shopItems: z.array(shopItemSchema).nullable().describe('Shop items included in a shop order, null for non-shop events'),
    tierName: z.string().nullable().describe('Membership tier name for subscription payments, null otherwise'),
    shipping: shippingSchema.nullable().describe('Shipping details for physical product orders, null otherwise'),
  }))
  .output(z.object({
    kofiTransactionId: z.string().describe('Unique Ko-fi transaction ID'),
    paymentType: paymentEventType.describe('The type of payment: Donation, Subscription, Shop Order, or Commission'),
    fromName: z.string().describe('Name of the supporter who made the payment'),
    email: z.string().describe('Email address of the supporter'),
    message: z.string().nullable().describe('Message left by the supporter, null if private or empty'),
    ...
    isPublic: z.boolean().describe('Whether the supporter made the payment publicly visible'),
    isSubscriptionPayment: z.boolean().describe('Whether this payment is part of a subscription'),
    isFirstSubscriptionPayment: z.boolean().describe('Whether this is the first payment in a new subscription'),
    tierName: z.string().nullable().describe('Name of the membership tier, if applicable'),
    shopItems: z.array(shopItemSchema).nullable().describe('Array of shop items purchased, null for non-shop events'),
    shipping: shippingSchema.nullable().describe('Shipping information for physical product orders'),
    timestamp: z.string().describe('ISO 8601 timestamp of the payment event'),
  }))
  .webhook({
