...
let searchResultSchema = z.object({
  ...
  }).passthrough().optional().describe('Echo of the query parameters used'),
  organicResults: z.array(organicResultSchema).optional().describe('Organic search results'),
  ads: z.array(z.record(z.string(), z.any())).optional().describe('Paid advertisement results'),
  peopleAlsoAsk: z.array(z.record(z.string(), z.any())).optional().describe('People Also Ask questions'),
  relatedSearches: z.array(z.record(z.string(), z.any())).optional().describe('Related search suggestions'),
  knowledgeGraph: z.record(z.string(), z.any()).optional().describe('Knowledge panel data'),
  answerBox: z.record(z.string(), z.any()).optional().describe('Answer box / featured snippet'),
}).passthrough();
...
export let webSearch = SlateTool.create(
  ...
  .input(z.object({
    ...
    searchEngine: z.enum(['google', 'bing', 'yandex', 'duckduckgo']).optional().describe('Search engine to use (defaults to Google)'),
    location: z.string().optional().describe('Location for geotargeted results, e.g. "New York, United States"'),
    language: z.string().optional().describe('Language code (hl parameter), e.g. "en" for English'),
    country: z.string().optional().describe('Country code (gl parameter), e.g. "us" for United States'),
