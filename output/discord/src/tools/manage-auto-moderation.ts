...
let actionSchema = z.object({
  type: z.number().describe('The action type: 1 = BLOCK_MESSAGE, 2 = SEND_ALERT_MESSAGE, 3 = TIMEOUT'),
  metadata: z.record(z.string(), z.any()).optional().describe('Additional metadata for the action. For SEND_ALERT_MESSAGE: { channel_id }. For TIMEOUT: { duration_seconds }.'),
}).describe('Auto moderation action object');

let triggerMetadataSchema = z.object({
  keyword_filter: z.array(z.string()).optional().describe('Array of keyword strings to match (for KEYWORD trigger type)'),
  regex_patterns: z.array(z.string()).optional().describe('Array of regex patterns to match (for KEYWORD trigger type, max 10)'),
  presets: z.array(z.number()).optional().describe('Array of preset keyword list types: 1 = PROFANITY, 2 = SEXUAL_CONTENT, 3 = SLURS (for KEYWORD_PRESET trigger type)'),
  allow_list: z.array(z.string()).optional().describe('Array of strings that should not trigger the rule'),
  mention_total_limit: z.number().optional().describe('Total number of unique role and user mentions allowed per message (for MENTION_SPAM trigger type)'),
  mention_raid_protection_enabled: z.boolean().optional().describe('Whether to automatically detect mention raids'),
...
let ruleOutputSchema = z.object({
  ...
  triggerType: z.number().describe('The trigger type: 1 = KEYWORD, 3 = SPAM, 4 = KEYWORD_PRESET, 5 = MENTION_SPAM'),
...
export let manageAutoModerationTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'update', 'delete']).describe('The auto moderation action to perform'),
    guildId: z.string().describe('The guild ID to operate in'),
    ruleId: z.string().optional().describe('The auto moderation rule ID (required for get, update, delete)'),
    name: z.string().optional().describe('The name of the rule (required for create)'),
    eventType: z.number().optional().describe('The event type: 1 = MESSAGE_SEND (required for create)'),
    triggerType: z.number().optional().describe('The trigger type: 1 = KEYWORD, 3 = SPAM, 4 = KEYWORD_PRESET, 5 = MENTION_SPAM (required for create)'),
    triggerMetadata: triggerMetadataSchema.optional().describe('Trigger-specific metadata (depends on trigger type)'),
    actions: z.array(actionSchema).optional().describe('Array of actions to execute when the rule is triggered (required for create)'),
    enabled: z.boolean().optional().describe('Whether the rule is enabled (defaults to false)'),
    exemptRoles: z.array(z.string()).optional().describe('Array of role IDs that are exempt from this rule (max 20)'),
    exemptChannels: z.array(z.string()).optional().describe('Array of channel IDs that are exempt from this rule (max 50)'),
  }))
  .output(z.object({
    rule: ruleOutputSchema.optional().describe('The auto moderation rule (for get, create, update actions)'),
    rules: z.array(ruleOutputSchema).optional().describe('Array of auto moderation rules (for list action)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      ...
      if (input.eventType === undefined) throw new Error('eventType is required for create action');
      if (input.triggerType === undefined) throw new Error('triggerType is required for create action');
