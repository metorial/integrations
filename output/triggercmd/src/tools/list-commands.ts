...
export let listCommands = SlateTool.create(
  spec,
  {
    name: 'List Commands',
    key: 'list_commands',
  ...
  .input(z.object({
    computerId: z.string().optional().describe('ID of a specific computer to list commands for. If omitted, returns commands from all computers.'),
  }))
  .output(z.object({
    commands: z.array(z.object({
      ...
      allowParams: z.boolean().optional().describe('Whether the command accepts parameters'),
      computerRef: z.string().optional().describe('Reference to the computer this command belongs to'),
      runCount: z.number().optional().describe('Number of times this command has been run'),
