...
export let createProjectTool = SlateTool.create(
  ...
  .input(z.object({
    name: z.string().describe('Name of the project'),
    info: z.string().optional().describe('Description or additional information about the project'),
    color: z.string().optional().describe('Color code for the project (e.g., hex color)'),
    acronym: z.string().optional().describe('Short acronym for the project'),
    active: z.boolean().optional().describe('Whether the project is active (defaults to true)'),
    billable: z.boolean().optional().describe('Whether the project is billable'),
    clientId: z.number().optional().describe('ID of the client to associate this project with'),
    externalId: z.string().optional().describe('External ID for syncing with other systems')
  }))
  .output(projectOutputSchema)
  .handleInvocation(async (ctx) => {
