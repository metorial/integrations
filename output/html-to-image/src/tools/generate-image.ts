...
export let generateImage = SlateTool.create(
  ...
  .input(z.object({
    ...
    url: z.string().optional().describe('Public URL to screenshot instead of providing HTML'),
    ...
    renderWhenReady: z.boolean().optional().describe('Wait for ScreenshotReady() to be called from JavaScript before capturing'),
    ...
    colorScheme: z.enum(['light', 'dark']).optional().describe('Color scheme preference for the page'),
    timezone: z.string().optional().describe('IANA timezone identifier (e.g., "America/New_York")'),
    blockConsentBanners: z.boolean().optional().describe('Automatically hide cookie consent popups')
  }))
  .output(z.object({
