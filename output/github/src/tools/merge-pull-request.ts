...
export let mergePullRequest = SlateTool.create(
  ...
  .input(z.object({
    ...
    sha: z.string().optional().describe('SHA that head must match to allow merge (safety check)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.mergePullRequest(ctx.input.owner, ctx.input.repo, ctx.input.pullNumber, {
