...
export let managePullRequest = SlateTool.create(
  spec,
  {
...
When updating: provide the pull request number along with fields to change.`,
  ...
  .input(z.object({
    ...
    pullNumber: z.number().optional().describe('Pull request number to update. Omit to create a new PR.'),
    ...
    head: z.string().optional().describe('Head branch (required when creating). Use "fork_owner:branch" for cross-repo PRs.'),
    base: z.string().optional().describe('Base branch to merge into (required when creating)'),
    draft: z.boolean().optional().describe('Create as draft PR'),
    state: z.enum(['open', 'closed']).optional().describe('PR state (only for updates)'),
    maintainerCanModify: z.boolean().optional().describe('Allow maintainers to push to the head branch'),
  }))
  .output(z.object({
