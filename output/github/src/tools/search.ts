...
Supports qualifiers for filtering (e.g., "language:python stars:>100" for repositories).`,
  ...
  .input(z.object({
    type: z.enum(['repositories', 'code', 'issues', 'users']).describe('Type of resource to search'),
    query: z.string().describe('Search query with optional GitHub search qualifiers'),
    sort: z.string().optional().describe('Sort field (varies by type, e.g., "stars", "updated", "comments")'),
  ...
  .output(z.object({
    ...
    items: z.array(z.record(z.string(), z.any())).describe('Search result items (shape depends on search type)'),
  }))
