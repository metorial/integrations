...
export let retrieveMemory = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'topK range is 1-100, defaults to 20.',
    ],
    tags: {
      destructive: false,
      readOnly: true,
  ...
  .input(z.object({
    ...
    topK: z.number().optional().describe('Number of top results to return (1-100, defaults to 20)'),
  }))
  .output(z.object({
    results: z.array(retrievalResultSchema).describe('Retrieved text chunks with similarity scores'),
  }))
