...
export let runAgent = SlateTool.create(
  spec,
  {
    name: 'Run Agent',
    key: 'run_agent',
  ...
  .input(z.object({
    model: z.string().describe('LLM model in provider:model format, e.g. "openai:gpt-4o-mini"'),
    input: z.union([
      z.string(),
      z.array(inputMessageSchema),
    ]).describe('Input text or array of messages'),
    llmProviderKey: z.string().describe('API key for the LLM provider (e.g. OpenAI API key)'),
