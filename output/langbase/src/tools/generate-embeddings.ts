...
export let generateEmbeddings = SlateTool.create(
  ...
  .input(z.object({
    chunks: z.array(z.string()).describe('Array of text chunks to generate embeddings for (max 100)'),
  ...
  .output(z.object({
    embeddings: z.array(z.array(z.number())).describe('2D array of embedding vectors, one per input chunk'),
