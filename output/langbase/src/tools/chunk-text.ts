...
export let chunkText = SlateTool.create(
  spec,
  {
    name: 'Chunk Text',
    key: 'chunk_text',
  ...
  .input(z.object({
    ...
    chunkMaxLength: z.number().optional().describe('Maximum length of each chunk (1024-30000, defaults to 1024)'),
    chunkOverlap: z.number().optional().describe('Number of overlapping characters between chunks (min 256)'),
  }))
  .output(z.object({
