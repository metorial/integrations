...
export let createThread = SlateTool.create(
  spec,
  {
    name: 'Create Thread',
    key: 'create_thread',
  ...
  .input(z.object({
    threadId: z.string().optional().describe('Custom thread ID. Auto-generated if not provided.'),
    metadata: z.record(z.string(), z.string()).optional().describe('Key-value metadata for the thread'),
    messages: z.array(messageSchema).optional().describe('Initial messages to add to the thread'),
  }))
  .output(z.object({
...
export let updateThread = SlateTool.create(
  spec,
  {
    name: 'Update Thread',
    key: 'update_thread',
  ...
  .input(z.object({
    ...
    metadata: z.record(z.string(), z.string()).describe('Updated key-value metadata for the thread'),
  }))
  .output(z.object({
...
export let deleteThread = SlateTool.create(
  spec,
  {
    name: 'Delete Thread',
    key: 'delete_thread',
...
export let appendMessages = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    messages: z.array(messageOutputSchema).describe('The appended messages with their assigned IDs'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client(ctx.auth.token);
