...
export let getSiteAudit = SlateTool.create(
  spec,
  {
    name: 'Get Site Audit',
    key: 'get_site_audit',
...
Use to access technical SEO audit results and identify on-site issues that need attention.`,
  ...
  .input(z.object({
    reportType: z.enum(['projects', 'health-score', 'issues', 'page-explorer']).describe('Type of Site Audit report to retrieve'),
    projectId: z.string().optional().describe('Site Audit project ID (required for all reports except "projects")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.reportType) {
      case 'projects':
        ...
        break;
      case 'health-score':
        if (!ctx.input.projectId) throw new Error('projectId is required for health-score report');
      ...
      case 'issues':
        ...
        break;
      case 'page-explorer':
        if (!ctx.input.projectId) throw new Error('projectId is required for page-explorer report');
