...
export let getDomainHistory = SlateTool.create(
  spec,
  {
...
Use to track trends and analyze how a site's SEO profile has changed over time.`,
  ...
  .input(z.object({
    ...
    country: z.string().optional().describe('Two-letter country code (applicable for keywords and total-search-volume history)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.historyType) {
      ...
      case 'metrics':
        ...
        break;
      case 'keywords':
        result = await client.getKeywordsHistory({ ...baseParams, country: ctx.input.country });
        break;
      case 'pages':
        result = await client.getPagesHistory(baseParams);
        break;
      case 'total-search-volume':
        result = await client.getTotalSearchVolumeHistory({ ...baseParams, country: ctx.input.country });
        break;
    }
