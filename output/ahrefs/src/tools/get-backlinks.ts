...
export let getBacklinks = SlateTool.create(
  spec,
  {
    name: 'Get Backlinks',
    key: 'get_backlinks',
...
Supports filtering for broken backlinks and allows sorting, filtering, and pagination for detailed link profile analysis.`,
  ...
  .input(z.object({
    ...
    select: z.string().optional().describe('Comma-separated list of fields to return (refer to Ahrefs API docs for available fields)'),
    ...
    includeBroken: z.boolean().optional().describe('If true, also fetches broken backlinks separately'),
  }))
  .output(z.object({
    backlinks: z.any().describe('List of backlinks with source URLs and link attributes'),
    brokenBacklinks: z.any().optional().describe('List of broken backlinks (only if includeBroken is true)'),
  }))
