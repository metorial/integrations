...
export class Client {
  ...
  async listOrganizationEvents(organizationId: string, params?: {
    status?: string;
    order_by?: string;
    page?: number;
    page_size?: number;
  ...
  async createEvent(organizationId: string, event: {
    name: { html: string };
    ...
    let response = await api.post(`/organizations/${organizationId}/events/`, { event }, {
      headers: this.headers,
    });
    return response.data;
  }
  ...
  async updateEvent(eventId: string, event: {
    ...
    let response = await api.post(`/events/${eventId}/`, { event }, {
      headers: this.headers,
    });
    return response.data;
  }
  ...
  async publishEvent(eventId: string) {
    let response = await api.post(`/events/${eventId}/publish/`, {}, {
      headers: this.headers,
    });
    return response.data;
  }
  ...
  async unpublishEvent(eventId: string) {
    let response = await api.post(`/events/${eventId}/unpublish/`, {}, {
      headers: this.headers,
    });
    return response.data;
  }
  ...
  async createTicketClass(eventId: string, ticketClass: {
    name: string;
    ...
    let response = await api.post(`/events/${eventId}/ticket_classes/`, { ticket_class: ticketClass }, {
      headers: this.headers,
    });
    return response.data;
  }

  async updateTicketClass(eventId: string, ticketClassId: string, ticketClass: {
    name?: string;
    quantity_total?: number;
    cost?: string;
    donation?: boolean;
    ...
    let response = await api.post(`/events/${eventId}/ticket_classes/${ticketClassId}/`, { ticket_class: ticketClass }, {
      headers: this.headers,
    });
    return response.data;
  }
  ...
  async listEventOrders(eventId: string, params?: {
    status?: string;
  ...
  async listOrganizationOrders(organizationId: string, params?: {
    status?: string;
    changed_since?: string;
    page?: number;
  }) {
  ...
  async createVenue(organizationId: string, venue: {
    name: string;
    address?: {
      address_1?: string;
      address_2?: string;
    ...
    let response = await api.post(`/organizations/${organizationId}/venues/`, { venue }, {
      headers: this.headers,
    });
    return response.data;
  }
  ...
  async createOrganizer(organizationId: string, organizer: {
    name: string;
    description?: { html: string };
    logo?: { id: string };
    website?: string;
  }) {
    let response = await api.post(`/organizations/${organizationId}/organizers/`, { organizer }, {
      headers: this.headers,
    });
    return response.data;
  }
  ...
  async listOrganizationDiscounts(organizationId: string, params?: {
    scope?: string;
    page?: number;
    page_size?: number;
  }) {
  ...
  async createDiscount(organizationId: string, discount: {
    code: string;
    type: string;
    amount_off?: string;
    percent_off?: string;
    ...
    let response = await api.post(`/organizations/${organizationId}/discounts/`, { discount }, {
      headers: this.headers,
    });
    return response.data;
  }
  ...
  async updateDiscount(discountId: string, discount: {
    code?: string;
    type?: string;
    amount_off?: string;
    percent_off?: string;
    ...
    let response = await api.post(`/discounts/${discountId}/`, { discount }, {
      headers: this.headers,
    });
    return response.data;
  }
  ...
  async createWebhook(organizationId: string, webhook: {
    endpoint_url: string;
    actions: string;
    event_id?: string;
  }) {
