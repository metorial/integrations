...
export let manageUserAssignments = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    userAssignmentId: z.string().optional().describe('Assignment ID (required for update/remove)'),
    ...
    windowForDeletingTime: z.number().optional().describe('Days allowed for deleting time (-1=unlimited, 0=none, 1-30=days)'),
    windowForAddingOfflineTime: z.number().optional().describe('Days allowed for adding offline time (-1=unlimited, 0=none, 1-30=days)'),
  }))
  .output(z.object({
    assignments: z.array(z.record(z.string(), z.unknown())).optional().describe('List of user assignments (for list action)'),
    assignment: z.record(z.string(), z.unknown()).optional().describe('Created or updated user assignment'),
    removed: z.boolean().optional().describe('Whether the assignment was removed'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client(ctx.auth.token);
    let { action, projectId, userAssignmentId, userId, role, hourlyRate, allowLoggingTime, windowForDeletingTime, windowForAddingOfflineTime } = ctx.input;
    ...
    if (action === 'create') {
      let assignment = await client.createUserAssignment(projectId, {
        ...
        role,
        hourlyRate,
        flagAllowLoggingTime: allowLoggingTime !== undefined ? (allowLoggingTime ? 1 : 0) : undefined,
