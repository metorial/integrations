...
export let getProject = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeUserAssignments: z.boolean().optional().describe('Include user assignment details for the project'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let project = await client.getProject(ctx.input.projectId, ctx.input.includeUserAssignments);
