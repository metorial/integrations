...
export let createPayment = SlateTool.create(
  ...
  .input(z.object({
    paymentType: z.enum(['charge', 'authorization', 'refund']).describe('Type of payment to create'),
    orderId: z.string().describe('Order ID to associate the payment with'),
    amountInCents: z.number().describe('Amount in cents'),
    mode: z.enum(['manual', 'off_session', 'request', 'terminal', 'capture']).optional().describe('Payment mode (for charges)'),
