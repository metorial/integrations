...
export let createCustomer = SlateTool.create(
  ...
  .input(z.object({
    ...
    discountPercentage: z.number().optional().describe('Default discount percentage for this customer'),
    depositType: z.enum(['none', 'percentage_total', 'percentage', 'fixed']).optional().describe('Type of deposit required'),
    depositValue: z.number().optional().describe('Deposit value (percentage or fixed amount depending on deposit type)'),
    taxRegionId: z.string().optional().describe('ID of the tax region to apply to this customer'),
    tags: z.array(z.string()).optional().describe('Tags to assign to the customer'),
    properties: z.array(z.record(z.string(), z.any())).optional().describe('Custom properties such as addresses or phone numbers'),
  }))
  .output(z.object({
