...
export let listProducts = SlateTool.create(
  ...
  .input(z.object({
    ...
    filterTrackable: z.boolean().optional().describe('Filter by whether products are individually trackable'),
  ...
  .output(z.object({
    productGroups: z.array(z.record(z.string(), z.any())).describe('List of product group records'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.filterTrackable !== undefined) filters['trackable'] = String(ctx.input.filterTrackable);
