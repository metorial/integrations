...
export let productEvents = SlateTrigger.create(
  spec,
  {
    name: 'Product Events',
    key: 'product_events',
    description: 'Triggers when a product group is created, updated, or archived, or when a product variation is created.',
  }
)
  .input(z.object({
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      let productId = data?.data?.id || data?.product_group_id || data?.product_id || data?.id || '';
    ...
    handleEvent: async (ctx) => {
      let attrs = ctx.input.payload?.data?.attributes || ctx.input.payload?.product_group || ctx.input.payload?.product || ctx.input.payload || {};
