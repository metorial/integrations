...
export let createOptiml = SlateTool.create(
  spec,
  {
    name: 'Create OptiML',
    key: 'create_optiml',
...
Supports classification and regression tasks.`,
  ...
  .input(z.object({
    ...
    metric: z.string().optional().describe('Optimization metric (e.g., "accuracy", "f_measure", "precision", "recall", "r_squared")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.maxTrainingTime !== undefined) body.max_training_time = ctx.input.maxTrainingTime;
