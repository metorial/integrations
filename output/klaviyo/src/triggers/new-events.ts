...
export let newEvents = SlateTrigger.create(
  spec,
  {
    name: 'New Events',
    key: 'new_events',
    description: '[Polling fallback] Polls for new events (tracked actions) in Klaviyo. Detects new purchases, email interactions, custom events, and other tracked activities.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let updatedTimestamp = newEvents.length > 0
    ...
    handleEvent: async (ctx) => {
      let eventType = (ctx.input.eventName ?? 'unknown')
        .toLowerCase()
        .replace(/\s+/g, '_');
