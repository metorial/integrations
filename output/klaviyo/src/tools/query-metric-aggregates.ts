...
export let queryMetricAggregates = SlateTool.create(
  spec,
  {
...
Useful for building reports on email opens, revenue, clicks, conversions, and other performance metrics.`,
  ...
  .input(z.object({
    ...
    measurements: z.array(z.string()).describe('Aggregate measurements to compute (e.g., ["count", "sum_value", "unique"])'),
    interval: z.string().optional().describe('Time bucket interval (e.g., "day", "week", "month")'),
    filter: z.array(z.string()).optional().describe('Filter conditions as array of strings'),
    groupBy: z.array(z.string()).optional().describe('Fields to group results by (e.g., ["$flow"])'),
    timezone: z.string().optional().describe('Timezone for interval bucketing (e.g., "America/New_York")'),
