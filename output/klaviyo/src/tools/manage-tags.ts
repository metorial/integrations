...
export let manageTags = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['list_tags', 'create_tag', 'update_tag', 'delete_tag', 'list_tag_groups', 'create_tag_group']).describe('Action to perform'),
    ...
    exclusive: z.boolean().optional().describe('Whether the tag group is exclusive (only one tag per resource, for create_tag_group)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('Tag group results'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { action, tagId, tagGroupId, name, exclusive, filter, sort, pageCursor, pageSize } = ctx.input;
