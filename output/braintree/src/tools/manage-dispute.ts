...
let disputeOutputSchema = z.object({
  ...
  kind: z.string().optional().describe('Dispute kind (chargeback, pre_arbitration, retrieval)'),
...
export let findDispute = SlateTool.create(
  spec,
  {
    name: 'Find Dispute',
    key: 'find_dispute',
...
export let acceptDispute = SlateTool.create(
  spec,
  {
    name: 'Accept Dispute',
    key: 'accept_dispute',
  ...
  .handleInvocation(async (ctx) => {
    ...
    let output = dispute.id ? mapDispute(dispute) : {
...
export let addDisputeEvidence = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    category: z.string().optional().describe('Evidence category (e.g. "GENERAL", "DEVICE_ID", "TRACKING_NUMBER", "CARRIER_NAME")'),
...
export let finalizeDispute = SlateTool.create(
  spec,
  {
