...
export let manageProductVariations = SlateTool.create(spec, {
  ...
  .input(z.object({
    ...
    }).optional().describe('Variation dimensions'),
    attributes: z.array(z.object({
      name: z.string().describe('Attribute name (must match parent product attribute)'),
      option: z.string().describe('Attribute option value'),
    })).optional().describe('Attribute values for this variation'),
    image: z.object({
      src: z.string().describe('Image URL'),
      name: z.string().optional(),
      alt: z.string().optional(),
    }).optional().describe('Variation image'),
    force: z.boolean().optional().default(false).describe('Force permanent deletion'),
  }))
  .output(z.object({
    variations: z.array(variationSchema).optional().describe('List of variations (for list action)'),
    variation: variationSchema.optional().describe('Single variation (for create/update action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      ...
      if (ctx.input.attributes) data.attributes = ctx.input.attributes.map(a => ({ name: a.name, option: a.option }));
    ...
    if (action === 'update') {
      ...
      let variation = await client.updateProductVariation(productId, ctx.input.variationId, data);
