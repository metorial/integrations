...
export let webSearch = SlateTool.create(
  ...
  .input(z.object({
    ...
    sources: z.array(z.enum(['web', 'news', 'images'])).optional().describe('Search sources to query'),
    limit: z.number().optional().describe('Maximum number of results to return'),
    lang: z.string().optional().describe('Language code (e.g., "en", "es", "de", "fr", "ja")'),
    categories: z.array(z.string()).optional().describe('Category filters (e.g., "github", "research", "pdf")'),
    timeFilter: z.string().optional().describe('Time-based filter (e.g., "month" for recent results)'),
    scrapeFormats: z.array(z.enum(['markdown', 'html', 'links', 'screenshot'])).optional().describe('Formats to process each result page into'),
    onlyMainContent: z.boolean().optional().describe('Extract only the main content when scraping'),
  }))
  .output(z.object({
    ...
    results: z.array(z.object({
      ...
      })).optional().describe('Extracted links from the result page'),
