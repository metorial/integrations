...
export let takeScreenshot = SlateTool.create(
  ...
  .input(z.object({
    ...
    quality: z.number().min(0).max(100).optional().describe('Image quality (0-100), only for JPEG and WebP'),
    omitBackground: z.boolean().optional().describe('Make background transparent (PNG only)'),
    clip: z.object({
      x: z.number().describe('X coordinate'),
      y: z.number().describe('Y coordinate'),
      width: z.number().describe('Width of the clipping region'),
      height: z.number().describe('Height of the clipping region'),
    }).optional().describe('Clip a specific region of the page'),
    ...
    }).optional().describe('Wait for a CSS selector before taking the screenshot'),
