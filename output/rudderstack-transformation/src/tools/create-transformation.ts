...
When **publish** is false (default), the transformation is created as a draft and cannot be connected to destinations. Set **publish** to true to make it live for incoming event traffic and optionally connect destinations.`,
  ...
  .input(z.object({
    ...
    publish: z.boolean().optional().describe('If true, publishes the transformation and makes it live for incoming traffic. Defaults to false.'),
    destinationIds: z.array(z.string()).optional().describe('Array of destination IDs to connect (only applied when publish is true)'),
  }))
  .output(z.object({
