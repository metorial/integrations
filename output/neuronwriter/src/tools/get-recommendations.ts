...
export let getRecommendations = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'If status is not "ready", wait and retry after ~60 seconds.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .output(z.object({
    status: z.string().describe('Query analysis status (waiting, in progress, ready, not found)'),
    ...
    metrics: z.record(z.string(), z.any()).optional().describe('Content metrics including target word count and readability scores based on SERP median values'),
    termsSummary: z.record(z.string(), z.any()).optional().describe('Recommended terms for title, description, H1, H2, and content in text format'),
    terms: z.record(z.string(), z.any()).optional().describe('Detailed term data with usage percentages and recommended occurrence ranges'),
    ideas: z.record(z.string(), z.any()).optional().describe('Content ideas including autocomplete suggestions, People Also Ask, and questions from competitors'),
    competitors: z.array(z.any()).optional().describe('SERP competitor URLs, titles, and content scores'),
    entities: z.array(z.any()).optional().describe('Relevant named entities with importance and relevance scores'),
  }))
