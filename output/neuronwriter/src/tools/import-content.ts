...
export let importContent = SlateTool.create(
  spec,
  {
    name: 'Import Content',
    key: 'import_content',
  ...
  .input(z.object({
    ...
    containerId: z.string().optional().describe('HTML element ID to extract content from (URL import only)'),
    containerClass: z.string().optional().describe('HTML element class to extract content from (URL import only)'),
  }))
  .output(z.object({
