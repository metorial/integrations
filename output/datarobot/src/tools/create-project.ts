...
export let createProject = SlateTool.create(
  ...
  .input(z.object({
    ...
    mode: z.enum(['auto', 'quick', 'manual', 'comprehensive']).optional().describe('Autopilot mode to use when target is set'),
    metric: z.string().optional().describe('Optimization metric to use (e.g. AUC, RMSE, LogLoss)'),
