...
export let startAutopilot = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    mode: z.enum(['auto', 'quick', 'manual', 'comprehensive']).optional().describe('Autopilot mode'),
    metric: z.string().optional().describe('Optimization metric (e.g. AUC, RMSE, LogLoss, FVE Gamma Deviance)'),
    positiveClass: z.string().optional().describe('Positive class label for binary classification'),
    blueprintThreshold: z.number().optional().describe('Max runtime in hours for models to be included in Autopilot'),
    accuracyOptimizedMb: z.boolean().optional().describe('Include additional longer-running models'),
    smartDownsampled: z.boolean().optional().describe('Enable smart downsampling for imbalanced datasets'),
    majorityDownsamplingRate: z.number().optional().describe('Downsampling rate for the majority class (0-100)'),
