...
export let makePredictions = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    rows: z.array(z.record(z.string(), z.any())).describe('Array of data rows as key-value objects with feature names and values'),
    maxExplanations: z.number().optional().describe('Number of prediction explanations to include per row (0 = none)'),
  }))
  .output(z.object({
    predictions: z.array(z.record(z.string(), z.any())).describe('Array of prediction results'),
  }))
