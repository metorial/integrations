...
export let createDeployment = SlateTool.create(
  ...
  .input(z.object({
    ...
    modelId: z.string().optional().describe('ID of a trained model from a project leaderboard'),
    ...
    importance: z.enum(['LOW', 'MODERATE', 'HIGH', 'CRITICAL']).optional().describe('Deployment importance level'),
  }))
  .output(z.object({
