...
export let getModel = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeFeatureImpact: z.boolean().optional().describe('Include feature impact scores (must have been computed previously)'),
  }))
  .output(z.object({
    ...
    trainingRowCount: z.number().optional().nullable().describe('Number of rows used for training'),
    ...
    featureImpact: z.array(z.object({
      ...
      redundantWith: z.string().optional().nullable().describe('Feature this is redundant with'),
    })).optional().describe('Feature impact scores if requested'),
  }))
