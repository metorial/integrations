...
export let getInvoice = SlateTool.create(
  ...
  .output(z.object({
    ...
    lineItems: z.array(z.record(z.string(), z.any())).optional().describe('Invoice line items if requested'),
    trackedLineItems: z.array(z.record(z.string(), z.any())).optional().describe('Tracked line items if requested'),
    payments: z.array(z.record(z.string(), z.any())).optional().describe('Invoice payments if requested'),
  }))
