...
let filterSchema = z.object({
  valueMatchTypeId: z.number().describe('Comparison operator: 1 = Equals, 2 = Not Equals, etc.'),
  value: z.string().describe('The value to filter by'),
});

let filterGroupSchema = z.object({
  filterGroupTypeId: z.number().describe('Type of filter. Use the search filters tool to discover available types for invoices.'),
...
export let searchInvoices = SlateTool.create(
  ...
  .input(z.object({
    filterGroupCollections: z.array(filterGroupCollectionSchema).optional().describe('Filter collections for narrowing results'),
  }))
  .output(z.object({
