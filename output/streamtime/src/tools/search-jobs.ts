...
let filterSchema = z.object({
  valueMatchTypeId: z.number().describe('Comparison operator: 1 = Equals, 2 = Not Equals, 3 = Contains, etc.'),
  value: z.string().describe('The value to filter by'),
});

let filterGroupSchema = z.object({
  filterGroupTypeId: z.number().describe('Type of filter (e.g., 1 = Company Name, 3 = Job Status). Use the search filters tool to discover available types.'),
  conditionMatchTypeId: z.number().describe('Logic between filters in this group: 1 = AND, 2 = OR'),
...
export let searchJobs = SlateTool.create(
  ...
  .input(z.object({
    filterGroupCollections: z.array(filterGroupCollectionSchema).optional().describe('Filter collections for narrowing results. If empty, returns all jobs.'),
  }))
  .output(z.object({
