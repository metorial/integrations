...
export let maestroRun = SlateTool.create(
  spec,
  {
    name: 'Maestro Run',
    key: 'maestro_run',
  ...
  .input(z.object({
    ...
    requirements: z.array(requirementSchema).max(10).optional().describe('Validation requirements the output should satisfy'),
    models: z.array(z.string()).optional().describe('Models to use (e.g. jamba-mini, jamba-large, or third-party model identifiers). Let Maestro select automatically if omitted'),
    budget: z.enum(['low', 'medium', 'high']).optional().describe('Compute budget controlling reasoning depth'),
    includeDataSources: z.boolean().optional().describe('Include retrieved data sources in the response'),
    includeRequirementsResult: z.boolean().optional().describe('Include requirements validation results in the response'),
  ...
  .output(z.object({
    ...
    requirementsResult: z.any().optional().describe('Per-requirement validation scores and overall score'),
    dataSources: z.array(z.any()).optional().describe('Retrieved data sources used in the response'),
  }))
