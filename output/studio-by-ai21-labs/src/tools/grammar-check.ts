...
export let grammarCheck = SlateTool.create(
  spec,
  {
    name: 'Grammar Check',
    key: 'grammar_check',
  ...
  .output(z.object({
    corrections: z.array(z.object({
      ...
      correctionType: z.string().describe('Type of correction (e.g. Grammar, Spelling, Punctuation)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: corrections.length > 0
        ? `Found **${corrections.length}** grammatical issue(s): ${corrections.map((c: any) => `"${c.originalText}" → "${c.suggestion}" (${c.correctionType})`).slice(0, 5).join(', ')}${corrections.length > 5 ? '...' : ''}`
