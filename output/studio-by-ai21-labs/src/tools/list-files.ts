...
export let listFiles = SlateTool.create(
  ...
  .input(z.object({
    labels: z.array(z.string()).optional().describe('Filter by these labels (case-sensitive)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let files = (Array.isArray(result) ? result : result.files ?? result.data ?? []).map((f: any) => ({
