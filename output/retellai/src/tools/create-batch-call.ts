...
export let createBatchCall = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Source number must be purchased from or imported to Retell.',
    ],
  }
)
  .input(z.object({
    ...
    triggerTimestamp: z.number().optional().describe('Unix timestamp in ms to schedule the batch. Omit for immediate execution.'),
    tasks: z.array(z.object({
      toNumber: z.string().describe('Destination phone number in E.164 format'),
      overrideAgentId: z.string().optional().describe('Agent ID override for this specific call'),
      dynamicVariables: z.record(z.string(), z.string()).optional().describe('Dynamic variables to inject into the prompt'),
      metadata: z.record(z.string(), z.any()).optional().describe('Custom metadata for this call'),
    })).describe('List of call tasks to execute'),
    reservedConcurrency: z.number().optional().describe('Concurrency reserved for non-batch calls (0 or more)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.reservedConcurrency !== undefined) body.reserved_concurrency = ctx.input.reservedConcurrency;
