...
let agentSchema = z.object({
  ...
  lastModificationTimestamp: z.number().optional().describe('Last modification timestamp in milliseconds since epoch'),
});
...
export let listAgents = SlateTool.create(
  ...
  .input(z.object({
    limit: z.number().min(1).max(1000).optional().describe('Maximum number of agents to return (1-1000, default 1000)'),
    paginationKey: z.string().optional().describe('Pagination key (agent ID) to continue fetching the next page'),
  }))
  .output(z.object({
