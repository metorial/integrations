...
export let makePhoneCall = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    fromNumber: z.string().describe('Source phone number in E.164 format (e.g. +14157774444)'),
    toNumber: z.string().describe('Destination phone number in E.164 format (e.g. +12137774445)'),
    overrideAgentId: z.string().optional().describe('Agent ID to use for this call only, overriding the number default'),
    dynamicVariables: z.record(z.string(), z.string()).optional().describe('Key-value pairs to inject into the prompt and tool descriptions'),
    metadata: z.record(z.string(), z.any()).optional().describe('Arbitrary metadata to attach to the call'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.dynamicVariables) body.retell_llm_dynamic_variables = ctx.input.dynamicVariables;
