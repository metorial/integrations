...
export let executeSql = SlateTool.create(
  spec,
  {
    name: 'Execute SQL',
    key: 'execute_sql',
  ...
  .input(z.object({
    statement: z.string().describe('SQL statement(s) to execute. Separate multiple statements with semicolons.'),
    database: z.string().optional().describe('Target database for this execution. Overrides the default.'),
    schema: z.string().optional().describe('Target schema for this execution. Overrides the default.'),
    warehouse: z.string().optional().describe('Warehouse to use for this execution. Overrides the default.'),
    role: z.string().optional().describe('Role to use for this execution. Overrides the default.'),
    timeout: z.number().optional().describe('Maximum execution time in seconds. 0 uses the maximum (604800).'),
    async: z.boolean().optional().describe('If true, execute asynchronously and return a statement handle for polling.'),
    ...
    })).optional().describe('Bind variable values keyed by position (e.g. "1", "2"). Use ? placeholders in the SQL.'),
    parameters: z.record(z.string(), z.any()).optional().describe('Session parameters (e.g. multi_statement_count, query_tag, timezone).'),
  }))
  .output(z.object({
    ...
    rows: z.array(z.array(z.string().nullable())).optional().describe('Result data as arrays of string values'),
    ...
    }).optional().describe('DML statistics for INSERT/UPDATE/DELETE operations'),
    statementHandles: z.array(z.string()).optional().describe('Handles for individual statements when multiple were submitted'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (result.stats) {
      if (result.stats.numRowsInserted) messageParts.push(`**Rows inserted**: ${result.stats.numRowsInserted}`);
      if (result.stats.numRowsUpdated) messageParts.push(`**Rows updated**: ${result.stats.numRowsUpdated}`);
      if (result.stats.numRowsDeleted) messageParts.push(`**Rows deleted**: ${result.stats.numRowsDeleted}`);
    }
