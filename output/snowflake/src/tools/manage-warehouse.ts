...
export let manageWarehouse = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'delete', 'resume', 'suspend', 'abort']).describe('Operation to perform'),
    warehouseName: z.string().optional().describe('Warehouse name (required for all actions except list)'),
    like: z.string().optional().describe('SQL LIKE pattern to filter warehouses when listing'),
    showLimit: z.number().optional().describe('Maximum number of warehouses to return when listing'),
    createMode: z.enum(['errorIfExists', 'orReplace', 'ifNotExists']).optional().describe('Creation behavior for create action'),
    warehouseSize: z.enum([
      'XSMALL', 'SMALL', 'MEDIUM', 'LARGE', 'XLARGE',
      'XXLARGE', 'XXXLARGE', 'X4LARGE', 'X5LARGE', 'X6LARGE',
    ]).optional().describe('Warehouse size for create action'),
    autoSuspend: z.number().optional().describe('Seconds of inactivity before auto-suspend (0 to disable)'),
    autoResume: z.boolean().optional().describe('Whether to automatically resume the warehouse when a query is submitted'),
    minClusterCount: z.number().optional().describe('Minimum cluster count for multi-cluster warehouses'),
    maxClusterCount: z.number().optional().describe('Maximum cluster count for multi-cluster warehouses'),
    comment: z.string().optional().describe('Warehouse comment'),
    ifExists: z.boolean().optional().describe('When true, delete succeeds even if the warehouse does not exist'),
  }))
  .output(z.object({
    warehouses: z.array(z.record(z.string(), z.any())).optional().describe('List of warehouses (for list action)'),
    warehouse: z.record(z.string(), z.any()).optional().describe('Warehouse details (for get/create actions)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!warehouseName) {
      throw new Error('warehouseName is required for get, create, delete, resume, suspend, and abort actions');
    }
    ...
    if (action === 'create') {
      ...
      if (ctx.input.minClusterCount !== undefined) body.min_cluster_count = ctx.input.minClusterCount;
      if (ctx.input.maxClusterCount !== undefined) body.max_cluster_count = ctx.input.maxClusterCount;
