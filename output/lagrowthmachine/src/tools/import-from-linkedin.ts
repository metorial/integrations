...
export let importFromLinkedIn = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    linkedinUrl: z.string().describe('LinkedIn search URL, Sales Navigator URL, or LinkedIn post URL'),
    autoImport: z.boolean().optional().describe('Automatically import matching leads'),
    excludeContactedLeads: z.boolean().optional().describe('Skip leads that have been previously contacted'),
    linkedinPostCategory: z.string().optional().describe('For post URLs: filter by engagement type (e.g., "like", "comment")'),
  }))
  .output(z.object({
