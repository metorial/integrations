...
export let campaignEvents = SlateTrigger.create(
  ...
  .output(z.object({
    ...
    email: z.string().optional().describe('Email address of the lead'),
    linkedinUrl: z.string().optional().describe('LinkedIn profile URL of the lead'),
    timestamp: z.string().optional().describe('Timestamp when the campaign ended for this lead'),
  }))
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let inputs = events.map((event: any) => {
        let eventId = event.id || event.eventId || `campaign.ended-${event.leadId || ''}-${Date.now()}`;
