...
let jobResultSchema = z.object({
  ...
  applyLinks: z.array(z.object({
    title: z.string().optional().describe('Platform name'),
    link: z.string().optional().describe('Application URL'),
  })).optional().describe('Links to apply'),
});

export let jobsSearch = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    query: z.string().describe('Job search query (e.g., "software engineer", "marketing manager in New York")'),
    ...
    nextPageToken: z.string().optional().describe('Pagination token for next page of results'),
  }))
  .output(z.object({
