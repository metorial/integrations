...
export let webSearch = SlateTool.create(
  ...
  .input(z.object({
    query: z.string().describe('Search query terms. Supports operators like site:, inurl:, intitle:'),
    engine: z.enum(['google', 'google_light', 'bing', 'baidu', 'yahoo', 'yandex', 'duckduckgo', 'naver'])
      .optional()
      .describe('Search engine to use. Defaults to "google"'),
    location: z.string().optional().describe('Geographic location for search (e.g., "New York, NY")'),
    country: z.string().optional().describe('Country code for results (e.g., "us", "gb", "de")'),
    language: z.string().optional().describe('Interface language code (e.g., "en", "fr", "de")'),
    device: z.enum(['desktop', 'mobile', 'tablet']).optional().describe('Device type for results'),
    ...
    safeSearch: z.enum(['active', 'blur', 'off']).optional().describe('SafeSearch content filtering'),
