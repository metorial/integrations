...
export let createGoal = SlateTool.create(
  ...
  .input(z.object({
    slug: z.string().describe('URL-friendly name for the goal (lowercase, hyphens, no spaces)'),
    title: z.string().describe('Human-readable title for the goal'),
    goalType: z.enum(['hustler', 'biker', 'fatloser', 'gainer', 'inboxer', 'drinker', 'custom']).describe('Type of goal'),
    gunits: z.string().describe('Units for the goal value (e.g., "hours", "pages", "kg")'),
    goaldate: z.number().optional().describe('Unix timestamp of target end date (provide exactly 2 of goaldate/goalval/rate)'),
    goalval: z.number().optional().describe('Target value at the goal date (provide exactly 2 of goaldate/goalval/rate)'),
    rate: z.number().optional().describe('Required rate of change per day (provide exactly 2 of goaldate/goalval/rate)'),
    initval: z.number().optional().describe('Initial value for the goal (default 0)'),
    secret: z.boolean().optional().describe('Whether the goal is secret (not publicly visible)'),
