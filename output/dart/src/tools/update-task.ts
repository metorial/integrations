...
export let updateTask = SlateTool.create(
  ...
  .input(z.object({
    ...
    assignee: z.string().nullable().optional().describe('New assignee name or email, or null to unassign'),
    assignees: z.array(z.string()).nullable().optional().describe('New assignee list, or null to clear'),
    priority: z.enum(['Critical', 'High', 'Medium', 'Low']).nullable().optional().describe('New priority, or null to clear'),
    tags: z.array(z.string()).optional().describe('Updated tags (replaces existing)'),
    startAt: z.string().nullable().optional().describe('Start date (YYYY-MM-DD), or null to clear'),
    dueAt: z.string().nullable().optional().describe('Due date (YYYY-MM-DD), or null to clear'),
    size: z.union([z.string(), z.number()]).nullable().optional().describe('Size/estimate, or null to clear'),
    parentId: z.string().nullable().optional().describe('Parent task ID, or null to remove from parent'),
    customProperties: z.record(z.string(), z.any()).optional().describe('Custom property values keyed by property name'),
    taskRelationships: z.object({
    ...
    }).optional().describe('Task relationships to set')
  }))
  .output(taskSchema)
