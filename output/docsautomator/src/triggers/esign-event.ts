...
export let esignEvent = SlateTrigger.create(
  spec,
  {
    name: 'E-Signature Event',
    key: 'esign_event',
    description: 'Triggers when an e-signature event occurs, including session creation (manual delivery), session completion (all signers signed), or session declined (a signer declined). Configure the webhook URL in the automation\'s output settings in DocsAutomator.',
  }
)
  .input(z.object({
    eventType: z.string().describe('The e-signature event type (e.g., "esign.session_created", "esign.session_completed", "esign.session_declined").'),
  ...
  .output(z.object({
    ...
    signedPdfUrl: z.string().optional().describe('URL of the signed PDF (for completed sessions).'),
    ...
    signers: z.array(signerSchema).optional().describe('Signer details (for completed sessions).'),
    declinedBy: declinedBySchema.optional().describe('Details of the signer who declined (for declined sessions).'),
    declineReason: z.string().optional().describe('Reason provided by the signer for declining.'),
    signingLinks: z.array(signingLinkSchema).optional().describe('Signing links for manual delivery (for session_created events).'),
    fieldValues: z.record(z.string(), z.unknown()).optional().describe('Field values entered during signing (for completed sessions).'),
    sourceData: z.record(z.string(), z.unknown()).optional().describe('Original source data from the data source.'),
