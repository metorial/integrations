...
let eventTypeEnum = z.enum(['EQ', 'TN', 'TC', 'WF', 'FL', 'ET', 'DR', 'SW', 'SI', 'VO', 'LS']).optional()
  .describe('Event type code: EQ=Earthquake, TN=Tsunami, TC=Tropical Cyclone, WF=Wildfire, FL=Flood, ET=Extreme Temperature, DR=Drought, SW=Severe Storm, SI=Sea Ice, VO=Volcano, LS=Landslide');

export let getNaturalDisasters = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    continent: z.string().optional().describe('Continent code (AFR, ANT, ASIA, AUS, EUR, NAR, SAR, Ocean)'),
