...
export let getSoil = SlateTool.create(
  spec,
  {
    name: 'Get Soil Data',
    key: 'get_soil',
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (isHistorical) {
      result = await client.getSoilHistory(ctx.input.lat, ctx.input.lng, ctx.input.from!, ctx.input.to!);
    } else {
    ...
    let summary = soilRecords.length > 0
