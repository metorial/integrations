...
export let geocode = SlateTool.create(
  ...
  .input(z.object({
    ...
    lat: z.number().min(-90).max(90).optional().describe('Latitude for reverse geocoding (-90 to 90)'),
    lng: z.number().min(-180).max(180).optional().describe('Longitude for reverse geocoding (-180 to 180)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.place) {
      ...
      throw new Error('Provide a place name for forward geocoding, or lat/lng for reverse geocoding.');
    }
