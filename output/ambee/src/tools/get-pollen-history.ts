...
let pollenHistorySchema = z.object({
  Count: z.object({
...
export let getPollenHistory = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.place) {
      result = await client.getPollenHistoryByPlace(ctx.input.place, ctx.input.from, ctx.input.to, ctx.input.speciesRisk);
    } else if (ctx.input.lat !== undefined && ctx.input.lng !== undefined) {
      result = await client.getPollenHistoryByLatLng(ctx.input.lat, ctx.input.lng, ctx.input.from, ctx.input.to, ctx.input.speciesRisk);
    } else {
