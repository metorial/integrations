...
export let getVerse = SlateTool.create(
  spec,
  {
    name: 'Get Verse',
    key: 'get_verse',
  ...
  .input(z.object({
    ...
    chapterId: z.string().optional().describe('Chapter ID to list all verses in that chapter (e.g., "GEN.1")'),
    verseId: z.string().optional().describe('Specific verse ID to retrieve content (e.g., "GEN.1.1")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    throw new Error('Either chapterId (to list verses) or verseId (to retrieve content) must be provided.');
  })
  .build();
