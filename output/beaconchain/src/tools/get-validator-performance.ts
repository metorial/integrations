...
let validatorSelectorSchema = z.object({
  validatorIdentifiers: z.array(z.union([z.string(), z.number()])).optional().describe('Array of validator indices or public keys'),
  dashboardId: z.number().optional().describe('Dashboard ID to select validators from'),
});

export let getValidatorPerformance = SlateTool.create(
  spec,
  {
...
Use aggregate mode for a summary across validators, or list mode for per-validator performance at a specific epoch.`,
  ...
  .input(z.object({
    ...
    mode: z.enum(['aggregate', 'list']).default('aggregate').describe('Whether to return aggregated or per-validator performance'),
    epoch: z.number().optional().describe('Specific epoch (list mode only). Defaults to latest finalized.'),
    pageSize: z.number().optional().describe('Number of results per page (list mode only, 1-100)'),
    cursor: z.string().optional().describe('Pagination cursor from a previous response'),
  }))
  .output(z.object({
    performance: z.any().describe('Performance data: aggregate summary or per-validator list'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.mode === 'aggregate') {
      ...
      let response = await client.getValidatorPerformanceList(
        selector,
        ctx.input.epoch,
        { pageSize: ctx.input.pageSize, cursor: ctx.input.cursor }
      );
      result = {
