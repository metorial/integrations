...
export let getExecutionAddress = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    includeTokens: z.boolean().optional().describe('Include ERC-20 token holdings for the address'),
  ...
  .output(z.object({
    addressSummary: z.any().describe('Address summary including ETH balance and transaction count'),
    tokens: z.any().optional().describe('ERC-20 token balances for the address'),
    gasPrices: z.any().optional().describe('Current gas price estimates (rapid, fast, standard, slow)'),
  }))
