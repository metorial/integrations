...
export let getSyncCommittee = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    includeValidators: z.boolean().optional().describe('Include the list of validators assigned to this sync committee period'),
  ...
  .output(z.object({
    ...
    validators: z.any().optional().describe('Validators assigned to this sync committee period'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includeValidators) {
      let validatorResponse = await client.getSyncCommitteeValidators(
        ctx.input.period,
