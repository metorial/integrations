...
export let getBlockDetails = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    includeRewards: z.boolean().optional().describe('Include block rewards data (transaction fees and MEV)'),
  }))
  .output(z.object({
    blockOverview: z.any().describe('Execution layer block details'),
    blockRewards: z.any().optional().describe('Block rewards including transaction fees and MEV'),
  }))
