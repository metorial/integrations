...
export let getValidatorRewards = SlateTool.create(
  spec,
  {
...
Use the aggregate mode for cumulative totals or the list mode for epoch-by-epoch detail.`,
  ...
  .input(z.object({
    ...
    mode: z.enum(['list', 'aggregate']).default('list').describe('Whether to return per-epoch list or cumulative aggregate'),
    epoch: z.number().optional().describe('Specific epoch to filter rewards for (list mode only)'),
    pageSize: z.number().optional().describe('Number of results per page (1-100, default 10)'),
    cursor: z.string().optional().describe('Pagination cursor from a previous response'),
  }))
  .output(z.object({
    rewards: z.any().describe('Reward data, either a list of per-epoch entries or an aggregate summary'),
    paging: z.any().optional().describe('Pagination info with next_cursor if more results are available'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.mode === 'aggregate') {
      ...
      let response = await client.getValidatorRewardsList(
        selector,
        ctx.input.epoch,
