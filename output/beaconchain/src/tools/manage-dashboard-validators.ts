...
export let manageDashboardValidators = SlateTool.create(
  spec,
  {
...
Dashboards and groups must be created through the Beaconcha.in web UI first.`,
  ...
  .input(z.object({
    ...
    validators: z.array(z.union([z.string(), z.number()])).optional().describe('Validator indices or public keys to add/remove. Required for "add" and "remove" actions.'),
    groupId: z.number().optional().describe('Group ID to assign validators to (for "add") or clear all validators from (for "clear_group")'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let actionDesc = ctx.input.action === 'add' ? 'Added' :
      ctx.input.action === 'remove' ? 'Removed' : 'Cleared group';
