...
export let getStakingEntities = SlateTool.create(
  spec,
  {
...
This is a **Pro-tier** feature requiring a Scale or Enterprise plan.`,
  ...
  .input(z.object({
    entity: z.string().optional().describe('Entity name to look up sub-entities for. If omitted, returns the full entity overview list.'),
    pageSize: z.number().optional().describe('Number of results per page (1-100)'),
    cursor: z.string().optional().describe('Pagination cursor from a previous response'),
  }))
  .output(z.object({
    entities: z.any().describe('Entity or sub-entity list with validator counts, BeaconScore, and share data'),
