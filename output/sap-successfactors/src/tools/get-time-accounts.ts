...
export let getTimeAccounts = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    filter: z.string().optional().describe('OData $filter expression for advanced filtering'),
    select: z.string().optional().describe('Comma-separated fields to return'),
    top: z.number().optional().describe('Maximum records to return').default(100),
    skip: z.number().optional().describe('Number of records to skip'),
  }))
  .output(z.object({
    timeAccounts: z.array(z.record(z.string(), z.unknown())).describe('List of time account records with balances'),
