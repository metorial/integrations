...
export let getGoals = SlateTool.create(
  ...
  .input(z.object({
    planId: z.number().optional().describe('Goal plan template ID to query goals from. Omit to list goal plan templates instead.'),
  ...
  .output(z.object({
    goalPlans: z.array(z.record(z.string(), z.unknown())).optional().describe('List of goal plan templates (when planId is not provided)'),
    goals: z.array(z.record(z.string(), z.unknown())).optional().describe('List of goals within the specified plan (when planId is provided)'),
