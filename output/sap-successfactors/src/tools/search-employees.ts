...
export let searchEmployees = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum 1000 records per request by default',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    filter: z.string().optional().describe('OData $filter expression (e.g., "department eq \'HR\' and status eq \'active\'")'),
    select: z.string().optional().describe('Comma-separated fields to return (e.g., "userId,firstName,lastName,email,department")'),
    ...
    includeCount: z.boolean().optional().describe('Whether to include total count in results').default(false),
  }))
  .output(z.object({
    employees: z.array(z.record(z.string(), z.unknown())).describe('List of matching employee records'),
    totalCount: z.number().optional().describe('Total number of matching records (if includeCount was true)'),
  }))
