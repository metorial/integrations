...
export let queryOdataEntity = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum 1000 records per request typically enforced by SAP',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    entitySet: z.string().describe('The OData entity set name (e.g., "User", "EmpJob", "cust_myCustomEntity")'),
    entityKey: z.string().optional().describe('Simple key value to retrieve a specific entity'),
    compoundKeys: z.record(z.string(), z.union([z.string(), z.number()])).optional().describe('Compound key fields for entities with multiple key properties'),
    filter: z.string().optional().describe('OData $filter expression'),
    select: z.string().optional().describe('Comma-separated fields to return'),
    expand: z.string().optional().describe('Navigation properties to expand'),
    orderBy: z.string().optional().describe('Sort order (e.g., "lastModifiedDateTime desc")'),
    top: z.number().optional().describe('Maximum records to return').default(100),
    skip: z.number().optional().describe('Number of records to skip'),
    includeCount: z.boolean().optional().describe('Include total record count').default(false),
  }))
  .output(z.object({
    entity: z.record(z.string(), z.unknown()).optional().describe('Single entity record (when key is provided)'),
    entities: z.array(z.record(z.string(), z.unknown())).optional().describe('List of entity records (when querying)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.entityKey) {
      let entity = await client.getEntity(ctx.input.entitySet, ctx.input.entityKey, queryOptions);
      return {
        output: { entity },
    ...
    if (ctx.input.compoundKeys && Object.keys(ctx.input.compoundKeys).length > 0) {
      let entity = await client.getEntityByCompoundKey(ctx.input.entitySet, ctx.input.compoundKeys as Record<string, string | number>, queryOptions);
      return {
        output: { entity },
