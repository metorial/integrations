...
export let getPerformanceReviews = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    formDataId: z.number().optional().describe('Specific performance review form data ID to retrieve'),
    filter: z.string().optional().describe('OData $filter expression (e.g., "formStatus eq \'completed\'")'),
    select: z.string().optional().describe('Comma-separated fields to return'),
    expand: z.string().optional().describe('Navigation properties to expand (e.g., "formContents,formAuditTrails")'),
    top: z.number().optional().describe('Maximum records to return when searching').default(50),
    skip: z.number().optional().describe('Number of records to skip'),
  }))
  .output(z.object({
    review: z.record(z.string(), z.unknown()).optional().describe('Single performance review (when formDataId is provided)'),
    reviews: z.array(z.record(z.string(), z.unknown())).optional().describe('List of performance reviews (when searching)'),
