...
export let manageTimeOff = SlateTool.create(
  ...
  .input(z.object({
    operation: z.enum(['search', 'create']).describe('Whether to search existing time-off records or create a new request'),
    ...
    top: z.number().optional().describe('Maximum records to return (for search)').default(100),
    ...
    startDate: z.string().optional().describe('Start date in YYYY-MM-DD format (for create)'),
  ...
  .output(z.object({
    timeOffRecords: z.array(z.record(z.string(), z.unknown())).optional().describe('List of time-off records (for search)'),
    createdRecord: z.record(z.string(), z.unknown()).optional().describe('The newly created time-off record (for create)'),
    totalCount: z.number().optional().describe('Total count of matching records (for search)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.operation === 'create') {
      if (!ctx.input.userId || !ctx.input.timeType || !ctx.input.startDate || !ctx.input.endDate) {
        throw new Error('userId, timeType, startDate, and endDate are required for creating a time-off request');
      }
      ...
      if (ctx.input.quantityInDays !== undefined) data.quantityInDays = ctx.input.quantityInDays;
