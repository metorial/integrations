...
let estimateSchema = z.object({
  ...
  status: z.string().optional().describe('Status: Draft, Sent, Viewed, Cancelled, Declined, Revised, Accepted, or Archived'),
...
export let createEstimate = SlateTool.create(
  ...
  .input(z.object({
    ...
    })).optional().describe('Estimate line items')
  }))
...
export let updateEstimate = SlateTool.create(
  ...
  .input(z.object({
    ...
    tax3: z.number().optional().describe('Third tax rate percentage'),
    items: z.array(z.object({
      estimateItemId: z.number().optional().describe('ID of existing item to update. Omit to create new.'),
...
export let getEstimates = SlateTool.create(
  spec,
  {
    name: 'Get Estimates',
    key: 'get_estimates',
  ...
  .input(z.object({
    estimateId: z.number().optional().describe('ID of a specific estimate. If omitted, lists estimates.'),
...
export let sendEstimate = SlateTool.create(
  spec,
  {
    name: 'Send Estimate',
    key: 'send_estimate',
...
let mapEstimate = (est: any) => ({
  ...
  items: est.estimate_items ? est.estimate_items.map((item: any) => ({
