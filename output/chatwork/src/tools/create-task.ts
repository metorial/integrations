...
export let createTask = SlateTool.create(
  ...
  .input(z.object({
    ...
    assigneeIds: z.array(z.number()).min(1).describe('Account IDs of users to assign the task to'),
    dueDate: z.string().optional().describe('Due date as an ISO 8601 string (e.g., "2024-12-31T23:59:59Z")'),
    limitType: z.enum(['none', 'date', 'time']).optional().describe('Type of deadline: "none" (no deadline display), "date" (date only), "time" (date and time)')
  }))
  .output(z.object({
