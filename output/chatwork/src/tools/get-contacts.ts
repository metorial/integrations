...
export let getContacts = SlateTool.create(
  ...
  .input(z.object({
    includeRequests: z.boolean().optional().describe('Whether to also fetch pending incoming contact requests. Defaults to false.')
  }))
  .output(z.object({
    ...
    })).optional().describe('Pending incoming contact requests if requested')
  }))
