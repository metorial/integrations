...
export let messageEvent = SlateTrigger.create(
  spec,
  {
    name: 'Message Event',
    key: 'message_event',
    description: 'Triggers when a message is created, updated, or the user is mentioned in a chat room. Webhooks must be configured manually in the Chatwork webhook settings.'
  }
)
  .input(z.object({
    eventType: z.enum(['message_created', 'message_updated', 'mention_to_me']).describe('Type of message event'),
    ...
    recipientAccountId: z.number().optional().describe('Account ID of the mentioned user (only for mention_to_me)'),
    body: z.string().describe('Message body'),
    sendTime: z.number().describe('Message send time as Unix timestamp'),
    updateTime: z.number().describe('Message update time as Unix timestamp (0 if never updated)')
  }))
  .output(z.object({
    messageId: z.string().describe('Message ID'),
    roomId: z.number().describe('Room ID where the event occurred'),
    senderAccountId: z.number().describe('Account ID of the message sender'),
    recipientAccountId: z.number().optional().describe('Account ID of the mentioned user (only for mentions)'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let eventType = data.webhook_event_type as 'message_created' | 'message_updated' | 'mention_to_me';

      let senderAccountId = eventType === 'mention_to_me'
        ? event.from_account_id!
        : event.account_id!;

      let recipientAccountId = eventType === 'mention_to_me'
        ? event.to_account_id
        : undefined;
