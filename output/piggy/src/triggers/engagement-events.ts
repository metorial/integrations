...
export let engagementEvents = SlateTrigger.create(
  spec,
  {
    name: 'Engagement Events',
    key: 'engagement_events',
    description: 'Triggers when a form submission is completed, a referral is completed, or a promotion/reward is updated.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    resourceType: z.string().describe('Type of resource (form_submission, referral, promotion, reward)'),
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      return {
        inputs: [{
          eventType: ENGAGEMENT_EVENT_TYPES.includes(eventType) ? eventType : 'form_submission_completed',
