...
export let contactEvents = SlateTrigger.create(
  spec,
  {
    name: 'Contact Events',
    key: 'contact_events',
    description: 'Triggers when a contact is created, updated, or a new contact identifier is created.',
  }
)
  .input(z.object({
    ...
    webhookUuid: z.string().optional().describe('UUID of the webhook event for deduplication'),
  ...
  .output(z.object({
    ...
    identifierValue: z.string().optional().describe('Contact identifier value (for identifier events)'),
  ...
  .webhook({
      ...
      for (let eventType of CONTACT_EVENT_TYPES) {
        try {
          let result = await client.createWebhookSubscription({
            ...
            ...(eventType === 'contact_updated' ? { attributes: ['email', 'first_name', 'last_name', 'phone'] } : {}),
          });
    ...
    autoUnregisterWebhook: async (ctx) => {
      let client = new Client({ token: ctx.auth.token });
      let details = ctx.input.registrationDetails as { subscriptions: Array<{ subscriptionUuid: string }> };
    ...
    handleEvent: async (ctx) => {
      ...
      if (output.contactUuid && !output.email) {
        ...
        } catch { /* contact data enrichment failed */ }
      }
