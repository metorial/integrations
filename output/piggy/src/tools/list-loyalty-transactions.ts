...
export let listLoyaltyTransactions = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    type: z.enum(['credit_receptions', 'reward_receptions']).optional().describe('Filter by transaction type'),
    limit: z.number().min(1).max(100).optional().describe('Number of transactions per page (max 100)'),
    page: z.number().optional().describe('Page number'),
    sort: z.string().optional().describe('Sort field (e.g. "created_at" or "-created_at")'),
    createdAfter: z.string().optional().describe('Filter transactions after this ISO 8601 datetime'),
    createdBefore: z.string().optional().describe('Filter transactions before this ISO 8601 datetime'),
  }))
  .output(z.object({
