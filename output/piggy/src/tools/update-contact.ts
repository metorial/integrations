...
export let updateContact = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'System values (credit balance, creation date) cannot be updated.',
    ],
  }
)
  .input(z.object({
    contactUuid: z.string().describe('UUID of the contact to update'),
    attributes: z.record(z.string(), z.any()).describe('Key-value pairs of attributes to update (e.g. { "first_name": "John", "last_name": "Doe", "phone": "+31612345678" })'),
  }))
  .output(z.object({
    contactUuid: z.string().describe('UUID of the updated contact'),
    email: z.string().optional().describe('Email address'),
    updatedAttributes: z.record(z.string(), z.any()).optional().describe('Updated attribute values'),
  }))
