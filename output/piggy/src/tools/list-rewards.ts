...
let rewardSchema = z.object({
  ...
  rewardType: z.string().optional().describe('Type of reward (standard, digital, collectable)'),
}).passthrough();
...
export let listRewards = SlateTool.create(
  ...
  .input(z.object({
    ...
    contactUuid: z.string().optional().describe('Filter rewards claimable by a specific contact'),
  }))
  .output(z.object({
