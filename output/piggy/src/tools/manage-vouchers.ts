...
let voucherSchema = z.object({
  ...
  status: z.string().optional().describe('Voucher status (ACTIVE, REDEEMED, EXPIRED, DEACTIVATED, INACTIVE, LOCKED)'),
...
export let manageVouchers = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list', 'find', 'create', 'update', 'redeem']).describe('Action to perform'),
    // List params
    promotionUuid: z.string().optional().describe('Filter/assign by promotion UUID (required for create)'),
    contactUuid: z.string().optional().describe('Filter by or assign to contact UUID'),
    status: z.enum(['ACTIVE', 'REDEEMED', 'EXPIRED', 'DEACTIVATED', 'INACTIVE', 'LOCKED']).optional().describe('Filter by or set voucher status'),
