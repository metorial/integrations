...
let productOutputSchema = z.object({
  ...
  visibility: z.number().optional().describe('Product visibility (1=not visible, 2=catalog, 3=search, 4=catalog+search)'),
  typeId: z.string().optional().describe('Product type (simple, configurable, bundle, grouped, virtual, downloadable)'),
  ...
  customAttributes: z.array(customAttributeSchema).optional().describe('Custom product attributes'),
});
...
export let manageProduct = SlateTool.create(
  ...
  .input(z.object({
    ...
    name: z.string().optional().describe('Product name'),
    typeId: z.string().optional().describe('Product type: simple, configurable, bundle, grouped, virtual, downloadable'),
    attributeSetId: z.number().optional().describe('Attribute set ID (default is usually 4)'),
    price: z.number().optional().describe('Product price'),
    status: z.number().optional().describe('Product status: 1 (enabled), 2 (disabled)'),
    visibility: z.number().optional().describe('Visibility: 1 (not visible), 2 (catalog), 3 (search), 4 (catalog+search)'),
    weight: z.number().optional().describe('Product weight'),
    customAttributes: z.array(customAttributeSchema).optional().describe('Custom attributes to set on the product'),
  }))
  .output(z.object({
    product: productOutputSchema.optional().describe('Product details (for get/create/update)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.attributeSetId !== undefined) productData.attribute_set_id = ctx.input.attributeSetId;
