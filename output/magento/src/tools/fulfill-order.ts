...
export let fulfillOrder = SlateTool.create(
  spec,
  {
    name: 'Fulfill Order',
    key: 'fulfill_order',
  ...
  .input(z.object({
    ...
    lineItems: z.array(lineItemSchema).optional().describe('Specific items to process (omit to process all eligible items)'),
    capture: z.boolean().optional().describe('Whether to capture payment when invoicing (default: true)'),
    notify: z.boolean().optional().describe('Whether to notify the customer by email'),
    comment: z.string().optional().describe('Comment to include with the fulfillment action'),
    tracks: z.array(trackSchema).optional().describe('Tracking information (for ship action)'),
    adjustmentPositive: z.number().optional().describe('Adjustment refund amount to add (for refund action)'),
    adjustmentNegative: z.number().optional().describe('Adjustment fee to subtract (for refund action)'),
    shippingAmount: z.number().optional().describe('Shipping amount to refund (for refund action)'),
  }))
  .output(z.object({
