...
export let manageJobTool = SlateTool.create(
  spec,
  {
    name: 'Manage Job',
    key: 'manage_job',
  ...
  .input(z.object({
    ...
    action: z.enum(['parse', 'start', 'delete']).describe('The action to perform on the job'),
    autoStart: z.boolean().optional().describe('When parsing, whether to auto-start verification after parsing completes'),
    runSample: z.boolean().optional().describe('When starting, run as a free sample instead of full verification'),
  }))
  .output(z.object({
