...
export let recordingStatusChangeTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Recording Status Change',
    key: 'recording_status_change',
    description: 'Triggers when a recording\'s status changes (processing, done, failed, or deleted).',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    mediaUrl: z.string().nullable().describe('Pre-signed URL for the recording media, if available'),
  }))
  .webhook({
    handleRequest: async (ctx) => {
      ...
      return {
        inputs: [{
          ...
          botId,
          status,
          mediaUrl: recordingData.url ? String(recordingData.url) : (recordingData.media_url ? String(recordingData.media_url) : null),
        }],
      };
    },
