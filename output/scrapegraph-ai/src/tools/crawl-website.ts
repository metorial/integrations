...
Two modes: **AI extraction** (structured output with a prompt) and **markdown conversion** (no AI, lower cost). Supports crawl rules for include/exclude paths, sitemap-based discovery, and webhook notifications on completion.`,
    ...
    constraints: [
      'Maximum pages default is 20. Increase maxPages for larger crawls.',
    ],
    tags: {
      destructive: false,
      readOnly: true,
  ...
  .input(z.object({
    ...
    prompt: z.string().optional().describe('Instructions on what data to extract (required for AI extraction mode)'),
    depth: z.number().optional().describe('Number of link levels to follow (default: 1)'),
    breadth: z.number().optional().describe('Maximum links to follow per depth level (null for unlimited)'),
    maxPages: z.number().optional().describe('Maximum total pages to crawl (default: 20)'),
    extractionMode: z.boolean().optional().describe('true for AI-powered extraction (default), false for markdown conversion mode'),
    outputSchema: z.record(z.string(), z.unknown()).optional().describe('JSON schema defining the structure of the expected output per page'),
    rules: z.object({
      includePaths: z.array(z.string()).optional().describe('Path patterns to include (supports wildcards, e.g., "/products/*")'),
      excludePaths: z.array(z.string()).optional().describe('Path patterns to exclude (takes precedence over includePaths)'),
      sameDomain: z.boolean().optional().describe('Only crawl pages on the same domain'),
      exclude: z.array(z.string()).optional().describe('Regex patterns to exclude from crawling'),
    }).optional().describe('Rules for controlling crawl behavior'),
    sitemap: z.boolean().optional().describe('Use sitemap.xml for page discovery'),
    webhookUrl: z.string().optional().describe('URL to receive a POST notification when the crawl completes'),
    waitMs: z.number().optional().describe('Milliseconds to wait per page load (default: 3000)'),
  }))
  .output(z.object({
    status: z.string().describe('Status of the crawl job (e.g., "done")'),
    llmResult: z.unknown().optional().describe('Structured extraction results from AI processing'),
    ...
    })).optional().describe('Array of crawled pages with their content'),
