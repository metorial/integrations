...
export let getRequestStatus = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    service: z.enum(['smartscraper', 'searchscraper', 'markdownify', 'scrape', 'crawl', 'sitemap', 'agentic-scrapper']).describe('The service type the request was made to'),
    requestId: z.string().describe('The unique request ID returned from the original request'),
  }))
  .output(z.object({
    requestId: z.string().optional().describe('Unique identifier of the request'),
    status: z.string().describe('Current status of the request (queued, processing, completed, failed)'),
