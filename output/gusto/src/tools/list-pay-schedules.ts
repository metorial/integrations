...
export let listPaySchedules = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    paySchedules: z.array(z.object({
      payScheduleId: z.string().describe('UUID of the pay schedule'),
      frequency: z.string().optional().describe('Pay frequency (weekly, biweekly, semi-monthly, monthly)'),
