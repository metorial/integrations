...
export let emailActivityTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Email Activity',
    key: 'email_activity',
    description: '[Polling fallback] Polls for new email activity events including deliveries, bounces, opens, clicks, spam complaints, and unsubscribes. Use this as an alternative to webhooks for monitoring email events.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let lastPollDate = ctx.state?.lastPollDate || new Date(Date.now() - 5 * 60 * 1000).toISOString();
      ...
      let inputs = events.map((event: any) => ({
        ...
        eventId: event.event_id || `${event.email_id || ''}-${event.event || ''}-${event.date || ''}`,
      ...
      let newLastPollDate = events.length > 0
        ? events[0]!.date || lastPollDate
        : lastPollDate;
