...
export let emailEventsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Email Events',
    key: 'email_events',
    description: 'Triggers when email events occur, such as delivery, open, click, bounce, spam report, unsubscribe, or rejection. Receives real-time webhook notifications from SMTP2GO.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    bounceType: z.string().optional().describe('Hard or soft bounce classification (for bounce events)'),
  ...
  .webhook({
    autoRegisterWebhook: async (ctx) => {
      ...
      let result = await client.addWebhook({
        ...
        events: ['processed', 'delivered', 'open', 'click', 'bounce', 'spam', 'unsubscribe', 'resubscribe', 'reject'],
        outputType: 'json',
      });
    ...
    handleRequest: async (ctx) => {
      ...
      let inputs = events.map((event: any) => ({
        ...
        eventId: event.event_id || event.id || `${event.email_id || ''}-${event.event || ''}-${event.date || Date.now()}`,
