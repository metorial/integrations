...
let emailEventTypes = z.enum([
  ...
  'bounce', 'spam', 'unsubscribe', 'resubscribe', 'reject',
]);
...
export let createWebhook = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'Free plans are limited to 1 webhook; paid plans allow up to 10.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    ...
    events: z.array(emailEventTypes).optional().describe('Email event types to trigger the webhook'),
    smsEvents: z.boolean().optional().describe('Whether to include SMS events'),
    usernames: z.array(z.string()).optional().describe('SMTP users, API keys, or authenticated IPs to scope the webhook to'),
    outputType: z.enum(['json', 'form']).optional().describe('Webhook output format'),
    authorizationHeader: z.string().optional().describe('Authorization header value sent with webhook requests'),
    customHeaders: z.array(z.string()).optional().describe('Custom email headers to include in webhook data'),
  }))
  .output(z.object({
...
export let editWebhook = SlateTool.create(
  spec,
  {
    name: 'Edit Webhook',
    key: 'edit_webhook',
...
export let removeWebhook = SlateTool.create(
  spec,
  {
    name: 'Remove Webhook',
    key: 'remove_webhook',
