...
let docsetSchema = z.object({
  ...
  properties: z.record(z.string(), z.any()).optional().describe('Custom properties of the DocSet'),
...
export let listDocsets = SlateTool.create(
  spec,
  {
    name: 'List DocSets',
    key: 'list_docsets',
...
export let getDocset = SlateTool.create(
  spec,
  {
    name: 'Get DocSet',
    key: 'get_docset',
...
export let createDocset = SlateTool.create(
  spec,
  {
    name: 'Create DocSet',
    key: 'create_docset',
  ...
  .input(z.object({
    ...
    properties: z.record(z.string(), z.any()).optional().describe('Custom properties for the DocSet'),
    schema: z.object({
    ...
    }).optional().describe('Schema defining properties that can be extracted from documents'),
  }))
  .output(docsetSchema)
  .handleInvocation(async (ctx) => {
    let client = new Client({
...
export let updateDocset = SlateTool.create(
  spec,
  {
    name: 'Update DocSet',
    key: 'update_docset',
  ...
  .input(z.object({
    ...
    properties: z.record(z.string(), z.any()).optional().describe('Updated properties (replaces existing)'),
  }))
  .output(docsetSchema)
  .handleInvocation(async (ctx) => {
    let client = new Client({
...
export let deleteDocset = SlateTool.create(
  spec,
  {
    name: 'Delete DocSet',
    key: 'delete_docset',
