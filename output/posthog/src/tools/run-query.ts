...
export let runQueryTool = SlateTool.create(
  spec,
  {
...
Use this for custom analytics queries, aggregations, and data exploration that go beyond predefined insights.`,
  ...
  .input(z.object({
    query: z.record(z.string(), z.any()).describe('HogQL query object (e.g. { "kind": "HogQLQuery", "query": "SELECT event, count() FROM events GROUP BY event" })'),
  }))
  .output(z.object({
