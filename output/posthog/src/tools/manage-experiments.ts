...
export let listExperimentsTool = SlateTool.create(
  spec,
  {
...
export let getExperimentTool = SlateTool.create(
  spec,
  {
    name: 'Get Experiment',
    key: 'get_experiment',
...
export let createExperimentTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    parameters: z.record(z.string(), z.any()).optional().describe('Additional experiment parameters (metrics, holdout groups, etc.)'),
  }))
  .output(experimentOutput)
  .handleInvocation(async (ctx) => {
    let client = createClient(ctx.config, ctx.auth);
    ...
    if (ctx.input.featureFlagKey !== undefined) payload.feature_flag_key = ctx.input.featureFlagKey;
...
export let updateExperimentTool = SlateTool.create(
  spec,
  {
