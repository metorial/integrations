...
export let newEventsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'New Events',
    key: 'new_events',
    description: 'Triggers when new events are captured in PostHog. Polls for recent events using HogQL queries and deduplicates based on event UUID.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let lastTimestamp = ctx.state?.lastTimestamp || new Date(Date.now() - 60000).toISOString();
