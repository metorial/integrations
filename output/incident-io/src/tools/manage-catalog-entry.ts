...
export let manageCatalogEntry = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    attributeValues: z.record(z.string(), z.object({
      value: z.object({
        literal: z.string().optional(),
        catalogEntryId: z.string().optional(),
      }),
    })).optional().describe('Attribute values keyed by attribute ID'),
  }))
  .output(z.object({
    catalogEntryId: z.string().optional(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let transformedAttributes: Record<string, { value: { literal?: string; catalog_entry_id?: string } }> | undefined;
