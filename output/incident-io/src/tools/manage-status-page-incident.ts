...
let componentStatusSchema = z.object({
  ...
  status: z.string().describe('Status to set for the component (e.g., "operational", "degraded_performance", "partial_outage", "major_outage")'),
});

export let manageStatusPageIncident = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    statusPageIncidentId: z.string().optional().describe('Required for update and post_update actions'),
    ...
    incidentStatus: z.string().optional().describe('Status of the incident (e.g., "investigating", "identified", "monitoring", "resolved")'),
    message: z.string().optional().describe('Update message (required for create and post_update)'),
    notifySubscribers: z.boolean().optional().describe('Whether to notify status page subscribers'),
    componentStatuses: z.array(componentStatusSchema).optional().describe('Affected components and their statuses'),
    idempotencyKey: z.string().optional().describe('Deduplication key for create action; auto-generated if omitted'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (input.action === 'create') {
      if (!input.statusPageId || !input.name || !input.incidentStatus || !input.message) {
        throw new Error('statusPageId, name, incidentStatus, and message are required for creating a status page incident.');
      }
    ...
    if (input.action === 'update') {
      if (!input.statusPageIncidentId) {
        throw new Error('statusPageIncidentId is required for updating a status page incident.');
      }
    ...
    if (input.action === 'post_update') {
      if (!input.statusPageIncidentId || !input.message) {
        throw new Error('statusPageIncidentId and message are required for posting an update.');
      }
