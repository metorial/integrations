...
export let publishMediaTool = SlateTool.create(
  spec,
  {
    name: 'Publish Media',
    key: 'publish_media',
  ...
  .input(z.object({
    mediaType: z.enum(['IMAGE', 'REELS', 'STORIES', 'CAROUSEL']).describe('Type of media to publish'),
    imageUrl: z.string().optional().describe('Public URL of the image to publish (for IMAGE and STORIES types)'),
    videoUrl: z.string().optional().describe('Public URL of the video to publish (for REELS and STORIES types)'),
    caption: z.string().optional().describe('Caption for the post. Supports hashtags and mentions.'),
    ...
    shareToFeed: z.boolean().optional().describe('Whether to share Reels to the main feed (default: true)'),
    ...
    })).optional().describe('Users to tag in the image'),
    ...
    })).optional().describe('Items for carousel posts (2-10 items)'),
...
let waitForContainer = async (client: InstagramClient, containerId: string, maxAttempts = 30) => {
