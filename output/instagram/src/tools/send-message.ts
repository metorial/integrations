...
export let sendMessageTool = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['send', 'private_reply', 'list_conversations']).describe('The messaging action to perform'),
    recipientId: z.string().optional().describe('Instagram-scoped user ID of the message recipient — required for "send" action'),
    ...
    commentId: z.string().optional().describe('Comment ID to privately reply to — required for "private_reply" action'),
  ...
  .output(z.object({
    ...
    conversations: z.array(z.object({
      ...
      })).optional().describe('Conversation participants'),
    })).optional().describe('List of recent conversations (for "list_conversations" action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'private_reply') {
      if (!ctx.input.commentId) throw new Error('commentId is required for "private_reply" action');
