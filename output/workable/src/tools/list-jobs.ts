...
let jobSchema = z.object({
  ...
  departmentHierarchy: z.array(z.object({
    departmentId: z.string().optional(),
    name: z.string().optional()
  })).optional().describe('Department hierarchy'),
  location: z.object({
...
export let listJobsTool = SlateTool.create(
  ...
  .input(z.object({
    state: z.enum(['draft', 'published', 'closed', 'archived']).optional().describe('Filter jobs by state'),
    ...
    createdAfter: z.string().optional().describe('ISO 8601 date — only return jobs created after this date'),
    updatedAfter: z.string().optional().describe('ISO 8601 date — only return jobs updated after this date')
  }))
  .output(z.object({
