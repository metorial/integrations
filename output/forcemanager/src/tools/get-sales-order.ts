...
Fetch by ID or list/search with filtering. When retrieving by ID, line items can be included automatically.`,
  ...
  .input(z.object({
    ...
    includeLines: z.boolean().optional().describe('Include line items when fetching by ID (default: false)'),
    page: z.number().optional().describe('Page number (0-indexed)')
  }))
  .output(z.object({
    salesOrders: z.array(z.any()).describe('List of matching sales order records'),
    lines: z.array(z.any()).optional().describe('Line items (only when fetching single order with includeLines)'),
