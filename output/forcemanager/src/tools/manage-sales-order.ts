...
When creating an order, you can also include line items.`,
  ...
  .input(z.object({
    ...
    salesOrderId: z.number().optional().describe('Sales order ID (required for update and delete)'),
    fields: salesOrderFields.optional().describe('Order fields (required for create, optional for update)'),
    lines: z.array(orderLineSchema).optional().describe('Line items to add when creating an order')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'update') {
      ...
      return {
        output: { salesOrderId: ctx.input.salesOrderId, message: 'Sales order updated successfully', salesOrder },
    ...
    if (ctx.input.action === 'delete') {
      ...
      return {
        output: { salesOrderId: ctx.input.salesOrderId, message: 'Sales order deleted successfully' },
