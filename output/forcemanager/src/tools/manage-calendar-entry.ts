...
let calendarFields = z.object({
  ...
  isTask: z.boolean().optional().describe('Whether this is a task (true) or an event (false)'),
  completed: z.boolean().optional().describe('Whether the task is completed'),
  comment: z.string().optional().describe('Notes or description'),
  notificationMinutes: z.number().optional().describe('Notification time in minutes before the event'),
  extId: z.string().optional().describe('External system ID'),
}).describe('Calendar entry fields');

export let manageCalendarEntry = SlateTool.create(
  spec,
  {
...
Calendar entries can be events or tasks, support all-day scheduling, notifications, and can be linked to accounts, contacts, and opportunities.`,
  ...
  .input(z.object({
    ...
    entryId: z.number().optional().describe('Calendar entry ID (required for update and delete)'),
    fields: calendarFields.optional().describe('Calendar entry fields (required for create, optional for update)')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'update') {
      ...
      return {
        output: { entryId: ctx.input.entryId, message: 'Calendar entry updated successfully', entry },
