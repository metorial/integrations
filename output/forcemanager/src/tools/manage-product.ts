...
Products include model names, pricing, cost, category/family classification, and discount limits.`,
  ...
  .input(z.object({
    ...
    fields: productFields.optional().describe('Product fields (required for create, optional for update)')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'update') {
      ...
      return {
        output: { productId: ctx.input.productId, message: 'Product updated successfully', product },
