...
export let createCoupon = SlateTool.create(
  spec,
  {
    ...
    constraints: ['Maximum 200 coupons per form.', 'Stripe coupons cannot be managed through this endpoint.'],
  }
)
  .input(z.object({
    formSlugOrId: z.string().describe('The form slug, custom slug, or unique ID'),
    ...
    expiresAt: z.string().nullable().optional().describe('Expiration date-time in UTC, or null for no expiry'),
  }))
