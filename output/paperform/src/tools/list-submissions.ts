...
export let listSubmissions = SlateTool.create(
  ...
  .input(z.object({
    ...
    sort: z.enum(['ASC', 'DESC']).optional().describe('Sort direction by creation date (default DESC)'),
    beforeDate: z.string().optional().describe('Return results created before this UTC date-time'),
    afterDate: z.string().optional().describe('Return results created after this UTC date-time'),
  }))
  .output(z.object({
