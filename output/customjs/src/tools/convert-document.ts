...
export let convertDocument = SlateTool.create(
  ...
  .input(z.object({
    conversion: z.enum(['markdown_to_pdf', 'markdown_to_html', 'html_to_docx']).describe('The conversion to perform.'),
    content: z.string().describe('The source content to convert (Markdown or HTML depending on the conversion type).'),
  }))
  .output(z.object({
    resultBase64: z.string().optional().describe('Base64-encoded binary file. Returned for markdown_to_pdf and html_to_docx.'),
