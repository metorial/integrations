...
export let generateImage = SlateTool.create(
  ...
  .input(z.object({
    mode: z.enum(['html_to_png', 'screenshot']).describe('Whether to convert HTML to PNG or capture a website screenshot.'),
    html: z.string().optional().describe('HTML content to convert to PNG. Required for html_to_png mode.'),
    url: z.string().optional().describe('URL of the website to capture. Required for screenshot mode.'),
    commands: z.array(z.object({
      action: z.enum(['click', 'type', 'scroll', 'wait', 'waitForSelector', 'hover']).describe('Browser action to perform.'),
      selector: z.string().optional().describe('CSS selector for the target element. Required for click, type, waitForSelector, hover.'),
      value: z.union([z.string(), z.number()]).optional().describe('Value for the action: text for type, milliseconds for wait, pixels for scroll.'),
    })).optional().describe('Browser automation commands to run before taking the screenshot. Only for screenshot mode.'),
    cropBox: z.object({
    ...
    }).optional().describe('Bounding box to crop the screenshot. Only for screenshot mode.'),
  }))
  .output(z.object({
