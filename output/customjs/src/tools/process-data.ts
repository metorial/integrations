...
export let processData = SlateTool.create(
  spec,
  {
    name: 'Process Data',
    key: 'process_data',
  ...
  .input(z.object({
    mode: z.enum(['json_path', 'regex']).describe('Processing mode: json_path for JSONPath queries, regex for regex operations.'),
    jsonData: z.string().optional().describe('JSON string to query. Required for json_path mode.'),
    jsonPath: z.string().optional().describe('JSONPath expression, e.g., "$[0].name" or "$.users[*].email". Required for json_path mode.'),
    text: z.string().optional().describe('Input text to process. Required for regex mode.'),
    regexPattern: z.string().optional().describe('Regular expression pattern. Required for regex mode.'),
    regexFlags: z.string().optional().describe('Regex flags (e.g., "g", "i", "gi"). Optional for regex mode.'),
    regexOperation: z.enum(['Extract', 'Replace', 'Test', 'Split']).optional().describe('Regex operation to perform. Required for regex mode.'),
    replacement: z.string().optional().describe('Replacement text for the Replace operation.'),
  }))
  .output(z.object({
    result: z.any().describe('The processing result. Type depends on operation: array for JSONPath/Extract/Split, string for Replace, boolean for Test.'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.mode === 'json_path') {
      ...
      if (!ctx.input.regexOperation) throw new Error('regexOperation is required for regex mode.');
