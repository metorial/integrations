...
export let auth = SlateAuth.create()
  ...
  .addOauth({
    ...
    scopes: [
      {
        title: 'Signature',
        description: 'Access to the eSignature REST API for sending, signing, and managing envelopes',
        scope: 'signature',
      },
      {
        title: 'Extended',
    ...
    inputSchema: z.object({
      environment: z.enum(['demo', 'production']).default('demo').describe('DocuSign environment'),
    }),
    ...
    handleCallback: async (ctx) => {
      ...
      let tokenResponse = await axiosInstance.post('/oauth/token', new URLSearchParams({
      ...
      let defaultAccount = userInfo.accounts?.find((a: any) => a.is_default) || userInfo.accounts?.[0];
    ...
    getProfile: async (ctx: any) => {
      ...
      return {
        profile: {
          ...
          name: userInfo.name,
          accountId: ctx.output.accountId,
          accountName: userInfo.accounts?.find((a: any) => a.account_id === ctx.output.accountId)?.account_name,
        },
      };
    },
  });
