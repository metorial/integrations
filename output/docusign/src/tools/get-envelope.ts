...
export let getEnvelope = SlateTool.create(
  spec,
  {
    name: 'Get Envelope',
    key: 'get_envelope',
  ...
  .input(z.object({
    ...
    includeRecipients: z.boolean().optional().default(false).describe('Include recipient details in the response'),
    includeDocuments: z.boolean().optional().default(false).describe('Include document listing in the response'),
    includeTabs: z.boolean().optional().default(false).describe('Include tab (form field) data for recipients'),
  }))
  .output(z.object({
    envelopeId: z.string().describe('ID of the envelope'),
    status: z.string().describe('Current status of the envelope (e.g., sent, delivered, completed, declined, voided)'),
    ...
    recipients: z.any().optional().describe('Recipient details (signers, carbon copies, etc.)'),
    documents: z.array(z.object({
      documentId: z.string(),
      name: z.string(),
      order: z.string().optional(),
    })).optional().describe('List of documents in the envelope'),
  }))
