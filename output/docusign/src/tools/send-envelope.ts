...
let signerSchema = z.object({
  ...
  recipientId: z.string().optional().describe('Unique ID for the recipient within the envelope. Auto-assigned if omitted.'),
  routingOrder: z.string().optional().describe('Signing order (e.g., "1", "2"). Use same value for parallel signing.'),
  clientUserId: z.string().optional().describe('Set this to make the signer an embedded signer. Must be unique per signer.'),
  tabs: z.record(z.string(), z.any()).optional().describe('Tabs (form fields) for this signer, e.g., signHereTabs, dateSignedTabs'),
});
...
let documentSchema = z.object({
  ...
  documentId: z.string().describe('Unique ID for the document within the envelope (e.g., "1", "2")'),
  documentBase64: z.string().describe('Base64-encoded document content'),
  fileExtension: z.string().optional().describe('File extension (e.g., "pdf", "docx"). Inferred from name if omitted.'),
  order: z.string().optional().describe('Display order of the document'),
});

export let sendEnvelope = SlateTool.create(
  spec,
  {
    name: 'Send Envelope',
    key: 'send_envelope',
...
Set **status** to \`"sent"\` to send immediately or \`"created"\` to save as a draft.`,
  ...
  .input(z.object({
    ...
    carbonCopies: z.array(carbonCopySchema).optional().describe('Recipients who receive a copy of the signed documents'),
    notification: z.object({
      reminderEnabled: z.boolean().optional().describe('Enable email reminders'),
      reminderDelay: z.string().optional().describe('Days before first reminder (e.g., "3")'),
      reminderFrequency: z.string().optional().describe('Days between reminders (e.g., "2")'),
      ...
      expireWarn: z.string().optional().describe('Days before expiration to warn (e.g., "3")'),
    }).optional().describe('Reminder and expiration settings'),
  }))
  .output(z.object({
