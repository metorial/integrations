...
export let downloadDocument = SlateTool.create(
  ...
  .input(z.object({
    ...
    documentId: z.string().optional().describe('ID of the specific document to download. Use "combined" for all documents, "certificate" for the certificate of completion.'),
    listOnly: z.boolean().optional().default(false).describe('If true, only lists available documents without downloading'),
  }))
  .output(z.object({
    ...
    })).optional().describe('List of available documents in the envelope'),
    documentBase64: z.string().optional().describe('Base64-encoded document content (when downloading)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let docName = targetDoc?.name || (targetId === 'combined' ? 'Combined Documents' : targetId === 'certificate' ? 'Certificate of Completion' : `Document ${targetId}`);
