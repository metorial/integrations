...
let emergencyContactSchema = z.object({
...
Only provide the fields you want to change; omitted fields remain unchanged.`,
  ...
  .input(z.object({
    ...
    reportsToEmployeeId: z.string().optional().nullable().describe('Manager employee ID, or null to make head of company'),
    ...
    maritalStatus: z.enum(['Single', 'Separated', 'Married', 'Widowed']).optional().describe('Marital status'),
    ...
    driverLicenseIssuedDate: z.string().optional().describe('Driver license issued date (YYYY-MM-DD)'),
    driverLicenseExpiryDate: z.string().optional().describe('Driver license expiry date (YYYY-MM-DD)'),
    driverLicenseIssuingCountry: z.string().optional().describe('Driver license issuing country'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.emergencyContact) {
      ...
      if (ctx.input.emergencyContact.fullName !== undefined) ec.full_name = ctx.input.emergencyContact.fullName;
      if (ctx.input.emergencyContact.relationshipTypeId !== undefined) ec.relationship_type_id = ctx.input.emergencyContact.relationshipTypeId;
      if (ctx.input.emergencyContact.phone !== undefined) ec.phone = ctx.input.emergencyContact.phone;
      if (ctx.input.emergencyContact.address !== undefined) ec.address = ctx.input.emergencyContact.address;
