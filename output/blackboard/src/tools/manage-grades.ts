...
export let createGradeColumn = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    ...
    let column = await client.createGradeColumn(ctx.input.courseId, {
      ...
      grading: (ctx.input.gradingType || ctx.input.dueDate || ctx.input.attemptsAllowed) ? {
...
export let listGradeColumns = SlateTool.create(
  spec,
  {
...
export let updateGradeColumn = SlateTool.create(
  spec,
  {
...
export let recordGrade = SlateTool.create(
  spec,
  {
    name: 'Record Grade',
    key: 'record_grade',
  ...
  .handleInvocation(async (ctx) => {
    ...
    let grade = await client.updateGrade(ctx.input.courseId, ctx.input.columnId, ctx.input.userId, {
...
export let getGrade = SlateTool.create(
  spec,
  {
    name: 'Get Grade',
    key: 'get_grade',
  ...
  .handleInvocation(async (ctx) => {
    let client = new Client({ baseUrl: ctx.config.baseUrl, token: ctx.auth.token });
    let grade = await client.getGrade(ctx.input.courseId, ctx.input.columnId, ctx.input.userId);
...
export let listGrades = SlateTool.create(
  spec,
  {
    name: 'List Grades',
    key: 'list_grades',
  ...
  .input(z.object({
    courseId: z.string().describe('Course identifier'),
    columnId: z.string().optional().describe('Grade column ID — list all student grades for this column'),
    userId: z.string().optional().describe('User identifier — list all column grades for this user'),
    offset: z.number().optional().describe('Number of items to skip'),
    limit: z.number().optional().describe('Maximum results to return'),
  }))
  .output(z.object({
