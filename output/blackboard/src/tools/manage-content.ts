...
export let createContent = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    body: z.string().optional().describe('Content body (HTML)'),
    description: z.string().optional().describe('Content description'),
    parentId: z.string().optional().describe('Parent folder ID — omit to add to the top level'),
    position: z.number().optional().describe('Position among siblings (0-based)'),
    contentHandlerId: z.string().optional().describe('Content handler type (e.g., "resource/x-bb-document")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let content = await client.createContent(ctx.input.courseId, {
      ...
      availability: {
        ...
        adaptiveRelease: (ctx.input.releaseStart || ctx.input.releaseEnd) ? {
...
export let listContent = SlateTool.create(
  ...
  .input(z.object({
    courseId: z.string().describe('Course identifier'),
    parentId: z.string().optional().describe('Parent folder ID — omit for top-level content'),
...
export let updateContent = SlateTool.create(
  spec,
  {
...
export let deleteContent = SlateTool.create(
  spec,
  {
