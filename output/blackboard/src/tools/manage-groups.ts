...
export let createGroup = SlateTool.create(
  ...
  .input(z.object({
    ...
    enrollmentType: z.enum(['InstructorOnly', 'SelfEnrollment', 'SignupSheet']).optional().describe('How members can join'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let group = await client.createCourseGroup(ctx.input.courseId, {
      ...
      enrollment: (ctx.input.enrollmentType || ctx.input.memberLimit) ? {
...
export let manageGroupMembers = SlateTool.create(
  ...
  .output(z.object({
    ...
    })).optional().describe('List of group members (for list action)'),
