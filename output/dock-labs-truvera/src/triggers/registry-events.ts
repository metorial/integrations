...
export let registryEvents = SlateTrigger.create(
  spec,
  {
    ...
    key: 'registry_events',
    description: 'Triggers when a revocation registry is created or deleted. Configure the webhook URL in the Dock Certs dashboard under Developer > Webhooks.',
  }
)
  .input(z.object({
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let registryId = (data.registryId ?? (data.data as Record<string, unknown> | undefined)?.id) as string | undefined;
