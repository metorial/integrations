...
export let credentialEvents = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers when a credential is created, issued, revoked, or unrevoked. Configure the webhook URL in the Dock Certs dashboard under Developer > Webhooks.',
  }
)
  .input(z.object({
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let credentialId = (data.credentialId ?? (data.data as Record<string, unknown> | undefined)?.id) as string | undefined;
      let timestamp = (data.timestamp ?? data.created_at ?? new Date().toISOString()) as string;

      // Only process credential events
      let credentialEventTypes = ['credential_create', 'credential_issued', 'credential_revoke', 'credential_unrevoke'];
