...
export let schemaEvents = SlateTrigger.create(
  spec,
  {
    name: 'Schema Events',
    key: 'schema_events',
    description: 'Triggers when a credential schema is created. Configure the webhook URL in the Dock Certs dashboard under Developer > Webhooks.',
  }
)
  .input(z.object({
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let schemaId = (data.schemaId ?? (data.data as Record<string, unknown> | undefined)?.id) as string | undefined;
