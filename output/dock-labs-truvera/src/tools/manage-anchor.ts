...
export let manageAnchor = SlateTool.create(
  spec,
  {
    name: 'Manage Anchor',
    key: 'manage_anchor',
...
Multiple documents can be batched into a single anchor via a Merkle tree to save cost.`,
  ...
  .input(z.object({
    ...
    documents: z.array(z.string()).optional().describe('Document strings or hashes to anchor or verify against'),
  ...
  .output(z.object({
    anchor: z.record(z.string(), z.unknown()).optional().describe('The anchor document with root hash and proofs'),
    anchors: z.array(z.record(z.string(), z.unknown())).optional().describe('List of anchors'),
    verified: z.boolean().optional().describe('Whether the anchor verification passed'),
    verificationResult: z.record(z.string(), z.unknown()).optional().describe('Detailed verification result'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'list': {
        let results = await client.listAnchors({ offset: ctx.input.offset, limit: ctx.input.limit });
        return {
