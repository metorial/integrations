...
export let getCredentials = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    credentialId: z.string().optional().describe('Credential ID to retrieve. Omit to list all credentials'),
  ...
  .output(z.object({
    credential: z.record(z.string(), z.unknown()).optional().describe('The retrieved credential document'),
    credentials: z.array(z.record(z.string(), z.unknown())).optional().describe('List of credentials'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let results = await client.listCredentials({ offset: ctx.input.offset, limit: ctx.input.limit });
    return {
