...
Also supports revoking and unrevoking credentials within a registry.`,
  ...
  .input(z.object({
    action: z.enum(['create', 'get', 'list', 'delete', 'revoke', 'unrevoke']).describe('Operation to perform'),
    registryId: z.string().optional().describe('Registry ID (required for get, delete, revoke, unrevoke)'),
    policyDids: z.array(z.string()).optional().describe('DIDs that control this registry (required for create)'),
    registryType: z.enum(['StatusList2021Entry', 'CredentialStatusList2017', 'DockVBAccumulator2022']).optional().describe('Type of revocation registry'),
    addOnly: z.boolean().optional().describe('If true, revocations cannot be undone'),
    credentialIds: z.array(z.string()).optional().describe('Credential IDs to revoke or unrevoke'),
  ...
  .output(z.object({
    ...
    registries: z.array(z.record(z.string(), z.unknown())).optional().describe('List of registries'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'list': {
        let results = await client.listRegistries({ offset: ctx.input.offset, limit: ctx.input.limit });
        return {
      ...
      case 'revoke': {
        ...
        let result = await client.revokeCredential(ctx.input.registryId, ctx.input.credentialIds);
        return {
          output: {
            registry: result,
            jobId: result.id as string | undefined,
      ...
      case 'unrevoke': {
        if (!ctx.input.registryId) throw new Error('registryId is required for unrevoke action');
        if (!ctx.input.credentialIds || ctx.input.credentialIds.length === 0) {
          throw new Error('credentialIds is required for unrevoke action');
        }
        let result = await client.unrevokeCredential(ctx.input.registryId, ctx.input.credentialIds);
        return {
          output: {
            registry: result,
            jobId: result.id as string | undefined,
