...
let templateOutputSchema = z.object({
  ...
  publishedVersionId: z.string().optional().nullable().describe('Currently published version ID'),
...
export let createTemplate = SlateTool.create(
  ...
  .input(z.object({
    html: z.string().describe('HTML content of the template. Use {{variable}} for merge variables.'),
    ...
    engine: z.enum(['legacy', 'handlebars']).optional().describe('Template engine. Defaults to legacy.'),
  }))
...
export let listTemplates = SlateTool.create(
  ...
  .input(z.object({
    limit: z.number().optional().describe('Number of templates to return (max 100, default 10)'),
...
export let updateTemplate = SlateTool.create(
  ...
  .input(z.object({
    ...
    publishedVersion: z.string().optional().describe('Version ID to publish (starts with "vrsn_")'),
  }))
  .output(templateOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
...
export let createTemplateVersion = SlateTool.create(
  spec,
  {
