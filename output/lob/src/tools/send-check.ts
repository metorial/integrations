...
export let sendCheck = SlateTool.create(
  spec,
  {
    name: 'Send Check',
    key: 'send_check',
  ...
  .input(z.object({
    ...
    message: z.string().optional().describe('Message to print in the letter accompanying the check'),
    ...
    checkBottom: z.string().optional().describe('Custom artwork for the check bottom (HTML or template ID)'),
    attachment: z.string().optional().describe('Additional page to include with the check (HTML, template ID, or PDF URL)'),
...
export let cancelCheck = SlateTool.create(
  spec,
  {
    name: 'Cancel Check',
    key: 'cancel_check',
