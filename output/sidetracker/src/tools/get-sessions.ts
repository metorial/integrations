...
export let getSessions = SlateTool.create(
  ...
  .output(z.object({
    ...
    sessions: z.array(z.record(z.string(), z.unknown())).describe('Array of session data objects'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
    let result = await client.getSessions(ctx.input.domainId, ctx.input.page, ctx.input.pageSize);
