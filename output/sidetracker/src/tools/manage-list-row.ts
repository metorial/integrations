...
export let manageListRow = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['add', 'update', 'delete']).describe('The operation to perform on the list row'),
    listId: z.string().describe('Unique ID of the target list'),
    rowId: z.string().optional().describe('ID of the row to update or delete'),
    sessionId: z.string().optional().describe('Session ID to delete the associated row (only for delete action)'),
    rowData: z.record(z.string(), z.unknown()).optional().describe('Key-value data for the row (required for add/update)'),
  }))
  .output(z.object({
    success: z.boolean().describe('Whether the operation was successful'),
    row: z.record(z.string(), z.unknown()).optional().describe('The created or updated row data (not returned for delete)'),
  }))
