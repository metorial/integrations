...
let employeeSchema = z.object({
  ...
  employmentType: z.string().optional().describe('Employment type (e.g. full-time, part-time, contractor)'),
...
export let listEmployees = SlateTool.create(
  ...
  .input(z.object({
    includeTerminated: z.boolean().optional().default(false).describe('Include terminated employees in the results'),
    sendAllRoles: z.boolean().optional().describe('When including terminated employees, set to true to return all roles regardless of provisioning rules'),
