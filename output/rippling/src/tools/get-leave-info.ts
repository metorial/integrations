...
export let listLeaveTypes = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    managedBy: z.string().optional().describe('Filter by managing system (PTO, LEAVES, TILT)'),
  }))
  .output(z.object({
...
export let getLeaveBalances = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    roleId: z.string().describe('The role ID (employee identifier) to get leave balances for'),
  }))
  .output(z.object({
    roleId: z.string().describe('Employee role ID'),
    balances: z.array(z.object({
      companyLeaveTypeId: z.string().optional().describe('Company leave type identifier'),
      isUnlimited: z.boolean().optional().describe('Whether the balance is unlimited'),
      remainingBalanceMinutes: z.number().optional().describe('Remaining balance in minutes'),
      remainingBalanceMinutesWithFuture: z.number().optional().describe('Remaining balance in minutes including future leave requests'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let balances = (Array.isArray(data?.balances || data) ? (data.balances || data) : []).map((b: any) => ({
