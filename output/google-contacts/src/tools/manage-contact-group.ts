...
export let createContactGroup = SlateTool.create(
  ...
  .input(z.object({
    ...
    clientData: z.array(customFieldSchema).optional().describe('Optional key-value pairs for client-specific data'),
  }))
...
export let updateContactGroup = SlateTool.create(
  ...
  .input(z.object({
    resourceName: z.string().describe('Resource name of the contact group (e.g., "contactGroups/abc123")'),
    name: z.string().describe('New name for the contact group'),
    clientData: z.array(customFieldSchema).optional().describe('Updated key-value pairs for client-specific data'),
  }))
  .output(contactGroupSchema)
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
    let result = await client.updateContactGroup(ctx.input.resourceName, ctx.input.name, ctx.input.clientData);
    let group = formatContactGroup(result);
...
export let deleteContactGroup = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    resourceName: z.string().describe('Resource name of the contact group to delete (e.g., "contactGroups/abc123")'),
    deleteContacts: z.boolean().optional().describe('If true, also deletes the contacts in the group (default false)'),
  }))
  .output(z.object({
