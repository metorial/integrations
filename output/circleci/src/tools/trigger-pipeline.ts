...
export let triggerPipeline = SlateTool.create(
  spec,
  {
...
The project slug format is \`vcs-slug/org-name/repo-name\` (e.g., \`gh/my-org/my-repo\` or \`github/my-org/my-repo\`).`,
  ...
  .input(z.object({
    projectSlug: z.string().describe('Project slug in the format vcs-slug/org-name/repo-name (e.g., gh/my-org/my-repo)'),
    branch: z.string().optional().describe('Branch to trigger the pipeline on'),
    tag: z.string().optional().describe('Tag to trigger the pipeline on (mutually exclusive with branch)'),
    parameters: z.record(z.string(), z.any()).optional().describe('Custom pipeline parameters as key-value pairs')
  }))
  .output(z.object({
