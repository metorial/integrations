...
export let deletePost = SlateTool.create(
  ...
  .input(z.object({
    ...
    bulk: z.array(z.string()).optional().describe('Array of Ayrshare Post IDs for bulk deletion'),
    deleteAllScheduled: z.boolean().optional().describe('Delete all pending scheduled posts for the profile'),
  }))
  .output(z.object({
    status: z.string().describe('Deletion status'),
    deletedPosts: z.array(z.record(z.string(), z.unknown())).optional().describe('Details of deleted posts per platform'),
  }))
