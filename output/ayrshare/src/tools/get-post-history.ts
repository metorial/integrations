...
export let getPostHistory = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    limit: z.number().min(1).max(1000).optional().describe('Maximum number of posts to return (default: 25, max: 1000)'),
    ...
    lastDays: z.number().optional().describe('Return posts from the last N days (default: 30, 0 for all)'),
    status: z.enum(['success', 'error', 'processing', 'pending', 'paused', 'deleted', 'awaiting approval']).optional().describe('Filter by post status'),
