...
Requires Business plan.`,
  ...
  .input(z.object({
    title: z.string().describe('Unique title for the profile (displayed on social account linking page)'),
    disableSocial: z.array(platformEnum).optional().describe('Social networks to disable for this profile'),
    ...
    email: z.string().optional().describe('Email address (required if team is true, receives invite email)'),
  }))
  .output(z.object({
...
export let getProfiles = SlateTool.create(
  ...
  .input(z.object({
    limit: z.number().min(1).max(5000).optional().describe('Maximum profiles to return (default: 5000)'),
    ...
    hasActiveSocialAccounts: z.boolean().optional().describe('Filter for profiles with or without connected social accounts'),
  }))
  .output(z.object({
    profiles: z.array(z.object({
      ...
      activeSocialAccounts: z.array(z.string()).optional().describe('Connected social platforms'),
...
export let deleteProfile = SlateTool.create(
  ...
  .input(z.object({
    ...
    title: z.string().optional().describe('Profile title (alternative to Profile Key for identification)'),
  }))
  .output(z.object({
