...
export let socialAccountTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Social Account Changed',
    key: 'social_account_changed',
    description: 'Triggered when a user profile links, unlinks, or refreshes a social network connection. Covers both user-initiated and system-initiated (e.g., expired token) actions.',
  },
)
  .input(z.object({
  ...
  .output(z.object({
    platform: z.string().optional().describe('Social platform that was linked/unlinked/refreshed'),
    actionType: z.string().optional().describe('Action type: link, unlink, or refresh'),
