...
Useful for archiving web pages, generating reports from web content, or taking screenshots for documentation.`,
  ...
  .input(z.object({
    ...
    outputFormat: z.enum(['pdf', 'png', 'jpg']).describe('Output format: "pdf" for document, "png" or "jpg" for screenshot'),
    ...
    printBackground: z.boolean().optional().describe('Include background graphics (PDF only)'),
    displayHeaderFooter: z.boolean().optional().describe('Display header and footer (PDF only)'),
    waitUntil: z.string().optional().describe('When to consider navigation complete (e.g., "load", "networkidle0")'),
    ...
    waitForCompletion: z.boolean().optional().default(true).describe('Wait for capture to complete'),
  }))
  .output(z.object({
    jobId: z.string().describe('ID of the capture job'),
    status: z.string().describe('Current status of the job'),
    resultUrl: z.string().optional().describe('Temporary download URL for the captured file'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.marginBottom !== undefined) captureTask.margin_bottom = ctx.input.marginBottom;
    ...
    if (ctx.input.printBackground !== undefined) captureTask.print_background = ctx.input.printBackground;
    if (ctx.input.displayHeaderFooter !== undefined) captureTask.display_header_footer = ctx.input.displayHeaderFooter;
