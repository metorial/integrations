...
export let processPdf = SlateTool.create(
  spec,
  {
    name: 'Process PDF',
    key: 'process_pdf',
...
Select the desired operation and configure its specific options. Results are exported to a temporary URL.`,
  ...
  .input(z.object({
    ...
    operation: z.enum(['ocr', 'encrypt', 'decrypt', 'split', 'extract', 'rotate', 'pdfa']).describe('PDF operation to perform'),
    password: z.string().optional().describe('Password for decryption, or user password for encryption'),
    ownerPassword: z.string().optional().describe('Owner password for encryption'),
    pages: z.string().optional().describe('Page specification (e.g., "1-3,5" for extract/split)'),
    rotation: z.number().optional().describe('Rotation angle in degrees (90, 180, 270) for rotate operation'),
    ...
    options: z.record(z.string(), z.any()).optional().describe('Additional operation-specific options'),
    tag: z.string().optional().describe('Tag to label the job'),
    waitForCompletion: z.boolean().optional().default(true).describe('Wait for processing to complete'),
  }))
  .output(z.object({
    jobId: z.string().describe('ID of the PDF processing job'),
    status: z.string().describe('Current status of the job'),
    resultUrls: z.array(z.object({
      url: z.string().describe('Temporary download URL'),
      filename: z.string().describe('Filename of the result'),
    })).optional().describe('Download URLs for the processed files (split may produce multiple)'),
  }))
