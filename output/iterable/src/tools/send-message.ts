...
export let sendMessage = SlateTool.create(
  ...
  .input(z.object({
    channel: z.enum(['email', 'push', 'sms', 'inApp', 'webPush']).describe('Messaging channel to use'),
    ...
    sendAt: z.string().optional().describe('ISO 8601 datetime to schedule the send. If omitted, sends immediately.'),
    allowRepeatMarketingSends: z.boolean().optional().describe('If true, allows sending marketing messages to users who have already received one in this campaign'),
    templateFields: z.record(z.string(), z.any()).optional().describe('Dynamic data fields passed to the template for personalization')
  }))
  .output(z.object({
