...
export let getWidget = SlateTool.create(
  spec,
  {
    name: 'Get Widget',
    key: 'get_widget',
  ...
  .input(z.object({
    ...
    render: z.boolean().optional().describe('If true, renders the widget content instead of returning raw data.'),
    widgetName: z.string().optional().describe('Widget name to use when rendering (alternative to widgetId for render).'),
  }))
  .output(z.object({
...
export let updateWidget = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Only one widget can be updated at a time.',
    ],
  }
)
  .input(z.object({
