...
export let booksManageInvoice = SlateTool.create(spec, {
  ...
  .input(z.object({
    ...
    action: z.enum(['create', 'update', 'delete', 'status']).describe('Operation to perform'),
    invoiceId: z.string().optional().describe('Invoice ID (required for update, delete, status)'),
    ...
    discount: z.string().optional().describe('Discount at the invoice level (e.g., "10%" or "50.00")'),
    statusAction: z.enum(['sent', 'void', 'draft']).optional().describe('Status action to apply (for action="status")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'status') {
      ...
      if (!ctx.input.statusAction) throw new Error('statusAction is required for status change');
      let result = await client.markInvoiceStatus(ctx.input.invoiceId, ctx.input.statusAction);
      return {
        output: { invoiceId: ctx.input.invoiceId, status: ctx.input.statusAction, message: result?.message },
