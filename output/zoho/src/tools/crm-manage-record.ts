...
export let crmManageRecord = SlateTool.create(spec, {
  ...
  .input(z.object({
    module: z.string().describe('CRM module API name (e.g., "Leads", "Contacts", "Accounts", "Deals")'),
    action: z.enum(['create', 'update', 'delete']).describe('Operation to perform'),
    recordId: z.string().optional().describe('Record ID (required for update and delete)'),
    recordData: z.union([
      z.array(z.record(z.string(), z.any())),
      z.record(z.string(), z.any()),
    ]).optional().describe('Record data: an object for update, or array of objects for create. Field API names as keys.'),
    triggers: z.array(z.enum(['workflow', 'approval', 'blueprint'])).optional().describe('Workflow triggers to execute on this operation'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      let records = Array.isArray(ctx.input.recordData) ? ctx.input.recordData : [ctx.input.recordData || {}];
    ...
    if (ctx.input.action === 'update') {
      ...
      let data = Array.isArray(ctx.input.recordData) ? ctx.input.recordData[0] || {} : ctx.input.recordData || {};
      let result = await client.updateRecord(ctx.input.module, ctx.input.recordId, data, ctx.input.triggers);
      let results = (result?.data || []).map((r: any) => ({
