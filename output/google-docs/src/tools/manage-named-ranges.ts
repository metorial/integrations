  ...
  .input(z.object({
    ...
    operations: z.array(z.discriminatedUnion('action', [
      ...
      z.object({
        ...
        name: z.string().optional().describe('Name of the named range to delete (alternative to namedRangeId)')
      })
    ])).min(1).describe('List of named range operations to perform')
  }))
  .output(z.object({
    documentId: z.string().describe('ID of the document'),
    operationsApplied: z.number().describe('Number of operations successfully applied'),
    createdRangeIds: z.array(z.string()).optional().describe('IDs of newly created named ranges')
  }))
