...
export let listInsights = SlateTool.create(
  spec,
  {
    name: 'List Insights',
    key: 'list_insights',
  ...
  .input(z.object({
    userId: z.string().optional().describe('Filter insights by a specific user ID. If provided, returns only that user\'s personal insights.'),
    sort: z.enum(['created_at:asc', 'created_at:desc', 'title:asc', 'title:desc']).optional().describe('Sort order (only for workspace-wide listing)'),
