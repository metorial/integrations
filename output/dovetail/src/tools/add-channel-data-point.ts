...
export let addChannelDataPoint = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    timestamp: z.string().describe('ISO 8601 timestamp of when the feedback was originally created'),
    sourceTitle: z.string().optional().describe('Title of the feedback source (e.g., ticket subject, review title)'),
    sourceUrl: z.string().optional().describe('URL linking back to the original feedback source'),
    metadata: z.record(z.string(), z.unknown()).optional().describe('Additional metadata key-value pairs to attach'),
  }))
  .output(z.object({
