...
export let getInsight = SlateTool.create(
  spec,
  {
    name: 'Get Insight',
    key: 'get_insight',
  ...
  .input(z.object({
    ...
    exportFormat: z.enum(['html', 'markdown']).optional().describe('Optionally export the insight in this format'),
  }))
  .output(z.object({
    insightId: z.string(),
    title: z.string(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.exportFormat) {
      exportedContent = await client.exportInsight(ctx.input.insightId, ctx.input.exportFormat);
    }
