...
export let manageInsight = SlateTool.create(
  spec,
  {
    name: 'Manage Insight',
    key: 'manage_insight',
  ...
  .input(z.object({
    ...
    published: z.boolean().optional().describe('Whether the insight should be published (update only)'),
    contributors: z.array(z.string()).optional().describe('Contributor user IDs (update only)'),
