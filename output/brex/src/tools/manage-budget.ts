...
export let manageBudget = SlateTool.create(
  spec,
  {
    name: 'Manage Budget',
    key: 'manage_budget',
...
To create a budget, omit **budgetId**. To update, provide **budgetId**. To archive, set **archive** to true.`,
  ...
  .input(z.object({
    budgetId: z.string().optional().describe('ID of an existing budget to update or archive. Omit to create.'),
    ...
    periodType: z.enum(['MONTHLY', 'QUARTERLY', 'YEARLY', 'ONE_TIME']).optional().describe('Budget period type'),
    limit: z.object({
      amount: z.number().describe('Limit amount in cents'),
      currency: z.string().optional().describe('Currency code (defaults to USD)'),
    }).optional().describe('Spend limit for the budget'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let buildBudgetData = () => {
      ...
      if (ctx.input.parentBudgetId !== undefined) data.parent_budget_id = ctx.input.parentBudgetId;
      if (ctx.input.ownerUserIds !== undefined) data.owner_user_ids = ctx.input.ownerUserIds;
      if (ctx.input.memberUserIds !== undefined) data.member_user_ids = ctx.input.memberUserIds;
