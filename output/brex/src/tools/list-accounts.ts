...
let accountSchema = z.object({
  ...
  }).nullable().optional().describe('Current balance (cash accounts only)'),
  availableBalance: z.object({
    amount: z.number().describe('Balance in cents'),
    currency: z.string().nullable().describe('Currency code'),
  }).nullable().optional().describe('Available balance (cash accounts only)'),
  accountNumber: z.string().nullable().optional().describe('Account number (cash accounts only)'),
  routingNumber: z.string().nullable().optional().describe('Routing number (cash accounts only)'),
...
export let listAccounts = SlateTool.create(
  ...
  .input(z.object({
    accountType: z.enum(['card', 'cash', 'all']).optional().describe('Filter by account type (defaults to all)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (type === 'card' || type === 'all') {
      try {
        ...
        accounts.push(...cardAccounts.items.map((a: any) => ({
          accountId: a.id,
          accountType: 'card',
          name: a.name ?? null,
          status: a.status,
    ...
    if (type === 'cash' || type === 'all') {
      try {
        ...
        accounts.push(...cashAccounts.items.map((a: any) => ({
          accountId: a.id,
          accountType: 'cash',
          name: a.name ?? null,
          status: a.status,
