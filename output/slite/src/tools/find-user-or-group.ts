...
export let findUserOrGroup = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    query: z.string().optional().describe('Search query (email or name for users, name for groups)'),
    includeArchived: z.boolean().optional().describe('Include archived users in search results (users only)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('Matched users (when lookupType is "user")'),
    groups: z.array(z.object({
      groupId: z.string().describe('Group ID'),
      name: z.string().describe('Group name'),
      description: z.string().optional().describe('Group description')
    })).optional().describe('Matched groups (when lookupType is "group")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (lookupType === 'user') {
      ...
      let result = await client.searchUsers(query, ctx.input.includeArchived, ctx.input.cursor);
