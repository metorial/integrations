...
export let auditKnowledgeBase = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    category: z.enum(['all', 'public', 'inactive', 'empty']).describe('Category of notes to list'),
    reviewStateList: z.array(z.enum(['Verified', 'Outdated', 'VerificationRequested', 'VerificationExpired'])).optional().describe('Filter by review states (not available for inactive/empty)'),
