...
export let manageNoteLifecycle = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['verify', 'flag_outdated', 'archive', 'unarchive', 'update_owner']).describe('Lifecycle action to perform'),
    verifyUntil: z.string().optional().describe('ISO 8601 expiration date for verification (only for "verify" action). Omit for no expiration.'),
    outdatedReason: z.string().optional().describe('Reason for flagging as outdated (required for "flag_outdated" action)'),
    ownerUserId: z.string().optional().describe('User ID to assign as new owner (for "update_owner" action)'),
    ownerGroupId: z.string().optional().describe('Group ID to assign as new owner (for "update_owner" action)')
  }))
  .output(z.object({
    ...
    }).optional().describe('Updated owner')
  }))
    ...
    switch (action) {
      ...
      case 'update_owner':
        if (!ctx.input.ownerUserId && !ctx.input.ownerGroupId) {
          throw new Error('Either ownerUserId or ownerGroupId is required for the update_owner action');
        }
