...
export let getNote = SlateTool.create(
  ...
  .input(z.object({
    ...
    format: z.enum(['md', 'html', 'sliteml']).optional().default('md').describe('Content format to return: md (Markdown), html, or sliteml'),
    includeChildren: z.boolean().optional().default(false).describe('Whether to also fetch and return child notes')
  }))
  .output(z.object({
    ...
    archivedAt: z.string().nullable().describe('Timestamp when the note was archived, or null'),
    reviewState: z.string().optional().describe('Review state: Verified, Outdated, VerificationRequested, or VerificationExpired'),
    owner: z.object({
      userId: z.string().optional(),
      groupId: z.string().optional()
    }).optional().describe('Owner of the note'),
    children: z.array(z.object({
      noteId: z.string(),
      title: z.string(),
      url: z.string()
    })).optional().describe('Child notes if includeChildren was true')
  }))
