...
let messageSchema = z.object({
  ...
  attributes: z.record(z.string(), z.string()).optional().describe('System attributes such as SenderId, SentTimestamp, ApproximateReceiveCount'),
});

let queueAttributeSchema = z.record(z.string(), z.string()).describe('Queue attributes as key-value pairs');
...
let outputSchema = z.object({
  ...
  queueUrl: z.string().optional().describe('Queue URL (for create_queue and other operations)'),
  messages: z.array(messageSchema).optional().describe('Received messages (for receive_messages)'),
  messageId: z.string().optional().describe('ID of the sent message (for send_message)'),
  md5OfMessageBody: z.string().optional().describe('MD5 digest of the sent message body (for send_message)'),
  attributes: queueAttributeSchema.optional().describe('Queue attributes (for get_queue_attributes)'),
  nextToken: z.string().optional().describe('Pagination token for the next page of results (for list_queues)'),
  success: z.boolean().optional().describe('Whether the operation completed successfully'),
});

export let manageSqsTool = SlateTool.create(
  spec,
  {
    name: 'Manage SQS',
    key: 'manage_sqs',
  ...
  .input(z.object({
    ...
    queueUrl: z.string().optional().describe('Full URL of the SQS queue (required for all operations except list_queues and create_queue)'),
    queueName: z.string().optional().describe('Name of the queue to create (required for create_queue). FIFO queue names must end with ".fifo". Max 80 characters.'),
    queueNamePrefix: z.string().optional().describe('Filter queues by name prefix (for list_queues)'),
    maxResults: z.number().optional().describe('Maximum number of queue URLs to return, 1-1000 (for list_queues)'),
    nextToken: z.string().optional().describe('Pagination token from a previous list_queues response'),
    messageBody: z.string().optional().describe('The message content to send, max 256 KB (required for send_message)'),
    delaySeconds: z.number().optional().describe('Delay in seconds before the message becomes visible, 0-900 (for send_message and create_queue attributes)'),
    maxNumberOfMessages: z.number().optional().describe('Maximum number of messages to receive, 1-10 (for receive_messages). Default: 1'),
    waitTimeSeconds: z.number().optional().describe('Long poll wait time in seconds, 0-20 (for receive_messages). 0 = short polling'),
    visibilityTimeout: z.number().optional().describe('Duration in seconds that received messages are hidden, 0-43200 (for receive_messages)'),
    receiptHandle: z.string().optional().describe('Receipt handle of the message to delete (required for delete_message)'),
    attributeNames: z.array(z.string()).optional().describe('Attribute names to retrieve, e.g. ["All"], ["ApproximateNumberOfMessages", "QueueArn"] (for get_queue_attributes)'),
    attributes: z.record(z.string(), z.string()).optional().describe('Queue attributes for create_queue, e.g. { "VisibilityTimeout": "60", "DelaySeconds": "5", "MaximumMessageSize": "131072" }'),
  }))
  .output(outputSchema)
  .handleInvocation(async (ctx) => {
    ...
    if (operation === 'list_queues') {
      ...
      if (ctx.input.maxResults !== undefined) params['MaxResults'] = String(ctx.input.maxResults);
      ...
      let prefixLabel = ctx.input.queueNamePrefix ? ` matching prefix "${ctx.input.queueNamePrefix}"` : '';
    ...
    if (operation === 'delete_message') {
      ...
      if (!ctx.input.receiptHandle) throw new Error('receiptHandle is required for delete_message');
    ...
    if (operation === 'get_queue_attributes') {
      if (!ctx.input.queueUrl) throw new Error('queueUrl is required for get_queue_attributes');
