...
export let manageKeyEvents = SlateTool.create(
  spec,
  {
...
Previously known as "conversion events" in Google Analytics.`,
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'get', 'update', 'delete']).describe('Action to perform on key events.'),
    keyEventId: z.string().optional().describe('ID of the key event (required for get, update, and delete actions).'),
    eventName: z.string().optional().describe('Event name to mark as a key event (required for create). Must match an existing GA4 event name.'),
    countingMethod: z.enum(['ONCE_PER_EVENT', 'ONCE_PER_SESSION']).optional().describe('How to count conversions. ONCE_PER_EVENT counts every occurrence, ONCE_PER_SESSION counts once per session.'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of key events (for list action).'),
    keyEvent: z.object({
      name: z.string().optional(),
      eventName: z.string().optional(),
      countingMethod: z.string().optional(),
      ...
      custom: z.boolean().optional(),
      deletable: z.boolean().optional(),
    }).optional().describe('Key event details (for get/create/update actions).'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'update') {
      ...
      let result = await client.updateKeyEvent(ctx.input.keyEventId, updateFields.join(','), body);
      return {
        output: { keyEvent: result },
