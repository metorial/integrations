...
The Measurement Protocol supplements automatic data collection — it does not replace gtag, Tag Manager, or Firebase.`,
  ...
  .input(z.object({
    clientId: z.string().describe('A unique client identifier. Use the GA4 client_id cookie value, or generate a UUID for server-side events.'),
    userId: z.string().optional().describe('Optional known user ID to associate events with a logged-in user across devices.'),
    events: z.array(z.object({
      name: z.string().describe('Event name (e.g., "purchase", "sign_up", "generate_lead"). Must be 40 chars or fewer.'),
      params: z.record(z.string(), z.any()).optional().describe('Key-value pairs of event parameters (e.g., { "value": 25.00, "currency": "USD" }).'),
    })).min(1).max(25).describe('Array of events to send. 1 to 25 events per request.'),
    userProperties: z.record(z.string(), z.object({
      value: z.any().describe('The property value.'),
    })).optional().describe('User properties to set (e.g., { "membership_tier": { "value": "gold" } }).'),
    consent: z.object({
      adUserData: z.enum(['GRANTED', 'DENIED']).optional().describe('Consent for ad-related user data.'),
      adPersonalization: z.enum(['GRANTED', 'DENIED']).optional().describe('Consent for ad personalization.'),
    }).optional().describe('Consent settings for the events.'),
  }))
  .output(z.object({
