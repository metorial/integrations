...
let funnelFilterExpressionSchema: z.ZodType<any> = z.object({
  funnelFieldFilter: z.object({
  ...
  funnelEventFilter: z.object({
    eventName: z.string().optional().describe('Event name to match (e.g., "page_view", "purchase").'),
  }).optional(),
  andGroup: z.object({
...
export let runFunnelReport = SlateTool.create(
  spec,
  {
...
Use this to analyze conversion funnels like checkout flows, onboarding sequences, or any multi-step user journey.`,
  ...
  .input(z.object({
    ...
    })).optional().describe('Optional date ranges. Defaults to last 30 days if not specified.'),
    steps: z.array(z.object({
      name: z.string().describe('Descriptive name for this funnel step (e.g., "Viewed Product", "Added to Cart").'),
      filterExpression: funnelFilterExpressionSchema.optional().describe('Filter expression defining which users qualify for this step.'),
      isOpenFunnel: z.boolean().optional().describe('If true, users can enter the funnel at this step without completing prior steps.'),
      withinDurationFromPriorStep: z.string().optional().describe('Maximum time from the prior step (e.g., "3600s" for 1 hour).'),
    })).min(2).describe('Funnel steps in order. At least 2 steps required.'),
    isOpenFunnel: z.boolean().optional().describe('If true, users can enter the funnel at any step (open funnel). Default is closed funnel.'),
    breakdownDimension: z.string().optional().describe('Optional dimension to break down funnel results by (e.g., "deviceCategory", "country").'),
  }))
  .output(z.object({
    funnelTable: z.any().optional().describe('Funnel table with step-by-step conversion data.'),
