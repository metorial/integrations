...
export let auth = SlateAuth.create()
  .output(z.object({
    token: z.string().describe('OAuth 2.0 access token for Data API and Admin API requests.'),
    refreshToken: z.string().optional().describe('OAuth 2.0 refresh token for long-lived access.'),
    expiresAt: z.string().optional().describe('ISO 8601 timestamp when the access token expires.'),
    measurementId: z.string().optional().describe('Measurement ID for web streams (e.g., "G-XXXXXXX"). Required for Measurement Protocol.'),
    apiSecret: z.string().optional().describe('API secret for Measurement Protocol. Stream-specific.'),
  }))
  .addOauth({
    type: 'auth.oauth',
    name: 'OAuth 2.0',
    ...
    scopes: [
      ...
      },
      {
        ...
        description: 'Edit access to Google Analytics configuration (also grants read access).',
    ...
    inputSchema: z.object({
      measurementId: z.string().optional().describe('Measurement ID for web streams (e.g., "G-XXXXXXX"). Only needed for sending events via Measurement Protocol.'),
      apiSecret: z.string().optional().describe('API secret for Measurement Protocol. Only needed for sending events.'),
    }),
  ...
  .addCustomAuth({
    ...
    inputSchema: z.object({
      ...
      apiSecret: z.string().describe('API secret for the Measurement Protocol. Generated in GA4 Admin > Data Streams.'),
    }),
