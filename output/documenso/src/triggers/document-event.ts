...
export let documentEventTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Document Event',
    key: 'document_event',
    description: 'Triggers when a document lifecycle event occurs: created, sent, opened, signed, completed, rejected, or cancelled. Requires webhook configuration in Documenso team settings.',
    instructions: [
  ...
  .input(z.object({
    eventType: z.string().describe('The document event type (e.g. document.created, document.signed)'),
    ...
    rejectionReason: z.string().optional().describe('Reason for rejection (for document.rejected events)'),
  }))
  .output(z.object({
    documentId: z.string().describe('ID of the affected document'),
    title: z.string().describe('Title of the document'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let rejectionReason = payload.rejectionReason ? String(payload.rejectionReason) : undefined;
