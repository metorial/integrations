...
export let useTemplateTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    recipientOverrides: z.array(z.object({
      recipientId: z.number().describe('ID of the placeholder recipient in the template'),
    ...
    })).optional().describe('Override recipient details from the template'),
    prefillFields: z.array(z.object({
      fieldId: z.number().describe('ID of the field in the template'),
      value: z.string().describe('Value to prefill the field with'),
    })).optional().describe('Prefill field values'),
  }))
  .output(z.object({
