...
export let listInstances = SlateTool.create(
  spec,
  {
    name: 'List Instances',
    key: 'list_instances',
  ...
  .input(z.object({
    ...
    pageSize: z.number().optional().describe('Maximum number of results per page (max 1000)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await compute.listInstances(ctx.auth, folderId, ctx.input.pageSize, ctx.input.pageToken);
...
export let getInstance = SlateTool.create(
  spec,
  {
    name: 'Get Instance',
    key: 'get_instance',
...
export let createInstance = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    bootDiskTypeId: z.string().optional().describe('Disk type (e.g. network-hdd, network-ssd)'),
    subnetId: z.string().describe('Subnet ID for the primary network interface'),
    assignPublicIp: z.boolean().optional().describe('Whether to assign a public IPv4 address'),
    metadata: z.record(z.string(), z.string()).optional().describe('Instance metadata (e.g. ssh-keys, user-data)'),
    labels: z.record(z.string(), z.string()).optional().describe('Instance labels'),
  }))
  .output(z.object({
...
export let controlInstance = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    instanceId: z.string().describe('ID of the VM instance'),
    action: z.enum(['start', 'stop', 'restart', 'delete']).describe('Action to perform on the instance'),
  }))
  .output(z.object({
...
export let updateInstance = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    labels: z.record(z.string(), z.string()).optional().describe('New labels (replaces existing)'),
