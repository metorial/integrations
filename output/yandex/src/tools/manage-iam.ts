...
export let listServiceAccounts = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await iam.listServiceAccounts(ctx.auth, folderId, ctx.input.pageSize, ctx.input.pageToken);
...
export let manageServiceAccount = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    serviceAccountId: z.string().optional().describe('ID of the service account (required for delete)'),
  ...
  .handleInvocation(async (ctx) => {
    if (ctx.input.action === 'create') {
      ...
      if (!ctx.input.serviceAccountId) throw new Error('serviceAccountId is required for deletion');
...
export let manageApiKeys = SlateTool.create(
  ...
  .input(z.object({
    ...
    serviceAccountId: z.string().optional().describe('Service account ID (required for list and create)'),
  ...
  .output(z.object({
    apiKeys: z.array(z.object({
      ...
      createdAt: z.string().optional().describe('Creation timestamp'),
    })).optional().describe('List of API keys'),
    apiKeyId: z.string().optional().describe('Created or deleted API key ID'),
    secret: z.string().optional().describe('Secret part of the API key (only returned on creation)'),
  }))
  .handleInvocation(async (ctx) => {
    if (ctx.input.action === 'list') {
      if (!ctx.input.serviceAccountId) throw new Error('serviceAccountId is required for listing API keys');
      ...
      if (!ctx.input.serviceAccountId) throw new Error('serviceAccountId is required for creating API keys');
