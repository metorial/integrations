...
export let listDisks = SlateTool.create(
  spec,
  {
    name: 'List Disks',
    key: 'list_disks',
  ...
  .output(z.object({
    disks: z.array(z.object({
      ...
      instanceIds: z.array(z.string()).optional().describe('IDs of instances using this disk'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await compute.listDisks(ctx.auth, folderId, ctx.input.pageSize, ctx.input.pageToken);
...
export let manageDisk = SlateTool.create(
  spec,
  {
    name: 'Manage Disk',
    key: 'manage_disk',
  ...
  .input(z.object({
    ...
    typeId: z.string().optional().describe('Disk type (network-hdd, network-ssd, network-ssd-nonreplicated)'),
