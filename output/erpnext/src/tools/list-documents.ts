...
export let listDocuments = SlateTool.create(
  ...
  .input(z.object({
    ...
    fields: z.array(z.string()).optional().describe('Fields to return (e.g., ["name", "customer_name", "status"]). Defaults to ["name"].'),
    filters: z.any().optional().describe('AND filters as a JSON object or array of [field, operator, value] tuples'),
    orFilters: z.any().optional().describe('OR filters as a JSON object or array of [field, operator, value] tuples'),
    orderBy: z.string().optional().describe('Sort order (e.g., "creation desc", "modified asc")'),
