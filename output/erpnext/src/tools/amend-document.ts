...
export let amendDocument = SlateTool.create(
  spec,
  {
    name: 'Amend Document',
    key: 'amend_document',
  ...
  .input(z.object({
    ...
    documentData: z.record(z.string(), z.any()).optional().describe('Fields to override in the amended copy')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let document = await client.amendDocument(ctx.input.doctype, ctx.input.documentName, ctx.input.documentData);
