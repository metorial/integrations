...
export let captureScreenshot = SlateTool.create(
  ...
  .input(z.object({
    ...
    fullPage: z.boolean().optional().describe('Capture the full page height including scrollable content'),
    ...
    renderJs: z.boolean().optional().describe('Enable JavaScript rendering (recommended for dynamic pages)'),
    ...
    wait: z.number().optional().describe('Time in milliseconds to wait after page load before capturing'),
    waitFor: z.string().optional().describe('CSS selector to wait for before capturing screenshot')
  }))
  .output(z.object({
