...
export let googleSearch = SlateTool.create(
  ...
  .input(z.object({
    ...
    searchType: z.enum(['search', 'news', 'maps', 'images', 'lens', 'shopping', 'ai_mode']).optional().describe('Type of Google search to perform'),
    ...
    location: z.string().optional().describe('Location string for geo-targeted results (e.g., "New York,NY,United States")'),
    ...
    additionalParams: z.record(z.string(), z.string()).optional().describe('Additional Google search parameters (e.g., gl, hl, tbs)')
  }))
  .output(z.object({
    results: z.any().describe('Structured Google search results as JSON, including organic results, ads, knowledge graph, and other features')
  }))
