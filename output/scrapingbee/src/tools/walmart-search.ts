...
export let walmartSearch = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    productId: z.string().optional().describe('Walmart product ID for a specific product lookup'),
    sortBy: z.string().optional().describe('Sort order for search results (e.g., "best_match", "price_low", "price_high", "best_seller", "rating_high")'),
    minPrice: z.number().optional().describe('Minimum price filter'),
    maxPrice: z.number().optional().describe('Maximum price filter'),
    fulfillmentType: z.string().optional().describe('Fulfillment type filter (e.g., "pickup", "shipping")'),
    fulfillmentSpeed: z.string().optional().describe('Fulfillment speed filter (e.g., "today", "tomorrow", "twoDay")'),
    domain: z.string().optional().describe('Walmart domain (e.g., "walmart.com")'),
    page: z.number().optional().describe('Page number for pagination'),
    deliveryZip: z.string().optional().describe('ZIP code for delivery availability (product lookup only)'),
    storeId: z.string().optional().describe('Walmart store ID for in-store availability (product lookup only)')
  }))
  .output(z.object({
