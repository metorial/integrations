...
export let exportJobResult = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    type: z.enum(['result', 'logs']).optional().describe('Type of data to export. Defaults to "result".'),
  ...
  .output(z.object({
    downloadLink: z.string().optional().nullable().describe('URL to download the exported CSV file.'),
