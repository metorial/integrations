...
export let manageList = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'update', 'delete', 'add_rows']).describe('The list action to perform.'),
    listId: z.string().optional().describe('List ID. Required for get, update, delete, and add_rows.'),
    name: z.string().optional().describe('List name. Required for create, optional for update.'),
    description: z.string().optional().describe('List description. Used with create or update.'),
    columns: z.array(z.object({
      name: z.string().describe('Column name.'),
      type: z.string().optional().describe('Column type.'),
    })).optional().describe('Column definitions. Used with create.'),
    rows: z.array(z.record(z.string(), z.string())).optional().describe('Data rows to add. Used with add_rows. Each row is an object with column names as keys.'),
  ...
  .output(z.object({
    ...
    lists: z.array(z.record(z.string(), z.any())).optional().nullable().describe('Array of lists when using "list" action.'),
    listDetails: z.record(z.string(), z.any()).optional().nullable().describe('List details when using "get" or "create" action.'),
