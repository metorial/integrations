...
export let getJobResults = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    collection: z.number().optional().describe('Collection index to retrieve results from. Defaults to 1.'),
    modified: z.number().optional().describe('Set to 1 to only get new/changed rows.'),
  }))
  .output(z.object({
    rows: z.array(z.record(z.string(), z.any())).describe('Array of scraped data rows. Each row is an object with field names as keys.'),
