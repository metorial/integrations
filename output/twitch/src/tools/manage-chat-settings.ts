...
export let manageChatSettings = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    followerModeDurationMinutes: z.number().optional().describe('Minimum follow time in minutes (0 = any follower)'),
    slowMode: z.boolean().optional().describe('Enable slow mode'),
    slowModeWaitTimeSeconds: z.number().optional().describe('Time in seconds between messages (3-120)'),
    subscriberMode: z.boolean().optional().describe('Enable subscriber-only mode'),
    uniqueChatMode: z.boolean().optional().describe('Enable unique-message-only mode (no duplicate messages)'),
  }))
  .output(z.object({
    emoteMode: z.boolean(),
