...
export let manageModeration = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Timeout duration is 1 second to 1,209,600 seconds (2 weeks).',
    ],
    tags: {
      destructive: true,
    },
  ...
  .input(z.object({
    ...
    action: z.enum(['ban', 'unban', 'delete_message', 'shield_mode_on', 'shield_mode_off']).describe('Moderation action to perform'),
    targetUserId: z.string().optional().describe('Target user ID (required for ban/unban)'),
    durationSeconds: z.number().optional().describe('Timeout duration in seconds (for ban action; omit for permanent ban)'),
    reason: z.string().optional().describe('Reason for the ban or timeout'),
    messageId: z.string().optional().describe('Message ID to delete (for delete_message action)'),
  }))
  .output(z.object({
    success: z.boolean(),
    action: z.string(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'ban': {
        if (!ctx.input.targetUserId) throw new Error('targetUserId is required for ban action');
      ...
      case 'unban': {
        if (!ctx.input.targetUserId) throw new Error('targetUserId is required for unban action');
      ...
      case 'delete_message': {
        if (!ctx.input.messageId) throw new Error('messageId is required for delete_message action');
        await client.deleteChatMessage(ctx.input.broadcasterId, moderatorId, ctx.input.messageId);
        return {
