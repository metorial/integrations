...
export let startCommercial = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    lengthSeconds: z.number().describe('Commercial length in seconds (30, 60, 90, 120, 150, or 180)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.startCommercial(ctx.input.broadcasterId, ctx.input.lengthSeconds);
