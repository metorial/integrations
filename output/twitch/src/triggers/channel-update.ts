...
export let channelUpdate = SlateTrigger.create(
  spec,
  {
    name: 'Channel Update',
    key: 'channel_update',
    description: 'Triggers when a channel\'s title, game/category, language, or other properties change. Polls for changes to channel info.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      if (state?.broadcasterId) {
        let changes: string[] = [];
        if (state.lastTitle !== undefined && state.lastTitle !== channel.title) changes.push('title');
        if (state.lastGameId !== undefined && state.lastGameId !== channel.game_id) changes.push('game');
        if (state.lastLanguage !== undefined && state.lastLanguage !== channel.broadcaster_language) changes.push('language');
