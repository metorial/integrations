...
export let newFollower = SlateTrigger.create(
  spec,
  {
    name: 'New Follower',
    key: 'new_follower',
    description: 'Triggers when the authenticated broadcaster gains a new follower. Polls the followers endpoint for new entries.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let newestFollowedAt = result.followers.length > 0 && result.followers[0]
