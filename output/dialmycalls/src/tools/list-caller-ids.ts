...
export let listCallerIds = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    callerIdId: z.string().optional().describe('Fetch a specific caller ID by ID. If omitted, lists all caller IDs.'),
    range: z.string().optional().describe('Pagination range, e.g. "records=201-300"'),
  }))
  .output(z.object({
    callerIds: z.array(z.object({
