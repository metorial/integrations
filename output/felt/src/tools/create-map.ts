...
export let createMap = SlateTool.create(
  spec,
  {
    name: 'Create Map',
    key: 'create_map',
  ...
  .input(z.object({
    ...
    basemap: z.string().optional().describe('Basemap style: "default", "light", "dark", "satellite", a raster tile URL, or a hex color'),
    publicAccess: z.enum(['private', 'view_only', 'view_and_comment', 'view_comment_and_edit']).optional().describe('Access level for the map'),
    layerUrls: z.array(z.string()).optional().describe('Tile URLs for raster layers to add on creation'),
  }))
  .output(z.object({
