...
export let getMap = SlateTool.create(
  ...
  .output(z.object({
    ...
    layerGroups: z.array(z.record(z.string(), z.unknown())).describe('Layer groups on the map'),
    elements: z.record(z.string(), z.unknown()).nullable().describe('Elements (GeoJSON FeatureCollection)'),
    elementGroups: z.array(z.record(z.string(), z.unknown())).describe('Element groups on the map'),
  }))
