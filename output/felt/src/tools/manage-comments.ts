...
export let manageComments = SlateTool.create(
  ...
  .input(z.object({
    ...
    commentId: z.string().optional().describe('Comment ID (required for resolve and delete actions)'),
    exportFormat: z.enum(['json', 'csv', 'geojson']).optional().describe('Format for export (defaults to "json")'),
  }))
  .output(z.object({
