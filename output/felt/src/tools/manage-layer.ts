...
export let manageLayer = SlateTool.create(
  ...
  .input(z.object({
    ...
    refreshPeriod: z.string().optional().describe('Refresh schedule (e.g. "15 min", "hour", "day", "disabled")'),
    legendVisibility: z.string().optional().describe('Legend visibility setting'),
    metadata: z.record(z.string(), z.unknown()).optional().describe('Layer metadata (attribution, source, description, etc.)'),
    style: z.record(z.string(), z.unknown()).optional().describe('Felt Style Language (FSL) style object'),
    refresh: z.boolean().optional().describe('Set to true to trigger a data refresh for live layers'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    // Update properties
    let hasPropertyUpdate = ctx.input.name !== undefined || ctx.input.caption !== undefined ||
      ctx.input.layerGroupId !== undefined || ctx.input.orderingKey !== undefined ||
      ctx.input.refreshPeriod !== undefined || ctx.input.legendVisibility !== undefined ||
