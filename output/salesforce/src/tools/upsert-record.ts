...
export let upsertRecord = SlateTool.create(
  ...
  .input(z.object({
    objectType: z.string().describe('The Salesforce object type (e.g., Account, Contact, or a custom object)'),
    externalIdField: z.string().describe('The API name of the external ID field (e.g., External_Id__c)'),
    externalIdValue: z.string().describe('The external ID value to match on'),
    fieldValues: z.record(z.string(), z.any()).describe('Field name-value pairs to set on the record')
  }))
  .output(z.object({
    ...
    created: z.boolean().optional().describe('True if a new record was created, false if an existing record was updated')
  }))
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.upsertRecord(ctx.input.objectType, ctx.input.externalIdField, ctx.input.externalIdValue, ctx.input.fieldValues);
