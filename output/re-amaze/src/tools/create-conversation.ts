...
export let createConversation = SlateTool.create(
  ...
  .input(z.object({
    ...
    channelSlug: z.string().describe('Channel slug where the conversation should be created'),
    ...
    customerData: z.record(z.string(), z.string()).optional().describe('Custom attributes for the customer (one level deep key-value pairs)'),
    tagList: z.array(z.string()).optional().describe('Tags to apply to the conversation'),
    status: z.number().optional().describe('Status code: 0=Open, 1=Responded, 2=Done, 5=On Hold'),
    assigneeEmail: z.string().optional().describe('Email of the staff member to assign'),
    customFields: z.record(z.string(), z.string()).optional().describe('Custom field key-value pairs for the conversation'),
