...
export let listArticles = SlateTool.create(
  ...
  .input(z.object({
    status: z.enum(['published', 'draft', 'internal']).optional().describe('Filter articles by publication status'),
    query: z.string().optional().describe('Keyword search query'),
    topicSlug: z.string().optional().describe('Topic slug to scope articles to a specific topic'),
...
export let updateArticle = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['published', 'draft', 'internal']).optional().describe('Updated publication status'),
