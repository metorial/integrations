...
let conversationSchema = z.object({
  ...
  }).optional().describe('Customer who started the conversation'),
  assignee: z.string().nullable().optional().describe('Staff member assigned to the conversation'),
...
export let listConversations = SlateTool.create(
  ...
  .input(z.object({
    filter: z.enum(['open', 'archived', 'unassigned', 'all']).optional().describe('Filter conversations by status group'),
    customerEmail: z.string().optional().describe('Filter conversations for a specific customer email'),
    sort: z.enum(['updated', 'changed']).optional().describe('"updated" for latest customer activity, "changed" for any update'),
    ...
    origin: z.string().optional().describe('Channel origin filter (e.g., email, chat, twitter, facebook, sms)'),
    startDate: z.string().optional().describe('Filter conversations created after this ISO 8601 date'),
    endDate: z.string().optional().describe('Filter conversations created before this ISO 8601 date'),
