...
Reports can be filtered by date range.`,
  ...
  .input(z.object({
    reportType: z.enum(['volume', 'response_time', 'staff', 'tags', 'channel_summary']).describe('Type of report to retrieve'),
    startDate: z.string().optional().describe('Start date for the report range (ISO 8601 format)'),
    endDate: z.string().optional().describe('End date for the report range (ISO 8601 format)'),
  }))
  .output(z.object({
