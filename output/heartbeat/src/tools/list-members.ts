...
export let listMembers = SlateTool.create(
  ...
  .input(z.object({
    limit: z.number().min(1).max(100).optional().describe('Number of users per page (1-100)'),
    startingAfter: z.string().optional().describe('Cursor for pagination — pass the last user ID from the previous page'),
    groupId: z.string().optional().describe('Filter users by group ID'),
    role: z.string().optional().describe('Filter users by role'),
    createdAfter: z.string().optional().describe('ISO date string — only return users created after this date'),
    createdBefore: z.string().optional().describe('ISO date string — only return users created before this date'),
  }))
  .output(z.object({
