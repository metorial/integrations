...
export let createChannel = SlateTool.create(
  ...
  .input(z.object({
    ...
    channelType: z.enum(['thread', 'chat', 'voice']).optional().describe('Type of channel to create'),
    emoji: z.string().optional().describe('Emoji to represent the channel'),
    isReadOnly: z.boolean().optional().describe('Whether the channel is read-only for members'),
  }))
  .output(z.object({
