...
export let manageStorageFiles = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list', 'upload', 'download', 'delete']).describe('The file operation to perform'),
    storageZoneName: z.string().describe('Name of the storage zone'),
    path: z.string().optional().describe('File or directory path relative to the zone root. For list, this is the directory path. For upload/download/delete, this is the full file path.'),
    content: z.string().optional().describe('File content to upload (upload action only)'),
  }))
  .output(z.object({
    files: z.array(fileSchema).optional().describe('List of files/directories (list action)'),
  ...
  .handleInvocation(async (ctx) => {
    if (!ctx.auth.storageToken) {
      throw new Error('Storage Zone Password is required for file operations. Please configure it in the authentication settings.');
    }
    ...
    switch (ctx.input.action) {
      ...
      case 'upload': {
        await client.uploadFile(ctx.input.storageZoneName, ctx.input.path!, ctx.input.content!);
        return {
      ...
      case 'download': {
        ...
        return {
          output: { fileContent: typeof content === 'string' ? content : JSON.stringify(content) },
