...
export let purgeCache = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['purge_url', 'purge_pull_zone']).describe('Whether to purge a single URL or an entire pull zone'),
    url: z.string().optional().describe('The full CDN URL to purge (required for purge_url)'),
    pullZoneId: z.number().optional().describe('The pull zone ID to purge (required for purge_pull_zone)'),
    cacheTag: z.string().optional().describe('Only purge items with this cache tag (purge_pull_zone only). If omitted, the entire zone cache is purged.'),
  }))
  .output(z.object({
