...
let dnsRecordSchema = z.object({
  ...
  type: z.number().optional().describe('Record type code (0=A, 1=AAAA, 2=CNAME, 3=TXT, 4=MX, 5=Redirect, 6=Flatten, 7=PullZone, 8=SRV, 9=CAA, 10=PTR, 12=NS)'),
...
let dnsZoneSchema = z.object({
  ...
  customNameserversEnabled: z.boolean().optional().describe('Whether custom nameservers are enabled'),
...
export let manageDnsZone = SlateTool.create(
  ...
  .input(z.object({
    ...
    dnsZoneId: z.number().optional().describe('DNS zone ID. Required for get, update, and delete actions.'),
    ...
    customNameserversEnabled: z.boolean().optional().describe('Enable custom nameservers (update)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'update': {
        ...
        if (ctx.input.customNameserversEnabled !== undefined) data.CustomNameserversEnabled = ctx.input.customNameserversEnabled;
        ...
        if (ctx.input.loggingEnabled !== undefined) data.LoggingEnabled = ctx.input.loggingEnabled;
