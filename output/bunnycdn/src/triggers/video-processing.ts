...
export let videoProcessing = SlateTrigger.create(
  spec,
  {
    name: 'Video Processing Status',
    key: 'video_processing',
    description: 'Triggers when a video processing status changes in a Bunny Stream library. Covers all lifecycle events including queued, processing, encoding, finished, failed, resolution finished, pre-signed upload events, caption generation, and title/description generation. Configure the webhook URL in your video library settings using the Manage Video Library tool or the bunny.net dashboard.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    videoLibraryId: z.number().describe('Video library ID'),
    statusCode: z.number().describe('Numeric status code'),
    statusName: z.string().describe('Human-readable status name (queued, processing, encoding, finished, resolution_finished, failed, presigned_upload_started, presigned_upload_finished, presigned_upload_failed, captions_generated, title_or_description_generated)'),
  }))
  .webhook({
    handleRequest: async (ctx) => {
      let data = await ctx.request.json() as { VideoLibraryId: number; VideoGuid: string; Status: number };
