...
export let extractNewsLinksTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    url: z.string().describe('News website URL to extract links from (e.g. "https://www.nytimes.com")'),
    prefix: z.string().optional().describe('Only return links starting with this prefix'),
    includeSubDomains: z.boolean().optional().describe('Include links from sub-domains of the given URL'),
  }))
  .output(z.object({
