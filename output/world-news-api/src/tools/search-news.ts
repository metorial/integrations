...
let newsArticleSchema = z.object({
  ...
  category: z.string().nullable().describe('Article category (e.g. politics, sports, technology)'),
  ...
  sentiment: z.number().nullable().describe('Sentiment score from -1 (negative) to +1 (positive)'),
});
...
export let searchNewsTool = SlateTool.create(
  ...
  .input(z.object({
    text: z.string().min(3).max(100).optional().describe('Keywords to match in news content (3–100 characters)'),
    textMatchIndexes: z.string().optional().describe('Where to search: "title", "content", or "title,content"'),
    sourceCountry: z.string().optional().describe('ISO 3166 country code of news source (e.g. "us", "gb", "de")'),
    language: z.string().optional().describe('ISO 639-1 language code (e.g. "en", "de", "fr")'),
    minSentiment: z.number().min(-1).max(1).optional().describe('Minimum sentiment score (-1 to 1)'),
    maxSentiment: z.number().min(-1).max(1).optional().describe('Maximum sentiment score (-1 to 1)'),
    earliestPublishDate: z.string().optional().describe('Earliest publish date (e.g. "2024-01-15" or "2024-01-15 10:00:00")'),
    latestPublishDate: z.string().optional().describe('Latest publish date (e.g. "2024-06-01" or "2024-06-01 23:59:59")'),
    newsSources: z.string().optional().describe('Comma-separated news source URLs (e.g. "https://www.bbc.co.uk")'),
    authors: z.string().optional().describe('Comma-separated author names'),
    categories: z.string().optional().describe('Comma-separated categories (e.g. "politics,sports,technology")'),
    entities: z.string().optional().describe('Semantic entity filter (e.g. "ORG:Tesla,PER:Elon Musk")'),
    locationFilter: z.string().optional().describe('Geo filter: "latitude,longitude,radiusKm" (e.g. "51.509865,-0.118092,15")'),
    ...
    maxResults: z.number().min(1).max(100).optional().describe('Number of results to return (1–100, default 10)'),
  }))
  .output(z.object({
