...
export let createNote = SlateTool.create(
  ...
  .input(z.object({
    ...
    noteCategoryId: z.number().optional().describe('Note category ID (e.g., Phone Call, SMS)'),
    notifyUserIds: z.array(z.number()).optional().describe('User IDs to notify about this note')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.noteCategoryId !== undefined) noteData.note_category_id = ctx.input.noteCategoryId;
    if (ctx.input.notifyUserIds !== undefined) noteData.notify_user_ids = ctx.input.notifyUserIds;
