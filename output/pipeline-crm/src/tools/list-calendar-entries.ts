...
export let listCalendarEntries = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    sort: z.string().optional().describe('Sort field (e.g., "start_time", "due_date", "created_at")')
  }))
  .output(z.object({
    entries: z.array(z.object({
      calendarEntryId: z.number().describe('Unique entry ID'),
      name: z.string().nullable().optional().describe('Entry title'),
      type: z.string().nullable().optional().describe('Entry type (CalendarEvent or CalendarTask)'),
      ...
      complete: z.boolean().nullable().optional().describe('Whether the task is complete'),
