...
export let inviteTeamMember = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    inviteCollaborators: z.boolean().optional().describe('Permission to invite collaborators'),
    ...
    userGroups: z.array(z.string()).optional().describe('User groups to assign the member to'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.editFormResults !== undefined) permissions.edit_form_results = ctx.input.editFormResults;
    if (ctx.input.inviteCollaborators !== undefined) permissions.invite_collaborators = ctx.input.inviteCollaborators;
