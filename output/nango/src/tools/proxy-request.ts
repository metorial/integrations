...
export let proxyRequest = SlateTool.create(
  spec,
  {
    name: 'Proxy Request',
    key: 'proxy_request',
  ...
  .input(z.object({
    ...
    queryParams: z.record(z.string(), z.string()).optional().describe('Query parameters to forward'),
    retries: z.number().optional().describe('Number of retry attempts with exponential backoff (default: 0)'),
    baseUrlOverride: z.string().optional().describe('Override the provider base URL'),
    headers: z.record(z.string(), z.string()).optional().describe('Additional headers to forward to the external API'),
  }))
  .output(z.object({
