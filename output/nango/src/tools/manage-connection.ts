...
export let manageConnection = SlateTool.create(
  ...
  .input(z.object({
    ...
    forceRefresh: z.boolean().optional().describe('Force token refresh regardless of expiry (for get action)'),
    includeRefreshToken: z.boolean().optional().describe('Include refresh token in response (for get action)'),
    credentials: z.record(z.string(), z.any()).optional().describe('Auth credentials to import (for create action)'),
    metadata: z.record(z.string(), z.any()).optional().describe('Custom metadata (for create action)'),
    connectionConfig: z.record(z.string(), z.any()).optional().describe('OAuth connection config (for create action)'),
    tags: z.record(z.string(), z.string()).optional().describe('Key-value tags (for create action)'),
  }))
  .output(z.object({
    success: z.boolean(),
    connection: z.object({
    ...
    }).optional().describe('Connection details (not returned for delete)'),
  }))
