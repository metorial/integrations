...
export let photoshopJobEvents = SlateTrigger.create(
  spec,
  {
    name: 'Photoshop Job Completion Events',
    key: 'photoshop_job_events',
    description: 'Triggers when an asynchronous Photoshop API job completes. Receives completion notifications via Adobe I/O Events webhooks, eliminating the need to poll for job status.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    })).optional().describe('Output file details'),
    ...
    handleRequest: async (ctx) => {
      ...
      let inputs = events.map((event: any) => {
        ...
        return {
          ...
          jobId: eventData.jobId || eventData.job_id ||
            eventData.activitystreams_activity?.object?.id,
          jobStatus: eventData.status || eventData.jobStatus ||
            eventData.activitystreams_activity?.object?.status,
          outputs: eventData.outputs || eventData.output ||
