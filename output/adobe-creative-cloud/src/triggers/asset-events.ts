...
export let assetEvents = SlateTrigger.create(
  spec,
  {
    ...
    key: 'asset_events',
    description: 'Triggers when files or directories are created, updated, or deleted in Creative Cloud Assets storage. Receives webhook notifications from Adobe I/O Events.',
  }
)
  .input(z.object({
    eventType: z.string().describe('Event type identifier'),
    eventId: z.string().describe('Unique event ID'),
    resourceType: z.enum(['file', 'directory']).describe('Whether the event is for a file or directory'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let inputs = events.map((event: any) => {
        ...
        let objectType = eventData.activitystreams_activity?.object?.type ||
        ...
        if (objectType.toLowerCase().includes('directory') || objectType.toLowerCase().includes('folder') ||
        ...
        return {
          ...
          resourceType,
          action,
          assetPath: eventData.activitystreams_activity?.object?.id ||
            eventData.activitystreams_object?.id || eventData.xdmAsset?.path,
          assetUrl: eventData.activitystreams_activity?.object?.url ||
            eventData.activitystreams_object?.url || eventData.xdmAsset?.asset_id,
