...
export let libraryEvents = SlateTrigger.create(
  spec,
  {
    name: 'Creative Cloud Library Events',
    key: 'library_events',
    description: 'Triggers when Creative Cloud Libraries are created, updated, or deleted. Library updates include adding, modifying, or removing elements within a library. Receives webhook notifications from Adobe I/O Events.',
  }
)
  .input(z.object({
    ...
    handleRequest: async (ctx) => {
      ...
      let inputs = events.map((event: any) => {
        ...
        return {
          ...
          action,
          libraryId: eventData.activitystreams_activity?.object?.id ||
            eventData.activitystreams_object?.id || eventData.xdmEntity?.library_id,
          libraryName: eventData.activitystreams_activity?.object?.name ||
