...
export let generativeExpand = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    prompt: z.string().optional().describe('Text prompt to guide the content generated in expanded areas'),
    numVariations: z.number().optional().describe('Number of variations to generate (default: 1)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let placement = (ctx.input.insetLeft !== undefined || ctx.input.insetTop !== undefined ||
      ctx.input.insetRight !== undefined || ctx.input.insetBottom !== undefined)
      ? {
        insets: {
