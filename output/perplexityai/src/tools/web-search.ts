...
Supports domain filtering, date filtering, recency filtering, language filtering, and location-based search.`,
  ...
  .input(z.object({
    query: z.union([z.string(), z.array(z.string())]).describe('Search query or array of queries for batch search'),
    country: z.string().max(2).optional().describe('ISO 3166-1 alpha-2 country code for location-based results'),
    maxResults: z.number().min(1).max(20).optional().describe('Maximum number of results to return (1-20, default 10)'),
    maxTokens: z.number().optional().describe('Maximum tokens for context across all results'),
    maxTokensPerPage: z.number().optional().describe('Maximum tokens per individual page snippet'),
    searchDomainFilter: z.array(z.string()).optional().describe('Limit to specific domains (prefix with - to exclude, max 20)'),
    searchLanguageFilter: z.array(z.string()).optional().describe('Filter by language (ISO 639-1 codes, max 20)'),
    searchRecencyFilter: z.enum(['hour', 'day', 'week', 'month', 'year']).optional().describe('Filter by publication recency'),
    searchAfterDate: z.string().optional().describe('Results published after this date (MM/DD/YYYY)'),
    searchBeforeDate: z.string().optional().describe('Results published before this date (MM/DD/YYYY)'),
    lastUpdatedAfter: z.string().optional().describe('Results updated after this date (MM/DD/YYYY)'),
    lastUpdatedBefore: z.string().optional().describe('Results updated before this date (MM/DD/YYYY)'),
  }))
  .output(z.object({
