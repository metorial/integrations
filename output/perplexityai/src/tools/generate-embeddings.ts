...
Standard models are best for independent texts and search queries. Contextualized models are optimized for document chunks that benefit from surrounding context. All models produce INT8-quantized embeddings natively.`,
  ...
  .input(z.object({
    ...
    ]).default('pplx-embed-v1-4b').describe('Embedding model to use'),
  ...
  .output(z.object({
    embeddings: z.array(z.object({
      ...
      embedding: z.union([z.string(), z.array(z.number())]).describe('Embedding vector (base64-encoded INT8 or float array)'),
