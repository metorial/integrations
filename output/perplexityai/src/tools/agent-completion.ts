...
export let agentCompletion = SlateTool.create(
  spec,
  {
...
You can use presets as-is or override any parameter. Models use the \`provider/model-name\` format (e.g., \`openai/gpt-5.4\`, \`anthropic/claude-sonnet-4-6\`).`,
  ...
  .input(z.object({
    ...
    model: z.string().optional().describe('Model to use in provider/model-name format (e.g., "openai/gpt-5.4", "anthropic/claude-sonnet-4-6", "perplexity/sonar")'),
    models: z.array(z.string()).optional().describe('Fallback chain of models (up to 5). Tries each in order until one succeeds.'),
    preset: z.enum(['fast-search', 'pro-search', 'deep-research', 'advanced-deep-research']).optional().describe('Pre-configured preset optimized for a specific use case'),
    instructions: z.string().optional().describe('System instructions to guide the agent behavior'),
    enableWebSearch: z.boolean().optional().describe('Enable the web search tool for real-time information'),
    enableUrlFetch: z.boolean().optional().describe('Enable the URL fetch tool for reading specific pages'),
    ...
    reasoningEffort: z.enum(['minimal', 'low', 'medium', 'high']).optional().describe('Reasoning depth for supported models'),
  ...
  .output(z.object({
    ...
    output: z.array(z.record(z.string(), z.unknown())).describe('Full structured output items from the agent'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let reasoning = (ctx.input.reasoningEffort || ctx.input.reasoningBudgetTokens)
      ? {
