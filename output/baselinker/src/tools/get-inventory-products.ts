...
export let getInventoryProducts = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    mode: z.enum(['list', 'detail']).describe('"list" for browsing/filtering, "detail" for full product data by IDs'),
    ...
    page: z.number().optional().describe('Page number for pagination (list mode, 1000 products per page)'),
