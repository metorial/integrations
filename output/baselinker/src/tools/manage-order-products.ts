...
export let manageOrderProducts = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    storage: z.string().optional().describe('Product source type: "db", "shop", or "warehouse"'),
    ...
    warehouseId: z.number().optional().describe('Source warehouse ID (BaseLinker inventory only)'),
    attributes: z.string().optional().describe('Product attributes, e.g. "Colour: blue"'),
    priceBrutto: z.number().optional().describe('Single item gross price (required for "add")'),
    ...
    orderProductId: z.number().optional().describe('Order product ID (required for "update" and "remove")'),
  }))
  .output(z.object({
    orderProductId: z.number().optional().describe('ID of the added or affected order product'),
