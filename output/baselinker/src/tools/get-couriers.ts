...
export let getCouriers = SlateTool.create(
  spec,
  {
    name: 'Get Couriers',
    key: 'get_couriers',
  ...
  .input(z.object({
    courierCodeForFields: z.string().optional().describe('Courier code to fetch required fields for package creation'),
    orderIdForPackages: z.number().optional().describe('Order ID to fetch existing shipment packages for'),
    packageIdsForStatus: z.array(z.number()).optional().describe('Package IDs to fetch status history for'),
  }))
  .output(z.object({
    ...
    packageStatusHistory: z.any().optional().describe('Status history for the specified packages'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.packageIdsForStatus && ctx.input.packageIdsForStatus.length > 0) {
      let statusResult = await client.getCourierPackagesStatusHistory(ctx.input.packageIdsForStatus);
