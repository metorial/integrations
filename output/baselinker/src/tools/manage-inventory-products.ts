...
export let manageInventoryProducts = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    productId: z.string().optional().describe('Product ID for update/delete. Omit when creating.'),
    ...
    stock: z.record(z.string(), z.number()).optional().describe('Stock quantities by warehouse ID (format "bl_<id>")'),
    locations: z.record(z.string(), z.string()).optional().describe('Warehouse locations by warehouse ID'),
    textFields: z.record(z.string(), z.string()).optional().describe('Text fields: "name", "description", "name|en", etc.'),
    images: z.record(z.string(), z.string()).optional().describe('Image URLs by position (1-16)'),
    bundleProducts: z.record(z.string(), z.number()).optional().describe('Bundle product IDs and quantities'),
  }))
  .output(z.object({
