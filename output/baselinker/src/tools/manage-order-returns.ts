...
export let manageOrderReturns = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    // For list
    orderId: z.number().optional().describe('Order ID to filter returns (list) or to create return for (create)'),
    dateFrom: z.number().optional().describe('Filter returns created after this unix timestamp (list mode)'),
    idFrom: z.number().optional().describe('Fetch returns with ID greater than this (list mode, for pagination)'),

    // For create
    reasonId: z.number().optional().describe('Return reason ID (create/update)'),
    adminComments: z.string().optional().describe('Admin comments for the return (create/update)'),
    products: z.array(z.object({
      orderProductId: z.number().describe('Order product ID to return'),
      quantity: z.number().describe('Quantity to return'),
    })).optional().describe('Products to return with quantities (create)'),
