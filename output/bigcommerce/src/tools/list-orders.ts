...
export let listOrders = SlateTool.create(
  ...
  .input(z.object({
    ...
    limit: z.number().optional().describe('Number of orders per page (max: 250, default: 50)'),
    statusId: z.number().optional().describe('Filter by order status ID'),
    customerId: z.number().optional().describe('Filter by customer ID'),
    minDateCreated: z.string().optional().describe('Filter orders created after this date (RFC 2822 format)'),
    maxDateCreated: z.string().optional().describe('Filter orders created before this date (RFC 2822 format)'),
    minDateModified: z.string().optional().describe('Filter orders modified after this date'),
    maxDateModified: z.string().optional().describe('Filter orders modified before this date'),
    ...
    sortBy: z.enum(['id', 'customer_id', 'date_created', 'date_modified', 'status_id', 'channel_id']).optional().describe('Sort field'),
