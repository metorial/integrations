...
export let listProducts = SlateTool.create(
  ...
  .input(z.object({
    ...
    limit: z.number().optional().describe('Number of products per page (max: 250, default: 50)'),
    ...
    availability: z.enum(['available', 'disabled', 'preorder']).optional().describe('Filter by availability status'),
    sortBy: z.enum(['id', 'name', 'sku', 'price', 'date_modified', 'date_last_imported', 'inventory_level', 'is_visible', 'total_sold']).optional().describe('Sort results by field'),
  ...
  .output(z.object({
    ...
    totalProducts: z.number().optional().describe('Total number of products matching the filter'),
