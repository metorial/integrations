...
export let manageCoupon = SlateTool.create(
  spec,
  {
    name: 'Manage Coupon',
    key: 'manage_coupon',
  ...
  .input(z.object({
    ...
    type: z.enum([
      ...
      'free_shipping', 'percentage_discount',
    ]).optional().describe('Discount type (required for create)'),
    ...
    appliesTo: z.object({
      entityType: z.enum(['categories', 'products']).describe('Entity type the coupon applies to'),
      entityIds: z.array(z.number()).describe('Array of entity IDs'),
    }).optional().describe('What the coupon applies to'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.maxUsesPerCustomer !== undefined) data.max_uses_per_customer = ctx.input.maxUsesPerCustomer;
