...
export let inventoryEvents = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers when inventory levels change at specific locations, or when SKU/variant inventory is updated directly or via orders.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    resourceId: z.number().describe('The ID of the affected resource (product, variant, or location)'),
