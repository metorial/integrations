...
export let manageSecret = SlateTool.create(
  spec,
  {
    name: 'Manage Secret',
    key: 'manage_secret',
  ...
  .input(z.object({
    ...
    secretId: z.string().optional().describe('Secret group ID (required for get, update, delete)'),
    ...
    secretType: z.enum(['environment-arguments', 'environment', 'arguments']).optional().describe('Secret type (required for create)'),
    priority: z.number().optional().describe('Priority 0-100 for override ordering'),
    variables: z.record(z.string(), z.string()).optional().describe('Key-value pairs of secret variables'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of secret groups'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      ...
      if (!ctx.input.secretType) throw new Error('secretType is required for creating a secret');
