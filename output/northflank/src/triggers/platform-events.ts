...
export let platformEvents = SlateTrigger.create(
  spec,
  {
    name: 'Platform Events',
    key: 'platform_events',
    description: 'Receives webhook notifications for Northflank platform events including builds, job runs, backups, billing, autoscaling, and infrastructure alerts.',
  }
)
  .input(z.object({
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      let eventType = ctx.request.headers.get('x-northflank-notification-integration-event-type') || body?.type || 'unknown';
      let eventId = ctx.request.headers.get('x-northflank-notification-integration-event-id') || body?.id || `evt-${Date.now()}`;
      let notificationId = ctx.request.headers.get('x-northflank-notification-integration-id') || undefined;
      let notificationName = ctx.request.headers.get('x-northflank-notification-integration-name') || undefined;
    ...
    handleEvent: async (ctx) => {
      ...
      let resourceId = payload?.resourceId || payload?.serviceId || payload?.jobId || payload?.addonId || payload?.id;
      let resourceName = payload?.resourceName || payload?.serviceName || payload?.jobName || payload?.addonName || payload?.name;
