...
export let getConversation = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeMessages: z.boolean().optional().default(false).describe('Whether to also fetch messages for this conversation'),
  }))
  .output(z.object({
    conversation: z.record(z.string(), z.any()).describe('Conversation details'),
    messages: z.array(z.record(z.string(), z.any())).optional().describe('Messages in the conversation (if requested)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includeMessages && ctx.input.mailboxId) {
      ...
      messages = Array.isArray(messagesResult) ? messagesResult : (messagesResult.messages ?? messagesResult.data ?? []);
    }
