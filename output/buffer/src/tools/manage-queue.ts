...
export let manageQueueTool = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['reorder', 'shuffle', 'move_to_top']).describe('Queue management action to perform'),
    profileId: z.string().optional().describe('Profile ID. Required for reorder and shuffle actions.'),
    updateId: z.string().optional().describe('Update ID. Required for move_to_top action.'),
    order: z.array(z.string()).optional().describe('Ordered array of update IDs for the reorder action'),
  ...
  .output(z.object({
    ...
    updatedUpdateIds: z.array(z.string()).describe('IDs of updates in the resulting queue order')
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (!ctx.input.profileId) throw new Error('profileId is required for reorder and shuffle actions');
