...
export let getSandboxMessage = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['get', 'get_html', 'get_text', 'spam_report', 'html_analysis', 'mark_read', 'delete', 'forward']).describe('What to do with the message'),
    forwardEmail: z.string().optional().describe('Email address to forward to (required for forward action)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'get_html') {
      ...
      return {
        output: { messageId, htmlBody: typeof html === 'string' ? html : JSON.stringify(html) },
    ...
    if (action === 'get_text') {
      ...
      return {
        output: { messageId, textBody: typeof text === 'string' ? text : JSON.stringify(text) },
