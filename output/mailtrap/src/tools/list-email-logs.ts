...
export let listEmailLogs = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    cursor: z.string().optional().describe('Pagination cursor from previous response (nextPageCursor)'),
    sentAfter: z.string().optional().describe('Filter messages sent after this ISO 8601 date'),
    sentBefore: z.string().optional().describe('Filter messages sent before this ISO 8601 date'),
    ...
    sendingStream: z.enum(['transactional', 'bulk']).optional().describe('Filter by sending stream'),
