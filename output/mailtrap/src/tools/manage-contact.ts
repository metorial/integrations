...
export let manageContact = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'get', 'update', 'delete']).describe('Action to perform on the contact'),
    contactIdentifier: z.string().optional().describe('Contact ID or email address. Required for get, update, and delete.'),
    email: z.string().optional().describe('Contact email address. Required for create and optional for update.'),
    fields: z.record(z.string(), z.any()).optional().describe('Custom contact fields (e.g., { "first_name": "John", "city": "NYC" })'),
    listIds: z.array(z.number()).optional().describe('List IDs to assign the contact to (for create)'),
    listIdsIncluded: z.array(z.number()).optional().describe('List IDs to add the contact to (for update)'),
    listIdsExcluded: z.array(z.number()).optional().describe('List IDs to remove the contact from (for update)'),
  ...
  .output(z.object({
    ...
    fields: z.record(z.string(), z.any()).optional().describe('Custom fields of the contact'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { action, contactIdentifier, email, fields, listIds, listIdsIncluded, listIdsExcluded, unsubscribed } = ctx.input;
    ...
    if (action === 'get') {
      if (!contactIdentifier) throw new Error('contactIdentifier is required for getting a contact');
    ...
    if (action === 'update') {
      if (!contactIdentifier) throw new Error('contactIdentifier is required for updating a contact');
    ...
    if (action === 'delete') {
      if (!contactIdentifier) throw new Error('contactIdentifier is required for deleting a contact');
