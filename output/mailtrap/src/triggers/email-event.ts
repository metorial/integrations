...
export let emailEvent = SlateTrigger.create(
  spec,
  {
    name: 'Email Event',
    key: 'email_event',
    description: 'Receive real-time webhook notifications for email delivery events including delivery, open, click, unsubscribe, spam complaint, bounce, soft bounce, suspension, and reject. Configure the webhook URL in Mailtrap dashboard under Settings → Webhooks.',
    instructions: [
  ...
  .input(z.object({
    ...
    customVariables: z.record(z.string(), z.string()).optional().describe('Custom variables attached to the email'),
  ...
  .output(z.object({
    ...
    sendingDomainName: z.string().optional().describe('Sending domain name'),
    category: z.string().optional().describe('Email category'),
    customVariables: z.record(z.string(), z.string()).optional().describe('Custom variables from the email'),
