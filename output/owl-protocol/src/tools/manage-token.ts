...
export let getToken = SlateTool.create(
  spec,
  {
    name: 'Get Token',
    key: 'get_token',
...
Returns the token's on-chain data and associated metadata (name, description, image, attributes).`,
  ...
  .output(z.object({
    ...
    metadata: z.record(z.string(), z.unknown()).optional().describe('Token metadata including name, description, image, and attributes'),
...
export let updateToken = SlateTool.create(
  spec,
  {
...
Use this to change a token's name, description, image, or attributes after minting.`,
  ...
  .input(z.object({
    ...
    metadata: z.object({
      ...
      })).optional().describe('Updated token attributes'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    ctx.info({ message: 'Updating token metadata', contractAddress: ctx.input.contractAddress, tokenId: ctx.input.tokenId });
