...
export let newUsers = SlateTrigger.create(
  spec,
  {
    name: 'New User Created',
    key: 'new_users',
    description: 'Triggers when a new project user is created. Polls for newly added users in the project.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      return {
        inputs: newUsers.map((u: { userId?: string; id?: string; email?: string; fullName?: string; externalId?: string; walletAddress?: string; safeAddress?: string }) => ({
