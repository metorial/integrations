...
export let listLists = SlateTool.create(
  ...
  .input(z.object({
    limit: z.number().optional().default(50).describe('Number of lists to return (default: 50)'),
...
export let createList = SlateTool.create(
  ...
  .input(z.object({
    ...
    listType: z.enum(['USER_LIST', 'COMPANY_LIST']).optional().default('USER_LIST').describe('Type of list to create: USER_LIST for leads, COMPANY_LIST for companies'),
  }))
  .output(z.object({
