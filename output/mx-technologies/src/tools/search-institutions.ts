...
let institutionSchema = z.object({
  ...
  mediumLogoUrl: z.string().optional().nullable().describe('URL of the institution logo (medium)'),
  smallLogoUrl: z.string().optional().nullable().describe('URL of the institution logo (small)'),
  url: z.string().optional().nullable().describe('Institution website URL'),
  supportsAccountVerification: z.boolean().optional().nullable().describe('Whether the institution supports account verification'),
  supportsOauth: z.boolean().optional().nullable().describe('Whether the institution supports OAuth connections'),
  supportsTransactionHistory: z.boolean().optional().nullable().describe('Whether extended transaction history is supported'),
});
...
let credentialSchema = z.object({
  ...
  fieldType: z.string().optional().nullable().describe('Type of field (TEXT, PASSWORD, OPTIONS, etc.)'),
...
export let searchInstitutions = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    supportsAccountVerification: z.boolean().optional().describe('Filter to institutions supporting account verification'),
...
export let readInstitution = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    institutionCode: z.string().describe('Unique code of the institution (e.g., "chase", "mx_bank")'),
  }))
  .output(z.object({
