...
- **Delete**: provide networkId and set action to "delete".`,
  ...
  .input(z.object({
    ...
    attributes: z.record(z.string(), z.any()).optional().describe('Additional network attributes'),
  }))
  .output(z.object({
    network: z.record(z.string(), z.any()).optional().describe('Network details (for create/update)'),
