...
export let manageNetworkSecretKey = SlateTool.create(
  spec,
  {
...
- **revoke**: Removes the secret key entirely.`,
  ...
  .input(z.object({
    ...
    action: z.enum(['generate', 'rotate', 'revoke']).describe('Secret key operation to perform'),
  }))
  .output(z.object({
    secretKey: z.record(z.string(), z.any()).optional().describe('Secret key details (for generate/rotate)'),
