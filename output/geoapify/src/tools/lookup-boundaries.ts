...
export let lookupBoundaries = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    direction: z.enum(['part_of', 'consists_of']).describe('"part_of" to find containing boundaries, "consists_of" to find sub-boundaries'),
    placeId: z.string().optional().describe('Place ID from geocoding results'),
    lat: z.number().optional().describe('Latitude (for part_of only, alternative to placeId)'),
    lon: z.number().optional().describe('Longitude (for part_of only, alternative to placeId)'),
    geometry: z.string().optional().describe('Geometry detail level (e.g. "geometry_1000" for simplified polygons)'),
