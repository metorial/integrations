...
export let ipGeolocation = SlateTool.create(
  ...
  .input(z.object({
    ipAddress: z.string().optional().describe('IP address to geolocate. Leave empty to detect the caller\'s IP.'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Languages spoken in the detected location'),
  }))
