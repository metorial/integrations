...
let geocodeResultSchema = z.object({
  ...
  addressLine2: z.string().optional().describe('Second address line (e.g. city, state, country)'),
...
export let geocodeAddress = SlateTool.create(
  ...
  .input(z.object({
    ...
    lang: z.string().optional().describe('ISO 639-1 language code for results (e.g. "en", "de", "fr")'),
    limit: z.number().optional().describe('Maximum number of results (default: 5)'),
    filter: z.string().optional().describe('Geographic filter constraint (e.g. "countrycode:us", "circle:lon,lat,radius")'),
    bias: z.string().optional().describe('Geographic proximity bias (e.g. "proximity:lon,lat", "countrycode:us")'),
  }))
  .output(z.object({
