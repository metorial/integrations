...
export let manageOnlineArchiveTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    criteria: z.object({
      ...
      dateFormat: z.string().optional().describe('Date format (ISODATE, EPOCH_SECONDS, EPOCH_MILLIS, EPOCH_NANOSECONDS)'),
      expireAfterDays: z.number().optional().describe('Number of days after which data is archived'),
      query: z.string().optional().describe('JSON query string for CUSTOM criteria'),
    }).optional().describe('Criteria for determining which documents to archive'),
    ...
    })).optional().describe('Fields to partition archived data by for efficient querying'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      if (!ctx.input.databaseName || !ctx.input.collectionName) {
        throw new Error('databaseName and collectionName are required for creating an online archive.');
      }
      if (!ctx.input.criteria) throw new Error('criteria is required for creating an online archive.');
    ...
    if (action === 'update') {
      ...
      let archive = await client.updateOnlineArchive(projectId, clusterName, ctx.input.archiveId, data);
      return {
        output: { archive },
