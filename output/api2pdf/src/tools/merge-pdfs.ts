...
export let mergePdfs = SlateTool.create(
  ...
  .input(z.object({
    urls: z.array(z.string()).min(2).describe('URLs of PDF files to merge, in desired order'),
    ...
    extraHttpHeaders: z.record(z.string(), z.string()).optional().describe('Extra HTTP headers when fetching the source PDFs'),
  }))
  .output(z.object({
