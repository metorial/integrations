...
export let createZip = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    })).min(1).describe('List of files to include in the ZIP archive'),
    ...
    extraHttpHeaders: z.record(z.string(), z.string()).optional().describe('Extra HTTP headers when fetching the source files'),
  }))
  .output(z.object({
