...
export let sendMessage = SlateTool.create(
  ...
  .input(z.object({
    ...
    type: z.enum(['chat', 'private_note']).describe('Message type: "chat" for visible messages, "private_note" for agent-only'),
    ...
    userId: z.number().optional().describe('Drift user ID to send the message as (defaults to bot)'),
  }))
  .output(z.object({
