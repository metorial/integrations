...
export let listConversations = SlateTool.create(
  ...
  .input(z.object({
    status: z.enum(['open', 'closed', 'pending']).optional().describe('Filter by conversation status'),
    limit: z.number().optional().describe('Max conversations to return (max 50, default 25)'),
  ...
  .output(z.object({
    conversations: z.array(z.object({
      ...
      participants: z.array(z.number()).optional().describe('List of participant user IDs'),
      conversationTags: z.array(z.object({
        name: z.string(),
        color: z.string().optional(),
      })).optional().describe('Tags applied to the conversation'),
    })),
