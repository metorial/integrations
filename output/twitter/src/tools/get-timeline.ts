...
export let getTimeline = SlateTool.create(
  ...
  .input(z.object({
    ...
    timelineType: z.enum(['user', 'mentions', 'home']).describe('Type of timeline to retrieve'),
    maxResults: z.number().min(1).max(100).optional().describe('Number of results (1-100, default 10)'),
    ...
    exclude: z.array(z.enum(['retweets', 'replies'])).optional().describe('Exclude retweets and/or replies'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { userId, timelineType, maxResults, sinceId, untilId, startTime, endTime, exclude, paginationToken } = ctx.input;

    let result;
    if (timelineType === 'user') {
      result = await client.getUserTimeline(userId, { maxResults, sinceId, untilId, startTime, endTime, exclude, paginationToken });
    } else if (timelineType === 'mentions') {
      result = await client.getUserMentions(userId, { maxResults, sinceId, untilId, startTime, endTime, paginationToken });
    } else {
      result = await client.getReverseChronologicalTimeline(userId, { maxResults, sinceId, untilId, startTime, endTime, exclude, paginationToken });
    }
