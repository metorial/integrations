...
export let createToken = SlateTool.create(
  ...
  .input(z.object({
    ...
    licenseeNumber: z.string().optional().describe('Licensee number. Required for SHOP tokens.'),
    successUrl: z.string().optional().describe('URL to redirect to after successful shop purchase'),
    cancelUrl: z.string().optional().describe('URL to redirect to if shop purchase is cancelled'),
    apiKeyRole: z.string().optional().describe('Role for APIKEY tokens (Licensee, Analytics, Operation, Maintenance, Admin)'),
