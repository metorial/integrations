...
export let earningsEvent = SlateTrigger.create(
  spec,
  {
    name: 'Earnings Event',
    key: 'earnings_event',
    description: 'Triggers when new earnings reports appear on the financial calendar. Polls the earnings calendar for upcoming and recent earnings announcements.'
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    earningsDate: z.string().optional().nullable().describe('Earnings date'),
    beforeAfterMarket: z.string().optional().nullable().describe('Whether reported before or after market hours'),
    ...
    epsDifference: z.number().optional().nullable().describe('Difference between actual and estimated EPS'),
    epsDifferencePercent: z.number().optional().nullable().describe('Percentage difference from estimate')
  }))
  .polling({
    options: {
    ...
    pollEvents: async (ctx) => {
      ...
      let nextWeek = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]!;
