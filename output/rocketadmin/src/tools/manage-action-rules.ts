...
export let manageActionRules = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    ruleEvent: z.string().optional().describe('Event type that triggers the rule (e.g., "created", "updated", "deleted")'),
    webhookUrl: z.string().optional().describe('Webhook URL to call when the rule triggers'),
    ruleData: z.record(z.string(), z.unknown()).optional().describe('Additional rule configuration'),
  }))
  .output(z.object({
    rules: z.array(z.record(z.string(), z.unknown())).optional().describe('List of action rules'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { action, connectionId, tableName, ruleId, ruleTitle, ruleEvent, webhookUrl, ruleData } = ctx.input;
