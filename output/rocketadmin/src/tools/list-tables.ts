...
export let listTables = SlateTool.create(
  ...
  .input(z.object({
    ...
    tableName: z.string().optional().describe('If provided, returns the structure of this specific table'),
  }))
  .output(z.object({
    tables: z.array(z.record(z.string(), z.unknown())).optional().describe('List of tables in the connection'),
    structure: z.record(z.string(), z.unknown()).optional().describe('Table structure details including columns and types'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.tableName) {
      let structure = await client.getTableStructure(ctx.input.connectionId, ctx.input.tableName);
      return {
        output: { structure },
