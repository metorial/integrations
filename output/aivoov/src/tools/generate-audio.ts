...
let textSegmentSchema = z.object({
  voiceId: z.string().describe('Voice ID to use for this segment (e.g., "en-US-RyanMultilingualNeural"). Use the List Voices tool to discover available voice IDs.'),
  text: z.string().describe('Text content to convert to speech.'),
  pitch: z.number().min(-50).max(50).optional().describe('Pitch adjustment from -50 to 50. Omit to use the default.'),
  speakingRate: z.number().min(20).max(200).optional().describe('Speaking rate from 20 to 200. Omit to use the default.'),
  volume: z.number().min(-40).max(40).optional().describe('Volume adjustment from -40 to 40. Omit to use the default.'),
});

export let generateAudio = SlateTool.create(
  spec,
  {
    name: 'Generate Audio',
    key: 'generate_audio',
  ...
  .input(z.object({
    segments: z.array(textSegmentSchema).min(1).describe('One or more text segments to convert to speech. Each segment specifies a voice and text, with optional SSML controls.'),
  }))
  .output(z.object({
