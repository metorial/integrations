...
export let digitalWrite = SlateTool.create(
  spec,
  {
    name: 'Digital Write',
    key: 'digital_write',
  ...
  .input(z.object({
    ...
    states: z.array(z.enum(['HIGH', 'LOW'])).describe('List of states corresponding to each pin')
  }))
  .output(z.object({
    success: z.string().describe('Whether the write operation was successful ("1" for success)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let pinStateDesc = ctx.input.pins.map((p, i) => `pin ${p} → ${ctx.input.states[i]}`).join(', ');
