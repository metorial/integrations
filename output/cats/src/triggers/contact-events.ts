...
let CONTACT_EVENTS = ['contact.created', 'contact.updated', 'contact.deleted', 'contact.status_changed'] as const;
...
export let contactEvents = SlateTrigger.create(
  spec,
  {
    name: 'Contact Events',
    key: 'contact_events',
    description: 'Triggers when a contact is created, updated, deleted, or has its status changed.',
  }
)
  .input(z.object({
