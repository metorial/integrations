...
export let createPipeline = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    let pipelineId = result?.id?.toString() ?? result?._links?.self?.href?.split('/').pop() ?? '';
...
export let updatePipelineStatus = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    fireTriggers: z.boolean().optional().describe('Whether to fire status triggers (default: true)'),
