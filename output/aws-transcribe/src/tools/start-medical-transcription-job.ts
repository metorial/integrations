...
export let startMedicalTranscriptionJob = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    jobName: z.string().describe('Unique name for the medical transcription job (1-200 chars, no spaces)'),
    mediaFileUri: z.string().describe('S3 URI of the medical audio file'),
    languageCode: z.string().default('en-US').describe('Language code (currently only en-US supported)'),
    specialty: z.enum(['PRIMARYCARE']).default('PRIMARYCARE').describe('Medical specialty'),
    type: z.enum(['CONVERSATION', 'DICTATION']).describe('CONVERSATION for dialogue, DICTATION for single-speaker notes'),
    ...
    mediaFormat: z.enum(['mp3', 'mp4', 'wav', 'flac', 'ogg', 'amr', 'webm']).optional().describe('Format of the media file'),
    ...
    settings: z.object({
      ...
      maxSpeakerLabels: z.number().optional().describe('Maximum number of speakers (2-10)'),
      channelIdentification: z.boolean().optional().describe('Enable multi-channel identification'),
      showAlternatives: z.boolean().optional().describe('Enable alternative transcriptions'),
      maxAlternatives: z.number().optional().describe('Maximum number of alternatives (2-10)'),
      vocabularyName: z.string().optional().describe('Custom medical vocabulary name'),
    }).optional().describe('Medical transcription settings'),
    contentIdentificationType: z.enum(['PHI']).optional().describe('Identify protected health information (PHI)'),
