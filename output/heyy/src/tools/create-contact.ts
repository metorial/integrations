...
export let createContactTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    email: z.string().nullable().optional().describe('Email address (must be valid email format)'),
    phoneNumber: z.string().nullable().optional().describe('Phone number of the contact'),
    labels: z.array(z.object({
      name: z.string().describe('Label name'),
    })).optional().describe('Labels to assign to the contact'),
    attributes: z.array(z.object({
      externalId: z.string().describe('Attribute identifier (starts with letter, alphanumeric + underscores only)'),
      value: z.string().describe('Attribute value'),
    })).optional().describe('Custom attributes for the contact'),
  }))
  .output(z.object({
