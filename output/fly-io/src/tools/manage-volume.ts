...
export let manageVolume = SlateTool.create(
  spec,
  {
    name: 'Manage Volume',
    key: 'manage_volume',
  ...
  .input(z.object({
    ...
    action: z.enum(['get', 'update', 'extend', 'delete', 'list_snapshots', 'create_snapshot']).describe('Action to perform'),
    snapshotRetention: z.number().optional().describe('Snapshot retention in days (for update, 1-60)'),
    autoBackupEnabled: z.boolean().optional().describe('Enable/disable auto-backup (for update)'),
    sizeGb: z.number().optional().describe('New size in GB (for extend; must be larger than current size)'),
  }))
  .output(z.object({
    volume: z.object({
    ...
    }).optional().describe('Volume details (for get, update, extend)'),
    ...
    })).optional().describe('Snapshots list (for list_snapshots)'),
    snapshotCreated: z.boolean().optional().describe('Whether snapshot was created'),
    needsRestart: z.boolean().optional().describe('Whether the machine needs restart after extend'),
  }))
