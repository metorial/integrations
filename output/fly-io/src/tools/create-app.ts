...
export let createApp = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'App names must be globally unique across all Fly.io users.',
    ],
  }
)
  .input(z.object({
    ...
    network: z.string().optional().describe('Optional custom network name to isolate this app into its own IPv6 private network'),
    enableSubdomains: z.boolean().optional().describe('Whether to enable subdomains for the app'),
  }))
  .output(z.object({
