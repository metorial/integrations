...
export let controlMachine = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['start', 'stop', 'restart', 'suspend', 'cordon', 'uncordon']).describe('Action to perform'),
    signal: z.string().optional().describe('Stop/restart signal (e.g. "SIGTERM", "SIGINT"). Only used with stop and restart actions.'),
    timeout: z.number().optional().describe('Grace period in seconds before SIGKILL. Only used with stop and restart actions.'),
  }))
  .output(z.object({
    machineId: z.string().describe('Machine identifier'),
    action: z.string().describe('Action that was performed'),
    previousState: z.string().optional().describe('Machine state before the action (only returned for start)'),
    migrated: z.boolean().optional().describe('Whether the machine was migrated to a new host (only returned for start)'),
  }))
