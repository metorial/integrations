...
export let getAnalytics = SlateTool.create(
  ...
  .input(z.object({
    ...
    dimension: z.enum([
      'status', 'domains', 'reasons', 'ips', 'devices',
      'protocols', 'queryTypes', 'ipVersions', 'dnssec',
      'encryption', 'destinations',
    ]).describe('Analytics dimension to query'),
    from: z.string().optional().describe('Start date (ISO 8601, Unix timestamp, or relative like "-7d")'),
    to: z.string().optional().describe('End date (ISO 8601, Unix timestamp, or relative like "now")'),
    deviceId: z.string().optional().describe('Filter by device ID'),
    status: z.string().optional().describe('Filter by status (e.g., "blocked")'),
    destinationType: z.enum(['countries', 'gafam']).optional().describe('For destinations dimension: "countries" or "gafam"'),
  ...
  .output(z.object({
    ...
    meta: z.record(z.string(), z.unknown()).optional().describe('Response metadata including pagination'),
  }))
