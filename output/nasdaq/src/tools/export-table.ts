...
export let exportTable = SlateTool.create(
  spec,
  {
    name: 'Export Table',
    key: 'export_table',
...
Use this for large datasets that exceed the 10,000-row pagination limit.`,
  ...
  .input(z.object({
    ...
    filters: z.record(z.string(), z.string()).optional().describe('Optional row-level filters to apply to the export.'),
  }))
  .output(z.object({
    downloadLink: z.string().describe('URL to download the exported ZIP file.'),
    fileStatus: z.string().describe('File status: "fresh" (ready), "creating" (in progress), or "regenerating" (outdated, new version being created).'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let response = await client.requestBulkExport(ctx.input.tablePath, ctx.input.filters as Record<string, string> | undefined);
