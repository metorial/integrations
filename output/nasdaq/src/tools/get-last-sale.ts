...
export let getLastSale = SlateTool.create(
  spec,
  {
    name: 'Get Last Sale',
    key: 'get_last_sale',
...
Also supports fetching the latest bid/ask quote or most recent trade. Requires OAuth2 authentication with the Real-Time/Delayed REST API.`,
  ...
  .input(z.object({
    symbols: z.array(z.string()).describe('List of stock ticker symbols (e.g., ["AAPL", "MSFT", "GOOG"]).'),
    dataType: z.enum(['sale', 'trade', 'quote']).default('sale').describe('Type of data: "sale" for last sale, "trade" for last trade, "quote" for bid/ask.'),
  ...
  .output(z.object({
    quotes: z.array(z.record(z.string(), z.any())).describe('Array of quote/sale records for the requested symbols.'),
  }))
