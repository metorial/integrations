...
export let decodeL2Batch = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    transactionHash: z.string().describe('L1 transaction hash of the batch to decode (0x-prefixed)'),
    batchIndex: z.number().optional().describe('Frame number (Optimism) or sequence number (Arbitrum) for pagination'),
    includeStateDiffs: z.boolean().optional().describe('Whether to include state diffs for zkSync batches. Defaults to true.')
  }))
  .output(z.object({
    network: z.string().describe('L2 chain identifier (e.g., "optimism", "arbitrum", "zksync")'),
