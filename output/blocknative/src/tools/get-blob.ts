...
export let getBlob = SlateTool.create(
  spec,
  {
    name: 'Get Blob',
    key: 'get_blob',
  ...
  .input(z.object({
    ...
    includeData: z.boolean().optional().describe('Whether to include the raw blob data in the response. Defaults to true.')
  }))
  .output(z.object({
    ...
    blobData: z.string().describe('Raw blob data as 0x-prefixed hex (empty string if includeData was false)')
  }))
