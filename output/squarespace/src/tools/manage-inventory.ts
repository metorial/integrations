...
export let manageInventory = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['retrieve', 'adjust']).describe('Whether to retrieve inventory or adjust stock levels'),
    variantIds: z.array(z.string()).optional().describe('Specific variant IDs to retrieve (for retrieve action)'),
    cursor: z.string().optional().describe('Pagination cursor for listing all inventory'),
    idempotencyKey: z.string().optional().describe('Unique key to prevent duplicate adjustments (required for adjust action)'),
    incrementOperations: z.array(z.object({
      variantId: z.string().describe('Variant ID'),
      quantity: z.number().describe('Quantity to add')
    })).optional().describe('Increase stock for variants'),
    decrementOperations: z.array(z.object({
      variantId: z.string().describe('Variant ID'),
      quantity: z.number().describe('Quantity to subtract')
    })).optional().describe('Decrease stock for variants'),
    setFiniteOperations: z.array(z.object({
      variantId: z.string().describe('Variant ID'),
      quantity: z.number().describe('Exact quantity to set')
    })).optional().describe('Set exact stock quantities'),
    setUnlimitedOperations: z.array(z.string()).optional().describe('Variant IDs to mark as unlimited stock')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'adjust') {
      ...
      let hasOperations = (ctx.input.incrementOperations?.length || 0) +
