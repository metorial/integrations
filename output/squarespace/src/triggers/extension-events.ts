...
export let extensionEvents = SlateTrigger.create(
  spec,
  {
    name: 'Extension Uninstall',
    key: 'extension_events',
    description: 'Triggers when a user uninstalls an extension (OAuth client) from their Squarespace site. Useful for cleanup when a user disconnects the integration.'
  }
)
  .input(z.object({
  ...
  .output(z.object({
    websiteId: z.string().optional().describe('The website from which the extension was uninstalled'),
    uninstalledAt: z.string().optional().describe('ISO 8601 timestamp of the uninstall event')
  }))
  .webhook({
