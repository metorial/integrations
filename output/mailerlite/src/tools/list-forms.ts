...
export let listForms = SlateTool.create(
  ...
  .input(z.object({
    ...
    formId: z.string().optional().describe('Form ID — if provided, returns subscribers for this form'),
    name: z.string().optional().describe('Filter forms by name'),
    subscriberStatus: z.enum(['active', 'unsubscribed', 'unconfirmed', 'bounced', 'junk']).optional().describe('Filter form subscribers by status (only when formId is provided)'),
