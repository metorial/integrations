...
export let createOrUpdateSubscriber = SlateTool.create(
  spec,
  {
...
**Note:** If a subscriber's status is unsubscribed, bounced, or junk, you cannot reactivate them via the API.`,
  ...
  .input(z.object({
    ...
    fields: z.record(z.string(), z.any()).optional().describe('Custom field values as key-value pairs (e.g., { "name": "John", "company": "Acme" })'),
    groupIds: z.array(z.string()).optional().describe('Group IDs to assign the subscriber to'),
    status: z.enum(['active', 'unsubscribed', 'unconfirmed', 'bounced', 'junk']).optional().describe('Subscriber status'),
    subscribedAt: z.string().optional().describe('Date when the subscriber was added (ISO 8601 format)'),
    ipAddress: z.string().optional().describe('IP address of the subscriber'),
    optedInAt: z.string().optional().describe('Date when the subscriber opted in (ISO 8601 format)'),
