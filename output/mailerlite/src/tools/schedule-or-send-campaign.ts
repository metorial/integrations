...
export let scheduleOrSendCampaign = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['send', 'schedule', 'cancel']).describe('"send" for instant delivery, "schedule" to set a future date, "cancel" to revert to draft'),
    date: z.string().optional().describe('Scheduled date in YYYY-MM-DD format (required for schedule action)'),
    ...
    timezoneId: z.number().optional().describe('Timezone ID for scheduling (get IDs from List Timezones)'),
  }))
  .output(z.object({
