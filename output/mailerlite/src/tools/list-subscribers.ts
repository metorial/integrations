...
export let listSubscribers = SlateTool.create(
  ...
  .input(z.object({
    status: z.enum(['active', 'unsubscribed', 'unconfirmed', 'bounced', 'junk']).optional().describe('Filter subscribers by status'),
    limit: z.number().optional().describe('Number of subscribers to return per page (default 25, max 50)'),
  ...
  .output(z.object({
    ...
    nextCursor: z.string().optional().nullable().describe('Cursor for the next page of results'),
  }))
