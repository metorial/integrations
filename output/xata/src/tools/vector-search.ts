...
export let vectorSearch = SlateTool.create(
  spec,
  {
    name: 'Vector Search',
    key: 'vector_search',
  ...
  .input(z.object({
    ...
    queryVector: z.array(z.number()).describe('Query vector (must match the column dimension)'),
    similarityFunction: z.enum(['cosineSimilarity', 'l1', 'l2']).optional().describe('Similarity function to use (default: cosineSimilarity)'),
