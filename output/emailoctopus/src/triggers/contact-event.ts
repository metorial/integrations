...
export let contactEvent = SlateTrigger.create(
  spec,
  {
    name: 'Contact Event',
    key: 'contact_event',
    description: 'Fires when a contact is created, updated, deleted, bounced, clicked, complained, opened, or unsubscribed. Webhooks must be configured in the EmailOctopus dashboard under Integrations & APIs → Webhooks.',
  }
)
  .input(z.object({
    ...
    contactFields: z.record(z.string(), z.string()).optional().describe('Custom field values'),
    ...
    campaignId: z.string().optional().describe('Associated campaign ID (for campaign-related events)'),
