...
let destinationOutputSchema = z.object({
  ...
  config: z.record(z.string(), z.any()).optional().describe('Service-specific destination configuration'),
...
export let createDestination = SlateTool.create(
  ...
  .input(z.object({
    ...
    service: z.string().describe('Destination service type (e.g., "snowflake", "big_query", "redshift")'),
    region: z.string().optional().describe('Data processing region (e.g., "GCP_US_EAST4", "AWS_US_EAST_1")'),
    timeZoneOffset: z.string().optional().describe('Time zone offset (e.g., "+0", "-5")'),
    daylightSavingTimeEnabled: z.boolean().optional().describe('Enable daylight saving time adjustment'),
    config: z.record(z.string(), z.any()).optional().describe('Service-specific destination configuration'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.daylightSavingTimeEnabled !== undefined) body.daylight_saving_time_enabled = ctx.input.daylightSavingTimeEnabled;
