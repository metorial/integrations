...
let storySchema = z.object({
  ...
  keyArticleTitle: z.string().optional().describe('Title of the representative article for this story'),
  keyArticleUrl: z.string().optional().describe('URL of the representative article for this story'),
});

export let searchStories = SlateTool.create(
  spec,
  {
    name: 'Search Stories',
    key: 'search_stories',
  ...
  .input(z.object({
    ...
    showDuplicates: z.boolean().optional().describe('Include duplicate story clusters (default: false)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let stories = (result.results || []).map((s) => ({
      ...
      people: (s.people || []).map((p) => ({ wikidataId: p.wikidataId || '', name: p.name, count: p.count })),
      companies: (s.companies || []).map((c) => ({ companyId: c.id || '', name: c.name, count: c.count })),
