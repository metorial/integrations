...
export let searchArticles = SlateTool.create(
  ...
  .input(z.object({
    query: z.string().optional().describe('Search keywords with optional Boolean operators (AND, OR, NOT)'),
    ...
    excludeSource: z.string().optional().describe('Exclude articles from this source domain'),
    sourceGroup: z.enum(['top10', 'top50', 'top100', 'top500', 'top1000']).optional().describe('Filter by pre-defined source group'),
    category: z.string().optional().describe('Filter by category (e.g. Tech, Business, Politics, Sports, Finance, Entertainment, Health, Environment, Travel, Auto)'),
    topic: z.string().optional().describe('Filter by topic'),
    country: z.string().optional().describe('Filter by country code (ISO 3166-1 alpha-2, e.g. "us", "gb")'),
    language: z.string().optional().describe('Filter by language code (e.g. "en", "es")'),
    label: z.string().optional().describe('Filter by smart label (e.g. Opinion, Analysis, Fact Check, Paid News)'),
    excludeLabel: z.string().optional().describe('Exclude articles with this smart label (e.g. Non-news, Opinion)'),
    ...
    clusterId: z.string().optional().describe('Get all articles belonging to a specific story cluster'),
    dateFrom: z.string().optional().describe('Start date filter (ISO 8601)'),
    dateTo: z.string().optional().describe('End date filter (ISO 8601)'),
    showReprints: z.boolean().optional().describe('Include reprints/duplicates (default: true)'),
    sortBy: z.enum(['date', 'relevance', 'addDate']).optional().describe('Sort order for results'),
