...
export let searchTasks = SlateTool.create(
  ...
  .input(z.object({
    ...
    projectIds: z.array(z.string()).optional().describe('Filter by project GIDs (any match)'),
    tagIds: z.array(z.string()).optional().describe('Filter by tag GIDs (any match)'),
    sectionIds: z.array(z.string()).optional().describe('Filter by section GIDs (any match)'),
    completed: z.boolean().optional().describe('Filter by completion status'),
    isSubtask: z.boolean().optional().describe('Filter by whether task is a subtask'),
    dueOnBefore: z.string().optional().describe('Tasks due on or before this date (YYYY-MM-DD)'),
    dueOnAfter: z.string().optional().describe('Tasks due on or after this date (YYYY-MM-DD)'),
    ...
    modifiedOnBefore: z.string().optional().describe('Tasks modified on or before this date'),
    modifiedOnAfter: z.string().optional().describe('Tasks modified on or after this date'),
    sortBy: z.enum(['due_date', 'created_at', 'completed_at', 'likes', 'modified_at']).optional().describe('Sort field'),
