...
To **create** a webhook, provide a URL and events to subscribe to. To **update** an existing webhook, provide the webhook ID along with the fields to change. To **disable**, set the enabled field to false.`,
  ...
  .input(z.object({
    webhookId: z.string().optional().describe('ID of an existing webhook to update. Leave empty to create a new webhook.'),
    url: z.string().optional().describe('The URL that will receive webhook event payloads'),
    events: z.array(z.string()).optional().describe('Array of event types to subscribe to (e.g. list.completed, verification.completed)'),
    enabled: z.boolean().optional().describe('Whether the webhook is enabled. Set to false to disable.'),
  }))
  .output(z.object({
