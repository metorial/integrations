...
export let trackEvent = SlateTool.create(
  ...
  .input(z.object({
    ...
    eventProperties: z.record(z.string(), z.any()).optional().describe('Additional properties for the event'),
    occurredAt: z.string().optional().describe('When the event occurred (UNIX timestamp). Defaults to now'),
  }))
  .output(z.object({
