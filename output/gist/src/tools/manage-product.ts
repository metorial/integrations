...
export let manageProduct = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'get', 'create_variant', 'update_variant', 'get_variant']).describe('Action to perform'),
    productId: z.string().optional().describe('Product ID (for update/get or as parent for variants)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let buildBody = () => {
      ...
      if (ctx.input.inventoryQuantity !== undefined) body.inventory_quantity = ctx.input.inventoryQuantity;
      if (ctx.input.category) body.category = ctx.input.category;
      if (ctx.input.imageUrl) body.image_url = ctx.input.imageUrl;
      if (ctx.input.productId && (ctx.input.action === 'create_variant' || ctx.input.action === 'update_variant')) {
    ...
    switch (ctx.input.action) {
      case 'create': {
        ...
        return {
          output: { productId: String(product.id), name: product.name, price: product.price, sku: product.sku },
      ...
      case 'create_variant': {
        ...
        return {
          output: { variantId: String(variant.id), name: variant.name, price: variant.price, sku: variant.sku },
