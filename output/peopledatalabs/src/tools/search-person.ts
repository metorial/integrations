...
export let searchPerson = SlateTool.create(
  spec,
  {
    name: 'Search People',
    key: 'search_person',
...
Returns matching person records with pagination support.`,
  ...
  .input(z.object({
    sql: z.string().optional().describe('SQL query to search for persons (e.g. "SELECT * FROM person WHERE location_country=\'united states\' AND job_title=\'software engineer\'")'),
