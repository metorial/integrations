...
export let cleanCompany = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    cleanedName: z.string().nullable().optional().describe('Cleaned, canonical company name'),
    fuzzyMatch: z.boolean().nullable().optional().describe('Whether the match was fuzzy rather than exact'),
...
export let cleanLocation = SlateTool.create(
  spec,
  {
    name: 'Clean Location',
    key: 'clean_location',
  ...
  .input(z.object({
    location: z.string().describe('Raw location string to clean (e.g. "sf bay area" or "new york, ny")'),
  }))
  .output(z.object({
...
export let cleanSchool = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    fuzzyMatch: z.boolean().nullable().optional().describe('Whether the match was fuzzy rather than exact'),
    type: z.string().nullable().optional().describe('School type (e.g. "university", "college")'),
