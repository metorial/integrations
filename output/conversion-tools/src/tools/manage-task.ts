...
**Delete** permanently removes all files associated with the task (paid plans only).`,
  ...
  .input(z.object({
    ...
    action: z.enum(['update_retention', 'delete']).describe('The management action to perform'),
    retentionMode: z.enum(['standard_24h', 'ttl_15m']).optional().describe('New retention mode (required when action is "update_retention")'),
  }))
  .output(z.object({
    taskId: z.string().describe('The task ID'),
    action: z.string().describe('The action that was performed'),
    retentionMode: z.string().optional().describe('Updated retention mode (for update_retention action)'),
    dateExpires: z.string().optional().describe('New expiration date (for update_retention action)'),
