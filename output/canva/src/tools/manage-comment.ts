...
export let createComment = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Message must be 1-2048 characters.',
      'Maximum 100 replies per thread.',
    ],
  }
)
  .input(z.object({
    designId: z.string().describe('The design ID to comment on'),
    message: z.string().min(1).max(2048).describe('Comment text (1-2048 chars). Mention users via [user_id:team_id].'),
    threadId: z.string().optional().describe('If provided, creates a reply to this thread instead of a new thread'),
    assigneeId: z.string().optional().describe('User ID to assign the comment to (new threads only, user must be mentioned)'),
  }))
  .output(z.object({
    thread: commentThreadSchema.optional().describe('Created comment thread (when creating a new thread)'),
...
export let getCommentThread = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    designId: z.string().describe('The design ID'),
    threadId: z.string().describe('The comment thread ID'),
    includeReplies: z.boolean().optional().describe('Whether to also fetch replies for this thread'),
    repliesLimit: z.number().min(1).max(100).optional().describe('Number of replies to fetch (1-100, default 50)'),
  }))
  .output(z.object({
    thread: commentThreadSchema.describe('The comment thread'),
    replies: z.array(replySchema).optional().describe('Replies in the thread (when includeReplies is true)'),
