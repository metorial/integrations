...
export let createDesign = SlateTool.create(
  spec,
  {
    name: 'Create Design',
    key: 'create_design',
    ...
    constraints: [
      'Custom dimensions must be between 40 and 8000 pixels.',
    ],
  }
)
  .input(z.object({
    presetType: z.enum(['doc', 'whiteboard', 'presentation']).optional().describe('Preset design type. Use this OR custom dimensions, not both.'),
    width: z.number().min(40).max(8000).optional().describe('Custom width in pixels (use with height instead of presetType)'),
    height: z.number().min(40).max(8000).optional().describe('Custom height in pixels (use with width instead of presetType)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let designType: { type: 'preset'; name: string } | { type: 'custom'; width: number; height: number };
