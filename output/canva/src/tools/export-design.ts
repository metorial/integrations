...
export let exportDesign = SlateTool.create(
  spec,
  {
    name: 'Export Design',
    key: 'export_design',
  ...
  .input(z.object({
    ...
    formatType: z.enum(['pdf', 'jpg', 'png', 'gif', 'pptx', 'mp4']).describe('Export format'),
    quality: z.number().min(1).max(100).optional().describe('Image quality for JPG exports (1-100)'),
    width: z.number().min(40).max(25000).optional().describe('Output width in pixels (JPG, PNG, GIF)'),
    height: z.number().min(40).max(25000).optional().describe('Output height in pixels (JPG, PNG, GIF)'),
    transparentBackground: z.boolean().optional().describe('Use transparent background for PNG exports'),
    asSingleImage: z.boolean().optional().describe('Export all pages as a single image for PNG exports'),
    ...
    pdfSize: z.enum(['a4', 'a3', 'letter', 'legal']).optional().describe('Page size for PDF exports'),
    mp4Quality: z.string().optional().describe('Video quality for MP4 exports (e.g., "horizontal_1080p")'),
    pages: z.array(z.number()).optional().describe('Specific page indices to export (0-based)'),
  }))
  .output(z.object({
    jobId: z.string().describe('The export job ID'),
    status: z.string().describe('Job status: "in_progress", "success", or "failed"'),
    downloadUrls: z.array(z.string()).optional().describe('Download URLs (valid for 24 hours, present when status is "success")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.formatType === 'jpg') {
      ...
      if (ctx.input.transparentBackground !== undefined) format['transparent_background'] = ctx.input.transparentBackground;
      if (ctx.input.asSingleImage !== undefined) format['as_single_image'] = ctx.input.asSingleImage;
      if (ctx.input.exportQuality) format['export_quality'] = ctx.input.exportQuality;
...
export let getExportJob = SlateTool.create(
  spec,
  {
    name: 'Get Export Job',
    key: 'get_export_job',
  ...
  .output(z.object({
    jobId: z.string().describe('The export job ID'),
    status: z.string().describe('Job status: "in_progress", "success", or "failed"'),
    downloadUrls: z.array(z.string()).optional().describe('Download URLs (valid for 24 hours)'),
