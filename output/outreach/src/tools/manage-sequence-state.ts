...
export let manageSequenceState = SlateTool.create(
  spec,
  {
...
Use the **create** action to enroll a prospect in a sequence, or **update** to pause/resume enrollment.`,
  ...
  .input(z.object({
    ...
    sequenceStateId: z.string().optional().describe('Sequence state ID (required for update)'),
    ...
    state: z.enum(['active', 'paused', 'finished', 'disabled']).optional().describe('Enrollment state'),
  }))
  .output(z.object({
