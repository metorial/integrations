...
export let runBrowserAgent = SlateTool.create(
  spec,
  {
...
Starts the agent task, waits for completion, and returns the final result and step-by-step history.`,
  ...
  .input(z.object({
    ...
    agentType: z.enum(['browser_use', 'claude_computer_use', 'openai_cua']).default('browser_use').describe('Type of AI agent to use'),
    llm: z.string().optional().describe('LLM model to use (e.g., "gemini-2.0-flash", "claude-sonnet-4-5", "computer-use-preview")'),
    maxSteps: z.number().optional().describe('Maximum number of actions the agent can take (default: 20)'),
    maxFailures: z.number().optional().describe('Maximum consecutive failures before stopping (default: 3)'),
    useVision: z.boolean().optional().describe('Enable vision mode for the agent (Browser Use only, default: true)'),
    keepBrowserOpen: z.boolean().optional().describe('Keep the browser session open after task completion'),
    ...
    }).optional().describe('Custom API keys for the underlying LLM providers'),
  }))
  .output(z.object({
    ...
    finalResult: z.string().optional().nullable().describe('Final output/result of the agent task'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.keepBrowserOpen !== undefined) params.keepBrowserOpen = ctx.input.keepBrowserOpen;
    ...
    if (ctx.input.useCustomApiKeys !== undefined) params.useCustomApiKeys = ctx.input.useCustomApiKeys;
