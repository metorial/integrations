...
The session can be connected to via Playwright, Puppeteer, or Selenium.`,
  ...
  .input(z.object({
    ...
    proxyCountry: z.string().optional().describe('Two-letter country code for proxy location'),
    ...
    }).optional().describe('Screen resolution'),
    ...
    profileId: z.string().optional().describe('Profile ID to attach for persistent browser state'),
    persistProfileChanges: z.boolean().optional().describe('Whether to save profile changes after session ends'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.timeoutMinutes !== undefined) params.timeoutMinutes = ctx.input.timeoutMinutes;
    if (ctx.input.enableWebRecording !== undefined) params.enableWebRecording = ctx.input.enableWebRecording;
    if (ctx.input.enableVideoWebRecording !== undefined) params.enableVideoWebRecording = ctx.input.enableVideoWebRecording;
...
export let stopSession = SlateTool.create(
  spec,
  {
    name: 'Stop Session',
    key: 'stop_session',
...
export let listSessions = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    let client = new HyperbrowserClient({ token: ctx.auth.token });
    let result = await client.listSessions({ page: ctx.input.page, perPage: ctx.input.perPage });
