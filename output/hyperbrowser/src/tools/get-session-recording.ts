...
Can get rrweb DOM-based replay recording URLs and MP4 video recording URLs. Video recordings must be explicitly enabled when creating the session.`,
  ...
  .input(z.object({
    ...
    includeVideo: z.boolean().optional().describe('Also retrieve the MP4 video recording URL (if available)'),
  }))
  .output(z.object({
    ...
    downloadsUrl: z.string().optional().nullable().describe('URL to retrieve downloaded files'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let recordingUrl = (recordingResult.url ?? recordingResult.recordingUrl) as string | null | undefined;
    ...
    if (ctx.input.includeVideo) {
      ...
      videoRecordingUrl = (videoResult.url ?? videoResult.videoRecordingUrl) as string | null | undefined;
    }

    let downloadsResult = await client.getSessionDownloadsUrl(ctx.input.sessionId);
    let downloadsUrl = (downloadsResult.url ?? downloadsResult.downloadsUrl) as string | null | undefined;
