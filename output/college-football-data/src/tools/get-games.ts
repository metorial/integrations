...
export let getGames = SlateTool.create(
  spec,
  {
    name: 'Get Games',
    key: 'get_games',
  ...
  .input(z.object({
    ...
    seasonType: z.enum(['regular', 'postseason', 'both']).optional().describe('Type of season games to return'),
    week: z.number().optional().describe('Week number to filter by'),
    team: z.string().optional().describe('Team name to filter by'),
    conference: z.string().optional().describe('Conference abbreviation to filter by (e.g. "SEC", "B1G")'),
    classification: z.enum(['fbs', 'fcs', 'ii', 'iii']).optional().describe('Division classification'),
    gameId: z.number().optional().describe('Specific game ID to retrieve'),
  }))
  .output(z.object({
    games: z.array(z.any()).describe('Array of game objects with scores, teams, venue, and metadata'),
  }))
