...
export let getGameStats = SlateTool.create(
  spec,
  {
    name: 'Get Game Stats',
    key: 'get_game_stats',
  ...
  .input(z.object({
    ...
    includePlayerStats: z.boolean().optional().default(true).describe('Whether to include individual player statistics'),
    includeAdvancedBoxScore: z.boolean().optional().default(false).describe('Whether to include the advanced box score with EPA metrics'),
  }))
  .output(z.object({
    ...
    advancedBoxScore: z.any().optional().describe('Advanced box score with EPA and efficiency data'),
  }))
