...
export let getTeamStats = SlateTool.create(
  spec,
  {
    name: 'Get Team Stats',
    key: 'get_team_stats',
  ...
  .input(z.object({
    ...
    includeAdvanced: z.boolean().optional().default(false).describe('Whether to include advanced stats (SP+, success rate, EPA, etc.)'),
    excludeGarbageTime: z.boolean().optional().default(false).describe('Whether to exclude garbage time plays from advanced stats'),
  }))
  .output(z.object({
    basicStats: z.array(z.any()).describe('Basic team season statistics'),
    advancedStats: z.array(z.any()).optional().describe('Advanced team season statistics with EPA and efficiency metrics'),
  }))
