...
export let createSegment = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Segments process one at a time per subscription.',
    ],
  }
)
  .input(z.object({
    name: z.string().describe('Name for the new segment'),
    visitorIds: z.array(z.string()).optional().describe('List of visitor IDs to include in the segment'),
    accountIds: z.array(z.string()).optional().describe('List of account IDs to include in the segment'),
  }))
  .output(z.object({
