...
export let pendoEvents = SlateTrigger.create(
  spec,
  {
    name: 'Pendo Events',
    key: 'pendo_events',
    description: 'Receives webhook events from Pendo including guide displayed, track events received, NPS/poll events, visitor/account created, and email unsubscribe events. Configure webhooks in Pendo under Settings > Integrations > Webhooks.',
    instructions: [
  ...
  .output(z.object({
    ...
    trackEventName: z.string().optional().describe('Track event name if this is a track event'),
    npsRating: z.number().optional().describe('NPS rating if this is an NPS submission'),
    pollResponse: z.string().optional().describe('Poll response if this is a poll submission'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let inputs = events.map((event: any) => {
        ...
        let eventId = event.id || event.eventId || `${eventType}-${event.visitorId || ''}-${event.timestamp || Date.now()}`;
...
let resolveEventType = (event: any): string => {
  ...
  if (normalized.includes('track') && normalized.includes('event')) return 'track_event.received';
  if (normalized.includes('nps') && normalized.includes('display')) return 'nps_survey.displayed';
  if (normalized.includes('nps') && normalized.includes('submit')) return 'nps_survey.submitted';
