...
let summarySchema = z.object({
  status: z.string().describe('Verification status code (V=Verified, P=Partial, U=Unverified, A=Ambiguous, C=Conflict, R=Reverted)'),
...
}).optional().describe('Verification summary with status and match scores');

let geoDataSchema = z.object({
  latitude: z.number().optional().describe('Latitude coordinate'),
  longitude: z.number().optional().describe('Longitude coordinate'),
  accuracy: z.string().optional().describe('Accuracy description'),
}).optional().describe('Geographic coordinates (only if geoData option enabled)');
...
export let verifyInternationalAddress = SlateTool.create(
  ...
  .output(z.object({
    formattedAddress: z.string().optional().describe('Full formatted address as a single string'),
    ...
    errors: z.record(z.string(), z.array(z.string())).optional().describe('Errors or corrections applied'),
  }))
