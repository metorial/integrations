...
export let listProposals = SlateTool.create(
  ...
  .input(z.object({
    status: z.enum(['draft', 'pending', 'accepted', 'rejected', 'clarification']).optional().describe('Filter proposals by status'),
    archived: z.boolean().optional().describe('Filter by archived state (true = archived only, false = non-archived only)'),
