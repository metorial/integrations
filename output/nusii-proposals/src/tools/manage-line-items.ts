...
export let listLineItems = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    sectionId: z.string().optional().describe('Filter by section ID (returns all items in that section without pagination)'),
    page: z.number().optional().describe('Page number when listing all items (defaults to 1)'),
    perPage: z.number().optional().describe('Items per page when listing all items (defaults to 25)')
  }))
  .output(z.object({
...
export let createLineItem = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    costType: z.enum(['fixed', 'recurring', 'per']).optional().describe('Pricing type: fixed, recurring, or per-unit'),
    recurringType: z.string().optional().describe('Interval for recurring items (e.g. monthly, yearly, hourly, weekly, quarterly)'),
    perType: z.string().optional().describe('Unit type for per-unit items (e.g. per_unit, per_hour, per_item)'),
...
export let updateLineItem = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    costType: z.enum(['fixed', 'recurring', 'per']).optional().describe('Updated pricing type'),
...
export let deleteLineItem = SlateTool.create(
  spec,
  {
