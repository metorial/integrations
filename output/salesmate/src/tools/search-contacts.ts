...
let searchRuleSchema = z.object({
  fieldName: z.string().describe('Field name to filter on (e.g., "email", "lastName", "company")'),
  condition: z.string().describe('Filter condition (e.g., "EQUALS", "CONTAINS", "STARTS_WITH", "GREATER_THAN")'),
...
export let searchContacts = SlateTool.create(
  ...
  .input(z.object({
    fields: z.array(z.string()).describe('List of field names to return (e.g., ["firstName", "lastName", "email", "company"])'),
    ...
    pageSize: z.number().optional().describe('Number of records per page (max 100). Defaults to 25.'),
  }))
  .output(z.object({
    contacts: z.array(z.record(z.string(), z.unknown())).describe('Array of matching contact records'),
