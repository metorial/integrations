...
let searchRuleSchema = z.object({
  ...
  condition: z.string().describe('Filter condition (e.g., "EQUALS", "CONTAINS", "STARTS_WITH")'),
...
export let searchCompanies = SlateTool.create(
  ...
  .input(z.object({
    fields: z.array(z.string()).describe('List of field names to return (e.g., ["name", "website", "phone"])'),
  ...
  .output(z.object({
    companies: z.array(z.record(z.string(), z.unknown())).describe('Array of matching company records'),
