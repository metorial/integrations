...
let searchRuleSchema = z.object({
  fieldName: z.string().describe('Field name to filter on (e.g., "title", "pipeline", "stage", "status")'),
...
export let searchDeals = SlateTool.create(
  ...
  .input(z.object({
    fields: z.array(z.string()).describe('List of field names to return (e.g., ["title", "pipeline", "stage", "dealValue"])'),
  ...
  .output(z.object({
    deals: z.array(z.record(z.string(), z.unknown())).describe('Array of matching deal records'),
