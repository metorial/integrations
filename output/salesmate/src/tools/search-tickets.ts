...
let searchRuleSchema = z.object({
  fieldName: z.string().describe('Field name to filter on (e.g., "title", "status", "priority", "pipeline")'),
...
export let searchTickets = SlateTool.create(
  ...
  .input(z.object({
    fields: z.array(z.string()).describe('List of field names to return (e.g., ["title", "status", "priority", "stage"])'),
  ...
  .output(z.object({
    tickets: z.array(z.record(z.string(), z.unknown())).describe('Array of matching ticket records'),
