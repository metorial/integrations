...
export let findPeople = SlateTool.create(
  spec,
  {
...
Separate multiple job titles with "OR" to search for several roles at once (e.g. "CEO OR CTO OR CFO").`,
  ...
  .input(z.object({
    jobTitle: z.string().optional().describe('Job title filter. Separate multiple titles with "OR" (e.g. "CEO OR CTO")'),
    domain: z.string().optional().describe('Company domain (e.g. "algolia.com")'),
    linkedinId: z.string().optional().describe('Company LinkedIn ID (e.g. "1441", "2728700")'),
    ...
    fuzzy: z.boolean().optional().default(false).describe('Enable fuzzy matching for broader results'),
  }))
  .output(z.object({
    people: z.array(z.any()).describe('List of matching people with their profile data and match scores'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let companyLabel = ctx.input.companyName || ctx.input.domain || ctx.input.linkedinId || 'the company';
