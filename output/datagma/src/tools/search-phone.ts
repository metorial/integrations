...
export let searchPhone = SlateTool.create(
  spec,
  {
...
Optionally verify if the phone number is linked to a WhatsApp account.`,
  ...
  .input(z.object({
    ...
    profileUrl: z.string().optional().describe('Social media profile URL (LinkedIn, not Sales Navigator)'),
    minimumMatch: z.number().optional().default(1).describe('Minimum match score (0-1). Lower values return more results but with less accuracy'),
    whatsappCheck: z.boolean().optional().default(false).describe('Verify if the phone number is linked to a WhatsApp account'),
  }))
  .output(z.object({
    person: z.any().optional().describe('Person data including names, phones, emails, jobs, addresses'),
    possiblePersons: z.array(z.any()).optional().describe('Alternative person matches if multiple candidates found'),
