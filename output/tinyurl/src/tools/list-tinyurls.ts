...
export let listTinyUrls = SlateTool.create(
  spec,
  {
    name: 'List TinyURLs',
    key: 'list_tinyurls',
...
Use the **search** parameter with format \`alias:value\` or \`tag:value\` to filter by alias or tag name.`,
  ...
  .input(z.object({
    status: z.enum(['available', 'archived']).optional().default('available').describe('Filter by link status'),
    ...
    search: z.string().optional().describe('Search filter in format "alias:value" or "tag:value"'),
  }))
  .output(z.object({
