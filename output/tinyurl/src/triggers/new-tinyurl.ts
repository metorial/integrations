...
export let newTinyUrl = SlateTrigger.create(
  spec,
  {
    name: 'New TinyURL Created',
    key: 'new_tinyurl',
    description: 'Triggers when a new TinyURL is created on your account. Polls for newly created links.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let newLinks = links.filter((link) => !knownAliases.includes(`${link.domain}/${link.alias}`));
