...
export let managePagesTool = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Requires current revn and vern values from the file to avoid conflicts.',
    ],
  }
)
  .input(z.object({
    action: z.enum(['add', 'rename', 'delete', 'move']).describe('The page operation to perform'),
    fileId: z.string().describe('ID of the file'),
    revn: z.number().describe('Current file revision number (get from Get File tool)'),
    vern: z.number().default(0).describe('Current file version number (get from Get File tool)'),
    pageId: z.string().optional().describe('ID of the page (required for "rename", "delete", "move")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (action) {
      ...
      case 'rename': {
        if (!pageId || !name) throw new Error('pageId and name are required for rename action');
      ...
      case 'move': {
        if (!pageId || index === undefined) throw new Error('pageId and index are required for move action');
