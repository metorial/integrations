...
export let manageProjectTool = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'rename', 'delete', 'pin', 'unpin']).describe('The operation to perform'),
    teamId: z.string().optional().describe('ID of the team (required for "create", "pin", "unpin")'),
    projectId: z.string().optional().describe('ID of the project (required for "rename", "delete", "pin", "unpin")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (action) {
      case 'create': {
        if (!teamId || !name) throw new Error('teamId and name are required for create action');
      ...
      case 'rename': {
        if (!projectId || !name) throw new Error('projectId and name are required for rename action');
      ...
      case 'pin': {
        if (!teamId || !projectId) throw new Error('teamId and projectId are required for pin action');
      ...
      case 'unpin': {
        if (!teamId || !projectId) throw new Error('teamId and projectId are required for unpin action');
