...
let commentSchema = z.object({
  ...
  canDelete: z.boolean().optional().describe('Whether the current user can delete this comment'),
});
...
export let addCommentTool = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.createComment(ctx.input.filePath, ctx.input.message) as Record<string, unknown>;
...
export let listCommentsTool = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {
        comments,
        totalResults: typeof result.total_results === 'number' ? result.total_results : undefined,
      },
