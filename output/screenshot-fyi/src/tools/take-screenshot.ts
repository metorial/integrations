...
export let takeScreenshot = SlateTool.create(
  ...
  .input(z.object({
    url: z.string().describe('The full URL of the website to capture (e.g., https://example.com).'),
    fullPage: z.boolean().optional().describe('Capture the entire page from header to footer. When false, only the visible viewport is captured.'),
    width: z.number().optional().describe('Viewport width in pixels (e.g., 1280 for desktop, 375 for mobile).'),
    height: z.number().optional().describe('Viewport height in pixels (e.g., 800 for desktop, 812 for mobile).'),
    darkMode: z.boolean().optional().describe('Capture the website in dark mode. Defaults to false.'),
    disableCookieBanners: z.boolean().optional().describe('Automatically remove cookie banners, popups, and chat widgets. Defaults to true.'),
    format: z.enum(['png', 'jpg', 'jpeg']).optional().describe('Output image format. Defaults to jpg.'),
  }))
  .output(z.object({
