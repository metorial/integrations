...
let attributeSchema = userIdentifierSchema.extend({
  ...
  pushSubscribe: z.enum(['opted_in', 'subscribed', 'unsubscribed']).optional().describe('Push notification subscription status'),
  emailSubscribe: z.enum(['opted_in', 'subscribed', 'unsubscribed']).optional().describe('Email subscription status'),
  customAttributes: z.record(z.string(), z.any()).optional().describe('Key-value pairs of custom attributes to set on the user profile')
});
...
let eventSchema = userIdentifierSchema.extend({
  ...
  properties: z.record(z.string(), z.any()).optional().describe('Custom properties for the event')
});
...
let purchaseSchema = userIdentifierSchema.extend({
  ...
  properties: z.record(z.string(), z.any()).optional().describe('Custom properties for the purchase')
});

export let trackUsers = SlateTool.create(
  spec,
  {
    name: 'Track Users',
    key: 'track_users',
  ...
  .input(z.object({
    attributes: z.array(attributeSchema).optional().describe('User profile attributes to set or update'),
  ...
  .output(z.object({
    ...
    errors: z.array(z.any()).optional().describe('Non-fatal errors encountered during processing')
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.attributes) {
      body.attributes = ctx.input.attributes.map(attr => {
        ...
        if (attr.userAlias) mapped.user_alias = { alias_name: attr.userAlias.aliasName, alias_label: attr.userAlias.aliasLabel };
    ...
    if (ctx.input.events) {
      body.events = ctx.input.events.map(evt => {
        ...
        if (evt.userAlias) mapped.user_alias = { alias_name: evt.userAlias.aliasName, alias_label: evt.userAlias.aliasLabel };
    ...
    if (ctx.input.purchases) {
      body.purchases = ctx.input.purchases.map(p => {
        ...
        if (p.userAlias) mapped.user_alias = { alias_name: p.userAlias.aliasName, alias_label: p.userAlias.aliasLabel };
    ...
    let counts = [];
    if (ctx.input.attributes?.length) counts.push(`${result.attributes_processed ?? ctx.input.attributes.length} attributes`);
    if (ctx.input.events?.length) counts.push(`${result.events_processed ?? ctx.input.events.length} events`);
    if (ctx.input.purchases?.length) counts.push(`${result.purchases_processed ?? ctx.input.purchases.length} purchases`);
