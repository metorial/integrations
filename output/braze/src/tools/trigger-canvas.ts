...
export let triggerCanvas = SlateTool.create(
  spec,
  {
    name: 'Trigger Canvas',
    key: 'trigger_canvas',
  ...
  .input(z.object({
    ...
    recipients: z.array(z.object({
      ...
      canvasEntryProperties: z.record(z.string(), z.any()).optional().describe('Personalization properties for this recipient at Canvas entry'),
      sendToExistingOnly: z.boolean().optional().describe('If true, only send to existing users (defaults to true)')
    })).optional().describe('List of recipients to enter into the Canvas'),
    broadcast: z.boolean().optional().describe('Set to true to send to all users in the Canvas audience'),
    canvasEntryProperties: z.record(z.string(), z.any()).optional().describe('Global Canvas entry properties applied to all recipients')
  }))
  .output(z.object({
