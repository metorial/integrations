...
export let manageFavorites = SlateTool.create(spec, {
  ...
  .input(z.object({
    ...
    resourceType: z.enum(['workbook', 'view', 'datasource', 'project', 'flow']).optional().describe('Type of resource (for add/remove)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'add') {
      await client.addFavorite(userId, ctx.input.resourceType!, ctx.input.resourceId!, ctx.input.label || ctx.input.resourceType!);
      return {
        output: { added: true },
