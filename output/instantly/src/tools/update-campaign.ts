...
export let updateCampaign = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['activate', 'pause']).optional().describe('Launch or pause the campaign.'),
    ...
    stopOnAutoReply: z.boolean().optional().describe('Stop sending when an auto-reply is received.'),
    ...
    campaignSchedule: z.any().optional().describe('Full campaign schedule object to replace the existing schedule.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (fields.stopOnAutoReply !== undefined) updatePayload.stop_on_auto_reply = fields.stopOnAutoReply;
    ...
    if (fields.campaignSchedule !== undefined) updatePayload.campaign_schedule = fields.campaignSchedule;
