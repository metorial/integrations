...
export let manageLeadLabels = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    labelId: z.string().optional().describe('ID of the label (for "update" and "delete" actions).'),
    name: z.string().optional().describe('Label name (for "create" and "update" actions).'),
    color: z.string().optional().describe('Label color hex code (for "create" and "update" actions).'),
    limit: z.number().min(1).max(100).optional().describe('Number of labels to return (for "list" action).'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of labels (for "list" action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create' && ctx.input.name) {
      let result = await client.createLeadLabel({ name: ctx.input.name, color: ctx.input.color });
      return {
