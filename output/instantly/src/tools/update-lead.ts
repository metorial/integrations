...
export let updateLead = SlateTool.create(
  ...
  .input(z.object({
    leadId: z.string().optional().describe('ID of the lead to update (for contact detail updates).'),
    ...
    customVariables: z.record(z.string(), z.any()).optional().describe('Custom variables to set or update.'),
    leadEmail: z.string().optional().describe('Lead email address (for interest status update).'),
    interestValue: z.number().nullable().optional().describe('New interest status value, or null to reset.'),
    campaignId: z.string().optional().describe('Campaign ID to scope the interest status update.'),
    listId: z.string().optional().describe('Lead list ID to scope the interest status update.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.leadId) {
      ...
      if (ctx.input.companyName !== undefined) updatePayload.company_name = ctx.input.companyName;
      ...
      if (ctx.input.personalization !== undefined) updatePayload.personalization = ctx.input.personalization;
      if (ctx.input.customVariables !== undefined) updatePayload.custom_variables = ctx.input.customVariables;
