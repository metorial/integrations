...
export let manageCustomTags = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'delete', 'assign', 'unassign']).describe('Action to perform.'),
    tagName: z.string().optional().describe('Name of the tag to create (for "create" action).'),
    tagId: z.string().optional().describe('ID of the tag (for "delete", "assign", "unassign" actions).'),
    resourceIds: z.array(z.string()).optional().describe('IDs of campaigns or accounts to assign/unassign the tag (for "assign"/"unassign" actions).'),
    search: z.string().optional().describe('Search tags by name (for "list" action).'),
    limit: z.number().min(1).max(100).optional().describe('Number of tags to return (for "list" action).'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if ((action === 'assign' || action === 'unassign') && ctx.input.tagId && ctx.input.resourceIds) {
