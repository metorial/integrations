...
export let campaignEvents = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers on campaign and email activity events including email sent, opened, clicked, replied, bounced, auto-reply received, campaign completed, and account errors.',
  }
)
  .input(z.object({
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      let emailEventTypes = [
        ...
        'campaign_completed', 'account_error',
      ];
      ...
      let eventId = data.id ?? data.event_id ?? `${eventType}_${data.lead_email ?? ''}_${data.timestamp ?? Date.now()}`;
