...
export let checkTrust = SlateTool.create(
  spec,
  {
...
Optionally perform a live lookup and check for custom stopwords in the website's HTML.`,
  ...
  .input(z.object({
    ...
    live: z.boolean().optional().describe('Perform a live lookup for the most current assessment'),
    stopwords: z.string().optional().describe('Comma-separated stopwords to check in website HTML'),
  }))
  .output(z.object({
