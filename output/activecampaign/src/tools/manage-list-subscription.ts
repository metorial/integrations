...
export let manageListSubscription = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['subscribe', 'unsubscribe']).describe('Whether to subscribe or unsubscribe the contact')
  }))
  .output(z.object({
    success: z.boolean(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.updateContactListStatus(ctx.input.contactId, ctx.input.listId, statusCode);
