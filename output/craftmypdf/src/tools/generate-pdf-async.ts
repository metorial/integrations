...
export let generatePdfAsync = SlateTool.create(
  spec,
  {
...
Use this for large or batch PDF generation tasks where you don't need the result immediately.`,
  ...
  .input(
    z.object({
      ...
      webhookUrl: z.string().describe('URL to receive the webhook callback when the PDF is ready.'),
      ...
      expiration: z.number().optional().describe('Expiration time in minutes for the generated PDF URL (1-10080).'),
  ...
  .output(
    z.object({
      transactionRef: z.string().describe('Transaction reference to track the async generation.'),
