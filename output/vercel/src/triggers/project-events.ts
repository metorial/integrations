...
export let projectEventsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Project Events',
    key: 'project_events',
    description: 'Fires when project lifecycle events occur: created, removed, renamed, domain changes, and environment variable changes.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    envVarKey: z.string().optional().nullable().describe('Environment variable key (for env var events)'),
    envVarTarget: z.array(z.string()).optional().nullable().describe('Environment variable targets (for env var events)'),
  }))
  .webhook({
