...
export let manageDomainsTool = SlateTool.create(
  spec,
  {
    name: 'Manage Domains',
    key: 'manage_domains',
  ...
  .input(z.object({
    action: z.enum(['list', 'list_project', 'add', 'remove', 'add_to_project', 'remove_from_project', 'verify']).describe('Action to perform'),
    domain: z.string().optional().describe('Domain name (required for add, remove, add_to_project, remove_from_project, verify)'),
    projectIdOrName: z.string().optional().describe('Project ID or name (required for project-level actions)'),
    redirect: z.string().optional().describe('Redirect target domain (for add_to_project)'),
    redirectStatusCode: z.number().optional().describe('Redirect status code 301 or 302 (for add_to_project)'),
    gitBranch: z.string().optional().describe('Git branch to associate with domain (for add_to_project)'),
  }))
  .output(z.object({
    ...
    })).optional().describe('List of domains (for list actions)'),
    domain: z.object({
      name: z.string().describe('Domain name'),
      verified: z.boolean().optional().describe('Whether domain is verified'),
    }).optional().describe('Domain details (for single-domain actions)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'add_to_project') {
      if (!domain || !projectIdOrName) throw new Error('domain and projectIdOrName are required');
