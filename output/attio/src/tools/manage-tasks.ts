...
let taskOutputSchema = z.object({
  ...
  linkedRecords: z.array(z.object({
...
export let listTasksTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    assignee: z.string().optional().describe('Filter by assignee (email, workspace member ID, or null for unassigned)'),
...
export let createTaskTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    isCompleted: z.boolean().optional().default(false).describe('Whether the task starts as completed'),
    linkedRecords: z.array(linkedRecordSchema).optional().describe('Records to link to this task'),
    assignees: z.array(assigneeSchema).optional().describe('Workspace members to assign to this task'),
  }))
...
export let updateTaskTool = SlateTool.create(
  spec,
  {
    name: 'Update Task',
    key: 'update_task',
  ...
  .input(z.object({
    ...
    deadlineAt: z.string().optional().nullable().describe('New deadline (ISO 8601 or YYYY-MM-DD), or null to remove'),
...
export let deleteTaskTool = SlateTool.create(
  spec,
  {
    name: 'Delete Task',
    key: 'delete_task',
