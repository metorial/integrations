...
let recipientSchema = z.object({
  ...
  company: z.string().optional().describe('Recipient company name, appears below their name on the envelope'),
...
let lineItemSchema = z.object({
  ...
  variables: z.record(z.string(), z.string()).optional().describe('Template variable substitutions, e.g. { "firstName": "John" }'),
  shipToMe: z.boolean().optional().describe('Send the card to the sender with a blank envelope instead of mailing to recipient'),
  shippingMethod: z.string().optional().describe('Shipping method: standard, tracked, or express (availability varies by region)'),
});

export let placeOrder = SlateTool.create(
  spec,
  {
    name: 'Place Order',
    key: 'place_order',
...
Supports variable substitution in templates, custom sender/return addresses, shipping method selection, and idempotent requests to prevent duplicate orders.`,
  ...
  .input(z.object({
    lines: z.array(lineItemSchema).min(1).describe('One or more card line items to include in this order'),
    requestedArrivalDate: z.string().optional().describe('Preferred arrival date in ISO 8601 format'),
    purchaseOrderNumber: z.string().optional().describe('Purchase order number for auditing purposes'),
    idempotencyKey: z.string().optional().describe('Unique key (e.g. UUID v4) to ensure idempotent order placement'),
  }))
  .output(z.object({
    ...
    lines: z.array(z.record(z.string(), z.unknown())).describe('Order line items with details'),
    cost: z.record(z.string(), z.unknown()).optional().describe('Cost breakdown for the order'),
