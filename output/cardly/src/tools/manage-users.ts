...
export let sendInvitation = SlateTool.create(
  ...
  .input(z.object({
    ...
    permissions: z.array(z.enum(permissionOptions)).min(1).describe('Permissions to grant the invited user'),
  }))
  .output(z.object({
...
export let manageInvitation = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    email: z.string().optional().describe('Email address of the invitation (alternative to invitationId)'),
  }))
  .output(z.object({
