...
export let generatePreview = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    }).optional().describe('Sender/return address details'),
    variables: z.record(z.string(), z.string()).optional().describe('Template variable substitutions'),
  }))
  .output(z.object({
    preview: z.record(z.string(), z.unknown()).describe('Preview data including PDF URL and cost estimate'),
  }))
