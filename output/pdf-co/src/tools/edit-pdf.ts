...
export let editPdf = SlateTool.create(
  spec,
  {
    name: 'Edit PDF',
    key: 'edit_pdf',
...
Combines multiple editing capabilities in a single tool — specify which operations to perform using the appropriate parameters.`,
  ...
  .input(z.object({
    ...
    })).optional().describe('Text annotations to add to the PDF'),
    ...
    })).optional().describe('Images to insert into the PDF'),
    formFields: z.array(z.object({
      fieldName: z.string().describe('Name of the form field to fill'),
      text: z.string().describe('Value to set for the form field'),
    })).optional().describe('Form fields to populate'),
    searchStrings: z.array(z.string()).optional().describe('Text strings to search for (used with replaceStrings or deleteMatchedText)'),
    replaceStrings: z.array(z.string()).optional().describe('Replacement strings corresponding to each search string'),
    deleteMatchedText: z.boolean().optional().describe('Delete text matching searchStrings instead of replacing'),
    ...
    pagesToDelete: z.string().optional().describe('Pages to remove from the PDF, e.g. "0,3,5"'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!result) {
      throw new Error('No editing operations specified. Please provide at least one edit action.');
    }
