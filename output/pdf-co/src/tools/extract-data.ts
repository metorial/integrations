...
export let parseInvoice = SlateTool.create(
  spec,
  {
    name: 'Parse Invoice',
    key: 'parse_invoice',
...
Extracts vendor info, customer info, invoice details, payment information, line items, and more.`,
  ...
  .output(z.object({
    parsedData: z.any().describe('Structured invoice data including vendor, customer, invoice details, payment info, and line items'),
...
Use template ID "1" for the built-in general invoice template, or specify a custom template ID for your own extraction rules.`,
  ...
  .input(z.object({
    ...
    templateId: z.string().describe('Document parser template ID. Use "1" for the built-in general invoice template.'),
...
export let classifyDocument = SlateTool.create(
  spec,
  {
...
Provide custom classification rules in CSV format, or use a URL to an external CSV rules file.`,
  ...
  .input(z.object({
    ...
    rulesCsv: z.string().optional().describe('Classification rules in CSV format, e.g. "VendorA,OR,keyword1,keyword2\\nVendorB,AND,keyword3,keyword4"'),
    rulesCsvUrl: z.string().optional().describe('URL to an external CSV file containing classification rules'),
    caseSensitive: z.boolean().optional().describe('Case-sensitive keyword matching (default: true)'),
    password: z.string().optional().describe('Password for protected PDF files'),
  }))
  .output(z.object({
