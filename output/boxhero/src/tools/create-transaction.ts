...
Optionally attach a partner (supplier for Stock In, customer for Stock Out) and a memo.`,
  ...
  .input(z.object({
    ...
    items: z.array(z.object({
      ...
      quantity: z.number().describe('Quantity to transact (positive for in/adjust-up, negative for adjust-down)'),
    })).min(1).describe('Items and quantities for this transaction'),
    toLocationId: z.number().optional().describe('Destination location ID (required for "in" and "move" in Business mode)'),
    fromLocationId: z.number().optional().describe('Source location ID (required for "out" and "move" in Business mode)'),
    partnerId: z.number().optional().describe('Partner ID — supplier for Stock In, customer for Stock Out'),
    memo: z.string().optional().describe('Optional notes or memo for the transaction'),
    transactionTime: z.string().optional().describe('Transaction timestamp in ISO 8601 format (defaults to current time)'),
  }))
  .output(z.object({
