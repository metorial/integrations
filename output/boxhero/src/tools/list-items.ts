...
let itemSchema = z.object({
  ...
  quantity: z.number().optional().describe('Current stock quantity (available when filtering by location)'),
...
export let listItems = SlateTool.create(
  ...
  .input(z.object({
    locationIds: z.array(z.number()).optional().describe('Filter items by one or more location IDs'),
    limit: z.number().optional().describe('Maximum number of items to return per page (default 100)'),
