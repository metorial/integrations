...
export let manageNumbers = SlateTool.create(
  spec,
  {
    name: 'Manage Numbers',
    key: 'manage_numbers',
...
Combines number search, purchase, and management in one tool.`,
  ...
  .input(z.object({
    action: z.enum(['search', 'list', 'buy', 'cancel', 'update']).describe('Action to perform'),
    country: z.string().optional().describe('Two-letter country code (required for search, buy, cancel, update)'),
    msisdn: z.string().optional().describe('Phone number in E.164 format without + (required for buy, cancel, update)'),
    type: z.enum(['landline', 'mobile-lvn', 'landline-toll-free']).optional().describe('Number type filter for search'),
    pattern: z.string().optional().describe('Pattern to match when searching (e.g., "1234")'),
    features: z.string().optional().describe('Comma-separated features filter: SMS, VOICE, MMS'),
    ...
    applicationId: z.string().optional().describe('Link number to this application ID (for update action)'),
    moHttpUrl: z.string().optional().describe('Inbound SMS webhook URL (for update action)'),
    voiceCallbackType: z.string().optional().describe('Voice callback type: "app" or "sip" or "tel" (for update action)'),
    voiceCallbackValue: z.string().optional().describe('Voice callback value (for update action)'),
    voiceStatusCallback: z.string().optional().describe('Voice status webhook URL (for update action)'),
  }))
  .output(z.object({
    ...
    })).optional().describe('List of numbers'),
    success: z.boolean().optional().describe('Whether the action succeeded (for buy/cancel/update)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'buy': {
        if (!ctx.input.country || !ctx.input.msisdn) throw new Error('country and msisdn are required for buy');
      ...
      case 'cancel': {
        if (!ctx.input.country || !ctx.input.msisdn) throw new Error('country and msisdn are required for cancel');
      ...
      case 'update': {
        if (!ctx.input.country || !ctx.input.msisdn) throw new Error('country and msisdn are required for update');
