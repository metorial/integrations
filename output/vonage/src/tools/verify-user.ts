...
export let verifyUser = SlateTool.create(
  spec,
  {
    name: 'Verify User',
    key: 'verify_user',
...
Requires the **API Key, Secret & Application JWT** auth method.`,
  ...
  .input(z.object({
    brand: z.string().describe('Brand name shown in the verification message (e.g., "Acme Corp")'),
    to: z.string().describe('Phone number or email to verify in E.164 format for phone, or email address'),
    workflows: z.array(z.object({
      channel: z.enum(['sms', 'whatsapp', 'whatsapp_interactive', 'voice', 'email', 'silent_auth']).describe('Verification channel'),
    ...
    })).min(1).describe('Ordered list of verification channels with automatic failover'),
    codeLength: z.number().optional().describe('Length of the verification code (4-10, default: 4)'),
    channelTimeout: z.number().optional().describe('Seconds to wait before trying the next channel (default: 300)'),
    locale: z.string().optional().describe('Locale for the verification message (e.g., "en-us", "fr-fr")'),
  }))
  .output(z.object({
    requestId: z.string().describe('Verification request ID (needed to check or cancel the verification)'),
