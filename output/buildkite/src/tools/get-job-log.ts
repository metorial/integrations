...
export let getJobLog = SlateTool.create(
  spec,
  {
    name: 'Get Job Log',
    key: 'get_job_log',
  ...
  .input(z.object({
    ...
    includeEnvironment: z.boolean().optional().describe('Also fetch the job environment variables'),
  }))
  .output(z.object({
    ...
    environment: z.record(z.string(), z.string()).optional().describe('Environment variables for the job, if requested'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let log = await client.getJobLog(ctx.input.pipelineSlug, ctx.input.buildNumber, ctx.input.jobId);

    let environment: Record<string, string> | undefined;
    if (ctx.input.includeEnvironment) {
      let envData = await client.getJobEnvironment(ctx.input.pipelineSlug, ctx.input.buildNumber, ctx.input.jobId);
