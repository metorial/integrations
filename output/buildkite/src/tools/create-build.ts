...
export let createBuild = SlateTool.create(
  spec,
  {
    name: 'Create Build',
    key: 'create_build',
  ...
  .input(z.object({
    ...
    commit: z.string().describe('Commit SHA to build (use "HEAD" for the latest commit on the branch)'),
    branch: z.string().describe('Branch to build'),
    message: z.string().optional().describe('Build message (defaults to the commit message)'),
    env: z.record(z.string(), z.string()).optional().describe('Environment variables to set for the build'),
    metaData: z.record(z.string(), z.string()).optional().describe('Metadata key-value pairs to attach to the build'),
    ignorePipelineBranchFilters: z.boolean().optional().describe('Ignore pipeline branch filter settings'),
