...
export let createPipeline = SlateTool.create(
  ...
  .input(z.object({
    ...
    repository: z.string().describe('URL of the source code repository (e.g. git@github.com:org/repo.git)'),
    configuration: z.string().optional().describe('Pipeline step configuration in YAML format'),
    ...
    branchConfiguration: z.string().optional().describe('Branch filter pattern (e.g. "main develop release/*")'),
    ...
    teamUuids: z.array(z.string()).optional().describe('UUIDs of teams to assign the pipeline to'),
    clusterUuid: z.string().optional().describe('UUID of the cluster to assign the pipeline to'),
