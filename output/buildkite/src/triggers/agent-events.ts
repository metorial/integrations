...
export let agentEvents = SlateTrigger.create(
  spec,
  {
    name: 'Agent Events',
    key: 'agent_events',
    description: 'Triggers when an agent lifecycle event occurs (connected, lost, disconnected, stopping, stopped, blocked). Useful for automating infrastructure scaling and monitoring agent health. Configure a webhook in your Buildkite organization\'s Notification Services settings.',
  }
)
  .input(z.object({
    eventType: z.string().describe('Type of agent event (agent.connected, agent.lost, agent.disconnected, agent.stopping, agent.stopped, agent.blocked)'),
