...
export let manageAppSettings = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Updating app settings triggers an app restart.',
    ],
  }
)
  .input(z.object({
    ...
    settings: z.record(z.string(), z.string().nullable()).optional().describe('Settings to add/update. Set value to null to remove a setting. Required for update action.'),
  }))
  .output(z.object({
    settings: z.record(z.string(), z.string()).describe('Current application settings after the operation'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let addedOrUpdated = Object.keys(inputSettings).filter((k) => inputSettings[k] !== null && inputSettings[k] !== undefined);
