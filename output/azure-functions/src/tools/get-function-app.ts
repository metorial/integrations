...
export let getFunctionApp = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    includeAppSettings: z.boolean().default(false).describe('Whether to include application settings (environment variables) in the response'),
  }))
  .output(z.object({
    appName: z.string().describe('Name of the function app'),
    ...
    appSettings: z.record(z.string(), z.string()).optional().describe('Application settings (if requested)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let isLinux = app.kind?.toLowerCase().includes('linux') || config.properties?.linuxFxVersion;
    ...
    let output = {
      ...
      runtimeStack: config.properties?.linuxFxVersion || config.properties?.javaVersion || config.properties?.netFrameworkVersion || undefined,
      functionsVersion: config.properties?.functionsRuntimeScaleMonitoringEnabled !== undefined ? undefined : undefined,
      tags: app.tags,
      appSettings,
