...
export let manageProject = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'updateYaml', 'updateEnvVars', 'updateBuildNumber', 'clearCache', 'delete']).describe('Operation to perform'),
    accountName: z.string().optional().describe('Account name (required for most operations)'),
    projectSlug: z.string().optional().describe('Project slug (required for most operations)'),
    repositoryProvider: z.enum(['gitHub', 'bitBucket', 'vso', 'gitLab', 'kiln', 'stash', 'git', 'mercurial', 'subversion']).optional().describe('Repository provider for creating a project'),
    repositoryName: z.string().optional().describe('Repository name for creating a project (e.g. owner/repo)'),
    projectSettings: z.record(z.string(), z.unknown()).optional().describe('Full project settings object for update'),
    ...
    })).optional().describe('Environment variables for updateEnvVars'),
    nextBuildNumber: z.number().optional().describe('Next build number for updateBuildNumber'),
  }))
  .output(z.object({
    project: z.record(z.string(), z.unknown()).optional().describe('Created or updated project details'),
    ...
    switch (action) {
      case 'create': {
        if (!ctx.input.repositoryProvider || !ctx.input.repositoryName) {
          throw new Error('repositoryProvider and repositoryName are required for creating a project');
        }
      ...
      case 'updateYaml': {
        if (!accountName || !projectSlug || !ctx.input.yamlConfig) {
          throw new Error('accountName, projectSlug, and yamlConfig are required for updateYaml');
        }
      ...
      case 'updateEnvVars': {
        if (!accountName || !projectSlug || !ctx.input.environmentVariables) {
          throw new Error('accountName, projectSlug, and environmentVariables are required for updateEnvVars');
        }
        await client.updateProjectEnvironmentVariables(accountName, projectSlug, ctx.input.environmentVariables);
        return {
          output: { success: true },
      ...
      case 'updateBuildNumber': {
        if (!accountName || !projectSlug || ctx.input.nextBuildNumber === undefined) {
          throw new Error('accountName, projectSlug, and nextBuildNumber are required for updateBuildNumber');
        }
        await client.updateProjectBuildNumber(accountName, projectSlug, ctx.input.nextBuildNumber);
        return {
          output: { success: true },
