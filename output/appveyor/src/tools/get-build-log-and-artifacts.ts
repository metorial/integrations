...
let artifactSchema = z.object({
  ...
  type: z.string().optional().describe('Artifact type (Auto, File, Zip, NuGetPackage, WebDeployPackage, etc.)'),
  size: z.number().optional().describe('Artifact size in bytes'),
  created: z.string().optional().describe('Artifact creation timestamp'),
});

export let getBuildLogAndArtifacts = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['log', 'listArtifacts', 'downloadArtifact']).describe('Operation to perform'),
    jobId: z.string().describe('Build job ID'),
    artifactFileName: z.string().optional().describe('Artifact file name (required for downloadArtifact)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'downloadArtifact': {
        ...
        let content = await client.downloadBuildJobArtifact(ctx.input.jobId, ctx.input.artifactFileName);
        return {
