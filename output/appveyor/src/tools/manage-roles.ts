...
export let manageRoles = SlateTool.create(
  ...
  .input(z.object({
    ...
    roleSettings: z.record(z.string(), z.unknown()).optional().describe('Full role settings for update, including permission groups'),
  }))
  .output(z.object({
    roles: z.array(z.record(z.string(), z.unknown())).optional().describe('List of roles'),
    role: z.record(z.string(), z.unknown()).optional().describe('Role details with permissions'),
