...
export let manageBuild = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['rerun', 'cancel', 'delete']).describe('Operation to perform on the build'),
    buildId: z.number().optional().describe('Build ID (required for rerun and delete)'),
    reRunIncomplete: z.boolean().optional().describe('Only re-run incomplete jobs when re-running'),
    ...
    buildVersion: z.string().optional().describe('Build version string (required for cancel)'),
  }))
  .output(z.object({
    build: z.record(z.string(), z.unknown()).optional().describe('Build details (for rerun)'),
    ...
    switch (ctx.input.action) {
      ...
      case 'cancel': {
        if (!ctx.input.accountName || !ctx.input.projectSlug || !ctx.input.buildVersion) {
          throw new Error('accountName, projectSlug, and buildVersion are required for cancel');
        }
        await client.cancelBuild(ctx.input.accountName, ctx.input.projectSlug, ctx.input.buildVersion);
        return {
          output: { success: true },
