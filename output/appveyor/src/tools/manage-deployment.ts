...
export let manageDeployment = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['start', 'get', 'cancel', 'projectHistory', 'environmentHistory']).describe('Operation to perform'),
    deploymentId: z.number().optional().describe('Deployment ID (required for get, cancel)'),
    environmentName: z.string().optional().describe('Target environment name (required for start)'),
    accountName: z.string().optional().describe('Account name (required for start, projectHistory)'),
    projectSlug: z.string().optional().describe('Project slug (required for start, projectHistory)'),
    buildVersion: z.string().optional().describe('Build version to deploy (required for start)'),
    buildJobId: z.string().optional().describe('Specific build job ID to deploy'),
    environmentVariables: z.record(z.string(), z.string()).optional().describe('Custom environment variables for the deployment'),
    environmentId: z.number().optional().describe('Environment ID (required for environmentHistory)'),
  }))
  .output(z.object({
    deployment: z.record(z.string(), z.unknown()).optional().describe('Deployment details'),
    deployments: z.array(z.record(z.string(), z.unknown())).optional().describe('Deployment history'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'start': {
        if (!ctx.input.environmentName || !ctx.input.accountName || !ctx.input.projectSlug || !ctx.input.buildVersion) {
          throw new Error('environmentName, accountName, projectSlug, and buildVersion are required for start');
        }
      ...
      case 'projectHistory': {
        ...
        let result = await client.getProjectDeployments(ctx.input.accountName, ctx.input.projectSlug);
