...
export let executePartiql = SlateTool.create(
  spec,
  {
...
Useful for users familiar with SQL syntax as an alternative to DynamoDB's native expression-based API.`,
  ...
  .input(z.object({
    ...
    parameters: z.array(attributeValueSchema).optional().describe('Positional parameters for the statement using ? placeholders'),
    consistentRead: z.boolean().optional().default(false).describe('Use strongly consistent read for SELECT statements'),
    nextToken: z.string().optional().describe('Pagination token from a previous execution'),
    limit: z.number().optional().describe('Maximum number of items to return'),
  }))
  .output(z.object({
    items: z.array(z.record(z.string(), z.any())).optional().describe('Result items for SELECT statements'),
    nextToken: z.string().optional().describe('Pagination token if more results are available'),
  }))
