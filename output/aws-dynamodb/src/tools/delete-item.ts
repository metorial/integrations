...
export let deleteItem = SlateTool.create(
  ...
  .input(z.object({
    ...
    key: z.record(z.string(), attributeValueSchema).describe('Primary key of the item to delete in DynamoDB JSON format'),
    conditionExpression: z.string().optional().describe('Condition that must be met for the delete to succeed'),
    ...
    returnDeletedItem: z.boolean().optional().default(false).describe('Return the deleted item'),
  }))
  .output(z.object({
    success: z.boolean().describe('Whether the operation succeeded'),
    deletedItem: z.record(z.string(), z.any()).optional().describe('The deleted item if returnDeletedItem was true'),
  }))
