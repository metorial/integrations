...
Supports strongly consistent reads.`,
  ...
  .input(z.object({
    ...
    projectionExpression: z.string().optional().describe('Comma-separated list of attributes to retrieve'),
    expressionAttributeNames: z.record(z.string(), z.string()).optional().describe('Substitution tokens for reserved words in projection expression'),
  }))
  .output(z.object({
    found: z.boolean().describe('Whether the item was found'),
    item: z.record(z.string(), z.any()).optional().describe('The retrieved item in DynamoDB JSON format'),
  }))
