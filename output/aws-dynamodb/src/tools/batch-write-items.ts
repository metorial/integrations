...
export let batchWriteItems = SlateTool.create(
  spec,
  {
...
Supports up to 25 put/delete operations per batch. Does not support update operations — use individual UpdateItem for that.`,
  ...
  .input(z.object({
    operations: z.record(z.string(), z.array(z.object({
      putItem: z.record(z.string(), attributeValueSchema).optional().describe('Item to put in DynamoDB JSON format'),
      deleteKey: z.record(z.string(), attributeValueSchema).optional().describe('Primary key of item to delete in DynamoDB JSON format'),
    }))).describe('Map of table name to array of put/delete operations'),
  }))
  .output(z.object({
    unprocessedItems: z.record(z.string(), z.any()).optional().describe('Items that were not processed (retry these)'),
    hasUnprocessed: z.boolean().describe('Whether there are unprocessed items requiring retry'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let hasUnprocessed = result.UnprocessedItems && Object.keys(result.UnprocessedItems).length > 0;

    let totalOps = Object.values(ctx.input.operations).reduce((sum, ops) => sum + ops.length, 0);
