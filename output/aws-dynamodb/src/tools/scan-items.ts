...
export let scanItems = SlateTool.create(
  spec,
  {
    name: 'Scan Items',
    key: 'scan_items',
...
More flexible but less efficient than Query — reads every item in the table. Use Query when possible for better performance.`,
  ...
  .input(z.object({
    ...
    filterExpression: z.string().optional().describe('Filter expression to apply after scanning'),
    ...
    exclusiveStartKey: z.record(z.string(), attributeValueSchema).optional().describe('Pagination token from a previous scan'),
  }))
  .output(z.object({
