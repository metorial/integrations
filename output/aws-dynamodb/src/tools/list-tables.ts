...
export let listTables = SlateTool.create(
  ...
  .input(z.object({
    ...
    startAfterTable: z.string().optional().describe('Table name to start listing after (for pagination)'),
  }))
  .output(z.object({
    tableNames: z.array(z.string()).describe('List of table names'),
    lastEvaluatedTableName: z.string().optional().describe('Last table name evaluated, use as startAfterTable for next page'),
  }))
