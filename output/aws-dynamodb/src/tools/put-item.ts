...
}).describe('DynamoDB attribute value in JSON format. Use {S: "value"} for strings, {N: "123"} for numbers, {BOOL: true} for booleans, etc.'));

export let putItem = SlateTool.create(
  spec,
  {
    name: 'Put Item',
    key: 'put_item',
...
Supports conditional writes via condition expressions.`,
  ...
  .input(z.object({
    ...
    item: z.record(z.string(), attributeValueSchema).describe('Item to put in DynamoDB JSON format'),
    conditionExpression: z.string().optional().describe('Condition that must be met for the put to succeed'),
    ...
    expressionAttributeValues: z.record(z.string(), attributeValueSchema).optional().describe('Substitution tokens for attribute values in expressions'),
    returnOldItem: z.boolean().optional().default(false).describe('Return the old item if it was replaced'),
  }))
  .output(z.object({
    success: z.boolean().describe('Whether the operation succeeded'),
    oldItem: z.record(z.string(), z.any()).optional().describe('Previous item if returnOldItem was true and item existed'),
  }))
