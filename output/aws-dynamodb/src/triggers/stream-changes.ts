...
export let streamChanges = SlateTrigger.create(
  spec,
  {
    name: 'Table Item Changes',
    key: 'table_item_changes',
    description: 'Polls DynamoDB Streams for item-level changes (inserts, updates, deletes) on a table with streams enabled.',
    instructions: [
  ...
  .input(z.object({
    ...
    newImage: z.record(z.string(), z.any()).optional().describe('Item after the change (if stream view type includes new image)'),
    oldImage: z.record(z.string(), z.any()).optional().describe('Item before the change (if stream view type includes old image)'),
    sequenceNumber: z.string().optional().describe('Sequence number of the stream record'),
    approximateCreationTimestamp: z.string().optional().describe('Approximate time the event occurred'),
    ...
    pollEvents: async (ctx) => {
      ...
      for (let shard of shards) {
        ...
        if (!iterator) {
          ...
          let iterType = processedShards.includes(shardId) ? 'AFTER_SEQUENCE_NUMBER' as const : 'LATEST' as const;
