...
export let emailSendingEvents = SlateTrigger.create(
  spec,
  {
  ...
  .output(z.object({
    sourceType: z.enum(['campaign', 'loop', 'transactional']).describe('Type of email that was sent'),
    ...
    })).optional().describe('Related mailing lists (for campaign emails)'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let sendingEvents = ['campaign.email.sent', 'loop.email.sent', 'transactional.email.sent'];
      ...
      let mailingListsRaw = data.mailingLists as Array<{ id?: string; name?: string }> | undefined;
    ...
    handleEvent: async (ctx) => {
      let { eventName, eventTime, webhookId, contactIdentity, email, campaignId, loopId, transactionalId, mailingLists } = ctx.input;
