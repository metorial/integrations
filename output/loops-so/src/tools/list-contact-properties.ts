...
export let listContactProperties = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    filter: z.enum(['all', 'custom']).optional().describe('Filter properties: "all" returns default and custom, "custom" returns only custom properties (defaults to "all")')
  }))
  .output(z.object({
    properties: z.array(z.object({
      ...
      type: z.string().describe('Property data type (e.g., string, number, boolean, date)')
