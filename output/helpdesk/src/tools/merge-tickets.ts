...
export let mergeTickets = SlateTool.create(
  spec,
  {
    name: 'Merge Tickets',
    key: 'merge_tickets',
  ...
  .input(z.discriminatedUnion('mode', [
    z.object({
      ...
      parentTicketId: z.string().describe('The ID of the parent ticket that child tickets will be merged into'),
      childTicketIds: z.array(z.string()).min(1).describe('Array of ticket IDs to merge into the parent ticket (at least one required)'),
    }),
    z.object({
  ...
  .output(z.object({
    success: z.boolean().describe('Whether the merge/unmerge operation completed successfully'),
