...
let auditLogEntrySchema = z.object({
  ...
  authorType: z.string().optional().describe('The type of the author who performed the action'),
...
export let queryAuditLog = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'delete']).optional().describe('Filter by action type'),
    entityType: z.string().optional().describe('Filter by entity type (e.g., ticket, agent, team)'),
    authorType: z.string().optional().describe('Filter by author type'),
    authorID: z.string().optional().describe('Filter by author ID'),
    createdFrom: z.string().optional().describe('Filter entries created on or after this ISO 8601 date'),
    createdTo: z.string().optional().describe('Filter entries created on or before this ISO 8601 date'),
