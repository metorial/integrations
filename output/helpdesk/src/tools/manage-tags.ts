...
export let manageTags = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'update', 'delete']).describe('Operation to perform on tags'),
    ...
    teamIDs: z.array(z.string()).optional().describe('Team IDs to scope this tag to (optional for create and update)'),
  }))
  .output(z.object({
    tags: z.array(tagSchema).optional().describe('List of tags (for list action)'),
    tag: tagSchema.optional().describe('Single tag details (for get, create, update actions)'),
