...
let listItemOutputSchema = z.object({
  ...
  fields: z.record(z.string(), z.any()).optional().describe('Custom field values of the list item'),
});
...
export let manageListItems = SlateTool.create(spec, {
  ...
  constraints: [
    'Maximum of 5000 items can be returned in a single list request.',
  ],
  tags: {
    destructive: false,
    readOnly: false,
  ...
  .input(z.object({
    ...
    itemId: z.string().optional().describe('List item ID (required for get, update, delete)'),
    fields: z.record(z.string(), z.any()).optional().describe('Field values for create or update, keyed by column name'),
    ...
    top: z.number().optional().describe('Maximum number of items to return (for list action)'),
    skip: z.number().optional().describe('Number of items to skip for pagination (for list action)'),
  }))
  .output(z.object({
    item: listItemOutputSchema.optional().describe('Single list item (for get, create, update)'),
    items: z.array(listItemOutputSchema).optional().describe('List of items (for list action)'),
    deleted: z.boolean().optional().describe('Whether the item was deleted (for delete action)'),
