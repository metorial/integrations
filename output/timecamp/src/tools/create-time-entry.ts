...
export let createTimeEntry = SlateTool.create(
  ...
  .input(z.object({
    date: z.string().optional().describe('Date for the entry (YYYY-MM-DD). Defaults to today if not specified.'),
    start: z.string().optional().describe('Start datetime (YYYY-MM-DD HH:MM:SS). Use with end.'),
    end: z.string().optional().describe('End datetime (YYYY-MM-DD HH:MM:SS). Use with start.'),
    duration: z.number().optional().describe('Duration in seconds. Use if not providing start/end.'),
