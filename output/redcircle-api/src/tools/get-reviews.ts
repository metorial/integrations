...
export let getReviews = SlateTool.create(
  ...
  .input(z.object({
    ...
    reviewerType: z.enum(['verified_purchase', 'all']).optional().describe('Filter by reviewer type.'),
    reviewStars: z.enum(['all_stars', 'five_star', 'four_star', 'three_star', 'two_star', 'one_star']).optional().describe('Filter by star rating.'),
    reviewMediaType: z.enum(['all_reviews', 'media_reviews_only']).optional().describe('Filter to only reviews with media.'),
    sortBy: z.enum(['most_recent', 'high_to_low_rating', 'low_to_high_rating', 'most_helpful']).optional().describe('Sort order for reviews.'),
    page: z.number().optional().describe('Page number.'),
    maxPage: z.number().optional().describe('Auto-paginate up to this page and concatenate results.'),
  }))
  .output(z.object({
    reviews: z.array(z.any()).describe('Array of customer reviews with rating, title, body, date, author, and feedback counts.'),
    summary: z.any().optional().describe('Review summary including overall rating, total ratings, rating breakdown, and recommended percentage.'),
