...
export let searchProducts = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum 2,000 requests per minute.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    searchTerm: z.string().optional().describe('Keywords to search for on Target (e.g. "highlighter pens").'),
    url: z.string().optional().describe('A Target search results page URL. Overrides searchTerm if provided.'),
    categoryId: z.string().optional().describe('Filter results by a Target category ID.'),
    sortBy: z.enum(['best_seller', 'price_high_to_low', 'price_low_to_high', 'best_match', 'highest_rating', 'newly_listed', 'featured']).optional().describe('Sort order for search results.'),
    deliveryType: z.enum(['in_store_pickup', 'same_day_delivery', 'buy_at_store', 'ship_to_home']).optional().describe('Filter by delivery/fulfillment type.'),
    rating: z.enum(['one_star', 'two_star', 'three_star', 'four_star', 'five_star']).optional().describe('Filter by minimum star rating.'),
    includeOutOfStock: z.boolean().optional().describe('Include out-of-stock products in results. Defaults to false.'),
    facets: z.string().optional().describe('Comma-separated facet IDs for filtering (e.g. brand, price range).'),
    page: z.number().optional().describe('Page number to retrieve.'),
    maxPage: z.number().optional().describe('Auto-paginate up to this page number and concatenate results.'),
    customerZipcode: z.string().optional().describe('US zipcode to localize results. Overrides the global config zipcode.'),
  }))
  .output(z.object({
    searchResults: z.array(z.any()).describe('Array of product search results with position, product details, offers, and fulfillment info.'),
    pagination: z.any().optional().describe('Pagination info including current page, total pages, and total results.'),
    relatedQueries: z.array(z.any()).optional().describe('Related search queries suggested by Target.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includeOutOfStock !== undefined) params.include_out_of_stock = ctx.input.includeOutOfStock;
