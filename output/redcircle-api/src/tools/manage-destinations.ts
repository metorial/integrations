...
export let manageDestinations = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum 50 destinations per account.',
    ],
  }
)
  .input(z.object({
    action: z.enum(['list', 'create', 'update', 'delete']).describe('Action to perform.'),
    destinationId: z.string().optional().describe('Destination ID. Required for update and delete actions.'),
    ...
    enabled: z.boolean().optional().describe('Whether the destination is enabled (create/update).'),
    storageType: z.enum(['s3', 'gcs', 'azure', 'oss', 's3_compatible']).optional().describe('Cloud storage type (create).'),

    // S3
    s3AccessKeyId: z.string().optional().describe('AWS access key ID (S3/S3-compatible).'),
    s3SecretAccessKey: z.string().optional().describe('AWS secret access key (S3/S3-compatible).'),
    s3BucketName: z.string().optional().describe('S3 bucket name.'),
    s3PathPrefix: z.string().optional().describe('S3 path prefix. Supports tokens: %COLLECTION_ID%, %COLLECTION_NAME%, %RESULT_SET_ID%, %DATE%.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      if (ctx.input.azureContainerName) body.azure_container_name = ctx.input.azureContainerName;
