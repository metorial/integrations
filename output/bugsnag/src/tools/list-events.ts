...
export let listEvents = SlateTool.create(
  ...
  .input(z.object({
    ...
    errorId: z.string().optional().describe('Error ID to list events for (omit for all project events)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let mapped = events.map((e: any) => ({
      ...
      exceptionClass: e.exception_class || e.exceptions?.[0]?.errorClass || e.exceptions?.[0]?.error_class,
