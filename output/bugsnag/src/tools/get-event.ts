...
export let getEvent = SlateTool.create(
  ...
  .output(z.object({
    ...
    receivedAt: z.string().optional().describe('ISO 8601 timestamp when the event was received'),
    exceptions: z.array(exceptionSchema).optional().describe('Exception details with stacktraces'),
    ...
    }).optional().describe('User who experienced the error'),
    app: z.object({
    ...
    }).optional().describe('HTTP request details'),
    ...
    })).optional().describe('Breadcrumb trail leading to the error'),
    ...
    })).optional().describe('Active feature flags'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let exClass = event.exceptions?.[0]?.errorClass || event.exceptions?.[0]?.error_class || 'Unknown';
