...
export let getPivots = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    pivotField: z.string().optional().describe('Specific pivot field to get values for (e.g., device.browser, device.os_name). Omit to list all available pivots.'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Available pivot dimensions (when no pivotField specified)'),
    pivotValues: z.array(pivotValueSchema).optional().describe('Pivot value distribution (when pivotField specified)'),
  }))
