...
export let getErrorTrends = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    errorId: z.string().optional().describe('Error ID for error-specific trends (omit for project-level trends)'),
    resolution: z.enum(['1h', '2h', '6h', '12h', '1d', '2d', '7d']).optional().describe('Time bucket resolution'),
  }))
  .output(z.object({
