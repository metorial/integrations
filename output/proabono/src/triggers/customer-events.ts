...
export let customerEvents = SlateTrigger.create(
  spec,
  {
    ...
    key: 'customer_events',
    description: 'Triggers on customer lifecycle events including creation, billing, charging, suspension, and grey-listing. Configure the webhook in ProAbono BackOffice under Integration > My Webhooks, point it to the provided URL, and select customer-related events.',
  }
)
  .input(z.object({
    eventType: z.string().describe('ProAbono event type (e.g., CustomerAdded, CustomerSuspended)'),
  ...
  .output(z.object({
    referenceCustomer: z.string().optional().describe('Customer reference in your application'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let isCustomerEvent = customerEventTypes.some((t) => eventType === t) ||
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        type: typeMap[eventType] || `customer.${eventType.replace(/^Customer/, '').toLowerCase()}`,
