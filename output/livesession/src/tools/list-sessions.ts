...
let visitorSchema = z.object({
  ...
  firstSessionTimestamp: z.string().optional().describe('Timestamp of the visitor\'s first session'),
  lastSessionTimestamp: z.string().optional().describe('Timestamp of the visitor\'s last session')
...
let sessionSchema = z.object({
  ...
  }).optional().describe('Browser information'),
  os: z.object({
    name: z.string().optional(),
    version: z.string().optional()
  }).optional().describe('Operating system information'),
  resolution: z.object({
...
export let listSessions = SlateTool.create(
  ...
  .input(z.object({
    ...
    timezone: z.string().optional().describe('IANA timezone for date filtering (e.g. "America/New_York")'),
    dateFrom: z.string().optional().describe('Start date filter (ISO 8601 or relative: "today", "yesterday", etc.)'),
    dateTo: z.string().optional().describe('End date filter (ISO 8601 or relative: "today", "yesterday", etc.)')
  }))
  .output(z.object({
