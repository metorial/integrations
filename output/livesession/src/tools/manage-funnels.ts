...
let filterDataStringSchema = z.object({
  ...
  operator: z.string().describe('Comparison operator: equals, contains, starts_with, ends_with, in, not_in')
...
let filterDataIntSchema = z.object({
  ...
  operator: z.string().describe('Comparison operator: equals, greater_than, less_than, in, not_in')
...
let dateRangeSchema = z.object({
  from: z.string().describe('Start date (relative: TODAY, YESTERDAY, TODAY_MINUS_7_DAYS, TODAY_MINUS_30_DAYS, BEGINNING_OF_WEEK, BEGINNING_OF_MONTH, BEGINNING_OF_PREV_MONTH or absolute ISO 8601)'),
  to: z.string().describe('End date (relative or absolute ISO 8601)')
}).describe('Date range for funnel computation');

let conversionValueSchema = z.object({
...
let computeInputSchema = z.object({
...
export let createFunnel = SlateTool.create(
  spec,
  {
    name: 'Create Funnel',
    key: 'create_funnel',
  ...
  .input(z.object({
    ...
    isPublic: z.boolean().optional().describe('Whether the funnel should be publicly visible'),
    compute: z.array(computeInputSchema).describe('Computation settings with steps, date range, and filters')
  }))
  .output(funnelSchema)
...
export let deleteFunnel = SlateTool.create(
  spec,
  {
    name: 'Delete Funnel',
    key: 'delete_funnel',
...
export let updateFunnel = SlateTool.create(
  spec,
  {
    name: 'Update Funnel',
    key: 'update_funnel',
...
export let setFunnelFavourite = SlateTool.create(
  spec,
  {
