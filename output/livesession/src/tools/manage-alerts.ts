...
export let createAlert = SlateTool.create(
  ...
  .input(z.object({
    ...
    slackChannelId: z.string().optional().describe('Slack channel ID (required if provider is "slack")'),
    webhookId: z.string().optional().describe('Webhook endpoint ID (required if provider is "webhooks")')
  }))
  .output(alertSchema)
  .handleInvocation(async (ctx) => {
    let client = new Client(ctx.auth.token);
...
export let updateAlert = SlateTool.create(
  ...
  .input(z.object({
    ...
    provider: z.enum(['slack', 'webhooks']).optional().describe('Updated notification provider'),
