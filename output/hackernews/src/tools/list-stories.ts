...
export let listStories = SlateTool.create(
  spec,
  {
    name: 'List Stories',
    key: 'list_stories',
...
Returns story IDs with optional full story details. Up to 500 stories are available for top/new/best, and up to 200 for ask/show/job.`,
  ...
  .input(z.object({
    category: z.enum(['top', 'new', 'best', 'ask', 'show', 'job']).describe('Story category to retrieve'),
    limit: z.number().optional().default(30).describe('Maximum number of stories to return (default: 30)'),
    fetchDetails: z.boolean().optional().default(false).describe('Whether to fetch full details for each story (default: false, returns only IDs)'),
  }))
  .output(z.object({
    category: z.string().describe('The category that was queried'),
    totalAvailable: z.number().describe('Total number of story IDs available in this category'),
    storyIds: z.array(z.number()).optional().describe('Array of story IDs (when fetchDetails is false)'),
    stories: z.array(z.object({
      ...
      createdAt: z.string().optional().describe('ISO 8601 timestamp when the story was created'),
    })).optional().describe('Full story details (when fetchDetails is true)'),
  }))
