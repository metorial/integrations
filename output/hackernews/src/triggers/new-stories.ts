...
export let newStories = SlateTrigger.create(
  spec,
  {
    name: 'New Stories',
    key: 'new_stories',
    description: 'Triggers when new stories are posted to Hacker News. Polls for newly submitted stories and returns their full details.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let newMaxId = idsToFetch.length > 0
        ? Math.max(...idsToFetch)
        : lastSeenMaxId;
