...
export let keywordsByAsinTool = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum of 10 ASINs per request.',
  ...
  .input(z.object({
    asins: z.array(z.string()).min(1).max(10).describe('List of Amazon ASINs to research (1-10)'),
    includeVariants: z.boolean().optional().describe('Include product variants in keyword results'),
  ...
  .output(z.object({
    keywords: z.array(keywordDataSchema).describe('List of keywords associated with the queried ASINs'),
