...
export class SatisMeterClient {
  ...
  async getSurveyStatistics(params: SurveyStatisticsParams) {
    ...
    let response = await axiosV3.get(`/projects/${projectId}/campaigns/${campaignId}/statistics`, {
      headers: { Authorization: `Bearer ${this.token}` },
      params: queryParams,
    });
    return response.data;
  ...
  async upsertUser(params: UpsertUserParams) {
    let response = await axiosLegacy.post('/users', {
  ...
  async updateUnsubscribedEmails(projectId: string, emails: string[]) {
    let response = await axiosV2.patch(`/project-unsubscribes/${projectId}`, {
      data: {
        id: projectId,
