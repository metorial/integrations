...
export let listResponsesTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    surveyId: z.string().optional().describe('Filter responses to a specific survey. If omitted, returns responses from all surveys.'),
    startDate: z.string().optional().describe('Start of the date range filter in ISO 8601 format (e.g., 2024-01-01T00:00:00.000Z)'),
    endDate: z.string().optional().describe('End of the date range filter in ISO 8601 format (e.g., 2024-12-31T23:59:59.999Z)'),
    pageCursor: z.string().optional().describe('Cursor for pagination. Use the nextPageCursor from a previous response to get the next page.'),
    pageSize: z.number().optional().describe('Number of responses per page (1-100, default 20)'),
  }))
  .output(z.object({
    responses: z.array(z.object({
      ...
      answers: z.array(z.any()).optional().describe('Array of answer objects for each survey question'),
      category: z.string().optional().describe('Response category (e.g., promoter, passive, detractor)'),
      completed: z.boolean().optional().describe('Whether the user completed all survey questions'),
      created: z.string().optional().describe('Timestamp when the response was created'),
      method: z.string().optional().describe('Survey delivery method (In-App, Mobile, Email)'),
      user: z.any().optional().describe('User information associated with the response'),
    })).describe('List of survey responses'),
    nextPageCursor: z.string().optional().describe('Cursor to fetch the next page of results'),
  }))
