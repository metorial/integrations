...
export let listUsersTool = SlateTool.create(
  ...
  .input(z.object({
    userId: z.string().optional().describe('Filter by a specific userId to look up one user. If omitted, returns all users.'),
  }))
  .output(z.object({
    users: z.array(z.object({
      userInternalId: z.string().optional().describe('SatisMeter internal user ID (use this for delete operations)'),
      userId: z.string().optional().describe('External user ID as provided during creation'),
      traits: z.record(z.string(), z.unknown()).optional().describe('User custom traits/properties'),
      ...
      unsubscribed: z.boolean().optional().describe('Whether the user is unsubscribed from surveys'),
