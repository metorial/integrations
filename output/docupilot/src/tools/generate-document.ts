...
export let generateDocument = SlateTool.create(
  ...
  .input(z.object({
    ...
    mergeData: z.record(z.string(), z.unknown()).describe('Data to merge into the template. Keys must match the template merge field tokens.'),
    outputType: z.enum(['html', 'pdf', 'png', 'docx', 'pptx', 'xlsx']).optional().describe('Override the default output format'),
  }))
  .output(z.object({
    fileUrl: z.string().optional().describe('Secure download URL for the generated document (valid for 24 hours)'),
