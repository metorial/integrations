...
export let fileActivity = SlateTrigger.create(
  spec,
  {
    name: 'File Activity',
    key: 'file_activity',
    description: 'Triggers when file activity occurs on your Files.com site. Receives webhook events for file create, read, update, delete, move, and copy actions. The webhook behavior is automatically registered on your site root folder.',
  },
)
  .input(z.object({
    action: z.string().describe('File action: create, read, update, destroy, move, copy'),
    interface: z.string().optional().describe('Interface used: web, ftp, sftp, dav, restapi, robot'),
