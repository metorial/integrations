...
export let retrieveDocuments = SlateTool.create(
  spec,
  {
...
Supports metadata filtering, reranking for relevance, and limiting chunks per document for diversity.`,
  ...
  .input(z.object({
    ...
    topK: z.number().optional().describe('Maximum number of chunks to return. Higher values return more results but increase latency.'),
    rerank: z.boolean().optional().describe('Enable relevance reranking to filter out irrelevant results'),
    filter: z.record(z.string(), z.any()).optional().describe('Metadata filter using comparison operators ($eq, $ne, $gt, $gte, $lt, $lte, $in, $nin) and logical operators ($and, $or)'),
    maxChunksPerDocument: z.number().optional().describe('Maximum chunks per document for result diversity'),
  ...
  .output(z.object({
    chunks: z.array(z.object({
      ...
      score: z.number().describe('Relevance score (relative, not comparable across queries)'),
      chunkIndex: z.number().describe('Position index within the document'),
      metadata: z.record(z.string(), z.any()).describe('Chunk-level metadata (e.g., page numbers, timestamps)'),
      documentMetadata: z.record(z.string(), z.any()).describe('Parent document metadata'),
