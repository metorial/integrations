...
Metadata updates are applied without re-processing the document. Content updates replace the previous version and trigger re-indexing.`,
  ...
  .input(z.object({
    ...
    metadata: z.record(z.string(), z.any()).optional().describe('Metadata to merge. Set a key to null to delete it.'),
    updateUrl: z.string().optional().describe('New URL to replace the document content from'),
    updateRawData: z.union([z.string(), z.record(z.string(), z.any())]).optional().describe('New raw text or JSON data to replace the document content'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (updatedFields.length === 0) {
      throw new Error('No update fields provided. Specify at least one of: metadata, updateUrl, or updateRawData.');
    }
