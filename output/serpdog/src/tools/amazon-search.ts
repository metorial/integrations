...
export let amazonSearch = SlateTool.create(
  ...
  .input(z.object({
    type: z.enum(['search', 'product', 'reviews']).describe('Type of Amazon data to extract'),
    searchUrl: z.string().optional().describe('Encoded Amazon search page URL (required for "search" type)'),
    asin: z.string().optional().describe('Amazon Standard Identification Number (required for "product" type)'),
    productId: z.string().optional().describe('Amazon product ID (required for "reviews" type)'),
    amazonDomain: z.string().optional().describe('Amazon domain TLD (e.g., "com", "co.uk", "de"). Defaults to "com".'),
    premium: z.boolean().optional().describe('Use premium proxies for improved success. Costs more credits.'),
    country: z.string().optional().describe('Proxy country (only used with premium). ISO 3166 format.'),
    excludeSponsored: z.boolean().optional().describe('Exclude sponsored results from search results'),
