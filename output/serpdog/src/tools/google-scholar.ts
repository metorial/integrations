...
export let googleScholar = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    authorId: z.string().optional().describe('Google Scholar author ID for author profile lookup'),
    ...
    abstractOnly: z.boolean().optional().describe('Set to true to return only articles with abstracts'),
    cluster: z.string().optional().describe('Article ID to find all available versions'),
    excludeCitations: z.boolean().optional().describe('Set to true to exclude citations from results'),
    safeSearch: z.enum(['active', 'off']).optional().describe('Safe search filter. Defaults to "off".'),
    authorSort: z.enum(['title', 'pubdate']).optional().describe('Sort author articles by title or publication date'),
    authorViewCoauthors: z.boolean().optional().describe('View co-authors of the author'),
    citationId: z.string().optional().describe('Citation ID for fetching individual article citation details'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.authorId) {
      let data = await client.googleScholarAuthor({
        ...
        viewOp: ctx.input.authorViewCoauthors ? 'list_colleagues' : (ctx.input.citationId ? 'view_citation' : undefined),
