...
export let googleSearch = SlateTool.create(
  ...
  .input(z.object({
    ...
    country: z.string().optional().describe('Country code in ISO 3166 Alpha-2 format (e.g., "us", "gb", "de"). Defaults to "us".'),
    language: z.string().optional().describe('Language of the results (e.g., "en_us", "de"). Defaults to "en_us".'),
    numResults: z.number().optional().describe('Number of results per page'),
    page: z.number().optional().describe('Page offset (0 for first page, 10 for second, 20 for third, etc.)'),
    languageRestrict: z.string().optional().describe('Restrict results to a language, e.g., "lang_en"'),
    location: z.string().optional().describe('UULE-encoded location for geo-targeted results'),
    timeFilter: z.string().optional().describe('Time filter: "d" (day), "w" (week), "m" (month), "y" (year)'),
    noAutoCorrect: z.boolean().optional().describe('Set to true to disable auto-correction of misspelled queries'),
    advancedFilter: z.string().optional().describe('Advanced TBS filter parameter for search results'),
    safeSearch: z.enum(['active', 'off']).optional().describe('Safe search filter for adult content. Defaults to "off".'),
    googleDomain: z.string().optional().describe('Google domain for local results (e.g., "google.co.in"). Defaults to "google.com".'),
    returnHtml: z.boolean().optional().describe('Set to true to return raw HTML response instead of JSON'),
    lite: z.boolean().optional().describe('Use the lite search endpoint for faster, cheaper results with fewer featured snippets'),
  }))
  .output(z.object({
