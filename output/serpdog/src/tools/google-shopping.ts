...
export let googleShopping = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    productId: z.string().optional().describe('Google Product ID for detailed product info (from Google Shopping URL)'),
    ...
    includeOffers: z.boolean().optional().describe('Include product offers/sellers (when using productId)'),
    includeSpecs: z.boolean().optional().describe('Include product specifications (when using productId)'),
    includeReviews: z.boolean().optional().describe('Include product reviews (when using productId)'),
    filters: z.string().optional().describe('Sorting/filtering options for product offers or reviews'),
  }))
  .output(z.object({
