...
export let yelpSearch = SlateTool.create(
  spec,
  {
    name: 'Yelp Search',
    key: 'yelp_search',
  ...
  .input(z.object({
    location: z.string().describe('The location to search in (e.g., "New York, NY", "San Francisco, CA")'),
    query: z.string().optional().describe('Search query for specific businesses or services (e.g., "pizza", "plumber")'),
    sortBy: z.enum(['recommended', 'rating', 'review_count']).optional().describe('Sort results by recommendation (default), highest rated, or most reviewed'),
