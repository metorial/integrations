...
export let createLipsync = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    inputAudioUrl: z.string().optional().describe('Public URL of the audio file to lip-sync with'),
    useAudioFromVideo: z.boolean().optional().describe('Use the audio from the source video instead of a separate audio file'),
    ...
    webhookUrl: z.string().optional().describe('URL to receive completion webhook notifications'),
  }))
  .output(z.object({
    inferenceId: z.string().describe('Inference ID to track lip-sync generation status'),
    videoUrl: z.string().nullable().describe('URL to the output video (available when complete)'),
