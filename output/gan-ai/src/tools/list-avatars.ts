...
export let listAvatars = SlateTool.create(
  ...
  .input(z.object({
    avatarId: z.string().optional().describe('If provided, returns detailed info for this specific avatar'),
    title: z.string().optional().describe('Filter avatars by title'),
    status: z.array(z.enum(['consent_pending', 'processing', 'consent_failed', 'failed', 'published', 'deleted'])).optional().describe('Filter by status'),
    skip: z.number().optional().describe('Number of records to skip'),
    limit: z.number().optional().describe('Maximum number of records to return (default 10)'),
    startDatetime: z.string().optional().describe('Filter avatars created after this ISO 8601 datetime'),
    endDatetime: z.string().optional().describe('Filter avatars created before this ISO 8601 datetime'),
  }))
  .output(z.object({
    ...
    avatars: z.array(z.object({
      ...
      })).nullable().optional().describe('Avatar processing check results'),
      consentResults: z.array(z.object({
        checkName: z.string(),
        checkStatus: z.string(),
      })).nullable().optional().describe('Consent verification check results'),
    })),
  }))
      ...
      return {
        output: {
          ...
          avatars: [{
            ...
            processingResults: details.avatar_processing_results?.map((r) => ({
              checkName: r.check_name,
              checkStatus: r.check_status,
            })) ?? null,
            consentResults: details.consent_verification_results?.map((r) => ({
              checkName: r.check_name,
              checkStatus: r.check_status,
            })) ?? null,
          }],
