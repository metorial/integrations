...
export let manageConsent = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    consentVideoUrl: z.string().optional().describe('Public URL of consent video (required for submit_consent, MP4, max 100MB, max 30s)'),
  }))
  .output(z.object({
    passcode: z.string().optional().describe('Consent passcode to be spoken in the consent video'),
    passcodeExpiresAt: z.number().optional().describe('Passcode expiration timestamp'),
    consentSubmitted: z.boolean().optional().describe('Whether the consent video was submitted successfully'),
  }))
