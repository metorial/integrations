...
export let generatePhotoAvatarVideo = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    text: z.string().optional().describe('Script text for the avatar to speak (use with voiceSampleUrl)'),
    audioUrl: z.string().optional().describe('Public URL of audio file (use alone, without text/voiceSampleUrl)'),
    voiceSampleUrl: z.string().optional().describe('Public URL of a voice sample for TTS (use with text)'),
  }))
  .output(z.object({
    photoAvatarId: z.string().describe('Photo avatar ID used'),
    photoAvatarInferenceId: z.string().describe('Inference ID to track video generation status'),
    title: z.string().nullable().describe('Video title'),
    status: z.string().describe('Current generation status'),
    videoUrl: z.string().nullable().describe('URL to the generated video (available when complete)'),
    ...
    creditDetails: z.object({
      ttsCost: z.number(),
      ganCost: z.number(),
    }).nullable().describe('Credit costs for this generation'),
  }))
