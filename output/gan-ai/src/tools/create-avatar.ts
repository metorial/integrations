...
export let createAvatar = SlateTool.create(
  spec,
  {
    name: 'Create Avatar',
    key: 'create_avatar',
  ...
  .input(z.object({
    baseVideoUrl: z.string().describe('Public URL of the MP4 video to create the avatar from'),
  ...
  .output(z.object({
    ...
    status: z.string().describe('Current avatar status (consent_pending, processing, published, failed, etc.)'),
