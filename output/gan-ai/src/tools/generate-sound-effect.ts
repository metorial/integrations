...
export let generateSoundEffect = SlateTool.create(
  ...
  .input(z.object({
    prompt: z.string().min(5).max(256).describe('Text description of the desired sound effect'),
    numVariations: z.number().min(1).max(4).optional().describe('Number of variations to generate (default 3)'),
    durationSeconds: z.number().min(1).max(20).optional().describe('Duration in seconds (default 1)'),
    creativity: z.number().min(0).max(1).optional().describe('Creativity level from 0 (faithful) to 1 (creative), default 0'),
  }))
  .output(z.object({
