...
export let searchProfiles = SlateTool.create(
  spec,
  {
...
Can also count matching profiles without returning full data.`,
  ...
  .input(z.object({
    ...
    filters: z.array(z.record(z.string(), z.unknown())).optional().describe('Array of segmentation filter expressions (e.g. [{"kind":"property","prop":"email","op":"eq","value":"user@example.com"}])'),
    filtersOp: z.enum(['and', 'or']).optional().describe('Logical operator joining filters. Defaults to "and"'),
    countOnly: z.boolean().optional().describe('If true, only return the count of matching profiles'),
    limit: z.number().min(1).max(500).optional().describe('Number of profiles to return (1-500, default 50)'),
    ...
    expandProfile: z.enum(['all', 'min']).optional().describe('Level of profile detail: "all" for full properties, "min" for minimal'),
    expandCompany: z.enum(['all', 'min', 'skip']).optional().describe('Level of company detail: "all", "min", or "skip"'),
  }))
  .output(z.object({
    profiles: z.array(z.record(z.string(), z.unknown())).optional().describe('Array of matching user profiles'),
    count: z.number().optional().describe('Count of matching profiles (when countOnly is true)'),
    cursor: z.object({
      limit: z.number().optional(),
      before: z.string().optional(),
    }).optional().describe('Pagination cursor for fetching more results'),
  }))
