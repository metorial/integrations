...
let surveySchema = z.object({
  ...
  }).optional().describe('Microsurvey interaction statistics'),
});
...
let mapSurvey = (survey: Record<string, unknown>) => ({
  ...
  stats: survey.stats ? {
    startedCount: (survey.stats as Record<string, unknown>).started_count as number | undefined,
    completedCount: (survey.stats as Record<string, unknown>).completed_count as number | undefined,
    exitedCount: (survey.stats as Record<string, unknown>).exited_count as number | undefined,
    lastStartedAt: (survey.stats as Record<string, unknown>).last_started_at as string | null | undefined,
    lastCompletedAt: (survey.stats as Record<string, unknown>).last_completed_at as string | null | undefined,
    lastExitedAt: (survey.stats as Record<string, unknown>).last_exited_at as string | null | undefined,
  } : undefined,
});

export let listMicrosurveys = SlateTool.create(
  spec,
  {
...
Returns microsurvey details including name, status, segment targeting, and response statistics.`,
  ...
  .input(z.object({
    surveyId: z.string().optional().describe('Chameleon microsurvey ID to retrieve a specific microsurvey'),
    limit: z.number().min(1).max(500).optional().describe('Number of microsurveys to return (1-500, default 50)'),
