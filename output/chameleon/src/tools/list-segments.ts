...
let segmentSchema = z.object({
  ...
  items: z.array(z.record(z.string(), z.unknown())).optional().describe('Segmentation filter expressions'),
...
Optionally list experiences (tours, surveys, launchers) connected to a segment.`,
  ...
  .input(z.object({
    segmentId: z.string().optional().describe('Chameleon segment ID to retrieve a specific segment'),
    experienceKind: z.enum(['tour', 'survey', 'launcher']).optional().describe('If set along with segmentId, retrieves experiences of this type connected to the segment'),
    limit: z.number().min(1).max(500).optional().describe('Number of segments to return (1-500, default 50)'),
  ...
  .output(z.object({
    ...
    experiences: z.array(z.record(z.string(), z.unknown())).optional().describe('Experiences connected to the segment'),
    cursor: z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.segmentId && ctx.input.experienceKind) {
      let result = await client.getSegmentExperiences(ctx.input.segmentId, ctx.input.experienceKind);
      return {
        output: {
