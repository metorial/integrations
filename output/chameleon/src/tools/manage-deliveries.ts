...
export let manageDeliveries = SlateTool.create(
  spec,
  {
...
Use this to schedule or send experiences to individual users on demand.`,
  ...
  .input(z.object({
    ...
    profileId: z.string().optional().describe('Filter deliveries by user profile ID, or target user for create'),
    ...
    options: z.record(z.string(), z.unknown()).optional().describe('Custom data for merge tags'),
    from: z.string().optional().describe('Earliest delivery time (ISO8601)'),
    until: z.string().optional().describe('Latest delivery time (ISO8601 or interval like "+30d")'),
    useSegmentation: z.boolean().optional().describe('Apply audience filters'),
    once: z.boolean().optional().describe('Only deliver if user has not previously seen this experience'),
    skipTriggers: z.boolean().optional().describe('Bypass first-step triggers (default true)'),
  ...
  .output(z.object({
    ...
    deliveries: z.array(deliverySchema).optional().describe('Array of deliveries (for list action)'),
