...
export let manageInventory = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['equip', 'buy-gear', 'hatch', 'feed', 'purchase', 'sell']).describe('Inventory action to perform'),
    equipType: z.enum(['equipped', 'costume', 'pet', 'mount']).optional().describe('Equipment type for equip action'),
    ...
    itemType: z.string().optional().describe('Item category type (for purchase/sell: "eggs", "hatchingPotions", "food", "quests", "gear")'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'equip') {
      if (!ctx.input.equipType || !ctx.input.itemKey) throw new Error('equipType and itemKey are required for equip action');
    ...
    if (ctx.input.action === 'hatch') {
      if (!ctx.input.egg || !ctx.input.hatchingPotion) throw new Error('egg and hatchingPotion are required for hatch action');
    ...
    if (ctx.input.action === 'feed') {
      if (!ctx.input.pet || !ctx.input.food) throw new Error('pet and food are required for feed action');
    ...
    if (ctx.input.action === 'purchase') {
      if (!ctx.input.itemType || !ctx.input.itemKey) throw new Error('itemType and itemKey are required for purchase action');
    ...
    if (ctx.input.action === 'sell') {
      if (!ctx.input.itemType || !ctx.input.itemKey) throw new Error('itemType and itemKey are required for sell action');
