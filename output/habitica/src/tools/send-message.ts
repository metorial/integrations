...
export let sendMessage = SlateTool.create(
  ...
  .input(z.object({
    messageType: z.enum(['group', 'private']).describe('Whether to send a group chat message or a private message'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!ctx.input.targetUserId) throw new Error('targetUserId is required for private messages');
