...
export let listGroups = SlateTool.create(
  ...
  .input(z.object({
    query: z.string().optional().describe('Search query to filter groups. Supports name and email prefix matching.'),
    domain: z.string().optional().describe('Domain to scope the listing to'),
    userKey: z.string().optional().describe('List only groups that this user or group is a member of (email or ID)'),
    maxResults: z.number().optional().describe('Maximum number of results (1-200). Defaults to 200.'),
