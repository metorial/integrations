...
export let getActivityReports = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    applicationName: z.enum(['admin', 'login', 'drive', 'mobile', 'token', 'groups', 'saml', 'calendar', 'gplus', 'user_accounts', 'access_transparency', 'context_aware_access', 'chrome', 'data_studio', 'keep', 'jamboard', 'meet', 'rules']).describe('Application to get activity logs for'),
    eventName: z.string().optional().describe('Specific event name to filter by'),
    startTime: z.string().optional().describe('Start of the time range in RFC 3339 format (e.g. 2024-01-01T00:00:00Z)'),
    endTime: z.string().optional().describe('End of the time range in RFC 3339 format'),
    filters: z.string().optional().describe('Event parameter filter (e.g. "doc_title==My Document")'),
    ...
    maxResults: z.number().optional().describe('Max results per page (1-1000). Defaults to 100.'),
