...
export let manageUserAliases = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    alias: z.string().optional().describe('Email alias to add or remove (required for add/remove actions)')
  }))
  .output(z.object({
    ...
    })).optional().describe('List of aliases (returned for list action)'),
