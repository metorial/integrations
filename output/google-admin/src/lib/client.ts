...
export class Client {
  ...
  async createUser(userData: {
    ...
    addresses?: Array<{ type: string; formatted?: string; streetAddress?: string; locality?: string; region?: string; postalCode?: string; country?: string; primary?: boolean }>;
    organizations?: Array<{ name?: string; title?: string; department?: string; primary?: boolean }>;
  ...
  async undeleteUser(userKey: string, orgUnitPath?: string) {
    await directoryApi.post(`/users/${encodeURIComponent(userKey)}/undelete`, {
  ...
  async makeUserAdmin(userKey: string, isAdmin: boolean) {
    await directoryApi.post(`/users/${encodeURIComponent(userKey)}/makeAdmin`, {
      status: isAdmin
    }, {
      headers: this.headers
    });
  ...
  async createUserAlias(userKey: string, alias: string) {
    let response = await directoryApi.post(`/users/${encodeURIComponent(userKey)}/aliases`, {
      alias
    }, {
      headers: this.headers
    });
  ...
  async deleteUserAlias(userKey: string, alias: string) {
    await directoryApi.delete(`/users/${encodeURIComponent(userKey)}/aliases/${encodeURIComponent(alias)}`, {
      headers: this.headers
    });
  }
  ...
  async updateGroup(groupKey: string, groupData: Record<string, any>) {
    let response = await directoryApi.put(`/groups/${encodeURIComponent(groupKey)}`, groupData, {
      headers: this.headers
    });
    return response.data;
  }
  ...
  async listGroupMembers(groupKey: string, params: {
    maxResults?: number;
    pageToken?: string;
    roles?: string;
  } = {}) {
  ...
  async addGroupMember(groupKey: string, memberData: {
    email: string;
    role?: string;
    type?: string;
  }) {
    let response = await directoryApi.post(`/groups/${encodeURIComponent(groupKey)}/members`, memberData, {
      headers: this.headers
    });
    return response.data;
  }
  ...
  async updateGroupMember(groupKey: string, memberKey: string, memberData: { role: string }) {
    let response = await directoryApi.put(`/groups/${encodeURIComponent(groupKey)}/members/${encodeURIComponent(memberKey)}`, memberData, {
      headers: this.headers
    });
    return response.data;
  }
  ...
  async removeGroupMember(groupKey: string, memberKey: string) {
    await directoryApi.delete(`/groups/${encodeURIComponent(groupKey)}/members/${encodeURIComponent(memberKey)}`, {
      headers: this.headers
    });
  }
  ...
  async updateGroupSettings(groupEmail: string, settings: Record<string, any>) {
    let response = await groupsSettingsApi.put(`/${encodeURIComponent(groupEmail)}`, settings, {
      headers: this.headers,
      params: { alt: 'json' }
    });
    return response.data;
  ...
  async getOrgUnit(orgUnitPath: string) {
    ...
    let response = await directoryApi.get(`/customer/${this.customerId}/orgunits/${encodeURIComponent(encodedPath)}`, {
      headers: this.headers
    });
    return response.data;
  }
  ...
  async createOrgUnit(orgUnitData: {
    ...
    let response = await directoryApi.post(`/customer/${this.customerId}/orgunits`, orgUnitData, {
      headers: this.headers
    });
    return response.data;
  }
  ...
  async updateOrgUnit(orgUnitPath: string, orgUnitData: Record<string, any>) {
    let encodedPath = orgUnitPath.startsWith('/') ? orgUnitPath.substring(1) : orgUnitPath;
    let response = await directoryApi.put(`/customer/${this.customerId}/orgunits/${encodeURIComponent(encodedPath)}`, orgUnitData, {
      headers: this.headers
    });
    return response.data;
  }
  ...
  async deleteOrgUnit(orgUnitPath: string) {
    let encodedPath = orgUnitPath.startsWith('/') ? orgUnitPath.substring(1) : orgUnitPath;
    await directoryApi.delete(`/customer/${this.customerId}/orgunits/${encodeURIComponent(encodedPath)}`, {
      headers: this.headers
    });
  }
  ...
  async createRoleAssignment(assignmentData: {
    ...
    let response = await directoryApi.post(`/customer/${this.customerId}/roleassignments`, assignmentData, {
      headers: this.headers
    });
    return response.data;
  }
  ...
  async deleteRoleAssignment(roleAssignmentId: string) {
    await directoryApi.delete(`/customer/${this.customerId}/roleassignments/${roleAssignmentId}`, {
      headers: this.headers
    });
  }
  ...
  async listChromeOsDevices(params: {
    query?: string;
    maxResults?: number;
    pageToken?: string;
    orderBy?: string;
  ...
  async getChromeOsDevice(deviceId: string) {
    let response = await directoryApi.get(`/customer/${this.customerId}/devices/chromeos/${deviceId}`, {
      headers: this.headers,
      params: { projection: 'FULL' }
    });
    return response.data;
  ...
  async updateChromeOsDevice(deviceId: string, deviceData: Record<string, any>) {
    let response = await directoryApi.put(`/customer/${this.customerId}/devices/chromeos/${deviceId}`, deviceData, {
      headers: this.headers
    });
    return response.data;
  }
  ...
  async performChromeOsDeviceAction(deviceId: string, action: string) {
    await directoryApi.post(`/customer/${this.customerId}/devices/chromeos/${deviceId}/action`, {
      action
    }, {
      headers: this.headers
    });
  ...
  async listMobileDevices(params: {
    query?: string;
    maxResults?: number;
    pageToken?: string;
    orderBy?: string;
  ...
  async getMobileDevice(resourceId: string) {
    let response = await directoryApi.get(`/customer/${this.customerId}/devices/mobile/${resourceId}`, {
      headers: this.headers,
      params: { projection: 'FULL' }
    });
    return response.data;
  ...
  async performMobileDeviceAction(resourceId: string, action: string) {
    await directoryApi.post(`/customer/${this.customerId}/devices/mobile/${resourceId}/action`, {
      action
    }, {
      headers: this.headers
    });
  ...
  async getDomain(domainName: string) {
    let response = await directoryApi.get(`/customer/${this.customerId}/domains/${domainName}`, {
      headers: this.headers
    });
    return response.data;
  }
  ...
  async addDomain(domainName: string) {
    let response = await directoryApi.post(`/customer/${this.customerId}/domains`, {
      domainName
    }, {
      headers: this.headers
    });
  ...
  async listCalendarResources(params: {
    maxResults?: number;
    pageToken?: string;
    query?: string;
    orderBy?: string;
  ...
  async getCalendarResource(resourceId: string) {
    let response = await directoryApi.get(`/customer/${this.customerId}/resources/calendars/${resourceId}`, {
      headers: this.headers
    });
    return response.data;
  }
  ...
  async createCalendarResource(resourceData: {
    ...
    let response = await directoryApi.post(`/customer/${this.customerId}/resources/calendars`, resourceData, {
      headers: this.headers
    });
    return response.data;
  }
  ...
  async updateCalendarResource(resourceId: string, resourceData: Record<string, any>) {
    let response = await directoryApi.put(`/customer/${this.customerId}/resources/calendars/${resourceId}`, resourceData, {
      headers: this.headers
    });
    return response.data;
  }
  ...
  async deleteCalendarResource(resourceId: string) {
    await directoryApi.delete(`/customer/${this.customerId}/resources/calendars/${resourceId}`, {
      headers: this.headers
    });
  }
  ...
  async listActivities(params: {
    ...
    let response = await reportsApi.get(`/activity/users/${encodeURIComponent(params.userKey)}/applications/${params.applicationName}`, {
      headers: this.headers,
      params: {
  ...
  async getUserUsageReport(params: {
    ...
    let response = await reportsApi.get(`/usage/users/${encodeURIComponent(params.userKey)}/dates/${params.date}`, {
      headers: this.headers,
      params: {
        parameters: params.parameters,
        pageToken: params.pageToken,
  ...
  async undeleteAlert(alertId: string) {
    let response = await alertCenterApi.post(`/alerts/${alertId}:undelete`, {}, {
      headers: this.headers
    });
    return response.data;
  }
  ...
  async createAlertFeedback(alertId: string, feedbackType: string) {
    let response = await alertCenterApi.post(`/alerts/${alertId}/feedback`, {
      type: feedbackType
    }, {
      headers: this.headers
    });
  ...
  async batchDeleteAlerts(alertIds: string[]) {
    let response = await alertCenterApi.post('/alerts:batchDelete', {
      alertId: alertIds
    }, {
      headers: this.headers
    });
  ...
  async batchUndeleteAlerts(alertIds: string[]) {
    let response = await alertCenterApi.post('/alerts:batchUndelete', {
      alertId: alertIds
    }, {
      headers: this.headers
    });
  ...
  async listLicenseAssignments(productId: string, customerId: string, params: {
    maxResults?: number;
    pageToken?: string;
  } = {}) {
  ...
  async getLicenseAssignment(productId: string, skuId: string, userId: string) {
    let response = await licensingApi.get(`/${productId}/sku/${skuId}/user/${encodeURIComponent(userId)}`, {
      headers: this.headers
    });
    return response.data;
  }
  ...
  async assignLicense(productId: string, skuId: string, userId: string) {
    let response = await licensingApi.post(`/${productId}/sku/${skuId}/user`, {
      userId
    }, {
      headers: this.headers
    });
  ...
  async revokeLicense(productId: string, skuId: string, userId: string) {
    await licensingApi.delete(`/${productId}/sku/${skuId}/user/${encodeURIComponent(userId)}`, {
      headers: this.headers
    });
  }
  ...
  async getSchema(schemaKey: string) {
    let response = await directoryApi.get(`/customer/${this.customerId}/schemas/${schemaKey}`, {
      headers: this.headers
    });
    return response.data;
  }
  ...
  async listPrivileges() {
    let response = await directoryApi.get(`/customer/${this.customerId}/roles/ALL/privileges`, {
      headers: this.headers
    });
    return response.data;
  }
  ...
  async watchUsers(params: {
    ...
    let response = await directoryApi.post('/users/watch', {
      id: params.channelId,
      type: 'web_hook',
  ...
  async stopChannel(channelId: string, resourceId: string) {
    await directoryApi.post('/channels/stop', {
      id: channelId,
      resourceId
    }, {
      headers: this.headers
  ...
  async listDataTransfers(params: {
