...
export let createContact = SlateTool.create(
  ...
  .input(z.object({
    ...
    externalId: z.string().optional().describe('External identifier from the source platform'),
    ...
    campaignId: z.string().optional().describe('AutoRequest campaign ID to enroll the contact in'),
    customAttributes: z.record(z.string(), z.unknown()).optional().describe('Custom key-value attributes for the contact'),
  }))
  .output(z.object({
