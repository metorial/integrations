...
export let uploadMedia = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['initiate', 'check-status']).describe('Action: "initiate" a new upload or "check-status" of an existing one'),
    fileName: z.string().optional().describe('File name with extension (required for "initiate")'),
    mediaId: z.string().optional().describe('Media ID to check status for (required for "check-status")'),
  }))
  .output(z.object({
    mediaId: z.string().describe('ID of the media'),
    uploadUrl: z.string().nullable().describe('Presigned URL for uploading the file (only for "initiate")'),
    status: z.string().nullable().describe('Processing status: "processing", "ready", or "failed" (only for "check-status")'),
    originalUrl: z.string().nullable().describe('URL of the processed media (only when status is "ready")'),
  }))
