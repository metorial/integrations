...
Supports multiple AI models and adapter types for different generation modes: text-to-image, image-to-image transformation, edge-based generation, and generative upscaling.`,
  ...
  .input(z.object({
    ...
    sourceImageUrl: z.string().optional().describe('URL of an input image for image-to-image generation'),
    modelType: z.enum([
      'realistic', 'fantasy', 'premium', 'z-image-turbo',
      'flux2-klein9b', 'qwen', 'see-dream-4.5',
      'gemini-3-pro-image-preview', 'google-gemini-image-flash',
    ]).optional().describe('AI model to use for generation'),
    adapterType: z.enum(['generate_background', 'control', 'control2', 'upscale']).optional()
      .describe('Generation adapter: generate_background (default), control (image+edges), control2 (edges only), upscale (generative upscale)'),
    ...
    generativeUpscale: z.boolean().optional().describe('Enable generative upscaling to target dimensions'),
