...
let financialStatementSchema = z.record(z.string(), z.object({
...
export let getStockFinancials = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'Use timeframe "annual" for 10-K reports and "quarterly" for 10-Q reports.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    timeframe: z.enum(['annual', 'quarterly', 'ttm']).optional().describe('Reporting timeframe'),
    ...
    periodOfReportDateGte: z.string().optional().describe('Report period date on or after (YYYY-MM-DD)'),
    periodOfReportDateLte: z.string().optional().describe('Report period date on or before (YYYY-MM-DD)'),
