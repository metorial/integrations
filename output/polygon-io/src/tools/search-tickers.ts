...
export let searchTickers = SlateTool.create(
  ...
  .input(z.object({
    search: z.string().optional().describe('Search query to filter tickers by name or symbol'),
    ticker: z.string().optional().describe('Exact ticker symbol to look up'),
    type: z.string().optional().describe('Filter by ticker type (e.g., CS, ETF, CRYPTO, FX)'),
    market: z.enum(['stocks', 'crypto', 'fx', 'otc', 'indices']).optional().describe('Filter by market'),
    ...
    sort: z.enum(['ticker', 'name', 'market', 'locale', 'primary_exchange', 'type', 'currency_symbol', 'currency_name', 'base_currency_symbol', 'base_currency_name', 'last_updated_utc']).optional().describe('Field to sort by'),
