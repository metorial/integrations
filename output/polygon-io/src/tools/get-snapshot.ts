...
let snapshotResultSchema = z.object({
  ...
  }).optional().describe('Last trade data'),
  lastQuote: z.object({
  ...
  }).optional().describe('Current day aggregate data'),
  previousDay: z.object({
    open: z.number().optional(),
    high: z.number().optional(),
    low: z.number().optional(),
    close: z.number().optional(),
    volume: z.number().optional(),
    volumeWeighted: z.number().optional(),
  }).optional().describe('Previous day aggregate data'),
...
Use this to quickly check current prices and daily statistics without fetching historical data.`,
  ...
  .input(z.object({
    tickers: z.array(z.string()).optional().describe('One or more ticker symbols to snapshot'),
    direction: z.enum(['gainers', 'losers']).optional().describe('Get top gainers or losers instead of specific tickers'),
