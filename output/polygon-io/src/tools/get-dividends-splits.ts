...
let dividendSchema = z.object({
  ...
  frequency: z.number().optional().describe('Frequency (0=one-time, 1=annual, 2=biannual, 4=quarterly, 12=monthly)'),
  dividendType: z.string().optional().describe('Type (CD=cash, SC=special cash, LT=long-term, ST=short-term)'),
...
export let getDividendsSplits = SlateTool.create(
  ...
  .input(z.object({
    ...
    dataType: z.enum(['dividends', 'splits', 'both']).optional().default('both').describe('Type of data to retrieve'),
