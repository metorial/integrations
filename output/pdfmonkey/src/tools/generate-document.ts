...
Optionally set a custom filename via the \`filename\` field, or password-protect the PDF via the \`password\` field. For image generation, use \`imageType\`, \`imageWidth\`, \`imageHeight\`, and \`imageQuality\` fields.`,
  ...
  .input(z.object({
    ...
    payload: z.record(z.string(), z.unknown()).optional().describe('Dynamic data to populate the template with. Must be a JSON object matching the template fields.'),
    synchronous: z.boolean().optional().default(false).describe('If true, waits for generation to complete before responding. Defaults to false (async).'),
    filename: z.string().optional().describe('Custom filename for the generated PDF (without extension)'),
    password: z.string().optional().describe('Password to encrypt the generated PDF with AES-256'),
    imageType: z.enum(['webp', 'png', 'jpg']).optional().describe('Image output format (for image templates only)'),
    imageWidth: z.number().optional().describe('Image width in pixels (for image templates only)'),
    imageHeight: z.number().optional().describe('Image height in pixels (for image templates only)'),
    imageQuality: z.number().optional().describe('Image quality percentage for WebP (for image templates only, default 100)'),
  }))
  .output(z.object({
