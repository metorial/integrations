...
let sizeItemSchema = z.object({
  sizeId: z.string().describe('Unique identifier for this size variant (e.g., "thumbnail", "medium", "large")'),
  strategy: z.enum(['exact', 'portrait', 'landscape', 'auto', 'crop', 'square', 'fit', 'fill', 'none']).describe('Resize strategy. Use "none" to return the optimized original without resizing.'),
  ...
  quality: z.number().min(1).max(100).optional().describe('Override global quality for this size (1-100)'),
  samplingScheme: z.enum(['4:2:0', '4:2:2', '4:4:4']).optional().describe('Override chroma subsampling for this size'),
  storagePath: z.string().optional().describe('Override storage path for this size when using external storage'),
});
...
export let generateImageSetsTool = SlateTool.create(
  spec,
  {
...
Useful for creating responsive image variants (thumbnails, mobile, desktop, retina) from a single source image.`,
  ...
  .input(z.object({
    ...
    sizes: z.array(sizeItemSchema).min(1).max(10).describe('Array of size configurations (max 10)'),
    lossy: z.boolean().optional().describe('Global lossy optimization setting. Individual sizes can override.'),
    quality: z.number().min(1).max(100).optional().describe('Global quality setting (1-100). Individual sizes can override.'),
    samplingScheme: z.enum(['4:2:0', '4:2:2', '4:4:4']).optional().describe('Global chroma subsampling scheme'),
    convert: z.object({
      format: z.enum(['jpeg', 'png', 'gif', 'webp', 'avif']).describe('Target image format'),
      background: z.string().optional().describe('Background color for transparency handling'),
    }).optional().describe('Convert all sizes to a different format'),
    preserveMeta: z.array(z.enum(['date', 'copyright', 'geotag', 'orientation', 'profile'])).optional().describe('Metadata types to preserve'),
    autoOrient: z.boolean().optional().describe('Automatically correct orientation based on EXIF data'),
    callbackUrl: z.string().optional().describe('URL to receive results via POST when processing completes'),
  }))
  .output(z.object({
    success: z.boolean().describe('Whether the operation was successful'),
    results: z.array(imageSetResultSchema).optional().describe('Optimization results for each requested size'),
