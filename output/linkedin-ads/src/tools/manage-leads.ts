...
export let listLeadForms = SlateTool.create(
  ...
  .output(z.object({
    leadForms: z.array(z.object({
      ...
      })).optional().describe('Form questions'),
    })),
  }))
...
export let getLeadFormResponses = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    accountId: z.string().optional().describe('Numeric ID of the ad account (alternative to leadFormId)'),
    startTime: z.number().optional().describe('Filter responses submitted after this epoch timestamp (ms)'),
    endTime: z.number().optional().describe('Filter responses submitted before this epoch timestamp (ms)'),
    pageSize: z.number().optional().describe('Number of results per page'),
    pageToken: z.string().optional().describe('Page token for pagination'),
  }))
  .output(z.object({
    responses: z.array(z.object({
      ...
      associatedEntity: z.string().optional().describe('URN of the associated campaign/creative'),
    })),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
