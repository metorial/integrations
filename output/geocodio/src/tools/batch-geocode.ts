...
Set **direction** to "forward" for addresses or "reverse" for coordinates. Enrichment fields can be appended to all results.`,
  ...
  .input(z.object({
    direction: z.enum(['forward', 'reverse']).describe('Geocoding direction: "forward" for addresses to coordinates, "reverse" for coordinates to addresses'),
    queries: z.array(z.string()).describe('Array of address strings (forward) or "latitude,longitude" strings (reverse)'),
