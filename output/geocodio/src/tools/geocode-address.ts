...
Provide the address either as a single string or as individual components (street, city, state, postalCode).`,
  ...
  .input(z.object({
    address: z.string().optional().describe('Full address as a single string, e.g. "1109 N Highland St, Arlington, VA 22201"'),
    ...
    country: z.string().optional().describe('Country code to force lookup: "US", "CA", or "MX"'),
    fields: z.array(z.string()).optional().describe('Data enrichment fields to append, e.g. ["timezone", "census", "cd"]'),
    limit: z.number().optional().describe('Maximum number of results to return'),
  }))
  .output(z.object({
    formattedAddress: z.string().optional().describe('Input address as parsed and formatted by Geocodio'),
    results: z.array(z.object({
      ...
      accuracyType: z.string().describe('Type of accuracy match, e.g. "rooftop", "range_interpolation"'),
      source: z.string().describe('Data source used for this result'),
      stableAddressKey: z.string().optional().describe('Persistent identifier for deduplication and re-fetching'),
      addressComponents: z.record(z.string(), z.any()).optional().describe('Parsed address components'),
      fields: z.record(z.string(), z.any()).optional().describe('Appended enrichment data'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let addressDisplay = ctx.input.address || [ctx.input.street, ctx.input.city, ctx.input.state, ctx.input.postalCode].filter(Boolean).join(', ');
