...
export let createRequester = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    departmentIds: z.array(z.number()).optional().describe('Department IDs the requester belongs to'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: `Created requester **#${requester.id}**: "${requester.first_name} ${requester.last_name || ''}"`.trim(),
    };
  }).build();

export let getRequester = SlateTool.create(
  spec,
  {
    name: 'Get Requester',
    key: 'get_requester',
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: `Retrieved requester **#${requester.id}**: "${requester.first_name} ${requester.last_name || ''}"`.trim(),
    };
  }).build();

export let listRequesters = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    query: z.string().optional().describe('Filter query (e.g. "email:\'user@example.com\'")'),
...
export let updateRequester = SlateTool.create(
  spec,
  {
