...
export let listTickets = SlateTool.create(
  ...
  .input(z.object({
    ...
    orderBy: z.string().optional().describe('Field to order by: created_at, due_by, updated_at, status'),
    orderType: z.enum(['asc', 'desc']).optional().describe('Sort direction'),
    updatedSince: z.string().optional().describe('Only return tickets updated since this date (ISO 8601)'),
    include: z.string().optional().describe('Comma-separated embedded resources: requester, stats, tags'),
