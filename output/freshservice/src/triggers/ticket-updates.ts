...
export let ticketUpdates = SlateTrigger.create(
  spec,
  {
    name: 'Ticket Updates',
    key: 'ticket_updates',
    description: 'Triggers when tickets are created or updated in Freshservice. Polls for recently modified tickets.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let knownTicketIds = ((ctx.state as Record<string, unknown>)?.knownTicketIds as number[]) || [];
