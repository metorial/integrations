...
let layerParamsSchema = z.object({
  ...
  imagePosition: z.string().optional().describe('Image cropping mode: "cro" (crop), "ai_face" (AI face detection), "cov" (cover), or "align" (alignment-based)'),
  imageAlignH: z.string().optional().describe('Horizontal alignment: "left", "center", "right", or "full"'),
  imageAlignV: z.string().optional().describe('Vertical alignment: "top", "center", "bottom", or "full"'),
  imageEffect: z.string().optional().describe('CSS filter value for image effects, e.g. "grayscale(100%)", "blur(5px)", "sepia(100%)"'),
  opacity: z.number().optional().describe('Layer opacity from 0.0 (transparent) to 1.0 (opaque)'),
...
let pageParamsSchema = z.object({
  ...
  templateId: z.string().optional().describe('Override template ID for this specific page (batch mode)'),
...
export let generateImage = SlateTool.create(
  ...
  .input(z.object({
    ...
    format: z.enum(['png', 'jpeg', 'webp', 'avif', 'pdf']).optional().describe('Output format. Defaults to template settings.'),
    metadata: z.string().optional().describe('Custom metadata string to attach to the generated image'),
    params: z.array(layerParamsSchema).optional().describe('Layer customizations for single-page generation'),
    pages: z.array(pageParamsSchema).optional().describe('Page definitions for multi-page generation'),
  }))
  .output(z.object({
    ...
    retinaThumbnailUrl: z.string().optional().describe('URL of the retina-quality thumbnail'),
