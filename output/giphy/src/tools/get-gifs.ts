...
export let getGifs = SlateTool.create(
  spec,
  {
    name: 'Get GIFs by ID',
    key: 'get_gifs',
    ...
    constraints: [
      'Maximum 100 IDs per request when fetching multiple GIFs.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    gifIds: z.array(z.string()).min(1).max(100).describe('Array of GIPHY GIF IDs to retrieve (1-100)'),
  }))
  .output(z.object({
    results: z.array(gifSchema).describe('Array of retrieved GIFs'),
    pagination: paginationSchema.optional().describe('Pagination information (for multi-ID requests)'),
  }))
