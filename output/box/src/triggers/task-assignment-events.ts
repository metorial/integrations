...
let taskAssignmentEventTypes = [
  'TASK_ASSIGNMENT.CREATED', 'TASK_ASSIGNMENT.UPDATED'
] as const;

export let taskAssignmentEvents = SlateTrigger.create(
  spec,
  {
  ...
  .input(z.object({
    eventType: z.string().describe('The Box webhook event type (e.g. TASK_ASSIGNMENT.CREATED)'),
