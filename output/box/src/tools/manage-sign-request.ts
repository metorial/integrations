...
export let manageSignRequest = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['create', 'get', 'list', 'cancel']).describe('The sign request operation to perform'),
    signRequestId: z.string().optional().describe('Sign request ID (required for get and cancel)'),
    signers: z.array(z.object({
      email: z.string().describe('Signer email address'),
      role: z.enum(['signer', 'approver', 'final_copy_reader']).optional().describe('Signer role'),
      order: z.number().optional().describe('Signing order (for sequential signing)')
    })).optional().describe('List of signers (required for create)'),
    sourceFileIds: z.array(z.string()).optional().describe('IDs of files to be signed (required for create)'),
    parentFolderId: z.string().optional().describe('Folder ID where signed documents will be stored (required for create)'),
    name: z.string().optional().describe('Name for the sign request'),
    emailSubject: z.string().optional().describe('Custom email subject for the signature request'),
    emailMessage: z.string().optional().describe('Custom email message body'),
    isDocumentPreparationNeeded: z.boolean().optional().describe('Whether document preparation UI should be shown before sending'),
  ...
  .output(z.object({
    ...
    })).optional().describe('Signer details and statuses'),
    cancelled: z.boolean().optional().describe('True if the request was cancelled'),
    signRequests: z.array(z.object({
      signRequestId: z.string(),
      status: z.string().optional(),
      name: z.string().optional(),
      createdAt: z.string().optional()
    })).optional().describe('List of sign requests')
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
    let { action, signRequestId, signers, sourceFileIds, parentFolderId, name, emailSubject, emailMessage, isDocumentPreparationNeeded, daysValid, limit, marker } = ctx.input;
    ...
    // create
    if (!signers || signers.length === 0) throw new Error('signers are required for create action');
    if (!sourceFileIds || sourceFileIds.length === 0) throw new Error('sourceFileIds are required for create action');
