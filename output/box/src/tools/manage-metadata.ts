...
export let manageMetadata = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['apply', 'get', 'get_all', 'update', 'remove', 'list_templates', 'get_template']).describe('The metadata operation to perform'),
    fileId: z.string().optional().describe('File ID (required for apply, get, get_all, update, remove)'),
    scope: z.string().optional().describe('Metadata template scope: "enterprise" or "global" (required for apply, get, update, remove, list_templates, get_template)'),
    templateKey: z.string().optional().describe('Metadata template key (required for apply, get, update, remove, get_template)'),
    metadata: z.record(z.string(), z.any()).optional().describe('Metadata key-value pairs to apply (for apply action)'),
    operations: z.array(z.object({
      op: z.enum(['add', 'replace', 'remove', 'test', 'move', 'copy']).describe('Patch operation type'),
    ...
    })).optional().describe('JSON Patch operations for updating metadata')
  }))
  .output(z.object({
    metadata: z.record(z.string(), z.any()).optional().describe('Applied or retrieved metadata key-value pairs'),
    metadataEntries: z.array(z.record(z.string(), z.any())).optional().describe('All metadata instances on the file'),
    ...
    })).optional().describe('Available metadata templates'),
    template: z.object({
      templateKey: z.string(),
      displayName: z.string().optional(),
      scope: z.string().optional(),
    ...
    }).optional().describe('Metadata template details'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'get_template') {
      if (!scope || !templateKey) throw new Error('scope and templateKey are required for get_template action');
    ...
    if (!scope || !templateKey) throw new Error('scope and templateKey are required for this action');
