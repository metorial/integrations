...
export let getFileInfo = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeVersions: z.boolean().optional().describe('Whether to also retrieve file version history')
  }))
  .output(z.object({
    ...
    ownedBy: z.object({
      ...
      name: z.string(),
      login: z.string()
    }).optional().describe('File owner information'),
    sharedLink: z.object({
      url: z.string(),
      access: z.string().optional()
    }).optional().nullable().describe('Shared link details if set'),
    ...
    })).optional().describe('Version history entries')
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });

    let file = await client.getFileInfo(ctx.input.fileId, [
      'id', 'name', 'size', 'type', 'created_at', 'modified_at', 'description',
      'parent', 'owned_by', 'shared_link'
    ]);
