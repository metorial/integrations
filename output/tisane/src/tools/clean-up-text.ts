...
export let cleanUpText = SlateTool.create(
  spec,
  {
    name: 'Clean Up Text',
    key: 'clean_up_text',
    ...
    constraints: [
      'Does not process binary content.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .output(z.object({
    cleanedText: z.string().describe('Pure plaintext with all markup, scripts, styles, and JSON removed'),
  }))
