...
export let getParticipants = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    status: z.string().optional().describe('Filter participants by status (e.g., "attended", "did-not-attend")'),
  }))
  .output(z.object({
    participants: z.array(participantSchema).describe('List of participants for the event date'),
  }))
