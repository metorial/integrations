...
export let getEvent = SlateTool.create(
  ...
  .output(z.object({
    ...
    eventType: z.string().optional().describe('Type of the event (standard, series, automated)'),
    registrationUrl: z.string().optional().describe('Public registration URL for the event'),
    eventDates: z.array(eventDateSchema).optional().describe('Scheduled dates for this event'),
  }))
