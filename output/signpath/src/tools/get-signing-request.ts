...
export let getSigningRequest = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    workflowStatus: z.string().describe('Workflow status (e.g., WaitingForApproval, Processing, Completed)'),
    isFinalStatus: z.boolean().describe('Whether the signing request has reached a final status'),
    ...
    signedArtifactLink: z.string().describe('URL to download the signed artifact (available when completed)'),
    createdDate: z.string().describe('Date when the signing request was created'),
    parameters: z.record(z.string(), z.string()).describe('User-defined parameters of the signing request'),
