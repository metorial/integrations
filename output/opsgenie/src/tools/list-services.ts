...
export let listServices = SlateTool.create(
  ...
  .input(z.object({
    ...
    limit: z.number().optional().describe('Number of services to return (max 100, default 20)'),
    offset: z.number().optional().describe('Start index for pagination'),
    sort: z.enum(['name', 'updatedAt', 'insertedAt', 'createdAt']).optional().describe('Field to sort by'),
