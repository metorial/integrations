...
export let manageSchedule = SlateTool.create(
  ...
  .input(z.object({
    ...
    scheduleIdentifier: z.string().optional().describe('Schedule ID or name (required for update/delete)'),
    identifierType: z.enum(['id', 'name']).optional().describe('Type of identifier provided. Defaults to "id"'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'delete': {
        ...
        await client.deleteSchedule(ctx.input.scheduleIdentifier, ctx.input.identifierType ?? 'id');
        return {
          output: {
