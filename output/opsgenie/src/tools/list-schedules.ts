...
export let listSchedules = SlateTool.create(
  ...
  .input(z.object({
    expandRotations: z.boolean().optional().describe('Whether to include rotation details in the response'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let data = await client.listSchedules(ctx.input.expandRotations ? { expand: 'rotation' } : {});
