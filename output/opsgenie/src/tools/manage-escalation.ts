...
let escalationRuleSchema = z.object({
  condition: z.enum(['if-not-acked', 'if-not-closed']).describe('Condition to trigger this rule'),
  notifyType: z.enum(['default', 'next', 'previous', 'users', 'admins', 'all']).describe('Notification type'),
  delay: z.object({
...
let repeatSchema = z.object({
  ...
  resetRecipientStates: z.boolean().optional().describe('Whether to reset recipient notification states'),
  closeAlertAfterAll: z.boolean().optional().describe('Whether to close the alert after all repeats'),
}).optional();

export let manageEscalation = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    escalationIdentifier: z.string().optional().describe('Escalation ID or name (required for update/delete)'),
    identifierType: z.enum(['id', 'name']).optional().describe('Type of identifier. Defaults to "id"'),
    ...
    rules: z.array(escalationRuleSchema).optional().describe('Escalation rules defining notification order and timing'),
    ...
    switch (ctx.input.action) {
      ...
      case 'update': {
        if (!ctx.input.escalationIdentifier) {
          throw new Error('escalationIdentifier is required when updating an escalation policy.');
        }
      ...
      case 'delete': {
        if (!ctx.input.escalationIdentifier) {
          throw new Error('escalationIdentifier is required when deleting an escalation policy.');
        }
        await client.deleteEscalation(ctx.input.escalationIdentifier, ctx.input.identifierType ?? 'id');
        return {
          output: {
