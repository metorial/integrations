...
export let listCandidates = SlateTool.create(
  ...
  .input(z.object({
    ...
    limit: z.number().min(1).max(100).optional().describe('Number of candidates to return (1-100, default 100)'),
    offset: z.number().min(0).optional().describe('Number of records to skip for pagination'),
  }))
  .output(z.object({
    candidates: z.array(z.record(z.string(), z.any())).describe('Array of candidate objects'),
