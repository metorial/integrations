...
export let listTeams = SlateTool.create(
  ...
  .input(z.object({
    teamId: z.string().optional().describe('If provided, lists the members of this specific team instead of listing all teams'),
    limit: z.number().min(1).max(100).optional().describe('Number of records to return (1-100, default 100)'),
    offset: z.number().min(0).optional().describe('Number of records to skip for pagination'),
  }))
  .output(z.object({
    teams: z.array(z.record(z.string(), z.any())).optional().describe('Array of team objects (when listing teams)'),
    members: z.array(z.record(z.string(), z.any())).optional().describe('Array of team member objects (when listing members of a specific team)'),
