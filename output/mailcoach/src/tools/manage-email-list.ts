...
export let manageEmailList = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    emailListUuid: z.string().optional().describe('UUID of the email list (required for update and delete)'),
    ...
    allowFormSubscriptions: z.boolean().optional().describe('Whether to allow form subscriptions'),
    requiresConfirmation: z.boolean().optional().describe('Whether double opt-in confirmation is required'),
    campaignsFeedEnabled: z.boolean().optional().describe('Whether campaigns RSS feed is enabled'),
    reportRecipients: z.string().optional().describe('Comma-separated email addresses for campaign reports'),
  ...
  .output(z.object({
    emailList: emailListOutputSchema.nullable().describe('The created or updated email list, null when deleted'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'delete') {
      if (!ctx.input.emailListUuid) throw new Error('emailListUuid is required for delete action');
    ...
    if (ctx.input.action === 'create') {
      ...
      if (!ctx.input.defaultFromEmail) throw new Error('defaultFromEmail is required for create action');
    ...
    // update
    if (!ctx.input.emailListUuid) throw new Error('emailListUuid is required for update action');
    ...
    let result = await client.updateEmailList(ctx.input.emailListUuid, {
      ...
      allow_form_subscriptions: ctx.input.allowFormSubscriptions ?? existing.allow_form_subscriptions,
      requires_confirmation: ctx.input.requiresConfirmation ?? existing.requires_confirmation,
      campaigns_feed_enabled: ctx.input.campaignsFeedEnabled ?? existing.campaigns_feed_enabled,
      ...
      report_campaign_summary: ctx.input.reportCampaignSummary ?? existing.report_campaign_summary,
      report_email_list_summary: ctx.input.reportEmailListSummary ?? existing.report_email_list_summary,
    });
