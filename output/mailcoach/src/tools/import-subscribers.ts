...
export let importSubscribers = SlateTool.create(
  ...
  .input(z.object({
    ...
    subscribeUnsubscribed: z.boolean().optional().describe('Whether to re-subscribe previously unsubscribed addresses'),
    unsubscribeOthers: z.boolean().optional().describe('Whether to unsubscribe addresses not in the import'),
  }))
  .output(z.object({
    importUuid: z.string().describe('UUID of the import'),
    status: z.string().describe('Current status of the import'),
    importedSubscribersCount: z.number().nullable().describe('Number of subscribers imported'),
