...
export let sendCampaign = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['send', 'send_test']).describe('Whether to send the campaign or send a test email'),
    campaignUuid: z.string().describe('UUID of the campaign to send'),
    testEmail: z.string().optional().describe('Email address(es) for test send (comma-separated, up to 10). Required for send_test action.'),
  }))
  .output(z.object({
