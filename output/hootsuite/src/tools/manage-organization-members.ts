...
export let manageOrganizationMembersTool = SlateTool.create(
  spec,
  {
...
Use **invite** to add a new member by email. Use **remove** to delete a member from the organization.`,
  ...
  .input(z.object({
    ...
    fullName: z.string().optional().describe('Full name of the new member (required for invite)'),
  ...
  .output(z.object({
    ...
    success: z.boolean().optional().describe('Whether the action succeeded (for invite/remove)')
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'get') {
      ...
      let permissions = await client.getOrganizationMemberPermissions(organizationId, ctx.input.memberId).catch(() => null);
