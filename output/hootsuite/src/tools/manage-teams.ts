...
Use **list_members** to see members in a team. Use **list_social_profiles** to see social profiles assigned to a team.`,
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'list_members', 'add_member', 'remove_member', 'list_social_profiles']).describe('Action to perform'),
    organizationId: z.string().optional().describe('Organization ID (required for list, create)'),
    teamId: z.string().optional().describe('Team ID (required for get, list_members, add_member, remove_member, list_social_profiles)'),
    teamName: z.string().optional().describe('Name for the new team (required for create)'),
    memberId: z.string().optional().describe('Member ID (required for add_member, remove_member)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('Social profiles accessible to the team'),
    cursor: z.string().optional().describe('Pagination cursor'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'list') {
      if (!ctx.input.organizationId) throw new Error('organizationId is required for list action');
      let result = await client.getOrganizationTeams(ctx.input.organizationId, ctx.input.cursor);
      ...
      return {
        output: { teams, members: undefined, socialProfiles: undefined, cursor: result.cursor, success: undefined },
    ...
    if (action === 'list_members') {
      ...
      return {
        output: { teams: undefined, members, socialProfiles: undefined, cursor: result.cursor, success: undefined },
    ...
    if (action === 'add_member') {
      ...
      return {
        output: { teams: undefined, members: undefined, socialProfiles: undefined, cursor: undefined, success: true },
    ...
    if (!ctx.input.teamId) throw new Error('teamId is required for list_social_profiles action');
    ...
    return {
      output: { teams: undefined, members: undefined, socialProfiles, cursor: undefined, success: undefined },
