...
export let createPollTool = SlateTool.create(
  spec,
  {
    name: 'Create Poll',
    key: 'create_poll',
...
Configure voting mode, visibility, and participant requirements.`,
  ...
  .input(z.object({
    ...
    description: z.string().optional().describe('Description providing context for the poll'),
    location: z.string().optional().describe('Location for the meeting or event'),
    pollType: z.enum(['TEXT', 'DATE']).describe('Type of poll: TEXT for general options, DATE for scheduling'),
    textOptions: z.array(textOptionSchema).optional().describe('Options for a text-based poll'),
    dateOptions: z.array(dateOptionSchema).optional().describe('Options for a date-based poll'),
    ...
    ifNeedBe: z.boolean().optional().describe('Enable YES/NO/MAYBE voting mode (default: YES/NO only)'),
    hidden: z.boolean().optional().describe('Hide participant votes from other participants'),
    ...
    askPhone: z.boolean().optional().describe('Require participants to provide their phone number'),
    askAddress: z.boolean().optional().describe('Require participants to provide their address'),
    timezone: z.string().optional().describe('Timezone for date-based polls, e.g. "America/New_York"')
  }))
  .output(z.object({
    pollId: z.string().describe('Unique identifier for the created poll'),
    adminKey: z.string().optional().describe('Admin key for managing the poll (store securely)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let options = ctx.input.pollType === 'TEXT'
