...
export let getBoards = SlateTool.create(
  spec,
  {
    name: 'Get Boards',
    key: 'get_boards',
  ...
  .input(z.object({
    memberId: z.string().optional().describe('Member ID to list boards for. Defaults to the authenticated user ("me")'),
    organizationId: z.string().optional().describe('Workspace/organization ID to list boards for. If provided, lists boards from this workspace instead'),
    filter: z.enum(['open', 'closed', 'all']).optional().describe('Filter boards by status. Defaults to "open"'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.organizationId) {
      rawBoards = await client.getOrganizationBoards(ctx.input.organizationId, ctx.input.filter);
    } else {
