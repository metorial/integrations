...
export let search = SlateTool.create(
  ...
  .input(z.object({
    ...
    modelTypes: z.string().optional().describe('Comma-separated model types to search: "cards", "boards", "organizations", "members". Defaults to "cards,boards"'),
    boardIds: z.string().optional().describe('Comma-separated board IDs to restrict search to'),
    cardsLimit: z.number().optional().describe('Max number of cards to return (default 10, max 1000)'),
    boardsLimit: z.number().optional().describe('Max number of boards to return (default 10)'),
  }))
  .output(z.object({
