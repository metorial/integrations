...
export let manageLabels = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['get', 'create', 'update', 'delete', 'assign', 'unassign']).describe('Action to perform'),
    boardId: z.string().optional().describe('Board ID (required for get, create)'),
    labelId: z.string().optional().describe('Label ID (required for update, delete, assign, unassign)'),
    cardId: z.string().optional().describe('Card ID (required for assign, unassign)'),
    name: z.string().optional().describe('Label name'),
    color: z.string().optional().describe('Label color (green, yellow, orange, red, purple, blue, sky, lime, pink, black)'),
  }))
  .output(z.object({
