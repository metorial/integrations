...
export class TrelloClient {
  ...
  async getBoard(boardId: string, fields?: string): Promise<any> {
    let response = await this.http.get(`/boards/${boardId}`, {
      params: this.params({ fields: fields || 'id,name,desc,closed,url,shortUrl,idOrganization,prefs,labelNames' }),
    });
  ...
  async addBoardMember(boardId: string, memberId: string, memberType: string = 'normal'): Promise<any> {
  ...
  async getCard(cardId: string, fields?: string): Promise<any> {
    let response = await this.http.get(`/cards/${cardId}`, {
      params: this.params({
        fields: fields || 'id,name,desc,closed,url,shortUrl,idBoard,idList,idMembers,idLabels,due,dueComplete,start,pos,cover',
        members: true,
  ...
  async getCards(listId: string, fields?: string): Promise<any[]> {
    let response = await this.http.get(`/lists/${listId}/cards`, {
      params: this.params({
        fields: fields || 'id,name,desc,closed,url,idBoard,idList,idMembers,idLabels,due,dueComplete,start,pos',
      }),
    });
    return response.data;
  }
  ...
  async getBoardCards(boardId: string, filter?: string): Promise<any[]> {
    let response = await this.http.get(`/boards/${boardId}/cards`, {
      params: this.params({
        filter: filter || 'open',
        fields: 'id,name,desc,closed,url,idBoard,idList,idMembers,idLabels,due,dueComplete,start,pos',
      }),
    });
    return response.data;
  }
  ...
  async addCheckItem(checklistId: string, name: string, checked?: boolean, pos?: string): Promise<any> {
  ...
  async updateCheckItem(cardId: string, checkItemId: string, data: Record<string, any>): Promise<any> {
  ...
  async getNotifications(memberId: string = 'me', options?: {
    readFilter?: string;
    limit?: number;
  }): Promise<any[]> {
  ...
  async setCardCustomFieldValue(cardId: string, customFieldId: string, value: Record<string, any>): Promise<any> {
