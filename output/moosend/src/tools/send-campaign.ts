...
export let sendCampaign = SlateTool.create(spec, {
  ...
  .input(z.object({
    ...
    action: z.enum(['send', 'schedule', 'unschedule', 'send_test']).describe('Action to perform on the campaign'),
    scheduleDateTime: z.string().optional().describe('Date and time to schedule the campaign (e.g. "2024-06-15T10:00:00"). Required when action is "schedule".'),
    scheduleTimezone: z.string().optional().describe('Timezone for the scheduled send (e.g. "Eastern Standard Time"). Defaults to account timezone.'),
    testEmails: z.array(z.string()).optional().describe('Email addresses for test send (max 5). Required when action is "send_test".'),
  }))
  .output(z.object({
    campaignId: z.string().describe('ID of the campaign'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (action) {
      ...
      case 'schedule':
        ...
        await client.scheduleCampaign(campaignId, ctx.input.scheduleDateTime, ctx.input.scheduleTimezone);
        break;
      case 'unschedule':
