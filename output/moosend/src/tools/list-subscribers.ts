...
export let listSubscribers = SlateTool.create(spec, {
  ...
  .input(z.object({
    ...
    status: z.enum(['Subscribed', 'Unsubscribed', 'Bounced', 'Removed']).describe('Filter subscribers by status'),
    ...
    since: z.string().optional().describe('Only return subscribers added/changed since this date (e.g. "2024-01-01")'),
  }))
  .output(z.object({
