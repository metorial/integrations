...
let mailingListSchema = z.object({
  ...
  segmentId: z.string().optional().describe('Optional segment ID to filter recipients within the list'),
});
...
export let createCampaign = SlateTool.create(spec, {
  ...
  .input(z.object({
    ...
    mailingLists: z.array(mailingListSchema).min(1).describe('Mailing lists to send the campaign to'),
    webLocation: z.string().optional().describe('URL to hosted HTML email content'),
    confirmationToEmail: z.string().optional().describe('Email to receive campaign send confirmation'),
    isAB: z.boolean().optional().default(false).describe('Whether this is an A/B test campaign'),
    abCampaignType: z.enum(['Subject', 'Content', 'Sender']).optional().describe('Type of A/B test variation'),
    ...
    abWinnerSelectionType: z.enum(['OpenRate', 'TotalUniqueClicks']).optional().describe('How to determine the A/B test winner'),
    trackInGoogleAnalytics: z.boolean().optional().describe('Enable Google Analytics tracking'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.confirmationToEmail) body.ConfirmationToEmail = ctx.input.confirmationToEmail;
    ...
    if (ctx.input.listPercentage !== undefined) body.ListPercentage = String(ctx.input.listPercentage);
    if (ctx.input.abWinnerSelectionType) body.ABWinnerSelectionType = ctx.input.abWinnerSelectionType;
    if (ctx.input.trackInGoogleAnalytics !== undefined) body.TrackInGoogleAnalytics = String(ctx.input.trackInGoogleAnalytics);
    if (ctx.input.dontTrackLinkClicks !== undefined) body.DontTrackLinkClicks = String(ctx.input.dontTrackLinkClicks);
