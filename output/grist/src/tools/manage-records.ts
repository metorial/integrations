...
export let fetchRecords = SlateTool.create(
  spec,
  {
    name: 'Fetch Records',
    key: 'fetch_records',
  ...
  .input(z.object({
    ...
    filter: z.record(z.string(), z.array(z.any())).optional().describe('Filter by column values: keys are column IDs, values are arrays of allowed values'),
    sort: z.string().optional().describe('Sort specification: comma-separated column IDs, prefix with "-" for descending'),
...
export let addRecords = SlateTool.create(
  spec,
  {
    name: 'Add Records',
    key: 'add_records',
  ...
  .input(z.object({
    ...
    noparse: z.boolean().optional().describe('If true, skip Grist type parsing and accept raw values'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.addRecords(ctx.input.documentId, ctx.input.tableId, ctx.input.records, ctx.input.noparse);
...
export let updateRecords = SlateTool.create(
  ...
  .input(z.object({
    ...
    records: z.array(z.object({
      ...
      fields: z.record(z.string(), z.any()).describe('Field values to update, keyed by column ID'),
...
export let addOrUpdateRecords = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    tableId: z.string().describe('Table ID'),
    records: z.array(z.object({
      require: z.record(z.string(), z.any()).describe('Column values to match against existing records'),
      fields: z.record(z.string(), z.any()).optional().describe('Column values to set on matched or new records'),
    })).describe('Records to upsert'),
    onmany: z.enum(['first', 'none', 'all']).optional().describe('When multiple records match: "first" updates the first, "none" skips, "all" updates all'),
    noadd: z.boolean().optional().describe('If true, do not add new records when no match is found'),
    ...
    allowEmptyRequire: z.boolean().optional().describe('If true, allow empty require objects (matches all records)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    await client.addOrUpdateRecords(ctx.input.documentId, ctx.input.tableId, ctx.input.records, {
