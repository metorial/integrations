...
export let newEmail = SlateTrigger.create(
  spec,
  {
    ...
    key: 'new_email',
    description: 'Triggers when an email is sent from or received into a shared mailbox. Covers both inbound and outbound email activity.',
  }
)
  .input(z.object({
    ...
    tags: z.array(z.record(z.string(), z.unknown())).optional().describe('Tags on the conversation'),
  ...
  .output(z.object({
    ...
    tags: z.array(z.record(z.string(), z.unknown())).optional().describe('Conversation tags'),
    timestamp: z.string().optional().describe('Timestamp of the email event'),
  }))
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let inputs = events.map((event: Record<string, any>) => {
        ...
        let conversationId = String(conversation.conversation_id ?? conversation.id ?? event.id ?? '');
