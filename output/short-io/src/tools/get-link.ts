...
export let getLink = SlateTool.create(
  ...
  .input(z.object({
    ...
    domain: z.string().optional().describe('Domain hostname for path or original URL lookups. Falls back to config default.'),
    path: z.string().optional().describe('Short URL path/slug to expand.'),
    originalURL: z.string().optional().describe('Original destination URL to find the link for.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.linkId) {
      ...
      if (!domain) {
        throw new Error('Domain is required for path lookup. Provide it in the input or set a default in config.');
      }
      link = await client.expandLink(domain, ctx.input.path);
    } else if (ctx.input.originalURL) {
      let domain = ctx.input.domain || ctx.config.domain;
      if (!domain) {
        throw new Error('Domain is required for original URL lookup. Provide it in the input or set a default in config.');
      }
      link = await client.getLinkByOriginalUrl(domain, ctx.input.originalURL);
    } else {
      throw new Error('Provide either linkId, domain + path, or domain + originalURL to look up a link.');
    }
