...
export let manageSegment = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['create', 'delete', 'activate', 'deactivate', 'upload_keys', 'remove_keys', 'get_keys']).describe('Operation to perform on the segment.'),
    segmentName: z.string().describe('Name of the segment.'),
    trafficTypeId: z.string().optional().describe('Traffic type ID (required for "create" action).'),
    description: z.string().optional().describe('Segment description (for "create" action).'),
    environmentId: z.string().optional().describe('Environment ID (required for activate, deactivate, upload_keys, remove_keys, get_keys).'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'create': {
        ...
        if (!ctx.input.trafficTypeId) throw new Error('trafficTypeId is required to create a segment.');
      ...
      case 'upload_keys': {
        ...
        if (!ctx.input.keys || ctx.input.keys.length === 0) throw new Error('keys array is required and must not be empty.');
        await client.uploadSegmentKeys(envId, ctx.input.segmentName, ctx.input.keys);
        return {
          output: { segmentName: ctx.input.segmentName, action: 'upload_keys', success: true, keyCount: ctx.input.keys.length },
      ...
      case 'remove_keys': {
        ...
        if (!ctx.input.keys || ctx.input.keys.length === 0) throw new Error('keys array is required and must not be empty.');
        await client.removeSegmentKeys(envId, ctx.input.segmentName, ctx.input.keys, ctx.input.comment);
        return {
          output: { segmentName: ctx.input.segmentName, action: 'remove_keys', success: true, keyCount: ctx.input.keys.length },
      ...
      case 'get_keys': {
        ...
        return {
          output: { segmentName: ctx.input.segmentName, action: 'get_keys', success: true, keys, keyCount: keys.length },
