...
export let manageUsers = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['invite', 'list', 'get', 'update', 'delete']).describe('Action to perform.'),
    ...
    status: z.string().optional().describe('User status filter (for list) or new status (for update). Values: PENDING, ACTIVE, DEACTIVATED.'),
    twoFactorAuth: z.boolean().optional().describe('Two-factor auth setting (for update).'),
    groupIds: z.array(z.string()).optional().describe('Group IDs to assign the user to (for invite).'),
