...
let bucketInputSchema = z.object({
  ...
  size: z.number().describe('Percentage allocation (0-100). All bucket sizes in a rule must sum to 100.'),
});

export let createFeatureFlag = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    trafficTypeName: z.string().describe('Traffic type name or ID to associate the flag with (e.g., "user").'),
    ...
    environmentId: z.string().optional().describe('If provided, creates the flag definition in this environment.'),
    treatments: z.array(treatmentInputSchema).optional().describe('Treatments for the flag definition. Required if environmentId is provided.'),
    defaultTreatment: z.string().optional().describe('The treatment served when no rules match. Required if environmentId is provided.'),
    defaultRule: z.array(bucketInputSchema).optional().describe('Default rule for percentage-based rollout. Required if environmentId is provided.'),
    trafficAllocation: z.number().optional().describe('Percentage of traffic to include in the flag (0-100). Defaults to 100.'),
  }))
  .output(z.object({
    flagId: z.string(),
    flagName: z.string(),
