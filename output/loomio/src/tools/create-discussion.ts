...
export let createDiscussion = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    description: z.string().optional().describe('Body content of the discussion in Markdown or HTML'),
    descriptionFormat: z.enum(['md', 'html']).default('md').describe('Format of the description content'),
    groupId: z.number().optional().describe('ID of the group to create the discussion in'),
    recipientUserIds: z.array(z.number()).optional().describe('User IDs to notify about this discussion'),
    recipientEmails: z.array(z.string()).optional().describe('Email addresses to notify about this discussion'),
    recipientAudience: z.string().optional().describe('Set to "group" to notify the entire group'),
  }))
  .output(z.object({
