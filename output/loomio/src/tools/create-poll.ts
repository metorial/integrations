...
export let createPoll = SlateTool.create(
  spec,
  {
    name: 'Create Poll',
    key: 'create_poll',
  ...
  .input(z.object({
    ...
    pollType: z.enum(['proposal', 'poll', 'count', 'score', 'ranked_choice', 'meeting', 'dot_vote']).describe('Type of poll to create'),
    details: z.string().optional().describe('Description/details of the poll in Markdown or HTML'),
    detailsFormat: z.enum(['md', 'html']).default('md').describe('Format of the details content'),
    closingAt: z.string().optional().describe('ISO 8601 datetime when the poll closes'),
    options: z.array(z.string()).optional().describe('Poll option names. For proposals: agree/disagree/abstain/block. For meetings: ISO 8601 datetimes. For others: arbitrary strings.'),
    groupId: z.number().optional().describe('ID of the group to create the poll in'),
    discussionId: z.number().optional().describe('ID of an existing discussion thread to attach the poll to'),
    anonymous: z.boolean().optional().describe('Whether votes are anonymous'),
    hideResults: z.enum(['off', 'until_vote', 'until_closed']).optional().describe('When to show results: off (always visible), until_vote (visible after voting), until_closed (visible after closing)'),
    shuffleOptions: z.boolean().optional().describe('Whether to randomize option order for each voter'),
    specifiedVotersOnly: z.boolean().optional().describe('Restrict voting to specified recipients only'),
    recipientUserIds: z.array(z.number()).optional().describe('User IDs to invite to the poll'),
    recipientEmails: z.array(z.string()).optional().describe('Email addresses to invite to the poll'),
    recipientAudience: z.string().optional().describe('Set to "group" to invite the entire group'),
    notifyRecipients: z.boolean().optional().describe('Whether to send notification emails to recipients'),
  }))
  .output(z.object({
