...
export let manageMemberships = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    removeAbsent: z.boolean().default(false).describe('If true, remove existing members whose emails are not in the provided list. Use with caution.'),
  }))
  .output(z.object({
    added: z.array(z.string()).describe('Email addresses that were newly added'),
    removed: z.array(z.string()).describe('Email addresses that were removed (only when removeAbsent is true)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.removeAbsent) {
      ctx.warn('removeAbsent is enabled — members not in the provided email list will be removed from the group.');
    }
