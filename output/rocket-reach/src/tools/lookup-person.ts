...
let emailSchema = z.object({
  ...
  lastValidationCheck: z.string().nullable().optional().describe('Timestamp of last validation'),
});
...
export let lookupPerson = SlateTool.create(
  spec,
  {
    name: 'Lookup Person',
    key: 'lookup_person',
...
This is the primary tool for **contact enrichment** — use it after searching for people to get their contact information.`,
  ...
  .input(z.object({
    name: z.string().optional().describe('Full name of the person. Must be used together with currentEmployer.'),
    currentEmployer: z.string().optional().describe('Current employer of the person. Must be used together with name.'),
    linkedinUrl: z.string().optional().describe('LinkedIn profile URL for direct lookup'),
    profileId: z.number().optional().describe('RocketReach profile ID from a previous search'),
  ...
  .output(z.object({
    ...
    status: z.string().optional().describe('Lookup status: complete, progress, searching, or not queued'),
    ...
    currentEmployerIndustry: z.string().nullable().optional().describe('Current employer industry'),
    ...
    recommendedProfessionalEmail: z.string().nullable().optional().describe('Recommended professional email'),
    recommendedPersonalEmail: z.string().nullable().optional().describe('Recommended personal email'),
    emails: z.array(emailSchema).optional().describe('All known email addresses with validation details'),
    ...
    links: z.record(z.string(), z.any()).nullable().optional().describe('Social media and other profile links'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let statusMessage = result.status === 'complete'
    ...
    let contactSummary = result.status === 'complete'
