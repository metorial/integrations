...
Use this to proactively check email validity before sending, clean individual addresses, or validate emails at point of capture.`,
  ...
  .input(z.object({
    ...
    timeout: z.number().optional().describe('Maximum time in milliseconds for verification (e.g., 30000 for 30 seconds)'),
  }))
  .output(z.object({
    emailAddress: z.string().describe('The email address that was verified'),
    status: z.string().describe('Verification status: valid, invalid, unknown, or catch_all'),
    subStatus: subStatusSchema.describe('Sub-status with code and description for more detail'),
    ...
    role: z.string().describe('Whether the email is a role-based address like admin@ or support@ (yes/no)'),
    gibberish: z.string().describe('Whether the email appears to be gibberish (yes/no)'),
    aiVerdict: z.string().optional().describe('AI-generated verdict about email deliverability'),
    suggestedEmailAddress: z.string().optional().describe('Suggested correction if a typo is detected'),
    bounceType: z.string().optional().describe('Type of bounce, if applicable'),
    detailInfo: detailInfoSchema.describe('Detailed info including account, domain, MX record, and SMTP provider'),
    verifiedOn: z.string().optional().describe('Timestamp of when the verification occurred'),
