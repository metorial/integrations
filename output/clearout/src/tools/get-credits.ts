...
Returns available credits, daily verify limit, reset date, and low balance threshold.`,
  ...
  .output(z.object({
    ...
    availableDailyVerifyLimit: z.number().optional().describe('Remaining daily verification limit'),
    resetDailyVerifyLimitDate: z.string().optional().describe('Date/time when the daily verify limit resets'),
    lowCreditBalanceMinThreshold: z.number().optional().describe('Minimum threshold for low credit balance warning'),
  }))
