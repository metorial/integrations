...
let fileSourceSchema = z.object({
  ...
  fileId: z.string().optional().describe('ConvertAPI file ID of a previously uploaded or converted file'),
...
let convertedFileSchema = z.object({
  ...
  fileId: z.string().nullable().describe('ConvertAPI file ID (available when storeFile is true)'),
...
Supports format-specific parameters like page size, orientation, quality, and image dimensions.`,
  ...
  .input(z.object({
    sourceFormat: z.string().describe('Source file format extension (e.g., "docx", "html", "png")'),
    destinationFormat: z.string().describe('Target file format extension (e.g., "pdf", "jpg", "xlsx")'),
    file: fileSourceSchema,
    storeFile: z.boolean().optional().default(false).describe('Store converted file on ConvertAPI server for chaining or download'),
    conversionParameters: z.record(z.string(), z.string()).optional().describe('Format-specific conversion parameters (e.g., {"PageSize": "A4", "Quality": "80"})'),
  }))
  .output(z.object({
