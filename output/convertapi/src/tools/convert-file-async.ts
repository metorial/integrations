...
export let convertFileAsync = SlateTool.create(
  spec,
  {
...
Use this for large files or long-running conversions to avoid timeouts. Retrieve results with the **Get Async Job Result** tool.`,
    ...
    constraints: [
      'Job results are available for 3 hours after completion.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    ...
    storeFile: z.boolean().optional().default(true).describe('Store converted file on ConvertAPI server for download'),
    conversionParameters: z.record(z.string(), z.string()).optional().describe('Format-specific conversion parameters'),
  }))
  .output(z.object({
