...
export let chatQuery = SlateTool.create(
  spec,
  {
...
Use this to understand code, find implementations, explain architecture, list PRs, generate summaries, and more. Supports slash commands like \`/findCode\`, \`/explain\`, \`/listPRs\`, \`/issueThemes\`, \`/onboard\`, and \`/visualize\`.`,
  ...
  .input(z.object({
    ...
    enableArtifacts: z.boolean().optional().describe('Enable artifact generation in responses (e.g., code snippets, diagrams)'),
    advancedAgent: z.boolean().optional().describe('Enable advanced agent mode for complex cross-source reasoning across PRs, issues, and code'),
    enableDocs: z.boolean().optional().default(true).describe('Include documentation context in responses'),
    limitSources: z.number().optional().describe('Maximum number of source references to include (default: 5)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let refSummary = result.references.length > 0
