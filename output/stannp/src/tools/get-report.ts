...
export let getReportSummary = SlateTool.create(
  spec,
  {
...
export let listMailpieces = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    startDate: z.string().describe('Start date in YYYY-MM-DD format'),
    endDate: z.string().describe('End date in YYYY-MM-DD format (inclusive)'),
    status: z.string().optional().describe('Filter by status (e.g. delivered, returned, cancelled)'),
    tag: z.string().optional().describe('Filter by tag'),
  }))
  .output(z.object({
    mailpieces: z.array(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let mailpieces = Array.isArray(result) ? result.map((m: any) => ({
