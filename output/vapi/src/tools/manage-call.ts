...
export let manageCall = SlateTool.create(
  ...
  .input(z.object({
    ...
    assistantId: z.string().optional().describe('Assistant ID to use for the call (for create)'),
    squadId: z.string().optional().describe('Squad ID to use for the call (for create)'),
    workflowId: z.string().optional().describe('Workflow ID to use for the call (for create)'),
    phoneNumberId: z.string().optional().describe('Vapi phone number ID to call from (for outbound calls)'),
    customerNumber: z.string().optional().describe('Customer phone number to call (E.164 format, for outbound calls)'),
    customerSipUri: z.string().optional().describe('Customer SIP URI to call (for outbound SIP calls)'),
    customerName: z.string().optional().describe('Customer name for context'),
    scheduledAt: z.string().optional().describe('ISO 8601 timestamp to schedule the call for a future time'),
    maxDurationSeconds: z.number().optional().describe('Maximum call duration in seconds'),
    serverUrl: z.string().optional().describe('Server URL for receiving webhook events for this call')
  }))
  .output(z.object({
    callId: z.string().optional().describe('ID of the call'),
    type: z.string().optional().describe('Call type (inboundPhoneCall, outboundPhoneCall, webCall)'),
    status: z.string().optional().describe('Call status (scheduled, queued, ringing, in-progress, forwarding, ended)'),
