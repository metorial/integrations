...
export let getExtractedData = SlateTool.create(
  ...
  .input(z.object({
    docId: z.string().describe('Unique identifier of the document to retrieve extracted data from'),
  }))
  .output(z.object({
    sections: z.record(z.string(), z.any()).describe('Extracted data organized by sections (e.g., "Basic Information", "Buyer Detail", "Line Items")'),
    metaData: z.record(z.string(), z.any()).describe('Document metadata including status, title, type, and processing information'),
  }))
