...
export let uploadDocument = SlateTool.create(
  ...
  .input(z.object({
    ...
    fileUrl: z.string().optional().describe('Public URL of the file to upload. Required when source is "url".'),
    base64Content: z.string().optional().describe('Base64-encoded file content. Required when source is "base64".'),
    filename: z.string().optional().describe('Filename with extension. Required when source is "base64", optional for URL uploads.'),
    documentType: z.string().describe('Document type for processing (e.g., "invoice", "bank_statements", "passport_front")'),
    userDocId: z.string().optional().describe('Custom identifier for internal tracking'),
    docMetaData: z.string().optional().describe('JSON string with additional metadata to attach to the document'),
    generateReviewUrl: z.boolean().optional().describe('If true, includes a temporary review URL in the response'),
