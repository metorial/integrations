...
export let manageItemGroup = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    groupId: z.string().optional().describe('Group ID to update (omit to create a new group)'),
    name: z.string().optional().describe('Group name'),
    description: z.string().optional().describe('Group description'),
    sortOrder: z.number().optional().describe('Sort order position'),
    autoExpand: z.boolean().optional().describe('Whether the group auto-expands in the checkout'),
  }))
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.groupId) {
      ...
      if (ctx.input.sortOrder !== undefined) patches.push({ op: 'replace', path: '/sortorder', value: ctx.input.sortOrder });
      if (ctx.input.autoExpand !== undefined) patches.push({ op: 'replace', path: '/autoexpand', value: ctx.input.autoExpand });
