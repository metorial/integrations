...
export let createEditorExpress = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    showAnonymousSigners: z.boolean().optional().describe('Show anonymous signer options in the editor'),
    showMeSigner: z.boolean().optional().describe('Show "me" signer option in the editor'),
    showSigningOptions: z.boolean().optional().describe('Show signing options panel in the editor'),
    showTeamMembers: z.boolean().optional().describe('Show team members list in the editor')
  }))
  .output(z.object({
    editorExpressDetails: z.any().describe('EditorExpress session details including editor access URL')
  }))
...
export let getEditorExpress = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
    let editorExpressDetails = await client.retrieveEditorExpress(ctx.input.editorExpressTokenId);
...
export let removeEditorExpress = SlateTool.create(
  spec,
  {
