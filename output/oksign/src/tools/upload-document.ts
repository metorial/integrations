...
export let uploadDocument = SlateTool.create(
  ...
  .input(z.object({
    ...
    filename: z.string().describe('Filename including extension (e.g. "contract.pdf"). Must be US-ASCII only.'),
    contentType: z.enum([
      'application/pdf',
      'application/msword',
  ...
  .output(z.object({
    documentId: z.string().describe('Unique document identifier assigned by OKSign, used for all subsequent operations')
  }))
