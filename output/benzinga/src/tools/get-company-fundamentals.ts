...
export let getCompanyFundamentalsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    dataType: z.enum(['overview', 'profile', 'valuationRatios', 'financials']).optional().default('overview').describe('Type of fundamentals data to retrieve'),
    asOf: z.string().optional().describe('As-of date for historical data (YYYY-MM-DD)'),
    period: z.string().optional().describe('Financial period for statements (e.g. "3M", "12M")'),
    reportType: z.string().optional().describe('Report type filter for financial statements'),
  }))
  .output(z.object({
    fundamentals: z.array(z.record(z.string(), z.any())).describe('Array of fundamentals data records'),
