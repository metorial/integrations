...
export let initializeTransaction = SlateTool.create(
  spec,
  {
...
Amounts are in the **smallest currency unit** (e.g., kobo for NGN: NGN 100 = 10000).`,
  ...
  .input(z.object({
    ...
    amount: z.number().describe('Amount in smallest currency unit (e.g., kobo). NGN 100 = 10000'),
    currency: z.string().optional().describe('Currency code (e.g., NGN, GHS, ZAR, KES, USD)'),
    reference: z.string().optional().describe('Unique transaction reference. Auto-generated if not provided'),
    callbackUrl: z.string().optional().describe('URL to redirect to after payment'),
    metadata: z.record(z.string(), z.any()).optional().describe('Custom metadata to attach to the transaction'),
    channels: z.array(z.string()).optional().describe('Payment channels to use (e.g., card, bank, ussd, qr, mobile_money, bank_transfer)'),
    ...
    transactionCharge: z.number().optional().describe('Flat fee (in smallest currency unit) to charge the subaccount for this transaction'),
    bearer: z.enum(['account', 'subaccount']).optional().describe('Who bears the Paystack charges'),
  }))
  .output(z.object({
