...
export let listDisputes = SlateTool.create(
  spec,
  {
    name: 'List Disputes',
    key: 'list_disputes',
  ...
  .input(z.object({
    ...
    status: z.string().optional().describe('Filter by dispute status (e.g., awaiting-merchant-feedback, resolved)'),
...
export let resolveDispute = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    resolution: z.enum(['merchant-accepted', 'declined']).describe('Resolution: merchant-accepted (accept with refund) or declined (reject with evidence)'),
    message: z.string().describe('Message/evidence for the resolution'),
    refundAmount: z.number().optional().describe('Refund amount if accepting (in smallest currency unit)'),
