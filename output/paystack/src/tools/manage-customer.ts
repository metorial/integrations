...
export let createCustomer = SlateTool.create(
  ...
  .input(z.object({
    ...
    metadata: z.record(z.string(), z.any()).optional().describe('Custom metadata for the customer'),
  }))
  .output(z.object({
...
export let getCustomer = SlateTool.create(
  ...
  .output(z.object({
    ...
    totalTransactionValue: z.array(z.any()).optional().describe('Total transaction values per currency'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new PaystackClient({ token: ctx.auth.token });
...
export let updateCustomer = SlateTool.create(
  ...
  .input(z.object({
    ...
    riskAction: z.enum(['default', 'allow', 'deny']).optional().describe('Set risk action: allow (whitelist), deny (blacklist), or default'),
  }))
  .output(z.object({
