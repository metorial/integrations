...
export let listPeople = SlateTool.create(
  ...
  .input(z.object({
    query: z.string().optional().describe('Search query to filter people by name, email, or phone'),
    page: z.number().optional().describe('Page number (starts at 1)'),
    perPage: z.number().optional().describe('Number of results per page (max 250)'),
    includes: z.string().optional().describe('Comma-separated list of related data to include (e.g., "companies,tags")'),
  }))
  .output(z.object({
