...
let themeColorsSchema = z.object({
...
}).optional().describe('Theme color settings');

let themeFontSchema = z.string().optional().describe('Font family name');

export let manageTheme = SlateTool.create(
  spec,
  {
    name: 'Manage Theme',
    key: 'manage_theme',
  ...
  .input(z.object({
    themeId: z.string().optional().describe('Theme ID (required for retrieve, update, and delete)'),
    ...
    hasTransparentButton: z.boolean().optional().describe('Whether the submit button is transparent'),
    roundedCorners: z.boolean().optional().describe('Whether to use rounded corners on buttons'),
  }))
  .output(z.object({
    ...
    }).optional().describe('Theme colors'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.hasTransparentButton !== undefined) themeData.has_transparent_button = ctx.input.hasTransparentButton;
    if (ctx.input.roundedCorners !== undefined) themeData.rounded_corners = ctx.input.roundedCorners;
    ...
    throw new Error('Provide either a name to create a theme, or a themeId to retrieve/update/delete one.');
  }).build();
