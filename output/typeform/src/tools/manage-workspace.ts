...
export let manageWorkspace = SlateTool.create(
  ...
  .input(z.object({
    workspaceId: z.string().optional().describe('Workspace ID (required for retrieve, update, and delete)'),
    ...
    removeMembers: z.array(z.string()).optional().describe('Email addresses of members to remove'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    // Update
    if (ctx.input.workspaceId && (ctx.input.name || ctx.input.addMembers || ctx.input.removeMembers)) {
      let operations: any[] = [];
      if (ctx.input.name) {
    ...
    throw new Error('Provide either a name to create a workspace, or a workspaceId to retrieve/update/delete one.');
  }).build();
