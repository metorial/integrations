...
export let manageTranslation = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    translationPayload: z.any().optional().describe('Translation payload object to set for the language'),
    autoTranslate: z.boolean().optional().describe('Set to true to auto-translate the form into the specified language'),
    delete: z.boolean().optional().describe('Set to true to delete the translation for the specified language'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Translation statuses for all languages'),
    translation: z.any().optional().describe('Translation payload for the requested language'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.translationPayload && ctx.input.language) {
      let result = await client.updateTranslation(ctx.input.formId, ctx.input.language, ctx.input.translationPayload);
      return {
        output: {
          formId: ctx.input.formId,
          language: ctx.input.language,
