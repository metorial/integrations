...
let fieldInputSchema = z.object({
  ...
  type: z.enum([
    ...
    'dropdown', 'rating', 'opinion_scale', 'file_upload', 'number',
    'website', 'yes_no', 'legal', 'statement', 'picture_choice',
    'phone_number', 'group', 'payment',
  ]).describe('Field type'),
  ...
  })).optional().describe('Choices for multiple_choice, dropdown, or picture_choice fields'),
  properties: z.any().optional().describe('Additional field-specific properties (e.g. allowMultipleSelections, steps for rating, shape for rating)'),
});
...
export let createForm = SlateTool.create(
  ...
  .input(z.object({
    ...
    workspaceUrl: z.string().optional().describe('Workspace API URL to create the form in (e.g. https://api.typeform.com/workspaces/{id})'),
    ...
    welcomeScreenDescription: z.string().optional().describe('Description text for the welcome screen'),
    thankYouScreenTitle: z.string().optional().describe('Title for the thank you screen'),
    thankYouScreenMessage: z.string().optional().describe('Message for the thank you screen'),
    ...
    hiddenFields: z.array(z.string()).optional().describe('Hidden field keys to accept via URL parameters'),
  }))
  .output(z.object({
