...
export let formResponse = SlateTrigger.create(
  spec,
  {
    name: 'Form Response',
    key: 'form_response',
    description: 'Triggered when a form response is submitted (complete or partial). Receives the full response data including answers, hidden fields, scores, and metadata. Set the formId in config for automatic webhook registration.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    hiddenFields: z.record(z.string(), z.string()).optional().describe('Hidden field values passed via form URL'),
    calculatedScore: z.number().optional().describe('Calculated quiz score'),
    variables: z.array(z.object({
