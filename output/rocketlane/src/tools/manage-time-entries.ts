...
export let createTimeEntry = SlateTool.create(
  ...
  .input(z.object({
    ...
    userId: z.number().optional().describe('ID of the user logging time (defaults to API key owner)'),
...
export let listTimeEntries = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeCategories: z.boolean().optional().describe('Also return available time entry categories'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let timeEntries = Array.isArray(result) ? result : (result.timeEntries ?? result.data ?? []);

    let categories: any[] | undefined;
    if (ctx.input.includeCategories) {
      let catResult = await client.listTimeEntryCategories();
      categories = Array.isArray(catResult) ? catResult : (catResult.categories ?? catResult.data ?? []);
    }
