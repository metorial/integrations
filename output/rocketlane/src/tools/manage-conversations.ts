...
export let createConversation = SlateTool.create(
  ...
  .input(z.object({
    ...
    })).optional().describe('Members to include in the conversation'),
  }))
  .output(z.object({
...
export let listConversations = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    let conversations = Array.isArray(result) ? result : (result.conversations ?? result.data ?? []);
...
export let createComment = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'Provide either conversationId or taskId, not both.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
...
export let listComments = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'Provide either conversationId or taskId to filter comments.',
    ],
    tags: {
      readOnly: true,
    },
