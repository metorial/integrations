...
export let createRunTool = SlateTool.create(
  spec,
  {
    name: 'Create Run',
    key: 'create_run',
  ...
  .input(z.object({
    ...
    isDestroy: z.boolean().optional().describe('Whether this is a destroy run (default: false)'),
    autoApply: z.boolean().optional().describe('Override auto-apply setting for this run'),
    planOnly: z.boolean().optional().describe('Create a speculative plan that cannot be applied (default: false)'),
    refreshOnly: z.boolean().optional().describe('Only refresh state without planning changes (default: false)'),
    configurationVersionId: z.string().optional().describe('Configuration version ID for API-driven runs'),
    targetAddrs: z.array(z.string()).optional().describe('Resource addresses to target (e.g., ["aws_instance.web"])'),
    replaceAddrs: z.array(z.string()).optional().describe('Resource addresses to force replace')
  }))
  .output(z.object({
    runId: z.string(),
    status: z.string(),
