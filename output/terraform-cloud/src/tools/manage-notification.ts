...
export let listNotificationsTool = SlateTool.create(
  spec,
  {
...
export let createNotificationTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    destinationType: z.enum(['generic', 'slack', 'microsoft-teams', 'email']).describe('Destination type for notifications'),
    url: z.string().optional().describe('Webhook URL (required for generic, slack, and microsoft-teams)'),
    webhookSecret: z.string().optional().describe('HMAC secret for generic webhook signature verification'),
    enabled: z.boolean().optional().describe('Whether the notification is enabled (default: true)'),
    triggers: z.array(z.string()).describe('List of event triggers (e.g., ["run:created", "run:completed", "run:errored"])'),
    emailAddresses: z.array(z.string()).optional().describe('Email addresses for email notifications'),
...
export let deleteNotificationTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    notificationConfigurationId: z.string().describe('The notification configuration ID to delete')
  }))
  .output(z.object({
    deleted: z.boolean()
  }))
