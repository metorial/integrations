...
export let runMonitoringTest = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    getResults: z.boolean().optional().describe('If true, get recent results instead of triggering a new run'),
    testRunId: z.string().optional().describe('Specific test run ID to get results for'),
    resultCount: z.number().optional().describe('Number of recent results to fetch (default: 10)'),
  }))
  .output(z.object({
    ...
    })).optional().describe('List of test run results'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.getResults) {
      if (ctx.input.testRunId) {
        let runDetails = await client.getTestRun(ctx.input.bucketKey, ctx.input.testId, ctx.input.testRunId);
        return {
          output: { runDetails },
    ...
    let result = await client.runTest(ctx.input.bucketKey, ctx.input.testId, ctx.input.environmentId);
    return {
      output: {
        triggered: true,
