...
export let manageVirtualService = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'update', 'delete', 'deploy', 'undeploy']).describe('Operation to perform'),
    workspaceId: z.number().optional().describe('Workspace ID (required for list/create)'),
    serviceId: z.number().optional().describe('Virtual service ID (required for update/delete/deploy/undeploy)'),
    ...
    thinkTime: z.number().optional().describe('Artificial delay in ms between request and response'),
    unmatchedRequestPolicy: z.enum(['error', 'redirect']).optional().describe('What to do with non-matching requests'),
    liveSystemUrl: z.string().optional().describe('URL of the live service for request redirection'),
  }))
  .output(z.object({
    ...
    })).optional().describe('List of virtual services'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'list') {
      if (!ctx.input.workspaceId) throw new Error('workspaceId is required for listing virtual services');
    ...
    if (ctx.input.action === 'update') {
      if (!ctx.input.serviceId) throw new Error('serviceId is required for updating a virtual service');
    ...
    if (ctx.input.action === 'deploy') {
      if (!ctx.input.serviceId) throw new Error('serviceId is required for deploying a virtual service');
    ...
    if (ctx.input.action === 'undeploy') {
      if (!ctx.input.serviceId) throw new Error('serviceId is required for undeploying a virtual service');
    ...
    if (ctx.input.action === 'delete') {
      if (!ctx.input.serviceId) throw new Error('serviceId is required for deleting a virtual service');
