...
let inboxTaskSchema = z.object({
  ...
  overallProcess: workdayReferenceSchema.optional().describe('The overall business process this task belongs to'),
  stepType: workdayReferenceSchema.optional().describe('The type of step in the business process'),
});

export let getInboxTasks = SlateTool.create(
  spec,
  {
...
export let actionInboxTask = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'approve') {
      result = await client.approveInboxTask(ctx.input.workerId, ctx.input.taskId, ctx.input.comment);
    } else {
      result = await client.denyInboxTask(ctx.input.workerId, ctx.input.taskId, ctx.input.comment);
    }
