...
export let transcriptionUpdated = SlateTrigger.create(
  spec,
  {
    name: 'Transcription State Changed',
    key: 'transcription_state_changed',
    description: 'Triggers when a transcription changes state (e.g. from transcribing to done, or to failed). Polls transcriptions in the organization and detects state transitions.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      if (!orgId) {
        ctx.warn('Organization ID is not configured. Cannot poll for transcription state changes.');
      ...
      let previousStates: Record<string, string> = (ctx.state as Record<string, string> | null) || {};
      let inputs: any[] = [];
