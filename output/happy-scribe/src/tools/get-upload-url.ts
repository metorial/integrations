...
export let getUploadUrl = SlateTool.create(
  ...
  .input(z.object({
    filename: z.string().describe('Name of the media file including extension (e.g. "interview.mp3", "meeting.wav").'),
  }))
  .output(z.object({
    signedUrl: z.string().describe('Temporary signed URL for uploading the file via PUT request.'),
  }))
