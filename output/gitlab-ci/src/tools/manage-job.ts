...
export let manageJob = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['get', 'log', 'retry', 'cancel', 'play', 'erase']).describe('Action to perform on the job'),
    variables: z.array(z.object({
      key: z.string(),
      value: z.string()
    })).optional().describe('Variables for "play" action on manual jobs')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let actionVerb = action === 'get' ? 'Fetched' : action === 'retry' ? 'Retried' : action === 'cancel' ? 'Canceled' : 'Played';
