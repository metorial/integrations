...
export let jobEvents = SlateTrigger.create(
  spec,
  {
    name: 'Job Events',
    key: 'job_events',
    description: 'Triggered when a CI/CD job status changes (e.g. created, pending, running, success, failed). Includes job details, stage, runner info, and associated pipeline/project.'
  }
)
  .input(z.object({
    eventType: z.string(),
