...
let chargeSchema = z.object({
  ...
  localPrice: z.object({
    amount: z.string(),
    currency: z.string(),
  }).optional().describe('Requested price'),
  hostedUrl: z.string().optional().describe('URL for the hosted payment page'),
  status: z.string().optional().describe('Current charge status (NEW, PENDING, COMPLETED, EXPIRED, etc.)'),
...
export let manageCommerceCharges = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['create', 'list', 'get', 'cancel', 'resolve']).describe('Operation to perform'),
    chargeCodeOrId: z.string().optional().describe('Charge code or ID (required for get, cancel, resolve)'),
    name: z.string().optional().describe('Name/title for the charge (required for create)'),
    description: z.string().optional().describe('Description of the charge (required for create)'),
    pricingType: z.enum(['fixed_price', 'no_price']).optional().describe('Pricing type (required for create)'),
    amount: z.string().optional().describe('Price amount (required for fixed_price)'),
    currency: z.string().optional().describe('Price currency, e.g., "USD" (required for fixed_price)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      ...
      let localPrice = ctx.input.pricingType === 'fixed_price' && ctx.input.amount && ctx.input.currency
...
let mapCharge = (c: any) => ({
  ...
  localPrice: c.local_price ? { amount: c.local_price.amount, currency: c.local_price.currency } : undefined,
