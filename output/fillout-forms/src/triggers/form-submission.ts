...
export let formSubmission = SlateTrigger.create(
  spec,
  {
    ...
    key: 'form_submission',
    description: 'Triggers when a new submission is received for a Fillout form. The webhook must be registered for a specific form.',
  }
)
  .input(z.object({
  ...
  .webhook({
    autoRegisterWebhook: async (ctx) => {
      ...
      if (!formId) {
        throw new Error('formId is required in state to register a webhook. Set formId in the trigger configuration.');
      }
