...
export let findSimilarDomains = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    similarDomains: z.array(z.string()).describe('List of competitor and similar domain names'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    return {
      output,
      message: output.similarDomains.length > 0
