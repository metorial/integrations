...
export let getSitemap = SlateTool.create(
  ...
  .input(z.object({
    channelName: z.string().describe('The channel/site name to retrieve the sitemap for (e.g., "website")'),
    format: z.enum(['flat', 'nested']).default('flat').describe('Sitemap format: "flat" for path-keyed dictionary or "nested" for hierarchical tree'),
  ...
  .output(z.object({
    sitemap: z.any().describe('Sitemap data — flat (object keyed by path) or nested (array of page nodes with children)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let sitemap = ctx.input.format === 'nested'
