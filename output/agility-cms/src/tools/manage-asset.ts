...
export let manageAsset = SlateTool.create(
  ...
  .input(z.object({
    operation: z.enum(['delete', 'move']).describe('Operation: "delete" to remove the asset, "move" to relocate it'),
    mediaId: z.number().describe('The media ID of the asset'),
    destinationFolder: z.string().optional().describe('Target folder path for move operations'),
  }))
  .output(z.object({
