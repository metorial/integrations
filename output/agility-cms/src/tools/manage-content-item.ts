...
export let manageContentItem = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    operation: z.enum(['save', 'delete']).describe('Operation to perform: "save" to create/update, "delete" to remove'),
    contentId: z.number().optional().describe('Content item ID. Required for update and delete. Omit for create.'),
    referenceName: z.string().optional().describe('Reference name of the content model. Required for save operations.'),
    fields: z.record(z.string(), z.any()).optional().describe('Content fields as key-value pairs. Required for save operations.'),
    properties: z.record(z.string(), z.any()).optional().describe('Optional item properties to set (e.g., itemOrder)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.referenceName) {
      item.properties = { ...(ctx.input.properties || {}), referenceName: ctx.input.referenceName };
