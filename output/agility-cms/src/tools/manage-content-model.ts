...
export let manageContentModel = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    operation: z.enum(['get', 'save', 'delete']).describe('Operation: "get" to retrieve, "save" to create/update, "delete" to remove'),
    modelId: z.number().optional().describe('Model ID. Required for delete. Can be used for get.'),
    referenceName: z.string().optional().describe('Model reference name. Can be used for get.'),
    modelData: z.record(z.string(), z.any()).optional().describe('Full model definition for save operations (includes fields, displayName, etc.)'),
  }))
  .output(z.object({
    model: z.record(z.string(), z.any()).optional().describe('The retrieved or saved model definition'),
