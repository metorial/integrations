...
export let contactEvent = SlateTrigger.create(
  spec,
  {
    name: 'Contact Event',
    key: 'contact_event',
    description: 'Triggered when a contact is created, or when tags are added to or removed from a contact.',
  }
)
  .input(z.object({
    eventType: z.string().describe('The webhook event type (contact_created, contact_tag_added, contact_tag_removed)'),
        ...
        inputs: [
          {
            ...
            eventTimestamp: event.timeStamp || contactData.createdAt || new Date().toISOString(),
            contactData,
          },
        ],
      };
