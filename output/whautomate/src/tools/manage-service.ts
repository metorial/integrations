...
export let manageService = SlateTool.create(
  ...
  .input(z.object({
    ...
    serviceType: z.enum(['APPOINTMENT', 'CLASS', 'ADD_ON']).optional().describe('Type of service'),
    ...
    numberOfParticipants: z.number().optional().describe('Max participants (for CLASS type)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      ...
      if (ctx.input.durationMinutes !== undefined) data.durationMinutes = ctx.input.durationMinutes;
      if (ctx.input.categoryId) data.categoryId = ctx.input.categoryId;
      if (ctx.input.staffIds) data.staffIds = ctx.input.staffIds;
      if (ctx.input.enableOnlineBooking !== undefined) data.enableOnlineBooking = ctx.input.enableOnlineBooking;
      if (ctx.input.enableOnlinePayment !== undefined) data.enableOnlinePayment = ctx.input.enableOnlinePayment;
      if (ctx.input.numberOfParticipants !== undefined) data.numberOfParticipants = ctx.input.numberOfParticipants;
