...
export let searchAppointments = SlateTool.create(
  ...
  .input(z.object({
    appointmentId: z.string().optional().describe('Get a specific appointment by ID. If provided, other filters are ignored.'),
    ...
    status: z.string().optional().describe('Filter by status (e.g., Booked, Cancelled, Visited, No-Show)'),
    page: z.number().optional().describe('Page number for pagination'),
    pageSize: z.number().optional().describe('Results per page'),
  }))
  .output(z.object({
    appointments: z.array(z.record(z.string(), z.any())).describe('Array of appointment records'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let appointments = Array.isArray(result) ? result : (result.appointments || result.data || []);
