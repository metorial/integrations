...
export let manageStaff = SlateTool.create(
  spec,
  {
    name: 'Manage Staff',
    key: 'manage_staff',
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'get_availability', 'create_block', 'delete_block']).describe('Action to perform'),
    staffId: z.string().optional().describe('Staff member ID (required for get, get_availability, create_block, delete_block)'),
    blockId: z.string().optional().describe('Availability block ID (required for delete_block)'),
    fromDate: z.string().optional().describe('Start date for availability blocks (YYYY-MM-DD, for get_availability)'),
    toDate: z.string().optional().describe('End date for availability blocks (YYYY-MM-DD, for get_availability)'),
    blockStartDate: z.string().optional().describe('Block start datetime (for create_block)'),
  ...
  .output(z.object({
    ...
    staffMembers: z.array(z.record(z.string(), z.any())).optional().describe('List of staff members'),
    availabilityBlocks: z.array(z.record(z.string(), z.any())).optional().describe('Staff availability blocks'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'get_availability') {
      ...
      let blocks = Array.isArray(result) ? result : (result.availabilityBlocks || result.data || []);
      return {
