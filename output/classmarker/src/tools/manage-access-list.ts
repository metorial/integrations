...
export let manageAccessList = SlateTool.create(
  ...
  .output(z.object({
    ...
    codesTotal: z.number().describe('Total number of codes in the access list after the operation'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let codesChanged = action === 'add'
      ? (data.num_codes_added || 0)
      : (data.num_codes_deleted || 0);
