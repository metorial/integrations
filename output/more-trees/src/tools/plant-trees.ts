...
export let plantTrees = SlateTool.create(
  spec,
  {
    name: 'Plant Trees',
    key: 'plant_trees',
  ...
  .input(z.object({
    quantity: z.number().int().positive().optional().describe('Number of trees to plant for yourself. Required when not gifting to others.'),
    recipients: z.array(z.object({
      email: z.string().optional().describe('Email address of the recipient'),
      accountCode: z.string().optional().describe('More Trees account code of the recipient'),
      name: z.string().describe('Name of the recipient'),
      quantity: z.number().int().positive().describe('Number of trees to gift to this recipient'),
    })).optional().describe('List of recipients when gifting trees to others. If provided, trees are gifted instead of planted for self.'),
    projectId: z.number().int().optional().describe('Project ID to plant in. Uses default project if omitted.'),
    treeId: z.number().int().optional().describe('Tree type ID to plant. Uses default tree if omitted.'),
    test: z.boolean().optional().describe('Enable test/dry-run mode to validate without persisting changes.'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Recipient details when gifting trees'),
  }))
