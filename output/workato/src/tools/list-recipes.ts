...
export let listRecipesTool = SlateTool.create(
  spec,
  {
    name: 'List Recipes',
    key: 'list_recipes',
  ...
  .input(z.object({
    ...
    running: z.boolean().optional().describe('Filter by running state (true=running, false=stopped)'),
    adapterNamesAny: z.string().optional().describe('Comma-separated connector names to filter by (matches any)'),
    page: z.number().optional().describe('Page number (default: 1)'),
    perPage: z.number().optional().describe('Results per page (max: 100)'),
    order: z.enum(['activity', 'default']).optional().describe('Sort order'),
    updatedAfter: z.string().optional().describe('Only return recipes updated after this ISO 8601 timestamp'),
  }))
  .output(z.object({
