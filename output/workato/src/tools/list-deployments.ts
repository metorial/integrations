...
export let listDeploymentsTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    environmentType: z.enum(['sandbox', 'test', 'stage', 'uat', 'preprod', 'prod']).optional().describe('Filter by target environment'),
    state: z.enum(['pending', 'success', 'failed']).optional().describe('Filter by deployment state'),
  }))
  .output(z.object({
    deployments: z.array(z.object({
      ...
      performedByName: z.string().nullable().describe('Person who performed the deployment'),
