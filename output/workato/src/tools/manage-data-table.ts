...
export let manageDataTableTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list_tables', 'get_table', 'create_table', 'delete_table', 'query_records', 'create_record', 'update_record', 'delete_record'])
    ...
    columns: z.array(z.object({
      type: z.string().describe('Column type (string, boolean, date, date_time, integer, number, file, relation)'),
    ...
    })).optional().describe('Column schema (for create_table)'),
    recordId: z.string().optional().describe('Record ID (for update_record/delete_record)'),
    recordData: z.record(z.string(), z.unknown()).optional().describe('Record field values (for create_record/update_record)'),
    selectColumns: z.array(z.string()).optional().describe('Columns to return (for query_records)'),
    where: z.record(z.string(), z.unknown()).optional().describe('Filter conditions (for query_records)'),
    ...
    continuationToken: z.string().optional().describe('Pagination token (for query_records)'),
  ...
  .output(z.object({
    ...
    tables: z.array(z.record(z.string(), z.unknown())).optional().describe('List of data tables'),
    ...
    nextContinuationToken: z.string().nullable().optional().describe('Token for next page of records'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'list_tables') {
      let result = await client.listDataTables({ page: ctx.input.page, perPage: ctx.input.perPage });
    ...
    if (action === 'create_table') {
      ...
      let result = await client.createDataTable({ name: tableName, folderId, schema: columns });
    ...
    if (action === 'update_record') {
      if (!recordId || !recordData) throw new Error('Record ID and data are required for update_record');
