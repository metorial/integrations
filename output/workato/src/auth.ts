...
export let auth = SlateAuth.create()
  ...
  .addTokenAuth({
    ...
    inputSchema: z.object({
      token: z.string().describe('Workato API client token (Bearer token). Create one under Workspace admin > API clients.'),
      dataCenter: z.enum(['us', 'eu', 'jp', 'sg', 'au']).default('us')
    ...
    getProfile: async (ctx: { output: { token: string }; input: { token: string; dataCenter: string } }) => {
