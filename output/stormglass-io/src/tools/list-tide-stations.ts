...
export let listTideStationsTool = SlateTool.create(
  spec,
  {
...
Use the bounding box filter to find stations in a specific region.`,
  ...
  .input(z.object({
    boundingBox: z.string().optional().describe('Geographic bounding box in format "topRightLat,topRightLng:bottomLeftLat,bottomLeftLng" (e.g., "60.0,30.0:50.0,20.0"). If omitted, returns all stations globally.'),
  }))
  .output(z.object({
