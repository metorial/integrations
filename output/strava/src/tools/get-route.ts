...
export let getRoute = SlateTool.create(
  spec,
  {
    name: 'Get Route',
    key: 'get_route',
  ...
  .input(z.object({
    ...
    includeStreams: z.boolean().optional().describe('Include route streams (latlng, distance, altitude)'),
    exportFormat: z.enum(['gpx', 'tcx']).optional().describe('Export route in GPX or TCX format')
  }))
  .output(z.object({
