...
export let deletePoints = SlateTool.create(
  spec,
  {
    name: 'Delete Points',
    key: 'delete_points',
  ...
  .input(z.object({
    ...
    pointIds: z.array(z.union([z.number(), z.string()])).optional().describe('Specific point IDs to delete'),
    filter: z.any().optional().describe('Filter condition for bulk deletion (uses Qdrant filter syntax with must/should/must_not clauses)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.deletePoints(ctx.input.collectionName, selector, ctx.input.wait ?? true);
