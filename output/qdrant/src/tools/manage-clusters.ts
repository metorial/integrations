...
export let manageClusters = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'delete', 'restart', 'suspend', 'unsuspend']).describe('Cluster operation to perform'),
    clusterId: z.string().optional().describe('Cluster ID (required for get, delete, restart, suspend, unsuspend)'),
    clusterName: z.string().optional().describe('Name for the new cluster (required for create)'),
    cloudProvider: z.string().optional().describe('Cloud provider ID (e.g., "aws", "gcp") for create'),
    ...
    deleteBackups: z.boolean().optional().describe('Also delete backups when deleting cluster'),
  }))
  .output(z.object({
    cluster: clusterSchema.optional().describe('Cluster details (for get/create actions)'),
    clusters: z.array(clusterSchema).optional().describe('List of clusters (for list action)'),
    success: z.boolean().describe('Whether the operation succeeded'),
  }))
  .handleInvocation(async (ctx) => {
    if (!ctx.auth.managementToken) {
      throw new Error('Cloud Management API key is required for cluster operations. Configure it in authentication.');
    }
    if (!ctx.config.accountId) {
      throw new Error('Account ID is required for cluster operations. Configure it in settings.');
    }
    ...
    if (ctx.input.action === 'create') {
      if (!ctx.input.clusterName || !ctx.input.cloudProvider || !ctx.input.region || !ctx.input.packageId) {
        throw new Error('clusterName, cloudProvider, region, and packageId are required for create');
      }
