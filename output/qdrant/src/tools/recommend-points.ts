...
export let recommendPoints = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    positive: z.array(z.any()).describe('Positive examples: point IDs (number/string) or raw vectors (number arrays)'),
    negative: z.array(z.any()).optional().describe('Negative examples: point IDs or raw vectors to move away from'),
    strategy: z.enum(['average_vector', 'best_score']).optional().describe('Scoring strategy (default: average_vector)'),
