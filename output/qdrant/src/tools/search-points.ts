...
export let searchPoints = SlateTool.create(
  spec,
  {
    name: 'Search Points',
    key: 'search_points',
  ...
  .input(z.object({
    ...
    queryVector: z.any().optional().describe('Query vector (array of numbers) or a complex query object (e.g., {fusion: "rrf"} for hybrid search)'),
    filter: z.any().optional().describe('Filter conditions to narrow search (Qdrant filter syntax)'),
    limit: z.number().optional().describe('Maximum number of results (default: 10)'),
    offset: z.number().optional().describe('Number of results to skip'),
    scoreThreshold: z.number().optional().describe('Minimum score threshold for results'),
    vectorName: z.string().optional().describe('Name of the vector space to search (for named vector collections)'),
    withPayload: z.boolean().optional().describe('Include payloads in results (default: true)'),
    withVector: z.boolean().optional().describe('Include vectors in results (default: false)'),
    prefetch: z.any().optional().describe('Prefetch configuration for hybrid/multi-stage search'),
    ...
    }).optional().describe('Search algorithm parameters'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.searchParams) {
      params = {};
      if (ctx.input.searchParams.hnswEf !== undefined) params.hnsw_ef = ctx.input.searchParams.hnswEf;
      if (ctx.input.searchParams.exact !== undefined) params.exact = ctx.input.searchParams.exact;
    }
