...
Use this to scrape structured information from web pages without needing to process raw screenshots. Useful for link previews, content extraction, SEO analysis, and page auditing.`,
  ...
  .input(z.object({
    ...
    openGraph: z.boolean().optional().describe('Parse Open Graph tags (og:title, og:description, og:image, etc.). Default: false'),
    ...
    imageSize: z.boolean().optional().describe('Return screenshot dimensions. Default: false'),
    content: z.boolean().optional().describe('Return the full page content. Default: false'),
    contentFormat: z.enum(['html', 'markdown']).optional().describe('Format for page content extraction. Default: html'),
    httpResponseStatusCode: z.boolean().optional().describe('Include the HTTP status code from the target site. Default: false'),
    httpResponseHeaders: z.boolean().optional().describe('Include HTTP response headers from the target site. Default: false'),

    // Page loading config
    waitUntil: z.enum(['load', 'domcontentloaded', 'networkidle0', 'networkidle2']).optional().describe('Page load event to wait for'),
    ...
    authorization: z.string().optional().describe('HTTP Authorization header for authenticated pages'),
  ...
  .output(z.object({
    ...
    metadata: z.record(z.string(), z.unknown()).optional().describe('Extracted metadata including requested fields (page_title, open_graph, icon, fonts, image_size, content, http_response_status_code, http_response_headers)'),
