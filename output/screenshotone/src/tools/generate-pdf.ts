...
Supports configurable paper size, orientation, margins, background printing, and single-page fitting. Includes all page customization features such as content blocking, script injection, dark mode, and device emulation.`,
  ...
  .input(z.object({
    ...
    // PDF options
    pdfPrintBackground: z.boolean().optional().describe('Include background colors and graphics. Default: false'),
    pdfFitOnePage: z.boolean().optional().describe('Fit all content onto a single page. Default: false'),
    pdfLandscape: z.boolean().optional().describe('Use landscape orientation. Default: false (portrait)'),
    pdfPaperFormat: z.enum(['a0', 'a1', 'a2', 'a3', 'a4', 'a5', 'a6', 'legal', 'letter', 'tabloid']).optional().describe('Paper size. Default: letter'),
    ...
    deviceScaleFactor: z.number().min(1).max(5).optional().describe('Device pixel ratio (1-5)'),
