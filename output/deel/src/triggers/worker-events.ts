...
export let workerEvents = SlateTrigger.create(
  spec,
  {
    name: 'Worker Events',
    key: 'worker_events',
    description: 'Triggered when worker/people events occur: created, updated, or deleted. Includes events for direct employees, contractors, and EOR employees.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    hiringType: z.string().optional().describe('Hiring type (contractor, employee, eor_employee)'),
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: {
          ...
          workerName: resource?.full_name ?? resource?.name ?? [resource?.first_name, resource?.last_name].filter(Boolean).join(' '),
