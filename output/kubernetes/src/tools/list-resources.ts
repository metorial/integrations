...
Use **labelSelector** and **fieldSelector** to filter results. Pagination is supported via **limit** and **continueToken**.`,
  ...
  .input(z.object({
    resourceType: z.string().describe(`Resource type to list. Supported: ${supportedTypes.join(', ')}`),
    namespace: z.string().optional().describe('Namespace to list resources from. Ignored for cluster-scoped resources.'),
    labelSelector: z.string().optional().describe('Label selector to filter resources (e.g. "app=nginx,tier=frontend")'),
    fieldSelector: z.string().optional().describe('Field selector to filter resources (e.g. "metadata.name=my-pod")'),
  ...
  .output(z.object({
    ...
    namespace: z.string().optional().describe('The namespace the resources were listed from'),
