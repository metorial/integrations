...
export let applyResource = SlateTool.create(
  spec,
  {
    name: 'Apply Resource',
    key: 'apply_resource',
...
Accepts a full resource manifest as a JSON object. The kind, apiVersion, and metadata.name are required.`,
  ...
  .input(z.object({
    manifest: z.any().describe('Full Kubernetes resource manifest as a JSON object. Must include apiVersion, kind, and metadata.name.'),
    namespace: z.string().optional().describe('Namespace to apply the resource in. Overrides metadata.namespace if provided.'),
  }))
  .output(z.object({
