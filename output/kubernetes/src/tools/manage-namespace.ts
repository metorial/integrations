...
export let manageNamespace = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    namespaceName: z.string().optional().describe('Name of the namespace (required for create/update)'),
    labels: z.record(z.string(), z.string()).optional().describe('Labels to set on the namespace'),
    annotations: z.record(z.string(), z.string()).optional().describe('Annotations to set on the namespace'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.patchResource('namespaces', namespaceName, patch, undefined, 'strategic');
    return {
      output: {
        namespaces: [{
          namespaceName: result.metadata.name,
