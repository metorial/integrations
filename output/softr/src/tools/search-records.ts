...
let filterConditionSchema: z.ZodType<any> = z.lazy(() => z.object({
  operator: z.string().describe('Filter operator (e.g., IS, IS_NOT, CONTAINS, GREATER_THAN, IS_EMPTY, AND, OR, IS_BETWEEN, etc.)'),
  field: z.string().optional().describe('Field ID to filter on (for binary/unary operators)'),
  value: z.unknown().optional().describe('Value to compare against (for binary operators)'),
  values: z.array(z.unknown()).optional().describe('Array of values (for IS_ONE_OF, IS_NOT_ONE_OF, HAS_ANY_OF, etc.)'),
  conditions: z.array(filterConditionSchema).optional().describe('Nested conditions (for AND/OR composite operators)'),
}));
...
- **Composite:** AND, OR (for combining multiple conditions)`,
  ...
  .input(z.object({
    ...
    }).optional().describe('Filter conditions for the search'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let paging = (ctx.input.offset !== undefined || ctx.input.limit !== undefined)
