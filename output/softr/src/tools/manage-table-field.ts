...
let fieldOutputSchema = z.object({
  ...
  allowMultipleEntries: z.boolean().optional().describe('Whether multiple entries are allowed'),
...
export let manageTableField = SlateTool.create(
  spec,
  {
...
- To **delete**: provide \`fieldId\` and set \`delete\` to true.`,
  ...
  .input(z.object({
    ...
    fieldId: z.string().optional().describe('ID of the field (required for get/update/delete)'),
    ...
    allowMultipleEntries: z.boolean().optional().describe('Whether to allow multiple entries (for add)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { databaseId, tableId, fieldId, name, type, options, description, allowMultipleEntries, required: isRequired, defaultValue } = ctx.input;
    ...
    throw new Error('Invalid input: provide fieldId (to get/update/delete) or name + type (to add).');
  })
  .build();
