...
let contactResultSchema = z.object({
  ...
  custom: z.record(z.string(), z.string()).optional().describe('Custom tracking fields from the original request'),
...
export let enrichmentCompleted = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers when a contact enrichment or reverse email lookup batch completes, is canceled, or runs out of credits. Fires once per batch with all results. You can also receive per-contact events in real-time.',
  }
)
  .input(z.object({
    eventType: z.enum(['batch_completed', 'contact_finished']).describe('Whether this is a batch completion or individual contact event'),
    enrichmentId: z.string().describe('Enrichment batch ID'),
    batchName: z.string().optional().describe('Name of the enrichment batch'),
    status: z.string().optional().describe('Batch status (FINISHED, CANCELED, CREDITS_INSUFFICIENT, etc.)'),
    contacts: z.array(contactResultSchema).optional().describe('Enriched contacts (for batch completion)'),
    contact: contactResultSchema.optional().describe('Single enriched contact (for contact_finished events)'),
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        id: ctx.input.eventType === 'contact_finished'
