...
export let listBuilds = SlateTool.create(
  ...
  .input(z.object({
    repoSlugOrId: z.string().optional().describe('Repository slug (e.g. "owner/repo") or numeric ID. If omitted, returns builds for the authenticated user.'),
    branchName: z.string().optional().describe('Filter builds by branch name.'),
    state: z.enum(['created', 'received', 'started', 'passed', 'failed', 'errored', 'canceled']).optional().describe('Filter builds by state.'),
    eventType: z.enum(['push', 'pull_request', 'api', 'cron']).optional().describe('Filter builds by triggering event type.'),
