...
export let manageCrons = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'There can be only one cron per branch on a repository.',
    ],
  }
)
  .input(z.object({
    action: z.enum(['list', 'create', 'get', 'delete']).describe('Action to perform.'),
    repoSlugOrId: z.string().optional().describe('Repository slug (e.g. "owner/repo") or numeric ID. Required for list and create.'),
    ...
    interval: z.enum(['daily', 'weekly', 'monthly']).optional().describe('Cron schedule interval. Required for create.'),
    dontRunIfRecentBuildExists: z.boolean().optional().describe('Skip the cron run if there was a recent build. Defaults to false.'),
  }))
  .output(z.object({
    crons: z.array(z.object({
      ...
      dontRunIfRecentBuildExists: z.boolean().describe('Whether to skip if recent build exists'),
    ...
    })).optional().describe('List of cron jobs'),
    cron: z.object({
      cronId: z.number().describe('Cron ID'),
      branchName: z.string().optional().describe('Branch name'),
      interval: z.string().describe('Schedule interval'),
      ...
      active: z.boolean().optional().describe('Whether the cron is active'),
      createdAt: z.string().optional().describe('Creation timestamp'),
    }).optional().describe('Cron job details'),
