...
export let getJobLog = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['get', 'delete']).default('get').describe('Whether to retrieve or delete the log.'),
    format: z.enum(['text', 'json']).default('text').describe('Output format for the log. Only applies when action is "get".'),
  }))
  .output(z.object({
    logId: z.number().optional().describe('Log ID'),
    content: z.string().nullable().optional().describe('Log content (text format or JSON stringified)'),
