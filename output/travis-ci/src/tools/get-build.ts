...
export let getBuild = SlateTool.create(
  ...
  .output(z.object({
    ...
    pullRequestTitle: z.string().nullable().optional().describe('Pull request title, if applicable'),
    pullRequestNumber: z.number().nullable().optional().describe('Pull request number, if applicable'),
    jobs: z.array(z.object({
      jobId: z.number().describe('Job ID'),
      state: z.string().optional().describe('Job state'),
    })).optional().describe('Jobs associated with this build'),
  }))
