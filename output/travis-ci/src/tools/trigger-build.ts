...
export let triggerBuild = SlateTool.create(
  ...
  .input(z.object({
    ...
    branch: z.string().optional().describe('Branch to build. Defaults to the repository default branch.'),
    message: z.string().optional().describe('Custom message for the build request.'),
    config: z.record(z.string(), z.any()).optional().describe('Custom build configuration that overrides .travis.yml settings.'),
  }))
  .output(z.object({
