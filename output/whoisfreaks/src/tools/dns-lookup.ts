...
export let dnsLookup = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Bulk DNS lookups support up to 100 domains per request.',
    ],
    tags: {
      destructive: false,
      readOnly: true,
  ...
  .input(z.object({
    domainName: z.string().optional().describe('Single domain name to look up DNS records for'),
    domainNames: z.array(z.string()).optional().describe('Array of domain names for bulk DNS lookup (max 100)'),
    recordType: z.string().default('all').describe('DNS record type(s): "a", "aaaa", "cname", "mx", "ns", "txt", "spf", "ptr", "soa", "all", or comma-separated like "a,mx,ns"'),
  }))
  .output(z.object({
