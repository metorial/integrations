...
export let sslCertificateLookup = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    domainName: z.string().describe('Domain name to retrieve the SSL certificate for (e.g. "example.com")'),
    includeChain: z.boolean().optional().describe('Whether to retrieve the full SSL certificate chain'),
    includeRaw: z.boolean().optional().describe('Whether to include the raw OpenSSL response'),
  }))
  .output(z.object({
