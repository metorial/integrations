...
export let executeQuery = SlateTool.create(
  ...
  .input(z.object({
    ...
    parameters: z.record(z.string(), z.string()).optional().describe('Dynamic parameters to pass to the query as key-value pairs'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let results = await client.executeQuery(ctx.input.queryId, ctx.input.parameters as Record<string, string> | undefined);
