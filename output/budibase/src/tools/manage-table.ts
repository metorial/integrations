...
export let manageTable = SlateTool.create(
  ...
  .input(z.object({
    ...
    primaryDisplay: z.string().optional().describe('Column name to use as the primary display value'),
    schema: z.record(z.string(), z.any()).optional().describe('Column definitions keyed by column name, e.g. { "Name": { "type": "string" }, "Age": { "type": "number" } }'),
  }))
  .output(z.object({
