...
let querySchema = z.object({
  allOr: z.boolean().optional().describe('Use OR logic instead of AND for combining filters'),
  string: z.record(z.string(), z.string()).optional().describe('Prefix matching on string fields'),
  fuzzy: z.record(z.string(), z.string()).optional().describe('Substring matching on string fields'),
  ...
  range: z.record(z.string(), z.object({ low: z.any(), high: z.any() })).optional().describe('Range filtering with low/high bounds'),
  empty: z.record(z.string(), z.string()).optional().describe('Filter for rows where column is empty'),
  notEmpty: z.record(z.string(), z.string()).optional().describe('Filter for rows where column is not empty'),
  oneOf: z.record(z.string(), z.array(z.any())).optional().describe('Match any value in the provided array'),
  contains: z.record(z.string(), z.array(z.any())).optional().describe('Array column contains all specified values'),
  notContains: z.record(z.string(), z.array(z.any())).optional().describe('Array column does not contain specified values'),
  containsAny: z.record(z.string(), z.array(z.any())).optional().describe('Array column contains any of the specified values'),
}).optional();

export let searchRows = SlateTool.create(
  spec,
  {
    name: 'Search Rows',
    key: 'search_rows',
  ...
  .input(z.object({
    ...
    bookmark: z.union([z.string(), z.number()]).optional().describe('Pagination cursor from a previous search result'),
    limit: z.number().optional().describe('Maximum number of rows to return'),
  }))
  .output(z.object({
    rows: z.array(z.record(z.string(), z.any())).describe('List of matching rows'),
    bookmark: z.union([z.string(), z.number()]).optional().describe('Cursor for the next page of results'),
