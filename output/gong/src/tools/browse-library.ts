...
export let browseLibrary = SlateTool.create(
  spec,
  {
    name: 'Browse Library',
    key: 'browse_library',
  ...
  .input(z.object({
    folderId: z.string().optional().describe('Folder ID to list calls from (omit to list all folders)'),
    workspaceId: z.string().optional().describe('Filter folders by workspace'),
  }))
  .output(z.object({
    folders: z.array(libraryFolderSchema).optional().describe('Library folders (when no folderId is provided)'),
    ...
    })).optional().describe('Calls in the specified folder'),
  }))
