...
let aiBusinessSchema = z.object({
  ...
  categories: z.array(z.object({
    alias: z.string(),
    title: z.string(),
  })).optional().describe('Business categories'),
...
export let aiChat = SlateTool.create(
  spec,
  {
    name: 'AI Chat',
    key: 'ai_chat',
    ...
    constraints: [
      'Requires a Premium plan API key.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    query: z.string().describe('Natural language query (e.g., "Find the best pizza places near me")'),
    chatId: z.string().optional().describe('Session ID from a previous response for multi-turn conversations'),
  ...
  .output(z.object({
    ...
    types: z.array(z.string()).optional().describe('Response types (e.g., "business_search")'),
    businesses: z.array(aiBusinessSchema).optional().describe('Businesses referenced in the response'),
  }))
