...
let eventSchema = z.object({
  ...
  endDate: z.string(),
  timezone: z.string(),
  statusCode: z.string().describe('Event status: onsale, offsale, canceled, postponed, rescheduled'),
...
export let searchEventsTool = SlateTool.create(
  spec,
  {
    name: 'Search Events',
    key: 'search_events',
  ...
  .input(z.object({
    keyword: z.string().optional().describe('Search keyword to match against event name, venue, or attraction'),
    attractionId: z.string().optional().describe('Filter by attraction ID'),
    venueId: z.string().optional().describe('Filter by venue ID'),
    classificationName: z.string().optional().describe('Filter by classification name (e.g., "Music", "Sports", "Arts & Theatre")'),
    ...
    countryCode: z.string().optional().describe('ISO country code (e.g., US, CA, GB, AU, MX, NZ)'),
    ...
    latlong: z.string().optional().describe('Latitude,longitude for geo search (e.g., "34.0522,-118.2437")'),
    radius: z.string().optional().describe('Radius for geo search (used with latlong)'),
    unit: z.string().optional().describe('Unit for radius: "miles" or "km" (default: miles)'),
    startDateTime: z.string().optional().describe('Start date/time filter in ISO 8601 UTC (e.g., 2025-01-01T00:00:00Z)'),
    endDateTime: z.string().optional().describe('End date/time filter in ISO 8601 UTC'),
    source: z.string().optional().describe('Source platform filter: ticketmaster, universe, frontgate, tmr'),
    sort: z.string().optional().describe('Sort order (e.g., date,asc | name,asc | relevance,desc | distance,asc)'),
