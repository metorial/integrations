...
export let reactivatePurchase = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    customerEmail: z.string().optional().describe('Customer email to send the reactivation to. Defaults to the email on the original purchase.'),
    refulfill: z.boolean().optional().describe('Set to true to re-send the fulfillment/download email'),
  }))
  .output(z.object({
