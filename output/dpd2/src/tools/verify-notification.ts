...
export let verifyNotification = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    notificationParams: z.record(z.string(), z.any()).describe('All parameters received from the DPD notification POST, passed through for verification'),
  }))
  .output(z.object({
