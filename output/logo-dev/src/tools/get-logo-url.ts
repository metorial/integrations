...
export let getLogoUrl = SlateTool.create(
  ...
  .input(z.object({
    identifier: z.string().describe('The lookup value — a domain (e.g., "shopify.com"), stock ticker (e.g., "AAPL"), crypto symbol (e.g., "BTC"), ISIN (e.g., "US0378331005"), or company name (e.g., "Shopify").'),
    lookupType: z.enum(['domain', 'ticker', 'crypto', 'isin', 'name']).default('domain').describe('Type of identifier being used for the logo lookup.'),
    format: z.enum(['jpg', 'png', 'webp']).optional().describe('Image format. Defaults to jpg.'),
    size: z.number().min(1).max(800).optional().describe('Image size in pixels (1–800). Defaults to 128.'),
    theme: z.enum(['dark', 'light']).optional().describe('Adjusts logo colors for visibility on dark or light backgrounds.'),
    greyscale: z.boolean().optional().describe('Desaturate the logo to greyscale.'),
    retina: z.boolean().optional().describe('Double the image resolution for high-density displays.'),
    fallback: z.enum(['monogram', '404']).optional().describe('Behavior when no logo is found. "monogram" returns a letter placeholder (default), "404" returns an HTTP 404.'),
  }))
  .output(z.object({
