...
export let auth = SlateAuth.create()
  ...
  .addCustomAuth({
    ...
    inputSchema: z.object({
      ...
      scopes: z.string().optional().describe('Space-separated list of scopes (e.g. "users.read users.write time_clock.read")'),
    }),
    getOutput: async (ctx: { input: { clientId: string; clientSecret: string; scopes?: string } }) => {
      let http = createAxios({
        baseURL: 'https://api.connecteam.com',
      });
    ...
    getProfile: async (ctx: { output: { token: string }; input: { clientId: string; clientSecret: string; scopes?: string } }) => {
      let http = createAxios({
        baseURL: 'https://api.connecteam.com',
        headers: {
