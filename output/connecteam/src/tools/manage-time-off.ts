...
export let manageTimeOff = SlateTool.create(
  ...
  .input(z.object({
    ...
    policyTypeId: z.string().optional().describe('Policy type ID (for get_balances, update_balance)'),
    ...
    requestBody: z.any().optional().describe('Request body for create_request or update_request'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update_request') {
      ...
      let result = await client.updateTimeOffRequest(ctx.input.requestId, ctx.input.requestBody);
      return {
        output: { result },
