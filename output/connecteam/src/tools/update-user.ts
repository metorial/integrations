...
export let updateUser = SlateTool.create(
  ...
  .input(z.object({
    ...
    users: z.array(z.object({
      ...
      isArchived: z.boolean().optional().describe('Set true to archive, false to restore'),
    ...
    })).optional().describe('Users to update (for "update" action)'),
    archiveUserIds: z.array(z.number()).optional().describe('User IDs to archive (for "archive" action)'),
    promoteDetails: z.object({
    ...
    }).optional().describe('Promotion details (for "promote_to_admin" action)'),
  }))
  .output(z.object({
