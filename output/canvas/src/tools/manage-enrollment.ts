...
export let manageEnrollmentTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['enroll', 'conclude', 'delete', 'deactivate']).describe('Action to perform'),
    userId: z.string().optional().describe('User ID to enroll (required for enroll action)'),
    enrollmentId: z.string().optional().describe('Enrollment ID (required for conclude/delete/deactivate)'),
    enrollmentRole: z.enum(['StudentEnrollment', 'TeacherEnrollment', 'TaEnrollment', 'ObserverEnrollment', 'DesignerEnrollment']).optional().describe('Enrollment role type (required for enroll)'),
    courseSectionId: z.string().optional().describe('Specific section to enroll into'),
    enrollmentState: z.enum(['active', 'invited', 'inactive']).optional().describe('Initial enrollment state'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'enroll') {
      ...
      if (!ctx.input.enrollmentRole) throw new Error('enrollmentRole is required for enroll action');
      ...
      if (!ctx.input.enrollmentId) throw new Error('enrollmentId is required for conclude/delete/deactivate');
      let task = ctx.input.action === 'deactivate' ? 'deactivate' as const : ctx.input.action as 'conclude' | 'delete';
