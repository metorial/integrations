...
export let listUsersTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    searchTerm: z.string().optional().describe('Partial user name or full ID to search for (min 3 chars)'),
    enrollmentType: z.array(z.enum(['student', 'teacher', 'ta', 'observer', 'designer'])).optional().describe('Filter by enrollment role'),
