...
export let listEnrollmentsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    enrollmentType: z.array(z.enum(['StudentEnrollment', 'TeacherEnrollment', 'TaEnrollment', 'ObserverEnrollment', 'DesignerEnrollment'])).optional().describe('Filter by enrollment role type'),
    enrollmentState: z.array(z.enum(['active', 'invited', 'creation_pending', 'deleted', 'rejected', 'completed', 'inactive'])).optional().describe('Filter by enrollment state'),
