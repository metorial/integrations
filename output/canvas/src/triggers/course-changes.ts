...
export let courseChangesTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Course Changes',
    key: 'course_changes',
    description: 'Detects new or updated courses by polling the courses API. Triggers when courses are created, updated, or change state.',
  }
)
  .input(z.object({
    ...
    changeType: z.enum(['new', 'updated']).describe('Whether this is a newly detected course or an updated one'),
  }))
  .output(z.object({
    courseId: z.string().describe('Course ID'),
    courseName: z.string().describe('Course name'),
