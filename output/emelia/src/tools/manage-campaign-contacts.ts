...
export let manageCampaignContacts = SlateTool.create(
  spec,
  {
...
- **set_custom_field**: Set a custom field value on a contact.`,
  ...
  .input(z.object({
    action: z.enum(['add', 'list', 'remove', 'add_to_list', 'set_custom_field']).describe('Operation to perform'),
    ...
    contactId: z.string().optional().describe('Contact ID (required for remove, set_custom_field)'),
    email: z.string().optional().describe('Contact email (for email/advanced campaigns)'),
    linkedInUrl: z.string().optional().describe('LinkedIn profile URL (for LinkedIn/advanced campaigns)'),
    firstName: z.string().optional().describe('Contact first name'),
    lastName: z.string().optional().describe('Contact last name'),
    phone: z.string().optional().describe('Contact phone number'),
    companyName: z.string().optional().describe('Contact company name'),
    customFields: z.record(z.string(), z.string()).optional().describe('Custom fields as key-value pairs'),
    listId: z.string().optional().describe('List ID (required for add_to_list)'),
    contacts: z.array(z.record(z.string(), z.unknown())).optional().describe('Array of contacts (for add_to_list)'),
  ...
  .output(z.object({
    ...
    contacts: z.array(z.record(z.string(), z.unknown())).optional().describe('List of contacts (for list action)'),
    contact: z.record(z.string(), z.unknown()).optional().describe('Contact details (for add action)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'add') {
      ...
      if (campaignType === 'email') {
        ...
        if (!ctx.input.linkedInUrl) throw new Error('LinkedIn URL is required for LinkedIn campaigns');
    ...
    if (action === 'add_to_list') {
      ...
      if (!ctx.input.contacts || ctx.input.contacts.length === 0) throw new Error('Contacts array is required');

      if (campaignType === 'email') {
        await client.addContactsToEmailList({
          listId: ctx.input.listId,
          contacts: ctx.input.contacts as Array<{ email: string; firstName?: string; lastName?: string; phone?: string; companyName?: string; customFields?: Record<string, string> }>,
        });
      } else if (campaignType === 'linkedin') {
        await client.addContactsToLinkedInList({
          listId: ctx.input.listId,
          contacts: ctx.input.contacts as Array<{ linkedInUrl: string; firstName?: string; lastName?: string; email?: string; companyName?: string; customFields?: Record<string, string> }>,
        });
      } else {
    ...
    if (action === 'set_custom_field') {
      if (!contactId) throw new Error('Contact ID is required');
      if (!ctx.input.fieldName || ctx.input.fieldValue === undefined) throw new Error('Field name and value are required');

      if (campaignType === 'email') {
        await client.setContactCustomField(contactId, { field: ctx.input.fieldName, value: ctx.input.fieldValue });
      } else if (campaignType === 'linkedin') {
        await client.setLinkedInContactCustomField(contactId, { field: ctx.input.fieldName, value: ctx.input.fieldValue });
      } else {
        await client.setAdvancedContactCustomField(contactId, { field: ctx.input.fieldName, value: ctx.input.fieldValue });
      }
