...
export let manageEmailProviders = SlateTool.create(
  spec,
  {
...
- **list_warmups**: List warmup status for all providers.`,
  ...
  .input(z.object({
    action: z.enum(['list', 'add', 'delete', 'enable_warmup', 'disable_warmup', 'list_warmups']).describe('Operation to perform'),
    providerId: z.string().optional().describe('Provider ID (required for delete, enable_warmup, disable_warmup)'),
    providerConfig: z.record(z.string(), z.unknown()).optional().describe('Provider configuration (for add)'),
  }))
  .output(z.object({
    providers: z.array(z.record(z.string(), z.unknown())).optional().describe('List of email providers'),
    provider: z.record(z.string(), z.unknown()).optional().describe('Provider details'),
    warmups: z.array(z.record(z.string(), z.unknown())).optional().describe('Warmup statuses'),
