...
let purchaseOrderSchema = z.object({
  ...
  status: z.string().nullable().optional().describe('PO status (e.g. draft, pending_approval, ordered, soft_closed, closed)'),
...
export let searchPurchaseOrders = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    status: z.string().optional().describe('Filter by status (e.g. "draft", "pending_approval", "ordered", "soft_closed", "closed")'),
    ...
    createdAfter: z.string().optional().describe('Filter POs created after this date (ISO 8601)'),
    updatedAfter: z.string().optional().describe('Filter POs updated after this date (ISO 8601)'),
    filters: z.record(z.string(), z.string()).optional().describe('Additional Coupa query filters as key-value pairs, e.g. {"order_lines.description[contains]": "laptop"}'),
    orderBy: z.string().optional().describe('Field to sort by (e.g. "created_at", "updated_at", "po_number")'),
    sortDirection: z.enum(['asc', 'desc']).optional().describe('Sort direction'),
    limit: z.number().optional().describe('Maximum number of results (default 50, max 50)'),
    offset: z.number().optional().describe('Offset for pagination'),
  }))
  .output(z.object({
    purchaseOrders: z.array(purchaseOrderSchema).describe('List of matching purchase orders'),
