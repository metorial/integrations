...
export let listCollections = SlateTool.create(
  ...
  .input(z.object({
    status: z.enum(['all', 'idle', 'queued', 'running']).optional().describe('Filter collections by status.'),
    searchTerm: z.string().optional().describe('Search collections by name.'),
    searchType: z.enum(['contains', 'starts_with', 'ends_with', 'exact']).optional().describe('Search behavior for name matching.'),
    onlyWithResults: z.boolean().optional().describe('Only return collections that have result sets.'),
    onlyWithoutResults: z.boolean().optional().describe('Only return collections without result sets.'),
    createdBefore: z.string().optional().describe('ISO 8601 UTC timestamp filter for creation date.'),
    createdAfter: z.string().optional().describe('ISO 8601 UTC timestamp filter for creation date.'),
    lastRunBefore: z.string().optional().describe('ISO 8601 UTC timestamp filter for last run date.'),
    lastRunAfter: z.string().optional().describe('ISO 8601 UTC timestamp filter for last run date.'),
    ...
    sortBy: z.enum(['created_at', 'last_run', 'name', 'priority', 'status']).optional().describe('Sort field.'),
  ...
  .output(z.object({
    ...
    totalCount: z.number().optional().describe('Total number of collections matching the filter.'),
