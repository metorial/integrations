...
export let listReportingGroupsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    reportingGroupId: z.string().optional().describe('Filter by a specific reporting group ID'),
    campaignId: z.string().optional().describe('Filter reporting groups associated with a specific campaign'),
  }))
  .output(z.object({
    reportingGroups: z.array(z.object({
      reportingGroupId: z.string().describe('Unique identifier of the reporting group'),
      name: z.string().describe('Name of the reporting group'),
      campaignIds: z.array(z.string()).describe('List of campaign IDs in this reporting group'),
    }).passthrough()).describe('List of reporting groups'),
  }))
