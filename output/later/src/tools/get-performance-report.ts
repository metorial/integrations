...
let networkMetricsSchema = z.object({
...
let reportEntrySchema = z.object({
  ...
  networkBreakdown: z.record(z.string(), networkMetricsSchema).describe('Metrics broken down by social network (Facebook, Instagram, TikTok, Pinterest)'),
}).passthrough();

export let getPerformanceReportTool = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Either campaignId or reportingGroupId must be provided.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    reportingGroupId: z.string().optional().describe('Reporting group ID to report on (alternative to campaignId)'),
    ...
    groupBy: z.enum(['year', 'quarter', 'month', 'week_monday', 'week_sunday', 'day']).optional().describe('Time period to group results by'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let grouping = ctx.input.groupBy
      ? ` grouped by ${ctx.input.groupBy}`
      : '';
