...
export let getScope = SlateTool.create(spec, {
  ...
  .output(scopeSchema.extend({
    population: z.record(z.string(), z.any()).optional().describe('Population configuration including included and excluded cohort IDs'),
    payload: z.record(z.string(), z.any()).optional().describe('Payload configuration including outcome IDs, persona set IDs, etc.'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new FaradayClient({ token: ctx.auth.token });
...
export let createScope = SlateTool.create(spec, {
  ...
  .input(z.object({
    ...
    population: z.object({
      cohortIds: z.array(z.string()).describe('Array of cohort UUIDs to include in the population'),
      exclusionCohortIds: z.array(z.string()).optional().describe('Array of cohort UUIDs to exclude'),
    }).describe('Defines which people to include in predictions'),
    payload: z.object({
      outcomeIds: z.array(z.string()).optional().describe('Outcome UUIDs to include propensity scores for'),
      personaSetIds: z.array(z.string()).optional().describe('Persona set UUIDs to include assignments for'),
      cohortIds: z.array(z.string()).optional().describe('Additional cohort IDs for membership inclusion'),
      recommenderIds: z.array(z.string()).optional().describe('Recommender UUIDs to include recommendations'),
      attributes: z.array(z.string()).optional().describe('Trait/attribute names to include (prefix Faraday traits with "fig/")'),
      explainability: z.boolean().optional().describe('Whether to include prediction explainability'),
    }).describe('Specifies what prediction data to include for each person'),
    preview: z.boolean().optional().describe('When true, provides limited records without billing'),
  }))
  .output(scopeSchema)
  .handleInvocation(async (ctx) => {
    let client = new FaradayClient({ token: ctx.auth.token });
    ...
    let body: Record<string, any> = {
      ...
      population: {
        ...
        ...(ctx.input.population.exclusionCohortIds && { exclusion_cohort_ids: ctx.input.population.exclusionCohortIds }),
      },
      payload: {} as Record<string, any>,
    };

    if (ctx.input.payload.outcomeIds) body.payload.outcome_ids = ctx.input.payload.outcomeIds;
    if (ctx.input.payload.personaSetIds) body.payload.persona_set_ids = ctx.input.payload.personaSetIds;
    if (ctx.input.payload.cohortIds) body.payload.cohort_ids = ctx.input.payload.cohortIds;
    if (ctx.input.payload.recommenderIds) body.payload.recommender_ids = ctx.input.payload.recommenderIds;
    if (ctx.input.payload.attributes) body.payload.attributes = ctx.input.payload.attributes;
    if (ctx.input.payload.explainability !== undefined) body.payload.explainability = ctx.input.payload.explainability;
