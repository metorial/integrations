...
let targetSchema = z.object({
  ...
  connectionId: z.string().optional().describe('UUID of the associated connection (for replication targets)'),
  representation: z.record(z.string(), z.any()).optional().describe('Output format configuration'),
...
- **Hosted CSV**: Faraday-hosted CSV download. Set options type to \`hosted_csv\`.`,
  ...
  .input(z.object({
    ...
    representation: z.object({
      mode: z.enum(['hashed', 'referenced', 'identified', 'aggregated']).describe('Output format mode'),
      aggregate: z.string().optional().describe('Aggregation level (e.g., "person", "residence")'),
    }).describe('Output format configuration'),
    options: z.record(z.string(), z.any()).describe('Target type configuration (must include "type" field, e.g., "lookup_api", "hosted_csv")'),
    connectionId: z.string().optional().describe('UUID of the connection for replication targets'),
    filter: z.record(z.string(), z.any()).optional().describe('Filter configuration to restrict exported rows'),
    limit: z.record(z.string(), z.any()).optional().describe('Row count or percentile-based limit configuration'),
    humanReadable: z.boolean().optional().describe('Replace machine IDs with readable names in output'),
  }))
  .output(targetSchema)
  .handleInvocation(async (ctx) => {
    let client = new FaradayClient({ token: ctx.auth.token });
