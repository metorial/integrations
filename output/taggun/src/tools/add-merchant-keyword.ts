...
export let addMerchantKeyword = SlateTool.create(
  spec,
  {
...
Changes to the model are applied daily. Use this to improve extraction accuracy for merchants that are frequently encountered in your receipt processing workflows.`,
    instructions: [
      'Use the exact merchant name as it appears on receipts.',
    ],
    constraints: [
      'Model updates take effect within 24 hours.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    merchantName: z.string().describe('The merchant name keyword to add to the prediction model'),
  }))
  .output(z.object({
