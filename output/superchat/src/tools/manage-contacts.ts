...
let contactResponseSchema = z.object({
  ...
  })).optional().describe('Contact handles (phone numbers, emails)'),
  customAttributes: z.array(z.record(z.string(), z.any())).optional().describe('Custom attribute values'),
...
export let createContact = SlateTool.create(
  ...
  .input(z.object({
    handles: z.array(contactHandleSchema).min(1).describe('At least one handle (phone or email) is required'),
    firstName: z.string().optional().describe('Contact first name'),
    lastName: z.string().optional().describe('Contact last name'),
    gender: z.enum(['female', 'male', 'diverse']).optional().describe('Contact gender'),
    customAttributes: z.array(z.record(z.string(), z.any())).optional().describe('Custom attribute objects'),
  }))
...
export let updateContact = SlateTool.create(
  ...
  .input(z.object({
    ...
    customAttributes: z.array(z.record(z.string(), z.any())).optional().describe('Updated custom attributes'),
  }))
  .output(contactResponseSchema)
  .handleInvocation(async (ctx) => {
    let client = new SuperchatClient({ token: ctx.auth.token });
