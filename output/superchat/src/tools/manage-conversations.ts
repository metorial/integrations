...
let conversationSchema = z.object({
  ...
  })).optional().describe('Users assigned to the conversation'),
  labels: z.array(z.object({
    labelId: z.string().describe('Label ID'),
    name: z.string().optional().describe('Label name'),
  })).optional().describe('Labels attached to the conversation'),
...
export let updateConversation = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['open', 'snoozed', 'done']).optional().describe('New conversation status'),
    snoozedUntil: z.string().optional().describe('UTC timestamp until when conversation is snoozed (required when status is "snoozed")'),
    assignedUserIds: z.array(z.string()).optional().describe('User IDs to assign to the conversation'),
    labelIds: z.array(z.string()).optional().describe('Label IDs to attach to the conversation'),
