...
export let updateFile = SlateTool.create(
  ...
  .input(z.object({
    ...
    customCoordinates: z.string().optional().describe('Custom focus area in format "x,y,width,height"'),
    customMetadata: z.record(z.string(), z.any()).optional().describe('Custom metadata key-value pairs to set'),
    extensions: z.array(z.record(z.string(), z.any())).optional().describe('Extensions to apply, e.g. [{"name": "google-auto-tagging", "minConfidence": 80}]'),
    webhookUrl: z.string().optional().describe('Webhook URL for extension processing results'),
    isPublished: z.boolean().optional().describe('Set publish status of the file'),
    includeFileVersions: z.boolean().optional().describe('Apply publish status to all file versions'),
  }))
  .output(z.object({
    ...
    customMetadata: z.record(z.string(), z.any()).optional().nullable().describe('Updated custom metadata'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.customCoordinates !== undefined) updateParams.customCoordinates = ctx.input.customCoordinates;
    if (ctx.input.customMetadata !== undefined) updateParams.customMetadata = ctx.input.customMetadata;
