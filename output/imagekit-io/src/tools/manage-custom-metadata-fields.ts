...
let fieldSchema = z.object({
  ...
  schema: z.record(z.string(), z.any()).describe('Field schema definition including type, constraints, and default values'),
});

export let manageCustomMetadataFields = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    label: z.string().optional().describe('Display label (required for create, optional for update)'),
    schema: z.record(z.string(), z.any()).optional().describe('Field schema definition, e.g. {"type": "Number", "minValue": 0, "maxValue": 100}'),
    includeDeleted: z.boolean().optional().describe('Include deleted fields in list results'),
  }))
  .output(z.object({
    fields: z.array(fieldSchema).optional().describe('List of custom metadata fields (for list operation)'),
    field: fieldSchema.optional().describe('Created or updated field (for create/update operations)'),
    deleted: z.boolean().optional().describe('Whether the field was deleted (for delete operation)'),
  }))
