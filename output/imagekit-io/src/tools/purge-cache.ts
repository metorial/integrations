...
export let purgeCache = SlateTool.create(
  ...
  .input(z.object({
    operation: z.enum(['purge', 'status']).describe('"purge" to initiate a cache purge, "status" to check a purge request status'),
    url: z.string().optional().describe('Full ImageKit URL to purge (required for purge operation)'),
    purgeRequestId: z.string().optional().describe('Purge request ID to check status (required for status operation)'),
  }))
  .output(z.object({
    purgeRequestId: z.string().optional().describe('ID of the purge request (for purge operation)'),
    status: z.string().optional().describe('Purge status: "Pending" or "Complete" (for status operation)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.operation === 'status') {
      if (!ctx.input.purgeRequestId) throw new Error('purgeRequestId is required for status operation');
