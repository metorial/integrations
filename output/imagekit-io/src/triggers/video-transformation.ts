...
export let videoTransformation = SlateTrigger.create(
  spec,
  {
    name: 'Video Transformation',
    key: 'video_transformation',
    description: 'Triggered when a video transformation is accepted, completed, or fails. Covers the full lifecycle of asynchronous video encoding operations.',
  }
)
  .input(z.object({
    eventType: z.enum(['accepted', 'ready', 'error']).describe('Type of video transformation event'),
    ...
    transformationOptions: z.record(z.string(), z.any()).optional().describe('Transformation options applied'),
    outputUrl: z.string().optional().describe('URL of the transformed output (when ready)'),
    videoMetadata: z.record(z.string(), z.any()).optional().describe('Video metadata of the output (when ready)'),
    timings: z.record(z.string(), z.any()).optional().describe('Timing information (when ready)'),
    errorReason: z.string().optional().describe('Error reason (when error)'),
  }))
  .output(z.object({
    assetUrl: z.string().describe('URL of the source video asset'),
    transformationStatus: z.enum(['accepted', 'ready', 'error']).describe('Current status of the transformation'),
    requestUrl: z.string().optional().describe('URL that triggered the transformation'),
    transformationOptions: z.record(z.string(), z.any()).optional().describe('Options used for the transformation'),
    outputUrl: z.string().optional().describe('URL of the transformed video (available when ready)'),
    ...
    downloadDuration: z.number().optional().describe('Time taken to download the source in ms'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let status = eventType.replace('video.transformation.', '') as 'accepted' | 'ready' | 'error';
