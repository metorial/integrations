...
let templateComponentDefinition = z.object({
  ...
  format: z.enum(['TEXT', 'IMAGE', 'VIDEO', 'DOCUMENT', 'LOCATION']).optional().describe('Header format (required for HEADER type)'),
  text: z.string().optional().describe('Component text content. Use {{1}}, {{2}}, etc. for dynamic variables'),
  buttons: z.array(z.object({
    ...
    url: z.string().optional().describe('URL (for URL type). Use {{1}} for dynamic suffix'),
    example: z.array(z.string()).optional().describe('Example values for dynamic parts')
  })).optional().describe('Buttons array (required for BUTTONS type)')
});
...
Use pagination to browse through large template libraries.`,
  ...
  .input(z.object({
    limit: z.number().optional().default(20).describe('Number of templates to return (default 20)'),
    afterCursor: z.string().optional().describe('Pagination cursor for the next page of results')
  }))
  .output(z.object({
    ...
    nextCursor: z.string().optional().describe('Cursor for the next page, if more results exist')
  }))
...
Templates have three categories: **MARKETING**, **UTILITY**, and **AUTHENTICATION**.`,
  ...
  .input(z.object({
    ...
    components: z.array(templateComponentDefinition).describe('Template component definitions'),
    allowCategoryChange: z.boolean().optional().describe('Allow Meta to reassign the category if needed')
  }))
  .output(z.object({
...
export let deleteTemplate = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Deleting a template removes all language versions of that template'
    ],
    tags: {
      destructive: true,
      readOnly: false
