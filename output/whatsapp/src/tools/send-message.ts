...
let mediaSourceSchema = z.object({
...
For text messages within the 24-hour customer service window, use this tool. For messages outside the window, use the Send Template Message tool instead.`,
  ...
  .input(z.object({
    ...
    type: z.enum(['text', 'image', 'video', 'audio', 'document', 'location', 'contacts', 'sticker', 'reaction'])
      .describe('Type of message to send'),
    text: z.object({
      body: z.string().describe('Message text content'),
      previewUrl: z.boolean().optional().describe('Whether to show a URL preview for links in the message')
    }).optional().describe('Required when type is "text"'),
    image: mediaSourceSchema.extend({
      caption: z.string().optional().describe('Image caption')
    }).optional().describe('Required when type is "image"'),
    video: mediaSourceSchema.extend({
      caption: z.string().optional().describe('Video caption')
    }).optional().describe('Required when type is "video"'),
    audio: mediaSourceSchema.optional().describe('Required when type is "audio"'),
    document: mediaSourceSchema.extend({
      filename: z.string().optional().describe('Filename to display to recipient'),
      caption: z.string().optional().describe('Document caption')
    }).optional().describe('Required when type is "document"'),
    location: z.object({
      latitude: z.number().describe('Latitude coordinate'),
      longitude: z.number().describe('Longitude coordinate'),
      name: z.string().optional().describe('Location name'),
      address: z.string().optional().describe('Location address')
    }).optional().describe('Required when type is "location"'),
    contacts: z.array(contactSchema).optional().describe('Required when type is "contacts"'),
    sticker: mediaSourceSchema.optional().describe('Required when type is "sticker". Must be WEBP format, 512x512px'),
    reaction: z.object({
      messageId: z.string().describe('ID of the message to react to'),
      emoji: z.string().describe('Emoji to react with (e.g. \ud83d\udc4d). Send empty string to remove reaction')
    }).optional().describe('Required when type is "reaction"')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (type) {
      case 'text': {
        ...
        result = await client.sendTextMessage(to, ctx.input.text.body, ctx.input.text.previewUrl);
        break;
      }
      case 'image': {
      ...
      case 'document': {
        if (!ctx.input.document) throw new Error('document field is required for document messages');
        result = await client.sendDocumentMessage(to, ctx.input.document);
        break;
      }
      case 'location': {
        if (!ctx.input.location) throw new Error('location field is required for location messages');
        result = await client.sendLocationMessage(to, ctx.input.location);
        break;
      }
      case 'contacts': {
        if (!ctx.input.contacts) throw new Error('contacts field is required for contacts messages');
      ...
      case 'sticker': {
        if (!ctx.input.sticker) throw new Error('sticker field is required for sticker messages');
        result = await client.sendStickerMessage(to, ctx.input.sticker);
        break;
      }
      case 'reaction': {
        if (!ctx.input.reaction) throw new Error('reaction field is required for reaction messages');
        result = await client.sendReactionMessage(to, ctx.input.reaction.messageId, ctx.input.reaction.emoji);
        break;
      }
    }
