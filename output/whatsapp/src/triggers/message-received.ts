...
export let messageReceived = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers when an incoming WhatsApp message is received. Covers text, image, audio, video, document, sticker, location, contacts, interactive replies, reactions, and order messages.'
  }
)
  .input(z.object({
    messageId: z.string().describe('Unique message ID'),
    messageType: z.string().describe('Message type (text, image, audio, video, document, sticker, location, contacts, interactive, reaction, order, system)'),
  ...
  .output(z.object({
    ...
    interactiveType: z.string().optional().describe('Interactive response type (button_reply, list_reply)'),
    ...
    referral: z.any().optional().describe('Referral data if message came from an ad or post'),
    context: z.object({
    ...
    }).optional().describe('Message context for replies')
  }))
  .webhook({
