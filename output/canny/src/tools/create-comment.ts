...
export let createCommentTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    imageURLs: z.array(z.string()).optional().describe('Image URLs to attach to the comment'),
    parentId: z.string().optional().describe('Parent comment ID for threaded replies'),
    internal: z.boolean().optional().describe('Whether this comment is internal-only (not visible to end users)'),
    shouldNotifyVoters: z.boolean().optional().describe('Whether to email voters about this comment'),
