...
export let controlImage = SlateTool.create(
  spec,
  {
...
Each mode takes a reference image and a text prompt to guide the output.`,
  ...
  .input(z.object({
    ...
    image: z.string().describe('Base64-encoded reference image (sketch, structural guide, or style reference).'),
    prompt: z.string().describe('Text prompt describing the desired output image.'),
    negativePrompt: z.string().optional().describe('Text describing what to avoid in the output.'),
    controlStrength: z.number().min(0).max(1).optional().describe('How strongly the reference guides the output (0-1). Used by sketch and structure modes.'),
    fidelity: z.number().min(0).max(1).optional().describe('How closely to match the style reference (0-1). Used by style mode only.'),
