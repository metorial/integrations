...
export let scheduleDripCampaign = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    initiator: z.string().describe('Identifier for who triggered the campaign (e.g. user name or system name).'),
    sendDate: z.string().optional().describe('Date to start the campaign in YYYY-MM-DD format. Omit to start immediately.'),
    thirdPartyContactId: z.string().optional().describe('External CRM contact ID for tracking.'),
    recipientSalutation: z.string().optional().describe('Recipient salutation (e.g. "Mr.", "Mrs.", "Dr.").'),
    ...
    recipientBirthDate: z.string().optional().describe('Recipient birth date in YYYY-MM-DD format for birthday personalization.'),
    recipientPhoneNumber: z.string().optional().describe('Recipient phone number.'),
    recipientAnniversaryDate: z.string().optional().describe('Recipient anniversary date in YYYY-MM-DD format for anniversary personalization.'),
    fromFirstName: z.string().optional().describe('Sender first name (overrides campaign default).'),
    fromLastName: z.string().optional().describe('Sender last name (overrides campaign default).'),
    fromAddressLine1: z.string().optional().describe('Sender street address line 1 (overrides campaign default).'),
    fromAddressLine2: z.string().optional().describe('Sender street address line 2 (overrides campaign default).'),
    ...
    fromPostalCode: z.string().optional().describe('Sender postal code (overrides campaign default).'),
    fromCountry: z.string().optional().describe('Sender country (overrides campaign default).'),
  }))
  .output(z.object({
