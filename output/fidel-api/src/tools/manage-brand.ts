...
let brandSchema = z.object({
  ...
  metadata: z.record(z.string(), z.any()).optional().describe('Custom metadata attached to the brand'),
});

export let createBrand = SlateTool.create(
  spec,
  {
    name: 'Create Brand',
    key: 'create_brand',
  ...
  .input(z.object({
    ...
    metadata: z.record(z.string(), z.any()).optional().describe('Custom metadata to attach to the brand'),
  }))
  .output(brandSchema)
...
export let updateBrand = SlateTool.create(
  spec,
  {
    name: 'Update Brand',
    key: 'update_brand',
  ...
  .input(z.object({
    ...
    metadata: z.record(z.string(), z.any()).optional().describe('Updated metadata for the brand'),
  }))
  .output(brandSchema)
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
...
export let deleteBrand = SlateTool.create(
  spec,
  {
    name: 'Delete Brand',
    key: 'delete_brand',
