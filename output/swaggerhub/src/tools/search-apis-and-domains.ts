...
let apiEntrySchema = z.object({
  ...
  })).optional().describe('Associated properties and links'),
});

export let searchApisAndDomains = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    visibility: z.enum(['PUBLIC', 'PRIVATE', 'ANY']).optional().describe('Filter by visibility'),
    state: z.enum(['ALL', 'PUBLISHED', 'UNPUBLISHED']).optional().describe('Filter by published state'),
    ...
    sort: z.enum(['NAME', 'UPDATED', 'CREATED', 'OWNER', 'BEST_MATCH', 'TITLE']).optional().describe('Sort field'),
