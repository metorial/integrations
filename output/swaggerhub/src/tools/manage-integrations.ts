...
let integrationSchema = z.object({
  ...
  configType: z.string().optional().describe('Integration type (e.g., GITHUB_PULL_REQUEST, AWS_API_GATEWAY)'),
  enabled: z.boolean().optional().describe('Whether the integration is enabled'),
}).passthrough();

export let manageIntegrations = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'create', 'update', 'delete', 'execute']).describe('Operation to perform'),
    integrationId: z.string().optional().describe('Integration ID (required for update, delete, execute)'),
    configuration: z.record(z.string(), z.unknown()).optional().describe('Integration configuration object (required for create/update)'),
  }))
  .output(z.object({
    integrations: z.array(integrationSchema).optional().describe('List of integrations (for list action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (action) {
      ...
      case 'create': {
        if (!configuration) throw new Error('Configuration is required to create an integration.');
        let integration = await client.createApiIntegration(owner, apiName, version, configuration);
        return {
      ...
      case 'update': {
        ...
        let integration = await client.updateApiIntegration(owner, apiName, version, integrationId, configuration);
        return {
          output: { integration, success: true },
