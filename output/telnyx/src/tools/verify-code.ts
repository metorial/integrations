...
export let verifyCode = SlateTool.create(
  spec,
  {
    name: 'Verify Code',
    key: 'verify_code',
  ...
  .input(z.object({
    phoneNumber: z.string().describe('The phone number the code was sent to, in E.164 format'),
    code: z.string().describe('The verification code to check'),
    verifyProfileId: z.string().describe('ID of the Verify Profile used for the verification'),
  }))
  .output(z.object({
    phoneNumber: z.string().describe('Phone number verified'),
    status: z.string().optional().describe('Verification result status (e.g., "accepted", "rejected")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let status = result?.response_code === 'accepted' ? 'accepted' : (result?.response_code ?? result?.status ?? 'unknown');
