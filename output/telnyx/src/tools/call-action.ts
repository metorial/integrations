...
export let callAction = SlateTool.create(
  spec,
  {
    name: 'Call Action',
    key: 'call_action',
  ...
  .input(z.object({
    ...
    action: z.enum([
      'answer', 'hangup', 'transfer', 'bridge', 'speak',
      'play_audio', 'send_dtmf', 'record_start', 'record_stop',
      'record_pause', 'record_resume', 'gather_using_speak',
      'gather_using_audio', 'gather_stop', 'enqueue', 'leave_queue',
      'fork_start', 'fork_stop',
    ]).describe('The call action to perform'),
    speakText: z.string().optional().describe('Text for text-to-speech (for "speak" and "gather_using_speak" actions)'),
    ...
    transferTo: z.string().optional().describe('Phone number or SIP URI to transfer to (for "transfer" action)'),
    audioUrl: z.string().optional().describe('URL of audio file to play (for "play_audio" and "gather_using_audio" actions)'),
    dtmfDigits: z.string().optional().describe('DTMF digits to send (for "send_dtmf" action)'),
    callControlIdToBridge: z.string().optional().describe('Call control ID to bridge with (for "bridge" action)'),
    actionParams: z.record(z.string(), z.any()).optional().describe('Additional action-specific parameters as key-value pairs'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.callControlIdToBridge) params.call_control_id = ctx.input.callControlIdToBridge;
