...
export let manageVerifyProfile = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    profileId: z.string().optional().describe('Verify profile ID (required for get, update, delete)'),
    ...
    defaultTimeoutSecs: z.number().optional().describe('Default verification code timeout in seconds'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of verify profiles'),
    profile: z.object({
      profileId: z.string().describe('Profile ID'),
      name: z.string().optional().describe('Profile name'),
      webhookUrl: z.string().optional().describe('Webhook URL'),
