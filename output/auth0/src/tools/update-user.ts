...
export let updateUserTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    userMetadata: z.record(z.string(), z.unknown()).optional().describe('Updated user metadata (merged with existing)'),
    appMetadata: z.record(z.string(), z.unknown()).optional().describe('Updated application metadata (merged with existing)'),
    ...
    connection: z.string().optional().describe('Connection name (required when updating email/password)'),
  }))
  .output(z.object({
