...
export let manageUserRolesTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    roleIds: z.array(z.string()).optional().describe('Role IDs to assign or remove (required for assign/remove)'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Current roles (returned for "list" action)'),
