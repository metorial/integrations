...
export let manageResourceServersTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    resourceServerId: z.string().optional().describe('Resource server ID (required for get, update, delete)'),
    name: z.string().optional().describe('API name (required for create)'),
    identifier: z.string().optional().describe('API identifier/audience URL (required for create, must be unique)'),
    scopes: z.array(z.object({
      value: z.string().describe('Scope name'),
      description: z.string().optional().describe('Scope description'),
    })).optional().describe('API scopes/permissions'),
    ...
    skipConsentForVerifiableFirstPartyClients: z.boolean().optional().describe('Skip consent for first-party clients'),
  ...
  .output(z.object({
    ...
    }).optional().describe('Resource server details'),
    resourceServers: z.array(z.object({
      resourceServerId: z.string(),
      name: z.string(),
      identifier: z.string(),
    })).optional().describe('List of resource servers'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'list') {
      ...
      let servers = (Array.isArray(result) ? result : result.resource_servers ?? []).map((rs: any) => ({
        resourceServerId: rs.id,
        name: rs.name,
        identifier: rs.identifier,
      }));
    ...
    if (ctx.input.action === 'get') {
      if (!ctx.input.resourceServerId) throw new Error('resourceServerId is required for get action');
    ...
    if (ctx.input.action === 'create') {
      ...
      let rs = await client.createResourceServer({
        ...
        skipConsentForVerifiableFirstPartyClients: ctx.input.skipConsentForVerifiableFirstPartyClients,
      });
      return {
        output: { resourceServer: mapRS(rs) },
    ...
    if (ctx.input.action === 'update') {
      if (!ctx.input.resourceServerId) throw new Error('resourceServerId is required for update action');
    ...
    if (ctx.input.action === 'delete') {
      if (!ctx.input.resourceServerId) throw new Error('resourceServerId is required for delete action');
