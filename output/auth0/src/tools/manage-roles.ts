...
export let manageRolesTool = SlateTool.create(
  spec,
  {
    name: 'Manage Roles',
    key: 'manage_roles',
  ...
  .input(z.object({
    ...
    name: z.string().optional().describe('Role name (required for create, optional for update)'),
  ...
  .output(z.object({
    ...
    }).optional().describe('Role details (for get, create, update)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      let role = await client.createRole({ name: ctx.input.name, description: ctx.input.description });
      return {
        output: { role: mapRole(role) },
