...
export let createUserTool = SlateTool.create(
  ...
  .input(z.object({
    connection: z.string().describe('Connection name for the user (e.g., "Username-Password-Authentication")'),
    email: z.string().optional().describe('User email address'),
    password: z.string().optional().describe('User password (required for database connections)'),
    ...
    userMetadata: z.record(z.string(), z.unknown()).optional().describe('Custom user metadata (non-sensitive, editable by user)'),
    appMetadata: z.record(z.string(), z.unknown()).optional().describe('Custom application metadata (admin-controlled)'),
