...
export let getUserTool = SlateTool.create(
  ...
  .output(z.object({
    ...
    userMetadata: z.record(z.string(), z.unknown()).optional().describe('Custom user metadata'),
    appMetadata: z.record(z.string(), z.unknown()).optional().describe('Custom application metadata'),
    identities: z.array(z.record(z.string(), z.unknown())).optional().describe('Linked identity provider accounts'),
  }))
