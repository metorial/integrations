...
export let manageConnectionsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    connectionId: z.string().optional().describe('Connection ID (required for get, update, delete)'),
    name: z.string().optional().describe('Connection name (required for create)'),
    strategy: z.string().optional().describe('Connection strategy, e.g., "auth0", "google-oauth2", "samlp" (required for create)'),
    options: z.record(z.string(), z.unknown()).optional().describe('Connection-specific options'),
    enabledClients: z.array(z.string()).optional().describe('Client IDs that can use this connection'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'list') {
      ...
      let connections = (Array.isArray(result) ? result : result.connections ?? []).map(mapConn);
      return {
        output: { connections },
    ...
    if (ctx.input.action === 'update') {
      if (!ctx.input.connectionId) throw new Error('connectionId is required for update action');
    ...
    if (ctx.input.action === 'delete') {
      if (!ctx.input.connectionId) throw new Error('connectionId is required for delete action');
