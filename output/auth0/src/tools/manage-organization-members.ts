...
export let manageOrganizationMembersTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    userIds: z.array(z.string()).optional().describe('User IDs to add or remove (required for add/remove)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('Organization members (for list action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'remove') {
      if (!ctx.input.userIds?.length) throw new Error('userIds are required for remove action');
