...
export let logEventsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Log Events',
    key: 'log_events',
    description: 'Receive Auth0 tenant log events in real-time via webhook log streams. Covers authentication, user actions, management API operations, system notifications, and more.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    logId: z.string().describe('Unique log event ID'),
    eventType: z.string().describe('Event type code (e.g., "s" for successful login, "f" for failed login)'),
