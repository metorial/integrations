...
export let scrapeWalmart = SlateTool.create(
  spec,
  {
    name: 'Scrape Walmart',
    key: 'scrape_walmart',
  ...
  .input(z.object({
    action: z.enum(['product', 'search', 'reviews']).describe('Type of data to retrieve from Walmart'),
    sku: z.string().optional().describe('Walmart product SKU/ID (numeric, 8-20 characters). Required for "product" and "reviews" actions.'),
    query: z.string().optional().describe('Search query to find products on Walmart. Required for "search" action.'),
    tld: z.string().optional().describe('Top-level domain for the Walmart site (e.g. ".com", ".ca")'),
  }))
  .output(z.object({
    results: z.record(z.string(), z.unknown()).describe('Structured product data, search results, or reviews from Walmart'),
  }))
