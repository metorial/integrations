...
export let provisionScimUser = SlateTool.create(
  spec,
  {
...
**Note:** InVision was shut down on December 31, 2024. This tool will only work if the service is still accessible.`,
  ...
  .input(z.object({
    ...
    userName: z.string().describe('Username for the new user (typically their email address)'),
    ...
    active: z.boolean().optional().default(true).describe('Whether the user should be active upon creation'),
  }))
  .output(z.object({
