...
export let clearCache = SlateTool.create(
  spec,
  {
    name: 'Clear Cache',
    key: 'clear_cache',
  ...
  .input(z.object({
    query: z.string().describe('Wildcard pattern for URLs to clear. Use `%` for multi-character and `?` for single-character wildcards.'),
    checkStatus: z.boolean().optional().describe('If true, also checks the current clear cache job status after scheduling.'),
  }))
  .output(z.object({
    clearResponse: z.record(z.string(), z.unknown()).describe('Response from the clear cache request.'),
    jobStatus: z.record(z.string(), z.unknown()).optional().describe('Current status of the clear cache job, if status check was requested.'),
  }))
