...
export let getConversation = SlateTool.create(
  ...
  .output(z.object({
    ...
    messages: z.array(z.object({
      ...
      messageType: z.string().optional().describe('Event type (answer, lookup_answer, is_resolved_question, support_escalation)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let conv = await client.getConversation(ctx.config.teamId, ctx.input.botId, ctx.input.conversationId);
