...
Describe the conversion you need in plain English along with the source document. Supports both file URLs and base64-encoded file content.`,
  ...
  .input(z.object({
    instructions: z.string().describe('Plain English description of the desired conversion, e.g. "Convert this PDF to Word document"'),
    fileUrl: z.string().optional().describe('Public URL of the source file to convert'),
    base64String: z.string().optional().describe('Base64-encoded content of the source file'),
    fileName: z.string().optional().describe('Original file name with extension, e.g. "report.pdf"'),
    outputFormat: z.string().optional().describe('Desired output format, e.g. "docx", "pdf", "html", "markdown"'),
  }))
  .output(z.object({
