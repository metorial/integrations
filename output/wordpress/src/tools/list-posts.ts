...
export let listPostsTool = SlateTool.create(
  spec,
  {
    name: 'List Posts',
    key: 'list_posts',
  ...
  .input(z.object({
    status: z.string().optional().describe('Filter by post status (draft, publish, pending, private, future, trash). Defaults to "publish"'),
    search: z.string().optional().describe('Search term to filter posts by'),
    category: z.string().optional().describe('Filter by category name (WP.com) or category ID (self-hosted)'),
    tag: z.string().optional().describe('Filter by tag name (WP.com) or tag ID (self-hosted)'),
    author: z.string().optional().describe('Filter by author ID'),
    perPage: z.number().optional().describe('Number of posts per page (default: 20, max: 100)'),
    page: z.number().optional().describe('Page number for pagination (default: 1)'),
    orderBy: z.string().optional().describe('Order results by field (date, title, modified, ID)'),
    order: z.enum(['ASC', 'DESC']).optional().describe('Sort order (ASC or DESC, default: DESC)'),
    after: z.string().optional().describe('Only return posts published after this ISO 8601 date'),
    before: z.string().optional().describe('Only return posts published before this ISO 8601 date')
  }))
  .output(z.object({
...
export let getPostTool = SlateTool.create(
  spec,
  {
    name: 'Get Post',
    key: 'get_post',
