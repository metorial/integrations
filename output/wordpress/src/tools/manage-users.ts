...
let userOutputSchema = z.object({
  ...
  roles: z.array(z.string()).describe('User roles (administrator, editor, author, contributor, subscriber)'),
...
export let listUsersTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    roles: z.string().optional().describe('Filter by role (administrator, editor, author, contributor, subscriber)'),
