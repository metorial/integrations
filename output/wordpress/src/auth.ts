...
export let auth = SlateAuth.create()
  .output(z.object({
    ...
    authMethod: z.enum(['oauth', 'application_password']).describe('Which authentication method is being used')
  }))
  .addOauth({
    type: 'auth.oauth',
    ...
    scopes: [
      ...
      { title: 'Sites', description: 'Access to site information and settings', scope: 'sites' },
      ...
      { title: 'Taxonomy', description: 'Manage categories, tags, and taxonomies', scope: 'taxonomy' },
      { title: 'Follow', description: 'Manage blog following and subscriptions', scope: 'follow' },
      ...
      { title: 'Global', description: 'Comprehensive access across all sites', scope: 'global' }
    ],
    ...
    handleCallback: async (ctx) => {
      let response = await wpcomAxios.post('/oauth2/token', new URLSearchParams({
        client_id: ctx.clientId,
  ...
  .addCustomAuth({
    ...
    inputSchema: z.object({
      ...
      applicationPassword: z.string().describe('Application password generated from WordPress admin (Users → Edit User → Application Passwords)')
    }),
    ...
    getProfile: async (ctx: { output: { token: string }; input: { username: string; applicationPassword: string } }) => {
