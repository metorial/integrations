...
let threadSchema = z.object({
  ...
  }).optional().describe('Who created the thread'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {
        ...
        assigneeName: data.assignee ? [data.assignee.first, data.assignee.last].filter(Boolean).join(' ') : null,
        customerEmail: data.primaryCustomer?.email ?? null,
        customerName: data.primaryCustomer ? [data.primaryCustomer.first, data.primaryCustomer.last].filter(Boolean).join(' ') : null,
