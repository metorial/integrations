...
export let createCustomer = SlateTool.create(
  ...
  .input(z.object({
    ...
    phones: z.array(z.object({
      type: z.enum(['home', 'work', 'mobile', 'fax', 'pager', 'other']).default('work').describe('Phone type'),
      value: z.string().describe('Phone number'),
...
export let updateCustomer = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.organization !== undefined) profileUpdates.organization = ctx.input.organization;
