...
export let listMailboxes = SlateTool.create(
  ...
  .input(z.object({
    mailboxId: z.number().optional().describe('Get details for a specific mailbox, including folders and custom fields'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of mailboxes (when no specific mailboxId is provided)'),
    mailbox: z.object({
      mailboxId: z.number(),
      name: z.string(),
    ...
    }).optional().describe('Detailed mailbox info (when mailboxId is provided)'),
  }))
