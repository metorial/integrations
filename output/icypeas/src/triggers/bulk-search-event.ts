...
export let bulkSearchEvent = SlateTrigger.create(
  spec,
  {
    name: 'Bulk Search Event',
    key: 'bulk_search_event',
    description: 'Receives webhook notifications for bulk search events, including per-item results and bulk search completion summaries.'
  }
)
  .input(z.object({
    eventType: z.enum(['item_processed', 'bulk_completed']).describe('Type of bulk search event'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let isBulkDone = data?.finished !== undefined || data?.stats !== undefined || data?.status === 'done';
      let eventType: 'item_processed' | 'bulk_completed' = isBulkDone ? 'bulk_completed' : 'item_processed';
