...
export let updateTemplate = SlateTool.create(
  ...
  .input(z.object({
    templateIdOrVersionId: z.string().describe('Template ID or Version ID of the template to update.'),
    ...
    deployedAt: z.number().optional().describe('UTC Unix timestamp to set the deployed version.'),
    expireAt: z.number().optional().describe('UTC Unix timestamp for template expiration/deletion.'),
  }))
  .output(z.object({
