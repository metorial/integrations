...
export let createFunction = SlateTool.create(
  ...
  .input(z.object({
    functionId: z.string().describe('Function name (4-63 chars, lowercase letters, numbers, hyphens)'),
    location: z.string().optional().describe('Region to deploy the function to. Defaults to configured region.'),
    description: z.string().optional().describe('Description of the function'),
    labels: z.record(z.string(), z.string()).optional().describe('Labels to apply to the function'),
    environment: z.enum(['GEN_1', 'GEN_2']).optional().describe('Function generation. Defaults to GEN_2.'),
    runtime: z.string().describe('Runtime environment (e.g. "nodejs20", "python312", "go122", "java17", "dotnet8", "ruby33")'),
    entryPoint: z.string().describe('Name of the function to execute (entry point)'),
    sourceStorageBucket: z.string().optional().describe('Cloud Storage bucket containing the source archive'),
    sourceStorageObject: z.string().optional().describe('Cloud Storage object path for the source archive'),
    sourceUploadUrl: z.string().optional().describe('Upload URL from generateUploadUrl for the source code'),
    ...
    buildEnvironmentVariables: z.record(z.string(), z.string()).optional().describe('Build-time environment variables'),
    timeoutSeconds: z.number().optional().describe('Function execution timeout in seconds (max 540 for Gen 1, max 3600 for Gen 2)'),
    availableMemory: z.string().optional().describe('Memory to allocate (e.g. "256Mi", "1Gi", "2Gi")'),
    ...
    environmentVariables: z.record(z.string(), z.string()).optional().describe('Runtime environment variables'),
    serviceAccountEmail: z.string().optional().describe('Service account email for the function to run as'),
    ingressSettings: z.enum(['ALLOW_ALL', 'ALLOW_INTERNAL_ONLY', 'ALLOW_INTERNAL_AND_GCLB']).optional().describe('Ingress traffic control'),
    vpcConnector: z.string().optional().describe('VPC connector resource name for egress'),
    vpcConnectorEgressSettings: z.enum(['PRIVATE_RANGES_ONLY', 'ALL_TRAFFIC']).optional().describe('Egress settings for VPC connector'),
    allTrafficOnLatestRevision: z.boolean().optional().describe('Whether to route all traffic to latest revision'),
    eventTrigger: z.object({
      eventType: z.string().describe('Type of event to trigger the function (e.g. "google.cloud.storage.object.v1.finalized")'),
      triggerRegion: z.string().optional().describe('Region for the event trigger'),
      eventFilters: z.array(eventFilterInputSchema).optional().describe('Filters for event matching'),
      pubsubTopic: z.string().optional().describe('Pub/Sub topic resource name'),
      retryPolicy: z.enum(['RETRY_POLICY_DO_NOT_RETRY', 'RETRY_POLICY_RETRY']).optional().describe('Retry policy for failed executions'),
      serviceAccountEmail: z.string().optional().describe('Service account for the event trigger')
    }).optional().describe('Event trigger configuration. Omit for HTTP-triggered functions.')
  }))
  .output(z.object({
    ...
    functionName: z.string().optional().describe('Created function resource name if completed')
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.timeoutSeconds !== undefined) serviceConfig['timeoutSeconds'] = ctx.input.timeoutSeconds;
    ...
    if (ctx.input.maxInstanceCount !== undefined) serviceConfig['maxInstanceCount'] = ctx.input.maxInstanceCount;
    if (ctx.input.minInstanceCount !== undefined) serviceConfig['minInstanceCount'] = ctx.input.minInstanceCount;
    if (ctx.input.environmentVariables) serviceConfig['environmentVariables'] = ctx.input.environmentVariables;
    if (ctx.input.serviceAccountEmail) serviceConfig['serviceAccountEmail'] = ctx.input.serviceAccountEmail;
    ...
    if (ctx.input.vpcConnectorEgressSettings) serviceConfig['vpcConnectorEgressSettings'] = ctx.input.vpcConnectorEgressSettings;
    if (ctx.input.allTrafficOnLatestRevision !== undefined) serviceConfig['allTrafficOnLatestRevision'] = ctx.input.allTrafficOnLatestRevision;
