...
export let getFunction = SlateTool.create(
  ...
  .input(z.object({
    functionName: z.string().describe('Short function name (e.g. "my-function") or fully qualified resource name (e.g. "projects/my-project/locations/us-central1/functions/my-function")'),
    location: z.string().optional().describe('Region where the function is deployed. Only needed when using a short function name. Defaults to configured region.')
  }))
  .output(z.object({
    ...
    stateMessages: z.array(stateMessageSchema).optional().describe('Informational messages about function state'),
    ...
    serviceAccountEmail: z.string().optional().describe('Service account used by the function'),
    ...
    ingressSettings: z.string().optional().describe('Ingress settings (ALLOW_ALL, ALLOW_INTERNAL_ONLY, etc.)'),
    ...
    }).optional().describe('Event trigger configuration')
  }))
