...
let bindingSchema = z.object({
  role: z.string().describe('IAM role (e.g. "roles/cloudfunctions.invoker", "roles/run.invoker")'),
  members: z.array(z.string()).describe('Members bound to the role (e.g. "allUsers", "user:email@example.com", "serviceAccount:sa@project.iam.gserviceaccount.com")')
});

export let manageIamPolicy = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['get', 'set']).describe('"get" to retrieve the current policy, "set" to update it'),
    functionName: z.string().describe('Short function name'),
    location: z.string().optional().describe('Region of the function. Defaults to configured region.'),
    bindings: z.array(bindingSchema).optional().describe('IAM bindings to set. Required when action is "set".'),
    etag: z.string().optional().describe('Etag from a previous get response for optimistic concurrency')
  }))
  .output(z.object({
