...
export let generateUploadUrl = SlateTool.create(
  ...
  .input(z.object({
    location: z.string().optional().describe('Region for the upload. Defaults to configured region.'),
    kmsKeyName: z.string().optional().describe('KMS key name for encryption'),
    environment: z.enum(['GEN_1', 'GEN_2']).optional().describe('Target function environment')
  }))
  .output(z.object({
    ...
    }).optional().describe('Storage source reference to use when creating/updating the function')
  }))
