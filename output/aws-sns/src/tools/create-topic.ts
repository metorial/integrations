...
export let createTopic = SlateTool.create(
  spec,
  {
    name: 'Create Topic',
    key: 'create_topic',
  ...
  .input(z.object({
    ...
    contentBasedDeduplication: z.boolean().optional().describe('Enable content-based deduplication for FIFO topics'),
    tracingConfig: z.enum(['PassThrough', 'Active']).optional().describe('X-Ray tracing mode'),
    tags: z.record(z.string(), z.string()).optional().describe('Key-value tags to attach to the topic'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.createTopic(ctx.input.name, {
