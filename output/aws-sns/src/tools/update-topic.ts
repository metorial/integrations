...
export let updateTopic = SlateTool.create(
  spec,
  {
    name: 'Update Topic',
    key: 'update_topic',
  ...
  .input(z.object({
    ...
    deliveryPolicy: z.string().optional().describe('JSON delivery retry policy for HTTP/S endpoints'),
    policy: z.string().optional().describe('JSON access control policy'),
    kmsMasterKeyId: z.string().optional().describe('KMS key ID for server-side encryption (empty string to disable)'),
    tracingConfig: z.enum(['PassThrough', 'Active']).optional().describe('X-Ray tracing mode'),
    contentBasedDeduplication: z.boolean().optional().describe('Enable content-based deduplication (FIFO topics only)'),
    archivePolicy: z.string().optional().describe('JSON message retention policy (FIFO topics only)'),
    addTags: z.record(z.string(), z.string()).optional().describe('Tags to add or update on the topic'),
    removeTagKeys: z.array(z.string()).optional().describe('Tag keys to remove from the topic'),
  }))
  .output(z.object({
    topicArn: z.string().describe('ARN of the updated topic'),
    updatedAttributes: z.array(z.string()).describe('List of attribute names that were updated'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.contentBasedDeduplication !== undefined) {
      attrUpdates.push({ name: 'ContentBasedDeduplication', value: String(ctx.input.contentBasedDeduplication) });
    }
    ...
    if (ctx.input.addTags && Object.keys(ctx.input.addTags).length > 0) {
      await client.tagResource(ctx.input.topicArn, ctx.input.addTags as Record<string, string>);
