...
let subscriptionSchema = z.object({
  ...
  protocol: z.string().describe('Delivery protocol (http, https, email, sms, sqs, lambda, etc.)'),
...
export let listSubscriptions = SlateTool.create(
  ...
  .input(z.object({
    topicArn: z.string().optional().describe('Optionally filter subscriptions by topic ARN. If omitted, lists all subscriptions in the account.'),
