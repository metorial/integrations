...
export let topicNotification = SlateTrigger.create(
  spec,
  {
    name: 'Topic Notification',
    key: 'topic_notification',
    description: 'Receives notifications from SNS topic HTTP/HTTPS subscriptions. Handles subscription confirmation automatically and processes incoming messages, including notification and unsubscribe confirmation events.',
  }
)
  .input(z.object({
    notificationType: z.enum(['Notification', 'SubscriptionConfirmation', 'UnsubscribeConfirmation']).describe('Type of SNS message'),
    ...
    subscribeUrl: z.string().optional().describe('URL to confirm subscription (for confirmation messages)'),
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      if (snsMessageType === 'UnsubscribeConfirmation' || body.Type === 'UnsubscribeConfirmation') {
        return {
          inputs: [{
