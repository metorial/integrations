...
export let convertImage = SlateTool.create(
  spec,
  {
    name: 'Convert Image',
    key: 'convert_image',
  ...
  .input(z.object({
    sourceUrl: z.string().describe('Publicly accessible URL of the image to compress and convert'),
    targetType: z.union([
      z.string(),
      z.array(z.string()),
    ]).describe('Target MIME type(s). Single type like "image/webp", array like ["image/webp","image/png"] for smallest, or "*/*" for auto-smallest.'),
    background: z.string().optional().describe('Background color for transparent images when converting to non-transparent format. Hex value like "#ffffff", or "white"/"black".'),
    preserve: z.array(z.enum(['copyright', 'creation', 'location'])).optional().describe('Metadata to preserve. "creation" and "location" are JPEG only.'),
  }))
  .output(z.object({
