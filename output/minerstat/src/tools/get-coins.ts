...
export let getCoinsTool = SlateTool.create(
  spec,
  {
    name: 'Get Coins',
    key: 'get_coins',
  ...
  .input(z.object({
    tickers: z.string().optional().describe('Comma-separated coin tickers to filter (e.g. "BTC,BCH,BSV")'),
    algorithm: z.string().optional().describe('Comma-separated algorithm names to filter (e.g. "SHA-256,Scrypt")')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    if (!ctx.auth.developerApiKey) {
      throw new Error('Developer API key is required to query coin data. Please provide it in your authentication credentials.');
    }
