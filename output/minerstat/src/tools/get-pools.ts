...
export let getPoolsTool = SlateTool.create(
  spec,
  {
    name: 'Get Pools',
    key: 'get_pools',
  ...
  .handleInvocation(async (ctx) => {
    if (!ctx.auth.developerApiKey) {
      throw new Error('Developer API key is required to query pool data. Please provide it in your authentication credentials.');
    }
