...
export let manageTagsTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    tagName: z.string().optional().describe('Tag name with prefix, e.g. "POOL:ethermine" or "WALLET:myBtc" (required for create/update/delete)'),
    address: z.string().optional().describe('Address for the tag (required for create/update)'),
    tagType: z.enum(['pools', 'wallets']).optional().describe('Filter by tag type (for list action)'),
  ...
  .handleInvocation(async (ctx) => {
    if (!ctx.auth.token) {
      throw new Error('Bearer token is required for tag management. Use the "API Credentials" authentication method.');
    }
