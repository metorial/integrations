...
let guardianSchema = z.object({
  ...
  }).optional().describe('Guardian profile information'),
});
...
export let manageGuardians = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    studentId: z.string().describe('User ID or email of the student. Use "me" for the current user.'),
    action: z.enum(['list', 'invite', 'list_invitations', 'cancel_invitation', 'remove'])
      .describe('The guardian action to perform'),
    guardianId: z.string().optional().describe('Guardian ID (required for remove)'),
    invitedEmailAddress: z.string().optional().describe('Guardian email address (required for invite)'),
    invitationId: z.string().optional().describe('Invitation ID (required for cancel_invitation)'),
  ...
  .output(z.object({
    ...
    invitations: z.array(guardianInvitationSchema).optional().describe('List of guardian invitations'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'list') {
      let result = await client.listGuardians(studentId, ctx.input.pageSize, ctx.input.pageToken);
    ...
    if (action === 'invite') {
      ...
      let result = await client.createGuardianInvitation(studentId, ctx.input.invitedEmailAddress);
      return {
    ...
    if (action === 'cancel_invitation') {
      ...
      let result = await client.patchGuardianInvitation(studentId, ctx.input.invitationId, 'COMPLETE');
      return {
        output: { invitation: result, success: true },
