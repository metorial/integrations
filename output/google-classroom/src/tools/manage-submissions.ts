...
let submissionSchema = z.object({
  ...
  state: z.string().optional().describe('Submission state (NEW, CREATED, TURNED_IN, RETURNED, RECLAIMED_BY_STUDENT)'),
...
export let manageSubmissions = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'grade', 'turn_in', 'return', 'reclaim']).describe('The submission action to perform'),
    submissionId: z.string().optional().describe('Submission ID (required for grade, turn_in, return, reclaim)'),
    ...
    assignedGrade: z.number().optional().describe('Grade to assign to the submission (for grade)'),
    draftGrade: z.number().optional().describe('Draft grade to set on the submission (for grade)'),
    pageSize: z.number().optional().describe('Maximum number of submissions to return (for list)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'grade') {
      ...
      let result = await client.patchStudentSubmission(
        courseId, courseWorkId, submissionId, updateFields, maskParts.join(',')
      );
