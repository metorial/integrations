...
let columnDefinitionSchema = z.object({
  ...
  dataType: z.string().describe('MySQL data type (e.g., VARCHAR(255), INT, BIGINT, TEXT, DATETIME, JSON, DECIMAL(10,2))'),
  nullable: z.boolean().optional().default(true).describe('Whether the column allows NULL values'),
  defaultValue: z.string().optional().describe('SQL expression for the default value (e.g., "CURRENT_TIMESTAMP", "0", "NULL")'),
  autoIncrement: z.boolean().optional().default(false).describe('Whether this column auto-increments'),
...
For altering tables, supports adding columns, dropping columns, renaming columns, modifying column types, and renaming the table.`,
  ...
  .input(z.object({
    ...
    engine: z.string().optional().default('InnoDB').describe('Storage engine for create action (default: InnoDB)'),
    charset: z.string().optional().describe('Default character set (e.g., utf8mb4)'),
    collation: z.string().optional().describe('Default collation (e.g., utf8mb4_unicode_ci)'),
    ...
    modifyColumns: z.array(z.object({
    ...
    })).optional().describe('Columns to modify'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      for (let col of ctx.input.columns) {
        ...
        if (col.references) {
          let refTable = qualifiedTableName(col.references.tableName, col.references.databaseName);
        ...
        for (let modify of ctx.input.modifyColumns) {
          ...
          if (modify.newDataType) {
            ...
            if (modify.dropDefault) {
              statements.push(`ALTER TABLE ${fullTableName} ALTER COLUMN ${escapeIdentifier(modify.columnName)} DROP DEFAULT`);
            }
            if (modify.defaultValue) {
              statements.push(`ALTER TABLE ${fullTableName} ALTER COLUMN ${escapeIdentifier(modify.columnName)} SET DEFAULT ${modify.defaultValue}`);
            }
          }
        }
      }
      ...
      if (statements.length === 0) {
        throw new Error('No alter operations specified. Provide addColumns, dropColumns, renameColumn, modifyColumns, or renameTable.');
      }
