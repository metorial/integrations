...
Requires a WHERE clause to target specific rows unless confirmDeleteAll is explicitly set.`,
  ...
  .input(z.object({
    ...
    where: z.string().optional().describe('SQL WHERE clause (without the WHERE keyword). Required unless confirmDeleteAll is true.'),
    confirmDeleteAll: z.boolean().optional().default(false).describe('Set to true to confirm deleting all rows when no WHERE clause is provided'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!ctx.input.where && !ctx.input.confirmDeleteAll) {
      throw new Error('A WHERE clause is required to delete rows. Set confirmDeleteAll to true to delete all rows.');
    }
