...
Requires a WHERE clause to target specific rows unless confirmUpdateAll is explicitly set.`,
  ...
  .input(z.object({
    ...
    values: z.record(z.string(), z.any()).describe('Object of column names and their new values'),
    where: z.string().optional().describe('SQL WHERE clause (without the WHERE keyword). Required unless confirmUpdateAll is true.'),
    confirmUpdateAll: z.boolean().optional().default(false).describe('Set to true to confirm updating all rows when no WHERE clause is provided'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!ctx.input.where && !ctx.input.confirmUpdateAll) {
      throw new Error('A WHERE clause is required to update rows. Set confirmUpdateAll to true to update all rows.');
    }

    let setClauses = Object.entries(ctx.input.values).map(([col, val]) =>
      `${escapeIdentifier(col)} = ${formatValue(val)}`
    );
    ...
    ctx.info(`Updating rows in ${fullTableName}${ctx.input.where ? ` where ${ctx.input.where}` : ' (all rows)'}`);
