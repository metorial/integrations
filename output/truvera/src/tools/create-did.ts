...
export let createDid = SlateTool.create(
  spec,
  {
    name: 'Create DID',
    key: 'create_did',
  ...
  .input(z.object({
    didType: z.enum(['cheqd', 'key']).default('cheqd').describe('DID method type. "cheqd" creates an on-chain DID; "key" creates a non-registry DID.'),
    keyType: z.enum(['ed25519', 'bjj']).default('ed25519').describe('Cryptographic key type. Use "bjj" for BBS+ zero-knowledge proof credentials.'),
