...
export let issueCredential = SlateTool.create(
  ...
  .input(z.object({
    ...
    credentialTypes: z.array(z.string()).optional().describe('Credential type identifiers (e.g. ["BasicCredential"])'),
    ...
    subject: z.any().describe('Credential subject data. Can be a single object or array of objects. Should include "id" (holder DID) and claim attributes.'),
    ...
    registryId: z.string().optional().describe('Revocation registry ID to enable revocation for this credential'),
    revocable: z.boolean().optional().describe('Whether the credential should be revocable'),
    algorithm: z.enum(['ed25519', 'dockbbs']).default('ed25519').describe('Signature algorithm. Use "dockbbs" for BBS+ zero-knowledge proofs.'),
    format: z.enum(['jsonld', 'jwt', 'sdjwt']).default('jsonld').describe('Credential format'),
    persist: z.boolean().optional().describe('Whether to persist the credential on Truvera (encrypted)'),
    password: z.string().optional().describe('Password for encrypted persistence (4-32 characters)'),
    recipientEmail: z.string().optional().describe('Email address to send the credential to'),
    recipientDid: z.string().optional().describe('DID of the credential recipient for wallet delivery'),
    distribute: z.boolean().optional().describe('Whether to distribute the credential to the holder'),
