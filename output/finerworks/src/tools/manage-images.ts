...
let imageSchema = z.object({
  ...
  products: z.array(imageProductSchema).optional().describe('Associated virtual inventory products'),
});
...
export let listImages = SlateTool.create(
  ...
  .input(z.object({
    ...
    sortField: z.enum(['id', 'title', 'date_added']).optional().default('id').describe('Field to sort by'),
    sortDirection: z.enum(['ASC', 'DESC']).optional().default('DESC').describe('Sort direction'),
    uploadDateFrom: z.string().optional().describe('Filter images uploaded after this date (ISO 8601)'),
    uploadDateTo: z.string().optional().describe('Filter images uploaded before this date (ISO 8601)'),
    includeProducts: z.boolean().optional().default(true).describe('Include associated virtual inventory products'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: `Found **${images.length}** image(s) (page ${ctx.input.pageNumber ?? 1}, ${data.count ?? 0} total). ${images.slice(0, 5).map((i: any) => `"${i.title ?? i.fileName ?? i.guid}"`).join(', ')}${images.length > 5 ? '...' : ''}`,
    };
  }).build();

export let addImages = SlateTool.create(
  spec,
  {
    name: 'Add Images',
    key: 'add_images',
  ...
  .input(z.object({
    images: z.array(z.object({
      ...
      privateHiresUri: z.string().describe('URL to the high-resolution image file for printing'),
      pixelWidth: z.number().int().describe('Image width in pixels'),
      pixelHeight: z.number().int().describe('Image height in pixels'),
    })).min(1).max(5).describe('Images to add'),
  }))
  .output(z.object({
...
export let updateImages = SlateTool.create(
  spec,
  {
    name: 'Update Images',
    key: 'update_images',
  ...
  .input(z.object({
    ...
    })).min(1).describe('Images to update'),
  }))
  .output(z.object({
