...
export let listTasks = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Page size is limited to 1–10 items per page.',
    ],
  }
)
  .input(z.object({
    ...
    status: z.string().optional().describe('Filter by task status: "successful", "failed", or "in-progress"'),
    fromDate: z.number().optional().describe('Filter tasks created after this Unix timestamp'),
    toDate: z.number().optional().describe('Filter tasks created before this Unix timestamp'),
    sort: z.string().optional().describe('Sort order, e.g. "-createdAt" for newest first'),
    bulkRunId: z.string().optional().describe('Filter tasks belonging to a specific bulk run'),
  }))
  .output(z.object({
