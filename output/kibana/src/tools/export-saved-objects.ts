...
export let exportSavedObjects = SlateTool.create(
  spec,
  {
...
Specify either object types to export all objects of those types, or provide specific object IDs to export selectively.`,
  ...
  .input(z.object({
    types: z.array(z.string()).optional().describe('Types of saved objects to export (e.g., ["dashboard", "visualization"])'),
    objects: z.array(z.object({
      type: z.string().describe('Type of the saved object'),
      objectId: z.string().describe('ID of the saved object')
    })).optional().describe('Specific saved objects to export'),
    includeReferencesDeep: z.boolean().optional().describe('Include all referenced objects recursively (default false)'),
    excludeExportDetails: z.boolean().optional().describe('Exclude the export summary object from the output')
  }))
  .output(z.object({
