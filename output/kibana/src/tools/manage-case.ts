...
export let searchCases = SlateTool.create(
  spec,
  {
    name: 'Search Cases',
    key: 'search_cases',
  ...
  .input(z.object({
    ...
    status: z.enum(['open', 'in-progress', 'closed']).optional().describe('Filter by case status'),
...
export let manageCase = SlateTool.create(
  spec,
  {
    name: 'Manage Case',
    key: 'manage_case',
  ...
  .input(z.object({
    ...
    caseId: z.string().optional().describe('ID of the case (required for get, update, delete)'),
    version: z.string().optional().describe('Case version (required for update, retrieved from get)'),
    title: z.string().optional().describe('Title of the case (required for create)'),
    description: z.string().optional().describe('Description of the case (required for create)'),
    owner: z.string().optional().describe('Owner application: "securitySolution", "observability", or "cases" (required for create)'),
    status: z.enum(['open', 'in-progress', 'closed']).optional().describe('Case status'),
    severity: z.enum(['low', 'medium', 'high', 'critical']).optional().describe('Case severity'),
  ...
  .handleInvocation(async (ctx) => {
    let client = createClient(ctx);
    let { action, caseId, version, title, description, owner, status, severity, tags, includeComments } = ctx.input;
...
export let addCaseComment = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    owner: z.string().describe('Owner application: "securitySolution", "observability", or "cases"'),
    ...
    alertIndex: z.string().optional().describe('Alert index name (required when attaching an alert)')
  }))
  .output(z.object({
