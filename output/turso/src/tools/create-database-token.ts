...
export let createDatabaseToken = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    expiration: z.string().optional().describe('Token expiration (e.g., "2w1d30m", "never")'),
    authorization: z.enum(['full-access', 'read-only']).optional().describe('Token authorization level'),
  }))
  .output(z.object({
