...
export let createConnection = SlateTool.create(
  ...
  .input(z.object({
    ...
    config: z.record(z.string(), z.unknown()).optional().describe('Plugin-specific connection configuration'),
...
export let updateConnection = SlateTool.create(
  ...
  .input(z.object({
    ...
    config: z.record(z.string(), z.unknown()).optional().describe('Updated plugin-specific connection configuration'),
  }))
  .output(connectionOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new Client({
...
export let testConnection = SlateTool.create(
  spec,
  {
