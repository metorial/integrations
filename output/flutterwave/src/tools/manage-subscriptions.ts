...
export let manageSubscriptions = SlateTool.create(
  ...
  .input(z.object({
    ...
    subscriptionId: z.number().optional().describe('Subscription ID (required for cancel/activate)'),
    ...
    status: z.enum(['active', 'cancelled']).optional().describe('Filter by status (for list)'),
  }))
  .output(z.object({
