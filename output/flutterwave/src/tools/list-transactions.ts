...
export let listTransactions = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['successful', 'failed', 'pending']).optional().describe('Filter by transaction status'),
    ...
    paymentType: z.string().optional().describe('Filter by payment type (card, account, ussd, mobilemoney)'),
  }))
  .output(z.object({
