...
export let createProject = SlateTool.create(
  ...
  .input(z.object({
    ...
    planPeriodFrom: z.string().optional().describe('Planning period start (ISO 8601 datetime)'),
    planPeriodTo: z.string().optional().describe('Planning period end (ISO 8601 datetime)'),
    equipmentPeriodFrom: z.string().optional().describe('Equipment usage start (ISO 8601 datetime)'),
    equipmentPeriodTo: z.string().optional().describe('Equipment usage end (ISO 8601 datetime)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.equipmentPeriodFrom) body.equipment_period_from = ctx.input.equipmentPeriodFrom;
