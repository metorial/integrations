...
export let restoreBranch = SlateTool.create(
  spec,
  {
    name: 'Restore Branch',
    key: 'restore_branch',
  ...
  .input(z.object({
    ...
    preserveUnderName: z.string().optional().describe('Name to preserve the current branch state under before restoring'),
  }))
  .output(z.object({
