...
let suppressionEventInputSchema = z.object({
  ...
  suppressionType: z.string().optional().describe('Suppression type (e.g., unsubscribed, hard_bounce, spam_complaint, manually_suppressed)'),
...
export let suppressionEventsTrigger = SlateTrigger.create(spec, {
  ...
  description: 'Triggers when an email is added to or removed from the suppression list in Remarkety. Suppression types include unsubscribed, hard_bounce, spam_complaint, and manually_suppressed.'
})
  .input(suppressionEventInputSchema)
  .output(z.object({
    email: z.string().describe('Suppressed email address'),
    timestamp: z.string().describe('Event timestamp'),
    suppressionType: z.string().optional().describe('Type of suppression (e.g., unsubscribed, hard_bounce, spam_complaint, manually_suppressed)'),
    reason: z.string().optional().describe('Reason for suppression action'),
    clientIpAddress: z.string().optional().describe('Client IP address associated with the action')
  }))
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      let type = typeMap[ctx.input.eventType] || `email_suppression.${ctx.input.eventType.replace('email-suppression/', '')}`;
