...
export let trackEvent = SlateTool.create(
  ...
  .input(z.object({
    ...
    anonymousId: z.string().optional().describe('Anonymous identifier for users not yet identified'),
    event: z.string().describe('Name of the event (e.g. "Order Completed")'),
    properties: z.record(z.string(), z.any()).optional().describe('Properties associated with the event'),
    context: z.record(z.string(), z.any()).optional().describe('Context object with additional info like IP, userAgent, etc.'),
    timestamp: z.string().optional().describe('ISO 8601 timestamp for the event (for historical import)'),
    integrations: z.record(z.string(), z.any()).optional().describe('Selective destination forwarding (e.g. {"All": true, "Mixpanel": false})'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    if (!ctx.auth.writeKey) {
      throw new Error('A write key is required to use the Tracking API. Configure it in your authentication settings.');
    }
