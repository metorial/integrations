...
export let browseCatalog = SlateTool.create(
  ...
  .input(z.object({
    catalogType: z.enum(['sources', 'destinations', 'warehouses']).describe('Type of catalog to browse'),
    metadataId: z.string().optional().describe('Specific metadata ID to get details for (sources/destinations only)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let rawItems = result?.sourceCatalogItems ?? result?.destinationsCatalog ?? result?.warehousesCatalog ?? [];
