...
export let getUsage = SlateTool.create(
  ...
  .input(z.object({
    metricType: z.enum(['api_calls', 'mtu', 'delivery']).describe('Type of usage metric to query'),
    period: z.string().optional().describe('Usage period (e.g. "current", "2024-01" for API calls/MTU)'),
    destinationId: z.string().optional().describe('Destination ID (required for delivery metrics)'),
    ...
    granularity: z.string().optional().describe('Granularity: "DAY" or "HOUR" (for delivery metrics)'),
  }))
  .output(z.object({
