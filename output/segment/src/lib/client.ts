...
export class SegmentClient {
  ...
  async listConnectedDestinationsFromSource(sourceId: string, params: { count?: number; cursor?: string } = {}) {
    let pagination: Record<string, any> = {};
    if (params.count) pagination['pagination.count'] = params.count;
    if (params.cursor) pagination['pagination.cursor'] = params.cursor;

    let response = await this.http.get(`/v1/sources/${sourceId}/destinations`, { params: pagination });
    return response.data?.data;
  }

  async listConnectedWarehousesFromSource(sourceId: string, params: { count?: number; cursor?: string } = {}) {
    let pagination: Record<string, any> = {};
    if (params.count) pagination['pagination.count'] = params.count;
    if (params.cursor) pagination['pagination.cursor'] = params.cursor;

    let response = await this.http.get(`/v1/sources/${sourceId}/warehouses`, { params: pagination });
    return response.data?.data;
  }
  ...
  async listDeliveryMetrics(destinationId: string, params: {
    sourceId?: string;
    startTime?: string;
    endTime?: string;
    granularity?: string;
    ...
    let response = await this.http.get(`/v1/destinations/${destinationId}/delivery-metrics`, { params: query });
    return response.data?.data;
  }

  // ─── Destination Subscriptions ──────────────────────────────────

  async listDestinationSubscriptions(destinationId: string, params: { count?: number; cursor?: string } = {}) {
    let pagination: Record<string, any> = {};
    if (params.count) pagination['pagination.count'] = params.count;
    if (params.cursor) pagination['pagination.cursor'] = params.cursor;

    let response = await this.http.get(`/v1/destinations/${destinationId}/subscriptions`, { params: pagination });
    return response.data?.data;
  }

  async createDestinationSubscription(destinationId: string, data: {
    name: string;
    actionId: string;
    trigger: string;
    enabled?: boolean;
    ...
    let response = await this.http.post(`/v1/destinations/${destinationId}/subscriptions`, data);
    return response.data?.data?.subscription;
  }

  async updateDestinationSubscription(destinationId: string, subscriptionId: string, data: {
    name?: string;
    trigger?: string;
    enabled?: boolean;
    settings?: Record<string, any>;
  }) {
    let response = await this.http.patch(`/v1/destinations/${destinationId}/subscriptions/${subscriptionId}`, data);
    return response.data?.data?.subscription;
  }

  async removeDestinationSubscription(destinationId: string, subscriptionId: string) {
    await this.http.delete(`/v1/destinations/${destinationId}/subscriptions/${subscriptionId}`);
  }

  // ─── Destination Filters ───────────────────────────────────────

  async listDestinationFilters(destinationId: string, params: { count?: number; cursor?: string } = {}) {
    let pagination: Record<string, any> = {};
    if (params.count) pagination['pagination.count'] = params.count;
    if (params.cursor) pagination['pagination.cursor'] = params.cursor;

    let response = await this.http.get(`/v1/destinations/${destinationId}/filters`, { params: pagination });
    return response.data?.data;
  }

  async getDestinationFilter(destinationId: string, filterId: string) {
    let response = await this.http.get(`/v1/destinations/${destinationId}/filters/${filterId}`);
    return response.data?.data?.filter;
  }

  async createDestinationFilter(destinationId: string, data: {
    sourceId: string;
    title: string;
    description?: string;
    if: string;
  ...
  async updateDestinationFilter(destinationId: string, filterId: string, data: {
    title?: string;
    description?: string;
    if?: string;
    actions?: Array<{
    ...
    let response = await this.http.patch(`/v1/destinations/${destinationId}/filters/${filterId}`, data);
    return response.data?.data?.filter;
  }
  ...
  async previewDestinationFilter(destinationId: string, data: {
    filter: { if: string; actions: Array<{ type: string; fields?: Record<string, any> }> };
    input: Record<string, any>;
  }) {
    let response = await this.http.post(`/v1/destinations/${destinationId}/filters/preview`, data);
    return response.data?.data;
  }
  ...
  async listTrackingPlanRules(trackingPlanId: string, params: { count?: number; cursor?: string } = {}) {
    let pagination: Record<string, any> = {};
    if (params.count) pagination['pagination.count'] = params.count;
    if (params.cursor) pagination['pagination.cursor'] = params.cursor;

    let response = await this.http.get(`/v1/tracking-plans/${trackingPlanId}/rules`, { params: pagination });
    return response.data?.data;
  }

  async updateTrackingPlanRules(trackingPlanId: string, data: {
  ...
  async replaceTrackingPlanRules(trackingPlanId: string, data: {
    rules: Array<Record<string, any>>;
  }) {
  ...
  async removeTrackingPlanRules(trackingPlanId: string, data: {
    rules: Array<Record<string, any>>;
  }) {
    let response = await this.http.delete(`/v1/tracking-plans/${trackingPlanId}/rules`, { data });
    return response.data?.data;
  }

  async listSourcesFromTrackingPlan(trackingPlanId: string, params: { count?: number; cursor?: string } = {}) {
    let pagination: Record<string, any> = {};
    if (params.count) pagination['pagination.count'] = params.count;
    if (params.cursor) pagination['pagination.cursor'] = params.cursor;

    let response = await this.http.get(`/v1/tracking-plans/${trackingPlanId}/sources`, { params: pagination });
    return response.data?.data;
  }

  async connectSourceToTrackingPlan(trackingPlanId: string, sourceId: string) {
    let response = await this.http.post(`/v1/tracking-plans/${trackingPlanId}/sources`, { sourceId });
    return response.data?.data;
  }
  ...
  async addSourceToWarehouse(warehouseId: string, sourceId: string) {
    let response = await this.http.post(`/v1/warehouses/${warehouseId}/connected-sources/${sourceId}`);
    return response.data?.data;
  }
  ...
  async listConnectedSourcesFromWarehouse(warehouseId: string, params: { count?: number; cursor?: string } = {}) {
    let pagination: Record<string, any> = {};
    if (params.count) pagination['pagination.count'] = params.count;
    if (params.cursor) pagination['pagination.cursor'] = params.cursor;

    let response = await this.http.get(`/v1/warehouses/${warehouseId}/connected-sources`, { params: pagination });
    return response.data?.data;
  }
  ...
  async listFunctions(params: { count?: number; cursor?: string; resourceType?: string } = {}) {
    let query: Record<string, any> = {};
  ...
  async createFunction(data: {
    code: string;
    settings?: Array<{ name: string; label: string; description?: string; type: string; required?: boolean }>;
    displayName: string;
    resourceType: string;
    description?: string;
  }) {
  ...
  async updateReverseEtlModel(modelId: string, data: {
    name?: string;
    description?: string;
    enabled?: boolean;
    query?: string;
  ...
  async createSourceRegulation(sourceId: string, data: {
    regulationType: string;
    subjectType: string;
    subjectIds: string[];
  }) {
  ...
  async getApiCallUsage(params: { period: string; count?: number; cursor?: string } = { period: 'current' }) {
  ...
  async getMtuUsage(params: { period: string; count?: number; cursor?: string } = { period: 'current' }) {
    let query: Record<string, any> = { period: params.period };
    if (params.count) query['pagination.count'] = params.count;
    if (params.cursor) query['pagination.cursor'] = params.cursor;
