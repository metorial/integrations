...
export let marginSchema = z.object({
...
export let viewPortSchema = z.object({
...
}).optional().describe('Browser viewport configuration');

export let waitForSelectorSchema = z.object({
  selector: z.string().describe('CSS selector to wait for'),
  timeout: z.number().optional().describe('Maximum wait time in milliseconds'),
}).optional().describe('Wait for a CSS selector to appear before generating');

export let headerFooterTemplateSchema = z.object({
...
export let generatePreviewSchema = z.object({
...
}).optional().describe('Generate a thumbnail preview of the first page');
...
export let authenticationSchema = z.object({
  username: z.string().describe('Username for Basic Auth'),
  password: z.string().describe('Password for Basic Auth'),
}).optional().describe('HTTP Basic Authentication credentials');

export let waitUntilSchema = z.enum(['load', 'domcontentloaded', 'networkidle0', 'networkidle2']).optional().describe('Page load completion strategy');

export let paperFormatSchema = z.enum(['letter', 'legal', 'tabloid', 'ledger', 'a0', 'a1', 'a2', 'a3', 'a4', 'a5', 'a6']).optional().describe('Paper format');

export let imageTypeSchema = z.enum(['png', 'jpg', 'webp']).optional().describe('Output image format');

export let baseOptionsSchema = z.object({
  delay: z.number().optional().describe('Wait time in milliseconds after page loads before generating'),
  timeout: z.number().optional().describe('Maximum page load time in milliseconds (default 30000)'),
  waitUntil: waitUntilSchema,
  waitForSelector: waitForSelectorSchema,
  autoScroll: z.boolean().optional().describe('Scroll page to bottom before generating to trigger lazy-loaded content'),
  timeZone: z.string().optional().describe('Browser timezone using IANA name (e.g. "America/New_York")'),
  ...
  width: z.string().optional().describe('Override width using CSS units (e.g. "8.5in", "210mm")'),
  height: z.string().optional().describe('Override height using CSS units (e.g. "11in", "297mm")'),
});

export let pdfOptionsSchema = baseOptionsSchema.extend({
  format: paperFormatSchema,
  margin: marginSchema,
  pageRanges: z.string().optional().describe('Page ranges to print (e.g. "1-5", "8", "1-3, 5")'),
  printBackground: z.boolean().optional().describe('Include background graphics (default true)'),
  preferCSSPageSize: z.boolean().optional().describe('Use CSS @page size instead of format parameter'),
...
export let imageOptionsSchema = baseOptionsSchema.extend({
  ...
  transparent: z.boolean().optional().describe('Render with transparent background (PNG/WebP only)'),
...
export let jobResponseSchema = z.object({
  ...
  previewUrl: z.string().optional().describe('URL to the preview image (if preview was requested)'),
