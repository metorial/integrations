...
export let listInvoices = SlateTool.create(
  ...
  .input(z.object({
    view: z.enum(['all', 'recent_open_or_overdue', 'open', 'overdue', 'open_or_overdue', 'draft', 'paid', 'scheduled_to_email', 'thank_you_emails', 'reminder_emails', 'last_N_months']).optional().describe('Filter invoices by status'),
    sort: z.enum(['created_at', '-created_at', 'updated_at', '-updated_at']).optional().describe('Sort order. Prefix with - for descending.'),
    updatedSince: z.string().optional().describe('ISO 8601 timestamp. Only return invoices updated after this time.'),
