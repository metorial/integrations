...
export let createExpense = SlateTool.create(
  ...
  .input(z.object({
    ...
    rebillType: z.enum(['Unbilled', 'Billed']).optional().describe('Whether this expense can be rebilled'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.nativeGrossValue) expenseData.native_gross_value = ctx.input.nativeGrossValue;
    ...
    if (ctx.input.manualSalesTaxAmount) expenseData.manual_sales_tax_amount = ctx.input.manualSalesTaxAmount;
