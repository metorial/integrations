...
export let getProfitAndLoss = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    fromDate: z.string().optional().describe('Start date in YYYY-MM-DD format (defaults to accounting year start)'),
    toDate: z.string().optional().describe('End date in YYYY-MM-DD format (defaults to today)'),
  }))
  .output(z.object({
...
export let getBalanceSheet = SlateTool.create(
  spec,
  {
...
export let getTrialBalance = SlateTool.create(
  spec,
  {
