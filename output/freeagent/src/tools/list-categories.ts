...
export let listCategories = SlateTool.create(
  ...
  .input(z.object({
    subAccounts: z.boolean().optional().describe('Include sub-accounts instead of top-level accounts'),
  }))
  .output(z.object({
    categories: z.record(z.string(), z.any()).describe('Categories grouped by type (admin_expenses_categories, cost_of_sales_categories, income_categories, general_categories)'),
  }))
