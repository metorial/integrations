...
export let emailInvoice = SlateTool.create(
  spec,
  {
    name: 'Email Invoice',
    key: 'email_invoice',
  ...
  .input(z.object({
    ...
    useTemplate: z.boolean().optional().describe('Use the saved email template instead of custom subject/body'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.emailToSender !== undefined) emailParams.email.email_to_sender = ctx.input.emailToSender;
