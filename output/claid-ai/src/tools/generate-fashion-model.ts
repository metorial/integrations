...
export let generateFashionModel = SlateTool.create(
  spec,
  {
...
This is an async operation — a task ID is returned immediately for status polling.`,
  ...
  .input(z.object({
    ...
    modelImageUrl: z.string().optional().describe('URL of the desired model image. Omit for a randomly chosen model.'),
    numberOfImages: z.number().min(1).max(4).optional().describe('Number of output images (1–4, default: 1)'),
    pose: z.string().optional().describe('Pose description (e.g. "full body, front view, neutral stance, arms relaxed")'),
    background: z.string().optional().describe('Background description (e.g. "minimalistic studio background")'),
    aspectRatio: z.enum(['1:1', '2:3', '3:2', '3:4', '4:3', '9:16', '16:9', '21:9', '4:5', '5:4']).optional().describe('Output aspect ratio'),
    outputFormat: z.enum(['png', 'jpeg']).optional().describe('Output image format (default: png)'),
