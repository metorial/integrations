...
let placementSchema = z.object({
  type: z.enum(['absolute', 'original', 'padding']).optional().describe('Placement type: "absolute" for manual control, "original" to keep input position, "padding" for uniform spacing'),
  scale: z.number().min(0.01).max(1).optional().describe('Object scale relative to scene (0.01–1.0)'),
  positionX: z.number().min(0).max(1).optional().describe('Horizontal position (0–1, left to right)'),
  positionY: z.number().min(0).max(1).optional().describe('Vertical position (0–1, top to bottom)'),
  rotationDegree: z.number().min(0).max(360).optional().describe('Rotation in degrees (0–360)'),
}).optional().describe('Object placement configuration');

let promptSceneSchema = z.object({
  prompt: z.string().optional().describe('Text prompt describing the background scene. Omit to use Autoprompt AI.'),
  negativePrompt: z.string().optional().describe('Elements to exclude from the generated background'),
  preference: z.enum(['fast', 'optimal', 'best']).optional().describe('Quality preset balancing speed vs. quality'),
  steps: z.number().min(1).max(49).optional().describe('Inference steps (1–49) for quality control'),
}).optional().describe('Prompt-based scene generation');

let templateSceneSchema = z.object({
  templateUrl: z.string().describe('URL of the reference background template image'),
  templateMode: z.enum(['transform', 'adjust', 'lock']).optional().describe('How to apply the template'),
  view: z.enum(['top', 'front']).optional().describe('Camera perspective'),
  prompt: z.string().optional().describe('Additional prompt to customize the templated scene'),
}).optional().describe('Template-based scene generation');

let effectSceneSchema = z.object({
  ...
  aspectRatio: z.string().optional().describe('Output aspect ratio (e.g. "1:1", "4:3", "16:9")'),
}).optional().describe('Effect-based background');

export let generateBackground = SlateTool.create(
  spec,
  {
...
Provide a product image URL and configure object placement, scene type, and output preferences.`,
  ...
  .input(z.object({
    ...
    numberOfImages: z.number().min(1).max(4).optional().describe('Number of images to generate (1–4)'),
    outputFormat: z.enum(['png', 'jpeg', 'webp', 'avif']).optional().describe('Output format'),
  ...
  .output(z.object({
    ...
    }).optional().describe('Input image metadata'),
