...
export let getMessageStatus = SlateTool.create(
  ...
  .input(z.object({
    requestIds: z.array(z.string()).min(1).describe('Request IDs from Send Prepared Message responses')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let results = Array.isArray(result.results) ? result.results : [result.results].filter(Boolean);
