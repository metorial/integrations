...
let channelSchema = z.object({
  ...
  value: z.string().describe('Channel identifier (email address, phone number, push token, etc.)')
});
...
let addressSchema = z.object({
  ...
  countryCode: z.string().optional().describe('ISO 3166-1 alpha-2 country code (e.g., "US", "GB")')
}).optional();
...
Supports setting channels (email, SMS, Viber, etc.), personal info, address, custom fields, and segment membership.`,
  ...
  .input(z.object({
    channels: z.array(channelSchema).min(1).describe('At least one contact channel is required'),
    ...
    groups: z.array(z.string()).optional().describe('Static segment names to add the contact to'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.externalCustomerId) payload.externalCustomerId = ctx.input.externalCustomerId;
