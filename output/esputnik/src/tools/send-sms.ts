...
If the phone number does not match an existing contact, a new contact will be created automatically.`,
  ...
  .input(z.object({
    phoneNumber: z.string().describe('Recipient phone number in international format (e.g., "+380501234567")'),
    text: z.string().optional().describe('SMS message text (required if not using a template)'),
    ...
    contactId: z.number().optional().describe('eSputnik contact ID to associate the message with'),
    externalCustomerId: z.string().optional().describe('External customer ID to associate the message with')
  }))
  .output(z.object({
