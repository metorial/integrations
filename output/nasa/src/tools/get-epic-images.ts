...
let epicImageSchema = z.object({
  ...
  centroidCoordinates: z.object({
    lat: z.number().describe('Centroid latitude'),
    lon: z.number().describe('Centroid longitude'),
  }).optional().describe('Center point coordinates of the Earth image'),
});

export let getEpicImages = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    collection: z.enum(['natural', 'enhanced']).optional().describe('Image collection type. Defaults to natural color.'),
    date: z.string().optional().describe('Specific date (YYYY-MM-DD) to retrieve images for. Omit for most recent.'),
    listDates: z.boolean().optional().describe('Set to true to list all available dates instead of retrieving images.'),
  }))
  .output(z.object({
    images: z.array(epicImageSchema).optional().describe('List of EPIC images'),
    availableDates: z.array(z.string()).optional().describe('List of available dates when listDates is true'),
  }))
