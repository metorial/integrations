...
export let lookupSmallBody = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    designation: z.string().optional().describe('IAU designation (e.g., "433" for Eros, "1P" for Halley\'s Comet)'),
    spkId: z.number().optional().describe('JPL SPK-ID number'),
    searchName: z.string().optional().describe('Search string matching against names (e.g., "eros", "halley")'),
  }))
  .output(z.object({
    object: z.record(z.string(), z.any()).optional().describe('Object identification information'),
    orbit: z.record(z.string(), z.any()).optional().describe('Orbital elements and parameters'),
