...
export let getEarthImagery = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    date: z.string().optional().describe('Date (YYYY-MM-DD) for the imagery. Defaults to most recent available.'),
    dim: z.number().optional().describe('Width and height of the image in degrees. Default is 0.025.'),
    assetsOnly: z.boolean().optional().describe('Set to true to return only asset metadata (date, resource URL) without the actual image.'),
  }))
  .output(z.object({
    imageUrl: z.string().optional().describe('URL to the satellite image'),
    date: z.string().optional().describe('Date of the imagery'),
    resourceUrl: z.string().optional().describe('URL to the imagery resource/scene on cloud storage'),
  }))
