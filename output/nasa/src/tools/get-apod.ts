...
export let getApod = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    date: z.string().optional().describe('Specific date to retrieve (YYYY-MM-DD). Defaults to today.'),
    startDate: z.string().optional().describe('Start of date range (YYYY-MM-DD). Use with endDate for a range of entries.'),
    endDate: z.string().optional().describe('End of date range (YYYY-MM-DD). Defaults to today if startDate is set.'),
    count: z.number().optional().describe('Number of random APOD entries to return. Cannot be used with date or date range.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let first = entries[0];
    let label = entries.length === 1 && first
