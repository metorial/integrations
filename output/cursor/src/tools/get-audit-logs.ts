...
export let getAuditLogs = SlateTool.create(
  ...
  .input(z.object({
    startTime: z.string().optional().describe('Start time (ISO 8601, relative like "7d", or epoch). Defaults to 7 days ago.'),
    endTime: z.string().optional().describe('End time (ISO 8601, relative like "now", or epoch). Defaults to now.'),
    eventTypes: z.string().optional().describe('Comma-separated event types to filter (e.g. "login,logout,member_added")'),
    search: z.string().optional().describe('Search term to filter events'),
    page: z.number().optional().describe('Page number (default 1)'),
    pageSize: z.number().max(500).optional().describe('Results per page (default 100, max 500)'),
    users: z.string().optional().describe('Filter by user email addresses or encoded user IDs'),
  }))
  .output(z.object({
