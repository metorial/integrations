...
export let launchAgent = SlateTool.create(
  spec,
  {
    name: 'Launch Agent',
    key: 'launch_agent',
  ...
  .input(z.object({
    ...
    promptImages: z.array(imageSchema).max(5).optional().describe('Up to 5 images to include with the prompt'),
    model: z.string().optional().describe('AI model to use (omit for automatic selection)'),
    sourceRepository: z.string().optional().describe('GitHub repository URL (e.g. https://github.com/org/repo)'),
    sourceRef: z.string().optional().describe('Git ref (branch, tag, commit) to start from'),
    sourcePrUrl: z.string().optional().describe('Pull request URL to work on instead of a repository'),
    autoCreatePr: z.boolean().optional().describe('Whether to automatically create a PR with changes'),
    ...
    openAsCursorGithubApp: z.boolean().optional().describe('Open the PR as the Cursor GitHub App'),
    skipReviewerRequest: z.boolean().optional().describe('Skip requesting reviewers on the PR'),
    webhookUrl: z.string().optional().describe('URL to receive webhook notifications on agent completion'),
    webhookSecret: z.string().optional().describe('Secret for signing webhook payloads (min 32 characters)'),
  }))
  .output(z.object({
