...
export let generateVideo = SlateTool.create(
  ...
  .input(z.object({
    ...
    })).optional().describe('List of modifications for Overlay/Multi Overlay build packs'),
    frames: z.array(z.array(z.object({
      name: z.string().describe('Layer name'),
      text: z.string().optional().describe('Text content'),
      image_url: z.string().optional().describe('Image URL'),
    }))).optional().describe('Frames for Multi Overlay build pack, each frame is a list of modifications'),
    frameDurations: z.array(z.number()).optional().describe('Duration in seconds for each frame in Multi Overlay'),
    trimToLengthInSeconds: z.number().optional().describe('Trim the output video to this length'),
    ...
    createGifPreview: z.boolean().optional().describe('Also generate a GIF preview of the video'),
