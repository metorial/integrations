...
export let manageTemplate = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'duplicate', 'delete', 'import']).describe('Operation to perform'),
    templateUid: z.string().optional().describe('UID of the template (required for update, duplicate, delete)'),
    ...
    tags: z.array(z.string()).optional().describe('Tags to assign to the template (for create or update)'),
    metadata: z.string().optional().describe('Custom metadata (for create or update)'),
    publicationIds: z.array(z.string()).optional().describe('IDs of library publications to import (for import action)'),
  }))
  .output(z.object({
    templateUid: z.string().optional().describe('UID of the created/updated/duplicated template'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update') {
      if (!ctx.input.templateUid) throw new Error('templateUid is required for updating a template');
    ...
    if (action === 'duplicate') {
      if (!ctx.input.templateUid) throw new Error('templateUid is required for duplicating a template');
    ...
    if (action === 'delete') {
      if (!ctx.input.templateUid) throw new Error('templateUid is required for deleting a template');
