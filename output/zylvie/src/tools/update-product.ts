...
export let updateProduct = SlateTool.create(
  ...
  .input(z.object({
    ...
    pricingModel: z.enum(['one-time', 'subscription', 'delayed']).optional().describe('New pricing model'),
    display: z.enum(['featured', 'listed', 'unlisted', 'unpublished']).optional().describe('New display visibility'),
    ...
    interval: z.enum(['day', 'week', 'month', 'year']).optional().describe('Billing interval'),
    ...
    collectAddressAndPhone: z.boolean().optional().describe('Whether to collect buyer address and phone'),
    ...
    categories: z.array(z.string()).optional().describe('List of categories (replaces existing)'),
    tags: z.array(z.string()).optional().describe('List of tags (replaces existing)'),
    productImages: z.array(z.string()).optional().describe('List of image URLs (replaces existing)'),
    productFiles: z.array(z.string()).optional().describe('List of file URLs (replaces existing)'),
    ...
    redemptionInstructions: z.string().optional().describe('License key redemption instructions (HTML)'),

    excludeFromAutomations: z.boolean().optional().describe('Whether to exclude from automations'),
  }))
  .output(z.object({
