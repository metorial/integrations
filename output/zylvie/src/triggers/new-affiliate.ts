...
export let newAffiliateTrigger = SlateTrigger.create(
  spec,
  {
    name: 'New Affiliate Sign-Up',
    key: 'new_affiliate',
    description: 'Triggers when a visitor signs up to be an affiliate or accepts an invitation to become one. Includes affiliate details, commission rates, and allowed products.',
  }
)
  ...
  .output(z.object({
    ...
    allowedForAllProducts: z.boolean().describe('Whether affiliate can promote all products'),
    allowedProducts: z.array(productSchema).describe('Specific products the affiliate can promote'),
  }))
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      let allowedProducts = ((productsData.allowed_products || []) as Record<string, unknown>[]).map(mapProduct);
