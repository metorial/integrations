...
export let newPagePost = SlateTrigger.create(
  spec,
  {
    name: 'New Page Post',
    key: 'new_page_post',
    description: '[Polling fallback] Polls a Facebook Page for new posts. Triggers whenever a new post is published to the specified Page.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let state = ctx.state as { lastPollTime?: string; pageId?: string; knownPostIds?: string[] } | null;
