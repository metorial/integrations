...
export let manageProject = SlateTool.create(
  ...
  .input(z.object({
    ...
    description: z.string().optional().describe('Project description'),
    purpose: z.string().optional().describe('Project purpose (e.g., "Web Application", "Service or API")'),
    environment: z.string().optional().describe('Environment: Development, Staging, or Production'),
...
export let manageProjectResources = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    resourceUrns: z.array(z.string()).optional().describe('Resource URNs to assign (required for assign)')
  }))
  .output(z.object({
    ...
    })).optional().describe('Project resources')
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
    ...
    let resources = await client.assignResourcesToProject(ctx.input.projectId, ctx.input.resourceUrns);
