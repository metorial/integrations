...
let mediaAttachment = z.object({
  ...
  options: z.record(z.string(), z.unknown()).optional().describe('Platform-specific media options')
});

let facebookOptions = z.object({
  postType: z.enum(['REELS', 'STORY']).optional().describe('Post type. Omit for standard post'),
...
let instagramOptions = z.object({
  postType: z.number().optional().describe('0 = Post, 2 = Story. Reels auto-detected from media'),
  firstComment: z.boolean().optional().describe('Enable first comment'),
  firstCommentContent: z.string().optional().describe('Content of the first comment'),
...
let twitterOptions = z.object({
  threads: z.array(twitterThread).optional().describe('Thread chain with multiple tweets')
}).optional();

let tiktokOptions = z.object({
  title: z.string().optional().describe('Caption with auto-detected hashtags/mentions'),
  privacyLevel: z.enum(['PUBLIC_TO_EVERYONE', 'MUTUAL_FOLLOW_FRIENDS', 'FOLLOWER_OF_CREATOR', 'SELF_ONLY']).describe('Privacy level (required for TikTok)'),
...
let mastodonOptions = z.object({
  visibility: z.enum(['public', 'unlisted', 'private', 'direct']).optional().describe('Post visibility'),
  firstCommentEnabled: z.boolean().optional().describe('Enable first comment'),
  firstCommentContent: z.string().optional().describe('Content of the first comment'),
  pollOptions: z.array(z.string()).optional().describe('Poll choices (up to 4, max 25 chars each)'),
  pollExpiresIn: z.number().optional().describe('Poll expiration in seconds (300/900/3600/18000/86400/259200/432000)'),
  pollMultiple: z.boolean().optional().describe('Allow multiple poll selections')
}).optional();

let threadsOptions = z.object({
  link: z.string().optional().describe('Link to embed')
}).optional();

let platformOptions = z.object({
...
}).optional().describe('Platform-specific options. Only provide options for the target channel\'s social network');
...
export let schedulePost = SlateTool.create(
  spec,
  {
    name: 'Schedule Post',
    key: 'schedule_post',
...
Posts can be scheduled for a future date or published immediately by omitting the publish time. Provide platform-specific options matching the target channel's social network.`,
  ...
  .input(z.object({
    ...
    publishOn: z.string().optional().describe('ISO 8601 datetime for scheduled publishing. Omit for immediate publishing'),
