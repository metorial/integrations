...
export let listPermissionsTool = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    permissions: z.array(z.object({
      ...
      principalType: z.string().optional().describe('Type of principal (user, group, domain, anyone)'),
      ...
      accessLevel: z.string().optional().describe('Access level (readonly, write, comment)'),
    })),
    aclSettings: z.object({
...
export let addPermissionTool = SlateTool.create(
  spec,
  {
    name: 'Add Permission',
    key: 'add_permission',
  ...
  .input(z.object({
    ...
    principalType: z.enum(['email', 'domain', 'anyone']).describe('Type of principal to grant access to'),
    principalEmail: z.string().optional().describe('Email address (required when principalType is "email")'),
    principalDomain: z.string().optional().describe('Domain name (required when principalType is "domain")'),
    suppressNotification: z.boolean().optional().describe('Suppress email notification to the principal'),
  }))
  .output(z.object({
...
export let removePermissionTool = SlateTool.create(
  spec,
  {
