...
export let createDocTool = SlateTool.create(
  spec,
  {
    name: 'Create Doc',
    key: 'create_doc',
  ...
  .input(z.object({
    ...
    sourceDocId: z.string().optional().describe('ID of an existing doc to copy as a template'),
    folderId: z.string().optional().describe('ID of the folder to create the doc in'),
    timezone: z.string().optional().describe('Timezone for the doc (e.g. "America/New_York")'),
  }))
  .output(z.object({
