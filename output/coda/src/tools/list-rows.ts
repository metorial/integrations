...
export let listRowsTool = SlateTool.create(
  spec,
  {
    name: 'List Rows',
    key: 'list_rows',
  ...
  .input(z.object({
    ...
    query: z.string().optional().describe('Filter rows by column value (format: "columnName:value")'),
    sortBy: z.string().optional().describe('Sort order (e.g. "natural" or "createdAt")'),
    useColumnNames: z.boolean().optional().describe('Use column names as keys instead of IDs'),
    valueFormat: z.enum(['simple', 'simpleWithArrays', 'rich']).optional().describe('Format for cell values'),
    syncToken: z.string().optional().describe('Token from a previous request to fetch only changed rows'),
  ...
  .output(z.object({
    ...
    nextSyncToken: z.string().optional().describe('Token for incremental sync in subsequent requests'),
  }))
