...
export let transcriptionCompleted = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers when a meeting has been processed and the transcript is ready. Configure the webhook URL in the Fireflies Developer Settings. The webhook payload is enriched with full transcript details including title, summary, attendees, and URLs.',
  }
)
  .input(z.object({
    meetingId: z.string().describe('The meeting/transcript ID'),
    eventType: z.string().describe('The event type from the webhook'),
    clientReferenceId: z.string().nullable().optional().describe('Custom reference ID if set during upload'),
  }))
  .output(z.object({
    ...
    clientReferenceId: z.string().nullable().describe('Custom reference ID if set during upload'),
    summary: z.object({
    ...
    }).nullable().describe('AI-generated meeting summary'),
  }))
  .webhook({
