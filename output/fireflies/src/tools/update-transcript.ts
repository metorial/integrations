...
export let updateTranscript = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    privacy: z.enum(['link', 'owner', 'participants', 'teammatesandparticipants', 'teammates']).optional().describe('New privacy level for the meeting'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.privacy) {
      let result = await client.updateMeetingPrivacy(ctx.input.transcriptId, ctx.input.privacy);
