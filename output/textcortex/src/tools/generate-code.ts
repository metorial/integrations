...
export let generateCode = SlateTool.create(
  spec,
  {
    name: 'Generate Code',
    key: 'generate_code',
  ...
  .input(z.object({
    prompt: z.string().describe('Natural language description of the code or SQL query to generate'),
    programmingLanguage: z.string().optional().describe('Target programming language (e.g., "python", "javascript", "sql", "java", "go")'),
    ...
    generationCount: z.number().min(1).max(10).optional().describe('Number of code variations to generate (default: 1)'),
  }))
  .output(z.object({
    codeOutputs: z.array(z.object({
