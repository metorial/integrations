...
export let summarizeText = SlateTool.create(
  ...
  .input(z.object({
    ...
    fileId: z.string().optional().describe('TextCortex file ID to summarize (alternative to raw text)'),
    mode: z.enum(['default', 'embeddings']).optional().describe('Summarization mode. Default: "default"'),
    model: z.enum(['velox-1', 'alta-1', 'sophos-1', 'chat-sophos-1']).optional().describe('AI model to use'),
    maxTokens: z.number().optional().describe('Maximum number of tokens for the summary (default: 512)'),
    temperature: z.number().min(0).max(1).optional().describe('Creativity level from 0 to 1. Default: 0.7'),
    generationCount: z.number().min(1).max(5).optional().describe('Number of summary variations to generate (default: 1)'),
