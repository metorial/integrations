...
export let moderateChannel = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['ban', 'unban', 'mute', 'unmute', 'freeze', 'unfreeze']).describe('Moderation action to perform'),
    targetUserId: z.string().optional().describe('User ID to ban/unban/mute/unmute (required for user actions)'),
    seconds: z.number().optional().describe('Duration in seconds for ban/mute (-1 for indefinite)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let actionMsg = action === 'freeze' ? `Froze channel **${channelUrl}**.`
      : action === 'unfreeze' ? `Unfroze channel **${channelUrl}**.`
