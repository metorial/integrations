...
export let createUser = SlateTool.create(
  ...
  .input(z.object({
    ...
    issueAccessToken: z.boolean().optional().describe('Whether to issue an access token for the user'),
    issueSessionToken: z.boolean().optional().describe('Whether to issue a session token for the user'),
    metadata: z.record(z.string(), z.string()).optional().describe('Key-value metadata to attach to the user'),
  }))
  .output(z.object({
