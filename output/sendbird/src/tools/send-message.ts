...
export let sendMessage = SlateTool.create(
  ...
  .input(z.object({
    ...
    messageType: z.enum(['MESG', 'FILE', 'ADMM']).describe('MESG for text, FILE for file, ADMM for admin message'),
    userId: z.string().optional().describe('Sender user ID (required for MESG and FILE)'),
    message: z.string().optional().describe('Text content of the message (for MESG and ADMM)'),
    ...
    mentionType: z.enum(['users', 'channel']).optional().describe('Type of mention: "users" to mention specific users, "channel" to mention all'),
    mentionedUserIds: z.array(z.string()).optional().describe('User IDs to mention (when mentionType is "users")'),
