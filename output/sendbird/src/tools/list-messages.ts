...
export let listMessages = SlateTool.create(
  ...
  .input(z.object({
    ...
    prevLimit: z.number().optional().describe('Number of messages to load before the reference point (default 15)'),
    nextLimit: z.number().optional().describe('Number of messages to load after the reference point (default 15)'),
    reverse: z.boolean().optional().describe('If true, return messages in reverse chronological order'),
    senderId: z.string().optional().describe('Filter messages by sender user ID'),
    messageType: z.enum(['MESG', 'FILE', 'ADMM']).optional().describe('Filter by message type'),
