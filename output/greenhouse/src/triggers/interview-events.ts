...
export let interviewEventsTrigger = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers when interview-related events occur in Greenhouse, including interview deleted and scorecard deleted.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    interviewId: z.string().nullable().describe('The scheduled interview ID (for interview deleted events)'),
    scorecardId: z.string().nullable().describe('The scorecard ID (for scorecard deleted events)'),
