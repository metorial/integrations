...
export let candidateEventsTrigger = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers when candidate-related events occur in Greenhouse, including hired, rejected, stage change, merged, deleted, unhired, unrejected, updated, and anonymized.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    previousStage: z.string().nullable().describe('Previous stage name (for stage change events)'),
    newStage: z.string().nullable().describe('New stage name (for stage change events)'),
    rejectionReason: z.string().nullable().describe('Rejection reason (for rejection events)'),
    mergedCandidateId: z.string().nullable().describe('The ID of the candidate that was merged into (for merge events)'),
  }))
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: {
          ...
          rejectionReason: application?.rejection_reason?.name ?? payload?.rejection_reason?.name ?? null,
          mergedCandidateId: payload?.merged_candidate_id?.toString() ?? payload?.head_candidate?.id?.toString() ?? null,
        },
      };
    },
  }).build();
