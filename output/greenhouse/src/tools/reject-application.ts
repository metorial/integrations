...
export let rejectApplicationTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    emailTemplateId: z.string().optional().describe('Email template ID for the rejection email'),
    sendEmailAt: z.string().optional().describe('ISO 8601 timestamp for when to send the rejection email'),
  }))
  .output(z.object({
    success: z.boolean(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let rejectionEmail = ctx.input.sendRejectionEmail
      ? {
          ...
          emailTemplateId: ctx.input.emailTemplateId ? parseInt(ctx.input.emailTemplateId) : undefined,
        }
      : undefined;

    await client.rejectApplication(parseInt(ctx.input.applicationId), {
      rejectionReasonId: ctx.input.rejectionReasonId ? parseInt(ctx.input.rejectionReasonId) : undefined,
