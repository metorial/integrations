...
export let validateEmail = SlateTool.create(
  ...
  .output(z.object({
    ...
    suggestion: z.string().optional().describe('Suggested correction if a typo was detected'),
    hasValidAddressSyntax: z.boolean().optional().describe('Whether the email has valid syntax'),
    hasMxOrARecord: z.boolean().optional().describe('Whether the domain has MX or A records'),
    isSuspectedDisposableAddress: z.boolean().optional().describe('Whether the address appears disposable'),
    isSuspectedRoleAddress: z.boolean().optional().describe('Whether the address appears to be a role address (e.g. admin@)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {
        ...
        hasValidAddressSyntax: r.checks?.domain?.has_valid_address_syntax ?? r.has_valid_address_syntax ?? undefined,
        hasMxOrARecord: r.checks?.domain?.has_mx_or_a_record ?? r.has_mx_or_a_record ?? undefined,
        isSuspectedDisposableAddress: r.checks?.additional?.is_suspected_disposable_address ?? r.is_suspected_disposable_address ?? undefined,
        isSuspectedRoleAddress: r.checks?.additional?.is_suspected_role_address ?? r.is_suspected_role_address ?? undefined,
      },
