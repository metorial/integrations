...
Use \`groupBy\` to control the aggregation (e.g., "count" for totals, "timeseries" for time-based data, "countries" for geographic breakdown).`,
  ...
  .input(z.object({
    event: z.enum(['clicks', 'leads', 'sales', 'composite']).optional().describe('Event type to analyze (default: clicks)'),
    groupBy: z.enum([
      'count', 'timeseries', 'continents', 'regions', 'countries', 'cities',
      'devices', 'browsers', 'os', 'trigger', 'referers', 'referer_urls',
      'top_links', 'top_urls', 'top_domains', 'top_folders', 'top_link_tags',
      'utm_sources', 'utm_mediums', 'utm_campaigns', 'utm_terms', 'utm_contents',
      'top_partners'
    ]).optional().describe('How to group/aggregate the analytics data'),
    ...
    interval: z.enum(['24h', '7d', '30d', '90d', '1y', 'mtd', 'qtd', 'ytd', 'all']).optional().describe('Time interval (default: 24h)'),
    start: z.string().optional().describe('Start date/time in ISO format (overrides interval)'),
