...
export let createLink = SlateTool.create(
  ...
  .input(z.object({
    ...
    domain: z.string().optional().describe('Custom domain for the short link (defaults to workspace primary domain)'),
    slug: z.string().optional().describe('Custom slug/key for the short link (random 7-char generated if omitted)'),
    externalId: z.string().optional().describe('Your own system ID to associate with the link'),
    tenantId: z.string().optional().describe('Tenant identifier within your system'),
    trackConversion: z.boolean().optional().describe('Enable conversion tracking for this link'),
    ...
    tagNames: z.array(z.string()).optional().describe('Tag names to assign (case-insensitive, tags created if not existing)'),
    ...
    rewrite: z.boolean().optional().describe('Enable link cloaking (shows your domain in browser)'),
    ...
    geo: z.record(z.string(), z.string()).optional().describe('Country-based targeting: { "US": "https://us.example.com" }'),
    ...
    webhookIds: z.array(z.string()).optional().describe('Webhook IDs to trigger on link clicks')
  }))
  .output(z.object({
