...
Use "merge" with optional merge strategy and close-source-branch settings.`,
  ...
  .input(z.object({
    ...
    action: z.enum(['approve', 'unapprove', 'merge', 'decline', 'request_changes', 'remove_change_request']).describe('Action to perform'),
    mergeStrategy: z.enum(['merge_commit', 'squash', 'fast_forward']).optional().describe('Merge strategy (only for "merge" action)'),
    closeSourceBranch: z.boolean().optional().describe('Close source branch after merge (only for "merge" action)'),
    mergeMessage: z.string().optional().describe('Custom merge commit message (only for "merge" action)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (action) {
      ...
      case 'merge': {
        ...
        if (ctx.input.closeSourceBranch !== undefined) mergeBody.close_source_branch = ctx.input.closeSourceBranch;
