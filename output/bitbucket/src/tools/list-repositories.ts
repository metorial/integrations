...
export let listRepositoriesTool = SlateTool.create(
  spec,
  {
...
Use the **query** parameter for Bitbucket's query language filtering (e.g. \`name ~ "myrepo"\`).`,
  ...
  .input(z.object({
    query: z.string().optional().describe('Bitbucket query language filter (e.g. name ~ "myrepo")'),
    sort: z.string().optional().describe('Field to sort by, prefix with - for descending (e.g. "-updated_on")'),
