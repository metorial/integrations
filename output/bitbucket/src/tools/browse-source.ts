...
export let browseSourceTool = SlateTool.create(
  spec,
  {
    name: 'Browse Source',
    key: 'browse_source',
...
Returns directory listings or file contents depending on the path.`,
  ...
  .input(z.object({
    ...
    path: z.string().optional().describe('File or directory path (e.g. "src/index.ts"). Leave empty for the root directory.'),
  }))
  .output(z.object({
