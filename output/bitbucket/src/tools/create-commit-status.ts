...
export let createCommitStatusTool = SlateTool.create(
  spec,
  {
...
The **key** uniquely identifies the status — updating with the same key replaces the previous status.`,
  ...
  .input(z.object({
    ...
    state: z.enum(['SUCCESSFUL', 'FAILED', 'INPROGRESS', 'STOPPED']).describe('Build status state'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let status = await client.createCommitStatus(ctx.input.repoSlug, ctx.input.commitHash, body);
