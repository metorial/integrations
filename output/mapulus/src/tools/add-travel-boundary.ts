...
export let addTravelBoundary = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    mode: z.string().optional().describe('Travel mode: "driving", "walking", "cycling", or "transit"'),
    value: z.number().optional().describe('Travel time or distance value'),
    unit: z.string().optional().describe('Unit for the value: "minutes", "kilometers", "miles"'),
  }))
  .output(z.object({
    ...
    }).passthrough().optional().describe('The travel boundary configuration'),
  }).passthrough())
