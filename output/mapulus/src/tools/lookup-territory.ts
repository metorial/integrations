...
export let lookupTerritory = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    boundaryType: z.string().describe('Type of boundary: "suburb", "postcode", "lga", or other supported Australian boundary type'),
    boundaryName: z.string().describe('Name or identifier of the boundary (e.g. suburb name, postcode number, LGA name)'),
  }))
  .output(z.object({
    locations: z.array(locationSchema).describe('Locations found within the specified territory'),
