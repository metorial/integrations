...
export let chatbotkitEventsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'ChatBotKit Events',
    key: 'chatbotkit_events',
    description: 'Receives webhook events from ChatBotKit including bot activity, conversation events, and integration operations. Configure the webhook URL in your ChatBotKit dashboard settings.',
    instructions: [
  ...
  .input(z.object({
    eventType: z.string().describe('Event type (e.g. bot.created, conversation.completed, integration.updated)'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let inputs = events.map((event: any) => {
        ...
        let eventId = event.id || event.eventId || `${eventType}-${event.createdAt || Date.now()}`;
