...
export let createUploadSessionTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    conflictBehavior: z.enum(['rename', 'replace', 'fail']).optional().describe('What to do if a file with the same name exists'),
  }))
  .output(z.object({
    uploadUrl: z.string().describe('URL to upload file chunks to via PUT requests with Content-Range headers'),
    expirationDateTime: z.string().describe('ISO 8601 timestamp when the upload session expires'),
  }))
