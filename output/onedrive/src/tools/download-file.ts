...
export let downloadFileTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    itemPath: z.string().optional().describe('Path to the file (e.g. "/Documents/report.docx")'),
    format: z.string().optional().describe('Convert the file to this format before download (e.g. "pdf", "html", "jpg"). Only supported for certain file types.'),
  }))
  .output(z.object({
    downloadUrl: z.string().describe('Pre-authenticated short-lived URL to download the file content'),
