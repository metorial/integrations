...
export let copyItemTool = SlateTool.create(
  ...
  .input(z.object({
    driveId: z.string().optional().describe('ID of the source drive. Defaults to the user\'s personal OneDrive.'),
    ...
    destinationDriveId: z.string().optional().describe('ID of the destination drive if different from source'),
    newName: z.string().optional().describe('New name for the copy. If omitted, the original name is used.'),
  }))
  .output(z.object({
