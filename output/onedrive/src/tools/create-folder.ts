...
export let createFolderTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    parentPath: z.string().optional().describe('Path to the parent folder (e.g. "/Documents")'),
    folderName: z.string().describe('Name for the new folder'),
    conflictBehavior: z.enum(['rename', 'replace', 'fail']).optional().describe('What to do if a folder with the same name exists. Defaults to "fail".'),
  }))
  .output(z.object({
