...
let knowledgeGraphSchema = z.object({
...
export let webSearchTool = SlateTool.create(
  spec,
  {
    name: 'Web Search',
    key: 'web_search',
  ...
  .input(z.object({
    ...
    engine: z.enum(['google', 'bing', 'duckduckgo', 'yahoo', 'yandex', 'baidu', 'naver']).default('google').describe('Search engine to use'),
    location: z.string().optional().describe('Location to simulate search from (e.g., "Austin, Texas")'),
    ...
    page: z.number().optional().describe('Page number (0-indexed offset for Google, 1-indexed for others)'),
    ...
    dateRange: z.string().optional().describe('Date range filter (e.g., "d" for past day, "w" for past week, "m" for past month, "y" for past year)'),
  ...
  .output(z.object({
    searchMetadata: z.object({
      ...
      timeTaken: z.number().optional().describe('Time taken to process the search in seconds'),
    }).optional(),
    ...
    relatedSearches: z.array(relatedSearchSchema).optional().describe('Related search suggestions'),
    relatedQuestions: z.array(relatedQuestionSchema).optional().describe('People also ask questions'),
  }))
