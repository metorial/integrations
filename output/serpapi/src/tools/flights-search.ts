...
let flightResultSchema = z.object({
  ...
  })).optional().describe('Layover details'),
});

export let flightsSearchTool = SlateTool.create(
  spec,
  {
    name: 'Flights Search',
    key: 'flights_search',
  ...
  .input(z.object({
    departureAirport: z.string().describe('Departure airport IATA code (e.g., "JFK", "LAX")'),
    ...
    travelClass: z.enum(['1', '2', '3', '4']).optional().describe('Travel class: 1=Economy, 2=Premium Economy, 3=Business, 4=First'),
    adults: z.number().optional().describe('Number of adult passengers'),
    stops: z.enum(['0', '1', '2', '3']).optional().describe('Max stops: 0=non-stop, 1=up to 1, 2=up to 2, 3=any'),
  ...
  .output(z.object({
    ...
    priceInsights: z.object({
      ...
      priceLevel: z.string().optional().describe('Price level indicator (low, typical, high)'),
    }).optional().describe('Price insight information'),
  }))
