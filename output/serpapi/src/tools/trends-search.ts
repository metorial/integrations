...
export let trendsSearchTool = SlateTool.create(
  spec,
  {
    name: 'Google Trends',
    key: 'google_trends',
  ...
  .input(z.object({
    query: z.string().optional().describe('Search term to analyze trends for (required unless fetching trending searches)'),
    dataType: z.enum(['interest_over_time', 'compared_breakdown_by_region', 'related_queries', 'related_topics', 'trending_now']).default('interest_over_time').describe('Type of trends data to retrieve'),
    geo: z.string().optional().describe('Geographic filter (e.g., "US", "GB", "US-CA" for California)'),
    category: z.number().optional().describe('Google Trends category ID'),
    timeRange: z.string().optional().describe('Time range (e.g., "today 12-m" for past 12 months, "today 5-y" for past 5 years, "2020-01-01 2023-12-31" for custom range)'),
    language: z.string().optional().describe('Language code'),
    noCache: z.boolean().optional().describe('Force fresh results'),
  }))
  .output(z.object({
    interestOverTime: z.array(z.object({
    ...
    })).optional().describe('Interest over time data points'),
    interestByRegion: z.array(z.object({
      location: z.string().optional(),
      value: z.number().optional(),
      extractedValue: z.number().optional(),
      maxValueIndex: z.number().optional(),
    })).optional().describe('Interest by region/subregion'),
    relatedQueries: z.array(z.object({
      query: z.string().optional(),
      value: z.string().optional(),
      extractedValue: z.number().optional(),
      link: z.string().optional(),
    })).optional().describe('Related search queries'),
    relatedTopics: z.array(z.object({
      ...
      extractedValue: z.number().optional(),
      link: z.string().optional(),
    })).optional().describe('Related topics'),
    trendingSearches: z.array(z.object({
      query: z.string().optional(),
      link: z.string().optional(),
    })).optional().describe('Currently trending searches'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let interestByRegion = (data.compared_breakdown_by_region || data.interest_by_region || []).map((d: any) => ({
    ...
    let relatedQueries = (data.related_queries?.rising || data.related_queries?.top || []).map((d: any) => ({
      query: d.query,
      value: d.value,
      extractedValue: d.extracted_value,
      link: d.link,
    ...
    let relatedTopics = (data.related_topics?.rising || data.related_topics?.top || []).map((d: any) => ({
      title: d.topic?.title,
      type: d.topic?.type,
      value: d.value,
      extractedValue: d.extracted_value,
    ...
    let trendingSearches = (data.trending_searches || data.daily_search_trends || []).map((d: any) => ({
    ...
    return {
      ...
      message: ctx.input.dataType === 'trending_now'
