...
let scholarResultSchema = z.object({
  ...
  })).optional().describe('Paper authors'),
...
export let scholarSearchTool = SlateTool.create(
  spec,
  {
    name: 'Scholar Search',
    key: 'scholar_search',
  ...
  .input(z.object({
    ...
    cites: z.string().optional().describe('Google Scholar result ID to find papers that cite this paper'),
    authorId: z.string().optional().describe('Google Scholar author ID to retrieve author profile and publications'),
    ...
    page: z.number().optional().describe('Page number (0-indexed, each page has ~10 results)'),
  ...
  .output(z.object({
    ...
    }).optional().describe('Author profile information (when searching by authorId)'),
