...
export let videoSearchTool = SlateTool.create(
  spec,
  {
    name: 'Video Search',
    key: 'video_search',
  ...
  .input(z.object({
    ...
    engine: z.enum(['youtube', 'google_videos']).default('youtube').describe('Video search engine to use'),
    language: z.string().optional().describe('Language code (e.g., "en")'),
    country: z.string().optional().describe('Country code (e.g., "us")'),
    sortBy: z.string().optional().describe('YouTube sort/filter token (e.g., "CAI=" for upload date)'),
  ...
  .output(z.object({
    ...
    nextPageToken: z.string().optional().describe('Token for fetching the next page of results (YouTube)'),
  }))
