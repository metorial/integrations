...
export let auth = SlateAuth.create()
  ...
  .addCustomAuth({
    ...
    inputSchema: z.object({
      personalAccessKey: z.string().describe('Personal Access Key generated from My Organization > Security in the Appcircle dashboard'),
      authBaseUrl: z.string().default('https://auth.appcircle.io').describe('Auth base URL. Change for self-hosted deployments.'),
    }),
  ...
  .addCustomAuth({
    ...
    inputSchema: z.object({
      ...
      organizationId: z.string().optional().describe('Organization ID (required for sub-organizations)'),
      authBaseUrl: z.string().default('https://auth.appcircle.io').describe('Auth base URL. Change for self-hosted deployments.'),
    }),
    ...
    getOutput: async (ctx) => {
      ...
      let response = await http.post(
        `${ctx.input.authBaseUrl}/auth/v1/api-key/token`,
        body,
        {
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
