...
export class KaggleClient {
  ...
  async listCompetitionFiles(competitionName: string, params: {
    pageToken?: string;
    pageSize?: number;
  } = {}) {
  ...
  async downloadCompetitionFile(competitionName: string, fileName: string) {
    let response = await this.http.get(`/competitions/data/download/${competitionName}/${fileName}`, {
  ...
  async submitToCompetition(competitionName: string, blobFileTokens: string, submissionDescription: string) {
  ...
  async listCompetitionSubmissions(competitionName: string, params: {
    group?: string;
    page?: number;
    pageSize?: number;
  } = {}) {
  ...
  async getCompetitionLeaderboard(competitionName: string) {
    let response = await this.http.get(`/competitions/${competitionName}/leaderboard/download`);
    return response.data;
  }
  ...
  async listDatasetFiles(ownerSlug: string, datasetSlug: string, params: {
  ...
  async downloadDataset(ownerSlug: string, datasetSlug: string, params: {
    datasetVersionNumber?: number;
  } = {}) {
  ...
  async downloadDatasetFile(ownerSlug: string, datasetSlug: string, fileName: string, params: {
    datasetVersionNumber?: number;
  } = {}) {
    let response = await this.http.get(`/datasets/download/${ownerSlug}/${datasetSlug}/${fileName}`, {
      params: {
        datasetVersionNumber: params.datasetVersionNumber,
      },
      responseType: 'arraybuffer',
  ...
  async createDatasetVersion(ownerSlug: string, datasetSlug: string, body: {
    versionNotes: string;
    subtitle?: string;
    description?: string;
    files: Array<{ token: string; description?: string }>;
    ...
    let response = await this.http.post(`/datasets/create/version/${ownerSlug}/${datasetSlug}`, body);
    return response.data;
  }

  async uploadDatasetFile(fileName: string, contentLength: number, lastModifiedDateUtc: number) {
    let response = await this.http.post('/datasets/upload/file/' + encodeURIComponent(fileName), null, {
      params: {
        contentLength,
        lastModifiedDateUtc,
      },
  ...
  async updateModel(ownerSlug: string, modelSlug: string, body: {
    title?: string;
    subtitle?: string;
    isPrivate?: boolean;
    description?: string;
  ...
  async getModelInstance(ownerSlug: string, modelSlug: string, framework: string, variation: string) {
    let response = await this.http.get(`/models/${ownerSlug}/${modelSlug}/${framework}/${variation}`);
    return response.data;
  }

  async createModelInstance(ownerSlug: string, modelSlug: string, body: {
    framework: string;
    overview: string;
    instanceSlug: string;
    licenseName?: string;
    ...
    let response = await this.http.post(`/models/${ownerSlug}/${modelSlug}/create/instance`, body);
    return response.data;
  }

  async updateModelInstance(ownerSlug: string, modelSlug: string, framework: string, variation: string, body: {
    overview?: string;
    licenseName?: string;
    fineTunable?: boolean;
    trainingData?: string[];
    ...
    let response = await this.http.post(`/models/${ownerSlug}/${modelSlug}/${framework}/${variation}/update`, body);
    return response.data;
  }

  async deleteModelInstance(ownerSlug: string, modelSlug: string, framework: string, variation: string) {
    let response = await this.http.post(`/models/${ownerSlug}/${modelSlug}/${framework}/${variation}/delete`);
    return response.data;
  }

  async createModelInstanceVersion(ownerSlug: string, modelSlug: string, framework: string, variation: string, body: {
    versionNotes?: string;
    files: Array<{ token: string; description?: string }>;
  }) {
  ...
  async downloadModelInstance(ownerSlug: string, modelSlug: string, framework: string, variation: string, versionNumber?: number) {
  ...
  async deleteModelInstanceVersion(ownerSlug: string, modelSlug: string, framework: string, variation: string, versionNumber: number) {
    let response = await this.http.post(
