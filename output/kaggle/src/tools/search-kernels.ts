...
export let searchKernels = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    language: z.enum(['all', 'python', 'r', 'sqlite', 'julia']).optional().describe('Filter by programming language'),
    kernelType: z.enum(['all', 'notebook', 'script']).optional().describe('Filter by kernel type'),
    outputType: z.enum(['all', 'visualization', 'data']).optional().describe('Filter by output type'),
    sortBy: z.enum(['hotness', 'commentCount', 'dateCreated', 'dateRun', 'relevance', 'scoreAscending', 'scoreDescending', 'viewCount', 'voteCount']).optional().describe('Sort order for results'),
    dataset: z.string().optional().describe('Filter by dataset reference (owner/dataset-slug)'),
