...
export let checkAsyncStatus = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    statusId: z.string().describe('The status ID returned from an async document creation request.'),
  }))
  .output(z.object({
    status: z.enum(['queued', 'working', 'completed', 'failed', 'killed']).describe('Current processing status of the document.'),
    downloadUrl: z.string().optional().describe('URL to download the completed document. Only present when status is "completed".'),
    numberOfPages: z.number().optional().describe('Number of pages in the generated document. Only present when completed.'),
    message: z.string().optional().describe('Status message from DocRaptor.'),
    validationErrors: z.string().optional().describe('Error details if the document failed to generate.'),
  }))
