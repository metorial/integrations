...
export let leadFormSubmit = SlateTrigger.create(
  spec,
  {
...
The webhook URL must be configured in the lead form extension settings within Google Ads. Each lead includes the submitted user data, campaign/ad group context, and a Google Click ID.`,
  ...
  .input(z.object({
    ...
    assetGroupId: z.string().optional().describe('Asset group ID (only for Performance Max campaigns)'),
    gclid: z.string().optional().describe('Google Click ID'),
    isTest: z.boolean().optional().describe('Whether this is a test lead'),
    userColumnData: z.array(z.object({
      columnId: z.string().describe('Data type identifier (e.g., FULL_NAME, EMAIL, PHONE_NUMBER, POSTAL_CODE, CITY, COUNTRY)'),
      stringValue: z.string().describe('The submitted value'),
    })).optional().describe('User-submitted form data'),
  ...
  .output(z.object({
    ...
    isTest: z.boolean().optional().describe('Whether this is a test lead'),
    formId: z.string().optional().describe('Lead form ID'),
    userData: z.record(z.string(), z.string()).optional().describe('User-submitted data as key-value pairs (e.g., { "FULL_NAME": "John Doe", "EMAIL": "john@example.com" })'),
  }))
  .webhook({
    handleRequest: async (ctx) => {
      ...
      if (Array.isArray(data.user_column_data || data.userColumnData)) {
        input.userColumnData = (data.user_column_data || data.userColumnData).map((col: any) => ({
