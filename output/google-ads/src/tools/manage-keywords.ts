...
export let manageKeywords = SlateTool.create(
  spec,
  {
...
Keywords determine when ads are shown based on user search queries. Each keyword has a match type controlling how broadly it matches search terms.`,
  ...
  .input(z.object({
    ...
    adGroupId: z.string().optional().describe('Ad group ID (required for ad group keywords)'),
    campaignId: z.string().optional().describe('Campaign ID (required for campaign-level negative keywords)'),
    ...
    matchType: z.enum(['EXACT', 'PHRASE', 'BROAD']).optional().describe('Keyword match type'),
  ...
  .output(z.object({
    criterionResourceName: z.string().optional().describe('Resource name of the keyword criterion'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.isNegative && ctx.input.campaignId) {
      ...
      if (operation === 'create') {
        ...
        return {
          output: { criterionResourceName: result.results?.[0]?.resourceName, mutateResults: result },
    ...
    // Ad group keywords
    if (operation === 'remove') {
      if (!ctx.input.adGroupId || !ctx.input.criterionId) throw new Error('adGroupId and criterionId required for remove');
      ...
      return {
        output: { criterionResourceName: `customers/${cid}/adGroupCriteria/${ctx.input.adGroupId}~${ctx.input.criterionId}`, mutateResults: result },
    ...
    // Update
    if (!ctx.input.adGroupId || !ctx.input.criterionId) throw new Error('adGroupId and criterionId required for update');
    ...
    if (ctx.input.cpcBidMicros !== undefined) { updateData.cpcBidMicros = ctx.input.cpcBidMicros; maskFields.push('cpcBidMicros'); }
    if (ctx.input.finalUrls !== undefined) { updateData.finalUrls = ctx.input.finalUrls; maskFields.push('finalUrls'); }
