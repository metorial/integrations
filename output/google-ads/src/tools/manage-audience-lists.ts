...
export let manageAudienceLists = SlateTool.create(
  spec,
  {
...
User lists can be applied to campaigns or ad groups for audience targeting, bid adjustments, or exclusions.`,
  ...
  .input(z.object({
    ...
    membershipLifeSpan: z.number().optional().describe('Number of days a user stays in the list (set 10000 for unlimited)'),
    membershipStatus: z.enum(['OPEN', 'CLOSED']).optional().describe('Whether the list is open or closed for new members'),
    listType: z.enum(['CRM_BASED', 'RULE_BASED', 'LOGICAL']).optional().describe('Type of user list (required for create)'),
    crmBasedUserList: z.object({
      uploadKeyType: z.enum(['CONTACT_INFO', 'CRM_ID', 'MOBILE_ADVERTISING_ID']).optional().describe('Type of CRM data'),
      dataSourceType: z.enum(['FIRST_PARTY', 'THIRD_PARTY_CREDIT_BUREAU', 'THIRD_PARTY_VOTER_FILE']).optional().describe('Source of the CRM data'),
    }).optional().describe('CRM-based user list configuration'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (operation === 'create') {
      ...
      if (ctx.input.membershipLifeSpan) listData.membershipLifeSpan = ctx.input.membershipLifeSpan;
      ...
      return {
        output: { userListResourceName: result.results?.[0]?.resourceName, mutateResults: result },
    ...
    if (ctx.input.description !== undefined) { updateData.description = ctx.input.description; maskFields.push('description'); }
    if (ctx.input.membershipLifeSpan !== undefined) { updateData.membershipLifeSpan = ctx.input.membershipLifeSpan; maskFields.push('membershipLifeSpan'); }
    if (ctx.input.membershipStatus !== undefined) { updateData.membershipStatus = ctx.input.membershipStatus; maskFields.push('membershipStatus'); }
