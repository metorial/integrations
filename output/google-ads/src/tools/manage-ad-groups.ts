...
export let manageAdGroups = SlateTool.create(
  spec,
  {
...
Supports setting the ad group name, status, type, CPC bid, and targeting URL.`,
  ...
  .input(z.object({
    ...
    type: z.enum(['SEARCH_STANDARD', 'DISPLAY_STANDARD', 'SHOPPING_PRODUCT_ADS', 'VIDEO_BUMPER', 'VIDEO_TRUE_VIEW_IN_STREAM', 'VIDEO_NON_SKIPPABLE_IN_STREAM']).optional().describe('Ad group type (required for create)'),
  ...
  .output(z.object({
    adGroupResourceName: z.string().optional().describe('Resource name of the created/updated ad group'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (operation === 'remove') {
      ...
      return {
        output: { adGroupResourceName: `customers/${cid}/adGroups/${ctx.input.adGroupId}`, mutateResults: result },
        message: `Ad group **${ctx.input.adGroupId}** removed.`,
      };
    }

    if (operation === 'create') {
      if (!ctx.input.campaignId) throw new Error('campaignId is required for create operation');
      ...
      return {
        output: { adGroupResourceName: result.results?.[0]?.resourceName, mutateResults: result },
