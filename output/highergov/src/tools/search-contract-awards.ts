...
export let searchContractAwards = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    awardType: z.enum(['prime', 'idv', 'subcontract']).default('prime').describe('Type of contract award to search'),
    awardId: z.string().optional().describe('Specific contract award ID'),
    parentAwardId: z.string().optional().describe('Parent award ID to find task orders under a contract'),
    ...
    lastModifiedDate: z.string().optional().describe('Filter to awards modified on or after this date (YYYY-MM-DD)'),
    naicsCode: z.string().optional().describe('NAICS code to filter by industry classification'),
    pscCode: z.string().optional().describe('Product Service Code to filter by'),
    searchId: z.string().optional().describe('HigherGov saved search ID (prime contracts only)'),
    vehicleKey: z.number().optional().describe('HigherGov contract vehicle key (prime and IDV only)'),
    ordering: z.string().optional().describe('Sort order (e.g. -action_date, -current_total_value_of_award)'),
  ...
  .output(z.object({
    awards: z.array(z.record(z.string(), z.unknown())).describe('List of contract award records'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let typeLabel = input.awardType === 'idv' ? 'IDV' : input.awardType === 'subcontract' ? 'subcontract' : 'prime contract';
