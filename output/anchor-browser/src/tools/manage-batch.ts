...
export let manageBatch = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'list', 'get', 'cancel', 'delete', 'retry']).describe('Operation to perform'),
    batchId: z.string().optional().describe('Batch ID (required for get, cancel, delete, retry)'),
    count: z.number().optional().describe('Number of sessions to create in the batch (1-1000, for create)'),
    sessionConfig: z.record(z.string(), z.unknown()).optional().describe('Session configuration to apply to all batch sessions (for create)'),
    metadata: z.record(z.string(), z.unknown()).optional().describe('Custom metadata for the batch (for create)'),
    ...
    retryFailedOnly: z.boolean().optional().describe('Only retry failed sessions (default: true)'),
    maxRetries: z.number().optional().describe('Maximum retries per session (1-3, default: 1)'),
  }))
  .output(z.object({
