...
export let createSession = SlateTool.create(
  ...
  .input(z.object({
    ...
    tags: z.array(z.string()).optional().describe('Tags for session organization and filtering'),
    enableRecording: z.boolean().optional().describe('Enable session recording for later review'),
    proxyCountry: z.string().optional().describe('Two-letter country code for residential proxy targeting (e.g. "US", "GB")'),
    maxDurationMinutes: z.number().optional().describe('Maximum session duration in minutes (default: 20)'),
    idleTimeoutMinutes: z.number().optional().describe('Idle timeout in minutes before session auto-terminates (default: 5)'),
    readOnlyLiveView: z.boolean().optional().describe('If true, live view is read-only (no interaction)'),
    profileName: z.string().optional().describe('Name of a browser profile to load (restores cookies/state)'),
    persistProfile: z.boolean().optional().describe('Whether to persist the browser profile when session ends'),
    ...
    extensionIds: z.array(z.string()).optional().describe('IDs of Chrome extensions to load in the session'),
    enableExtraStealth: z.boolean().optional().describe('Enable extra stealth mode to avoid bot detection'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (input.enableRecording !== undefined) session.recording = { active: input.enableRecording };
    ...
    if (input.readOnlyLiveView !== undefined) session.live_view = { read_only: input.readOnlyLiveView };
    ...
    if (input.enablePopupBlocker !== undefined) browser.popup_blocker = { active: input.enablePopupBlocker };
    if (input.enableCaptchaSolver !== undefined) browser.captcha_solver = { active: input.enableCaptchaSolver };
    ...
    if (input.enableExtraStealth !== undefined) browser.extra_stealth = { active: input.enableExtraStealth };
