...
let productOutputSchema = z.object({
  ...
  incomeAccount: z.object({
    accountId: z.string().optional(),
    name: z.string().optional()
  }).optional().describe('Income account associated with the product'),
  expenseAccount: z.object({
    accountId: z.string().optional(),
    name: z.string().optional()
  }).optional().describe('Expense account associated with the product'),
  defaultSalesTaxes: z.array(z.object({
    salesTaxId: z.string().optional(),
    name: z.string().optional()
  })).optional().describe('Default sales taxes for the product'),
...
let mapProduct = (p: any) => ({
  ...
  incomeAccount: p.incomeAccount ? { accountId: p.incomeAccount.id, name: p.incomeAccount.name } : undefined,
  expenseAccount: p.expenseAccount ? { accountId: p.expenseAccount.id, name: p.expenseAccount.name } : undefined,
  defaultSalesTaxes: p.defaultSalesTaxes?.map((t: any) => ({ salesTaxId: t.id, name: t.name })),
...
export let listProducts = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.listProducts(ctx.input.businessId, ctx.input.page || 1, ctx.input.pageSize || 20);
...
export let createProduct = SlateTool.create(
  ...
  .input(z.object({
    ...
    expenseAccountId: z.string().optional().describe('ID of the expense account to associate'),
    defaultSalesTaxIds: z.array(z.string()).optional().describe('IDs of default sales taxes to apply'),
    isSold: z.boolean().optional().describe('Whether the product is sold to customers'),
    isBought: z.boolean().optional().describe('Whether the product is purchased from vendors')
  }))
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!result.didSucceed) {
      throw new Error(`Failed to create product: ${result.inputErrors.map(e => e.message).join(', ')}`);
    }
...
export let updateProduct = SlateTool.create(
  ...
  .input(z.object({
    ...
    defaultSalesTaxIds: z.array(z.string()).optional().describe('Updated default sales tax IDs'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!result.didSucceed) {
      throw new Error(`Failed to update product: ${result.inputErrors.map(e => e.message).join(', ')}`);
    }
...
export let archiveProduct = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!result.didSucceed) {
      throw new Error(`Failed to archive product: ${result.inputErrors.map(e => e.message).join(', ')}`);
    }
