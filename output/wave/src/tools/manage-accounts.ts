...
let accountOutputSchema = z.object({
  ...
  }).optional().describe('Account type (ASSET, LIABILITY, EQUITY, INCOME, EXPENSE)'),
  subtype: z.object({
    name: z.string().optional(),
    value: z.string().optional(),
    type: z.object({
  ...
  }).optional().describe('Account subtype (e.g., CASH_AND_BANK, ACCOUNTS_RECEIVABLE, CREDIT_CARD)'),
  currency: z.object({
    code: z.string().optional(),
    symbol: z.string().optional(),
    name: z.string().optional()
  }).optional().describe('Account currency'),
  ...
  balanceInBusinessCurrency: z.number().optional().describe('Balance in business default currency'),
...
export let listAccounts = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.listAccounts(ctx.input.businessId, ctx.input.page || 1, ctx.input.pageSize || 20);
...
export let createAccount = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!result.didSucceed) {
      throw new Error(`Failed to create account: ${result.inputErrors.map(e => e.message).join(', ')}`);
    }
...
export let updateAccount = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!result.didSucceed) {
      throw new Error(`Failed to update account: ${result.inputErrors.map(e => e.message).join(', ')}`);
    }
...
export let archiveAccount = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!result.didSucceed) {
      throw new Error(`Failed to archive account: ${result.inputErrors.map(e => e.message).join(', ')}`);
    }
