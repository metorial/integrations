...
let customerOutputSchema = z.object({
  ...
  }).optional().describe('Currency preference'),
  address: z.object({
...
export let listCustomers = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.listCustomers(ctx.input.businessId, ctx.input.page || 1, ctx.input.pageSize || 20);
...
export let createCustomer = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!result.didSucceed) {
      throw new Error(`Failed to create customer: ${result.inputErrors.map(e => e.message).join(', ')}`);
    }
...
export let updateCustomer = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!result.didSucceed) {
      throw new Error(`Failed to update customer: ${result.inputErrors.map(e => e.message).join(', ')}`);
    }
...
export let deleteCustomer = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!result.didSucceed) {
      throw new Error(`Failed to delete customer: ${result.inputErrors.map(e => e.message).join(', ')}`);
    }
