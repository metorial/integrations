...
export let listAuthorizations = SlateTool.create(
  ...
  .input(z.object({
    status: z.enum(['gated', 'authorized', 'revoked']).optional().describe('Filter by authorization status'),
...
export let grantAuthorization = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    ndaBypass: z.boolean().optional().describe('Skip NDA signature requirement (default: false)'),
...
export let updateAuthorization = SlateTool.create(
  ...
  .input(z.object({
    ...
    accessGroupIds: z.array(z.string()).optional().describe('Updated list of access group IDs (replaces existing groups)'),
    status: z.literal('revoked').optional().describe('Set to "revoked" to revoke the authorization')
  }))
