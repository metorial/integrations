...
let questionnaireConnectionSchema = z.object({
...
let questionnaireSchema = z.object({
  ...
  status: z.string().describe('Status: processing, started, ready_for_review, approved, or completed'),
  ...
  originalFormat: z.string().optional().describe('Original format: spreadsheet, file, or portal'),
  crmLink: z.string().nullable().optional().describe('CRM link'),
...
export let listQuestionnaires = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    status: z.enum(['processing', 'started', 'ready_for_review', 'approved', 'completed']).optional().describe('Filter by questionnaire status'),
...
export let submitQuestionnaire = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    domain: z.string().describe('Customer domain that sent the questionnaire (e.g., "google.com")'),
    email: z.string().describe('Email of the person creating/submitting the questionnaire'),
    originalFormat: z.enum(['spreadsheet', 'file', 'portal']).describe('Questionnaire format'),
    questionnaireType: z.enum(['security_questionnaire', 'rfp']).optional().describe('Type (default: security_questionnaire)'),
