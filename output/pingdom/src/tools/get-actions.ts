...
export let getActions = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    limit: z.number().optional().describe('Maximum number of actions to return (default 100)'),
    ...
    status: z.string().optional().describe('Filter by status: "sent", "delivered", "error", "not_delivered", "no_credits"'),
    via: z.string().optional().describe('Filter by channel: "email", "sms", "twitter", "iphone", "android"'),
  }))
  .output(z.object({
    actions: z.object({
