...
export let updateCheck = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    resolution: z.number().optional().describe('Check interval in minutes: 1, 5, 15, 30, or 60'),
    ...
    sendNotificationWhenDown: z.number().optional().describe('Consecutive failures before alerting'),
    notifyAgainEvery: z.number().optional().describe('Notify again every N results while down'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.integrationIds !== undefined) data.integrationids = ctx.input.integrationIds.join(',');
    ...
    if (ctx.input.shouldNotContain !== undefined) data.shouldnotcontain = ctx.input.shouldNotContain;
