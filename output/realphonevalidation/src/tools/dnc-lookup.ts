...
export let dncLookup = SlateTool.create(
  spec,
  {
    name: 'DNC Lookup',
    key: 'dnc_lookup',
  ...
  .input(z.object({
    ...
    mode: z.enum(['standard', 'plus']).default('standard').describe('Lookup mode: "standard" returns DNC status and phone type; "plus" also includes connection status'),
  }))
  .output(z.object({
    nationalDnc: z.boolean().describe('Whether the number is on the National Do Not Call Registry'),
    ...
    isLitigator: z.boolean().describe('Whether the number is associated with a known TCPA litigator or serial plaintiff'),
    isMobile: z.boolean().describe('Whether the number is a mobile/cell phone'),
    connectionStatus: z.string().optional().describe('Connection status: connected or disconnected (plus mode only)'),
