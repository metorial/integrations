...
export let checkReassignedNumber = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    consentDate: z.string().describe('Date when consent was originally obtained, in YYYY-MM-DD format'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let statusDescription = result.reassigned === 'yes'
      ? 'has been reassigned'
