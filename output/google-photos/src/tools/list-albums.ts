...
export let listAlbums = SlateTool.create(
  spec,
  {
    name: 'List Albums',
    key: 'list_albums',
  ...
  .input(z.object({
    pageSize: z.number().min(1).max(50).optional().describe('Maximum number of albums to return (1-50, default 20)'),
    pageToken: z.string().optional().describe('Pagination token from a previous request to retrieve the next page'),
    excludeNonAppCreatedData: z.boolean().optional().describe('If true, only return albums created by this app'),
  }))
  .output(z.object({
