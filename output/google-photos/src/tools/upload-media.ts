...
export let uploadMedia = SlateTool.create(
  ...
  .input(z.object({
    items: z.array(z.object({
      uploadToken: z.string().describe('Upload token obtained from the bytes upload step'),
      fileName: z.string().describe('Filename for the media item (e.g. "vacation.jpg")'),
      description: z.string().max(1000).optional().describe('Description for the media item'),
    })).min(1).max(50).describe('Media items to create (max 50)'),
