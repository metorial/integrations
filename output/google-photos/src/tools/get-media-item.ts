...
let mediaMetadataSchema = z.object({
  ...
  }).optional().describe('Photo-specific metadata'),
  video: z.object({
    cameraMake: z.string().optional().describe('Camera manufacturer'),
    cameraModel: z.string().optional().describe('Camera model'),
    fps: z.number().optional().describe('Frame rate'),
    status: z.string().optional().describe('Processing status (PROCESSING, READY, FAILED)'),
  }).optional().describe('Video-specific metadata'),
}).optional();

export let getMediaItem = SlateTool.create(
  spec,
  {
    name: 'Get Media Item',
    key: 'get_media_item',
  ...
  .input(z.object({
    mediaItemIds: z.array(z.string()).min(1).max(50).describe('One or more media item IDs to retrieve (max 50)'),
  }))
  .output(z.object({
    mediaItems: z.array(z.object({
      ...
      baseUrl: z.string().optional().describe('Base URL for accessing the media bytes (expires in 60 minutes)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.mediaItemIds.length === 1) {
      ...
      mediaItems = result.mediaItemResults
        .filter((r): r is typeof r & { mediaItem: NonNullable<typeof r.mediaItem> } => !!r.mediaItem)
        .map((r) => r.mediaItem);
    }
