...
export let orderFeedback = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    fraudlabsproId: z.string().describe('FraudLabs Pro transaction ID from the original screening'),
    action: z.enum(['APPROVE', 'REJECT', 'REJECT_BLACKLIST']).describe('Action to take: APPROVE, REJECT, or REJECT_BLACKLIST (reject and add to blacklist)'),
    note: z.string().optional().describe('Optional note to attach to this feedback for reference'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let actionLabel = ctx.input.action === 'APPROVE' ? 'approved' : ctx.input.action === 'REJECT' ? 'rejected' : 'rejected and blacklisted';
