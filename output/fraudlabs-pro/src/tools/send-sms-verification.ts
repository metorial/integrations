...
export let sendSmsVerification = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    phoneNumber: z.string().describe('Recipient phone number in E.164 format (e.g., +12015550123)'),
    messageTemplate: z.string().describe('SMS message template with <otp> placeholder for the generated code (max 140 chars)'),
    countryCode: z.string().optional().describe('ISO 3166 country code for basic phone number validation'),
    otpTimeout: z.number().optional().describe('OTP expiration duration in seconds (15-86400, default 3600)'),
  }))
  .output(z.object({
