...
export let getOrderResult = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    idType: z.enum(['FLP_ID', 'ORDER_ID']).optional().describe('Type of ID provided. Defaults to FLP_ID if not specified.'),
  }))
  .output(z.object({
    ...
    userOrderId: z.string().optional().describe('Your order ID if it was provided during screening'),
    triggeredRules: z.array(z.any()).optional().describe('List of fraud rules triggered'),
    ipGeolocation: z.record(z.string(), z.any()).optional().describe('IP geolocation and proxy detection details'),
    billingAddress: z.record(z.string(), z.any()).optional().describe('Billing address validation results'),
    shippingAddress: z.record(z.string(), z.any()).optional().describe('Shipping address validation results'),
    emailAddress: z.record(z.string(), z.any()).optional().describe('Email address validation results'),
    phoneNumber: z.record(z.string(), z.any()).optional().describe('Phone number validation results'),
    creditCard: z.record(z.string(), z.any()).optional().describe('Credit card validation results'),
    device: z.record(z.string(), z.any()).optional().describe('Device fingerprint validation results'),
  }))
