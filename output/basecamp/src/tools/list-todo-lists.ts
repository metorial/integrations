...
export let listTodoListsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    status: z.enum(['active', 'archived', 'trashed']).optional().default('active').describe('Filter lists by status'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let lists = await client.listTodoLists(ctx.input.projectId, ctx.input.todoSetId, { status: statusParam });
