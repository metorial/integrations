...
export let manageTodoListTool = SlateTool.create(
  spec,
  {
...
To **update**, provide the \`projectId\`, \`todoListId\`, and the new \`name\`.`,
  ...
  .input(z.object({
    ...
    todoSetId: z.string().optional().describe('ID of the to-do set (required for create, found in project dock)'),
    ...
    description: z.string().optional().describe('Description of the to-do list (supports HTML)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      if (!ctx.input.todoSetId) throw new Error('todoSetId is required for creating a to-do list');
      ...
      return {
        output: {
          ...
          completedCount: list.completed_ratio?.split('/')[0] ? Number(list.completed_ratio.split('/')[0]) : 0,
          incompletedCount: 0,
        },
    ...
    // update
    if (!ctx.input.todoListId) throw new Error('todoListId is required for updating a to-do list');
    ...
    return {
      output: {
        ...
        description: list.description ?? null,
        completedCount: list.completed_ratio?.split('/')[0] ? Number(list.completed_ratio.split('/')[0]) : 0,
        incompletedCount: list.completed_ratio?.split('/')[1] ? Number(list.completed_ratio.split('/')[1]) : 0,
      },
