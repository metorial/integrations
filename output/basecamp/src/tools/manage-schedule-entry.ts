...
export let manageScheduleEntryTool = SlateTool.create(
  spec,
  {
...
To **update**, provide the \`projectId\`, \`entryId\`, and the fields to change.`,
  ...
  .input(z.object({
    ...
    scheduleId: z.string().optional().describe('ID of the schedule (required for create, found in project dock)'),
    entryId: z.string().optional().describe('ID of the schedule entry (required for update)'),
    ...
    participantIds: z.array(z.number()).optional().describe('Array of person IDs to add as participants'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      if (!ctx.input.scheduleId) throw new Error('scheduleId is required for creating a schedule entry');
      if (!ctx.input.summary) throw new Error('summary is required for creating a schedule entry');
      if (!ctx.input.startsAt) throw new Error('startsAt is required for creating a schedule entry');
      if (!ctx.input.endsAt) throw new Error('endsAt is required for creating a schedule entry');
    ...
    // update
    if (!ctx.input.entryId) throw new Error('entryId is required for updating a schedule entry');
