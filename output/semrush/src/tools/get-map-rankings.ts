...
export let getMapRankings = SlateTool.create(
  spec,
  {
...
Available to all Semrush users without consuming API units. Requires OAuth 2.0 authentication.`,
  ...
  .input(z.object({
    reportType: z.enum(['campaigns', 'campaign_details', 'keywords', 'heatmap', 'competitors']).describe('Type of map rank data to retrieve'),
    campaignId: z.string().optional().describe('Campaign ID (required for all except "campaigns")'),
    keywordId: z.string().optional().describe('Keyword ID (required for heatmap)'),
    limit: z.number().optional().describe('Maximum number of results'),
    offset: z.number().optional().describe('Number of results to skip'),
  }))
  .output(z.object({
    campaigns: z.array(z.record(z.string(), z.unknown())).optional().describe('List of map rank tracker campaigns'),
    campaign: z.record(z.string(), z.unknown()).optional().describe('Campaign details'),
    keywords: z.array(z.record(z.string(), z.unknown())).optional().describe('Tracked keywords with ranking data'),
    heatmap: z.record(z.string(), z.unknown()).optional().describe('Heatmap geo-grid data'),
    competitors: z.array(z.record(z.string(), z.unknown())).optional().describe('Top local competitors'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.reportType) {
      ...
      case 'campaign_details': {
        if (!ctx.input.campaignId) throw new Error('campaignId is required for campaign_details.');
      ...
      case 'heatmap': {
        ...
        let heatmap = await client.getMapRankTrackerHeatmap(ctx.input.campaignId, ctx.input.keywordId);
        return {
          output: { heatmap },
