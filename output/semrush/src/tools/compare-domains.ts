...
export let compareDomains = SlateTool.create(
  spec,
  {
...
Returns keywords with position data for each domain, enabling competitive gap analysis.`,
  ...
  .input(z.object({
    domains: z.array(z.string()).min(2).max(5).describe('List of 2-5 domain names to compare'),
    database: z.string().optional().describe('Regional database code (e.g., us, uk, de)'),
    limit: z.number().optional().default(50).describe('Maximum number of keywords to return'),
    offset: z.number().optional().describe('Number of results to skip for pagination'),
  }))
  .output(z.object({
    sharedKeywords: z.array(z.record(z.string(), z.unknown())).describe('Keywords with position data for each compared domain'),
  }))
