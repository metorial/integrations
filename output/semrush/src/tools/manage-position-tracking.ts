...
export let managePositionTracking = SlateTool.create(
  spec,
  {
...
Requires OAuth 2.0 authentication.`,
  ...
  .input(z.object({
    action: z.enum(['list_campaigns', 'create_campaign', 'get_keywords', 'add_keywords', 'remove_keywords', 'get_report']).describe('Action to perform'),
    projectId: z.string().describe('Semrush project ID'),
    campaignId: z.string().optional().describe('Campaign ID (required for keyword and report actions)'),
    ...
    location: z.string().optional().describe('Location for tracking (e.g., "United States")'),
    ...
    competitors: z.array(z.string()).optional().describe('Competitor domains (for create_campaign)'),
    limit: z.number().optional().describe('Maximum number of results'),
    offset: z.number().optional().describe('Number of results to skip'),
  }))
  .output(z.object({
    campaigns: z.array(z.record(z.string(), z.unknown())).optional().describe('List of position tracking campaigns'),
    campaign: z.record(z.string(), z.unknown()).optional().describe('Created campaign details'),
    keywords: z.array(z.record(z.string(), z.unknown())).optional().describe('Tracked keywords'),
    report: z.array(z.record(z.string(), z.unknown())).optional().describe('Position tracking report data'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'add_keywords': {
        ...
        if (!ctx.input.keywords?.length) throw new Error('keywords array is required for add_keywords.');
      ...
      case 'remove_keywords': {
        if (!ctx.input.campaignId) throw new Error('campaignId is required for remove_keywords.');
        if (!ctx.input.keywords?.length) throw new Error('keywords array is required for remove_keywords.');
