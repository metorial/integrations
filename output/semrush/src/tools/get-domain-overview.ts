...
export let getDomainOverview = SlateTool.create(
  spec,
  {
...
Supports pulling data from different regional databases and retrieving historical trends.`,
  ...
  .input(z.object({
    ...
    database: z.string().optional().describe('Regional database code (e.g., us, uk, de). Overrides the global config.'),
    includeHistory: z.boolean().optional().default(false).describe('Include historical monthly data'),
    historyLimit: z.number().optional().describe('Max number of historical data points to return'),
  }))
  .output(z.object({
    overview: z.array(z.record(z.string(), z.unknown())).describe('Current domain overview metrics across databases'),
    allDatabaseRanks: z.array(z.record(z.string(), z.unknown())).optional().describe('Rankings across all databases'),
    history: z.array(z.record(z.string(), z.unknown())).optional().describe('Historical monthly data'),
  }))
