...
export let readRecords = SlateTool.create(
  spec,
  {
    name: 'Read Records',
    key: 'read_records',
  ...
  .input(z.object({
    model: z.string().describe('The Odoo model to read from (e.g., "res.partner", "sale.order")'),
  ...
  .output(z.object({
    records: z.array(z.record(z.string(), z.unknown())).describe('Array of record data for the requested IDs'),
  }))
