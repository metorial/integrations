...
export let createCheckout = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    fields: z.array(z.string()).optional().describe('Form fields to display (e.g., ["Email", "Name"])'),
    disablePayments: z.array(z.string()).optional().describe('Payment methods to disable (e.g., ["paypal", "bitcoin"])'),
    successUrl: z.string().optional().describe('URL to redirect to after successful payment'),
    ...
    webhookUrl: z.string().optional().describe('URL for instant payment notification webhook'),
    defaultValues: z.record(z.string(), z.string()).optional().describe('Pre-filled customer data (e.g., { "name": "Bob", "email": "bob@example.com" })'),
    metadata: z.record(z.string(), z.unknown()).optional().describe('Custom metadata to attach to the checkout')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let checkoutUrl = typeof result === 'string' ? result : result?.checkout_url || result?.url || JSON.stringify(result);
