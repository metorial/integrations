...
let layerModificationSchema = z.record(z.string(), z.any()).describe('Layer modifications keyed by layer name. Each layer can have properties like text, color, image_url, font_family, font_size, opacity, x, y, width, height, rotation, hide, etc.');
...
export let createRender = SlateTool.create(
  spec,
  {
    name: 'Create Render',
    key: 'create_render',
...
Use **layers** for single-page modifications or **pages** for per-page modifications in multi-page templates.`,
  ...
  .input(z.object({
    ...
    templateIds: z.array(z.string()).optional().describe('List of template IDs for batch rendering. Overrides templateId if provided.'),
    format: z.enum(['jpg', 'png', 'webp', 'pdf', 'mp4']).optional().describe('Output format. Default: jpg'),
    renderName: z.string().optional().describe('Custom name for the render'),
    externalId: z.string().optional().describe('External identifier to link with your own system'),
    ...
    duration: z.number().optional().describe('Video duration in milliseconds (max 90000). Default: 5000'),
    fps: z.number().optional().describe('Frames per second for video (1-60). Default: 30'),
    layers: layerModificationSchema.optional(),
    pages: z.array(pageSchema).optional().describe('Per-page layer modifications for multi-page templates'),
    merge: z.boolean().optional().describe('Merge multi-page renders into a single PDF')
  }))
  .output(z.any().describe('Render result - single render object or array for multi-page/batch renders. Each render includes renderId, url, width, height, format, templateId, templateName, and createdAt.'))
