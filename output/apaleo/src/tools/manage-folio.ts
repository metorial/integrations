...
export let manageFolio = SlateTool.create(
  spec,
  {
    name: 'Manage Folio',
    key: 'manage_folio',
  ...
  .input(z.object({
    ...
    action: z.enum(['post_charge', 'post_payment', 'post_refund', 'close']).describe('Action to perform'),
    name: z.string().optional().describe('Charge description/name (for post_charge)'),
    serviceType: z.string().optional().describe('Service type for the charge (e.g., FoodAndBeverages, Other)'),
    method: z.string().optional().describe('Payment method (for post_payment/post_refund, e.g., Cash, CreditCard, BankTransfer)'),
    amount: moneySchema.optional().describe('Amount (required for post_charge, post_payment, post_refund)'),
    ...
    vatType: z.string().optional().describe('VAT type for charge (e.g., Null, Reduced, Normal)'),
