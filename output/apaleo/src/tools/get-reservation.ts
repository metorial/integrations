...
export let getReservation = SlateTool.create(
  ...
  .output(z.object({
    ...
    }).optional().describe('Primary guest'),
    booker: z.object({
      firstName: z.string().optional(),
      lastName: z.string().optional(),
      email: z.string().optional(),
    }).optional().describe('Booker information'),
    property: z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let guestName = [r.primaryGuest?.firstName, r.primaryGuest?.lastName].filter(Boolean).join(' ') || 'Unknown guest';
