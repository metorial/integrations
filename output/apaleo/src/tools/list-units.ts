...
export let listUnits = SlateTool.create(
  spec,
  {
    name: 'List Units',
    key: 'list_units',
  ...
  .input(z.object({
    ...
    condition: z.enum(['Clean', 'Dirty', 'CleanToBeInspected']).optional().describe('Filter by room condition'),
  ...
  .output(z.object({
    ...
    }).passthrough()).describe('List of rooms'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let units = (result.units || []).map((u: any) => ({
      ...
      unitGroup: u.unitGroup ? { unitGroupId: u.unitGroup.id, name: u.unitGroup.name } : undefined,
      status: u.status ? {
