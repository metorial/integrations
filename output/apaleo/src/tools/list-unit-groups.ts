...
export let listUnitGroups = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    }).passthrough()).describe('List of room types'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let unitGroups = (result.unitGroups || []).map((ug: any) => ({
      ...
      rank: ug.rank,
      type: ug.type,
      property: ug.property ? { propertyId: ug.property.id, name: ug.property.name } : undefined,
    }));
