...
let reservationSchema = z.object({
  reservationId: z.string().describe('Unique reservation ID'),
  bookingId: z.string().optional().describe('Associated booking ID'),
  status: z.string().optional().describe('Reservation status (e.g., Confirmed, InHouse, CheckedOut, Canceled, NoShow)'),
  ...
  }).optional().describe('Primary guest information'),
  property: z.object({
    propertyId: z.string().optional(),
    name: z.string().optional(),
  }).optional().describe('Property details'),
  unitGroup: z.object({
    unitGroupId: z.string().optional(),
    name: z.string().optional(),
  }).optional().describe('Room type details'),
  unit: z.object({
    unitId: z.string().optional(),
    name: z.string().optional(),
  }).optional().describe('Assigned room'),
  totalGrossAmount: z.object({
    amount: z.number().optional(),
    currency: z.string().optional(),
  }).optional().describe('Total gross amount'),
...
export let listReservations = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['Confirmed', 'InHouse', 'CheckedOut', 'Canceled', 'NoShow']).optional().describe('Filter by reservation status'),
    from: z.string().optional().describe('Filter reservations arriving from this date (YYYY-MM-DD)'),
    to: z.string().optional().describe('Filter reservations arriving until this date (YYYY-MM-DD)'),
    textSearch: z.string().optional().describe('Search by guest name, confirmation number, or other text'),
    ...
    pageSize: z.number().optional().describe('Number of results per page (default 100, max 1000)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let reservations = (result.reservations || []).map((r: any) => ({
      ...
      unitGroup: r.unitGroup ? { unitGroupId: r.unitGroup.id, name: r.unitGroup.name } : undefined,
