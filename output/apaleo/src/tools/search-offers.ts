...
export let searchOffers = SlateTool.create(
  spec,
  {
    name: 'Search Offers',
    key: 'search_offers',
  ...
  .input(z.object({
    propertyId: z.string().optional().describe('Property ID to search (uses default from config if not set)'),
  ...
  .output(z.object({
    ...
    }).passthrough()).describe('Available offers'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new ApaleoClient(ctx.auth.token);
    let propertyId = ctx.input.propertyId || ctx.config.propertyId;
    if (!propertyId) throw new Error('propertyId is required — set it in config or provide it as input');
