...
export class ApaleoClient {
  ...
  async createBooking(body: {
    paymentAccount?: { accountNumber?: string; accountHolder?: string; expiryMonth?: string; expiryYear?: string; payerEmail?: string; isVirtual?: boolean };
    booker?: { title?: string; firstName: string; lastName: string; email?: string; phone?: string; address?: { addressLine1?: string; postalCode?: string; city?: string; countryCode: string } };
    comment?: string;
    ...
    reservations: Array<{
      ...
      channelCode?: string;
      externalCode?: string;
      primaryGuest?: { title?: string; firstName: string; lastName: string; email?: string; phone?: string };
      guaranteeType?: string;
      travelPurpose?: string;
      timeSlices?: Array<{ ratePlanId: string; totalAmount?: { amount: number; currency: string } }>;
      services?: Array<{ serviceId: string; count?: number; amount?: { amount: number; currency: string }; dates?: Array<{ serviceDate: string; count?: number; amount?: { amount: number; currency: string } }> }>;
      prePaymentAmount?: { amount: number; currency: string };
      commission?: { commissionAmount?: { amount: number; currency: string }; beforeCommissionAmount?: { amount: number; currency: string } };
      promoCode?: string;
  ...
  async amendReservation(reservationId: string, body: {
    arrival?: string;
    departure?: string;
    adults?: number;
    childrenAges?: number[];
  ...
  async cancelReservation(reservationId: string) {
    let response = await this.api.put(`/booking/v1/reservation-actions/${reservationId}/cancel`);
    return response.data;
  }

  async checkInReservation(reservationId: string) {
    let response = await this.api.put(`/booking/v1/reservation-actions/${reservationId}/checkin`);
    return response.data;
  }

  async checkOutReservation(reservationId: string) {
    let response = await this.api.put(`/booking/v1/reservation-actions/${reservationId}/checkout`);
    return response.data;
  }

  async noShowReservation(reservationId: string) {
    let response = await this.api.put(`/booking/v1/reservation-actions/${reservationId}/noshow`);
    return response.data;
  }

  async assignUnit(reservationId: string, unitId: string) {
    let response = await this.api.put(`/booking/v1/reservation-actions/${reservationId}/assign-unit/${unitId}`);
    return response.data;
  }

  async unassignUnit(reservationId: string) {
    let response = await this.api.put(`/booking/v1/reservation-actions/${reservationId}/unassign-unit`);
    return response.data;
  }
  ...
  async listRatePlans(params: {
    ...
    if (params.isSubjectToCityTax !== undefined) query.isSubjectToCityTax = params.isSubjectToCityTax;
    if (params.pageNumber) query.pageNumber = params.pageNumber;
    if (params.pageSize) query.pageSize = params.pageSize;
  ...
  async postCharge(folioId: string, body: {
    serviceType: string;
    name: string;
  ...
  async postPayment(folioId: string, body: {
    method: string;
    amount: { amount: number; currency: string };
    receipt?: string;
  }) {
  ...
  async postRefund(folioId: string, body: {
    method: string;
    amount: { amount: number; currency: string };
    receipt?: string;
    reason?: string;
  ...
  async createCompany(body: {
    ...
    propertyId: string;
    taxId?: string;
    address?: { addressLine1?: string; addressLine2?: string; postalCode?: string; city?: string; regionCode?: string; countryCode: string };
  }) {
  ...
  async triggerNightAudit(propertyId: string) {
    let response = await this.api.put(`/settings/v1/night-audit-actions/${propertyId}/trigger`);
    return response.data;
  }
