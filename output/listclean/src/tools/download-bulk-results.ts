...
export let downloadBulkResults = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    filter: z.enum(['all', 'clean', 'dirty']).default('all').describe('Filter results: all emails, only clean (valid), or only dirty (invalid)')
  }))
  .output(z.object({
