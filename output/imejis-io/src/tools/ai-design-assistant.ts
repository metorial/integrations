...
export let aiDesignAssistantTool = SlateTool.create(
  spec,
  {
...
Useful for quickly creating templates without manual editing — describe the layout, style, colors, and content in plain text.`,
  ...
  .input(z.object({
    prompt: z.string().describe('Natural language description of the design to create or how to modify an existing design.'),
    designId: z.string().optional().describe('ID of an existing design to update. Omit to create a new design from scratch.'),
  }))
  .output(z.object({
    ...
    url: z.string().optional().describe('URL to view or edit the design in the Imejis.io platform.'),
  }))
