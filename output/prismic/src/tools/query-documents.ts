...
Supports filtering by document type, tags, full-text search, and custom predicates. Results are paginated.`,
  ...
  .input(z.object({
    documentType: z.string().optional().describe('Filter by custom type (e.g., "blog_post", "page")'),
    tags: z.array(z.string()).optional().describe('Filter by one or more tags'),
    predicates: z.array(z.string()).optional().describe('Custom Prismic predicates for advanced filtering'),
    fullTextSearch: z.string().optional().describe('Full-text search query'),
    lang: z.string().optional().describe('Language code to filter by (use "*" for all languages)'),
    orderings: z.string().optional().describe('Ordering expression, e.g., "[my.blog_post.date desc]"'),
    pageSize: z.number().optional().describe('Number of results per page (1-100, default 20)'),
    page: z.number().optional().describe('Page number to retrieve (default 1)'),
    after: z.string().optional().describe('Document ID to paginate after (for cursor-based pagination)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.tags && ctx.input.tags.length > 0) {
      predicates.push(`[:d = at(document.tags, [${ctx.input.tags.map(t => `"${t}"`).join(',')}])]`);
    }
