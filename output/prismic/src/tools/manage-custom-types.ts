...
export let listCustomTypes = SlateTool.create(
  spec,
  {
...
Requires a Write API token.`,
  ...
  .input(z.object({}))
  .output(z.object({
    customTypes: z.array(customTypeOutputSchema).describe('All custom types in the repository'),
  }))
...
export let getCustomType = SlateTool.create(
  spec,
  {
...
export let createCustomType = SlateTool.create(
  spec,
  {
...
Requires a Write API token.`,
  ...
  .input(z.object({
    typeId: z.string().describe('Unique ID for the custom type (lowercase, hyphens allowed)'),
    label: z.string().describe('Human-readable label'),
    repeatable: z.boolean().describe('Whether multiple documents of this type can exist'),
    status: z.boolean().optional().describe('Whether the type is active (default true)'),
    json: z.record(z.string(), z.any()).describe('JSON schema defining the type structure (tabs with field definitions)'),
  }))
  .output(customTypeOutputSchema)
  .handleInvocation(async (ctx) => {
    if (!ctx.auth.writeToken) {
...
export let updateCustomType = SlateTool.create(
  spec,
  {
...
export let deleteCustomType = SlateTool.create(
  spec,
  {
