...
export let createProxyEmail = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    realAddresses: z.array(z.string()).optional().describe('Real email addresses to forward incoming mail to. Leave empty for no forwarding.'),
    proxyAddress: z.string().optional().nullable().describe('Desired proxy email address. Leave empty to auto-generate one on a ProxiedMail domain.'),
    isBrowsable: z.boolean().optional().describe('Enable browsing received emails via the API. Defaults to false.'),
    callbackUrl: z.string().optional().nullable().describe('Webhook URL to receive HTTP POST notifications when emails arrive at this proxy address.'),
  }))
  .output(z.object({
    ...
    realAddresses: z.record(z.string(), realAddressInfoSchema).describe('Map of real email addresses and their verification status'),
