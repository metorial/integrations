...
export let updateProxyEmail = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    realAddresses: z.array(z.string()).optional().describe('Updated list of real email addresses for forwarding'),
    callbackUrl: z.string().optional().nullable().describe('Updated webhook callback URL. Set to null to remove.'),
    description: z.string().optional().nullable().describe('Updated description for the proxy email'),
    isBrowsable: z.boolean().optional().describe('Enable or disable API browsing of received emails'),
  }))
  .output(z.object({
    ...
    realAddresses: z.record(z.string(), realAddressInfoSchema).describe('Updated forwarding addresses and their status'),
