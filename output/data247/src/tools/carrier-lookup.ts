...
let carrierResultSchema = z.object({
  ...
  type: z.string().optional().describe('Phone type: "M" for mobile, "L" for landline, "V" for VOIP'),
...
export let carrierLookup = SlateTool.create(
  spec,
  {
    name: 'Carrier Lookup',
    key: 'carrier_lookup',
  ...
  .input(z.object({
    phones: z.array(z.string()).min(1).max(100).describe('Phone numbers to look up. Include country code for international numbers.'),
    lookupType: z.enum(['usa', 'international', 'carrier_type']).default('usa').describe('Type of lookup: "usa" for US numbers with full details, "international" for worldwide numbers, "carrier_type" for USA/Canada carrier type only'),
  }))
  .output(z.object({
    results: z.array(carrierResultSchema).describe('Carrier information for each phone number'),
  }))
