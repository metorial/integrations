...
export let fraudDetection = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    checkType: z.enum(['identity', 'phone_spam']).describe('"identity" to score user signup legitimacy, "phone_spam" to check if a phone is a known spam caller'),
    ...
    companyName: z.string().optional().describe('Company/organization name (identity check)'),
    ...
    ip: z.string().optional().describe('IP address the user signed up from (identity check)'),
    email: z.string().optional().describe('Email address (identity check)'),
    phone: z.string().optional().describe('Phone number (required for phone_spam, optional for identity check)'),
  }))
  .output(z.object({
    weight: z.string().optional().describe('Legitimacy score from 1-100 (identity check). Under 50 is suspicious, over 80 is likely legitimate.'),
    isSpam: z.string().optional().describe('Whether the phone number is a known spam caller (phone_spam check)'),
    phone: z.string().optional().describe('The queried phone number'),
    additionalAttributes: z.record(z.string(), z.string()).optional().describe('Additional attributes returned by the service'),
  }))
