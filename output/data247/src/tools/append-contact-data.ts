...
export let appendContactData = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    appendType: z.enum(['phone', 'email']).describe('Type of data to append: "phone" to find phone numbers, "email" to find email addresses'),
    ...
    phone: z.string().optional().describe('Phone number (used for email append when address is unavailable)'),
  }))
  .output(z.object({
