...
export let startSession = SlateTool.create(
  spec,
  {
    name: 'Start Session',
    key: 'start_session',
  ...
  .input(z.object({
    ...
    source: z.string().default('production').describe('Source environment (e.g., "production", "staging", "playground")'),
    ...
    outputs: z.record(z.string(), z.any()).optional().describe('Output data for the session'),
    ...
    userProperties: z.record(z.string(), z.any()).optional().describe('User properties associated with this session'),
    metadata: z.record(z.string(), z.any()).optional().describe('Additional metadata'),
    startTime: z.number().optional().describe('Epoch timestamp in milliseconds for session start'),
    endTime: z.number().optional().describe('Epoch timestamp in milliseconds for session end'),
  }))
  .output(z.object({
...
export let deleteSession = SlateTool.create(
  spec,
  {
    name: 'Delete Session',
    key: 'delete_session',
