...
export let listAccessTokens = SlateTool.create(
  ...
  .output(z.object({
    ...
    tokens: z.array(z.object({
      ...
      lastUsed: z.string().nullable().describe('ISO timestamp of the last use, or null if never used.'),
    })),
  }))
...
export let createAccessToken = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    label: z.string().describe('Display label for the token (e.g., "CI/CD Pipeline", "Dev Machine").'),
    scopes: z.array(z.string()).describe('Permission scopes for the token (e.g., ["repo:read"]).'),
  }))
  .output(z.object({
    tokenUuid: z.string().describe('UUID of the created token.'),
    tokenValue: z.string().describe('The generated token value. Store securely - this is only shown once.'),
...
export let updateAccessToken = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    isActive: z.boolean().optional().describe('Set to false to deactivate the token, true to reactivate.'),
  }))
  .output(z.object({
...
export let deleteAccessToken = SlateTool.create(
  spec,
  {
