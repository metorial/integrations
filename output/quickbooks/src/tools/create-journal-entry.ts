...
export let createJournalEntry = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Total debits must equal total credits for the journal entry to be valid.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    lines: z.array(journalLineSchema).min(2).describe('Journal entry lines (must include at least one debit and one credit)'),
