...
export let createItem = SlateTool.create(
  ...
  .input(z.object({
    ...
    assetAccountId: z.string().optional().describe('Inventory asset account ID (required for Inventory type)'),
    quantityOnHand: z.number().optional().describe('Starting quantity on hand (required for Inventory type)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.expenseAccountId) itemData.ExpenseAccountRef = { value: ctx.input.expenseAccountId };
    if (ctx.input.assetAccountId) itemData.AssetAccountRef = { value: ctx.input.assetAccountId };
...
export let updateItem = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.quantityOnHand !== undefined) updateData.QtyOnHand = ctx.input.quantityOnHand;
