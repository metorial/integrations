...
export let getInvoice = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['get', 'send', 'void']).default('get').describe('Action to perform on the invoice'),
    emailAddress: z.string().optional().describe('Email address for sending (overrides default)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let actionLabel = ctx.input.action === 'send' ? 'Sent' : ctx.input.action === 'void' ? 'Voided' : 'Retrieved';
