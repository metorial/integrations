...
export let queryEntities = SlateTool.create(
  spec,
  {
    name: 'Query Entities',
    key: 'query_entities',
  ...
  .input(z.object({
    entityType: z.enum([
      'Customer', 'Invoice', 'Bill', 'Item', 'Account', 'Vendor',
      'Payment', 'Estimate', 'SalesReceipt', 'Purchase', 'JournalEntry',
      'BillPayment', 'CreditMemo', 'Deposit', 'Transfer', 'Employee',
      'Department', 'Class', 'Term', 'TaxCode', 'TaxRate',
    ...
    startPosition: z.number().optional().describe('Starting position for pagination (1-based)'),
  }))
  .output(z.object({
...
export let searchCustomersAndVendors = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    searchTerm: z.string().describe('Search term to match against display name, company name, or email'),
    contactType: z.enum(['customer', 'vendor', 'both']).default('both').describe('Type of contacts to search'),
  ...
  .output(z.object({
    ...
    })).optional().describe('Matching customers'),
    vendors: z.array(z.object({
      ...
      phone: z.string().optional(),
      balance: z.number().optional(),
    })).optional().describe('Matching vendors'),
  }))
  .handleInvocation(async (ctx) => {
    let client = createClientFromContext(ctx);
    ...
    if (ctx.input.contactType === 'customer' || ctx.input.contactType === 'both') {
      let rawCustomers = await client.queryCustomers(nameFilter, undefined, ctx.input.maxResults);
