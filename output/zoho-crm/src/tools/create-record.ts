...
Optionally control which workflow triggers fire upon creation.`,
  ...
  .input(z.object({
    ...
    records: z.array(z.record(z.string(), z.any())).min(1).describe('Array of record objects with field API names as keys'),
    triggers: z.array(z.enum(['approval', 'workflow', 'blueprint'])).optional().describe('Workflow triggers to fire on creation'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.createRecords(ctx.input.module, ctx.input.records, ctx.input.triggers);
