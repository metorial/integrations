...
Optionally control which workflow triggers fire upon update.`,
  ...
  .input(z.object({
    ...
    recordId: z.string().optional().describe('ID of the single record to update (use with recordData)'),
    recordData: z.record(z.string(), z.any()).optional().describe('Fields to update on the single record'),
    records: z.array(z.record(z.string(), z.any())).optional().describe('Array of record objects each with an "id" field for bulk updates'),
    triggers: z.array(z.enum(['approval', 'workflow', 'blueprint'])).optional().describe('Workflow triggers to fire on update'),
  }))
  .output(z.object({
