...
Supports tracking events attributed to a specific person, anonymous events (associated later), and page/screen views.`,
  ...
  .input(z.object({
    personIdentifier: z.string().optional().describe('The person identifier to attribute this event to. Omit for anonymous events.'),
    eventName: z.string().describe('The name of the event (e.g. "purchase", "button_click", "/home")'),
    eventType: z.enum(['event', 'page', 'screen']).default('event').describe('The type of event: standard event, page view, or screen view'),
    properties: z.record(z.string(), z.unknown()).optional().describe('Custom properties/data to include with the event'),
    anonymousId: z.string().optional().describe('An anonymous ID for events without a known person identifier'),
    timestamp: z.number().optional().describe('Unix timestamp to backdate the event (seconds since epoch)'),
  }))
  .output(z.object({
