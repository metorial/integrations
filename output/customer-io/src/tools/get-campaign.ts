...
export let getCampaign = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeMetrics: z.boolean().optional().describe('Whether to also fetch campaign metrics'),
    metricsPeriod: z.enum(['hours', 'days', 'weeks', 'months']).optional().describe('Time period for metrics aggregation'),
    metricsSteps: z.number().optional().describe('Number of time periods to return'),
  }))
  .output(z.object({
    campaign: z.record(z.string(), z.unknown()).describe('The campaign object with full details'),
    actions: z.array(z.record(z.string(), z.unknown())).optional().describe('Campaign actions/steps'),
