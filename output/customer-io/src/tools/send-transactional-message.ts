...
export let sendTransactionalMessage = SlateTool.create(
  spec,
  {
...
You can reference a pre-built template by its transactional message ID, or provide the full message content inline.`,
  ...
  .input(z.object({
    channel: z.enum(['email', 'push', 'sms']).describe('The channel to send the transactional message through'),
    transactionalMessageId: z.union([z.string(), z.number()]).describe('The ID or trigger name of the transactional message template in Customer.io'),
    to: z.string().optional().describe('Recipient address — email address for email, phone number for SMS'),
    identifiers: z.record(z.string(), z.string()).optional().describe('Person identifiers (e.g. { "id": "user123" } or { "email": "test@example.com" })'),
    messageData: z.record(z.string(), z.unknown()).optional().describe('Key-value data to populate Liquid merge fields in the template'),
    subject: z.string().optional().describe('Email subject (overrides template subject, email only)'),
    body: z.string().optional().describe('Email HTML body (overrides template body, email only)'),
    from: z.string().optional().describe('Sender email address (overrides template sender, email only)'),
    ...
    disableMessageRetention: z.boolean().optional().describe('If true, Customer.io will not retain the message body after sending'),
    sendToUnsubscribed: z.boolean().optional().describe('If true, sends even if the recipient has unsubscribed'),
  }))
  .output(z.object({
    deliveryId: z.string().optional().describe('The unique delivery ID for the sent message'),
