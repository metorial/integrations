...
export let getMatchDetailsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    includeEvents: z.boolean().optional().default(true).describe('Include match events (goals, cards, etc.)'),
    includeLineups: z.boolean().optional().default(true).describe('Include team lineups and formations'),
    includeStatistics: z.boolean().optional().default(true).describe('Include match statistics'),
  }))
  .output(z.object({
    fixtureId: z.number().describe('Fixture ID'),
    events: z.array(eventSchema).nullable().describe('Match events (goals, cards, substitutions)'),
