...
export let linkedinJobSearch = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    experienceLevel: z.enum(['internship', 'entry_level', 'associate', 'mid_senior', 'director']).optional().describe('Required experience level'),
    jobType: z.enum(['full_time', 'part_time', 'contract', 'temporary', 'internship', 'volunteer']).optional().describe('Employment type'),
    when: z.enum(['day', 'week', 'month']).optional().describe('Time filter for when jobs were posted'),
    flexibility: z.enum(['remote', 'flexible', 'on_site']).optional().describe('Work arrangement type'),
    distance: z.number().optional().describe('Search radius in miles (5, 10, 25, 50, or 100)'),
    geoId: z.string().optional().describe('LinkedIn geographic area ID (default: 92000000 for worldwide)'),
