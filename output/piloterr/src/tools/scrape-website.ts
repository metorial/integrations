...
- **Web Unlocker**: Advanced anti-bot bypass for protected sites like Cloudflare, Akamai, PerimeterX, DataDome (2 credits).`,
  ...
  .input(z.object({
    ...
    mode: z.enum(['crawler', 'rendering', 'webunlocker']).default('crawler').describe('Scraping mode to use'),
    allowRedirects: z.boolean().optional().describe('Whether to follow redirects (crawler and webunlocker modes)'),
    returnPageSource: z.boolean().optional().describe('Return raw page source instead of parsed HTML (crawler and webunlocker modes)'),
    waitInSeconds: z.number().optional().describe('Seconds to wait before capturing content (rendering mode, 0-30)'),
    waitFor: z.string().optional().describe('CSS or XPath selector to wait for before capturing (rendering mode)'),
    blockAds: z.boolean().optional().describe('Block ads during rendering (rendering mode)'),
    timeout: z.number().optional().describe('Request timeout in seconds (rendering mode, default 30)')
  }))
  .output(z.object({
