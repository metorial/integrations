...
export let sendNotecard = SlateTool.create(
  spec,
  {
    name: 'Send Notecard',
    key: 'send_notecard',
...
Set **preview** to true to generate a visual preview without placing an order.`,
  ...
  .input(z.object({
    ...
    imageTemplateId: z.number().optional().describe('ID of a saved image template for the exterior'),
    message: z.string().optional().describe('Handwritten interior message (supports %FIRST_NAME%, etc.)'),
    ...
    handwritingColor: z.string().optional().describe('Ink color: blue, black, green, purple, red, or hex'),
    handwritingRealism: z.boolean().optional().describe('Enable realism effect for AI fonts'),
    useCustomBackground: z.boolean().optional().describe('Enable custom interior background'),
    customBackgroundImage: z.string().optional().describe('URL of the interior background image (1650x2475px)'),
