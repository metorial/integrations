...
}).optional().describe('Custom return address');
...
Supports personalization tags like \`%FIRST_NAME%\`, \`%COMPANY%\`, etc. in messages. Set **preview** to true to generate a visual preview without placing an order.`,
  ...
  .input(z.object({
    ...
    imageTemplateId: z.number().optional().describe('ID of a saved image template for the front'),
    message: z.string().optional().describe('Handwritten message content (supports %FIRST_NAME%, %COMPANY%, etc.)'),
    ...
    handwritingColor: z.string().optional().describe('Ink color: blue, black, green, purple, red, or hex like #4287f5'),
    handwritingRealism: z.boolean().optional().describe('Enable realism effect for AI fonts'),
    useCustomBackground: z.boolean().optional().describe('Enable custom background behind the message'),
    customBackgroundImage: z.string().optional().describe('URL of the custom background image'),
    ...
    subAccountId: z.number().optional().describe('Sub-account ID to associate with the order'),
  ...
  .output(z.object({
    ...
    previews: z.array(z.string()).optional().describe('Preview image URLs (when in preview mode)'),
  }))
