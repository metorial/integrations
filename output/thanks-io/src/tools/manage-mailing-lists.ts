...
export let manageMailingLists = SlateTool.create(
  spec,
  {
...
Mailing lists organize recipients for targeted mail campaigns and can be associated with sub-accounts.`,
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'get', 'delete', 'list_recipients']).describe('Action to perform'),
    mailingListId: z.number().optional().describe('Mailing list ID (for get, delete, list_recipients)'),
    description: z.string().optional().describe('Mailing list name/description (for create)'),
    ...
    recipientLimit: z.number().optional().describe('Max recipients to return (for list_recipients, max 10000)'),
    updatedSince: z.string().optional().describe('Only recipients updated since this date (YYYY-MM-DD HH:MM:SS)'),
  }))
  .output(z.object({
    mailingListId: z.number().optional().describe('Mailing list ID'),
    mailingListDescription: z.string().optional().nullable().describe('Mailing list description'),
    mailingLists: z.array(z.record(z.string(), z.unknown())).optional().describe('List of mailing lists'),
    recipients: z.array(z.record(z.string(), z.unknown())).optional().describe('List of recipients'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'get') {
      if (!ctx.input.mailingListId) throw new Error('mailingListId is required for get action');
    ...
    if (action === 'delete') {
      if (!ctx.input.mailingListId) throw new Error('mailingListId is required for delete action');
    ...
    if (action === 'list_recipients') {
      if (!ctx.input.mailingListId) throw new Error('mailingListId is required for list_recipients action');
