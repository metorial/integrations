...
export let listBuilds = SlateTool.create(
  ...
  .input(z.object({
    ...
    limit: z.number().optional().describe('Maximum number of builds to return. Defaults to 10.')
  }))
  .output(z.object({
    builds: z.array(z.object({
      buildNumber: z.number().describe('Build number'),
      buildUrl: z.string().describe('URL of the build'),
      result: z.string().optional().nullable().describe('Build result (SUCCESS, FAILURE, UNSTABLE, ABORTED, or null if running)'),
