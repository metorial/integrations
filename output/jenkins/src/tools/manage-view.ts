...
export let manageView = SlateTool.create(
  spec,
  {
    name: 'Manage View',
    key: 'manage_view',
...
**delete** to remove a view, **add_job** to add a job to a view, or **remove_job** to remove a job from a view.`,
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'delete', 'add_job', 'remove_job']).describe('Action to perform'),
    viewName: z.string().optional().describe('Name of the view. Required for all actions except "list".'),
    xmlConfig: z.string().optional().describe('XML configuration for the view. Required for "create".'),
    jobName: z.string().optional().describe('Name of the job. Required for "add_job" and "remove_job".')
  }))
  .output(z.object({
    ...
    })).optional().describe('List of views (for "list" action)'),
    ...
    })).optional().describe('Jobs in the view (for "get" action)'),
