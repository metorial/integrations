...
export let manageQueue = SlateTool.create(
  ...
  .input(z.object({
    ...
    queueItemId: z.number().optional().describe('Queue item ID to cancel. Required for "cancel" action.')
  }))
  .output(z.object({
    items: z.array(z.object({
      ...
      inQueueSince: z.number().optional().describe('Timestamp when item entered the queue')
    })).optional().describe('Queue items (for "list" action)'),
    cancelled: z.boolean().optional().describe('Whether the cancellation was successful (for "cancel" action)')
  }))
