...
export let exportSegment = SlateTool.create(
  spec,
  {
    name: 'Export Segment',
    key: 'export_segment',
  ...
  .input(z.object({
    segmentId: z.string().describe('ID of the segment to export, or "everyone" for all users'),
    exportType: z.enum(['TYPE_EVENT', 'TYPE_INDIVIDUAL']).describe('Type of data to export: events or individual users'),
    format: z.enum(['FORMAT_CSV', 'FORMAT_JSON', 'FORMAT_NDJSON']).describe('Output format for the export'),
    startTime: z.string().optional().describe('Start of the time range (ISO 8601, inclusive)'),
    endTime: z.string().optional().describe('End of the time range (ISO 8601, exclusive)'),
    segmentStartTime: z.string().optional().describe('Override the segment\'s built-in time range start (ISO 8601)'),
    segmentEndTime: z.string().optional().describe('Override the segment\'s built-in time range end (ISO 8601)')
  }))
  .output(z.object({
