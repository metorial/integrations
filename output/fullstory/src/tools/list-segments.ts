...
export let listSegments = SlateTool.create(
  ...
  .input(z.object({
    limit: z.number().optional().describe('Number of segments to return (default 20, max 100)'),
    paginationToken: z.string().optional().describe('Token for retrieving the next page of results'),
  ...
  .output(z.object({
    ...
    nextPaginationToken: z.string().optional().describe('Token for the next page, absent if no more results')
  }))
