...
export let updateTask = SlateTool.create(
  ...
  .input(z.object({
    ...
    priority: z.enum(['not set', 'critical', 'important', 'normal', 'minor']).optional().describe('New priority'),
    status: z.string().optional().describe('New status (e.g., "backlog", "todo", "doing", "done", "closed", "feedback", or custom column name)'),
    assignedToId: z.number().nullable().optional().describe('User ID to assign, or null to unassign all'),
    assignedToEmail: z.string().optional().describe('Email of the assignee'),
    unassignUserId: z.number().optional().describe('Specific user ID to unassign from the task'),
    tagNames: z.array(z.string()).optional().describe('New set of tags (replaces existing tags)'),
    externalId: z.string().optional().describe('External tracking ID'),
    updaterEmail: z.string().optional().describe('Email of the user making the update (for audit trail)'),
  }))
  .output(z.object({
