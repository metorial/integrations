...
export let sendTemplateRequest = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    })).optional().describe('CC recipients mapped to template CC roles'),
    customFields: z.array(z.object({
      name: z.string().describe('Name of the template merge field'),
      value: z.string().describe('Value to pre-fill'),
    })).optional().describe('Pre-filled values for template merge fields'),
  ...
  .output(z.object({
    ...
    signatures: z.array(z.object({
      ...
      signerRole: z.string().optional().describe('Template role assigned to this signer'),
