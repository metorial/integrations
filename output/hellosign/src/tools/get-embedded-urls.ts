...
export let getEmbeddedUrls = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    signatureId: z.string().optional().describe('Signature ID (required for "sign" type, found in the signatures array of a request)'),
    templateId: z.string().optional().describe('Template ID (required for "edit" type)'),
    skipSignerRoles: z.boolean().optional().describe('Skip signer roles step in template editor (for "edit" type)'),
    skipSubjectMessage: z.boolean().optional().describe('Skip subject/message step in template editor (for "edit" type)'),
  }))
  .output(z.object({
