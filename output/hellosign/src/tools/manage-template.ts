...
export let manageTemplate = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['update', 'delete', 'add_user', 'remove_user']).describe('Action to perform'),
    ...
    userEmailAddress: z.string().optional().describe('Email of user to add/remove (for "add_user"/"remove_user")'),
    userAccountId: z.string().optional().describe('Account ID of user to add/remove (alternative to email)'),
  }))
  .output(z.object({
