...
export let downloadFiles = SlateTool.create(
  spec,
  {
    name: 'Download Files',
    key: 'download_files',
  ...
  .input(z.object({
    resourceType: z.enum(['signature_request', 'template']).describe('Whether to download files from a signature request or a template'),
    resourceId: z.string().describe('ID of the signature request or template'),
    fileType: z.enum(['pdf', 'zip']).optional().describe('File format to download (default: pdf)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.resourceType === 'signature_request') {
      let result = await client.getSignatureRequestFiles(ctx.input.resourceId, ctx.input.fileType);
