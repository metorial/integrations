...
export let getSignatureRequest = SlateTool.create(
  ...
  .output(z.object({
    ...
    })).optional().describe('Completed form field responses'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let signatures = (result.signatures || []).map((s: any) => ({
      ...
      lastViewedAt: s.last_viewed_at ? new Date(s.last_viewed_at * 1000).toISOString() : undefined,
      lastRemindedAt: s.last_reminded_at ? new Date(s.last_reminded_at * 1000).toISOString() : undefined,
    ...
    let statusSummary = signatures.map((s: any) => `${s.signerName}: ${s.statusCode}`).join(', ');
