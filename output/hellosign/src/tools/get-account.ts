...
export let getAccount = SlateTool.create(
  ...
  .output(z.object({
    ...
    isPaidHs: z.boolean().optional().describe('Whether the account has a paid HelloSign plan'),
    isPaidHf: z.boolean().optional().describe('Whether the account has a paid HelloFax plan'),
    quotas: z.object({
      templatesLeft: z.number().optional().describe('Number of templates remaining'),
      apiSignatureRequestsLeft: z.number().optional().describe('API signature requests remaining'),
    ...
    }).optional().describe('Account quotas'),
