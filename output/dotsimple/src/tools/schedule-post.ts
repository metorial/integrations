...
export let schedulePost = SlateTool.create(
  ...
  .input(z.object({
    ...
    postNow: z.boolean().optional().describe('If true, publish the post immediately. If false, schedule for the configured date/time.'),
    addToQueue: z.boolean().optional().describe('If true, add the post to the publishing queue instead of scheduling'),
  }))
  .output(z.object({
