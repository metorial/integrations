...
export let manageDeployments = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    environment: z.string().optional().describe('Deployment environment (for list filter or create)'),
    ...
    localUsername: z.string().optional().describe('Username of who deployed (for list filter or create)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { action, projectId, deployId, environment, revision, repository, localUsername, limit } = ctx.input;
    ...
    switch (action) {
      ...
      case 'create': {
        if (!ctx.auth.projectToken) {
          throw new Error('A project API key is required to record deployments. Configure it in your authentication settings.');
        }
