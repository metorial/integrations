...
export let manageUptime = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    matchType: z.enum(['success', 'exact', 'include', 'exclude']).optional().describe('Response match type'),
    match: z.string().optional().describe('Match pattern for response validation'),
    requestMethod: z.enum(['GET', 'POST', 'PUT', 'PATCH', 'DELETE']).optional().describe('HTTP method'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let { action, projectId, siteId, name, url, frequency, matchType, match, requestMethod, validateSsl, active } = ctx.input;
    ...
    switch (action) {
      ...
      case 'create': {
        ...
        let created = await client.createSite(projectId, {
          name, url, frequency, matchType, match, requestMethod, validateSsl, active,
        });
        return {
