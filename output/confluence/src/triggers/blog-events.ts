...
let blogEventTypes = [
  'blog_created', 'blog_updated', 'blog_moved',
  'blog_trashed', 'blog_restored', 'blog_removed', 'blog_viewed'
] as const;

export let blogEvents = SlateTrigger.create(spec, {
  name: 'Blog Post Events',
  key: 'blog_events',
  description: 'Triggered when blog posts are created, updated, moved, trashed, restored, removed, or viewed.'
})
  .input(z.object({
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      try {
        ...
        output.title = ctx.input.rawPayload?.blog?.title || ctx.input.rawPayload?.content?.title;
      }
