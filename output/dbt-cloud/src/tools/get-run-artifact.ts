...
export let getRunArtifactTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    path: z.string().describe('Artifact file path (e.g., "manifest.json", "run_results.json", "catalog.json")'),
    step: z.number().optional().describe('Step index (1-based) to retrieve artifacts from. Defaults to the latest step.')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let artifact = await client.getRunArtifact(ctx.input.runId, ctx.input.path, ctx.input.step);
