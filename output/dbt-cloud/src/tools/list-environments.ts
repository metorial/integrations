...
export let listEnvironmentsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    limit: z.number().optional().describe('Maximum number of environments to return (max 100)'),
  ...
  .output(z.object({
    environments: z.array(z.object({
      ...
      type: z.string().optional().describe('Environment type (e.g., deployment, development)'),
      dbtVersion: z.string().optional().describe('dbt version used in this environment'),
      useCustomBranch: z.boolean().optional().describe('Whether this environment uses a custom branch'),
      customBranch: z.string().nullable().optional().describe('Custom branch name if applicable'),
