...
export let createCheckoutSession = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    mode: z.enum(['payment', 'subscription', 'setup']).optional().describe('Checkout mode (required for create)'),
    successUrl: z.string().optional().describe('URL to redirect after successful payment (required for create)'),
    ...
    })).optional().describe('Line items for the checkout session'),
  ...
  .output(z.object({
    ...
    url: z.string().optional().nullable().describe('Checkout page URL (redirect customer here)'),
    ...
    amountTotal: z.number().optional().nullable().describe('Total amount in smallest currency unit'),
    ...
    })).optional().describe('List of sessions'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      ...
      if (ctx.input.allowPromotionCodes !== undefined) params.allow_promotion_codes = ctx.input.allowPromotionCodes;
