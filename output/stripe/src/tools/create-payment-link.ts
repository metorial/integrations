...
export let createPaymentLink = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    })).optional().describe('Line items for the payment link (required for create)'),
  ...
  .output(z.object({
    ...
    paymentLinks: z.array(z.object({
      ...
      url: z.string(),
      active: z.boolean(),
    })).optional().describe('List of payment links'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'get') {
      if (!ctx.input.paymentLinkId) throw new Error('paymentLinkId is required for get action');
