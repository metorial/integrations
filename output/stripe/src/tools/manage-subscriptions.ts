...
export let manageSubscriptions = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'get', 'update', 'cancel', 'pause', 'resume', 'list']).describe('Operation to perform'),
    subscriptionId: z.string().optional().describe('Subscription ID (required for get/update/cancel/pause/resume)'),
    customerId: z.string().optional().describe('Customer ID (required for create)'),
    priceId: z.string().optional().describe('Price ID to subscribe to (for create or adding items)'),
    items: z.array(z.object({
      priceId: z.string().describe('Price ID'),
      quantity: z.number().optional().describe('Quantity'),
    })).optional().describe('Subscription items (for create or update)'),
    trialPeriodDays: z.number().optional().describe('Number of trial days'),
    trialEnd: z.string().optional().describe('Unix timestamp or "now" for when the trial ends'),
    cancelAtPeriodEnd: z.boolean().optional().describe('If true, cancel at end of current billing period'),
    ...
    status: z.enum(['active', 'past_due', 'unpaid', 'canceled', 'incomplete', 'incomplete_expired', 'trialing', 'paused', 'all']).optional().describe('Filter by status (for list)'),
  }))
  .output(z.object({
    ...
    cancelAtPeriodEnd: z.boolean().optional().describe('Whether it will cancel at period end'),
    ...
    subscriptions: z.array(z.object({
      ...
      created: z.number(),
    })).optional().describe('List of subscriptions'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      ...
      if (ctx.input.trialPeriodDays !== undefined) params.trial_period_days = ctx.input.trialPeriodDays;
      ...
      if (ctx.input.defaultPaymentMethodId) params.default_payment_method = ctx.input.defaultPaymentMethodId;
    ...
    if (action === 'get') {
      if (!ctx.input.subscriptionId) throw new Error('subscriptionId is required for get action');
    ...
    if (action === 'update') {
      if (!ctx.input.subscriptionId) throw new Error('subscriptionId is required for update action');
      ...
      if (ctx.input.cancelAtPeriodEnd !== undefined) params.cancel_at_period_end = ctx.input.cancelAtPeriodEnd;
      if (ctx.input.couponId) params.coupon = ctx.input.couponId;
      if (ctx.input.promotionCodeId) params.promotion_code = ctx.input.promotionCodeId;
      if (ctx.input.defaultPaymentMethodId) params.default_payment_method = ctx.input.defaultPaymentMethodId;
    ...
    if (action === 'cancel') {
      if (!ctx.input.subscriptionId) throw new Error('subscriptionId is required for cancel action');

      if (ctx.input.cancelAtPeriodEnd) {
        let sub = await client.updateSubscription(ctx.input.subscriptionId, { cancel_at_period_end: true });
        return {
          output: {
            subscriptionId: sub.id,
            customerId: sub.customer,
    ...
    if (action === 'pause') {
      if (!ctx.input.subscriptionId) throw new Error('subscriptionId is required for pause action');
    ...
    if (action === 'resume') {
      if (!ctx.input.subscriptionId) throw new Error('subscriptionId is required for resume action');
