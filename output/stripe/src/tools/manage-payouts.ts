...
export let managePayouts = SlateTool.create(
  ...
  .input(z.object({
    ...
    statusFilter: z.enum(['pending', 'paid', 'failed', 'canceled']).optional().describe('Filter by status (for list)'),
  }))
  .output(z.object({
    ...
    payouts: z.array(z.object({
      ...
      created: z.number(),
    })).optional().describe('List of payouts'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      if (ctx.input.amount === undefined) throw new Error('amount is required for create action');
