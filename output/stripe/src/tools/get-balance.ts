...
export let getBalance = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['get_balance', 'list_transactions', 'get_transaction']).describe('Operation to perform'),
    transactionId: z.string().optional().describe('Balance transaction ID (for get_transaction)'),
    ...
    type: z.string().optional().describe('Filter transactions by type (e.g., charge, refund, payout, transfer)'),
  }))
  .output(z.object({
    available: z.array(balanceAmountSchema).optional().describe('Available balance by currency'),
    ...
    transactionDescription: z.string().optional().nullable().describe('Transaction description'),
    ...
    transactions: z.array(z.object({
      ...
      fee: z.number(),
      created: z.number(),
    })).optional().describe('List of balance transactions'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'get_transaction') {
      if (!ctx.input.transactionId) throw new Error('transactionId is required for get_transaction action');
