...
export let manageCoupons = SlateTool.create(
  ...
  .input(z.object({
    ...
    amountOff: z.number().optional().describe('Fixed amount discount in smallest currency unit'),
    currency: z.string().optional().describe('Currency for amountOff'),
    duration: z.enum(['once', 'repeating', 'forever']).optional().describe('How long the discount lasts'),
    durationInMonths: z.number().optional().describe('Number of months (for repeating duration)'),
    maxRedemptions: z.number().optional().describe('Max number of times the coupon can be redeemed'),
  ...
  .output(z.object({
    ...
    coupons: z.array(z.object({
      ...
      duration: z.string(),
      valid: z.boolean(),
    })).optional().describe('List of coupons'),
    promotionCodes: z.array(z.object({
      promotionCodeId: z.string(),
      code: z.string(),
      couponId: z.string(),
      active: z.boolean(),
    })).optional().describe('List of promotion codes'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (resource === 'coupon') {
      if (action === 'create') {
        ...
        if (ctx.input.durationInMonths !== undefined) params.duration_in_months = ctx.input.durationInMonths;
        if (ctx.input.maxRedemptions !== undefined) params.max_redemptions = ctx.input.maxRedemptions;
    ...
    // Promotion codes
    if (action === 'create') {
      if (!ctx.input.couponId) throw new Error('couponId is required for promotion code creation');
