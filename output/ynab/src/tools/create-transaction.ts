...
let subtransactionInput = z.object({
  ...
  payeeName: z.string().optional().describe('Payee name (max 200 chars). Used if payeeId is not provided.'),
...
let transactionInput = z.object({
  ...
  amount: z.number().describe('Amount in milliunits (negative for outflows, positive for inflows)'),
  payeeId: z.string().optional().describe('Payee ID'),
  payeeName: z.string().optional().describe('Payee name (max 200 chars). Creates a new payee if no match exists.'),
  categoryId: z.string().optional().describe('Category ID'),
  memo: z.string().optional().describe('Memo (max 500 chars)'),
  cleared: z.enum(['cleared', 'uncleared', 'reconciled']).optional().describe('Cleared status'),
  approved: z.boolean().optional().describe('Whether the transaction is approved. Defaults to false.'),
  ...
  subtransactions: z.array(subtransactionInput).optional().describe('Split transaction parts. If provided, their amounts must sum to the transaction amount.'),
});
...
export let createTransaction = SlateTool.create(
  ...
  .input(z.object({
    ...
    transactions: z.array(transactionInput).min(1).describe('One or more transactions to create'),
  }))
  .output(z.object({
    transactionIds: z.array(z.string()).describe('IDs of created transactions'),
    duplicateImportIds: z.array(z.string()).optional().describe('Import IDs that were duplicates and skipped'),
