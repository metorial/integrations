...
let frequencyEnum = z.enum([
  ...
  'every4Weeks', 'monthly', 'everyOtherMonth', 'everyThreeMonths',
  'everyFourMonths', 'twiceAYear', 'yearly', 'everyOtherYear',
]);

export let manageScheduledTransaction = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    scheduledTransactionId: z.string().optional().describe('Required for update/delete actions'),
    accountId: z.string().optional().describe('Account ID (required for create)'),
    date: z.string().optional().describe('Date for the scheduled transaction (YYYY-MM-DD, must be future)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'delete') {
      ...
      let st = await client.deleteScheduledTransaction(budgetId, ctx.input.scheduledTransactionId);
      return {
        output: {
    ...
    if (action === 'create') {
      ...
      st = await client.updateScheduledTransaction(budgetId, ctx.input.scheduledTransactionId, data);
    }
