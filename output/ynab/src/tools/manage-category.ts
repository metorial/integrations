...
export let manageCategory = SlateTool.create(
  ...
  .input(z.object({
    ...
    categoryGroupId: z.string().optional().describe('Category group ID (required for create)'),
    ...
    goalType: z.enum(['TB', 'TBD', 'MF', 'NEED', 'DEBT']).nullable().optional().describe('Goal type (for update). Set to null to remove goal.'),
    goalTarget: z.number().nullable().optional().describe('Goal target amount in milliunits (for update)'),
    goalTargetDate: z.string().nullable().optional().describe('Goal target date YYYY-MM-DD (for update)'),
    ...
    budgeted: z.number().optional().describe('Budgeted (assigned) amount in milliunits for assign action'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      ...
      if (ctx.input.goalTargetDate !== undefined) updateData.goal_target_month = ctx.input.goalTargetDate;
      ...
      category = await client.updateMonthCategory(budgetId, ctx.input.month, ctx.input.categoryId, ctx.input.budgeted);
    }
