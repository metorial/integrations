...
let transactionSchema = z.object({
  ...
  transferAccountId: z.string().nullable().optional().describe('Transfer account ID if this is a transfer'),
...
export let listTransactions = SlateTool.create(
  ...
  .input(z.object({
    ...
    month: z.string().optional().describe('Filter by month (YYYY-MM-DD, first day of the month)'),
    sinceDate: z.string().optional().describe('Only return transactions on or after this date (YYYY-MM-DD)'),
    filterType: z.enum(['uncategorized', 'unapproved']).optional().describe('Filter by transaction type'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.accountId) {
      let result = await client.getTransactionsByAccount(budgetId, ctx.input.accountId, options);
      transactions = result.transactions;
      serverKnowledge = result.serverKnowledge;
    } else if (ctx.input.categoryId) {
      transactions = await client.getTransactionsByCategory(budgetId, ctx.input.categoryId, options);
