...
export let sendMediaTool = SlateTool.create(
  spec,
  {
    name: 'Send Media',
    key: 'send_media',
  ...
  .input(z.object({
    ...
    mediaType: z.enum(['photo', 'document', 'audio', 'video']).describe('Type of media to send'),
    mediaUrl: z.string().describe('URL or file_id of the media to send'),
    caption: z.string().optional().describe('Caption for the media'),
    parseMode: z.enum(['HTML', 'MarkdownV2', 'Markdown']).optional().describe('Caption formatting mode'),
    ...
    audioDuration: z.number().optional().describe('Duration of audio in seconds (audio only)'),
    ...
    videoDuration: z.number().optional().describe('Duration of video in seconds (video only)'),
