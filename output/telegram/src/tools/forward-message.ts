...
export let forwardMessageTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    fromChatId: z.string().describe('Source chat ID or @username where the original message exists'),
    messageId: z.number().describe('ID of the message to forward'),
    disableNotification: z.boolean().optional().describe('Forward silently without notification'),
    messageThreadId: z.number().optional().describe('Forum topic thread ID in the target supergroup'),
  }))
  .output(z.object({
    ...
    forwardOriginDate: z.number().optional().describe('Unix timestamp of the original message'),
  }))
