...
let inlineResultSchema = z.object({
  ...
  parseMode: z.enum(['HTML', 'MarkdownV2', 'Markdown']).optional().describe('Formatting for messageText'),
});

export let answerInlineQueryTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    cacheTime: z.number().optional().describe('Max time in seconds to cache results (default 300)'),
