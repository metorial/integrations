...
export let inlineQueryReceivedTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Inline Query Received',
    key: 'inline_query_received',
    description: 'Triggers when a user types "@botname query" in any chat, sending an inline query to the bot.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    inlineQueryId: z.string().optional().describe('Unique inline query identifier (use to answer the query)'),
    ...
    chosenResultId: z.string().optional().describe('ID of the chosen inline result (chosen_inline_result only)'),
    chosenInlineMessageId: z.string().optional().describe('Inline message ID of the chosen result'),
  }))
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      if (data.inline_query) {
        inputs.push({ updateId: data.update_id, eventType: 'inline_query', inlineData: data.inline_query });
      } else if (data.chosen_inline_result) {
        inputs.push({ updateId: data.update_id, eventType: 'chosen_inline_result', inlineData: data.chosen_inline_result });
      }
