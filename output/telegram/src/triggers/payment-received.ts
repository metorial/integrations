...
export let paymentReceivedTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Payment Event',
    key: 'payment_received',
    description: 'Triggers on payment-related events: shipping queries, pre-checkout queries, and purchased paid media.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    queryId: z.string().optional().describe('Shipping or pre-checkout query ID (use to answer)'),
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      if (data.shipping_query) {
        inputs.push({ updateId: data.update_id, eventType: 'shipping_query', paymentData: data.shipping_query });
      } else if (data.pre_checkout_query) {
        inputs.push({ updateId: data.update_id, eventType: 'pre_checkout_query', paymentData: data.pre_checkout_query });
      } else if (data.purchased_paid_media) {
        inputs.push({ updateId: data.update_id, eventType: 'purchased_paid_media', paymentData: data.purchased_paid_media });
      }
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        ...
        output: {
          ...
          shippingAddress: ctx.input.eventType === 'shipping_query' ? d.shipping_address : undefined,
