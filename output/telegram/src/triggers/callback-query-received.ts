...
export let callbackQueryReceivedTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Callback Query Received',
    key: 'callback_query_received',
    description: 'Triggers when a user presses an inline keyboard button and a callback query is received.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    callbackQueryId: z.string().describe('Unique callback query identifier (use to answer the query)'),
    ...
    inlineMessageId: z.string().optional().describe('Inline message ID if the button was on an inline message'),
