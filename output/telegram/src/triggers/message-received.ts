...
export let messageReceivedTrigger = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers when a new message, edited message, channel post, or edited channel post is received by the bot.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    forwardOriginDate: z.number().optional().describe('Unix timestamp of the original forwarded message'),
  }))
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      if (data.message) {
        ...
        inputs.push({ updateId: data.update_id, eventType: 'edited_message', message: data.edited_message });
      } else if (data.channel_post) {
        inputs.push({ updateId: data.update_id, eventType: 'channel_post', message: data.channel_post });
      } else if (data.edited_channel_post) {
        inputs.push({ updateId: data.update_id, eventType: 'edited_channel_post', message: data.edited_channel_post });
      }
