...
export let listApplications = SlateTool.create(
  ...
  .input(z.object({
    limit: z.number().min(1).max(100).optional().describe('Maximum number of applications to return'),
    skip: z.number().min(0).optional().describe('Number to skip for pagination'),
    filter: z.string().optional().describe('Filter expression, e.g. "displayLabel:$eq:Slack"'),
