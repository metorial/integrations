...
export let manageUser = SlateTool.create(
  ...
  .input(z.object({
    ...
    employeeType: z.string().optional().describe('Employee type, e.g. "Full-Time", "Contractor"'),
    ...
    enableMfa: z.boolean().optional().describe('Enable user portal multi-factor authentication'),
    passwordNeverExpires: z.boolean().optional().describe('Whether the password never expires'),
    ...
    })).optional().describe('Custom attributes to set on the user'),
    phoneNumbers: z.array(z.object({
      number: z.string().describe('Phone number'),
      type: z.string().optional().describe('Phone type: mobile, home, work, work_mobile, work_fax, other'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let buildUserData = () => {
      ...
      if (ctx.input.employeeIdentifier !== undefined) data.employeeIdentifier = ctx.input.employeeIdentifier;
      ...
      if (ctx.input.alternateEmail !== undefined) data.alternateEmail = ctx.input.alternateEmail;
      if (ctx.input.description !== undefined) data.description = ctx.input.description;
      if (ctx.input.ldapBindingUser !== undefined) data.ldap_binding_user = ctx.input.ldapBindingUser;
      if (ctx.input.enableMfa !== undefined) data.enable_user_portal_multifactor = ctx.input.enableMfa;
      if (ctx.input.passwordNeverExpires !== undefined) data.password_never_expires = ctx.input.passwordNeverExpires;
      if (ctx.input.passwordlessSudo !== undefined) data.passwordless_sudo = ctx.input.passwordlessSudo;
