...
export let manageSystem = SlateTool.create(
  spec,
  {
    name: 'Manage System',
    key: 'manage_system',
  ...
  .input(z.object({
    ...
    allowMultiFactorAuthentication: z.boolean().optional().describe('Allow MFA for this system'),
    allowPublicKeyAuthentication: z.boolean().optional().describe('Allow public key authentication'),
    allowSshPasswordAuthentication: z.boolean().optional().describe('Allow SSH password authentication'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'update') {
      ...
      if (ctx.input.allowMultiFactorAuthentication !== undefined) data.allowMultiFactorAuthentication = ctx.input.allowMultiFactorAuthentication;
      if (ctx.input.allowPublicKeyAuthentication !== undefined) data.allowPublicKeyAuthentication = ctx.input.allowPublicKeyAuthentication;
      if (ctx.input.allowSshPasswordAuthentication !== undefined) data.allowSshPasswordAuthentication = ctx.input.allowSshPasswordAuthentication;
      if (ctx.input.allowSshRootLogin !== undefined) data.allowSshRootLogin = ctx.input.allowSshRootLogin;
