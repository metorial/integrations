...
export let manageCommand = SlateTool.create(
  spec,
  {
    name: 'Manage Command',
    key: 'manage_command',
  ...
  .input(z.object({
    ...
    commandType: z.enum(['linux', 'windows']).optional().describe('Command type: "linux" (for Mac/Linux) or "windows"'),
    user: z.string().optional().describe('User to run the command as (e.g. "root", "000000000000")'),
    sudo: z.boolean().optional().describe('Run with sudo privileges'),
    schedule: z.string().optional().describe('Cron schedule expression for scheduled commands'),
    scheduleRepeatType: z.string().optional().describe('Schedule repeat type'),
    launchType: z.enum(['manual', 'trigger', 'repeated']).optional().describe('How the command is launched'),
    ...
    shell: z.string().optional().describe('Shell to use (e.g. /bin/bash, /bin/sh, powershell)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let buildData = () => {
      ...
      if (ctx.input.scheduleRepeatType !== undefined) data.scheduleRepeatType = ctx.input.scheduleRepeatType;
