...
export let tableEvents = SlateTrigger.create(
  spec,
  {
    name: 'Table Events',
    key: 'table_events',
    description: 'Triggers when rows are created, updated, or deleted in a Baserow table, or when views or fields change. Set up a webhook in Baserow (table settings > Webhooks) and point it to this trigger\'s URL.',
    instructions: [
  ...
  .input(z.object({
    eventType: z.string().describe('The event type (e.g. rows.created, rows.updated, rows.deleted, field.created, view.created)'),
    ...
    items: z.array(z.record(z.string(), z.any())).optional().describe('Affected row/field/view data'),
    oldItems: z.array(z.record(z.string(), z.any())).optional().describe('Previous state of rows (for update events)')
  }))
  .output(z.object({
    ...
    databaseId: z.number().optional().describe('The database ID'),
    workspaceId: z.number().optional().describe('The workspace ID'),
    items: z.array(z.record(z.string(), z.any())).describe('Affected data items (rows, fields, or views)'),
    oldItems: z.array(z.record(z.string(), z.any())).optional().describe('Previous state of rows (only for rows.updated events)')
  }))
  .webhook({
