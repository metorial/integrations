...
export let listUnitsTool = SlateTool.create(
  spec,
  {
    name: 'List Units',
    key: 'list_units',
  ...
  .input(z.object({
    ...
    state: z.enum(['available', 'occupied', 'blocked', 'archived']).optional().describe('Filter units by state'),
    search: z.string().optional().describe('Search query to filter units'),
    updatedAfter: z.string().optional().describe('Only return units updated after this UTC timestamp (e.g. 2024-01-01T00:00:00.000Z)'),
