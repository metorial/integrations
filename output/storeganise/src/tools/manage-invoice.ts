...
export let manageInvoiceTool = SlateTool.create(
  spec,
  {
    name: 'Manage Invoice',
    key: 'manage_invoice',
  ...
  .input(z.object({
    ...
    action: z.enum(['update', 'add_payment', 'add_comment', 'delete']).describe('Action to perform on the invoice'),
    updateFields: z.object({
      state: z.string().optional().describe('New invoice state (e.g. "sent", "paid", "void")'),
      dueDate: z.string().optional().describe('Updated due date in ISO format'),
      notes: z.string().optional().describe('Updated notes'),
    }).optional().describe('Fields to update (required when action is "update")'),
    paymentAmount: z.number().optional().describe('Payment amount (required when action is "add_payment")'),
    paymentMethod: z.string().optional().describe('Payment method description (e.g. "cash", "bank_transfer", "card")'),
    comment: z.string().optional().describe('Comment text (required when action is "add_comment")'),
  }))
  .output(z.object({
    invoice: z.record(z.string(), z.any()).optional().describe('Updated invoice details (not returned for delete action)'),
