...
export let manageCollections = SlateTool.create(
  ...
  .input(z.object({
    ...
    collectionId: z.number().optional().describe('Collection ID (required for get, update, delete)'),
    title: z.string().optional().describe('Collection title (required for create; optional for update)'),
    customData: z.string().optional().describe('Custom reference data (max 1024 chars; for create or update)'),
    templateUuids: z.array(z.string()).optional().describe('Template UUIDs to include (for create; max 500)'),
    addTemplateUuids: z.array(z.string()).optional().describe('Template UUIDs to add to the collection (for update)'),
    removeTemplateUuids: z.array(z.string()).optional().describe('Template UUIDs to remove from the collection (for update)'),
    perPage: z.number().optional().describe('Results per page (max 100; for list)'),
  }))
  .output(z.object({
    collection: collectionOutputSchema.optional().describe('Single collection (for get, create, update)'),
    collections: z.array(collectionOutputSchema).optional().describe('List of collections (for list)'),
    deleted: z.boolean().optional().describe('Whether the collection was deleted (for delete)'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new PlacidClient({ token: ctx.auth.token });

    let mapCollection = (c: { id: number; title: string; custom_data: string | null; template_uuids: string[] }) => ({
      collectionId: c.id,
      title: c.title,
    ...
    switch (ctx.input.action) {
      ...
      case 'get': {
        if (ctx.input.collectionId === undefined) throw new Error('collectionId is required for "get" action');
      ...
      case 'update': {
        if (ctx.input.collectionId === undefined) throw new Error('collectionId is required for "update" action');
      ...
      case 'delete': {
        if (ctx.input.collectionId === undefined) throw new Error('collectionId is required for "delete" action');
