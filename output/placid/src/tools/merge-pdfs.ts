...
}).optional().describe('Transfer merged PDF to S3-compatible storage');
...
export let mergePdfs = SlateTool.create(
  ...
  .input(z.object({
    pdfUrls: z.array(z.string()).min(2).max(10).describe('URLs of PDF files to merge (2-10)'),
    webhookUrl: z.string().optional().describe('URL to receive a POST notification when merge completes'),
