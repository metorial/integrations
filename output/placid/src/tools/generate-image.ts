...
}).optional().describe('Transfer generated image to S3-compatible storage');
...
export let generateImage = SlateTool.create(
  ...
  .input(z.object({
    ...
    layers: z.record(z.string(), z.record(z.string(), z.unknown())).optional().describe('Layer data keyed by layer name. Each value is an object of layer properties (e.g., {"title": {"text": "Hello"}, "img": {"image": "https://..."}})'),
    width: z.number().optional().describe('Output width in pixels (aspect ratio is maintained)'),
    height: z.number().optional().describe('Output height in pixels (aspect ratio is maintained)'),
    ...
    createNow: z.boolean().optional().describe('Set true for synchronous generation instead of queued'),
  ...
  .output(z.object({
    ...
    imageUrl: z.string().nullable().describe('URL of the generated image (null if still queued)'),
