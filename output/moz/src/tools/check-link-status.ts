...
export let checkLinkStatusTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    sources: z.array(z.string()).min(1).max(50).describe('Source URLs or domains to check (up to 50)'),
    targetScope: z.enum(['page', 'subdomain', 'root_domain']).optional().describe('Scope of the target'),
    sourceScope: z.enum(['page', 'subdomain', 'root_domain']).optional().describe('Scope of the sources'),
  }))
  .output(z.object({
