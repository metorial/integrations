...
export let getUsageAndIndexTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    startTimestamp: z.string().optional().describe('Start time as Unix timestamp string (seconds since epoch)'),
    endTimestamp: z.string().optional().describe('End time as Unix timestamp string (seconds since epoch)'),
    includeIndexMetadata: z.boolean().optional().describe('Also fetch index metadata (default: true)'),
  }))
  .output(z.object({
    rowsConsumed: z.number().optional().describe('Number of API rows consumed in the period'),
    indexId: z.string().optional().describe('Current index ID (changes when data is updated)'),
    spamScoreUpdateDays: z.array(z.string()).optional().describe('Dates when spam score updates occurred'),
  }))
