...
export let getTopPagesTool = SlateTool.create(
  spec,
  {
    name: 'Get Top Pages',
    key: 'get_top_pages',
  ...
  .input(z.object({
    ...
    scope: z.enum(['subdomain', 'root_domain']).optional().describe('Scope of analysis (default: root_domain)'),
    sort: z.enum(['page_authority', 'root_domains_to_page', 'external_pages_to_page']).optional().describe('Sort order (default: page_authority)'),
    filter: z.enum(['all', 'status_200', 'status_301', 'status_302', 'status_4xx', 'status_5xx']).optional().describe('Filter by HTTP status code'),
