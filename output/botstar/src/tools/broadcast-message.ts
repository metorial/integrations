...
export let broadcastMessage = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    })).min(1).describe('Array of text messages to send'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    if (!ctx.auth.botToken) {
      throw new Error('Bot-level access token (botToken) is required for broadcasting messages. Configure it in authentication settings.');
    }
