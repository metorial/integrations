...
export let manageBotAttributes = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    attributeId: z.string().optional().describe('ID of the attribute (required for update and delete)'),
    name: z.string().optional().describe('Name of the attribute (required for create)'),
    dataType: z.enum(['string', 'number', 'date']).optional().describe('Data type (required for create)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of attributes (for list action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      ...
      let result = await client.createBotAttribute(botId, { name, dataType, desc: description, value }, env);
      return {
        output: {
