...
let bankTransactionOutputSchema = z.object({
  ...
  type: z.string().optional().describe('Transaction type: RECEIVE, SPEND, RECEIVE-OVERPAYMENT, etc.'),
...
export let createBankTransaction = SlateTool.create(
  spec,
  {
...
export let listBankTransactions = SlateTool.create(
  ...
  .input(z.object({
    ...
    modifiedAfter: z.string().optional().describe('Only return transactions modified after this date (ISO 8601)'),
    where: z.string().optional().describe('Xero API where filter, e.g. `Type=="RECEIVE"` or `BankAccount.AccountID==guid("...")`'),
    order: z.string().optional().describe('Order results, e.g. "Date DESC"'),
  }))
  .output(z.object({
    bankTransactions: z.array(bankTransactionOutputSchema).describe('List of bank transactions'),
