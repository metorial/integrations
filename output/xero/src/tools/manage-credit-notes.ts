...
export let createCreditNote = SlateTool.create(
  ...
  .input(z.object({
    type: z.enum(['ACCRECCREDIT', 'ACCPAYCREDIT']).describe('ACCRECCREDIT for customer credits, ACCPAYCREDIT for supplier credits'),
...
export let listCreditNotes = SlateTool.create(
  ...
  .input(z.object({
    ...
    modifiedAfter: z.string().optional().describe('Only return credit notes modified after this date (ISO 8601)'),
...
export let updateCreditNote = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    status: z.enum(['DRAFT', 'SUBMITTED', 'AUTHORISED', 'VOIDED']).optional().describe('New status'),
    lineItems: z.array(lineItemSchema).optional().describe('Replacement line items (only for draft credit notes)'),
