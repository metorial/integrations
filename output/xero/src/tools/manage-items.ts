...
export let listItems = SlateTool.create(
  ...
  .input(z.object({
    ...
    modifiedAfter: z.string().optional().describe('Only return items modified after this date (ISO 8601)'),
  }))
  .output(z.object({
...
export let createItem = SlateTool.create(
  ...
  .input(z.object({
    ...
    inventoryAssetAccountCode: z.string().optional().describe('Inventory asset account code (required if tracking inventory)'),
  }))
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.purchaseDescription) itemData.PurchaseDescription = ctx.input.purchaseDescription;
    ...
    if (ctx.input.isTrackedAsInventory !== undefined) itemData.IsTrackedAsInventory = ctx.input.isTrackedAsInventory;
    if (ctx.input.inventoryAssetAccountCode) itemData.InventoryAssetAccountCode = ctx.input.inventoryAssetAccountCode;

    if (ctx.input.salesUnitPrice !== undefined || ctx.input.salesAccountCode || ctx.input.salesTaxType) {
      itemData.SalesDetails = {};
      if (ctx.input.salesUnitPrice !== undefined) itemData.SalesDetails.UnitPrice = ctx.input.salesUnitPrice;
      if (ctx.input.salesAccountCode) itemData.SalesDetails.AccountCode = ctx.input.salesAccountCode;
      if (ctx.input.salesTaxType) itemData.SalesDetails.TaxType = ctx.input.salesTaxType;
    }

    if (ctx.input.purchaseUnitPrice !== undefined || ctx.input.purchaseAccountCode || ctx.input.purchaseTaxType) {
      itemData.PurchaseDetails = {};
      if (ctx.input.purchaseUnitPrice !== undefined) itemData.PurchaseDetails.UnitPrice = ctx.input.purchaseUnitPrice;
      if (ctx.input.purchaseAccountCode) itemData.PurchaseDetails.AccountCode = ctx.input.purchaseAccountCode;
      if (ctx.input.purchaseTaxType) itemData.PurchaseDetails.TaxType = ctx.input.purchaseTaxType;
    }
...
export let updateItem = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.purchaseDescription) updateData.PurchaseDescription = ctx.input.purchaseDescription;

    if (ctx.input.salesUnitPrice !== undefined || ctx.input.salesAccountCode) {
      updateData.SalesDetails = {};
      if (ctx.input.salesUnitPrice !== undefined) updateData.SalesDetails.UnitPrice = ctx.input.salesUnitPrice;
      if (ctx.input.salesAccountCode) updateData.SalesDetails.AccountCode = ctx.input.salesAccountCode;
    }

    if (ctx.input.purchaseUnitPrice !== undefined || ctx.input.purchaseAccountCode) {
      updateData.PurchaseDetails = {};
      if (ctx.input.purchaseUnitPrice !== undefined) updateData.PurchaseDetails.UnitPrice = ctx.input.purchaseUnitPrice;
      if (ctx.input.purchaseAccountCode) updateData.PurchaseDetails.AccountCode = ctx.input.purchaseAccountCode;
    }
