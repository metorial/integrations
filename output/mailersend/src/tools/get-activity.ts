...
Returns detailed activity records including recipient info, timestamps, and event details.`,
  ...
  .input(z.object({
    ...
    dateTo: z.number().describe('End of date range as Unix timestamp (UTC). Max 7 days after dateFrom.'),
    events: z.array(z.enum([
      'queued', 'sent', 'delivered', 'soft_bounced', 'hard_bounced',
      'deferred', 'opened', 'clicked', 'unsubscribed', 'spam_complaints',
      'survey_opened', 'survey_submitted',
    ])).optional().describe('Filter by specific event types.'),
    page: z.number().optional().describe('Page number for pagination.'),
    limit: z.number().min(10).max(100).optional().describe('Results per page (10-100, default 25).'),
  }))
  .output(z.object({
    activities: z.array(z.record(z.string(), z.unknown())).describe('List of activity records with event details.'),
