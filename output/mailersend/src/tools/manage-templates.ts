...
export let listTemplates = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    let total = (result.meta as Record<string, unknown>)?.total as number ?? templates.length;
...
export let getTemplate = SlateTool.create(
  ...
  .output(z.object({
    ...
    imagePath: z.string().nullable().describe('Template preview image URL.'),
    createdAt: z.string().describe('Creation timestamp.'),
    templateData: z.record(z.string(), z.unknown()).describe('Full template data including stats and domain info.'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
