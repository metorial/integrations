...
export let verifySingleEmail = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    verificationResult: z.record(z.string(), z.unknown()).describe('Full verification result including status, risk assessment, and details.'),
  }))
...
export let createVerificationList = SlateTool.create(
  spec,
  {
...
export let getVerificationList = SlateTool.create(
  spec,
  {
  ...
  .output(z.object({
    ...
    status: z.string().describe('Current status (e.g., "queued", "verifying", "completed").'),
    listData: z.record(z.string(), z.unknown()).describe('Full list data including statistics breakdown.'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
