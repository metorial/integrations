...
let suppressionTypeEnum = z.enum(['blocklist', 'hard-bounces', 'spam-complaints', 'unsubscribes']);
...
Suppression lists automatically prevent sending to problematic addresses.`,
  ...
  .output(z.object({
    entries: z.array(z.record(z.string(), z.unknown())).describe('List of suppression entries.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let total = (result.meta as Record<string, unknown>)?.total as number ?? (result.data || []).length;
...
export let addToSuppressions = SlateTool.create(
  spec,
  {
...
For blocklist, you can add both specific recipients and wildcard patterns.`,
  ...
  .input(z.object({
    ...
    patterns: z.array(z.string()).optional().describe('Wildcard patterns to suppress (blocklist only, e.g., "*@example.com").'),
  }))
  .output(z.object({
    entries: z.array(z.record(z.string(), z.unknown())).describe('Created suppression entries.'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
...
export let removeFromSuppressions = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    suppressionType: suppressionTypeEnum.describe('Type of suppression list to remove from.'),
