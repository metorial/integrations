...
export let createWebinar = SlateTool.create(
  spec,
  {
    name: 'Create Webinar',
    key: 'create_webinar',
  ...
  .input(z.object({
    ...
    type: z.number().default(5).describe('5=webinar, 6=recurring no fixed time, 9=recurring with fixed time'),
    startTime: z.string().optional().describe('Webinar start time in UTC (yyyy-MM-ddTHH:mm:ssZ)'),
    ...
    settings: z.object({
      ...
      registrationType: z.number().optional().describe('1=register once, 2=register each time, 3=register once choose occurrences'),
      ...
      questionAndAnswer: z.object({
        enable: z.boolean().describe('Enable Q&A'),
        allowAnonymousQuestions: z.boolean().optional().describe('Allow anonymous questions'),
      }).optional().describe('Q&A settings'),
    }).optional().describe('Webinar settings'),
  }))
  .output(z.object({
