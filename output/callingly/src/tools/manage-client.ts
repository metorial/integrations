...
- **deactivate** — Deactivate a client account.`,
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'delete', 'activate', 'deactivate']).describe('Operation to perform'),
    clientId: z.string().optional().describe('Client ID (required for delete, activate, deactivate)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of clients'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'list') {
      ...
      let clientsArray = Array.isArray(result) ? result : (result.clients ?? result.data ?? []);
