...
export let manageTeamAgents = SlateTool.create(
  spec,
  {
...
- **update** — Update an agent's priority and lead cap within the team.`,
  ...
  .input(z.object({
    ...
    agentIds: z.array(z.string()).optional().describe('Agent IDs to assign (required for "assign" action)'),
    agentId: z.string().optional().describe('Agent ID (required for "remove" and "update" actions)'),
    priority: z.number().optional().describe('Priority level for the agent within the team (for "update" action)'),
    cap: z.number().optional().describe('Lead cap for the agent within the team (for "update" action)'),
  }))
  .output(z.object({
    ...
    })).optional().describe('List of agents on the team'),
