...
let buildSchema = z.object({
  ...
  status: z.string().optional().describe('Build status (e.g., success, failed, error, running, complete)'),
...
export let listBuilds = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.array(z.enum(['running', 'error', 'failed', 'complete', 'success'])).optional().describe('Filter by build status'),
    name: z.string().optional().describe('Filter by build name'),
    start: z.string().optional().describe('Only return builds created after this time (ISO 8601)'),
    end: z.string().optional().describe('Only return builds created before this time (ISO 8601)'),
