...
export let testJobEvents = SlateTrigger.create(
  spec,
  {
    name: 'Test Job Events',
    key: 'test_job_events',
    description: 'Receive webhook notifications when test jobs complete on Sauce Labs. Covers both Virtual Device Cloud (VDC) and Real Device Cloud (RDC) test jobs. Configure in Sauce Labs under Account > Webhooks.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    automationBackend: z.string().optional().describe('Automation framework (webdriver, cypress, appium, etc.)'),
    ...
    modificationTime: z.number().optional().describe('Last modification time (Unix timestamp)'),
  ...
  .webhook({
    handleRequest: async (ctx) => {
      ...
      let eventType = data.passed === true ? 'job.passed'
        : data.passed === false ? 'job.failed'
