...
export let visualTestingEvents = SlateTrigger.create(
  spec,
  {
    name: 'Visual Testing Build Completed',
    key: 'visual_testing_events',
    description: 'Receive webhook notifications when a Visual Testing build is completed on Sauce Labs. Includes build status, snapshot summary, and project details. Configure in Sauce Labs under Account > Webhooks.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    status: z.string().optional().describe('Build status (APPROVED, EMPTY, EQUAL, ERRORED, QUEUED, REJECTED, UNAPPROVED)'),
    branch: z.string().optional().describe('Branch name'),
    project: z.string().optional().describe('Project name'),
    dataCenter: z.string().optional().describe('Data center (us-west-1, eu-central-1, us-east-1)'),
    summary: z.record(z.string(), z.number()).optional().describe('Snapshot counts by status'),
  }))
  .webhook({
