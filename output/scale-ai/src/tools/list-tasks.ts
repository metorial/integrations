...
export let listTasks = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['pending', 'completed', 'canceled', 'error']).optional().describe('Filter by task status'),
    taskType: z.string().optional().describe('Filter by task type (e.g., imageannotation)'),
    tags: z.string().optional().describe('Filter by tags (comma-separated)'),
    customerReviewStatus: z.string().optional().describe('Filter by review status: accepted, fixed, commented, rejected (comma-separated for multiple)'),
    completedAfter: z.string().optional().describe('Filter tasks completed after this ISO 8601 timestamp'),
    completedBefore: z.string().optional().describe('Filter tasks completed before this ISO 8601 timestamp'),
    createdAfter: z.string().optional().describe('Filter tasks created after this ISO 8601 timestamp'),
    createdBefore: z.string().optional().describe('Filter tasks created before this ISO 8601 timestamp'),
    updatedAfter: z.string().optional().describe('Filter tasks updated after this ISO 8601 timestamp'),
    updatedBefore: z.string().optional().describe('Filter tasks updated before this ISO 8601 timestamp'),
    limit: z.number().min(1).max(100).optional().describe('Max number of tasks to return (1-100, default 100)'),
  ...
  .output(z.object({
    ...
    }).passthrough()).describe('List of tasks'),
