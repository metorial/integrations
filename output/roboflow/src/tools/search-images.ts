...
export let searchImagesTool = SlateTool.create(
  spec,
  {
    name: 'Search Images',
    key: 'search_images',
    ...
    constraints: [
      'Maximum limit is 250 images per request.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    prompt: z.string().optional().describe('Text query for semantic/CLIP-based image search'),
    ...
    inDataset: z.boolean().optional().describe('Filter to images in the dataset (true) or not yet added (false)'),
    batchId: z.string().optional().describe('Filter images by batch ID'),
    offset: z.number().optional().describe('Pagination offset (default 0)'),
    limit: z.number().optional().describe('Maximum number of results to return (default 50, max 250)'),
  }))
  .output(z.object({
