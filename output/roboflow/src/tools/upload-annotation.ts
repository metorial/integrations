...
export let uploadAnnotationTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    annotationFileName: z.string().optional().describe('Filename for the annotation (e.g., "image.txt" for YOLO format)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    await client.uploadAnnotation(ctx.input.projectId, ctx.input.imageId, ctx.input.annotationText, {
