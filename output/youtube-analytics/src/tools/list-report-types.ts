...
export let listReportTypes = SlateTool.create(
  spec,
  {
...
Optionally include system-managed report types for content owners.`,
  ...
  .input(z.object({
    includeSystemManaged: z.boolean().optional().describe('Include system-managed report types (available for content owners).'),
  ...
  .output(z.object({
    reportTypes: z.array(z.object({
      reportTypeId: z.string().describe('Report type identifier used when creating reporting jobs.'),
