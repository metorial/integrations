...
Use the **list** action to view existing groups, **create** to make a new group, **update** to rename a group, or **delete** to remove a group.`,
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'update', 'delete']).describe('Action to perform on groups.'),
    groupId: z.string().optional().describe('Group ID (required for update and delete, optional for list to fetch a specific group).'),
    title: z.string().optional().describe('Group title (required for create and update).'),
    itemType: z.enum(['youtube#video', 'youtube#playlist', 'youtube#channel', 'youtubePartner#asset']).optional().describe('Type of items in the group (required for create).'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of groups (for list action).'),
    group: z.object({
      groupId: z.string().describe('Group ID.'),
      title: z.string().describe('Group title.'),
      publishedAt: z.string().describe('When the group was created.'),
      itemCount: z.number().describe('Number of items in the group.'),
      itemType: z.string().describe('Type of items in the group.'),
    }).optional().describe('Created or updated group (for create/update actions).'),
    deleted: z.boolean().optional().describe('Whether the group was deleted (for delete action).'),
