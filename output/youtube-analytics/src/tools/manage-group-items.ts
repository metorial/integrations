...
export let manageGroupItems = SlateTool.create(
  spec,
  {
...
Use **list** to see current items, **add** to add a resource, or **remove** to delete a specific item from the group.`,
    ...
    constraints: [
      'Groups can contain a maximum of 500 items.',
    ],
    tags: {
      destructive: false,
    },
  ...
  .input(z.object({
    ...
    resourceId: z.string().optional().describe('Resource ID to add, e.g. a video ID, playlist ID, channel ID, or asset ID (required for add).'),
    resourceKind: z.enum(['youtube#video', 'youtube#playlist', 'youtube#channel', 'youtubePartner#asset']).optional().describe('Kind of the resource being added (required for add).'),
    groupItemId: z.string().optional().describe('Group item ID to remove (required for remove). This is the association ID returned when listing items, not the resource ID.'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of group items (for list action).'),
    item: z.object({
      groupItemId: z.string().describe('Group item association ID.'),
      groupId: z.string().describe('Group ID.'),
      resourceKind: z.string().describe('Kind of the resource.'),
      resourceId: z.string().describe('Resource ID.'),
    }).optional().describe('Added group item (for add action).'),
    removed: z.boolean().optional().describe('Whether the item was removed (for remove action).'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new YouTubeAnalyticsClient({ token: ctx.auth.token });
    let { action, groupId, resourceId, resourceKind, groupItemId, onBehalfOfContentOwner } = ctx.input;
    ...
    if (action === 'add') {
      ...
      let item = await client.addGroupItem(groupId, resourceId, resourceKind, onBehalfOfContentOwner);
