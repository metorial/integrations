...
export let manageFile = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create_or_update', 'delete']).describe('Operation: create/update a file, or delete it'),
    ...
    startBranch: z.string().optional().describe('Create the target branch from this branch if it doesn\'t exist')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create_or_update') {
      if (!ctx.input.content && ctx.input.content !== '') throw new Error('Content is required for create/update');
