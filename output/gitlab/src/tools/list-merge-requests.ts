...
export let listMergeRequests = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    projectId: z.string().optional().describe('Project ID or path. Omit for global MR list.'),
    state: z.enum(['opened', 'closed', 'merged', 'all']).optional().describe('Filter by MR state'),
    ...
    scope: z.enum(['created_by_me', 'assigned_to_me', 'all']).optional().describe('Scope of merge requests'),
  ...
  .output(z.object({
    mergeRequests: z.array(z.object({
