...
export let browseRepository = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['tree', 'file', 'compare']).describe('Action: "tree" to list files, "file" to read content, "compare" to diff two refs'),
    path: z.string().optional().describe('File or directory path (for tree/file actions)'),
    ref: z.string().optional().describe('Branch, tag, or commit SHA to browse (defaults to default branch)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('Directory listing (tree action)'),
    file: z.object({
    ...
    }).optional().describe('File content (file action)'),
    comparison: z.object({
    ...
    }).optional().describe('Branch comparison (compare action)')
  }))
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'file': {
        ...
        let file = await client.getFileContent(ctx.input.projectId, ctx.input.path, ctx.input.ref);
      ...
      case 'compare': {
        if (!ctx.input.from || !ctx.input.to) throw new Error('Both "from" and "to" refs are required for compare');
        let comparison = await client.compareBranches(ctx.input.projectId, ctx.input.from, ctx.input.to);
