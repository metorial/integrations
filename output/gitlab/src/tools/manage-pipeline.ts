...
export let managePipeline = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'retry', 'cancel']).describe('Operation: create a new pipeline, or retry/cancel an existing one'),
    ...
    })).optional().describe('CI/CD variables to pass when triggering (create only)')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'create': {
        if (!ctx.input.ref) throw new Error('Ref (branch/tag) is required to create a pipeline');
        pipeline = await client.createPipeline(ctx.input.projectId, ctx.input.ref, ctx.input.variables);
        break;
      }
      case 'retry': {
    ...
    let actionVerb = { create: 'Triggered', retry: 'Retried', cancel: 'Cancelled' }[ctx.input.action];
