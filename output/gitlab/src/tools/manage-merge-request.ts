...
export let manageMergeRequest = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'merge', 'approve']).describe('Operation to perform'),
    projectId: z.string().describe('Project ID or URL-encoded path'),
    mergeRequestIid: z.number().optional().describe('Merge request IID (required for update, merge, approve)'),
    ...
    targetBranch: z.string().optional().describe('Target branch (required for create, optional for update)'),
    ...
    stateEvent: z.enum(['close', 'reopen']).optional().describe('Close or reopen the MR (update only)'),
    mergeCommitMessage: z.string().optional().describe('Custom merge commit message (merge only)'),
    squashCommitMessage: z.string().optional().describe('Custom squash commit message (merge only)')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'update': {
        if (!ctx.input.mergeRequestIid) throw new Error('Merge request IID is required for update');
      ...
      case 'merge': {
        if (!ctx.input.mergeRequestIid) throw new Error('Merge request IID is required for merge');
      ...
      case 'approve': {
        if (!ctx.input.mergeRequestIid) throw new Error('Merge request IID is required for approve');
    ...
    let actionVerb = { create: 'Created', update: 'Updated', merge: 'Merged', approve: 'Approved' }[ctx.input.action];
