...
export let listIssues = SlateTool.create(
  ...
  .input(z.object({
    projectId: z.string().optional().describe('Project ID or path to scope issues to a specific project. Omit for global issue list.'),
    ...
    scope: z.enum(['created_by_me', 'assigned_to_me', 'all']).optional().describe('Scope of issues to return'),
    orderBy: z.enum(['created_at', 'updated_at', 'priority', 'due_date', 'label_priority', 'weight']).optional().describe('Order by field'),
