...
let actionSchema = z.object({
  type: z.enum(['wait', 'click', 'fill', 'press', 'scroll', 'screenshot', 'executeJavascript']).describe('Type of browser action to perform'),
  selector: z.string().optional().describe('CSS selector for the target element (for wait, click, fill actions)'),
  milliseconds: z.number().optional().describe('Duration to wait in milliseconds (for wait action)'),
  text: z.string().optional().describe('Text to fill or key to press (for fill, press actions)'),
...
export let scrapePageTool = SlateTool.create(
  spec,
  {
    name: 'Scrape Page',
    key: 'scrape_page',
...
Use this to get clean, readable content from any web page. Supports browser automation actions like clicking buttons, filling forms, and scrolling before extraction.`,
  ...
  .input(z.object({
    ...
    formats: z.array(z.enum(['markdown', 'html', 'rawHtml', 'screenshot', 'links'])).optional().describe('Output formats to return. Defaults to markdown if not specified.'),
    onlyMainContent: z.boolean().optional().describe('Extract only the main content, excluding navigation, footers, etc. Defaults to true.'),
    includeTags: z.array(z.string()).optional().describe('Only include content from these CSS selectors'),
    excludeTags: z.array(z.string()).optional().describe('Exclude content matching these CSS selectors'),
    waitFor: z.number().optional().describe('Milliseconds to wait before extracting content'),
    ...
    actions: z.array(actionSchema).optional().describe('Browser actions to perform before extracting content'),
  }))
  .output(z.object({
