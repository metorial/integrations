...
export let getAgentStatusTool = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'Provide the agentId returned by the Run Agent tool.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .output(z.object({
    ...
    extractedData: z.any().optional().describe('Data gathered by the agent (available when completed)'),
