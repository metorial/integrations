...
export let updateOrder = SlateTool.create(
  ...
  .input(z.object({
    ...
    status: z.enum(['pending', 'voided', 'timeouted', 'cancelled', 'failed', 'refunded', 'paid', 'complete']).optional().describe('New order status'),
    statusFulfillment: z.enum(['not_fulfilled', 'fulfilled']).optional().describe('New fulfillment status'),
    ...
    metadata: z.record(z.string(), z.string()).optional().describe('Custom key-value metadata to attach to the order'),
  }))
  .output(z.object({
    orderId: z.string(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.statusFulfillment !== undefined) attributes.status_fulfillment = ctx.input.statusFulfillment;
    if (ctx.input.usn !== undefined) attributes.usn = ctx.input.usn;
    if (ctx.input.noteAdministrator !== undefined) attributes.note_administrator = ctx.input.noteAdministrator;
