...
export let listConversations = SlateTool.create(
  ...
  .input(z.object({
    ...
    includeTranscript: z.boolean().optional().describe('Include full transcript for a specific conversation'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.conversationId) {
      ...
      if (ctx.input.includeTranscript) {
        let transcriptResult = await client.getConversationTranscript(ctx.input.conversationId);
