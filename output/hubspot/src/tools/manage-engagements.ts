...
export let createEngagement = SlateTool.create(
  spec,
  {
...
Use **notes** for internal comments, **emails** for email tracking, **calls** for call logs, **meetings** for meeting records, and **tasks** for action items.`,
  ...
  .input(z.object({
    engagementType: z.enum(['notes', 'emails', 'calls', 'meetings', 'tasks']).describe('Type of engagement to create'),
    properties: z.record(z.string(), z.any()).describe('Engagement properties'),
    associations: z.array(z.object({
      to: z.object({ id: z.string().describe('ID of the CRM object to associate with') }),
      types: z.array(z.object({
        associationCategory: z.string().describe('Association category (e.g., HUBSPOT_DEFINED)'),
        associationTypeId: z.number().describe('Association type ID'),
      })),
    })).optional().describe('Associations to CRM records (contacts, companies, deals)'),
  }))
  .output(engagementOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new HubSpotClient(ctx.auth.token);
    let result = await client.createEngagement(ctx.input.engagementType, ctx.input.properties, ctx.input.associations);
...
export let getEngagement = SlateTool.create(
  spec,
  {
    name: 'Get Engagement',
    key: 'get_engagement',
  ...
  .input(z.object({
    engagementType: z.enum(['notes', 'emails', 'calls', 'meetings', 'tasks']).describe('Type of engagement'),
    engagementId: z.string().describe('HubSpot engagement ID'),
  ...
  .handleInvocation(async (ctx) => {
    let client = new HubSpotClient(ctx.auth.token);
    let result = await client.getEngagement(ctx.input.engagementType, ctx.input.engagementId, ctx.input.properties);
...
export let updateEngagement = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    let client = new HubSpotClient(ctx.auth.token);
    let result = await client.updateEngagement(ctx.input.engagementType, ctx.input.engagementId, ctx.input.properties);
...
export let deleteEngagement = SlateTool.create(
  spec,
  {
