...
let ownerOutputSchema = z.object({
  ...
  })).optional().describe('Teams the owner belongs to'),
});

export let listOwners = SlateTool.create(
  spec,
  {
    name: 'List Owners',
    key: 'list_owners',
  ...
  .input(z.object({
    limit: z.number().min(1).max(500).optional().describe('Number of owners to return (max 500)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.listOwners(ctx.input.limit || 100, ctx.input.after, ctx.input.email);
...
export let getOwner = SlateTool.create(
  spec,
  {
    name: 'Get Owner',
    key: 'get_owner',
