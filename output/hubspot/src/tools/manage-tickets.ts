...
let ticketPropertySchema = z.record(z.string(), z.any()).describe('Ticket properties as key-value pairs (e.g., subject, content, hs_pipeline, hs_pipeline_stage, hs_ticket_priority, hubspot_owner_id)');
...
export let createTicket = SlateTool.create(
  ...
  .input(z.object({
    ...
    })).optional().describe('Associations to create with the ticket (e.g., contacts, companies)'),
  }))
  .output(ticketOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new HubSpotClient(ctx.auth.token);
    let result = await client.createObject('tickets', ctx.input.properties, ctx.input.associations);
...
export let getTicket = SlateTool.create(
  ...
  .input(z.object({
    ...
    associations: z.array(z.string()).optional().describe('Associated object types to include'),
  }))
  .output(ticketOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new HubSpotClient(ctx.auth.token);
    let result = await client.getObject('tickets', ctx.input.ticketId, ctx.input.properties, ctx.input.associations);
...
export let updateTicket = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    let client = new HubSpotClient(ctx.auth.token);
    let result = await client.updateObject('tickets', ctx.input.ticketId, ctx.input.properties);
...
export let listTickets = SlateTool.create(
  ...
  .input(z.object({
    limit: z.number().min(1).max(100).optional().describe('Number of tickets to return (max 100)'),
  ...
  .handleInvocation(async (ctx) => {
    let client = new HubSpotClient(ctx.auth.token);
    let result = await client.listObjects('tickets', ctx.input.limit || 10, ctx.input.after, ctx.input.properties);
