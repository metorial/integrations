...
let dealPropertySchema = z.record(z.string(), z.any()).describe('Deal properties as key-value pairs (e.g., dealname, amount, dealstage, pipeline, closedate, hubspot_owner_id)');
...
export let createDeal = SlateTool.create(
  ...
  .input(z.object({
    ...
    })).optional().describe('Associations to create with the deal (e.g., contacts, companies)'),
  }))
  .output(dealOutputSchema)
  .handleInvocation(async (ctx) => {
    let client = new HubSpotClient(ctx.auth.token);
    let result = await client.createObject('deals', ctx.input.properties, ctx.input.associations);
...
export let getDeal = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    let client = new HubSpotClient(ctx.auth.token);
    let result = await client.getObject('deals', ctx.input.dealId, ctx.input.properties, ctx.input.associations);
...
export let listDeals = SlateTool.create(
  ...
  .input(z.object({
    limit: z.number().min(1).max(100).optional().describe('Number of deals to return (max 100)'),
  ...
  .handleInvocation(async (ctx) => {
    let client = new HubSpotClient(ctx.auth.token);
    let result = await client.listObjects('deals', ctx.input.limit || 10, ctx.input.after, ctx.input.properties);
