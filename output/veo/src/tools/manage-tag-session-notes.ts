...
export let manageTagSessionNotes = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'create', 'delete']).describe('Action: "list" to get notes, "create" to add a note, "delete" to remove one'),
    tagId: z.string().optional().describe('ID of the specific tag within the session (required for "create" and "delete")'),
    message: z.string().optional().describe('Note text (required for "create")'),
    parentCommentId: z.number().optional().describe('ID of the parent note for threaded replies (optional for "create")'),
    noteId: z.string().optional().describe('ID of the note to delete (required for "delete")'),
  }))
  .output(z.object({
    notes: z.array(z.record(z.string(), z.any())).optional().describe('List of notes with nested replies (when action is "list")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      await client.createTagSessionNote(ctx.input.tagSessionId, ctx.input.tagId, ctx.input.message, ctx.input.parentCommentId);
    ...
    await client.deleteTagSessionNote(ctx.input.tagSessionId, ctx.input.tagId, ctx.input.noteId);
