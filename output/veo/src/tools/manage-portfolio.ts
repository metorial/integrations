...
export let managePortfolio = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['get', 'create', 'update', 'delete']).describe('Action to perform on the portfolio'),
    portfolioId: z.string().optional().describe('ID of the portfolio (required for get, update, delete)'),
    name: z.string().optional().describe('Portfolio name (required for create, optional for update)'),
    description: z.string().optional().describe('Portfolio description'),
  }))
  .output(z.object({
    portfolio: z.record(z.string(), z.any()).optional().describe('Portfolio data (for get and create)'),
