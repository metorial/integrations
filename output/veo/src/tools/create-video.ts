...
export let createVideo = SlateTool.create(
  spec,
  {
    name: 'Create Video',
    key: 'create_video',
  ...
  .input(z.object({
    ...
    recordedStamp: z.string().describe('ISO 8601 timestamp of when the video was recorded (e.g. 2024-01-15T10:30:00Z)'),
    mimeType: z.string().optional().default('video/mp4').describe('MIME type of the video file to upload (e.g. video/mp4, video/quicktime)'),
  }))
  .output(z.object({
    videoId: z.string().describe('ID of the newly created video resource'),
    uploadUrl: z.string().optional().describe('Azure blob storage URL for uploading the video file'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    try {
      ...
      uploadUrl = typeof uploadToken === 'string' ? uploadToken : uploadToken?.url ?? uploadToken?.Url;
    } catch (e) {
