...
let signerSchema = z.object({
  ...
  signerType: z.enum(['Signer', 'InPersonSigner', 'Reviewer']).optional().describe('Type of signer. Defaults to Signer'),
  signerOrder: z.number().optional().describe('Signing order position (used with enableSigningOrder)'),
  ...
  authenticationCode: z.string().optional().describe('Access code the signer must enter to view the document'),
  enableEmailOTP: z.boolean().optional().describe('Require email OTP verification for this signer'),
});
...
export let sendDocument = SlateTool.create(
  ...
  .input(z.object({
    ...
    message: z.string().optional().describe('Message included in the signing notification email'),
    ...
    fileUrls: z.array(z.string()).optional().describe('Publicly accessible URLs to document files (PDF, Word, etc.)'),
    brandId: z.string().optional().describe('Brand ID for custom branding'),
    labels: z.array(z.string()).optional().describe('Tags/labels for categorizing the document'),
    expiryDays: z.number().optional().describe('Number of days before the document expires'),
    enableReassign: z.boolean().optional().describe('Allow signers to reassign the document'),
    enablePrintAndSign: z.boolean().optional().describe('Allow signers to print, sign physically, and upload'),
    enableSigningOrder: z.boolean().optional().describe('Enforce sequential signing order'),
    disableEmails: z.boolean().optional().describe('Suppress email notifications to signers'),
    disableExpiryAlert: z.boolean().optional().describe('Suppress expiry alert emails'),
    onBehalfOf: z.string().optional().describe('Send the document on behalf of this email address'),
    ...
    }).optional().describe('Automatic reminder configuration'),
  }))
  .output(z.object({
