...
export let downloadDocument = SlateTool.create(
  ...
  .input(z.object({
    ...
    downloadType: z.enum(['document', 'auditLog']).default('document').describe('What to download: the signed document PDF or the audit trail'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = ctx.input.downloadType === 'auditLog'
