...
export let manageDocument = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['revoke', 'remind', 'delete']).describe('The action to perform on the document'),
    revokeMessage: z.string().optional().describe('Reason for revoking (required when action is "revoke")'),
    reminderMessage: z.string().optional().describe('Custom message to include with the reminder'),
    onBehalfOf: z.string().optional().describe('Perform the action on behalf of this email address'),
  }))
  .output(z.object({
