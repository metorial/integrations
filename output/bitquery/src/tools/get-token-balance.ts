...
export let getTokenBalance = SlateTool.create(
  spec,
  {
...
Use this to check portfolio holdings, verify wallet balances, or monitor address token positions.`,
  ...
  .input(z.object({
    ...
    limit: z.number().min(1).max(1000).default(50).describe('Maximum number of token balances to return'),
  }))
  .output(z.object({
  ...
  ethereum(network: $network) {
    address(address: {is: $address}) {
      ...
      balances = raw.slice(0, limit).map((b: any) => ({
        tokenSymbol: b.currency?.symbol,
        tokenName: b.currency?.name,
  ...
  EVM(network: $network, dataset: combined) {
    BalanceUpdates(
      ...
      let data = await client.query(query, { network: blockchain, address: walletAddress, limit });
