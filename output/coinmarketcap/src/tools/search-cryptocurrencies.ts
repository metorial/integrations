...
let mapEntrySchema = z.object({
  ...
  firstHistoricalData: z.string().nullable().describe('Earliest available historical data timestamp'),
  ...
  }).nullable().describe('Token platform information if token-based'),
});

export let searchCryptocurrencies = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    symbol: z.string().optional().describe('Comma-separated symbols to look up (e.g., "BTC,ETH,SOL")'),
    listingStatus: z.enum(['active', 'inactive', 'untracked']).optional().describe('Filter by listing status. Default: active'),
  ...
  .output(z.object({
    cryptocurrencies: z.array(mapEntrySchema).describe('List of matched cryptocurrency entries'),
  }))
