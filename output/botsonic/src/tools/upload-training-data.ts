...
The uploaded data will be processed asynchronously. Use the "List Training Data" tool to check processing status.`,
  ...
  .input(z.object({
    ...
    uploadType: z.enum(['file', 'text', 'urls']).describe('The type of training data to upload'),
    uploadId: z.string().optional().describe('Unique identifier for the upload (required for file and text uploads)'),
    fileUrl: z.string().optional().describe('URL of the file to upload (for file upload type)'),
    ...
    title: z.string().optional().describe('Title for the text content (for text upload type)'),
    urls: z.array(z.string()).optional().describe('List of URLs to upload (for urls upload type)'),
    isSitemap: z.boolean().optional().describe('Whether the URLs are from a sitemap (for urls upload type)'),
    sitemapRoot: z.string().optional().describe('Root URL of the sitemap (for urls upload type)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.uploadType === 'file') {
      ...
      if (!ctx.input.urls || ctx.input.urls.length === 0) throw new Error('urls array is required for URL uploads');
