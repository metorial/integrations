...
export let listObjectVersionsTool = SlateTool.create(
  spec,
  {
...
Use **prefix** to filter versions for a specific object or folder.`,
  ...
  .input(z.object({
    ...
    prefix: z.string().optional().describe('Filter versions by key prefix (use exact key for a single object)'),
    ...
    versionIdMarker: z.string().optional().describe('Start listing after this version ID (for pagination, use with keyMarker)')
  }))
  .output(z.object({
