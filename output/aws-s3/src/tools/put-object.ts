...
export let putObjectTool = SlateTool.create(
  spec,
  {
    name: 'Put Object',
    key: 'put_object',
...
Supports optional settings for content type, storage class, encryption, access control, tags, and custom metadata.`,
  ...
  .input(z.object({
    ...
    storageClass: z.enum(['STANDARD', 'REDUCED_REDUNDANCY', 'STANDARD_IA', 'ONEZONE_IA', 'INTELLIGENT_TIERING', 'GLACIER', 'DEEP_ARCHIVE', 'GLACIER_IR']).optional().describe('Storage class for the object'),
    serverSideEncryption: z.enum(['AES256', 'aws:kms']).optional().describe('Server-side encryption algorithm'),
    acl: z.enum(['private', 'public-read', 'public-read-write', 'authenticated-read', 'bucket-owner-read', 'bucket-owner-full-control']).optional().describe('Canned ACL for the object'),
    tagging: z.string().optional().describe('URL-encoded tag set (e.g., "key1=value1&key2=value2")'),
    metadata: z.record(z.string(), z.string()).optional().describe('Custom metadata key-value pairs to attach to the object')
  }))
  .output(z.object({
    objectKey: z.string().describe('Key of the uploaded object'),
    eTag: z.string().describe('Entity tag (hash) of the uploaded object'),
    versionId: z.string().optional().describe('Version ID if versioning is enabled on the bucket'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.putObject(ctx.input.bucketName, ctx.input.objectKey, ctx.input.content, {
