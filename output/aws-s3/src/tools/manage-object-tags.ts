...
export let manageObjectTagsTool = SlateTool.create(
  spec,
  {
...
When setting tags, provide the **complete** tag set — existing tags are replaced entirely.`,
  ...
  .input(z.object({
    ...
    })).optional().describe('Tags to set (required for "set" action, replaces all existing tags)')
  }))
  .output(z.object({
    ...
    tags: z.array(z.object({
      key: z.string().describe('Tag key'),
      value: z.string().describe('Tag value')
    })).optional().describe('Current tags on the object (returned for "get" and "set" actions)'),
