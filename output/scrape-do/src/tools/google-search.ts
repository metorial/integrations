...
export let googleSearch = SlateTool.create(
  ...
  .input(z.object({
    ...
    gl: z.string().optional().describe('Country code for geo-location targeting (ISO 3166-1 alpha-2, e.g., "us", "gb")'),
    hl: z.string().optional().describe('Host language code (e.g., "en", "de", "fr")'),
    cr: z.string().optional().describe('Country restriction filter for strict geo-filtering'),
    lr: z.string().optional().describe('Language restriction filter (e.g., "lang_en")'),
    location: z.string().optional().describe('Precise location string for targeting (e.g., "New York, NY")'),
    uule: z.string().optional().describe('UULE-encoded location for precise geo-targeting'),
    start: z.number().optional().describe('Pagination offset (0-based index of first result)'),
    num: z.number().optional().describe('Number of results per page'),
    safe: z.string().optional().describe('SafeSearch filter level'),
    tbs: z.string().optional().describe('Time-based search filter (e.g., "qdr:d" for past day, "qdr:w" for past week, "qdr:m" for past month)'),
  ...
  .output(z.object({
    searchResults: z.any().describe('Structured JSON containing all search result types (organic, ads, knowledge graph, local pack, videos, etc.)'),
  }))
