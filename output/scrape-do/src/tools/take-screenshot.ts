...
Configure the viewport size, device type, and geo-targeting to capture location- or device-specific views.`,
  ...
  .input(z.object({
    ...
    screenshotType: z.enum(['viewport', 'full', 'selector']).optional().default('viewport').describe('Type of screenshot: "viewport" for visible area, "full" for full page, "selector" for a specific element'),
    selector: z.string().optional().describe('CSS selector for partial screenshot (only used when screenshotType is "selector")'),
    ...
    customWait: z.number().optional().describe('Additional wait time in milliseconds after page load'),
    waitSelector: z.string().optional().describe('CSS selector to wait for before capturing'),
