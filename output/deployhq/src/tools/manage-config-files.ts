...
let configFileSchema = z.object({
  ...
  })).optional().describe('Servers this config file is deployed to'),
});

export let listConfigFiles = SlateTool.create(
  spec,
  {
...
export let createConfigFile = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    projectPermalink: z.string().describe('The permalink (slug) of the project'),
    path: z.string().describe('File path relative to deployment root (e.g., config/database.yml)'),
    body: z.string().describe('File contents'),
    allServers: z.boolean().optional().describe('Upload to all servers (default: true)'),
    serverIdentifiers: z.array(z.string()).optional().describe('Specific server or server group identifiers to deploy to'),
  }))
...
export let updateConfigFile = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    serverIdentifiers: z.array(z.string()).optional().describe('Specific server or server group identifiers'),
  }))
  .output(configFileSchema)
  .handleInvocation(async (ctx) => {
    let client = new Client({
    ...
    let f = await client.updateConfigFile(ctx.input.projectPermalink, ctx.input.configFileIdentifier, data);
...
export let deleteConfigFile = SlateTool.create(
  spec,
  {
