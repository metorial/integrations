...
export let getDeployment = SlateTool.create(
  ...
  .output(z.object({
    ...
    status: z.string().describe('Deployment status (e.g., completed, failed, pending, running)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let d = await client.getDeployment(ctx.input.projectPermalink, ctx.input.deploymentIdentifier);
