...
export let manageRecipient = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['create', 'get', 'list', 'update', 'delete', 'add_discovered', 'archive_discovered', 'list_discovered']).describe('Action to perform'),
    recipientId: z.string().optional().describe('Recipient ID (required for get, update, delete, archive_discovered)'),
    ...
    recipientWarranties: z.string().optional().describe('Recipient data protection warranties/guarantees'),
    role: z.string().optional().describe('Recipient role'),
    discoveredRecipientIds: z.array(z.string()).optional().describe('Array of discovered recipient UUIDs (for add_discovered action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (action) {
      case 'create': {
        if (!ctx.input.name || !ctx.input.status || !ctx.input.dataStorageLocation || !ctx.input.recipientWarranties) {
          throw new Error('name, status, dataStorageLocation, and recipientWarranties are required for creating a recipient');
        }
      ...
      case 'update': {
        ...
        if (ctx.input.dataStorageLocation !== undefined) updatePayload.dataStorageLocation = ctx.input.dataStorageLocation;
        if (ctx.input.recipientWarranties !== undefined) updatePayload.recipientWarranties = ctx.input.recipientWarranties;
      ...
      case 'archive_discovered': {
        if (!recipientId) throw new Error('recipientId is required for archive_discovered action');
