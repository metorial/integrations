...
export let incomingEventTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Bot Event',
    key: 'bot_event',
    description: 'Triggers when new events are received by a bot, including messages, custom events, and system events.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let newEvents = lastSeenId
        ? events.filter((e) => e.id !== lastSeenId && new Date(e.createdAt as string) > new Date(ctx.state?.lastSeenAt as string || '1970-01-01'))
        : events;
