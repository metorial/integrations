...
export let markInvoiceAsPaid = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    paymentMethod: z.string().describe('Payment method (e.g. BankTransfer, CreditCard, Cash, Check)'),
    metadata: z.record(z.string(), z.any()).optional().describe('Custom metadata to store on the payment'),
  }))
  .output(z.object({
    invoiceId: z.number(),
    paid: z.boolean(),
