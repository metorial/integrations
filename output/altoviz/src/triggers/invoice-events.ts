...
export let invoiceEvents = SlateTrigger.create(
  ...
  .input(z.object({
    eventType: z.string().describe('Event type (InvoiceCreated, InvoiceUpdated, InvoiceDeleted)'),
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      return {
        type: eventTypeMap[ctx.input.eventType] || `invoice.${ctx.input.eventType.toLowerCase()}`,
