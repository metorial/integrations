...
export let getPodioAppInfo = SlateTool.create(
  spec,
  {
...
- Recent activity (recently modified item IDs) for an app.`,
  ...
  .input(z.object({
    action: z.enum(['get_app', 'list_apps', 'get_field_categories', 'recent_activity']).describe('The operation to perform'),
    appId: z.string().optional().describe('The Podio App ID (required for get_app, get_field_categories, recent_activity)'),
    spaceId: z.string().optional().describe('The Podio Space/Workspace ID (required for list_apps)'),
    fieldId: z.string().optional().describe('The field ID (required for get_field_categories)'),
  }))
  .output(z.object({
    app: z.any().optional().describe('The app payload (for get_app)'),
    apps: z.any().optional().describe('Array of apps (for list_apps)'),
    categories: z.any().optional().describe('Category field values (for get_field_categories)'),
    recentItemIds: z.any().optional().describe('Recently active item IDs (for recent_activity)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'get_field_categories') {
      if (!ctx.input.appId || !ctx.input.fieldId) throw new Error('appId and fieldId are required for get_field_categories');
