...
export let queryMysql = SlateTool.create(
  spec,
  {
    name: 'Query MySQL',
    key: 'query_mysql',
...
Optionally specify a connection name if multiple MySQL databases are configured.`,
  ...
  .input(z.object({
    queryType: z.enum(['query', 'array', 'command']).describe('Type of query: "query" for single row, "array" for multiple rows, "command" for non-SELECT'),
    sql: z.string().describe('SQL statement with ? placeholders for parameters'),
    params: z.array(z.any()).optional().describe('Array of parameter values matching the ? placeholders in order'),
    connectionName: z.string().optional().describe('Named MySQL connection (if multiple are configured)'),
  }))
  .output(z.object({
