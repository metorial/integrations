...
export let sendOauthApiRequest = SlateTool.create(
  spec,
  {
...
The service must be previously configured in your ProcFu account settings.`,
  ...
  .input(z.object({
    serviceName: z.string().describe('The OAuth API service name as configured in your ProcFu account'),
    ...
    headers: z.string().optional().describe('Comma-separated headers (defaults to "Content-type: application/json,Accept: application/json")'),
