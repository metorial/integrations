...
let recipientSchema = z.object({
  ...
  role: z.string().optional().describe('Recipient role in the document (must match a role defined in the template)'),
...
export let createDocument = SlateTool.create(
  ...
  .input(z.object({
    templateId: z.string().describe('UUID of the PandaDoc template to create the document from'),
    name: z.string().optional().describe('Document name. Overrides the template name if provided.'),
    recipients: z.array(recipientSchema).describe('List of document recipients (signers, approvers, CC)'),
    tokens: z.array(tokenSchema).optional().describe('Template tokens to populate with values'),
    fields: z.record(z.string(), z.any()).optional().describe('Fields to pre-fill, keyed by field name with their values'),
    metadata: z.record(z.string(), z.string()).optional().describe('Custom key-value metadata to attach to the document'),
    tags: z.array(z.string()).optional().describe('Tags to apply to the document'),
    folderUuid: z.string().optional().describe('UUID of the folder to place the document in'),
    ownerEmail: z.string().optional().describe('Email of the document owner'),
    detectTitleVariables: z.boolean().optional().describe('Whether to resolve variables in the document title'),
    pricingTables: z.array(z.any()).optional().describe('Pricing table data to populate in the document'),
    contentPlaceholders: z.array(z.object({
      blockId: z.string().describe('Content placeholder block ID from the template'),
      contentLibraryItems: z.array(z.object({
        contentLibraryItemId: z.string().describe('Content library item UUID'),
      })).describe('Content library items to insert'),
    })).optional().describe('Content placeholders to fill with content library items'),
  }))
  .output(z.object({
