...
let campaignOutputSchema = z.object({
  ...
  budgetAmount: z.number().optional().describe('Budget amount in lowest currency denomination'),
  ...
  isContinuous: z.boolean().optional().describe('Whether the campaign runs until manually paused'),
...
Supports A/B testing through multiple creative variations in the creatives field.`,
  ...
  .input(z.object({
    ...
    campaignId: z.number().optional().describe('Campaign ID (required for get, update, delete)'),
    title: z.string().optional().describe('Campaign name'),
    platform: z.enum(['facebook', 'google']).optional().describe('Ad platform'),
    status: z.enum(['active', 'inactive', 'draft', 'paused']).optional().describe('Campaign status'),
    targetType: z.enum(['app', 'conversion', 'dynamic_product', 'event', 'likes', 'link', 'page_messaging', 'post_engagement', 'video_views']).optional().describe('Campaign objective/target type'),
    ...
    creatives: z.record(z.string(), z.any()).optional().describe('Creative configuration (platform-specific). Each field accepts an array of variations for A/B testing.'),
    targeting: z.record(z.string(), z.any()).optional().describe('Campaign-level targeting settings'),
    ...
    instagramAccountId: z.string().optional().describe('Instagram account ID for the campaign'),
  ...
  .output(z.object({
    campaign: campaignOutputSchema.optional().describe('Campaign details (for create, get, update)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.externalAdAccountId) data.external_ad_account_id = ctx.input.externalAdAccountId;
