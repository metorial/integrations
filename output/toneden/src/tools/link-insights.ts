...
export let linkInsights = SlateTool.create(
  spec,
  {
...
Time-series data supports aggregation by time interval, geography, referrer, and service.`,
  ...
  .input(z.object({
    ...
    includeTimeSeries: z.boolean().optional().describe('Also retrieve time-series analytics data'),
    metric: z.enum(['clicks', 'clickthroughs', 'rsvps']).optional().describe('Metric for time-series data (required when includeTimeSeries is true)'),
    ...
    aggregateGeo: z.enum(['city', 'country', 'region']).optional().describe('Geographic aggregation level'),
  ...
  .output(z.object({
    ...
    timeSeries: z.array(z.record(z.string(), z.any())).optional().describe('Time-series data points'),
  }))
