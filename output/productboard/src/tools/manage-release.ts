...
export let listReleasesTool = SlateTool.create(
  spec,
  {
    name: 'List Releases',
    key: 'list_releases',
  ...
  .input(z.object({
    ...
    includeGroups: z.boolean().optional().describe('Also fetch release groups (default: false)'),
  }))
  .output(z.object({
    releases: z.array(z.record(z.string(), z.any())).describe('List of releases'),
    releaseGroups: z.array(z.record(z.string(), z.any())).optional().describe('List of release groups, if requested'),
...
export let createReleaseTool = SlateTool.create(
  spec,
  {
    name: 'Create Release',
    key: 'create_release',
  ...
  .input(z.object({
    ...
    releaseGroupId: z.string().optional().describe('ID of the release group to add this release to'),
  }))
  .output(z.object({
...
export let updateReleaseTool = SlateTool.create(
  spec,
  {
    name: 'Update Release',
    key: 'update_release',
...
export let deleteReleaseTool = SlateTool.create(
  spec,
  {
    name: 'Delete Release',
    key: 'delete_release',
...
export let assignFeatureToReleaseTool = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    let client = new Client({ token: ctx.auth.token });
    let assignment = await client.assignFeatureToRelease(ctx.input.featureId, ctx.input.releaseId);
