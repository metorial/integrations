...
export let triggerCall = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    country: z.string().optional().describe('Two-letter ISO country code for phone number formatting (e.g., US, GB)'),
    agents: z.array(agentSchema).optional().describe('Specific agents to route the call to, overriding the regular call queue'),
    customParams: z.record(z.string(), z.string()).optional().describe('Additional custom parameters to attach to the call (key-value pairs)'),
  }))
  .output(z.object({
