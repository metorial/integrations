...
export let getSpeedToLeadReports = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    maxCalls: z.number().optional().describe('Number of calls to return (default 20, max 500)'),
    page: z.number().optional().describe('Page number for pagination (default 1)'),
    searchString: z.string().optional().describe('Search across call IDs, agent names, URLs, comments, custom parameters, phone numbers'),
    status: z.enum(['NO_ANSWER', 'OPERATOR_ANSWERED', 'CLIENT_ANSWERED']).optional().describe('Filter by call status'),
    ...
    filterAnswerTimeFrom: z.number().optional().describe('Minimum agent answer time in seconds'),
    filterAnswerTimeTo: z.number().optional().describe('Maximum agent answer time in seconds'),
