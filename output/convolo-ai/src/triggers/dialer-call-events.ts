...
export let dialerCallEvents = SlateTrigger.create(
  spec,
  {
    name: 'Dialer Call Events',
    key: 'dialer_call_events',
    description: 'Triggers on Power Dialer call lifecycle events: callStarted, callRinging, callAnswered, callEnded, and webphoneSummary. Webhooks must be configured in the Brightcall Dialer dashboard. All event types are sent to the webhook endpoint and filtered by event type.',
  }
)
  .input(z.object({
    eventType: z.string().describe('Event type: callStarted, callRinging, callAnswered, callEnded, or webphoneSummary'),
  ...
  .output(z.object({
    ...
    callState: z.number().optional().describe('Call state: 0=unanswered, 1=answered, 2=failed'),
    ...
    crmContactFields: z.array(z.object({
      title: z.string().optional(),
      value: z.string().optional(),
      isWebsite: z.boolean().optional(),
    })).optional().describe('CRM contact data fields'),
    additionalData: z.any().optional().describe('Additional data from the webhook configuration'),
  }))
  .webhook({
    handleRequest: async (ctx) => {
      ...
      if (lastSegment && ['callStarted', 'callRinging', 'callAnswered', 'callEnded', 'webphoneSummary'].includes(lastSegment)) {
        eventType = lastSegment;
      }

      let callId = data.callId || data.call_id || data.id || data.uniqueId || `dialer-${Date.now()}`;
    ...
    handleEvent: async (ctx) => {
      ...
      let crmContactFields: Array<{ title?: string; value?: string; isWebsite?: boolean }> | undefined;
      ...
      return {
        ...
        output: {
          ...
          aiSummary: payload.convoloAiRecordingSummary || payload.aiSummary || payload.ai_summary,
          aiTranscript: payload.convoloAiTranscript || payload.aiTranscript || payload.transcript,
