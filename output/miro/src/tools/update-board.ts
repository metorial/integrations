...
export let updateBoard = SlateTool.create(
  spec,
  {
    name: 'Update Board',
    key: 'update_board',
  ...
  .input(z.object({
    ...
    }).optional().describe('Updated sharing policy'),
    ...
    }).optional().describe('Updated permissions policy')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.sharingPolicy || ctx.input.permissionsPolicy) {
      ...
      if (ctx.input.permissionsPolicy) updateParams.policy.permissionsPolicy = ctx.input.permissionsPolicy;
    }
