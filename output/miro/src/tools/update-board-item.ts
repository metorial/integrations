...
export let updateBoardItem = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    itemType: z.enum(['sticky_note', 'card', 'text', 'shape', 'generic']).describe('Type of the item to update'),
    content: z.string().optional().describe('Updated text content (sticky notes, text, shapes)'),
    ...
    switch (ctx.input.itemType) {
      ...
      case 'generic':
        item = await client.updateItemPosition(ctx.input.boardId, ctx.input.itemId, {
          position: (ctx.input.x !== undefined || ctx.input.y !== undefined) ? {
            x: ctx.input.x,
            y: ctx.input.y
          } : undefined,
