...
export let getNotificationPolicies = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({}))
  .output(z.object({
    receiver: z.string().optional().describe('Default receiver (contact point) for unmatched alerts'),
    groupBy: z.array(z.string()).optional().describe('Labels used to group alerts'),
    groupWait: z.string().optional().describe('Time to wait before sending the first notification for a new group'),
    groupInterval: z.string().optional().describe('Minimum time between notifications for a group'),
    repeatInterval: z.string().optional().describe('Minimum time before re-sending a notification'),
    routes: z.array(z.any()).optional().describe('Child routing rules with matchers and overrides'),
  }))
...
export let updateNotificationPolicies = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    receiver: z.string().describe('Default receiver (contact point name) for unmatched alerts'),
    groupBy: z.array(z.string()).optional().describe('Labels used to group alerts'),
    groupWait: z.string().optional().describe('Time to wait before first notification (e.g. "30s", "1m")'),
    groupInterval: z.string().optional().describe('Minimum time between notifications for a group (e.g. "5m")'),
    repeatInterval: z.string().optional().describe('Minimum time before re-sending (e.g. "4h")'),
    routes: z.array(z.any()).optional().describe('Child routing rules with matchers, receivers, and nested routes'),
  }))
  .output(z.object({
