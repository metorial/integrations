...
let alertSchema = z.object({
  ...
  values: z.record(z.string(), z.any()).optional().describe('Metric values that triggered the alert'),
  generatorURL: z.string().optional().describe('URL to the alert rule in Grafana'),
  fingerprint: z.string().optional().describe('Unique fingerprint for the alert based on labels'),
...
export let alertNotification = SlateTrigger.create(
  spec,
  {
    name: 'Alert Notification',
    key: 'alert_notification',
    description: 'Receives Grafana alert notifications via webhook. Triggers when alert rules change state (firing or resolved). Configure a webhook contact point in Grafana pointing to the provided webhook URL.',
  }
)
  .input(z.object({
