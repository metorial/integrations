...
let selectorSchema = z.object({
  ...
  state: z.enum(['attached', 'visible', 'hidden']).optional().describe('Element state to wait for. Defaults to "visible".'),
});
...
let actionSchema = z.object({
  ...
  delay: z.number().optional().describe('Delay in milliseconds between keystrokes (for "type" action)'),
  url: z.string().optional().describe('URL pattern to wait for (for waitForRequest/waitForResponse actions)'),
  source: z.string().optional().describe('JavaScript code to evaluate (for "evaluate" action)'),
  onError: z.enum(['return', 'ignore']).optional().describe('Error handling: "return" stops and returns error, "ignore" continues'),
});

let networkCaptureSchema = z.object({
  filterType: z.enum(['url']).optional().describe('Filter type for network capture'),
  matchType: z.enum(['contains', 'startsWith', 'endsWith', 'exact', 'regex']).optional().describe('How to match the filter value'),
...
export let renderPage = SlateTool.create(
  spec,
  {
    name: 'Render Page',
    key: 'render_page',
...
Use this when pages rely on JavaScript to load content. For static/server-rendered pages, the **Fetch Page** tool is faster and cheaper.`,
    ...
    constraints: [
      'Browser execution time is limited to 60 seconds total.',
    ],
    tags: {
      readOnly: true,
      destructive: false,
  ...
  .input(z.object({
    ...
    actions: z.array(actionSchema).optional().describe('Browser actions to perform before capturing output'),
    networkCapture: z.array(networkCaptureSchema).optional().describe('Rules for capturing background network requests'),
    javascript: z.boolean().optional().describe('Enable or disable JavaScript execution. Defaults to true.'),
    includeIframes: z.boolean().optional().describe('Embed iframe content in the returned HTML'),
    referer: z.string().optional().describe('Referer header value for the browser request'),
    includeScreenshot: z.boolean().optional().describe('Also capture a screenshot of the rendered page'),
    fullPageScreenshot: z.boolean().optional().describe('Capture full page screenshot instead of viewport only'),
    geolocation: z.string().optional().describe('Country code for request origin (e.g. "US", "GB")'),
    ...
    includeResponseCookies: z.boolean().optional().describe('Include response cookies in the result'),
    sessionId: z.string().optional().describe('Session ID (UUID) for maintaining state across requests'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Results of browser actions'),
    networkCapture: z.array(z.object({
      url: z.string().describe('Captured request URL'),
      method: z.string().optional().describe('HTTP method'),
      statusCode: z.number().optional().describe('Response status code'),
      httpResponseBody: z.string().optional().describe('Base64-encoded captured response body'),
      httpResponseHeaders: z.array(headerSchema).optional().describe('Captured response headers'),
    })).optional().describe('Captured network requests'),
