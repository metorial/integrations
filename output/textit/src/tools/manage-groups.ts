...
export let manageGroups = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'Groups with active campaigns or triggers cannot be deleted.',
    ],
    tags: {
      destructive: true,
    },
  ...
  .input(z.object({
    ...
    groupUuid: z.string().optional().describe('UUID of the group (required for update/delete)'),
    name: z.string().optional().describe('Name of the group (required for create and update)'),
  }))
  .output(z.object({
