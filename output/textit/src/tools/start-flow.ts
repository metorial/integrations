...
export let startFlow = SlateTool.create(
  spec,
  {
    name: 'Start Flow',
    key: 'start_flow',
  ...
  .input(z.object({
    ...
    contactUuids: z.array(z.string()).optional().describe('Contact UUIDs to start in the flow (max 100)'),
    groupUuids: z.array(z.string()).optional().describe('Group UUIDs whose contacts will be started in the flow (max 100)'),
    urns: z.array(z.string()).optional().describe('URNs to start in the flow (max 100)'),
    restartParticipants: z.boolean().optional().describe('Whether to restart contacts already in this flow (default: true)'),
    excludeActive: z.boolean().optional().describe('Whether to exclude contacts active in other flows (default: false)'),
    params: z.record(z.string(), z.any()).optional().describe('Extra parameters accessible in the flow via @trigger.params (max 10KB)'),
  }))
  .output(z.object({
