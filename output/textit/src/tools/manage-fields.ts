...
export let manageFields = SlateTool.create(
  ...
  .input(z.object({
    ...
    fieldKey: z.string().optional().describe('Key of the field to update (required for update, auto-generated on create)'),
    name: z.string().describe('Display name of the field'),
    type: z.enum(['text', 'number', 'datetime', 'state', 'district', 'ward']).describe('Data type of the field'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'create') {
      ...
      field = await client.updateField(ctx.input.fieldKey!, { name: ctx.input.name, type: ctx.input.type });
    }
