...
export let sendBroadcast = SlateTool.create(
  ...
  .input(z.object({
    text: z.record(z.string(), z.string()).describe('Message text translations keyed by ISO-639-3 language code (e.g., {"eng": "Hello"})'),
    urns: z.array(z.string()).optional().describe('Recipient URNs (max 100)'),
    contactUuids: z.array(z.string()).optional().describe('Recipient contact UUIDs (max 100)'),
    groupUuids: z.array(z.string()).optional().describe('Recipient group UUIDs'),
    baseLanguage: z.string().optional().describe('Default translation language code (ISO-639-3)'),
  }))
  .output(z.object({
