...
export let bulkContactAction = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    contactUuids: z.array(z.string()).describe('List of contact UUIDs to perform the action on'),
    action: z.enum(['add', 'remove', 'block', 'unblock', 'archive', 'restore', 'interrupt', 'delete']).describe('The bulk action to perform'),
    groupUuid: z.string().optional().describe('Group UUID (required for "add" and "remove" actions)'),
  }))
  .output(z.object({
