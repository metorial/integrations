...
export let manageCampaigns = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'update']).describe('Whether to create a new campaign or update an existing one'),
    campaignUuid: z.string().optional().describe('UUID of the campaign to update (required for update)'),
