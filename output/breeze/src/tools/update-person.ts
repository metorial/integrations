...
export let updatePerson = SlateTool.create(
  ...
  .input(z.object({
    ...
    fields: z.record(z.string(), z.any()).describe('Profile field values to update, as a map of field ID to new value'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let person = await client.updatePerson(ctx.input.personId, JSON.stringify(ctx.input.fields));
