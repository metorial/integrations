...
export let listPeople = SlateTool.create(
  ...
  .input(z.object({
    ...
    details: z.boolean().optional().describe('Set to true to return full person details instead of just ID and name'),
    tagIds: z.array(z.string()).optional().describe('Filter people by tag IDs'),
    status: z.string().optional().describe('Filter people by status (e.g., "active", "inactive", "archived")'),
  }))
  .output(z.object({
