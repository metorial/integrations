...
export let manageVolunteers = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    roleIds: z.array(z.string()).optional().describe('Role IDs to assign (required for update action, replaces existing roles)'),
  }))
  .output(z.object({
    volunteers: z.array(z.any()).optional().describe('List of volunteers (for list action)'),
    success: z.boolean().optional().describe('Whether the operation succeeded (for add/remove/update)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'update':
        ...
        result = await client.updateVolunteer(ctx.input.instanceId, ctx.input.personId, ctx.input.roleIds);
