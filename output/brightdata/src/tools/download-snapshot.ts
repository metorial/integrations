...
export let downloadSnapshot = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    format: z.enum(['json', 'ndjson', 'csv']).optional().describe('Desired output format for the data.'),
    compress: z.boolean().optional().describe('Whether to compress the response data.'),
  }))
  .output(z.object({
    records: z.array(z.record(z.string(), z.unknown())).describe('Array of scraped data records.'),
  }))
