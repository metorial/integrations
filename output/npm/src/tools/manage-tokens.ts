...
export let manageTokens = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'delete']).describe('"list" to view tokens, "create" to make a new token, "delete" to revoke a token'),
    ...
    expiresIn: z.string().optional().describe('Expiration for new token (e.g. "7d", "30d", "90d", or ISO date)'),
    permission: z.enum(['read-only', 'read-write']).optional().describe('Permission level for new token (default "read-only")'),
    cidrWhitelist: z.array(z.string()).optional().describe('CIDR IP ranges allowed to use the token'),
    ...
    orgs: z.array(z.string()).optional().describe('Restrict token to specific organizations'),
    bypass2fa: z.boolean().optional().describe('Allow token to bypass 2FA for automated workflows'),
    otp: z.string().optional().describe('One-time password for 2FA verification'),
    tokenKey: z.string().optional().describe('Token key/ID to delete (required for "delete" action)'),
  }))
  .output(z.object({
    tokens: z.array(tokenInfoSchema).optional().describe('List of tokens (for "list" action)'),
    ...
    }).optional().describe('Newly created token details (for "create" action)'),
