...
let versionSchema = z.object({
  ...
  devDependencies: z.record(z.string(), z.string()).optional().describe('Development dependencies'),
  peerDependencies: z.record(z.string(), z.string()).optional().describe('Peer dependencies'),
  engines: z.record(z.string(), z.string()).optional().describe('Node/npm engine requirements'),
  ...
  }).optional().describe('Distribution information'),
}).passthrough();

export let getPackage = SlateTool.create(
  spec,
  {
    name: 'Get Package',
    key: 'get_package',
...
Optionally fetch a specific version or use abbreviated mode for smaller response payloads with only installation-relevant data.`,
  ...
  .input(z.object({
    packageName: z.string().describe('Full package name, including scope if applicable (e.g. "react" or "@angular/core")'),
    version: z.string().optional().describe('Specific semver version to retrieve (e.g. "18.2.0"). Omit to get the full package document.'),
    abbreviated: z.boolean().optional().describe('If true, returns only installation-relevant metadata (much smaller payload). Ignored when a specific version is requested.'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Package maintainers'),
    distTags: z.record(z.string(), z.string()).optional().describe('Distribution tags mapping tag names to versions'),
    keywords: z.array(z.string()).optional().describe('Package keywords'),
    versions: z.array(versionSchema).optional().describe('Available versions (present when fetching full metadata or a specific version)'),
    requestedVersion: versionSchema.optional().describe('The specific version requested, if a version was specified'),
    time: z.record(z.string(), z.string()).optional().describe('Timestamps for each published version'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new NpmRegistryClient({ token: ctx.auth.token || undefined });

    if (ctx.input.version) {
      let versionData = await client.getPackageVersion(ctx.input.packageName, ctx.input.version);
      return {
        output: {
          packageName: versionData.name,
          description: versionData.description,
          latestVersion: versionData.version,
          license: typeof versionData.license === 'string' ? versionData.license : versionData.license?.type,
    ...
    return {
      output: {
        ...
        latestVersion,
        license: typeof metadata.license === 'string' ? metadata.license : metadata.license?.type,
