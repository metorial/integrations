...
export let getDownloads = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    packageName: z.string().optional().describe('Package name. Omit for aggregate counts across all npm packages. Required for per-version mode.'),
    period: z.string().optional().describe('Time period: "last-day", "last-week", "last-month", or a date range "YYYY-MM-DD:YYYY-MM-DD". Defaults to "last-week". Ignored for per-version mode.'),
    mode: z.enum(['point', 'range', 'per-version']).optional().describe('Download count mode. "point" returns total count, "range" returns day-by-day breakdown, "per-version" returns downloads by version for the last 7 days. Defaults to "point".'),
    packages: z.array(z.string()).optional().describe('List of package names for bulk query (up to 128). Overrides packageName if provided.'),
  }))
  .output(z.object({
    totalDownloads: z.number().optional().describe('Total downloads in the period (point mode)'),
    start: z.string().optional().describe('Start date of the period'),
    end: z.string().optional().describe('End date of the period'),
    dailyDownloads: z.array(z.object({
      day: z.string().describe('Date in YYYY-MM-DD format'),
      downloads: z.number().describe('Download count for the day'),
    })).optional().describe('Day-by-day download breakdown (range mode)'),
    versionDownloads: z.record(z.string(), z.number()).optional().describe('Downloads per version for the last 7 days (per-version mode)'),
    bulkResults: z.record(z.string(), z.object({
      totalDownloads: z.number().describe('Total downloads'),
      start: z.string().optional(),
      end: z.string().optional(),
    })).optional().describe('Per-package results for bulk queries'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.packages && ctx.input.packages.length > 0) {
      if (mode === 'point') {
        ...
        let bulkResults: Record<string, { totalDownloads: number; start?: string; end?: string }> = {};
        ...
        for (let [pkg, stats] of Object.entries(data || {})) {
          let s = stats as any;
          if (s) {
            let total = (s.downloads || []).reduce((sum: number, d: any) => sum + (d.downloads || 0), 0);
            bulkResults[pkg] = {
