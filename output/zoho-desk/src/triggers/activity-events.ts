...
export let activityEvents = SlateTrigger.create(
  spec,
  {
    ...
    key: 'activity_events',
    description: 'Triggered when calls, calendar events, time entries, or departments are created, updated, or deleted.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    resourceId: z.string().describe('ID of the affected resource'),
    resourceType: z.string().describe('Type of resource (call, event, time_entry, department)'),
