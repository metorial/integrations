...
Returns model metadata including name, description, category, and status.`,
  ...
  .input(z.object({
    query: z.string().optional().describe('Free-text search query to filter models by name, description, or category'),
    category: z.string().optional().describe('Filter models by category, e.g. "text-to-image", "image-to-video"'),
    endpointId: z.union([z.string(), z.array(z.string())]).optional().describe('Specific endpoint ID(s) to look up, e.g. "fal-ai/flux/dev"'),
  ...
  .output(z.object({
    models: z.array(z.object({
      ...
      category: z.string().optional().describe('Model category such as text-to-image, image-to-video'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: `Found **${models.length}** model(s).${result.hasMore ? ' More results available.' : ''}${models.slice(0, 5).map(m => `\n- **${m.displayName || m.endpointId}** (${m.category || 'unknown category'})`).join('')}`,
    };
  }).build();
