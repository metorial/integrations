...
Provide an audio URL and receive a full transcription with optional metadata.`,
  ...
  .input(z.object({
    modelId: z.string().optional().describe('Model endpoint ID, defaults to "fal-ai/whisper"'),
    audioUrl: z.string().describe('URL of the audio file to transcribe'),
    language: z.string().optional().describe('Language code (e.g. "en", "fr") to hint the transcription language'),
    task: z.enum(['transcribe', 'translate']).optional().describe('Task to perform: transcribe in source language or translate to English'),
    diarize: z.boolean().optional().describe('Enable speaker diarization to identify different speakers'),
    chunkLevel: z.enum(['segment', 'word']).optional().describe('Level of timestamp chunking for the output'),
    numSpeakers: z.number().optional().describe('Expected number of speakers for diarization'),
  }))
  .output(z.object({
    ...
    chunks: z.array(z.object({
      ...
      timestamp: z.array(z.number()).optional().describe('Start and end timestamps in seconds'),
      speaker: z.string().optional().describe('Speaker label if diarization is enabled'),
    })).optional().describe('Transcription chunks with timestamps and optional speaker labels'),
    timings: z.record(z.string(), z.any()).optional().describe('Timing information for the transcription process'),
  }))
