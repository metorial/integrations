...
Runs synchronously and returns the generated video URL.`,
  ...
  .input(z.object({
    modelId: z.string().describe('Model endpoint ID, e.g. "fal-ai/kling-video/v1/standard/text-to-video", "fal-ai/ltx-video"'),
    ...
    imageUrl: z.string().optional().describe('Input image URL for image-to-video generation'),
    videoUrl: z.string().optional().describe('Input video URL for video-to-video transformation'),
    aspectRatio: z.string().optional().describe('Aspect ratio for the output video, e.g. "16:9", "9:16", "1:1"'),
    ...
    additionalParams: z.record(z.string(), z.any()).optional().describe('Additional model-specific parameters'),
  }))
  .output(z.object({
