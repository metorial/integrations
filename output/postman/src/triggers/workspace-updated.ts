...
export let workspaceUpdatedTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Workspace Updated',
    key: 'workspace_updated',
    description: 'Triggers when a Postman workspace changes. Detects new workspaces and modifications to existing ones by polling the workspaces list.',
  }
)
  .input(z.object({
  ...
  .polling({
    ...
    pollEvents: async (ctx) => {
      ...
      let inputs: Array<{ workspaceId: string; workspaceName: string; type?: string; visibility?: string }> = [];
