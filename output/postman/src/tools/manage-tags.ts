...
export let manageTagsTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    entityType: z.enum(['collection', 'workspace', 'api']).optional().describe('Entity type (required for get and update)'),
    entityId: z.string().optional().describe('Entity ID (required for get and update)'),
    tagSlugs: z.array(z.string()).optional().describe('Tag slugs to set (required for update)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'get') {
      if (!entityType || !entityId) throw new Error('entityType and entityId are required for get.');
    ...
    if (action === 'update') {
      if (!entityType || !entityId) throw new Error('entityType and entityId are required for update.');
    ...
    let result = await client.getEntitiesByTag(tagSlug, entityType ? { entityType } : undefined);
