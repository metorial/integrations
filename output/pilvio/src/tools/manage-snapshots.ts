...
export let manageSnapshots = SlateTool.create(
  ...
  .input(z.object({
    ...
    replicaUuid: z.string().optional().describe('UUID of the snapshot/replica (required for "delete" and "restore")'),
    replicaType: z.enum(['snapshot', 'backup']).optional().describe('Filter by type when listing (default: snapshot)'),
  }))
  .output(z.object({
    replicas: z.array(replicaSchema).optional().describe('List of replicas (for "list" action)'),
