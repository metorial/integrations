...
export let createVm = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Disk size is specified as a string (e.g., "20" for 20 GB).',
    ],
  }
)
  .input(z.object({
    name: z.string().describe('Name for the virtual machine'),
    osName: z.string().optional().describe('Operating system name (e.g., "Ubuntu"). Omit for custom/empty disk.'),
    osVersion: z.string().optional().describe('Operating system version (e.g., "22.04"). Omit for custom/empty disk.'),
    ...
    billingAccountId: z.number().optional().describe('Billing account ID (optional if using restricted token)'),
    ...
    sourceVmUuid: z.string().optional().describe('Source VM UUID when creating from a snapshot/backup'),
    sourceReplicaUuid: z.string().optional().describe('Source replica (snapshot/backup) UUID'),
    bootDiskUuid: z.string().optional().describe('Existing unattached disk UUID to use as boot disk'),
    reservePublicIp: z.boolean().optional().describe('Whether to reserve a public IPv4 (default: true)'),
