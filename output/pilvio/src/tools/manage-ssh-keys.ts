...
export let manageSshKeys = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    switch (action) {
      ...
      case 'create': {
        if (!ctx.input.name || !ctx.input.publicKey) throw new Error('name and publicKey are required for create action');
        let key = await client.createSshKey({ name: ctx.input.name, publicKey: ctx.input.publicKey });
        return {
      ...
      case 'rename': {
        if (!ctx.input.sshKeyUuid || !ctx.input.name) throw new Error('sshKeyUuid and name are required for rename action');
