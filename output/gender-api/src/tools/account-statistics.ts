...
export let accountStatistics = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({}))
  .output(z.object({
    isLimitReached: z.boolean().describe('Whether the account credit limit has been reached'),
    ...
    lastMonthDate: z.string().describe('Year-month of the last billing period (e.g. "2024-03")'),
    lastMonthCreditsUsed: z.number().describe('Credits consumed in the previous billing period'),
  }))
