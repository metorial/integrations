...
let localizationSchema = z.object({
  country: z.string().optional().describe('ISO 3166-1 alpha-2 country code (e.g. "US", "DE", "IT") to improve prediction accuracy'),
  locale: z.string().optional().describe('Browser locale string (e.g. "en_US", "de_DE") used to infer country'),
  ip: z.string().optional().describe('IPv4 or IPv6 address used for geolocation-based country inference'),
});
...
export let detectGender = SlateTool.create(
  ...
  .input(z.object({
    firstName: z.string().optional().describe('A single first name to classify (e.g. "Elizabeth")'),
    fullName: z.string().optional().describe('A full name to split and classify (e.g. "Theresa Miller")'),
    email: z.string().optional().describe('An email address from which to extract and classify the name'),
    strict: z.boolean().optional().describe('When using fullName, if true the API will not guess the last name when uncertain'),
  ...
  .output(z.object({
    ...
    lastName: z.string().optional().describe('The extracted last name (only for full name or email queries)'),
