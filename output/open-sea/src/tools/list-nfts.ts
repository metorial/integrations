...
export let listNfts = SlateTool.create(
  spec,
  {
    name: 'List NFTs',
    key: 'list_nfts',
  ...
  .input(z.object({
    ...
    chain: z.string().optional().describe('Blockchain (required for contract/owner lookups). Defaults to config chain or ethereum.'),
    filterCollection: z.string().optional().describe('When listing by owner, optionally filter to a specific collection slug'),
  ...
  .output(z.object({
    ...
    nextCursor: z.string().nullable().describe('Cursor for fetching the next page, null if no more results'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.collectionSlug) {
      ...
      throw new Error('Provide at least one of collectionSlug, contractAddress, or ownerAddress.');
    }
