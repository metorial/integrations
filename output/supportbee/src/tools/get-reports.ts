...
export let getReports = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Report data covers a maximum 30-day window.'
    ]
  }
)
  .input(z.object({
    reportType: z.enum(['avg_first_response_time', 'tickets_count', 'replies_count']).describe('Type of report to retrieve'),
    user: z.string().optional().describe('Filter by user ID'),
    team: z.string().optional().describe('Filter by team ID'),
    label: z.string().optional().describe('Filter by label name'),
    since: z.string().optional().describe('Start date for the report period (ISO 8601 format)')
  }))
  .output(z.object({
