...
let elevationPointSchema = z.object({
  ...
  resolutionMeters: z.number().optional().describe('Maximum distance between data points from which the elevation was interpolated, in meters'),
});

export let getElevationTool = SlateTool.create(
  spec,
  {
    name: 'Get Elevation',
    key: 'get_elevation',
  ...
  .input(z.object({
    ...
    samples: z.number().optional().describe('If provided, treats locations as a path and samples this many evenly-spaced points along it'),
  }))
  .output(z.object({
