...
export let computeRouteMatrixTool = SlateTool.create(
  spec,
  {
    ...
    instructions: [
      'Each waypoint needs either latitude/longitude or a placeId.',
    ],
    tags: {
      readOnly: true,
    },
  ...
  .input(z.object({
    ...
    travelMode: z.enum(['DRIVE', 'BICYCLE', 'WALK', 'TWO_WHEELER']).optional().describe('Travel mode (default: DRIVE)'),
    routingPreference: z.enum(['TRAFFIC_UNAWARE', 'TRAFFIC_AWARE', 'TRAFFIC_AWARE_OPTIMAL']).optional().describe('Traffic routing preference'),
  }))
  .output(z.object({
    elements: z.array(matrixElementSchema).describe('Matrix elements with duration and distance for each origin-destination pair'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let elements: Array<Record<string, unknown>> = Array.isArray(response) ? response : [response];
    ...
    let mappedElements = elements.map((el) => ({
      ...
      status: (el.status as Record<string, unknown>)?.code as string | undefined || el.status as string | undefined,
