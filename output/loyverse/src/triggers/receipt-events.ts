...
export let receiptEvents = SlateTrigger.create(
  spec,
  {
    name: 'Receipt Events',
    key: 'receipt_events',
    description: 'Triggers when a receipt (sale) is created or updated. Useful for syncing sales data to external accounting or analytics systems.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    refundForReceiptNumber: z.string().nullable().optional().describe('Original receipt if refund'),
