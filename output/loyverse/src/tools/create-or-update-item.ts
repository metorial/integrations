...
export let createOrUpdateItem = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Receipts created via API cannot have more than one payment type.',
    ],
  }
)
  .input(z.object({
    ...
    option2Name: z.string().nullable().optional().describe('Second option name (e.g., Color)'),
    option3Name: z.string().nullable().optional().describe('Third option name'),
    variants: z.array(z.object({
      variantId: z.string().optional().describe('Variant ID to update; omit for new variant'),
      ...
      })).optional().describe('Store-specific pricing'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.primarySupplierId !== undefined) body.primary_supplier_id = ctx.input.primarySupplierId;
