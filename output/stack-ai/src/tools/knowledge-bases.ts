...
export let listKnowledgeBases = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    pageSize: z.number().optional().describe('Number of results per page (max 1000, default 50)'),
  }))
  .output(z.object({
    knowledgeBases: z.array(z.record(z.string(), z.unknown())).describe('List of knowledge base objects'),
...
export let getKnowledgeBase = SlateTool.create(
  spec,
  {
...
export let createKnowledgeBase = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    connectionId: z.string().optional().describe('ID of an existing connection to use as a data source'),
    connectionSourceIds: z.array(z.string()).optional().describe('Specific source IDs within the connection to index'),
    indexingParams: z.record(z.string(), z.unknown()).optional().describe('Custom indexing parameters (chunking strategy, embedding model, etc.)'),
  }))
  .output(z.object({
    knowledgeBase: z.record(z.string(), z.unknown()).describe('The newly created knowledge base'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({
      token: ctx.auth.token,
...
export let updateKnowledgeBase = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    indexingParams: z.record(z.string(), z.unknown()).optional().describe('Updated indexing parameters'),
  }))
  .output(z.object({
...
export let deleteKnowledgeBase = SlateTool.create(
  spec,
  {
...
export let syncKnowledgeBase = SlateTool.create(
  spec,
  {
...
export let listKnowledgeBaseResources = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    cursor: z.string().optional().describe('Pagination cursor from a previous response'),
    pageSize: z.number().optional().describe('Number of results per page (max 100, default 50)'),
  }))
  .output(z.object({
    resources: z.array(z.record(z.string(), z.unknown())).describe('List of resource objects in the knowledge base'),
    hasMore: z.boolean().optional().describe('Whether more results are available'),
    cursor: z.string().optional().describe('Cursor for fetching the next page'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.listKnowledgeBaseResources(ctx.input.knowledgeBaseId, ctx.input.cursor, ctx.input.pageSize);
...
export let deleteKnowledgeBaseResource = SlateTool.create(
  spec,
  {
