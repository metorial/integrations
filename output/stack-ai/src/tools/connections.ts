...
export let listConnections = SlateTool.create(
  ...
  .output(z.object({
    connections: z.array(z.record(z.string(), z.unknown())).describe('List of connection objects'),
  }))
...
export let getConnection = SlateTool.create(
  ...
  .output(z.object({
    ...
    health: z.record(z.string(), z.unknown()).optional().describe('Health check result, if requested'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({
      token: ctx.auth.token,
...
export let browseConnectionResources = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    query: z.string().optional().describe('Search query to filter resources. If omitted, lists top-level resources'),
  }))
  .output(z.object({
    resources: z.array(z.record(z.string(), z.unknown())).describe('List of resources available through the connection'),
  }))
  .handleInvocation(async (ctx) => {
    let client = new Client({
      token: ctx.auth.token,
    ...
    if (ctx.input.query) {
      resources = await client.searchConnectionResources(ctx.input.connectionId, ctx.input.query);
    } else {
