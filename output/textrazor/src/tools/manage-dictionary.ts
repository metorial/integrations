...
let dictionarySchema = z.object({
  ...
  matchType: z.string().optional().describe('Matching mode: "stem" for relaxed matching or "token" for literal matching'),
  caseInsensitive: z.boolean().optional().describe('Whether matching is case-insensitive'),
  language: z.string().optional().describe('ISO-639-2 language code or "any" for all languages'),
});

export let manageDictionary = SlateTool.create(
  spec,
  {
...
Use the **action** field to specify the operation. When creating, configure matching behavior with matchType, caseInsensitive, and language options.`,
  ...
  .input(z.object({
    ...
    dictionaryId: z.string().optional().describe('Dictionary ID (required for create, get, delete)'),
    matchType: z.enum(['stem', 'token']).optional().describe('Matching mode for create: "stem" (relaxed) or "token" (literal)'),
    caseInsensitive: z.boolean().optional().describe('Enable case-insensitive matching for create'),
    language: z.string().optional().describe('ISO-639-2 language code or "any" for create'),
  }))
  .output(z.object({
    dictionaries: z.array(dictionarySchema).optional().describe('List of dictionaries (for list action)'),
    dictionary: dictionarySchema.optional().describe('Single dictionary (for get/create actions)'),
  }))
