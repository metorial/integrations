...
export let domainIntelligence = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    includeTechnologies: z.boolean().optional().describe('Include technologies used by the domain (default: true)'),
    includeSimilar: z.boolean().optional().describe('Include similar domains (default: true)'),
    includeStatus: z.boolean().optional().describe('Include webmail/disposable status (default: true)'),
  }))
  .output(z.object({
    domain: z.string().describe('Analyzed domain'),
    totalEmails: z.number().nullable().optional().describe('Total email count for the domain'),
    departmentBreakdown: z.record(z.string(), z.number()).optional().describe('Email count by department'),
    seniorityBreakdown: z.record(z.string(), z.number()).optional().describe('Email count by seniority level'),
    isWebmail: z.boolean().nullable().optional().describe('Whether domain is a webmail provider'),
    isDisposable: z.boolean().nullable().optional().describe('Whether domain is disposable'),
    technologies: z.array(technologySchema).optional().describe('Technologies used by the domain'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let technologies = (Array.isArray(techData) ? techData : techData.technologies || []).map((t: any) => ({
      slug: t.slug,
      name: t.name,
      icon: t.icon,
      website: t.website,
