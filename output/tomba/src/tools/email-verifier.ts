...
export let emailVerifier = SlateTool.create(
  spec,
  {
    name: 'Email Verifier',
    key: 'email_verifier',
  ...
  .output(z.object({
    ...
    score: z.number().nullable().optional().describe('Verification confidence score (0-100)'),
    ...
    acceptAll: z.boolean().nullable().optional().describe('Whether domain accepts all emails'),
    greylisted: z.boolean().nullable().optional().describe('Whether the email is greylisted'),
    block: z.boolean().nullable().optional().describe('Whether the email is blocked'),
    gibberish: z.boolean().nullable().optional().describe('Whether the email appears to be gibberish'),
    disposable: z.boolean().nullable().optional().describe('Whether the email is from a disposable domain'),
    webmail: z.boolean().nullable().optional().describe('Whether the email is a webmail address'),
    regex: z.boolean().nullable().optional().describe('Whether the email passes regex validation'),
