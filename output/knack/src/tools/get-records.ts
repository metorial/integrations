...
let filterRuleSchema = z.object({
  ...
  operator: z.enum([
    ...
    'is before', 'is after',
    'is during the current', 'is during the previous', 'is during the next',
    'is before the previous', 'is after the next',
...
export let getRecords = SlateTool.create(
  ...
  .input(z.object({
    ...
    rowsPerPage: z.number().optional().describe('Number of records per page (defaults to 25, max 1000)'),
    ...
    }).optional().describe('Filter configuration for narrowing results'),
  }))
  .output(z.object({
    records: z.array(z.record(z.string(), z.any())).describe('Array of record objects with field key-value pairs'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.sceneKey && ctx.input.viewKey) {
      ctx.info(`Fetching records via view-based access: ${ctx.input.sceneKey}/${ctx.input.viewKey}`);
