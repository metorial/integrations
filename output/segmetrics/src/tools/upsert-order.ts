...
export let upsertOrder = SlateTool.create(
  spec,
  {
...
All monetary amounts are specified in **cents** (e.g., 4999 = $49.99).`,
    ...
    constraints: [
      'All monetary amounts must be in cents.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    ...
    isRefunded: z.number().optional().describe('Whether the order has been refunded (0 or 1).'),
    items: z.array(z.object({
      name: z.string().describe('Name of the product/item.'),
      productId: z.string().optional().describe('Product identifier. Auto-created if it doesn\'t exist.'),
      amount: z.number().describe('Amount for this line item in cents.'),
      totalPaid: z.number().optional().describe('Total paid for this line item in cents.'),
