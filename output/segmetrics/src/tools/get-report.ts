...
The report ID corresponds to a saved report created in the SegMetrics UI.`,
  ...
  .input(z.object({
    reportType: z.enum(['leads', 'revenue', 'ads', 'subscriptions']).describe('Type of report to retrieve.'),
    ...
    scale: z.enum(['day', 'week', 'month']).optional().describe('Time scale for the graph data.'),
  }))
  .output(z.object({
    report: z.unknown().describe('Full report data including KPIs, graph data, and table breakdowns.'),
  }))
