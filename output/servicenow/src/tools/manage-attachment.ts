...
export let manageAttachment = SlateTool.create(
  ...
  .input(z.object({
    ...
    tableName: z.string().optional().describe('Table name to filter or attach to (e.g. "incident")'),
    recordId: z.string().optional().describe('sys_id of the record to list/attach files to'),
    attachmentId: z.string().optional().describe('sys_id of the attachment to delete (required for delete action)'),
    fileName: z.string().optional().describe('Name of the file to upload (required for upload action)'),
    contentType: z.string().optional().default('text/plain').describe('MIME type of the file (e.g. "text/plain", "application/pdf")'),
    content: z.string().optional().describe('File content as text/base64 string (required for upload action)'),
    limit: z.number().optional().default(20).describe('Maximum attachments to list'),
  }))
  .output(z.object({
    attachments: z.array(z.record(z.string(), z.any())).optional().describe('List of attachments (for list action)'),
    attachment: z.record(z.string(), z.any()).optional().describe('The uploaded attachment record (for upload action)'),
    deleted: z.boolean().optional().describe('Whether the attachment was deleted (for delete action)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'upload') {
      if (!ctx.input.tableName || !ctx.input.recordId || !ctx.input.fileName || !ctx.input.content) {
        throw new Error('tableName, recordId, fileName, and content are required for upload action');
      }
