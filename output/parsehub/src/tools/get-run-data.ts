...
export let getRunData = SlateTool.create(
  spec,
  {
    name: 'Get Run Data',
    key: 'get_run_data',
  ...
  .input(z.object({
    ...
    projectToken: z.string().optional().describe('Token of the project to get the last ready run data from (use with useLastReady)'),
    useLastReady: z.boolean().optional().describe('When true, retrieves data from the most recent completed run of the project instead of a specific run'),
  }))
  .output(z.object({
