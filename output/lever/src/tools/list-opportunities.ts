...
export let listOpportunitiesTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    origin: z.enum(['applied', 'sourced', 'referred', 'university', 'agency', 'internal']).optional().describe('Filter by opportunity origin'),
    postingId: z.string().optional().describe('Filter by posting ID'),
    stageId: z.string().optional().describe('Filter by pipeline stage ID'),
    archiveStatus: z.enum(['archived', 'active']).optional().describe('Filter by archive status. Defaults to active.'),
    createdAtStart: z.string().optional().describe('Filter by creation date start (ISO 8601 timestamp)'),
    createdAtEnd: z.string().optional().describe('Filter by creation date end (ISO 8601 timestamp)'),
    updatedAtStart: z.string().optional().describe('Filter by update date start (ISO 8601 timestamp)'),
    updatedAtEnd: z.string().optional().describe('Filter by update date end (ISO 8601 timestamp)'),
    ...
    expand: z.array(z.enum(['applications', 'stage', 'owner', 'followers', 'sourcedBy', 'contact'])).optional().describe('Related objects to include in the response'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.createdAtStart) params.created_at_start = new Date(ctx.input.createdAtStart).getTime();
    if (ctx.input.createdAtEnd) params.created_at_end = new Date(ctx.input.createdAtEnd).getTime();
    if (ctx.input.updatedAtStart) params.updated_at_start = new Date(ctx.input.updatedAtStart).getTime();
    if (ctx.input.updatedAtEnd) params.updated_at_end = new Date(ctx.input.updatedAtEnd).getTime();
