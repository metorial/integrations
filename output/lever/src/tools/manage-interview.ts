...
export let manageInterviewTool = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Only externally managed panels can be updated or deleted via the API.',
    ],
  }
)
  .input(z.object({
    ...
    interviewId: z.string().optional().describe('Interview ID (required for updating or deleting)'),
    ...
    feedbackReminderFrequency: z.enum(['none', 'daily', 'frequent']).optional().describe('Feedback reminder frequency'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'update') {
      ...
      if (ctx.input.interviewerIds) updateData.interviewers = ctx.input.interviewerIds.map(id => ({ userId: id }));
      if (ctx.input.feedbackTemplateId) updateData.feedbackTemplate = ctx.input.feedbackTemplateId;
      if (ctx.input.feedbackReminderFrequency) updateData.feedbackReminderFrequency = ctx.input.feedbackReminderFrequency;
    ...
    // Create
    if (!ctx.input.opportunityId) throw new Error('opportunityId is required for create action');
    ...
    if (ctx.input.interviewerIds) createData.interviewers = ctx.input.interviewerIds.map(id => ({ userId: id }));
    if (ctx.input.feedbackTemplateId) createData.feedbackTemplate = ctx.input.feedbackTemplateId;
    if (ctx.input.feedbackReminderFrequency) createData.feedbackReminderFrequency = ctx.input.feedbackReminderFrequency;
    if (ctx.input.timezone) createData.timezone = ctx.input.timezone;

    let result = await client.createInterview(ctx.input.opportunityId, ctx.input.panelId, createData);
    return {
      output: {
        interviewId: result.data.id,
        interview: result.data,
