...
export let opportunityEventsTrigger = SlateTrigger.create(
  spec,
  {
    ...
    description: 'Triggers when an opportunity is created (application), hired, stage changed, archived/unarchived, or deleted in Lever.',
  }
)
  .input(z.object({
  ...
  .output(z.object({
    ...
    applicationId: z.string().optional().describe('Application ID (for application created events)'),
