...
export let createDataSource = SlateTool.create(
  ...
  .input(z.object({
    accountId: z.number().describe('ID of the Databox account to associate this data source with'),
    title: z.string().describe('Human-readable name for the data source'),
    timezone: z.string().optional().describe('IANA timezone string (e.g. "UTC", "America/New_York"). Defaults to "UTC"'),
  }))
  .output(z.object({
