...
export let ingestData = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum **100 records** per ingestion request.',
    ],
  }
)
  .input(z.object({
    datasetId: z.string().describe('UUID of the dataset to ingest data into'),
    records: z.array(z.record(z.string(), z.unknown())).describe('Array of data records to ingest. Each record is a JSON object with column names as keys'),
  }))
  .output(z.object({
