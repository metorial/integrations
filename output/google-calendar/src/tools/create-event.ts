...
let eventDateTimeSchema = z.object({
  dateTime: z.string().optional().describe('RFC3339 timestamp (e.g. "2024-01-15T09:00:00-05:00"). Use for timed events.'),
  date: z.string().optional().describe('Date in YYYY-MM-DD format. Use for all-day events instead of dateTime.'),
  timeZone: z.string().optional().describe('IANA time zone (e.g. "America/New_York"). Defaults to the calendar\'s time zone.')
}).describe('Event start/end time. Provide either dateTime (timed events) or date (all-day events).');
...
Use **"primary"** as the calendarId to create events on the user's primary calendar.`,
  ...
  .input(z.object({
    ...
    recurrence: z.array(z.string()).optional().describe('Recurrence rules in RRULE format (e.g. ["RRULE:FREQ=WEEKLY;COUNT=10"])'),
    reminders: z.object({
    ...
    colorId: z.string().optional().describe('Color ID for the event (use get_colors tool to see available IDs)'),
    visibility: z.enum(['default', 'public', 'private', 'confidential']).optional().describe('Visibility of the event'),
    transparency: z.enum(['opaque', 'transparent']).optional().describe('"opaque" means busy, "transparent" means available'),
    guestsCanModify: z.boolean().optional().describe('Whether guests can modify the event'),
    guestsCanInviteOthers: z.boolean().optional().describe('Whether guests can invite other guests'),
    guestsCanSeeOtherGuests: z.boolean().optional().describe('Whether guests can see other guests'),
    addGoogleMeet: z.boolean().optional().describe('If true, automatically creates a Google Meet conference link'),
    sendUpdates: z.enum(['all', 'externalOnly', 'none']).optional().describe('Whether to send notifications to attendees. Defaults to none.')
  }))
  .output(z.object({
    ...
    hangoutLink: z.string().optional().describe('Google Meet link if conference was created'),
