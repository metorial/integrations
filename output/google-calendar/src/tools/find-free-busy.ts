...
export let findFreeBusy = SlateTool.create(
  spec,
  {
    name: 'Find Free/Busy',
    key: 'find_free_busy',
  ...
  .input(z.object({
    ...
    calendarIds: z.array(z.string()).describe('List of calendar IDs to check availability for'),
    timeZone: z.string().optional().describe('IANA time zone for the query (e.g. "America/New_York")')
  }))
  .output(z.object({
    ...
    calendars: z.record(z.string(), z.object({
      ...
      errors: z.array(z.object({
        domain: z.string(),
        reason: z.string()
      })).optional().describe('Any errors for this calendar')
