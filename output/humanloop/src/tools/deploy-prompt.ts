...
export let deployPrompt = SlateTool.create(
  spec,
  {
    name: 'Deploy Prompt',
    key: 'deploy_prompt',
  ...
  .input(z.object({
    ...
    environmentId: z.string().optional().describe('Environment ID (required for deploy/undeploy)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'deploy') {
      if (!ctx.input.environmentId) throw new Error('environmentId is required for deploy action');
      if (!ctx.input.versionId) throw new Error('versionId is required for deploy action');
      let result = await client.deployPromptVersion(ctx.input.promptId, ctx.input.environmentId, ctx.input.versionId);
      return {
    ...
    if (ctx.input.action === 'undeploy') {
      if (!ctx.input.environmentId) throw new Error('environmentId is required for undeploy action');
