...
let datapointSchema = z.object({
  inputs: z.record(z.string(), z.any()).optional().describe('Input variables for the datapoint'),
  messages: z.array(z.object({
    role: z.string().describe('Role of the message sender'),
    content: z.string().describe('Content of the message'),
  })).optional().describe('Conversation messages for the datapoint'),
  target: z.record(z.string(), z.any()).optional().describe('Expected target output for the datapoint'),
});
...
export let manageDataset = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'get', 'list', 'delete', 'list_datapoints']).describe('Action to perform'),
    datasetId: z.string().optional().describe('Dataset ID (required for get, update, delete, list_datapoints)'),
    path: z.string().optional().describe('Path for the dataset (e.g. "folder/my-dataset"). Used for create.'),
    datapointAction: z.enum(['set', 'add', 'remove']).optional().describe('How to handle datapoints: "set" replaces all, "add" appends, "remove" deletes'),
    ...
    includeDatapoints: z.boolean().optional().describe('Include datapoints in the get response'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'list_datapoints') {
      if (!ctx.input.datasetId) throw new Error('datasetId is required for list_datapoints action');
