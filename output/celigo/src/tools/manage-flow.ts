...
For "create" and "update", provide the flow configuration in **flowData**. For "enable", "disable", "run", "clone", and "delete", only the **flowId** is needed.`,
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'enable', 'disable', 'run', 'clone', 'delete']).describe('The operation to perform on the flow'),
    flowId: z.string().optional().describe('ID of the flow (required for all actions except "create")'),
    flowData: z.record(z.string(), z.any()).optional().describe('Flow configuration data (required for "create" and "update")')
  }))
  .output(z.object({
