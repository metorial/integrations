...
export let invokeRpc = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    args: z.record(z.string(), z.any()).optional().describe('Arguments to pass to the function'),
    schema: z.string().optional().describe('Database schema containing the function (defaults to public)'),
  }))
  .output(z.object({
