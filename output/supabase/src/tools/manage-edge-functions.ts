...
export let manageEdgeFunctions = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    functionSlug: z.string().optional().describe('Function slug (required for get, update, delete)'),
    ...
    body: z.string().optional().describe('Function source code (required for create, optional for update)'),
    verifyJwt: z.boolean().optional().describe('Whether to verify JWT tokens (default: true)'),
  }))
  .output(z.object({
    ...
    })).optional().describe('List of functions (for list action)'),
    functionDetails: z.object({
      ...
      createdAt: z.string().optional().describe('Creation timestamp'),
      updatedAt: z.string().optional().describe('Last update timestamp'),
    }).optional().describe('Function details (for get, create, update actions)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update') {
      if (!ctx.input.functionSlug) throw new Error('functionSlug is required for update action');
