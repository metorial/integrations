...
export let forwardGeocode = SlateTool.create(
  ...
  .input(z.object({
    query: z.string().max(50).describe('Address or place name to geocode (max 50 characters)'),
    countryCode: z.string().length(2).optional().describe('ISO 3166-1 alpha-2 country code to restrict results (e.g., "US", "GB", "DE")'),
  }))
  .output(z.object({
