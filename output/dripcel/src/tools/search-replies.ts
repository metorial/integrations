...
export let searchReplies = SlateTool.create(
  spec,
  {
    name: 'Search Replies',
    key: 'search_replies',
  ...
  .input(z.object({
    campaignId: z.union([z.string(), z.array(z.string())]).optional().describe('Filter by campaign ID(s)'),
    msisdn: z.union([z.string(), z.array(z.string())]).optional().describe('Filter by contact cell number(s)'),
    ...
    ]).optional().describe('Filter by reply classification'),
    receivedAfter: z.string().optional().describe('Filter replies received on or after this date (ISO 8601)'),
    receivedBefore: z.string().optional().describe('Filter replies received on or before this date (ISO 8601)'),
    replyId: z.union([z.string(), z.array(z.string())]).optional().describe('Filter by specific reply ID(s)')
  }))
  .output(z.object({
