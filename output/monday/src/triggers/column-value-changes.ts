...
export let columnValueChangesTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Column Value Changes',
    key: 'column_value_changes',
    description: 'Fires when any column value changes on an item or sub-item on a board. Includes the column ID, previous value, and new value.',
  },
)
  .input(z.object({
  ...
  .webhook({
    ...
    handleRequest: async (ctx) => {
      ...
      let itemId = event.pulseId ? String(event.pulseId) : (event.itemId ? String(event.itemId) : null);
    ...
    handleEvent: async (ctx) => {
      let isSubitem = ctx.input.eventType === 'change_subitem_column_value' || !!ctx.input.parentItemId;
