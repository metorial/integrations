...
export let listBoardsTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    boardKind: z.enum(['public', 'private', 'share']).optional().describe('Filter by board type'),
    state: z.enum(['active', 'all', 'archived', 'deleted']).optional().describe('Filter by board state'),
    limit: z.number().optional().describe('Maximum number of boards to return (default: 25)'),
