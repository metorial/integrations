...
- **User**: \`{ "id": userId, "username": "username" }\``,
  ...
  .input(z.object({
    ...
    fields: z.record(z.string(), z.any()).describe('Fields to update as a JSON object where keys are field IDs and values match the field data type'),
    fullUpdate: z.boolean().optional().default(false).describe('If true, performs a full update replacing the entire record. Defaults to false (partial update).')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.fullUpdate) {
      record = await client.updateRecordFull(ctx.input.listId, ctx.input.recordId, ctx.input.fields);
    } else {
      record = await client.updateRecord(ctx.input.listId, ctx.input.recordId, ctx.input.fields);
    }
