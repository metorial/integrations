...
let contactSchema = z.object({
  ...
  customFields: z.record(z.string(), z.string()).optional().describe('Custom fields with numeric keys (1-6)'),
});
...
export let listContacts = SlateTool.create(
  ...
  .input(z.object({
    ...
    page: z.number().optional().describe('Page number'),
    perPage: z.number().optional().describe('Results per page'),
    status: z.enum(['active', 'optout', 'bounced']).optional().describe('Filter by contact status'),
    search: z.string().optional().describe('Search term'),
    searchBy: z.enum(['email', 'first_name', 'last_name']).optional().describe('Field to search by'),
...
export let createContactsBulk = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    updateExisting: z.boolean().optional().describe('If true, updates existing contacts that match by email'),
  }))
  .output(z.object({
