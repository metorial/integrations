...
export let createCollector = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    type: z.enum(['weblink', 'email', 'sms', 'popup_invitation', 'popup_survey', 'embedded_survey']).describe('Type of collector'),
    name: z.string().optional().describe('Name for the collector'),
    thankYouMessage: z.string().optional().describe('Custom thank-you message shown after completion'),
    closeDate: z.string().optional().describe('Date to close the collector (YYYY-MM-DDTHH:MM:SS)'),
    redirectUrl: z.string().optional().describe('URL to redirect respondents after survey completion'),
    allowMultipleResponses: z.boolean().optional().describe('Whether to allow multiple responses from same respondent'),
    anonymousType: z.enum(['not_anonymous', 'partially_anonymous', 'fully_anonymous']).optional().describe('Level of response anonymity'),
...
export let updateCollector = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    anonymousType: z.enum(['not_anonymous', 'partially_anonymous', 'fully_anonymous']).optional().describe('Anonymity level'),
...
export let listCollectors = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    sortBy: z.enum(['id', 'date_modified', 'type', 'status', 'name']).optional().describe('Sort field'),
...
export let deleteCollector = SlateTool.create(
  spec,
  {
