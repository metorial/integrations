...
export let listContacts = SlateTool.create(
  ...
  .input(z.object({
    search: z.string().optional().describe('Free-text search across contact name, company, and VAT number.'),
    ...
    ordering: z.string().optional().describe('Sort field. Prefix with "-" for descending. E.g. "display_name", "-created".'),
    page: z.number().optional().describe('Page number for pagination (starts at 1).'),
    pageSize: z.number().optional().describe('Number of results per page (max 250, default 100).'),
    modifiedAfter: z.string().optional().describe('Only return contacts modified after this date (ISO 8601 or YYYY-MM-DD).'),
  }))
  .output(z.object({
