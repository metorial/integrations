...
let lineItemSchema = z.object({
  ...
  quantity: z.string().optional().describe('Quantity (as string, e.g. "1", "2.5"). Defaults to "1".'),
  unitTotal: z.string().optional().describe('Unit price including tax. Used when calculatorMode is "total".'),
  unitValue: z.string().optional().describe('Unit price excluding tax. Used when calculatorMode is "initial".'),
  unit: z.string().optional().describe('Unit of measurement (e.g. "hrs", "pcs").'),
  productId: z.string().optional().describe('Optional product/service ID. Auto-fills title, description, unit, and value from the product.'),
  taxes: z.array(z.string()).optional().describe('Array of tax IDs to apply to this line item.'),
});
...
export let createInvoice = SlateTool.create(
  ...
  .input(z.object({
    ...
    documentTypeId: z.string().describe('Document type ID (determines numbering and format).'),
    ...
    draft: z.boolean().optional().describe('Whether to create as draft (true) or issue immediately (false). Defaults to true.'),
    calculatorMode: z.enum(['total', 'initial']).optional().describe('"total" = unitTotal is tax-inclusive; "initial" = unitValue is tax-exclusive. Defaults to "total".'),
    currencyCode: z.string().optional().describe('Currency code (e.g. "EUR", "USD"). Defaults to client currency.'),
    ...
    paidOnReceipt: z.string().optional().describe('Amount paid immediately upon receipt (as string).'),
    paymentMethod: z.string().optional().describe('Payment method code: "1"=domestic bank, "2"=foreign bank, "3"=cash, "4"=cheque, "6"=web banking, "7"=POS.'),
