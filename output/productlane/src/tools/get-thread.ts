...
export let getThread = SlateTool.create(spec, {
  ...
  .input(z.object({
    ...
    includeConversation: z.boolean().optional().describe('Include the full conversation history'),
  }))
  .output(z.object({
    ...
    conversation: z.array(z.any()).optional().describe('Full conversation history (when requested)'),
