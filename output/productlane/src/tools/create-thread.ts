...
export let createThread = SlateTool.create(spec, {
  ...
  .input(z.object({
    ...
    painLevel: z.enum(['UNKNOWN', 'LOW', 'MEDIUM', 'HIGH']).describe('Severity/pain level of the feedback'),
    ...
    state: z.enum(['NEW', 'PROCESSED', 'COMPLETED', 'SNOOZED', 'UNSNOOZED']).optional().describe('Initial thread state (defaults to NEW)'),
    origin: z.string().optional().describe('Source of the feedback (e.g. API, SLACK, EMAIL, PORTAL)'),
