...
export let generateMultiFormat = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    templateFormatNames: z.array(z.string()).optional()
      .describe('Array of format names to generate. Omit to generate all formats defined in the design.'),
    elements: z.record(z.string(), z.record(z.string(), z.any())).optional()
      .describe('Element overrides keyed by layer name. Each value is an object with element properties.'),
    imageFileType: z.enum(['png', 'jpeg', 'webp', 'avif', 'pdf', 'html5']).optional()
      .describe('Override default output file type.'),
    callbackUrl: z.string().optional()
      .describe('Webhook URL to receive the NEW_BANNER_BATCH event when generation completes.'),
    html5: z.object({
      clickTag: z.string().optional().describe('Click tag URL or macro for the HTML5 banner'),
      pageTitle: z.string().optional().describe('Page title for the HTML5 banner'),
      adNetwork: z.enum(['default', 'google-ads', 'google-marketing', 'adform', 'amazon-ads', 'adroll']).optional()
        .describe('Target ad network for HTML5 generation'),
      repeat: z.number().optional().describe('Animation repeat count. -1 for infinite, 0 for once.'),
      includeBackupImage: z.boolean().optional().describe('Include a static backup image in the HTML5 package'),
    }).optional().describe('HTML5 banner-specific options. Only used when imageFileType is "html5".'),
  }))
  .output(z.object({
