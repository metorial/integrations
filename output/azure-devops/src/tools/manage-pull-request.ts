...
export let managePullRequestTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['list', 'get', 'create', 'update', 'add_reviewer', 'add_comment']).describe('Action to perform'),
    pullRequestId: z.number().optional().describe('Pull request ID (required for get, update, add_reviewer, add_comment)'),

    // Create fields
    sourceRefName: z.string().optional().describe('Source branch ref (e.g. "refs/heads/feature"). Required for create.'),
    targetRefName: z.string().optional().describe('Target branch ref (e.g. "refs/heads/main"). Required for create.'),
    title: z.string().optional().describe('PR title (required for create, optional for update)'),
    ...
    // Update fields
    status: z.enum(['active', 'abandoned', 'completed']).optional().describe('Set PR status (for update)'),
    autoComplete: z.boolean().optional().describe('Enable auto-complete (for update)'),
    autoCompleteUserId: z.string().optional().describe('User ID to set as auto-complete initiator'),

    // Add reviewer
    reviewerId: z.string().optional().describe('Identity ID of the reviewer (for add_reviewer)'),
    vote: z.number().optional().describe('Vote: 10=approved, 5=approved with suggestions, 0=no vote, -5=waiting for author, -10=rejected (for add_reviewer)'),

    // Add comment
    commentContent: z.string().optional().describe('Comment text in markdown (for add_comment)'),

    // List filters
    listStatus: z.enum(['active', 'abandoned', 'completed', 'all']).optional().describe('Filter PRs by status (for list)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'get') {
      ...
      let pr = await client.getPullRequest(project, ctx.input.repositoryId, ctx.input.pullRequestId);
      return {
    ...
    if (ctx.input.action === 'update') {
      ...
      let pr = await client.updatePullRequest(project, ctx.input.repositoryId, ctx.input.pullRequestId, updateData);
      return {
        output: { pullRequest: mapPR(pr) },
    ...
    if (ctx.input.action === 'add_reviewer') {
      ...
      await client.createPullRequestReviewer(
        project, ctx.input.repositoryId, ctx.input.pullRequestId,
        ctx.input.reviewerId, ctx.input.vote,
      );
      let pr = await client.getPullRequest(project, ctx.input.repositoryId, ctx.input.pullRequestId);
      return {
        output: { pullRequest: mapPR(pr) },
