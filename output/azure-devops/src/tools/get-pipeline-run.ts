...
export let getPipelineRunTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    runId: z.number().optional().describe('Specific run ID to get details for. If omitted, lists recent runs.'),
    top: z.number().optional().describe('Max runs to return when listing (default 10)'),
    includeTimeline: z.boolean().optional().describe('Include build timeline (stages/jobs/steps) when fetching a specific run'),
  }))
  .output(z.object({
    run: z.object({
      runId: z.number(),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.listPipelineRuns(project, ctx.input.pipelineId, ctx.input.top || 10);
