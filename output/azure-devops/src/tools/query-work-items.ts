...
export let queryWorkItemsTool = SlateTool.create(
  spec,
  {
...
Use WIQL for complex queries, or provide simple filters (type, state, assignedTo, areaPath) to auto-generate a query.`,
  ...
  .input(z.object({
    ...
    wiql: z.string().optional().describe('Full WIQL query string. If provided, all filter params are ignored.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (!wiql) {
      ...
      if (ctx.input.workItemType) conditions.push(`[System.WorkItemType] = '${ctx.input.workItemType}'`);
      if (ctx.input.state) conditions.push(`[System.State] = '${ctx.input.state}'`);
      if (ctx.input.assignedTo) conditions.push(`[System.AssignedTo] = '${ctx.input.assignedTo}'`);
      if (ctx.input.areaPath) conditions.push(`[System.AreaPath] UNDER '${ctx.input.areaPath}'`);
