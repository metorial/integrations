...
Use the \`sort\` parameter to order results; prefix with \`-\` for descending.`,
  ...
  .input(z.object({
    ...
    where: z.string().optional().describe('Filter condition, e.g. (Status,eq,Active)~and(Age,gt,25)'),
    sort: z.string().optional().describe('Sort by field name. Prefix with - for descending, e.g. -CreatedAt'),
    fields: z.string().optional().describe('Comma-separated field names to include in the response'),
    limit: z.number().optional().describe('Maximum number of records to return (default 25, max 1000)'),
