...
Use this for modifying page metadata and property values. To modify page content (blocks), use the block management tools instead.`,
  ...
  .input(z.object({
    ...
    icon: z.record(z.string(), z.any()).nullable().optional().describe('New page icon, or null to remove'),
    cover: z.record(z.string(), z.any()).nullable().optional().describe('New page cover image, or null to remove'),
    archived: z.boolean().optional().describe('Set to true to archive the page, false to unarchive'),
    isLocked: z.boolean().optional().describe('Set to true to lock the page, false to unlock'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.archived !== undefined) changes.push(ctx.input.archived ? 'archived' : 'unarchived');
    if (ctx.input.isLocked !== undefined) changes.push(ctx.input.isLocked ? 'locked' : 'unlocked');
