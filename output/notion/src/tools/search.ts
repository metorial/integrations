...
Returns matching pages and databases with their metadata. Best suited for finding resources by name rather than exhaustive enumeration.`,
  ...
  .input(z.object({
    query: z.string().optional().describe('Search term to filter by title. Omit to list all accessible resources.'),
    filterType: z.enum(['page', 'database']).optional().describe('Restrict results to pages or databases only'),
    sortDirection: z.enum(['ascending', 'descending']).optional().default('descending').describe('Sort direction by last edited time'),
  ...
  .output(z.object({
    results: z.array(z.record(z.string(), z.any())).describe('Array of matching page and database objects'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let filter = ctx.input.filterType
      ? { property: 'object' as const, value: ctx.input.filterType === 'database' ? 'database' : 'page' }
      : undefined;
