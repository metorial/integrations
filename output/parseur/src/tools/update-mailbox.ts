...
export let updateMailbox = SlateTool.create(
  ...
  .input(z.object({
    ...
    aiEngine: z.enum(['DISABLED', 'GCP_AI_1', 'GCP_AI_2', 'GCP_AI_2_5']).optional().describe('AI engine to use'),
    processAttachments: z.boolean().optional().describe('Process email attachments'),
    retentionPolicy: z.number().nullable().optional().describe('Document retention in days (null to remove)'),
    ...
    splitPage: z.number().nullable().optional().describe('Split documents at this page count (null to disable)'),
