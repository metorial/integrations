...
export let listDocuments = SlateTool.create(
  ...
  .input(z.object({
    ...
    ordering: z.string().optional().describe('Sort field, prefix with - for descending (e.g. -created)'),
    status: z.string().optional().describe('Filter by status (e.g. PARSEDOK, INCOMING, PROGRESS)'),
    receivedAfter: z.string().optional().describe('Filter documents received after this date (ISO 8601)'),
    receivedBefore: z.string().optional().describe('Filter documents received before this date (ISO 8601)'),
    timezone: z.string().optional().describe('Timezone for date filters (e.g. America/New_York)'),
  ...
  .output(z.object({
    ...
    documents: z.array(z.object({
      ...
      parsedResult: z.record(z.string(), z.any()).nullable().describe('Parsed/extracted data (if withResult was true and document is processed)'),
    })),
  }))
