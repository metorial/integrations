...
export let createMailbox = SlateTool.create(
  ...
  .input(z.object({
    name: z.string().optional().describe('Name for the new mailbox. If omitted, a random name is generated.'),
    aiEngine: z.enum(['DISABLED', 'GCP_AI_1', 'GCP_AI_2', 'GCP_AI_2_5']).optional().describe('AI engine to use. Defaults to GCP_AI_2.'),
    templateSlug: z.string().optional().describe('Ready-made template slug (e.g. invoices, leads, travel, resume-cv, food-delivery, real-estate)'),
    ...
    allowedExtensions: z.array(z.string()).optional().describe('Restrict accepted file types (e.g. ["pdf", "docx"])'),
  }))
  .output(z.object({
