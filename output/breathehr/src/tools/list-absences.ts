...
export let listAbsences = SlateTool.create(
  spec,
  {
    name: 'List Absences',
    key: 'list_absences',
  ...
  .input(z.object({
    ...
    startDate: z.string().optional().describe('Filter absences starting from this date (format: YYYY-MM-DD)'),
    endDate: z.string().optional().describe('Filter absences ending before this date (format: YYYY-MM-DD)'),
    excludeCancelledAbsences: z.boolean().optional().describe('Exclude cancelled absence requests'),
