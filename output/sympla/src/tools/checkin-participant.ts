...
export let checkinParticipantTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    ticketId: z.number().optional().describe('Participant ticket ID (use this or ticketNumber)'),
    ticketNumber: z.string().optional().describe('Ticket number printed on the ticket (use this or ticketId)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let identifier = ctx.input.ticketId ? `ticket ID ${ctx.input.ticketId}` : `ticket number ${ctx.input.ticketNumber}`;
    return {
      output: {
        success: true,
      },
