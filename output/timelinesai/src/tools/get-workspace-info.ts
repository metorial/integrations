...
export let getWorkspaceInfo = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    includeQuotas: z.boolean().optional().describe('Also fetch workspace usage quotas (seats, messaging, API calls)'),
  }))
  .output(z.object({
    ...
    quotas: z.object({
      ...
      }).optional().describe('Seat quota'),
      messagingQuota: z.object({
        total: z.number().describe('Total messaging credits'),
        used: z.number().describe('Used messaging credits'),
      }).optional().describe('Messaging quota'),
      apiCallsQuota: z.object({
        total: z.number().describe('Total API calls'),
        used: z.number().describe('Used API calls'),
      }).optional().describe('API calls quota'),
    }).optional().describe('Workspace usage quotas'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.includeQuotas) {
      ...
      quotas = {
        ...
        messagingQuota: q?.messaging_quota ? { total: q.messaging_quota.total, used: q.messaging_quota.used } : undefined,
        apiCallsQuota: q?.api_calls_quota ? { total: q.api_calls_quota.total, used: q.api_calls_quota.used } : undefined,
      };
    }
