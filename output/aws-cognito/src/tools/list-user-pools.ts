...
export let listUserPools = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    maxResults: z.number().min(1).max(60).optional().describe('Maximum number of user pools to return (1-60, default 60)'),
    nextToken: z.string().optional().describe('Pagination token from a previous response')
  }))
  .output(z.object({
    userPools: z.array(z.object({
      userPoolId: z.string(),
      name: z.string(),
