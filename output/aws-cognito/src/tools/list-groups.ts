...
export let listGroups = SlateTool.create(
  ...
  .input(z.object({
    ...
    limit: z.number().min(1).max(60).optional().describe('Maximum number of groups to return (1-60)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let result = await client.listGroups(ctx.input.userPoolId, ctx.input.limit, ctx.input.nextToken);
