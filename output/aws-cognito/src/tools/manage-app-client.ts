...
let tokenValidityUnitsSchema = z.object({
...
export let manageAppClient = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    clientId: z.string().optional().describe('App client ID (required for get, update, delete)'),
    clientName: z.string().optional().describe('App client name (required for create)'),
    generateSecret: z.boolean().optional().describe('Generate a client secret (for create only)'),
    explicitAuthFlows: z.array(z.string()).optional().describe('Allowed auth flows (e.g., ALLOW_USER_SRP_AUTH, ALLOW_REFRESH_TOKEN_AUTH)'),
    allowedOAuthFlows: z.array(z.enum(['code', 'implicit', 'client_credentials'])).optional(),
    allowedOAuthScopes: z.array(z.string()).optional().describe('OAuth scopes (e.g., openid, email, profile)'),
    allowedOAuthFlowsUserPoolClient: z.boolean().optional().describe('Enable OAuth flows for this client'),
    ...
    supportedIdentityProviders: z.array(z.string()).optional().describe('IdPs to support (e.g., COGNITO, Google)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let buildClientParams = () => {
      ...
      if (ctx.input.generateSecret !== undefined) params.GenerateSecret = ctx.input.generateSecret;
      ...
      if (ctx.input.allowedOAuthScopes) params.AllowedOAuthScopes = ctx.input.allowedOAuthScopes;
      if (ctx.input.allowedOAuthFlowsUserPoolClient !== undefined) params.AllowedOAuthFlowsUserPoolClient = ctx.input.allowedOAuthFlowsUserPoolClient;
      ...
      if (ctx.input.defaultRedirectUri) params.DefaultRedirectURI = ctx.input.defaultRedirectUri;
      if (ctx.input.supportedIdentityProviders) params.SupportedIdentityProviders = ctx.input.supportedIdentityProviders;
      if (ctx.input.accessTokenValidity !== undefined) params.AccessTokenValidity = ctx.input.accessTokenValidity;
      if (ctx.input.idTokenValidity !== undefined) params.IdTokenValidity = ctx.input.idTokenValidity;
      if (ctx.input.refreshTokenValidity !== undefined) params.RefreshTokenValidity = ctx.input.refreshTokenValidity;
      ...
      if (ctx.input.enableTokenRevocation !== undefined) params.EnableTokenRevocation = ctx.input.enableTokenRevocation;
      if (ctx.input.preventUserExistenceErrors) params.PreventUserExistenceErrors = ctx.input.preventUserExistenceErrors;
    ...
    if (action === 'list') {
      let result = await client.listUserPoolClients(userPoolId, ctx.input.maxResults, ctx.input.nextToken);
