...
let lifecycleConditionSchema = z.object({
  ...
  createdBefore: z.string().optional().describe('Objects created before this date (YYYY-MM-DD)'),
  numNewerVersions: z.number().optional().describe('Number of newer versions (for versioned buckets)'),
  isLive: z.boolean().optional().describe('Whether the object is live or noncurrent'),
  matchesStorageClass: z.array(z.string()).optional().describe('Only apply if object is in one of these storage classes'),
  daysSinceNoncurrentTime: z.number().optional().describe('Days since the object became noncurrent'),
  daysSinceCustomTime: z.number().optional().describe('Days since the object\'s custom time'),
});

let lifecycleRuleSchema = z.object({
  action: z.object({
    type: z.enum(['Delete', 'SetStorageClass', 'AbortIncompleteMultipartUpload']).describe('Lifecycle action type'),
    storageClass: z.string().optional().describe('Target storage class (required for SetStorageClass)'),
  }),
  condition: lifecycleConditionSchema,
});

export let manageLifecycle = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    action: z.enum(['get', 'set', 'add', 'clear']).describe('Operation to perform on lifecycle rules'),
    rules: z.array(lifecycleRuleSchema).optional().describe('Lifecycle rules (required for "set" action)'),
