...
export let manageNotifications = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum 100 notification configurations per bucket.',
    ],
  }
)
  .input(z.object({
    ...
    topic: z.string().optional().describe('Pub/Sub topic in format "projects/{project}/topics/{topic}" (required for create)'),
    eventTypes: z.array(z.enum(['OBJECT_FINALIZE', 'OBJECT_METADATA_UPDATE', 'OBJECT_DELETE', 'OBJECT_ARCHIVE'])).optional().describe('Event types to listen for. Defaults to all types.'),
    objectNamePrefix: z.string().optional().describe('Only send notifications for objects with this name prefix'),
    payloadFormat: z.enum(['JSON_API_V1', 'NONE']).optional().describe('Notification payload format. Defaults to JSON_API_V1.'),
    customAttributes: z.record(z.string(), z.string()).optional().describe('Custom key-value attributes included in every notification message'),
    notificationId: z.string().optional().describe('ID of the notification to delete (required for delete)'),
  }))
  .output(z.object({
