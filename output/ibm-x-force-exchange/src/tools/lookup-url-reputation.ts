...
export let lookupUrlReputation = SlateTool.create(
  spec,
  {
...
Use this to check if a URL is associated with phishing, malware distribution, or other malicious activity.`,
  ...
  .input(z.object({
    url: z.string().describe('URL or domain to look up (e.g., "example.com" or "https://example.com/path")'),
    includeMalware: z.boolean().optional().describe('Also retrieve malware associated with this URL'),
  ...
  .output(z.object({
    ...
    categories: z.record(z.string(), z.any()).optional().describe('Content categories and their confidence values'),
    categoryDescriptions: z.record(z.string(), z.string()).optional().describe('Human-readable category descriptions'),
    application: z.record(z.string(), z.any()).optional().describe('Associated application profile'),
    history: z.array(z.object({
      url: z.string().optional().describe('URL for this history entry'),
      categories: z.record(z.string(), z.any()).optional().describe('Categories at this point in time'),
      score: z.number().optional().describe('Risk score at this point'),
      created: z.string().optional().describe('When this record was created'),
    })).optional().describe('Categorization history (if requested)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let scoreDisplay = result.score !== undefined ? ` — risk score **${result.score}/10**` : '';
