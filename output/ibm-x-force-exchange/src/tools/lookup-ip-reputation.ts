...
export let lookupIpReputation = SlateTool.create(
  spec,
  {
...
Supports IPv4 and IPv6 addresses. Categories include Spam, Malware, Bots, Scanning IPs, Anonymisation Services, and more.`,
  ...
  .input(z.object({
    ...
    includeMalware: z.boolean().optional().describe('Also retrieve malware associated with this IP'),
    includeHistory: z.boolean().optional().describe('Also retrieve reputation history for this IP'),
  }))
  .output(z.object({
    ip: z.string().describe('The queried IP address'),
    score: z.number().optional().describe('Overall risk score (1-10, where 10 is highest risk)'),
    reasonDescription: z.string().optional().describe('Human-readable reason for the risk score'),
    categories: z.array(categoryEntrySchema).optional().describe('Threat categories associated with this IP'),
    geo: z.record(z.string(), z.any()).optional().describe('Geolocation information for the IP'),
    ...
    history: z.array(historyEntrySchema).optional().describe('Reputation history entries (if requested)'),
    malwareCount: z.number().optional().describe('Number of associated malware samples (if requested)'),
    malwareSamples: z.array(z.object({
      ...
      firstSeen: z.string().optional().describe('When the malware was first seen from this IP'),
      lastSeen: z.string().optional().describe('When the malware was last seen from this IP'),
    })).optional().describe('Associated malware samples (if requested)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    let result: any = {
      ...
      asn: report.asns ? Object.keys(report.asns)[0] ? Number(Object.keys(report.asns)[0]) : undefined : undefined,
    ...
    let scoreDisplay = result.score !== undefined ? ` with risk score **${result.score}/10**` : '';
