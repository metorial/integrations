...
let collectionSchema = z.object({
  ...
  }).optional().describe('Collection owner'),
...
Use the **action** field to specify the operation: "list" for your own, "listPublic" for public, "search", "get", or "create".`,
  ...
  .input(z.object({
    action: z.enum(['list', 'listPublic', 'search', 'get', 'create']).describe('Operation to perform'),
    collectionId: z.string().optional().describe('Collection ID (required for "get" action)'),
    searchQuery: z.string().optional().describe('Search query (required for "search" action)'),
    title: z.string().optional().describe('Collection title (required for "create" action)'),
    description: z.string().optional().describe('Collection description (for "create" action)'),
    tlp: z.enum(['WHITE', 'GREEN', 'AMBER', 'RED']).optional().describe('TLP color marking (for "create" action)'),
    tags: z.array(z.string()).optional().describe('Tags (for "create" action)'),
  }))
  .output(z.object({
    collections: z.array(collectionSchema).optional().describe('List of collections (for list/search actions)'),
    collection: collectionSchema.optional().describe('Single collection (for get/create actions)'),
