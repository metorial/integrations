...
export let createEntry = SlateTool.create(
  ...
  .input(z.object({
    ...
    fields: z.record(z.string(), z.any()).describe('Entry fields keyed by field ID with locale sub-keys, e.g. {"title": {"en-US": "My Title"}}.'),
    publish: z.boolean().optional().describe('If true, publish the entry immediately after creation.')
  }))
  .output(z.object({
