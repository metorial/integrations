...
export let createAsset = SlateTool.create(
  ...
  .input(z.object({
    title: z.record(z.string(), z.string()).describe('Asset title by locale, e.g. {"en-US": "Photo"}.'),
    description: z.record(z.string(), z.string()).optional().describe('Asset description by locale.'),
    ...
    })).describe('File details by locale, e.g. {"en-US": {fileName: "photo.jpg", contentType: "image/jpeg", upload: "https://example.com/photo.jpg"}}.'),
    processAndPublish: z.boolean().optional().describe('If true, process and publish the asset after creation.')
  }))
  .output(z.object({
