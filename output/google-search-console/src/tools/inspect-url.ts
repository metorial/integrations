...
export let inspectUrl = SlateTool.create(
  spec,
  {
    name: 'Inspect URL',
    key: 'inspect_url',
    ...
    constraints: [
      'Limited to 2,000 queries per day and 600 queries per minute per property.'
    ],
    tags: {
      readOnly: true
    }
  ...
  .input(z.object({
    ...
    languageCode: z.string().optional().describe('IETF BCP-47 language code for translated issue messages (e.g., "en-US"). Defaults to "en-US"')
  }))
  .output(z.object({
    inspectionResultLink: z.string().optional().describe('Link to the URL Inspection result in Search Console'),
    indexStatusResult: z.object({
      verdict: z.string().optional().describe('Overall verdict: PASS, PARTIAL, FAIL, or NEUTRAL'),
      coverageState: z.string().optional().describe('Coverage state description'),
      robotsTxtState: z.string().optional().describe('Whether robots.txt allows crawling: ALLOWED or DISALLOWED'),
      indexingState: z.string().optional().describe('Whether indexing is allowed: INDEXING_ALLOWED, BLOCKED_BY_META_TAG, or BLOCKED_BY_HTTP_HEADER'),
      lastCrawlTime: z.string().optional().describe('Timestamp of last crawl'),
      pageFetchState: z.string().optional().describe('Page fetch status: SUCCESSFUL, SOFT_404, NOT_FOUND, SERVER_ERROR, etc.'),
      googleCanonical: z.string().optional().describe('The canonical URL selected by Google'),
      userCanonical: z.string().optional().describe('The canonical URL declared by the user'),
      sitemap: z.array(z.string()).optional().describe('Sitemaps that reference this URL'),
      referringUrls: z.array(z.string()).optional().describe('URLs that link to this page'),
      crawledAs: z.string().optional().describe('Crawler type used: DESKTOP or MOBILE')
    }).optional().describe('Index status information'),
    ampResult: z.object({
      verdict: z.string().optional().describe('AMP verdict: PASS, PARTIAL, FAIL, or NEUTRAL'),
      ...
      })).optional().describe('AMP issues found')
    }).optional().describe('AMP validation results'),
    mobileUsabilityResult: z.object({
      ...
      })).optional().describe('Mobile usability issues found')
    }).optional().describe('Mobile usability results'),
      ...
      detectedItems: z.array(z.object({
        ...
        items: z.array(z.object({
          ...
          issues: z.array(z.object({
            issueMessage: z.string().optional(),
            severity: z.string().optional()
          })).optional().describe('Issues for this item')
        })).optional().describe('Detected items of this type')
      })).optional().describe('Detected rich result types')
    }).optional().describe('Rich results analysis')
  }))
