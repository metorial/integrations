...
export let manageSubscriber = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'get', 'unsubscribe']).describe('Action to perform on the subscriber'),
    subscriberId: z.number().optional().describe('Subscriber ID (required for update, get, unsubscribe)'),
    emailAddress: z.string().optional().describe('Email address (required for create, optional for update)'),
    firstName: z.string().optional().describe('First name'),
    state: z.enum(['active', 'inactive']).optional().describe('Initial state (only for create)'),
