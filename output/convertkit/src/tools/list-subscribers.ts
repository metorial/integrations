...
export let listSubscribers = SlateTool.create(
  ...
  .input(z.object({
    status: z.enum(['active', 'inactive', 'bounced', 'complained', 'cancelled', 'all']).optional().describe('Filter by subscriber status'),
    ...
    sequenceId: z.number().optional().describe('List subscribers for this specific sequence'),
