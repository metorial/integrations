...
export let manageBroadcasts = SlateTool.create(
  ...
  .output(z.object({
    broadcasts: z.array(z.object({
      ...
      isPublic: z.boolean(),
    })).optional().describe('List of broadcasts (for list action)'),
    broadcast: z.object({
      broadcastId: z.number(),
      subject: z.string().nullable(),
    ...
    }).optional().describe('Single broadcast (for get, create, update)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (input.action === 'list') {
      let result = await client.listBroadcasts({ perPage: input.perPage, after: input.cursor });
