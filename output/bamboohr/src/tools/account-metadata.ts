...
export let getAccountFields = SlateTool.create(
  spec,
  {
...
export let getAccountMetadata = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    include: z.array(z.enum(['lists', 'tables', 'users'])).optional().describe('Which metadata to include. Defaults to all.')
  }))
  .output(z.object({
    lists: z.any().optional().describe('List definitions and values (departments, locations, etc.)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (includes.includes('lists')) {
      promises.push(client.getLists().then(d => { output.lists = d; }));
    }
    if (includes.includes('tables')) {
      promises.push(client.getTables().then(d => { output.tables = d; }));
    }
    if (includes.includes('users')) {
      promises.push(client.getUsers().then(d => { output.users = d; }));
    }
