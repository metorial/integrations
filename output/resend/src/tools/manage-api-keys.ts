...
export let createApiKey = SlateTool.create(
  ...
  .input(z.object({
    ...
    permission: z.enum(['full_access', 'sending_access']).optional().describe('Permission level. Defaults to full_access.'),
    domainId: z.string().optional().describe('Restrict key to a specific domain. Only with sending_access.'),
  }))
  .output(z.object({
