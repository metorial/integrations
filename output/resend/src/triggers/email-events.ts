...
export let emailEvents = SlateTrigger.create(
  spec,
  {
    name: 'Email Events',
    key: 'email_events',
    description: 'Triggers when email events occur such as sent, delivered, bounced, opened, clicked, complained, delivery delayed, received, or failed.',
  },
)
  .input(z.object({
        ...
        inputs: [
          {
            ...
            eventId: data.data?.email_id ? `${eventType}_${data.data.email_id}_${data.created_at || Date.now()}` : `${eventType}_${Date.now()}`,
