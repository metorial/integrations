...
let extractionResultSchema = z.object({
...
}).nullable().describe('Extraction result, null if processing is not yet complete');

export let getExtractionResult = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    extractionId: z.string().describe('Extraction ID returned from the async extraction request')
  }))
  .output(z.object({
