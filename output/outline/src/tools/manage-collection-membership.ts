...
export let manageCollectionMembership = SlateTool.create(
  spec,
  {
...
Supports granting read or read_write access at the user or group level.`,
  ...
  .input(z.object({
    ...
    action: z.enum(['add_user', 'remove_user', 'add_group', 'remove_group']).describe('Membership action to perform'),
    userId: z.string().optional().describe('User ID (required for add_user and remove_user)'),
    groupId: z.string().optional().describe('Group ID (required for add_group and remove_group)'),
    permission: z.enum(['read', 'read_write']).optional().describe('Permission level to grant (for add actions)'),
  }))
  .output(z.object({
