...
export let manageFirewallRulesTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['list', 'create', 'delete', 'list_ip_rules', 'create_ip_rule', 'delete_ip_rule']).describe('Operation to perform'),
    ...
    firewallAction: z.string().optional().describe('Action to take (block, challenge, js_challenge, managed_challenge, allow, log, bypass)'),
    ...
    ipMode: z.string().optional().describe('IP access rule mode (block, challenge, whitelist, js_challenge)'),
    ipTarget: z.string().optional().describe('IP access rule target type (ip, ip_range, asn, country)'),
    ipValue: z.string().optional().describe('IP access rule value (IP address, CIDR range, ASN, or country code)'),
