...
export let manageAccountTool = SlateTool.create(
  spec,
  {
    name: 'Manage Account',
    key: 'manage_account',
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'list_accounts') {
      let response = await client.listAccounts({ page: ctx.input.page, perPage: ctx.input.perPage });
    ...
    if (action === 'list_members') {
      if (!accountId) throw new Error('accountId is required');
      let response = await client.listAccountMembers(accountId, { page: ctx.input.page, perPage: ctx.input.perPage });
    ...
    if (action === 'add_member') {
      ...
      let response = await client.addAccountMember(accountId, ctx.input.email, ctx.input.roleIds);
      return {
    ...
    if (action === 'remove_member') {
      if (!accountId || !ctx.input.memberId) throw new Error('accountId and memberId are required');
