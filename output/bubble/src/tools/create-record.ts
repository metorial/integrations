...
export let createRecord = SlateTool.create(
  ...
  .input(z.object({
    dataType: z.string().describe('Name of the Bubble data type (table) to create a record in, e.g. "User", "Product", "Order".'),
    fields: z.record(z.string(), z.any()).describe('Key-value pairs of field names and their values for the new record. Field names must match the data type schema in your Bubble app.'),
  }))
  .output(z.object({
