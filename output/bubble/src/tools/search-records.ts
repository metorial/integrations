...
let constraintSchema = z.object({
  ...
  constraintType: z.string().describe('Type of constraint. Common values: "equals", "not equal", "is_empty", "is_not_empty", "text contains", "not text contains", "greater than", "less than", "in", "not in", "contains", "not contains", "empty", "not empty", "geographic_search".'),
  value: z.any().optional().describe('Value to compare against. Not needed for "is_empty" / "is_not_empty" constraints.'),
});
...
export let searchRecords = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Maximum 100 records per request. Use cursor for pagination.',
    ],
    tags: {
      destructive: false,
      readOnly: true,
  ...
  .input(z.object({
    ...
    constraints: z.array(constraintSchema).optional().describe('Array of filter constraints. Omit to return all records.'),
    sortField: z.string().optional().describe('Field name to sort results by.'),
    descending: z.boolean().optional().describe('Sort in descending order. Defaults to false (ascending).'),
    limit: z.number().optional().describe('Maximum number of records to return (max 100).'),
    cursor: z.number().optional().describe('Pagination cursor. Use the cursor value from a previous response to get the next page.'),
  }))
  .output(z.object({
    records: z.array(z.record(z.string(), z.any())).describe('Array of matching records with all field values.'),
    ...
    cursor: z.number().describe('Cursor position for pagination. Pass this as the cursor input to get the next page.'),
  }))
