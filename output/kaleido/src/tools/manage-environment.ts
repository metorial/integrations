...
Configure the blockchain protocol (Geth, Quorum, Besu, Corda, Fabric) and consensus algorithm (PoA, Raft, IBFT) when creating.`,
  ...
  .input(z.object({
    ...
    environmentId: z.string().optional().describe('Environment ID (required for get, update, delete)'),
    ...
    provider: z.enum(['geth', 'quorum', 'besu', 'corda', 'fabric']).optional().describe('Blockchain protocol (required for create)'),
    consensusType: z.enum(['poa', 'raft', 'ibft']).optional().describe('Consensus algorithm (required for create)'),
  ...
  .output(z.object({
    environments: z.array(z.object({
      ...
      state: z.string().optional().describe('Current state (setup, initializing, live, deleted)'),
    ...
    })).optional().describe('List of environments (for list action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'get') {
      let result = await client.getEnvironment(ctx.input.consortiumId, ctx.input.environmentId);
      return {
        output: {
          environmentId: result._id,
          name: result.name,
    ...
    if (ctx.input.action === 'update') {
      ...
      let result = await client.updateEnvironment(ctx.input.consortiumId, ctx.input.environmentId, updateParams);
      return {
        output: {
          environmentId: result._id,
          name: result.name,
