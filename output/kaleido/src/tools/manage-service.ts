...
Each service is bound to a membership and runs alongside the blockchain nodes.`,
  ...
  .input(z.object({
    ...
    serviceType: z.string().optional().describe('Service type identifier, e.g. "hdwallet", "ipfs", "documentstore", "app2app", "tether", "idregistry" (required for create)'),
    membershipId: z.string().optional().describe('Membership ID that owns the service (required for create)'),
  ...
  .output(z.object({
    ...
    })).optional().describe('List of services (for list action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.action === 'list') {
      let services = await client.listServices(ctx.input.consortiumId, ctx.input.environmentId);
    ...
    if (ctx.input.action === 'create') {
      ...
      let result = await client.createService(ctx.input.consortiumId, ctx.input.environmentId, {
        name: ctx.input.name,
    ...
    if (ctx.input.action === 'get') {
      let result = await client.getService(ctx.input.consortiumId, ctx.input.environmentId, ctx.input.serviceId);
      return {
        output: {
          serviceId: result._id,
          name: result.name,
    ...
    // delete
    await client.deleteService(ctx.input.consortiumId, ctx.input.environmentId, ctx.input.serviceId);
    return {
      output: {
