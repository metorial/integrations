...
export let exportDataTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    start: z.string().describe('Start date in YYYY-MM-DD format. For hourly export use YYYY-MM-DDTHH format.'),
    end: z.string().describe('End date in YYYY-MM-DD format. For hourly export use YYYY-MM-DDTHH format (same day only).'),
    fields: z.array(z.enum([
      'added_iso', 'added_unix', 'path', 'hostname', 'referrer', 'referrer_raw',
      'utm_source', 'utm_medium', 'utm_campaign', 'utm_content', 'utm_term',
      'browser_name', 'browser_version', 'os_name', 'os_version',
      'device_type', 'country_code', 'lang_region', 'lang_language',
      'screen_width', 'screen_height', 'viewport_width', 'viewport_height',
      'scroll_percentage', 'duration_seconds', 'session_id',
      'document_referrer', 'is_unique',
    ])).describe('Fields to include in the export. Only selected fields are returned.'),
    type: z.enum(['pageviews', 'events']).optional().describe('Filter to only page views or only events.'),
    timezone: z.string().optional().describe('IANA timezone identifier (e.g. "Europe/Amsterdam"). Defaults to UTC.'),
    includeRobots: z.boolean().optional().describe('Whether to include robot/bot traffic. Defaults to false.'),
  }))
  .output(z.object({
    dataPoints: z.array(z.record(z.string(), z.unknown())).describe('Array of exported data point records with the requested fields'),
