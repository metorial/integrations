...
Returns Base64-encoded WAV audio and optionally word-level timestamps for precise playback synchronization.`,
  ...
  .input(z.object({
    ...
    voice: z.string().optional().describe('Voice identifier (e.g., "expressive_ava", "prog_avery", "neural_jennifer"). Defaults to "expressive_ava".'),
    timestamps: z.boolean().optional().describe('Include word-level timing data in the response'),
    stylePrompt: z.string().optional().describe('Natural-language persona instructions for programmable voices only'),
