...
export let manageAgent = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['get_source_ips', 'set_source_ips', 'get_token_version', 'set_token_version']).describe('The operation to perform'),
    sourceIps: z.array(z.string()).optional().describe('List of IP addresses to set (for set_source_ips)'),
    tokenVersion: z.string().optional().describe('Token version to set (for set_token_version, currently only "1.0")')
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'set_source_ips') {
      if (!ctx.input.sourceIps) throw new Error('sourceIps is required for set_source_ips action');
    ...
    if (action === 'set_token_version') {
      if (!ctx.input.tokenVersion) throw new Error('tokenVersion is required for set_token_version action');
