...
export let createToken = SlateTool.create(
  spec,
  {
    name: 'Create Token',
    key: 'create_token',
...
Use a seller service ID (from the service directory) or a seller domain/URL to target the token. Set an appropriate expiration window (5–10 minutes recommended for one-time use).`,
  ...
  .input(z.object({
    type: z.enum(['kya', 'pay', 'kya+pay']).describe('Token type: kya (identity only), pay (payment only), or kya+pay (both)'),
    tokenAmount: z.string().optional().describe('Maximum charge amount in USD as a decimal string (required for pay and kya+pay)'),
    sellerServiceId: z.string().optional().describe('UUID of the seller service from the marketplace directory'),
    sellerDomainOrUrl: z.string().optional().describe('Website URL or domain of the seller (alternative to sellerServiceId)'),
    ...
    identityPermissions: z.array(z.string()).optional().describe('Additional identity fields to include in KYA or KYA+PAY tokens')
  }))
  .output(z.object({
    token: z.string().describe('Signed JWT token to pass to the seller when calling their API')
  }))
