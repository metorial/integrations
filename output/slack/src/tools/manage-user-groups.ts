...
export let manageUserGroups = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['create', 'update', 'enable', 'disable', 'list', 'set_members', 'list_members']).describe('User group management action'),
    userGroupId: z.string().optional().describe('User group ID (required for update/enable/disable/set_members/list_members)'),
    ...
    userIds: z.array(z.string()).optional().describe('Full list of member user IDs (for set_members action)'),
  ...
  .output(z.object({
    ...
    userGroups: z.array(userGroupSchema).optional().describe('List of user groups (for list action)'),
    members: z.array(z.string()).optional().describe('Member user IDs (for list_members action)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'disable') {
      if (!ctx.input.userGroupId) throw new Error('userGroupId is required for disable action');
    ...
    if (action === 'set_members') {
      if (!ctx.input.userGroupId) throw new Error('userGroupId is required for set_members action');
      if (!ctx.input.userIds) throw new Error('userIds is required for set_members action');
      let group = await client.updateUserGroupMembers(ctx.input.userGroupId, ctx.input.userIds);
      return {
        output: { userGroup: mapGroup(group) },
    ...
    if (action === 'list_members') {
      if (!ctx.input.userGroupId) throw new Error('userGroupId is required for list_members action');
