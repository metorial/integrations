...
export let manageChannelMembers = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['invite', 'kick', 'join', 'leave', 'list']).describe('The membership action to perform'),
    channelId: z.string().describe('Channel ID'),
    userIds: z.array(z.string()).optional().describe('User IDs to invite or a single user ID to remove (for invite/kick actions)'),
  ...
  .output(z.object({
    channelId: z.string().describe('Channel ID'),
    members: z.array(z.string()).optional().describe('List of member user IDs (for list action)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'invite') {
      if (!userIds || userIds.length === 0) throw new Error('userIds is required for invite action');
    ...
    if (action === 'kick') {
      if (!userIds || userIds.length === 0) throw new Error('userIds is required for kick action');
