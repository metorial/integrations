...
export let formSubmitted = SlateTrigger.create(
  spec,
  {
    name: 'Form Submitted',
    key: 'form_submitted',
  ...
  .input(z.object({
    ...
    answers: z.record(z.string(), z.any()).optional().describe('Form response answers as key-value pairs'),
    ...
    handleRequest: async (ctx) => {
      ...
      return {
        inputs: [{
          ...
          formName: formResponse.form_name ?? formResponse.formName ?? formResponse.form_title ?? undefined,
          respondentName: formResponse.respondent_name ?? formResponse.respondentName ?? formResponse.name ?? undefined,
          respondentEmail: formResponse.respondent_email ?? formResponse.respondentEmail ?? formResponse.email ?? undefined,
          answers: formResponse.answers ?? formResponse.responses ?? formResponse.fields ?? undefined,
