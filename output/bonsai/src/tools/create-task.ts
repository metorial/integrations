...
export let createTask = SlateTool.create(
  ...
  .input(z.object({
    ...
    assigneeEmail: z.string().optional().describe('Email of the team member to assign the task to'),
    ...
    startDate: z.string().optional().describe('Start date for the task (ISO 8601 format, e.g., "2025-01-15")'),
    dueDate: z.string().optional().describe('Due date for the task (ISO 8601 format, e.g., "2025-01-31")'),
    timeEstimate: z.number().optional().describe('Estimated time for the task in hours'),
    billingType: z.string().optional().describe('Billing type for the task (e.g., "billable", "non_billable")'),
