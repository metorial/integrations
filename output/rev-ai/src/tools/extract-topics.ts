...
export let extractTopics = SlateTool.create(
  spec,
  {
    name: 'Extract Topics',
    key: 'extract_topics',
...
Can submit plain text directly or poll an existing job for results.`,
  ...
  .input(z.object({
    ...
    jobId: z.string().optional().describe('Existing topic extraction job ID to retrieve results for'),
    metadata: z.string().optional().describe('Optional metadata to associate with the job'),
    scoreThreshold: z.number().optional().describe('Minimum relevance score (0-1) to include in results'),
  }))
  .output(z.object({
    ...
    })).optional().describe('Extracted topics ranked by relevance (only when job is completed)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: job.status === 'completed' && topics
