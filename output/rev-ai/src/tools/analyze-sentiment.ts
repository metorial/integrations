...
Can submit plain text directly or poll an existing job for results.`,
  ...
  .input(z.object({
    ...
    jobId: z.string().optional().describe('Existing sentiment analysis job ID to retrieve results for'),
  ...
  .output(z.object({
    ...
    messages: z.array(z.object({
      ...
      sentiment: z.string().describe('Sentiment label: "positive", "negative", or "neutral"'),
      ts: z.number().optional().describe('Start timestamp in seconds'),
      endTs: z.number().optional().describe('End timestamp in seconds'),
    })).optional().describe('Sentence-level sentiment results (only when job is completed)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    return {
      ...
      message: job.status === 'completed' && messages
