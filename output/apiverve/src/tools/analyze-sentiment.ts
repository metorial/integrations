...
export let analyzeSentiment = SlateTool.create(
  ...
  .output(z.object({
    sentimentLabel: z.string().describe('Sentiment label (e.g. "positive", "negative", "neutral")'),
    sentimentScore: z.number().describe('Raw sentiment score'),
    comparativeScore: z.number().describe('Comparative sentiment metric (score normalized by word count)'),
    ...
    normalizedScore: z.number().optional().describe('Score normalized to -1 to 1 range (paid plans only)'),
  }))
