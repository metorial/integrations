...
export let callApi = SlateTool.create(
  spec,
  {
    name: 'Call Any API',
    key: 'call_api',
...
Browse available endpoints at [docs.apiverve.com/api-endpoints](https://docs.apiverve.com/api-endpoints).`,
  ...
  .input(z.object({
    endpoint: z.string().describe('The API endpoint name (e.g. "translator", "uuidgenerator", "whoisdomainlookup")'),
    method: z.enum(['GET', 'POST']).describe('HTTP method to use'),
    parameters: z.record(z.string(), z.any()).optional().describe('Parameters to send with the request (query params for GET, body for POST)'),
  }))
  .output(z.object({
