...
export let storeResponse = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    responseDate: z.string().optional().describe('ISO 8601 timestamp for the response (defaults to now)'),
    preventDuplicates: z.boolean().optional().describe('Prevent duplicate responses (default: true)'),
    responseData: z.record(z.string(), z.unknown()).optional().describe('Response data keyed by question identifier (e.g. { "nps": 9, "feedback": "Great product!" })'),
  }))
  .output(z.object({
