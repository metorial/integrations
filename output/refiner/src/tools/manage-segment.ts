...
export let manageSegment = SlateTool.create(
  ...
  .input(z.object({
    ...
    action: z.enum(['add', 'remove']).describe('Whether to add or remove the user from the segment'),
  }))
  .output(z.object({
    success: z.boolean().describe('Whether the action was successful'),
    contactUuid: z.string().optional().describe('UUID of the contact (returned when adding)'),
