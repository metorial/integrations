...
let webhookPayloadSchema = z.object({
  ...
  responseData: z.record(z.string(), z.unknown()).nullable().describe('Survey response data (only for completed events)'),
...
export let surveyInteraction = SlateTrigger.create(
  spec,
  {
    name: 'Survey Interaction',
    key: 'survey_interaction',
    description: 'Triggers when a user interacts with a survey — completes it, sees it, or dismisses it. Provides full contact data, survey details, and response data for completed surveys.',
  },
)
  .input(z.object({
    triggeredEvent: z.string().describe('The event that triggered the webhook'),
