...
export let configureAutoFeedback = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    enabled: z.boolean().describe('Enable (true) or disable (false) automatic feedback requests'),
    requestsPerDay: z.number().optional().describe('Number of feedback requests to send per day in automatic mode'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (data.errorCode !== 0) {
      throw new Error(`Failed to configure auto feedback: ${data.errorMessage} (code: ${data.errorCode})`);
    }
