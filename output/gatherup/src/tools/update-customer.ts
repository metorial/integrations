...
export let updateCustomer = SlateTool.create(
  ...
  .input(z.object({
    ...
    preference: z.enum(['email', 'sms']).optional().describe('Updated communication preference'),
    customId: z.string().optional().describe('Updated custom identifier'),
    jobId: z.string().optional().describe('Updated job identifier'),
    tags: z.string().optional().describe('Updated tags (comma-separated). This overwrites existing tags.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (data.errorCode !== 0) {
      throw new Error(`Failed to update customer: ${data.errorMessage} (code: ${data.errorCode})`);
    }
