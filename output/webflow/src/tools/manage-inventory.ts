...
export let manageInventory = SlateTool.create(
  ...
  .input(z.object({
    ...
    inventoryType: z.enum(['infinite', 'finite']).optional().describe('Inventory tracking type'),
    quantity: z.number().optional().describe('New inventory quantity (only for finite inventory)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (inventoryType || quantity !== undefined) {
      let updated = await client.updateInventory(collectionId, itemId, { inventoryType, quantity });
      return {
        output: {
          itemId,
