...
export let pageEventsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'Page Events',
    key: 'page_events',
    description: 'Triggered when pages are created, have their metadata updated, or are deleted on a Webflow site.',
  }
)
  .input(z.object({
  ...
  .webhook({
    ...
    handleEvent: async (ctx) => {
      ...
      else if (ctx.input.triggerType === 'page_metadata_updated') eventType = 'page.metadata_updated';
