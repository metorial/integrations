...
export let hashtagStats = SlateTool.create(
  spec,
  {
    name: 'Hashtag Stats',
    key: 'hashtag_stats',
...
Use this to evaluate which hashtags to use in your content strategy based on current performance data.`,
    constraints: [
      'Up to 100 hashtags can be analyzed in a single request',
    ],
    tags: {
      destructive: false,
      readOnly: true,
  ...
  .input(z.object({
    hashtags: z.array(z.string()).min(1).max(100).describe('List of hashtags to get stats for (without # symbol)'),
  }))
  .output(z.object({
