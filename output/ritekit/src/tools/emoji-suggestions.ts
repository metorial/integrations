...
export let emojiSuggestions = SlateTool.create(
  spec,
  {
...
Use "suggest" mode to get a list of relevant emojis, or "emojify" mode to get back the text with emojis automatically inserted at relevant positions.`,
  ...
  .input(z.object({
    ...
    mode: z.enum(['suggest', 'emojify']).optional().describe('Mode: "suggest" returns emoji list, "emojify" inserts emojis into text (default: suggest)'),
  }))
  .output(z.object({
    emojis: z.array(z.object({
      emoji: z.string().describe('Emoji character'),
      score: z.number().describe('Relevance score'),
    })).optional().describe('List of suggested emojis (suggest mode)'),
    emojifiedText: z.string().optional().describe('Text with emojis automatically inserted (emojify mode)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: { emojis },
      message: `Found **${emojis.length}** emoji suggestions: ${emojis.slice(0, 10).map((e) => e.emoji).join(' ')}`,
    };
  }).build();
