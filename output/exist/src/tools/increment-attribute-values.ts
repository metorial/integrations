...
export let incrementAttributeValuesTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    increments: z.array(z.object({
      ...
      date: z.string().optional().describe('Date to apply the increment to (YYYY-MM-DD). Defaults to today.'),
    })).min(1).max(35).describe('Array of attribute value increments'),
  }))
  .output(z.object({
