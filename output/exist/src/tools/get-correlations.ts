...
let correlationSchema = z.object({
  ...
  secondPerson: z.string().describe('Human-readable second-person description (e.g. "you walk more")'),
...
export let getCorrelationsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    firstAttribute: z.string().optional().describe('Filter by or look up a specific attribute name'),
    secondAttribute: z.string().optional().describe('Second attribute for a specific pair lookup (requires firstAttribute)'),
    strongOnly: z.boolean().optional().describe('Only return strong correlations'),
    confidentOnly: z.boolean().optional().describe('Only return five-star confidence correlations'),
  ...
  .output(z.object({
    ...
    totalCount: z.number().optional().describe('Total number of correlations (not present for pair lookups)'),
