...
let attributeSchema = z.object({
  ...
  valueType: z.number().describe('Value type code (0=quantity, 1=decimal, 2=string, 3=duration, 4=time of day, 5=percentage, 7=boolean, 8=scale)'),
...
export let getAttributesTool = SlateTool.create(
  spec,
  {
    name: 'Get Attributes',
    key: 'get_attributes',
  ...
  .input(z.object({
    groups: z.string().optional().describe('Comma-separated group names to filter by (e.g. "activity,sleep,mood")'),
    attributeNames: z.string().optional().describe('Comma-separated attribute names to filter by (e.g. "steps,sleep_minutes")'),
    includeValues: z.boolean().optional().describe('Whether to include recent daily values for each attribute'),
    days: z.number().optional().describe('Number of days of values to return (max 31, default 1). Only used when includeValues is true.'),
    dateMax: z.string().optional().describe('Most recent date to include values for (YYYY-MM-DD). Only used when includeValues is true.'),
    ownedOnly: z.boolean().optional().describe('Only return attributes owned by this service'),
