...
export let newInsightsTrigger = SlateTrigger.create(
  spec,
  {
    name: 'New Insights',
    key: 'new_insights',
    description: 'Triggers when new insights are generated for the user\'s tracked data. Insights include daily events (e.g., highest steps in X days) and weekly/monthly summaries.',
  }
)
  .input(z.object({
    insightId: z.string().describe('Unique identifier for the insight (created timestamp + type)'),
  ...
  .output(z.object({
    ...
    priority: z.number().describe('Priority level (1=today, 2=this week, 3=last week, 4=last month)'),
    typeName: z.string().describe('Insight type identifier'),
    createdAt: z.string().describe('When the insight was generated'),
  }))
  .polling({
