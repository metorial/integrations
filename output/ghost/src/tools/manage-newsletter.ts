...
export let manageNewsletter = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    senderEmail: z.string().optional().describe('Sender email address (may require validation)'),
    senderReplyTo: z.enum(['newsletter', 'support']).optional().describe('Reply-to behavior'),
    subscribeOnSignup: z.boolean().optional().describe('Auto-subscribe new members'),
    titleFontCategory: z.enum(['serif', 'sans_serif']).optional().describe('Title font style'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'read') {
      if (!ctx.input.newsletterId) throw new Error('newsletterId is required for reading a newsletter');
      let result = await client.readNewsletter(ctx.input.newsletterId);
      let n = result.newsletters[0];
      return { output: mapNewsletter(n), message: `Retrieved newsletter **"${n.name}"** (${n.status}).` };
    }
    ...
    if (ctx.input.subscribeOnSignup !== undefined) data.subscribe_on_signup = ctx.input.subscribeOnSignup;
    if (ctx.input.titleFontCategory !== undefined) data.title_font_category = ctx.input.titleFontCategory;
    if (ctx.input.bodyFontCategory !== undefined) data.body_font_category = ctx.input.bodyFontCategory;
    if (ctx.input.showHeaderIcon !== undefined) data.show_header_icon = ctx.input.showHeaderIcon;
    if (ctx.input.showHeaderTitle !== undefined) data.show_header_title = ctx.input.showHeaderTitle;
    if (ctx.input.showHeaderName !== undefined) data.show_header_name = ctx.input.showHeaderName;
    if (ctx.input.showFeatureImage !== undefined) data.show_feature_image = ctx.input.showFeatureImage;
    ...
    if (action === 'update') {
      if (!ctx.input.newsletterId) throw new Error('newsletterId is required for updating a newsletter');
