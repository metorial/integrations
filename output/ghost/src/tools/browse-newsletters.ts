...
export let browseNewsletters = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    filter: z.string().optional().describe('Ghost NQL filter expression (e.g., "status:active")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let pagination = result.meta?.pagination ?? {
      page: 1, limit: 15, pages: 1, total: newsletters.length, next: null, prev: null,
    };
