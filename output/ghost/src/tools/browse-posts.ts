...
let postSchema = z.object({
  ...
  })).optional().describe('Post authors'),
});
...
export let browsePosts = SlateTool.create(
  spec,
  {
    name: 'Browse Posts',
    key: 'browse_posts',
  ...
  .input(z.object({
    filter: z.string().optional().describe('Ghost NQL filter expression (e.g., "status:published", "tag:news+status:published")'),
    include: z.string().optional().describe('Comma-separated list of related resources to include (e.g., "tags,authors")'),
    formats: z.string().optional().describe('Comma-separated content formats to include (e.g., "html,lexical")'),
    limit: z.number().optional().describe('Number of posts per page (default 15, use "all" by setting 0)'),
    page: z.number().optional().describe('Page number for pagination'),
    order: z.string().optional().describe('Sort order (e.g., "published_at desc", "title asc")'),
