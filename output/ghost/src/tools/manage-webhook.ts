...
export let manageWebhook = SlateTool.create(
  spec,
  {
    ...
    constraints: [
      'Ghost does not provide a browse/list endpoint for webhooks.',
    ],
  }
)
  .input(z.object({
    ...
    event: z.string().optional().describe('Trigger event (e.g., "post.published", "member.added")'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'delete') {
      if (!ctx.input.webhookId) throw new Error('webhookId is required for deleting a webhook');
      await client.deleteWebhook(ctx.input.webhookId);
      return {
        output: {
          webhookId: ctx.input.webhookId, event: '', targetUrl: '', name: null,
          status: 'deleted', apiVersion: '', lastTriggeredAt: null, createdAt: '', updatedAt: '',
        },
    ...
    if (action === 'create') {
      ...
      if (!ctx.input.targetUrl) throw new Error('targetUrl is required for creating a webhook');
      ...
      return { output: mapWebhook(w), message: `Created webhook for event **${w.event}** → \`${w.target_url}\`.` };
    }

    if (action === 'update') {
      if (!ctx.input.webhookId) throw new Error('webhookId is required for updating a webhook');
