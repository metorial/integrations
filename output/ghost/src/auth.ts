...
export let auth = SlateAuth.create()
  .output(z.object({
    token: z.string().describe('Admin API key in the format {id}:{secret} or a staff access token'),
    contentApiKey: z.string().optional().describe('Content API key for read-only access to published content'),
  }))
  .addCustomAuth({
    type: 'auth.custom',
    name: 'Admin API Key',
    ...
    inputSchema: z.object({
      adminApiKey: z.string().describe('Admin API key from Ghost Custom Integration (format: {id}:{secret})'),
      contentApiKey: z.string().optional().describe('Content API key from Ghost Custom Integration (optional, for read-only content access)'),
    }),
