...
export let timezone = SlateTool.create(
  spec,
  {
    name: 'Timezone',
    key: 'timezone',
...
- **Convert**: Convert a specific datetime from one timezone to another`,
  ...
  .input(z.object({
    mode: z.enum(['current_time', 'convert']).describe('Operation mode: "current_time" to get current time at a location, "convert" to convert between timezones'),
    location: z.string().describe('The location/timezone for current time lookup or the base location for conversion'),
    targetLocation: z.string().optional().describe('Target location/timezone for conversion. Required when mode is "convert".'),
    datetime: z.string().optional().describe('Datetime to convert in "YYYY-MM-DD HH:MM:SS" format. Required when mode is "convert".'),
  }))
  .output(z.object({
    baseTimezone: timezoneInfoSchema.optional().describe('Timezone info for the base/requested location'),
    targetTimezone: timezoneInfoSchema.optional().describe('Timezone info for the target location (conversion mode only)'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    if (mode === 'convert') {
      ...
      return {
        output: {
          baseTimezone: result.base_location ? mapTimezoneInfo(result.base_location) : undefined,
          targetTimezone: result.target_location ? mapTimezoneInfo(result.target_location) : undefined,
        },
