...
export let generateAvatar = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    imageSize: z.number().optional().describe('Size of the avatar image in pixels (e.g. 200 for 200x200)'),
    fontSize: z.number().optional().describe('Font size for the initials in pixels'),
    charLimit: z.number().optional().describe('Maximum number of initials to display (e.g. 2 for "JD")'),
    background: z.string().optional().describe('Background color hex code without # (e.g. "4A90D9")'),
    fontColor: z.string().optional().describe('Font color hex code without # (e.g. "FFFFFF")'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let avatarUrl = result?.url ?? result?.avatar_url ?? (typeof result === 'string' ? result : undefined);
