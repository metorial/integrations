...
export let validateEmail = SlateTool.create(
  ...
  .input(z.object({
    ...
    autoCorrect: z.boolean().optional().describe('Whether to suggest auto-corrected email if a typo is detected'),
  }))
  .output(z.object({
    email: z.string().describe('The email address that was validated'),
    autocorrect: z.string().optional().describe('Suggested corrected email address if a typo was detected'),
    deliverability: z.string().optional().describe('Deliverability status: DELIVERABLE, UNDELIVERABLE, or UNKNOWN'),
    ...
    isFreeEmail: z.boolean().optional().describe('Whether the email is from a free provider like Gmail'),
    isDisposableEmail: z.boolean().optional().describe('Whether the email is from a disposable/temporary provider'),
    isRoleEmail: z.boolean().optional().describe('Whether the email is a role-based address like info@ or admin@'),
    isCatchallEmail: z.boolean().optional().describe('Whether the domain is configured as catch-all'),
    isMxFound: z.boolean().optional().describe('Whether MX records were found for the domain'),
