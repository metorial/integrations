...
export let createCoupon = SlateTool.create(
  ...
  .input(z.object({
    ...
    scope: z.enum(['global', 'product', 'service', 'rent']).describe('Where the coupon applies'),
    definition: z.any().describe('Price expression defining the discount (type, value, restrictions)'),
    stackable: z.boolean().optional().describe('Whether this coupon can be combined with others'),
