...
export let createTask = SlateTool.create(
  ...
  .input(z.object({
    ...
    schedule: z.enum(['manual', 'arrival', 'departure']).describe('When the task is due relative to the event'),
    dueAt: z.string().optional().describe('Specific due date (ISO timestamp) for manual schedule'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.taskTemplateId !== undefined) data.task_template_id = ctx.input.taskTemplateId;
