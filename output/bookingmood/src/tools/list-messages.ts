...
let messageSchema = z.object({
  ...
  status: z.string().describe('Message status: manually-scheduled, scheduled-around-arrival, scheduled-around-departure, or sent'),
...
export let listMessages = SlateTool.create(
  ...
  .handleInvocation(async (ctx) => {
    ...
    let messages = await client.listMessages({
      select: 'id,booking_id,calendar_event_id,message_template_id,subject,type,status,send_at,created_at',
      filters,
