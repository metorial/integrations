...
export let createBooking = SlateTool.create(
  ...
  .input(z.object({
    ...
    occupancy: z.record(z.string(), z.number()).describe('Maps capacity group IDs to guest counts'),
    ...
    formValues: z.record(z.string(), z.any()).optional().describe('Custom form field responses keyed by service ID'),
    language: z.string().optional().describe('Preferred language for the booking'),
    redirectUrl: z.string().optional().describe('URL to redirect to after payment completion'),
  }))
  .output(z.object({
    bookingId: z.string().describe('UUID of the created booking'),
    reference: z.string().describe('Public reference for the customer'),
    paymentUrl: z.string().nullable().describe('Payment redirect URL, or null if no payment required'),
  }))
