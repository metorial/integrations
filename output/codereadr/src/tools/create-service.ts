...
export let createService = SlateTool.create(
  ...
  .input(z.object({
    validationMethod: z.enum(['record', 'ondevicerecord', 'database', 'ondevicedatabase', 'postback', 'webview']).describe('How scans are validated'),
    name: z.string().optional().describe('Name of the service'),
    description: z.string().optional().describe('Service description or URL for webview services'),
    databaseId: z.string().optional().describe('Database ID for database-type validation services'),
    postbackUrl: z.string().optional().describe('URL to receive scan data for postback-type services'),
    duplicateValue: z.string().optional().describe('Duplicate scan handling (e.g., "1" to allow, "0" to prevent)'),
    deviceDuplicateValue: z.string().optional().describe('Device-level duplicate handling'),
    periodStartDate: z.string().optional().describe('Service activation start date (YYYY-MM-DD)'),
    periodStartTime: z.string().optional().describe('Service activation start time (HH:mm:ss)'),
    periodEndDate: z.string().optional().describe('Service deactivation end date (YYYY-MM-DD)'),
    periodEndTime: z.string().optional().describe('Service deactivation end time (HH:mm:ss)'),
