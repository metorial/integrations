...
export let manageServiceQuestions = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    questionId: z.string().describe('ID of the question. Use comma-separated IDs for multiple, or "all" for all questions.'),
    operation: z.enum(['add', 'remove']).describe('Whether to add or remove the question from the service'),
    condition: z.enum(['shared_submit', 'pre_submit', 'post_submit', 'valid_scan', 'invalid_scan']).optional().describe('When to show the question (only for "add" operation)'),
    required: z.boolean().optional().describe('Whether the question is required (only for "add" operation)'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (ctx.input.operation === 'add') {
      await client.addQuestionToService(ctx.input.serviceId, ctx.input.questionId, ctx.input.condition, ctx.input.required);
    } else {
