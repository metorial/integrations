...
export let updateDocumentTool = SlateTool.create(
  ...
  .input(z.object({
    ...
    partialDocument: z.record(z.string(), z.any()).optional().describe('Partial document fields to merge into the existing document'),
    script: z.object({
      source: z.string().describe('The script source code'),
      lang: z.string().optional().describe('Script language (default: painless)'),
      params: z.record(z.string(), z.any()).optional().describe('Parameters to pass to the script'),
    }).optional().describe('Script-based update for complex modifications'),
  }))
  .output(z.object({
