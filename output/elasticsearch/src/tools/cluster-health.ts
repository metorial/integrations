...
export let clusterHealthTool = SlateTool.create(
  spec,
  {
    name: 'Cluster Health',
    key: 'cluster_health',
  ...
  .input(z.object({
    includeNodeStats: z.boolean().optional().describe('Include detailed node-level statistics'),
    nodeId: z.string().optional().describe('Specific node ID to get stats for (only used when includeNodeStats is true)'),
  }))
  .output(z.object({
    ...
    nodeStats: z.record(z.string(), z.any()).optional().describe('Node-level statistics (if requested)'),
  }))
