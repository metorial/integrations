...
export let manageSecurityTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    resourceType: z.enum(['user', 'role', 'api_key']).describe('The type of security resource to manage'),
    action: z.enum(['get', 'list', 'create', 'delete', 'invalidate']).describe('The action to perform'),
    ...
    clusterPrivileges: z.array(z.string()).optional().describe('Cluster-level privileges (for role create)'),
    indexPrivileges: z.array(z.object({
      names: z.array(z.string()).describe('Index patterns'),
      privileges: z.array(z.string()).describe('Index-level privileges'),
    })).optional().describe('Index-level privileges (for role create)'),
    ...
    apiKeyExpiration: z.string().optional().describe('Expiration time for the API key (e.g., "30d", "1h")'),
    apiKeyRoleDescriptors: z.record(z.string(), z.any()).optional().describe('Role descriptors to limit API key permissions'),
  }))
  .output(z.object({
    ...
    apiKeyEncoded: z.string().optional().describe('Base64-encoded API key for use in Authorization header'),
      ...
      case 'api_key': {
        switch (ctx.input.action) {
          ...
          case 'create': {
            ...
            if (ctx.input.apiKeyRoleDescriptors) body.role_descriptors = ctx.input.apiKeyRoleDescriptors;
