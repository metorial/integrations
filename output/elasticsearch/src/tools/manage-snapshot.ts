...
export let manageSnapshotTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['create_repository', 'list_repositories', 'create', 'get', 'delete', 'restore']).describe('Snapshot action to perform'),
    repositoryName: z.string().optional().describe('Snapshot repository name'),
    repositoryType: z.string().optional().describe('Repository type (e.g., fs, s3, azure, gcs) - required for create_repository'),
    repositorySettings: z.record(z.string(), z.any()).optional().describe('Repository settings (e.g., location, bucket) - required for create_repository'),
    snapshotName: z.string().optional().describe('Snapshot name (required for create, get, delete, restore)'),
    indices: z.string().optional().describe('Comma-separated list of indices to include in the snapshot'),
    restoreOptions: z.object({
      indices: z.string().optional().describe('Comma-separated indices to restore'),
      renamePattern: z.string().optional().describe('Regex pattern to rename restored indices'),
      renameReplacement: z.string().optional().describe('Replacement string for renamed indices'),
    }).optional().describe('Options for restore action'),
  }))
  .output(z.object({
    ...
    restoreDetails: z.record(z.string(), z.any()).optional().describe('Restore operation details'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      ...
      case 'create': {
        ...
        let result = await client.createSnapshot(ctx.input.repositoryName, ctx.input.snapshotName, body);
        return {
      ...
      case 'get': {
        ...
        let result = await client.getSnapshot(ctx.input.repositoryName, ctx.input.snapshotName);
        return {
      ...
      case 'delete': {
        ...
        let result = await client.deleteSnapshot(ctx.input.repositoryName, ctx.input.snapshotName);
        return {
          output: { acknowledged: result.acknowledged ?? true },
      ...
      case 'restore': {
        ...
        if (ctx.input.restoreOptions) {
          if (ctx.input.restoreOptions.indices) body.indices = ctx.input.restoreOptions.indices;
          if (ctx.input.restoreOptions.renamePattern) body.rename_pattern = ctx.input.restoreOptions.renamePattern;
          if (ctx.input.restoreOptions.renameReplacement) body.rename_replacement = ctx.input.restoreOptions.renameReplacement;
        }
        let result = await client.restoreSnapshot(ctx.input.repositoryName, ctx.input.snapshotName, body);
        return {
