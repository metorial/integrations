...
export let managePipelineTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    action: z.enum(['create', 'delete', 'get', 'list', 'simulate']).describe('The pipeline action to perform'),
    pipelineId: z.string().optional().describe('Pipeline ID (required for create, delete, get, and simulate with existing pipeline)'),
    description: z.string().optional().describe('Pipeline description (for create action)'),
    processors: z.array(z.record(z.string(), z.any())).optional().describe('Array of processor definitions (for create action)'),
    simulateDocuments: z.array(z.record(z.string(), z.any())).optional().describe('Sample documents to simulate the pipeline against'),
  }))
  .output(z.object({
    acknowledged: z.boolean().optional().describe('Whether the request was acknowledged'),
    pipelines: z.record(z.string(), z.any()).optional().describe('Pipeline definitions (for get/list)'),
    simulationResults: z.array(z.record(z.string(), z.any())).optional().describe('Results from pipeline simulation'),
  }))
  .handleInvocation(async (ctx) => {
    ...
    switch (ctx.input.action) {
      case 'create': {
        ...
        if (!ctx.input.processors) throw new Error('processors are required for create action');
      ...
      case 'simulate': {
        if (!ctx.input.simulateDocuments) throw new Error('simulateDocuments are required for simulate action');
