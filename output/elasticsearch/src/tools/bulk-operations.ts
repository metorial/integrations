...
export let bulkOperationsTool = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    operations: z.array(z.object({
      action: z.enum(['index', 'create', 'update', 'delete']).describe('The bulk action type'),
      indexName: z.string().describe('Target index for this operation'),
      documentId: z.string().optional().describe('Document ID (required for update and delete)'),
      document: z.record(z.string(), z.any()).optional().describe('Document body (required for index, create, update)'),
    ...
    for (let item of (result.items || [])) {
      ...
      if (detail.error) {
        errorItems.push({
          ...
          error: typeof detail.error === 'string' ? detail.error : (detail.error.reason || JSON.stringify(detail.error)),
        });
      } else {
        successCount++;
      }
