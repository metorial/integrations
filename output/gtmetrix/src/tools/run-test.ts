...
export let runTest = SlateTool.create(
  ...
  .input(z.object({
    ...
    reportType: z.enum(['lighthouse', 'legacy', 'lighthouse,legacy', 'none']).optional().describe('Type of report to generate. Default: lighthouse'),
    ...
    throttle: z.string().optional().describe('Connection throttle as "down/up/latency" in Kbps/Kbps/ms'),
    cookies: z.array(z.string()).optional().describe('Cookies to set for the test (e.g. ["name=value"])'),
    httpAuthUsername: z.string().optional().describe('HTTP authentication username for password-protected pages'),
    httpAuthPassword: z.string().optional().describe('HTTP authentication password for password-protected pages'),
    ...
    browserWidth: z.number().optional().describe('Browser viewport width in pixels (PRO only)'),
    browserHeight: z.number().optional().describe('Browser viewport height in pixels (PRO only)'),
