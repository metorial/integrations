...
export let listPages = SlateTool.create(
  ...
  .input(z.object({
    ...
    filterMonitored: z.enum(['yes', 'no']).optional().describe('Filter by monitoring status'),
    sort: z.string().optional().describe('Sort field (e.g. "-latest_report_time" for newest first). Prepend "-" for descending.'),
    pageSize: z.number().optional().describe('Number of results per page (1-500, default 10)'),
