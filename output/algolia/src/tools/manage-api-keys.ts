...
export let manageApiKeys = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'update', 'delete', 'restore']).describe('The action to perform on API keys'),
    keyValue: z.string().optional().describe('The API key string. Required for get, update, delete, and restore actions.'),
    acl: z.array(z.string()).optional().describe('List of permissions for the key (e.g. "search", "browse", "addObject", "deleteObject", "editSettings", "listIndexes"). Required for create, optional for update.'),
    indices: z.array(z.string()).optional().describe('List of index names this key can access. If empty or not provided, the key can access all indices.'),
    maxHitsPerQuery: z.number().optional().describe('Maximum number of hits this API key can retrieve in one query. Use 0 for unlimited.'),
    maxQueriesPerIPPerHour: z.number().optional().describe('Maximum number of API calls allowed from a given IP address per hour. Use 0 for unlimited.'),
    validity: z.number().optional().describe('Validity duration of the key in seconds. 0 means the key never expires.'),
    description: z.string().optional().describe('A human-readable description of the API key.'),
    referers: z.array(z.string()).optional().describe('List of allowed HTTP referers. Supports wildcards, e.g. "https://example.com/*".'),
    queryParameters: z.string().optional().describe('URL-encoded query parameters to force with every search request made with this key, e.g. "typoTolerance=strict&ignorePlurals=false".'),
  }))
  .output(z.any())
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'create') {
      ...
      if (ctx.input.maxHitsPerQuery !== undefined) params.maxHitsPerQuery = ctx.input.maxHitsPerQuery;
      if (ctx.input.maxQueriesPerIPPerHour !== undefined) params.maxQueriesPerIPPerHour = ctx.input.maxQueriesPerIPPerHour;
      ...
      if (ctx.input.queryParameters !== undefined) params.queryParameters = ctx.input.queryParameters;
