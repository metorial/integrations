...
export let manageIndices = SlateTool.create(
  spec,
  {
    name: 'Manage Indices',
    key: 'manage_indices',
  ...
  .input(z.object({
    action: z.enum(['list', 'delete', 'copy', 'move']).describe('The index management action to perform'),
    indexName: z.string().optional().describe('Name of the source index (required for delete, copy, and move)'),
    destinationIndexName: z.string().optional().describe('Name of the destination index (required for copy and move)'),
    copyScope: z.array(z.enum(['settings', 'synonyms', 'rules'])).optional().describe('Scope of the copy operation. When omitted, the entire index (records, settings, synonyms, rules) is copied. Provide one or more of "settings", "synonyms", "rules" to copy only those parts.'),
    page: z.number().optional().describe('Page number for listing indices (zero-based)'),
    hitsPerPage: z.number().optional().describe('Number of indices to return per page when listing'),
  }))
  .output(z.any())
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'copy') {
      ...
      let scopeLabel = copyScope && copyScope.length > 0 ? ` (scope: ${copyScope.join(', ')})` : '';
