...
export let manageComment = SlateTool.create(
  ...
  .input(z.object({
    ...
    postId: z.string().optional().describe('ID of the question or answer to comment on (required for "add")'),
    commentId: z.string().optional().describe('ID of the comment to edit or delete (required for "edit" and "delete")'),
    ...
    if (action === 'add') {
      ...
      return {
        output: {
          ...
          body: comment?.body,
          creationDate: comment?.creation_date ? new Date(comment.creation_date * 1000).toISOString() : undefined,
          success: true,
        },
