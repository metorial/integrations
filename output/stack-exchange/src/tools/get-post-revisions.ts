...
let revisionSchema = z.object({
  ...
  revisionType: z.string().optional().describe('Type of revision (e.g., single_user, vote_based)'),
  ...
  setCommunityWiki: z.boolean().optional().describe('Whether this revision set the post as community wiki'),
});

export let getPostRevisions = SlateTool.create(
  spec,
  {
  ...
  .handleInvocation(async (ctx) => {
    ...
    let revisions = result.items.map((r: any) => ({
      ...
      title: r.title,
      tags: r.tags,
      creationDate: r.creation_date ? new Date(r.creation_date * 1000).toISOString() : undefined,
