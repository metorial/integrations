...
export let managePipelines = SlateTool.create(
  ...
  .input(z.object({
    action: z.enum(['list', 'get', 'create', 'update', 'delete', 'list_couplings', 'add_app', 'remove_app']).describe('Operation to perform'),
    ...
    stage: z.string().optional().describe('Pipeline stage: "development", "staging", "production" (for add_app)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    if (action === 'update') {
      if (!ctx.input.pipelineIdOrName) throw new Error('pipelineIdOrName is required.');
      let pipeline = await client.updatePipeline(ctx.input.pipelineIdOrName, { name: ctx.input.name });
      return {
        output: { pipelines: [mapPipeline(pipeline)] },
    ...
    // remove_app
    if (!ctx.input.couplingId) throw new Error('couplingId is required for "remove_app" action.');
