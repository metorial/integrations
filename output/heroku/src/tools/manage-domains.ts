...
export let manageDomains = SlateTool.create(
  ...
  .input(z.object({
    ...
    hostname: z.string().optional().describe('Custom domain hostname (required for "add" and "remove")'),
    sniEndpoint: z.string().optional().describe('SNI endpoint ID to associate with the domain (for "add")')
  }))
  .output(z.object({
