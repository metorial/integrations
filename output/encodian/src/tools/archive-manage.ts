...
export let archiveManage = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    operation: z.enum(['create', 'extract']).describe('Whether to create or extract a ZIP archive'),
    // Create params
    ...
    })).optional().describe('Files to include in the archive (for create)'),
    ...
    // Extract params
    fileContent: z.string().optional().describe('Base64-encoded ZIP archive content (for extract)'),
  ...
  .output(z.object({
    ...
    fileContent: z.string().optional().describe('Base64-encoded archive content (for create)'),
    documents: z.array(z.object({
      fileName: z.string().describe('Extracted filename'),
      fileContent: z.string().describe('Base64-encoded extracted file content'),
    })).optional().describe('Extracted files (for extract)'),
