...
export let pdfManipulate = SlateTool.create(
  spec,
  {
    name: 'PDF Manipulate',
    key: 'pdf_manipulate',
...
Choose the desired operation and provide the PDF file content along with operation-specific parameters.`,
  ...
  .input(z.object({
    ...
    watermarkImageContent: z.string().optional().describe('Base64-encoded image for image watermark'),
    watermarkOpacity: z.number().optional().describe('Watermark opacity (0.0 to 1.0)'),
    // Page number params
    pageNumberPosition: z.string().optional().describe('Position for page numbers (e.g., BottomCenter, BottomLeft, BottomRight)'),
    ...
    searchText: z.string().optional().describe('Text to search for (for search/replace and redact)'),
    ...
    rotationAngle: z.number().optional().describe('Rotation angle in degrees (90, 180, 270)'),
    // Security params
