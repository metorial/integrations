...
Supports converting any common document format to PDF, converting between Office formats, HTML/URL to PDF or image, PDF to Word/Excel/images, JSON to Excel, and plain text to PDF.`,
    ...
    constraints: [
      'Supported output formats depend on the source file type.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    ...
    fileName: z.string().optional().describe('Source filename with extension (e.g., "report.docx")'),
    fileContent: z.string().optional().describe('Base64-encoded file content'),
    htmlContent: z.string().optional().describe('HTML content or URL for HTML-based conversions'),
    jsonContent: z.string().optional().describe('JSON string for JSON to Excel conversion'),
    textContent: z.string().optional().describe('Plain text content for text to PDF conversion'),
    outputFilename: z.string().optional().describe('Desired output filename'),
    outputFormat: z.string().optional().describe('Target output format (e.g., PDF, DOCX, XLSX, PNG, HTML, CSV, TIFF)'),
    pdfaCompliance: z.boolean().optional().describe('Enable PDF/A compliance for file-to-PDF conversion'),
    pdfaComplianceLevel: z.string().optional().describe('PDF/A compliance level (e.g., PDF_A_2A)'),
  }))
  .output(z.object({
