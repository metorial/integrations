...
export let utilitySecurity = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    encodingMethod: z.string().optional().describe('Data encoding method (e.g., Base64, Hex)'),
    // RSA params
    ...
    // Hash params
    hashAlgorithm: z.string().optional().describe('Hash algorithm (MD5, SHA1, SHA256, SHA384, SHA512)'),
    // HMAC params
  ...
  .output(z.object({
    result: z.string().describe('Operation result (encrypted/decrypted data, hash, token, password, etc.)'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    return {
      output: {
        result: result.result || result.Result || result.data || result.Data || result.password || result.Password || result.token || result.Token || '',
