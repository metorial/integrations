...
export let getViewerUrl = SlateTool.create(
  spec,
  {
    name: 'Get Viewer URL',
    key: 'get_viewer_url',
  ...
  .input(z.object({
    ...
    isPublic: z.boolean().optional().describe('If true, generates a public viewer URL (no authentication required). Defaults to false (signed URL).'),
    validForMinutes: z.number().optional().describe('Number of minutes the signed URL should remain valid. Defaults to 30. Only applies to signed (non-public) URLs.'),
    wait: z.boolean().optional().describe('If true, the viewer will wait for the comparison to finish processing instead of showing an error.'),
  }))
  .output(z.object({
    viewerUrl: z.string().describe('The viewer URL for the comparison'),
    validUntil: z.string().nullable().describe('UTC ISO 8601 timestamp when the signed URL expires. Null for public URLs.'),
  }))
