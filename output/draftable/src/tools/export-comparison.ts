...
export let exportComparison = SlateTool.create(
  spec,
  {
  ...
  .input(z.object({
    ...
    kind: z.enum(['left', 'right', 'combined', 'single_page']).describe('Export kind: "left" shows deletions, "right" shows insertions, "combined" shows side-by-side, "single_page" shows redline view'),
    includeCoverPage: z.boolean().optional().describe('Whether to include a cover page. Only applies to the "combined" export kind. Defaults to true.'),
  }))
  .output(z.object({
    ...
    downloadUrl: z.string().nullable().describe('URL to download the exported PDF. Available when ready is true.'),
