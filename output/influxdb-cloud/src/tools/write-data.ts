...
export let writeData = SlateTool.create(
  spec,
  {
    name: 'Write Data',
    key: 'write_data',
...
Line protocol format: \`measurement,tag1=val1 field1=val2 timestamp\`.`,
    ...
    constraints: [
      'Write size is limited by the InfluxDB Cloud plan.',
    ],
    tags: {
      destructive: false,
      readOnly: false,
  ...
  .input(z.object({
    ...
    precision: z.enum(['ns', 'us', 'ms', 's']).optional().describe('Timestamp precision (default: ns)'),
  }))
  .output(z.object({
