...
export let retrieveExtraction = SlateTool.create(
  spec,
  {
...
Returns the full extracted text along with confidence and document metadata.`,
  ...
  .input(z.object({
    ...
    textOnly: z.boolean().optional().describe('If true, returns only the extracted text without metadata. Defaults to false.'),
  }))
  .output(z.object({
    resultText: z.string().describe('The extracted text content from the document.'),
    confidenceMetadata: z.any().nullable().describe('Confidence scores for extracted text lines. Null if textOnly is true.'),
    metadata: z.any().nullable().describe('Additional document metadata. Null if textOnly is true.'),
  ...
  .handleInvocation(async (ctx) => {
    ...
    let textPreview = result.resultText.length > 200
