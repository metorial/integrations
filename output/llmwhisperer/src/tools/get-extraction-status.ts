...
export let getExtractionStatus = SlateTool.create(
  spec,
  {
...
Possible statuses: "accepted" (queued), "processing" (in progress), "processed" (ready to retrieve), "error" (failed), "retrieved" (already retrieved).`,
  ...
  .input(z.object({
    whisperHash: z.string().describe('The whisper hash returned from the extract document operation.'),
  }))
  .output(z.object({
    status: z.string().describe('Current job status: "accepted", "processing", "processed", "error", or "retrieved".'),
    ...
    pageDetails: z.array(z.object({
      ...
      executionTimeInSeconds: z.number().describe('Processing time for this page in seconds.'),
