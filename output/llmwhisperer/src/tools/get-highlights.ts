...
export let getHighlights = SlateTool.create(
  spec,
  {
    name: 'Get Highlights',
    key: 'get_highlights',
...
Requires that the extraction was performed with \`addLineNos\` enabled.`,
  ...
  .input(z.object({
    ...
    lines: z.string().describe('Line numbers to retrieve highlight data for (e.g., "1-5,7,21-").'),
  }))
  .output(z.object({
    lineHighlights: z.record(z.string(), z.object({
      ...
      raw: z.array(z.number()).describe('Raw bounding box data: [page_no, y, height, max_height].'),
