...
export let addUserToProduct = SlateTool.create(
  spec,
  {
...
Set **productId** to \`-1\` to create the user account without granting access to any product.`,
  ...
  .input(z.object({
    ...
    productId: z.string().describe('ID of the product to grant access to. Use "-1" to add the user without granting product access.'),
  }))
  .output(z.object({
  ...
  .handleInvocation(async (ctx) => {
    ...
    let productLabel = ctx.input.productId === '-1'
